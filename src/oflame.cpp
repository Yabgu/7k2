...
// <FlameType> flameType     determine how flame source is generated at base
// line
//
void Flame::init(short width, short height, short flameWidth,
                 FlameType flameType, const char *fileName) {
  ...
  bitmap =
      (unsigned char *)mem_resize(bitmap, width * height + 2 * sizeof(short));
...
Flame &Flame::operator=(Flame &f) {
  if (heat_map && f.heat_map &&
      map_width * map_height == f.map_width * f.map_height) {
  ...
  if (bitmap && f.bitmap &&
    ...
    if (f.bitmap) {
      bitmap = (unsigned char *)mem_add(f.map_width * f.map_height +
                                        2 * sizeof(short));
      memcpy(bitmap, f.bitmap, f.map_width * f.map_height + 2 * sizeof(short));
    }
  }
...
void Flame::heat_up(short h) { decay = h; }
  ...
  for (y = map_height - 2; y >= 0; --y) {
    ...
    for (x = 0; x <= map_width / 2; ++x, ++p) {
      ...
      if (v < decay || x <= 0) {
        ...
        // draw a point on the buffer
        p[map_width - random(3) + 1 + wind * random(2)] = v - random(decay);
      }
    }
...
//
// [unsigned char] shadeColor       first (and brightest) color of the color
// scale
...
void Flame::mask_bottom() {
  for (short x = 0; x < map_width; ++x) {
    ...
    unsigned char *p =
        bitmap + 2 * sizeof(short) + (map_height - 1) * map_width + x;
    for (y = 0; y < h; ++y) {
      *p = TRANSPARENT_CODE;
      p -= map_width;
    }
