...
unsigned short BitFileRead::input_bits(unsigned stringLen) {
  if (bit_offset + stringLen > (last_offset + sizeof(residue)) * 8) {
    ...
    if (byteFetch >= sizeof(residue)) // residue >>= 32 does not change to 0
      ...
      residue >>= 8 * byteFetch;
    filePtr->file_read(sizeof(residue) - byteFetch + (unsigned char *)&residue,
                       byteFetch);
...
void BitFileWrite::output_bits(unsigned short stringCode, unsigned stringLen) {
  if (residue_len + stringLen > sizeof(residue) * 8) {
    ...
    if (byteFlush >= sizeof(residue)) // if byteFlush == 4, residue >>= 32 does
                                      // not set residue to 0
      residue = 0;
    else
      residue >>= byteFlush * 8;
...
long Lzw::basic_compress(unsigned char *inPtr, long inByteLen,
                         BitStream *outStream) {
  unsigned char character;
...
// <BitStream *> inStream            bit stream, can be BitMemStream or
// BitFileRead <unsigned char *> outPtr          address of decompressed output
// data
//                                   (NULL to find the size of decompressed
//                                   data)
//
...
//
unsigned short Lzw::find_child_node(unsigned short parentCode,
                                    unsigned char childChar) {
  unsigned short index;
...
//
void Lzw::output_bits( unsigned char *outPtr, long &outLen, unsigned short
stringCode, unsigned int stringLen )
{
        // fill low bit first
        if( outPtr )
        {
...
unsigned short Lzw::input_bits( unsigned char *inPtr, long &inCnt, unsigned int
stringLen )
{
        inPtr += inCnt / 8;
        int s = inCnt % 8;
