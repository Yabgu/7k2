...
void Tornado::process_move() {
  ...
  double windDir =
      weather.wind_direct_rad() + (misc.random(31) - 15) * PI / 180.0;
...
void Tornado::hit_target() {
  ...
  if (damageXLoc < 0 || damageXLoc >= world.max_x_loc || damageYLoc < 0 ||
      damageYLoc >= world.max_y_loc)
    return;
  ...
  if (locPtr->unit_recno(UNIT_LAND)) {
    ...
    hit_building(); // pass to hit_building to check whether a building is in
                    // the location
  }
...
void TornadoArray::draw_dot() {
  ...
  for (i = 1; i <= size(); i++) {
    ...
    if (tornadoPtr->cur_x_loc() <= 0 ||
        tornadoPtr->cur_x_loc() + 1 >= MAX_WORLD_X_LOC ||
        tornadoPtr->cur_y_loc() <= 0 ||
    ...
    mapX = world.map_matrix->calc_loc_pos_x(tornadoPtr->cur_x_loc(),
                                            tornadoPtr->cur_y_loc()) +
           MAP_X1;
    mapY = world.map_matrix->calc_loc_pos_y(tornadoPtr->cur_x_loc(),
                                            tornadoPtr->cur_y_loc()) +
           MAP_Y1;
