...
int Town::process_wall() {
  ...
  if (godsWall) {
    if (target_wall_level <
        MAX_WALL_LEVEL) // avoid promote to max wall level repetitive
    {
      set_target_wall_level(MAX_WALL_LEVEL,
                            COMMAND_AUTO); // grant in process_wall as town may
                                           // appear before the seat of power
    }
  }
  // ###### end Gilbert 14/4 #######//
  ...
  if (current_wall_level > 0 && hit_points <= 0.0f) {
    ...
    if (hit_points < max_hit_points() && repair_flag && actualManPower > 0 &&
      ...
      if (nation_recno && actualManPower > 0) {
        ...
        float expenseAmt = (float)wallCost *
                           ((float)actualManPower * REPAIR_POINTS_PER_DAY) /
                           max_hit_points();
    ...
    if (godsWall) {
      ...
      if (actualManPower == 1) // since target_wall_man_power is always 1,
                               // disable manual when god happen
        actualManPower = 0;    //
    }
    ...
    if (actualManPower > 0 &&
      ...
      if (nation_recno) {
        ...
        float expenseAmt =
            (float)wallCost * actualManPower * 10 / MAX_WALL_PROGRESS;
        err_when(expenseAmt < 0);
...
void Town::set_repair_wall(int repairFlag, char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_TOWN_SET_REPAIR,
                                                         2 * sizeof(short));
    *shortPtr = town_recno;
...
void Town::set_target_wall_level(int newLevel, char remoteAction) {
  ...
  if (newLevel <
      current_wall_level) // target_wall_level never below current_wall_level
    return;

  if (!remoteAction && remote.is_enable()) {
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_TOWN_TARGET_WALL,
                                                         2 * sizeof(short));
    *shortPtr = town_recno;
    shortPtr[1] = newLevel;
    return;
