...
void UnitGod::japanese_kill_spy() {
  ...
  for (int i = spy_array.size(); i > 0; --i) {
    ...
    //		if( misc.random(1000) >= 900 - spyPtr->spy_skill )		// 10% - 20%
...
//--------- on towns ----------//
  ...
  */
  if (god_id == GOD_EGYPTIAN && townPtr->nation_recno == nation_recno) {
    ...
    for (headCount = 10;
         headCount > 0 && townPtr->population < MAX_TOWN_GROWTH_POPULATION;
         --headCount)
    // ##### end Gilbert 18/1 ######//
    {
...
void UnitGod::roman_cast_power(int unitRecno, int divider) {
  ...
  //RANK_SOLDIER)
  if (nation_recno == unitPtr->nation_recno && unitPtr->is_human() &&
...
void UnitGod::indian_cast_power(int unitRecno, int divider) {
  ...
  if (unitPtr->is_visible() &&
      nation_array.should_attack(nation_recno, unitPtr->nation_recno)) {
