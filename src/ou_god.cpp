...
int UnitGod::can_attack_target(int targetObjRecno)
{
    ...
    // ####### begin Gilbert 6/7 #########//
    //	if(
    //! nation_array[nation_recno]->get_relation_should_attack(targetObj->nation_recno)
    //) 		return 0;
    ...
    if (godInfo->target_monster_id && targetObj->monster_id() == godInfo->target_monster_id)
        // ####### end Gilbert 6/7 #######//
        return 1;
...
int UnitGod::process_attack()
{
    ...
    if (godInfo->can_attack)
    {
        if (!Unit::process_attack()) // return 1 if the unit just finished its
                                     // current attack
            return 0;
    }
    else
    {
        if (!Sprite::process_attack()) // return 1 if the unit just finished its
                                       // current attack
            return 0;
    }
...
void UnitGod::japanese_kill_spy()
{
    ...
    for (int i = spy_array.size(); i > 0; --i)
    {
        ...
        if ((spyPtr->cloaked_nation_recno != nation_recno && spyPtr->cloaked_nation_recno != 0) ||
            spyPtr->true_nation_recno == nation_recno)
            continue;

        // check range
        int baseObjRecno = spyPtr->get_host_base_obj_recno();
        if (base_obj_array.is_deleted(baseObjRecno) || area_distance(base_obj_array[baseObjRecno]) >= range)
            continue;

        // chance of escape

        //		if( misc.random(200) < spyPtr->spy_skill )
        //		if( misc.random(1000) >= 900 - spyPtr->spy_skill )		// 10% -
        // 20%
        // change of escape 			continue;
...
void UnitGod::japanese_cast_inn(int firmRecno)
{
    ...
    for (int i = 0; i < firmInn->inn_unit_count; ++i)
    {
        ...
        innUnit->set_hire_cost(); // recalculate so as to avoid casting it twice to
                                  // get 1/4
...
else if (locPtr->is_firm())
{
    ...
    GodInfo *godInfo = god_res[god_id];
    int divider =
        MIN(firmPtr->loc_width(), godInfo->cast_power_range) * MIN(firmPtr->loc_height(), godInfo->cast_power_range);
...
else if (locPtr->is_town())
{
    ...
    /*
                    if( god_id == GOD_JAPANESE && townPtr->nation_recno !=
       nation_recno)
                    {
                            int divider = STD_TOWN_LOC_WIDTH *
       STD_TOWN_LOC_HEIGHT;

                            float changePoints = (float)7 + misc.random(8);
       // decrease 7 to 15 loyalty points instantly

                            if( townPtr->nation_recno )
                                    townPtr->change_loyalty(-changePoints/divider);
                            else
                                    townPtr->change_resistance(nation_recno,
       -changePoints/divider);
                    }
                    else
    */
...
void UnitGod::roman_cast_power(int unitRecno, int divider)
{
    ...
    //		unitPtr->race_id == RACE_ROMAN && unitPtr->rank_id !=
    // RANK_SOLDIER)
    if (nation_recno == unitPtr->nation_recno && unitPtr->is_human() && unitPtr->rank_id != RANK_SOLDIER)
    {
...
void UnitGod::persian_cast_power(int unitRecno, int divider)
{
        ...
        {
                float changePoints = (float) unitPtr->max_hit_points() /
(6+misc.random(4));	 // divided by (6 to 9)
...
void UnitGod::japanese_cast_power(int unitRecno, int divider)
{
        ...
        if( unitPtr->nation_recno != nation_recno && unitPtr->race_id > 0 )
        {
                int changePoints = 7 + misc.random(8);		// decrease 7 to
15 loyalty points instantly
...
void UnitGod::mongol_cast_power(int unitRecno, int divider)
{
    ...
    int turnProbability = unitPtr->nation_recno ? 100 - (int)unitPtr->loyalty / 2 : 75;
...
RACE_MAYA )
        {
                int changePoints = 15 + misc.random(10);		// add
15 to 25 points to its combat level instantly

                int newCombatLevel = unitPtr->combat_level() +
changePoints/divider;
                ...
                unitPtr->hit_points = oldHitPoints;		// keep the hit
points unchanged.
        }
}
...
//--------- Begin of function UnitGod::persian_cast_power ---------//
//
void UnitGod::persian_cast_power(Worker* workerPtr, int nationRecno, int
divider)
{
        //-- only heal human units belonging to our nation --//
        ...
        {
                int changePoints = workerPtr->max_hit_points() /
(4+misc.random(4));	 // divided by (4 to 7)
...
//--------- Begin of function UnitGod::japanese_cast_power ---------//
//
void UnitGod::japanese_cast_power(Worker* workerPtr, int nationRecno, int
divider)
{
        //-- only cast on enemy units -----//
...
15 to 25 points to its combat level instantly
                ...
                int newCombatLevel = workerPtr->combat_level + MAX(1,
changePoints/divider);
...
//--------- Begin of function UnitGod::egyptian_cast_power ---------//
//
void UnitGod::egyptian_cast_power(Worker *workerPtr, int nationRecno, int
divider)
{
        // no effect
}
...
int UnitGod::right_click(int xLoc, int yLoc, int testing)
{
    ...
    case GOD_ROMAN: // own general/king or own camp
        if (targetObj->nation_recno == nation_recno)
        {
            ...
            {
                FirmCamp *camp;
                flag = (camp = targetObj->cast_to_Firm()->cast_to_FirmCamp()) && camp->overseer_recno;
            }
        }
        break;
    case GOD_MONGOL: // enemy unit
        flag = targetObj->nation_recno != nation_recno && targetObj->cast_to_Unit();
        break;
    case GOD_EGYPTIAN: // own town
        flag = targetObj->nation_recno == nation_recno && targetObj->cast_to_Town();
        break;
    case GOD_PERSIAN: // enemy firm
        flag = targetObj->nation_recno != nation_recno && targetObj->cast_to_Firm();
        break;
    ...
    if (flag)
    {
        if (!testing)
        {
            ...
            go_cast_power(xLoc, yLoc, COMMAND_PLAYER, targetObj->cast_to_Unit() ? baseObjRecno : 0);
        }
        return 1;
    }
    return 0;
