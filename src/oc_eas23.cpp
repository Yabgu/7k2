...
int CampaignEastWest::stage_23_create_game() {
  ...
  create_offensive_structure(
      STAGE_23_EASTERN_NATION_RECNO,
      1 + campaign_difficulty / 2 +
          misc.random(2)); // build 2 structures if the difficulty > 3
...
int CampaignEastWest::stage_23_create_town() {
  ...
  return create_town(STAGE_23_EASTERN_NATION_RECNO, townAddCount,
                     hasFortPercent, combatLevel);
}
...
int CampaignEastWest::stage_23_create_firm() {
  ...
  int townRecno2 = random_pick_town_with_camp(
      STAGE_23_EASTERN_NATION_RECNO, 2); // only pick towns with <= 2 links
  ...
  if (eastSituation == 1) {
    ...
    for (int i = 0; i < nationPtr->ai_camp_count; i++) {
      FirmCamp *firmCamp =
          firm_array[nationPtr->ai_camp_array[i]]->cast_to_FirmCamp();

      int addCount =
          MAX_SOLDIER - firmCamp->soldier_count -
          misc.random(
              3); // don't always full up, sometimes leave a free slots empty
