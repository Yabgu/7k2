...
// ---------- begin of function ColorTable::ColorTable ----------//
ColorTable::ColorTable(const ColorTable &ct) : abs_scale(ct.abs_scale), table_size(ct.table_size)
{
    if (ct.remap_table)
    {
...
HSVColor ColorTable::rgb2hsv(RGBColor &rgb)
{
    ...
    if (rgb.red <= rgb.green && rgb.red <= rgb.blue)
    {
        if (rgb.green >= rgb.blue)
        {
            ...
            return HSVColor(2.0 + (double)rgb.blue / rgb.green, rgb.blue != 0 ? 1.0 - (double)rgb.red / rgb.blue : 1.0,
                            rgb.green / 255.0);
        }
        else
        {
            ...
            return HSVColor(4.0 - (double)rgb.green / rgb.blue,
                            rgb.green != 0 ? 1.0 - (double)rgb.red / rgb.green : 1.0, rgb.blue / 255.0);
        }
    }
    else if (rgb.green <= rgb.red && rgb.green <= rgb.blue)
    {
        if (rgb.red >= rgb.blue)
        {
            ...
            return HSVColor(6.0 - (double)rgb.blue / rgb.red, rgb.blue != 0 ? 1.0 - (double)rgb.green / rgb.blue : 1.0,
                            rgb.red / 255.0);
        }
        else
        {
            // b is the primary, r is secondary
            return HSVColor(4.0 + (double)rgb.red / rgb.blue, rgb.red != 0 ? 1.0 - (double)rgb.green / rgb.red : 1.0,
                            rgb.blue / 255.0);
        }
    }
    else if (rgb.blue <= rgb.red && rgb.blue <= rgb.green)
    {
        if (rgb.red >= rgb.green)
        {
            ...
            return HSVColor((double)rgb.green / rgb.red, rgb.green != 0 ? 1.0 - (double)rgb.blue / rgb.green : 1.0,
                            rgb.red / 255.0);
        }
        else
        {
            // g is the primary, r is secondary
            return HSVColor(2.0 - (double)rgb.red / rgb.green, rgb.red != 0 ? 1.0 - (double)rgb.blue / rgb.red : 1.0,
                            rgb.green / 255.0);
        }
    }
    else
