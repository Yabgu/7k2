...
ColorTable::ColorTable(const ColorTable &ct)
    : abs_scale(ct.abs_scale), table_size(ct.table_size) {
  if (ct.remap_table) {
    remap_table =
        (WORD *)mem_add(table_size * (2 * abs_scale + 1) * sizeof(WORD));
    memcpy(remap_table, ct.remap_table,
           table_size * (2 * abs_scale + 1) * sizeof(WORD));
...
void ColorTable::init(int absScale, int tableSize, WORD *customTable) {
  ...
  memcpy(remap_table, customTable,
         tableSize * (2 * absScale + 1) * sizeof(WORD));
...
//
void ColorTable::generate_table(int absScale, PalDesc &palD,
  ...
  abs_scale = absScale;
  table_size = palSize;
  WORD *remapEntry = remap_table =
      (WORD *)mem_add(table_size * (2 * absScale + 1) * sizeof(WORD));
  remap_table_array = (WORD **)mem_add(sizeof(WORD *) * (2 * absScale + 1));
...
void ColorTable::generate_table_fast(int absScale, PalDesc &palD,
                                     RGBColor (*fp)(RGBColor, int, int)) {
  int palSize = palD.pal_size;
...
void ColorTable::generate_table(PalDesc &sPalD, PalDesc &palD) {
  ...
  for (int c = 0; c < sPalSize; ++c, ++remapEntry) {
    *remapEntry = c; // put a default value (as if it is a reserved color)
    ...
    // closeColor[] are the closest 8 colours, use hsv comparison to find the
    // nearest
    d = closeColor[0];
    *remapEntry = d;
...
int ColorTable::color_dist(RGBColor c1, RGBColor c2) {
  return sq((int)c2.red - c1.red) + sq((int)c2.green - c1.green) +
         sq((int)c2.blue - c1.blue);
}
...
int ColorTable::color_dist_hsv(RGBColor c1, RGBColor c2) {
  ...
  double dx = hsv2.saturation * cos(hsv2.hue * PI / 3.0) -
              hsv1.saturation * cos(hsv1.hue * PI / 3.0);
  double dy = hsv2.saturation * sin(hsv2.hue * PI / 3.0) -
              hsv1.saturation * sin(hsv1.hue * PI / 3.0);
...
int ColorTable::write_file(File *f) {
  return (f->file_put_long(abs_scale) && f->file_put_long(table_size) &&
          f->file_write(remap_table,
                        table_size * (2 * abs_scale + 1) * remap_table[0]));
}
...
int ColorTable::read_file(File *f) {
  ...
  remap_table =
      (WORD *)mem_add(table_size * (2 * abs_scale + 1) * remap_table[0]);
  if (!f->file_read(remap_table,
                    table_size * (2 * abs_scale + 1) * remap_table[0])) {
    mem_del(remap_table);
    remap_table = 0;
    return 0;
  ...
  remap_table_array =
      (WORD **)mem_add(sizeof(remap_table_array[0]) * (2 * abs_scale + 1));
  create_table_array();
  return 1;
}
...
//
PalDescFile::PalDescFile(char *fileName, int headerSize, int pitch, int size,
                         int bitWidth, BYTE *rPtr, int rCount)
