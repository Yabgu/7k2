...
static short size_x,
    size_y; // store the size of the unit whose path is to be found
static char mobile_type;
static int search_flags;
static int node_matrix[MAX_MAP_HEIGHT_A]
                      [MAX_MAP_WIDTH_A]; // stores the node index number of each
                                         // location
...
// static bool temp_occ[(MAX_UNIT_HEIGHT+2)*(MAX_UNIT_WIDTH+2)];		// on
// when PathFinder is being used, off when it isn't stores the temporary
// occupany in the area associated with the unit
...
static char nation_passable[MAX_NATION + 1] = {
    0}; // Note: position 0 is not used for faster access
...
// Gilbert : Improvment
// 1. node_arr, can be a static array like stack ,memset in reset() may be
// removed
// 2. node_matrix can use MAX_MAP_WIDTH instead of MAX_WORLD_X_LOC to reduce
...
//       store node_matrix[y][x] = node_count + base_node_count instead of
//       node_matrix[y][x] = node_count; base_node_count is updated (increase by
//       node_count) after a path searching
//    or change type of node_matrix from int to short, given MAX_NODE_NUM is
//    within 0x7fff
...
void PathFinder::init() {
  ...
  // node_matrix = (int *) mem_add(sizeof(int)*(MAX_WORLD_X_LOC+1)
  // *(MAX_WORLD_Y_LOC+1));
...
void PathFinder::reset() {
  ...
  //	memset(node_matrix, 0, sizeof(int)*(MAX_WORLD_X_LOC+1)
  //*(MAX_WORLD_Y_LOC+1) );
...
//
void PathFinder::set_attribute(short x, short y, char mobileType,
                               int searchFlags) {
...
//
void PathFinder::set_handle_power_nation(int handlePowerFlag,
                                         char *nationPassable) {
...
//
// <int> sourceX 		- the integral x-coordinate of the source
// location <int> sourceY 	   - the integral y-coordinate of the source
// location <int> destX 	   - the integral x-coordinate of the
// destination <int> destY		   - the integral y-coordinate of the
// destination
//
//	<int> destRangeX1, destRangeY1, destRangeX2, destRangeX3
//							- the range of the
//destination, 							  for normal movement, it is the area occupied by the building 							  for
//range attack, it is the area occupied by the building plus attack range
//
// [bool] raiseIt 	- whether the source location should be temporarily set
// unoccupied during the search
//						 	  (this is 0 when called by the
//reuse function because reuse does not have the unit in the source location)
//						 	  (default: true)
//
// [int]  maxTries   - number of node to tries, avoid excessive path searching
// in aggressive attack (default:0 - use default) [int]  stopAtRange - stop find
// path when range is less then (for range attack)
//
// returns: true if a path was found, false if not
//
bool PathFinder::find_path(int sourceX, int sourceY, int destX, int destY,
                           int destRangeX1, int destRangeY1, int destRangeX2,
                           int destRangeY2, bool raiseIt, bool raiseDest,
                           int maxTries, int stopAtRange) {
#ifdef DEBUG
  ...
  if (raiseIt) {
    for (i = 0; i < size_x; i++) {
      for (j = 0; j < size_y; j++) {
        if (!unitRecno)
          unitRecno =
              world.get_loc(sourceX + i, sourceY + j)->unit_recno(mobile_type);
  ...
  err_when((destRangeX2 - destRangeX1 + 1) * (destRangeY2 - destRangeY1 + 1) >
           PATH_FIND_LOC_BUFFER_SIZE);
  ...
  if (raiseDest) {
    ...
    for (yLoc = destRangeY1; yLoc <= destRangeY2; yLoc++) {
      ...
      for (xLoc = destRangeX1; xLoc <= destRangeX2; xLoc++, locPtr++, i++) {
        if (xLoc >= sourceX &&
            xLoc < sourceX + size_x && // overlap with the source location
  ...
  if (raiseIt) {
    for (i = 0; i < size_x; i++) {
      for (j = 0; j < size_y; j++)
        world.get_loc(sourceX + i, sourceY + j)
            ->set_unit(unitRecno, mobile_type);
    }
  }
  ...
  if (raiseDest) {
    ...
    for (yLoc = destRangeY1; yLoc <= destRangeY2; yLoc++) {
      ...
      for (xLoc = destRangeX1; xLoc <= destRangeX2; xLoc++, locPtr++, i++) {
        if (xLoc >= sourceX && xLoc < sourceX + size_x && yLoc >= sourceY &&
            yLoc < sourceY + size_y) {
          continue;
        }
...
// returns: true if a path was found, false if not
//
bool PathFinder::find_path2(int sourceX, int sourceY, int destX, int destY,
                            int maxTries, int stopAtRange) {
  // POTENTIAL BUG_HERE: because this function first sets the location
  // unoccupied and then occupied again after path searching has been done, this
  // function should be used with caution when used for finding a path from an
  ...
  // int dist = (destX-sourceX)*(destX-sourceX) +
...
#ifdef USE_NODE_COUNT_BASE
  node_matrix[sourceY][sourceX] =
      node_count + node_count_base; // node_count has been increased
  err_when(node_count_base + node_count > SAFE_NODE_COUNT_BASE);
//	if( node_count_base + node_count >= SAFE_NODE_COUNT_BASE )	//
//assume reset() can prevent 		normalize_node_count_base();
#else
  ...
  for (int tries = maxTries; tries > 0; --tries) {
    ...
    // if (bestPathNode->x == destX && bestPathNode->y == destY) //destination
    // found
    if (misc.points_distance(bestPathNode->x, bestPathNode->y, destX, destY) <=
        stopAtRange) {
  ...
  err_when(
      stack_pos); // should be zero all time, all pushes should have been popped
...
//
bool PathFinder::is_path_found() { return cur_path != NULL; }
...
//
PathNode *PathFinder::return_path_node() { return cur_path; }
...
//							location. The last node is the first
//movement point.
//
PathResult *PathFinder::retrieve(int &pathCount, int &totalSteps) {
  ...
  n = 1; /// BUG_HERE: There was a bug here, since the n=1 state iis originally
         /// before the if statememt
  ...
  int directionOfPathSoFarX2,
      directionOfPathSoFarY2; // store the current direction so far
...
void PathFinder::gen_childs(PathNode *n) {
  ...
  static char init_explore_dir_array[MAX_DIR] = {DIR_N, DIR_NW, DIR_NE, DIR_W,
                                                 DIR_E, DIR_SW, DIR_S,  DIR_SE};
  static char explore_dir_array[MAX_DIR][MAX_DIR] = // [initial direction][next
                                                    // search direction]
      {
  ...
  if (!parent) {
    // root node
    dirSeqCount = MAX_DIR;
    dirSeqArray = explore_dir_array[Sprite::get_dir(
        sour_x, sour_y, dest_x, dest_y)]; // overall direction
  } else {
    int goX;
    int goY;
    goX = n->x - parent->x;
    ...
    dirSeqCount =
        MAX_DIR -
        3; // exclude last 3 direction last it go backward (180 deg turn)
    // and 2 more direction are not necessary, mostly covered by previous node
    // (135 deg turn) actually, further 2 more (90 deg turn) rarely useful
  ...
  // reverse direction, such that less preferred put on the bottom of "open"
  // link list see the behaviour insert() function
...
void PathFinder::gen_child(PathNode *n, int x, int y) {
...
#endif
  {
    ...
    if (!old->closed_flag) {
      //			for(c = 0; c< MAX_CHILD_NODE && n->child[c];
      //c++); 				n->child[c] = old;
      ...
      if (g < old->g) {
        ...
        //---- improvement so that it might take a path with the same cost as
        //the one being calculated, even though it might not improve (nor
        //worsen) the cost
        //				if (misc.rand()%2)
        //				{
        //					old->parent = n;
    ...
    } else // if (old = get_from_list(x, y, closed)) //if the node is in the
           // closed list
    {
      //			for(c = 0; c< MAX_CHILD_NODE && n->child[c];
      //c++); 				n->child[c] = old;
      ...
      if (g < old->g) {
        ...
        //--- impprovement so that the pathfinder might consider equally good
        //paths of the same cost
        //				if (misc.rand()%2)
        //				{
        //					old->parent = n;
        //					propagate_down(old);
...
PathNode *PathFinder::get_closest_node() {
  //--- this function is generally called after pathfinding has failed, so that
  //the cloest node that can be reached can be found
  ...
  shortestDistance =
      pfind_close_dist(node_arr->x, node_arr->y); //	root node distance
  ...
  for (int q = low_queue; q <= high_queue; ++q) {
    ...
    while (nodePointer) {
      ...
      // ########## begin Gilbert 31/5 #########//
      //			bool replaceFlag;
      //			if( dist <= 1 )		// if it can reach next,
      //it handle specially
      //			{
      //				if( shortestDistance <= 1 )
      //					replaceFlag = nodePointer->g <
      //shortestG; 				else 					replaceFlag = true;
      //			}
      //			else
      //			{
      //				if( shortestDistance <= 1 )
      ...
      //				else
      //					replaceFlag =
      //H_TO_G_RATIO*dist+nodePointer->g <
      //H_TO_G_RATIO*shortestDistance+shortestG;
      //				// avoid take a very long path to get
      //only a little closer
      //				// if one path take  2 steps to get
      //distance 3
      //				// and one path take 7 steps to get
      //distance 2
      //				// take the first one
      //			}
      //			if( replaceFlag )
  ...
  while (stack_pos) {
    ...
    for (c = 0; c < 8; c++) {
      if ((child = father->child[c]) == NULL)
        break;
      if (father->g + 1 <
          child->g) // there are no children, or that the g value of
...
PathNode *PathFinder::pop() { return stack[--stack_pos]; }
//--end of function PathFinder::pop ---///

// ---begin of function PathFinder: insert-----///
void PathFinder::insert(PathNode *succ) {
  err_when(succ->closed_flag !=
           0); // assume closed_flag is 0 to save a memory write
