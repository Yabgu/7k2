...
static int node_matrix[MAX_MAP_HEIGHT_A][MAX_MAP_WIDTH_A]; // stores the node index number of each
                                                           // location
...
// static bool temp_occ[(MAX_UNIT_HEIGHT+2)*(MAX_UNIT_WIDTH+2)];		//
// on when PathFinder is being used, off when it isn't stores the temporary
...
//							- the range of the
// destination, 							  for normal movement, it is
// the area occupied by the building for range attack, it is the area occupied
// by the building plus attack range
//
// [bool] raiseIt 	- whether the source location should be temporarily set
// unoccupied during the search
//						 	  (this is 0 when called by
// the
...
//
bool PathFinder::find_path(int sourceX, int sourceY, int destX, int destY, int destRangeX1, int destRangeY1,
                           int destRangeX2, int destRangeY2, bool raiseIt, bool raiseDest, int maxTries,
                           int stopAtRange)
{
#ifdef DEBUG
    ...
    if (raiseDest)
    {
        ...
        for (yLoc = destRangeY1; yLoc <= destRangeY2; yLoc++)
        {
            ...
            for (xLoc = destRangeX1; xLoc <= destRangeX2; xLoc++, locPtr++, i++)
            {
                if (xLoc >= sourceX && xLoc < sourceX + size_x && yLoc >= sourceY && yLoc < sourceY + size_y)
                {
                    continue;
                }
...
PathResult *PathFinder::retrieve(int &pathCount, int &totalSteps)
{
    ...
    while (parent = node->parent)
    {
        ...
        if (directionOfPathSoFarX != directionOfPathSoFarX2 || directionOfPathSoFarY != directionOfPathSoFarY2)
        {
            base->parent = node;
            base = node;
            n++;
...
PathNode *PathFinder::get_closest_node()
{
    ...
    for (int q = low_queue; q <= high_queue; ++q)
    {
        ...
        while (nodePointer)
        {
            ...
            if (dist < shortestDistance || dist == shortestDistance && nodePointer->g < shortestG)
            // ########## begin Gilbert 31/5 #########//
            //			bool replaceFlag;
