...
//
// for directed effect, put initAction as SPRITE_IDLE and set appropriate
// initDir undirected effect, put initAction as SPRITE_DIE and set initDir to 0
// to find the life of sprite automatically, set effectLife to 0 or negative
//
void Effect::init(short spriteId, short startX, short startY, char initAction, char initDir, char dispLayer,
                  int effectLife)
{
    sprite_id = spriteId;
...
void Effect::create(short spriteId, short startX, short startY, char initAction, char initDir, char dispLayer,
                    int effectLife)
{
...
void Effect::program_effect_draw()
{
    ...
    {
    case 1:
        magic.draw_magic_firm_die(&vga_back, world.zoom_matrix->calc_zoom_x(cur_x, cur_y, get_z()),
                                  world.zoom_matrix->calc_zoom_y(cur_x, cur_y, get_z()), vga.pixel_format_flag,
                                  cur_frame);
        break;

    case 2:
        magic.draw_magic_forteen(&vga_back, world.zoom_matrix->calc_zoom_x(cur_x, cur_y, get_z()),
                                 world.zoom_matrix->calc_zoom_y(cur_x, cur_y, get_z()), vga.pixel_format_flag,
                                 cur_frame, cur_dir);
        break;

    case 3:
        magic.draw_magic_fifteen(&vga_back, world.zoom_matrix->calc_zoom_x(cur_x, cur_y, get_z()),
                                 world.zoom_matrix->calc_zoom_y(cur_x, cur_y, get_z()), vga.pixel_format_flag,
                                 cur_frame);
        break;
