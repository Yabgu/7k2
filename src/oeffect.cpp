...
//
void Effect::init(short spriteId, short startX, short startY, char initAction,
...
void Effect::create(short spriteId, short startX, short startY, char initAction,
                    char initDir, char dispLayer, int effectLife) {
  Effect *effectPtr = new Effect;
  effectPtr->init(spriteId, startX, startY, initAction, initDir, dispLayer,
                  effectLife);
...
void Effect::program_effect_draw() {
  ...
  case 1:
    magic.draw_magic_firm_die(
        &vga_back, world.zoom_matrix->calc_zoom_x(cur_x, cur_y, get_z()),
        world.zoom_matrix->calc_zoom_y(cur_x, cur_y, get_z()),
        vga.pixel_format_flag, cur_frame);
    break;

  case 2:
    magic.draw_magic_forteen(
        &vga_back, world.zoom_matrix->calc_zoom_x(cur_x, cur_y, get_z()),
        world.zoom_matrix->calc_zoom_y(cur_x, cur_y, get_z()),
...
void Effect::update_abs_pos(SpriteFrame *spriteFrame) {
  ...
  case 1:
    abs_x1 = ZoomMatrix::calc_abs_x(cur_x, cur_y, get_z()) -
  ...
  case 2:
    abs_x1 =
        ZoomMatrix::calc_abs_x(cur_x, cur_y, get_z()) - 50; // absolute position
