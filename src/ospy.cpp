...
void Spy::set_place(int spyPlace, int spyPlacePara)
{
    ...
    // ####### end Gilbert 20/4 #######//
    {
        ...
        if (firm_array[spy_place_para]->nation_recno != true_nation_recno)
            notify_cloaked_nation_flag = 1; // when a spy has been assigned to a firm, its notification flag
    ...
    else if (spy_place == SPY_TOWN)
    {
        ...
        if (town_array[spy_place_para]->nation_recno != true_nation_recno) // if it's our own town, don't change
...
void Spy::next_day()
{
    ...
    //		{
    //			Town* townPtr = town_array[spy_place_para];
    //			world.visit( townPtr->loc_x1, townPtr->loc_y1,
    // townPtr->loc_x2,
    ...
    //		{
    //			Firm* firmPtr = firm_array[spy_place_para];
    //			world.visit( firmPtr->loc_x1, firmPtr->loc_y1,
    // firmPtr->loc_x2,
    ...
    case SPY_FIRM:
        ...
        if (firm_array[spy_place_para]->cast_to_FirmCamp())
        {
            ...
            {
                err_when(!firmCamp->overseer_recno || unit_array[firmCamp->overseer_recno]->spy_recno != spy_recno);
            }
        }
        // no more worker spy
        ...
        //				{
        //					if(
        // firmPtr->worker_array[i].spy_recno==spy_recno )
        // break;
        //				}
        //				if( i==firmPtr->worker_count )		// not found
        // in
        // worker_array
        //				{
...
void Spy::process_town_action()
{
    ...
    if (action_mode == SPY_SOW_DISSENT)
    {
        ...
        // townPtr->race_spy_count_array[race_id-1] )	// only when there are
        // non-spy people
        if (townPtr->population > townPtr->spy_count) // only when there are non-spy people
        {
            ...
            float decValue = (float)spy_skill / 4 / townPtr->population; // the more people there, the longer
...
int Spy::can_sabotage()
{
    ...
    //			 firm_array[spy_place_para]->firm_id != FIRM_CAMP; 	//
    // no
...
void Spy::get_killed(int dispNews)
{
    ...
    if (true_nation_recno == nation_array.player_recno ||  // the player's spy is killed
        cloaked_nation_recno == nation_array.player_recno) // a spy cloaked as the player's people is
...
const char *Spy::action_str()
{
    ...
    {
        if ((spy_place == SPY_TOWN && town_array[spy_place_para]->nation_recno == true_nation_recno) ||
            (spy_place == SPY_FIRM && firm_array[spy_place_para]->nation_recno == true_nation_recno))
        {
...
void Spy::pay_expense()
{
    ...
    if (spy_place == SPY_FIRM)
    {
        ...
        if (firmPtr->nation_recno == true_nation_recno)
        {
            ...
            if (firmCamp && firmCamp->overseer_recno && unit_array[firmCamp->overseer_recno]->spy_recno == spy_recno)
            {
                inOwnFirm = 1;
            }
        }
...
int Spy::capture_firm()
{
    ...
    if (firm_res[firmPtr->firm_id]->need_overseer)
    {
        ...
        if (firmCamp)
        {
            if (!firmCamp->overseer_recno || unit_array[firmCamp->overseer_recno]->spy_recno != spy_recno)
            {
                return 0;
            }
            ...
            for (int i = 0; i < firmCamp->soldier_count; i++, soldierPtr++)
            {
                ...
                obeyFlag = misc.random(100) < obeyChance; // if obeyChance >= 100, all units
    ...
    else
    {
        ...
        */
        ...
        firmPtr->change_nation(true_nation_recno); // the firm change nation and the spies inside the
...
// is mobilized.
//					   should set to no when it's calling
// from
    ...
    if (firmPtr->cast_to_FirmCamp())
    {
        ...
        if (!spyUnitRecno)
        {
            ...
            for (i = firmCamp->soldier_count - 1; i >= 0; --i)
            {
                if (firmCamp->soldier_array[i].spy_recno == spy_recno)
                {
                    spyUnitRecno = firmCamp->mobilize_soldier(i + 1, COMMAND_AUTO); // note: mobilize_woker() will
                                                                                    // decrease Firm::player_spy_count
                    break;
                }
            }
    ...
    else if (firmPtr->cast_to_FirmTrain())
    {
        ...
        for (i = firmTrain->trainee_count - 1; i >= 0; --i)
        {
            if (firmTrain->trainee_array[i].spy_recno == spy_recno)
            {
                ...
                // ##### end Gilbert 27/1 ######//
                spyUnitRecno = firmTrain->mobilize_trainee(i + 1, COMMAND_AUTO); // note: mobilize_woker() will decrease
...
int Spy::assassinate(int targetUnitRecno, int remoteAction)
{
    ...
    if (trueNationRecno == nation_array.player_recno && firmPtr->firm_recno == firm_array.selected_recno)
    {
        if (firmCamp)
        {
...
int Spy::steal_tech(int targetNationRecno, char remoteAction)
{
    ...
    if (misc.random(100) < rating)
    {
        ...
        for (int i = 1; i <= tech_res.tech_count; ++i)
        {
            ...
            ++matchCount;
            if (misc.random(matchCount) == 0) // n-th match have 1/n chance to be selected, therefore evenly
                                              // distributed
                matchTech = i;
        }
...
int Spy::get_incident_rating(int targetNationRecno, int involvedNationRecno)
{
    ...
    if ((firmCamp = firm_array[spy_place_para]->cast_to_FirmCamp()))
    {
        for (int i = 0; i <= firmCamp->soldier_count; ++i)
        {
            ...
            if (otherSpy->true_nation_recno == targetNationRecno || otherSpy->true_nation_recno == involvedNationRecno)
            {
                sumResist += otherSpy->spy_skill;
                maxResist = MAX(otherSpy->spy_skill, maxResist);
                ++resistCount;
