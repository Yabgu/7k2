...
//
Spy::Spy() { memset(this, 0, sizeof(Spy)); }
...
//
void Spy::set_action_mode(int actionMode) { action_mode = actionMode; }
...
void Spy::set_place(int spyPlace, int spyPlacePara) {
  ...
  if (spy_place == SPY_FIRM) {
    ...
    if (!town_array.is_deleted(spy_place_para)) {
      ...
      if (town_array[spy_place_para]->spy_count <
          0) // self-fixing in case that this bug still presists.
  ...
  // ####### end Gilbert 20/4 #######//
  {
    ...
    if (firm_array[spy_place_para]->nation_recno != true_nation_recno)
      notify_cloaked_nation_flag =
          1; // when a spy has been assigned to a firm, its notification flag
             // should be set to 1, so the nation can control it as it is one of
             // its own units
    // ####### end Gilbert 20/4 #######//
  } else if (spy_place == SPY_TOWN) {
    town_array[spy_place_para]->spy_count++;
    ...
    if (town_array[spy_place_para]->nation_recno !=
        true_nation_recno) // if it's our own town, don't change
                           // notify_cloaked_nation_flag
...
void Spy::next_day() {
  ...
  if (exposed_flag) {
    //-- he will be killed immediately unless he is back in his original nation
    //color ---//
  ...
  //		{
  //			Town* townPtr = town_array[spy_place_para];
  //			world.visit( townPtr->loc_x1, townPtr->loc_y1, townPtr->loc_x2,
  //townPtr->loc_y2, EXPLORE_RANGE-1 );
  //		}
  //		else if( spy_place == SPY_FIRM )
  //		{
  //			Firm* firmPtr = firm_array[spy_place_para];
  //			world.visit( firmPtr->loc_x1, firmPtr->loc_y1, firmPtr->loc_x2,
  //firmPtr->loc_y2, EXPLORE_RANGE-1 );
  //		}
  //	}
  // ####### end Gilbert 18/5 ##########//
  ...
  if (spy_place == SPY_MOBILE && camouflage_count > 0) {
    if (true_nation_recno != cloaked_nation_recno)
      ...
      stop_camouflage(
          COMMAND_AUTO); // stop when changed back to original nation
  } else {
  ...
  case SPY_FIRM:
    ...
    //			{
    //				FirmWork* firmPtr =
    //firm_array[spy_place_para]->cast_to_FirmWork(); 				for( int i=0 ;
    //i<firmPtr->worker_count ; i++ )
    //				{
    //					if( firmPtr->worker_array[i].spy_recno==spy_recno
    //) 						break;
    //				}
    //				if( i==firmPtr->worker_count )		// not found in
    //worker_array
    //				{
    //					err_when( !firmPtr->overseer_recno ||
    //								 unit_array[firmPtr->overseer_recno]->spy_recno !=
    //spy_recno );
    //				}
    //			}
...
void Spy::process_town_action() {
  ...
  if (action_mode == SPY_SOW_DISSENT) {
    // if( townPtr->race_pop_array[race_id-1] >
    // townPtr->race_spy_count_array[race_id-1] )	// only when there are non-spy
    // people
    if (townPtr->population >
        townPtr->spy_count) // only when there are non-spy people
    {
      err_when(race_id != townPtr->race_id);
      // ###### patch begin Gilbert 13/7 #######//
      // float decValue = (float)spy_skill / 5 / townPtr->population;
      // // the more people there, the longer it takes to decrease the loyalty
      float decValue = (float)spy_skill / 4 /
                       townPtr->population; // the more people there, the longer
                                            // it takes to decrease the loyalty
      // ###### patch end Gilbert 13/7 #######//

      if (true_nation_recno)
        decValue *=
            1.0f + (float)tech_res[TECH_AGITATION]->get_nation_tech_level(
                       true_nation_recno);
...
void Spy::process_firm_action() {
  ...
  if (action_mode == SPY_SOW_DISSENT) {
    FirmCamp *firmCamp = firm_array[spy_place_para]->cast_to_FirmCamp();
    if (firmCamp) {
      int techLevel = true_nation_recno
                          ? tech_res[TECH_AGITATION]->get_nation_tech_level(
                                true_nation_recno)
                          : 0;
      ...
      if (firmCamp && firmCamp->overseer_recno) {
        ...
        if (unitPtr->race_id == race_id) {
          if (misc.random((100 - spy_skill + unitPtr->skill_level()) / 10) <=
                  techLevel
              // a commander with a higher leadership skill will be less
              // influenced by the spy's dissents
              && unitPtr->loyalty > 0) {
      ...
      for (int i = 0; i < firmCamp->soldier_count; i++, soldierPtr++) {
        ...
        if (!soldierPtr
                 ->spy_recno) // the loyalty of the spy himself does not change
        {
          if (misc.random(11 - spy_skill / 10) <= techLevel &&
...
int Spy::can_sabotage() {
  ...
  //			 firm_array[spy_place_para]->firm_id != FIRM_CAMP; 	// no
  //sabotage actino for military camp
  return 0;
}
...
void Spy::get_killed(int dispNews) {
  ...
  if (true_nation_recno ==
          nation_array.player_recno || // the player's spy is killed
      cloaked_nation_recno ==
          nation_array.player_recno) // a spy cloaked as the player's people is
                                     // killed in the player's firm or firm
  {
    // ####### begin Gilbert 17/6 ########//
    if (dispNews)
  ...
  nation_array[true_nation_recno]->change_reputation(
      (float)-SPY_KILLED_REPUTATION_DECREASE);
  ...
  //------- if the spy is in a firm -------//
    ...
    //### begin alex 31/3 ###//
    // err_here();		// only spies in towns and firms will get killed
    // instantly
  ...
  //--- If the spy is in an AI town or firm, the AI's relationship towards the
  //spy's owner nation will decrease ---//

  if (hostNationRecno && nation_array[hostNationRecno]->is_ai()) {
    nation_array[hostNationRecno]->change_ai_relation_level(true_nation_recno,
                                                            -5);
  }
  ...
  /*	switch( action_mode )
          {
                  case SPY_IDLE:
                  {
                          //---- if the spy is in a firm or town of its own
  nation ---//

                          if( (spy_place==SPY_TOWN &&
                                    town_array[spy_place_para]->nation_recno ==
  true_nation_recno) || (spy_place==SPY_FIRM &&
                                    firm_array[spy_place_para]->nation_recno ==
  true_nation_recno) )
                          {
                                  return "Counter-Spy";
                          }
...
void Spy::pay_expense() {
  ...
  if (spy_place == SPY_MOBILE || inOwnFirm) {
    if (nationPtr->food > 0) {
      ...
      if (info.game_date % NO_FOOD_LOYALTY_DECREASE_INTERVAL ==
          0) // decrease 1 loyalty point every 2 days
        change_loyalty(-1);
    }
  }
}
...
void Spy::drop_spy_identity() {
  ...
  spy_array.del_spy(spy_recno); // Spy::deinit() will take care of the rest of
                                // the initialization for the spy
}
...
void Spy::change_cloaked_nation(int newNationRecno) {
  ...
  //--- only mobile units and overseers can change nation, spies in firms or
  //towns cannot change nation, their nation recno must be the same as the town
  //or the firm's nation recno
  ...
  if (spy_place == SPY_MOBILE) {
    ...
    if (firmCamp && firmCamp->overseer_recno &&
        unit_array[firmCamp->overseer_recno]->spy_recno == spy_recno) {
      unit_array[firmCamp->overseer_recno]->spy_change_nation(newNationRecno,
                                                              COMMAND_AUTO);
      rc = 1;
    }
  }
  // ###### begin Gilbert  4/7 #########//
    ...
    if (mobiledUnitRecno) {
      ...
      unit_array[spy_place_para]->spy_change_nation(newNationRecno,
                                                    COMMAND_AUTO);
      rc = 1;
    }
  }
  ...
  if (rc) {
    ...
    err_here(); // cannot change a spy's cloaked_nation_recno in a firm and town
                // as the spy's cloaked nation recno must be the same as the
                // firm and town's nation recno
  }
}
  ...
  if (firm_res[firmPtr->firm_id]->need_overseer) {
    ...
    if (firmCamp) {
      ...
      for (int i = 0; i < firmCamp->soldier_count; i++, soldierPtr++) {
        ...
        obeyFlag =
            misc.random(100) < obeyChance; // if obeyChance >= 100, all units
                                           // will object the overseer
      ...
      if (firmPtr->nation_recno == nation_array.player_recno)
        news_array.firm_captured(spy_place_para, true_nation_recno,
                                 1); // 1 - the capturer is a spy
    ...
    //---- check whether it's true that the only units in the firms are our
    spies ---//
    ...
    for( int i=0 ; i<firmPtr->worker_count ; i++, workerPtr++ )
    {
            ...
            if( spy_array[workerPtr->spy_recno]->true_nation_recno !=
    true_nation_recno ) return 0;
    // this worker is a spy, but not belong to the same nation
    }
    */
    ...
    firmPtr->change_nation(
        true_nation_recno); // the firm change nation and the spies inside the
                            // firm will have their cloaked nation recno changed
  }
...
//
// [int] decPop - whether need to decrease the population of town when the spy
// is mobilized.
//					   should set to no when it's calling from
//training a unit. (default: 1)
//
// return: recno of the mobilized unit.
//
int Spy::mobilize_town_spy(int decPop) {
  ...
  Unit *unitPtr =
      unit_array[unitRecno]; // set the spy vars of the mobilized unit
  ...
  if (firmPtr->cast_to_FirmCamp()) {
    ...
    if (!spyUnitRecno) {
      ...
      for (i = firmCamp->soldier_count - 1; i >= 0; --i) {
        if (firmCamp->soldier_array[i].spy_recno == spy_recno) {
          spyUnitRecno = firmCamp->mobilize_soldier(
              i + 1, COMMAND_AUTO); // note: mobilize_woker() will decrease
    ...
    for (i = firmTrain->trainee_count - 1; i >= 0; --i) {
      if (firmTrain->trainee_array[i].spy_recno == spy_recno) {
        ...
        // ##### end Gilbert 27/1 ######//
        spyUnitRecno = firmTrain->mobilize_trainee(
            i + 1, COMMAND_AUTO); // note: mobilize_woker() will decrease
                                  // Firm::player_spy_count
        break;
      }
...
void Spy::think_become_king() {
  ...
  //--- if his nation is more power than the player's nation, the chance of
  //handing his nation over to his parent nation will be low unless the loyalty
  //is very high ---//

  int acceptLoyalty = 90 + hisNationPower - parentNationPower;

  if (spy_loyalty >= acceptLoyalty &&
      nation_array[cloaked_nation_recno]
          ->is_ai()) // never will a spy take over the player's nation
  {
...
void Spy::reward(int remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_SPY_REWARD, sizeof(short));
  ...
  nation_array[true_nation_recno]->add_expense(EXPENSE_REWARD_UNIT,
                                               (float)REWARD_COST);
}
...
void Spy::set_exposed(int remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    // packet structure <spy recno>
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_SPY_EXPOSED, sizeof(short));
    shortPtr[0] = spy_recno;
    return;
  }
  // ##### end Gilbert 26/9 #######//
...
int Spy::assassinate(int targetUnitRecno, int remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure <spy recno>
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_SPY_ASSASSINATE,
                                                         sizeof(short) * 2);
    shortPtr[0] = spy_recno;
  ...
  if (!get_assassinate_rating(targetUnitRecno, attackRating, defenseRating,
                              otherDefenderCount))
    return 0;
  ...
  int rc;
  int trueNationRecno =
      true_nation_recno; // need to save it first as the spy may be killed later
  ...
  // ##### end Gilbert 12/12 ######//
  {
    ...
    // ###### begin Gilbert 12/12 ##### //
    // int spyKillFlag = otherDefenderCount > 0 && attackRating - defenseRating
    // < 80;
    int spyKillFlag = otherDefenderCount > 0 &&
                      !(misc.random(100) < attackRating - defenseRating - 20);
...
//
int Spy::get_assassinate_rating(int targetUnitRecno, int &attackRating,
                                int &defenseRating, int &otherDefenderCount) {
  //---------- validate first -----------//
  ...
  if (true_nation_recno)
    assassinTechLevel =
        tech_res[TECH_ASSASSIN]->get_nation_tech_level(true_nation_recno);
  ...
  if (targetUnit->nation_recno)
    counterSpyLevel = tech_res[TECH_COUNTER_SPY]->get_nation_tech_level(
        targetUnit->nation_recno);
  ...
  if (firmPtr->cast_to_FirmCamp()) {
    ...
    for (int i = 0; i < firmCamp->soldier_count; i++) {
      ...
      if (spyRecno) {
        ...
        } else if (spyPtr->true_nation_recno ==
                   firmCamp->nation_recno) // enemy spy
        {
...
int Spy::can_camouflage() {
  return camouflage_power >= MIN_CAMOUFLAGE_POWER && true_nation_recno &&
         tech_res[TECH_CAMOUFLAGE]->get_nation_tech_level(true_nation_recno) &&
...
void Spy::start_camouflage(char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_SPY_CAMOUFLAGE,
                                                         2 * sizeof(short));
    shortPtr[0] = spy_recno;
    shortPtr[1] = 0;
    // ######### end Gilbert 7/10 ########//
  ...
  err_when(camouflage_power <
           MIN_CAMOUFLAGE_POWER); // call can_camouflage to disable camouflage
...
int Spy::steal_tech(int targetNationRecno, char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_SPY_STEAL_TECH,
                                                         2 * sizeof(short));
  ...
  //	if( !get_steal_tech_rating(targetUnitRecno, attackRating, defenseRating,
  //otherDefenderCount) ) 		return 0;
  ...
  if (misc.random(100) < rating) {
    ...
    for (int i = 1; i <= tech_res.tech_count; ++i) {
      ...
      if ((techInfo->race_id >= 0) !=
          (race_id >= 0)) // human get human's, monster get monster
        continue;
      ...
      if (techInfo->get_nation_tech_level(true_nation_recno) >=
          techInfo->get_nation_tech_level(targetNationRecno))
        continue; // skip tech we are more advanced

      ++matchCount;
      if (misc.random(matchCount) ==
          0) // n-th match have 1/n chance to be selected, therefore evenly
             // distributed
        matchTech = i;
    }
...
int Spy::get_steal_rating(int targetNationRecno) {
  ...
  int sumAssist = 0;
  int counterSpyLevel =
      tech_res[TECH_COUNTER_SPY]->get_nation_tech_level(targetNationRecno);
  ...
  if (resistCount == 0)
    chance -=
        20 - sumAssist / 10; // so a 100 spy skill still have chance to fail
...
//
int Spy::create_incident(int targetNationRecno, int involvedNationRecno,
                         char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_SPY_CREATE_INCIDENT, 3 * sizeof(short));
    // packet structure : <spy recno> <target nation recno> <involved natino
    // recno>
    shortPtr[0] = spy_recno;
    shortPtr[1] = targetNationRecno;
  ...
  if (nation_array.is_deleted(targetNationRecno) ||
      nation_array.is_deleted(involvedNationRecno))
    return 0;
