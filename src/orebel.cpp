...
int RebelArray::create_rebel(int unitRecno, int hostileNationRecno, int actionMode, int actionPara)
{
    ...
    for (int i = size(); i >= 1; i--)
    {
        ...
        if (rebelPtr->action_mode == actionMode && rebelPtr->action_para == actionPara)
        {
...
void RebelArray::drop_rebel_identity(int unitRecno)
{
    ...
    //----- if all rebels are dead and the rebel doesn't occupy a town, del the
    // rebel group ----//
...
void Rebel::process_leader_quit()
{
    ...
    {
        err_when(!town_recno); // if mobile_label_count is 0, the rebel group must
                               // have a town
    ...
    if (mobile_rebel_count > 1)
    {
        ...
        for (i = unit_array.size(); i > 0; i--)
        {
            ...
            if (unitPtr->unit_mode == UNIT_MODE_REBEL && unitPtr->unit_mode_para == leader_unit_recno)
            {
...
int Rebel::select_new_leader()
{
    ...
    for (i = unit_array.size(); i > 0; i--)
    {
        ...
        if (unitPtr->unit_mode == UNIT_MODE_REBEL && unitPtr->unit_mode_para == rebel_recno)
        {
...
void RebelArray::settle_town(int unitRecno, int townRecno)
{
    ...
    err_when(townPtr->race_id != unitPtr->race_id);

    if (rebelPtr->town_recno == 0 && townPtr->rebel_recno == 0 &&
        townPtr->population == 0) // only when the rebel is the one that creates the town.
    {
...
void Rebel::think_cur_action()
{
    ...
    if (i == 0) // if all rebels have stopped
    {
    ...
    /*
       ...
       town_array[action_para]->nation_recno == 0 )
                    {
       ...
       ---//
                    ...
                    {
                            all_rebel_attack(
       town_array[action_para]->base_obj_recno );
                    }
            }

            //----- if the rebel should be attacking a firm -----//

            else if( action_mode == REBEL_ATTACK_FIRM )
            {
                    if( firm_array.is_deleted(action_para) ||
       firm_array[action_para]->nation_recno == 0 )
                    {
       ...
       ---//
                    ...
                    {
                            all_rebel_attack(
       firm_array[action_para]->base_obj_recno );
                    }
            }
    */
}
...
void Rebel::next_day()
{
    ...
    if (town_recno)
    {
        ...
        if (town_array.is_deleted(town_recno))
        {
            ...
            //--- if the town has been destroyed, the rebel group becomes mobile
            // again, and a new leader needs to be selected.
        ...
        return; // don't think new action as this rebel defender need to go back to
                // its town.
    }
    ...
    err_when(mobile_rebel_count == 0 &&
             town_recno == 0); // if both are zero, this rebel group should have been deleted.
    ...
    if (mobile_rebel_count > 0) // if there are mobile rebel units on the map
    {
        if (action_mode == REBEL_IDLE)
            ...
            think_cur_action(); // think if there should be any changes to the current
                                // action
    }
...
int Rebel::think_settle_new()
{
    ...
    int xLoc2 = leaderXLoc + townWidth - 1;
    int yLoc2 = leaderYLoc + townHeight - 1;

    if (xLoc2 >= MAX_WORLD_X_LOC)
    {
        xLoc2 = MAX_WORLD_X_LOC - 1;
        leaderXLoc = xLoc2 - townWidth + 1;
    }

    if (yLoc2 >= MAX_WORLD_Y_LOC)
    {
        yLoc2 = MAX_WORLD_Y_LOC - 1;
        leaderYLoc = yLoc2 - townHeight + 1;
    }

    int regionId = world.get_region_id(leaderXLoc, leaderYLoc);

    if (world.locate_space(leaderXLoc, leaderYLoc, xLoc2, yLoc2, townWidth, townHeight, UNIT_LAND, regionId, 1))
    {
        action_mode = REBEL_SETTLE_NEW;
        action_para = leaderXLoc + INTER_PLACE_SPACE;
        action_para2 = leaderYLoc + INTER_PLACE_SPACE;
...
int Rebel::think_settle_to()
{
    ...
    for (i = 1; i <= town_array.size(); i++)
    {
        ...
        if (townPtr->race_id != leaderUnit->race_id) // only same race
            continue;

        if (world.get_region_id(townPtr->loc_x1, townPtr->loc_y1) != curRegionId)
            continue;

        //----- don't assign to hostile nations -----//

        if (townPtr->nation_recno && is_hostile_nation(townPtr->nation_recno))
            continue;

        //--- don't settle to slave towns controlled by Fryhtans ---//
...
int Rebel::think_capture_attack_town()
{
    ...
    for (int i = town_array.size(); i > 0; i--)
    {
        ...
        townDistance = misc.points_distance(leaderXLoc, leaderYLoc, townPtr->center_x, townPtr->center_y);
...
int Rebel::think_attack_firm()
{
    ...
    for (int i = firm_array.size(); i > 0; i--)
    {
        ...
        if (firmPtr->is_monster()) // they do not attack monster firms
            continue;

        if (world.get_region_id(firmPtr->loc_x1, firmPtr->loc_y1) != curRegionId)
            continue;

        firmDistance = misc.points_distance(leaderXLoc, leaderYLoc, firmPtr->center_x, firmPtr->center_y);
...
void Rebel::execute_new_action()
{
    ...
    for (int i = unit_array.size(); i >= 1; i--)
    {
        ...
        if (unitPtr->unit_mode == UNIT_MODE_REBEL && unitPtr->unit_mode_para == rebel_recno)
        {
            err_when(unit_group.size() >= mobile_rebel_count);

            unit_group.add_unit(i);
        }
    }

#ifdef DEBUG
    if (unit_group.size() + dieUnit != mobile_rebel_count)
        int debug = 0;
#endif

    if (unit_group.size() == 0) // the rebel units are dying so they are not added to unit_group.
        return;
    ...
    case REBEL_ATTACK_TOWN:
        err_when(town_array.is_deleted(action_para) ||
                 town_array[action_para]->nation_recno ==
                     0); // these should have been checked before calling this function
        unit_group.attack(town_array[action_para]->base_obj_recno, COMMAND_AI);
        break;

    case REBEL_ATTACK_FIRM:
        err_when(firm_array.is_deleted(action_para) ||
                 firm_array[action_para]->nation_recno ==
                     0); // these should have been checked before calling this function
        unit_group.attack(firm_array[action_para]->base_obj_recno, COMMAND_AI);
        break;

    case REBEL_SETTLE_NEW: {
        Location *locPtr = world.get_loc(action_para, action_para2);

        //---- if the town has already been built ----//

        if (locPtr->is_town() && town_array[locPtr->town_recno()]->rebel_recno == rebel_recno &&
            town_array[locPtr->town_recno()]->population < MAX_TOWN_POPULATION)
        {
            unit_group.assign(action_para, action_para2, UNIT_LAND, COMMAND_AI);
        }
        else //---- otherwise, order the first unit to build a town first ----//
        {
            unit_group.get_unit(1)->settle_town(action_para, action_para2, COMMAND_AI);
        }
        break;
    }
...
void Rebel::think_town_action()
{
    ...
    /*	// Comment out because Town::form_new_nation() will error when
       Town::rebel_recno > 0
...
//--------- Begin of function Rebel::all_rebel_attack ---------//
//
void Rebel::all_rebel_attack(int targetObjRecno)
{
    Unit *unitPtr;
...
//----------- End of function Rebel::all_rebel_attack ---------//
...
//
void Rebel::town_being_attacked(BaseObj *attackerObj)
{
    err_when(!town_recno);
    ...
    set_hostile_nation(attackerObj->nation_recno);
}
...
void Unit::process_rebel()
{
        ...
        {
                case REBEL_ATTACK_TOWN:
                        if( town_array.is_deleted(rebelPtr->action_para) )
// if the town has been destroyed. rebelPtr->set_action(REBEL_IDLE); else
                        {
                                Town* townPtr =
town_array[rebelPtr->action_para]; attack_town( townPtr->loc_x1, townPtr->loc_y1
);
                        }
                        break;

                case REBEL_SETTLE_NEW:
                        if( !world.can_build_town(rebelPtr->action_para,
rebelPtr->action_para2, sprite_recno) )
                        {
                                Location* locPtr =
world.get_loc(rebelPtr->action_para, rebelPtr->action_para2);

                                if( locPtr->is_town() &&
                                         town_array[locPtr->town_recno()]->race_id
== race_id && town_array[locPtr->town_recno()]->rebel_recno ==
rebelPtr->rebel_recno && town_array[locPtr->town_recno()]->population <
MAX_POPULATION )
                        ...
                        {
                                settle( rebelPtr->action_para,
rebelPtr->action_para2 );
                        }
                        break;
