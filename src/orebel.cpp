...
RebelArray::RebelArray()
    : DynArrayB(sizeof(Rebel *), 10, DEFAULT_REUSE_INTERVAL_DAYS) {}
//--------- End of function RebelArray::RebelArary ----------//

//------- Begin of function RebelArray::~RebelArray ----------//
//
RebelArray::~RebelArray() { deinit(); }
...
//
int RebelArray::create_rebel(int unitRecno, int hostileNationRecno,
                             int actionMode, int actionPara) {
  //------------------------------------------//
...
void Rebel::process_leader_quit() {
  ...
  if (mobile_rebel_count >
      0) // it must at least be 2: the dying leader + one rebel soldier
  {
    select_new_leader();
  } else {
...
void RebelArray::settle_town(int unitRecno, int townRecno) {
  ...
  if (rebelPtr->town_recno == 0 && townPtr->rebel_recno == 0 &&
      townPtr->population ==
          0) // only when the rebel is the one that creates the town.
  {
...
void Rebel::next_day() {
  ...
  err_when(
      mobile_rebel_count == 0 &&
      town_recno ==
          0); // if both are zero, this rebel group should have been deleted.
...
int Rebel::think_settle_new() {
  ...
  if (world.locate_space(leaderXLoc, leaderYLoc, xLoc2, yLoc2, townWidth,
                         townHeight, UNIT_LAND, regionId, 1)) {
...
int Rebel::think_settle_to() {
  ...
  Unit *leaderUnit = unit_array[leader_unit_recno];
  int curRegionId =
      world.get_region_id(leaderUnit->cur_x_loc(), leaderUnit->cur_y_loc());
...
void Rebel::execute_new_action() {
  ...
  if (unit_group.size() ==
      0) // the rebel units are dying so they are not added to unit_group.
    return;
  ...
  case REBEL_ATTACK_TOWN:
    err_when(
        town_array.is_deleted(action_para) ||
  ...
  case REBEL_SETTLE_NEW: {
    ...
    if (locPtr->is_town() &&
        town_array[locPtr->town_recno()]->rebel_recno == rebel_recno &&
    ...
    {
      unit_group.get_unit(1)->settle_town(action_para, action_para2,
                                          COMMAND_AI);
    }
    break;
  }
...
void Rebel::think_town_action() {
  if (town_recno == 0 ||
      mobile_rebel_count >
          0) // only when all rebel units has settled in the town
    return;
