...
unsigned char AnimLine::series_color_code[ANIMCOLOR_SERIES][ANIMLINE_PERIOD] = {
    ...
    {0xA4, 0xA5, 0xA6, 0xA7, 0xA7, 0xA6, 0xA5,
     0xA4}, // color series for selected (blue series)
    {0xB4, 0xB5, 0xB6, 0xB7, 0xB7, 0xB6, 0xB5,
     0xB4}, // color series for pointed (yellow series)
    {0xA8, 0xA9, 0xAA, 0xAB, 0xAB, 0xAA, 0xA9,
     0xA8}, // color series for pointed and selected (green series)
};
...
// [int] animatedFlag    whether the colors of the line is cycle   (default:1 =
// animated) [int] effectFlag      effect : -1=dimming B/W, 0=non-dimming B/W,
// 1=non-dimming series (default : 0)
//
void AnimLine::draw_line(VgaBuf *vgabuf, short x1, short y1, short x2, short y2,
                         int animatedFlag, int effectFlag) {
  ...
  if (x1 >= bound_x1 && x1 <= bound_x2 && x2 >= bound_x1 && x2 <= bound_x2 &&
    ...
    if (y1 == y2) {
      basic_hline(vgabuf, x1, x2, y2, animatedFlag, colorCode, 0);
    } else if (x1 == x2) {
      basic_vline(vgabuf, x1, y1, y2, animatedFlag, colorCode, 0);
    } else {
      basic_line(vgabuf, x1, y1, x2, y2, animatedFlag, colorCode, 0);
    }
  } else {
    int initPhase = 0;
    ...
    if (y1 == y2) {
      ...
      if (y1 < bound_y1 || y1 > bound_y2)
        return;
      if (x1 < bound_x1) {
        initPhase = bound_x1 - x1;
        x1 = bound_x1;
      } else if (x1 > bound_x2) {
        initPhase = x1 - bound_x2;
        x1 = bound_x2;
      }
      ...
      basic_hline(vgabuf, x1, x2, y1, animatedFlag, colorCode, initPhase);
    } else if (x1 == x2) {
      // vertical line
      if (x1 < bound_x1 || x1 > bound_x2)
        return;
      if (y1 < bound_y1) {
        initPhase = bound_y1 - y1;
        y1 = bound_y1;
      } else if (y1 > bound_y2) {
        initPhase = y1 - bound_y2;
        y1 = bound_y2;
      }
      ...
      basic_vline(vgabuf, x1, y1, y2, animatedFlag, colorCode, initPhase);
    } else {
      ...
      if (x1 < bound_x1 || x1 > bound_x2 || y1 < bound_y1 || y1 > bound_y2) {
        if (x1a < bound_x1 || x1a > bound_x2 || y1a < bound_y1 ||
            y1a > bound_y2) {
          if (x1b < bound_x1 || x1b > bound_x2 || y1b < bound_y1 ||
              y1b > bound_y2) {
            return;
          } else {
      ...
      if (x2 < bound_x1 || x2 > bound_x2 || y2 < bound_y1 || y2 > bound_y2) {
        if (x2a < bound_x1 || x2a > bound_x2 || y2a < bound_y1 ||
            y2a > bound_y2) {
          if (x2b < bound_x1 || x2b > bound_x2 || y2b < bound_y1 ||
              y2b > bound_y2) {
            return;
          } else {
            x2 = x2b;
      ...
      //			if( y1 == y2)
      //				basic_hline(vgabuf, x1, x2, y1,
      //animatedFlag); 			else if( x1 == x2) 				basic_vline(vgabuf, x1, y1, y2,
      //animatedFlag); 			else
      basic_line(vgabuf, x1, y1, x2, y2, animatedFlag, colorCode, initPhase);
    }
  }
}
...
// 1=non-dimming series (default : 0)
//
void AnimLine::thick_line(VgaBuf *vgabuf, short x1, short y1, short x2,
                          short y2, int animatedFlag, int effectFlag) {
...
// draw a clipped line
//
void AnimLine::basic_line(VgaBuf *vgabuf, short x1, short y1, short x2,
                          short y2, int animatedFlag, unsigned char *colorCode,
                          int initPhase) {
  ...
  short *bufPtr = vgabuf->buf_ptr(x1, y1);
  short linePhase;
  err_when(dx == 0 || dy == 0);
  ...
  if (abs(dy) <= abs(dx)) {
    ...
    d = 2 * dy - dx;
    int x = x1 - inc_x;
    linePhase = ((animatedFlag ? phase : 0) + initPhase) % ANIMLINE_PERIOD;
    err_when(linePhase < 0);
    do {
      x += inc_x;
      *bufPtr = VgaBuf::translate_color(colorCode[linePhase]);
...
//
void AnimLine::basic_hline(VgaBuf *vgabuf, short x1, short x2, short y1,
                           int animatedFlag, unsigned char *colorCode,
                           int initPhase) {
  err_when(x1 < bound_x1 || x1 > bound_x2);
  err_when(x2 < bound_x1 || x2 > bound_x2);
  err_when(y1 < bound_y1 || y1 > bound_y2);

  short linePhase = ((animatedFlag ? phase : 0) + initPhase) % ANIMLINE_PERIOD;
  err_when(linePhase < 0);
  short *bufPtr = vgabuf->buf_ptr(x1, y1);
...
//
void AnimLine::basic_vline(VgaBuf *vgabuf, short x1, short y1, short y2,
                           int animatedFlag, unsigned char *colorCode,
                           int initPhase) {
  err_when(x1 < bound_x1 || x1 > bound_x2);
  err_when(y1 < bound_y1 || y1 > bound_y2);
