...
unsigned char AnimLine::init_color_code[ANIMCOLOR_PERIOD][ANIMLINE_PERIOD] = {
    ...
    {0xa6, 0xa5, 0xa4, 0xa4, 0x9c, 0xa4, 0xa4, 0xa5}, {0xa7, 0xa6, 0xa5, 0xa4, 0x9c, 0xa4, 0xa5, 0xa6},
    {0xc7, 0xa7, 0xa6, 0x9c, 0x9f, 0x9c, 0xa6, 0xa7}, {0x90, 0x93, 0x98, 0x9c, 0x9f, 0x9c, 0x98, 0x93},
    {0xc7, 0xa7, 0xa6, 0x9c, 0x9f, 0x9c, 0xa6, 0xa7}, {0xa7, 0xa6, 0xa5, 0xa4, 0x9c, 0xa4, 0xa5, 0xa6}};
...
unsigned char AnimLine::series_color_code[ANIMCOLOR_SERIES][ANIMLINE_PERIOD] = {
    ...
    {0xA4, 0xA5, 0xA6, 0xA7, 0xA7, 0xA6, 0xA5, 0xA4}, // color series for selected (blue series)
    {0xB4, 0xB5, 0xB6, 0xB7, 0xB7, 0xB6, 0xB5, 0xB4}, // color series for pointed (yellow series)
    {0xA8, 0xA9, 0xAA, 0xAB, 0xAB, 0xAA, 0xA9, 0xA8}, // color series for pointed and selected (green series)
};
...
{
    if ((x1 < bound_x1 && x2 < bound_x1) || (x1 > bound_x2 && x2 > bound_x2) || (y1 < bound_y1 && y2 < bound_y1) ||
        (y1 > bound_y2 && y2 > bound_y2))
        return;
    ...
    if (x1 >= bound_x1 && x1 <= bound_x2 && x2 >= bound_x1 && x2 <= bound_x2 && y1 >= bound_y1 && y1 <= bound_y2 &&
        y2 >= bound_y1 && y2 <= bound_y2)
    {
...
void AnimLine::basic_line(VgaBuf *vgabuf, short x1, short y1, short x2, short y2, int animatedFlag,
                          unsigned char *colorCode, int initPhase)
{
...
//
void AnimLine::basic_hline(VgaBuf *vgabuf, short x1, short x2, short y1, int animatedFlag, unsigned char *colorCode,
                           int initPhase)
{
    err_when(x1 < bound_x1 || x1 > bound_x2);
    err_when(x2 < bound_x1 || x2 > bound_x2);
...
//
void AnimLine::basic_vline(VgaBuf *vgabuf, short x1, short y1, short y2, int animatedFlag, unsigned char *colorCode,
                           int initPhase)
{
    err_when(x1 < bound_x1 || x1 > bound_x2);
    err_when(y1 < bound_y1 || y1 > bound_y2);
