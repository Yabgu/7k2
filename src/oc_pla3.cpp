...
void CampaignEastWest::plot_a3_create_game() {
  ...
  plot_nation_recno1 = nation_array.new_nation(
      NATION_HUMAN, NATION_AI, RACE_VIKING, nation_array.random_unused_color());
  god_res[GOD_VIKING]->enable_know(plot_nation_recno1);

  plot_nation_recno2 = nation_array.new_nation(
      NATION_HUMAN, NATION_AI, RACE_MONGOL, nation_array.random_unused_color());
...

  {
    ...
    int townRecno = random_pick_town_with_camp(
        plot_nation_recno1, 2); // only pick towns with <= 2 links
    ...
    for (; trial > 0 && seatCount > 0; --trial) {
      if ((townRecno = random_pick_town_with_camp(
               plot_nation_recno1, 8))) // only pick town with <= 8 links
      {
        int firmRecno;
        if ((firmRecno = create_firm_next_to_place(
                 town_array[townRecno], FIRM_BASE, vikingNation->race_id))) {
    ...
    static short neverConsiderArray1[] = {TALK_PROPOSE_TRADE_TREATY,
                                          TALK_PROPOSE_FRIENDLY_TREATY,
                                          TALK_PROPOSE_ALLIANCE_TREATY, 0};

    vikingNation->get_relation(plot_enemy_nation_recno)
        ->set_ai_never_consider(neverConsiderArray1);

    //--- hard-wire the viking's relation with the player ---//

    static short neverConsiderArray2[] = {TALK_PROPOSE_ALLIANCE_TREATY,
                                          TALK_DECLARE_WAR, TALK_SURRENDER, 0};

    vikingNation->get_relation(nation_array.player_recno)
        ->set_ai_never_consider(neverConsiderArray2);

    //--- hard-wire the viking's relation with the mongol nation ---//

    static short neverConsiderArray3[] = {
        TALK_PROPOSE_TRADE_TREATY, TALK_PROPOSE_FRIENDLY_TREATY,
        TALK_PROPOSE_ALLIANCE_TREATY, TALK_SURRENDER, 0};

    vikingNation->get_relation(plot_nation_recno2)
...

  {
    ...
    int townRecno2 = random_pick_town_with_camp(
        plot_nation_recno2, 2); // only pick towns with <= 2 links
    ...
    mongolNation->get_relation(plot_enemy_nation_recno)
        ->set_ai_never_consider(neverConsiderArray1);
    ...
    mongolNation->get_relation(nation_array.player_recno)
        ->set_ai_never_consider(neverConsiderArray2);
    ...
    mongolNation->get_relation(plot_nation_recno1)
        ->set_ai_never_consider(neverConsiderArray3);
  }
  ...
  if (!nation_array.is_deleted(plot_nation_recno1) &&
    ...
    if (!nation_array.is_deleted(plot_enemy_nation_recno)) {
      NationRelation *nationRelation =
          nation_array[plot_nation_recno1]->get_relation(
      ...
      if (nationRelation->status == RELATION_ALLIANCE) {
        // enhance ai relation so it can accept player request declare war with
        // frythan

        nation_array[plot_nation_recno1]->change_ai_relation_level(
            nation_array.player_recno, +100);

        // hardware not to cease fire with fryhtan kingdom

        short resetWStr2[] = {TALK_REQUEST_CEASE_WAR, 0};
        nation_array[plot_nation_recno1]
            ->get_relation(plot_enemy_nation_recno)
    ...
    // fryhtan is defeated
      ...
      talk_res.ai_send_talk_msg(nation_array.player_recno, plot_nation_recno1,
                                TALK_SURRENDER);
