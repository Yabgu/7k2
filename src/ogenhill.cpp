        ...
        if (lowTerrain >= terrainType) {
          ...
          if (y > 0) {
            *aboveLoc = *locPtr;
            ...
            if (y == max_y_loc - 1)
              locPtr->terrain_id =
                  terrain_res.scan(priTerrain, secTerrain, patternId);
          }
        } else {
          ...
          if (y > 0) {
            aboveLoc->set_hill(hill_res.locate(patternId,
                                               hill_res[hillId]->sub_pattern_id,
                                               HIGH_HILL_PRIORITY, 0));
...
#define IS_SOUTH_EXIT_PATTERN(h)                                               \
  (h == SOUTH_PATTERN1 || h == SOUTH_PATTERN2 || h == SOUTH_PATTERN3 ||        \
   h == SOUTH_PATTERN4)
...
#define SOUTH_CENTRE_SPECIAL 'A'
  ...
  for (y = 1; y < max_y_loc - 1; ++y) {
    ...
    x = 0;
    locPtr = get_loc(x, y);
    for (; x<max_x_loc - 2; ++x, ++locPtr, lastExit = lastExit> 0 ? lastExit - 1
                                                                  : 0) {
      ...
      if (!lastExit && locPtr->has_hill() &&
          ...
          !h1->special_flag && (h1p = h1->pattern_id) &&
          IS_SOUTH_EXIT_PATTERN(h1p) && (locPtr + 1)->has_hill() &&
          (h2 = hill_res[(locPtr + 1)->hill_id1()])->priority ==
              HIGH_HILL_PRIORITY &&
          (h2p = h2->pattern_id) && IS_SOUTH_EXIT_PATTERN(h2p) &&
          (locPtr + 2)->has_hill() &&
          (h3 = hill_res[(locPtr + 2)->hill_id1()])->priority ==
              HIGH_HILL_PRIORITY &&
          (h3p = h3->pattern_id) && IS_SOUTH_EXIT_PATTERN(h3p) &&
        ...
        set_surr_power_off(x, y);
        //#### end alex 24/6 ####//
        if ((terrainId = terrain_res.scan(terrainType - 1, terrainType - 1, 0,
                                          0, 1, 0)) != 0)
        ...
        if ((terrainId = terrain_res.scan(terrainType, BOTTOM_MASK, terrainType,
                                          BOTTOM_MASK, terrainType, BOTTOM_MASK,
                                          terrainType, BOTTOM_MASK)) != 0)
          // ##### end Gilbert 14/10 #####//
          loc2->terrain_id = terrainId;
        err_when(loc2->has_hill() && loc2->walkable());
        ...
        loc2->walkable_reset();
        if ((terrainId =
                 terrain_res.scan(terrainType, terrainType - 1, SOUTH_PATTERN2,
                                  0, 1, SOUTH_CENTRE_SPECIAL)) != 0)
          loc2->terrain_id = terrainId;
        err_when(loc2->has_hill() && loc2->walkable());
...
#define IS_NORTH_EXIT_PATTERN(h)                                               \
  (h == NORTH_PATTERN1 || h == NORTH_PATTERN2 || h == NORTH_PATTERN3 ||        \
   h == NORTH_PATTERN4)
  ...
  for (y = 1; y < max_y_loc - 2; ++y) {
    ...
    for (; x >= 0; --x, --locPtr, lastExit = lastExit > 0 ? lastExit - 1 : 0) {
      ...
      if (!lastExit && locPtr->has_hill() &&
          (h1 = hill_res[locPtr->hill_id1()])->priority == HIGH_HILL_PRIORITY &&
          !h1->special_flag && (h1p = h1->pattern_id) &&
          IS_NORTH_EXIT_PATTERN(h1p) && (locPtr + 1)->has_hill() &&
          (h2 = hill_res[(locPtr + 1)->hill_id1()])->priority ==
              HIGH_HILL_PRIORITY &&
          (h2p = h2->pattern_id) && IS_NORTH_EXIT_PATTERN(h2p) &&
          (locPtr + 2)->has_hill() &&
          (h3 = hill_res[(locPtr + 2)->hill_id1()])->priority ==
              HIGH_HILL_PRIORITY &&
          (h3p = h3->pattern_id) && IS_NORTH_EXIT_PATTERN(h3p) &&
          get_loc(x + 1, y + 1)->walkable()) {
        short hillId, terrainId;
        Location *loc2;
        ...
        //	loc2->terrain_id = terrainId;
        if ((terrainId =
                 terrain_res.scan(terrainType, terrainType - 1, NORTH_PATTERN2,
                                  0, 1, NORTH_CENTRE_SPECIAL)) != 0)
          loc2->terrain_id = terrainId;
        err_when(loc2->has_hill() && loc2->walkable());
...
#define IS_WEST_EXIT_PATTERN(h)                                                \
  (h == WEST_PATTERN1 || h == WEST_PATTERN2 || h == WEST_PATTERN3 ||           \
   h == WEST_PATTERN4)
...
#define WEST_CENTRE_SPECIAL 'H'
  ...
  for (x = 1; x < max_x_loc - 1; ++x) {
    lastExit = 0;
    for (y = 0;
         y<max_y_loc - 4; ++y, lastExit = lastExit> 0 ? lastExit - 1 : 0) {
      locPtr = get_loc(x, y);
      HillBlockInfo *h1, *h2, *h3;
      char h1p, h2p, h3p;
      ...
      if (!lastExit && locPtr->has_hill() &&
          (h1 = hill_res[locPtr->hill_id1()])->priority == HIGH_HILL_PRIORITY &&
          !h1->special_flag && (h1p = h1->pattern_id) &&
          IS_WEST_EXIT_PATTERN(h1p) && get_loc(x, y + 1)->has_hill() &&
          get_loc(x, y + 2)->has_hill() &&
          (h2 = hill_res[get_loc(x, y + 2)->hill_id1()])->priority ==
              HIGH_HILL_PRIORITY &&
          (h2p = h2->pattern_id) && IS_WEST_EXIT_PATTERN(h2p) &&
          get_loc(x, y + 3)->has_hill() &&
          (h3 = hill_res[get_loc(x, y + 3)->hill_id1()])->priority ==
              HIGH_HILL_PRIORITY &&
          (h3p = h3->pattern_id) &&
        ...
        //	loc2->terrain_id = terrainId;
        if ((terrainId =
                 terrain_res.scan(terrainType, terrainType - 1, WEST_PATTERN2,
                                  0, 1, WEST_CENTRE_SPECIAL)) != 0)
          loc2->terrain_id = terrainId;
        err_when(loc2->has_hill() && loc2->walkable());
...
#define IS_EAST_EXIT_PATTERN(h)                                                \
  (h == EAST_PATTERN1 || h == EAST_PATTERN2 || h == EAST_PATTERN3 ||           \
   h == EAST_PATTERN4)
...
#define EAST_CENTRE_SPECIAL 'K'
  ...
  for (x = 1; x < max_x_loc - 1; ++x) {
    lastExit = 0;
    for (y = max_y_loc - 5; y >= 0;
         --y, lastExit = lastExit > 0 ? lastExit - 1 : 0) {
      locPtr = get_loc(x, y);
      HillBlockInfo *h1, *h2, *h3;
      char h1p, h2p, h3p;
      // three hill blocks on a row are pattern 9, 13, 17, 21
      ...
      if (!lastExit && locPtr->has_hill() &&
          (h1 = hill_res[locPtr->hill_id1()])->priority == HIGH_HILL_PRIORITY &&
          !h1->special_flag && (h1p = h1->pattern_id) &&
          IS_EAST_EXIT_PATTERN(h1p) && get_loc(x, y + 1)->has_hill() &&
          get_loc(x, y + 2)->has_hill() &&
          (h2 = hill_res[get_loc(x, y + 2)->hill_id1()])->priority ==
              HIGH_HILL_PRIORITY &&
          (h2p = h2->pattern_id) && IS_EAST_EXIT_PATTERN(h2p) &&
          get_loc(x, y + 3)->has_hill() &&
          (h3 = hill_res[get_loc(x, y + 3)->hill_id1()])->priority ==
              HIGH_HILL_PRIORITY &&
          (h3p = h3->pattern_id) &&
        ...
        //	loc2->terrain_id = terrainId;
        if ((terrainId =
                 terrain_res.scan(terrainType, terrainType - 1, EAST_PATTERN2,
