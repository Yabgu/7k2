...
void Firm::init(int xLoc, int yLoc, int nationRecno, int firmId, char *buildCode, short builderRecno)
{
    ...
    if (!buildCode || !firm_build_id) // some buildings like human offensive structures do not
    ...
    err_when(is_human() ==
             (firmId >= FIRM_LAIR && firmId <= FIRM_MAGIC)); // error when a human attempts to build a monster
                                                             // firm or a mosnter attempts to build a human firm
    ...
    abs_x1 =
        world.zoom_matrix->calc_abs_x(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, altitude) + firmBuild->min_offset_x;
    abs_y1 =
        world.zoom_matrix->calc_abs_y(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, altitude) + firmBuild->min_offset_y;
    ...
    under_construction = firmInfo->buildable; // whether the firm is under construction, if the firm is
...
void Firm::init_name()
{
    ...
    for (i = firm_array.size(); i > 0; i--)
    {
        ...
        if (firmPtr->firm_id == firm_id && firmPtr->closest_town_name_id == closest_town_name_id &&
...
void Firm::process_animation()
{
    ...
    if (hit_points < (max_hit_points() * 0.1))
    {
        if (misc.random(100) > 65)
        {
            ...
            if (effectId)
            {
                Effect::create(effectId, (loc_x1 + misc.random(20) % (loc_x2 - loc_x1 + 1)) * LOCATE_WIDTH,
                               (loc_y1 + misc.random(20) % (loc_y2 - loc_y1 + 1)) * LOCATE_HEIGHT, SPRITE_DIE, 1, 2, 0);
            }
        }
    }
...
void Firm::next_day()
{
    ...
    err_when(is_human() ==
             (firm_id >= FIRM_LAIR && firm_id <= FIRM_MAGIC)); // error when a human attempts to build a monster firm
    ...
    if (no_neighbor_space && info.game_date % 180 == firm_recno % 180)
    {
        ...
        if (nation_array[nation_recno]->find_best_place_loc(FIRM_INN, loc_x1, loc_y1, buildXLoc,
    ...
    if (rally_enable_flag && rally_dest_base_obj_recno && base_obj_array.is_deleted(rally_dest_base_obj_recno))
    {
...
void Firm::process_repair()
{
...

    {
        if (nation_recno)
        {
            ...
            float repairCost = (float)firm_res[firm_id]->setup_cost * REPAIR_POINTS_PER_DAY / max_hit_points();

            float repairLiveCost =
                (float)firm_res[firm_id]->setup_live_points_cost * REPAIR_POINTS_PER_DAY / max_hit_points();
...
void Firm::sell_firm(char remoteAction)
{
    ...
    float sellLiveIncome =
        (float)firm_res[firm_id]->setup_live_points_cost / 2 * (int)hit_points / (int)max_hit_points();
...
void Firm::process_construction()
{
    ...
    if (hit_points >= max_hit_points()) // finished construction
    {
        ...
        if (unitPtr && nation_recno == nation_array.player_recno)
            se_res.far_sound(center_x, center_y, 1, 'S', unitPtr->sprite_id, "FINS", 'F', firm_id);
        // ##### end Gilbert 15/10 ######//
        ...
        // ##### begin Gilbert 15/10 ######//
        if (builder_recno && builder_stay_after_construction()) // attempt to assign the unit into
...
int Firm::should_show_info()
{
    ...
    //(~nation_array)->revealed_by_phoenix(loc_x1, loc_y1) ) 		return
    // 1;
...
void Firm::change_nation(int newNationRecno)
{
    ...
    spy_array.change_cloaked_nation(SPY_FIRM, firm_recno, nation_recno,
    ...
    spy_array.set_action_mode(SPY_FIRM, firm_recno,
...
{
    if (destBaseObjRecno && base_obj_array.is_deleted(destBaseObjRecno) || !can_set_rally_point(destBaseObjRecno))
        return;
...
int Firm::send_to_rally_point(int unitRecno)
{
    if (unitRecno && rally_enable_flag)
    {
        ...
        if (rally_dest_base_obj_recno && !base_obj_array.is_deleted(rally_dest_base_obj_recno) &&
