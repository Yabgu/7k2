...
// [char*] buildCode = the build code of the firm, no need to give if the firm
// just have one build type [short] builderRecno = recno of the builder unit
//
void Firm::init(int xLoc, int yLoc, int nationRecno, int firmId,
  ...
  if (!buildCode ||
      !firm_build_id) // some buildings like human offensive structures do not
                      // have different buildings for different nationality, so
                      // firm_build_id will be 0
  ...
  if (!race_id) // some firms like human offensive structures do not have
                // race_id.
  ...
  err_when(
      ...
      (firmId >= FIRM_LAIR &&
       firmId <=
           FIRM_MAGIC)); // error when a human attempts to build a monster firm
                         // or a mosnter attempts to build a human firm
  ...
  abs_x1 = world.zoom_matrix->calc_abs_x(xLoc * LOCATE_WIDTH,
                                         yLoc * LOCATE_HEIGHT, altitude) +
           firmBuild->min_offset_x;
  abs_y1 = world.zoom_matrix->calc_abs_y(xLoc * LOCATE_WIDTH,
                                         yLoc * LOCATE_HEIGHT, altitude) +
           firmBuild->min_offset_y;
  ...
  // even not animation_full_size will have aniPartCount = 1 (set in
  // firmres.cpp)
  ...
  under_construction =
      firmInfo
          ->buildable; // whether the firm is under construction, if the firm is
                       // not buildable it is completed in the first place

  if (!under_construction) // if this firm doesn't been to be constructed, set
                           // its hit points to the maximum
  ...
  init_derived(); // init_derived() before set_world_matrix() so that
                  // init_derived has access to the original land info.
...
void Firm::deinit() {
  ...
  is_being_deleted = 1; // whether the place is currently being deleted, if it
                        // is set to true, some functions will be executed in an
                        // exceptional way, it is set in deinit()
  ...
  if (!sys.signal_exit_flag) {
    ...
    //	char* name2 = "Lishorr";
    //	if( !under_construction && hit_points < 1 && strcmp(name1, firm_name())
    //&& strcmp(name2, firm_name()) )
    if (!under_construction && hit_points < 1) {
      ...
      err_when(!effectId);
      Effect::create(effectId, loc_x1 * LOCATE_WIDTH, loc_y1 * LOCATE_HEIGHT,
                     SPRITE_IDLE, 1, 8, 0);
...
void Firm::init_name() {
  ...
  if (t == ' ' || !t) // if this firm does not have any short name, display the
                      // full name without displaying the town name together
    return;
  ...
  for (i = 0; i < 256;
       i++) // get the smallest id. which are not used by existing firms
  {
...
char *Firm::firm_name() {
  ...
  if (!closest_town_name_id) {
    ...
    str = text_firm.str_firm_name(firm_build_id,
                                  town_res.get_name(closest_town_name_id),
                                  firm_name_instance_id);
  }
  // ##### end Gilbert 4/9 ###### //
...
void Firm::set_world_matrix() {
  //--- if a nation set up a firm in a location that the player has explored,
...
int Firm::get_should_set_power() {
  ...
  if (firm_id == FIRM_MARKET) {
    ...
    //--- only set the shouldSetPower to 1 if the market is linked to a firm of
    //ours ---//
...
void Firm::restore_world_matrix() {
  ...
  if (should_set_power) // no power region for harbor as it build on coast which
                        // cannot be set with power region
...
void Firm::process_animation() {
  ...
  } else if (hit_points < (max_hit_points() * 0.3)) {
    ...
    if (firm_remain_frame_delay[i] < 0) {
      ...
      if (firmBuild->animate_full_size) {
        if (++firm_cur_frame[i] > frameCount)
          firm_cur_frame[i] = 1;
        firm_remain_frame_delay[i] =
            (char)firmBuild->frame_delay(firm_cur_frame[i]);
      } else {
        // not animate_full_size supports animation of looping and
        // random animation and partial animation
        ...
        for (int k = 1; k < frameCount; k++) {
          ...
          for (j = 0; j < bitmapCount; j++) {
            ...
            if (firmBitmap) {
              if (firmBitmap->ani_part == (i + 1)) {
                // unlike the animate_full_size animation, it search starting
                // from the 2nd frame for bitmaps (since now 1 frame may has
                // more than 1 bitmap) with same animation part no. of the
                // currently searching part no., i. This has no relationship
                // with the current_frame. Then restore this animation part's
                // delay and set its current_frame either randomly or
                // sequentially

                firm_remain_frame_delay[i] = firmBitmap->delay;
                if (firmBitmap->random_flag)
                  firm_cur_frame[i] = misc.random(100) % frameCount + 1;
                else if (++firm_cur_frame[i] > frameCount)
                  firm_cur_frame[i] = 1;
...
void Firm::next_day() {
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    err_when(
        j ==
        townPtr->linked_firm_count); // the link is brokwn, only one side has it
  }
  ...
  if (is_ai) {
    if (info.game_date % 30 == firm_recno % 30 ||
        !ai_link_checked) // once 30 days or when the link has been changed.
    {
  ...
  if (no_neighbor_space && info.game_date % 180 == firm_recno % 180) {
    ...
    if (nation_array[nation_recno]->find_best_place_loc(
...
void Firm::process_repair() {
  if (repair_flag && hit_points < max_hit_points() &&
      info.game_date > last_attack_date + 1) // can only do construction when
                                             // the firm is not under attack //

  {
    if (nation_recno) {
      ...
      if (nationPtr->cash < repairCost ||
          nationPtr->live_points < repairLiveCost)
        return;
...
//
int Firm::year_expense() { return firm_res[firm_id]->year_cost; }
...
void Firm::sell_firm(char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_FIRM_SELL, sizeof(short));
    *shortPtr = firm_recno;
    return;
  }
  ...
  float sellIncome = (float)firm_res[firm_id]->setup_cost / 2 *
                     (int)hit_points / (int)max_hit_points();
  float sellLiveIncome = (float)firm_res[firm_id]->setup_live_points_cost / 2 *
                         (int)hit_points / (int)max_hit_points();
...
void Firm::destruct_firm(char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <firm recno>
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_FIRM_DESTRUCT, sizeof(short));
    *shortPtr = firm_recno;
    return;
  }
...
void Firm::process_construction() {
  ...
  if (hit_points >= max_hit_points()) // finished construction
  {
    ...
    // ##### begin Gilbert 15/10 ######//
    if (builder_recno &&
        builder_stay_after_construction()) // attempt to assign the unit into
                                           // the firm. return 1 if assigned
                                           // successfully
                                           // ##### end Gilbert 15/10 ######//
      builder_recno = 0;
    else
...
void Firm::cancel_construction(char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_FIRM_CANCEL, sizeof(short));
  ...
  nationPtr->add_expense(EXPENSE_CONSTRUCTION,
                         (float)-firm_res[firm_id]->setup_cost / 2);

  if (nationPtr->is_monster())
    nationPtr->change_live_points(
        (float)firm_res[firm_id]->setup_live_points_cost / 2);
...
void Firm::start_upgrade(int upgradeFirmId, char remoteAction) {
  // 	err_when( upgrading_firm_id );		// if the upgrade process has
  // already started
  ...
  if (!remoteAction && remote.is_enable()) {
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_FIRM_UPGRADE, 2 * sizeof(short));
...
int Firm::should_show_info() {
  ...
  if (builder_recno) {
    if (unit_array[builder_recno]->true_nation_recno() ==
  ...
  //(~nation_array)->revealed_by_phoenix(loc_x1, loc_y1) ) 		return 1;
  if (nation_array.player_recno &&
      (~nation_array)->revealed_by_phoenix(center_x, center_y))
    return 1;
...
void Firm::change_nation(int newNationRecno) {
  ...
  spy_array.change_cloaked_nation(
      SPY_FIRM, firm_recno, nation_recno,
  ...
  release_link(); // need to update link because firms are only linked to firms
                  // of the same nation
...
//
int Firm::can_assign_capture() { return 0; }
...
void Firm::set_repair_flag(char repairFlag, char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_FIRM_SET_REPAIR,
                                                         2 * sizeof(short));
    *shortPtr = firm_recno;
...
//
void Firm::set_rally_point(int destBaseObjRecno, int x, int y,
                           char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_FIRM_SET_RALLY_DEST, 4 * sizeof(short));
    *shortPtr = firm_recno;
...
void Firm::clear_rally_point(char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <firm recno>
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_FIRM_CLEAR_RALLY_DEST, sizeof(short));
    *shortPtr = firm_recno;
    return;
  }
...
int Firm::send_to_rally_point(int unitRecno) {
  if (unitRecno && rally_enable_flag) {
    ...
    if (rally_dest_base_obj_recno &&
        !base_obj_array.is_deleted(rally_dest_base_obj_recno) &&
        base_obj_array[rally_dest_base_obj_recno]->can_accept_assign(
            unitRecno, nation_recno)) {
