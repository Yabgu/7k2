...
short Sprite::abs_x1,
    Sprite::abs_y1; // the absolute postion, taking in account of sprite offset
...
void Sprite::init(short spriteId, short startXLoc, short startYLoc) {
  ...
  cur_dir =
      misc.random(MAX_SPRITE_DIR_TYPE); // facing any of the eight directions
  cur_frame = 1;
  final_dir = cur_dir;
...
SpriteFrame *Sprite::cur_sprite_frame(int *needMirror) {
  ...
  case SPRITE_SHIP_EXTRA_MOVE:
    ...
    if (guard_count) {
      ...
      return sprite_frame_res[sprite_info->guard_move_array[curDir]
                                  .first_frame_recno +
                              cur_frame - 1];
    } else
      return sprite_frame_res
          [sprite_info->move_array[curDir].first_frame_recno + cur_frame - 1];
  ...
  case SPRITE_ATTACK:
    ...
    if (guard_count) {
      ...
      return sprite_frame_res[guardStopAction->first_frame_recno +
                              MIN(guard_count, guardStopAction->frame_count) -
                              1];
    } else
      return sprite_frame_res[sprite_info->attack_array[cur_attack][curDir]
                                  .first_frame_recno +
                              cur_frame - 1];
  ...
  case SPRITE_WAIT:
    // air unit needs it own stop frames to float on air
    {
      if (guard_count) {
        ...
        SpriteGuardStop *guardStopAction =
            sprite_info->guard_stop_array + curDir;
        return sprite_frame_res[guardStopAction->first_frame_recno +
                                MIN(guard_count, guardStopAction->frame_count) -
                                1];
      } else {
  ...
  case SPRITE_DIE:
    if (sprite_info->die
            .first_frame_recno) // only if this sprite has dying frame
    {
      if (needMirror)
        *needMirror = 0; // no need to mirror at any direction
      return sprite_frame_res[sprite_info->die.first_frame_recno + cur_frame -
                              1];
    }

  default:
    return sprite_frame_res[sprite_info->move_array[curDir].first_frame_recno +
                            cur_frame - 1];
  }
}
...
void Sprite::update_abs_pos(SpriteFrame *spriteFrame) {
  ...
  int tempZ = get_z();
  abs_x1 = ZoomMatrix::calc_abs_x(cur_x, cur_y, tempZ) +
           spriteFrame->offset_x; // absolute position
...
void Sprite::draw(int, int drawFlag) {
  ...
  world.zoom_matrix->put_bitmap_offset(
      ...
      spriteFrame->offset_y, NULL, needMirror,
      (sprite_info->remap_bitmap_flag ? 2 : 1) | (drawFlag << 2));
}
//----------- End of function Sprite::draw -----------//

void Sprite::draw_abs(int colorSchemeId, int scrnX, int scrnY, int clipX1,
...
#define REMAP_KEY_COUNT 8
    memcpy(sprite_info->palw_ptr + FIRST_REMAP_KEY,
           colorRemapTable + FIRST_REMAP_KEY,
...
int Sprite::get_abs_rect(int scrnX, int scrnY, int clipX1, int clipY1,
                         int clipX2, int clipY2, int *rectX1, int *rectY1,
...
int Sprite::need_mirror(UCHAR dispDir) {
  return (dispDir < 8 || sprite_info->turn_resolution <= 8)
             ? (dispDir & 7) >= 5
             : (dispDir & 7) >= 4;
}
...
int Sprite::is_stealth() {
  ...
  if (config.fog_of_war) {
    ...
    return world.get_loc(cur_x_loc(), cur_y_loc())->visibility() <=
           EXPLORED_VISIBILITY;
