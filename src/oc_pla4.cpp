...
void CampaignEastWest::plot_a4_create_game() {
  plot_nation_recno1 =
      nation_array.new_nation(NATION_MONSTER, NATION_AI, -MONSTER_MINOTAUROS,
  ...
  for (int i = 1; i <= site_array.size(); ++i) {
    ...
    if (world.can_build_firm(sitePtr->map_x_loc, sitePtr->map_y_loc,
                             FIRM_ALCHEMY)) {
      int firmRecno = firm_array.generate_firm(
          sitePtr->map_x_loc, sitePtr->map_y_loc, plot_nation_recno1,
          FIRM_ALCHEMY, monster_res[MONSTER_MINOTAUROS]->firm_build_code);
      ...
      if (firmRecno && (minePtr = firm_array[firmRecno]) &&
          minotaurNation->find_best_place_loc(FIRM_LAIR, minePtr->loc_x1,
                                              minePtr->loc_y1, buildXLoc,
                                              buildYLoc)) {
        create_monster_lair(buildXLoc, buildYLoc, plot_nation_recno1,
                            MAX_SOLDIER - misc.random(MAX_SOLDIER / 2),
                            20 + campaign_difficulty * 10);
      }
    }
  }
}
