...
void News::town_rebel() {
  ...
  */
  str = text_news.process(text_news.get_format_str(11, "TRBL"),
                          town_res.get_name(short_para1), short_para2,
                          nation_name1());
}
...
void News::migrate() {
  ...
  */
  ...
  if (nation_array.player_recno && nation_recno1 == nation_array.player_recno) {
    String toTownName;
    if (nation_recno2)
      toTownName = text_news.str_town_of_nation(town_res.get_name(short_para2),
                                                nation_name2());
    else
      toTownName = town_res.get_name(short_para2);

    str = text_news.process(text_news.get_format_str(12, "MIGE"),
                            town_res.get_name(short_para1), (char *)toTownName,
                            race_res[short_para3]->name, short_para4,
                            firm_res[short_para5]->worker_title);
  } else {
    String fromTownName;
    if (nation_recno1)
      fromTownName = text_news.str_town_of_nation(
          town_res.get_name(short_para1), nation_name1());
    else
      fromTownName = town_res.get_name(short_para1);
    str = text_news.process(
        text_news.get_format_str(13, "MIGI"), (char *)fromTownName,
        town_res.get_name(short_para2), race_res[short_para3]->name,
        short_para4, firm_res[short_para5]->worker_title);
  }
}
...
void News::nation_surrender() {
  ...
  if (nation_array.player_recno && nation_recno2 == nation_array.player_recno)
    str =
        text_news.process(text_news.get_format_str(16, "SURY"), nation_name1());
  else
...
void News::king_die() {
  ...
  if (nation_array.player_recno && nation_recno1 == nation_array.player_recno) {
    ...
    str = text_news.process(text_news.get_format_str(18, "YDIE"),
                            text_unit.str_title(nation_race_id1, RANK_KING),
                            king_name1());
  } else {
    //		if( nation_race_id1 >= 0 )
    //			str  = "King ";
  ...
  */
  ...
  if (short_para2 > 0) // it is possible that there is no town in the map
    firmPhrase = text_news.str_firm_near_town(
        firm_res.get_build(short_para1)->name, town_res.get_name(short_para2));
  else
...
void News::firm_idle() {
  ...
  //	str  = "Your ";
  //	str += firm_res.get_build(short_para1)->name;
  //	if( short_para2 > 0 )			// it is possible that there is no town
  //in the map
  //	{
  //		str += " near ";
  //		str += town_res.get_name(short_para2);
...
void News::your_spy_killed() {
  ...
  */
  ...
  if (short_para3 == SPY_FIRM) {
    ...
    if (short_para2 > 0 && nation_recno2) {
      firmPhrase = text_news.str_firm_near_town_of_nation(
          ...
          nation_name2());
               ...
               0) // it is possible that there is no town in the map
    {
      firmPhrase =
          text_news.str_firm_near_town(firm_res.get_build(short_para1)->name,
                                       town_res.get_name(short_para2));
    } else if (nation_recno2) // not for independent town.
    {
      firmPhrase = text_news.str_firm_of_nation(
          firm_res.get_build(short_para1)->name, nation_name2());
    } else {
      firmPhrase = firm_res.get_build(short_para1)->name;
    }
    ...
    String townPhrase;
    if (nation_recno2) // not for independent town.
      townPhrase = text_news.str_town_of_nation(town_res.get_name(short_para2),
                                                nation_name2());
    else
  ...
  */
  if (nation_recno1 == 0) // independent unit joining your force
  {
    ...
    if (short_para3 != RANK_SOLDIER) {
      unitPhrase = text_unit.str_unit_titled(
          short_para1, get_unit_name(short_para1, short_para2), short_para3);
    } else {
    ...
    else if (short_para1 > 0 &&
             (short_para4 == UNIT_WAGON // human, wagon or civilian
...
void News::general_form_new_nation() {
  ...
  */
  ...
  if (nation_array.player_recno && nation_recno1 == nation_array.player_recno) {
    ...
    String unitPhrase;
    unitPhrase = text_unit.str_unit_titled(
        short_para1, get_unit_single_name(short_para1, short_para2),
        RANK_GENERAL);
...
void News::assassinator_caught() {
  ...
  //"; 	if( short_para1 == RANK_KING ) 		str += "King."; 	else 		str += "General.";
  ...
  */
  ...
  case NEWS_LOC_UNIT: {
    ...
    if (short_para4 == RANK_KING) // King/All High, no name
      ...
      unitPhrase = text_unit.str_unit_titled(
          short_para2, get_unit_name(short_para2, short_para3), short_para4);
    else
      unitPhrase = get_unit_name(short_para2, short_para3);

    if (!short_para5) { // injured
  ...
  case NEWS_LOC_FIRM: {
    ...
    if (short_para3)
      firmPhrase =
          text_news.str_firm_near_town(firm_res.get_build(short_para2)->name,
...
void News::earthquake_damage() {
  ...
  */
  ...
  if (short_para1 == 1) {
    ...
    if (short_para3 == 0)
      str = text_news.process(text_news.get_format_str(80, "EQUI"), short_para2,
                              playerNationTitle);
    else
      str = text_news.process(text_news.get_format_str(81, "EQUK"), short_para2,
                              playerNationTitle, short_para3);
  } else if (short_para1 == 2) {
    str = text_news.process(text_news.get_format_str(82, "EQTK"), short_para2);
  } else if (short_para1 == 3) {
    if (short_para3 == 0)
      str =
          text_news.process(text_news.get_format_str(83, "EQFI"), short_para2);
    else
      str = text_news.process(text_news.get_format_str(84, "EQFK"), short_para2,
                              short_para3);
  } else {
    err_here();
    str = "";
...
void News::event_soldier_die() {
  ...
  */
  if (short_para4 >= 0) // race_id
  {
    str = text_news.process(text_news.get_format_str(94, "EVHP"), short_para3,
                            firm_res.get_build(short_para1)->name);
  } else {
    str = text_news.process(text_news.get_format_str(95, "EVFP"), short_para3,
                            firm_res.get_build(short_para1)->name);
  }
}
...
void News::event_town_people_killed() {
  ...
  */
  ...
  str = text_news.process(text_news.get_format_str(97, "EVTP"), short_para2,
                          town_res.get_name(short_para1));
}
...
void News::multi_retire() {
  ...
  str =
      text_news.process(text_news.get_format_str(133, "MP_R"), nation_name1());
}
...
void News::multi_quit_game() {
  ...
  str =
      text_news.process(text_news.get_format_str(134, "MP_Q"), nation_name1());
}
...
void News::multi_connection_lost() {
  ...
  str =
      text_news.process(text_news.get_format_str(136, "MP_L"), nation_name1());
}
