...
void SpriteFrameRes::load_info() {
  ...
  sprite_frame_array =
      (SpriteFrame *)mem_add(sizeof(SpriteFrame) * sprite_frame_count);
  ...
  for (i = 0; i < dbSpriteFrame->rec_count(); i++) {
    ...
    if (i == 0 || strcmp(spriteName, lastSpriteName)) {
      int j;
      for (j = 1; j <= sprite_res.sprite_info_count &&
                  strcmp(spriteName, sprite_res[j]->sprite_code);
           j++)
        ;
    ...
    memcpy(&spriteFrame->bitmap_offset, frameRec->bitmap_offset,
           sizeof(uint32_t));

    // modify offset_x/y for 7k2, assume loc_width = loc_height = 1
    spriteFrame->offset_x += -spriteInfo->loc_width * LOCATE_WIDTH / 2 -
                             (-spriteInfo->loc_width * ZOOM_LOC_X_WIDTH / 2 +
                              -spriteInfo->loc_height * ZOOM_LOC_Y_WIDTH / 2);
    spriteFrame->offset_y += -spriteInfo->loc_height * LOCATE_HEIGHT / 2 -
                             (-spriteInfo->loc_width * ZOOM_LOC_X_HEIGHT / 2 +
                              -spriteInfo->loc_height * ZOOM_LOC_Y_HEIGHT / 2);

    //	spriteFrame->offset_x += -LOCATE_WIDTH/2 - (-ZOOM_LOC_X_WIDTH/2 +
    //-ZOOM_LOC_Y_WIDTH/2); 	spriteFrame->offset_y += -LOCATE_HEIGHT/2 -
    //(-ZOOM_LOC_X_HEIGHT/2 + -ZOOM_LOC_Y_HEIGHT/2);
