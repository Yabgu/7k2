...
void SpriteFrameRes::load_info()
{
    ...
    for (i = 0; i < dbSpriteFrame->rec_count(); i++)
    {
        ...
        {
            int j;
            for (j = 1; j <= sprite_res.sprite_info_count && strcmp(spriteName, sprite_res[j]->sprite_code); j++)
                ;
        ...
        spriteFrame->offset_y +=
            -spriteInfo->loc_height * LOCATE_HEIGHT / 2 -
            (-spriteInfo->loc_width * ZOOM_LOC_X_HEIGHT / 2 + -spriteInfo->loc_height * ZOOM_LOC_Y_HEIGHT / 2);
