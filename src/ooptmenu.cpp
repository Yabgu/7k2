...
enum
{
    BASIC_OPTION_X_SPACE = 78,
    BASIC_OPTION_HEIGHT = 32
};

enum
{
    COLOR_OPTION_X_SPACE = 35,
    COLOR_OPTION_HEIGHT = 32
};

enum
{
    SERVICE_OPTION_X_SPACE = 180,
    SERVICE_OPTION_HEIGHT = 139
};

enum
{
    SLIDE_BUTTON_WIDTH = 16,
...
OptionMenu::OptionMenu()
    : music_group(2), help_group(3), news_group(2), report_group(2), show_icon_group(2), show_path_group(4),
      inside_info_group(3), scroll_method_group(2)
{
    active_flag = 0;
...
void OptionMenu::enter(char untilExitFlag)
{
    ...
    se_vol_slide.init_slide(bx + 140, by + 118, bx + 140 + 10 * SLIDE_BUTTON_WIDTH + 9 * 2,
    ...
    for (i = 0; i < 2; ++i)
    {
        ...
        music_group[i].create(cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
    ...
    //		SLIDE_BUTTON_WIDTH, disp_slide_bar);
    frame_speed_slide.init_slide(bx + 140, by + 338, bx + 140 + 10 * SLIDE_BUTTON_WIDTH + 9 * 2,
    ...
    //		SLIDE_BUTTON_WIDTH, disp_slide_bar);
    scroll_speed_slide.init_slide(bx + 140, by + 398, bx + 140 + 10 * SLIDE_BUTTON_WIDTH + 9 * 2,
    ...
    // 162+BASIC_OPTION_HEIGHT-1, 			disp_virtual_button,
    // ButtonCustomPara(NULL, race_table[i]));
    //	}
    ...
    for (i = 0; i < 3; ++i)
    {
        ...
        help_group[i].create(cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
    ...
    for (i = 0; i < 2; ++i)
    {
        ...
        news_group[i].create(cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
    ...
    for (i = 0; i < 2; ++i)
    {
        ...
        report_group[i].create(cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
    ...
    for (i = 0; i < 2; ++i)
    {
        ...
        show_icon_group[i].create(cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
    ...
    for (i = 0; i < 4; ++i)
    {
        ...
        show_path_group[i].create(cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
    ...
    for (i = 0; i < 3; ++i)
    {
        ...
        inside_info_group[i].create(cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
    ...
    for (i = 0; i < 2; ++i)
    {
        ...
        scroll_method_group[i].create(cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
                                      ButtonCustomPara(text_game_menu.str_scroll_method_options(i), i), 0, 0);
        cx = cx2 + 5;
    }

    // --------- other buttons --------//
    start_button.create_text(bx + BUTTON2_X1, by + BUTTON2_Y1, bx + BUTTON2_X2, by + BUTTON2_Y2,
                             text_game_menu.str_return());
    cancel_button.create_text(bx + BUTTON4_X1, by + BUTTON4_Y1, bx + BUTTON4_X2, by + BUTTON4_Y2,
...
void OptionMenu::disp(int needRepaint)
{
    ...
    if (refresh_flag)
    {
        if (refresh_flag & IGOPTION_PAGE)
        {
            ...
            font_bold_black.put(bx + 124, by + 98,
...
static void disp_slide_bar(SlideBar *slideBar, int)
{
    ...
    if (w > 0)
        vga.active_buf->put_bitmap_area_trans_decompress(slideBar->scrn_x1, slideBar->scrn_y1, bitmap0, 0, 0, w - 1,
                                                         hBase - 1);
    if (w < wBase)
        vga.active_buf->put_bitmap_area_trans_decompress(slideBar->scrn_x1, slideBar->scrn_y1, bitmap1, w, 0, wBase - 1,
                                                         hBase - 1);
    mouse.show_area();
}
