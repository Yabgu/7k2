...
enum { BASIC_OPTION_X_SPACE = 78, BASIC_OPTION_HEIGHT = 32 };

enum { COLOR_OPTION_X_SPACE = 35, COLOR_OPTION_HEIGHT = 32 };

enum { SERVICE_OPTION_X_SPACE = 180, SERVICE_OPTION_HEIGHT = 139 };

enum { SLIDE_BUTTON_WIDTH = 16, SLIDE_BUTTON_HEIGHT = 16 };
...
// static char reverse_race_table[MAX_RACE] =		// race translation
// table
//{
//	5, 3, 1, 6, 4, 0, 2
//};
...
OptionMenu::OptionMenu()
    : music_group(2), help_group(3), news_group(2), report_group(2),
...
void OptionMenu::set_active() {
  ...
  if (!color_remap_table) {
    ...
    memcpy(color_remap_table, colorTable.get_table(0),
           sizeof(*color_remap_table) * 0x100);
  }
...
void OptionMenu::enter(char untilExitFlag) {
  ...
  se_vol_slide.init_slide(
      bx + 140, by + 118, bx + 140 + 10 * SLIDE_BUTTON_WIDTH + 9 * 2,
      by + 118 + SLIDE_BUTTON_HEIGHT - 1, SLIDE_BUTTON_WIDTH, disp_slide_bar);
  se_vol_slide.set(0, 100,
                   tempConfig.sound_effect_flag ? tempConfig.sound_effect_volume
                                                : 0);

  // -------- initialize music volume --------//
  //	music_vol_slide.init_slide(566, 123, 722, 123+SLIDE_BUTTON_HEIGHT-1,
  //		SLIDE_BUTTON_WIDTH, disp_slide_bar);
  //	music_vol_slide.init_slide(bx+140, by+173,
  //bx+140+10*SLIDE_BUTTON_WIDTH+9*2, by+173+SLIDE_BUTTON_HEIGHT-1,
  //		SLIDE_BUTTON_WIDTH, disp_slide_bar);
  //	music_vol_slide.set(0, 100, tempConfig.music_flag ?
  //tempConfig.wav_music_volume : 0);

  int cx = bx + 124;
  int cy = by + 173;
  for (i = 0; i < 2; ++i) {
    int cx2 = cx + font_thin_black.text_width(text_game_menu.str_no_yes(i)) + 9;
    music_group[i].create(
        cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
        ButtonCustomPara(text_game_menu.str_no_yes(i), i), 0, 0);
    cx = cx2 + 5;
  }
  ...
  //		SLIDE_BUTTON_WIDTH, disp_slide_bar);
  frame_speed_slide.init_slide(
      bx + 140, by + 338, bx + 140 + 10 * SLIDE_BUTTON_WIDTH + 9 * 2,
      by + 338 + SLIDE_BUTTON_HEIGHT - 1, SLIDE_BUTTON_WIDTH, disp_slide_bar);
  frame_speed_slide.set(
      0, 31, tempConfig.frame_speed <= 30 ? tempConfig.frame_speed : 31);
  ...
  //		SLIDE_BUTTON_WIDTH, disp_slide_bar);
  scroll_speed_slide.init_slide(
      bx + 140, by + 398, bx + 140 + 10 * SLIDE_BUTTON_WIDTH + 9 * 2,
      by + 398 + SLIDE_BUTTON_HEIGHT - 1, SLIDE_BUTTON_WIDTH, disp_slide_bar);
  ...
  //			181+(i+1)*BASIC_OPTION_X_SPACE-1,
  //162+BASIC_OPTION_HEIGHT-1, 			disp_virtual_button, ButtonCustomPara(NULL,
  //race_table[i]));
  //	}
  ...
  for (i = 0; i < 3; ++i) {
    int cx2 =
        cx + font_thin_black.text_width(text_game_menu.str_help_options(i)) + 9;
    help_group[i].create(
        cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
        ButtonCustomPara(text_game_menu.str_help_options(i), i), 0, 0);
    cx = cx2 + 5;
  }
  ...
  for (i = 0; i < 2; ++i) {
    int cx2 =
        cx + font_thin_black.text_width(text_game_menu.str_news_options(i)) + 9;
    news_group[i].create(
        cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
        ButtonCustomPara(text_game_menu.str_news_options(i), i), 0, 0);
    cx = cx2 + 5;
  }
  ...
  for (i = 0; i < 2; ++i) {
    int cx2 = cx +
              font_thin_black.text_width(text_game_menu.str_report_options(i)) +
              9;
    report_group[i].create(
        cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
        ButtonCustomPara(text_game_menu.str_report_options(i), i), 0, 0);
    cx = cx2 + 5;
  }
  ...
  for (i = 0; i < 2; ++i) {
    int cx2 =
        cx +
        font_thin_black.text_width(text_game_menu.str_show_icon_options(i)) + 9;
    show_icon_group[i].create(
        cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
        ButtonCustomPara(text_game_menu.str_show_icon_options(i), i), 0, 0);
    cx = cx2 + 10;
  }
  ...
  for (i = 0; i < 4; ++i) {
    int cx2 =
        cx +
        font_thin_black.text_width(text_game_menu.str_show_path_options(i)) + 9;
    show_path_group[i].create(
        cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
        ButtonCustomPara(text_game_menu.str_show_path_options(i), i), 0, 0);
    if (i & 1) {
      cx = bx + 417;
  ...
  for (i = 0; i < 3; ++i) {
    int cx2 =
        cx +
        font_thin_black.text_width(text_game_menu.str_inside_info_options(i)) +
        9;
    inside_info_group[i].create(
        cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
        ButtonCustomPara(text_game_menu.str_inside_info_options(i), i), 0, 0);
    cx = cx2 + 5;
  }
  ...
  for (i = 0; i < 2; ++i) {
    int cx2 = cx +
              font_thin_black.text_width(
                  text_game_menu.str_scroll_method_options(i)) +
              9;
    scroll_method_group[i].create(
        cx, cy, cx2, cy + font_thin_black.max_font_height + 3, disp_text_button,
        ButtonCustomPara(text_game_menu.str_scroll_method_options(i), i), 0, 0);
    cx = cx2 + 5;
  }

  // --------- other buttons --------//
  start_button.create_text(bx + BUTTON2_X1, by + BUTTON2_Y1, bx + BUTTON2_X2,
                           by + BUTTON2_Y2, text_game_menu.str_return());
  cancel_button.create_text(bx + BUTTON4_X1, by + BUTTON4_Y1, bx + BUTTON4_X2,
  ...
  if (untilExitFlag) {
    while (is_active()) {
      ...
      if (config.music_flag) {
        if (!music.is_playing())
          music.play(music.random_bgm_track(music.song_id),
...
void OptionMenu::disp(int needRepaint) {
  ...
  err_when(!color_remap_table ||
           !background_bitmap); // call set_active to load palette and bitmap
  ...
  if (refresh_flag) {
    if (refresh_flag & IGOPTION_PAGE) {
      ...
      vga_back.put_bitmap_trans_remap(bx, by, background_bitmap,
                                      color_remap_table);
      ...
      font_bold_black.put(
          bx + 124, by + 98,
          text_game_menu.str_se_vol()); // "Sound Effects Volume" );
      //			font_bold_black.put( bx+124, by+153, "Music
      //Volume" );
      font_bold_black.put(bx + 124, by + 153,
                          text_game_menu.str_music()); // "Music" );
      font_bold_black.put(bx + 124, by + 208,
                          text_game_menu.str_help()); // "Help" );
      font_bold_black.put(
          bx + 124, by + 263,
          text_game_menu.str_display_news()); // "Display News" );
      font_bold_black.put(bx + 124, by + 318,
                          text_game_menu.str_game_speed()); //"Game Speed" );
      font_bold_black.put(
          bx + 124, by + 378,
          text_game_menu.str_scroll_speed()); //"Scrolling Speed" );

      font_bold_black.put(
          bx + 417, by + 98,
          text_game_menu.str_report_bg()); // "Reports Background" );
      font_bold_black.put(
          bx + 417, by + 163,
          text_game_menu.str_show_unit_id()); // "Show Unit Identities" );
      font_bold_black.put(
          bx + 417, by + 238,
          text_game_menu.str_show_unit_path()); //"Show Unit Paths" );
      font_bold_black.put(
          bx + 417, by + 313,
          text_game_menu.str_inside_info()); //"Inside Information" );
      font_bold_black.put(
          bx + 417, by + 373,
          text_game_menu.str_scroll_method()); //"Scrolling Method" );
    ...
    if (refresh_flag & IGOPTION_SE_VOL) {
      se_vol_slide.paint(
          tempConfig.sound_effect_flag ? (tempConfig.sound_effect_volume) : 0);
    }
    if (refresh_flag & IGOPTION_MUSIC_VOL) {
      // music_vol_slide.paint(tempConfig.music_flag ?
      // percent_to_slide_volume(tempConfig.wav_music_volume) : 0);
    ...
    if (refresh_flag & IGOPTION_GAME_SPEED) {
      frame_speed_slide.paint(
          tempConfig.frame_speed <= 30 ? tempConfig.frame_speed : 31);
    }
...
int OptionMenu::detect() {
  ...
  if (se_vol_slide.detect() == 1) {
    ...
    if (se_vol_slide.view_recno > 0)
      tempConfig.sound_effect_volume =
          slide_to_percent_volume(se_vol_slide.view_recno);
    else
  ...
  //		tempConfig.wav_music_volume =
  //slide_to_percent_volume(music_vol_slide.view_recno); 		if(
  //tempConfig.music_flag )
  //		{
  //			music.change_volume( tempConfig.wav_music_volume );
  //		}
  //	}
    ...
    tempConfig.frame_speed =
        frame_speed_slide.view_recno <= 30 ? frame_speed_slide.view_recno : 99;
  ...
  //			if( config.music_flag )
  //			{
  //				music.play( race_button[i].custom_para.value +
  //1, 					sys.cdrom_drive ? MUSIC_CD_THEN_WAV : 0 );
  //			}
  //			else
  //			{
  ...
  if (help_group.detect() >= 0) {
    ...
    tempConfig.show_all_unit_icon =
        show_icon_group[show_icon_group()].custom_para.value;
    // refresh_flag |= IGOPTION_SHOW_ICON;
    update_flag |= IGOPTION_SHOW_ICON;
  } else if (show_path_group.detect() >= 0) {
    tempConfig.show_unit_path =
        show_path_group[show_path_group()].custom_para.value;
    // refresh_flag |= IGOPTION_DRAW_PATH;
    update_flag |= IGOPTION_DRAW_PATH;
  } else if (inside_info_group.detect() >= 0) {
    tempConfig.disp_extend_info =
        inside_info_group[inside_info_group()].custom_para.value;
    update_flag |= IGOPTION_INSIDE_INFO;
  } else if (scroll_method_group.detect() >= 0) {
    tempConfig.scroll_method =
        scroll_method_group[scroll_method_group()].custom_para.value;
...
static void disp_text_button(ButtonCustom *button, int) {
  ...
  fontPtr->put(button->x1, button->y1, (char *)button->custom_para.ptr, 1,
               button->x2);
  //	fontPtr->space_width += 8;
}
...
static void disp_slide_bar(SlideBar *slideBar, int) {
  ...
  err_when(((Bitmap *)bitmap0)->get_width() !=
           ((Bitmap *)bitmap1)->get_width());
  err_when(((Bitmap *)bitmap0)->get_height() !=
           ((Bitmap *)bitmap1)->get_height());
  int wBase = ((Bitmap *)bitmap0)->get_width();
  int hBase = ((Bitmap *)bitmap0)->get_height();
  int w = wBase * (slideBar->view_recno - slideBar->min_recno) /
          (slideBar->max_recno - slideBar->min_recno);
  if (w < 0)
    w = 0;
  if (w > wBase)
    w = wBase;

  // not using rect_left() and rect_right() because bar_width is not appropriate
  // for this kind of slidebar

  mouse.hide_area(slideBar->scrn_x1, slideBar->scrn_y1, slideBar->scrn_x2,
                  slideBar->scrn_y2);
  if (w > 0)
    vga.active_buf->put_bitmap_area_trans_decompress(
        slideBar->scrn_x1, slideBar->scrn_y1, bitmap0, 0, 0, w - 1, hBase - 1);
  if (w < wBase)
    vga.active_buf->put_bitmap_area_trans_decompress(
        slideBar->scrn_x1, slideBar->scrn_y1, bitmap1, w, 0, wBase - 1,
        hBase - 1);
  mouse.show_area();
}
