...
void Database::open(const char *fileName, int bufferAll, int importRecSpec) {
  close(); // if there is a opened file attached to current database, close it
           // first
  ...
  if (importRecSpec) {
    ...
    for (dbf_field_count = 0;
         recordSpecSize > 0 && dbf_record_spec[dbf_field_count].field_name[0] !=
                                   RECORD_SPEC_TERM_CHAR;
...
void Database::open_from_buf(char *dataPtr, int importRecSpec) {
  close(); // if there is a open_from_bufed file attached to current database,
           // close it first
  ...
  dbf_buf_allocated = 0; // we didn't allocate the buffer, so don't bother to
                         // free it in deinit()
...
char *Database::read(long recNo) {
  ...
  {
    if (recNo == last_read_recno) // the record to be read is the same as one in
                                  // buffer, simply return it
      return rec_buf;
...
int Database::write_file(char *, char *fileName) {
  ...
  dbf_header.data_offset =
      sizeof(DbfHeader) + sizeof(DbfRec) * dbf_field_count + sizeof(termChar);
