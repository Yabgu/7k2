...
void Nation::init(int nationType, int raceId, int colorSchemeId,
                  DWORD playerId) {
  ...
  if (is_ai())
    set_auto_grant_loyalty(REBEL_LOYALTY +
                           1); // prevent their people from rebelling
}
...
void Nation::init_all_ai_info() {
  ...
  init_ai_info(&ai_factory_array, ai_factory_count, ai_factory_size,
               AI_FACTORY_INIT_SIZE);
  init_ai_info(&ai_market_array, ai_market_count, ai_market_size,
               AI_MARKET_INIT_SIZE);
  init_ai_info(&ai_research_array, ai_research_count, ai_research_size,
               AI_RESEARCH_INIT_SIZE);
  ...
  init_ai_info(&ai_special_array, ai_special_count, ai_special_size,
               AI_SPECIAL_INIT_SIZE);
  // ####### begin Gilbert 4/5 ########//
  init_ai_info(&ai_offend_array, ai_offend_count, ai_offend_size,
               AI_OFFEND_INIT_SIZE);
  // ####### end Gilbert 4/5 ##########//
  // ####### begin Gilbert 10/3 #######//
  init_ai_info(&ai_lishorr_array, ai_lishorr_count, ai_lishorr_size,
               AI_LISHORR_INIT_SIZE);
  // ####### end Gilbert 10/3 #######//
  init_ai_info(&ai_mons_train_array, ai_mons_train_count, ai_mons_train_size,
               AI_MONS_TRAIN_INIT_SIZE);
  init_ai_info(&ai_mons_alchemy_array, ai_mons_alchemy_count,
               ai_mons_alchemy_size, AI_MONS_ALCHEMY_INIT_SIZE);
  init_ai_info(&ai_mons_fort_array, ai_mons_fort_count, ai_mons_fort_size,
               AI_MONS_FORT_INIT_SIZE);
  init_ai_info(&ai_mons_animal_array, ai_mons_animal_count, ai_mons_animal_size,
               AI_MONS_ANIMAL_INIT_SIZE);
  init_ai_info(&ai_incubator_array, ai_incubator_count, ai_incubator_size,
               AI_INCUBATOR_INIT_SIZE);
  init_ai_info(&ai_mons_magic_array, ai_mons_magic_count, ai_mons_magic_size,
               AI_MONS_MAGIC_INIT_SIZE);
  init_ai_info(&ai_monster_array, ai_monster_count, ai_monster_size,
               AI_MONSTER_INIT_SIZE);

  init_ai_info(&ai_caravan_array, ai_caravan_count, ai_caravan_size,
               AI_CARAVAN_INIT_SIZE);
  init_ai_info(&ai_ship_array, ai_ship_count, ai_ship_size, AI_SHIP_INIT_SIZE);
  init_ai_info(&ai_general_array, ai_general_count, ai_general_size,
               AI_GENERAL_INIT_SIZE);
}
...
//
void Nation::init_ai_info(short **aiInfoArrayPtr, short &aiInfoCount,
                          short &aiInfoSize, int arrayInitSize) {
...
void Nation::init_personalty() {
  ...
  pref_spy =
      MIN(60, pref_spy); // if the pref is too high, the AI will use it like
                         // crazy and will introduce weird strategies
...
int Nation::think_succeed_king() {
  ...
  if (bestUnitPtr) {
    ...
    if (bestUnitPtr->is_visible()) // it may still be not visible if there is no
                                   // space for the unit to be mobilized
    {
      if (bestUnitPtr->spy_recno &&
          bestUnitPtr->true_nation_recno() ==
              nation_recno) // if this is a spy and he's our spy
        spy_array[bestUnitPtr->spy_recno]
            ->drop_spy_identity(); // revert the spy to a normal unit
...
void Nation::ai_improve_relation() {
  ...
  for (int i = nation_array.size(); i > 0; i--) {
    ...
    //--- decrease the started_war_on_us_count once per year, gradually
    //forgiving other nations' wrong doing ---//
...
void Nation::think_new_research() {
  ...
  if (tech_res[TECH_FARMING]->can_research(nation_recno)) {
    if (!ai_has_enough_food() || misc.random(100 - pref_food_reserve) < 10) {
  ...
  if (!bestTechId && !bestTechClassId) {
    if (pref_use_weapon > 50) {
      int totalWeaponTech = tech_res.tech_class(TECH_CLASS_WEAPON)
                                ->total_tech_count(nation_recno);
  ...
  if (!bestTechId && !bestTechClassId) {
    if (pref_spy > 50) {
      int totalSpyTech = tech_res.tech_class(TECH_CLASS_SPYING)
                             ->total_tech_count(nation_recno);
  ...
  if (!bestTechId && !bestTechClassId) {
    //--- pick the race with most jobless population and we don't have
    //researched the special training structure for it yet ---//

    for (i = 0; i < MAX_RACE; i++) {
      //-- if we haven't research the special unit training structure for this
      //race yet --//
  ...
  if (!bestTechId) {
    ...
    for (int i = techClass->tech_count; i > 0; i--) {
      ...
      if (techInfo->can_research(nation_recno) &&
          (!techInfo->race_id ||
           racePopArray[techInfo->race_id - 1] >
               0)) // if this tech is race specific, only research it when we
                   // have the people of that race
      {
        bestTechId = techId;
        break;
      }
