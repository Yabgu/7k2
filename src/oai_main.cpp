...
void Nation::init_personalty()
{
    ...
    pref_spy = MIN(60, pref_spy); // if the pref is too high, the AI will use it like
...
void Nation::process_ai()
{
    ...
    if (!remote.is_enable() && game.game_mode != GAME_TUTORIAL && config.ai_aggressiveness == OPTION_NONE)
    {
        if (cash > 1000)
    ...
    if (bestUnitPtr)
    {
                                       ...
                                       // space for the unit to be mobilized
        {
            if (bestUnitPtr->spy_recno &&
                bestUnitPtr->true_nation_recno() == nation_recno)       // if this is a spy and he's our spy
    ...
    if (!bestTechId)
    {
        ...
        for (int i = techClass->tech_count; i > 0; i--)
        {
            ...
            if (techInfo->can_research(nation_recno) &&
                (!techInfo->race_id ||
                 racePopArray[techInfo->race_id - 1] > 0)) // if this tech is race specific, only research it when
                                                           // we have the people of that race
            {
                bestTechId = techId;
                break;
            }
