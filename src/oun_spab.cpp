...
int Unit::think_use_special_ability() {
  ...
  if (is_human()) {
    ...
    else if (skill.max_combat_level >
             MAX_COMBAT_BATTLE) // potential hero can trigger special power
      spLevel = 1;
    else
      return 0;

    if (info.game_date - last_special_ability_start_date <
        SPECIAL_ABILITY_DELAY)
      return 0;
...
int Unit::is_special_ability_effective() {
  if (special_abilty_id && info.game_date - last_special_ability_start_date <
                               SPECIAL_ABILITY_DURATION)
