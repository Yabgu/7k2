...
void World::gen_rocks(int nGrouped, int nLarge, int nSmall)
{
    ...
    while ((nGrouped > 0 || nLarge > 0 || nSmall > 0) && --trial > 0)
    {
        ...
        {
            short x =
                (GAP + LARGE_ROCK_SIZE) + misc.random(max_x_loc - HUGE_ROCK_SIZE + 1 - 2 * (GAP + LARGE_ROCK_SIZE));
            short y =
                (GAP + LARGE_ROCK_SIZE) + misc.random(max_y_loc - HUGE_ROCK_SIZE + 1 - 2 * (GAP + LARGE_ROCK_SIZE));
            ...
            if (can_add_rock(x, y, x2, y2))
            {
                ...
                //		if (misc.random(10) >5)
                rockRecno =
                    rock_res.search("R", LARGE_ROCK_SIZE + 1, HUGE_ROCK_SIZE, LARGE_ROCK_SIZE + 1, HUGE_ROCK_SIZE, -1,
                                    0, terrain_res[get_loc(x, y)->terrain_id]->average_type);
                //		else
        ...
        if (nLarge > 0)
        {
            ...
            if (can_add_rock(x, y, x2, y2))
            {
                ...
                //	if (misc.random(10) >5)
                rockRecno =
                    rock_res.search("R", SMALL_ROCK_SIZE + 1, LARGE_ROCK_SIZE, SMALL_ROCK_SIZE + 1, LARGE_ROCK_SIZE, -1,
                                    0, terrain_res[get_loc(x, y)->terrain_id]->average_type);
                //	else
...
void World::add_dirt(short dirtRecno, short x1, short y1)
{
    ...
    for (short dy = 0; dy < dirtInfo->loc_height && y1 + dy < max_y_loc; ++dy)
    {
        for (short dx = 0; dx < dirtInfo->loc_width && x1 + dx < max_x_loc; ++dx)
        {
            ...
            if ((dirtInfo->rock_type == DIRT_BLOCKING_TYPE) || (dirtInfo->rock_type == DIRT_LAKE_TYPE))
                locPtr->walkable_off();
        }
    }
}
    ...
    while ((nGrouped > 0 || nLarge > 0 || nSmall > 0) && --trial > 0)
    {
        ...
        if (nGrouped > 0)
        {
            ...
            if (can_add_rock(x, y, x2, y2) && can_add_dirt(x, y, x2, y2))
            {
                short rockRecno;
                rockRecno =
                    rock_res.search("DEP", LARGE_ROCK_SIZE + 1, HUGE_ROCK_SIZE, LARGE_ROCK_SIZE + 1, HUGE_ROCK_SIZE, -1,
                                    0, terrain_res[get_loc(x, y)->terrain_id]->average_type);
                if (!rockRecno)
                    continue;
        ...
        if (nLarge > 0)
        {
            ...
            if (can_add_rock(x, y, x2, y2) && can_add_dirt(x, y, x2, y2))
            {
                short rockRecno;
                rockRecno =
                    rock_res.search("DEP", SMALL_ROCK_SIZE + 1, LARGE_ROCK_SIZE, SMALL_ROCK_SIZE + 1, LARGE_ROCK_SIZE,
                                    -1, 0, terrain_res[get_loc(x, y)->terrain_id]->average_type);
                if (!rockRecno)
                    continue;
...
//									 generated based on
// the
// layout. Otherwise 									 mountains are
// generated based on altitude.
//
...
#define MAX_ALTITUDE 255
        ...
        int mountainAddCount = MAX_WORLD_X_LOC * MAX_WORLD_Y_LOC * (30 + misc.random(20)) / 100 /
    ...
    for (y = 0; y < max_y_loc - HUGE_ROCK_SIZE; y++)
    {
        for (x = 0; x < max_x_loc - HUGE_ROCK_SIZE; x++)
        {
            ...
            if (can_add_rock(x, y, x2, y2))
            {
                ...
                rockRecno = rock_res.search("R", LARGE_ROCK_SIZE + 1, HUGE_ROCK_SIZE, LARGE_ROCK_SIZE + 1,
                                            HUGE_ROCK_SIZE, -1, 0, 0);
