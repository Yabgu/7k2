...
void World::add_rock(short rockRecno, short x1, short y1) {
  ...
  for (short dy = 0; dy < rockInfo->loc_height && y1 + dy < max_y_loc; ++dy) {
    for (short dx = 0; dx < rockInfo->loc_width && x1 + dx < max_x_loc; ++dx) {
      ...
      // ##### begin Gilbert 5/11 ######//
      //			locPtr->set_power_off();		// no
      //longer set_surr_power_off to avoid building too near
...
//						   central big rock and several smaller
//rocks around
...
void World::gen_rocks(int nGrouped, int nLarge, int nSmall) {
  ...
  while ((nGrouped > 0 || nLarge > 0 || nSmall > 0) && --trial > 0) {
    // generate grouped rocks i.e. small stones must have same terrsin as huge
    // ones
    if (nGrouped > 0) {
      short x =
          (GAP + LARGE_ROCK_SIZE) + misc.random(max_x_loc - HUGE_ROCK_SIZE + 1 -
                                                2 * (GAP + LARGE_ROCK_SIZE));
      short y =
          (GAP + LARGE_ROCK_SIZE) + misc.random(max_y_loc - HUGE_ROCK_SIZE + 1 -
                                                2 * (GAP + LARGE_ROCK_SIZE));
      ...
      if (can_add_rock(x, y, x2, y2)) {
        short rockRecno;
        //		if (misc.random(10) >5)
        rockRecno = rock_res.search(
            "R", LARGE_ROCK_SIZE + 1, HUGE_ROCK_SIZE, LARGE_ROCK_SIZE + 1,
            HUGE_ROCK_SIZE, -1, 0,
            terrain_res[get_loc(x, y)->terrain_id]->average_type);
        //		else
        //		rockRecno = rock_res.search("P",LARGE_ROCK_SIZE
        //+1,HUGE_ROCK_SIZE,LARGE_ROCK_SIZE +1,HUGE_ROCK_SIZE,-1,0,
        //			terrain_res[get_loc(x,y)->terrain_id]->average_type
        //);
        if (!rockRecno)
          continue;
        ...
        if (rockInfo->valid_terrain(
                terrain_res[get_loc(x2, y)->terrain_id]->average_type) &&
            rockInfo->valid_terrain(
                terrain_res[get_loc(x, y2)->terrain_id]->average_type) &&
            rockInfo->valid_terrain(
                terrain_res[get_loc(x2, y2)->terrain_id]->average_type)) {
          ...
          for (int subTrial = 12; subTrial > 0; --subTrial) {
            ...
            short sx = (short)(x + tempwidth / 2 +
                               (tempwidth / 2 + GAP) * cos(r * PI / 180));
            short sy = (short)(y + tempheight / 2 +
                               (tempheight / 2 + GAP) * sin(r * PI / 180));
            ...
            if (can_add_rock(sx, sy, sx2, sy2)) {
              short rock2Recno;
              //	if (misc.random(10) >5)
              rock2Recno = rock_res.search(
                  "R", 1, LARGE_ROCK_SIZE, 1, LARGE_ROCK_SIZE, -1, 0,
                  terrain_res[get_loc(sx, sy)->terrain_id]->average_type);
              //	else
              //	rock2Recno =
              //rock_res.search("P",1,LARGE_ROCK_SIZE,1,LARGE_ROCK_SIZE,-1,0,
              //		terrain_res[get_loc(sx,sy)->terrain_id]->average_type
              //);
              if (!rock2Recno)
                continue;
              ...
              if (rock2Info->valid_terrain(
                      terrain_res[get_loc(sx2, sy)->terrain_id]
                          ->average_type) &&
                  rock2Info->valid_terrain(
                      terrain_res[get_loc(sx, sy2)->terrain_id]
                          ->average_type) &&
                  rock2Info->valid_terrain(
                      terrain_res[get_loc(sx2, sy2)->terrain_id]
                          ->average_type)) {
    ...
    if (nLarge > 0) {
      ...
      if (can_add_rock(x, y, x2, y2)) {
        ...
        //	if (misc.random(10) >5)
        rockRecno = rock_res.search(
            "R", SMALL_ROCK_SIZE + 1, LARGE_ROCK_SIZE, SMALL_ROCK_SIZE + 1,
            LARGE_ROCK_SIZE, -1, 0,
            terrain_res[get_loc(x, y)->terrain_id]->average_type);
        //	else
        //	rockRecno = rock_res.search("P",
        //SMALL_ROCK_SIZE+1,LARGE_ROCK_SIZE,SMALL_ROCK_SIZE+1,LARGE_ROCK_SIZE,-1,0,
        //		terrain_res[get_loc(x,y)->terrain_id]->average_type );
        if (!rockRecno)
          continue;
    ...
    if (nSmall > 0) {
      ...
      if (can_add_rock(x, y, x2, y2)) {
        ...
        //	if (misc.random(10) >5)
        rockRecno = rock_res.search(
            "R", 1, SMALL_ROCK_SIZE, 1, SMALL_ROCK_SIZE, -1, 0,
            terrain_res[get_loc(x, y)->terrain_id]->average_type);
        //	else
        //	rockRecno = rock_res.search("P",
        //1,SMALL_ROCK_SIZE,1,SMALL_ROCK_SIZE,-1,0,
        //		terrain_res[get_loc(x,y)->terrain_id]->average_type );
        if (!rockRecno)
          continue;
  ...
  while ((nGrouped > 0 || nLarge > 0 || nSmall > 0) && --trial > 0) {
    ...
    if (nGrouped > 0) {
      ...
      if (can_add_rock(x, y, x2, y2) && can_add_dirt(x, y, x2, y2)) {
        short rockRecno;
        rockRecno = rock_res.search(
            "DEP", LARGE_ROCK_SIZE + 1, HUGE_ROCK_SIZE, LARGE_ROCK_SIZE + 1,
            HUGE_ROCK_SIZE, -1, 0,
            terrain_res[get_loc(x, y)->terrain_id]->average_type);
        if (!rockRecno)
          continue;
        ...
        if (rockInfo->valid_terrain(
          ...
          for (int subTrial = 12; subTrial > 0; --subTrial) {
            ...
            if (can_add_rock(sx, sy, sx2, sy2) &&
              ...
              short rock2Recno;
              rock2Recno = rock_res.search(
                  scanRockType, 1, LARGE_ROCK_SIZE, 1, LARGE_ROCK_SIZE, -1, 0,
                  terrain_res[get_loc(sx, sy)->terrain_id]->average_type);
    ...
    if (nLarge > 0) {
      ...
      if (can_add_rock(x, y, x2, y2) && can_add_dirt(x, y, x2, y2)) {
        short rockRecno;
        rockRecno = rock_res.search(
            "DEP", SMALL_ROCK_SIZE + 1, LARGE_ROCK_SIZE, SMALL_ROCK_SIZE + 1,
            LARGE_ROCK_SIZE, -1, 0,
            terrain_res[get_loc(x, y)->terrain_id]->average_type);
        if (!rockRecno)
          continue;
    ...
    if (nSmall > 0) {
      ...
      if (can_add_rock(x, y, x2, y2) && can_add_dirt(x, y, x2, y2)) {
        short rockRecno;
        rockRecno = rock_res.search(
            "DEP", 1, SMALL_ROCK_SIZE, 1, SMALL_ROCK_SIZE, -1, 0,
            terrain_res[get_loc(x, y)->terrain_id]->average_type);
        if (!rockRecno)
          continue;
...
//layout. Otherwise 									 mountains are generated based on altitude.
//
...
#define MAX_ALTITUDE 255
    ...
    int mountainAddCount =
        MAX_WORLD_X_LOC * MAX_WORLD_Y_LOC * (30 + misc.random(20)) / 100 /
        (HUGE_ROCK_SIZE * HUGE_ROCK_SIZE / 2); // 10% to 30% of mountains
  ...
  for (y = 0; y < max_y_loc - HUGE_ROCK_SIZE; y++) {
    for (x = 0; x < max_x_loc - HUGE_ROCK_SIZE; x++) {
      int mX =
          (x + misc.random(5) - 2) * MOUNTAIN_LAYOUT_X_COUNT / MAX_WORLD_X_LOC;
      int mY =
          (y + misc.random(5) - 2) * MOUNTAIN_LAYOUT_Y_COUNT / MAX_WORLD_Y_LOC;

      int mX2 = x * MOUNTAIN_LAYOUT_X_COUNT / MAX_WORLD_X_LOC;
      int mY2 = y * MOUNTAIN_LAYOUT_Y_COUNT / MAX_WORLD_Y_LOC;

      if (mountainLayout) {
        if (!mountainLayout[mY * MOUNTAIN_LAYOUT_X_COUNT +
                            mX] && // additional areas around the marked area
            !mountainLayout[mY2 * MOUNTAIN_LAYOUT_X_COUNT +
                            mX2]) // the marked area
        {
          continue;
        }
      ...
      if (can_add_rock(x, y, x2, y2)) {
        ...
        rockRecno =
            rock_res.search("R", LARGE_ROCK_SIZE + 1, HUGE_ROCK_SIZE,
                            LARGE_ROCK_SIZE + 1, HUGE_ROCK_SIZE, -1, 0, 0);
  ...
  for (y = 0; y < max_y_loc - SMALL_ROCK_SIZE; y++) {
    for (x = 0; x < max_x_loc - SMALL_ROCK_SIZE; x++) {
      ...
      if (can_add_rock(x, y, x2, y2)) {
        ...
        rockRecno = rock_res.search("R", 1, SMALL_ROCK_SIZE, 1, SMALL_ROCK_SIZE,
                                    -1, 0, 0);
  ...
  for (y = 0; y < max_y_loc; y++) {
    for (x = 0; x < max_x_loc; x++) {
      int mX =
          (x + misc.random(3) - 1) * MOUNTAIN_LAYOUT_X_COUNT / MAX_WORLD_X_LOC;
      int mY =
          (y + misc.random(3) - 1) * MOUNTAIN_LAYOUT_Y_COUNT / MAX_WORLD_Y_LOC;
      ...
      if (can_add_rock(x, y, x2, y2)) {
        ...
        rockRecno = rock_res.search("R", 1, SMALLEST_ROCK_SIZE, 1,
                                    SMALLEST_ROCK_SIZE, -1, 0, 0);
  ...
  /* {
          ...
          for( i = pointCount-1; i >= 0; --i )
          {
                  ...
                  for( j = 0; j < pointCount; ++j )
                  {
                          if( j ==i )
                                  continue;
                          if( misc.points_distance(pointX[i], pointY[i],
  pointX[j], pointY[j]) < 20 )
                          {
                                  dropFlag++;
                                  break;
                          }
                  ...
                  if( dropFlag )
                  {
                          misc.del_array_rec( pointX, pointCount, sizeof(short),
  i+1 ); misc.del_array_rec( pointY, pointCount, sizeof(short), i+1 );
                          --pointCount;
                  }
          }

          // find how many points are very disconnected from other points
  (blocked by mountain)
