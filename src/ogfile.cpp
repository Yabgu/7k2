...
//
// ###### begin Gilbert 26/5 ########//
int GameFile::save_game(const char *path, const char *fileName,
                        const char *gameDesc)
// ###### end Gilbert 26/5 ########//
{
  File file;
  ...
  if (GetDiskFreeSpace(
          NULL, // address of root path, NULL means the current root directory
          &sectorPerCluster, &bytePerSector, &freeCluster, &totalCluster)) {
    DWORD freeSpace =
        DWORD((double)freeCluster * sectorPerCluster * bytePerSector / 1024.0);
    ...
    if (!(rc = freeSpace >= MIN_FREE_SPACE)) {
      errStr =
          text_game_menu.str_out_of_disk_space(); // "Insufficient disk space !
  ...
  if (rc) {
    rc = file.file_create(
        file_name, 0,
        1); // 0=tell File don't handle error itself
  ...
  if (rc) {
    ...
    if (!rc)
      errStr =
          text_game_menu
              .str_save_error_header(); // "Error creating saved game header.";
    ...
    if (rc) {
      ...
      if (!rc)
        errStr =
            text_game_menu
                .str_save_error_general(); // "Error writing saved game data.";
    }
  }
...
#ifndef DEBUG
    // errStr = "Insufficient disk space ! The game is not saved.";		// use this
    // message for all types of error message in the release version
...
int GameFile::load_game(const char *path, const char *fileName) {
  ...
  if (!file.file_open(file_name, 0, 1)) // 0=tell File don't handle error itself
  {
    rc = 0;
    errMsg =
        text_game_menu.str_load_error_open(); // "Cannot open save game file";
  }
  ...
  if (rc) {
    if (!file.file_read(
            this,
            sizeof(GameFile))) // read the whole object from the saved game file
    {
      rc = 0;
      errMsg =
          text_game_menu.str_load_error_header(); // "Cannot read file header";
    }
    if (rc) {
      if (!validate_header()) {
        rc = 0;
        errMsg =
            text_game_menu.str_load_error_old(); // "Save game incompatible";
      } else
        strcpy(file_name,
               file.file_name); // set the file_name to the actual file
    }
  }
  ...
  if (rc) {
    ...
    game.deinit_all(); // deinit last game first, 1-it is called during loading
                       // of a game
  ...
  sys.signal_exit_flag = 4; // 4-means loading the game successfully. set it to
...
void GameFile::set_file_name(const char *path, const char *extension) {
  enum {
    NAME_PREFIX_LEN = 4, // Maximum 4 characters in name prefix, e.g. "ENLI" for
                         // "Enlight Enterprise"
    NAME_NUMBER_LEN = 3
  };
  ...
  // for( i=0 ; i<(int) strlen(baseName) && (int) str.len() < NAME_PREFIX_LEN ;
  // i++ )
  ...
  for (i = 1; i <= game_file_array.size(); i++) {
    ...
    if (strnicmp(gameFile->file_name, str, strLen) == 0) {
      ...
      // #### begin Gilbert 2/11 #####//
      curNumber =
          atoi(gameFile->file_name +
               strLen); // +1 is to pass the "_" between the name and the number
      // #### end Gilbert 2/11 #####//

      if (curNumber >
          lastNumber + 1) // normally, curNumber should be lastNumber+1
        break;
  ...
  for (i = NAME_NUMBER_LEN - str2.len(); i > 0;
       i--) // add "0" before the number if the len of the number <
            // NAME_NUMBER_LEN
    str += "0";
...
void GameFile::load_process() {
  info.start_play_time =
      misc.get_time(); // the time player start playing the game
  config.disable_ai_flag = 0;

  //-- if the player is in the diplomatic message screen, rebuild the talk
  //choice list --//
...
int GameFile::write_game_header(File *filePtr) {
  ...
  player_name[NationArray::CUSTOM_NAME_LEN] = '\0';
  ...
  return filePtr->file_write(
      this, sizeof(GameFile)); // write the whole object to the saved game file
}
