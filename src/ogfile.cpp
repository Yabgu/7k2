...
// ###### end Gilbert 26/5 ########//
{
                         ...
                         &sectorPerCluster, &bytePerSector, &freeCluster, &totalCluster))
    {
        ...
        {
            errStr = text_game_menu.str_out_of_disk_space(); // "Insufficient disk space !
    ...
    if (rc)
    {
        rc = file.file_create(file_name, 0,
...
#ifndef DEBUG
        // errStr = "Insufficient disk space ! The game is not saved.";		// use
        // this message for all types of error message in the release version
...
void GameFile::set_file_name(const char *path, const char *extension)
{
    ...
    for (i = 0, j = 0; j < NAME_PREFIX_LEN + 1; ++i)
    {
        if ((size_t)i < strlen(baseName) && j < NAME_PREFIX_LEN)
        {
            ...
            if ((nameChar >= 'A' && nameChar <= 'Z') || (nameChar >= '0' && nameChar <= '9'))
            {
                addStr[0] = nameChar;
                str += addStr;
                ++j;
    ...
    for (i = NAME_NUMBER_LEN - str2.len(); i > 0; i--) // add "0" before the number if the len of the number <
                                                       // NAME_NUMBER_LEN
        str += "0";
