...
void CampaignEastWest::stage_3_init_vars() {
  ...
  next_rebel_date =
      info.game_date; // so that there will be a rebel in the beginning
...
int CampaignEastWest::stage_3_create_town() {
  ...
  for (int i = townStateArray.size(); i > 0; i--) {
    ...
    if (playerTownAddCount > 0) {
      int buildFort = (i % 3 == 0); // 33% have fort and the rest don't. Rebels
                                    // only happens in towns without forts.
...
void CampaignEastWest::stage_3_next_day() {
  ...
  if (rebel_count % 3 == 0 ||
      misc.random((MAX_CAMPAIGN_DIFFICULTY - campaign_difficulty) / 2 +
                  nation_array.nation_count - 1) == 0 &&
