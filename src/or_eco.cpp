...
void Info::disp_economy(int refreshFlag) {
  ...
  vga_back.d3_panel_up(INCOME_BROWSE_X1, INCOME_BROWSE_Y1, INCOME_BROWSE_X2,
                       INCOME_BROWSE_Y1 + 20);

  font_bld.put(x, y, text_reports.str_income_item()); // "Income Item" );
                                                      // SXM
#if (defined(CHINESE))
  font_bld.put(x + 350 + 16, y,
               text_reports.str_yearly_income()); // "Yearly Income" );
#else
  font_bld.put(x + 350, y,
               text_reports.str_yearly_income()); // "Yearly Income" );
#endif
  // SXM
  ...
  if (nation_array[info.viewing_nation_recno]->income_365days(INCOME_CHEAT) >
          0 &&
      (sys.testing_session ||
       info.viewing_nation_recno ==
           nation_array
               .player_recno)) // only display cheat amount in debug mode or
                               // cheat amount of the player's kingdom, do not
                               // display cheat amount on AI kingdoms
  {
  ...
  if (refreshFlag == INFO_REPAINT) {
    // ####### begin Gilbert 19/10 ######//
    browse_income.init(INCOME_BROWSE_X1, INCOME_BROWSE_Y1 + 22,
                       INCOME_BROWSE_X2, INCOME_BROWSE_Y2 - 20, 0, 18,
                       incomeCount, put_income_rec, 1);
    // ####### begin Gilbert 19/10 ######//

    browse_income.open(
        browse_income_recno); // if refreshFlag is INFO_UPDATE, keep the
                              // original top_rec_no of the browser
  } else {
    browse_income.paint();
  ...
  vga_back.d3_panel_up(EXPENSE_BROWSE_X1, EXPENSE_BROWSE_Y1, EXPENSE_BROWSE_X2,
                       EXPENSE_BROWSE_Y1 + 20);
...
// SXM
  ...
  if (refreshFlag == INFO_REPAINT) {
    // ####### begin Gilbert 19/10 ######//
    browse_expense.init(EXPENSE_BROWSE_X1, EXPENSE_BROWSE_Y1 + 22,
                        EXPENSE_BROWSE_X2, EXPENSE_BROWSE_Y2 - 20, 0, 18,
                        EXPENSE_TYPE_COUNT, put_expense_rec, 1);
    // ####### end Gilbert 19/10 ######//

    browse_expense.open(
        browse_expense_recno); // if refreshFlag is INFO_UPDATE, keep the
                               // original top_rec_no of the browser
  } else {
    browse_expense.paint();
    browse_expense.open(
        browse_expense_recno,
        EXPENSE_TYPE_COUNT); // if refreshFlag is INFO_UPDATE, keep the original
                             // top_rec_no of the browser
  }
...
static void disp_total() {
  ...
  vga_back.d3_panel_up(INCOME_BROWSE_X1, INCOME_BROWSE_Y2 - 18,
                       INCOME_BROWSE_X2, INCOME_BROWSE_Y2);

  int x = INCOME_BROWSE_X1 + 9;
  int y = INCOME_BROWSE_Y2 - 16;

  font_bld.put(
      x, y, text_reports.str_total_yearly_income()); // "Total Yearly Income" );
  font_bld.put(x + 370, y, misc.format((int)totalIncome, 2));

  //---------- display total expense ----------//

  vga_back.d3_panel_up(EXPENSE_BROWSE_X1, EXPENSE_BROWSE_Y2 - 18,
                       EXPENSE_BROWSE_X2, EXPENSE_BROWSE_Y2);
  ...
  font_bld.put(
      x, y,
      text_reports.str_total_yearly_expense()); // "Total Yearly Expenses" );
  ...
  vga_back.d3_panel_up(EXPENSE_BROWSE_X1, EXPENSE_BROWSE_Y2 + 4,
                       EXPENSE_BROWSE_X2, ZOOM_Y2 - 6);
...
static void put_income_rec(int recNo, int x, int y, int refreshFlag) {
  ...
  //	static char* income_des_array[INCOME_TYPE_COUNT] =		// see
  //TextResReports::str_income_desc
  //	{
  //		"Sale of Goods",
  //		"Exports",
  //		"Taxes",
  ...
  font_bld.put(
      x, y, text_reports.str_income_desc(recNo - 1, nationPtr->is_monster()));
  font_bld.put(x + 370, y,
               misc.format((int)nationPtr->income_365days(recNo - 1), 2));
}
...
static void put_expense_rec(int recNo, int x, int y, int refreshFlag) {
  ...
  font_bld.put(
      x, y,
      text_reports.str_expense_desc(
          recNo - 1, nationPtr->is_monster())); // expense_des_array[recNo-1] );
  font_bld.put(x + 370, y,
               misc.format((int)nationPtr->expense_365days(recNo - 1), 2));
}
