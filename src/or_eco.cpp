...
void Info::disp_economy(int refreshFlag)
{
    ...
    if (nation_array[info.viewing_nation_recno]->income_365days(INCOME_CHEAT) > 0 &&
        (sys.testing_session ||
         info.viewing_nation_recno == nation_array.player_recno)) // only display cheat amount in debug mode or
    ...
    {
        // ####### begin Gilbert 19/10 ######//
        browse_income.init(INCOME_BROWSE_X1, INCOME_BROWSE_Y1 + 22, INCOME_BROWSE_X2, INCOME_BROWSE_Y2 - 20, 0, 18,
                           incomeCount, put_income_rec, 1);
        // ####### begin Gilbert 19/10 ######//

        browse_income.open(browse_income_recno); // if refreshFlag is INFO_UPDATE, keep the
...
// SXM
    ...
    {
        // ####### begin Gilbert 19/10 ######//
        browse_expense.init(EXPENSE_BROWSE_X1, EXPENSE_BROWSE_Y1 + 22, EXPENSE_BROWSE_X2, EXPENSE_BROWSE_Y2 - 20, 0, 18,
                            EXPENSE_TYPE_COUNT, put_expense_rec, 1);
        // ####### end Gilbert 19/10 ######//

        browse_expense.open(browse_expense_recno); // if refreshFlag is INFO_UPDATE, keep the
                                                   // original top_rec_no of the browser
    }
    else
    {
...
static void put_expense_rec(int recNo, int x, int y, int refreshFlag)
{
    ...
    font_bld.put(x, y,
                 text_reports.str_expense_desc(recNo - 1, nationPtr->is_monster())); // expense_des_array[recNo-1] );
