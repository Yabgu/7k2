...
// <char> noCost		  - if this is 1, the unit will be created with
// no cost.
//
...
int FirmLair::can_recruit()
{
    ...
    if (nation_recno)
    {
        ...
        return nationPtr->cash >= unitInfo->build_cost && nationPtr->live_points >= unitInfo->build_live_points_cost;
    }
    else
    {
        return 1;
...
int FirmLair::can_grant_to_non_own_lair(int grantNationRecno, int checkCash)
{
    ...
    if (!nation_array[grantNationRecno]->is_monster()) // only monster nations can grant to independent
                                                       // lairs
        return 0;
    ...
    for (int i = 0; i < linked_firm_count; i++)
    {
        ...
        if (firmLair && firmLair->nation_recno == grantNationRecno && firmLair->overseer_recno)
        {
            return 1;
        }
    }
...
int FirmLair::grant_to_non_own_lair(int grantNationRecno, int remoteAction)
{
    ...
    if (overseer_recno)
        unit_array[overseer_recno]->betray(grantNationRecno); // change the nation of the overseer which will then
...
void FirmLair::natural_breed()
{
    ...
    int unitCount = 0;
    if (!unit_array.is_deleted(overseer_recno) && unit_array[overseer_recno]->race_id == race_id)
        unitCount++;
