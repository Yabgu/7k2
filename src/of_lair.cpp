...
FirmLair::FirmLair() {
  memset(sizeof(FirmCamp) + (char *)this, 0,
         sizeof(FirmLair) - sizeof(FirmCamp));
}
//------ end of function FirmLair::FirmLair -------//

//------ begin of function FirmLair::init_derived -------//
//
void FirmLair::init_derived() { FirmCamp::init_derived(); }
...
void FirmLair::next_day() {
  ...
  err_when(nation_recno &&
           nation_array[nation_recno]
               ->is_human()); // humans should not have any firm lairs
}
...
// <char> noCost		  - if this is 1, the unit will be created with no
// cost.
//
void FirmLair::recruit_soldier(char remoteAction, char noCost) {
  ...
  if (nation_recno &&
      nation_array[nation_recno]->cash < (float)RECRUIT_MONSTER_COST)
    return;
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_FIRM_RECRUIT, sizeof(short));
    *shortPtr = firm_recno;
    // ##### end Gilbert 7/10 ######//
    return;
  }
  ...
  if (nation_recno && !noCost) {
    ...
    nation_array[nation_recno]->change_live_points(
        (float)-unitInfo->build_live_points_cost);
  }
}
...
//
void FirmLair::pay_expense() { FirmCamp::pay_expense(); }
...
void FirmLair::being_attack_hit(BaseObj *attackerObj, float damagePoint) {
  ...
  //---- if this is an independent lair, FirmCamp::being_attack_hit() will not
  //invoke defense ---//
...
int FirmLair::collect_town_tribute(int collectNow) {
  ...
  for (i = 0; i < linked_town_count; i++) {
    ...
    if (linked_town_enable_array[i] == LINK_EE && townPtr->nation_recno == 0 &&
...
void FirmLair::buy_tech(int techId, char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_LAIR_BUY_TECH,
                                                         2 * sizeof(short));
  ...
  nation_array[nation_recno]->change_live_points(
      (float)-techInfo->live_points_cost());
}
...
int FirmLair::can_grant_to_non_own_lair(int grantNationRecno, int checkCash) {
  ...
  if (!nation_array[grantNationRecno]
           ->is_monster()) // only monster nations can grant to independent
                           // lairs
    return 0;
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    ...
    if (firmLair && firmLair->nation_recno == grantNationRecno &&
...
int FirmLair::grant_to_non_own_lair(int grantNationRecno, int remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_F_LAIR_GRANT, 2 * sizeof(short));
    shortPtr[0] = firm_recno;
  ...
  grantNation->add_expense(EXPENSE_GRANT_OTHER_TOWN,
                           (float)grant_to_non_own_lair_cost());

  //--------- change nation now ----------//

  if (overseer_recno)
    unit_array[overseer_recno]->betray(
        grantNationRecno); // change the nation of the overseer which will then
                           // change the nation of the firm
  else
