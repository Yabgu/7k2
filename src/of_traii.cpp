...
int FirmTrain::cur_train_type; // parameter unitPara to recruit_train, determine
                               // to train civilian spy or infantry spy
int FirmTrain::required_linked_firm[TRAIN_UNIT_TYPE_MAX] = {0, FIRM_FORT,
                                                            FIRM_SPECIAL};
...
static short edit_combat_x1, edit_combat_y1, edit_combat_x2, edit_combat_y2,
    edit_combat_enable;
static short edit_combat_pot_x1, edit_combat_pot_y1, edit_combat_pot_x2,
    edit_combat_pot_y2, edit_combat_pot_enable;
static short edit_skill_x1, edit_skill_y1, edit_skill_x2, edit_skill_y2,
    edit_skill_enable;
static short edit_skill_pot_x1, edit_skill_pot_y1, edit_skill_pot_x2,
    edit_skill_pot_y2, edit_skill_pot_enable;
static short edit_spy_x1, edit_spy_y1, edit_spy_x2, edit_spy_y2,
    edit_spy_enable;
static short edit_spy_pot_x1, edit_spy_pot_y1, edit_spy_pot_x2, edit_spy_pot_y2,
    edit_spy_pot_enable;
...
void FirmTrain::disp_firm_info(int dispY1, int refreshFlag) {
  if (refreshFlag == INFO_REPAINT) {
    ...
    button_patrol.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 235, 'A',
                         "PATROL");
    button_patrol.enable_flag = 0;

    // ###### begin Gilbert 11/3 ########//
    button_go_rally.create(INFO_X1 + 13 + BUTTON_DISTANCE * 2, INFO_Y1 + 235,
                           'A', "RALLY-GO");
    button_clear_rally.create(INFO_X1 + 13 + BUTTON_DISTANCE * 3, INFO_Y1 + 235,
                              'A', "RALLY-NO");
    // ###### end Gilbert 11/3 ########//
  }
  ...
  if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count) {
    ...
    if (is_own()) {
      ...
      button_spy_mobilize.enable_flag =
          traineePtr->is_own_spy() && !traineePtr->is_under_training;
...
void FirmTrain::detect_firm_info() {
  ...
  // if one button is enable, the other button detect() is not called at all to
  // avoid incorrect help text
  ...
  // ##### end Gilbert 2/11 ######//
  {
    ...
    if (trainee_count > 0) {
      ...
      se_res.sound(center_x, center_y, 1, 'S', unitInfo->sprite_id, "ACK");
             ...
             selected_trainee_id > 0 && selected_trainee_id <= trainee_count) {
    err_when(
        spy_array.is_deleted(trainee_array[selected_trainee_id - 1].spy_recno));
    ...
    if (!remote.is_enable()) {
      ...
      short *shortPtr =
          (short *)remote.new_send_queue_msg(MSG_SPY_LEAVE_FIRM, sizeof(short));
...
void FirmTrain::disp_trainee_list(int dispY1, int refreshFlag) {
  ...
  for (int inc = -1; inc <= 1; inc += 2) {
    ...
    for (int i = inc >= 0 ? 1 : trainee_count; i > 0 && i <= trainee_count;
         i += inc) {
      // display trainee i
      ...
      if (inc < 0) {
        ...
        // ####### begin Gilbert 18/11 ######//
        //				info.draw_unit_icon( x+TRAINEE_X_SPACING/2,
        //y, 					unitId, nation_recno, windowX1, windowY1, windowX2, windowY2,
        //					(traineePtr->combat_level() < 20) ?
        //(((20 - traineePtr->combat_level()) <<6)+ 33) : 1);

        int darkness = BASIC_SKILL_TRAIN - representAttribute;
        if (darkness < 0)
          darkness = 0;
        info.draw_unit_icon(
            x + TRAINEE_X_SPACING / 2, y, unitId, nation_recno, windowX1,
        ...
        if (info.draw_unit_icon(
                x + TRAINEE_X_SPACING / 2, y, unitId, nation_recno, windowX1,
                windowY1, windowX2, windowY2,
                inAreaFlag | (i == selected_trainee_id ? 3 : 0)) &
            4) {
        ...
        if (!dispSpyMenu) {
          if (disp_combat_or_skill) {
            ...
            if (attribute >= 0) {
              font->center_put(x, yHp, x + TRAINEE_X_SPACING,
                               yHp + font->max_font_height,
                               misc.format(attribute));
            }
          } else if (ownSpy) // display spy icon
            vga.active_buf->put_bitmap_trans(x + TRAINEE_X_SPACING / 2 - 8,
          ...
          // display spy skill
          err_when(!ownSpy);
          font_whbl.center_put(x, yHp, x + TRAINEE_X_SPACING,
                               yHp + font->max_font_height,
...
int FirmTrain::detect_trainee_list() {
  ...
  for (int i = 1; i <= trainee_count; ++i) {
    ...
    int rc = info.draw_unit_icon(
        x + TRAINEE_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1,
...
void FirmTrain::disp_trainee_info(int dispY1, int refreshFlag) {
  ...
  if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count) {
    ...
    if (traineePtr->race_id) {
      ...
      //	info.disp_loyalty( x, y+14, x+60, traineePtr->loyalty,
      //traineePtr->loyalty, nation_recno, refreshFlag, disp_combat_or_skill==4
      //);
      ...
      if (unit_res[traineePtr->unit_id]->class_info.has_combat_level) {
        ...
        font_snds.right_put(INFO_X2 - 10, y + 12,
                            misc.format(traineePtr->combat_level(), 4));
      ...
      if (unit_res[traineePtr->unit_id]->class_info.has_skill_level &&
        ...
        font_snds.right_put(INFO_X2 - 10, y + 26,
                            misc.format(traineePtr->skill_level(), 4));
        //	font_snds.put( x2, y+28, traineePtr->skill_level() );
      }

      // ------- display spy skill --------//
      if (traineePtr->spy_skill > 0) {
        x2 = (disp_combat_or_skill == 3 ? font_blu2 : font_snds)
                 .put(x, y + 26, text_unit.str_spy_skill()) +
             10;
        font_snds.right_put(INFO_X2 - 100, y + 26,
                            misc.format((int)traineePtr->spy_skill, 4));
        ...
        font_snds.right_put(
            INFO_X2 - 100, y + 26,
            misc.format(spy_array[traineePtr->spy_recno]->spy_skill, 4));
        //	font_snds.put( x2, y+28,
        //spy_array[traineePtr->spy_recno]->spy_skill );
      }
    }
    //		else
    //			font_san.field( x, y, "Loyalty", x+100, "N/A", INFO_X2-2,
    //refreshFlag );		// no loyalty because it does not belong to your
    //empire
  }
}
...
void FirmTrain::detect_trainee_info() {
  ...
  if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count) {
    ...
    } else if (traineePtr->spy_skill > 0 &&
               mouse.single_click(x, y + 28, x + 79, y + 41)) {
      disp_combat_or_skill = 3; // detect spying skill
    } else if (traineePtr->skill_level() &&
...
void FirmTrain::disp_train_menu(int refreshFlag) {
  ...
  int raceCount = scan_best_recruitable_town(
      townRecnoArray, 0, required_linked_firm[cur_train_type]);
  ...
  for (int inc = -1; inc <= 1; inc += 2) {
    ...
    for (int i = inc >= 0 ? 0 : raceCount - 1; i >= 0 && i < raceCount;
         i += inc) {
      ...
      if (inc < 0) {
        ...
        info.draw_unit_icon(x + TRAINEE_X_SPACING / 2, y, unitId,
                            townPtr->nation_recno, windowX1, windowY1, windowX2,
                            windowY2, 1);
      } else {
        // second round is ascending to draw the frame

        if (info.draw_unit_icon(x + TRAINEE_X_SPACING / 2, y, unitId,
                                townPtr->nation_recno, windowX1, windowY1,
                                windowX2, windowY2, inAreaFlag) &
            4) {
          inAreaFlag = 0; // frame for mouse cursor is drawn, disable the frame
        }

        // display loyalty
        font_whbl.center_put(x, yLoyalty, x + TRAINEE_X_SPACING,
                             yLoyalty + font_snds.max_font_height,
...
//
void FirmTrain::disp_spy_menu(int refreshFlag) { err_here(); }
// ------- end of function FirmTrain::disp_spy_menu ------//

// ------- begin of function FirmTrain::detect_spy_menu ------//
//
void FirmTrain::detect_spy_menu() { err_here(); }
...
void FirmTrain::disp_edit_mode(int &refreshFlag, int &y, int dispBG) {
  ...
  if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count) {
    ...
    if (unitInfo->class_info.has_combat_level &&
      ...
      x = edit_combat_x2 =
          font_blue.put((edit_combat_x1 = x + 20), (edit_combat_y1 = y),
                        traineePtr->skill.actual_combat_level(NULL), 0, x2);
      edit_combat_y2 = edit_combat_y1 + font_blue.max_font_height;
      edit_combat_enable = 1;

      x = font_whbl.put(x, y, " (", 0, x2);
      x = edit_combat_pot_x2 = font_blue.put(
          (edit_combat_pot_x1 = x), (edit_combat_pot_y1 = y),
          misc.format(traineePtr->train_combat_potential, 3), 0, x2);
    ...
    if (unitInfo->class_info.has_skill_level &&
        traineePtr->train_skill_potential > 0) {
      x = font_whbl.put(x1, y, text_unit.str_leadership(), 0,
                        x2); //	"Leadership"
      x = edit_skill_x2 =
          font_blue.put((edit_skill_x1 = x + 20), (edit_skill_y1 = y),
                        traineePtr->skill.actual_skill_level(NULL), 0, x2);
      edit_skill_y2 = edit_skill_y1 + font_blue.max_font_height;
      edit_skill_enable = 1;

      x = font_whbl.put(x, y, " (", 0, x2);
      x = edit_skill_pot_x2 = font_blue.put(
          (edit_skill_pot_x1 = x), (edit_skill_pot_y1 = y),
          misc.format(traineePtr->train_skill_potential, 3), 0, x2);
    ...
    if (traineePtr->train_spy_potential > 0) {
      ...
      x = edit_spy_x2 = font_blue.put((edit_spy_x1 = x + 20), (edit_spy_y1 = y),
                                      (int)traineePtr->spy_skill, 0, x2);
      edit_spy_y2 = edit_spy_y1 + font_blue.max_font_height;
      edit_spy_enable = 1;

      x = font_whbl.put(x, y, " (", 0, x2);
      x = edit_spy_pot_x2 =
          font_blue.put((edit_spy_pot_x1 = x), (edit_spy_pot_y1 = y),
                        misc.format(traineePtr->train_spy_potential, 3), 0, x2);
...
int FirmTrain::detect_edit_mode() {
  ...
  if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count) {
    ...
    if (edit_combat_pot_enable) {
      if (mouse.any_click(edit_combat_pot_x1, edit_combat_pot_y1,
                          edit_combat_pot_x2, edit_combat_pot_y2,
                          LEFT_BUTTON)) {
    ...
    if (edit_skill_pot_enable) {
      if (mouse.any_click(edit_skill_pot_x1, edit_skill_pot_y1,
                          edit_skill_pot_x2, edit_skill_pot_y2, LEFT_BUTTON)) {
      ...
      if (mouse.any_click(edit_skill_pot_x1, edit_skill_pot_y1,
                          edit_skill_pot_x2, edit_skill_pot_y2, RIGHT_BUTTON)) {
        traineePtr->train_skill_potential -= 5;
    ...
    if (edit_spy_pot_enable) {
      if (mouse.any_click(edit_spy_pot_x1, edit_spy_pot_y1, edit_spy_pot_x2,
                          edit_spy_pot_y2, LEFT_BUTTON)) {
