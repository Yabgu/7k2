...
void FirmTrain::detect_firm_info()
{
    ...
    if (button_mobilize_visible && button_mobilize.detect() && selected_trainee_id > 0 &&
        selected_trainee_id <= trainee_count)
    {
        // ####### begin Gilbert 26/1 ###//
    ...
    else if (button_spy_mobilize_visible && button_spy_mobilize.detect() && selected_trainee_id > 0 &&
             selected_trainee_id <= trainee_count)
    {
    ...
    for (int inc = -1; inc <= 1; inc += 2)
    {
        ...
        for (int i = inc >= 0 ? 1 : trainee_count; i > 0 && i <= trainee_count; i += inc)
        {
            ...
            if (inc < 0)
            {
                ...
                // ####### begin Gilbert 18/11 ######//
                //				info.draw_unit_icon(
                // x+TRAINEE_X_SPACING/2,
                // y, 					unitId, nation_recno, windowX1, windowY1,
                // windowX2, windowY2,
                ...
                if (darkness < 0)
                    darkness = 0;
                info.draw_unit_icon(x + TRAINEE_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1, windowX2,
                                    windowY2, (traineePtr->is_under_training) ? ((darkness << 6) + 33) : 1);
                // ####### end Gilbert 18/11 ######//
            }
            else
            {
                ...
                if (info.draw_unit_icon(x + TRAINEE_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1,
                                        windowX2, windowY2, inAreaFlag | (i == selected_trainee_id ? 3 : 0)) &
                    4)
                {
                ...
                if (!dispSpyMenu)
                {
                    if (disp_combat_or_skill)
                    {
                        ...
                        vga.active_buf->put_bitmap_trans(x + TRAINEE_X_SPACING / 2 - 8, yHp - 5,
...
int FirmTrain::detect_trainee_list()
{
    ...
    for (int i = 1; i <= trainee_count; ++i)
    {
        ...
        int rc = info.draw_unit_icon(x + TRAINEE_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1, windowX2,
                                     windowY2,
...
void FirmTrain::disp_trainee_info(int dispY1, int refreshFlag)
{
    ...
    if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count)
    {
        ...
        if (traineePtr->race_id)
        {
            ...
            if (unit_res[traineePtr->unit_id]->class_info.has_loyalty)
            {
                info.disp_loyalty(x, y + 12, INFO_X2 - 99 - font_snds.text_width(misc.format(traineePtr->loyalty, 4)),
                                  //		font_snds.text_width(misc.format(traineePtr->loyalty,
                                  // 4))
            ...
            if (unit_res[traineePtr->unit_id]->class_info.has_skill_level && traineePtr->skill_level() > 0)
            {
        ...
        //		else
        //			font_san.field( x, y, "Loyalty", x+100, "N/A",
        // INFO_X2-2,
...
void FirmTrain::detect_trainee_info()
{
    ...
    if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count)
    {
        ...
        if (unitInfo->class_info.has_loyalty && mouse.single_click(x, y + 14, x + 79, y + 27))
        {
    ...
    for (int inc = -1; inc <= 1; inc += 2)
    {
        ...
        for (int i = inc >= 0 ? 0 : raceCount - 1; i >= 0 && i < raceCount; i += inc)
        {
            ...
            if (inc < 0)
            {
                ...
                info.draw_unit_icon(x + TRAINEE_X_SPACING / 2, y, unitId, townPtr->nation_recno, windowX1, windowY1,
                                    windowX2, windowY2, 1);
            }
            else
            {
                ...
                if (info.draw_unit_icon(x + TRAINEE_X_SPACING / 2, y, unitId, townPtr->nation_recno, windowX1, windowY1,
...
void FirmTrain::disp_edit_mode(int &refreshFlag, int &y, int dispBG)
{
    ...
    if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count)
    {
        ...
        if (unitInfo->class_info.has_loyalty) // && rank_id != RANK_KING )
        {
            ...
            x = edit_loyalty_x2 =
                font_blue.put((edit_loyalty_x1 = x + 20), (edit_loyalty_y1 = y), traineePtr->loyalty, 0, x2);
        ...
        if (unitInfo->class_info.has_combat_level && traineePtr->train_combat_potential > 0)
        {
            ...
            x = edit_combat_pot_x2 = font_blue.put((edit_combat_pot_x1 = x), (edit_combat_pot_y1 = y),
        ...
        if (unitInfo->class_info.has_skill_level && traineePtr->train_skill_potential > 0)
        {
            ...
            x = font_whbl.put(x, y, " (", 0, x2);
            x = edit_skill_pot_x2 = font_blue.put((edit_skill_pot_x1 = x), (edit_skill_pot_y1 = y),
        ...
        if (traineePtr->train_spy_potential > 0)
        {
            ...
            x = edit_spy_x2 =
                font_blue.put((edit_spy_x1 = x + 20), (edit_spy_y1 = y), (int)traineePtr->spy_skill, 0, x2);
            edit_spy_y2 = edit_spy_y1 + font_blue.max_font_height;
            edit_spy_enable = 1;

            x = font_whbl.put(x, y, " (", 0, x2);
            x = edit_spy_pot_x2 = font_blue.put((edit_spy_pot_x1 = x), (edit_spy_pot_y1 = y),
...
int FirmTrain::detect_edit_mode()
{
    ...
    if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count)
    {
        ...
        {
            if (mouse.any_click(edit_combat_pot_x1, edit_combat_pot_y1, edit_combat_pot_x2, edit_combat_pot_y2,
                                LEFT_BUTTON))
            {
        ...
        {
            if (mouse.any_click(edit_skill_pot_x1, edit_skill_pot_y1, edit_skill_pot_x2, edit_skill_pot_y2,
                                LEFT_BUTTON))
            {
