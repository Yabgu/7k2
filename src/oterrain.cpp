...
static int
    terrain_type_color_array[TOTAL_TERRAIN_TYPE] = // the color of each terrain
                                                   // type on the small map
    {
        0x0A,
        0x0D,
        0x2D,
...
static const char
    *map_tile_name_array[TOTAL_TERRAIN_TYPE] = // the color of each terrain type
                                               // on the small map
    {
        "TERA_DG",
        "TERA_LG",
...
static int terrain_type_min_height_array[TOTAL_TERRAIN_TYPE]
                                        [3] = // the color of each terrain type
                                              // on the small map
    {
        {0, 15, 70},     // G-, G0, G+
        {85, 100, 155},  // F-, F0, F+
...
char *TerrainInfo::get_bitmap(unsigned frameNo) {
  return frameNo && anim_frames
             ? anim_bitmap_ptr[frameNo % (unsigned)anim_frames]
             : NULL;
}
...
TerrainRes::TerrainRes() { init_flag = 0; }
...
void TerrainRes::init() {
  ...
  for (i = 0; i < TOTAL_TERRAIN_TYPE; i++) {
    if (map_tile_name_array[i])
      // ###### begin Gilbert 9/9 ########//
      // map_tile_ptr_array[i] = image_spict.get_ptr(map_tile_name_array[i]) +
      // sizeof(short)*2;
      map_tile_ptr_array[i] =
          image_tpict.get_ptr(map_tile_name_array[i]) + sizeof(short) * 2;
    // ###### end Gilbert 9/9 ########//
  }
  ...
  for (i = 1; i <= TOTAL_TERRAIN_TYPE; i++) {
    terrainId = scan(
        i, ALL_MASK, i, ALL_MASK, i, ALL_MASK, i, ALL_MASK,
        1); // scan for terrain of all four types. 1-take the first instance of
            // the terrain bitmap that matches the given terrain types

    if (terrainId) {
      terrain_type_array[i - 1].first_terrain_id = terrainId;
      terrain_type_array[i - 1].last_terrain_id =
          terrainId + terrain_res[terrainId]->alternative_count_with_extra;
      terrain_type_array[i - 1].min_height =
          terrain_type_min_height_array[i - 1][0];
    } else {
      // BUGHERE: should not reach this point after all terrain type are
      // complete
...
void TerrainRes::load_info() {
  ...
  terrain_info_array =
      (TerrainInfo *)mem_add(sizeof(TerrainInfo) * terrain_count);
  ...
  for (i = 0; i < terrain_count; i++) {
    ...
    memcpy(&file_name_array[i * terrainRec->FILE_NAME_LEN],
           terrainRec->file_name, terrainRec->FILE_NAME_LEN);
    ...
    terrainInfo->special_flag =
        terrainRec->special_flag == ' ' ? 0 : terrainRec->special_flag;
    // ######## end Gilbert 12/2 #######//

    terrainInfo->secondary_type = terrain_code(terrainRec->secondary_type);
    terrainInfo->pattern_id =
        misc.atoi(terrainRec->pattern_id, terrainRec->PATTERN_ID_LEN);
...
// [int] firstInstance  - whether return the first instance of the terrain
// instead of a random instance
//								  (default: 0)
...
//					  ==0 - no terrain of the specific base types
//found
//
int TerrainRes::scan(int nwType, int nwSubType, int neType, int neSubType,
  ...
  for (; terrainId <= terrainIdMax; terrainId++, terrainInfo++) {
    if (terrainInfo->nw_type == nwType && terrainInfo->nw_subtype & nwSubType &&
        terrainInfo->ne_type == neType && terrainInfo->ne_subtype & neSubType &&
        terrainInfo->sw_type == swType && terrainInfo->sw_subtype & swSubType &&
        terrainInfo->se_type == seType && terrainInfo->se_subtype & seSubType &&
      ...
      if (firstInstance)
        ...
        if (includeExtra) {
          ...
          return terrainId +
                 misc.random(terrainInfo->alternative_count_with_extra + 1);
        } else {
          err_when(terrainInfo->alternative_count_without_extra < 0);
          return terrainId +
                 misc.random(terrainInfo->alternative_count_without_extra + 1);
        }
      }
    } else {
      ...
      if (((terrainInfo->average_type == primaryType &&
        ...
        if (firstInstance)
          ...
          if (includeExtra) {
            ...
            err_when(terrainInfo->alternative_count_without_extra < 0);
            return terrainId +
                   misc.random(terrainInfo->alternative_count_without_extra +
                               1);
          }
        }
      } else {
...
//
// <int>						nwPatternId		pattern_id of a
// terrain,
//													obtained
//by terrain_res[terrain_id]->pattern_id
// <TerrainSubInfo *>	resultArray		an array to be filled with
// matches
// <int>						maxResult		the no. of elements of
// the resultArray
// <int>						return			no. of
// matches
//
...
void TerrainRes::load_sub_info() {
  ...
  ter_sub_array =
      (TerrainSubInfo *)mem_add(sizeof(TerrainSubInfo) * ter_sub_rec_count);
  ...
  for (i = 0; i < ter_sub_rec_count; i++) {
    ...
    terrainSubInfo->sub_no =
        misc.atoi(terrainSubRec->sub_no, terrainSubRec->SUB_NO_LEN);
    terrainSubInfo->step_id =
        misc.atoi(terrainSubRec->step_id, terrainSubRec->STEP_ID_LEN);
    terrainSubInfo->old_pattern_id =
        misc.atoi(terrainSubRec->old_pattern_id, terrainSubRec->PATTERN_ID_LEN);
    terrainSubInfo->new_pattern_id =
        misc.atoi(terrainSubRec->new_pattern_id, terrainSubRec->PATTERN_ID_LEN);

    // sec_adj is useful when a pure type is changing to boundary type.
    // eg. a GG square can be changed to SG or GS sqare
    terrainSubInfo->sec_adj =
        misc.atoi(terrainSubRec->sec_adj, terrainSubRec->SEC_ADJ_LEN);
  ...
  ter_sub_index = (TerrainSubInfo **)mem_add(sizeof(TerrainSubInfo *) *
                                             ter_sub_index_count);
  ...
  for (i = 0; i < ter_sub_rec_count; i++) {
    ...
    if (terrainSubInfo->step_id == 1) {
      ...
      if (lastTerrainSubInfo &&
          lastTerrainSubInfo->sub_no == terrainSubInfo->sub_no &&
...
void TerrainRes::load_anim_info() {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (memcmp(terrainAnimRec.base_file, lastAnimRec.base_file,
               terrainAnimRec.FILE_NAME_LEN)) {
      // string not equal
      if (lastAnimRec.filename[0] != ' ' && animFrameCount > 0) {
        ...
        for (l = 0; l < terrain_count; ++l) {
          if (memcmp(&file_name_array[terrainAnimRec.FILE_NAME_LEN * l],
                     lastAnimRec.base_file,
                     terrainAnimRec.FILE_NAME_LEN) == 0) {
            TerrainInfo *terrainInfo = terrain_info_array + l;
            err_when(terrainInfo->anim_frames > 0);
            terrainInfo->anim_frames = animFrameCount;
            terrainInfo->anim_bitmap_ptr =
                (char **)mem_add(sizeof(char *) * animFrameCount);
        ...
        for(int special = 0; special <= 1; ++special)
        {
                j = scan( terrain_code(lastAnimRec.average_type),
                        ...
                        misc.atoi(lastAnimRec.pattern_id,
        lastAnimRec.PATTERN_ID_LEN), 1,0,special); if( j > 0)
                {
                        k =
        terrain_info_array[j-1].alternative_count_with_extra; for(l = j; l <=
        j+k; ++l)
                        {
                                if(
        memcmp(&file_name_array[terrainAnimRec.FILE_NAME_LEN *(l-1)],
                                        lastAnimRec.base_file,
        terrainAnimRec.FILE_NAME_LEN) == 0)
                                {
                                        TerrainInfo *terrainInfo =
        terrain_info_array+j-1; err_when(terrainInfo->anim_frames > 0);
                                        terrainInfo->anim_frames =
        animFrameCount; terrainInfo->anim_bitmap_ptr = (char **) mem_add(
                                                sizeof(char *)*animFrameCount);
                                        memcpy(terrainInfo->anim_bitmap_ptr,
        animFrameBitmap, sizeof(char *)*animFrameCount); replaceCount++;
                                }
                        }
                }
        }
    ...
    animFrameCount++;
    animFrameBitmap[misc.atoi(terrainAnimRec.frame_no,
                              terrainAnimRec.FRAME_NO_LEN) -
                    1] = bitmapPtr;
  }
  ...
  if (lastAnimRec.filename[0] != ' ' && animFrameCount > 0) {
    ...
    terrain_info_array+j-1; err_when(terrainInfo->anim_frames > 0);
                                    terrainInfo->anim_frames = animFrameCount;
                                    terrainInfo->anim_bitmap_ptr = (char **)
    mem_add( sizeof(char *)*animFrameCount);
                                    memcpy(terrainInfo->anim_bitmap_ptr,
    animFrameBitmap, sizeof(char *)*animFrameCount); replaceCount++;
                            }
                    }
...
void TerrainRes::load_tex_info() {
  ...
  texture_array =
      (TerrainTextureInfo *)mem_add(sizeof(TerrainTextureInfo) * texture_count);
  ...
  for (i = 0; i < texture_count; i++) {
    ...
    memcpy(&bitmapOffset, textureRec->bitmap_ptr, sizeof(uint32_t));
...
void TerrainRes::load_pal() {
  ...
  PalDesc palDesc(palSrc, sizeof(palSrc[0]), 0x100, 8, reservedColorArray,
                  sizeof(reservedColorArray));
  vga_color_table = new ColorTable;
  vga_color_table->generate_table(MAX_BRIGHTNESS_ADJUST_DEGREE, palDesc,
                                  ColorTable::bright_func);
}
// ----- End of function TerrainRes::load_pal --------//

// ----- Begin of function TerrainRes::get_texture ----------//
TerrainTextureInfo *TerrainRes::get_texture(char terrainType,
...
char *TerrainRes::mask_texture(char *maskPtr, char *tex1, char *tex2,
                               char *outTex) {
  ...
  err_when(((Bitmap *)maskPtr)->get_width() != 32 ||
           ((Bitmap *)maskPtr)->get_height() != 32);
  err_when(((Bitmap *)tex1)->get_width() != 32 ||
           ((Bitmap *)tex1)->get_height() != 32);
  err_when(((Bitmap *)tex2)->get_width() != 32 ||
           ((Bitmap *)tex2)->get_height() != 32);
