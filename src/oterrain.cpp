...
static int terrain_type_color_array[TOTAL_TERRAIN_TYPE] = // the color of each terrain
...
static const char *map_tile_name_array[TOTAL_TERRAIN_TYPE] = // the color of each terrain type
                                                             // on the small map
    {
        "TERA_DG",
        "TERA_LG",
...
static int terrain_type_min_height_array[TOTAL_TERRAIN_TYPE][3] = // the color of each terrain type
                                                                  // on the small map
    {
        {0, 15, 70},     // G-, G0, G+
        {85, 100, 155},  // F-, F0, F+
...
void TerrainRes::init()
{
    ...
    {
        terrainId = scan(i, ALL_MASK, i, ALL_MASK, i, ALL_MASK, i, ALL_MASK,
...
void TerrainRes::load_info()
{
    ...
    for (i = 0; i < terrain_count; i++)
    {
        ...
        memcpy(&file_name_array[i * terrainRec->FILE_NAME_LEN], terrainRec->file_name, terrainRec->FILE_NAME_LEN);
...
//					  ==0 - no terrain of the specific base
// types
// found
//
int TerrainRes::scan(int nwType, int nwSubType, int neType, int neSubType, int swType, int swSubType, int seType,
                     int seSubType, int firstInstance, int includeExtra, int special)
{
    ...
    {
        if (terrainInfo->nw_type == nwType && terrainInfo->nw_subtype & nwSubType && terrainInfo->ne_type == neType &&
            terrainInfo->ne_subtype & neSubType && terrainInfo->sw_type == swType &&
            terrainInfo->sw_subtype & swSubType && terrainInfo->se_type == seType &&
            terrainInfo->se_subtype & seSubType && terrainInfo->special_flag == special)
        {
            if (firstInstance)
                return terrainId;
            else
...
// found
//
int TerrainRes::scan(int primaryType, int secondaryType, int patternId, int firstInstance, int includeExtra,
                     int special)
{
...
//
// <int>						nwPatternId		pattern_id of
// a terrain,
//													obtained
// by terrain_res[terrain_id]->pattern_id
// <TerrainSubInfo *>	resultArray		an array to be filled with
// matches
// <int>						maxResult		the no. of elements
// of the resultArray
// <int>						return			no.
// of matches
//
int TerrainRes::search_pattern(int nwPatternId, TerrainSubInfo **resultArray, int maxResult)
{
    err_when(!init_flag);
...
void TerrainRes::load_anim_info()
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (memcmp(terrainAnimRec.base_file, lastAnimRec.base_file, terrainAnimRec.FILE_NAME_LEN))
        {
            ...
            if (lastAnimRec.filename[0] != ' ' && animFrameCount > 0)
            {
                ...
                {
                    if (memcmp(&file_name_array[terrainAnimRec.FILE_NAME_LEN * l], lastAnimRec.base_file,
        ...
        animFrameCount++;
        animFrameBitmap[misc.atoi(terrainAnimRec.frame_no, terrainAnimRec.FRAME_NO_LEN) - 1] = bitmapPtr;
    }
    ...
    if (lastAnimRec.filename[0] != ' ' && animFrameCount > 0)
    {
        ...
        for (l = 0; l < terrain_count; ++l)
        {
            if (memcmp(&file_name_array[terrainAnimRec.FILE_NAME_LEN * (l - 1)], lastAnimRec.base_file,
                       terrainAnimRec.FILE_NAME_LEN) == 0)
            {
...
{
    err_if(terrainId < 1 || terrainId > terrain_count) err_now("TerrainRes::operator[]");
