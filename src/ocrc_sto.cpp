...
CrcStore::CrcStore() : nations(0), units(0), firms(0), towns(0), bullets(0), rebels(0), spies(0), sites(0), talk_msgs(0)
{
}
...
int CrcStore::compare_remote(DWORD remoteMsgId, char *dataPtr)
{
    ...
    if (rc)
    {
        ...
        for (diffOffset = 0, p1 = vq->queue_buf, p2 = dataPtr; diffOffset < vq->length() && *p1 == *p2;
             ++diffOffset, ++p1, ++p2)
            ;
