...
CrcStore::CrcStore()
    : nations(0), units(0), firms(0), towns(0), bullets(0), rebels(0), spies(0),
      sites(0), talk_msgs(0) {}

void CrcStore::init() { deinit(); }
...
void CrcStore::record_bullets() {
  ...
  *(short *)bullets.reserve(sizeof(short)) = bullet_array.size();
  ...
  for (short bulletRecno = 1; bulletRecno <= bullet_array.size();
       ++bulletRecno) {
    CRC_TYPE checkNum = 0;
...
void CrcStore::record_talk_msgs() {
  ...
  *(short *)talk_msgs.reserve(sizeof(short)) = talk_res.talk_msg_count();
  ...
  for (short talkRecno = 1; talkRecno <= talk_res.talk_msg_count();
       ++talkRecno) {
    CRC_TYPE checkNum = 0;
...
void CrcStore::send_all() {
  char *charPtr;
  charPtr =
      (char *)remote.new_send_queue_msg(MSG_COMPARE_NATION, nations.length());
  ...
  charPtr =
      (char *)remote.new_send_queue_msg(MSG_COMPARE_BULLET, bullets.length());
  memcpy(charPtr, bullets.queue_buf, bullets.length());

  charPtr =
      (char *)remote.new_send_queue_msg(MSG_COMPARE_REBEL, rebels.length());
  ...
  charPtr =
      (char *)remote.new_send_queue_msg(MSG_COMPARE_TALK, talk_msgs.length());
...
int CrcStore::compare_remote(DWORD remoteMsgId, char *dataPtr) {
  ...
  if (rc) {
    ...
    for (diffOffset = 0, p1 = vq->queue_buf, p2 = dataPtr;
         diffOffset < vq->length() && *p1 == *p2; ++diffOffset, ++p1, ++p2)
      ;
