...
void Info::disp_news_log(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    browse_news.init(NEWS_BROWSE_X1, NEWS_BROWSE_Y1, NEWS_BROWSE_X2,
                     NEWS_BROWSE_Y2, 0, 32, news_array.size(), put_news_rec, 1);
  ...
  image_icon.put_back(ZOOM_X2 - 27, ZOOM_Y2 - 23,
...
void Info::detect_news_log() {
  ...
  if (mouse.single_click(ZOOM_X2 - 27, ZOOM_Y2 - 23, ZOOM_X2 - 27 + 14,
                         ZOOM_Y2 - 23 + 14))
...
static void put_news_rec(int recNo, int x, int y, int refreshFlag) {
  ...
  News *newsPtr =
      news_array[news_array.size() - recNo + 1]; // display in reversed order

  font_bld.put(x + 20, y, date.date_str(newsPtr->news_date, 1));

  talk_res.msg_add_nation_color = 1;
  font_bld.put_paragraph(x + 110, y, browse_news.ix2 - 30, y + 30,
                         newsPtr->msg());
  ...
  if (newsPtr->is_loc_valid()) {
    ...
    if (mouse.single_click(x + 2, y + 1, x + 17, y + 16)) {
      vga.use_front();
      world.go_loc(newsPtr->loc_x, newsPtr->loc_y,
