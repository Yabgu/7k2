...
static void validate_selected_unit_array(short *selectedUnitArray,
                                         short &selectedCount);
...
void RemoteMsg::process_msg() {
  ...
  MsgProcessFP msgProcessFP =
      msg_process_function_array[id - FIRST_REMOTE_MSG_ID];

  (this->*msgProcessFP)(); // call the corrsponding function to return the news
                           // process_msg
}
...
void RemoteMsg::new_nation() {
  ...
  int nationRecno =
      nation_array.new_nation(NATION_HUMAN, NATION_REMOTE, nationPtr->race_id,
                              nationPtr->color_scheme_id, nationPtr->player_id);
...
void RemoteMsg::update_game_setting() {
  ...
  for (i = 0; i < nationCount; i++) {
    ...
    // int NationType;
    if (dpPlayerId ==
        remote.self_player_id()) // if this nation is the player's nation
    {
    ...
    if (nationRecno != nation_array.new_nation(NATION_HUMAN, nationType, raceId,
                                               colorSchemeId, dpPlayerId)) {
...
// to start the game.
//
void RemoteMsg::start_game() { game.started_flag = 1; }
...
//			long_log = NULL;
      ...
      if ((remote.sync_test_level & 1) && (remote.sync_test_level >= 0)) {
        remote.sync_test_level =
            ~remote.sync_test_level; // signal error encountered
...
void RemoteMsg::unit_move() {
  ...
  if (unit_group_remote.validate_unit_array(remote.nation_processing) > 0) {
    unit_group_remote.move(shortPtr[0], shortPtr[1], shortPtr[2] != 0,
                           COMMAND_REMOTE);
  }
}
...
void RemoteMsg::unit_assign() {
  ...
  // packet structure : <xLoc> <yLoc> <target mobile type> <actionNationRecno>
  // <forceMoveFlag> <write_mem...>

  unit_group_remote.read_mem(shortPtr + 5);
  if (unit_group_remote.validate_unit_array(remote.nation_processing) > 0) {
    unit_group_remote.assign(shortPtr[0], shortPtr[1], shortPtr[2],
                             COMMAND_REMOTE, shortPtr[3], shortPtr[4] != 0);
  }
}
...
void RemoteMsg::unit_change_nation() {
  ...
  //	unit_array.change_nation( shortPtr[0], selectedUnitArray, selectedCount,
  //COMMAND_REMOTE );   // 1-remote action
}
...
//
static void validate_selected_unit_array(short *selectedUnitArray,
                                         short &selectedUnitCount) {
  for (int i = 0; i < selectedUnitCount; i++) {
    Unit *unitPtr;
    if (unit_array.is_deleted(selectedUnitArray[i]) ||
        !(unitPtr = unit_array[selectedUnitArray[i]]) ||
        !unitPtr->is_visible() ||
        !unitPtr->is_nation(remote.nation_processing)) {
      memmove(selectedUnitArray + i, selectedUnitArray + i + 1,
              sizeof(short) * (selectedUnitCount - i - 1));
      selectedUnitCount--;
      i--; // stay with the current recno as the records have been moved. The
           // recno in the current position is actually the next record.
    }
  }
}
...
void RemoteMsg::unit_build_firm() {
  ...
  if (unitCount > 0) {
    ...
    long_log->printf("unit %d build firm %d at (%d,%d)\n", shortPtr[0],
                     shortPtr[3], shortPtr[1], shortPtr[2]);
#endif
    unit_array[*shortPtr]->build_firm(shortPtr[1], shortPtr[2], shortPtr[3],
                                      COMMAND_REMOTE);
  }
}
...
void RemoteMsg::unit_burn() {
  ...
  shortPtr[1], shortPtr[2]); #endif unit_array[*shortPtr]->burn(shortPtr[1],
  shortPtr[2], COMMAND_REMOTE);
          }
  */
}
...
void RemoteMsg::units_settle() {
  ...
  for (int i = 0; i < shortPtr[2]; ++i) {
    unit_array[shortPtr[3 + i]]->settle_town(shortPtr[0], shortPtr[1],
                                             COMMAND_REMOTE);
  }
}
...
void RemoteMsg::units_assign_to_ship() {
  /*
          ...
          // packet structure : <xLoc> <yLoc> <ship recno> <no. of units>
  <divided> <unit recno ...> short *shortPtr = (short *)data_buf;
          validate_selected_unit_array(shortPtr+5, shortPtr[3]);

          if( shortPtr[3] > 0)
          {
  #ifdef DEBUG
                  long_log->printf("assign to ship at (%d,%d), units : ",
  shortPtr[0], shortPtr[1]); for(int i = 0; i < shortPtr[3]; ++i)
                  {
                  ...
                  long_log->printf("\n");
  #endif
                  unit_array.assign_to_ship(shortPtr[0], shortPtr[1],
  shortPtr[4],  shortPtr+5, shortPtr[3], COMMAND_REMOTE, shortPtr[2]);
          }
  */
}
...
void RemoteMsg::units_ship_to_beach() {
  /*
          ...
          // packet structure : <xLoc> <yLoc> <no. of units> <divided> <unit
  recno ...> short *shortPtr = (short *)data_buf;
          validate_selected_unit_array(shortPtr+4, shortPtr[2]);

          if( shortPtr[2] > 0)
          {
  #ifdef DEBUG
                  long_log->printf("move ships to beach at (%d,%d), units : ",
  shortPtr[0], shortPtr[1]); for(int i = 0; i < shortPtr[2]; ++i)
                  {
                  ...
                  long_log->printf("\n");
  #endif
                  unit_array.ship_to_beach(shortPtr[0], shortPtr[1],
  shortPtr[3],  shortPtr+4, shortPtr[2], COMMAND_REMOTE);
          }
  */
}
// ------- End of function RemoteMsg::units_assign_to_ship ---------//
...
void RemoteMsg::unit_succeed_king() {
  ...
  if (unitCount > 0 && !nation_array.is_deleted(shortPtr[1]) &&
      ...
      unit_array[*shortPtr]
          ->can_succeed_king()) // prevent pressing succeed twice
  {
#ifdef DEBUG
  ...
  if (unitCount > 0) {
    ...
    if (unitPtr->unit_id != UNIT_CARAVAN) {
      ...
      long_log->printf("caravan %d set stop %d at (%d,%d)\n", shortPtr[0],
                       shortPtr[1], shortPtr[2], shortPtr[3]);
#endif
      caravanPtr = (UnitCaravan *)unitPtr;
      caravanPtr->set_stop(shortPtr[1], shortPtr[2], shortPtr[3],
                           COMMAND_REMOTE);
      // if( unit_array.selected_recno == *shortPtr )
      //	info.disp();
    }
  ...
  if (unitCount > 0) {
    ...
    if (unitPtr->unit_id == UNIT_CARAVAN) {
      ...
      long_log->printf("caravan %d set %s \n", shortPtr[0],
                       shortPtr[1] ? "auto" : "manual");
#endif
      caravanPtr = (UnitCaravan *)unitPtr;
      caravanPtr->default_market_trade_mode = (char)shortPtr[1];
      for (int i = 0; i < MAX_STOP_FOR_CARAVAN; ++i)
        if (caravanPtr->stop_array[i].firm_recno &&
            caravanPtr->stop_array[i].firm_id == FIRM_MARKET)
          caravanPtr->set_stop_pick_up(
              i + 1, shortPtr[1] ? AUTO_PICK_UP : MANUAL_PICK_UP, COMMAND_AUTO);
    } else {
      err_here();
    }
  }
...
void RemoteMsg::ship_unload_unit() {
  ...
  shortPtr[0], shortPtr[1]); #endif shipPtr->unload_unit(shortPtr[1],
  COMMAND_REMOTE); if( unit_array.selected_recno == *shortPtr ) info.disp();
                          }
                  }
                  else
                  {
...
void RemoteMsg::ship_unload_all_units() {
  ...
  shortPtr[0]); #endif UnitMarine *shipPtr = (UnitMarine *)unitPtr;
...
void RemoteMsg::ship_change_goods() {
  ...
  shortPtr[0], shortPtr[1], shortPtr[2]); #endif shipPtr = (UnitMarine
  *)unitPtr;
                          // shipPtr->stop_array[shortPtr[1]].pick_up_type =
  (char) shortPtr[2]; shipPtr->set_stop_pick_up(shortPtr[1], shortPtr[2],
  COMMAND_REMOTE);
                          // if( unit_array.selected_recno == *shortPtr )
                          //	info.disp();
                  }
...
void RemoteMsg::ship_set_stop() {
  ...
  shortPtr[0], shortPtr[1], shortPtr[2], shortPtr[3]); #endif shipPtr =
  (UnitMarine *)unitPtr; shipPtr->set_stop(shortPtr[1], shortPtr[2],
...
void RemoteMsg::ship_del_stop() {
  ...
  shortPtr[0], shortPtr[1]); #endif shipPtr = (UnitMarine *)unitPtr;
...
void RemoteMsg::god_cast() {
  ...
  // packet structure : <unit recno> <loc x> <loc y> <power type> <target base
  // obj recno>
  short *shortPtr = (short *)data_buf;
  ...
  if (unitCount > 0) {
    ...
    long_log->printf("god %d cast power %d at(%d,%d)\n", shortPtr[0],
                     shortPtr[3], shortPtr[1], shortPtr[2]);
#endif
    unitPtr->go_cast_power(shortPtr[1], shortPtr[2], COMMAND_REMOTE,
                           shortPtr[4]);
  }
}
...
void RemoteMsg::units_add_way_point() {
  ...
  shortPtr[1]); for(int i = 0; i < shortPtr[2]; ++i)
                  {
                  ...
                  long_log->printf("\n");
  #endif
                  unit_array.add_way_point(shortPtr[0], shortPtr[1], shortPtr+3,
  shortPtr[2], COMMAND_REMOTE); // 1-remote action
          }
  */
}
...
void RemoteMsg::units_transform_mfort() {
  ...
  if ((validate_selected_unit_array(shortPtr + 0, unitCount), unitCount > 0) &&
      (validate_selected_unit_array(shortPtr + 3, builderCount),
       builderCount == MAX_EXTRA_BUILDER)) {
    FirmMonsterFortress::transform_from_units(
        shortPtr[0], shortPtr[1], shortPtr[2], shortPtr + 3, COMMAND_REMOTE);
  }
}
...
void RemoteMsg::mobilize_worker() {
  ...
  if (validate_firm(*shortPtr)) {
    ...
    if (firmPtr->cast_to_FirmCamp()) {
      ...
      if (shortPtr[1] >= 1 && shortPtr[1] <= camp->soldier_count &&
          !camp->soldier_array[shortPtr[1] - 1].is_under_training()) {
      ...
      if (shortPtr[1] == -1 // mobilize all trainee
          || shortPtr[1] >= 1 && shortPtr[1] <= train->trainee_count &&
                 !train->trainee_array[shortPtr[1] - 1].is_under_training) {
...
#endif
      firm_array[*shortPtr]->toggle_firm_link(shortPtr[1], shortPtr[2],
                                              COMMAND_REMOTE);
    }
  }
}
...
void RemoteMsg::firm_toggle_link_town() {
  ...
  if (validate_firm(*shortPtr)) {
    ...
    if (shortPtr[1] <= firmPtr->linked_town_count &&
      ...
      //			if(
      //firmPtr->can_set_active_link(linkedTownRecno) )
      //			{
      //				if(
      //firmPtr->get_linked_town_status(linkedTownRecno) == LINK_EE )
      //				{
...
void RemoteMsg::firm_pull_town_people() {
  /*
          ...
          // packet structure : <firm recno> <town recno> <race Id or 0> <force
  Pull> short *shortPtr = (short *)data_buf;

          if( validate_firm(*shortPtr) && validate_town(shortPtr[1]) )
          {
  #ifdef DEBUG
                  long_log->printf("firm %d %s pull race %d from town %d\n",
  shortPtr[0], shortPtr[3] ? "forcely" : "", shortPtr[2], shortPtr[1]); #endif
                  firm_array[*shortPtr]->pull_town_people(shortPtr[1],
  COMMAND_REMOTE, shortPtr[2], shortPtr[3]);
          }
  */
}
...
void RemoteMsg::firm_set_worker_home() {
  ...
  shortPtr[0], shortPtr[2], shortPtr[1]); #endif
                  firm_array[*shortPtr]->set_worker_home_town(shortPtr[1],
  COMMAND_REMOTE, shortPtr[2]);
          }
  */
}
...
void RemoteMsg::firm_set_active_link() {
  ...
  if (validate_firm(*shortPtr) &&
      (!shortPtr[1] || validate_town(shortPtr[1]))) {
...
void RemoteMsg::firm_recruit() {
  ...
  if (validate_firm(*shortPtr)) {
    ...
    if (firmPtr->cast_to_FirmFort()) {
      firmPtr->cast_to_FirmFort()->recruit_soldier(
          shortPtr[1], shortPtr[2] != 0, COMMAND_REMOTE);
    } else if (firmPtr->cast_to_FirmTrain()) {
      firmPtr->cast_to_FirmTrain()->recruit_trainee(shortPtr[1], shortPtr[2],
                                                    COMMAND_REMOTE);
    } else if (firmPtr->cast_to_FirmLair()) {
      firmPtr->cast_to_FirmLair()->recruit_soldier(COMMAND_REMOTE);
    } else if (firmPtr->cast_to_FirmMonsterTrain()) {
      firmPtr->cast_to_FirmMonsterTrain()->recruit_trainee(shortPtr[2],
                                                           COMMAND_REMOTE);
    } else {
      err_here();
    }
...
void RemoteMsg::firm_worker_count() {
  ...
  if (validate_firm(*shortPtr)) {
    ...
    if (firmPtr->cast_to_FirmWork()) {
      firmPtr->cast_to_FirmWork()->set_needed_worker_count(shortPtr[1],
                                                           COMMAND_REMOTE);
    } else {
      err_here();
    }
...
void RemoteMsg::firm_set_rally_dest() {
  ...
  if (validate_firm(*shortPtr)) {
    firm_array[*shortPtr]->set_rally_point(shortPtr[1], shortPtr[2],
                                           shortPtr[3], COMMAND_REMOTE);
  }
}
...
void RemoteMsg::camp_patrol() {
  ...
  if (validate_firm(*shortPtr)) {
    ...
    FirmMonsterFortress *fortress =
        firm_array[*shortPtr]->cast_to_FirmMonsterFortress();
    if (camp) {
  ...
  if (validate_firm(*shortPtr)) {
    ...
    if (inn) {
      ...
      if (shortPtr[1] >= 1 && shortPtr[1] <= animal->animal_unit_count) {
        ...
        //				if( shortPtr[2] ==
        //nation_array.player_recno) 					inn->put_info(INFO_REPAINT);
      }
    } else {
      err_here();
    }
...
void RemoteMsg::research_start() {
  /*
          ...
          if( validate_firm(*shortPtr) )
          {
                  FirmResearch *research =
  firm_array[*shortPtr]->cast_to_FirmResearch(); if(research)
                  {
  #ifdef DEBUG
                          long_log->printf("tower of science %d start research
  tech %d\n", shortPtr[0], shortPtr[1]); #endif
...
void RemoteMsg::build_ship() {
  /*
          ...
          {
                  FirmHarbor *harbor =
  firm_array[*shortPtr]->cast_to_FirmHarbor(); if(!harbor)
                  {
                          err_here();
                  }
                  else
                  {
  #ifdef DEBUG
                          long_log->printf("harbor %d start building unit id
  %d\n", shortPtr[0], shortPtr[1]); #endif
...
void RemoteMsg::sail_ship() {
  ...
  shortPtr[1], shortPtr[0]); #endif harbor->sail_ship(shortPtr[1],
  COMMAND_REMOTE);
                  }
          }
  */
...
void RemoteMsg::skip_build_ship() {
  ...
  shortPtr[0]); #endif harbor->cancel_build_unit();
                  }
          }
  */
}
...
void RemoteMsg::cancel_train() {
  ...
  if (validate_firm(*shortPtr)) {
    Firm *firmPtr = firm_array[*shortPtr];
    if (firmPtr->cast_to_FirmCamp()) {
      firmPtr->cast_to_FirmCamp()->cancel_train_soldier(shortPtr[1],
                                                        COMMAND_REMOTE);
    } else if (firmPtr->cast_to_FirmTrain()) {
      firmPtr->cast_to_FirmTrain()->cancel_train_trainee(shortPtr[1],
                                                         COMMAND_REMOTE);
    } else if (firmPtr->cast_to_FirmMonsterTrain()) {
      firmPtr->cast_to_FirmMonsterTrain()->cancel_train_trainee(shortPtr[1],
                                                                COMMAND_REMOTE);
    } else {
      err_here();
    }
...
void RemoteMsg::change_conversion() {
  ...
  short *shortPtr = (short *)data_buf;
  if (validate_firm(*shortPtr)) {
    FirmMonsterAlchemy *alchemy =
        firm_array[*shortPtr]->cast_to_FirmMonsterAlchemy();
    if (alchemy) {
...
void RemoteMsg::magic_cast() {
  ...
  if (validate_firm(*shortPtr)) {
    ...
    if (magic) {
      magic->cast_magic(shortPtr[1], shortPtr[2], shortPtr[3], shortPtr[4],
                        COMMAND_REMOTE);
    } else {
      err_here();
    }
  ...
  if (validate_town(*shortPtr)) {
    ...
    if (shortPtr[1] <= townPtr->linked_town_count &&
      ...
      long_log->printf("town %d %s town link %d\n", shortPtr[0],
                       shortPtr[2] ? "set" : "clear", shortPtr[1]);
#endif
      town_array[*shortPtr]->toggle_town_link(shortPtr[1], shortPtr[2],
                                              COMMAND_REMOTE);
    }
  }
}
...
void RemoteMsg::unit_auto_retreat() {
  ...
  if (*shortPtr > 0) {
    ...
    if (unit_array[*shortPtr]->rank_id == RANK_KING)
      long_log->printf(
          "unit King %d auto retreats when hit point drops to %d percentage\n",
          shortPtr[0], shortPtr[1]);
    else
      long_log->printf("unit General %d auto retreats when hit point drops to "
                       "%d percentage\n",
                       shortPtr[0], shortPtr[1]);
#endif
    ...
    err_when(!nationRecno);
#ifdef DEBUG
    long_log->printf("nation %d auto retreats all generals when their hit "
                     "point drops to %d percentage\n",
...
void RemoteMsg::wall_build() {
  ...
  shortPtr[0], shortPtr[1], shortPtr[2]); #endif world.build_wall_tile(
  shortPtr[1], shortPtr[2], shortPtr[0], COMMAND_REMOTE);
          }
  */
}
...
void RemoteMsg::wall_destruct() {
  ...
  shortPtr[0], shortPtr[1], shortPtr[2]); #endif world.destruct_wall_tile(
  shortPtr[1], shortPtr[2], shortPtr[0], COMMAND_REMOTE);
          }
  */
}
...
void RemoteMsg::spy_create_incident() {
  ...
  // packet structure : <spy recno> <target nation recno> <involved nation
  // recno>
  short *shortPtr = (short *)data_buf;
  ...
  if (!spy_array.is_deleted(*shortPtr) &&
    ...
    spy_array[*shortPtr]->create_incident(shortPtr[1], shortPtr[2],
                                          COMMAND_REMOTE);
  }
}
...
void RemoteMsg::send_talk_msg() {
  ...
  if (!nation_array.is_deleted(talkMsg->to_nation_recno)) {
    ...
    long_log->printf("talk message from %d to %d, id %d, para1=%d, para2=%d\n",
                     ...
                     talkMsg->talk_id, talkMsg->talk_para1,
                     talkMsg->talk_para2);
#endif
...
void RemoteMsg::reply_talk_msg() {
  ...
  if (!talk_res.is_talk_msg_deleted(talkMsgRecno)) {
    ...
    long_log->printf("reply talk message %d, %d\n", talkMsgRecno,
                     data_buf[sizeof(int)]);
#endif
    talk_res.reply_talk_msg(talkMsgRecno, data_buf[sizeof(int)],
                            COMMAND_REMOTE);
  }
...
void RemoteMsg::nation_set_should_attack() {
  ...
  if (!nation_array.is_deleted(*shortPtr) &&
    ...
    long_log->printf("nation %d %s instruct attack nation %d\n", shortPtr[0],
                     shortPtr[2] ? "set" : "clear", shortPtr[1]);
#endif
    nation_array[shortPtr[0]]->set_relation_should_attack(
        shortPtr[1], (char)shortPtr[2], COMMAND_REMOTE);
  }
}
...
void RemoteMsg::ship_selected() {
  /*
          ...
          #ifdef DEBUG
                  Unit *unitPtr = unit_array[*shortPtr];
                  err_when(unit_res[unitPtr->unit_id]->unit_class !=
     UNIT_CLASS_SHIP || unitPtr->unit_id==UNIT_TRANSPORT); #endif
...
//
// Packet structure : <to nation recno> <from nation recno>
// <char[CHAT_STR_LEN+1]>
//
void RemoteMsg::chat() {
  short *shortPtr = (short *)data_buf;
...
void RemoteMsg::camp_promote() {
  ...
  if (validate_firm(*shortPtr)) {
    if (firm_array[*shortPtr]->cast_to_FirmCamp()) {
      firm_array[*shortPtr]->cast_to_FirmCamp()->promote_soldier(
          shortPtr[1], COMMAND_REMOTE);
    } else {
      err_here();
    }
  }
