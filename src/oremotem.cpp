...
void RemoteMsg::new_nation()
{
    ...
    int nationRecno = nation_array.new_nation(NATION_HUMAN, NATION_REMOTE, nationPtr->race_id,
...
void RemoteMsg::unit_assign()
{
    ...
    if (unit_group_remote.validate_unit_array(remote.nation_processing) > 0)
    {
        unit_group_remote.assign(shortPtr[0], shortPtr[1], shortPtr[2], COMMAND_REMOTE, shortPtr[3], shortPtr[4] != 0);
    }
}
...
static void validate_selected_unit_array(short *selectedUnitArray, short &selectedUnitCount)
{
    ...
    {
        Unit *unitPtr;
        if (unit_array.is_deleted(selectedUnitArray[i]) || !(unitPtr = unit_array[selectedUnitArray[i]]) ||
...
static short validate_firm(short firmRecno, unsigned flags)
{
    ...
    Firm *firmPtr;
    if (firmRecno && !firm_array.is_deleted(firmRecno) && (firmPtr = firm_array[firmRecno]) &&
...
static short validate_town(short townRecno, unsigned flags)
{
    Town *townPtr;
    err_when(!(flags & 1) && remote.nation_processing == 0);
    if (townRecno && !town_array.is_deleted(townRecno) && (townPtr = town_array[townRecno]) &&
...
#endif
        unit_array[*shortPtr]->build_firm(shortPtr[1], shortPtr[2], shortPtr[3], COMMAND_REMOTE);
    }
}
...
void RemoteMsg::unit_resign()
{
    ...
    if (unitCount > 0 && unit_array[*shortPtr]->is_nation(shortPtr[1]) && unit_array[*shortPtr]->can_resign())
    {
#ifdef DEBUG
...
void RemoteMsg::unit_succeed_king()
{
    ...
    if (unitCount > 0 && !nation_array.is_deleted(shortPtr[1]) && unit_array[*shortPtr]->nation_recno == shortPtr[1] &&
...
void RemoteMsg::change_spy_nation()
{
    ...
    if (unitCount > 0)
    {
        ...
        if (unit_array.selected_recno == *shortPtr || unit_array[*shortPtr]->selected_flag)
            info.disp();
    }
}
...
#endif
            spy_array[unit_array[*shortPtr]->spy_recno]->notify_cloaked_nation_flag = (char)shortPtr[1];
        }
        if (unit_array.selected_recno == *shortPtr || unit_array[*shortPtr]->selected_flag)
            info.disp();
    }
...
void RemoteMsg::firm_toggle_link_firm()
{
    ...
    if (validate_firm(*shortPtr))
    {
        ...
        if (shortPtr[1] <= firmPtr->linked_firm_count &&
            (linkedFirmRecno = firmPtr->linked_firm_array[shortPtr[1] - 1]) && validate_firm(linkedFirmRecno, 1))
        {
#ifdef DEBUG
            long_log->printf("firm %d %s firm link %d\n", shortPtr[0], shortPtr[2] ? "set" : "clear", shortPtr[1]);
#endif
...
void RemoteMsg::firm_toggle_link_town()
{
    ...
    if (validate_firm(*shortPtr))
    {
        ...
        if (shortPtr[1] <= firmPtr->linked_town_count &&
            (linkedTownRecno = firmPtr->linked_town_array[shortPtr[1] - 1]) && validate_town(linkedTownRecno, 1))
        {
#ifdef DEBUG
...
void RemoteMsg::firm_bribe()
{
    ...
    if (validate_firm(*shortPtr, 1) && !spy_array.is_deleted(shortPtr[1]))
    {
        ...
        {
            FirmCamp *camp = firmPtr->cast_to_FirmCamp();
            if (shortPtr[2] == 0 && camp->overseer_recno || shortPtr[2] >= 1 && shortPtr[2] <= camp->soldier_count)
            {
...
void RemoteMsg::firm_reward()
{
    ...
    if (validate_firm(*shortPtr))
    {
        ...
        {
            FirmCamp *camp = firmPtr->cast_to_FirmCamp();
            if (shortPtr[1] == 0 && camp->overseer_recno || shortPtr[1] >= 1 && shortPtr[1] <= camp->soldier_count)
            {
...
void RemoteMsg::town_toggle_link_firm()
{
    ...
    if (validate_town(*shortPtr))
    {
        ...
        if (shortPtr[1] <= townPtr->linked_firm_count &&
            (linkedFirmRecno = townPtr->linked_firm_array[shortPtr[1] - 1]) && validate_firm(linkedFirmRecno, 1))
        {
...
void RemoteMsg::town_toggle_link_town()
{
    ...
    if (validate_town(*shortPtr))
    {
        ...
        if (shortPtr[1] <= townPtr->linked_town_count &&
            (linkedTownRecno = townPtr->linked_town_array[shortPtr[1] - 1]) && validate_town(linkedTownRecno, 1))
        {
#ifdef DEBUG
...
void RemoteMsg::send_talk_msg()
{
    ...
    {
#ifdef DEBUG
        long_log->printf("talk message from %d to %d, id %d, para1=%d, para2=%d\n", talkMsg->from_nation_recno,
                         talkMsg->to_nation_recno, talkMsg->talk_id, talkMsg->talk_para1, talkMsg->talk_para2);
#endif
...
void RemoteMsg::compare_remote_object()
{
    ...
    if ((remote.sync_test_level & 2) && (remote.sync_test_level >= 0) && crc_store.compare_remote(id, data_buf))
    {
        remote.sync_test_level = ~remote.sync_test_level; // signal error encountered
        if (sys.debug_session)
            err.run("Multiplayer Random Seed Sync Error.");
...
void RemoteMsg::camp_swap_item()
{
    ...
    if (validate_firm(*shortPtr))
    {
        if (firm_array[*shortPtr]->cast_to_FirmCamp())
        {
            firm_array[*shortPtr]->cast_to_FirmCamp()->swap_item(shortPtr[1], shortPtr[2], shortPtr[3], COMMAND_REMOTE);
        }
        else if (firm_array[*shortPtr]->cast_to_FirmMonsterFortress())
        {
            firm_array[*shortPtr]->cast_to_FirmMonsterFortress()->swap_item(shortPtr[1], shortPtr[2], shortPtr[3],
                                                                            COMMAND_REMOTE);
        }
        else
        {
