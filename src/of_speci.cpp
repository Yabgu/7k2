...
//
void FirmSpecial::detect_info() { FirmTrain::detect_info(); }
...
void FirmSpecial::disp_firm_info(int dispY1, int refreshFlag) {
  ...
  font_snds.center_put(INFO_X1 + 88, INFO_Y1 + 15, INFO_X1 + 217, INFO_Y1 + 74,
                       unit_res[unitId]->name, 0, 0);

  if (!should_show_info())
    return;

  if (is_own()) {
    button_train.enable_flag =
...
void FirmSpecial::detect_firm_info() {
  ...
  if (rc) {
    ...
    if (!town_array.is_deleted(active_link_town_recno) &&
      ...
      err_when(raceCount >
               1); // if race_id is specified, only one town is returned
      if (rc == 1 && raceCount == 1) {
...
int FirmSpecial::represent_attribute(int traineeId) {
  ...
  //	return trainee_array[traineeId-1].skill.combat_level;		// don't
  //display show_combat_level
