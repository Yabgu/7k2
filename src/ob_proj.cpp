...
void Projectile::init(char parentType, short parentRecno, short targetXLoc, short targetYLoc, char targetMobileType)
{
    ...
    if( dz >= 10.0)
            cur_dir = cur_dir & 7 | 8;					//
    pointing up else if( dz <= -10.0) cur_dir = cur_dir & 7 | 16;
    // pointing down else cur_dir &= 7;
    */
...
short Projectile::get_z()
{
    ...
    return a * cur_step * (cur_step - total_step - 1) + (z_dest - z_init) * cur_step / (total_step + 1) + z_init;
}
