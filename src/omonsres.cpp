...
MonsterRes::MonsterRes() { init_flag = 0; }
...
void MonsterRes::load_monster_info() {
  ...
  monster_info_array =
      (MonsterInfo *)mem_add(sizeof(MonsterInfo) * monster_count);
  ...
  for (i = 0; i < monster_count; i++) {
    ...
    monsterInfo->unit_id =
        misc.atoi(monsterRec->unit_id, monsterRec->UNIT_ID_LEN);
    monsterInfo->level = monsterRec->level - '0';

    misc.rtrim_fld(monsterInfo->firm_build_code, monsterRec->firm_build_code,
                   monsterRec->FIRM_BUILD_CODE_LEN);
...
void MonsterRes::second_init() {
  // ###### begin Gilbert 14/3 ########//
  long backupSeed = misc.get_random_seed(); // backup random seed in order to
  ...
  MonsterNamePart *nameArrayA =
      (MonsterNamePart *)mem_add(sizeof(MonsterNamePart) * nameACount);
  ...
  MonsterNamePart *nameArrayB =
      (MonsterNamePart *)mem_add(sizeof(MonsterNamePart) * nameBCount);
  ...
  MonsterNamePart *nameArrayC =
      (MonsterNamePart *)mem_add(sizeof(MonsterNamePart) * nameCCount);
  ...
  static const char *nameFormatArray[MONSTER_NAME_COUNT_FORMAT] = {
      "ABABC", "ABC", "ABCABC", "ABAB", "BCBC", "BCABC"};
  ...
  for (i = 0; i < MONSTER_NAME_COUNT; i++) {
    const char *nameFormat =
        nameFormatArray[misc.random(MONSTER_NAME_COUNT_FORMAT)];
