...
void FirmLair::disp_camp_info(int dispY1, int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    // ######## begin Gilbert 31/12 ######//
    //		button_train.create( INFO_X1+13+BUTTON_DISTANCE*2, INFO_Y1+281, 'A',
    //"TRAIN" ); 		button_select_research.create( INFO_X1+13+BUTTON_DISTANCE,
    //INFO_Y1+281, 'A', "RESEARCH" );

    button_train.create(INFO_X1 + 13, INFO_Y1 + 281, 'A', "F_TRAIN");
    button_select_research.create(INFO_X1 + 13 + 3 * BUTTON_DISTANCE,
                                  INFO_Y1 + 281, 'A', "F_RESE");
  ...
  if (nation_recno) {
    if (is_own()) {
      ...
      button_select_research.enable_flag =
          tech_res.researchable_tech_count(nation_recno, race_id, 0) >
          0; // 0-don't check live points
      button_select_research.paint();
    }
  } else {
    // ###### begin Gilbert 30/12 ######//
    if ((button_grant.visible_flag = (char)nation_array.player_recno)) {
      button_grant.enable_flag = can_grant_to_non_own_lair(
          nation_array.player_recno,
          0); // 0-don't check if the player has enough cash or not
    }
    // ###### end Gilbert 30/12 ######//
    button_grant.paint();
  }
  ...
  if (nation_recno) {
    if (is_own()) {
      ...
      // ###### begin Gilbert 26/1 #######//
        ...
        if (soldier_count > 0) {
          cancel_train_soldier(soldier_count,
                               COMMAND_PLAYER); // cancel the last soldier
...
void FirmLair::disp_research_menu(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    for (int techId = 1; techId <= tech_res.tech_count &&
                         added_count < MAX_LAIR_RESEARCH_OPTION;
         ++techId) {
      TechInfo *techInfo = tech_res[techId];

      if (techInfo->race_id == race_id &&
          tech_res[techId]->can_research(nation_recno, 0)) {
        button_research_array[added_count].create(
            x, y + added_count * ySpacing, x + 187,
            y + added_count * ySpacing + 34, i_disp_research_button,
  ...
  for (int i = 0; i < added_count; ++i) {
    ...
    button_research_array[i].enable_flag =
        nation_recno && tech_res[techId]->can_research(nation_recno);
...
void i_disp_research_button(ButtonCustom *button, int repaintBody) {
  ...
  if (!button->enable_flag) {
    ...
    font_bld.center_put(
        button->x1, midY, button->x2, button->y2 - 2,
    ...
    vga.active_buf->bar(button->x1, button->y1, button->x2, button->y2,
                        V_WHITE);
    vga.active_buf->draw_d3_up_border(button->x1, button->y1, button->x2,
                                      button->y2);
    font_bld.center_put(button->x1, button->y1 + 2, button->x2, midY, str);
    font_bld.center_put(
        button->x1, midY, button->x2, button->y2 - 2,
...
void FirmLair::disp_grant_menu(int refreshFlag) {
  ...
  font_whbl.center_put_paragraph(
      INFO_X1 + 40, INFO_Y1, INFO_X2 - 30, INFO_Y1 + 100,
      text_firm.str_buy_lair(
          grant_to_non_own_lair_cost(),
          can_grant_to_non_own_lair(nation_array.player_recno, 1)),
      2, 0, 0);

  if (!can_grant_to_non_own_lair(nation_array.player_recno, 1)) {
    //		str = "You do not have enough money!";
    //		font_whbl.center_put_paragraph( INFO_X1 +20 , INFO_Y1 + 80, INFO_X2
    //-10, INFO_Y1 +100, str, 2, 0, 0 );
    font_whbl.center_put_paragraph(
  ...
  if (can_grant_to_non_own_lair(nation_array.player_recno, 1)) {
    button_yes.paint_text(INFO_X1 + 50, INFO_Y1 + 100,
                          text_firm.str_yes()); // "Yes" );
    button_no.paint_text(INFO_X1 + 155, INFO_Y1 + 100,
                         text_firm.str_no()); //  "No" );
  } else {
    button_yes.reset();
    button_no.paint_text(INFO_X1 + 102, INFO_Y1 + 122,
...
int FirmLair::should_show_info() {
  if (FirmCamp::should_show_info() ||
      can_grant_to_non_own_lair(nation_array.player_recno, 0))
    return 1;
  else
    return 0;
}
