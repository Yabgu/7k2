...
void Unit::draw(int, int drawFlag)
{
    ...
    world.zoom_matrix->put_bitmap_offset(cur_x, cur_y, get_z(), bitmapPtr, spriteFrame->offset_x, spriteFrame->offset_y,
                                         colorRemapTable, needMirror,
                                         (sprite_info->remap_bitmap_flag ? 2 : 1) | (drawFlag << 2));

    if (is_freezed())
    {
        if (((Bitmap *)bitmapPtr)->get_height() < 66 && ((Bitmap *)bitmapPtr)->get_width() < 46)
        {
            bitmapPtr = image_spict.get_ptr("ICE");
            world.zoom_matrix->put_bitmap_offset(cur_x, cur_y, get_z(),
                                                 //	bitmapPtr, -((Bitmap *)bitmapPtr)->get_width()/2,
                                                 // spriteFrame->offset_y,
                                                 bitmapPtr, -((Bitmap *)bitmapPtr)->get_width() / 2,
                                                 -((Bitmap *)bitmapPtr)->get_height() + 25, NULL, 0, 5);
        }
        else if (((Bitmap *)bitmapPtr)->get_height() < 93)
        {
            bitmapPtr = image_spict.get_ptr("ICE2");
            world.zoom_matrix->put_bitmap_offset(cur_x, cur_y, get_z(), bitmapPtr,
                                                 -((Bitmap *)bitmapPtr)->get_width() / 2, spriteFrame->offset_y - 10,
                                                 //	bitmapPtr, -((Bitmap *)bitmapPtr)->get_width()/2, -((Bitmap
                                                 //*)bitmapPtr)->get_height()+50,
                                                 NULL, 0, 5);
        }
        else
        {
            ...
            world.zoom_matrix->put_bitmap_offset(cur_x, cur_y, get_z(), bitmapPtr,
                                                 20 - ((Bitmap *)bitmapPtr)->get_width() / 2,
                                                 spriteFrame->offset_y - 20,
                                                 //	bitmapPtr, -((Bitmap *)bitmapPtr)->get_width()/2, -((Bitmap
                                                 //*)bitmapPtr)->get_height()+50,
                                                 NULL, 0, 5);
        }
    ...
    else if ((is_own() || config.show_ai_info || skill_level() > 0) && config.show_all_unit_icon)
        draw_rank_icon();
    ...
    if (max_power < 0)
    {
        if ((config.frame_speed != 0) && (sys.frame_count % 2 == abs(max_power % 2)))
            max_power--;
        world.zoom_matrix->put_center_text(cur_x, cur_y, get_z() - 12 * ((max_power) + 10), misc.format(cur_power, 2),
                                           0, &font_sop);
    }
    ...
    case UNIT_RHINO: {
        ...
        world.zoom_matrix->put_bitmap_offset(cur_x, cur_y, get_z(), bitmapPtr, -((Bitmap *)bitmapPtr)->get_width() >> 1,
                                             spriteInfo->max_height +
                                                 (spriteInfo->loc_width + spriteInfo->loc_height) * 6 -
                                                 ((Bitmap *)bitmapPtr)->get_height(),
...
#define HIT_BAR_TYPE_COUNT 3
    ...
    int pointX = (curBarWidth - 1) * (int)hit_points / (int)max_hit_points(); // the separating point between the area
                                                                              // with hit point and the area without
    // ######## begin Gilbert 4/5 ########//
    if (pointX < 0)
        pointX = 0;
    // ######## end Gilbert 4/5 ########//
    ...
    IMGbar(dataPtr, truePitch, 0, 0, pointX, 0,
           vga_back.translate_color(hitBarColor + HIT_BAR_LIGHT_BORDER)); // top - with hit

    if (pointX < curBarWidth - 1)
        IMGbar(dataPtr, truePitch, pointX + 1, 0, curBarWidth - 1, 0,
               vga_back.translate_color(NO_BAR_LIGHT_BORDER)); // top - without hit

    IMGbar(dataPtr, truePitch, 0, 0, 0, 2,
           vga_back.translate_color(hitBarColor + HIT_BAR_LIGHT_BORDER)); // left - with hit

    IMGbar(dataPtr, truePitch, 1, 2, pointX, 2,
           vga_back.translate_color(hitBarColor + HIT_BAR_DARK_BORDER)); // bottom - with hit

    if (pointX < curBarWidth - 1)
        IMGbar(dataPtr, truePitch, pointX + 1, 2, curBarWidth - 1, 2,
               vga_back.translate_color(NO_BAR_DARK_BORDER)); // bottom - without hit

    if (pointX == curBarWidth - 1)
        IMGbar(dataPtr, truePitch, curBarWidth - 1, 1, curBarWidth - 1, 1,
               vga_back.translate_color(hitBarColor + HIT_BAR_DARK_BORDER)); // right -with hit
    else
        IMGbar(dataPtr, truePitch, curBarWidth - 1, 1, curBarWidth - 1, 1,
               vga_back.translate_color(NO_BAR_DARK_BORDER)); // right -without hit

    IMGbar(dataPtr, truePitch, 1, 1, MIN(pointX, curBarWidth - 2), 1,
           vga_back.translate_color(hitBarColor + HIT_BAR_BODY)); // bar body

    if (pointX < curBarWidth - 2)
        IMGbar(dataPtr, truePitch, pointX + 1, 1, curBarWidth - 2, 1,
               vga_back.translate_color(NO_BAR_BODY)); // bar body

    //--------- display the bar now ---------//

    SpriteInfo *spriteInfo = sprite_res[unit_res[unit_id]->sprite_id];

    world.zoom_matrix->put_bitmapW_offset(cur_x, cur_y, get_z(), (short *)sys.common_data_buf, -maxHitBarWidth / 2,
                                          spriteInfo->max_height + (spriteInfo->loc_width + spriteInfo->loc_height) * 6,
                                          0, 0);

    //----- display rank icon (only for own units) -----//

    //	if( is_own() || config.show_ai_info || skill_level() > 0 || rank_id >
    // RANK_SOLDIER )
...
void Unit::draw_rank_icon()
{
    ...
    /*	if( is_own() )
            {
                    ...
                    if(*str)
                    {
                            ...
                            world.zoom_matrix->put_bitmap_offset( cur_x, cur_y,
       get_z(), iconPtr, -maxHitBarWidth/2 - ((Bitmap *)iconPtr)->get_width(),
                                    spriteInfo->max_height +
       (spriteInfo->loc_width + spriteInfo->loc_height) * 6, NULL, 0, 0);

                            yOffset += ((Bitmap *)iconPtr)->get_height();
       // put spy icon lower
                    }
            }
    ...
    if (!iconName)
    {
        if (spy_recno && (true_nation_recno() == nation_array.player_recno || config.show_ai_info))
        {
            iconName = "U_SPY";
        }
        else if (hero_id && is_own())
    ...
    {
        world.zoom_matrix->put_bitmap_offset(cur_x, cur_y, get_z(), iconPtr, -((Bitmap *)iconPtr)->get_width() / 2,
                                             spriteInfo->max_height +
                                                 (spriteInfo->loc_width + spriteInfo->loc_height) * 6 -
                                                 ((Bitmap *)iconPtr)->get_height(),
                                             NULL, 0, 0);
    }
    ...
    if (is_own() || config.show_ai_info)
    {
        if (item.id && (iconPtr = item_res.item_unit_interface(item.id)))
            world.zoom_matrix->put_bitmap_offset(
                cur_x, cur_y, get_z(), iconPtr, -maxHitBarWidth / 2 - ((Bitmap *)iconPtr)->get_width(),
                spriteInfo->max_height + (spriteInfo->loc_width + spriteInfo->loc_height) * 6 -
                    ((Bitmap *)iconPtr)->get_height() / 2,
                NULL, 0, 0);
    }
    ...
    if ((is_own() || config.show_ai_info) && is_royal)
    {
        ...
        if (iconPtr)
            world.zoom_matrix->put_bitmap_offset(cur_x, cur_y, get_z(), iconPtr, maxHitBarWidth / 2,
                                                 spriteInfo->max_height +
                                                     (spriteInfo->loc_width + spriteInfo->loc_height) * 6 -
                                                     ((Bitmap *)iconPtr)->get_height() / 2,
                                                 NULL, 0, 0);
