...
void Unit::draw(int, int drawFlag) {
  ...
  short *colorRemapTable =
      sprite_info->get_color_remap_table(nation_recno, 0); // selected_flag);
  ...
  world.zoom_matrix->put_bitmap_offset(
      cur_x, cur_y, get_z(), bitmapPtr, spriteFrame->offset_x,
      spriteFrame->offset_y, colorRemapTable, needMirror,
  ...
  else if ((is_own() || config.show_ai_info || skill_level() > 0) &&
           config.show_all_unit_icon)
    draw_rank_icon();

  SpriteInfo *spriteInfo = sprite_res[unit_res[unit_id]->sprite_id];
  if (max_power < 0) {
    if ((config.frame_speed != 0) &&
        (sys.frame_count % 2 == abs(max_power % 2)))
      max_power--;
    world.zoom_matrix->put_center_text(cur_x, cur_y,
                                       get_z() - 12 * ((max_power) + 10),
                                       misc.format(cur_power, 2), 0, &font_sop);
  }
...
#define HIT_BAR_TYPE_COUNT 3
  ...
  enum { HIT_BAR_LIGHT_BORDER = 0, HIT_BAR_DARK_BORDER = 3, HIT_BAR_BODY = 1 };
  ...
  int pointX = (curBarWidth - 1) * (int)hit_points /
               (int)max_hit_points(); // the separating point between the area
  ...
  IMGbar(dataPtr, truePitch, 0, 0, pointX, 0,
         vga_back.translate_color(hitBarColor +
                                  HIT_BAR_LIGHT_BORDER)); // top - with hit
  ...
  world.zoom_matrix->put_bitmapW_offset(
      cur_x, cur_y, get_z(), (short *)sys.common_data_buf, -maxHitBarWidth / 2,
      spriteInfo->max_height +
          (spriteInfo->loc_width + spriteInfo->loc_height) * 6,
      0, 0);
...
void Unit::draw_rank_icon() {
  ...
  if (iconName && (iconPtr = image_icon.get_ptr(iconName))) {
    world.zoom_matrix->put_bitmap_offset(
        cur_x, cur_y, get_z(), iconPtr, -((Bitmap *)iconPtr)->get_width() / 2,
        spriteInfo->max_height +
            (spriteInfo->loc_width + spriteInfo->loc_height) * 6 -
  ...
  if ((is_own() || config.show_ai_info) && is_royal) {
    ...
    if (iconPtr)
      world.zoom_matrix->put_bitmap_offset(
          cur_x, cur_y, get_z(), iconPtr, maxHitBarWidth / 2,
          spriteInfo->max_height +
              (spriteInfo->loc_width + spriteInfo->loc_height) * 6 -
              ((Bitmap *)iconPtr)->get_height() / 2,
          NULL, 0, 0);
