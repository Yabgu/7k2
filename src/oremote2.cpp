...
// <int>   dataSize    - the data size of the RemoteMsg needed (size of
// RemoteMsg::data_buf)
//
// Data structure:
//
...
RemoteMsg *Remote::new_msg(int msgId, int dataSize) {
  ...
  //--- use common_msg_buf, if the requested one is not bigger than
  //common_msg_buf ---//
  ...
  } else //---- allocate a new RemoteMsg if the requested one is bigger than
         //common_msg_buf ----//
  {
...
void Remote::free_msg(RemoteMsg *remoteMsgPtr) {
  ...
  if (memPtr != common_msg_buf) // the real allocation block start 2 bytes
                                // before remoteMsgPtr
    mem_del(memPtr);
}
...
void Remote::send_msg(RemoteMsg *remoteMsgPtr, int receiverId) {
  ...
  char *memPtr =
      (char *)remoteMsgPtr -
      sizeof(short); // the preceeding <short> is the size of RemoteMsg
...
char *Remote::new_send_queue_msg(int msgId, int dataSize) {
  char *dataPtr =
      send_queue[0].reserve(sizeof(short) + sizeof(DWORD) + dataSize);
...
int Remote::send_queue_now(int receiverId) {
  ...
  //----------------------------------------------//
  int sendFlag = 1;
  // if(!mp_ptr->send( receiverId, send_queue[0].queue_buf,
  // send_queue[0].length()))
  if (ec_remote.send(ec_remote.get_ec_player_id(receiverId),
                     send_queue[0].queue_buf, send_queue[0].length()) <= 0)
    sendFlag = 0;
  packet_send_count++;
...
#define LAST_FRAME_HISTORY 8
  ...
  while ((recvBuf = ec_remote.receive(&from, (long unsigned int *)&recvLen)) !=
         NULL) {
    ...
    /*
                    if( !power.enable_flag )		// power.enable_flag
       determines whether the multiplayer game has started or not
                    {
                            int validateLen = receive_queue[0].length();
                            memcpy( receive_queue[0].reserve(msgListSize),
       recvBuf, msgListSize ); if( !receive_queue[0].validate_queue(validateLen)
    ...
    */
    ...
    //--- if message id !=MSG_QUEUE_HEADER, this packet is sent by
    //send_free_msg(), not by send_queue_now() ---//
...
void Remote::process_receive_queue() {
  ...
  //---- if not started yet, process the message without handling the message
  //sequence ----//

  if (!power.enable_flag) {
    for (rqt.traverse_set_start(); !rqt.traverse_finish();
         rqt.traverse_next()) {
      err_when(++loopCount > 1000);
  ...
  {
    LOG_BEGIN;
    for (int nationRecno = 1; nationRecno <= nation_array.size();
         ++nationRecno) {
      ...
      for (rqt.traverse_set_start(); !rqt.traverse_finish();
        ...
        if (remoteMsgPtr->id == MSG_QUEUE_HEADER) {
          ...
          if (msgNation == nationRecno) // struct of data_buf: <DWORD>
                                        // frameCount + <short> nationRecno
          {
            processFlag = 1;
          } else //--- if this is a message queue that should be processed now
                 //---//
          {
            if (processFlag) //--- if we were previously processing the right
                             //queue, now the processing is complete by this
                             //point ---//
          ...
          if (msgNation ==
              nationRecno) // struct of data_buf:<short> nationRecno
   ...
   while( processedSize < queuedSize )
        {
                ...
                if( remoteMsgPtr->id )
                {
                        if( remoteMsgPtr->id<FIRST_REMOTE_MSG_ID ||
remoteMsgPtr->id>LAST_REMOTE_MSG_ID ) return 0;
                }
...
//                     (default: 0)
//
// <int> requestFrameCount - need to send the backup buffer of the requested
// frame count
//
// return : <int> 1 - sent successfully, the message has been received
//                    by the receivers.
...
int Remote::send_backup_now(int receiverId, DWORD requestFrameCount) {
  ...
  for (int n = 1; n < SEND_QUEUE_BACKUP; ++n) {
    if (requestFrameCount == send_frame_count[n]) {
      ...
      if (send_queue[n].length() > 0) {
        if (handle_vga_lock)
          vga_front.temp_unlock();
        // retFlag = mp_ptr->send(receiverId, send_queue[n].queue_buf,
        // send_queue[n].length());
        retFlag =
            ec_remote.send(ec_remote.get_ec_player_id(receiverId),
                           send_queue[n].queue_buf, send_queue[n].length()) > 0;
        packet_send_count++;
        if (handle_vga_lock)
          vga_front.temp_restore_lock();
      }
  ...
  if (requestFrameCount < send_frame_count[SEND_QUEUE_BACKUP - 1])
    err.run("requestFrameCount:%d < backup2_frame_count:%d", requestFrameCount,
            send_frame_count[SEND_QUEUE_BACKUP - 1]);
  return 0;
}
...
void Remote::init_send_queue(DWORD frameCount, short nationRecno) {
  ...
  // put into the queue : <message length>, MSG_QUEUE_HEADER, <frameCount>,
  // <nationRecno>
  ...
  *(DWORD *)sendPtr = send_frame_count[0] =
      next_send_frame(nationRecno, frameCount + process_frame_delay);
  sendPtr += sizeof(DWORD);
...
void Remote::init_receive_queue(DWORD frameCount) {
  ...
  for (n = 0; n < process_frame_delay; ++n) {
    // nations are not created yet, put MSG_QUEUE_HEADER/MSG_NEXT_FRAME and
    // MSG_QUEUE_TRAILER even though the nation will not exist
