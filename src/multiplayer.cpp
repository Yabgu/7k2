...
const uint16_t UDP_GAME_PORT = 19256;
...
SessionDesc::SessionDesc(const char *name, const char *pass,
                         ENetAddress *address) {
  id = 0;
...
MultiPlayer::MultiPlayer() : current_sessions(sizeof(SessionDesc), 10) {
  init_flag = 0;
  lobbied_flag = 0;
...
MultiPlayer::~MultiPlayer() { deinit(); }
...
void MultiPlayer::init(ProtocolType protocol_type) {
  ...
  if (enet_address_set_host(&lan_broadcast_address, "255.255.255.255") != 0) {
    return;
  }
...
int MultiPlayer::init_lobbied(int maxPlayers, char *cmdLine) {
  MSG("Launching a multiplayer game from command line maxPlayers=%d, "
      "cmdLine='%s'\n",
      maxPlayers, cmdLine);
...
int MultiPlayer::is_lobbied() { return lobbied_flag; }
...
char *MultiPlayer::get_lobbied_name() { return config.player_name; }
...
int MultiPlayer::is_update_available() { return update_available; }
...
int MultiPlayer::open_port(uint16_t port, int fallback) {
  ...
  host = enet_host_create(&address, 2 * MAX_NATION, 2, 0, 0);
  if (host == NULL) {
    if (!fallback)
      return 0;
...
int MultiPlayer::set_remote_session_provider(const char *server) {
  if (enet_address_set_host(&remote_session_provider_address, server) == 0) {
...
SessionDesc *MultiPlayer::get_current_session() { return &joined_session; }

// create a new session
//
// <char *> sessionName      arbitary name to identify a session, input from
// user <char *> playerName       name to identify the local player's name in
// this session <int>    maxPlayers       maximum no. of players in a session
//
// return 1 if success
int MultiPlayer::create_session(char *sessionName, char *password,
                                char *playerName, int maxPlayers) {
...
int MultiPlayer::join_session(SessionDesc *session, char *playerName) {
  ...
  peer = enet_host_connect(host, &session->address, 1, 0);
  if (!peer) {
    close_port();
    return 0;
  }
...
// Adds a player that previously was unknown. Creates a player descriptor if
// needed, and initiates contact if requested. If contact is not requested, then
// the player is added to the pending pool, and waits for contact from that
// player.
//
// Returns 1 when the player was added succesfully.
// Returns 0 when the player was not added.
int MultiPlayer::add_player(uint32_t playerId, char *name, ENetAddress *address,
                            char contact) {
...
int MultiPlayer::send(uint32_t to, void *data, uint32_t msg_size) {
  ...
  packet = enet_packet_create(data, msg_size, packet_mode);
  ...
  *from = 0;
  ...
  case ENET_EVENT_TYPE_RECEIVE:
    if (event.packet->dataLength < MP_RECV_BUFFER_SIZE) {
      memcpy(recv_buf, event.packet->data, event.packet->dataLength);
...
{
        return memcmp( &((SessionDesc *)a)->guidInstance, &((SessionDesc
*)b)->guidInstance, sizeof(GUID) );
}
*/
...
void MultiPlayer::sort_sessions(int sortType) {
  ...
  // BUGHERE : quick_sort is a DynArray function but current_sessions is
  // DynArrayB
  switch (sortType) {
  case 1:
