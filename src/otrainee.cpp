...
//
void Trainee::init(int raceId, int initLoyalty, int spyRecno, int trainType,
                   int srcTownRecno) {
...
void Trainee::advanced_train(int firmRecno) {
  ...
  if (train_spy_potential && spy_skill < MAX_SKILL_TRAIN) {
    int incValue =
        (MAX_SKILL_TRAIN - (int)spy_skill) * train_spy_potential / 100;
  ...
  if (train_combat_potential && combat_level() < MAX_COMBAT_TRAIN) {
    int incValue =
        (MAX_COMBAT_TRAIN - (int)combat_level()) * train_combat_potential / 100;
    ...
    if (race_id == RACE_EGYPTIAN && nationRecno &&
      ...
      incValue = incValue * (MAX_WORKER * 2) /
                 (MAX_WORKER * 2 +
                  god_res[GOD_EGYPTIAN]->nation_prayer_count(nationRecno));
    }
  ...
  if (train_skill_potential && skill_level() < MAX_SKILL_TRAIN) {
    int incValue =
        (MAX_SKILL_TRAIN - (int)skill_level()) * train_skill_potential / 100;
...
int Trainee::is_own(int firmRecno) {
  return nation_array.player_recno &&
         is_nation(firmRecno, nation_array.player_recno);
}
...
void Trainee::change_loyalty(int loyaltyChange) {
  //	if( town_recno ) 		// for those live in town, their loyalty are
  //based on town people loyalty. 		return;
