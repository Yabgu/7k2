...
void Trainee::advanced_train(int firmRecno)
{
    ...
    {
        int incValue = (MAX_SKILL_TRAIN - (int)spy_skill) * train_spy_potential / 100;
    ...
    {
        int incValue = (MAX_COMBAT_TRAIN - (int)combat_level()) * train_combat_potential / 100;
        ...
        // penalty of egyptain
        if (race_id == RACE_EGYPTIAN && nationRecno && god_res[GOD_EGYPTIAN]->nation_prayer_count(nationRecno) > 0)
        {
            incValue = incValue * (MAX_WORKER * 2) /
                       (MAX_WORKER * 2 + god_res[GOD_EGYPTIAN]->nation_prayer_count(nationRecno));
        }
    ...
    {
        int incValue = (MAX_SKILL_TRAIN - (int)skill_level()) * train_skill_potential / 100;
...
{
    //	if( town_recno ) 		// for those live in town, their loyalty
    // are
