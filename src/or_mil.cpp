...
void Info::disp_military(int refreshFlag) {
  ...
  vga_back.d3_panel_up(TROOP_BROWSE_X1, TROOP_BROWSE_Y1, TROOP_BROWSE_X2,
                       TROOP_BROWSE_Y1 + 33);

  font_bld.put(x, y + 7, text_reports.str_mil_commander()); // "Commander" );
                                                            // SXM
#if (defined(CHINESE))
  font_bld.put(x + 210 + 12, y + 7,
               text_unit.str_leadership());              // "Leadership" );
  font_bld.put(x + 287, y + 7, text_unit.str_loyalty()); // "Loyalty" );
  font_bld.center_put_paragraph(x + 358 - 4, y + 6, x + 406, y + 24,
                                text_unit.str_hit_points(), 0, 0, 0);
  font_bld.center_put_paragraph(x + 404, y + 6, x + 487, y + 24,
                                text_reports.str_mil_commanded_soldiers(), 0, 0,
                                0);
  ...
  font_bld.center_put_paragraph(x + 358, y, x + 406, y + 24,
                                text_unit.str_hit_points(), 0, 0, 0);
  // font_bld.put( x+406, y   , "Commanded" );
  // font_bld.put( x+415, y+13, "Soldiers" );
  font_bld.center_put_paragraph(x + 404, y, x + 487, y + 24,
                                text_reports.str_mil_commanded_soldiers(), 0, 0,
                                0);
  font_bld.put(x + 485, y + 7, text_reports.str_mil_status()); // "Status" );
#endif
  ...
  if (refreshFlag == INFO_REPAINT) {
    browse_troop.init(TROOP_BROWSE_X1, TROOP_BROWSE_Y1 + 35, TROOP_BROWSE_X2,
                      TROOP_BROWSE_Y2 - 20, 0, 22, troop_filter(),
                      put_troop_rec, 1);
  ...
  vga_back.d3_panel_up(UNIT_BROWSE_X1, UNIT_BROWSE_Y1, UNIT_BROWSE_X2,
                       UNIT_BROWSE_Y1 + 20);

  font_bld.put(x, y, text_reports.str_mil_unit_type()); // "Unit Type" );
                                                        // SXM
#if (defined(CHINESE))
  font_bld.put(x + 300 + 12, y,
               text_reports.str_mil_no_of_units()); // "No. of Units" );
#else
  font_bld.put(x + 300, y,
               text_reports.str_mil_no_of_units()); // "No. of Units" );
#endif
  // SXM

  if (refreshFlag == INFO_REPAINT) {
    browse_unit.init(UNIT_BROWSE_X1, UNIT_BROWSE_Y1 + 22, UNIT_BROWSE_X2,
                     UNIT_BROWSE_Y2 - 20, 0, 18, unit_filter(), put_unit_rec,
                     1);
...
static void disp_troop_total() {
  ...
  for (i = viewingNation->ai_camp_count - 1; i >= 0; i--) {
    // totalSoldier += firm_array[ viewingNation->ai_camp_array[i]
    // ]->worker_count;
    FirmCamp *firmCamp;
    if ((firmCamp =
             firm_array[viewingNation->ai_camp_array[i]]->cast_to_FirmCamp())) {
  ...
  vga_back.d3_panel_up(TROOP_BROWSE_X1, TROOP_BROWSE_Y2 - 18, TROOP_BROWSE_X2,
                       TROOP_BROWSE_Y2);
  ...
  font_bld.put(x, y,
               text_reports.str_total_commander(browse_troop.total_rec()));
...
#define COMMANDED_SOLDIER_SHIFT 0
  ...
  font_bld.put(x + 160 + COMMANDED_SOLDIER_SHIFT, y,
               text_reports.str_total_soldier_under(totalCommandedSoldier));
...
#endif
  font_bld.put(x + 410 + TOTAL_SOLDIER_SHIFT, y,
               text_reports.str_total_soldier(totalSoldier));
...
static void disp_unit_total() {
  ...
  for (int unitId = 1; unitId <= MAX_UNIT_TYPE; unitId++) {
    ...
    int unitCount =
        unitInfo->nation_unit_count_array[info.viewing_nation_recno - 1];
    int generalCount =
        unitInfo->nation_general_count_array[info.viewing_nation_recno - 1];
  ...
  vga_back.d3_panel_up(UNIT_BROWSE_X1, UNIT_BROWSE_Y2 - 18, UNIT_BROWSE_X2,
                       UNIT_BROWSE_Y2);
...
static void put_troop_rec(int recNo, int x, int y, int refreshFlag) {
  ...
  font_bld.put(x + 478, y,
               text_reports.str_mil_unit_status(unitPtr->unit_mode));
}
