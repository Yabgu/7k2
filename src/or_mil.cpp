...
void Info::disp_military(int refreshFlag)
{
    ...
    {
        browse_troop.init(TROOP_BROWSE_X1, TROOP_BROWSE_Y1 + 35, TROOP_BROWSE_X2, TROOP_BROWSE_Y2 - 20, 0, 22,
                          troop_filter(), put_troop_rec, 1);
    ...
    if (refreshFlag == INFO_REPAINT)
    {
        browse_unit.init(UNIT_BROWSE_X1, UNIT_BROWSE_Y1 + 22, UNIT_BROWSE_X2, UNIT_BROWSE_Y2 - 20, 0, 18, unit_filter(),
                         put_unit_rec, 1);
...
static int unit_filter(int recNo)
{
    ...
    for (int unitId = 1; unitId <= MAX_UNIT_TYPE; unitId++)
    {
        ...
        if (!unitInfo->race_id && unitInfo->nation_unit_count_array[info.viewing_nation_recno - 1] > 0)
        {
            unitTypeCount++;
        }
