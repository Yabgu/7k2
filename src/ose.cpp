...
void SERequest::clear_request() { req_used = 0; }
// ------ End Function SERequest::clear_request -------//

// ------ Begin Function SECtrl::SECtrl -------//
SECtrl::SECtrl(Audio *audioPtr)
    : audio_ptr(audioPtr), res_supp(audioPtr->wav_res) {
  init_flag = 0;
  audio_flag = 0;
  req_pool = NULL;
  last_cycle = NULL;
...
//
// <int> soundEffect        the id of the sound effect, return from
// SECtrl::search_effect_id <long> vol               volume (0 - 100 max
// loudness) <long> pan               pan (-10000 = full left; 10000 = full
// right) note the request is abolished if vol is 0 or soundEffect is 0
//
    ...
    for (k = 0; k < cached_size; ++k) {
      ...
      for (i = seRequest->req_used - 1; i >= 0; --i) {
        if (seRequest->wave_ptr) {
          ...
          audio_ptr->play_wav(seRequest->resx_id, seRequest->play_vol[i]);
        }
        last_cycle[j]++;
        chCount--;
      }
