...
//
int Power::choose_cursor(int scrnX, int scrnY, ScreenObjectType selectedObjectType, short selectedObjectRecno,
                         ScreenObjectType pointingObjectType, short pointingObjectRecno)
{
    ...
    if (command_id)
    {
        if (scrnX >= ZOOM_X1 && scrnX <= ZOOM_X2 && scrnY >= ZOOM_Y1 && scrnY <= ZOOM_Y2)
        {
            switch (command_id)
            {
            ...
            case COMMAND_SETTLE: {
                ...
                Unit *activeUnit;
                if (nation_array.player_recno && !nation_array.is_deleted(nation_array.player_recno))
            ...
            case COMMAND_SET_CARAVAN_STOP:
                // ####### begin Gilbert 4/3 ######//
                if (pointingObjectType == SCREEN_OBJECT_FRIEND_FIRM || pointingObjectType == SCREEN_OBJECT_ENEMY_FIRM ||
                    pointingObjectType == SCREEN_OBJECT_ALLIED_FIRM)
                {
                    pointingObjectId = firm_array[pointingObjectRecno]->firm_id;
                    if (pointingObjectId == FIRM_MARKET || pointingObjectId == FIRM_MINE ||
        ...
        if (scrnX >= ZOOM_X1 && scrnX <= ZOOM_X2 && scrnY >= ZOOM_Y1 && scrnY <= ZOOM_Y2)
        {
            ...
            case SCREEN_OBJECT_SPY_GROUP: {
                ...
                case SCREEN_OBJECT_ALLIED_FIRM: {
                    ...
                    for (i = 1; i < arraySize; ++i)
                    {
                        ...
                        if (unit_array[i]->selected_flag && firmPtr->can_accept_assign(i, nation_array.player_recno))
                        {
                            return CURSOR_ASSIGN;
                        }
                    }
            ...
            case SCREEN_OBJECT_FRIEND_UNIT: {
                ...
                case SCREEN_OBJECT_FRIEND_TOWN:
                    ...
                    //&&
                    // unitPtr->skill_level()==0 )
                    if (town_array[pointingObjectRecno]->can_accept_assign(selectedObjectRecno,
                ...
                case SCREEN_OBJECT_FRIEND_FIRM:
                    if (firm_array[pointingObjectRecno]->can_accept_assign(selectedObjectRecno,
                                                                           nation_array.player_recno))
                        return CURSOR_ASSIGN;
                    else
                        return CURSOR_UNIT_C;
            ...
            case SCREEN_OBJECT_FRIEND_TOWN: {
                ...
                case SCREEN_OBJECT_FRIEND_FIRM: {
                    ...
                    int centerX = world.zoom_matrix->calc_zoom_x(
                        ...
                        (pFirm->loc_y1 + pFirm->loc_y2 + 1) * LOCATE_HEIGHT / 2, pFirm->altitude);
                ...
                case SCREEN_OBJECT_FRIEND_TOWN:
                    if (selectedObjectRecno != pointingObjectRecno)
                    {
                        ...
                        int centerX = world.zoom_matrix->calc_zoom_x(
                            ...
                            (pTown->loc_y1 + pTown->loc_y2 + 1) * LOCATE_HEIGHT / 2, pTown->altitude);
                        int centerY = world.zoom_matrix->calc_zoom_y(
                            (pTown->loc_x1 + pTown->loc_x2 + 1) * LOCATE_WIDTH / 2,
                            (pTown->loc_y1 + pTown->loc_y2 + 1) * LOCATE_HEIGHT / 2, pTown->altitude);
                        if (misc.points_distance(mouse.cur_x, mouse.cur_y, centerX + ZOOM_X1, centerY + ZOOM_Y1) <=
                                11 &&
...
int Power::choose_cursor_units(short selectedUnitRecno, short pointingUnitRecno)
{
    ...
    if (u1 && u2)
    {
        ...
        //		else if( u1->vehicle_id == pointingUnitId &&		// can
        // ride
        // on 			u1->solider_id != 0)		// make sure u1 is a rider, not a
        // riding unit
        //		{
        //			return CURSOR_ASSIGN;
        //		}
