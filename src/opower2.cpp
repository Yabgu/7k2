...
char *Power::get_link_icon(char linkStatus, int sameNation) {
  ...
  case LINK_DD:
    if (sameNation)
      ...
      return image_icon.get_ptr(
          "LINK_DD"); // red cross for different nation firms
  }
...
int Power::choose_cursor(int scrnX, int scrnY,
                         ScreenObjectType selectedObjectType,
                         short selectedObjectRecno,
                         ScreenObjectType pointingObjectType,
                         short pointingObjectRecno) {
  int selectedObjectId = 0;
  int pointingObjectId = 0;

  if (command_id) {
    if (scrnX >= ZOOM_X1 && scrnX <= ZOOM_X2 && scrnY >= ZOOM_Y1 &&
        scrnY <= ZOOM_Y2) {
      switch (command_id) {
        ...
        if (pointingObjectType == SCREEN_OBJECT_FRIEND_FIRM ||
          ...
          if (pointingObjectId == FIRM_MARKET ||
            ...
            if (!nation_array.is_deleted(
                    firm_array[pointingObjectRecno]->nation_recno) &&
                ((UnitCaravan *)unit_array[selectedObjectRecno])
                    ->can_set_stop(pointingObjectRecno))
    ...
    if (scrnX >= ZOOM_X1 && scrnX <= ZOOM_X2 && scrnY >= ZOOM_Y1 &&
      ...
      case SCREEN_OBJECT_SPY_UNIT: {
        ...
        case SCREEN_OBJECT_ALLIED_UNIT: {
          ...
          if (nationRecno == pUnit->nation_recno) {
            // same nation
            return choose_cursor_units(selectedObjectRecno,
                                       pointingObjectRecno);
          } else {
        ...
        case SCREEN_OBJECT_ALLIED_TOWN: {
          ...
          if (pTown->can_accept_assign(selectedObjectRecno,
        ...
        case SCREEN_OBJECT_ALLIED_FIRM: {
          ...
          if (pFirm->can_accept_assign(selectedObjectRecno,
                                       nation_array.player_recno))
            return CURSOR_ASSIGN;
          else {
        ...
        case SCREEN_OBJECT_ALLIED_TOWN: {
          ...
          for (i = 1; i < arraySize; ++i) {
            ...
            if (unit_array.is_deleted(i) ||
                !(unitPtr = unit_array[i])->selected_flag)
              continue;
      ...
      case SCREEN_OBJECT_FRIEND_UNIT: {
        ...
        case SCREEN_OBJECT_FRIEND_TOWN:
          //							if( unitPtr->race_id ==
          //town_array[pointingObjectRecno]->race_id
          //								&& rankId == RANK_SOLDIER &&
          //unitPtr->skill_level()==0 )
          if (town_array[pointingObjectRecno]->can_accept_assign(
                  selectedObjectRecno, nation_array.player_recno))
            return CURSOR_ASSIGN;
          else
            return CURSOR_UNIT_C;

        case SCREEN_OBJECT_FRIEND_FIRM:
          if (firm_array[pointingObjectRecno]->can_accept_assign(
                  selectedObjectRecno, nation_array.player_recno))
            return CURSOR_ASSIGN;
          else
            return CURSOR_UNIT_C;
      ...
      case SCREEN_OBJECT_FRIEND_TOWN: {
        ...
        case SCREEN_OBJECT_FRIEND_FIRM: {
          ...
          int centerY = world.zoom_matrix->calc_zoom_y(
              ...
              (pFirm->loc_y1 + pFirm->loc_y2 + 1) * LOCATE_HEIGHT / 2,
              pFirm->altitude);
          if (misc.points_distance(mouse.cur_x, mouse.cur_y, centerX + ZOOM_X1,
                                   centerY + ZOOM_Y1) <= 11 &&
        ...
        case SCREEN_OBJECT_FRIEND_TOWN:
          if (selectedObjectRecno != pointingObjectRecno) {
            ...
            int centerY = world.zoom_matrix->calc_zoom_y(
                ...
                (pTown->loc_y1 + pTown->loc_y2 + 1) * LOCATE_HEIGHT / 2,
                pTown->altitude);
            if (misc.points_distance(mouse.cur_x, mouse.cur_y,
                                     centerX + ZOOM_X1,
                                     centerY + ZOOM_Y1) <= 11 &&
...
//
int Power::choose_cursor_units(short selectedUnitRecno,
                               short pointingUnitRecno) {
  ...
  if (u1 && u2) {
    ...
    if (u2Ptr->can_accept_assign(selectedUnitRecno)) {
      return CURSOR_ASSIGN;
    } else if (pointingUnitId ==
                   UNIT_EXPLOSIVE_CART && // check trigger explosive cart
               u1Ptr->max_attack_range() > 1) {
      return CURSOR_TRIGGER_EXPLODE;
    }
    //		else if( u1->vehicle_id == pointingUnitId &&		// can ride
    //on 			u1->solider_id != 0)		// make sure u1 is a rider, not a riding
    //unit
    //		{
    //			return CURSOR_ASSIGN;
    //		}
    ...
    if (unit_array.selected_count == 1) {
      ...
      if (unitPtr->nation_recno == nation_array.player_recno)
        ...
        return SCREEN_OBJECT_SPY_UNIT;
               ...
               (~nation_array)->get_relation_status(unitPtr->nation_recno) >=
                   RELATION_FRIENDLY)
  ...
  if (firm_array.selected_recno) {
    ...
    if (firmPtr->is_own())
      return SCREEN_OBJECT_FRIEND_FIRM;
             ...
             (~nation_array)->get_relation_status(firmPtr->nation_recno) >=
                 RELATION_FRIENDLY)
  ...
  if (town_array.selected_recno) {
    ...
    if (townPtr->is_own())
      return SCREEN_OBJECT_FRIEND_TOWN;
             ...
             (~nation_array)->get_relation_status(townPtr->nation_recno) >=
                 RELATION_FRIENDLY)
...
//
ScreenObjectType Power::find_pointing_type(Location *locPtr,
                                           short *pointingRecno) {
  short dummyId;
