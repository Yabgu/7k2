...
void WarPointArray::draw_dot()
{
    ...
    static unsigned char dotColor[DRAW_PHASE_PERIOD / 2] = {0xa0, 0xa4, 0xa8, 0x00, 0xb0, 0xb4, 0xb8, 0x00};
    ...
    int rowWriteInc = ((world.map_matrix->loc_y_height * WARPOINT_ZONE_SIZE) >> LOC_XY_MUL_SHIFT) * vgaBufPitch +
                      ((world.map_matrix->loc_y_width * WARPOINT_ZONE_SIZE) >> LOC_XY_MUL_SHIFT);
    int colWriteInc = ((world.map_matrix->loc_x_height * WARPOINT_ZONE_SIZE) >> LOC_XY_MUL_SHIFT) * vgaBufPitch +
                      ((world.map_matrix->loc_x_width * WARPOINT_ZONE_SIZE) >> LOC_XY_MUL_SHIFT);

    // pointer for drawing a cross
    short *rowWritePtr1 = vga_back.buf_ptr(world.map_matrix->calc_loc_pos_x(0, (WARPOINT_ZONE_SIZE - 1) / 2) + MAP_X1,
                                           world.map_matrix->calc_loc_pos_y(0, (WARPOINT_ZONE_SIZE - 1) / 2) + MAP_Y1);
    short *rowWritePtr2 = vga_back.buf_ptr(world.map_matrix->calc_loc_pos_x((WARPOINT_ZONE_SIZE - 1) / 2, 0) + MAP_X1,
    ...
    for (y = 0; y < WARPOINT_ZONE_ROW; ++y, (rowWritePtr1 += rowWriteInc), (rowWritePtr2 += rowWriteInc))
    {
        ...
        for (x = 0; x < WARPOINT_ZONE_COLUMN;
             ++x, ++warPt, (colWritePtr1 += colWriteInc), (colWritePtr2 += colWriteInc))
        {
            if (warPt->strength > 0)
            {
                ...
                for (int i = 0; i < WARPOINT_ZONE_SIZE / 2;
                     ++i, (writePtr1 += vgaBufPitch + 1), (writePtr2 += vgaBufPitch - 1))
                {
                    *writePtr1 = color;
                    *writePtr2 = color;
                }
