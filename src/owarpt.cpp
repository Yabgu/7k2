...
inline void WarPoint::decay() { strength >>= 1; }
...
// ------ begin of function WarPointArray::~WarPointArray -------//
WarPointArray::~WarPointArray() { deinit(); }
// ------ end of function WarPointArray::~WarPointArray -------//
...
void WarPointArray::init() {
  ...
  war_point = (WarPoint *)mem_add(sizeof(WarPoint) * WARPOINT_ZONE_COLUMN *
                                  WARPOINT_ZONE_ROW);
  memset(war_point, 0,
         sizeof(WarPoint) * WARPOINT_ZONE_COLUMN * WARPOINT_ZONE_ROW);
...
void WarPointArray::draw_dot() {
  ...
  int rowWriteInc = ((world.map_matrix->loc_y_height * WARPOINT_ZONE_SIZE) >>
                     LOC_XY_MUL_SHIFT) *
                        vgaBufPitch +
                    ((world.map_matrix->loc_y_width * WARPOINT_ZONE_SIZE) >>
                     LOC_XY_MUL_SHIFT);
  int colWriteInc = ((world.map_matrix->loc_x_height * WARPOINT_ZONE_SIZE) >>
                     LOC_XY_MUL_SHIFT) *
                        vgaBufPitch +
                    ((world.map_matrix->loc_x_width * WARPOINT_ZONE_SIZE) >>
                     LOC_XY_MUL_SHIFT);

  // pointer for drawing a cross
  short *rowWritePtr1 = vga_back.buf_ptr(
      world.map_matrix->calc_loc_pos_x(0, (WARPOINT_ZONE_SIZE - 1) / 2) +
          MAP_X1,
      world.map_matrix->calc_loc_pos_y(0, (WARPOINT_ZONE_SIZE - 1) / 2) +
          MAP_Y1);
  short *rowWritePtr2 = vga_back.buf_ptr(
      world.map_matrix->calc_loc_pos_x((WARPOINT_ZONE_SIZE - 1) / 2, 0) +
          MAP_X1,
      world.map_matrix->calc_loc_pos_y((WARPOINT_ZONE_SIZE - 1) / 2, 0) +
          MAP_Y1);

  for (y = 0; y < WARPOINT_ZONE_ROW;
       ++y, (rowWritePtr1 += rowWriteInc), (rowWritePtr2 += rowWriteInc)) {
    ...
    for (x = 0; x < WARPOINT_ZONE_COLUMN; ++x, ++warPt,
        (colWritePtr1 += colWriteInc), (colWritePtr2 += colWriteInc)) {
      if (warPt->strength > 0) {
        // draw a cross, UNEXPLORED_COLOR is not needed to check
        short *writePtr1 = colWritePtr1;
        short *writePtr2 = colWritePtr2;
        for (int i = 0; i < WARPOINT_ZONE_SIZE / 2; ++i,
                 (writePtr1 += vgaBufPitch + 1),
                 (writePtr2 += vgaBufPitch - 1)) {
          *writePtr1 = color;
          *writePtr2 = color;
        }
      }
