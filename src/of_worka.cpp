...
int FirmWork::think_inc_worker_supply() {
  ...
  if (availableWorkforce >=
      needed_worker_count - worker_count) // if there are enough labor supply
    return 0;
  ...
  int fromTownRecno =
      most_jobless_town(bestToTown->race_id, bestToTown->town_recno);
  ...
  nation_array[nation_recno]->add_action(bestToTown->loc_x1, bestToTown->loc_y1,
                                         fromTown->loc_x1, fromTown->loc_y1,
                                         ACTION_AI_SETTLE_TOWN, 0,
...
int FirmWork::ai_build_neighbor_town(int builderUnitRecno) {
  ...
  if (nationPtr->is_action_exist(-1, -1, loc_x1, loc_y1, ACTION_AI_BUILD_TOWN,
                                 townRaceId)) {
    return 0;
  }
