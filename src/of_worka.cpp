...
{
    //--- only do so if the actual number of workers must smaller than the needed
    // number ---//

    if (needed_worker_count - worker_count < 5)
        return 0;

    //----- if there are linked towns that can supply labor already, don't bother
    // settling a new town ------//
    ...
    nation_array[nation_recno]->add_action(bestToTown->loc_x1, bestToTown->loc_y1, fromTown->loc_x1, fromTown->loc_y1,
                                           ACTION_AI_SETTLE_TOWN, 0, needed_worker_count - worker_count);
...
int FirmWork::ai_build_neighbor_town(int builderUnitRecno)
{
    ...
    if (!nationPtr->find_best_place_loc(0, loc_x1, loc_y1, buildXLoc,
                                        buildYLoc)) // 0-build a town
    {
        no_neighbor_space = 1;
        return 0;
    }
    ...
    if (nationPtr->is_action_exist(-1, -1, loc_x1, loc_y1, ACTION_AI_BUILD_TOWN, townRaceId))
    {
        return 0;
    }
    ...
    nationPtr->add_action(buildXLoc, buildYLoc, loc_x1, loc_y1, ACTION_AI_BUILD_TOWN, townRaceId, 1, builderUnitRecno);
    ...
    for (i = nationPtr->action_count(); i > 0; i--)
    {
        ...
        if (actionNode->action_mode == ACTION_AI_SETTLE_TOWN)
        {
            ...
            for (j = 0; j < linked_town_count; j++)
            {
                ...
                if (townPtr->loc_x1 == actionNode->action_x_loc && townPtr->loc_y1 == actionNode->action_y_loc)
                {
                    return 1;
                }
            }
