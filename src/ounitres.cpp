...
void UnitRes::init() {
  ...
  for (int i = 0; i < unit_info_count; i++, unitInfo++) {
    for (int j = 0; j < unitInfo->attack_count; j++) {
      err_when(
          attack_info_array[unitInfo->first_attack + j - 1].eqv_attack_next >
          unitInfo->attack_count);
    }
  }
...
void UnitRes::second_init() {
  ...
  for (int i = 0; i < unit_info_count; i++, unitInfo++) {
    memset(unitInfo->nation_tech_level_array, 0,
           sizeof(unitInfo->nation_tech_level_array));
    memset(unitInfo->nation_unit_count_array, 0,
           sizeof(unitInfo->nation_unit_count_array));
    memset(unitInfo->nation_general_count_array, 0,
           sizeof(unitInfo->nation_general_count_array));
  }
}
...
void UnitRes::load_info() {
  ...
  for (i = 0; i < dbUnit->rec_count(); i++) {
    ...
    unitInfo->dll_sprite_id =
        misc.atoi(unitRec->dll_sprite_id, unitRec->SPRITE_ID_LEN);
    ...
    unitInfo->visual_range =
        misc.atoi(unitRec->visual_range, unitRec->UNIT_PARA_LEN);
    unitInfo->visual_extend =
        misc.atoi(unitRec->visual_extend, unitRec->UNIT_PARA_LEN);
    unitInfo->stealth = misc.atoi(unitRec->stealth, unitRec->UNIT_PARA_LEN);
    unitInfo->hit_points =
        misc.atoi(unitRec->hit_points, unitRec->UNIT_PARA_LEN);
    unitInfo->armor = misc.atoi(unitRec->armor, unitRec->UNIT_PARA_LEN);

    unitInfo->build_days =
        misc.atoi(unitRec->build_days, unitRec->BUILD_DAYS_LEN);
    unitInfo->build_cost = misc.atoi(unitRec->build_cost, unitRec->COST_LEN);
    unitInfo->build_live_points_cost =
        misc.atoi(unitRec->build_live_points_cost, unitRec->COST_LEN);
    ...
    unitInfo->carry_unit_capacity =
        misc.atoi(unitRec->carry_unit_capacity, unitRec->CARRY_CAPACITY_LEN);
    unitInfo->carry_goods_capacity =
        misc.atoi(unitRec->carry_goods_capacity, unitRec->CARRY_CAPACITY_LEN);

    unitInfo->transform_unit_id =
        misc.atoi(unitRec->transform_unit_id, unitRec->SPRITE_ID_LEN);
    unitInfo->transform_combat_level =
        misc.atoi(unitRec->transform_combat_level, unitRec->UNIT_PARA_LEN);
    unitInfo->guard_combat_level =
        misc.atoi(unitRec->guard_combat_level, unitRec->UNIT_PARA_LEN);
    ...
    if (unitRec->general_icon_file_name[0] != '\0' &&
        unitRec->general_icon_file_name[0] != ' ') {
    ...
    if (unitRec->king_icon_file_name[0] != '\0' &&
        unitRec->king_icon_file_name[0] != ' ') {
    ...
    unitInfo->soldier_small_icon_ptr =
        res_small_icon.read_imported(bitmapOffset);

    if (unitRec->general_small_icon_file_name[0] != '\0' &&
        unitRec->general_small_icon_file_name[0] != ' ') {
      memcpy(&bitmapOffset, unitRec->general_small_icon_ptr, sizeof(uint32_t));
      unitInfo->general_small_icon_ptr =
          res_general_small_icon.read_imported(bitmapOffset);
    } else {
      unitInfo->general_small_icon_ptr = unitInfo->soldier_small_icon_ptr;
    }

    if (unitRec->king_small_icon_file_name[0] != '\0' &&
        unitRec->king_small_icon_file_name[0] != ' ') {
      memcpy(&bitmapOffset, unitRec->king_small_icon_ptr, sizeof(uint32_t));
      unitInfo->king_small_icon_ptr =
          res_king_small_icon.read_imported(bitmapOffset);
    } else {
      unitInfo->king_small_icon_ptr = unitInfo->soldier_small_icon_ptr;
    }
    if (unitRec->unit_icon_file_name[0] != '\0' &&
        unitRec->unit_icon_file_name[0] != ' ') {
      memcpy(&bitmapOffset, unitRec->unit_icon_ptr, sizeof(uint32_t));
      unitInfo->unit_icon_ptr = res_unit_icon.read_imported(bitmapOffset);
    }
    unitInfo->unit_icon_offsetx =
        misc.atoi(unitRec->unit_icon_offsetx, unitRec->OFFSET_LEN);
    unitInfo->unit_icon_offsety =
        misc.atoi(unitRec->unit_icon_offsety, unitRec->OFFSET_LEN);

    unitInfo->unit_icon_offsetx +=
        -LOCATE_WIDTH / 2 - (-ZOOM_LOC_X_WIDTH / 2 + -ZOOM_LOC_Y_WIDTH / 2);
    unitInfo->unit_icon_offsety +=
        -LOCATE_HEIGHT / 2 - (-ZOOM_LOC_X_HEIGHT / 2 + -ZOOM_LOC_Y_HEIGHT / 2);

    unitInfo->first_attack =
        misc.atoi(unitRec->first_attack, unitRec->UNIT_PARA_LEN);
    unitInfo->attack_count =
        misc.atoi(unitRec->attack_count, unitRec->UNIT_PARA_LEN);
    unitInfo->die_effect_id =
        misc.atoi(unitRec->die_effect_id, unitRec->UNIT_PARA_LEN);

    unitInfo->defense_attribute.init(
        misc.atoi(unitRec->sturdiness, unitRec->UNIT_PARA_LEN),
        misc.atoi(unitRec->is_wood, unitRec->IS_WOOD_LEN));

    memset(unitInfo->nation_tech_level_array, 1,
           sizeof(unitInfo->nation_tech_level_array));
  ...
  for (i = 0; i < unit_info_count; i++) {
    ...
    if (unitInfo->vehicle_unit_id) {
      unit_info_array[unitInfo->vehicle_unit_id - 1].vehicle_id =
          unitInfo->vehicle_id;
...
void UnitRes::load_attack_info() {
  ...
  attack_info_array =
      (AttackInfo *)mem_add(sizeof(AttackInfo) * attack_info_count);
  ...
  for (i = 0; i < dbUnitAttack->rec_count(); i++) {
    ...
    attackInfo->combat_level =
        misc.atoi(attackRec->combat_level, attackRec->COMBAT_LEVEL_LEN);
    attackInfo->attack_delay =
        misc.atoi(attackRec->attack_delay, attackRec->UNIT_PARA_LEN);
    attackInfo->attack_range =
        misc.atoi(attackRec->attack_range, attackRec->UNIT_PARA_LEN);
    attackInfo->attack_damage =
        misc.atoi(attackRec->attack_damage, attackRec->UNIT_PARA_LEN);
    attackInfo->pierce_damage =
        misc.atoi(attackRec->pierce_damage, attackRec->UNIT_PARA_LEN);
    attackInfo->effect_out_frame =
        misc.atoi(attackRec->effect_out_frame, attackRec->UNIT_PARA_LEN);
    attackInfo->bullet_out_frame =
        misc.atoi(attackRec->bullet_out_frame, attackRec->UNIT_PARA_LEN);
    if (attackInfo->effect_out_frame == 0)
      attackInfo->bullet_out_frame = attackInfo->bullet_out_frame;
    attackInfo->bullet_delay =
        misc.atoi(attackRec->bullet_delay, attackRec->UNIT_PARA_LEN);
    attackInfo->bullet_speed =
        misc.atoi(attackRec->bullet_speed, attackRec->UNIT_PARA_LEN);
    attackInfo->bullet_radius =
        misc.atoi(attackRec->bullet_radius, attackRec->UNIT_PARA_LEN);
    attackInfo->bullet_sprite_id =
        misc.atoi(attackRec->bullet_sprite_id, attackRec->UNIT_PARA_LEN);
    if ((attackInfo->attack_range > 1) && (attackInfo->bullet_sprite_id == 0))
      err_here();

    attackInfo->dll_bullet_sprite_id =
        misc.atoi(attackRec->dll_bullet_sprite_id, attackRec->UNIT_PARA_LEN);
    attackInfo->eqv_attack_next =
        misc.atoi(attackRec->eqv_attack_next, attackRec->UNIT_PARA_LEN);

    attackInfo->min_power =
        misc.atoi(attackRec->min_power, attackRec->UNIT_PARA_LEN);
    attackInfo->consume_power =
        misc.atoi(attackRec->consume_power, attackRec->UNIT_PARA_LEN);
    attackInfo->fire_radius =
        misc.atoi(attackRec->fire_radius, attackRec->UNIT_PARA_LEN);
    attackInfo->effect_id =
        misc.atoi(attackRec->effect_id, attackRec->UNIT_PARA_LEN);
    attackInfo->effect2_id =
        misc.atoi(attackRec->effect2_id, attackRec->UNIT_PARA_LEN);
    attackInfo->effect_dis =
        misc.atoi(attackRec->effect_dis, attackRec->EFFECT_LOC_LEN);

    attackInfo->attack_attribute.init(
        misc.atoi(attackRec->sturdiness, attackRec->UNIT_PARA_LEN),
...
int UnitInfo::is_loaded() { return sprite_res[sprite_id]->is_loaded(); }
...
//
void UnitInfo::unit_change_nation(int newNationRecno, int oldNationRecno,
                                  int rankId) {
...
//             bit 2 = title for civilian, soldier, and which defined for bit 1
//             (detailed title)
const char *UnitRes::unit_name(int flags, int unitId, int nameId, int heroId,
                               int rankId, char unitMode) {
  static String str;
  str = "";
  ...
  if (raceId && unitInfo->unit_class != UNIT_CLASS_WAGON) {
    ...
    //		if( unitInfo->unit_class == UNIT_CLASS_WEAPON && get_weapon_version()
    //> 1 )
    //		{
    //			str += " ";
    ...
    return text_unit.str_non_race_unit_name(
        unitInfo->name,
        0, // version not available
        (!firstNameOnly && unitInfo->unit_class != UNIT_CLASS_GOD
             ? nameId
             : 0), // God doesn't have any series no.
        0);
  }
}
...
void UnitRes::load_class_info() {
  ...
  unit_class_array =
      (UnitClassInfo *)mem_add(sizeof(UnitClassInfo) * unit_class_count);
  ...
  for (int i = 0; i < unit_class_count; i++) {
    ...
    unitClassInfo->has_combat_level = unitClassRec->has_combat_level != ' ' &&
                                      unitClassRec->has_combat_level != '0';
    unitClassInfo->has_skill_level = unitClassRec->has_skill_level != ' ' &&
                                     unitClassRec->has_skill_level != '0';
    unitClassInfo->has_spy_skill = unitClassRec->has_spy_skill != ' ' &&
                                   unitClassRec->has_spy_skill != '0';
    unitClassInfo->has_loyalty =
        unitClassRec->has_loyalty != ' ' && unitClassRec->has_loyalty != '0';
    unitClassInfo->has_weapon_version =
        unitClassRec->has_weapon_version != ' ' &&
        unitClassRec->has_weapon_version != '0';
    unitClassInfo->monster_side =
        unitClassRec->monster_side != ' ' && unitClassRec->monster_side != '0';
    unitClassInfo->life =
        (char)misc.atoi(unitClassRec->life, unitClassRec->LIFE_LEN);
  }
}
