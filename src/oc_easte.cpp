...
int CampaignEastWest::process_event() {
  ...
  return (this->*eventFP)(); // call the corrsponding function to return the
                             // news msg
}
...
int CampaignEastWest::event_fryhtan_takeover() {
  ...
  if (!cur_monster_nation_recno)
    cur_monster_nation_recno =
        random_pick_monster_campaign_nation(0, eastern_nation_recno);
  // ###### end Ban 30/6 #######//

  if (random_pick_attack_state(cur_monster_nation_recno,
                               western_nation_recno)) {
    CampaignNation *cNation = get_nation(cur_monster_nation_recno);

    int textId =
        random(2) + 1; // there are two different sets of text for this event.
                       // The program randomly picks one to display.
    ...
    disp_letter(0, res_event.read(textId == 1 ? (char *)"1B" : (char *)"1B1"),
                cNation->race_name(), cNation->nation_name(),
                minister_name(MINISTER_OF_WAR));
  } else // if the current monster nation does not have a state next to the
         // player's kingdom, randomly pick another monster nation
  {
...
int CampaignEastWest::event_random_point_1() {
  ...
  if (misc.random(2) == 0 && !has_event_run(EVENT_FRYHTAN_BREAK_UP) &&
      state_array.nation_state_count(cur_monster_nation_recno) >=
          2) // the Fryhtan kingdom should at least have 2 states
  {
    if (player_beat_fryhtan_count >= player_beat_fryhtan_max ||
        (player_beat_fryhtan_count >= 1 && eastern_nation_recno == 0) ||
        misc.random((player_beat_fryhtan_max - player_beat_fryhtan_count) /
                    2) == 0) {
  ...
  //--- hero debut if the player has too few states left or the Eastern empire
  //is destroyed ---//
  ...
  if (misc.chance(16) || misc.random(6) == 0) // 1/6 chance X 2
  {
    ...
    return 0; // also return so that random point 1 will be run again and make
              // the randomization more accurate
  }

  if (misc.chance(16) && state_array.nation_state_count(western_nation_recno) >
                             1) // Fryhtan will demand one state
  {
  ...
  if (misc.chance(16)) {
    ...
    if (!has_stage_run(STAGE_DEFENSE) || misc.random(10)) {
      ...
      cur_monster_nation_recno = least_state_nation(
          -1,
          western_nation_recno); // -1 the nation must be a monster nation. the
                                 // nation must be next to western empire
  ...
  if (misc.chance(32)) {
    if (monster_take_over_east_count >= 1 &&
      ...
      if (get_triangle_states2(playerStateRecno, eastStateRecno,
                               monsterStateRecno)) {
    ...
    cur_monster_nation_recno = random_pick_monster_campaign_nation(
        0, eastern_nation_recno, 1); // 1-must next to
...
int CampaignEastWest::event_state_betrayal() {
  ...
  //--- if the player does not have any other state next to the captured state,
  //then don't offer the fight back option ---//
  ...
  disp_letter(0, res_event.read("3C"),
              get_nation(eastern_nation_recno)
                  ->minister_name(MINISTER_OF_FOREIGN_AFFAIRS3_ASSIST));

  //---- Do you want to declare war with the eastern empire and attack the
  //betrayed state? -----//
...
int CampaignEastWest::event_fryhtan_offer_peace() {
  //--- get a monster nation with the most states (that means it has taken over
  //a state of the player already) ---//

  cur_monster_nation_recno = most_state_nation(
      -1, western_nation_recno); // -1 the nation must be a monster nation and
                                 // it must be next to the western empire
  ...
  disp_letter(0, res_event.read("4A"), cNation->race_name(),
              cNation->nation_name(), cNation->minister_name(MINISTER_ORDO),
              cNation->king_name());
...
int CampaignEastWest::event_give_throne() {
  disp_narrative(res_event.read("5A"), old_king_name, king_name(),
                 old_king_name, king_name());
...
int CampaignEastWest::event_random_point_2() {
  ...
  if (misc.random(4) == 0 &&
      random_pick_attack_state(western_nation_recno,
                               cur_monster_nation_recno) &&
  ...
  if (eastern_nation_recno) {
    ...
    if ((get_relation_status(eastern_nation_recno) >= RELATION_NEUTRAL ||
         misc.random(2) == 0) &&
        misc.random(event_run_count(EVENT_EASTERN_EMPIRE_OFFER_HELP) + 2) ==
            0) // start with moderate probability and decrease as it has been
               // run more times
    {
    ...
    //----- if relationship with Eastern Empire < Neutral ----//
             ...
             random_pick_attack_state(eastern_nation_recno,
                                      western_nation_recno) &&
             state_array.nation_state_count(western_nation_recno) > 1 &&
             !defeated_eastern_empire_invasion && // the eastern empire will
                                                  // never attack again if it
                                                  // has been defeated.
             !has_stage_run(STAGE_EASTERN_EMPIRE_INVADE_OUR_LAND) &&
             misc.random(2) == 0) // 50% chance
    {
      if (random_pick_attack_state(eastern_nation_recno,
                                   western_nation_recno)) {
  ...
  if (misc.random(2) == 0 ||
      has_event_run(
          EVENT_FRYHTAN_BREAK_UP)) // if a Fryhtan kingdom has broken up, run
                                   // the Fryhtan attacks Fryhtan event
  {
    //-- try getting 3 states next to each other, only run the event if the
    //function returns successfully --//

    int humanStateRecno, monsterStateRecno1, monsterStateRecno2;

    if (get_triangle_states1(humanStateRecno, monsterStateRecno1,
                             monsterStateRecno2)) {
  ...
  if (player_pick_attack_target(CAMPAIGN_NATION_MONSTER)) {
    ...
    if (state_array.is_adjacent_to_nation(target_state_recno,
                                          eastern_nation_recno)) {
      if (misc.random(2) == 0 &&
          !has_event_run(EVENT_ASK_EASTERN_EMPIRE_FOR_HELP)) {
    ...
    //--- If the player is not next to any of the Fryhtan states (being
    //separated by the Eastern Empire), then have the Fryhtan attacking the
    //Eastern Empire ---//
...
int CampaignEastWest::event_eastern_empire_offer_help() {
  ...
  if (!get_triangle_states2(humanStateRecno, eastStateRecno,
                            monsterStateRecno)) {
    set_event(EVENT_RANDOM_POINT_2);
    return 1;
  }
  ...
  disp_letter(0, res_event.read("7A"),
              get_nation(eastern_nation_recno)
                  ->minister_name(MINISTER_OF_FOREIGN_AFFAIRS2));
  ...
  if (detect_letter() == 1) // Agree
  {
    ...
    cur_monster_nation_recno =
        state_array[monsterStateRecno]->campaign_nation_recno;

    set_relation_status(western_nation_recno, eastern_nation_recno,
                        RELATION_ALLIANCE);
...
// 				  0 - failed.
//
int CampaignEastWest::get_triangle_states2(int &humanStateRecno,
                                           int &eastStateRecno,
                                           int &monsterStateRecno) {
  for (humanStateRecno = state_array.size(); humanStateRecno > 0;
       humanStateRecno--) {
    ...
    for (eastStateRecno = state_array.size(); eastStateRecno > 0;
         eastStateRecno--) {
      ...
      for (monsterStateRecno = state_array.size(); monsterStateRecno > 0;
           monsterStateRecno--) {
        if (!get_nation(state_array[monsterStateRecno]->campaign_nation_recno)
                 ->is_monster())
          continue;
...
int CampaignEastWest::event_fryhtan_attack_fryhtan() {
  ///--- get 3 states next to each other with one player state, and two other
  ///monster states ---//
  ...
  //-- display the narrative describing the conflict between the two Fryhtan
  //kingdoms --//

  CampaignNation *monsterNation1 =
      get_nation(state_array[monsterStateRecno1]->campaign_nation_recno);
  CampaignNation *monsterNation2 =
      get_nation(state_array[monsterStateRecno2]->campaign_nation_recno);

  disp_narrative(res_event.read("8A"), monsterNation1->race_name(),
                 monsterNation1->nation_name(), monsterNation2->race_name(),
                 monsterNation2->nation_name());

  attack_state(monsterStateRecno1, monsterStateRecno2, -1, 1,
               3); // -1 - the attacker loses

  //-- ask if the player wants to attack a state of either of the Fryhtan
  //kingdoms --//
  ...
  {
    cur_monster_nation_recno =
        state_array[monsterStateRecno1]->campaign_nation_recno;
    attacker_state_recno = humanStateRecno;
...
// 				  0 - failed.
//
int CampaignEastWest::get_triangle_states1(int &humanStateRecno,
                                           int &monsterStateRecno1,
                                           int &monsterStateRecno2) {
  for (humanStateRecno = state_array.size(); humanStateRecno > 0;
       humanStateRecno--) {
    //---- look for a player state first ----//
    ...
    for (monsterStateRecno1 = state_array.size(); monsterStateRecno1 > 0;
         monsterStateRecno1--) {
      if (!get_nation(state_array[monsterStateRecno1]->campaign_nation_recno)
               ->is_monster())
        continue;
      ...
      for (monsterStateRecno2 = state_array.size(); monsterStateRecno2 > 0;
           monsterStateRecno2--) {
        if (!get_nation(state_array[monsterStateRecno2]->campaign_nation_recno)
                 ->is_monster())
          continue;
...
int CampaignEastWest::event_fryhtan_ask_for_joint_attack() {
  ...
  if (misc.random(2) ==
      0) // either Fryhtan kingdom A or B will ask for a joint attack
  {
  ...
  cur_monster_nation_recno =
      state_array[attacker_state_recno]->campaign_nation_recno;
  target_monster_nation_recno =
      state_array[target_state_recno]->campaign_nation_recno;

  err_when(!get_nation(cur_monster_nation_recno)->is_monster());

  //-------------------------------------------//

  CampaignNation *monsterNation =
      get_nation(state_array[attacker_state_recno]->campaign_nation_recno);

  disp_letter(0, res_event.read("9A"), monsterNation->race_name(),
              monsterNation->minister_name(MINISTER_ORDO),
  ...
  {
    if (misc.random(2) ==
        0) //--- you get Fryhtan mad and it turns against you ---//
    {
...
int CampaignEastWest::event_random_point_3() {
  ...
  //-- if the player refused to disband the Fryhtans, there is a chance that the
  //Fryhtan will revolt. ---//
  ...
  //--- if Fryhtan nations still exist, but Fryhtan nations are separate from
  //human nations by the Eastern empire's territory ---//
...
int CampaignEastWest::event_fryhtan_think_surrender() {
  ...
  if (!(cur_monster_nation_recno && // this function is called after a fryhtan
                                    // kingdom is deleted, and
                                    // cur_monster_nation_recno keeps the nation
                                    // recno of the fryhtan kingdom
  ...
  disp_letter(0, res_event.read("11A"), cNation->king_name(),
              cNation->race_name(), cNation->king_name(), cNation->race_name());
  ...
  if (detect_letter() == 1) {
    ...
    disp_letter(0, res_event.read("11B"), cNation->race_name(),
                minister_name(MINISTER_OF_WAR));

    if (detect_letter() == 1)
      can_use_fryhtan =
          1; // set this so that in later scenarios, the player can decide
             // whether to send a Fryhtan troop or a human troop
  }
...
int CampaignEastWest::event_ask_eastern_empire_for_help() {
  ...
  err_when(state_array[playerStateRecno]->campaign_nation_recno !=
           CAMPAIGN_PLAYER_NATION_RECNO);
  ...
  if (detect_letter() == 1) // want to ask for help
  {
    ...
    } else if (state_array.nation_state_count(western_nation_recno) >
               1) // 70% the Eastern Empire demand land for the help. Only if
                  // the western nation has more than one state currently
    {
      disp_letter(0, res_event.read("12B"),
                  get_nation(eastern_nation_recno)
                      ->minister_name(MINISTER_OF_FOREIGN_AFFAIRS2_ASSIST));

      if (detect_letter() == 1) // You agree
      {
        //--- hand a state over to the eastern empire ---//

        random_pick_attack_state(
            eastern_nation_recno, western_nation_recno,
            playerStateRecno); // first try to pick a state next to the eastern
                               // empire, exclude playerStateRecno from target
                               // choices as it is where the player will send
                               // his troop

        int stateRecno = target_state_recno;

        if (!stateRecno)
          stateRecno = random_pick_state(
              western_nation_recno); // if not, randomly pick a state in the
                                     // western empire
...
int CampaignEastWest::event_random_point_4() {
  ...
  if (packed_nation_count() == 1 ||
      (eastern_nation_recno == 0 && new_kingdom_nation_recno == 0)) {
  ...
  if (saved_player_state_count == 0)
    saved_player_state_count =
        state_array.nation_state_count(western_nation_recno);
  ...
  if (misc.chance(35) && eastern_nation_recno &&
      ...
      state_array.nation_state_count(eastern_nation_recno) >=
          2) // the eastern empire must have at least 2 states for this to
             // happen
  {
    ...
    if (stateRecno) {
      ...
      disp_letter(0, res_event.read("14A"),
  ...
  //------ the state turns towards you ------//
    ...
    if (stateRecno) {
      state_change_nation(stateRecno, western_nation_recno);
      disp_letter(0, res_event.read("14B"),
                  minister_name(MINISTER_OF_FOREIGN_AFFAIRS));
    }
  ...
  //--- if there is an independent state and there is no new kingdom, 100%
  //chance that it will form a new kingdom. ---//

  if (state_array.nation_state_count(0) > 0 && new_kingdom_nation_recno == 0) {
    ...
    disp_letter(0, res_event.read("14C"),
                minister_name(MINISTER_OF_FOREIGN_AFFAIRS));
  ...
  //--- if there is only one rivaling kingdom left and it only has one state
  //remaining, go to Game::the Last Battle ---//
  ...
  if (new_kingdom_nation_recno &&
      misc.random(2) == 0) // if there is a new kingdom and 50% chance
  {
    if (misc.random(2) == 0 && !has_stage_run(STAGE_NEW_KINGDOM_INVASION) &&
        random_pick_attack_state(new_kingdom_nation_recno,
                                 western_nation_recno)) {
    ...
    //-- new kingdom attacks eastern empire -----//
             ...
             (misc.random(2) == 0 ||
              get_nation(eastern_nation_recno)
                      ->get_relation_status(new_kingdom_nation_recno) ==
                  RELATION_HOSTILE)) {
      if (random_pick_attack_state(new_kingdom_nation_recno,
                                   eastern_nation_recno)) {
        int winFlag = misc.random(2);

        disp_strategic_screen();
        attack_state(attacker_state_recno, target_state_recno, winFlag, 1,
                     3); // 50% chance win and 50% chance lose

        const char *kingName =
            get_nation(new_kingdom_nation_recno)->king_name();
  ...
  if (eastern_nation_recno && new_kingdom_nation_recno &&
      state_array.nation_state_count(eastern_nation_recno) >
          1) // if Eastern empire only has one state left, run STAGE_LAST_BATTLE
             // instead
  {
    //--- if the new kingdom still exists and the Eastern empire just has one
    //state left, then finish the new kingdom first before continue to attack
    //the Eastern Empire (so that the state of the Eastern empire is always the
    //first state to conquer.)
  ...
  } else if (eastern_nation_recno && // if Eastern empire only has one state
                                     // left, run STAGE_LAST_BATTLE instead
             state_array.nation_state_count(eastern_nation_recno) > 1) {
    int targetNationRecno = player_pick_attack_target(CAMPAIGN_NATION_AI);
    ...
    if (targetNationRecno == eastern_nation_recno) {
      if (!has_stage_run(STAGE_ATTACK_EASTERN_EMPIRE_2))
        ...
        set_event(EVENT_PLAYER_TAKE_OVER_EAST); // take over the state without
                                                // an interactive game.
  ...
  } else if (eastern_nation_recno == 0 &&
             new_kingdom_nation_recno) // the eastern kingdom has been
                                       // destroyed, but the new kingdom is
                                       // still alive.
  {
    if (random_pick_attack_state(western_nation_recno,
                                 new_kingdom_nation_recno)) {
...
int CampaignEastWest::event_ultimate_victory() {
  int curDate =
      date.julian(game_year, misc.random(12) + 1,
                  misc.random(28) + 1); // randomly set the current date
...
int CampaignEastWest::event_fryhtan_invade_east() {
  ...
  if (!cur_monster_nation_recno ||
      !random_pick_attack_state(cur_monster_nation_recno,
                                eastern_nation_recno)) {
  ...
  //-- 50% chance that the Fryhtan wins and 50% chance that the Easten Empire
  //wins --//
...
int CampaignEastWest::event_fryhtan_break_up() {
  ...
  int newNationRecno =
      add_nation(CAMPAIGN_NATION_MONSTER, originalNation->race_id);
  ...
  int attackStateRecno = random_pick_attack_state(
      originalNation->campaign_nation_recno, newNationRecno);
  ...
  CampaignNation *attackerNation =
      get_nation(state_array[attackStateRecno]->campaign_nation_recno);

  disp_narrative(
      res_event.read(attackResult == 1 ? (char *)"18B" : (char *)"18C"),
      attackerNation->king_name());

  attack_state(attackStateRecno, stateRecno, attackResult, 1, 3);

  //------- the player take the opportunity to take over one of the weakened
  //state --//
  ...
  attackStateRecno = random_pick_state(
      western_nation_recno,
      stateRecno); // pick a player state that is next to the battled state

  if (attackStateRecno) // if the player has a state next to the breaking up
                        // Fryhtan kingdom
...
int CampaignEastWest::event_player_take_over_fryhtan() {
  ...
  disp_narrative(res_event.read("19A"),
  ...
  if (misc.random(3) == 0) {
    return_event_id =
        EVENT_RANDOM_POINT_3; // direct Game:Restore Prosperity to go back to
                              // EVENT_POINT_3 when it is finished.
...
int CampaignEastWest::event_independent_surrender() {
  ...
  disp_narrative(res_event.read(
      "21A")); // an letter from the independent state about surrendering to you
...
int CampaignEastWest::event_fryhtan_army_revolt() {
  ...
  if (fryhtan_surrendered_state_recno &&
      state_array[fryhtan_surrendered_state_recno]->campaign_nation_recno ==
          western_nation_recno) {
  ...
  disp_narrative(res_event.read(
      "22A")); // an letter from the independent state about surrendering to you
...
int CampaignEastWest::event_player_take_over_east() {
  ...
  if (misc.random(3) == 0 && !has_stage_run(STAGE_RESTORE_PROSPERITY)) {
    return_event_id =
        EVENT_RANDOM_POINT_4; // direct Game:Restore Prosperity to go back to
                              // EVENT_POINT_4 when it is finished.
