...
int CampaignEastWest::event_fryhtan_takeover()
{
    ...
    if (random_pick_attack_state(cur_monster_nation_recno, western_nation_recno))
    {
        ...
        int textId = random(2) + 1; // there are two different sets of text for this event.
                                    // The program randomly picks one to display.

        disp_narrative(res_event.read(textId == 1 ? (char *)"1A" : (char *)"1A1"), cNation->race_name(),
                       cNation->nation_name());
...
int CampaignEastWest::event_random_point_1()
{
    ...
    if (state_array.nation_state_count(western_nation_recno) <= hero_debut_state_count || eastern_nation_recno == 0)
    {
        if (random_pick_attack_state(cur_monster_nation_recno, western_nation_recno))
        {
    ...
    if (misc.chance(16))
    {
        ...
        if (!has_stage_run(STAGE_DEFENSE) || misc.random(10))
        {
            ...
            cur_monster_nation_recno =
                least_state_nation(-1,
                                   western_nation_recno); // -1 the nation must be a monster nation.
                                                          // the nation must be next to western empire
    ...
    if (misc.chance(32))
    {
        if (monster_take_over_east_count >= 1 && !has_event_run(EVENT_EAST_ASK_FOR_JOINT_ATTACK))
        {
...
int CampaignEastWest::event_fryhtan_offer_peace()
{
    ...
    cur_monster_nation_recno = most_state_nation(-1, western_nation_recno); // -1 the nation must be a monster nation
                                                                            // and it must be next to the western empire

    CampaignNation *cNation = get_nation(cur_monster_nation_recno);

    //------------------------------------------//

    disp_letter(0, res_event.read("4A"), cNation->race_name(), cNation->nation_name(),
                cNation->minister_name(MINISTER_ORDO), cNation->king_name());
...
int CampaignEastWest::event_random_point_2()
{
    ...
    if (misc.random(4) == 0)
    {
        ...
        else if (!has_stage_run(STAGE_GROW_POPULATION))
        {
            ...
            if (cNation->race_id != RACE_EGYPTIAN && cNation->know_base_array[RACE_EGYPTIAN - 1] == 0)
            {
                return_event_id = EVENT_RANDOM_POINT_2;
    ...
    if (misc.random(4) == 0 && random_pick_attack_state(western_nation_recno, cur_monster_nation_recno) &&
    ...
    if (state_array.nation_state_count(western_nation_recno) >= 4 && !has_stage_run(STAGE_DEEP_INTO_FRYHTAN_LAND))
    {
        if (player_pick_attack_target(CAMPAIGN_NATION_MONSTER))
        {
    ...
    if (eastern_nation_recno)
    {
        ...
        //----- if relationship with Eastern Empire < Neutral ----//
                 ...
                 !defeated_eastern_empire_invasion && // the eastern empire will
                                                      ...
                                                      // has been defeated.
                 !has_stage_run(STAGE_EASTERN_EMPIRE_INVADE_OUR_LAND) && misc.random(2) == 0) // 50% chance
        {
            if (random_pick_attack_state(eastern_nation_recno, western_nation_recno))
            {
    ...
    if (misc.random(2) == 0 || has_event_run(EVENT_FRYHTAN_BREAK_UP)) // if a Fryhtan kingdom has broken up, run
                                                                      // the Fryhtan attacks Fryhtan event
    {
        ...
        {
            if (has_event_run(EVENT_FRYHTAN_ATTACK_FRYHTAN) && !has_event_run(EVENT_FRYHTAN_ASK_FOR_JOINT_ATTACK))
            {
...
int CampaignEastWest::event_eastern_empire_offer_help()
{
    ...
    disp_letter(0, res_event.read("7A"), get_nation(eastern_nation_recno)->minister_name(MINISTER_OF_FOREIGN_AFFAIRS2));
...
int CampaignEastWest::get_triangle_states2(int &humanStateRecno, int &eastStateRecno, int &monsterStateRecno)
{
    for (humanStateRecno = state_array.size(); humanStateRecno > 0; humanStateRecno--)
    {
        ...
        {
            if (state_array[eastStateRecno]->campaign_nation_recno != eastern_nation_recno)
            {
                continue;
            }
            ...
            for (monsterStateRecno = state_array.size(); monsterStateRecno > 0; monsterStateRecno--)
            {
                ...
                if (humanState->is_adjacent(monsterStateRecno) && eastState->is_adjacent(monsterStateRecno))
                {
                    return 1;
                }
            }
...
int CampaignEastWest::event_fryhtan_attack_fryhtan()
{
    ...
    disp_letter(0, res_event.read("8B"), minister_name(MINISTER_OF_WAR), monsterNation1->race_name(),
                monsterNation2->race_name(), monsterNation2->race_name(), monsterNation1->race_name());
    ...
    for (humanStateRecno = state_array.size(); humanStateRecno > 0; humanStateRecno--)
    {
        ...
        for (monsterStateRecno1 = state_array.size(); monsterStateRecno1 > 0; monsterStateRecno1--)
        {
            ...
            for (monsterStateRecno2 = state_array.size(); monsterStateRecno2 > 0; monsterStateRecno2--)
            {
                ...
                if (monsterState1->campaign_nation_recno != state_array[monsterStateRecno2]->campaign_nation_recno)
                {
                    if (humanState->is_adjacent(monsterStateRecno2) && monsterState1->is_adjacent(monsterStateRecno2))
                    {
                        return 1;
                    }
                }
...
int CampaignEastWest::event_fryhtan_ask_for_joint_attack()
{
    ...
    disp_letter(0, res_event.read("9A"), monsterNation->race_name(), monsterNation->minister_name(MINISTER_ORDO),
    ...
    else if (rc == 2) // Refuse
    {
        ...
        else //--- the Fryhtan attacks the target alone -----//
        {
            if (misc.random(2) == 0) // the Fryhtan wins
            {
                ...
                disp_narrative(res_event.read("9C1"), monsterNation->race_name(), monsterNation->king_name());
            }
            else // the Fryhtan loses
            {
                attack_state(attacker_state_recno, target_state_recno, -1, 1, 3);

                disp_narrative(res_event.read("9C2"), monsterNation->race_name(), monsterNation->king_name());
            }
...
int CampaignEastWest::event_random_point_3()
{
    ...
    if (!disband_fryhtan_reply)
    {
        if (use_fryhtan_count >= 1 &&
            (use_fryhtan_count >= use_fryhtan_max || misc.random(use_fryhtan_max - use_fryhtan_count) == 0))
        {
    ...
    for (int i = state_array.size(); i > 0; i--)
    {
            ...
            state_array.is_adjacent_to_nation(i, CAMPAIGN_PLAYER_NATION_RECNO))
        {
            ...
            if (cur_monster_nation_recno && random_pick_attack_state(western_nation_recno, cur_monster_nation_recno))
            {
...
int CampaignEastWest::event_fryhtan_think_surrender()
{
    ...
    disp_letter(0, res_event.read("11A"), cNation->king_name(), cNation->race_name(), cNation->king_name(),
                cNation->race_name());
    ...
    if (detect_letter() == 1)
    {
        ...
        if (detect_letter() == 1)
            can_use_fryhtan = 1; // set this so that in later scenarios, the player can decide
    ...
    if (detect_letter() == 1) // want to ask for help
    {
                    ...
                    // the western nation has more than one state currently
        {
            disp_letter(0, res_event.read("12B"),
                        get_nation(eastern_nation_recno)->minister_name(MINISTER_OF_FOREIGN_AFFAIRS2_ASSIST));
            ...
            if (detect_letter() == 1) // You agree
            {
                ...
                if (!stateRecno)
                    stateRecno = random_pick_state(western_nation_recno); // if not, randomly pick a state in the
                                                                          // western empire
...
int CampaignEastWest::event_random_point_4()
{
    ...
    if (misc.chance(35) && eastern_nation_recno && new_kingdom_nation_recno == 0 &&
        state_array.nation_state_count(eastern_nation_recno) >= 2) // the eastern empire must have at least 2 states
                                                                   // for this to happen
    {
    ...
    {
        if ((state_array.nation_state_count(western_nation_recno) - saved_player_state_count >= 2 &&
             misc.random(2) == 0) ||
            misc.random(4) == 0)
        {
    ...
    if (new_kingdom_nation_recno && misc.random(2) == 0) // if there is a new kingdom and 50% chance
    {
        ...
        //-- new kingdom attacks eastern empire -----//
                 ...
                 (misc.random(2) == 0 ||
                  get_nation(eastern_nation_recno)->get_relation_status(new_kingdom_nation_recno) == RELATION_HOSTILE))
        {
            if (random_pick_attack_state(new_kingdom_nation_recno, eastern_nation_recno))
            {
                ...
                if (winFlag)
                {
                    disp_letter(0, res_event.read("14D1"), kingName, minister_name(MINISTER_OF_FOREIGN_AFFAIRS));
                }
                else
                {
    ...
    if (eastern_nation_recno && new_kingdom_nation_recno &&
        state_array.nation_state_count(eastern_nation_recno) > 1) // if Eastern empire only has one state left, run
                                                                  // STAGE_LAST_BATTLE instead
    {
    ...
    else if (eastern_nation_recno == 0 && new_kingdom_nation_recno) // the eastern kingdom has been
...
{
    int curDate = date.julian(game_year, misc.random(12) + 1,
                              misc.random(28) + 1); // randomly set the current date

    disp_letter(0, res_event.read("15A"), date.date_str(curDate), minister_name(MINISTER_OF_WAR));
...
int CampaignEastWest::event_east_ask_for_joint_attack()
{
    ...
    disp_letter(0, res_event.read("17A"), cNation->minister_name(MINISTER_OF_FOREIGN_AFFAIRS));
...
int CampaignEastWest::event_fryhtan_break_up()
{
    ...
    disp_narrative(res_event.read(attackResult == 1 ? (char *)"18B" : (char *)"18C"), attackerNation->king_name());
    ...
    attackStateRecno = random_pick_state(western_nation_recno,
...
int CampaignEastWest::event_player_take_over_fryhtan()
{
    ...
    if (misc.random(3) == 0)
    {
        return_event_id = EVENT_RANDOM_POINT_3; // direct Game:Restore Prosperity to go back to
...
int CampaignEastWest::event_fryhtan_army_revolt()
{
    ...
    else
    {
        ...
        for (int i = state_array.size(); i > 0 && revoltStateRecno == 0; i--)
        {
            ...
            {
                if (state_array[j]->campaign_nation_recno != western_nation_recno && state_array[j]->is_adjacent(i))
                {
                    revoltStateRecno = i;
                    break;
                }
...
int CampaignEastWest::event_player_take_over_east()
{
    ...
    {
        return_event_id = EVENT_RANDOM_POINT_4; // direct Game:Restore Prosperity to go back to
