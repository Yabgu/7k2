...
void Unit::process_mode_camp_defender()
{
    ...
    int startPoint = detectInterval * ((info.game_date - last_attack_date) %
                                       DETECT_INTERVAL_DIVIDER); //	use (info.game_date-last_attack_date) so
                                                                 // that the unit always look for nearest
                                                                 // enemies first
    ...
    for (int i = startPoint; i <= endPoint; i++)
    {
        ...
        if (unitRecno)
        {
            ...
            if (unitPtr->nation_recno != nation_recno && should_aggressive_attack(unitPtr->base_obj_recno))
            {
