...
void Unit::set_mode(char modeId, short modePara) {
  ...
  //--- in defense mode, set the last_attack_date to today for determing when
  //the unit should return home ---//
...
void Unit::process_mode_town_defender() {
  ...
  if (town_array.is_deleted(unit_mode_para) ||
      !town_array[unit_mode_para]->can_accept_assign(
          sprite_recno)) // or it no longer accepts this unit
  {
...
void Unit::process_mode_camp_defender() {
  ...
  if (firm_array.is_deleted(unit_mode_para) ||
      !firm_array[unit_mode_para]->can_accept_assign(
          sprite_recno)) // or it no longer accepts this unit
  {
    set_mode(0); // reset the mode to normal
    return;
  ...
  enum {
    DETECT_INTERVAL_DIVIDER = 6
  }; // it takes 6 days to complete a single detect action
  ...
  int startPoint =
      detectInterval *
      ((info.game_date - last_attack_date) %
       DETECT_INTERVAL_DIVIDER); //	use (info.game_date-last_attack_date) so
                                 //that the unit always look for nearest enemies
                                 //first
  ...
  for (int i = startPoint; i <= endPoint; i++) {
    misc.cal_move_around_a_point(i, attackScanRange, attackScanRange, xOffset,
                                 yOffset);
