...
//
PlayerProfile::PlayerProfile() { memset(this, 0, sizeof(*this)); }
...
//
int PlayerProfile::is_registered() { return player_name[0]; }
...
int PlayerProfile::load(char *fileName) {
  ...
  if (f.file_handle != NULL && f.file_read(&tmp, sizeof(tmp)) &&
      tmp.decrypt()) {
    *this = tmp;
...
//
char *ProfileHero::unit_name(int nationRecno, int withTitle,
                             int firstNameOnly) {
  static String str;
