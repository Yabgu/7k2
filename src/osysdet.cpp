...
static void locate_unit_class(int unitClass,
...
void Sys::detect()
{
    ...
    if (game.is_campaign_mode() && game.campaign()->auto_test_flag)
    {
        ...
        {
            if (game.campaign()->auto_test_flag) // if in auto test mode, we must randomize the
...
void Sys::process_key(unsigned scanCode, unsigned skeyState)
{
    ...
    if (!(view_mode == MODE_NATION && info.nation_report_mode == NATION_REPORT_CHAT))
    {
...
void Sys::detect_letter_key(unsigned scanCode, unsigned skeyState)
{
    ...
    */
    ...
    if ((keyCode = mouse.is_key(scanCode, skeyState, (WORD)0, K_ON_NUMPAD)))
    {
        if (unit_array.selected_recno)
        {
            ...
            if (unitPtr->team_info)
            {
                ...
                //= LEADER_AT_TOP; 						break; case
                //'8':
                // unitPtr->team_info->formation_direction = LEADER_AT_TOP_LEFT;
                // break; case '7': unitPtr->team_info->formation_direction =
                ...
                // break; case '3': unitPtr->team_info->formation_direction =
                // LEADER_AT_RIGHT; break; case '2':
                // unitPtr->team_info->formation_direction =
                // LEADER_AT_BOTTOM_RIGHT; 						break; case
                // '1': unitPtr->team_info->formation_direction = LEADER_AT_BOTTOM;
    ...
    if ((keyCode = mouse.is_key(scanCode, skeyState, (WORD)0, K_IS_CTRL)))
    {
        ...
        {
            unit_group_array[keyCode - '1'].set_to_selected(); // unit_group_array[] start with 0, it is not a
                                                               // DynArray
            return;
        }
    }
    ...
    if ((keyCode = mouse.is_key(scanCode, skeyState, (WORD)0, K_IS_ALT)))
    {
        if (keyCode >= '1' && keyCode <= '9')
        {
            ...
            unit_group_array[keyCode - '1'].select_all(); // unit_group_array[] start with 0, it is not a
                                                          // DynArray
            return;
        }
    }
        ...
        case KEY_F1:
            if (game.game_mode != GAME_TUTORIAL || tutor.selectively_detect_flag == 0)
            {
                ...
                // ####### begin Gilbert 19/4 #########//
                {
                    ...
                    int chatNationRecno;
                    if ((skeyState & SHIFT_KEY_MASK) && filteredNation <= info.nation_filter() &&
...
void Sys::detect_cheat_key(unsigned scanCode, unsigned skeyState)
{
    ...
    case '+':
        ...
        if (firm_array.selected_recno)
        {
            ...
            {
                Nation *nationPtr;
                if ((nationPtr = firmPtr->nation_ptr()) && nationPtr->research_tech_id &&
...
void Sys::detect_campaign_cheat_key(unsigned scanCode, unsigned skeyState)
{
    ...
    if (campaignCheat == 1)
    {
        ...
        else if (keyCode >= '1' && keyCode <= MIN('5', '1' + game.win_condition_count - 1))
        {
        ...
        if (keyCode >= '6' && keyCode <= MIN('9', '6' + game.lose_condition_count - 1))
        {
...
static void locate_king_general(int rankId)
{
    ...
    for (int i = unit_array.size(); i > 0; i--)
    {
        ...
        if (unitPtr->nation_recno == nation_array.player_recno && unitPtr->rank_id == rankId)
        {
            short xLoc, yLoc;
...
static void locate_spy()
{
    ...
    for (int i = unit_array.size(); i > 0; i--)
    {
        ...
        if (unitPtr->true_nation_recno() == nation_array.player_recno && unitPtr->spy_recno && unitPtr->is_visible())
        {
            short xLoc, yLoc;
...
static void hide_map()
{
    ...
    for (i = base_obj_array.size(); i > 0; --i)
    {
        ...
        if (baseObj->is_visible() && baseObj->explore_for_player())
        {
            ...
            {
                world.unveil(baseObj->obj_loc_x1(), baseObj->obj_loc_y1(), baseObj->obj_loc_x2(),
                             baseObj->obj_loc_y2());
                world.visit(baseObj->obj_loc_x1(), baseObj->obj_loc_y1(), baseObj->obj_loc_x2(), baseObj->obj_loc_y2(),
                            EXPLORE_RANGE - 1);
            }
        }
    }
...
static void locate_free_item(int checkExplore)
{
    ...
    for (int i = site_array.size(); i > 0; i--)
    {
        ...
        if (sitePtr->site_type == SITE_ITEM &&
            (!checkExplore ||
             world.is_explored(sitePtr->map_x_loc, sitePtr->map_y_loc, sitePtr->map_x2, sitePtr->map_y2)))
        {
...
static void locate_firm_id(char *firmIdString, int nationRecno)
{
    ...
    for (int i = firm_array.size(); i > 0; i--)
    {
        ...
        if (strchr(firmIdString, (char)firmPtr->firm_id) && (nationRecno == -1 || firmPtr->nation_recno == nationRecno))
        {
