...
static void
locate_unit_class(int unitClass,
                  int nationRecno = -1); // nationRecno == -1 for all nations
...
void Sys::detect() {
  ...
  if (view_mode != MODE_NORMAL && report_disp_frame_no) {
    return;
  }
  ...
  if (VBrowse::press_record) // if the user is pulling the records of the
                             // browser up and down, calling detect() routines
                             // of other controls will confuse it
  {
  ...
  if (game.is_campaign_mode() && game.campaign()->auto_test_flag) {
    if (mouse.right_press) // disable auto test when the right mouse button is
                           // pressed
    {
      game.campaign()->auto_test_flag = 0;
    } else {
      if (game.campaign()
              ->auto_test_flag) // if in auto test mode, we must randomize the
                                // seed as retry will restore the original seed.
        misc.randomize();

      if (misc.random(3) > 0) // 2/3 chance winning
      {
        game.result_win_condition_recno = misc.random(game.win_condition_count);
        game.result_win_condition_id =
            game.win_condition[game.result_win_condition_recno].condition_id;
        game.game_end(nation_array.player_recno);
      } else {
        game.result_lose_condition_recno =
            misc.random(game.lose_condition_count);
        game.result_lose_condition_id =
            game.lose_condition[game.result_lose_condition_recno].condition_id;
        game.game_end(0);
      }
    }
  }
  ...
  if (!(view_mode == MODE_NATION &&
    ...
    if (sys.debug_session || sys.testing_session || scenario_cheat_flag) {
      ...
      if (nation_array.player_recno &&
          !remote.is_enable()) // not allowed in multiplayer mode
      {
        if ((~nation_array)->cheat_enabled_flag) {
          ...
          if (detect_key_str(1, "!!##%%&&")) {
            box.msg(
                text_game_menu.str_cheat_enable()); // "Cheat Mode Enabled." );
...
void Sys::detect_letter_key(unsigned scanCode, unsigned skeyState) {
  ...
  if ((keyCode = mouse.is_key(scanCode, skeyState, (WORD)0, 0))) {
    if (unit_array.selected_recno && keyCode >= '0' &&
        keyCode <= '0' + MAX_FORMATION) {
      if (unit_array.selected_recno) {
        Unit *unitPtr = unit_array[unit_array.selected_recno];

        if (unitPtr->team_info) {
          // ###### begin Gilbert 25/2 #######//
          // do not set formation_id here, let unit_group.formation set it (for
          // multiplayer game)
          //					unitPtr->team_info->formation_id =
          //keyCode-'0'; 					if( keyCode > '0' )		// '0' resets formation
          //					{
          //						unit_group.set(
          //unitPtr->team_info->member_unit_array,
          //unitPtr->team_info->member_count );
          //						unit_group.formation(unitPtr->team_info->formation_id,
          //COMMAND_PLAYER);
          //					}
          unit_group.set(unitPtr->team_info->member_unit_array,
                         unitPtr->team_info->member_count);
  ...
  if((keyCode = mouse.is_key(scanCode, skeyState, (WORD) 0, K_ON_NUMPAD)))
  {
          if( unit_array.selected_recno )
          {
                  ...
                  {
                          unit_group.set(unitPtr->team_info->member_unit_array,
  unitPtr->team_info->member_count ); unit_group.formation_turn(keyCode);
                  }
          }
          return;
  }
  */
  ...
  if ((keyCode = mouse.is_key(scanCode, skeyState, (WORD)0, K_ON_NUMPAD))) {
    if (unit_array.selected_recno) {
      ...
      if (unitPtr->team_info) {
        ...
        //				{
        //					case '9':
        //						unitPtr->team_info->formation_direction
        //= LEADER_AT_TOP; 						break; 					case '8':
        //						unitPtr->team_info->formation_direction
        //= LEADER_AT_TOP_LEFT; 						break; 					case '7':
        //						unitPtr->team_info->formation_direction
        //= LEADER_AT_LEFT; 						break; 					case '6':
        //						unitPtr->team_info->formation_direction
        //= LEADER_AT_TOP_RIGHT; 						break; 					case '4':
        //						unitPtr->team_info->formation_direction
        //= LEADER_AT_BOTTOM_LEFT; 						break; 					case '3':
        //						unitPtr->team_info->formation_direction
        //= LEADER_AT_RIGHT; 						break; 					case '2':
        //						unitPtr->team_info->formation_direction
        //= LEADER_AT_BOTTOM_RIGHT; 						break; 					case '1':
        //						unitPtr->team_info->formation_direction
        //= LEADER_AT_BOTTOM; 						break; 					default: 						return;
        //				}
        //				unit_group.set(unitPtr->team_info->member_unit_array,
        //unitPtr->team_info->member_count );
        //				unit_group.formation_turn(unitPtr->team_info->formation_direction,
        //COMMAND_PLAYER);
  ...
  if ((keyCode = mouse.is_key(scanCode, skeyState, (WORD)0, K_IS_CTRL))) {
    if (keyCode >= '1' && keyCode <= '9') {
      unit_group_array[keyCode - '1']
          .set_to_selected(); // unit_group_array[] start with 0, it is not a
                              // DynArray
      return;
    }
  }
  ...
  if ((keyCode = mouse.is_key(scanCode, skeyState, (WORD)0, K_IS_ALT))) {
    if (keyCode >= '1' && keyCode <= '9') {
      unit_group_array[keyCode - '1'].validate_unit_array(
          nation_array.player_recno);
      unit_group_array[keyCode - '1']
          .select_all(); // unit_group_array[] start with 0, it is not a
                         // DynArray
      return;
    }
  }
  ...
  if ((keyCode =
    ...
    case KEY_PGUP:
      ...
      else if (game.game_mode != GAME_TUTORIAL)
      ...
      if (game.game_mode != GAME_TUTORIAL ||
        ...
        // ####### begin Gilbert 19/4 #########//
        {
          ...
          if ((skeyState & SHIFT_KEY_MASK) &&
              ...
              nation_array[chatNationRecno = info.nation_filter(filteredNation)]
                  ->is_remote()) // shift-f1 go to chat mode directly
          {
...
void Sys::detect_cheat_key(unsigned scanCode, unsigned skeyState) {
  ...
  int keyCode =
      mouse.is_key(scanCode, skeyState, (WORD)0, K_IS_CTRL | K_CHAR_KEY);
  ...
  case 'b': // finish building a firm instantly or increase the hit points of a
            // firm to its max
    if (firm_array.selected_recno) {
      Firm *firmPtr = firm_array[firm_array.selected_recno];
  ...
  case '!':
    ...
    /*
       ...
       unit_array[unit_array.selected_recno];
                                    ...
                                    unitPtr->set_combat_level( MIN(100,
       unitPtr->combat_level()+20) );
                            }
                            break;
       ...
       unit_array[unit_array.selected_recno];
                                    ...
                                    if( unitPtr->skill_level() )
                                            unitPtr->skill.set_skill_level(
       MIN(MAX_SKILL_TRAIN, unitPtr->skill_level()+20) );
                            }
                            break;
       ...
       unit_array[unit_array.selected_recno];
                                    ...
                                    {
                                            Spy* spyPtr =
       spy_array[unitPtr->spy_recno];

                                            spyPtr->spy_skill = MIN(100,
       spyPtr->spy_skill+20);
                                    }
                            }
                            break;
             ...
             for( i=1; i<=nation_array.size() ; i++ )
             {
                if( !nation_array.is_deleted(i) )
                   god_res.enable_know_all(i);
                            ...
                            box.msg( "Now knowledge of seat of power is
       available to all nations." ); break;
    */
  case 'n':
    // #### begin Gilbert 9/2 #######//
...
/*
   ...
   ); else box.msg( "Now all unit icons will not be displayed." ); break;
*/
#ifdef DEBUG
/*
                case '`':
...
*/       //-*********** syn game test ***********-//
  ...
  if (game.is_campaign_mode() &&
      mouse.is_key(scanCode, skeyState, KEY_F12, 0)) {
    sys.signal_exit_flag = 3;
    game.campaign()->replay_stage_flag = 1;
  }
}
...
static int detect_scenario_cheat_key(unsigned scanCode, unsigned skeyState) {
  ...
  int keyCode = mouse.is_key(scanCode, skeyState, (WORD)0,
                             K_IS_CTRL | K_IS_ALT); // control shift key
  ...
  case 'q': //-- decrease population of a selected race in a selected village by
            //10 --//
    if (town_array.selected_recno) {
      ...
      if (townPtr->nation_recno == nation_array.player_recno) {
        ...
        if (i && townPtr->population) {
          for (j = 10; j > 0 && !town_array.is_deleted(townPtr->town_recno);
               --j)
  ...
  case 'm': //----- add natural resource to cursor pos / remove existing
            //resource ------//
    if (get_mouse_loc_in_zoom_map(curXLoc, curYLoc)) {
      ...
      } else if (locPtr->can_build_site(1) &&
                 !locPtr->is_power_off()) // add site
      {
        i = MAX_RAW_RESERVE_QTY * (50 + misc.random(50)) / 100;
        site_array.add_site(curXLoc, curYLoc, SITE_RAW,
                            misc.random(MAX_RAW) + 1, i);
        // box.msg( "Site added." );
        keyProcessed++;
      }
    }
    ...
    if (unit_array.selected_recno) {
      ...
      if (get_mouse_loc_in_zoom_map(curXLoc, curYLoc)) {
        ...
        if (locPtr->can_move(unitPtr->mobile_type)) {
          world.get_loc(unitPtr->next_x_loc(), unitPtr->next_y_loc())
              ->remove_unit(unitPtr->mobile_type);
          unitPtr->stop_order();
          ...
          world.get_loc(curXLoc, curYLoc)
              ->set_unit(unitPtr->sprite_recno, unitPtr->mobile_type);
          keyProcessed++;
        }
      }
    }
  ...
  case 's':
    if (info.default_viewing_nation_recno && nation_array.player_recno &&
        nation_array[info.default_viewing_nation_recno]->is_ai()) {
      nation_array[info.default_viewing_nation_recno]->surrender(
          nation_array.player_recno);
    }
    keyProcessed++;
    break;
...
int Sys::detect_set_speed(unsigned scanCode, unsigned skeyState) {
  ...
  //----- 'P' or space to pause and resume game ----//
    ...
    if (curSpeedMode == 0) // resume game
    {
      ...
      if (newSpeedMode ==
          0) // if the last speed mode is also 0, set to 3, the normal speed
        newSpeedMode = 3;
...
int Sys::detect_key_str(int keyStrId, const char *keyStr) {
  ...
  if (key_str_pos[keyStrId] >= (int)strlen(keyStr)) {
    key_str_pos[keyStrId] =
        0; // the full string has been entered successfully without any mistakes
    return 1;
  } else
    return 0;
}
...
static void locate_royal() {
  ...
  if (firmRecno && !firm_array.is_deleted(firmRecno)) {
    if (firm_array[firmRecno]->cast_to_FirmCamp()) {
      selectedSoldierId =
          firm_array[firmRecno]->cast_to_FirmCamp()->selected_soldier_id;
    }
  }
  ...
  while (--loopCount > 0) {
          ...
          if (findRoyalFlag) {
            if (selectedSoldierId == 0) {
              if (camp->overseer_recno &&
                  unit_array[camp->overseer_recno]->is_royal) {
                royalFound = 1;
                break;
              }
    ...
    if (unitRecno && unit_array.selected_recno == unitRecno ||
        firmRecno && firm_array.selected_recno == firmRecno &&
            selectedSoldierId == initSelectedSoldierId) {
      break;
    }
...
static int get_mouse_loc_in_zoom_map(int &x, int &y) {
  ...
  if (mouseX >= ZOOM_X1 && mouseX <= ZOOM_X2 && mouseY >= ZOOM_Y1 &&
      mouseY <= ZOOM_Y2) {
    char dummy;
    return world.zoom_matrix->get_detect_location(mouseX, mouseY, &x, &y,
                                                  &dummy);
  }
...
static void hide_map() {
  ...
  if (config
          .explore_whole_map) // no action if the setting of the map is explored
    return;
  ...
  if (nation_array.player_recno) {
    for (i = 1; i <= nation_array.size(); ++i) {
      if (i == nation_array.player_recno ||
          nation_array.is_deleted(i)) // reject player
        continue;
  ...
  for (i = base_obj_array.size(); i > 0; --i) {
    ...
    if (baseObj->is_visible() && baseObj->explore_for_player()) {
      ...
      if (baseObj->cast_to_Unit()) {
        ...
        world.unveil(baseObj->obj_loc_x1(), baseObj->obj_loc_y1(),
                     baseObj->obj_loc_x2(), baseObj->obj_loc_y2());
        world.visit(baseObj->obj_loc_x1(), baseObj->obj_loc_y1(),
                    baseObj->obj_loc_x2(), baseObj->obj_loc_y2(),
                    EXPLORE_RANGE - 1);
      }
    }
...
static void locate_independent_hero() {
  ...
  for (int i = unit_array.size(); i > 0; i--) {
    ...
    if (unitPtr->hero_id && unitPtr->is_visible() &&
...
static void locate_free_item(int checkExplore) {
  ...
  for (int i = site_array.size(); i > 0; i--) {
    ...
    if (sitePtr->site_type == SITE_ITEM &&
        (!checkExplore ||
         world.is_explored(sitePtr->map_x_loc, sitePtr->map_y_loc,
...
static int filter_unit(FilterUnitFunc filterFunc, int para1, int negate) {
  ...
  for (i = unit_array.size(); i > 0; --i) {
    ...
    if (!negate &&
            (*filterFunc)(unitPtr, para1) // negate xor (*filterFunc)(...)
  ...
  for (i = 1; i <= unit_array.size(); ++i) {
    ...
    if (!unitPtr->selected_flag)
      continue;
    if (!negate &&
            !(*filterFunc)(unitPtr, para1) // negate eqv (*filterFunc)(...)
        || negate && (*filterFunc)(unitPtr, para1)) {
      unitPtr->deselect();
    }
  }
