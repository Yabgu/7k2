...
void Town::setup_link() {
  ...
  for (firmRecno = firm_array.size(); firmRecno > 0; firmRecno--) {
    ...
    if (!firm_res.is_linkable_to_town(firmPtr->firm_id, firmPtr->race_id,
                                      race_id))
      continue;
    ...
    if (nation_recno == 0) // if this is an independent town
    {
      if (firmPtr->nation_recno == 0 ||
          resistance(firmPtr->nation_recno) > INDEPENDENT_LINK_RESISTANCE)
    ...
    if (linked_firm_count < MAX_LINKED_FIRM_TOWN) {
      err_when(linked_firm_count > 0 &&
               linked_firm_array[linked_firm_count - 1] ==
                   firmRecno); // BUGHERE - check double linking error
    ...
    if (firmPtr->linked_town_count < MAX_LINKED_TOWN_TOWN) {
      ...
      firmPtr->linked_town_enable_array[firmPtr->linked_town_count] =
          defaultLinkStatus;
  ...
  for (townRecno = town_array.size(); townRecno > 0; townRecno--) {
    ...
    if (misc.points_distance(townPtr->center_x, townPtr->center_y, center_x,
                             center_y) > world.effective_distance(0, 0)) {
      continue;
    }
...
//0)
//
void Town::toggle_firm_link(int linkId, int toggleFlag, char remoteAction,
                            int setBoth) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <town recno> <link Id> <toggle Flag>
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_TOWN_TOGGLE_LINK_FIRM, 3 * sizeof(short));
  ...
  //	int sameNation = linkedNationRecno == nation_recno ||		// if one
  //of the linked end is an indepdendent firm/nation, consider this link as a
  //single nation link 						  linkedNationRecno == 0 || 						  nation_recno == 0;
...
//0)
//
void Town::toggle_town_link(int linkId, int toggleFlag, char remoteAction,
                            int setBoth) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <town recno> <link Id> <toggle Flag>
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_TOWN_TOGGLE_LINK_TOWN, 3 * sizeof(short));
    shortPtr[0] = town_recno;
    shortPtr[1] = linkId;
    shortPtr[2] = toggleFlag;
    return;
  ...
  int linkedNationRecno =
      town_array[linked_town_array[linkId - 1]]->nation_recno == nation_recno;
...
int Town::has_linked_camp(int nationRecno, int needOverseer) {
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    ...
    if (firmPtr->cast_to_FirmCamp() && firmPtr->nation_recno == nationRecno) {
...
void Town::update_camp_link() {
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    //--- don't set it if the town and camp both belong to a human player, the
    //player will set it himself ---//

    if (firmPtr->nation_recno == nation_recno && nation_recno &&
