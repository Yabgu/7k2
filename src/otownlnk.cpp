...
void Town::setup_link()
{
    ...
    for (firmRecno = firm_array.size(); firmRecno > 0; firmRecno--)
    {
        ...
        if (nation_recno == 0)
        {
            ...
            if (!firmPtr->cast_to_FirmWork() && !firmPtr->cast_to_FirmMarket() && !firmPtr->cast_to_FirmCamp())
            {
                continue;
            }
        }
        ...
        if (linked_firm_count < MAX_LINKED_FIRM_TOWN)
        {
            err_when(linked_firm_count > 0 &&
                     linked_firm_array[linked_firm_count - 1] == firmRecno); // BUGHERE - check double linking error
...
void Town::toggle_firm_link(int linkId, int toggleFlag, char remoteAction, int setBoth)
{
    ...
    //	int sameNation = linkedNationRecno == nation_recno ||		// if
    // one
...
void Town::update_camp_link()
{
    ...
    for (i = 0; i < linked_firm_count; i++)
    {
        ...
        if (firmPtr->nation_recno == nation_recno && nation_recno && !nation_array[nation_recno]->is_ai())
        {
            continue;
        }
    ...
    for (i = 0; i < linked_firm_count; i++)
    {
        ...
        if (!firmPtr->cast_to_FirmCamp() || firmPtr->cast_to_FirmCamp()->overseer_recno == 0)
        {
            continue;
        }
