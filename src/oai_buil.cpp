...
void Nation::think_build_firm() {
  ...
  //--- think about attacking units that stand idle on mines ---//

  think_attack_units_standing_on_mine();
...
int Nation::think_build_mine() {
  ...
  int rawId =
      seek_mine(xLoc, yLoc, refXLoc,
                refYLoc); // reference location is the raw material location
  ...
  if (!config.explore_whole_map) {
    ...
    for (i = 0; i < ai_town_count; i++) {
      ...
      int rawDistance = misc.points_distance(xLoc, yLoc, townPtr->center_x,
                                             townPtr->center_y);
  ...
  return add_action(xLoc, yLoc, refXLoc, refYLoc, ACTION_AI_BUILD_FIRM,
                    FIRM_MINE);
}
...
int Nation::ai_should_build_mine() {
  ...
  if (total_jobless_population <
      16) // only build mine when you have enough population to support the
          // economic chain: mine + factory + camp
    return 0;
  ...
  //--- don't build additional mines unless we have enough population and demand
  //to support it ----//

  if (ai_mine_count >= 1) {
    if (true_profit_365days() < 0 ||
        total_population < 40 + pref_economic_development / 5)
      return 0;
  }
  ...
  //--- if the population is low, make sure existing mines are in full
  //production before building a new one ---//
  ...
  if (total_jobless_population < 30) {
    ...
    for (int i = 0; i < ai_mine_count; i++) {
      ...
      if (firmMine->linked_firm_count == 0 &&
          !firmMine->no_neighbor_space) // if the firm does not have any linked
                                        // firms, that means the firm is still
...
//
void Nation::ai_attack_unit_in_area(int xLoc1, int yLoc1, int xLoc2,
                                    int yLoc2) {
  int enemyXLoc, enemyYLoc, enemyCombatLevel = 0;
  int enemyStatus = RELATION_FRIENDLY;
  Location *locPtr;
  Unit *unitPtr;
  ...
  for (int yLoc = yLoc1; yLoc <= yLoc2; yLoc++) {
    for (int xLoc = xLoc1; xLoc <= xLoc2; xLoc++) {
      ...
      //----- if this is our spy cloaked in another nation, reveal its true
      //identity -----//
      ...
      if (unitPtr->nation_recno == nation_recno) {
        unitPtr
            ->think_normal_human_action(); // send the unit to a firm or a town
      } else //--- if it is an enemy unit, attack it ------//
      {
        int nationStatus = get_relation_status(unitPtr->nation_recno);

        if (nationStatus <
            enemyStatus) // if the status is worse than the current target
        {
          enemyXLoc = xLoc;
          enemyYLoc = yLoc;
          enemyStatus = nationStatus;
          enemyCombatLevel += (int)unitPtr->obj_power();
        }
      }
    }
  }
...
int Nation::think_destroy_raw_site_guard() {
  ...
  for (int i = site_array.size(); i > 0; i--) {
    ...
    int enemyCombatLevel =
        mobile_defense_combat_level(sitePtr->map_x_loc, sitePtr->map_y_loc,
                                    unitPtr->nation_recno, 1, hasWar);

    if (enemyCombatLevel ==
        -1) // a war is going on here, don't attack this target
      continue;

    if (ai_attack_target(sitePtr->map_x_loc, sitePtr->map_y_loc,
                         enemyCombatLevel, 0, 0, 0, 0, 1)) // 1-use all camps
      return 1;
  }
...
int Nation::ai_supported_inn_count() {
  ...
  innCount = MIN(3 + pref_hire_unit / 35,
                 innCount); // maximum 3 to 5 inns, minimum 1 inn.
...
//---- Begin of function Nation::think_attack_units_standing_on_mine ----//

int Nation::think_attack_units_standing_on_mine() {
  for (int i = site_array.size(); i > 0; i--) {
    if (site_array.is_deleted(i))
      continue;
    ...
    Location *siteLocPtr =
        world.get_loc(sitePtr->map_x_loc, sitePtr->map_y_loc);

    if (world.can_build_firm(sitePtr->map_x_loc, sitePtr->map_y_loc, FIRM_MINE))
      continue;

    //----- attack enemy units standing on this location ----//

    for (int yLoc = sitePtr->map_y_loc; yLoc <= sitePtr->map_y2; yLoc++) {
      for (int xLoc = sitePtr->map_x_loc; xLoc <= sitePtr->map_x2; xLoc++) {
        Location *locPtr = world.get_loc(xLoc, yLoc);

        int unitRecno = locPtr->unit_recno('L');

        if (!unitRecno)
          continue;

        if (unit_array[unitRecno]->cur_action !=
            SPRITE_IDLE) // only attack units that stand on the mine
          continue;

        int nationRecno = unit_array[unitRecno]->nation_recno;

        if (nationRecno == nation_recno)
          continue;

        //--- attack the units standing on the mine if the units' relationship
        //<= Neutral ---//

        if (get_relation_status(nationRecno) <= RELATION_NEUTRAL) {
          ai_attack_target(xLoc, yLoc, unit_array[unitRecno]->obj_power(), 0, 0,
                           0, 0, 1); // 1-use all camps
          return 1;
        }
      }
    }
...
//---- End of function Nation::think_attack_units_standing_on_mine ----//
