...
int Nation::think_build_mine()
{
    ...
    int rawId = seek_mine(xLoc, yLoc, refXLoc,
    ...
    if (!config.explore_whole_map)
    {
        ...
        for (i = 0; i < ai_town_count; i++)
        {
            ...
            if (info.game_date - info.game_start_date > rawDistance * (5 - config.ai_aggressiveness) / 5) // 3 to 5 / 5
            {
                break;
            }
        }
...
int Nation::ai_should_build_mine()
{
    ...
    if (total_jobless_population < 16) // only build mine when you have enough population to support the
    ...
    if (total_jobless_population < 30)
    {
        ...
        for (int i = 0; i < ai_mine_count; i++)
        {
            ...
            if (firmMine->linked_firm_count == 0 && !firmMine->no_neighbor_space) // if the firm does not have any
                                                                                  // linked firms, that means the firm
...
void Nation::ai_attack_unit_in_area(int xLoc1, int yLoc1, int xLoc2, int yLoc2)
{
    ...
    for (int yLoc = yLoc1; yLoc <= yLoc2; yLoc++)
    {
        for (int xLoc = xLoc1; xLoc <= xLoc2; xLoc++)
        {
            ...
            if (unitPtr->nation_recno != nation_recno && unitPtr->true_nation_recno() == nation_recno)
            {
...
int Nation::think_destroy_raw_site_guard()
{
    ...
    for (int i = site_array.size(); i > 0; i--)
    {
        ...
        int enemyCombatLevel =
            mobile_defense_combat_level(sitePtr->map_x_loc, sitePtr->map_y_loc, unitPtr->nation_recno, 1, hasWar);

        if (enemyCombatLevel == -1) // a war is going on here, don't attack this target
            continue;

        if (ai_attack_target(sitePtr->map_x_loc, sitePtr->map_y_loc, enemyCombatLevel, 0, 0, 0, 0,
                             1)) // 1-use all camps
            return 1;
    }
...
int Nation::think_attack_units_standing_on_mine()
{
    for (int i = site_array.size(); i > 0; i--)
    {
        ...
        Location *siteLocPtr = world.get_loc(sitePtr->map_x_loc, sitePtr->map_y_loc);
        ...
        for (int yLoc = sitePtr->map_y_loc; yLoc <= sitePtr->map_y2; yLoc++)
        {
            for (int xLoc = sitePtr->map_x_loc; xLoc <= sitePtr->map_x2; xLoc++)
            {
                ...
                if (unit_array[unitRecno]->cur_action != SPRITE_IDLE) // only attack units that stand on the mine
                    continue;
                ...
                {
                    ai_attack_target(xLoc, yLoc, unit_array[unitRecno]->obj_power(), 0, 0, 0, 0, 1); // 1-use all camps
                    return 1;
                }
            }
        }
