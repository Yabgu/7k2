...
void ScenarioEditor::init_monster_mode() {
  ...
  monster_firm_group_array =
      (short *)mem_add(firm_res.firm_group_count * sizeof(short));
...
void ScenarioEditor::disp_monster_main(int refreshFlag) {
  ...
  if (!unit_or_struct) {
    ...
    if (refreshFlag == INFO_REPAINT) {
      ...
      vbrowse_firm_build.init(INFO_X1 + 5, INFO_Y1 + 80, INFO_X2 - 5,
                              INFO_Y2 - 30, -1, 20, monster_firm_group_count,
                              disp_monster_firm_group);
  ...
  if (refreshFlag == INFO_REPAINT) {
    //		int raceLength = ((INFO_X2-4) - (INFO_X1+5) + 1) /
    //(1+MAX_MONSTER_TYPE); 		char raceAbbr[2] = " "; 		for( int r = 0; r <=
    //MAX_MONSTER_TYPE; ++r )
    //		{
    //			if( r > 0 )
    ...
    //		}
    spinner_race_group.create(
        INFO_X1 + 80, INFO_Y1 + 58, INFO_X1 + 210, INFO_Y1 + 78, 200, // 146,
  ...
  if (mouse.in_area(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2)) {
    ...
    // construction marker
      ...
      if (unit_or_struct == 0 && !vbrowse_unit_id.none_record) {
        ...
        err_when(
            monster_firm_group_array[monster_firm_group_browse_recno - 1] == 0);
        // put firm group
        // monster_firm_group_array[monster_firm_group_browse_recno] on (xLoc,
        // yLoc)

        FirmGroup *firmGroup = firm_res.get_group(
            monster_firm_group_array[monster_firm_group_browse_recno - 1]);
    ...
    if (world.zoom_matrix->get_detect_location(
      ...
      if (unit_or_struct == 0 && !vbrowse_unit_id.none_record) {
        ...
        if (world.check_unit_space(xLoc, yLoc, xLoc + spriteInfo->loc_width - 1,
                                   yLoc + spriteInfo->loc_height - 1,
                                   unitInfo->mobile_type)) {
          unit_array.add_unit(unitId, brush_player_recno, RANK_SOLDIER, 100,
                              xLoc, yLoc);
        }
      } else if (unit_or_struct == 1 && !vbrowse_firm_build.none_record) {
        err_when(
        ...
        if (world.can_build_firm(xLoc, yLoc, firmGroup->firm_id, 0)) {
          if (brush_player_recno &&
              nation_array[brush_player_recno]->is_human() &&
              firm_res[firmGroup->firm_id]->setup_live_points_cost > 0) {
            box.msg(
                text_editor.str_no_human()); // "Human Kingdom cannot possess
                                             // Fryhtan structure" );
          } else if (brush_player_recno == 0 &&
                     firmGroup->firm_id != FIRM_LAIR) {
            box.msg(
                text_editor.str_no_independent()); // "Cannot add any
                                                   // Independent structures" );
          } else {
            firm_array.generate_firm(xLoc, yLoc, brush_player_recno,
                                     firmGroup->firm_id, firmGroup->race_code);
          }
        }
      }
    }
...
void ScenarioEditor::collect_monster_unit(int raceId) {
  ...
  if (monster_unit_id_browse_recno >= 1 &&
      monster_unit_id_browse_recno <= monster_unit_id_count) {
    ...
    case UNIT_CLASS_MONSTER:
      if (!raceId || unitInfo->race_id == raceId) {
        ...
        if (i == oldSelectedUnitId)
          monster_unit_id_browse_recno =
              monster_unit_id_count; // after monster_unit_id_count incremented
      }
      break;
...
void ScenarioEditor::collect_monster_firm(int raceId) {
  ...
  if (monster_firm_group_browse_recno >= 1 &&
      monster_firm_group_browse_recno <= monster_firm_group_count) {
  ...
  for (int i = 1; i <= firm_res.firm_group_count; ++i) {
    ...
    if (firmGroup->race_id < 0 && (!raceId || firmGroup->race_id == raceId)) {
      ...
      if (oldSelectedFirmGroup == i) {
        monster_firm_group_browse_recno =
            monster_firm_group_count; // increased monster_firm_group_count
      }
    }
  }
}
...
static void disp_monster_unit_id(int recNo, int x, int y, int refreshFlag) {
  if (recNo > 0 && recNo <= scenario_editor.monster_unit_id_count)
    font_san.put(
        x + 6, y + 2,
        unit_res[scenario_editor.monster_unit_id_array[recNo - 1]]->name, 0,
...
static void disp_monster_firm_group(int recNo, int x, int y, int refreshFlag) {
  if (recNo > 0 && recNo <= scenario_editor.monster_firm_group_count) {
    String str;
    FirmGroup *firmGroup =
        firm_res.get_group(scenario_editor.monster_firm_group_array[recNo - 1]);
#if (0)
    // format is <firm> <race>
    ...
    if (firmGroup->firm_id !=
        FIRM_LAIR) // name in firm build already has monster name
    {
      str += " ";
