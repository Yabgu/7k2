...
void ScenarioEditor::disp_monster_main(int refreshFlag)
{
    ...
    if (!unit_or_struct)
    {
        ...
        {
            // init unit browser
            vbrowse_unit_id.init(INFO_X1 + 5, INFO_Y1 + 80, INFO_X2 - 5, INFO_Y2 - 30, -1, 20, monster_unit_id_count,
                                 disp_monster_unit_id);
    ...
    else
    {
        if (refreshFlag == INFO_REPAINT)
        {
            ...
            vbrowse_firm_build.init(INFO_X1 + 5, INFO_Y1 + 80, INFO_X2 - 5, INFO_Y2 - 30, -1, 20,
                                    monster_firm_group_count, disp_monster_firm_group);
    ...
    if (refreshFlag == INFO_REPAINT)
    {
        ...
        // );
        //		}
        spinner_race_group.create(INFO_X1 + 80, INFO_Y1 + 58, INFO_X1 + 210, INFO_Y1 + 78,
                                  200, // 146,
    ...
    if (mouse.in_area(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2))
    {
        ...
        else if (world.zoom_matrix->get_detect_location(mouse.cur_x, mouse.cur_y, &xLoc, &yLoc, NULL))
        {
            ...
            else if (unit_or_struct == 1 && !vbrowse_firm_build.none_record)
            {
                ...
                FirmGroup *firmGroup =
                    firm_res.get_group(monster_firm_group_array[monster_firm_group_browse_recno - 1]);
        ...
        if (world.zoom_matrix->get_detect_location(mouse.click_x(0), mouse.click_y(0), &xLoc, &yLoc, NULL))
        {
            ...
            else if (unit_or_struct == 1 && !vbrowse_firm_build.none_record)
            {
                ...
                if (world.can_build_firm(xLoc, yLoc, firmGroup->firm_id, 0))
                {
                    ...
                    {
                        box.msg(text_editor.str_no_human()); // "Human Kingdom cannot possess
                                                             // Fryhtan structure" );
                    }
                    else if (brush_player_recno == 0 && firmGroup->firm_id != FIRM_LAIR)
                    {
                        box.msg(text_editor.str_no_independent()); // "Cannot add any
