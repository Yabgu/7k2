...
void YLightning::init(double fromX, double fromY, double toX, double toY, char energy)
{
    for (int i = 0; i < MAX_BRANCH; ++i)
    {
        delete branch[i];
...
void YLightning::move_particle()
{
    ...
    if ((rand_seed() % 1000) <= branch_prob && used_branch < MAX_BRANCH)
    {
        ...
        double branchDist = Lightning::dist(destx - x, desty - y) * (32 + rand_seed() % 16) / 64.0;
        double branchAngle =
            atan2(desty - y, destx - x) + (4 + rand_seed() % 8) * (branch_left ? PI / -32.0 : PI / 32.0);
        branch_left = !branch_left;

        branch[used_branch]->init(x, y, x + branchDist * cos(branchAngle), y + branchDist * sin(branchAngle),
                                  branch_energy);
