...
void FirmDieRes::init()
{
    ...
    //	res_bitmap.init_imported(str,0,1);		// 0 - do not read all
    // data
    // into buffer
    str = DIR_RES;
    str += "PALFIRMD.RES";
...
void FirmDieRes::load_build_info()
{
    ...
    Database *dbFirmBuild = game_set.open_db(FIRM_BUILD_DB); // only one database can be opened at a
...
void FirmDie::init(short firmId, short firmBuildId, short nationRecno, short locX1, short locY1, short locX2,
                   short locY2, short alt)
{
    firm_id = firmId;
                ...
                if (firmBitmap->ani_part == die_flag)
                {
                    if (start_frame == 0)
                    {
                        ...
                        if (firmBitmap->random_flag == 'R' ||
                            (firmBitmap->random_flag > '0' && firmBitmap->random_flag <= '9'))
                        {
...
void FirmDie::draw(int displayLayer)
{
    ...
    if (firmBuild->ground_bitmap_recno)
    {
        ...
        //			->draw_at(loc_x1*ZOOM_LOC_WIDTH, loc_y1*ZOOM_LOC_HEIGHT,
        // NULL,
        // displayLayer);
    }
