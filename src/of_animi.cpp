...
void FirmAnimal::detect_info()
{
    ...
    if (button_hire.detect() && selected_unit_id > 0 && selected_unit_id <= animal_unit_count)
    {
        AnimalUnit *animalUnit = animal_unit_array + selected_unit_id - 1;
        int temp;
        ...
        se_res.far_sound(center_x, center_y, 1, 'S', unit_res[animalUnit->unit_id]->sprite_id, "RDY");
        if (remote.is_enable())
        {
    ...
    for (int inc = -1; inc <= 1; inc += 2)
    {
        ...
        for (int i = inc >= 0 ? 1 : animal_unit_count; i > 0 && i <= animal_unit_count; i += inc)
        {
            ...
            if (inc < 0)
            {
                ...
                info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, animalUnit->unit_id, nation_recno, windowX1, windowY1,
                                    windowX2, windowY2, 1);
            }
            else
            {
                ...
                if (info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, animalUnit->unit_id, nation_recno, windowX1,
                                        windowY1, windowX2, windowY2, inAreaFlag | (i == selected_unit_id ? 2 : 0)) &
                    4)
                {
                ...
                font_whbl.center_put(x, yHp, x + UNIT_X_SPACING, yHp + fontHeight,
                                     misc.format(animalUnit->combat_level()));
            }
        }
    }
}
...
void FirmAnimal::detect_unit_list()
{
    ...
    for (int i = 1; i <= animal_unit_count; ++i)
    {
        ...
        int rc = info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, animalUnit->unit_id, nation_recno, windowX1, windowY1,
                                     windowX2, windowY2,
...
void FirmAnimal::disp_unit_info(int dispY1, int refreshFlag)
{
    ...
    font_snds.put(x, y, text_firm.str_hire_animal_cost(animalUnit->hire_cost, animalUnit->live_point_cost), 0,
                  INFO_X2 - 6);
...
void FirmAnimal::draw(int displayLayer)
{
    ...
    int iconOffsetX[MAX_DISPLAY_SLOT * 2] = {90, 138, 90, 138, 42, 10, 50, 10, 50, 10};
    int iconOffsetY[MAX_DISPLAY_SLOT * 2] = {90, 90, 138, 138, 138, 0, 0, 30, 30, 60};
    ...
    if (displayLayer & 1)
    {
        ...
        /*		// display one icon for each type of animal
                        ...
                        {
                                for( i = 0; i < ANIMAL_TYPE_COUNT &&
           animal_unit_array[j].unit_id != animal_unit_id[i]; ++i ); if( i <
           ANIMAL_TYPE_COUNT ) animalCount[i]++;
                        }
                        ...
                        for( i = 0; i < MIN(ANIMAL_TYPE_COUNT,unitIconCount) ; ++i )
                        {
                                ...
                                {
                                        int spriteId =
           unit_res[animal_unit_id[i]]->sprite_id; SpriteInfo *spriteInfo =
           sprite_res[spriteId]; const int dirId = 3; int spriteFrameRecno =
           spriteInfo->stop_array[dirId].frame_recno; int frameCount =
           spriteInfo->stop_array[dirId].frame_count;

                                        if( spriteFrameRecno )
                                        {
                                                spriteFrameRecno += (sys.frame_count
           + firm_recno) % frameCount;	// animation

                                                SpriteFrame *spriteFrame =
           sprite_frame_res[spriteFrameRecno]; char* bitmapPtr =
           spriteInfo->res_bitmap.read_imported(spriteFrame->bitmap_offset); short*
           colorRemapTable = spriteInfo->get_color_remap_table( 0, 0);

                                                world.zoom_matrix->put_bitmap_offset(
                                                        loc_x1 * LOCATE_WIDTH +
           iconOffsetX[i], loc_y1 * LOCATE_HEIGHT + iconOffsetY[i], altitude,
           bitmapPtr, spriteFrame->offset_x, spriteFrame->offset_y, colorRemapTable,
           0, 1 );
                                        }
                                }
                        }
            ...
            if (spriteFrameRecno)
            {
                ...
                if (config.building_size == 1)
                    world.zoom_matrix->put_bitmap_offset(
                        ...
                        loc_y1 * LOCATE_HEIGHT + iconOffsetY[animalPtr->display_slot_id], altitude, bitmapPtr,
                        spriteFrame->offset_x, spriteFrame->offset_y, colorRemapTable, 0, 1);
                else
                    world.zoom_matrix->put_bitmap_offset(
                        loc_x1 * LOCATE_WIDTH + iconOffsetX[animalPtr->display_slot_id + 5],
                        loc_y1 * LOCATE_HEIGHT + iconOffsetY[animalPtr->display_slot_id + 5], altitude, bitmapPtr,
                        spriteFrame->offset_x, spriteFrame->offset_y, colorRemapTable, 0, 1);
            }
        }
    }
