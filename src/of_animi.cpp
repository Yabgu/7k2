...
void FirmAnimal::detect_info() {
  ...
  if (button_hire.detect() && selected_unit_id > 0 &&
      selected_unit_id <= animal_unit_count) {
    AnimalUnit *animalUnit = animal_unit_array + selected_unit_id - 1;
    int temp;
    ...
    se_res.far_sound(center_x, center_y, 1, 'S',
                     unit_res[animalUnit->unit_id]->sprite_id, "RDY");
    if (remote.is_enable()) {
...
void FirmAnimal::disp_unit_list(int dispY1, int refreshFlag) {
  ...
  for (int inc = -1; inc <= 1; inc += 2) {
    ...
    for (int i = inc >= 0 ? 1 : animal_unit_count;
         i > 0 && i <= animal_unit_count; i += inc) {
      ...
      if (inc < 0) {
        ...
        info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, animalUnit->unit_id,
                            nation_recno, windowX1, windowY1, windowX2,
                            windowY2, 1);
      } else {
        // second round is ascending to draw the frame

        if (info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, animalUnit->unit_id,
                                nation_recno, windowX1, windowY1, windowX2,
                                windowY2,
                                inAreaFlag | (i == selected_unit_id ? 2 : 0)) &
            4) {
...
void FirmAnimal::detect_unit_list() {
  ...
  for (int i = 1; i <= animal_unit_count; ++i) {
    ...
    int rc = info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, animalUnit->unit_id,
                                 nation_recno, windowX1, windowY1, windowX2,
...
void FirmAnimal::disp_unit_info(int dispY1, int refreshFlag) {
  ...
  font_snds.put(x, y,
                text_firm.str_hire_animal_cost(animalUnit->hire_cost,
                                               animalUnit->live_point_cost),
                0, INFO_X2 - 6);
  ...
  if (displayLayer & 1) {
    ...
    */
    ...
    for (j = 0; j < MAX_DISPLAY_SLOT; ++j) {
      ...
      if (animalPtr->display_slot_id < 0 ||
          animalPtr->display_slot_id >= MAX_DISPLAY_SLOT)
        continue;
      ...
      if (spriteFrameRecno) {
        spriteFrameRecno +=
            (sys.frame_count + firm_recno) % frameCount; // animation

        SpriteFrame *spriteFrame = sprite_frame_res[spriteFrameRecno];
        char *bitmapPtr =
            spriteInfo->res_bitmap.read_imported(spriteFrame->bitmap_offset);
        ...
        if (config.building_size == 1)
          world.zoom_matrix->put_bitmap_offset(
              ...
              altitude, bitmapPtr, spriteFrame->offset_x, spriteFrame->offset_y,
              colorRemapTable, 0, 1);
        else
          world.zoom_matrix->put_bitmap_offset(
              loc_x1 * LOCATE_WIDTH +
                  iconOffsetX[animalPtr->display_slot_id + 5],
              loc_y1 * LOCATE_HEIGHT +
                  iconOffsetY[animalPtr->display_slot_id + 5],
              altitude, bitmapPtr, spriteFrame->offset_x, spriteFrame->offset_y,
              colorRemapTable, 0, 1);
      }
    }
