...
//
void Game::reset_win_condition() { win_condition_count = 0; }
...
//
void Game::add_win_condition(int condId, int playerOnly, int p1, int p2,
                             int yearLimit) {
  err_when(win_condition_count >= MAX_WIN_CONDITION);

  win_condition[win_condition_count].init(condId, playerOnly, p1, p2, yearLimit,
                                          0);
...
void Game::del_win_condition(int i) {
  if (i > 0 && i <= win_condition_count) {
    misc.del_array_rec(win_condition, MAX_WIN_CONDITION,
                       sizeof(win_condition[0]), i);
    --win_condition_count;
  }
}
...
void Game::add_lose_condition(int condId, int playerOnly, int p1, int p2,
                              int yearLimit) {
  err_when(lose_condition_count >= MAX_LOSE_CONDITION);

  lose_condition[lose_condition_count].init(condId, playerOnly, p1, p2,
                                            yearLimit, 1);
...
void Game::del_lose_condition(int i) {
  if (i > 0 && i <= lose_condition_count) {
    misc.del_array_rec(lose_condition, MAX_LOSE_CONDITION,
                       sizeof(lose_condition[0]), i);
    --lose_condition_count;
  }
}
