...
void Log::mark_begin() {
  ...
  text_buffer[0].clear();
  *(text_buffer[0].reserve(sizeof(char))) =
      '\0'; // put a null character in the buffer
}
...
void Log::mark_end() {
  ...
  *(text_buffer[LOG_VERSION - 1].reserve(sizeof(char))) =
      '\0'; // put a null character in the buffer
...
void Log::mark(const char *msg, const char *file, int line) {
  ...
  String t(log_text);
  t += "\r\n";
  strcpy(text_buffer[0].reserve(t.len()) - 1,
         t); // minus 1 to remove the '\0' at the end
}
...
void Log::dump() {
  ...
  File f;
  for (int n = LOG_VERSION - 1; n >= 0;
       --n, filename[3]++) // AM_A.LOG, AM_B.LOG ...
  {
