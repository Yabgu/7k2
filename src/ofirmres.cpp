...
void FirmRes::load_firm_bitmap()
{
    ...
    for (i = 0; i < firm_bitmap_count; i++)
    {
        ...
        firmBitmap->offset_x +=
            -firmBitmap->loc_width * LOCATE_WIDTH / 2 -
            (-firmBitmap->loc_width * ZOOM_LOC_X_WIDTH / 2 + -firmBitmap->loc_height * ZOOM_LOC_Y_WIDTH / 2);
        firmBitmap->offset_y +=
            -firmBitmap->loc_height * LOCATE_HEIGHT / 2 -
            (-firmBitmap->loc_width * ZOOM_LOC_X_HEIGHT / 2 + -firmBitmap->loc_height * ZOOM_LOC_Y_HEIGHT / 2);
    }
}
...
void FirmRes::load_firm_group()
{
    ...
    if (config.building_size == 1)
        dbFirmGroup = game_set.open_db(FIRM_GROUP_DB); // only one database can be opened at a
...
void FirmRes::load_firm_build()
{
    ...
    if (config.building_size == 1)
        dbFirmBuild = game_set.open_db(FIRM_BUILD_DB); // only one database can be opened at a
                                                       // time, so we read FIRM.DBF first
    else
    ...
    for (i = 0; i < firm_build_count; i++)
    {
        ...
        // firmBuildRec->pal_file_name == '\0' ) 			firmBuild->palw_ptr = NULL;
        // else
        //		{
        //			long paletteOffset;
        ...
        //			firmBuild->palw_ptr = (short *)mem_add( 0x100 *
        // sizeof(short)
        //);
        //
        //			for( int j = 0; j < 0x100; ++j )
        //			{
        //				firmBuild->palw_ptr[j] = vga.make_pixel( j+(RGBColor
        //*)palPtr
        //);
        //			}
        //		}
        ...
        firmBuild->under_construction_bitmap_recno =
            misc.atoi(firmBuildRec->under_construction_bitmap_recno, firmBuildRec->BITMAP_RECNO_LEN);
...
int FirmInfo::nation_can_build(int nationRecno, int raceId, int checkCash)
{
    ...
    if (checkCash)
    {
        if (nationPtr->cash < setup_cost || nationPtr->live_points < setup_live_points_cost)
        {
            return 0;
        }
    }
    ...
    if (firm_id == FIRM_BASE) // only if the nation has acquired the myth to build it
    {
        ...
        return 0;
#else
        if (game.game_mode == GAME_TUTORIAL && nationRecno != nation_array.player_recno)
        {
            return 0;
        }

        if (!(nationPtr->base_count_array[raceId - 1] == 0 && nationPtr->know_base_array[raceId - 1]))
        {
            return 0;
        }
#endif
...
#ifdef DEMO
    if (firm_id == FIRM_OFFENSIVE_BUILDING_1 || firm_id == FIRM_OFFENSIVE_BUILDING_2 ||
        firm_id == FIRM_OFFENSIVE_BUILDING_3 || firm_id == FIRM_OFFENSIVE_BUILDING_4)
    {
        return 0; // cannot build monster fortress
    }
#endif
...
//						==0 if the firm does not have that
// build
// code
//
int FirmInfo::get_build_id(char *buildCode)
{
    ...
    //	{
    //		if( strcmp(buildCode,
    // firm_res.get_group(firmBuildId)->build_code)==0 ) 			return
    // firmBuildId;
    //	}
...
#define REMAP_KEY_COUNT 8
        memcpy(palw_ptr + FIRST_REMAP_KEY, game.get_color_remap_table(nationRecno, selectedFlag) + FIRST_REMAP_KEY,
...
void FirmBitmap::draw_at(int absX, int absY, int absZ, short *colorRemapTable, int displayLayer, FirmBuild *firmBuild)
{
    ...
    world.zoom_matrix->put_bitmap_offset(absX, absY, absZ, bitmapPtr, offset_x, offset_y, colorRemapTable, 0, 1);
}
...
//
int FirmRes::is_linkable_to_firm(int firmId1, int firmRaceId1, int firmNationRecno1, int firmId2, int firmRaceId2,
                                 int firmNationRecno2)
{
    //---- special case: nation lair to independent lair ----//

    if (firmId1 == FIRM_LAIR && firmId2 == FIRM_LAIR)
    {
        return (firmNationRecno1 == 0 && firmNationRecno2 > 0) || (firmNationRecno1 > 0 && firmNationRecno2 == 0);
    }
    ...
    return !(firmInfo1->same_race_worker || firmInfo2->same_race_worker) || firmRaceId1 == firmRaceId2;
}
