...
void FirmRes::init() {
  ...
  load_firm_bitmap(); // call load_firm_bitmap() first as load_firm_info() will
...
void FirmRes::load_firm_bitmap() {
  ...
  firm_bitmap_array =
      (FirmBitmap *)mem_add(sizeof(FirmBitmap) * firm_bitmap_count);
...
void FirmRes::load_firm_group() {
  ...
  if (config.building_size == 1)
    dbFirmGroup =
        game_set.open_db(FIRM_GROUP_DB); // only one database can be opened at a
  ...
  for (i = 0; i < firm_group_count; i++) {
    ...
    misc.rtrim_fld(firmGroup->firm_code, firmGroupRec->firm_code,
                   firmGroup->FIRM_CODE_LEN);

    firmGroup->group = misc.atoi(firmGroupRec->group, firmGroupRec->GROUP_LEN);

    firmGroup->race_id =
        misc.atoi(firmGroupRec->race_id, firmGroupRec->RACE_ID_LEN);

    misc.rtrim_fld(firmGroup->race_code, firmGroupRec->race_code,
                   firmGroup->RACE_CODE_LEN);
  }
}
...
void FirmRes::load_firm_build() {
  ...
  for (i = 0; i < firm_build_count; i++) {
    ...
    //		if( firmBuildRec->pal_file_name[0] == ' ' ||
    //firmBuildRec->pal_file_name == '\0' ) 			firmBuild->palw_ptr = NULL; 		else
    //		{
    //			long paletteOffset;
    //			memcpy(&paletteOffset, firmBuildRec->pal_offset,
    //sizeof(long)); 			BYTE *palPtr = 8+(BYTE
    //*)res_pal.read_imported(paletteOffset);	// skip 8 bytes header
    ...
    firmBuild->defense_attribute.init(
        misc.atoi(firmBuildRec->sturdiness, firmBuildRec->ATTRIBUTE_LEN),
    ...
    //		firmBuild->min_offset_y = misc.atoi(firmBuildRec->min_offset_y,
    //firmBuildRec->OFFSET_LEN);
    err_when(firmBuild->frame_count > MAX_FIRM_FRAME);

    firstFrameArray[i] =
        misc.atoi(firmBuildRec->first_frame, firmBuildRec->FIRST_FRAME_LEN);
  }
...
void FirmRes::load_firm_info() {
  ...
  if (config.building_size == 1)
    dbFirm = game_set.open_db(FIRM_DB); // only one database can be opened at a
                                        // time, so we read FIRM.DBF first
  else
  ...
  for (i = 0; i < firm_count; i++) {
    ...
    misc.rtrim_fld(firmInfo->short_name, firmRec->short_name,
                   firmRec->SHORT_NAME_LEN);

    misc.rtrim_fld(firmInfo->overseer_title, firmRec->overseer_title,
                   firmRec->TITLE_LEN);
    misc.rtrim_fld(firmInfo->worker_title, firmRec->worker_title,
                   firmRec->TITLE_LEN);
    ...
    firmInfo->max_hit_points =
        misc.atoi(firmRec->hit_points, firmRec->HIT_POINTS_LEN);

    firmInfo->first_build_id =
        misc.atoi(firmRec->first_build, firmRec->FIRST_BUILD_LEN);
    firmInfo->build_count =
        misc.atoi(firmRec->build_count, firmRec->BUILD_COUNT_LEN);

    firmInfo->first_group_id =
        misc.atoi(firmRec->first_group, firmRec->FIRST_GROUP_LEN);
    firmInfo->group_count =
        misc.atoi(firmRec->group_count, firmRec->GROUP_COUNT_LEN);

    firmInfo->need_overseer =
        firmInfo->overseer_title[0] && firmInfo->overseer_title[0] != ' ';
    firmInfo->need_worker =
        firmInfo->worker_title[0] && firmInfo->worker_title[0] != ' ';

    //		if( firmInfo->firm_id == FIRM_LAIR )			//
    //BUGHERE
    //		{
    ...
    firmInfo->setup_live_points_cost =
        misc.atoi(firmRec->setup_live_points_cost, firmRec->COST_LEN);
    ...
    if (firmRec->all_know == '1')
      memset(firmInfo->nation_tech_level_array, 1,
             sizeof(firmInfo->nation_tech_level_array));
    //		memset( firmInfo->nation_tech_level_array, 1,
    //sizeof(firmInfo->nation_tech_level_array) );
    ...
    if (firmInfo->first_build_id && firmInfo->build_count > 0) {
      for (int j = firmInfo->first_build_id;
           j < firmInfo->first_build_id + firmInfo->build_count; ++j) {
    ...
    {
      for (int j = firmInfo->first_group_id;
           j < firmInfo->first_group_id + firmInfo->group_count; ++j) {
    ...
    for (int j = 1; j <= MAX_MONSTER_TYPE; j++) {
      if (firmInfo->get_build_id(monster_res[j]->firm_build_code)) {
        ...
        else // if more than one monster type can build this firm, reset
             // only_build_race_id to zero
        {
  ...
  //-- the size of camp and fort must be the same as camp can be upgraded to
  //fort --//
...
FirmInfo *FirmRes::operator[](int firmId) {
  err_if(firmId < 1 || firmId > firm_count) err_now("FirmRes::operator[]");
...
int FirmInfo::can_build(int unitRecno, int checkCash) {
  ...
  if (unitPtr->is_monster())
    firmBuildId =
        get_build_id(monster_res[unitPtr->monster_id()]->firm_build_code);
  else
  ...
  //--- see if the unit can build the firm, this is determined by whether the
  //unit is a civilian or a military unit ---//

  if (unitPtr->is_human()) {
    // ####### begin Gilbert 6/5 #######//
    if (is_military) // if this is a military building, which can be built by
                     // all units (both civilian and military)
    {
      // except offensive structure which only be built by military unit (reason
      // : space for buttons...)
    ...
    // ####### end Gilbert 6/5 #######//
    else // if this is a civilian building, it can only be built by civilian
         // units.
      return unitPtr->is_civilian();
  } else {
    return 1;
...
int FirmInfo::nation_can_build(int nationRecno, int raceId, int checkCash) {
  ...
  //--- human nation cannot build monster firms before monster firms cost live
  //points ---//
  ...
  if (firm_id == FIRM_SPECIAL) {
    ...
    if (!tech_res[TECH_SPU_STRUCTURE(raceId)]->get_nation_tech_level(
            nationRecno))
      return 0;
  }
  ...
  if (firm_id ==
      FIRM_BASE) // only if the nation has acquired the myth to build it
  {
...
int FirmInfo::is_linkable_to_firm(int linkFirmId) {
  switch (firm_id) {
  case FIRM_FACTORY:
    return linkFirmId == FIRM_MINE ||
           linkFirmId == FIRM_MARKET; // || linkFirmId==FIRM_HARBOR;

  case FIRM_MINE:
    return linkFirmId == FIRM_FACTORY ||
           linkFirmId == FIRM_MARKET; // || linkFirmId==FIRM_HARBOR;

  case FIRM_MARKET:
    return linkFirmId == FIRM_MINE ||
           linkFirmId == FIRM_FACTORY; // || linkFirmId==FIRM_HARBOR;

  case FIRM_INN: // for an inn to scan for neighbor inns quickly, the link line
                 // is not displayed
  ...
  case FIRM_LAIR:
    ...
    //		case FIRM_HARBOR:
    //			return linkFirmId==FIRM_MARKET || linkFirmId==FIRM_MINE ||
...
//						==0 if the firm does not have that build
//code
//
int FirmInfo::get_build_id(char *buildCode) {
  ...
  // ###### begin Gilbert 4/11 ######//
  //	if( build_count==1 )		// if this firm has only one building
  //type 		return first_build_id;
  // ###### end Gilbert 4/11 ######//

  int firmBuildId = first_build_id;
  int firmGroupId = first_group_id;
  for (int i = 0; i < group_count;
       i++, firmGroupId++) // if this firm has one graphics for each race
  {
    if (strcmp(buildCode, firm_res.get_group(firmGroupId)->race_code) == 0)
      return (firmBuildId + firm_res.get_group(firmGroupId)->group - 1);
  }

  //	for( i=0 ; i<build_count ; i++, firmBuildId++ )              // if this
  //firm has one graphics for each race
  //	{
  //		if( strcmp(buildCode,
  //firm_res.get_group(firmBuildId)->build_code)==0 ) 			return firmBuildId;
  //	}
...
#define REMAP_KEY_COUNT 8
    memcpy(palw_ptr + FIRST_REMAP_KEY,
           game.get_color_remap_table(nationRecno, selectedFlag) +
               FIRST_REMAP_KEY,
           (sizeof(short) * REMAP_KEY_COUNT));
...
void FirmBitmap::draw_at(int absX, int absY, int absZ, short *colorRemapTable,
                         int displayLayer, FirmBuild *firmBuild) {
  ...
  world.zoom_matrix->put_bitmap_offset(absX, absY, absZ, bitmapPtr, offset_x,
                                       offset_y, colorRemapTable, 0, 1);
}
...
//
int FirmRes::is_linkable_to_firm(int firmId1, int firmRaceId1,
                                 int firmNationRecno1, int firmId2,
                                 int firmRaceId2, int firmNationRecno2) {
