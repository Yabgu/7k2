...
//
// do nore read item_info_array directly, some item may override the value in
// the table
//
static ItemInfo item_info_array[ITEM_TYPE_COUNT] = {
    //	    item_id	       name			rare   cost		ability_type
    //value   para_type	      use    use_min recover  max  magic_id/para
    //init_c/r  bmp name, idx, desc
    {ITEM_SWORD,
     "Berserker Sword",
     0,
     100,
     ITEM_ABILITY_MELEE_DAMAGE,
     5,
     ITEM_PARA_NONE,
     0,
     0,
    ...
    {ITEM_MONSTER4_BOOK,
     ...
     0,
     0,
     "Extra Damage against Minotauros"},
    {ITEM_MONSTER5_BOOK,
     "Grokken's Bane",
     3,
     1000,
    ...
    {ITEM_UNIT_CHANGE_NATION,
     ...
     0,
     0,
     "Convert an opponent to your side"},
    ...
    //	{ ITEM_SWORD, "Sword",    0,     5,    3, 0, 0, 0, 0, 0,   0, 0, "Close
    //attack +3" },
    //	{ ITEM_BOW,   "Bow",      0,     5,    0, 3, 0, 0, 0, 0,   0, 0, "Range
    //attack +3" },
    //	{ ITEM_ARMOR, "Armor",    0,     5,    0, 0, 3, 0, 0, 0,   0, 0,
    //"Defense +3" },
    //	{ ITEM_CROWN, "Crown",    2,   100,    0, 0, 0, 0,10, 0,   0, 0,
    //"Leadership +10" },
};
...
void ItemRes::init() {
  ...
  for (int i = 1; i <= ITEM_TYPE_COUNT; ++i) {
    item_info_array[i - 1].bitmap_idx =
        res_bitmap.get_index(item_info_array[i - 1].bitmap_name);
    item_info_array[i - 1].icon_idx =
        res_icon.get_index(item_info_array[i - 1].bitmap_name);
    item_info_array[i - 1].interface_idx =
        res_interface.get_index(item_info_array[i - 1].bitmap_name);
    item_info_array[i - 1].unit_interface_idx =
        res_unit_interface.get_index(item_info_array[i - 1].bitmap_name);

    err_when(item_info_array[i - 1].rare < MIN_RARITY ||
             item_info_array[i - 1].rare >
                 MAX_RARITY + 3); // higher than MAX_RARITY for scenario only
  }
...
int ItemRes::ability(Item &item, int itemAbilityId) {
  ...
  if (itemInfo->ability_type == itemAbilityId) {
    return itemInfo->ability_value + enhancement;
             ...
             (itemAbilityId == ITEM_ABILITY_MELEE_DAMAGE ||
              itemAbilityId == ITEM_ABILITY_ARROW_DAMAGE)) {
    return itemInfo->ability_value + enhancement;
  }
...
int ItemRes::can_pick(short itemId, int unitRecno, int unitId) {
  ...
  if (!unitInfo->race_id ||
      unitId == UNIT_WAGON) // only human or monster can equip
    return 0;
...
int ItemRes::can_equip(short itemId, int unitId, Skill &skill) {
  ...
  if (ability(tempItem, ITEM_ABILITY_ARROW_DAMAGE) > 0 &&
    ...
    UnitInfo *unitInfo = unit_res[unitId];
    for (int curAttack = unitInfo->attack_count - 1; curAttack >= 0;
         --curAttack) {
...
void ItemRes::next_day(Item &item) {
  ...
  case ITEM_PARA_CHARGE_TIMED:
    // err_when( item.para == 0 );	// not allow to be zero, avoid at all
    // cost
    if (item.para < 0) {
      // keep in mind para is negative and increasing to zero
      item.para += itemInfo->para_use_rate;
...
char *ItemRes::item_unit_interface(short itemId) {
  ...
  return res_unit_interface.get_data(
      item_info_array[itemId - 1].unit_interface_idx);
}
...
//
void ItemRes::use_manually(Item &item, int unitRecno, int targetBaseObjRecno,
                           int para2) {
  if (!item.id)
    return;
  err_when(item.id > ITEM_TYPE_COUNT);
  ...
  if (itemInfo->para_type == ITEM_PARA_USE_MANUAL) {
    ...
    case ITEM_SUMMON_SCROLL: {
      ...
      int summonUnitId =
          monster_res[misc.random(MAX_MONSTER_TYPE) + 1]->unit_id;
      SpriteInfo *summonSpriteInfo =
          sprite_res[unit_res[summonUnitId]->sprite_id];
      ...
      for (r = 0; r <= 2; ++r) {
        ...
        if (world.locate_space(xLoc1, yLoc1, xLoc2, yLoc2,
                               summonSpriteInfo->loc_width,
                               summonSpriteInfo->loc_height)) {
          int summonRecno =
              unit_array.add_unit(summonUnitId, unitPtr->nation_recno,
                                  RANK_SOLDIER, 100, xLoc1, yLoc1);
          if (summonRecno) {
            // ####### begin Gilbert 23/2 #########//
            ...
            err_when(monsterLevel < 3 ||
                     monsterLevel > 9); // tuned for level between 3 and 9
            int combatSkill =
                (7 * MAX_COMBAT_TRAIN + misc.random(MAX_COMBAT_TRAIN)) /
                (5 + monster_res[summonUnit->monster_id()]->level);
    ...
    case ITEM_UNIT_CHANGE_NATION: {
      ...
      if (!base_obj_array.is_deleted(targetBaseObjRecno) &&
          ...
          targetUnit->is_visible() && targetUnit->rank_id != RANK_KING &&
    ...
    case ITEM_TOWN_CHANGE_NATION: {
      ...
      if (!base_obj_array.is_deleted(targetBaseObjRecno) &&
          ...
          !targetTown->under_construction && unitPtr->nation_recno != 0 &&
          targetTown->nation_recno != unitPtr->nation_recno &&
          unitPtr->area_distance(targetTown) <= 8 &&
          (targetTown->surrender(unitPtr->nation_recno),
           targetTown->nation_recno == unitPtr->nation_recno)) {
        use_now(item);
      } else {
        if (unitPtr->is_own()) {
    ...
    case ITEM_TOWN_FREE: {
      ...
      if (!base_obj_array.is_deleted(targetBaseObjRecno) &&
          (targetTown = base_obj_array[targetBaseObjRecno]->cast_to_Town()) &&
          !targetTown->under_construction && targetTown->nation_recno != 0 &&
          unitPtr->area_distance(targetTown) <= 8 &&
