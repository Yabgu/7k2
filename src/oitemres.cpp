...
static ItemInfo item_info_array[ITEM_TYPE_COUNT] = {
    ...
    {ITEM_ARMOR,
     ...
     0,
     "Defense +5"},
    {ITEM_COMBAT, "Excalibur", 0, 200, ITEM_ABILITY_COMBAT_LEVEL, 30, ITEM_PARA_NONE, 0, 0, 0, 0, 0, 0, 0, 0, "SWORD_1",
     0,           0,           0, 0,   "Combat level +30"},
    {ITEM_LEADER,
     "The Art of War",
     2,
     300,
...
int ItemRes::can_equip(short itemId, int unitId, Skill &skill)
{
    ...
    if (ability(tempItem, ITEM_ABILITY_ARROW_DAMAGE) > 0 && ability(tempItem, ITEM_ABILITY_DAMAGE) == 0)
    {
        ...
        {
            AttackInfo *attackInfo = unit_res.get_attack_info(unitInfo->first_attack + curAttack);
            if (attackInfo->attack_range > 1 && skill.actual_skill_level(NULL) >= attackInfo->combat_level)
            {
                canRangeAttack = 1;
            }
        }
...
int ItemRes::random_para(short itemId, int rnd)
{
    ...
    {
        return item_info_array[itemId - 1].init_para_const + rnd % item_info_array[itemId - 1].init_para_random;
    }
    else
    {
    ...
    if (itemInfo->para_type == ITEM_PARA_USE_MANUAL)
    {
        ...
        case ITEM_SUMMON_SCROLL: {
            ...
            for (r = 0; r <= 2; ++r)
            {
                ...
                if (world.locate_space(xLoc1, yLoc1, xLoc2, yLoc2, summonSpriteInfo->loc_width,
                                       summonSpriteInfo->loc_height))
                {
                    int summonRecno =
                        unit_array.add_unit(summonUnitId, unitPtr->nation_recno, RANK_SOLDIER, 100, xLoc1, yLoc1);
                    if (summonRecno)
                    {
                        // ####### begin Gilbert 23/2 #########//
                        ...
                        int combatSkill = (7 * MAX_COMBAT_TRAIN + misc.random(MAX_COMBAT_TRAIN)) /
        ...
        case ITEM_UNIT_CHANGE_NATION: {
            ...
            if (!base_obj_array.is_deleted(targetBaseObjRecno) &&
                (targetUnit = base_obj_array[targetBaseObjRecno]->cast_to_Unit()) && targetUnit->is_visible() &&
                targetUnit->rank_id != RANK_KING && targetUnit->nation_recno != 0 && unitPtr->nation_recno != 0 &&
                !targetUnit->is_nation(unitPtr->nation_recno) && unitPtr->area_distance(targetUnit) <= 8 &&
        ...
        case ITEM_TOWN_CHANGE_NATION: {
            ...
            Town *targetTown;
            if (!base_obj_array.is_deleted(targetBaseObjRecno) &&
                (targetTown = base_obj_array[targetBaseObjRecno]->cast_to_Town()) && !targetTown->under_construction &&
                unitPtr->nation_recno != 0 && targetTown->nation_recno != unitPtr->nation_recno &&
        ...
        case ITEM_TOWN_FREE: {
            ...
            if (!base_obj_array.is_deleted(targetBaseObjRecno) &&
                (targetTown = base_obj_array[targetBaseObjRecno]->cast_to_Town()) && !targetTown->under_construction &&
                targetTown->nation_recno != 0 && unitPtr->area_distance(targetTown) <= 8 &&
