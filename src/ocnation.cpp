...
void CampaignNation::init() { memset(this, 0, sizeof(*this)); }
...
void CampaignNation::init_nation(int nationType, int raceId,
                                 int colorSchemeId) {
  ...
  if (is_human()) {
    ...
    //		str  = race_res[race_id]->get_name(king_name_id, 1);	// 1-first name
    //only 		str += "'s Kingdom";
    str = text_unit.str_unit_nation(
        race_id,
        race_res[race_id]->get_name(king_name_id, 1)); // 1-first name only
  } else {
    ...
    //		str += "'s Kwyzan";
    str =
        text_unit.str_unit_nation(race_id, monster_res.get_name(king_name_id));
  }
