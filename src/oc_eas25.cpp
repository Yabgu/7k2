...
void CampaignEastWest::stage_25_init_vars() {
  ...
  game.add_win_condition(E_REACH_TOTAL, 1, 0, goal_overall_score,
                         goal_year_limit);
...
int CampaignEastWest::stage_25_create_firm() {
  ...
  if (townRecno2)
    create_military_firm(townRecno2,
                         80); // this is the one that will rebel later
...
char *CampaignEastWest::stage_25_intro_text() {
  return substitute_text(res_stage.read("25INTRO"), goal_overall_score,
                         goal_year_limit);
}
...
char *CampaignEastWest::stage_25_goal_text() {
  return substitute_text(res_stage.read("25GOAL"), goal_overall_score,
                         goal_year_limit);
}
...
//
void CampaignEastWest::stage_25_next_day() { (this->*plot_next_day_FP)(); }
