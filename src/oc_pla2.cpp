...
void CampaignEastWest::plot_a2_create_game() {
  ...
  for (int techClassId = FIRST_TECH_CLASS_FRYHTAN;
       techClassId <= LAST_TECH_CLASS_FRYHTAN; ++techClassId) {
  ...
  /*
          ...
          int lairAddCount = 6 + misc.random(3);
     // add 6 to 8 Fryhtan Lairs int independentTownAddCount = 6 +
     misc.random(3);		// 6 to 8 independent towns int
     specialFirmCount= lairAddCount - misc.random(3);

          create_lair( 0, plot_enemy_nation_recno, lairAddCount,
     independentTownAddCount, specialFirmCount);
  */

  // --- build military building around non-slave independent town of player but
  // badly damage ----//
  ...
  for (int fortCount = 3; fortCount > 0; --fortCount) {
    ...
    for (int townRecno = town_array.size(); townRecno > 0; --townRecno) {
      ...
      if (townPtr->nation_recno == 0 && !townPtr->is_pay_tribute_to_monster()) {
        int weight =
            1000 -
            unit_array[(~nation_array)->king_unit_recno]->area_distance(
                townPtr) /
                8 +
            townPtr->population +
            (townPtr->race_id == (~nation_array)->race_id ? 20 : 0);
    ...
    if (!bestTownRecno)
      ...
      int firmRecno = create_firm_next_to_place(
          town_array[bestTownRecno],
      ...
      if (firmRecno) {
        ...
        firmPtr->hit_points =
            misc.random((int)firmPtr->hit_points) / 2.0f + 1.0f;
      }
