...
void Unit::think_independent_unit() {
  ...
  if (unit_mode) // don't let them think a new action if they have something to
                 // do currently
    return;
  ...
  if (unit_mode == UNIT_MODE_REBEL) {
    ...
    //--- if the group this rebel belongs to already has a rebel town, assign to
    //it now ---//
    ...
    if (rebelPtr->town_recno) {
      ...
      return; // don't do anything if the town has been destroyed,
              // Rebel::next_day() will take care of it.
    }
  }
  ...
  for (int i = town_array.size(); i > 0; i--) {
    ...
    if (townPtr->nation_recno || townPtr->population >= MAX_TOWN_POPULATION ||
        townPtr->region_id != regionId || townPtr->race_id != race_id) {
      continue;
    }
  ...
  if (bestTown) {
    err_when(unit_mode == UNIT_MODE_REBEL &&
             rebel_array[unit_mode_para]->town_recno &&
...
void Unit::think_independent_hero() {
  ...
  for (i = nation_array.size(); i > 0; i--) {
    ...
    if (sprite_recno % 2 ==
        0) // some heroes look at kill_monster_score, but some don't
