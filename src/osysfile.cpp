...
void Sys::load_game() {
  ...
  signal_exit_flag =
      1; // for deinit functions to recognize that this is an end game
         // deinitialization instead of a normal deinitialization
...
void Sys::save_game() {
  if (nation_array.player_recno ==
      0) // cannot save game when the player's kingdom has been destroyed
    return;

  if (remote.is_enable()) {
    DWORD *dwordPtr =
        (DWORD *)remote.new_send_queue_msg(MSG_REQUEST_SAVE, sizeof(DWORD));
    *dwordPtr =
        remote.next_send_frame(nation_array.player_recno,
                               sys.frame_count + remote.process_frame_delay) +
        2;
    return;
  }

  // game_file_array.init("*.SAV");                  // reload any save game
  // file
  game_file_array.reload(); // reload any save game file
  game_file_array.menu(-2); // save screen area to back buffer
  ...
  if (game_file_array.menu(1) == 1) {
    if (GameFile::last_read_success_flag)
      box.msg(
          text_game_menu.str_save_success()); // "Game Saved Successfully" );
  }
...
//-*********** syn game test ***********-//
  ...
  if (remote.is_enable() &&
      remote.sync_test_level >= 0 && // disable autosave after un-sync
