...
void BaseObj::being_attack_hit(BaseObj *attackerObj, float damagePoint) {
  ...
  if (damagePoint > 0.0) {
    ...
    if (damagePoint <= -1.0f) {
      damagePoint =
          -1.0f /
          damagePoint; // if defense too strong, damage = 1/(obj_armor - damage)
    } else if (damagePoint < 1.0f) {
      damagePoint = 1.0f;
    }

    if (damagePoint >
        originDamagePoint) // cannot be greater than the original damage
...
int BaseObj::area_distance(BaseObj *objPtr) {
  //	err_when( !obj_is_visible() );				// when a team member may
  //ask for help, this function will be called, so the error checking should not
  //be here. 	err_when( !objPtr->obj_is_visible() );

  register BaseObj *targetPtr = objPtr;

  return misc.area_distance(obj_loc_x1(), obj_loc_y1(), obj_loc_x2(),
                            obj_loc_y2(), targetPtr->obj_loc_x1(),
                            targetPtr->obj_loc_y1(), targetPtr->obj_loc_x2(),
                            targetPtr->obj_loc_y2());
}
...
void BaseObj::being_killed(BaseObj *attackerObj) {
  ...
  if (attackerObj && attackerObj->nation_recno &&
    ...
    nation_array[attackerObj->nation_recno]->change_live_points(
        gainedLivePoints);
  }
}
...
float BaseObj::calc_live_points_bonus() {
  ...
  float incBonus = (float)(info.game_date - info.game_start_date) /
                   (365 * 5); // life points will double in 5 years.
...
//
int BaseObj::is_stealth() { return 0; }
