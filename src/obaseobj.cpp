...
void BaseObj::being_attack_hit(BaseObj *attackerObj, float damagePoint)
{
    ...
    {
        err_when((world.get_loc(attackerObj->obj_loc_x1(), attackerObj->obj_loc_y1())->base_obj_recno(UNIT_AIR) !=
                  attackerObj->base_obj_recno) &&
                 (world.get_loc(attackerObj->obj_loc_x1(), attackerObj->obj_loc_y1())->base_obj_recno(UNIT_LAND) !=
                  attackerObj->base_obj_recno) &&
                 (world.get_loc(attackerObj->obj_loc_x1(), attackerObj->obj_loc_y1())->base_obj_recno(UNIT_SEA) !=
                  attackerObj->base_obj_recno));
    }
#endif
}
...
void BaseObj::being_attack_hit2(BaseObj *attackerObj)
{
    ...
    if (is_own())
    {
        ...
        if (obj_loc_x2() > obj_loc_x1() || obj_loc_y2() > obj_loc_y1())
        {
        ...
        if ((info.game_date - last_attack_date) > 6 && is_own())
        {
            ...
            else if (obj_type == OBJ_TOWN)
            {
                ...
                if (attackerObj)
                    news_array.town_attacked(cast_to_Town()->town_name(), cast_to_Town()->center_x,
...
{
    //	err_when( !obj_is_visible() );				// when a team member
    // may
    // ask for help, this function will be called, so the error checking should
    // not be here. 	err_when( !objPtr->obj_is_visible() );

    register BaseObj *targetPtr = objPtr;

    return misc.area_distance(obj_loc_x1(), obj_loc_y1(), obj_loc_x2(), obj_loc_y2(), targetPtr->obj_loc_x1(),
                              targetPtr->obj_loc_y1(), targetPtr->obj_loc_x2(), targetPtr->obj_loc_y2());
}
...
void BaseObj::being_killed(BaseObj *attackerObj)
{
    ...
    //--- if the attacker belongs to a Fryhtan nation, killing units gets live
    // points ---//

    if (attackerObj && attackerObj->nation_recno && nation_array[attackerObj->nation_recno]->is_monster())
    {
        // ##### patch begin Gilbert 28/3 ###### //
        float gainedLivePoints = calc_live_points_bonus();

        //--- if the attacker is not a main monster unit type, only gain 50% of the
        // live points ---//
...
float BaseObj::calc_live_points_bonus()
{
    ...
    // ####### patch begin Gilbert 28/3 #####//
    // float incBonus = (float) (info.game_date-info.game_start_date) / (365*20);
    // // life points will double in 20 years. incBonus = MIN(incBonus, 100);
...
{
    return is_own() || (nation_recno && nation_array[nation_recno]->is_allied_with_player);
}
