...
int FirmInn::think_del()
{
    ...
    if (ownNation->cash < 500 + 500 * ownNation->pref_cash_reserve / 100 && ownNation->profit_365days() < 0)
    {
        ai_del_firm();
        return 1;
    }

    if (ownNation->ai_inn_count >
        ownNation->ai_supported_inn_count() + 2) // if the current number of inns is more than the number the nation
                                                 // can support plus 2, then destroy the current one
    {
        ai_del_firm();
        return 1;
    }
...
int FirmInn::think_hire_spy()
{
    ...
    {
        if (innUnit->spy_skill == 0 || !unit_res[innUnit->unit_id]->is_civilian) // only hire civilian spies
            continue;
...
int FirmInn::think_assign_spy_to(int raceId, int innUnitRecno)
{
    ...
    for (int i = town_array.size(); i > 0; i--)
    {
        ...
        if (townPtr->nation_recno == 0)
        {
            ...
            {
                //--- current resistance == target resistance if we don't have any spies
                // in the town ---//

                if (townPtr->target_resistance(nation_recno) == townPtr->resistance(nation_recno))
                {
                    int unitRecno = hire(innUnitRecno);

                    nation_array[nation_recno]->add_action(townPtr->loc_x1, townPtr->loc_y1, -1, -1,
                                                           ACTION_AI_ASSIGN_SPY, townPtr->nation_recno, 1, unitRecno);
...
int FirmInn::think_hire_general()
{
    ...
    for (int i = 0; i < inn_unit_count; i++, innUnit++)
    {
        if (unit_res[innUnit->unit_id]->is_civilian) // only military unit, but can be either a spy
                                                     // general or a normal general
            continue;
...
int FirmInn::think_assign_general_to(int raceId, int innUnitRecno)
{
    ...
    Nation *ownNation = nation_array[nation_recno];
    int curRating, bestRating = 10; // the new one needs to be at least 10 points
    ...
    for (int i = ownNation->ai_camp_count - 1; i >= 0; i--)
    {
        ...
        int newLeadership = firmCamp->new_commander_leadership(raceId, innUnit->skill_level());
        ...
        if (curRating > bestRating)
        {
            ...
            if (ownNation->is_action_exist(firmCamp->loc_x1, firmCamp->loc_y1, -1, -1, ACTION_AI_ASSIGN_OVERSEER,
                                           firmCamp->firm_id)) // it can be a Camp or a Fort, so we use
                                                               // firmCamp->firm_id
            {
                continue;
            }
    ...
    ownNation->add_action(bestCamp->loc_x1, bestCamp->loc_y1, -1, -1, ACTION_AI_ASSIGN_OVERSEER, bestCamp->firm_id, 1,
                          unitRecno);
