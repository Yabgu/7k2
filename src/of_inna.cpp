...
int FirmInn::think_del() {
  ...
  if (ownNation->ai_inn_count >
      ownNation->ai_supported_inn_count() +
          2) // if the current number of inns is more than the number the nation
...
int FirmInn::think_hire_spy() {
  ...
  for (int i = 0; i < inn_unit_count; i++, innUnit++) {
    if (innUnit->spy_skill == 0 ||
        !unit_res[innUnit->unit_id]->is_civilian) // only hire civilian spies
      continue;
  ...
  for (int i = town_array.size(); i > 0; i--) {
    ...
    if (townPtr->nation_recno == 0) {
      for (int j = 0; j < MAX_RACE; j++) {
        ...
        if (townPtr->target_resistance(nation_recno) ==
            townPtr->resistance(nation_recno)) {
          int unitRecno = hire(innUnitRecno);

          nation_array[nation_recno]->add_action(
              townPtr->loc_x1, townPtr->loc_y1, -1, -1, ACTION_AI_ASSIGN_SPY,
              townPtr->nation_recno, 1, unitRecno);
...
int FirmInn::think_hire_general() {
  ...
  for (int i = 0; i < inn_unit_count; i++, innUnit++) {
    if (unit_res[innUnit->unit_id]
            ->is_civilian) // only military unit, but can be either a spy
...
int FirmInn::think_assign_general_to(int raceId, int innUnitRecno) {
  ...
  for (int i = ownNation->ai_camp_count - 1; i >= 0; i--) {
    ...
    int newLeadership =
        firmCamp->new_commander_leadership(raceId, innUnit->skill_level());
