...
void VBrowseIF2::init(int inX1, int inY1, int inX2, int inY2, int recWidth,
                      int recHeight, int totalRec, BrowDispFP dispFunc,
                      int dispFrame, int minInterSpace) {
  VBrowse::init(inX1, inY1, inX2, inY2, 1, recWidth, recHeight, totalRec,
                dispFunc, dispFrame, minInterSpace);
}
...
VBrowseIF2::VBrowseIF2() {
  vga_front_only = 0; // if 1, then it does all the bitmap processing on the
                      // front buffer only
}
...
//
void VBrowseIF2::paint() { disp_all(); }
...
int VBrowseIF2::detect() {
  ...
  //	if( detect_pull_flag && disp_frame && x_max_rec==1 )  // only pull when
  //the vbrowser is vertical, horizontal is technically possible, but not used
  //practically
  //	{
  //		if( detect_pull() )
  //			return rec_no;
  ...
  if (disp_frame) {
    if (mouse.any_click(x1 + 5, y1 - 6, x2 - 20, y2 - 14)) {
      recNo = top_rec_no +
              (mouse.click_y() - (y1 - 6)) / (rec_height + rec_y_space) *
                  x_max_rec +
  ...
  if (!mouse.any_click(x1 + 5, y1 - 6, x2 - 20, y2 - 14)) {
    if ((rc = scroll_bar.detect()) > 0) {
      ...
      if (disp_frame &&
          x_max_rec == 1) // only use record as scroller unit when it's a
                          // vertical browser with frame, refer to
                          // scroll_bar.init() in VBrowse::init_var()
      {
...
void VBrowseIF2::disp_all() {
  ...
  Vga::active_buf->put_bitmap_trans_decompress(x1 + 2, y1 - 20,
                                               image_button.read("ARROWUP"));
  Vga::active_buf->put_bitmap_trans_decompress(x1 + 2, y2 - 11,
                                               image_button.read("ARROWDN"));
...
void VBrowseIF2::disp_one(int recNo, int dispType) {
  ...
  if (disp_frame && dispType == CLEAR_HIGH && !vga.use_back_buf &&
    ...
    vga_util.blt_buf(x - 2, y + rec_height + 1, x + rec_width + 1,
                     y + rec_height + 1, 0);                      // bottom
    vga_util.blt_buf(x - 2, y - 2, x - 2, y + rec_height + 1, 0); // left
    vga_util.blt_buf(x + rec_width + 1, y - 2, x + rec_width + 1,
                     y + rec_height + 1, 0); // right
  }
  ...
  if (disp_frame && dispType == DISP_HIGH) {
    //	vga_back.rect( x-2, y-2, x+rec_width+1, y+rec_height+1, 2,
    //VGA_LIGHT_BLUE);
    Vga::active_buf->rect(x - 2, y - 11, x + rec_width + 1, y + rec_height - 7,
                          2, VGA_LIGHT_BLUE);
  }
}
