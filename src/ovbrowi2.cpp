...
void VBrowseIF2::init(int inX1, int inY1, int inX2, int inY2, int recWidth, int recHeight, int totalRec,
                      BrowDispFP dispFunc, int dispFrame, int minInterSpace)
{
    VBrowse::init(inX1, inY1, inX2, inY2, 1, recWidth, recHeight, totalRec, dispFunc, dispFrame, minInterSpace);
}
...
void VBrowseIF2::init_var(int totalRec, int recNo)
{
    ...
    scroll_bar.init(1, x1 + 3, y1 - 19, x1 + 34, y2 + 2, disp_max_rec,
                    ...
                    total_rec_num, 1,
                    1); // last 1 - Interface mode if_flag is 1
}
...
int VBrowseIF2::detect()
{
    ...
    if (disp_frame)
    {
        ...
        {
            recNo = top_rec_no + (mouse.click_y() - (y1 - 6)) / (rec_height + rec_y_space) * x_max_rec +
        ...
        {
            recNo = top_rec_no + (mouse.cur_y - (y1 - 6)) / (rec_height + rec_y_space) * x_max_rec +
    ...
    if (!mouse.any_click(x1 + 5, y1 - 6, x2 - 20, y2 - 14))
    {
        if ((rc = scroll_bar.detect()) > 0)
        {
            ...
            if (disp_frame && x_max_rec == 1) // only use record as scroller unit when it's a
