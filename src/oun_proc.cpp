...
void Unit::next_day() {
  ...
  if (nation_recno) {
    ...
    if (unit_array.is_deleted(unitRecno)) // if its hit points go down to 0,
                                          // is_deleted() will return 1.
      return;
  ...
  if (invulnerable_day_count > 10) {
    ...
    if (last_attack_date == info.game_date ||
        last_attack_date + 1 == info.game_date)
      --invulnerable_day_count;
  }
  ...
  if (config.king_undie_flag && rank_id == RANK_KING && nation_recno &&
      !nation_array[nation_recno]->is_ai()) {
...
#ifdef DEBUG
  err_when(race_id && unit_res[unit_id]->unit_class != UNIT_CLASS_HUMAN &&
  ...
  err_when(unit_mode == UNIT_MODE_REBEL &&
           nation_recno); // rebels must be independent
  ...
  err_when((rank_id == RANK_GENERAL || rank_id == RANK_KING) && !team_info);
  ...
  if (leader_unit_recno &&
      unit_mode != UNIT_MODE_REBEL) // in rebel mode, the leader_unit_recno is
                                    // not linked to team_info
  {
  ...
  if (cur_order.ai_action_id) {
    ...
    for (int actionRecno = nationPtr->action_count(); actionRecno > 0;
         actionRecno--) {
  ...
  err_when(unit_mode == UNIT_MODE_REBEL &&
           nation_recno); // all rebels must be independent units
  ...
  if (skill.actual_combat_level(NULL) >
      skill.max_combat_level) // raw combat level
...
void Unit::process_recover() {
  if (hit_points == 0.0f ||
      hit_points == max_hit_points()) // this unit is dead already
    return;
...
#define RECOVER_SLOW_DOWN 0.5f
  ...
  if (unit_mode == UNIT_MODE_OVERSEE || unit_mode == UNIT_MODE_IN_HARBOR) {
  ...
  //------ for units on ships --------//
    ...
    //--- if the ship where the unit is on is in the harbor, the unit recovers
    //faster ---//
...
//          mobile - no update
//
// Soldiers led by a general: in a military camp - updated in
// FirmCamp::update_loyalty()
//                            mobile - updated here
//
...
void Unit::update_loyalty() {
  ...
  if (unit_mode == UNIT_MODE_CONSTRUCT_FIRM || // constructor worker will not
                                               // change their loyalty when they
                                               // are in a building
  ...
  if (rank_id == RANK_GENERAL) {
    ...
    if (ownNation->king_unit_recno)
      targetLoyalty +=
          unit_array[ownNation->king_unit_recno]->skill_level() / 4;

    //-- if the unit is rewarded less than the amount of contribution he made,
    //he will become unhappy --//
  ...
  if (target_loyalty >
      loyalty) // only increase, no decrease. Decrease are caused by events.
               // Increases are made gradually
  {
    ...
    loyalty = newLoyalty;
  } else if (target_loyalty <
             loyalty) // only increase, no decrease. Decrease are caused by
                      // events. Increases are made gradually
  {
    loyalty--;
  }
...
void Unit::king_die() {
  ...
  if (techClass->tech_count > 0) {
    ...
    if (i <= techClass->tech_count) {
      ...
      if (get_cur_loc(xLoc, yLoc) &&
          site_array.add_site(xLoc, yLoc, SITE_WEAPON_BLUEPRINT, techId)) {
        tech_res[techId]->disable_know(
            nation_recno); // the nation lost its scroll
      }
    }
  }
...
int Unit::think_betray() {
  ...
  if (spy_recno) // spies do not betray here, spy has its own functions for
                 // betrayal
    return 0;
  ...
  if (!unit_res[unit_id]->race_id || !nation_recno || rank_id == RANK_KING ||
      spy_recno) {
    return 0;
  }
  ...
  int i,
      bestNationRecno = 0, nationScore,
      bestScore = loyalty; // the score must be larger than the current loyalty
  ...
  if (bestNationRecno) {
    ...
    if (rank_id != RANK_KING && is_visible() && !spy_recno) {
      resign(COMMAND_AUTO);
      return 1;
    }
  }
...
int Unit::betray(int newNationRecno) {
  ...
  if (spy_recno &&
      spy_array[spy_recno]->cloaked_nation_recno !=
          newNationRecno) // cloaked_nation_recno == newNationRecno if betray()
                          // is called by spy_change_nation() already
  {
  ...
  err_when(newNationRecno &&
           nation_array[newNationRecno]
               ->is_human() && // monsters in firms should not betray to humans
           is_monster() &&
           unit_mode == UNIT_MODE_OVERSEE);
  ...
  if (unit_mode == UNIT_MODE_CONSTRUCT_FIRM || // don't change nation when the
                                               // unit is constructing a firm
      unit_mode == UNIT_MODE_CONSTRUCT_TOWN ||
      unit_mode == UNIT_MODE_ON_SHIP) // don't change nation when the unit is
                                      // constructing a firm
  {
    return 0;
  }
  ...
  if (nation_recno == nation_array.player_recno ||
    ...
    //--- if this is a spy, don't display news message for betrayal as it is
    //already displayed in Unit::spy_change_nation() ---//
...
void Unit::change_nation(int newNationRecno) {
  ...
  if (nation_recno == nation_array.player_recno &&
      sprite_recno == unit_array.selected_recno && power.command_id) {
    power.command_id = 0;
  }
  ...
  //	unit_group_id = unit_array.cur_group_id++;      // separate from the
  //current group
  ...
  if (spy_recno && spy_array[spy_recno]->cloaked_nation_recno !=
                       nation_recno) // BUGHERE - fix bug on-fly
    spy_array[spy_recno]->cloaked_nation_recno = nation_recno;
}
...
void Unit::pay_expense() {
  ...
  //--- if it's a mobile spy or the spy is in its own firm, no need to pay
  //salary here as Spy::pay_expense() will do that ---//
  //
  ...
  //
  //  >your nation pays them $5 dollars per month. (your nation pays them no
  //  food)
  //
    ...
    if (rank_id != RANK_KING) {
      ...
      if (nationPtr->cash > 0) {
        if (rank_id == RANK_SOLDIER)
          nationPtr->add_expense(EXPENSE_MOBILE_UNIT,
                                 (float)SOLDIER_YEAR_SALARY / 365, 1);

        if (rank_id == RANK_GENERAL)
          nationPtr->add_expense(EXPENSE_GENERAL,
                                 (float)GENERAL_YEAR_SALARY / 365, 1);
      }
  ...
  } else //----- it's a non-human unit ------//
  {
    if (nationPtr->cash > 0) {
      ...
      nationPtr->add_expense(expenseType,
                             (float)unit_res[unit_id]->year_cost / 365, 1);
    } else // decrease hit points if the nation cannot pay the unit
    {
      if (unit_res[unit_id]->unit_class !=
          UNIT_CLASS_CARAVAN) // Even when caravans are not paid, they still
          ...
          if (hit_points == 0) {
            if (nation_recno == nation_array.player_recno) {
              ...
              if (unitClass == UNIT_CLASS_WEAPON ||
                  unitClass == UNIT_CLASS_MONS_WEAPON)
...
void Unit::reward(int rewardNationRecno) {
  ...
  if (spy_recno &&
      true_nation_recno() ==
          rewardNationRecno) // if the spy's owning nation rewards the spy
  {
    spy_array[spy_recno]->change_loyalty(REWARD_LOYALTY_INCREASE);
  }

  //--- if this spy's nation_recno & true_nation_recno() are both ==
  //rewardNationRecno, it's true loyalty and cloaked loyalty will both be
  //increased ---//
  ...
  nation_array[rewardNationRecno]->add_expense(EXPENSE_REWARD_UNIT,
                                               (float)REWARD_COST);
}
...
void Unit::spy_change_nation(int newNationRecno, char remoteAction) {
  ...
  if (newNationRecno &&
      nation_array.is_deleted(
          newNationRecno)) // this can happen in a multiplayer message
    return;
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_UNIT_SPY_NATION,
                                                         2 * sizeof(short));
  ...
  //--- when a spy change cloak to another nation, he can't cloak as a general,
  //he must become a soldier first ---//

  if (is_visible() && // if the spy is a commander in a camp, don't set its rank
                      // to soldier
      rank_id == RANK_GENERAL && newNationRecno != spyPtr->true_nation_recno) {
  ...
  if (spyPtr->true_nation_recno !=
      nation_array.player_recno) // only send news message if he is not the
                                 // player's own spy
  {
    if (rank_id == RANK_GENERAL || unit_mode == UNIT_MODE_OVERSEE ||
        spyPtr->notify_cloaked_nation_flag) {
      //-- if this spy's cloaked nation is the player's nation, the player will
      //be notified --//
    ...
    if (spyPtr->notify_cloaked_nation_flag) {
      if (newNationRecno ==
          nation_array.player_recno) // cloaked as the player's nation
        news_array.unit_betray(sprite_recno, newNationRecno);
    }
  }
  ...
  betray(newNationRecno); // call the betray function to change natino. There is
                          // no difference between a spy changing nation and a
                          // unit truly betrays
}
...
int Unit::can_spy_change_nation() {
  ...
  int xLoc1 = cur_x_loc() - SPY_ENEMY_RANGE,
      yLoc1 = cur_y_loc() - SPY_ENEMY_RANGE;
  int xLoc2 = cur_x_loc() + SPY_ENEMY_RANGE,
      yLoc2 = cur_y_loc() + SPY_ENEMY_RANGE;
  ...
  for (yLoc = yLoc1; yLoc <= yLoc2; yLoc++) {
    ...
    for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++) {
      ...
      if (unit_array.is_deleted(unitRecno)) // the unit is dying, its recno is
...
void Unit::resign(int remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_UNIT_RESIGN, 2 * sizeof(short));
    *shortPtr = sprite_recno;
  ...
  if (is_human())
    town_array.race_wander_pop_array[race_id - 1] +=
        2; // disbanding one resulted in two wandering units to make the effect
           // more significant
  ...
  if (spy_recno &&
      true_nation_recno() != nation_recno) // the spy is cloaked in an enemy
                                           // nation when it is resigned
  {
    //------ decrease reputation ------//
    ...
    //------- add news message -------//
    // #### begin Gilbert 24/12 ######//
    //		if( nation_recno == nation_array.player_recno )		// display when the
    //player has revealed an enemy spy 			info.spy_execute = 1;
    // #### end Gilbert 24/12 ######//

    if (true_nation_recno() ==
            nation_array
                .player_recno || // display when the player's spy is revealed or
                                 // the player has revealed an enemy spy
  ...
  if (rank_id == RANK_GENERAL) // if this is a general, news_array.general_die()
                               // will be called, set news_add_flag to 0 to
                               // suppress the display of thew news
    ...
    if (monsterKingdomsCount > 0) {
      ...
      for (nationRecno = 1; nationRecno <= nation_array.size(); ++nationRecno) {
        if (nationRecno != nation_recno &&
            !nation_array.is_deleted(nationRecno) &&
            nation_array[nationRecno]->is_monster()) {
          nation_array[nationRecno]->change_live_points(gainLivePoints /
                                                        monsterKingdomsCount);
        }
      }
    }
...
void Unit::explore_area() {
  ...
  if (explore_for_player()) {
    ...
    world.visit(next_x_loc(), next_y_loc(), next_x_loc() + loc_width - 1,
                next_y_loc() + loc_height - 1,
                unit_res[unit_id]->visual_range + scoutRange,
...
//
void Unit::explore_on_move(int curNextXLoc, int curNextYLoc, int newNextXLoc,
                           int newNextYLoc) {
  if (!config.explore_whole_map && is_visible()) {
    if (explore_for_player()) {
      ...
      int xLoc2 =
          MIN(MAX_WORLD_X_LOC - 1, newNextXLoc + EXPLORE_RANGE + scoutRange);
      int yLoc2 =
          MIN(MAX_WORLD_Y_LOC - 1, newNextYLoc + EXPLORE_RANGE + scoutRange);
      ...
      if (newNextYLoc < curNextYLoc) // if move upwards, explore upper area
        world.explore(xLoc1, yLoc1, xLoc2, yLoc1 + exploreWidth);
               ...
               curNextYLoc) // if move downwards, explore lower area
  ...
  if (config.fog_of_war && is_visible()) {
    if (explore_for_player()) {
      world.visit(obj_loc_x1(), obj_loc_y1(), obj_loc_x2(), obj_loc_y2(),
                  unit_res[unit_id]->visual_range +
                      item.ability(ITEM_ABILITY_SCOUT_RANGE),
                  unit_res[unit_id]->visual_extend);
    }
    ...
    // ----- visit small area when showing attacker path -------//
             ...
             base_obj_array[cur_order.para]->nation_recno ==
                 nation_array.player_recno) {
      world.visit(obj_loc_x1(), obj_loc_y1(), obj_loc_x2(), obj_loc_y2(), 1, 1);
    }
  }
}
