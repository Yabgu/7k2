...
void Unit::next_day()
{
    ...
    if (config.king_undie_flag && rank_id == RANK_KING && nation_recno && !nation_array[nation_recno]->is_ai())
    {
    ...
    err_when(race_id && unit_res[unit_id]->unit_class != UNIT_CLASS_HUMAN &&
             unit_res[unit_id]->unit_class != UNIT_CLASS_MONSTER && unit_res[unit_id]->unit_class != UNIT_CLASS_WAGON);
    ...
    if (leader_unit_recno && unit_mode != UNIT_MODE_REBEL) // in rebel mode, the leader_unit_recno is
...
void Unit::update_loyalty()
{
    ...
    if (target_loyalty > loyalty) // only increase, no decrease. Decrease are caused by events.
    ...
    else if (target_loyalty < loyalty) // only increase, no decrease. Decrease are caused by
...
int Unit::think_betray()
{
    ...
    if (!unit_res[unit_id]->race_id || !nation_recno || rank_id == RANK_KING || spy_recno)
    {
        return 0;
    }
    ...
    int i, bestNationRecno = 0, nationScore,
    ...
    for (i = nation_array.size(); i > 0; i--)
    {
        ...
        nationScore = (int)nationPtr->reputation + (nationPtr->overall_rating - curNation->overall_rating);
...
int Unit::betray(int newNationRecno)
{
    ...
    if (spy_recno && spy_array[spy_recno]->cloaked_nation_recno !=
                         newNationRecno) // cloaked_nation_recno == newNationRecno if
                                         // betray() is called by spy_change_nation() already
    {
    ...
    err_when(newNationRecno &&
             nation_array[newNationRecno]->is_human() && // monsters in firms should not betray to humans
    ...
    if (unit_mode == UNIT_MODE_CONSTRUCT_FIRM ||                                 // don't change nation when the
        ...
        unit_mode == UNIT_MODE_CONSTRUCT_TOWN || unit_mode == UNIT_MODE_ON_SHIP) // don't change nation when the unit is
                                                                                 // constructing a firm
    {
        return 0;
    }
    ...
    if (unit_mode == UNIT_MODE_OVERSEE && firm_array[unit_mode_para]->firm_id == FIRM_FORTRESS)
    {
        return 0;
    }
    ...
    if (nation_recno == nation_array.player_recno || newNationRecno == nation_array.player_recno)
    {
...
void Unit::change_nation(int newNationRecno)
{
    ...
    if (nation_recno == nation_array.player_recno && sprite_recno == unit_array.selected_recno && power.command_id)
    {
        power.command_id = 0;
    }
...
void Unit::pay_expense()
{
    ...
    if (spy_recno)
    {
        ...
        if (unit_mode == UNIT_MODE_OVERSEE && firm_array[unit_mode_para]->nation_recno == true_nation_recno())
        {
            return;
        }
    }
    ...
    else //----- it's a non-human unit ------//
    {
        ...
        {
            if (unit_res[unit_id]->unit_class != UNIT_CLASS_CARAVAN) // Even when caravans are not paid, they still
...
void Unit::spy_change_nation(int newNationRecno, char remoteAction)
{
    ...
    if (spyPtr->true_nation_recno != nation_array.player_recno) // only send news message if he is not the
                                                                // player's own spy
    {
        if (rank_id == RANK_GENERAL || unit_mode == UNIT_MODE_OVERSEE || spyPtr->notify_cloaked_nation_flag)
        {
...
int Unit::can_spy_change_nation()
{
    ...
    for (yLoc = yLoc1; yLoc <= yLoc2; yLoc++)
    {
        ...
        for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++)
        {
            ...
            if (unitPtr->true_nation_recno() != trueNationRecno && unitPtr->nation_recno != trueNationRecno)
            {
                return 0;
            }
        }
...
void Unit::resign(int remoteAction)
{
    ...
    if (is_human())
        town_array.race_wander_pop_array[race_id - 1] += 2; // disbanding one resulted in two wandering units to make
    ...
    if (spy_recno && true_nation_recno() != nation_recno) // the spy is cloaked in an enemy
                                                          // nation when it is resigned
    {
        ...
        //------- add news message -------//
        // #### begin Gilbert 24/12 ######//
        //		if( nation_recno == nation_array.player_recno )		// display when
        // the
        // player has revealed an enemy spy 			info.spy_execute =
        // 1;
        // #### end Gilbert 24/12 ######//

        if (true_nation_recno() == nation_array.player_recno || // display when the player's spy is revealed or
    ...
    // sharing between all other fryhtan kingdoms
    {
        ...
        {
            if (nationRecno != nation_recno && !nation_array.is_deleted(nationRecno) &&
...
void Unit::explore_area()
{
    ...
    if (explore_for_player())
    {
        ...
        world.unveil(next_x_loc() - scoutRange, next_y_loc() - scoutRange, next_x_loc() + loc_width + scoutRange - 1,
                     next_y_loc() + loc_height + scoutRange - 1);

        world.visit(next_x_loc(), next_y_loc(), next_x_loc() + loc_width - 1, next_y_loc() + loc_height - 1,
                    unit_res[unit_id]->visual_range + scoutRange, unit_res[unit_id]->visual_extend);
    }
}
