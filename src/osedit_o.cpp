...
void ScenarioEditor::disp_hero_main(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    button_inf_or_spu[0].create_text(
        INFO_X1 + 15, INFO_Y1 + 5, INFO_X1 + 110, INFO_Y1 + 25,
        text_editor.str_hero_infantry(), 0); // "Infantry", 0 );
    button_inf_or_spu[1].create_text(INFO_X1 + 115, INFO_Y1 + 5, INFO_X1 + 210,
                                     INFO_Y1 + 25, text_editor.str_hero_spu(),
                                     0); // "Special", 0 );
  }
  ...
  if (refreshFlag == INFO_REPAINT) {
    spinner_race_group.create(INFO_X1 + 80, INFO_Y1 + 58, INFO_X1 + 210,
                              ...
                              i_disp_race_spinner, hero_race_filter);
  }
    ...
    // construction marker
      ...
      if (!vbrowse_hero_id.none_record) {
        /*	//----- determine the unit id. of the hero -----//
                ...
                //	if( misc.random(2)==0 )
                                unitId =
           race_res[heroInfo->race_id]->infantry_unit_id;
                //	else
                //		unitId =
           race_res[heroInfo->race_id]->special_unit_id;
                }
  ...
  if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, 0)) // left click
  {
    ...
    if (world.zoom_matrix->get_detect_location(
      ...
      if (!vbrowse_hero_id.none_record) {
        ...
        if (hero_res[hero_id_array[hero_id_browse_recno - 1]]->appeared_flag !=
            0) {
        ...
        if (world.check_unit_space(xLoc, yLoc, xLoc + spriteInfo->loc_width - 1,
          ...
          int unitRecno = hero_res.create_powerful_unit(
              xLoc, yLoc, brush_player_recno, unitId, 50, 100);
          if (!unitRecno)
            return 0;
          unit_array[unitRecno]->hero_id =
              hero_res[hero_id_array[hero_id_browse_recno - 1]]->hero_id;
          err_when(hero_res[hero_id_array[hero_id_browse_recno - 1]]
                       ->appeared_flag != 0);
...
static void disp_hero_id(int recNo, int x, int y, int refreshFlag) {
  if (recNo > 0 && recNo <= scenario_editor.hero_id_count)
    font_san.put(x + 6, y + 2,
                 hero_res[scenario_editor.hero_id_array[recNo - 1]]->name, 0,
