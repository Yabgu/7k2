...
int Tutor::select_learning_campaign_menu() {
  ...
  int learningCampaignCount =
      sizeof(learningCampaignList) / sizeof(learningCampaignList[0]);
  ...
  */
  ...
  int pageUpX1 =
      (TLSCROLL_X2 + TLSCROLL_X1 - ((Bitmap *)arrowBitmap)->get_width()) / 2;
  int pageUpY1 =
      (TLSCROLL_Y2 + TLSCROLL_Y1 - ((Bitmap *)arrowBitmap)->get_height()) / 2;
  int pageUpX2 = pageUpX1 + ((Bitmap *)arrowBitmap)->get_width() - 1;
  int pageUpY2 = pageUpY1 + ((Bitmap *)arrowBitmap)->get_height() - 1;
  int pageDownX1 =
      (TRSCROLL_X2 + TRSCROLL_X1 - ((Bitmap *)arrowBitmap)->get_width()) / 2;
  int pageDownY1 =
      (TRSCROLL_Y2 + TRSCROLL_Y1 - ((Bitmap *)arrowBitmap)->get_height()) / 2;
  ...
  // ------- loop ---------//
  {
    ...
    while (1) {
      ...
      if (refreshFlag) {
        if (optionMode == OPTION_SPECIES) {
          if (refreshFlag & SGOPTION_PAGE) {
            ...
            int tutorId = campaign_id_array[browseRecno - 1] * 2 +
                          specy_array[browseRecno - 1] + 1;
            if (browseRecno &&
                image_tutorial.get_index(this->operator[](tutorId)->code)) {
              char *bitmapPtr =
                  image_tutorial.read(this->operator[](tutorId)->code);

              vga.active_buf->put_bitmap(
                  (VGA_WIDTH - ((Bitmap *)bitmapPtr)->get_width()) >> 1,
                  SCROLL_SHEET_Y1 + 10, bitmapPtr);
              font_bold_black.center_put(
                  0, SCROLL_SHEET_Y1 + 10 + ((Bitmap *)bitmapPtr)->get_height(),
                  VGA_WIDTH,
              ...
              int text_buf_size = 0;
              File *filePtr =
                  res_tutor_text.get_file(tutor[tutorId]->des_text, dataSize);
              ...
              if (dataSize > text_buf_size) {
                text_buf = mem_resize(
                    text_buf, dataSize); // allocate a buffer larger than we
              ...
              font_cara_w.center_put_paragraph(
                  SCROLL_SHEET_X1 + 30,
                  SCROLL_SHEET_Y1 + 10 + ((Bitmap *)bitmapPtr)->get_height(),
            ...
            font_cara_w.center_put(pageNoX1, pageNoY1 + 15, pageNoX2,
                                   pageNoY2 + 15, str);

            if (browseRecno > minTutorialRecno)
              //	vga.active_buf->put_bitmap_trans_decompress_hmirror(
              //pageUpX1, pageUpY1, arrowBitmap );
              vga.active_buf->put_bitmap_trans_decompress(pageUpX1, pageUpY1,
                                                          arrowBitmap2);

            if (browseRecno < maxTutorialRecno)
              vga.active_buf->put_bitmap_trans_decompress(
                  pageDownX1, pageDownY1, arrowBitmap);

            // ----- display start, cancel button ------//

            font_thin_black.center_put(BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2,
                                       BUTTON2_Y2, text_game_menu.str_start());
            font_thin_black.center_put(BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2,
                                       BUTTON4_Y2, text_game_menu.str_cancel());
      ...
      int keyCode = mouse.is_key(mouse.scan_code, mouse.event_skey_state,
                                 (WORD)0, K_IS_CTRL | K_CHAR_KEY);
      if (keyCode) {
      ...
      if (optionMode == OPTION_SPECIES) {
        ...
        if (browseRecno > minTutorialRecno &&
            mouse.any_click(TLSCROLL_X1, TLSCROLL_Y1, TLSCROLL_X2,
                            TLSCROLL_Y2)) {
          // previous
          --browseRecno;
          refreshFlag |= SGOPTION_PAGE;
          refreshFlag |= SGOPTION_BUILDING_SIZE;
                   ...
                   mouse.any_click(TRSCROLL_X1, TRSCROLL_Y1, TRSCROLL_X2,
                                   TRSCROLL_Y2)) {
          // next
          ++browseRecno;
          refreshFlag |= SGOPTION_PAGE;
      ...
      //campaign_id_array[browseRecno-1] || 	 (campaign_id_array[browseRecno-1]%2)
      //== 0) )
      {
        // set tutorial type
        ...
        retFlag = campaign_id_array[browseRecno - 1] * 2 +
                  specy_array[browseRecno - 1] + 1;
        break; // break  while(1)
               //{ LCG()	: 0, 1, 2, 3,
               //{ human   : 1, 3, 5, 7, ..
    ...
    if (!achieved_objective_array[i]) {
      ...
      case 1:
        if (cur_tutor_id == 1) {
          ...
          if (custom_detect_is_firm_selected(game.win_condition[2].para1) &&
              continue_box_pressed == 1 && achieved_objective_array[0] == 1) {
            achieved_objective_array[1] = 1;
            return 1;
          }
          continue_box_pressed = 0;
        } else if (cur_tutor_id == 2) { // detect seleting a town
          ...
          //allow_detect_obj_recno && 			detect_press_or_release_flag == 2 &&
          ...
          if (nation_array.player_recno && achieved_objective_array[0] == 1 &&
              custom_search_built_firm(FIRM_RESEARCH,
          ...
          if (achieved_objective_array[1] == 1 &&
              (detect_button(0, 0, 0, 0, "TRAIN", 1) ||
               (!firm_array.is_deleted(firm_array.selected_recno) &&
                firm_array[firm_array.selected_recno]
                    ->cast_to_FirmFort()
                    ->is_soldier_full()))) {
      ...
      case 4:
        if (cur_tutor_id == 1) {
          ...
          //			achieved_objective_array[3] == 1)
          if (custom_detect_is_any_unit_attacking_town(
                  game.win_condition[1].para1) &&
              achieved_objective_array[3] == 1) {
            achieved_objective_array[4] = 1;
            return 4;
          }
      ...
      case 5:
        if (cur_tutor_id == 1) {
          // detect continue_box clicked while the first firm selected
          if (custom_detect_is_firm_selected(game.win_condition[2].para1) &&
              firm_array[game.win_condition[2].para1]
                      ->cast_to_FirmFort()
                      ->soldier_count > 0 &&
          ...
          if (nation_array.player_recno && achieved_objective_array[4] == 1) {
            if (game.win_condition[1].is_condition_true(
                    (~nation_array)->nation_recno)) {
              achieved_objective_array[5] = 1;
              return 5;
            }
      ...
      case 6:
        if (cur_tutor_id == 1) {
          ...
          if ((achieved_objective_array[5] == 1 &&
              ...
              //		detect_left_or_right_flag == 2 &&
              //		achieved_objective_array[5] == 1))
              (custom_detect_is_any_unit_returning_to_firm(
                   game.win_condition[5].para1) &&
               achieved_objective_array[5] == 1)) {
            achieved_objective_array[6] = 1;
            return 6;
          }
          ...
          if (nation_array.player_recno && achieved_objective_array[5] == 1 &&
              custom_search_built_firm(FIRM_SPECIAL,
                                       (~nation_array)->nation_recno)) {
            achieved_objective_array[6] = 1;
            return 6;
          }
      ...
      case 7:
        if (cur_tutor_id == 1) {
          ...
          if (achieved_objective_array[6] == 1 && nation_array.player_recno &&
            ...
            Firm *firmPtr;
            for (int i = 0;
                 i < town_array[town_array.selected_recno]->linked_firm_count;
                 i++) {
              if (firm_array.is_deleted(town_array[town_array.selected_recno]
                                            ->linked_firm_array[i]))
                continue;

              firmPtr = firm_array[town_array[town_array.selected_recno]
                                       ->linked_firm_array[i]];
      ...
      case 11:
        if (cur_tutor_id == 1) {
          ...
          if (nation_array.player_recno && achieved_objective_array[10] == 1) {
            if (game.win_condition[2].is_condition_true(
                    (~nation_array)->nation_recno)) {
              achieved_objective_array[11] = 1;
              return 11;
            }
        ...
        if (cur_tutor_id == 5) {
          ...
          if (nation_array.player_recno && achieved_objective_array[10] == 1 &&
              custom_search_built_firm(FIRM_WAR_FACTORY,
                                       (~nation_array)->nation_recno)) {
            achieved_objective_array[11] = 1;
            return 11;
          }
      ...
      case 12:
        if (cur_tutor_id == 1) {
          ...
          if (custom_detect_is_town_selected(custom_search_independent_town(
                  0, config.race_id, 101, 0, 101, 0)) &&
              achieved_objective_array[11] == 1) {
      ...
      case 14:
        if (cur_tutor_id == 1) {
          ...
          if (custom_detect_is_any_unit_selected(0, UNIT_STOP, 1, RANK_SOLDIER,
                                                 0) &&
              achieved_objective_array[13] == 1 &&
      ...
      case 15:
        ...
        if (cur_tutor_id == 2) {
          ...
          if (nation_array.player_recno && achieved_objective_array[14] == 1 &&
              custom_search_built_firm(FIRM_BASE, (~nation_array)->nation_recno,
                                       1)) {
            achieved_objective_array[15] = 1;
            return 15;
          }
          ...
          for (int firmRecno = firm_array.size(); firmRecno > 0; --firmRecno) {
            ...
            if (firmPtr->cast_to_FirmLair()) {
              for (int t = 0; t < firmPtr->linked_firm_count; t++) {
                if (game.win_condition[3].para1 ==
                        firmPtr->linked_firm_array[t] &&
      ...
      case 21:
        if (cur_tutor_id == 1) {
          ...
          if (town_array.selected_recno &&
              ...
              town_array[town_array.selected_recno]->race_id ==
                  config.race_id &&
              achieved_objective_array[20] == 1) {
      ...
      case 23:
        if (cur_tutor_id == 1) {
          ...
          if (nation_array.player_recno && achieved_objective_array[22] == 1) {
            if (game.win_condition[3].is_condition_true(
                    (~nation_array)->nation_recno)) {
              achieved_objective_array[23] = 1;
              return 23;
            }
      ...
      case 24:
        if (cur_tutor_id == 1) {
          ...
          if (nation_array.player_recno) {
            if (game.win_condition[5].is_condition_true(
                    (~nation_array)->nation_recno)) {
          ...
          if (nation_array.player_recno && achieved_objective_array[23] == 1) {
            if (game.win_condition[4].is_condition_true(
                    (~nation_array)->nation_recno)) {
              achieved_objective_array[24] = 1;
              return 24;
            }
      ...
      case 25:
        if (cur_tutor_id == 1) {
          // waiting for condition three - enslave an town - is true
          if (nation_array.player_recno) {
            if (game.win_condition[6].is_condition_true(
                    (~nation_array)->nation_recno)) {
      ...
      case 33:
        if (cur_tutor_id == 9) {
          ...
          if (achieved_objective_array[32] == 1 &&
              custom_detect_is_any_unit_selected(0, UNIT_STOP, 1, -1, -1, 1,
                                                 1)) {
      ...
      case 34:
        if (cur_tutor_id == 9) {
          // detect continue_box clicked
          if (continue_box_pressed == 1 && achieved_objective_array[33] == 1 &&
              nation_array.player_recno &&
              !unit_array.is_deleted(unit_array.selected_recno) &&
              unit_array[unit_array.selected_recno]->nation_recno ==
                  (~nation_array)->nation_recno) {
      ...
      case 43:
        if (cur_tutor_id == 9) {
          ...
          // has an own spy inside &&
          // detect continue_box clicked
          if (achieved_objective_array[42] == 1 && continue_box_pressed == 1 &&
              nation_array.player_recno &&
              !firm_array.is_deleted(firm_array.selected_recno) &&
              firm_array[firm_array.selected_recno]->nation_recno !=
                  (~nation_array)->nation_recno &&
      ...
      default:
        if (nation_array.player_recno) {
          if (game.win_condition[i].is_condition_true(
                  (~nation_array)->nation_recno)) {
...
void Tutor::disp_objective_text(int objestiveRecNo) {
  ...
  case 1:
    if (cur_tutor_id == 1) {
      ...
      TutorTextBlock *tutorTextBlock =
          text_block_array + text_block_count + objestiveRecNo - 1;
      ...
      tutor_box[0].base_object_recno_no =
          town_array[game.win_condition[1].para1]->base_obj_recno;
      //	strcpy( tutor_box[1].text, "This is your first Fort. Click on it
      //to go next section." );
      tutor_box[1].base_object_recno_no =
          firm_array[game.win_condition[2].para1]->base_obj_recno;
    } else if (cur_tutor_id == 2) {
  ...
  case 9:
    if (cur_tutor_id == 1) {
      ...
      tutor_box[0].base_object_recno_no =
          town_array[game.win_condition[2].para1]->base_obj_recno;
  ...
  case 12:
    if (cur_tutor_id == 1) {
      ...
      selectively_detect_flag = 1;
      allow_detect_obj_recno =
          town_array[custom_search_independent_town(0, config.race_id, 101, 0,
                                                    101, 0)]
              ->base_obj_recno;
      allow_left_or_right_click = 1;
      allow_detect_button_rc = 1;
      world.go_loc(town_array[custom_search_independent_town(0, config.race_id,
                                                             101, 0, 101, 0)]
                       ->center_x,
                   town_array[custom_search_independent_town(0, config.race_id,
                                                             101, 0, 101, 0)]
                       ->center_y);
  ...
  case 15:
    if (cur_tutor_id == 1) {
      ...
      allow_detect_button_rc =
          1; // not really necessary as selectively_detect_flag will not allow
             // clicking button
  ...
  case 17:
    if (cur_tutor_id == 1) {
      ...
      selectively_detect_flag = 1;
      allow_detect_obj_recno =
          firm_array[game.win_condition[3].para1]->base_obj_recno;
      allow_left_or_right_click = 1;
      allow_detect_button_rc = 1;
  ...
  case 21:
    if (cur_tutor_id == 1) {
      ...
      //			allow_detect_button_x1 = INFO_X1 +12 +
      //2*BUTTON_DISTANCE; 			allow_detect_button_y1 = INFO_Y1 +280;
      //			allow_detect_button_x2 = INFO_X1 +15 +
      //3*BUTTON_DISTANCE; 			allow_detect_button_y2 = INFO_Y2;
  ...
  case 25:
    if (cur_tutor_id == 1) {
      ...
      //Independent Town." );
      tutor_box[0].base_object_recno_no =
          firm_array[game.win_condition[4].para1]->base_obj_recno;
    } else if (cur_tutor_id == 5) {
      // allow clicking and doing anything
      TutorTextBlock *tutorTextBlock =
          text_block_array + text_block_count + objestiveRecNo - 1;
  ...
  default:
    if (objestiveRecNo <= result_text_block_count) {
      TutorTextBlock *tutorTextBlock = text_block_array + text_block_count +
                                       intro_text_block_count + objestiveRecNo -
                                       1;
...
int Tutor::detect_button(short x1, short y1, short x2, short y2,
                         const char *buttonName, short buttonRc) {
  if (((x1 <= detect_button_x1) && (y1 <= detect_button_y1) &&
       (x2 >= detect_button_x2) && (y2 >= detect_button_y2)) ||
...
int Tutor::allow_mouse_click(short x, short y, short pressOrRelease,
                             short rightOrLeft) {
  ...
  char mobileType;
  if (x >= ZOOM_X1 && y >= ZOOM_Y1 && x <= ZOOM_X2 && y <= ZOOM_Y2 &&
      (selectively_detect_flag == 1 || selectively_detect_flag == 4 ||
       selectively_detect_flag == 5)) {
    Location *locPtr = power.test_detect(x, y, &mobileType, &xLoc, &yLoc);
    if (locPtr && locPtr->base_obj_recno()) {
      if ((allow_detect_obj_recno == locPtr->base_obj_recno() ||
           allow_detect_obj_recno < 0) &&
          (allow_left_or_right_click == rightOrLeft ||
           !allow_left_or_right_click)) {
    ...
    if (x >= not_allow_info_detect_x1 && y >= not_allow_info_detect_y1 &&
      ...
      return 1;
  else if (x >= INFO_X1 + 13 && y >= INFO_Y1 + 235 && x <= INFO_X2 &&
           y <= INFO_Y2 &&
           (selectively_detect_flag == 3 || selectively_detect_flag == 4))
    return 1;
  else if (x >= MENU_BUTTON_X1 && y >= MENU_BUTTON_Y1 &&
           x <= MENU_BUTTON_X1 + 23 && y <= MENU_BUTTON_Y1 + 76)
    return 1;
...
int Tutor::allow_button_click(short x1, short y1, short x2, short y2,
                              const char *buttonName, short buttonRc) {
  if (bypass_allow_button_click == 1)
    return 1;

  if ((x1 >= MENU_BUTTON_X1 && y1 >= MENU_BUTTON_Y1 &&
       x2 <= MENU_BUTTON_X1 + 27 && y2 <= MENU_BUTTON_Y1 + 80) ||
  ...
  if (((x1 >= allow_detect_button_x1) && (y1 >= allow_detect_button_y1) &&
       (x2 <= allow_detect_button_x2) && (y2 <= allow_detect_button_y2) &&
       (strcmp(allow_detect_button_name, "") == 0)) ||
      //		(strcmp(allow_detect_button_name, buttonName) == 0 &&
      //			strcmp(buttonName, "") != 0)
      strcmp(allow_detect_button_name, buttonName) == 0)
    // allow detect for first click only
    if ((strcmp(detect_button_name, "") == 0 && detect_button_x1 == 0 &&
         detect_button_y1 == 0 && detect_button_x2 == 0 &&
         detect_button_y2 == 0 && detect_button_rc == 0 &&
...
int Tutor::allow_unit_icon_click(short x1, short y1, UnitInfo *unitInfo,
                                 int lightness, short buttonRc) {
  ...
  if (allow_unit_info_detect_lightness >= 0 &&
      lightness != allow_unit_info_detect_lightness)
    return 0;
...
void Tutor::reset_detect_variables() {
  ...
  //ClickedButtonRc unless 							allow_detect_button_rc is disabled (8)  CHECK if
  //given allow_detect_button_name == help coed of the clicked button
  // else
  ...
  //ClickedRc unless 					allow_unit_info_detect_rc bounds is disabled
...
//
int Tutor::custom_detect_is_firm_selected(int firmRecno,
                                          int linkedTownNationRecno) {
  // variable may update within one day
  if (firm_array.selected_recno == firmRecno &&
      !firm_array.is_deleted(firm_array.selected_recno)) {
...
// //
int Tutor::custom_detect_is_any_unit_selected(int unitRecno, char orderMode,
                                              char visible, int rankId,
                                              int isCivilian, int isSpy,
                                              int onlyOneSelected) {
  Unit *unitPtr;
...
int Tutor::custom_search_independent_town(int startTownRecno, char raceId,
                                          int popUpLimit, int popLoLimit,
                                          int resisUpLimit, int resisLoLimit) {
  if (startTownRecno < 1)
    startTownRecno = 1;

  for (int townRecno = startTownRecno; townRecno <= town_array.size();
       townRecno++) {
    if (town_array.is_deleted(townRecno))
      continue;
...
// int linkedTownNationRecno = -1 when ignore
//
int Tutor::custom_search_built_firm(int firmId, int linkedTownNationRecno,
                                    int checkWorker) {
  int firmCount = 0;
  ...
  for (int firmRecno = firm_array.size(); firmRecno > 0; --firmRecno) {
    ...
    if (checkWorker) {
      if (!firmPtr->cast_to_FirmWork() ||
          firmPtr->cast_to_FirmWork()->worker_count < 1)
        continue;
    }
...
// // int linkedTownNationRecno = -1 when ignore
//
int Tutor::custom_detect_is_firm_type_selected(int firmId,
                                               int linkedTownNationRecno) {
  ...
  for (int i = 1; i <= tech_res.tech_count; i++, techInfo++) {
    if (classId < 0 || techInfo->class_id == classId) {
      if (techLevel <= techInfo->max_tech_level(nation_array.player_recno))
        ...
        techInfo->set_nation_tech_level(
            nation_array.player_recno,
...
void Tutor::set_view_mode(int viewMode) { sys.set_view_mode(viewMode); }
...
void Tutor::load_in_game_box(int tutorId, int objectiveRecno) {
  ...
  if (dataSize > in_game_box_text_buf_size) {
    in_game_box_text_buf = mem_resize(
        in_game_box_text_buf, dataSize); // allocate a buffer larger than we
                                         // need for the largest size possible
  ...
  while (!fileTxt.is_eof()) {
    ...
    readLen = fileTxt.read_paragraph(textPtr,
                                     in_game_box_text_buf_size - totalReadLen);

    if (objectiveRecno == objective_no &&
        in_box_count < MAX_IN_GAME_BOX_PER_OBJECTIVE) {
...
int Tutor::allow_this_hot_key(int hotKey) {
  ...
  case 'b': // filter bee and then cycle factory
    ...
    if (selectively_detect_flag == 5 || selectively_detect_flag == 0 ||
        (selectively_detect_flag == 1 &&
         !(!base_obj_array.is_deleted(allow_detect_obj_recno) &&
