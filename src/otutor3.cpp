...
int Tutor::select_learning_campaign_menu()
{
    ...
    // tutorial_finish_count[0] indicates which tutorial (campaign_id) of monster
    // has been finished tutorial_finish_count[1] indicates which tutorial
    // (campaign_id) of human has been finished
    ...
    // static char *learningCampaignList[] = { "Quick Start Tutorial", "Learning
    // Campaign One", "Learning Campaign Two", "Learning Campaign Three" };//,
    // "Fryhtan Campaign", "Fryhtan Learning Campaign", };
    ...
    /*
            ...
            // check in player profile to find which learning campaign have been
    finished for (i = 0; i < 2; i++) #ifdef BETA tutorial_finish_count[i] = 9;
    #else
                    tutorial_finish_count[i] =
    (player_profile.tutorial_finish_count % (int)pow(10.0, (i+1))) /
    (int)pow(10.0, i); #endif
    */
    ...
    /*	GetA playerNameField;
            ...
            speciesGroup[0].create( cx-w/2-10, cy-h/2-5, cx+w/2+10, cy+h/2+5,
                    i_disp_text_button, ButtonCustomPara(
       text_game_menu.str_species_humans(), 0 ), 0 );

            w = font_thin_black.text_width( text_game_menu.str_species_fryhtans()
       ); h = font_thin_black.text_height(); cx = 532; cy = 116;
            speciesGroup[1].create( cx-w/2-10, cy-h/2-5, cx+w/2+10, cy+h/2+5,
                    i_disp_text_button, ButtonCustomPara(
       text_game_menu.str_species_fryhtans(), 1 ), 0 );
            ...
            for( i = 0; i < buildingSizeGroup.button_num; ++i )
            {
                    ...
                    buildingSizeGroup[i].create( cx, cy, cx+w+10, cy+h+10,
                            i_disp_text_button,
       ButtonCustomPara(buildingSizeButtonStr[i], i+1), 0 ); cx += w + 12;
            }
       ...
       (SCROLL_SHEET_X1+SCROLL_SHEET_X2)/2-pictureWidth,
                                                                    SCROLL_SHEET_Y2-pictureHeight,
       "HUMANS" ); image_interface.put_front( (SCROLL_SHEET_X1+SCROLL_SHEET_X2)/2,
                                                                    SCROLL_SHEET_Y2-pictureHeight,
       "FRYHTANS" );
                                                    }
                                                    else
       ...
       (SCROLL_SHEET_X1+SCROLL_SHEET_X2)/2-pictureWidth,
                                                                    SCROLL_SHEET_Y2-pictureHeight,
       "HUMANS" ); image_interface.put_back( (SCROLL_SHEET_X1+SCROLL_SHEET_X2)/2,
                                                                    SCROLL_SHEET_Y2-pictureHeight,
       "FRYHTANS" );
                                                    }

                                                    // ----- display start, cancel
       button ------//

                                                    font_thin_black.center_put(
       BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2, text_game_menu.str_start()
       ); font_thin_black.center_put( BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2,
       BUTTON4_Y2, text_game_menu.str_cancel() );

                                                    for( i = OPTION_SPECIES; i <=
       OPTION_LEARNING_CAMPAIGN; ++i )
                                                    {
                                                            // red font for
       selected Font *fontPtr = (i == optionMode ? &font_bold_red :
       &font_bold_black );

                                                            fontPtr->center_put(OPTION_SWITCH_X1,
       OPTION_SWITCH_Y1+OPTION_SWITCH_Y_SPACING*i, OPTION_SWITCH_X2,
       OPTION_SWITCH_Y2+OPTION_SWITCH_Y_SPACING*i, misc.roman_number(i+1) );
                                                    }

                                                    vga.use_front();
                                                    vga_util.blt_buf( 0, 0,
       VGA_WIDTH-1, VGA_HEIGHT-1, 0 );
                                            }
       ...
       OPTION_SWITCH_Y2+OPTION_SWITCH_Y_SPACING*i, misc.roman_number(i+1) );
                                                    ...
                                                    font_bold_black.put( 230, 386,
       text_game_menu.str_building_set() );
       ...
       VGA_WIDTH-1, VGA_HEIGHT-1, 0 );
                                            ...
                                            if( refreshFlag &
       SGOPTION_BUILDING_SIZE ) buildingSizeGroup.paint(building_size-1);
                                    }
       ...
       MUSIC_CD_THEN_WAV : 0 );
                            ...
                            if( optionMode == OPTION_SPECIES )
                            {
                                    if( speciesGroup.detect() >= 0
                                            ||
       speciesGroup[speciesGroup()].detect(0,0,0,1) )	// detect pushed button,
       but suspend pop
                                    {
                                            if( speciesGroup() == 0 &&
       tempConfig.race_id < 0 )
                                                    tempConfig.race_id = 1;
       // change to the first human race else if( speciesGroup() == 1 &&
       tempConfig.race_id >= 0 ) tempConfig.race_id = -1;	// change to the first
       monster race

                                            optionMode = OPTION_LEARNING_CAMPAIGN;
                                            refreshFlag = SGOPTION_ALL;

                                            // reset learningCampaignGroup state
       as no. of button for human and
                                            // monster is different
                                            learningCampaignGroup.button_pressed =
       0; if (speciesGroup() == 0)
                                            {
                                                    specy = 0;
                                                    learningCampaignGroup.button_num
       = NO_OF_HUMAN_TUTORIAL;
                                            }
                                            else
                                            {
                                                    specy = 1;
                                                    learningCampaignGroup.button_num
       = NO_OF_MONSTER_TUTORIAL;
                                            }
                                    }
                                    else if( mouse.single_click( SCROLL_SHEET_X1,
       SCROLL_SHEET_Y1, (SCROLL_SHEET_X1+SCROLL_SHEET_X2)/2, SCROLL_SHEET_Y2) )
                                    {
                                            if( tempConfig.race_id < 0 )
                                                    tempConfig.race_id = 1;
       // change to the first human race
       ...
       as no. of button for human and
                                            // monster is different
                                            learningCampaignGroup.button_pressed =
       0; learningCampaignGroup.button_num = NO_OF_HUMAN_TUTORIAL; specy = 0;
                                    }
                                    else if( mouse.single_click(
       (SCROLL_SHEET_X1+SCROLL_SHEET_X2)/2+1, SCROLL_SHEET_Y1, SCROLL_SHEET_X2,
       SCROLL_SHEET_Y2) )
                                    {
                                            if( tempConfig.race_id >= 0 )
                                                    tempConfig.race_id = -1;
       // change to the first monster race
       ...
       as no. of button for human and
                                            // monster is different
                                            learningCampaignGroup.button_pressed =
       0; learningCampaignGroup.button_num = NO_OF_MONSTER_TUTORIAL; specy = 1;
                                    }
                            }
                            else
                            if( optionMode == OPTION_LEARNING_CAMPAIGN )
                            {
                                    if( learningCampaignGroup.detect() >= 0
                                            ||
       learningCampaignGroup[learningCampaignGroup()].detect(0,0,0,1) )	//
       detect pushed button, but suspend pop
                                    {
                                    //	optionMode = OPTION_SPECIES;
       // auto change to basic mode campaign_id = learningCampaignGroup();
                                            refreshFlag = SGOPTION_ALL;
                                    }
       ...
       OPTION_SWITCH_Y2+OPTION_SWITCH_Y_SPACING*i) )
                            ...
                            if( mouse.single_click( BUTTON2_X1, BUTTON2_Y1,
       BUTTON2_X2, BUTTON2_Y2 )
                                    && tutorial_finish_count[specy] >= campaign_id
       )
                            {
                                    // set tutorial type
                                    // 0 = old tutorial which has no objective
       checking and use disp_guideline
                                    // 1 = new tutorial which has objective
       checking and use disp_hints tutorial_type = (campaign_id >=
       NO_OF_QUICK_START_TUTORIAL_PER_SPECY) ? 0 : 1;
       ...
       frythan : 2, 4, 6, 8, ..
                            ...
                            if( mouse.single_click( BUTTON4_X1, BUTTON4_Y1,
    ...
    for (i = 0; i < buildingSizeGroup.button_num; ++i)
    {
        ...
        buildingSizeGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                                    ButtonCustomPara(buildingSizeButtonStr[i], i + 1), 0);
        cx += w + 12;
    }
    ...
    // ------- loop ---------//
    {
        ...
        while (1)
        {
                    ...
                    if (refreshFlag & SGOPTION_PAGE)
                    {
                        ...
                        if (browseRecno && image_tutorial.get_index(this->operator[](tutorId)->code))
                        {
                            ...
                            font_bold_black.center_put(0, SCROLL_SHEET_Y1 + 10 + ((Bitmap *)bitmapPtr)->get_height(),
                                                       VGA_WIDTH,
                                                       SCROLL_SHEET_Y1 + 40 + ((Bitmap *)bitmapPtr)->get_height(),
                                                       this->operator[](tutorId)->des);

                            //	if (!(tutorial_finish_count[specy_array[browseRecno-1]]
                            //>= campaign_id_array[browseRecno-1] ||
                            //			(campaign_id_array[browseRecno-1]%2) ==
                            // 0))
                            if (!((tutorial_finish_count + 1) >= ((browseRecno - 1) >> 1)))
                                vga.active_buf->bar_alpha(
                                    (VGA_WIDTH - ((Bitmap *)bitmapPtr)->get_width()) >> 1, SCROLL_SHEET_Y1 + 10,
                                    (VGA_WIDTH + ((Bitmap *)bitmapPtr)->get_width()) >> 1,
                                    SCROLL_SHEET_Y1 + 10 + ((Bitmap *)bitmapPtr)->get_height(), 1, V_BLACK);
                            ...
                            {
                                text_buf = mem_resize(text_buf, dataSize); // allocate a buffer larger than we
                            ...
                            font_cara_w.center_put_paragraph(SCROLL_SHEET_X1 + 30,
                        ...
                        if (browseRecno > minTutorialRecno)
                            //	vga.active_buf->put_bitmap_trans_decompress_hmirror(
                            // pageUpX1, pageUpY1, arrowBitmap );
            ...
            if (optionMode == OPTION_SPECIES)
            {
                ...
                if (browseRecno > minTutorialRecno &&
                    mouse.any_click(TLSCROLL_X1, TLSCROLL_Y1, TLSCROLL_X2, TLSCROLL_Y2))
                {
                    // previous
                    --browseRecno;
                    refreshFlag |= SGOPTION_PAGE;
                ...
                else if (browseRecno < maxTutorialRecno &&
                         mouse.any_click(TRSCROLL_X1, TRSCROLL_Y1, TRSCROLL_X2, TRSCROLL_Y2))
                {
                    // next
                    ++browseRecno;
                    refreshFlag |= SGOPTION_PAGE;
            ...
            // -------- detect start button --------//
            // tutorial_finish_count[0] indicates which tutorial (campaign_id) of
            // monster has been finished tutorial_finish_count[1] indicates which
            // tutorial (campaign_id) of human has been finished since tutorial
            // 1stM,1stH,3rdH,5thH can be played whenever player wants
            if (mouse.single_click(BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2) &&
                ((tutorial_finish_count + 1) >= ((browseRecno - 1) >> 1)))
            //	(tutorial_finish_count[specy_array[browseRecno-1]] >=
            // campaign_id_array[browseRecno-1] ||
            // (campaign_id_array[browseRecno-1]%2)
            //== 0) )
            {
                // set tutorial type
                // 0 = old tutorial which has no objective checking and use
                // disp_guideline 1 = new tutorial which has objective checking and use
                // disp_hints
                //	tutorial_type = (campaign_id_array[browseRecno-1] >=
                // NO_OF_QUICK_START_TUTORIAL_PER_SPECY) ? 0 : 1;
...
void Tutor::update_tutorial_finish_count()
{
    /*	int tempTutorId = cur_tutor_id - 1;
            ...
            if (tempTutorialFinishCount > player_profile.tutorial_finish_count &&
                            ((tempTutorialFinishCount -
    player_profile.tutorial_finish_count) == 10 || (tempTutorialFinishCount -
    player_profile.tutorial_finish_count) == 1))
            {
                    player_profile.tutorial_finish_count =
    tempTutorialFinishCount; player_profile.save();
            }*/
    ...
    if (tempTutorialFinishCount == player_profile.tutorial_finish_count + 1 && (id_array[cur_tutor_id] % 2 == 0))
    {
...
int Tutor::update_tutorial_objective_finish_count()
{
    ...
    for (int i = 0; i < intro_text_block_count + 1; i++)
    {
        if (!achieved_objective_array[i])
        {
            ...
            {
            case 0:
                if (cur_tutor_id == 3 || cur_tutor_id == 7 || cur_tutor_id == 4)
                {
                ...
                else if (cur_tutor_id == 9 || cur_tutor_id == 11)
                {
                    // starting of the tutorial
                    // should be no checking
                    // set all require condition in here
            ...
            case 1:
                if (cur_tutor_id == 1)
                {
                    ...
                    if (custom_detect_is_firm_selected(game.win_condition[2].para1) && continue_box_pressed == 1 &&
                        achieved_objective_array[0] == 1)
                    {
                ...
                { // detect seleting a town
                    ...
                    //		detect_obj_recno ==
                    // town_array[game.win_condition[1].para1]->base_obj_recno &&
                    //		town_array.selected_recno ==
                    // game.win_condition[1].para1) 	if (detect_obj_recno ==
                    // allow_detect_obj_recno && 			detect_press_or_release_flag ==
                    // 2 &&
                    //			detect_left_or_right_flag == 1 &&
                    //			achieved_objective_array[0] == 1)
                    if (custom_detect_is_town_selected(game.win_condition[1].para1) && achieved_objective_array[0] == 1)
                    {
                        achieved_objective_array[1] = 1;
                        return 1;
                    }
                }
                else if (cur_tutor_id == 3 || cur_tutor_id == 7 || cur_tutor_id == 11)
                {
                ...
                else if (cur_tutor_id == 4)
                {
                    // detect clicking on button F_RESE
                    if (achieved_objective_array[0] == 1 && detect_button(0, 0, 0, 0, "F_RESE", 1))
                    {
                        achieved_objective_array[1] = 1;
                        return 1;
                    }
                }
                else if (cur_tutor_id == 5)
                {
                    // detect continue_box clicked
                    if (continue_box_pressed == 1 && achieved_objective_array[0] == 1)
                    {
                ...
                else if (cur_tutor_id == 9)
                {
                    // wait until any Tower of Science have been built
                    if (nation_array.player_recno && achieved_objective_array[0] == 1 &&
            ...
            case 2:
                ...
                else if (cur_tutor_id == 2)
                {
                    ...
                    //		detect_obj_recno ==
                    // firm_array[game.win_condition[5].para1]->base_obj_recno &&
                    //		firm_array.selected_recno ==
                    // game.win_condition[5].para1) 	if (detect_obj_recno ==
                    // allow_detect_obj_recno && 			detect_press_or_release_flag ==
                    // 2 &&
                    //			detect_left_or_right_flag == 1 &&
                    //			achieved_objective_array[1] == 1)
                    if (custom_detect_is_firm_selected(game.win_condition[5].para1) && achieved_objective_array[1] == 1)
                    {
                        achieved_objective_array[2] = 1;
                        return 2;
                    }
                }
                else if (cur_tutor_id == 3 || cur_tutor_id == 7)
                ...
                else if (cur_tutor_id == 4)
                {
                    ...
                    if (achieved_objective_array[1] == 1 && detect_button(0, 0, 0, 0, "CANCEL", 1))
                    {
                        achieved_objective_array[2] = 1;
                        return 2;
                    }
                ...
                else if (cur_tutor_id == 5)
                {
                    ...
                    if (achieved_objective_array[1] == 1 && custom_search_built_firm(FIRM_MINE))
                    {
                        achieved_objective_array[2] = 1;
                        return 2;
                    }
                ...
                else if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[1] == 1 && custom_detect_is_firm_type_selected(FIRM_RESEARCH))
                    {
                        achieved_objective_array[2] = 1;
                        return 2;
                    }
                }
                else if (cur_tutor_id == 11)
                {
                    // detect continue_box clicked
            ...
            case 3:
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[2] == 1)
                    {
                ...
                else if (cur_tutor_id == 2)
                {
                    ...
                    if (achieved_objective_array[2] == 1 && detect_button(0, 0, 0, 0, "F_PATROL", 1))
                    {
                        achieved_objective_array[3] = 1;
                        return 3;
                    }
                ...
                else if (cur_tutor_id == 4)
                {
                    ...
                    // and is selected
                    if (achieved_objective_array[2] == 1 && custom_detect_is_firm_type_selected(FIRM_MAGIC) &&
                        nation_array.player_recno && custom_search_built_firm(FIRM_MAGIC))
                    {
                        achieved_objective_array[3] = 1;
                        return 3;
                    }
                ...
                else if (cur_tutor_id == 5)
                {
                    ...
                    if (achieved_objective_array[2] == 1 && custom_search_built_firm(FIRM_FACTORY))
                    {
                        achieved_objective_array[3] = 1;
                        return 3;
                    }
                ...
                {
                    // detect clicking on button F_PATROL
                    if (achieved_objective_array[2] == 1 && detect_button(0, 0, 0, 0, "RESEARCH", 1))
                    {
                        achieved_objective_array[3] = 1;
                        return 3;
                    }
            ...
            case 4:
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[3] == 1)
                    {
                ...
                else if (cur_tutor_id == 5)
                {
                    ...
                    if (achieved_objective_array[3] == 1 && custom_search_built_firm(FIRM_MARKET))
                    {
                        achieved_objective_array[4] = 1;
                        return 4;
                    }
            ...
            case 5:
                if (cur_tutor_id == 1)
                {
                    ...
                    if (custom_detect_is_firm_selected(game.win_condition[2].para1) &&
                        ...
                        continue_box_pressed == 1 && achieved_objective_array[4] == 1)
                    {
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[4] == 1)
                    {
                        achieved_objective_array[5] = 1;
                        return 5;
                    }
                ...
                else if (cur_tutor_id == 5)
                {
                    ...
                    if (achieved_objective_array[4] == 1 && custom_detect_is_firm_type_selected(FIRM_MARKET))
                    {
                        achieved_objective_array[5] = 1;
                        return 5;
                    }
            ...
            case 6:
                if (cur_tutor_id == 1)
                {
                    ...
                    if (achieved_objective_array[5] == 1 && detect_button(0, 0, 0, 0, "PATROL", 1))
                    {
                ...
                else if (cur_tutor_id == 2)
                {
                    ...
                    if ((achieved_objective_array[5] == 1 && detect_button(0, 0, 0, 0, "F_RETCMP", 1)) ||
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[5] == 1)
                    {
                        achieved_objective_array[6] = 1;
                        return 6;
                    }
                ...
                else if (cur_tutor_id == 9)
                {
                    ...
                    if (nation_array.player_recno && achieved_objective_array[5] == 1 &&
            ...
            case 7:
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[6] == 1)
                    {
                ...
                else if (cur_tutor_id == 2)
                {
                    ...
                    //			detect_left_or_right_flag == 1 &&
                    //			achieved_objective_array[6] == 1)
                    if (custom_detect_is_town_selected(game.win_condition[1].para1) && achieved_objective_array[6] == 1)
                    {
                        achieved_objective_array[7] = 1;
                        return 7;
                    }
                ...
                else if (cur_tutor_id == 5)
                {
                    ...
                    if (achieved_objective_array[6] == 1 && nation_array.player_recno &&
                ...
                else if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[6] == 1 && custom_detect_is_firm_type_selected(FIRM_SPECIAL))
                    {
                        achieved_objective_array[7] = 1;
                        return 7;
                    }
            ...
            case 8:
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[7] == 1)
                    {
                ...
                else if (cur_tutor_id == 2)
                {
                    ...
                    //			detect_left_or_right_flag == 1 &&
                    //			achieved_objective_array[7] == 1)
                    if (custom_detect_is_firm_selected(game.win_condition[5].para1) && achieved_objective_array[7] == 1)
                    {
                        achieved_objective_array[8] = 1;
                        return 8;
                    }
            ...
            case 9:
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && custom_detect_is_any_unit_selected(0, UNIT_STOP, 1, -1, 0) &&
                        achieved_objective_array[8] == 1)
                    {
                ...
                {
                    // detect clicking on button F_PATROL
                    if (achieved_objective_array[8] == 1 && detect_button(0, 0, 0, 0, "F_PATROL", 1))
                    {
                        achieved_objective_array[9] = 1;
                        return 9;
                    }
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[8] == 1)
                    {
                        achieved_objective_array[9] = 1;
                        return 9;
                    }
            ...
            case 10:
                if (cur_tutor_id == 1)
                {
                    ...
                    if (achieved_objective_array[9] == 1 && detect_button(0, 0, 0, 0, "RETCAMP", 1))
                    {
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[9] == 1)
                    {
                        achieved_objective_array[10] = 1;
                        return 10;
                    }
            ...
            case 11:
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[10] == 1)
                    {
                ...
                if (cur_tutor_id == 5)
                {
                    ...
                    if (achieved_objective_array[10] == 1 && custom_detect_is_any_town_selected())
                    {
                        achieved_objective_array[11] = 1;
                        return 11;
                    }
            ...
            case 12:
                if (cur_tutor_id == 1)
                {
                    ...
                    if (custom_detect_is_town_selected(
                            custom_search_independent_town(0, config.race_id, 101, 0, 101, 0)) &&
                        achieved_objective_array[11] == 1)
                    {
                ...
                else if (cur_tutor_id == 2)
                {
                    // detect clicking on button F_RETCMP
                    // detect right click on a firm
                    if ((achieved_objective_array[11] == 1 && detect_button(0, 0, 0, 0, "F_RETCMP", 1)) ||
                        //	(detect_obj_recno == allow_detect_obj_recno &&
                        //		detect_press_or_release_flag == 2 &&
                        //		detect_left_or_right_flag == 2 &&
                        //		achieved_objective_array[11] == 1))
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[11] == 1)
                    {
                        achieved_objective_array[12] = 1;
                        return 12;
                    }
                ...
                {
                    // wait until any War Factory have been selected
                    if (achieved_objective_array[11] == 1 && custom_detect_is_firm_type_selected(FIRM_WAR_FACTORY))
                    {
                        achieved_objective_array[12] = 1;
                        return 12;
                    }
            ...
            case 13:
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[12] == 1)
                    {
                ...
                else if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[12] == 1 && detect_button(0, 0, 0, 0, "MAKEWEAP", 1))
                    {
                        achieved_objective_array[13] = 1;
                        return 13;
                    }
            ...
            case 14:
                if (cur_tutor_id == 1)
                {
                    ...
                    if (custom_detect_is_any_unit_selected(0, UNIT_STOP, 1, RANK_SOLDIER, 0) &&
                        achieved_objective_array[13] == 1 && unit_array[unit_array.selected_recno]->is_human())
                    {
                ...
                else if (cur_tutor_id == 2)
                {
                    ...
                    if (!nation_array.player_recno || achieved_objective_array[13] == 0)
                        break;
                ...
                {
                    // detect on selecting any soldiers
                    if (custom_detect_view_mode(MODE_NATION) && achieved_objective_array[13] == 1)
                    {
                        achieved_objective_array[14] = 1;
                        return 14;
                    }
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[13] == 1)
                    {
                        achieved_objective_array[14] = 1;
#ifdef DEMO
            ...
            case 15:
                if (cur_tutor_id == 1)
                {
                    ...
                    if (achieved_objective_array[14] == 1 && detect_button(0, 0, 0, 0, "BUILD", 1))
                    {
                        achieved_objective_array[15] = 1;
                        return 15;
                    }
                ...
                if (cur_tutor_id == 2)
                {
                    ...
                    //		 ((Unit*)base_obj_array[detect_obj_recno])->nation_recno
                    //==
                    //(~nation_array)->nation_recno &&
                    //		 ((Unit*)base_obj_array[detect_obj_recno])->cur_order.mode
                    //== UNIT_STOP && 		 achieved_objective_array[14] == 1)
                    if (achieved_objective_array[14] == 1 && custom_detect_is_any_unit_selected(0, UNIT_STOP, 1, -1, 0))
                    {
                        achieved_objective_array[15] = 1;
                        return 15;
                    }
                ...
                else if (cur_tutor_id == 5)
                {
                    ...
                    if (custom_detect_talk_message_id(1) && achieved_objective_array[14] == 1)
                    {
                        achieved_objective_array[15] = 1;
                        return 15;
                    }
                ...
                else if (cur_tutor_id == 9)
                {
                    ...
                    if (nation_array.player_recno && achieved_objective_array[14] == 1 &&
            ...
            case 16:
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[15] == 1)
                    {
                        achieved_objective_array[16] = 1;
                        return 16;
                    }
                ...
                else if (cur_tutor_id == 2)
                {
                    ...
                    if (achieved_objective_array[15] == 1 && detect_button(0, 0, 0, 0, "F_PROMOT", 1))
                    {
                        achieved_objective_array[16] = 1;
                        return 16;
                    }
                ...
                {
                    // detect selecting a market
                    if (achieved_objective_array[15] == 1 && custom_detect_is_firm_type_selected(FIRM_MARKET))
                    {
                        achieved_objective_array[16] = 1;
                        return 16;
                    }
                ...
                else if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[15] == 1 && custom_detect_is_firm_type_selected(FIRM_BASE))
                    {
                        achieved_objective_array[16] = 1;
                        return 16;
                    }
            ...
            case 17:
                ...
                {
                    // detect seleting a firm
                    if (detect_obj_recno == allow_detect_obj_recno && detect_press_or_release_flag == 2 &&
                        detect_left_or_right_flag == 1 && achieved_objective_array[16] == 1)
                    {
                        achieved_objective_array[17] = 1;
                        return 17;
                    }
                ...
                else if (cur_tutor_id == 5)
                {
                    ...
                    if (achieved_objective_array[16] == 1 && detect_button(0, 0, 0, 0, "HIRECARA", 1))
                    {
                        achieved_objective_array[17] = 1;
                        return 17;
                    }
                ...
                else if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[16] == 1 && detect_button(0, 0, 0, 0, "GDINVOKE", 1))
                    {
                        achieved_objective_array[17] = 1;
                        return 17;
                    }
            ...
            case 18:
                if (cur_tutor_id == 1)
                {
                    ...
                    if (achieved_objective_array[17] == 1 && detect_button(0, 0, 0, 0, "PROMOTE", 1))
                    {
                        achieved_objective_array[18] = 1;
                        return 18;
                    }
                ...
                else if (cur_tutor_id == 2)
                {
                    ...
                    //(~nation_array)->nation_recno &&
                    //		 ((Unit*)base_obj_array[detect_obj_recno])->cur_order.mode
                    //== UNIT_STOP && 		 achieved_objective_array[17] == 1)
                    if (achieved_objective_array[17] == 1 && custom_detect_is_any_unit_selected(0, UNIT_STOP, 1, -1, 0))
                    {
                        achieved_objective_array[18] = 1;
                        return 18;
                    }
                ...
                else if (cur_tutor_id == 5)
                {
                    ...
                    if (achieved_objective_array[17] == 1 && custom_detect_is_unit_type_selected(UNIT_CARAVAN))
                    {
                        achieved_objective_array[18] = 1;
                        return 18;
                    }
                ...
                {
                    // detect clicking on any caravan
                    if (achieved_objective_array[17] == 1 && (custom_detect_is_unit_type_selected(UNIT_NORMAN_GOD) ||
            ...
            case 19:
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[18] == 1)
                    {
                        achieved_objective_array[19] = 1;
                        return 19;
                    }
                ...
                {
                    // detect clicking on button BUILD
                    if (achieved_objective_array[18] == 1 && detect_button(0, 0, 0, 0, "BUILD", 1))
                    {
                        achieved_objective_array[19] = 1;
                        return 19;
                    }
            ...
            case 20:
                if (cur_tutor_id == 1)
                {
                    ...
                    if (!nation_array.player_recno || achieved_objective_array[19] == 0)
                        break;
                ...
                {
                    // detect selecting a Seat of Power
                    if (achieved_objective_array[19] == 1 && custom_detect_is_firm_type_selected(FIRM_BASE))
                    {
                        achieved_objective_array[20] = 1;
                        return 20;
                    }
            ...
            case 21:
                ...
                else if (cur_tutor_id == 2)
                {
                    // detect seleting a firm
                    if (detect_obj_recno == allow_detect_obj_recno && detect_press_or_release_flag == 2 &&
                        detect_left_or_right_flag == 1 && achieved_objective_array[20] == 1)
                    {
                        achieved_objective_array[21] = 1;
                        return 21;
                    }
                ...
                {
                    // detect clicking on any caravan
                    if (achieved_objective_array[20] == 1 && custom_detect_is_unit_type_selected(UNIT_CARAVAN))
                    {
                        achieved_objective_array[21] = 1;
                        return 21;
                    }
                ...
                else if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[20] == 1 && detect_button(0, 0, 0, 0, "BLESS", 1))
                    {
                        achieved_objective_array[21] = 1;
                        return 21;
                    }
            ...
            case 22:
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed && achieved_objective_array[21] == 1)
                    {
                        achieved_objective_array[22] = 1;
                        return 22;
                    }
                ...
                else if (cur_tutor_id == 2)
                {
                    ...
                    if (achieved_objective_array[21] == 1 && detect_button(0, 0, 0, 0, "F_GRANT", 1))
                    {
                        achieved_objective_array[22] = 1;
                        return 22;
                    }
            ...
            case 23:
                if (cur_tutor_id == 1)
                {
                    ...
                    if (!nation_array.player_recno || achieved_objective_array[22] == 0)
                        break;

                    if (!town_array.is_deleted(par1) && town_array[par1]->nation_recno == (~nation_array)->nation_recno)
                    {
                ...
                else if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[22] == 1 && custom_detect_is_firm_type_selected(FIRM_INN))
                    {
                        achieved_objective_array[23] = 1;
                        return 23;
                    }
            ...
            case 25:
                ...
                {
                    // detect clicking on any caravan
                    if (achieved_objective_array[24] == 1 && custom_detect_is_unit_type_selected(UNIT_CARAVAN))
                    {
                        achieved_objective_array[25] = 1;
                        return 25;
                    }
            ...
            case 26:
                if (cur_tutor_id == 5)
                {
                    ...
                    if (achieved_objective_array[25] == 1 && detect_button(0, 0, 0, 0, "CARACOPY", 1))
                    {
                ...
                {
                    // detect selecting an Inn
                    if (achieved_objective_array[25] == 1 && custom_detect_is_firm_type_selected(FIRM_INN))
                    {
                        achieved_objective_array[26] = 1;
                        return 26;
                    }
            ...
            case 29:
                if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[28] == 1 && custom_detect_is_firm_type_selected(FIRM_SPY))
                    {
            ...
            case 30:
                if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[29] == 1 &&
                        (detect_button(0, 0, 0, 0, "TRAIN-C", 1) || detect_button(0, 0, 0, 0, "TRAIN-I", 1) ||
            ...
            case 31:
                if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[30] == 1 && nation_array.player_recno &&
                        custom_detect_is_any_town_selected())
                    {
                        if (town_array.is_deleted(town_array.selected_recno))
                            break;
                        ...
                        for (int i = spy_array.size(); i >= 1; i--)
                        {
                            ...
                            if (spyPtr->spy_place == SPY_TOWN && spyPtr->spy_place_para == town_array.selected_recno &&
            ...
            case 34:
                ...
                {
                    // detect continue_box clicked
                    if (continue_box_pressed == 1 && achieved_objective_array[33] == 1 && nation_array.player_recno &&
            ...
            case 35:
                if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[34] == 1 && detect_button(0, 0, 0, 0, "SPYNOTI0", 1))
                    {
            ...
            case 36:
                if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[35] == 1 && detect_button(0, 0, 0, 0, "SPYNOTI1", 1))
                    {
            ...
            case 38:
                if (cur_tutor_id == 9)
                {
                    ...
                    // has an own spy inside
                    if (achieved_objective_array[37] == 1 && nation_array.player_recno &&
                        !town_array.is_deleted(town_array.selected_recno) &&
                        town_array[town_array.selected_recno]->nation_recno != (~nation_array)->nation_recno)
                    {
                        Spy *spyPtr;
                        for (int i = spy_array.size(); i >= 1; i--)
                        {
            ...
            case 39:
                if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[38] == 1 && detect_button(0, 0, 0, 0, "SPYMENU", 1))
                    {
            ...
            case 40:
                if (cur_tutor_id == 9)
                {
                    ...
                    if (achieved_objective_array[39] == 1 && detect_button(0, 0, 0, 0, "CHG-MISS", 1))
                    {
            ...
            case 41:
                if (cur_tutor_id == 9)
                {
                    ...
                    // has an own spy inside
                    if (achieved_objective_array[40] == 1 && nation_array.player_recno &&
                        !firm_array.is_deleted(firm_array.selected_recno) &&
                        firm_array[firm_array.selected_recno]->nation_recno != (~nation_array)->nation_recno &&
                        firm_array[firm_array.selected_recno]->cast_to_FirmCamp())
                    {
                        ...
                        for (int i = spy_array.size(); i >= 1; i--)
                        {
                            ...
                            if (spyPtr->spy_place == SPY_FIRM && spyPtr->spy_place_para == firm_array.selected_recno &&
                                spyPtr->true_nation_recno == (~nation_array)->nation_recno)
                            {
            ...
            case 43:
                if (cur_tutor_id == 9)
                {
                    ...
                    // has an own spy inside &&
                    // detect continue_box clicked
                    if (achieved_objective_array[42] == 1 && continue_box_pressed == 1 && nation_array.player_recno &&
                        !firm_array.is_deleted(firm_array.selected_recno) &&
                        firm_array[firm_array.selected_recno]->nation_recno != (~nation_array)->nation_recno &&
                        firm_array[firm_array.selected_recno]->cast_to_FirmCamp())
                    {
            ...
            case 47:
                if (cur_tutor_id == 9)
                {
                    // detect clicking any of enemy firm which
                    // has an own spy inside
                    if (achieved_objective_array[46] == 1 && nation_array.player_recno &&
                        !firm_array.is_deleted(firm_array.selected_recno) &&
                        firm_array[firm_array.selected_recno]->nation_recno != (~nation_array)->nation_recno &&
                        firm_array[firm_array.selected_recno]->cast_to_FirmCamp())
                    {
            ...
            case 48:
                ...
                {
                    // detect clicking on button SPYMENU
                    if (achieved_objective_array[47] == 1 && detect_button(0, 0, 0, 0, "SPYMENU", 1))
                    {
...
void Tutor::disp_objective_text(int objestiveRecNo)
{
    ...
    case 1:
        if (cur_tutor_id == 1)
        {
            ...
            //	strcpy( tutor_box[0].text, "This is your first Town. Click on it
            // to see the population and loyalty level." );
            tutor_box[0].base_object_recno_no = town_array[game.win_condition[1].para1]->base_obj_recno;
            //	strcpy( tutor_box[1].text, "This is your first Fort. Click on it
            // to go next section." );
        ...
        else if (cur_tutor_id == 2)
        {
            ...
            load_in_game_box(cur_tutor_id, 1);
            //	strcpy( tutor_box[0].text, "This is an Independent Human Town.
            // Click on it." );
    ...
    case 2:
        ...
        else if (cur_tutor_id == 2)
        {
            ...
            load_in_game_box(cur_tutor_id, 2);
            //	strcpy( tutor_box[0].text, "This shows the Town's Resistance to
            // your rule." );
        ...
        else if (cur_tutor_id == 4)
        {
            ...
            load_in_game_box(cur_tutor_id, 2);
            //	strcpy( tutor_box[0].text, "These are your choices for Fryhtan
            // Technology. These choices will be different for every species of
            // Fryhtan." );
    ...
    case 4:
        ...
        else if (cur_tutor_id == 4)
        {
            ...
            load_in_game_box(cur_tutor_id, 4);
            //	strcpy( tutor_box[0].text, "This is your Power Bar. When it is
            // blue, your Mage Tor has enough power to project its power." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 160;
            tutor_box[0].screen_loc_centre_y = INFO_Y1 + 20;
            tutor_box[0].pointer_offset_x = 0;
            tutor_box[0].pointer_offset_y = -20;
            //	strcpy( tutor_box[1].text, "These are your options for using
            // your Mage Tor. Click on one and then Right-Click on your target." );
    ...
    case 5:
        if (cur_tutor_id == 1)
        {
            ...
            //	strcpy( tutor_box[0].text, "This is your first Fort. Click on it
            // to go next section." );
        ...
        else if (cur_tutor_id == 2)
        {
            ...
            load_in_game_box(cur_tutor_id, 5);
            //	strcpy( tutor_box[0].text, "Your troop is now enslaving this
            // Town." );
            tutor_box[0].base_object_recno_no = town_array[game.win_condition[1].para1]->base_obj_recno;
        }
        else if (cur_tutor_id == 4)
    ...
    case 6:
        ...
        {
            // not allow clicking and doing anything
            // allow any click inside the INFO area, allow and detect only SORTIE
            // button
            TutorTextBlock *tutorTextBlock = text_block_array + text_block_count + objestiveRecNo - 1;
            current_objective = objestiveRecNo;
            reset_detect_variables();
            ...
            //	strcpy( tutor_box[0].text, "Click here to view the Leadership
            // Level of all those in this Fort." );
        ...
        else if (cur_tutor_id == 2)
        {
            ...
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 150;
            tutor_box[0].screen_loc_centre_y = INFO_Y1 + 300;
            //	strcpy( tutor_box[1].text, "This is your Lair. Return your troop
            // to it." );
        ...
        else if (cur_tutor_id == 5)
        {
            ...
            load_in_game_box(cur_tutor_id, 6);
            //	strcpy( tutor_box[0].text, "This shows the goods that your
            // Market is selling and how much money you are making." );
    ...
    case 7:
        ...
        else if (cur_tutor_id == 2)
        {
            ...
            //	strcpy( tutor_box[0].text, "This is your Slave Town. Click on
            // it." );
            tutor_box[0].base_object_recno_no = allow_detect_obj_recno;
        }
        else if (cur_tutor_id == 5)
    ...
    case 8:
        ...
        else if (cur_tutor_id == 2)
        {
            ...
            //	strcpy( tutor_box[2].text, "This shows the amount of tribute
            // that you receive from your Slave Town." );
        ...
        else if (cur_tutor_id == 9)
        {
            ...
            load_in_game_box(cur_tutor_id, 8);
            //	strcpy( tutor_box[0].text, "This is the Train Special Units
            // Icon." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 40;
            tutor_box[0].screen_loc_centre_y = INFO_Y1 + 300;
        }
    ...
    case 12:
        if (cur_tutor_id == 1)
        {
            ...
            reset_detect_variables();
            selectively_detect_flag = 1;
            allow_detect_obj_recno =
                town_array[custom_search_independent_town(0, config.race_id, 101, 0, 101, 0)]->base_obj_recno;
            allow_left_or_right_click = 1;
            allow_detect_button_rc = 1;
            ...
            //	strcpy( tutor_box[0].text, "This is an Independent Town of same
            // nationality as your kingdom" );
            tutor_box[0].base_object_recno_no = allow_detect_obj_recno;
        }
        else if (cur_tutor_id == 2)
        {
    ...
    case 13:
        if (cur_tutor_id == 1)
        {
            ...
            //	strcpy( tutor_box[0].text, "This is the Independent Town's
            // Resistance Level." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 145;
            tutor_box[0].screen_loc_centre_y = INFO_Y1 + 180;
        }
        else if (cur_tutor_id == 2)
        {
            ...
            load_in_game_box(cur_tutor_id, 13);
            //	strcpy( tutor_box[0].text, "Click on your Lair when your troop
            // have all returned." );
            tutor_box[0].base_object_recno_no = allow_detect_obj_recno;
        }
        else if (cur_tutor_id == 5)
    ...
    case 14:
        ...
        else if (cur_tutor_id == 2)
        {
            ...
            //	strcpy( tutor_box[0].text, "Click here to view the Leadership
            // Level of all those in this Lair." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 150;
            tutor_box[0].screen_loc_centre_y = INFO_Y1 + 210;
            tutor_box[0].pointer_offset_y = -200;
        }
        else if (cur_tutor_id == 5)
        {
            ...
            load_in_game_box(cur_tutor_id, 14);
            //	strcpy( tutor_box[0].text, "This is the Kingdoms Button. You may
            // Click here or press F1 on your keyboard to bring up the Diplomacy Menu.
            // Press Esc to exit the Kingdoms menu." );
            if (current_display_mode.mode_id == MODE_ID_1024x768x16)
            {
    ...
    case 15:
        ...
        else if (cur_tutor_id == 2)
        {
            ...
            allow_detect_button_rc = 1;    // not really necessary as selectively_detect_flag will not allow
                                           // clicking button
    ...
    case 16:
        ...
        else if (cur_tutor_id == 5)
        {
            ...
            load_in_game_box(cur_tutor_id, 16);
            //	strcpy( tutor_box[0].text, "This shows the replay to your Trade
            // proposal." );
    ...
    case 17:
        ...
        {
            // not allow clicking and doing anything
            // and allow free access to lower left second INFO buttons.(ie. invoke god
            // icon)
            TutorTextBlock *tutorTextBlock = text_block_array + text_block_count + objestiveRecNo - 1;
            current_objective = objestiveRecNo;
            reset_detect_variables();
            ...
            load_in_game_box(cur_tutor_id, 17);
            //	strcpy( tutor_box[0].text, "This is the Invoke Greater Being
            // Icon." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 90;
            tutor_box[0].screen_loc_centre_y = INFO_Y1 + 300;
        }
    ...
    case 19:
        ...
        else if (cur_tutor_id == 9)
        {
            ...
            load_in_game_box(cur_tutor_id, 19);
            //	strcpy( tutor_box[0].text, "This text describes how to use your
            // Greater Being." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 80;
            tutor_box[0].screen_loc_centre_y = INFO_Y1 + 60;
            //	strcpy( tutor_box[1].text, "These are the Prayer Points that
            // sustain your Greater Being." );
    ...
    case 21:
        ...
        else if (cur_tutor_id == 9)
        {
            ...
            //			allow_detect_button_x1 = INFO_X1 +12 +
            // 2*BUTTON_DISTANCE; 			allow_detect_button_y1 = INFO_Y1
            // +280;
            //			allow_detect_button_x2 = INFO_X1 +15 +
            // 3*BUTTON_DISTANCE; 			allow_detect_button_y2 =
            // INFO_Y2;
            ...
            load_in_game_box(cur_tutor_id, 21);
            //	strcpy( tutor_box[1].text, "This is the Special Blessing Icon.
            // It is only visible when your Prayer Points are full." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 150;
            tutor_box[0].screen_loc_centre_y = INFO_Y1 + 300;
        }
        break;
    ...
    case 22:
        if (cur_tutor_id == 1)
        {
            ...
            //	strcpy( tutor_box[0].text, "This is the town that you want to
            // win." );
            if (!town_array.is_deleted(par1))
                tutor_box[0].base_object_recno_no = town_array[par1]->base_obj_recno;
            //	strcpy( tutor_box[1].text, "This shows the falling level of
            // Resistance in this Independent Town." );
            tutor_box[1].screen_loc_centre_x = INFO_X1 + 145;
            tutor_box[1].screen_loc_centre_y = INFO_Y1 + 180;
        }
        else if (cur_tutor_id == 2)
        ...
        else if (cur_tutor_id == 5)
        {
            ...
            load_in_game_box(cur_tutor_id, 22);
            //	strcpy( tutor_box[0].text, "This shows the selection of Goods
            // for your Caravan's load." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 100;
    ...
    case 24:
        if (cur_tutor_id == 1)
        {
            ...
            //	strcpy( tutor_box[0].text, "This is a Fryhtan Lair linked to a
            // still Independent Town." );
        ...
        else if (cur_tutor_id == 9)
        {
            ...
            load_in_game_box(cur_tutor_id, 24);
            //	strcpy( tutor_box[0].text, "This is where Mercenary Soldiers
            // gather in the Inn." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 100;
    ...
    case 25:
        if (cur_tutor_id == 1)
        {
            ...
            //	strcpy( tutor_box[0].text, "This is a Fryhtan Lair linked to an
            // Independent Town." );
    ...
    case 27:
        ...
        else if (cur_tutor_id == 9)
        {
            ...
            //	strcpy( tutor_box[0].text, "This is where Mercenary Spies gather
            // in the Inn." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 150;
    ...
    case 30:
        ...
        {
            // not allow clicking and doing anything
            // and allow free access to lower left first three INFO buttons.(ie. all
            // train button in spy college)
            TutorTextBlock *tutorTextBlock = text_block_array + text_block_count + objestiveRecNo - 1;
            current_objective = objestiveRecNo;
            reset_detect_variables();
            selectively_detect_flag = 3;
            ...
            //	strcpy( tutor_box[1].text, "Click on the following 3 Spy
            // Training Icon to train different kinds of Spies." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 90;
            tutor_box[0].screen_loc_centre_y = INFO_Y1 + 300;
        }
        break;
    ...
    case 32:
        if (cur_tutor_id == 9)
        {
            ...
            //	strcpy( tutor_box[0].text, "Click on this to see more Spying
            // options." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 150;
            tutor_box[0].screen_loc_centre_y = INFO_Y1 + 250;
        }
    ...
    case 34:
        if (cur_tutor_id == 9)
        {
            ...
            //	strcpy( tutor_box[0].text, "Click on one of these to change your
            // Spy's color." );
            tutor_box[0].screen_loc_centre_x = INFO_X1 + 160;
    ...
    default:
        if (objestiveRecNo <= result_text_block_count)
        {
            TutorTextBlock *tutorTextBlock =
                text_block_array + text_block_count + intro_text_block_count + objestiveRecNo - 1;
...
{
    if (((x1 <= detect_button_x1) && (y1 <= detect_button_y1) && (x2 >= detect_button_x2) &&
         (y2 >= detect_button_y2)) ||
...
int Tutor::allow_mouse_click(short x, short y, short pressOrRelease, short rightOrLeft)
{
    ...
    if (!selectively_detect_flag || in_game_menu.is_active() || option_menu.is_active() || power.win_opened ||
        box.opened_flag)
        return 1;
    ...
    else if (x >= INFO_X1 && y >= INFO_Y1 && x <= INFO_X2 && y <= INFO_Y2 &&
             (selectively_detect_flag == 2 || selectively_detect_flag == 5))
        if (x >= not_allow_info_detect_x1 && y >= not_allow_info_detect_y1 && x <= not_allow_info_detect_x2 &&
            y <= not_allow_info_detect_y2)
            return 0;
        else
            return 1;
    else if (x >= INFO_X1 + 13 && y >= INFO_Y1 + 235 && x <= INFO_X2 && y <= INFO_Y2 &&
             (selectively_detect_flag == 3 || selectively_detect_flag == 4))
        return 1;
    else if (x >= MENU_BUTTON_X1 && y >= MENU_BUTTON_Y1 && x <= MENU_BUTTON_X1 + 23 && y <= MENU_BUTTON_Y1 + 76)
        return 1;
...
int Tutor::allow_button_click(short x1, short y1, short x2, short y2, const char *buttonName, short buttonRc)
{
    ...
    if ((x1 >= MENU_BUTTON_X1 && y1 >= MENU_BUTTON_Y1 && x2 <= MENU_BUTTON_X1 + 27 && y2 <= MENU_BUTTON_Y1 + 80) ||
        (in_game_menu.is_active() || option_menu.is_active() || power.win_opened || box.opened_flag))
    {
        strcpy(detect_button_name, "");
        detect_button_x1 = 0;
        detect_button_y1 = 0;
    ...
    if (((x1 >= allow_detect_button_x1) && (y1 >= allow_detect_button_y1) && (x2 <= allow_detect_button_x2) &&
         (y2 <= allow_detect_button_y2) && (strcmp(allow_detect_button_name, "") == 0)) ||
        //		(strcmp(allow_detect_button_name, buttonName) == 0 &&
        //			strcmp(buttonName, "") != 0)
        strcmp(allow_detect_button_name, buttonName) == 0)
        // allow detect for first click only
        if ((strcmp(detect_button_name, "") == 0 && detect_button_x1 == 0 && detect_button_y1 == 0 &&
             detect_button_x2 == 0 && detect_button_y2 == 0 && detect_button_rc == 0 &&
             (allow_detect_button_rc == buttonRc || !allow_detect_button_rc)) ||
            selectively_detect_flag == 0 || selectively_detect_flag == 5)
        {
...
void Tutor::reset_detect_variables()
{
    ...
    //					allow_detect_obj_recno < 0
    // and (3)	CHECK allow_left_or_right_click == ClickedRightorLeft unless
    //					allow_left_or_right_click is disabled
    //###### each call to allow_mouse_click() WILL overwrite the followings
    //######// (4) WRITE detect_obj_recno = locPtr->base_obj_recno(); (5) WRITE
    // detect_press_or_release_flag = pressOrRelease; (6) WRITE
    // detect_left_or_right_flag = rightOrLeft;
    ...
    //###### if any button is clicked in this objective once, no more can be
    // clicked ######// (7)	 CHECK first allow_detect_button_rc ==
    // ClickedButtonRc unless 							allow_detect_button_rc is
    // disabled (8) CHECK if given allow_detect_button_name == help coed of the
    // clicked button else
    ...
    //(16) CHECK not_allow_unit_info_detect_x2, y2 bounds the
    // ClickedUnitIconBitmapX1, Y1 (17) CHECK allow_unit_info_detect_rc bounds ==
    // ClickedRc unless 					allow_unit_info_detect_rc bounds is
    // disabled
...
int Tutor::custom_detect_is_unit_selected(int unitRecno)
{
    ...
    if (unit_array.selected_recno == unitRecno && unit_array.selected_count == 1)
        return 1;
    return 0;
}
...
// //
int Tutor::custom_detect_is_any_unit_selected(int unitRecno, char orderMode, char visible, int rankId, int isCivilian,
                                              int isSpy, int onlyOneSelected)
{
    Unit *unitPtr;
...
int Tutor::custom_search_independent_town(int startTownRecno, char raceId, int popUpLimit, int popLoLimit,
                                          int resisUpLimit, int resisLoLimit)
{
    if (startTownRecno < 1)
        startTownRecno = 1;
...
int Tutor::custom_detect_is_firm_type_selected(int firmId, int linkedTownNationRecno)
{
    ...
    Firm *firmPtr = firm_array[firm_array.selected_recno];
    if (firmPtr->nation_recno == (~nation_array)->nation_recno && firmPtr->firm_id == firmId)
    {
        if (linkedTownNationRecno < 0)
            return 1;
...
{
    if (unit_array.is_deleted(unit_array.selected_recno) || unit_array.selected_count > 1)
        return 0;
    ...
    if (unitPtr->nation_recno == (~nation_array)->nation_recno && unitPtr->unit_id == unitId)
        return 1;
...
void Tutor::load_in_game_box(int tutorId, int objectiveRecno)
{
    ...
    {
        in_game_box_text_buf = mem_resize(in_game_box_text_buf, dataSize); // allocate a buffer larger than we
                                                                           // need for the largest size possible
    ...
    int readLen,
        totalReadLen = 0; // total length of text read into the tutor_text_buf
    int loopCount = 0;
    char *tokenStr;
...
int Tutor::allow_this_hot_key(int hotKey)
{
    ...
    case 'a': // find artifact on the ground
        if (selectively_detect_flag == 5 || selectively_detect_flag == 0)
            rc = 1;
        break;

    case 'b': // filter bee and then cycle factory
        // filter bee first
        //	if( filter_unit(filter_unit_id, UNIT_BEE) ) forget this in tutorial, who
        // cares ?
        //	{
        //		// empty
        //	}
        //	else
        if (selectively_detect_flag == 5 || selectively_detect_flag == 0 ||
            (selectively_detect_flag == 1 && !(!base_obj_array.is_deleted(allow_detect_obj_recno) &&
                                               base_obj_array[allow_detect_obj_recno]->obj_type != OBJ_FIRM)))
            rc = 1;
        break;
    ...
    case 'c': // search caravan
        if (selectively_detect_flag == 5 || selectively_detect_flag == 0 ||
            (selectively_detect_flag == 1 && !(!base_obj_array.is_deleted(allow_detect_obj_recno) &&
                                               base_obj_array[allow_detect_obj_recno]->obj_type != OBJ_UNIT)))
            rc = 1;
        break;
    ...
    case 'l': // load game
        if (selectively_detect_flag == 5 || selectively_detect_flag == 0 || selectively_detect_flag == 1)
            rc = 1;
        break;
