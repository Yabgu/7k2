...
void CampaignEastWest::plot_a5_create_game()
{
    ...
    if (monsterNation->is_monster())
    {
        ...
        for (int trial = 100; trial > 0; --trial)
        {
            ...
            int dist = misc.points_distance(xLoc, yLoc, playerKing->next_x_loc(), playerKing->next_y_loc());
        ...
        if (maxDist)
        {
            site_array.add_site(farXLoc, farYLoc, SITE_ITEM, itemId, item_res.random_para(itemId, misc.rand()));
        }

        // add teleporter
        if (nearDist < 1000)
        {
            site_array.add_site(nearXLoc, nearYLoc, SITE_ITEM, ITEM_TELEPORT,
                                item_res.random_para(ITEM_TELEPORT, misc.rand()));
        }

        // add presistent item (i.e. ring of life) to monster king, so it will not
        // take the bane
...
void CampaignEastWest::plot_a5_next_day()
{
    ...
    if (!nation_array.is_deleted(plot_enemy_nation_recno))
    {
        ...
        if (monsterNation && monsterNation->king_unit_recno && monsterNation->king_unit_recno == plot_nation_recno2)
        {
            Unit *monsterKing = unit_array[monsterNation->king_unit_recno];
