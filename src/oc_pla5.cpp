...
void CampaignEastWest::plot_a5_create_game() {
  ...
  if (monsterNation->is_monster()) {
    ...
    for (int trial = 100; trial > 0; --trial) {
      ...
      int dist = misc.points_distance(xLoc, yLoc, playerKing->next_x_loc(),
                                      playerKing->next_y_loc());
    ...
    // add monster bane
    if (maxDist) {
      site_array.add_site(farXLoc, farYLoc, SITE_ITEM, itemId,
                          item_res.random_para(itemId, misc.rand()));
    }
