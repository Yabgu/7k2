...
static void write_ai_info(File *filePtr, short *aiInfoArray, short aiInfoCount,
                          short aiInfoSize);
static void read_ai_info(File *filePtr, short **aiInfoArrayPtr,
                         short &aiInfoCount, short &aiInfoSize);
...
int UnitArray::read_file(File *filePtr) {
  ...
  for (i = size(); i > 0; i--) {
    DynArrayB::go(
        i); // since UnitArray has its own go() which will call GroupArray::go()
...
template <typename Visitor> static void visit_baseobj(Visitor *v, BaseObj *o) {
...
template <typename Visitor> static void visit_sprite(Visitor *v, Sprite *s) {
...
template <typename Visitor> static void visit_unitb(Visitor *v, UnitB *b) {
  //--------- profile vars ----------//
...
template <typename Visitor> static void visit_unit(Visitor *v, Unit *u) {
...
// just use Unit::write_file(), otherwise make its own derived copy of
// write_file()
//
int Unit::write_file(File *filePtr) {
  if (!write_with_record_size(filePtr, this, &visit_unit<FileWriterVisitor>,
                              UNIT_RECORD_SIZE))
    return 0;
...
int Unit::read_file(File *filePtr) {
  ...
  if (cur_path) {
    if (cur_path_result_id > 0) {
      ...
      if (!filePtr->file_read(cur_path,
                              cur_path_result_id * sizeof(PathResult)))
        return 0;
    } else {
      cur_path = NULL;
    }
...
int BulletArray::write_file(File *filePtr) {
  ...
  for (i = 1; i <= size(); i++) {
    ...
    if (!bulletPtr) // the bullet is deleted
    {
      ...
      filePtr->file_put_short(
          bulletPtr->sprite_id); // there is a bullet in this record
...
int BulletArray::read_file(File *filePtr) {
  ...
  for (i = 1; i <= size(); i++) {
    DynArrayB::go(i); // since BulletArray has its own go() which will call
                      // GroupArray::go()
...
template <typename Visitor> static void visit_bullet(Visitor *v, Bullet *b) {
  v->skip(4); // virtual table pointer
...
template <typename Visitor> static void visit_place(Visitor *v, Place *p) {
...
template <typename Visitor> static void visit_firm(Visitor *v, Firm *f) {
  v->skip(4); // virtual table pointer
...
static bool read_firm(File *file, Firm *firm) {
  return read_with_record_size(file, firm, &visit_firm<FileReaderVisitor>,
                               FIRM_RECORD_SIZE);
}

static bool write_firm(File *file, Firm *firm) {
  return write_with_record_size(file, firm, &visit_firm<FileWriterVisitor>,
                                FIRM_RECORD_SIZE);
}
...
int FirmArray::write_file(File *filePtr) {
  ...
  for (i = 1; i <= size(); i++) {
    ...
    if (!firmPtr) // the firm is deleted
    {
      ...
      //         {
      //            if( !filePtr->file_write( firmPtr->worker_array,
      //            MAX_WORKER*sizeof(Worker) ) )
      //               return 0;
      //         }
...
int FirmArray::read_file(File *filePtr) {
  ...
  for (i = 1; i <= firmCount; i++) {
    ...
    if (firmId == 0) // the firm has been deleted
    {
      ...
      //         {
      //            firmPtr->worker_array = (Worker*) mem_add(
      //            MAX_WORKER*sizeof(Worker) );
      //
      //            if( !filePtr->file_read( firmPtr->worker_array,
      //            MAX_WORKER*sizeof(Worker) ) )
      //               return 0;
      //         }
  ...
  for (i = size(); i > 0; i--) {
    DynArrayB::go(
        i); // since FirmArray has its own go() which will call GroupArray::go()
...
// just use Firm::write_file(), otherwise make its own derived copy of
// write_file()
//
...
//
// If the derived Firm don't have any special data,
// just use Firm::read_file(), otherwise make its own derived copy of
// read_file()
//
...
template <typename Visitor> static void visit_town(Visitor *v, Town *t) {
  v->skip(4); // virtual table pointer
...
static bool read_town(File *file, Town *town) {
  return read_with_record_size(file, town, &visit_town<FileReaderVisitor>,
                               TOWN_RECORD_SIZE);
}

static bool write_town(File *file, Town *town) {
  return write_with_record_size(file, town, &visit_town<FileWriterVisitor>,
                                TOWN_RECORD_SIZE);
}
...
int TownArray::write_file(File *filePtr) {
  ...
  for (i = 1; i <= size(); i++) {
    ...
    if (!townPtr) // the town is deleted
    {
      ...
      //			#ifdef DEBUG
      //				townPtr->verify_slot_object_id_array();
      //// for debugging only 			#endif
...
int TownArray::read_file(File *filePtr) {
  ...
  for (i = size(); i > 0; i--) {
    DynArrayB::go(
        i); // since TownArray has its own go() which will call GroupArray::go()
...
static void visit_nation_array(Visitor *v, NationArray *na) {
  ...
  for (int n = 0; n < MAX_NATION; n++) {
    visit_array<int8_t>(v, na->custom_nation_name_array[n],
                        NationArray::CUSTOM_NAME_LEN + 1);
    visit_array<int8_t>(v, na->custom_king_name_array[n],
                        NationArray::CUSTOM_NAME_LEN + 1);
  }
}
...
int NationArray::write_file(File *filePtr) {
  ...
  for (i = 1; i <= size(); i++) {
    ...
    if (!nationPtr) // the nation is deleted
    {
      ...
      filePtr->file_put_short(
          nationPtr->nation_class_id); // there is a nation in this record
...
int NationArray::read_file(File *filePtr) {
  ...
  for (i = size(); i > 0; i--) {
    DynArrayB::go(i); // since NationArray has its own go() which will call
                      // GroupArray::go()
...
template <typename Visitor> static void visit_nation(Visitor *v, Nation *nat) {
  v->skip(4); // virtual table pointer
...
int Nation::write_file(File *filePtr) {
  ...
  write_ai_info(filePtr, ai_research_array, ai_research_count,
                ai_research_size);
  ...
  // ######## end Gilbert 10/3 ##########//
  write_ai_info(filePtr, ai_mons_train_array, ai_mons_train_count,
                ai_mons_train_size);
  write_ai_info(filePtr, ai_mons_alchemy_array, ai_mons_alchemy_count,
                ai_mons_alchemy_size);
  write_ai_info(filePtr, ai_mons_fort_array, ai_mons_fort_count,
                ai_mons_fort_size);
  write_ai_info(filePtr, ai_mons_animal_array, ai_mons_animal_count,
                ai_mons_animal_size);
  write_ai_info(filePtr, ai_incubator_array, ai_incubator_count,
                ai_incubator_size);
  write_ai_info(filePtr, ai_mons_magic_array, ai_mons_magic_count,
                ai_mons_magic_size);
...
int Nation::read_file(File *filePtr) {
  ...
  if (!read_with_record_size(filePtr, this, &visit_nation<FileReaderVisitor>,
                             NATION_RECORD_SIZE))
    return 0;
  ...
  read_ai_info(filePtr, &ai_research_array, ai_research_count,
               ai_research_size);
  ...
  // ######## end Gilbert 10/3 ########//
  read_ai_info(filePtr, &ai_mons_train_array, ai_mons_train_count,
               ai_mons_train_size);
  read_ai_info(filePtr, &ai_mons_alchemy_array, ai_mons_alchemy_count,
               ai_mons_alchemy_size);
  read_ai_info(filePtr, &ai_mons_fort_array, ai_mons_fort_count,
               ai_mons_fort_size);
  read_ai_info(filePtr, &ai_mons_animal_array, ai_mons_animal_count,
               ai_mons_animal_size);
  read_ai_info(filePtr, &ai_incubator_array, ai_incubator_count,
               ai_incubator_size);
  read_ai_info(filePtr, &ai_mons_magic_array, ai_mons_magic_count,
               ai_mons_magic_size);
...
int TornadoArray::read_file(File *filePtr) {
  ...
  for (i = size(); i > 0; i--) {
    DynArrayB::go(i); // since TornadoArray has its own go() which will call
                      // GroupArray::go()
...
template <typename Visitor> static void visit_tornado(Visitor *v, Tornado *t) {
  v->skip(4); // virtual table pointer
...
int Tornado::write_file(File *filePtr) {
  return write_with_record_size(
      filePtr, this, &visit_tornado<FileWriterVisitor>, TORNADO_RECORD_SIZE);
}
...
//
int SpyArray::read_file(File *filePtr) { return DynArrayB::read_file(filePtr); }
...
int RegionArray::write_file(File *filePtr) {
  ...
  if (!filePtr->file_write(region_info_array,
                           sizeof(RegionInfo) * region_info_count))
    return 0;
  ...
  if (!filePtr->file_write(region_stat_array,
                           sizeof(RegionStat) * region_stat_count))
    return 0;
...
int RegionArray::read_file(File *filePtr) {
  deinit(); // to free memory of region_info_array, region_stat_count, and
            // connect_bits
  ...
  if (region_info_count > 0)
    region_info_array =
        (RegionInfo *)mem_add(sizeof(RegionInfo) * region_info_count);
  else
    region_info_array = NULL;

  if (!filePtr->file_read(region_info_array,
                          sizeof(RegionInfo) * region_info_count))
    return 0;
  ...
  region_stat_array =
      (RegionStat *)mem_add(region_stat_count * sizeof(RegionStat));

  if (!filePtr->file_read(region_stat_array,
                          sizeof(RegionStat) * region_stat_count))
    return 0;
...
int StateArray::read_file(File *filePtr) {
  ...
  if (init_flag) {
    ...
    state_info_array = (StateInfo *)mem_resize(state_info_array,
                                               sizeof(StateInfo) * state_count);
    ...
    if (!objType) // the object has been deleted
    {
      ...
      case OBJ_UNIT:
        ...
        if (add(unit_array[objExternRecno]) != i) {
          err_here(); // check create recno number is as same as that expected
                      // in the save game
          return 0;
        }
        break;
