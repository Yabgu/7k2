...
int Unit::think_aggressive_action() {
  ...
  if (has_pushed_order() ||
      force_move_flag) // can only push once, so if there is already a pushed
                       // order, we cannot create a new order to a new target
    return 0;
  ...
  //-- if this unit is attacking a firm or a town, think about change to attack
  //a unit --//

  if (cur_order.mode ==
      UNIT_ATTACK) // only in attack mode, as if the unit is still moving the
                   // target may be far away from the current position
  //	if( cur_action==SPRITE_ATTACK )		// only in attack mode, as if the
  //unit is still moving the target may be far away from the current position
  {
    ...
    if ((is_ai ||
         misc.random(2)) && // only AI unit will do so, human units will keep
                            // attacking the target no matter what it is.
        !base_obj_array.is_deleted(cur_order.para) &&
        !(baseObj = base_obj_array[cur_order.para])
             ->obj_can_attack(
                 0)) // if the target structure can't attack, then take care of
                     // it later, attack the enemy units first
    {
      //--- if the target is town and the town has defenders out, attack it has
      //no effect, so attack the defenders ---//

      if (baseObj->cast_to_Town() &&
          baseObj->cast_to_Town()->town_defender_count > 0) {
        //-- increase the attack scan range so that the chance of hitting
        //defenders is higher --//
...
//								 range to detect enemy as
//opposite to 								 detect a bit each time. (default: 0)
//
int Unit::think_aggressive_attack(int extendedRange, int fullDetect) {
  enum {
    DETECT_INTERVAL_DIVIDER = 3
  }; // it takes 3 days to complete a single detect action

  if (!fullDetect &&
      (info.game_date + sprite_recno) % DETECT_INTERVAL_DIVIDER != 0)
    return 0;
  ...
  for (scanner.restart(); !scanner.is_finish(); scanner.next()) {
    ...
    if ((mobile_type == UNIT_AIR ||
         world.get_loc(targetXLoc, targetYLoc)->region_id == regionId) &&
      ...
      if (areaDistance <= attackRange) {
        ...
        if (targetUnit->cur_action == SPRITE_MOVE &&
            misc.abs_direction_diff(targetUnit->cur_dir & 7,
                                    get_dir(cur_x, cur_y, targetUnit->cur_x,
                                            targetUnit->cur_y)) <= 1) {
          scanner.remove();
  ...
  if (!chosenTarget) {
    ...
    for (scanner.restart(); !scanner.is_finish(); scanner.next()) {
      ...
      if (path_finder.find_path(
              next_x_loc(), next_y_loc(), targetUnit->obj_loc_x1(),
              targetUnit->obj_loc_y1(), targetUnit->obj_loc_x1(),
              targetUnit->obj_loc_y1(), targetUnit->obj_loc_x2(),
              targetUnit->obj_loc_y2(), 1, 0, attackScanRange * 4,
              attackRange)) {
        chosenTarget = targetUnit;
  ...
  if (chosenTarget) {
    if (!has_pushed_order()) // if it already has a pushed order, don't push it
                             // again
      push_order();

    set_original_target(
        chosenTarget->obj_loc_x1(),
        chosenTarget
            ->obj_loc_y1()); // for unit to determine when to stop, we must set
                             // it before calling attack() as attack() will call
                             // validate_attack_target() which will check
                             // original_target_?_loc
  ...
  enum { DETECT_INTERVAL_DIVIDER = 6 };		// it takes 6 days to complete a
single detect action
  ...
  else
  {
          ...
          startPoint = detectInterval * ( (info.game_date-last_attack_date) %
DETECT_INTERVAL_DIVIDER);		//	use
(info.game_date-last_attack_date) so that the unit always look for nearest
enemies first endPoint   = startPoint + detectInterval - 1;

          if( sys.frame_count % DETECT_INTERVAL_DIVIDER ==
DETECT_INTERVAL_DIVIDER-1 )		// if this is last in the series
                  endPoint = totalPoint;
}
  ...
  {
          misc.cal_move_around_a_point(i, attackScanRange, attackScanRange,
xOffset, yOffset);
          ...
          if( unitRecno )
          {
                  ...
                  {
                          if( !has_pushed_order() )		// if it already
has a pushed order, don't push it again push_order();

                          set_original_target(xLoc, yLoc);			//
for unit to determine when to stop, we must set it before calling attack() as
attack() will call validate_attack_target() which will check
original_target_?_loc

                          attack(unitPtr->base_obj_recno, false);

                          if( cur_order.mode != UNIT_ATTACK )		// if
the attack action failed, resume order
                          {
                                  pop_order();
                                  return 0;
                          }
...
int Unit::should_aggressive_attack(int targetObjRecno) {
  ...
  //--- if the attacker and the target are both not independent, only attack
  //when the two are hostile ---//
  ...
  if (nation_recno && targetNationRecno) {
    NationRelation *nationRelation =
        nation_array[nation_recno]->get_relation(targetNationRecno);
  ...
  if (unit_mode != UNIT_MODE_TOWN_DEFENDER &&
    ...
    if (unitPtr &&
        unitPtr->is_civilian() && // don't attack civilian and independent units
                                  // unless they are
    ...
    if (unitPtr->nation_recno == 0) {
      ...
      if (nation_recno) // and this unit is a nation unit
      {
        ...
        if (unitPtr->unit_mode == UNIT_MODE_TOWN_DEFENDER) {
          if (town_array[unitPtr->unit_mode_para]->last_attack_nation_recno ==
              nation_recno)
            rc = 1;
        }

        //--- if this is a rebel, only attack it if we are already hostile with
        //the rebel group ---//

        if (unitPtr->unit_mode == UNIT_MODE_REBEL) {
          if (rebel_array[unitPtr->unit_mode_para]->is_hostile_nation(
                  nation_recno))
            rc = 1;
        }
      }
    ...
    //----- in Defensive or Stand Ground mode, only attack unit standing next to
    //itself ----//

    if (get_behavior_mode() != UNIT_AGGRESSIVE && !unit_mode) {
      // ####### begin Gilbert 31/5 ##########//
      //			int attackDistance = cal_distance(
      //targetObj->obj_loc_x1(), targetObj->obj_loc_y1(),
      //										targetObj->obj_loc_width(),
      //targetObj->obj_loc_height() ); 			if( attackDistance > max_attack_range() )
      //				return 0;
      if (!is_visible() ||
          !targetObj->is_visible()) // avoid invisible unit causes exception in
                                    // area_distance
        return 0;
      if (is_die_after_attack() ||
          area_distance(targetObj) > max_attack_range())
        return 0;
      // ####### end Gilbert 31/5 ##########//
    }
  }
  ...
  return attack_loc_offset_x != 0; // if attack_loc_offset_x == 0, that means no
