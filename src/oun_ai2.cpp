...
int Unit::think_aggressive_action()
{
    ...
    if (has_pushed_order() || force_move_flag) // can only push once, so if there is already a pushed
    ...
    if (cur_order.mode == UNIT_ATTACK) // only in attack mode, as if the unit is still moving the
                                       // target may be far away from the current position
    //	if( cur_action==SPRITE_ATTACK )		// only in attack mode, as if
    // the
    // unit is still moving the target may be far away from the current position
    {
        ...
        if ((is_ai || misc.random(2)) && // only AI unit will do so, human units will keep
                                         // attacking the target no matter what it is.
            !base_obj_array.is_deleted(cur_order.para) &&
            !(baseObj = base_obj_array[cur_order.para])
                 ->obj_can_attack(0)) // if the target structure can't attack, then take care of
...
//								 range to detect enemy
// as
// opposite to 								 detect a bit each time.
// (default: 0)
//
int Unit::think_aggressive_attack(int extendedRange, int fullDetect)
{
    ...
    for (scanner.restart(); !scanner.is_finish(); scanner.next())
    {
        ...
        if ((mobile_type == UNIT_AIR || world.get_loc(targetXLoc, targetYLoc)->region_id == regionId) &&
            targetUnit->nation_recno != nation_recno && should_aggressive_attack(targetUnit->base_obj_recno) &&
    ...
    if (!chosenTarget)
    {
        ...
        for (scanner.restart(); !scanner.is_finish(); scanner.next())
        {
            ...
            if (path_finder.find_path(next_x_loc(), next_y_loc(), targetUnit->obj_loc_x1(), targetUnit->obj_loc_y1(),
                                      targetUnit->obj_loc_x1(), targetUnit->obj_loc_y1(), targetUnit->obj_loc_x2(),
                                      targetUnit->obj_loc_y2(), 1, 0, attackScanRange * 4, attackRange))
            {
                chosenTarget = targetUnit;
    ...
    if (chosenTarget)
    {
        ...
        set_original_target(chosenTarget->obj_loc_x1(),
                            chosenTarget->obj_loc_y1()); // for unit to determine when to stop, we must set
  ...
  has a pushed order, don't push it again push_order();
                            ...
                            set_original_target(xLoc, yLoc); // for unit to
  determine when to stop, we must set it before calling attack() as attack() will
  call validate_attack_target() which will check original_target_?_loc
...
int Unit::should_aggressive_attack(int targetObjRecno)
{
    ...
    if (unit_mode != UNIT_MODE_TOWN_DEFENDER && unit_mode != UNIT_MODE_CAMP_DEFENDER)
    {
        ...
        if (unitPtr && unitPtr->is_civilian() && // don't attack civilian and independent units
                                                 // unless they are
            unitPtr->unit_mode != UNIT_MODE_TOWN_DEFENDER && unitPtr->unit_mode != UNIT_MODE_REBEL)
        {
            return 0;
        }
        ...
        if (get_behavior_mode() != UNIT_AGGRESSIVE && !unit_mode)
        {
            ...
            // targetObj->obj_loc_height() ); 			if( attackDistance
            // > max_attack_range() )
            //				return 0;
            if (!is_visible() || !targetObj->is_visible()) // avoid invisible unit causes exception in
                                                           // area_distance
                return 0;
    ...
    if (targetObj->obj_type == OBJ_UNIT)
    {
        ...
        if (spy_recno) // if this unit is our spy, don't attack own units
        {
            ...
            if (spyPtr->true_nation_recno == targetUnitPtr->nation_recno && spyPtr->notify_cloaked_nation_flag == 0)
            {
                return 0;
            }
        }
