...
//#define HELP_INACTIVE_TIME  ((float) 0.8)		// when the mouse is
//inactive for one second, display help

//##### begin trevor 10-12 #####//
// #define HELP_INACTIVE_TIME  ((float) 1.2)		// when the mouse is
// inactive for one second, display help
//##### end trevor 10-12 #####//
//##### begin Gilber 30/6 #####//
#define HELP_INACTIVE_TIME                                                     \
  1500 // when the mouse is inactive for one second, display help
//##### end Gilbert 30/6 #####//

enum {
  HELP_SCR_BUF_WIDTH = 400,  // 400
...
enum { X_MARGIN = 10, Y_MARGIN = 6 };

enum { MSG_WIN_WIDTH = 390, MSG_HEAD_HEIGHT = 16 };
...
void Help::load(char *helpFileName) {
  ...
  if (dataSize > help_text_buf_size) {
    help_text_buf = mem_resize(help_text_buf,
  ...
  int readLen,
      totalReadLen = 0; // total length of text read into the help_text_buf
  int loopCount = 0;
  char *tokenStr;
  ...
  while (!fileTxt.is_eof()) {
    ...
    // ##### begin Gilbert 9/9 ######//
    iptr->help_code[0] =
        '\0'; // to identitfy search help text by help_code or coordinate
    // ##### end Gilbert 9/9 ######//
    ...
    //------ if it's a help area position ------//
      ...
      iptr->monster_human_interface =
          (short)fileTxt.get_num(); // 0 when display for both interfaces
    ...
    readLen =
        fileTxt.read_paragraph(textPtr, help_text_buf_size - totalReadLen);
...
void Help::disp() {
  ...
  if (!should_disp()) {
    help_code[0] = '\0'; // reset it everytime after displaying, if the mouse is
                         // still on the button, help_code will be set again.
  ...
  //-------- other interface help ---------//
    ...
    case MODE_ID_800x600x16:
      ...
      for (; i < last_help_by_area; i++, helpInfo++) {
        if (!helpInfo->help_code[0] &&
            mouse.in_area(helpInfo->area800x600_x1, helpInfo->area800x600_y1,
    ...
    case MODE_ID_1024x768x16:
      ...
      for (; i < last_help_by_area; i++, helpInfo++) {
        if (!helpInfo->help_code[0] &&
            mouse.in_area(helpInfo->area1024x768_x1, helpInfo->area1024x768_y1,
                          helpInfo->area1024x768_x2,
                          helpInfo->area1024x768_y2) &&
            ((helpInfo->monster_human_interface == 1 && config.race_id < 0) ||
             (helpInfo->monster_human_interface == 2 && config.race_id > 0) ||
             (helpInfo->monster_human_interface == 0))) {
          break;
  ...
  if (help_code[0] && helpInfo) {
    disp_help(help_x, help_y, helpInfo->help_title, helpInfo->help_text_ptr);
...
//
void Help::disp_help(int centerX, int centerY, char *helpTitle,
                     char *helpDetail) {
  ...
  {
    winWidth =
        font_san.text_width(helpDetail, -1, MSG_WIN_WIDTH - X_MARGIN * 2) +
        X_MARGIN * 2;
    winHeight =
        Y_MARGIN * 2 + font_san.height() + 11 +
        font_san.text_height(MSG_LINE_SPACE); // text_width() must be called
                                              // before calling text_height()

    int titleWidth =
        font_san.text_width(helpTitle, -1, MSG_WIN_WIDTH - X_MARGIN * 2) +
        X_MARGIN * 2;
  ...
  //--- if the text is bigger than one text box can hold, use a scrollable text
  //box ---//
  ...
  long_save_buf.save_scr(x1, y1, x2,
                         y2); // save the screen to the private buffer in Help
  ...
  //	vga_front.bar( x1, y2-1, x2, y2, HELP_BOX_COLOR );        // Bottom
  //border 	vga_front.bar( x1, y1, x1+1, y2, HELP_BOX_COLOR );        // Left
  //border 	vga_front.bar( x2-1, y1, x2, y2, HELP_BOX_COLOR );        // Right
  //border
  ...
  if (dispHelpDetail) {
    ...
    // vga_front.bar( x1, y, x2, y+1, HELP_BOX_COLOR );  // line between
    // description and help text
    vga_front.bar(x1, y, x2 - 3, y + 1,
                  185); // line between description and help text

    font_san.put_paragraph(x1 + X_MARGIN, y + 4, x2 - X_MARGIN, y2 - Y_MARGIN,
                           helpDetail, MSG_LINE_SPACE);
  }

  //	if( sys.debug_session )
  sys.blt_virtual_buf();

  //--- in a single player game, pause the game when a help message is
  //disp_helplayed ---//
...
//
void Help::set_unit_help(int unitId, int rankId, int x1, int y1, int x2,
                         int y2) {
  if (!mouse.in_area(x1, y1, x2, y2))
    return;
...
//
void Help::set_custom_help(int x1, int y1, int x2, int y2,
                           const char *helpTitle, const char *helpDetail) {
  if (!mouse.in_area(x1, y1, x2, y2))
    return;
...
void Help::disp_short_help(VgaBuf *vgaBuf) {
  ...
  if (titleStr) {
    ...
    (vgaBuf->is_front ? short_front_buf : short_back_buf)
        .save_scr(x1, y1, x2, y2, vgaBuf);

    if (config.help_mode >= BRIEF_HELP) {
      // for safety, still capture the screen even though short help is not
      // displayed may cause problem when changing help_mode
...
//--------- Begin of function HelpSaveScreen::~HelpSaveScreen --------//
//
HelpSaveScreen::~HelpSaveScreen() { deinit(); }
//--------- End of function HelpSaveScreen::~HelpSaveScreen --------//
...
void HelpSaveScreen::rest_scr(VgaBuf *vgaBuf, int keepUnclear) {
  ...
  err_when(save_scr_x1 > save_scr_x2 || save_scr_y1 > save_scr_y2 ||
           save_scr_x1 < 0 || save_scr_y1 < 0 || save_scr_x2 >= VGA_WIDTH ||
           save_scr_y2 >= VGA_HEIGHT);
...
void HelpSaveScreen::hide_area(int x1, int y1, int x2, int y2) {
  ...
  if (!hiding_flag && misc.is_touch(x1, y1, x2, y2, save_scr_x1, save_scr_y1,
                                    save_scr_x2, save_scr_y2)) {
    // don't handle mouse cursor
    vga.active_buf->put_bitmapW_fast(save_scr_x1, save_scr_y1,
