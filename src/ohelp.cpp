...
void Help::disp()
{
    ...
    if (help_code[0])
    {
        ...
        {
            if (helpInfo->help_code[0] == help_code[0] && strcmp(helpInfo->help_code, help_code) == 0)
            {
                break;
            }
        }
    ...
    else
    {
        ...
        case MODE_ID_800x600x16:
            ...
            for (; i < last_help_by_area; i++, helpInfo++)
            {
                if (!helpInfo->help_code[0] &&
                    mouse.in_area(helpInfo->area800x600_x1, helpInfo->area800x600_y1, helpInfo->area800x600_x2,
                                  helpInfo->area800x600_y2) &&
        ...
        case MODE_ID_1024x768x16:
            ...
            {
                if (!helpInfo->help_code[0] &&
                    mouse.in_area(helpInfo->area1024x768_x1, helpInfo->area1024x768_y1, helpInfo->area1024x768_x2,
    ...
    else if (helpInfo)
    {
        ...
        case MODE_ID_800x600x16:
            disp_help((helpInfo->area800x600_x1 + helpInfo->area800x600_x2) / 2,
                      (helpInfo->area800x600_y1 + helpInfo->area800x600_y2) / 2, helpInfo->help_title,
                      helpInfo->help_text_ptr);
            break;

        case MODE_ID_1024x768x16:
            disp_help((helpInfo->area1024x768_x1 + helpInfo->area1024x768_x2) / 2,
                      (helpInfo->area1024x768_y1 + helpInfo->area1024x768_y2) / 2, helpInfo->help_title,
                      helpInfo->help_text_ptr);
            break;
...
void Help::disp_help(int centerX, int centerY, char *helpTitle, char *helpDetail)
{
        ...
        config.help_mode == DETAIL_HELP) // Detailed Help
    {
        ...
        winHeight = Y_MARGIN * 2 + font_san.height() + 11 +
    ...
    // border 	vga_front.bar( x1, y1, x1+1, y2, HELP_BOX_COLOR );        //
    // Left border 	vga_front.bar( x2-1, y1, x2, y2, HELP_BOX_COLOR ); //
    // Right border
...
int Help::should_disp()
{
    ...
    // ####### begin Gilbert 13/7 #########//
    if (last_mouse_x == mouse.cur_x && last_mouse_y == mouse.cur_y && !mouse.left_press &&
        !mouse.right_press) // && !mouse.any_click(2) )
    {
...
void HelpSaveScreen::rest_scr(VgaBuf *vgaBuf, int keepUnclear)
{
    ...
    err_when(save_scr_x1 > save_scr_x2 || save_scr_y1 > save_scr_y2 || save_scr_x1 < 0 || save_scr_y1 < 0 ||
             save_scr_x2 >= VGA_WIDTH || save_scr_y2 >= VGA_HEIGHT);
...
void HelpSaveScreen::hide_area(int x1, int y1, int x2, int y2)
{
    ...
    if (!hiding_flag && misc.is_touch(x1, y1, x2, y2, save_scr_x1, save_scr_y1, save_scr_x2, save_scr_y2))
    {
...
void HelpSaveScreen::show_area()
{
    ...
    {
        vga_front.read_bitmapW(save_scr_x1, save_scr_y1, save_scr_x2, save_scr_y2, save_scr_buf->bitmap_ptr());
        hiding_flag = 0;
    }
    --hide_area_count;
