...
void UnitCaravan::disp_info(int refreshFlag)
{
    ...
    //			unit_array.mp_pre_selected_caravan_recno==sprite_recno) //
    // is
    // selected 			return;
    //	}
    ...
    if (refreshFlag == INFO_REPAINT)
    {
        ...
        button_auto_trade_group[0].create(INFO_X1 + BUTTON_DISTANCE + 13, INFO_Y1 + 235,
                                          INFO_X1 + BUTTON_DISTANCE + 56 - 1, INFO_Y1 + 280, i_disp_auto_trade_button,
                                          ButtonCustomPara(this, 0), 0);

        // auto button
        button_auto_trade_group[1].create(INFO_X1 + 13, INFO_Y1 + 235, INFO_X1 + 56 - 1, INFO_Y1 + 280,
...
void UnitCaravan::disp_goods(int dispY1, int refreshFlag)
{
    for (int productId = 1; productId <= MAX_PRODUCT; ++productId)
    {
        ...
        font_whbl.right_put(x + BUTTON_ACTION_WIDTH - 3, y, misc.format(product_raw_qty_array[productId - 1]));
    }
    ...
    for (int rawId = 1; rawId <= MAX_RAW; ++rawId)
    {
        ...
        font_whbl.right_put(x + BUTTON_ACTION_WIDTH - 3, y, misc.format(raw_qty_array[rawId - 1]));
    }
}
        ...
        if (refreshFlag == INFO_REPAINT)
        {
            ...
            for (j = 0; j < MAX_GOODS_SELECT_BUTTON; ++j)
                button_select_array[i][j].create(INFO_X1 + 17 + carv_gdd_x[j], y + 34,
                                                 INFO_X1 + 17 + carv_gdd_x[j + 1] - 1, y + 61,
...
void UnitCaravan::detect_stop()
{
    ...
    for (i = 0; i < MAX_STOP_FOR_CARAVAN; i++)
    {
        if (is_own() && button_set_stop[i].detect())
            power.issue_command(COMMAND_SET_CARAVAN_STOP, sprite_recno,
        ...
        for (int b = 0; b < MAX_GOODS_SELECT_BUTTON; ++b)
        {
            ...
            {
                se_ctrl.immediate_sound(button_select_array[i][b].elastic_flag || button_select_array[i][b].pushed_flag
                                            ? (char *)"TURN_ON"
                                            : (char *)"TURN_OFF");
...
static void i_button_go_stop(ButtonCustom *button, int repaintBody)
{
    ...
    vga.active_buf->put_bitmap_remap(
        ...
        image_button.read(button->pushed_flag
                              ? (char *)"CARA-STD"
                              : mouse.in_area(button->x1, button->y1, button->x2, button->y2) ? (char *)"CARA-STH"
                                                                                              : (char *)"CARA-STU"),
    ...
    // put destination marker
    {
        ...
        {
            vga.active_buf->put_bitmap_trans_decompress(button->x1 + 18, button->y1 + 6, image_button.read("TRI-R"));
        }
    }
    ...
    if (firmPtr)
    {
        font_snds.put_paragraph(button->x1 + 34, button->y1 + 5 + (button->pushed_flag ? 1 : 0), button->x2 - 5,
                                button->y2 - 5 + (button->pushed_flag ? 1 : 0), firmPtr->firm_name());
    }
}
...
static void i_disp_caravan_select_button(ButtonCustom *button, int repaintBody)
{
    ...
    char *bitmapPtr = image_button.read(
        button->enable_flag ? (button->pushed_flag ? (char *)"CARA-GDD" : (char *)"CARA-GDL") : (char *)"CARA-GDL");
    vga.active_buf->put_bitmap_area_trans(button->x1 - carv_gdd_x[j], button->y1, bitmapPtr, carv_gdd_x[j], 0,
...
static void i_disp_auto_trade_button(ButtonCustom *button, int repaintBody)
{
    ...
    font_whbl.center_put_paragraph(button->x1, button->y1, button->x2, button->y2,
                                   button->custom_para.value ? text_unit.str_caravan_auto()
    ...
    {
        //		vga.active_buf->rect( button->x1, button->y1, button->x2,
        // button->y2,
        // 1, VGA_LIGHT_BLUE );
        ...
        vga.active_buf->put_bitmap_trans_decompress(
            button->x1 + (button->x2 - button->x1 - ((Bitmap *)triangleUp)->get_width()) / 2,
