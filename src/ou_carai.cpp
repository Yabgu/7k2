...
static ButtonCustom button_select_array[MAX_STOP_FOR_CARAVAN]
...
static int carv_gdd_x[MAX_PRODUCT + MAX_RAW + 1] = {0,   26,  47, 77,
                                                    101, 127, 151};
...
void UnitCaravan::disp_info(int refreshFlag) {
  ...
  //			unit_array.mp_pre_selected_caravan_recno==sprite_recno) // is
  //selected 			return;
  //	}
  ...
  if (refreshFlag == INFO_REPAINT) {
    button_duplicate_caravan.create(INFO_X1 + 13, INFO_Y1 + 281, 'A',
                                    "CARACOPY");

    // manual button
    button_auto_trade_group[0].create(
        INFO_X1 + BUTTON_DISTANCE + 13, INFO_Y1 + 235,
        INFO_X1 + BUTTON_DISTANCE + 56 - 1, INFO_Y1 + 280,
        i_disp_auto_trade_button, ButtonCustomPara(this, 0), 0);

    // auto button
    button_auto_trade_group[1].create(
        INFO_X1 + 13, INFO_Y1 + 235, INFO_X1 + 56 - 1, INFO_Y1 + 280,
    ...
    if (rc >= 0) {
      if (remote.is_enable()) {
        ...
        short *shortPtr = (short *)remote.new_send_queue_msg(
            MSG_U_CARA_SET_AUTO, 2 * sizeof(short));
        *shortPtr = sprite_recno;
        ...
        for (i = 0; i < MAX_STOP_FOR_CARAVAN; ++i)
          if (stop_array[i].firm_recno && stop_array[i].firm_id == FIRM_MARKET)
            set_stop_pick_up(i + 1, rc ? AUTO_PICK_UP : MANUAL_PICK_UP,
                             COMMAND_AUTO);
      }
    }
...
void UnitCaravan::disp_goods(int dispY1, int refreshFlag) {
  for (int productId = 1; productId <= MAX_PRODUCT; ++productId) {
    ...
    //		vga.active_buf->put_bitmap_trans( x+2, y,
    //raw_res.small_product_icon(productId) );
    font_whbl.right_put(x + BUTTON_ACTION_WIDTH - 3, y,
                        misc.format(product_raw_qty_array[productId - 1]));
  }
  ...
  for (int rawId = 1; rawId <= MAX_RAW; ++rawId) {
    ...
    //		vga.active_buf->put_bitmap_trans( x+2, y,
    //raw_res.small_raw_icon(rawId) );
    font_whbl.right_put(x + BUTTON_ACTION_WIDTH - 3, y,
                        misc.format(raw_qty_array[rawId - 1]));
  }
}
...
void UnitCaravan::disp_stop(int dispY1, int refreshFlag) {
  ...
  for (i = 0; i < MAX_STOP_FOR_CARAVAN; ++i, y += 70) {
    ...
    if (refreshFlag == INFO_REPAINT) {
      button_go_stop[i].create(INFO_X1 + 17, y + 2, INFO_X1 + 167, y + 35,
                               i_button_go_stop,
                               ButtonCustomPara(this, tradeStop->firm_recno));
      button_go_stop[i].set_help_code("CGOSTOP");

      button_set_stop[i].create(INFO_X1 + 169, y + 2, INFO_X1 + 215, y + 20,
                                i_button_set_stop, ButtonCustomPara(NULL, 0));
      button_set_stop[i].set_help_code("CSETSTOP");

      button_cancel_stop[i].create(INFO_X1 + 169, y + 21, INFO_X1 + 215, y + 39,
                                   i_button_cancel_stop,
                                   ButtonCustomPara(NULL, 0));
      button_cancel_stop[i].set_help_code("CDELSTOP");

      button_reset_stop[i].create(INFO_X1 + 169, y + 42, INFO_X1 + 215, y + 60,
                                  i_button_reset_stop,
                                  ButtonCustomPara(NULL, 0));
      button_reset_stop[i].set_help_code("CCLRSEL");

      for (j = 0; j < MAX_GOODS_SELECT_BUTTON; ++j)
        button_select_array[i][j].create(INFO_X1 + 17 + carv_gdd_x[j], y + 34,
                                         INFO_X1 + 17 + carv_gdd_x[j + 1] - 1,
                                         y + 61, i_disp_caravan_select_button,
                                         ButtonCustomPara(NULL, j), 0);
    }

    button_go_stop[i].custom_para =
        ButtonCustomPara(this, tradeStop->firm_recno);
    ...
    for (j = 0; j < MAX_GOODS_SELECT_BUTTON; ++j) {
      ...
      if (firmPtr) {
        if (firmPtr->cast_to_FirmMarket()) {
          ...
          if (rawId) {
            marketGoods =
                firmPtr->cast_to_FirmMarket()->market_raw_array(rawId);
            err_when(marketGoods && marketGoods->raw_id != rawId);
          } else if (productId) {
            marketGoods =
                firmPtr->cast_to_FirmMarket()->market_product_array(productId);
          ...
          if (productId &&
              firmPtr->cast_to_FirmFactory()->product_raw_id == productId) {
            stock = (int)firmPtr->cast_to_FirmFactory()->stock_qty;
          }
          // else if( rawId && firmPtr->cast_to_FirmFactory()->product_raw_id ==
          // rawId )
          //{
...
void UnitCaravan::detect_stop() {
  ...
  for (i = 0; i < MAX_STOP_FOR_CARAVAN; i++) {
    if (is_own() && button_set_stop[i].detect())
      power.issue_command(
          COMMAND_SET_CARAVAN_STOP, sprite_recno,
          i + 1); // i+1 - stop id., passed as a parameter of the command
    ...
    for (int b = 0; b < MAX_GOODS_SELECT_BUTTON; ++b) {
      if (button_select_array[i][b].detect()) {
        se_ctrl.immediate_sound(button_select_array[i][b].elastic_flag ||
                                        button_select_array[i][b].pushed_flag
                                    ? (char *)"TURN_ON"
                                    : (char *)"TURN_OFF");

        set_stop_pick_up(i + 1, b + FIRST_GOODS_SELECT_BUTTON,
                         COMMAND_PLAYER); // b = 1 - MAX_PICK_UP_GOODS
      }
    }
  }
}
...
static void i_button_go_stop(ButtonCustom *button, int repaintBody) {
  ...
  vga.active_buf->put_bitmap_remap(
      button->x1, button->y1,
      ...
      image_button.read(
          button->pushed_flag
              ? (char *)"CARA-STD"
              : mouse.in_area(button->x1, button->y1, button->x2, button->y2)
                    ? (char *)"CARA-STH"
                    : (char *)"CARA-STU"),
  ...
  // put destination marker
  {
    ...
    if (unitPtr->dest_stop_id > 0 &&
        unitPtr->dest_stop_id <= MAX_STOP_FOR_CARAVAN &&
        unitPtr->stop_array[unitPtr->dest_stop_id - 1].firm_recno ==
            button->custom_para.value) {
      vga.active_buf->put_bitmap_trans_decompress(
          button->x1 + 18, button->y1 + 6, image_button.read("TRI-R"));
    }
  }
  ...
  if (firmPtr) {
    font_snds.put_paragraph(
        button->x1 + 34, button->y1 + 5 + (button->pushed_flag ? 1 : 0),
...
//
static void i_disp_caravan_select_button(ButtonCustom *button,
                                         int repaintBody) {
  int j = button->custom_para.value;
  //	char *bitmapPtr = image_button.read( button->enable_flag ?
  //(button->pushed_flag ? "CARA-GDD" : "CARA-GDU") : "CARA-GDL" );
  char *bitmapPtr = image_button.read(
      button->enable_flag
          ? (button->pushed_flag ? (char *)"CARA-GDD" : (char *)"CARA-GDL")
          : (char *)"CARA-GDL");
  vga.active_buf->put_bitmap_area_trans(
      button->x1 - carv_gdd_x[j], button->y1, bitmapPtr, carv_gdd_x[j], 0,
      carv_gdd_x[j + 1] - 1, ((Bitmap *)bitmapPtr)->get_height() - 1);
}
...
static void i_disp_auto_trade_button(ButtonCustom *button, int repaintBody) {
  ...
  font_whbl.center_put_paragraph(button->x1, button->y1, button->x2, button->y2,
                                 button->custom_para.value
                                     ? text_unit.str_caravan_auto()
                                     : text_unit.str_caravan_manual(),
                                 DEFAULT_LINE_SPACE, 0, 0);

  // mark pushed

  if (button->pushed_flag) {
    //		vga.active_buf->rect( button->x1, button->y1, button->x2, button->y2,
    //1, VGA_LIGHT_BLUE );
    ...
    vga.active_buf->put_bitmap_trans_decompress(
        button->x1 +
            (button->x2 - button->x1 - ((Bitmap *)triangleUp)->get_width()) / 2,
