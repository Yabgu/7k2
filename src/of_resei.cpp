...
// ##### end Gilbert 7/10 ######//
static short edit_progress_x1, edit_progress_y1, edit_progress_x2,
    edit_progress_y2, edit_progress_enable;
...
void FirmResearch::put_info(int refreshFlag) {
  if (refreshFlag == INFO_REPAINT)
    ...
    if (last_menu_mode !=
        research_menu_mode) // if changing menu mode pass repaint to sub-menu
    {
...
void FirmResearch::disp_research_class_menu(int refreshFlag) {
  ...
  for (classTechId = FIRST_TECH_CLASS_RESEARCH;
       classTechId <= LAST_TECH_CLASS_RESEARCH; classTechId++) {
    int b = classTechId - FIRST_TECH_CLASS_RESEARCH;
    if (tech_res.tech_class(classTechId)
            ->researchable_tech_count(nation_array.player_recno) == 0)
      continue;

    //-----------------------------------------//

    if (refreshFlag == INFO_REPAINT) {
      //			button_research_class_array[b].create( x,
      //y+(b+1)*20, x +187, y+(b+1)*20+17, 				i_disp_research_class_button,
      //ButtonCustomPara( this, classTechId) );
      button_research_class_array[b].create(
          x, y + b * 20, x + 187, y + b * 20 + 17, i_disp_research_class_button,
          ButtonCustomPara(this, classTechId));
    }
...
void FirmResearch::detect_research_class_menu() {
  ...
  for (int i = FIRST_TECH_CLASS_RESEARCH; i <= LAST_TECH_CLASS_RESEARCH; ++i) {
    if (tech_res.tech_class(i)->researchable_tech_count(
    ...
    if (button_research_class_array[i - FIRST_TECH_CLASS_RESEARCH].detect()) {
      ...
      research_list_end =
          research_list_start + tech_res.tech_class(i)->tech_count;
...
void FirmResearch::disp_research_menu(int refreshFlag) {
  ...
  for (techId = research_list_start; techId < research_list_end; techId++) {
    if (refreshFlag == INFO_REPAINT) {
      // ##### begin Gilbert 5/11 ######//
      button_research_array[added_count].create(
          x, y + added_count * ySpacing, x + 187,
          y + added_count * ySpacing + 17, i_disp_research_button,
          ButtonCustomPara(this, techId));
      // ##### end Gilbert 5/11 ######//
    }

    if ((button_research_array[added_count].enable_flag =
             tech_res[techId]->can_research(nation_recno))) {
  ...
  if (added_count == 0) {
    ...
    //); 		str = "class has been researched !"; 		font_snds.center_put( INFO_X1+7,
    //INFO_Y1+45, INFO_X2, INFO_Y1+62, str );

    font_snds.center_put_paragraph(
        INFO_X1 + 17, INFO_Y1 + 11, INFO_X2 - 12, INFO_Y1 + 62,
...
void i_disp_research_button(ButtonCustom *button, int repaintBody) {
  ...
  int researchVersion = techInfo->get_nation_tech_level(firmPtr->nation_recno) +
                        1; // research the next non-researched version
  ...
  if (!button->enable_flag) {
    vga.active_buf->put_bitmap(button->x1, button->y1,
                               image_icon.read("BUILDUP2"));
    font_cara.center_put(button->x1, button->y1, button->x2, button->y2, str);
    vga.active_buf->bar_alpha(button->x1, button->y1, button->x2, button->y2, 1,
                              0);
  } else if (button->pushed_flag) {
    vga.active_buf->put_bitmap(button->x1, button->y1,
                               image_icon.read("BUILDDW2"));
    //		vga_util.blt_buf(button->x1, button->y1, button->x2, button->y2,
    //0);
    font_cara.center_put(button->x1 + 1, button->y1 + 1, button->x2 + 1,
                         button->y2 + 1, str);
  } else if (mouse.in_area(button->x1, button->y1, button->x2, button->y2)) {
    vga.active_buf->put_bitmap(button->x1, button->y1,
                               image_icon.read("BUILDSE2"));
    font_cara.center_put(button->x1, button->y1, button->x2, button->y2, str);
  } else {
    vga.active_buf->put_bitmap(button->x1, button->y1,
                               image_icon.read("BUILDUP2"));
...
void FirmResearch::disp_research_info(int dispY1, int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    int researchVersion = techInfo->get_nation_tech_level(nation_recno) +
                          1; // research the next non-researched version
  ...
  vga_front.indicator(0, x - 2, y + 21,
                      nation_array[nation_recno]->research_progress(),
                      (float)100, VGA_GRAY);
}
...
void FirmResearch::disp_firm_info(int dispY1, int refreshFlag) {
  ...
  if (!techId) {
    ...
    //); 		str = "currently being"; 		font_snds.center_put( INFO_X1+7, INFO_Y1+60,
    //INFO_X2, INFO_Y1+77, str ); 		str = "researched."; 		font_snds.center_put(
    //INFO_X1+7, INFO_Y1+89, INFO_X2, INFO_Y1+106, str );

    font_snds.center_put_paragraph(INFO_X1 + 45, INFO_Y1 + 31, INFO_X2 - 40,
                                   INFO_Y1 + 106, text_firm.str_no_research(),
                                   16, 0, 0);
  } else {
    //-------- display the icon of the researching item ---------//
    ...
    str += " %";
    font_snds.center_put(INFO_X1 + 7, INFO_Y1 + 97 + 18, INFO_X2,
                         INFO_Y1 + 114 + 18, str);
    ...
    //); 		if (ratio <11) 			vga.active_buf->put_bitmap_trans( INFO_X1 +62, INFO_Y1
    //+59, image_spict.read("MTR_10")); 		else 		if (ratio <40)
    //			vga.active_buf->put_bitmap_trans( INFO_X1 +62, INFO_Y1 +59,
    //image_spict.read("MTR_39")); 		else 			vga.active_buf->put_bitmap_trans(
    //INFO_X1 +62, INFO_Y1 +59, image_spict.read("MTR_40"));

    //		vga.active_buf->read_bitmapW( INFO_X1 +62, INFO_Y1 +59, INFO_X1 +75
    //+size, INFO_Y1 +90, hitPointBitmap ); 		vga.active_buf->put_bitmap_trans(
    //INFO_X1 +62, INFO_Y1 +59, image_spict.read("MTR_00"));
    ...
    if (size > 0) {
      ...
      vga.active_buf->put_bitmap_area_trans(
          INFO_X1 + 62, INFO_Y1 + 59, barBitmap, 0, 0, 13 + size,
          ((Bitmap *)barBitmap)->get_height() - 1);
    }
  }
  ...
  if (is_own()) {
    if (refreshFlag == INFO_REPAINT) {
      if (!is_monster())
        button_select_research.create(INFO_X1 + 13, INFO_Y1 + 281, 'A',
                                      "RESEARCH");
      else
        button_select_research.create(INFO_X1 + 13, INFO_Y1 + 281, 'A',
                                      "F_RESE");
...
void FirmResearch::disp_edit_mode(int &refreshFlag, int &y, int dispBG) {
  ...
  edit_progress_enable =
      nation_recno && nation_array[nation_recno]->research_tech_id != 0;
  if (edit_progress_enable) {
    ...
    x = edit_progress_x2 = font_blue.put(
        (edit_progress_x1 = x), (edit_progress_y1 = y),
        misc.format((int)nation_array[nation_recno]->research_progress(), 3), 0,
        x2);
...
int FirmResearch::detect_edit_mode() {
  ...
  if (edit_progress_enable && nation_recno) {
    ...
    if (mouse.any_click(edit_progress_x1, edit_progress_y1, edit_progress_x2,
                        edit_progress_y2, LEFT_BUTTON)) {
