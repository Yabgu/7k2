...
void FirmResearch::disp_research_class_menu(int refreshFlag)
{
    ...
    font_snds.put_paragraph(x, y, INFO_X2 - 6, INFO_Y1 + 52, text_firm.str_select_tech_class());
    ...
    for (classTechId = FIRST_TECH_CLASS_RESEARCH; classTechId <= LAST_TECH_CLASS_RESEARCH; classTechId++)
    {
        ...
        if (refreshFlag == INFO_REPAINT)
        {
            ...
            // y+(b+1)*20, x +187, y+(b+1)*20+17,
            // i_disp_research_class_button, ButtonCustomPara( this, classTechId) );
            button_research_class_array[b].create(x, y + b * 20, x + 187, y + b * 20 + 17, i_disp_research_class_button,
...
void FirmResearch::disp_research_menu(int refreshFlag)
{
    ...
    for (techId = research_list_start; techId < research_list_end; techId++)
    {
        ...
        {
            // ##### begin Gilbert 5/11 ######//
            button_research_array[added_count].create(x, y + added_count * ySpacing, x + 187,
    ...
    if (added_count == 0)
    {
        ...
        // str
        //); 		str = "class has been researched !"; 		font_snds.center_put(
        // INFO_X1+7,
...
void i_disp_research_button(ButtonCustom *button, int repaintBody)
{
    ...
    int researchVersion =
        techInfo->get_nation_tech_level(firmPtr->nation_recno) + 1; // research the next non-researched version
...
void FirmResearch::disp_research_info(int dispY1, int refreshFlag)
{
    ...
    if (refreshFlag == INFO_REPAINT)
    {
        ...
        int researchVersion =
            techInfo->get_nation_tech_level(nation_recno) + 1; // research the next non-researched version
...
void FirmResearch::disp_firm_info(int dispY1, int refreshFlag)
{
    ...
    if (!techId)
    {
        ...
        //		font_snds.center_put( INFO_X1+7, INFO_Y1+31, INFO_X2, INFO_Y1+48,
        // str
        //); 		str = "currently being"; 		font_snds.center_put( INFO_X1+7,
        // INFO_Y1+60,
        // INFO_X2, INFO_Y1+77, str ); 		str = "researched.";
    ...
    else
    {
        ...
        font_snds.center_put(INFO_X1 + 7, INFO_Y1 + 11, INFO_X2, INFO_Y1 + 28, text_firm.str_research_progress());
        ...
        font_snds.center_put(INFO_X1 + 7, INFO_Y1 + 97, INFO_X2, INFO_Y1 + 114, text_firm.str_completion_percent());
        ...
        //); 		if (ratio <11) 			vga.active_buf->put_bitmap_trans( INFO_X1 +62,
        // INFO_Y1
        //+59, image_spict.read("MTR_10")); 		else 		if (ratio
        //<40) 			vga.active_buf->put_bitmap_trans( INFO_X1 +62, INFO_Y1 +59,
        // image_spict.read("MTR_39")); 		else
        // vga.active_buf->put_bitmap_trans( INFO_X1 +62, INFO_Y1 +59,
        // image_spict.read("MTR_40"));

        //		vga.active_buf->read_bitmapW( INFO_X1 +62, INFO_Y1 +59, INFO_X1
        //+75 +size, INFO_Y1 +90, hitPointBitmap );
        // vga.active_buf->put_bitmap_trans(
        // INFO_X1 +62, INFO_Y1 +59, image_spict.read("MTR_00"));
        //		vga.active_buf->put_bitmapW( INFO_X1 +62, INFO_Y1 +59,
        // hitPointBitmap
        ...
        if (size > 0)
        {
            ...
            vga.active_buf->put_bitmap_area_trans(INFO_X1 + 62, INFO_Y1 + 59, barBitmap, 0, 0, 13 + size,
