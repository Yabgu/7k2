...
void GodRes::second_init() {
  ...
  //		}
  //		while( monsterTargetedArray[monsterId-1] == 2 );		// only a
  //maximum of two Greater Beings can have the monster target
  //
...
void GodRes::load_god_info() {
  ...
  for (i = 0; i < god_count; i++) {
    ...
    godInfo->exist_pray_points =
        misc.atoi(godRec->exist_pray_points, godRec->PRAY_POINTS_LEN);
    godInfo->power_pray_points =
        misc.atoi(godRec->power_pray_points, godRec->PRAY_POINTS_LEN);

    godInfo->can_cast_power = godRec->can_cast_power == '1';
    godInfo->cast_power_range =
        misc.atoi(godRec->cast_power_range, godRec->CAST_POWER_RANGE_LEN);

    godInfo->can_attack = godRec->can_attack == '1';

    godInfo->out_effect_id =
        misc.atoi(godRec->out_effect_id, godRec->OUT_EFFECT_ID_LEN);
    misc.rtrim_fld(godInfo->out_effect_code, godRec->out_effect_code,
                   godRec->OUT_EFFECT_CODE_LEN);
  }
}
...
void GodInfo::enable_know(int nationRecno) {
  ...
  nation_array[nationRecno]->know_base_array[race_id - 1] =
      1; // enable the nation to build the fortress of power
}
...
void GodInfo::disable_know(int nationRecno) {
  ...
  nation_array[nationRecno]->know_base_array[race_id - 1] =
      0; // enable the nation to build the fortress of power
}
//--------- End of function GodInfo::disable_know ---------//

//------- Begin of function GodInfo::invoke -------//
//
// <int> firmRecno  - the firm recno of the seat of power which invokes this
// god. <int> xLoc, yLoc - the x and y location of the god
//
...
short GodInfo::invoke(int firmRecno, int xLoc, int yLoc) {
  ...
  if (!world.locate_space(xLoc, yLoc, xLoc, yLoc, spriteInfo->loc_width,
                          spriteInfo->loc_height, UNIT_AIR))
    return 0;
...
GodInfo *GodRes::operator[](int godId) {
  err_if(godId < 1 || godId > god_count) err_now("GodRes::operator[]");
...
void GodRes::update_prayer_count() {
  ...
  int i;
  for (i = 0; i < god_count; i++) {
    memset(god_info_array[i].prayer_count, 0,
           sizeof(god_info_array[i].prayer_count));
  }
  ...
  for (int townRecno = firm_array.size(); townRecno > 0; --townRecno) {
    ...
    if (firmBasePtr && firmBasePtr->nation_recno) {
      ...
      err_when(firmBasePtr->nation_recno < 1 ||
               firmBasePtr->nation_recno > MAX_NATION);
      god_info_array[i - 1].prayer_count[firmBasePtr->nation_recno - 1] +=
          firmBasePtr->worker_count;
    }
  }
}
