...
void RockRes::load_info() {
  ...
  // Database *dbRock = game_set.open_db(ROCK_DB);	// only one database can be
  // opened at a time, so we read ROCK.DBF first
  ...
  for (i = 0; i < rock_info_count; i++) {
    ...
    rockInfo->first_bitmap_recno =
        misc.atoi(rockRec->first_bitmap_recno, rockRec->RECNO_LEN);
    rockInfo->max_frame = misc.atoi(rockRec->max_frame, rockRec->MAX_FRAME_LEN);

    //	if( rockRec->pal_file_name[0] == ' ' || rockRec->pal_file_name[0] ==
    //'\0' )
    //	{
...
void RockRes::load_bitmap_info() {
  ...
  rockDbName += ".RES";
  Database rockDbObj(rockDbName, 1);
  // Database *dbRock = game_set.open_db(ROCK_BITMAP_DB);	// only one database
  // can be opened at a time, so we read ROCK.DBF first
  Database *dbRock = &rockDbObj;

  rock_bitmap_count = (short)dbRock->rec_count();
  rock_bitmap_array =
      (RockBitmapInfo *)mem_add(sizeof(RockBitmapInfo) * rock_bitmap_count);
  ...
  for (i = 0; i < rock_bitmap_count; i++) {
    ...
    rockBitmapInfo->frame =
        misc.atoi(rockBitmapRec->frame, rockBitmapRec->FRAME_NO_LEN);
    rockBitmapInfo->delay =
        misc.atoi(rockBitmapRec->delay, rockBitmapRec->DELAY_LEN);
    rockBitmapInfo->next_frame =
        misc.atoi(rockBitmapRec->next_frame, rockBitmapRec->FRAME_NO_LEN);
    rockBitmapInfo->alt_next =
        misc.atoi(rockBitmapRec->alt_next, rockBitmapRec->FRAME_NO_LEN);
    rockBitmapInfo->offset_x =
        misc.atoi(rockBitmapRec->offset_x, rockBitmapRec->OFFSET_LEN);
    rockBitmapInfo->offset_y =
        misc.atoi(rockBitmapRec->offset_y, rockBitmapRec->OFFSET_LEN);
    ...
    rockBitmapInfo->offset_x =
        rockBitmapInfo->offset_x + width * ZOOM_LOC_X_WIDTH / 2 +
        height * ZOOM_LOC_Y_WIDTH / 2 - width * LOCATE_WIDTH / 2;
    rockBitmapInfo->offset_y =
        rockBitmapInfo->offset_y + width * ZOOM_LOC_X_HEIGHT / 2 +
...
// <long> path           a random number, related to the probability of choosing
// alt_next
//                       eg. choose_next(..,.., misc.random(x)); prob of using
//                       alt_next is 1/x
...
char RockRes::choose_next(short rockId, char curFrame, long path) {
  ...
  RockBitmapInfo *rockBitmapInfo =
      get_bitmap_info(get_bitmap_recno(rockId, curFrame));

  // -------- validate frame, next_frame and alt_next in rockBitmapInfo -------/
  //	err_when(get_anim_info(get_anim_recno(rockRecno,
  //rockBitmapInfo->next_frame))->frame != rockBitmapInfo->next_frame);
  //	err_when(get_anim_info(get_anim_recno(rockRecno,
  //rockBitmapInfo->alt_next))->frame != rockBitmapInfo->alt_next);
...
void RockRes::draw(short rockId, short xLoc, short yLoc, short curZ,
                   char curFrame) {
  RockInfo *rockInfo = get_rock_info(rockId);
  get_bitmap_info(get_bitmap_recno(rockId, curFrame))
      ->draw(rockInfo, xLoc, yLoc, curZ);
}
...
//
short RockRes::search(const char *rockTypes, short minWidth, short maxWidth,
                      short minHeight, short maxHeight, int animatedFlag,
                      int findFirst, char terrainType) {
  ...
  for (i = 1, rockInfo = rock_info_array; i <= rock_info_count;
       ++i, ++rockInfo) {
    if ((!rockTypes || strchr(rockTypes, rockInfo->rock_type)) &&
        ...
        rockInfo->loc_height >= minHeight &&
        rockInfo->loc_height <= maxHeight &&
...
void RockBitmapInfo::draw(RockInfo *rockInfo, short xLoc, short yLoc,
                          short curZ) {
  world.zoom_matrix->put_bitmap_offset(
      xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, curZ, bitmap_ptr, offset_x,
      offset_y,
