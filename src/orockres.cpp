...
void RockRes::load_info()
{
    ...
    // Database *dbRock = game_set.open_db(ROCK_DB);	// only one database can
    // be opened at a time, so we read ROCK.DBF first
...
void RockRes::load_bitmap_info()
{
    ...
    rockDbName += ".RES";
    Database rockDbObj(rockDbName, 1);
    // Database *dbRock = game_set.open_db(ROCK_BITMAP_DB);	// only one
    // database can be opened at a time, so we read ROCK.DBF first
    Database *dbRock = &rockDbObj;
                      ...
                      int animatedFlag, int findFirst, char terrainType)
{
    ...
    for (i = 1, rockInfo = rock_info_array; i <= rock_info_count; ++i, ++rockInfo)
    {
        if ((!rockTypes || strchr(rockTypes, rockInfo->rock_type)) &&
            (terrainType == 0 || rockInfo->valid_terrain(terrainType)) && rockInfo->loc_width >= minWidth &&
            rockInfo->loc_width <= maxWidth && rockInfo->loc_height >= minHeight && rockInfo->loc_height <= maxHeight &&
...
{
    world.zoom_matrix->put_bitmap_offset(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, curZ, bitmap_ptr, offset_x,
                                         offset_y, rockInfo->palw_ptr ? rockInfo->palw_ptr : vga.default_remap_table, 0,
                                         1);
}
