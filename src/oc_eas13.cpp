...
enum
{
    STAGE_13_EASTERN_NATION_RECNO = 2,
...
int CampaignEastWest::stage_13_create_game()
{
    ...
    stage_13_init_relation(); // must init relationship first so that allied units
...
void CampaignEastWest::stage_13_init_vars()
{
    ...
    game.add_win_condition(E_DEFEAT_KINGDOM, 1,
                           STAGE_13_MONSTER_NATION_RECNO); // the objective is to destroy the enemy
                                                           // kingdom
}
...
int CampaignEastWest::stage_13_create_lair()
{
    ...
    create_lair(cur_monster_nation_recno, STAGE_13_MONSTER_NATION_RECNO, lairAddCount, independentTownAddCount,
                specialFirmCount);
