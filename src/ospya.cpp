...
// static char* 	secret_report_str_array[] = { "Kingdoms", "Towns",
// "Economy", "Trade", "Military", "Technology", "Espionage" }; static char
// secret_view_mode_array[]  = { MODE_NATION, MODE_TOWN, MODE_ECONOMY,
// MODE_TRADE, MODE_MILITARY, MODE_TECH, MODE_SPY }; static char
// secret_view_skill_array[] = { 40, 20, 30, 30, 50, 40, 90 };
#define SECRET_REPORT_COUNT 6
// static char* 	secret_report_str_array[] = { "Kingdoms", "Towns",
// "Economy", "Trade", "Military", "Technology" };
static char secret_view_mode_array[] = {
    MODE_NATION, MODE_TOWN, MODE_ECONOMY, MODE_TRADE, MODE_MILITARY, MODE_TECH};
...
int SpyArray::add_spy(int unitRecno, int spySkill, int trueNation) {
  ...
  //--- spies hold a use right of the name id even though the unit itself will
  //register the usage right of the name already ---//

  race_res[spy.race_id]->use_name_id(
      spy.name_id); // the spy will free it up in deinit(). Keep an additional
                    // right because when a spy is assigned to a town, the
                    // normal program will free up the name id., so we have to
...
int SpyArray::find_town_spy(int townRecno, int spySeq) {
  ...
  for (int i = 1; i <= spyCount; i++) {
    ...
    if (spyPtr->spy_place == SPY_TOWN && spyPtr->spy_place_para == townRecno) {
...
//							   be mobilized.
//
void SpyArray::mobilize_all_spy(int spyPlace, int spyPlacePara,
                                int nationRecno) {
  Spy *spyPtr;
...
void SpyArray::disp_view_secret_menu(int spyRecno, int refreshFlag) {
  ...
  font_whbl.put_paragraph(
      ...
      text_firm.str_spy_steal_info()); // "Which category of information do you
                                       // wish to view?" );
  ...
  for (i = 0; i < SECRET_REPORT_COUNT; i++) {
    if (refreshFlag == INFO_REPAINT) {
      //			button_secret_report_array[i].create(
      //INFO_X1+18, y, INFO_X2-10, y+21, 				disp_text_button, ButtonCustomPara(
      //secret_report_str_array[i], i), 0 );
      button_secret_report_array[i].create(
          INFO_X1 + 18, y, INFO_X2 - 10, y + 21, disp_text_button,
          ButtonCustomPara(text_reports.str_report_mode(i + 1), i), 0);
    }
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    button_secret_report_cancel.create(INFO_X1 + 13 + 3 * BUTTON_DISTANCE,
                                       INFO_Y1 + 281, 'A', "PREVMENU");
  }
  button_view_secret.enable_flag =
      button_secret_report_array[button_secret_report_array()].enable_flag;
...
// <int> nationRecno - recno of the nation which this spy is going to
// investigate
//
...
// <int> fromNationRecno - change any spies in the place whose
// cloaked_nation_recno <int> toNationRecno     is fromNationRecno to
// toNationRecno.
//
void SpyArray::change_cloaked_nation(int spyPlace, int spyPlacePara,
                                     int fromNationRecno, int toNationRecno) {
  Spy *spyPtr;
  ...
  for (int i = spy_array.size(); i > 0; i--) {
    ...
    if (spyPlace == SPY_FIRM ||
        spyPlace == SPY_TOWN) // only check spy_place_para when spyPlace is
                              // SPY_TOWN or SPY_FIRM
    {
      if (spyPtr->spy_place_para != spyPlacePara)
        continue;
    }

    if (spyPlace == spyPtr->spy_place &&
        spyPlacePara == spyPtr->spy_place_para &&
        spyPtr->true_nation_recno == toNationRecno)
      spyPtr->set_action_mode(SPY_IDLE);

    //----- if the spy is associated with a unit (mobile or firm overseer), we
    //call Unit::spy_chnage_nation() ---//

    if (spyPlace == SPY_FIRM) {
      FirmCamp *firmCamp =
          firm_array[spyPtr->spy_place_para]->cast_to_FirmCamp();
      if (firmCamp) {
        int firmOverseerRecno = firmCamp->overseer_recno;

        if (firmOverseerRecno &&
            unit_array[firmOverseerRecno]->spy_recno == i) {
          unit_array[firmOverseerRecno]->spy_change_nation(toNationRecno,
                                                           COMMAND_AUTO);
          continue;
        }
      }
    } else if (spyPlace == SPY_MOBILE) {
      unit_array[spyPtr->spy_place_para]->spy_change_nation(toNationRecno,
                                                            COMMAND_AUTO);
      continue;
    }
...
//
int SpyArray::total_spy_skill_level(int spyPlace, int spyPlacePara,
                                    int spyNationRecno, int &spyCount) {
  int totalSpyLevel = 0;
  Spy *spyPtr;
...
int SpyArray::catch_spy(int spyPlace, int spyPlacePara) {
  ...
  if (spyPlace == SPY_TOWN) {
    ...
    if (firmPtr->cast_to_FirmCamp()) {
      ...
      FirmMonsterFortress *firmMonsterFortress =
          firmPtr->cast_to_FirmMonsterFortress();
      totalPop = firmMonsterFortress->archer_count +
                 firmMonsterFortress->extra_builder_count;
  ...
  for (i = size(); i > 0; i--) {
    ...
    if (spyPtr->spy_place == spyPlace &&
      ...
      if (spyPtr->true_nation_recno == nationRecno)
        counterSpySkill +=
            spyPtr->spy_skill + spyPtr->spy_skill * techLevel / 2;
      else
        enemySpyCount++;
    }
  }
  ...
  for (i = spy_array.size(); i > 0; i--) {
    ...
    if (spyPtr->action_mode ==
        SPY_IDLE) // it is very hard to get caught in sleep mode
      continue;

    if (spyPtr->spy_place == spyPlace &&
        spyPtr->spy_place_para == spyPlacePara &&
        spyPtr->true_nation_recno !=
            nationRecno) // doesn't get caught in sleep mode
    {
