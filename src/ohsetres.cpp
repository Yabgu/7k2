...
void HSetRes::load_eqv_pattern_info(unsigned maxPatternId) {
  ...
  Database *dbEqPattern = game_set.open_db(
      EQPATTERN_DB); // only one database can be opened at a time

  short record_count = (short)dbEqPattern->rec_count();
  eqv_pattern_count =
      (unsigned char *)mem_add(sizeof(unsigned char) * max_pattern_id);
  eqv_pattern_id = (unsigned char *)mem_add(sizeof(unsigned char) *
                                            max_pattern_id * MAX_EQV_PATTERN);

  memset(eqv_pattern_count, 0, sizeof(unsigned char) * maxPatternId);
  memset(eqv_pattern_id, 0,
         sizeof(unsigned char) * maxPatternId * MAX_EQV_PATTERN);

  //---------- read in EQHILL.DBF ---------//

  for (i = 0; i < record_count; i++) {
    EqPatternRec *eqPatternRec = (EqPatternRec *)dbEqPattern->read(i + 1);
    unsigned char patternId = (unsigned char)misc.atoi(
        eqPatternRec->pattern_id, eqPatternRec->PATTERN_ID_LEN);
    unsigned char eqPatternId = (unsigned char)misc.atoi(
        eqPatternRec->eqv_pattern_id, eqPatternRec->PATTERN_ID_LEN);

    if (eqv_pattern_count[patternId - 1] < MAX_EQV_PATTERN) {
      eqv_pattern_id[MAX_EQV_PATTERN * (patternId - 1) +
                     eqv_pattern_count[patternId - 1]] = eqPatternId;
...
void HSetRes::load_hset_info() {
  ...
  Database *dbHSet =
      game_set.open_db(HSET_DB); // only one database can be opened at a time
  ...
  for (i = 0; i < hset_info_count; i++) {
    ...
    hSetInfo->replaced_length =
        (char)misc.atoi(hSetRec->replaced_length, hSetRec->LENGTH_LEN);
    hSetInfo->replaced_length2 =
        (char)misc.atoi(hSetRec->replaced_length2, hSetRec->LENGTH_LEN);
    hSetInfo->replaced_width =
        (char)misc.atoi(hSetRec->replaced_width, hSetRec->LENGTH_LEN);
    hSetInfo->replaced_width2 =
        (char)misc.atoi(hSetRec->replaced_width2, hSetRec->LENGTH_LEN);
...
void HSetRes::load_hill_set_info() {
  ...
  Database *dbHillSet =
      game_set.open_db(HILLSET_DB); // only one database can be opened at a time

  hill_set_info_count = (short)dbHillSet->rec_count();

  hill_set_info_array =
      (HillSetInfo *)mem_add(sizeof(HillSetInfo) * hill_set_info_count);
  memset(hill_set_info_array, 0, sizeof(HillSetInfo) * hill_set_info_count);

  //---------- read in HILLSET.DBF ---------//

  for (i = 0, hillSetInfo = hill_set_info_array; i < hill_set_info_count;
       i++, hillSetInfo++) {
    ...
    hillSetInfo->pattern_id = (unsigned char)misc.atoi(
        hillSetRec->pattern_id, hillSetRec->PATTERN_ID_LEN);
...
unsigned char HSetRes::get_eqv_pattern_id(unsigned char patternId,
                                          unsigned char seqno) {
...
HSetInfo *HSetRes::random_hset(char isSide, char maxLength, char maxWidth,
                               char maxLength2, char maxWidth2) {
  ...
  for (int i = 0; i < hset_info_count; ++i) {
    ...
    if (hSetInfo->side == isSide && hSetInfo->replaced_length <= maxLength &&
