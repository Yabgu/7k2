...
void HSetRes::load_eqv_pattern_info(unsigned maxPatternId)
{
    ...
    for (i = 0; i < record_count; i++)
    {
        ...
        unsigned char eqPatternId =
            (unsigned char)misc.atoi(eqPatternRec->eqv_pattern_id, eqPatternRec->PATTERN_ID_LEN);

        if (eqv_pattern_count[patternId - 1] < MAX_EQV_PATTERN)
        {
            eqv_pattern_id[MAX_EQV_PATTERN * (patternId - 1) + eqv_pattern_count[patternId - 1]] = eqPatternId;
...
void HSetRes::load_hill_set_info()
{
    ...
    for (i = 0, hillSetInfo = hill_set_info_array; i < hill_set_info_count; i++, hillSetInfo++)
    {
        ...
        hillSetInfo->pattern_id = (unsigned char)misc.atoi(hillSetRec->pattern_id, hillSetRec->PATTERN_ID_LEN);
        ...
        if (hillSetInfo->step == 1)
        {
            ...
            err_when(hset_info_array[j - 1].set_id != j || hset_info_array[j - 1].set_id != hillSetInfo->set_id);
...
{
    err_when(seqno < 0 || seqno >= MAX_EQV_PATTERN || seqno >= get_eqv_pattern_count(patternId));
...
HSetInfo *HSetRes::random_hset(char isSide, char maxLength, char maxWidth, char maxLength2, char maxWidth2)
{
    ...
    for (int i = 0; i < hset_info_count; ++i)
    {
        ...
        if (hSetInfo->side == isSide && hSetInfo->replaced_length <= maxLength &&
            hSetInfo->replaced_width <= maxWidth && hSetInfo->replaced_length2 <= maxLength2 &&
