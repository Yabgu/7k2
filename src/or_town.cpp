...
void Info::disp_town(int refreshFlag)
{
    ...
    {
        browse_town.init(TOWN_BROWSE_X1, TOWN_BROWSE_Y1 + 22, TOWN_BROWSE_X2, TOWN_BROWSE_Y2 - 20, 0, 21, town_filter(),
                         put_town_rec, 1);
...
#define FIRM_LABEL_HEIGHT 36
    ...
    font_bld.center_put_paragraph(x + 130, y, x + 215, y + FIRM_LABEL_HEIGHT - 8, text_reports.str_unit_cost(),
                                  DEFAULT_LINE_SPACE, 0,
                                  0); // "Unit Cost" );
    font_bld.center_put_paragraph(x + 217, y, x + 328, y + FIRM_LABEL_HEIGHT - 8, text_reports.str_no_of_structures(),
                                  DEFAULT_LINE_SPACE, 0,
                                  0); // "No. of Structures" );
    font_bld.center_put_paragraph(x + 340, y, x + 438, y + FIRM_LABEL_HEIGHT - 8, text_reports.str_yearly_expense(),
                                  DEFAULT_LINE_SPACE, 0,
                                  0); // "Yearly Expense" );
    font_bld.center_put_paragraph(x + 440, y, x + 548, y + FIRM_LABEL_HEIGHT - 8, text_reports.str_yearly_income(),
                                  DEFAULT_LINE_SPACE, 0,
                                  0); // "Yearly Income" );
#else
    // one-line label
...
// SXM
    ...
    {
        // ###### begin Gilbert 19/10 ######//
        browse_firm.init(FIRM_BROWSE_X1, FIRM_BROWSE_Y1 + FIRM_LABEL_HEIGHT + 1, FIRM_BROWSE_X2, FIRM_BROWSE_Y2 - 20, 0,
                         18, firm_filter(), put_firm_rec, 1);
        // ###### end Gilbert 19/10 ######//
...
static void disp_total()
{
    ...
    font_bld.put(
        x, y,
        text_reports.str_total_other_units(viewNation->total_human_count + viewNation->total_monster_count)); // str );
    ...
    font_bld.put(x + 360, y,
                 text_reports.str_total_population(total_population + viewNation->total_human_count +
...
static void calc_firm_total()
{
    ...
    for (i = 1; i <= MAX_FIRM_TYPE; i++)
    {
        ...
        total_firm_cost += firmInfo->year_cost * firmInfo->nation_firm_count_array[info.viewing_nation_recno - 1];
