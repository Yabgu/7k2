...
void Info::disp_town(int refreshFlag) {
  ...
  vga_back.d3_panel_up(TOWN_BROWSE_X1, TOWN_BROWSE_Y1, TOWN_BROWSE_X2,
                       TOWN_BROWSE_Y1 + 20);
  ...
  font_bld.put(x + 400 + 6, y,
               text_reports.str_nationality()); // "Nationality" );
#else
  font_bld.put(x, y, text_reports.str_town());              // "Town" );
  ...
  if (refreshFlag == INFO_REPAINT) {
    browse_town.init(TOWN_BROWSE_X1, TOWN_BROWSE_Y1 + 22, TOWN_BROWSE_X2,
                     TOWN_BROWSE_Y2 - 20, 0, 21, town_filter(), put_town_rec,
                     1);
...
#define FIRM_LABEL_HEIGHT 36
  vga_back.d3_panel_up(FIRM_BROWSE_X1, FIRM_BROWSE_Y1, FIRM_BROWSE_X2,
                       FIRM_BROWSE_Y1 + FIRM_LABEL_HEIGHT - 1);
  font_bld.put(x, y + 8, text_reports.str_structure()); // "Structure" );
  font_bld.center_put_paragraph(x + 130, y, x + 215, y + FIRM_LABEL_HEIGHT - 8,
                                text_reports.str_unit_cost(),
                                DEFAULT_LINE_SPACE, 0, 0); // "Unit Cost" );
  font_bld.center_put_paragraph(x + 217, y, x + 328, y + FIRM_LABEL_HEIGHT - 8,
                                text_reports.str_no_of_structures(),
                                DEFAULT_LINE_SPACE, 0,
                                0); // "No. of Structures" );
  font_bld.center_put_paragraph(x + 340, y, x + 438, y + FIRM_LABEL_HEIGHT - 8,
                                text_reports.str_yearly_expense(),
                                DEFAULT_LINE_SPACE, 0,
                                0); // "Yearly Expense" );
  font_bld.center_put_paragraph(x + 440, y, x + 548, y + FIRM_LABEL_HEIGHT - 8,
                                text_reports.str_yearly_income(),
                                DEFAULT_LINE_SPACE, 0, 0); // "Yearly Income" );
#else
// one-line label
...
// SXM
  ...
  font_bld.put(x + 217 + 16, y,
               text_reports.str_no_of_structures()); // "No. of Structures" );
  font_bld.put(x + 340 + 28, y,
               text_reports.str_yearly_expense()); // "Yearly Expense" );
  font_bld.put(x + 450 + 17, y,
               text_reports.str_yearly_income()); // "Yearly Income" );
#elif (defined(ITALIAN))
  font_bld.put(x, y, text_reports.str_structure());       // "Structure" );
  font_bld.put(x + 110, y, text_reports.str_unit_cost()); // "Unit Cost" );
  font_bld.put(x + 217, y,
               text_reports.str_no_of_structures()); // "No. of Structures" );
  font_bld.put(x + 340, y,
               text_reports.str_yearly_expense()); // "Yearly Expense" );
  font_bld.put(x + 450, y,
               text_reports.str_yearly_income()); // "Yearly Income" );
#else // GERMAN and US
  font_bld.put(x, y, text_reports.str_structure());       // "Structure" );
  font_bld.put(x + 140, y, text_reports.str_unit_cost()); // "Unit Cost" );
  ...
  if (refreshFlag == INFO_REPAINT) {
    // ###### begin Gilbert 19/10 ######//
    browse_firm.init(FIRM_BROWSE_X1, FIRM_BROWSE_Y1 + FIRM_LABEL_HEIGHT + 1,
                     FIRM_BROWSE_X2, FIRM_BROWSE_Y2 - 20, 0, 18, firm_filter(),
                     put_firm_rec, 1);
    // ###### end Gilbert 19/10 ######//
...
static void disp_total() {
  ...
  vga_back.d3_panel_up(TOWN_BROWSE_X1, TOWN_BROWSE_Y2 - 18, TOWN_BROWSE_X2,
                       TOWN_BROWSE_Y2);
  ...
  font_bld.put(x, y,
               text_reports.str_total_towns(browse_town.total_rec())); // str );
  ...
  font_bld.put(x + 180, y,
               text_reports.str_total_citizens(total_population)); // str );
  ...
  //	str += ": ";
  //	str += total_peasant;
  font_bld.put(x + 360, y,
               text_reports.str_total_peasant(total_peasant)); // str );
  ...
  font_bld.put(x, y,
               text_reports.str_total_other_units(
                   viewNation->total_human_count +
                   viewNation->total_monster_count)); // str );
  ...
  font_bld.put(x + 360, y,
               text_reports.str_total_population(
                   total_population + viewNation->total_human_count +
                   viewNation->total_monster_count)); // str );
  ...
  vga_back.d3_panel_up(FIRM_BROWSE_X1, FIRM_BROWSE_Y2 - 18, FIRM_BROWSE_X2,
                       FIRM_BROWSE_Y2);

  font_bld.put(x, y, text_reports.str_total()); // "Total" );
  font_bld.put(x + 265, y, total_firm_count);
  font_bld.put(x + 370, y, misc.format(total_firm_cost, 2));
  font_bld.put(x + 470, y, misc.format(total_firm_income, 2));
}
...
static void put_firm_rec(int recNo, int x, int y, int refreshFlag) {
  ...
  font_bld.put(x + 155, y, misc.format(firmInfo->year_cost, 2));
  font_bld.put(x + 265, y, firmCount);
  font_bld.put(x + 370, y, misc.format(firmInfo->year_cost * firmCount, 2));
  font_bld.put(x + 470, y, misc.format(firm_income_array[firmId - 1], 2));
}
