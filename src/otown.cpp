...
//
Town::Town() {
  memset(sizeof(void *) + (char *)this, 0,
         sizeof(Town) - sizeof(void *)); // protect vptr
}
...
//								pass 0 if the town's
//construction is immediately complete, requiring no building time
//
void Town::init(int nationRecno, int raceId, int xLoc, int yLoc,
  ...
  ai_town =
      !nation_recno || nation_array[nation_recno]->nation_type ==
                           NATION_AI; // nation_recno==0 for independent towns
  ai_link_checked = 1; // check the linked towns and firms connected only if
                       // ai_link_checked==0

  independent_unit_join_nation_min_rating =
      100 + misc.random(150); // the minimum rating a nation must have in order
                              // for an independent unit to join it
  ...
  if (nation_recno) {
    nation_array[nation_recno]->add_town_info(
        town_recno); // update this for all towns
...
void Town::init_resistance() {
  ...
  //--- some independent towns have higher than normal combat level for its
  //defender ---//
...
void Town::set_world_matrix() {
  ...
  for (yLoc = MAX(loc_y1 - INTER_PLACE_SPACE, 0);
       yLoc <= loc_y2 + INTER_PLACE_SPACE && yLoc < world.max_y_loc; yLoc++) {
    for (xLoc = MAX(loc_x1 - INTER_PLACE_SPACE, 0);
         xLoc <= loc_x2 + INTER_PLACE_SPACE && xLoc < world.max_x_loc; xLoc++) {
  ...
  for (yLoc = loc_y1; yLoc <= loc_y2; yLoc++) {
    for (xLoc = loc_x1; xLoc <= loc_x2; xLoc++) {
      ...
      if (!locPtr->cargo_recno) // skip the location where the settle unit is
                                // standing
  ...
  //---- if the newly built firm is visual in the zoom window, redraw the zoom
  //buffer ----//

  if (is_in_zoom_win())
    sys.zoom_need_redraw =
        1; // set the flag on so it will be redrawn in the next frame
}
...
void Town::process_construction() {
  ...
  if (construction_completion_percent >= 100) {
    ...
    if (nation_recno ==
        nation_array.player_recno) //**BUGHERE - needs to be updated
    {
      se_res.far_sound(center_x, center_y, 1, 'S', unitPtr->sprite_id, "FINS",
                       'F', 1);
    }
...
//
void Town::complete_construction() { under_construction = 0; }
//--------- End of function Town::complete_construction -----------//

// ##### begin Gilbert 28/12 #######//
//---------- Begin of function Town::process --------//
//
void Town::process() { process_attack_target(); }
...
void Town::next_day() {
  ...
  if (nation_recno &&
    ...
    think_surrender(); // for nation town only, independent town surrender is
                       // handled in update_resistance()
    LOG_MSG(misc.get_random_seed());
  ...
  //--- recheck no_neighbor_space after a period, there may be new space
  //available now ---//
  ...
  if (no_neighbor_space && info.game_date % 180 == town_recno % 180) {
    ...
    //--- for independent town, since we can't call find_best_place_loc(), we
    //just set no_neighbor_space to 0 every 6 months, if it still has no space,
    //then no_neighbor_space will be set 1 again. ---//

    if (nation_recno == 0 ||
        nation_array[nation_recno]->find_best_place_loc(
            FIRM_INN, loc_x1, loc_y1, buildXLoc,
            buildYLoc)) // whether it's FIRM_INN or not really doesn't matter,
                        // just any firm type will do
      no_neighbor_space = 0;
...
//
void Town::next_month() { pay_tribute_to_monster(); }
...
void Town::pay_tribute_to_monster() {
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    if (firmPtr->firm_id == FIRM_LAIR && firmPtr->nation_recno &&
        ...
        resistance(firmPtr->nation_recno) <
            MONSTER_COLLECT_TOWN_TRIBUTE_LOYALTY) {
      lairCount++;
    }
  }
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    if (tributeAmount > 0) {
      ...
      nation_array[firmPtr->nation_recno]->add_income(INCOME_TRIBUTE,
                                                      tributeAmount);
    }
  }
}
...
float Town::tribute_to_lair(int firmRecno, int lairCount) {
  ...
  if (firmPtr->firm_id == FIRM_LAIR && firmPtr->nation_recno &&
    ...
    int commanderSkill =
        unit_array[firmPtr->cast_to_FirmLair()->overseer_recno]->skill_level();
...
//							  is paying tribute to this nation.
//Otherwise 							  just check if this town is paying tribute 							  to ANY nations.
//
...
void Town::process_food(int busyPeasant) {
  ...
  if (nation_recno) {
    yieldRate +=
        0.25f * tech_res[TECH_FARMING]->get_nation_tech_level(nation_recno);
    // ###### begin Gilbert 16/2 ######//
    // yieldRate += 0.03125f *
    // god_res[GOD_CARTH]->nation_prayer_count(nation_recno);
  ...
  if (!nation_array[nation_recno]->is_ai() &&
      config.start_up_cash <=
          OPTION_MODERATE) // if startup cash >= OPTION_HIGH, then not reduction
                           // in food production
  {
    foodYearProduction = PEASANT_FOOD_YEAR_PRODUCTION * 25 /
                         30; // less food production for the player
  } else
    foodYearProduction = PEASANT_FOOD_YEAR_PRODUCTION;

  nation_array[nation_recno]->add_food(float(jobless_population - busyPeasant) *
                                       yieldRate * foodYearProduction / 365);

  //---------- People consume food ----------//

  nation_array[nation_recno]->consume_food((float)population *
                                           UNIT_FOOD_YEAR_CONSUMPTION / 365);
}
...
void Town::process_auto() {
  if (!has_linked_own_camp) // can only collect or grant when there is a linked
                            // camp of its own.
    return;
  ...
  if (auto_grant_loyalty > 0) {
    if (accumulated_reward_penalty == 0 && loyalty < auto_grant_loyalty &&
        nationPtr->cash > 0) {
      reward(COMMAND_AI);
    }
  }
...
void Town::set_nation(int newNationRecno) {
  ...
  world.restore_power(loc_x1, loc_y1, loc_x2, loc_y2, town_recno,
                      0); // restore power of the old nation
  world.set_power(loc_x1, loc_y1, loc_x2, loc_y2,
                  newNationRecno); // set power of the new nation

  //--- update the cloaked_nation_recno of all spies in the firm ---//

  spy_array.change_cloaked_nation(
      SPY_TOWN, town_recno, nation_recno,
      newNationRecno); // check the cloaked nation recno of all spies in the
                       // firm
  ...
  if (nation_recno) // reset rebel_recno if the town is then ruled by a nation
  {
    if (rebel_recno) {
      ...
      err_when(rebelPtr->mobile_rebel_count >
               0); // change nation shouldn't happen if mobile_rebel_count > 0
  ...
  if (nation_recno == 0) // independent town set up by rebel
  {
    ...
    if (nation_array[nation_recno]->race_id == race_id)
      loyalty =
          60; // loyalty is higher if the ruler and the people are the same race
    else
      loyalty = 50;
  }
  ...
  town_combat_level = 0;
  independent_unit_join_nation_min_rating =
      100 + misc.random(150); // prevent it from changing nation again too soon
  ...
  release_link(); // we need to update links as when the nation link the links
                  // to original firms may be broken.
  setup_link();
  ...
  spy_array.set_action_mode(
      SPY_TOWN, town_recno,
      SPY_IDLE); // we need to reset it. e.g. when we have captured an enemy
                 // town, SPY_SOW_DISSENT action must be reset to SPY_IDLE
...
void Town::set_nation_unjob_worker() {
  if (nation_recno == 0) // if this is an independent town, citizens there can
                         // work for any firms
    return;
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    ...
    //--- if this firm longer has a valid link to the town after changing the
    //nation recno ---//
...
int Town::camp_influence(Unit *unitPtr) {
  return camp_influence(unitPtr->nation_recno, unitPtr->race_id,
                        unitPtr->skill_level());
}
//------- End of function Town::camp_influence ------//
...
int Town::camp_influence(InnUnit *innUnit, int nationRecno) {
  return camp_influence(nationRecno, unit_res[innUnit->unit_id]->race_id,
                        innUnit->skill_level());
}
//------- End of function Town::camp_influence ------//
...
int Town::camp_influence(int nationRecno, int raceId, int skillLevel) {
  ...
  int thisInfluence =
      MIN(100, skillLevel) /
      2; // 50% of the leadership, the maximum effective skill is 100

  if (race_res.is_same_race(race_id, raceId))
    thisInfluence +=
        thisInfluence /
        3; // 33% bonus if commander's race is the same as the town's.
  else
...
void Town::collect_yearly_tax() {
  ...
  nation_array[nation_recno]->add_income(INCOME_TAX,
                                         (float)population * taxPerUnit);
}
...
void Town::collect_tax(char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_TOWN_COLLECT_TAX, sizeof(short));
  ...
  int loyaltyDecrease =
      COLLECT_TAX_LOYALTY_DECREASE + accumulated_collect_tax_penalty / 5;
  ...
  taxCollected +=
      (beforeLoyalty - loyalty) * population * TAX_PER_UNIT / loyaltyDecrease;
...
void Town::reward(char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <town recno> <race id>
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_TOWN_REWARD, sizeof(short));
    shortPtr[0] = town_recno;
    return;
  }
  ...
  int loyaltyIncrease =
      TOWN_REWARD_LOYALTY_INCREASE - accumulated_reward_penalty / 5;
  ...
  nation_array[nation_recno]->add_expense(
      EXPENSE_GRANT_OWN_TOWN, (float)population * TOWN_REWARD_PER_UNIT);
}
...
int Town::think_surrender() {
  //--- if this is a slave town, it can never surrender to a human kingdom
  //unless the enslaving Fryhtan lair is destroyed ---//
  ...
  if (nation_recno) {
    //--- only surrender when there is no own camps, but has enemy camps linked
    //to this town ---//
  ...
  // ###### begin Gilbert 22/4 ########//
  int bestRating =
      nation_recno ? (int)loyalty : 0; // if it belongs to a nation, surrender
                                       // only if resistance below loyalty
  // ###### end Gilbert 22/4 ########//
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    ...
    if (firmCamp->cast_to_FirmCamp() &&
        firmCamp->nation_recno != nation_recno && firmCamp->nation_recno &&
        firmCamp->overseer_recno &&
        firmCamp->is_human()) // only surrender to humans
    {
      //----- for independent towns, only surrender if resistance <
      ...
      curRating = camp_influence(firmCamp); // see camp_influence() for details
...
void Town::surrender(int toNationRecno) {
  //--- if this is a rebel town and the mobile rebel count is > 0, don't
  //surrender (this function can be called by update_resistance() when
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    ...
    if (firmPtr->nation_recno == toNationRecno) {
      ...
      //--- if this is a camp and it has no linked town, set active linked town
      //to it ---//
...
#define REBEL_INTERVAL_MONTH 3 // don't rebel twice in less than 3 months
  ...
  if (town_defender_count > 0 ||
      info.game_date <
          last_attack_date + 30) // don't rebel within 30 days after being
                                 // attacked by a hostile unit
    return;
  ...
  //--- if the town only has one unit and that unit rebels, handle specially
  //with the below code ---//
  ...
  int rebelRecno =
      rebel_array.create_rebel(leaderUnitRecno, nation_recno, REBEL_ATTACK_TOWN,
                               town_recno); // create a rebel group

  //------- create other rebel units in the rebel group -----//

  int raceRebelCount = (int)(population - spy_count) * (30 + misc.random(30)) /
                       100; // 30% - 60% of the unit will rebel.

  err_when(raceRebelCount + 1 >
           MAX_TOWN_POPULATION); // plus 1 for the leader, cannot excess
                                 // MAX_TOWN_POPULATION, consider the case these
                                 // units settle immediately
  ...
  news_array.town_rebel(
      town_recno,
      rebelCount); // add the news first as after callijng ai_spy_town_rebel,
                   // the town may disappear as all peasants are gone
...
int Town::create_rebel_unit(int isLeader) {
  if (recruitable_pop(0) ==
      0) // if the unjob unit is a spy too, then don't rebel
    return 0;
  ...
  if (!world.locate_space(xLoc, yLoc, loc_x2, loc_y2, spriteInfo->loc_width,
  ...
  if (isLeader) {
    int combatLevel = 10 + population * 2 +
                      misc.random(10); // the higher the population is, the
                                       // higher the combat level will be
    int leadershipLevel =
        10 + population + misc.random(10); // the higher the population is, the
                                           // higher the combat level will be
...
void Town::being_attack_hit(BaseObj *attackerObj, float damagePoint) {
  ...
  if (attackerObj) {
    ...
    if (attackerObj->nation_recno ==
        nation_recno) // this can happen when the unit has just changed nation
      return;
    ...
    if (hit_points <= 0) {
      // only call out defender when the attacking unit is within the effective
      // defending distance

      if (misc.points_distance(attackerObj->obj_loc_x1(),
                               attackerObj->obj_loc_y1(), center_x,
                               center_y) <= EFFECTIVE_DEFEND_DISTANCE) {
        int loopCount = 0;
        ...
        while (1) {
          ...
          // ####### begin Gilbert 15/4 ########//
          // attack first, don't wait for next_day's call
          // think_aggressive_attack unit_array[?]->think_aggressive_attack( 0,
          // true );
          // ####### end Gilbert 15/4 ########//
  ...
  if (hit_points > 0.0f) {
    hit_points -= damagePoint / ATTACK_SLOW_DOWN /
                  5; // divided by 5 is to make it slower in destroying towns
  ...
  //----- if there are people in the town -----//
    ...
    if (town_defender_count == 0)
      ...
      received_hit_count +=
          damagePoint /
          3; // if there are still defenders out there, the number of people
             // killed will be fewer. This is a gameplay design to prevent
             // getting too many people unnecessarily killed (as the attacker
             // ultimate want to take over the town and the town people will
             // become his later.)
    ...
    if (received_hit_count >= RECEIVED_HIT_PER_KILL) {
      ...
      if (town_array.is_deleted(townRecno)) // the town may have been deleted
                                            // when all pop are killed
        return;
    }
    ...
    if (town_defender_count == 0) {
      //--- Resistance/loyalty of the town people decrease if the attacking
      //continues ---//
      //
      ...
      float loyaltyDec;
      int zeroLoyaltyResistance =
          0; // if the loyalty or resistance has reached 0
      ...
      if (nation_recno) // if the town belongs to a nation
      {
        ...
        if (has_linked_own_camp) // if it is linked to one of its camp, the
                                 // loyalty will decrease slower
      ...
      {
        if (!attackerNationRecno) // if independent units do not attack
                                  // independent towns
          return;
        ...
        loyaltyDec =
            (float)10 / population; // decrease faster for independent towns
                                    // than towns belonging to nations
        loyaltyDec = MIN(loyaltyDec, (float)1);

        resistance_array[attackerNationRecno - 1] -=
            loyaltyDec * damagePoint / (20 / ATTACK_SLOW_DOWN);
      ...
      //-- some citizens will surrender, although the entire town will never
      //surrender --//

      if (zeroLoyaltyResistance && attackerNationRecno &&
          nation_array[attackerNationRecno]->is_monster()) {
        if (misc.random(50) == 0) {
          int unitRecno =
              recruit(false, COMMAND_AUTO); // false-not a wagon, just a unit
...
#define MAX_ACTIVE_MARKET_COUNT 20
  ...
  for (int linkedFirmId = 0; linkedFirmId < linked_firm_count; linkedFirmId++) {
    ...
    for (i = 0; i < MAX_PRODUCT; i++) {
      //			marketGoods =
      //((FirmMarket*)firmPtr)->market_product_array[i];
      marketGoods = ((FirmMarket *)firmPtr)->market_product_array(i + 1);
      marketGoodsInfo = marketGoodsInfoArray + i;

      if (marketGoods &&
          marketGoodsInfo->market_count < MAX_ACTIVE_MARKET_COUNT) {
        thisSupply = marketGoods->stock_qty;

        marketGoodsInfo->market_ptr[marketGoodsInfo->market_count] =
            (FirmMarket *)firmPtr;

        marketGoodsInfo->total_supply += thisSupply;

        if (firmPtr->nation_recno ==
            nation_recno) // vars for later use, so that towns will always try
                          // to buy goods from their own markets first.
  ...
  float townDemand =
      (float)jobless_population * (float)PEASANT_GOODS_MONTH_DEMAND +
  ...
  for (i = 0; i < MAX_PRODUCT; i++) {
    ...
    for (j = 0; j < marketGoodsInfo->market_count; j++) {
      ...
      // a market's demand = its_supply + (town_demand-totalSupply) /
      // market_count
      //
      // If the totalSupply > town demand:
      ...
      //			marketGoods =
      //firmMarket->market_product_array[i];
      ...
      if (marketGoods) {
        ...
        if (marketGoodsInfo->total_supply < townDemand) {
          marketGoods->month_demand +=
              marketGoods->stock_qty +
              (townDemand - marketGoodsInfo->total_supply) /
                  marketGoodsInfo
                      ->market_count; // evenly distribute the excessive demand
                                      // on all markets
        } else //---- if the supply is larger than the demand -----//
        {
          ...
          if (firmMarket->nation_recno == nation_recno) {
            marketGoods->month_demand += ownShareDemand *
                                         marketGoods->stock_qty /
                                         marketGoodsInfo->total_own_supply;
          } else {
            marketGoods->month_demand += (townDemand - ownShareDemand) *
                                         marketGoods->stock_qty /
...
int Town::can_recruit_peasant() {
  ...
  //--- for the AI, only recruit if the loyalty still stay at 30 after
  //recruiting the unit ---//

  if (ai_town && nation_recno)
    minRecruitLoyalty +=
        3 + recruit_dec_loyalty(
                1, 0); // 1-recruit one unit only, 0-don't actually decrease it,
                       // just return the loyalty to be decreased.
...
//
int Town::can_train() { return recruitable_pop(1) > 0; }
...
void Town::set_auto_collect_tax_loyalty(int loyaltyLevel) {
  ...
  if (auto_collect_tax_loyalty > 0 && auto_grant_loyalty > 0 &&
...
int Town::can_grant_to_non_own_town(int grantNationRecno) {
  ...
  if (!nation_array[grantNationRecno]
           ->is_human()) // only human nations can grant to independent towns
    return 0;

  if (nation_recno == 0) // independent town
  {
    return has_linked_camp(grantNationRecno,
                           1); // 1-only count camps with overseers
  } else // for nation town, when the enemy doesn't have camps linked to it and
         // the granting nation has camps linked to it
  {
    return has_linked_camp(nation_recno, 0) ==
               0 && // 0-count camps regardless of the presence of overseers
           has_linked_camp(grantNationRecno,
                           1); // 1-only count camps with overseers
  }
}
...
int Town::grant_to_non_own_town(int grantNationRecno, int remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_TOWN_GRANT_INDEPENDENT, 2 * sizeof(short));
    shortPtr[0] = town_recno;
  ...
  int resistanceDec =
      IND_TOWN_GRANT_RESISTANCE_DECREASE - accumulated_enemy_grant_penalty / 5;
  ...
  grantNation->add_expense(EXPENSE_GRANT_OTHER_TOWN,
                           (float)population * IND_TOWN_GRANT_PER_UNIT);
...
int Town::closest_own_camp() {
  ...
  for (int i = linked_firm_count - 1; i >= 0; i--) {
    ...
    curDistance = misc.points_distance(center_x, center_y, firmPtr->center_x,
                                       firmPtr->center_y);
...
//										 if it is 0, then
//assume resistance is 0 										 if it's an independent town.
//
int Town::possible_defender_combat_level(int attackerNationRecno) {
  ...
  if (resistanceDiff >= 0) {
    //		float resistanceDecPerDefender = thisResistance/4;		// each
    //mobilized defender is counted as 5 points in Town::mobilize_defender()

    //		int defenderCount = int(resistanceDiff / resistanceDecPerDefender)+1;
    ...
    totalDefenseLevel +=
        town_combat_level * 2 *
        defenderCount; // *2 is defenseCombatLevel is actually the sum of hit
                       // points, not combat level

    totalDefenseLevel +=
        (int)hit_points *
        4; // the hit points of the wall, 50 is the arrow attack even in the
           // absence of a wall. * 2 for the stones throwing out from the wall
  }
...
//
char *Town::town_name() { return town_res.get_name(town_name_id); }
