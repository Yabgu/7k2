...
void Town::init(int nationRecno, int raceId, int xLoc, int yLoc, int builderRecno)
{
    ...
    ai_town =
        !nation_recno || nation_array[nation_recno]->nation_type == NATION_AI; // nation_recno==0 for independent towns
    ai_link_checked = 1; // check the linked towns and firms connected only if
                         // ai_link_checked==0

    independent_unit_join_nation_min_rating = 100 + misc.random(150); // the minimum rating a nation must have in order
...
void Town::deinit()
{
    ...
    if (nation_recno && nation_array[nation_recno]->largest_town_recno == town_recno)
    {
...
void Town::set_world_matrix()
{
    ...
    for (yLoc = MAX(loc_y1 - INTER_PLACE_SPACE, 0); yLoc <= loc_y2 + INTER_PLACE_SPACE && yLoc < world.max_y_loc;
         yLoc++)
    {
        for (xLoc = MAX(loc_x1 - INTER_PLACE_SPACE, 0); xLoc <= loc_x2 + INTER_PLACE_SPACE && xLoc < world.max_x_loc;
             xLoc++)
        {
...
void Town::process_construction()
{
    ...
    if (construction_completion_percent >= 100)
    {
        ...
        {
            se_res.far_sound(center_x, center_y, 1, 'S', unitPtr->sprite_id, "FINS", 'F', 1);
        }
...
void Town::next_day()
{
    ...
    if (nation_recno && (info.game_date % 15 == town_recno % 15 || loyalty == 0))
    {
    ...
    if (no_neighbor_space && info.game_date % 180 == town_recno % 180)
    {
        ...
        if (nation_recno == 0 ||
            nation_array[nation_recno]->find_best_place_loc(FIRM_INN, loc_x1, loc_y1, buildXLoc,
                                                            buildYLoc)) // whether it's FIRM_INN or not really doesn't
                                                                        // matter, just any firm type will do
            no_neighbor_space = 0;
...
void Town::pay_tribute_to_monster()
{
    ...
    for (i = 0; i < linked_firm_count; i++)
    {
        ...
        if (firmPtr->firm_id == FIRM_LAIR && firmPtr->nation_recno && !firmPtr->under_construction &&
...
float Town::tribute_to_lair(int firmRecno, int lairCount)
{
        ...
        firmPtr->cast_to_FirmLair()->overseer_recno)
    {
        ...
        return (float)population * MONSTER_TOWN_TRIBUTE_PER_UNIT / lairCount * (100 + MIN(commanderSkill, 100)) / 200;
    }
...
//							  is paying tribute to this
// nation.
// Otherwise 							  just check if this town is
// paying tribute to ANY nations.
//
...
void Town::process_food(int busyPeasant)
{
    ...
    if (!nation_array[nation_recno]->is_ai() &&
        config.start_up_cash <= OPTION_MODERATE) // if startup cash >= OPTION_HIGH, then not reduction
                                                 // in food production
    {
    ...
    nation_array[nation_recno]->add_food(float(jobless_population - busyPeasant) * yieldRate * foodYearProduction /
                                         365);
...
void Town::process_auto()
{
    ...
    {
        if (accumulated_collect_tax_penalty == 0 && loyalty >= auto_collect_tax_loyalty)
        {
    ...
    {
        if (accumulated_reward_penalty == 0 && loyalty < auto_grant_loyalty && nationPtr->cash > 0)
        {
            reward(COMMAND_AI);
        }
    }
...
void Town::set_nation(int newNationRecno)
{
    ...
    spy_array.change_cloaked_nation(SPY_TOWN, town_recno, nation_recno,
    ...
    spy_array.set_action_mode(SPY_TOWN, town_recno,
...
int Town::think_surrender()
{
    ...
    // ###### begin Gilbert 22/4 ########//
    int bestRating = nation_recno ? (int)loyalty : 0; // if it belongs to a nation, surrender
    ...
    for (int i = 0; i < linked_firm_count; i++)
    {
        ...
        if (firmCamp->cast_to_FirmCamp() && firmCamp->nation_recno != nation_recno && firmCamp->nation_recno &&
            firmCamp->overseer_recno && firmCamp->is_human()) // only surrender to humans
        {
            //----- for independent towns, only surrender if resistance <
            // SURRENDER_RESISTANCE

            if (nation_recno == 0 && resistance(firmCamp->nation_recno) >= SURRENDER_RESISTANCE)
            {
                continue;
            }
...
void Town::surrender(int toNationRecno)
{
    ...
    if (nation_recno == nation_array.player_recno || toNationRecno == nation_array.player_recno)
    {
    ...
    for (int i = 0; i < linked_firm_count; i++)
    {
        ...
        if (firmPtr->nation_recno == toNationRecno)
        {
            ...
            if (firmPtr->firm_id == FIRM_FORT)
            {
                ...
                if (firmFort->active_link_town_recno == 0 && firmFort->can_set_active_link(town_recno))
                {
...
#define REBEL_INTERVAL_MONTH 3 // don't rebel twice in less than 3 months
    ...
    if (town_defender_count > 0 || info.game_date < last_attack_date + 30) // don't rebel within 30 days after being
    ...
    int rebelRecno = rebel_array.create_rebel(leaderUnitRecno, nation_recno, REBEL_ATTACK_TOWN,
                                              town_recno); // create a rebel group

    //------- create other rebel units in the rebel group -----//

    int raceRebelCount =
        (int)(population - spy_count) * (30 + misc.random(30)) / 100; // 30% - 60% of the unit will rebel.

    err_when(raceRebelCount + 1 > MAX_TOWN_POPULATION); // plus 1 for the leader, cannot excess
    ...
    news_array.town_rebel(town_recno,
...
int Town::create_rebel_unit(int isLeader)
{
    ...
    if (isLeader)
    {
        int combatLevel = 10 + population * 2 + misc.random(10); // the higher the population is, the
                                                                 // higher the combat level will be
        int leadershipLevel = 10 + population + misc.random(10); // the higher the population is, the
                                                                 // higher the combat level will be
...
void Town::being_attack_hit(BaseObj *attackerObj, float damagePoint)
{
    ...
    if (attackerObj)
    {
        ...
        if (hit_points <= 0)
        {
            ...
            if (misc.points_distance(attackerObj->obj_loc_x1(), attackerObj->obj_loc_y1(), center_x, center_y) <=
    ...
    else if (population > 0)
    {
        ...
        if (town_defender_count == 0)
            ...
            received_hit_count += damagePoint / 3; // if there are still defenders out there, the number of people
        ...
        if (town_defender_count == 0)
        {
            ...
            else // if the town is an independent town
            {
                ...
                loyaltyDec = (float)10 / population; // decrease faster for independent towns
            ...
            if (zeroLoyaltyResistance && attackerNationRecno && nation_array[attackerNationRecno]->is_monster())
            {
                if (misc.random(50) == 0)
                {
    ...
    for (int linkedFirmId = 0; linkedFirmId < linked_firm_count; linkedFirmId++)
    {
        ...
        for (i = 0; i < MAX_PRODUCT; i++)
        {
            ...
            if (marketGoods && marketGoodsInfo->market_count < MAX_ACTIVE_MARKET_COUNT)
            {
                ...
                if (firmPtr->nation_recno == nation_recno) // vars for later use, so that towns will always try
            ...
            if (marketGoods)
            {
                ...
                if (marketGoodsInfo->total_supply < townDemand)
                {
                    marketGoods->month_demand +=
                        marketGoods->stock_qty + (townDemand - marketGoodsInfo->total_supply) /
                                                     marketGoodsInfo->market_count; // evenly distribute the excessive
                ...
                else //---- if the supply is larger than the demand -----//
                {
                    ...
                    {
                        marketGoods->month_demand +=
                            ownShareDemand * marketGoods->stock_qty / marketGoodsInfo->total_own_supply;
                    }
                    else
                    {
                        marketGoods->month_demand +=
                            (townDemand - ownShareDemand) * marketGoods->stock_qty / marketGoodsInfo->total_supply;
                    }
                }
            }
        }
...
int Town::can_recruit_peasant()
{
    ...
    if (ai_town && nation_recno)
        minRecruitLoyalty += 3 + recruit_dec_loyalty(1, 0); // 1-recruit one unit only, 0-don't actually decrease it,
...
void Town::set_auto_collect_tax_loyalty(int loyaltyLevel)
{
    ...
    if (auto_collect_tax_loyalty > 0 && auto_grant_loyalty > 0 && auto_grant_loyalty >= auto_collect_tax_loyalty)
    {
...
int Town::closest_own_camp()
{
    ...
    for (int i = linked_firm_count - 1; i >= 0; i--)
    {
        ...
        curDistance = misc.points_distance(center_x, center_y, firmPtr->center_x, firmPtr->center_y);
...
//										 if it is 0,
// then
// assume resistance is 0 										 if
// it's an independent town.
//
int Town::possible_defender_combat_level(int attackerNationRecno)
{
    ...
    if (resistanceDiff >= 0)
    {
        //		float resistanceDecPerDefender = thisResistance/4;		//
        // each
        // mobilized defender is counted as 5 points in Town::mobilize_defender()

        //		int defenderCount = int(resistanceDiff /
        // resistanceDecPerDefender)+1;
        //// no. of defenders will go out if you attack this town

        int defenderCount = int(resistanceDiff / 3) + 1;

        totalDefenseLevel += town_combat_level * 2 * defenderCount; // *2 is defenseCombatLevel is actually the sum of
                                                                    // hit points, not combat level

        totalDefenseLevel += (int)hit_points * 4; // the hit points of the wall, 50 is the arrow attack even in the
