...
#define ADD_SITE_RANGE 5
        ...
        for (int i = 1; i < ADD_SITE_RANGE * ADD_SITE_RANGE; i++)
        {
            ...
            if (world.can_build_site(xLoc, yLoc, siteWidth, siteHeight, gapSpace) && locPtr->region_id == regionId)
            {
                foundFlag = 1;
                break;
            }
...
#define REGION_SIZE_PER_RAW 100
    ...
    for (i = size(); i > 0; i--)
    {
        ...
        if (sitePtr->site_type == SITE_RAW && sitePtr->reserve_qty >= EXIST_RAW_RESERVE_QTY)
        {
            existRawSiteCount++;
        }
    }
    ...
    for (int k = 0; k < 10; k++) //	one loop may not be enough to generate all raw sites, have more
...
int SiteArray::create_raw_site(int regionId, int townRecno)
{
    ...
    for (i = 0; i < MAX_RAW; i++)
    {
        ...
        if (rawCountArray[rawId - 1] == minCount) // don't use this raw type unless it is one of the less
                                                  // available ones.
            break;
    }
    ...
    if (townRecno)
    {
        ...
        locX1 = townPtr->center_x - maxTownSiteDistance - (siteWidth - 1) / 2; // (siteWidth-1)/2 is the distance from
                                                                               // mine center_x to mine loc_x1
    ...
    if (world.locate_space_random(locX1, locY1, locX2, locY2, mineSize + INTER_PLACE_SPACE * 2,
                                  mineSize + INTER_PLACE_SPACE * 2, maxTries, regionId, 1))
    {
        if (world.can_build_firm(locX1 + INTER_PLACE_SPACE, locY1 + INTER_PLACE_SPACE, FIRM_MINE))
        {
            ...
            return add_site(locX1 + INTER_PLACE_SPACE, locY1 + INTER_PLACE_SPACE, SITE_RAW, rawId,
...
void SiteArray::ai_get_site_object()
{
    ...
    for (int i = 1; i <= size(); i++)
    {
        ...
        if (sitePtr->site_type == SITE_SCROLL || sitePtr->site_type == SITE_GOLD_COIN ||
            sitePtr->site_type == SITE_ITEM || sitePtr->site_type == SITE_WEAPON_BLUEPRINT)
        {
            sitePtr->ai_get_site_object();
        }
    }
...
#define MAX_UNIT_TO_ORDER 5
    ...
    int notifyGetRange =
        10 + (info.game_date - info.game_start_date) / 60; // it takes over 3 years to reach the maximum range 30
    ...
    if (firm_array.selected_recno)
    {
        ...
        if (firmPtr->firm_id == FIRM_MINE || firmPtr->firm_id == FIRM_ALCHEMY)
        {
            ...
            for (int count = 1; count <= arraySize; ++count)
            {
                ...
                if (sitePtr->map_x_loc >= x1 && sitePtr->map_x_loc <= x2 && sitePtr->map_y_loc >= y1 &&
                    sitePtr->map_y_loc <= y2)
                {
                    i = count;
                    break;
                }
