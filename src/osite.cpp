...
#define EXIST_RAW_RESERVE_QTY                                                  \
  (MAX_RAW_RESERVE_QTY / 20) // only sites with reserve qty >= 5% of
                             // MAX_RAW_RESERVE_QTY are counted as raw sites

//--------- Begin of function SiteArray::SiteArray -------//

SiteArray::SiteArray()
    : DynArrayB(sizeof(Site), 50, DEFAULT_REUSE_INTERVAL_DAYS) {}
//----------- End of function SiteArray::SiteArray -------//

//--------- Begin of function SiteArray::~SiteArray -------//

SiteArray::~SiteArray() { deinit(); }
//----------- End of function SiteArray::~SiteArray -------//
...
//
int SiteArray::add_site(int xLoc, int yLoc, int siteType, int objectId,
                        int reserveQty) {
...
#define ADD_SITE_RANGE 5
    ...
    for (int i = 1; i < ADD_SITE_RANGE * ADD_SITE_RANGE; i++) {
      misc.cal_move_around_a_point(i, ADD_SITE_RANGE, ADD_SITE_RANGE, xOffset,
                                   yOffset);
  ...
  if (sizeof(locPtr->extra_para) == sizeof(unsigned char) && recno() > 0xff) {
    linkout();
    return 0;
  }
  // #### end Gilbert 1/2 ######//
...
void SiteArray::del_site(int siteRecno) {
  err_if(siteRecno == 0) err_now("SiteArray::del_site");
...
//							  (if this is not given, it will use
//the existing std_raw_site_count)
//
void SiteArray::generate_raw_site(int rawGenCount) {
  if (rawGenCount)
    std_raw_site_count = rawGenCount; // use this number for determing whether
                                      // new sites should emerge in the future

#define MAX_RAW_REGION 3 // maximum no. of regions that has raw sites
#define SMALLEST_RAW_REGION                                                    \
  50 // only put raw on the region if its size is larger than this
#define REGION_SIZE_PER_RAW 100
  ...
  for (validRegionCount = 0; validRegionCount < regionCount;
       validRegionCount++) {
  ...
  if (validRegionCount ==
      0) // valid regions are those that are big enough to put raw sites
    return;
  ...
  err_when(validRegionCount > region_array.region_info_count ||
           validRegionCount > MAX_RAW_REGION);

  for (int k = 0; k < 10;
       k++) //	one loop may not be enough to generate all raw sites, have more
            //loops to make sure all are generated
  {
    for (i = 0; i < regionCount; i++) {
      ...
      for (j = regionRawCountArray[regionInfo->region_id - 1]; j < createCount;
           j++) // if currently there are already some, don't create new ones
      {
...
int SiteArray::create_raw_site(int regionId, int townRecno) {
  ...
  for (i = 0; i < MAX_RAW; i++) {
    ...
    if (rawCountArray[rawId - 1] ==
        minCount) // don't use this raw type unless it is one of the less
                  // available ones.
      break;
  }
  ...
  if (townRecno) {
    // ####### begin Gilbert 26/5 ########//
    //		#define MAX_TOWN_SITE_DISTANCE
    //(MAX_LINKED_FIRM_TOWN-STD_TOWN_LOC_WIDTH)
    const int maxTownSiteDistance = world.effective_distance(FIRM_MINE, 0);

    Town *townPtr = town_array[townRecno];

    locX1 = townPtr->center_x - maxTownSiteDistance -
            (siteWidth - 1) / 2; // (siteWidth-1)/2 is the distance from mine
                                 // center_x to mine loc_x1
  ...
  if (world.locate_space_random(
          locX1, locY1, locX2, locY2, mineSize + INTER_PLACE_SPACE * 2,
          mineSize + INTER_PLACE_SPACE * 2, maxTries, regionId, 1)) {
    if (world.can_build_firm(locX1 + INTER_PLACE_SPACE,
                             locY1 + INTER_PLACE_SPACE, FIRM_MINE)) {
      int reserveQty = MAX_RAW_RESERVE_QTY * (60 + misc.random(40)) / 100;
      return add_site(locX1 + INTER_PLACE_SPACE, locY1 + INTER_PLACE_SPACE,
                      SITE_RAW, rawId,
                      reserveQty); // xLoc+1 & yLoc+1 as the located size is
                                   // 3x3, the raw site is at the center of it
    }
  }
...
int SiteArray::scan_site(int xLoc, int yLoc, int siteType) {
  ...
  for (int i = site_array.size(); i > 0; i--) {
    ...
    if (siteType == 0 || sitePtr->site_type == siteType) {
      siteDis = misc.points_distance(xLoc, yLoc, sitePtr->map_x_loc,
                                     sitePtr->map_y_loc);
...
void SiteArray::next_day() {
  if (info.game_date % 30 == 0) {
    generate_raw_site(); // check if we need to generate existing raw sites are
                         // being used up and if we need to generate new ones
  }
...
#define MAX_UNIT_TO_ORDER 5
  ...
  int notifyGetRange =
      10 + (info.game_date - info.game_start_date) /
               60; // it takes over 3 years to reach the maximum range 30
  ...
  for (int i = 2; i < notifyGetRange * notifyGetRange; i++) {
    misc.cal_move_around_a_point(i, notifyGetRange, notifyGetRange, xOffset,
                                 yOffset);
    ...
    if (!unitPtr->race_id ||
        unitPtr->is_civilian()) // only military units of humans or monsters
      continue;
...
void SiteArray::go_to_a_raw_site() {
  ...
  for (j = 1; j <= arraySize; j++) {
    ...
    if (!sys.debug_session) // in debug session, browse through all types of
                            // sites
    {
      if (sitePtr->site_type != SITE_RAW)
        continue;
    ...
    if (!sitePtr->has_mine) {
      if (world.is_explored_quick(sitePtr->map_x_loc, sitePtr->map_y_loc,
                                  sitePtr->map_x2, sitePtr->map_y2)) {
        world.go_loc(sitePtr->map_x_loc, sitePtr->map_y_loc,
                     GO_LOC_SELECT_SITE); // 1-select the object on the location
        return;
      }
    }
  }
  ...
  for (j = 1; j <= arraySize; j++) {
    ...
    if (world.get_loc(sitePtr->map_x_loc, sitePtr->map_y_loc)->explored()) {
      world.go_loc(sitePtr->map_x_loc, sitePtr->map_y_loc,
                   1); // 1-select the object on the location
      return;
    }
  }
}
...
int Site::get_site_object(int unitRecno) {
  ...
  if (site_type == SITE_SCROLL) {
    if (god_res[object_id]->race_id == unitPtr->race_id) {
      ...
      news_array.scroll_acquired(unitPtr->nation_recno,
                                 god_res[object_id]->race_id);
    }
  }
  ...
  if (site_type == SITE_GOLD_COIN) {
    ...
    if (nation_array[unitPtr->nation_recno]->is_monster())
      nation_array[unitPtr->nation_recno]->change_live_points(
          (float)reserve_qty);
    // ##### end Gilbert 15/7 ########//
    objectTaken = 1;
  ...
  if (site_type == SITE_WEAPON_BLUEPRINT) {
    if (unitPtr->nation_recno) {
      if (tech_res[object_id]->get_nation_tech_level(unitPtr->nation_recno) ==
          0) {
...
int Site::is_stealth() {
  return config.blacken_map &&
         !world.is_explored_quick(map_x_loc, map_y_loc, map_x2, map_y2);
}
