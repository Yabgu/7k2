...
void FirmInn::put_info(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    button_buy_item.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 235,
                           INFO_X1 + 13 + BUTTON_DISTANCE + ITEM_ICON_WIDTH,
                           INFO_Y1 + 235 + ITEM_ICON_HEIGHT, disp_item_button,
  ...
  if (selected_unit_id > 0 && selected_unit_id <= inn_unit_count) {
    ...
    if ((button_buy_item.visible_flag = innUnit->item.id != 0)) {
      button_buy_item.enable_flag =
          is_own() && buy_item(selected_unit_id, 1); // checking only
  ...
  if( refreshFlag == INFO_REPAINT )
  {
          browse_hire.init( HIRE_BROWSE_X1, HIRE_BROWSE_Y1, HIRE_BROWSE_X2,
HIRE_BROWSE_Y2, 0, 25, inn_unit_count, put_hire_rec );
...
repaint the whole area as the detail area needs to disappear
                  {
...
//	disp_spy_button(INFO_X1+BUTTON_ACTION_WIDTH, HIRE_DET_Y2+4,
refreshFlag);
  */
}
...
void FirmInn::detect_info() {
  ...
  // if one is not visible, do not call detect() to avoid incorrect help text
  // displayed

  if (button_hire_visible && button_hire.detect() && selected_unit_id > 0 &&
      selected_unit_id <= inn_unit_count) {
    InnUnit *innUnit = inn_unit_array + selected_unit_id - 1;
    ...
    se_res.far_sound(center_x, center_y, 1, 'S',
                     unit_res[innUnit->unit_id]->sprite_id, "RDY");
    if (remote.is_enable()) {
      // packet structure : <firm recno>, <hire Id> <nation no> <spy escape>
      short *shortPtr =
          (short *)remote.new_send_queue_msg(MSG_F_INN_HIRE, 4 * sizeof(short));
    ...
    se_res.far_sound(
        center_x, center_y, 1, 'S',
        unit_res[inn_unit_array[selected_unit_id - 1].unit_id]->sprite_id,
        "RDY");

    err_when(
        spy_array.is_deleted(inn_unit_array[selected_unit_id - 1].spy_recno));
  ...
  if (selected_unit_id > 0 && selected_unit_id <= inn_unit_count) {
    ...
    if (button_buy_item.detect()) {
      ...
      if (!remote.is_enable()) {
        ...
        short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_INN_BUY_ITEM,
                                                             2 * sizeof(short));
      ...
      //			help.set_custom_help( button_buy_item.x1,
      //button_buy_item.y1, 				button_buy_item.x2, button_buy_item.y2, str,
  ...
  if( button_hire.detect() && inn_unit_count > 0 && browse_hire.recno() &&
  !browse_hire.none_record )
  {
          ...
          int spyEscape = innUnit->spy_recno
                  && spy_array[innUnit->spy_recno]->true_nation_recno ==
  nation_array.player_recno;
          ...
          if( is_own() || spyEscape )
          {
                  ...
                  if(remote.is_enable())
                  {
                          // packet structure : <firm recno>, <hire Id> <nation
  no> short *shortPtr=(short *)remote.new_send_queue_msg(MSG_F_INN_HIRE,
  4*sizeof(short)); shortPtr[0] = firm_recno; shortPtr[1] = browse_hire.recno();
                          shortPtr[2] = nation_recno;
...
void FirmInn::disp_unit_list(int dispY1, int refreshFlag) {
  ...
  for (int inc = -1; inc <= 1; inc += 2) {
    ...
    for (int i = inc >= 0 ? 1 : inn_unit_count; i > 0 && i <= inn_unit_count;
         i += inc) {
      InnUnit *innUnit = inn_unit_array + i - 1;

      int row =
          innUnit->spy_skill <= 0 ? 0 : 1; // soldier on row 0, spy on row 1
      // ##### begin Gilbert 7/10 ######//
      if (inc < 0)
      ...
      if (inc < 0) {
        ...
        info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, innUnit->unit_id,
                            nation_recno, windowX1, windowY1, windowX2,
                            windowY2, 1);
      } else {
        // second round is ascending to draw the frame

        if (info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, innUnit->unit_id,
                                nation_recno, windowX1, windowY1, windowX2,
                                windowY2,
                                inAreaFlag | (i == selected_unit_id ? 3 : 0)) &
            4) {
        ...
        {
          vga.active_buf->put_bitmap_trans(x + UNIT_X_SPACING / 2 - 8, yHp - 5,
                                           image_icon.read("U_SPY"));
        } else if (innUnit->hero_id) {
          vga.active_buf->put_bitmap_trans(x + UNIT_X_SPACING / 2 - 8, yHp - 5,
                                           image_icon.read("U_HERO"));
        } else {
          if (row == 0) // display combat skill and leadership
          {
            font_whbl.center_put(x, yHp, x + UNIT_X_SPACING, yHp + fontHeight,
                                 misc.format(innUnit->skill_level()));
          } else if (row == 1) // display spy skill
          {
            font_whbl.center_put(x, yHp, x + UNIT_X_SPACING, yHp + fontHeight,
                                 misc.format(innUnit->spy_skill));
          }
        }
        ...
        if (innUnit->item.id) {
          ...
          if (iconPtr)
            vga.active_buf->put_bitmap_trans(
                x + UNIT_X_SPACING / 2 - ((Bitmap *)iconPtr)->get_width() / 2,
                yHp + 53, iconPtr);
        }
      }
    }
...
void FirmInn::detect_unit_list() {
  ...
  for (int i = 1; i <= inn_unit_count; ++i) {
    ...
    int rc = info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, innUnit->unit_id,
                                 nation_recno, windowX1, windowY1, windowX2,
                                 windowY2, 8); // detect left button
...
//
// <int> 			 dispY1		 - the top y coordination of the info
// area <InnUnit*> hireInfoPtr - pointer to a HireInfo structure <int>
...
void FirmInn::disp_unit_info(int dispY1, int refreshFlag) {
  ...
  font_snds.right_put(INFO_X2 - 100, y,
                      misc.format(innUnit->combat_level(), 4));
  ...
  if (innUnit->item.id) {
    if (innUnit->spy_skill > 0) {
      ...
      font_snds.right_put(
          INFO_X2 - 10, y,
          misc.format(spy_array[innUnit->spy_recno]->spy_skill, 4));
    } else if (innUnit->hero_id) {
      //			String str;
      //			str  = race_res[
      //unit_res[innUnit->unit_id]->race_id ]->adjective; 			str += " Hero";
      //			font_red.right_put( INFO_X2-5, y, str );
      font_red.right_put(
          INFO_X2 - 5, y,
          text_unit.str_race_hero(
              race_res[unit_res[innUnit->unit_id]->race_id]->adjective));
    }
  }
  ...
  if (innUnit->skill_level() > 0) {
    ...
    font_snds.right_put(INFO_X2 - 100, y,
                        misc.format(innUnit->skill_level(), 4));
...
static void disp_item_button(ButtonCustom *button, int) {
  ...
  char *bitmapPtr =
      item_res.item_icon(button->custom_para.value); // bitmap on interface area

  if (!button->enable_flag) {
    vga.active_buf->put_bitmap_remap(
        button->x1, button->y1, bitmapPtr,
        (short *)vga.vga_color_table->get_table(-MAX_BRIGHTNESS_ADJUST_DEGREE *
                                                3 / 4));
  } else {
