...
void FirmInn::detect_info()
{
    ...
    if (button_hire_visible && button_hire.detect() && selected_unit_id > 0 && selected_unit_id <= inn_unit_count)
    {
        InnUnit *innUnit = inn_unit_array + selected_unit_id - 1;

        se_ctrl.immediate_sound("TAXGRANT");

        se_res.far_sound(center_x, center_y, 1, 'S', unit_res[innUnit->unit_id]->sprite_id, "RDY");
        if (remote.is_enable())
        {
    ...
    else if (button_spy_mobilize_visible && button_spy_mobilize.detect() && selected_unit_id > 0 &&
             selected_unit_id <= inn_unit_count)
    {
        se_res.far_sound(center_x, center_y, 1, 'S', unit_res[inn_unit_array[selected_unit_id - 1].unit_id]->sprite_id,
                         "RDY");
    ...
    if (selected_unit_id > 0 && selected_unit_id <= inn_unit_count)
    {
        ...
        else if (innUnit->item.id)
        {
            ...
            // button_buy_item.y1, 				button_buy_item.x2,
            // button_buy_item.y2, str, innUnit->item.item_desc() );

            help.set_custom_help(button_buy_item.x1, button_buy_item.y1, button_buy_item.x2, button_buy_item.y2,
                                 text_firm.str_buy_item_help(&innUnit->item), innUnit->item.item_desc());
        }
    }
    ...
    for (int inc = -1; inc <= 1; inc += 2)
    {
        ...
        for (int i = inc >= 0 ? 1 : inn_unit_count; i > 0 && i <= inn_unit_count; i += inc)
        {
            ...
            if (inc < 0)
            {
                ...
                info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, innUnit->unit_id, nation_recno, windowX1, windowY1,
                                    windowX2, windowY2, 1);
            }
            else
            {
                ...
                if (info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, innUnit->unit_id, nation_recno, windowX1, windowY1,
                                        windowX2, windowY2, inAreaFlag | (i == selected_unit_id ? 3 : 0)) &
                    4)
                {
...
void FirmInn::detect_unit_list()
{
    ...
    for (int i = 1; i <= inn_unit_count; ++i)
    {
        ...
        int rc = info.draw_unit_icon(x + UNIT_X_SPACING / 2, y, innUnit->unit_id, nation_recno, windowX1, windowY1,
                                     windowX2, windowY2,
...
//
// <int> 			 dispY1		 - the top y coordination of the
// info area <InnUnit*> hireInfoPtr - pointer to a HireInfo structure <int>
...
void FirmInn::disp_unit_info(int dispY1, int refreshFlag)
{
    ...
    if (innUnit->item.id)
    {
        ...
        else if (innUnit->hero_id)
        {
            //			String str;
            //			str  = race_res[
            // unit_res[innUnit->unit_id]->race_id ]->adjective;
            // str += " Hero";
            //			font_red.right_put( INFO_X2-5, y, str );
            font_red.right_put(INFO_X2 - 5, y,
                               text_unit.str_race_hero(race_res[unit_res[innUnit->unit_id]->race_id]->adjective));
        }
    }
    ...
    else
    {
        ...
        else if (innUnit->hero_id)
        {
            ...
            font_red.right_put(INFO_X2 - 95, y,
                               text_unit.str_race_hero(race_res[unit_res[innUnit->unit_id]->race_id]->adjective));
        }
    }
