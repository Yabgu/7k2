...
static int select_option2() {
    ...
    while (1) {
      ...
      if (refreshFlag) {
        ...
        if (optionMode != OPTION_SPECIES && refreshFlag & SGOPTION_DIFFICULTY) {
          ...
          font_bold_black.center_put(
              40, 105, 82, 125,
              misc.format(tempConfig.single_player_difficulty(0)), 1);
        }
      ...
      if (optionMode == OPTION_SPECIES) {
                ...
                0, 0, 0, 1)) // detect pushed button, but suspend pop
        {
          ...
          refreshFlag = SGOPTION_ALL;
                       ...
                       SCROLL_SHEET_Y1, SCROLL_SHEET_X2, SCROLL_SHEET_Y2)) {
          if (tempConfig.race_id >= 0)
            tempConfig.race_id = -1; // change to the first monster race
          optionMode = OPTION_BASIC; // auto change to basic mode
          refreshFlag = SGOPTION_ALL;
      ...
      // ------ detect basic option --------//
        ...
        if (tempConfig.race_id >= 0 && raceGroup.detect() >= 0) {
          ...
          tempConfig.race_id =
              monsterRaceGroup[monsterRaceGroup()].custom_para.value;
          refreshFlag |= SGOPTION_RACE;
        } else if (colorGroup.detect() >= 0) {
          tempConfig.player_nation_color =
              colorGroup[colorGroup()].custom_para.value;
  ...
  if (retFlag) {
    ...
    tempConfig.monster_difficulty_rating =
        tempConfig.single_player_difficulty(-1);
    config = tempConfig;
  }
