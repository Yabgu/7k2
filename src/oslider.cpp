...
void Slider::refresh() {
  ...
  if (barWidth > 0) {
    if (std_value) // if standard value is set, than use different color
    {
      ...
      Vga::active_buf->bar(slider_x1 + 2, slider_y1 + 2,
                           slider_x1 + 1 + MIN(barWidth, stdBarWidth),
      ...
      if (barWidth > stdBarWidth) {
        ...
        Vga::active_buf->bar(slider_x1 + 2 + stdBarWidth, slider_y1 + 2,
                             slider_x1 + 1 + barWidth, slider_y2 - 2,
                             bar_color2);
      }
    } else {
      ...
      Vga::active_buf->bar(slider_x1 + 2, slider_y1 + 2,
                           slider_x1 + 1 + barWidth, slider_y2 - 2, bar_color);
    }
  }
  ...
  if (barWidth < bar_width) {
    ...
    Vga::active_buf->bar(slider_x1 + 2 + barWidth, slider_y1 + 2, slider_x2 - 2,
                         slider_y2 - 2, Vga::active_buf->color_down);
  }
