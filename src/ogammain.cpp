...
void Game::disp_version() {
  ...
  //	#endif
  //	#ifdef BETA
  //		str = "This is a Beta version. Unauthorized distribution of this
  //Beta is strictly prohibited."; 	#endif

#if (defined(BETA))
  //	str = "This is a Beta version. Unauthorized distribution of this Beta is
  //strictly prohibited.";
...
//						 progress.
//      				 (pass 0 for starting the generating
//      game process.
//
void Game::disp_gen_game_status(int addStep) {
  ...
  const int POPUP_WINDOW_Y1 =
      (vga_front.buf_height() - POPUP_WINDOW_HEIGHT) / 2;
  ...
  if (vga_front.is_inited()) {
    if (game.is_campaign_mode()) {
      ...
      vga.active_buf->read_bitmapW(BAR_X1, BAR_Y1, BAR_X1 + size - 1,
                                   BAR_Y1 + 16 - 1, progressBitmap);
      ...
      ColorTable colorTable;
      {
        ...
        }
        {
          ...
          colorTable.generate_table_fast(0, palBufDesc,
                                         ColorTable::bright_func);
        }
        memcpy(colorRemapTable, colorTable.get_table(0),
               sizeof(*colorRemapTable) * 0x100);
      }
      ...
      vga.active_buf->put_bitmap_trans_remap(POPUP_WINDOW_X1, POPUP_WINDOW_Y1,
                                             backgroundBitmap, colorRemapTable);
      ...
      if (size > 237)
        size = 237;
      if (size > 0) {
        vga.active_buf->put_bitmap_area_trans(BAR_X1, BAR_Y1,
                                              image_menu.read("PROGRESS"), 0, 0,
                                              size - 1, 16 - 1);
      }
#else
      char fileName[] = "NW_00";
...

  {
    ...
    while (1) {
      ...
      if (sys.need_redraw_flag || misc.get_time() - lastRedrawTime > 8000) {
        ...
        sys.need_redraw_flag = 0;
        lastRedrawTime =
            misc.get_time(); // redraw every 8 sec. Such that if background
                             // erased at the beginning, redraw
      }
      ...
      if (refreshFlag) {
        if (refreshFlag & MMOPTION_PAGE) {
          ...
          //	font_zoom.right_put( 755, 62, "(c) Copyright 1999 by Enlight
          //Software and Trevor Chan" );
          ...
          font_thin_black.center_put_paragraph(BUTTON8_X1, BUTTON8_Y1,
                                               BUTTON8_X2, BUTTON8_Y2,
        ...
        if (refreshFlag & MMOPTION_RESOLUTION) {
          ...
          resStr = DisplayModeInfo::get_display_info(config.display_mode_id)
                       ->screen_width;
          resStr += "X";
          resStr += DisplayModeInfo::get_display_info(config.display_mode_id)
                        ->screen_height;
          ...
          font_thin_black.center_put(BUTTON9_X1, BUTTON9_Y1, BUTTON9_X2,
                                     BUTTON9_Y2, resStr);
      ...
      if (auto_load_file_name) {
        int rc = game_file.load_game(
            NULL, auto_load_file_name); // auto_load_file_name has the full
                                        // path, no need to give path
        ...
        }
        {
          ...
          game.deinit(); // game.deinit() is needed if game_file_array.menu
                         // fails
      ...
      if (emptyProfileTesting) // do this testing only once
      {
        ...
        if (PlayerProfile::load_count_profiles(NULL, 1) ==
            0) // maxLoad=1 for fast testing
        {
          box.msg(text_game_menu.str_first_welcome());

          // run_main_menu_option(0);			don't call run_main_menu_option,
          // we can't get it detect new profile
          if (player_profile.register_menu() ==
                  1 // return 1 = new profile created
              &&
              player_profile.is_registered() // check is_registered for safety
              &&
              box.ask(
                  text_game_menu.str_first_training())) // ask continue training
          {
            // ##### patch begin Gilbert 4/10 #####//
            game_file_array.init(
                player_profile.save_game_path(NULL),
                "*.SAV"); // necessary to set the path and extension
      ...
      if (player_profile.is_registered() && optionFlag[0] > 0 &&
        ...
        refreshFlag = MMOPTION_ALL;
                 ...
                 mouse.single_click(BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2,
                                    BUTTON4_Y2)) {
        run_main_menu_option(4); // hall of fame
        refreshFlag = MMOPTION_ALL;
      } else if (optionFlag[4] > 0 &&
                 mouse.single_click(BUTTON5_X1, BUTTON5_Y1, BUTTON5_X2,
                                    BUTTON5_Y2)) {
        run_main_menu_option(5); // credits
        refreshFlag = MMOPTION_ALL;
      } else if (mouse.single_click(BUTTON6_X1, BUTTON6_Y1, BUTTON6_X2,
                                    BUTTON6_Y2)) {
        run_main_menu_option(0); // profile
        refreshFlag = MMOPTION_ALL;
      } else if (mouse.single_click(BUTTON8_X1, BUTTON8_Y1, BUTTON8_X2,
                                    BUTTON8_Y2)) {
        // quit
        break;
      ...
      } else if (mouse.single_click(BUTTONA_X1, BUTTONA_Y1, BUTTONA_X2,
                                    BUTTONA_Y2)) {
...
void Game::single_player_menu() {
...

  {
    ...
    while (1) {
      ...
      // detect new campaign
      else if (optionFlag[1] && mouse.single_click(BUTTON2_X1, BUTTON2_Y1,
      ...
      else if (optionFlag[2] && mouse.single_click(BUTTON3_X1, BUTTON3_Y1,
      ...
      // detect load game
      else if (optionFlag[3] && mouse.single_click(BUTTON4_X1, BUTTON4_Y1,
      ...
      // run scenario
      else if (optionFlag[4] && mouse.single_click(BUTTON5_X1, BUTTON5_Y1,
                                                   BUTTON5_X2, BUTTON5_Y2)) {
        game_file_array.init(
            player_profile.save_game_path(NULL),
            "*.SAV"); // necessary to set the path and extension
...

  {
    ...
    while (1) {
      ...
      if (mouse.single_click(BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2)) {
        game_file_array.init(
            DIR_SCENARIO, "*.SCN"); // necessary to set the path and extension
  ...
  if (mp_obj.is_initialized() && mp_obj.is_lobbied()) {
    ...
    if (lobbiedName && player_profile.load_by_login_name(lobbiedName)) {
      if (player_profile.mp_new_game_flag == 1) // new game
      {
        ...
        game_file_array.init(
            player_profile.save_game_path(NULL),
            "*.SVM"); // necessary to set the path and extension
      ...
      } else if (player_profile.mp_new_game_flag == 2) // load game
      {
        ...
        if (loadFileName[0] &&
            game_file.load_game(player_profile.save_game_path(NULL),
                                loadFileName)) {
          ...
          if (game_file_array.menu(3, &loadedRecno) == 1) {
            ...
            load_mp_game(game_file_array[loadedRecno]->file_name, lobbied,
                         game_host);
          }
        }
        {
    ...
    while (1) {
      ...
      if (refreshFlag) {
        if (refreshFlag & SPOPTION_PAGE) {
          ...
          font_thin_black.center_put_paragraph(
              ...
              !launchMode ? text_game_menu.str_cancel()
                          : text_game_menu.str_quit(),
              0);
      ...
      // detect new game
        ...
        if (launchMode && !sys.signal_exit_flag) {
          ...
          // signal_exit_flag is zero usually means exit from multiplayer setup
          // menu
          refreshFlag = SPOPTION_ALL;
          continue;
        }
      ...
      // detect load game
        ...
        }
        {
          ...
          if (launchMode && !sys.signal_exit_flag) {
            // if exit from main game, signal_exit_flag is non-zero
            // signal_exit_flag is zero usually means exit from multiplayer
            // setup menu
            refreshFlag = SPOPTION_ALL;
            continue;
          }
