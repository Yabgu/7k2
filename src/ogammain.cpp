    ...
    if (vga_front.is_inited())
    {
        ...
        else
        {
            ...
            // put background bitmap
            {
                ...
                str += "NW.ICN";
                imageFile.file_open(str);
                vga.active_buf->put_large_bitmap_trans(POPUP_WINDOW_X1, POPUP_WINDOW_Y1, &imageFile, colorRemapTable);
            }
            ...
            if (size > 0)
            {
                vga.active_buf->put_bitmap_area_trans(BAR_X1, BAR_Y1, image_menu.read("PROGRESS"), 0, 0, size - 1,
                                                      16 - 1);
            }
#else
            char fileName[] = "NW_00";
...

    {
        ...
        while (1)
        {
            ...
            if (sys.need_redraw_flag || misc.get_time() - lastRedrawTime > 8000)
            {
                ...
                sys.need_redraw_flag = 0;
                lastRedrawTime = misc.get_time(); // redraw every 8 sec. Such that if background
                ...
                if (refreshFlag & MMOPTION_PAGE)
                {
                    ...
                    if (player_profile.is_registered())
                    {
                        if (optionFlag[0] > 0) // single player
                            font_thin_black.center_put_paragraph(BUTTON1_X1, BUTTON1_Y1, BUTTON1_X2, BUTTON1_Y2,
                                                                 text_game_menu.str_single_player(), 0);
                        if (optionFlag[1] > 0) // multi player
                            font_thin_black.center_put_paragraph(BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2,
                                                                 text_game_menu.str_multi_player(), 0);
                        if (optionFlag[2] > 0) // scenario editor
                            font_thin_black.center_put_paragraph(BUTTON3_X1, BUTTON3_Y1, BUTTON3_X2, BUTTON3_Y2,
                    ...
                    if (optionFlag[4] > 0)
                        font_thin_black.center_put_paragraph(BUTTON5_X1, BUTTON5_Y1, BUTTON5_X2, BUTTON5_Y2,
                                                             text_game_menu.str_credits(), 0);
                    font_thin_black.center_put_paragraph(BUTTON6_X1, BUTTON6_Y1, BUTTON6_X2, BUTTON6_Y2,
                    ...
                    font_thin_black.center_put_paragraph(BUTTONA_X1, BUTTONA_Y1, BUTTONA_X2, BUTTONA_Y2,
            ...
            if (auto_load_file_name)
            {
                int rc = game_file.load_game(NULL, auto_load_file_name); // auto_load_file_name has the full
            ...
            if (emptyProfileTesting) // do this testing only once
            {
                ...
                if (PlayerProfile::load_count_profiles(NULL, 1) == 0) // maxLoad=1 for fast testing
                {
                    ...
                    // run_main_menu_option(0);			don't call
                    // run_main_menu_option, we can't get it detect new profile
                        ...
                        && box.ask(text_game_menu.str_first_training())) // ask continue training
                    {
                        ...
                        game_file_array.init(player_profile.save_game_path(NULL),
            ...
            else if (optionFlag[3] > 0 && mouse.single_click(BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2, BUTTON4_Y2))
            {
                run_main_menu_option(4); // hall of fame
                refreshFlag = MMOPTION_ALL;
            }
            else if (optionFlag[4] > 0 && mouse.single_click(BUTTON5_X1, BUTTON5_Y1, BUTTON5_X2, BUTTON5_Y2))
            {
...
void Game::multi_player_menu(int lobbied, char *game_host)
{
    ...
    if (mp_obj.is_initialized() && mp_obj.is_lobbied())
    {
        ...
        if (lobbiedName && player_profile.load_by_login_name(lobbiedName))
        {
            ...
            else if (player_profile.mp_new_game_flag == 2) // load game
            {
                ...
                if (loadFileName[0] && game_file.load_game(player_profile.save_game_path(NULL), loadFileName))
                {
...

    {
        ...
        while (1)
        {
            ...
            if (refreshFlag)
            {
                if (refreshFlag & SPOPTION_PAGE)
                {
                    ...
                    font_thin_black.center_put_paragraph(
                        ...
                        !launchMode ? text_game_menu.str_cancel() : text_game_menu.str_quit(), 0);
            ...
            if (launchMode && mouse.single_click(BUTTON6_X1, BUTTON6_Y1, BUTTON6_X2, BUTTON6_Y2))
            {
                player_profile.register_menu();
                if (player_profile.is_registered())
                {
