...
VLenQueue::VLenQueue()
    : queue_buf(NULL), queue_buf_size(0), queued_size(0), queue_ptr(NULL) {}
...
VLenQueue::VLenQueue(VLenQueue &q)
    : queue_buf(mem_add(q.queue_buf_size)), queue_buf_size(q.queue_buf_size) {
...
int VLenQueue::length() { return queued_size; }
