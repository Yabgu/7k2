...
{
    int firmRecno =
        build_firm(xLoc, yLoc, nationRecno, firmId, buildCode, 0, false, true); // true-no cost for building the firm
...
void FirmArray::draw_dot(int filterFirmId)
{
    ...
    for (i = 0; i <= MAX_NATION; ++i)
    {
        ...
        {
            excitedColorArray[i] = slowExcitedColorArray[i] = vga_back.translate_color(V_WHITE);
        }
    }
    ...
    for (i = 1; i <= size(); i++)
    {
        ...
        { // minimize register variable at one time
            ...
            else
            {
                ...
                Unit *unitPtr;
                if (unit_array.selected_recno && (unitPtr = unit_array[unit_array.selected_recno]) &&
            ...
            rowWritePtr = vga_back.buf_ptr(world.map_matrix->calc_loc_pos_x(firmPtr->loc_x1, firmPtr->loc_y1) + MAP_X1,
...
void FirmArray::upgrade_firm(int oldFirmRecno, int newFirmId)
{
    ...
    oldFirmBackup = (Firm *)mem_add(oldFirmSize); // use mem_add() instead of new new bypass
    ...
    disable_reuse_interval();                // temporarily disable reuse interval checking new
                                             ...
                                             // deleted camp.
    base_obj_array.disable_reuse_interval(); // temporarily disable reuse interval checking
                                             // new let the new fort use the same recno as
                                             // the deleted camp.

    int newFirmRecno = build_firm(oldFirmBackup->loc_x1, oldFirmBackup->loc_y1, oldFirmBackup->nation_recno, newFirmId,
    ...
    oldFirmBackup->firm_id = newFirm->firm_id; // as we copy firmBackUp to newFirm, we need to preserve
                                               // some vars
    ...
    memcpy((char *)newFirm + sizeof(void *), (char *)oldFirmBackup + sizeof(void *), oldFirmSize - sizeof(void *));
