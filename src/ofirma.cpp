...
//
FirmArray::FirmArray()
    : DynArrayB(sizeof(Firm *), 50, DEFAULT_REUSE_INTERVAL_DAYS) {}
//----------- End of function FirmArray::FirmArray ---------//

//--------- Begin of function FirmArray::~FirmArray ---------//
//
FirmArray::~FirmArray() { deinit(); }
//----------- End of function FirmArray::~FirmArray ---------//

//--------- Begin of function FirmArray::init ---------//

void FirmArray::init() { process_recno = 1; }
...
// just have one build type
//
// Return : <int> the record no. of the newly added firm
//
int FirmArray::generate_firm(int xLoc, int yLoc, int nationRecno, int firmId,
                             char *buildCode) {
  int firmRecno = build_firm(xLoc, yLoc, nationRecno, firmId, buildCode, 0,
                             false, true); // true-no cost for building the firm
...
// Return : <int> the record no. of the newly added firm
//
int FirmArray::build_firm(int xLoc, int yLoc, int nationRecno, int firmId,
                          char *buildCode, short builderRecno, bool isUpgrade,
                          bool noCost) {
  ...
  firm_array[firmRecno]->init(xLoc, yLoc, nationRecno, firmId, buildCode,
                              builderRecno);

  // Firm::init() will set world matrix, it will use Firm::firm_recno to set the
  // location cargo
  ...
  if (nationRecno && !isUpgrade && !noCost) {
    ...
    nationPtr->add_expense(
        ...
        (float)firmInfo->setup_cost); // setup cost of the firm
...
int FirmArray::create_firm(int firmId) {
  ...
  //	static char firmAnimalTrainArray[] = { UNIT_JAGUAR, UNIT_GORILLA,
  //UNIT_RHINO, 0 };
  static char firmIncubatorTrainArray[] = {UNIT_BALLISTA, UNIT_CATAPULT,
                                           UNIT_CANNON, 0};
...
int FirmArray::process() {
  ...
  if (sys.day_frame_count == 0) {
    god_res
        .update_prayer_count(); // count no. of prayer of each god, each nation
  }
  // ####### end Gilbert 19/11 ######//
  ...
  for (i = 1; i <= size(); i++) {
            ...
            FRAMES_PER_DAY)) // only process each firm once per day
    {
      ...
      //			if( firmPtr->explore_for_player() )
      //			{
      //				world.visit( firmPtr->loc_x1,
      //firmPtr->loc_y1, firmPtr->loc_x2, firmPtr->loc_y2, EXPLORE_RANGE-1 );
      //			}
...
void FirmArray::draw_dot(int filterFirmId) {
  ...
  for (int n = 0; n < MAX_NATION + 1; ++n)
    nationColorArray[n] =
        vga_back.translate_color(nation_array.nation_color_array[n]);
  ...
  for (i = 0; i <= MAX_NATION; ++i) {
    if (i == 0 || !nation_array.is_deleted(i)) {
      ...
      excitedColorArray[i] = slowExcitedColorArray[i] =
          vga_back.translate_color(V_WHITE);
    }
  }
  ...
  for (i = 1; i <= size(); i++) {
    ...
    { // minimize register variable at one time
      ...
      if (firm_array.selected_recno !=
          i) // if the firm is currently selected, ignore all filter
      {
        //--------- check filter ---------//

        if (filterFirmId) {
          if (filterFirmId == FIRM_FORT ||
              filterFirmId == FIRM_CAMP) // Camp and Fort are the same group
          {
        ...
        if (world.map_matrix->map_mode == MAP_MODE_TRADE &&
            !(firmPtr->cast_to_FirmMine() || firmPtr->cast_to_FirmFactory() ||
              firmPtr->cast_to_FirmMarket()))
          continue;

        // filter selecFirmMonsterTrain(UNIT_BEE);ted nation
        if (world.map_matrix->filter_nation_flag &&
            world.map_matrix->filter_nation_recno != firmPtr->nation_recno)
          continue;
      }
      ...
      if (world.map_matrix->map_mode != MAP_MODE_TRADE) {
        ...
        err_when(MAX_STOP_FOR_CARAVAN !=
                 2); // current code check only two stops
        ...
        // ----- if selected firm link to this firm, flash -------//
                 ...
                 firmPtr->get_linked_firm_status(firm_array.selected_recno) ==
                     LINK_EE) {
          nationColor = slowExcitedColorArray[firmPtr->nation_recno];
        }
        ...
        // ----- if selected town linked to this firm, flash ------//
                 ...
                 firmPtr->get_linked_town_status(town_array.selected_recno) ==
                     LINK_EE) {
          nationColor = slowExcitedColorArray[firmPtr->nation_recno];
        } else {
          nationColor = nationColorArray[firmPtr->nation_recno];
      ...
      rowWritePtr = vga_back.buf_ptr(
          world.map_matrix->calc_loc_pos_x(firmPtr->loc_x1, firmPtr->loc_y1) +
              MAP_X1,
          world.map_matrix->calc_loc_pos_y(firmPtr->loc_x1, firmPtr->loc_y1) +
              MAP_Y1);
    }
    ...
    { // minimize register variable at one time
      ...
      for (y = 0; y < firmHeight;
           (rowWritePtr += y & 1 ? vgaBufPitch : -1), ++y) {
...
Firm *FirmArray::operator()() {
  ...
  err_if(!firmPtr) err_now("FirmArray[recno()] is deleted");
...
void FirmArray::upgrade_firm(int oldFirmRecno, int newFirmId) {
  ...
  // ##### begin Gilbert 4/11 ######//
  int oldSelectedRecno =
      firm_array.selected_recno; // del_firm may clear firm_array.selected_recno
  // ##### end Gilbert 4/11 ######//
  ...
  oldFirmBackup =
      (Firm *)mem_add(oldFirmSize); // use mem_add() instead of new new bypass
                                    // calling construcnewr and destrucnewr

  memcpy(oldFirmBackup, oldFirm, oldFirmSize);

  //--- reset the content of the derived class before deinit it so that it won't
  //call deinit functions unnecessarily ---//
  ...
  disable_reuse_interval(); // temporarily disable reuse interval checking new
                            // let the new fort use the same recno as the
                            // deleted camp.
  base_obj_array
      .disable_reuse_interval(); // temporarily disable reuse interval checking
                                 // new let the new fort use the same recno as
                                 // the deleted camp.

  int newFirmRecno = build_firm(oldFirmBackup->loc_x1, oldFirmBackup->loc_y1,
                                oldFirmBackup->nation_recno, newFirmId,
                                race_res[oldFirmBackup->race_id]->code, 0,
                                1); // 1-upgrade firm
  ...
  oldFirmBackup->firm_id =
      newFirm->firm_id; // as we copy firmBackUp to newFirm, we need to preserve
                        // some vars
  ...
  memcpy((char *)newFirm + sizeof(void *),
         (char *)oldFirmBackup + sizeof(void *), oldFirmSize - sizeof(void *));
