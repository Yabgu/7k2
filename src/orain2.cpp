...
void Rain::start_rain(int x1, int y1, int x2, int y2, short density,
                      double slope) {
  bound_x1 = x1;
  bound_y1 = y1;
  bound_x2 = x2;
...
//
void Rain::stop_rain() { drop_per_turn = 0; }
...
void Rain::new_drops() {
  ...
  while (dropRemain > 0 && maxScan-- > 0) {
    ...
    if (!drop_flag[i]) {
      ...
      short height = (bound_y2 - bound_y1) / 8 +
                     rand_seed() % (((bound_y2 - bound_y1) * 7) / 8);
      short speed = height / 4;
      drop[i].init(this, fromX, (short)bound_y1,
                   short(fromX + height * wind_slope), short(bound_y1 + height),
                   speed);
      drop_flag[i] = 1;
      active_drop++;
      dropRemain--;
