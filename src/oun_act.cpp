...
void Unit::pre_process() {
  ...
  //--- if the current order has been reset and there is a pushed order, pop the
  //order ---//
  ...
  case UNIT_MOVE: // nothing to do with move mode, as UnitB already takes care
                  // of it
#ifdef DEBUG
    startTime = misc.get_time();
#endif
...
void Unit::stop_order() {
  ...
  if (cur_order.ai_action_id && nation_recno)
    nation_array[nation_recno]->action_failure(cur_order.ai_action_id,
                                               sprite_recno);
...
void Unit::completed_order() {
  ...
  if (cur_order.ai_action_id && nation_recno)
    nation_array[nation_recno]->action_finished(cur_order.ai_action_id,
                                                sprite_recno);
...
void Unit::push_order() {
  err_when(has_pushed_order() >
           0); // cannot push when the only one stack space is used

  pushed_order = cur_order;

  // ##### begin Gilbert 31/5 ########//
  // if the unit is stopped, store current location, used when resuming the
  // action
...
//
int Unit::has_pushed_order() { return pushed_order.mode >= 0; }
...
//
void Unit::clear_pushed_order() { pushed_order.set(-1); }
...
void UnitOrder::set(int newMode, int newPara, int newXLoc, int newYLoc) {
  ...
  //--- set name_id to Unit::name_id if the order is attacking a unit. This is
  //for verifying attack target ---//
...
//
// Return the loc_x2 of current destination specified by UnitOrder::loc_x1 &&
// loc_y1
//
...
//
// Return the loc_y2 of current destination specified by UnitOrder::loc_x1 &&
// loc_y1
//
...
//
int Unit::is_next_to_order_place() { return is_in_order_place(1); }
...
//					     in a normal call, this is not
//passed.
//
int Unit::is_in_order_place(int extraLoc) {
  if (cur_order.mode == UNIT_BUILD_FIRM || cur_order.mode == UNIT_SETTLE_TOWN) {
    // ###### begin Gilbert 4/11 ######//
    //		return next_x_loc() >= cur_order.loc_x-extraLoc &&
    //				 next_x_loc() <= cur_order.place_loc_x2()+extraLoc &&		// either
    //inside the building site or just next to it 				 next_y_loc() >=
    //cur_order.loc_y-extraLoc && 				 next_y_loc() <=
    //cur_order.place_loc_y2()+extraLoc;
    return misc.area_distance(obj_loc_x1(), obj_loc_y1(), obj_loc_x2(),
                              obj_loc_y2(), cur_order.loc_x, cur_order.loc_y,
                              cur_order.place_loc_x2(),
                              cur_order.place_loc_y2()) <= extraLoc;
    // ###### end Gilbert 4/11 ######//
    ...
    //&& 				 next_y_loc() >= baseObj->obj_loc_y1()-extraLoc && 				 next_y_loc() <=
    //baseObj->obj_loc_y2()+extraLoc;
  ...
  // ####### begin Gilbert 19/2 #######//
    ...
    return misc.area_distance(obj_loc_x1(), obj_loc_y1(), obj_loc_x2(),
                              obj_loc_y2(), cur_order.loc_x, cur_order.loc_y,
                              cur_order.loc_x, cur_order.loc_y) <= extraLoc;
  }
  // ####### end Gilbert 19/2 #######//
  else {
    err_here();
...
void Unit::left_click(int shiftPressed) {
  ...
  } else // if this unit is currently NOT selected
  {
    ...
    //--- if there is none selected or this unit has a higher rank than the
...
void Unit::gain_experience() {
  ...
  //	if( !race_id )
  //		return; 		// no experience gain if unit is a living
  //being
  ...
  if (!unitInfo->class_info
           .has_weapon_version // get_weapon_level() share nation_contribution
  ...
  if (unitInfo->class_info.has_combat_level) {
    ...
    err_when(skill.actual_combat_level() < 0 ||
             skill.actual_combat_level(NULL) > 1000);
  ...
  //--- if this is a soldier led by a commander, increase the leadership of its
  //commander -----//
  ...
  if (!unit_array.is_deleted(leader_unit_recno)) {
    ...
    if (leaderXLoc >= 0 &&
        misc.points_distance(cur_x_loc(), cur_y_loc(), leaderXLoc,
                             leaderYLoc) <= EFFECTIVE_LEADING_DISTANCE) {
      leaderUnit->skill.inc_skill_level(0.06f);

      //-- give additional increase if the leader has skill potential on
      //leadership --//
...
void Unit::set_die() {
  ...
  //---- if this unit is led by a leader, only mobile units has
  //leader_unit_recno assigned to a leader -----//
...
void Unit::sound(const char *soundCode) {
  if (se_res.mark_command_time()) {
    se_res.far_sound(next_x_loc(), next_y_loc(), 1, 'S', sprite_id, soundCode);
  }
}
...
int Unit::teleport(int targetXLoc, int targetYLoc) {
  ...
  if (world.check_unit_space(targetXLoc, targetYLoc, targetXLoc + loc_width - 1,
                             targetYLoc + loc_height - 1, mobile_type)) {
    stop_order();

    int dx, dy;
    for (dy = 0; dy < loc_width; ++dy)
      for (dx = 0; dx < loc_height; ++dx)
        world.get_loc(next_x_loc() + dx, next_y_loc() + dy)
            ->remove_unit(mobile_type);
    ...
    for (dy = 0; dy < loc_width; ++dy)
      for (dx = 0; dx < loc_height; ++dx)
        world.get_loc(next_x_loc() + dx, next_y_loc() + dy)
            ->set_unit(sprite_recno, mobile_type);
