...
int Unit::is_in_order_place(int extraLoc)
{
    if (cur_order.mode == UNIT_BUILD_FIRM || cur_order.mode == UNIT_SETTLE_TOWN)
    {
        ...
        //				 next_x_loc() <= cur_order.place_loc_x2()+extraLoc &&		//
        // either
        // inside the building site or just next to it 				 next_y_loc()
        // >= cur_order.loc_y-extraLoc && 				 next_y_loc() <=
        // cur_order.place_loc_y2()+extraLoc;
        return misc.area_distance(obj_loc_x1(), obj_loc_y1(), obj_loc_x2(), obj_loc_y2(), cur_order.loc_x,
                                  cur_order.loc_y, cur_order.place_loc_x2(), cur_order.place_loc_y2()) <= extraLoc;
        // ###### end Gilbert 4/11 ######//
    }
    ...
    {
        return misc.area_distance(obj_loc_x1(), obj_loc_y1(), obj_loc_x2(), obj_loc_y2(), cur_order.loc_x,
                                  cur_order.loc_y, cur_order.loc_x, cur_order.loc_y) <= extraLoc;
    }
    // ####### end Gilbert 19/2 #######//
    else
    {
...
void Unit::left_click(int shiftPressed)
{
    ...
    else // if this unit is currently NOT selected
    {
        ...
        if (unit_array.selected_recno == 0 || rank_id > unit_array[unit_array.selected_recno]->rank_id)
        {
...
void Unit::gain_experience()
{
    ...
    //	if( !race_id )
    //		return; 		// no experience gain if unit is a
    // living
    // being
...
{
    return cur_action == SPRITE_IDLE && cur_order.mode == UNIT_STOP && !has_pushed_order();
}
...
int Unit::teleport(int targetXLoc, int targetYLoc)
{
    ...
    if (world.check_unit_space(targetXLoc, targetYLoc, targetXLoc + loc_width - 1, targetYLoc + loc_height - 1,
                               mobile_type))
    {
        stop_order();
