...
static void pack_clusters(ClusterInfo *clusterArray, int clusterCount,
...
int UnitGroup::get_write_len() {
  const int memberSize =
      (sizeof(*this) - sizeof(DynArray) + sizeof(short) - 1) / sizeof(short);
...
void UnitGroup::write_mem(short *msg) {
  const int memberSize =
      (sizeof(*this) - sizeof(DynArray) + sizeof(*msg) - 1) / sizeof(*msg);
  memcpy(msg, ((char *)this) + sizeof(DynArray),
         sizeof(*this) - sizeof(DynArray));
  msg[memberSize] = size();

  err_when(ele_size != sizeof(short)); // parameter type of set(short *, int),
...
void UnitGroup::read_mem(short *msg) {
  ...
  // restore data member
  memcpy(((char *)this) + sizeof(DynArray), msg,
         sizeof(*this) - sizeof(DynArray));
}
...
// selectedUnitArray.
//
// <int> nationRecno				player nation recno or remote player
// nation recno return number of valid units
//
...
void UnitGroup::stop(char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_UNIT_STOP, get_write_len());
    write_mem(shortPtr);
    return;
  }
  // ###### end Gilbert 19/11 ######//
...
void UnitGroup::attack(int targetObjRecno, char remoteAction) {
  if (base_obj_array.is_deleted(
          targetObjRecno)) // may be deleted after multiplayer message delay
    return;

  //----- player can only order his own units to attack, cannot order his spies
  //cloaked in other nations to attack, as this can easy lead nations into wars
  //----//
...
void UnitGroup::exe_attack(int targetObjRecno) {
  ...
  */
  ...
  //--and counting how many units will do range attack and close attack
  //respectively---//
  /*
  for(int i=1; i<=size(); i++)
  {
          ...
          if(unitPtr->cur_action == SPRITE_ATTACK && unitPtr->cur_order.para ==
targetObjRecno)
          {
                  numberAttacking++;
                  continue;
          ...
          // ##### begin Gilbert 4/11 ######//
          // distance = unitPtr->cal_distance(targetLocX, targetLocY,
targetWidth, targetHeight); distance = unitPtr->area_distance(targetObj);
          // ##### end Gilbert 4/11 ######//
          unitPtr->choose_best_attack_mode(distance,
targetObj->obj_mobile_type()); if(unitPtr->obj_loc_width() > maxWidth) maxWidth
= unitPtr->obj_loc_width(); if(unitPtr->obj_loc_height() > maxHeight) maxHeight
= unitPtr->obj_loc_height(); if((unitPtr->attack_range() < minAttackRange) &&
(unitPtr->attack_range()>1)) minAttackRange = unitPtr->attack_range();
...
targetObjRecno) continue;
  ...
  if(rangeAttack)
  {
          ...
          int xLoc2 = MIN(targetLocX+targetWidth-1+minAttackRange,
MAX_WORLD_X_LOC-1); int yLoc2 = MIN(targetLocY+targetHeight-1+minAttackRange,
MAX_WORLD_Y_LOC-1); int checkXLoc, checkYLoc; unitIndex = 0; unitPtr =
arrayForRangeAttack[unitIndex];
          ...
          if(unitIndex < rangeAttack)
          {
                  for(checkYLoc=yLoc1+1; checkYLoc<yLoc2; checkYLoc++)
                  {
                          ...
                          {
                                  unitPtr->attack_loc_offset_x =
xLoc1-targetLocX; unitPtr->attack_loc_offset_y = checkYLoc-targetLocY;
                                  if(unitPtr->obj_loc_height() > 1)
                                          checkYLoc +=
(unitPtr->obj_loc_height()-1); if(unitIndex < rangeAttack-1) unitPtr =
arrayForRangeAttack[++unitIndex]; else break;
                          }
                  }
          }
          ...
          if(unitIndex < rangeAttack)
          {
                  for(checkXLoc=xLoc1; checkXLoc<=xLoc2; checkXLoc++)
                  {
                          ...
                          {
                                  unitPtr->attack_loc_offset_x =
checkXLoc-targetLocX; unitPtr->attack_loc_offset_y = yLoc2-targetLocY;
                                  if(unitPtr->obj_loc_width() > 1)
                                          checkXLoc+=
(unitPtr->obj_loc_width()-1); if(unitIndex < rangeAttack-1) unitPtr=
arrayForRangeAttack[++unitIndex]; else break;
                          }
                  }
          }
          ...
          if(unitIndex < rangeAttack)
          {
                  for(checkYLoc=yLoc1+1; checkYLoc<yLoc2; checkYLoc++)
                  {
                          ...
                          {
                                  unitPtr->attack_loc_offset_x =
xLoc2-targetLocX; unitPtr->attack_loc_offset_y = checkYLoc-targetLocY;
                                  if(unitPtr->obj_loc_height() > 1)
...
arrayForRangeAttack[++unitIndex]; else break;
          ...
          if(unitIndex < rangeAttack)
          {
                  ...
                  {
                          unitPtr = arrayForRangeAttack[unitIndex];
                          //--settting attack_loc_offset_? to 0 will make them
cannot attack --// unitPtr->attack_loc_offset_x = 0;
                  ...
                  for(; index<targetWidth; index++)
                  {
                          if(unitPtr->can_move(targetLocX+index, testLocY))
                          {
                                  ...
                                  if(unitIndex < closeAttack-1)
                                          unitPtr =
arrayForCloseAttack[++unitIndex]; //get_next_unit else break;
                          }
                  }
          }
          if((unitIndex < closeAttack) && ((testLocX = targetLocX + targetWidth)
<= MAX_WORLD_X_LOC-maxWidth)) //has right edge
          {
                  ...
                  for(; index<targetHeight; index++)
                  {
                          if(unitPtr->can_move(testLocX, targetLocY+index))
                          {
                                  ...
                                  if(unitPtr->obj_loc_height() > 1)
                                          index +=
(unitPtr->obj_loc_height()-1); if(unitIndex < closeAttack-1) unitPtr =
arrayForCloseAttack[++unitIndex]; //get next unit else break;
                          }
                  }
          }
          if((unitIndex < closeAttack) && ((testLocY = targetLocY +
targetHeight) <= MAX_WORLD_Y_LOC-maxHeight)) //has bottom edge
          {
                  w = unitPtr->obj_loc_width();
                  while(w>=0)
                  {
                          ...
                          {
                                  index =
targetWidth-(unitPtr->obj_loc_width()-w); break;
                          }
                          w--;
                  }
                  ...
                  for(; index>=0; index--)
                  {
                          if(unitPtr->can_move(targetLocX+index, testLocY))
                          {
                                  ...
                                  {
                                          unitPtr =
arrayForCloseAttack[++unitIndex]; if(unitPtr->obj_loc_width() > 1) index -=
(unitPtr->obj_loc_width()-1);
                                  }
                                  else
                                          break;
                          }
          ...
          if(unitIndex < closeAttack && ((testLocX = targetLocX-maxWidth) >=0 ))
//has left edge
          {
                  ...
                  while(h>=0)
                  {
                          ...
                          {
                                  index =
targetHeight-(unitPtr->obj_loc_height()-h); break;
                          }
                          h--;
                  }
                  ...
                  for(; index>=0; index--)
                  {
                          if(unitPtr->can_move(testLocX, targetLocY+index))
                          {
                                  ...
                                  {
                                          unitPtr =
arrayForCloseAttack[++unitIndex]; if(unitPtr->obj_loc_height() > 1) index -=
(unitPtr->obj_loc_height()-1);
                                  }
                                  else
                                          break;
                          }
...
-----// if(unitIndex < closeAttack)
  ...
  */
  cluster_units_for_attack(targetObjRecno, targetLocX, targetLocY, targetWidth,
                           targetHeight);
  /*
...
//
void UnitGroup::move(int destXLoc, int destYLoc, bool forceMoveFlag,
                     char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <xLoc> <yLoc> <forceMoveFlag> <write_mem...>
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_UNIT_MOVE, sizeof(short) * 3 + get_write_len());
    shortPtr[0] = destXLoc;
    shortPtr[1] = destYLoc;
...
//									  (default: nation_recno
//of the unit)
//
void UnitGroup::assign(int destXLoc, int destYLoc, int targetMobileType,
                       char remoteAction, int actionNationRecno,
                       bool forceMoveFlag) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <xLoc> <yLoc> <action Nation Recno> <force move>
    // <write_mem...>
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_UNIT_ASSIGN, sizeof(short) * 5 + get_write_len());
    shortPtr[0] = destXLoc;
    shortPtr[1] = destYLoc;
  ...
  if (unit_group_land.size())
    unit_group_land.exe_assign(destXLoc, destYLoc, targetMobileType,
                               actionNationRecno, forceMoveFlag);
  if (unit_group_air.size())
    unit_group_air.exe_assign(destXLoc, destYLoc, targetMobileType,
                              actionNationRecno, forceMoveFlag);
}

void UnitGroup::exe_assign(int destXLoc, int destYLoc, int targetMobileType,
                           int actionNationRecno, bool forceMoveFlag) {
  //---------------------------------------//
  Unit *unitPtr;
  int locX, locY;
  ...
  {
    run_action(destXLoc, destYLoc, UNIT_ASSIGN, baseObjRecno, actionNationRecno,
               forceMoveFlag);
    return;
  }
...
void UnitGroup::transform_mfort(int destXLoc, int destYLoc, char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_UNITS_GO_TRANSFORM_MFORT, sizeof(short) * 2 + get_write_len());
    shortPtr[0] = destXLoc;
    shortPtr[1] = destYLoc;
  ...
  short builderUnit
      [MAX_EXTRA_BUILDER]; // = (short*)
                           // mem_add_clear(MAX_EXTRA_BUILDER*sizeof(short));
  int checkIndex = 0;
  ...
  for (i = 1; i <= size() && checkIndex < MAX_EXTRA_BUILDER; i++) {
    ...
    if (unitPtr->unit_id == cmdUnit->unit_id &&
        unitPtr->nation_recno == cmdUnit->nation_recno &&
        unitPtr->is_visible()) {
  ...
  if (checkIndex == MAX_EXTRA_BUILDER) {
    long dist2[MAX_EXTRA_BUILDER]
              [MAX_EXTRA_BUILDER]; // distance between each builder and each
                                   // destination
    ...
    for (i = 0; i < checkIndex; i++) {
      ...
      for (j = 0; j < checkIndex; j++) { // don't use points distance
        dist2[i][j] = (srcX - destX[j]) * (srcX - destX[j]) +
                      (srcY - destY[j]) * (srcY - destY[j]);
      }
    }
    ...
    int minSumDist2 = dist2[bestSet[0]][0] + dist2[bestSet[1]][1] +
                      dist2[bestSet[2]][2] + dist2[bestSet[3]][3];
        ...
        for (int i2 = 0; i2 < MAX_EXTRA_BUILDER; i2++) {
          ...
          for (int i3 = 0; i3 < MAX_EXTRA_BUILDER; i3++) {
            if (i3 == i0 || i3 == i1 || i3 == i2)
              continue;
            int sumDist2 =
                dist2[i0][0] + dist2[i1][1] + dist2[i2][2] + dist2[i3][3];
...
// [int]  actionNationRecno  - nation recno order the action, (default:-1) //
// need for assign [bool] forceMoveFlag		  - force move flag (default:
// false)
//
void UnitGroup::run_action(int destXLoc, int destYLoc, int orderId,
                           int orderPara, int actionNationRecno,
                           bool forceMoveFlag) {
  //--------------------------------------------------------------//
  ...
  for (int i = 1; i <= size(); i++) {
    ...
    thisDestXLoc =
        MIN(MAX_WORLD_X_LOC - unitPtr->obj_loc_width(), thisDestXLoc);
    thisDestYLoc =
        MIN(MAX_WORLD_Y_LOC - unitPtr->obj_loc_height(), thisDestYLoc);
  ...
  if (isInFormation) {
    ...
    if (nationRecno) {
      ...
      int srcLocPowerNation =
          world.get_loc(centerXLoc, centerYLoc)->power_nation_recno;
      int destLocPowerNation =
          world.get_loc(thisDestXLoc, thisDestYLoc)->power_nation_recno;
      if ((!srcLocPowerNation ||
           nationPtr->get_relation_passable(srcLocPowerNation)) &&
          (!destLocPowerNation ||
           nationPtr->get_relation_passable(destLocPowerNation))) {
        path_reuse.set_handle_power_nation(
            1, nationPtr->relation_passable_array, nationRecno);
      }
    }
    ...
    // centerXLoc, centerYLoc);
    cluster_units_for_path_reuse(MAX(0, destCenterX), MAX(0, destCenterY),
                                 centerXLoc, centerYLoc);
  // ###### end Gilbert 15/5 ########//
  ...
  for (int i = 1; i <= size(); i++) {
    ...
    case UNIT_ATTACK: {
      ...
      if (attackTargetRecno != unitPtr->base_obj_recno)
        rc = unitPtr->set_attack(attackTargetRecno,
                                 true); // true - reset current order
      break;
    }

    case UNIT_ASSIGN:
      rc = unitPtr->set_assign(destXLoc, destYLoc, actionNationRecno,
                               forceMoveFlag);
      break;
    ...
    //--- if the order is a move order or the attack or assign order failed,
    //change it to a move order ---//

    // ######## begin Gilbert 15/5 ########//
    if (!rc)
      //			unitPtr->set_move(destXLoc, destYLoc,
      //forceMoveFlag);
      unitPtr->set_move(destCenterX, destCenterY, forceMoveFlag);
    // ######## end Gilbert 15/5 ########//

    unitPtr->cur_path_result_id =
        savePathResultId; // restore it as UnitB::stop_move() resets it
...
void UnitGroup::return_camp(char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <write_mem...>
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_UNITS_RETURN_CAMP,
                                                         get_write_len());
    write_mem(shortPtr);
    return;
  }
...
//
void UnitGroup::right_click(int xLoc, int yLoc, char remoteAction,
                            bool addWayPoint, bool shiftKeyAttack,
                            bool forceMoveFlag) {
  ...
  err_when(&unit_group_land == this ||
           &unit_group_air ==
               this); // unit_group_land & unit_group_air are for storing the
                      // result, this UnitGroup cannot be one of them
  ...
  if (remoteAction == COMMAND_PLAYER) {
    ...
    //			// packet structure : <write_mem>
    //			short *shortPtr = (short *)remote.new_send_queue_msg(
    //MSG_UNITS_CLEAR_HAS_WAY_POINT, get_write_len() ); 			write_mem(shortPtr);
    //		}
    //		else
    //		{
    for (int i = size(); i > 0; i--)
  ...
  if (!unitPtr)
    return;
  int curNationRecno =
      unitPtr->nation_recno; // the nation recno of the selected unit, it is not
                             // necessarily the player's nation, as the unit can
                             // be a spy
  ...
  if (baseObjRecno && !base_obj_array[baseObjRecno]->is_stealth()) {
    ...
    if (targetNationRecno == nation_array.player_recno) {
      ...
      if (thisUnit && thisUnit->unit_id == UNIT_EXPLOSIVE_CART &&
          shiftKeyAttack) {
        attackNow = 1;
      }
    }
    ...
    if (!attackNow &&
        // targetNationRecno==curNationRecno &&			// may be
        // different from trevor's version
        base_obj_array[baseObjRecno]->can_accept_assign(
            unitPtr->sprite_recno, nation_array.player_recno))
    // this is for the cases like inn where the assignee must be independent
    // while the target is not independent
    {
      //--- whether the target object accepts this unit assigning to it ---//

      assign(xLoc, yLoc, base_obj_array[baseObjRecno]->obj_mobile_type(),
             remoteAction, nation_array.player_recno, forceMoveFlag);
      if (remoteAction == COMMAND_PLAYER && unitPtr->is_own())
        unitPtr->sound("ACK");
      return;
...
void UnitGroup::group_by_mobile_type() {
  ...
  err_when((size() > 0) &&
           (unit_group_land.size() == 0 &&
            unit_group_air.size() ==
                0)); // there should be at least one unit selected
}
...
void UnitGroup::exe_formation(int formationType) {
  ...
  err_when(mobile_type != UNIT_LAND &&
           mobile_type != UNIT_AIR); // mobile_type must be set first before
...
void UnitGroup::formation_turn(int direction, char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_UNITS_FORM_TURN, sizeof(short) + get_write_len());
...
void UnitGroup::exe_formation_turn(int direction) {
  ...
  for (int i = 1; i <= size(); i++) {
    ...
    if (unitPtr->team_info) {
      ...
      break;
               ...
               !unit_array.is_deleted(unitPtr->leader_unit_recno) &&
  ...
  //	if(!formationMode)
  //		formationMode =
  //unit_array[get_unit(1)->leader_unit_recno]->team_info->formation_id;
...
//--------------- begin function
//UnitGroup::cluster_unit_for_path_reuse--------//
//
// cluster units that are close together to use path reuse together
//
void UnitGroup::cluster_units_for_path_reuse(int destX, int destY, int centerX,
                                             int centerY) {
...
#define MAX_CLUSTERS 255
  ...
  unsigned char *processed = (unsigned char *)vLenQueueProcessed.reserve(
      unitsSelected * sizeof(*processed));
    ...
    if (!processed[ind]) {
      ...
      while (ind3 < unitsSelected) {
        if (!processed[ind3]) {
          ...
          // formula for finding if an additional unit can be part of a bigger
          // cluster
          // ####### begin Gilbert 11/5 ######//
          //					if ( (abs(minx - thisLocX) < clusterLimit
          //|| 							abs(maxx - thisLocX) < clusterLimit )&& 						  (abs(miny - thisLocY) <
          //clusterLimit || 							abs(maxy - thisLocY) < clusterLimit ))
          if (thisMaxX + clusterLimit > minx &&
              thisMinX - clusterLimit < maxx &&
              thisMaxY + clusterLimit > miny && thisMinY - clusterLimit < maxy)
          // ####### end Gilbert 11/5 ######//
          {
      ...
      clusterArray[clusterId - 1].init(minx, miny, maxx, maxy,
                                       ind2 + 1); // number of units is ind2+1
  ...
  for (c = 0; c < clusterId; ++c) {
    ...
    while (index < unitsSelected) {
      int checkCount = 0;
      for (; index < unitsSelected && checkCount < MAX_REUSE_NUMBER_OF_UNITS;
           ++index) {
        // procesed[index] means rejected, do not move it
        // err_when( !processed[index] );		// not inside any
        // cluster
      ...
      err_when(clusterInfo->count < MAX_REUSE_NUMBER_OF_UNITS &&
               checkCount != clusterInfo->count);
      ...
      if (checkCount > 0) {
        ...
        if (nationRecno) {
          Nation *nationPtr = nation_array[nationRecno];
          int srcLocPowerNation =
              world.get_loc(units[0]->next_x_loc(), units[0]->next_y_loc())
                  ->power_nation_recno;
          int destLocPowerNation =
              world.get_loc(tmpX, tmpY)->power_nation_recno;
          if ((!srcLocPowerNation ||
               nationPtr->get_relation_passable(srcLocPowerNation)) &&
              (!destLocPowerNation ||
               nationPtr->get_relation_passable(destLocPowerNation))) {
        ...
        err_when(units[0]->move_to_loc_x + units[0]->loc_width >
                 MAX_WORLD_X_LOC);
        err_when(units[0]->move_to_loc_y < 0);
        err_when(units[0]->move_to_loc_y + units[0]->loc_height >
                 MAX_WORLD_Y_LOC);
...
//
void UnitGroup::cluster_units_for_attack(int targetRecno, int targetLocX,
                                         int targetLocY, int targetWidth,
                                         int targetHeight) {
  Unit *unitPtr;
  int locX, locY;
  ...
  for (i = 1; i <= size(); i++) {
    ...
    if (unitPtr->cur_action == SPRITE_ATTACK &&
        unitPtr->cur_order.para == targetRecno)
      continue;
    if (unitPtr->can_attack())
      unitPtr->attack(targetRecno, true);
  }
...
int UnitGroup::form_team() {
  ...
  leaderUnit->team_info->add_member(
      leaderUnit->sprite_recno); // add itself to the team first
...
static void pack_clusters(ClusterInfo *clusterArray, int clusterCount,
  ...
  do {
    ...
    for (i = 0; i < clusterCount; ++i) {
      ...
      int freeDist = misc.area_distance(iminx, iminy, imaxx, imaxy, centerX,
                                        centerY, centerX, centerY);

      if (freeDist ==
          0) // overlapped don't consider ( if possible, translate together...)
        continue;
      ...
      for (j = 0; j < clusterCount; ++j) {
        ...
        int dist = misc.area_distance(
                       iminx, iminy, imaxx, imaxy, clusterArray[j].new_minx,
                       clusterArray[j].new_miny, clusterArray[j].new_maxx,
                       clusterArray[j].new_maxy) -
                   1;
