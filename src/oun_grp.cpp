...
int UnitGroup::is_visible(short recNo)
{
    ...
    return !unit_array.is_deleted(unitRecno) && unit_array[unitRecno]->is_visible();
}
...
void UnitGroup::select_all()
{
    ...
    for (int i = size(); i > 0; i--)
    {
        ...
        if (unitPtr->nation_recno == nation_array.player_recno && unitPtr->is_visible())
        {
            ...
            if (unit_array.selected_recno == 0 || unitPtr->rank_id > unit_array[unit_array.selected_recno]->rank_id)
            {
...
// selectedUnitArray.
//
// <int> nationRecno				player nation recno or remote
// player nation recno return number of valid units
//
...
int UnitGroup::validate_unit_array(int nationRecno)
{
    ...
    {
        Unit *unitPtr;
        if (!unit_array.is_deleted(*readPtr) && (unitPtr = unit_array[*readPtr]) && unitPtr->is_visible() &&
            unitPtr->is_nation(nationRecno))
        {
            *writePtr = *readPtr;
            ++writePtr;
            ++last_ele;
...
void UnitGroup::transform_mfort(int destXLoc, int destYLoc, char remoteAction)
{
    ...
    if (!remoteAction && remote.is_enable())
    {
        ...
        short *shortPtr =
            (short *)remote.new_send_queue_msg(MSG_UNITS_GO_TRANSFORM_MFORT, sizeof(short) * 2 + get_write_len());
        shortPtr[0] = destXLoc;
        shortPtr[1] = destYLoc;
    ...
    short builderUnit[MAX_EXTRA_BUILDER]; // = (short*)
    ...
    {
        long dist2[MAX_EXTRA_BUILDER][MAX_EXTRA_BUILDER]; // distance between each builder and each
                                                          // destination
...
// false)
//
void UnitGroup::run_action(int destXLoc, int destYLoc, int orderId, int orderPara, int actionNationRecno,
                           bool forceMoveFlag)
{
    //--------------------------------------------------------------//
...
//
void UnitGroup::right_click(int xLoc, int yLoc, char remoteAction, bool addWayPoint, bool shiftKeyAttack,
                            bool forceMoveFlag)
{
    ...
    err_when(&unit_group_land == this || &unit_group_air == this); // unit_group_land & unit_group_air are for storing
                                                                   // the result, this UnitGroup cannot be one of them
    ...
    if (remoteAction == COMMAND_PLAYER)
    {
        ...
        // MSG_UNITS_CLEAR_HAS_WAY_POINT, get_write_len() );
        // write_mem(shortPtr);
        //		}
        //		else
        //		{
    ...
    if (!unitPtr)
        return;
    int curNationRecno = unitPtr->nation_recno; // the nation recno of the selected unit, it is not
    ...
    if (baseObjRecno && !base_obj_array[baseObjRecno]->is_stealth())
    {
        ...
        if (size() == 1 && unitPtr->cast_to_UnitGod() && unitPtr->cast_to_UnitGod()->right_click(xLoc, yLoc, 0))
        {
...
void UnitGroup::group_by_mobile_type()
{
    ...
    err_when((size() > 0) &&
             (unit_group_land.size() == 0 && unit_group_air.size() == 0)); // there should be at least one unit selected
}
...
void UnitGroup::exe_formation(int formationType)
{
    ...
    err_when(mobile_type != UNIT_LAND && mobile_type != UNIT_AIR); // mobile_type must be set first before
        ...
        if (!processed[ind])
        {
            ...
            while (ind3 < unitsSelected)
            {
                if (!processed[ind3])
                {
                    ...
                    // cluster
                    // ####### begin Gilbert 11/5 ######//
                    //					if ( (abs(minx - thisLocX) <
                    // clusterLimit
                    //|| 							abs(maxx - thisLocX) < clusterLimit
                    //)&& (abs(miny
                    //- thisLocY) < clusterLimit || 							abs(maxy
                    //- thisLocY)
                    //<
                    // clusterLimit ))
                    if (thisMaxX + clusterLimit > minx && thisMinX - clusterLimit < maxx &&
            ...
            if (checkCount > 0)
            {
                ...
                if (nationRecno)
                {
                    ...
                    int srcLocPowerNation =
                        world.get_loc(units[0]->next_x_loc(), units[0]->next_y_loc())->power_nation_recno;
                ...
                units[0]->move_to_loc_x =
                    units[0]->obj_loc_x1() + tmpX - centerX + clusterInfo->new_minx - clusterInfo->old_minx;
                units[0]->move_to_loc_y =
                    units[0]->obj_loc_y1() + tmpY - centerY + clusterInfo->new_miny - clusterInfo->old_miny;
...
//
void UnitGroup::cluster_units_for_attack(int targetRecno, int targetLocX, int targetLocY, int targetWidth,
                                         int targetHeight)
{
    Unit *unitPtr;
    int locX, locY;
...
static void pack_clusters(ClusterInfo *clusterArray, int clusterCount, int centerX, int centerY)
{
    ...
    do
    {
        ...
        for (i = 0; i < clusterCount; ++i)
        {
            ...
            int freeDist = misc.area_distance(iminx, iminy, imaxx, imaxy, centerX, centerY, centerX, centerY);
            ...
            for (j = 0; j < clusterCount; ++j)
            {
                ...
                int dist =
                    misc.area_distance(iminx, iminy, imaxx, imaxy, clusterArray[j].new_minx, clusterArray[j].new_miny,
                                       clusterArray[j].new_maxx, clusterArray[j].new_maxy) -
                    1;

                // if either cluster is large
                // further decrease the gap so large group keep separated a little
                if (iclusterSize > MAX_TEAM_MEMBER || clusterArray[j].count > MAX_TEAM_MEMBER)
                {
                    dist -= 1;
                }
