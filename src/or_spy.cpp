...
#define SPY_BROWSE_HEIGHT 21
    ...
    {
        browse_spy.init(SPY_BROWSE_X1, SPY_BROWSE_Y1 + SPY_BROWSE_HEIGHT + 1, SPY_BROWSE_X2, SPY_BROWSE_Y2 - 20, 0, 21,
                        spy_filter(), put_spy_rec, 1);
...
static void put_spy_rec(int recNo, int x, int y, int refreshFlag)
{
    ...
    if (spyPtr->is_civilian)
        titledName = text_unit.str_civilian_titled(race_res[spyPtr->race_id]->get_name(spyPtr->name_id));
    else
        titledName = text_unit.str_unit_titled(spyPtr->race_id, race_res[spyPtr->race_id]->get_name(spyPtr->name_id),
