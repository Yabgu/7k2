...
#define SPY_BROWSE_HEIGHT 21
  ...
  vga_back.d3_panel_up(SPY_BROWSE_X1, SPY_BROWSE_Y1, SPY_BROWSE_X2,
                       SPY_BROWSE_Y1 + SPY_BROWSE_HEIGHT - 1);
...
#elif (defined(FRENCH))
  font_bld.put(x + 305, y + 14,
               text_reports.str_spy_location());           //"Location" );
  font_bld.put(x + 380, y, text_reports.str_skill());      // "Skill" );
  ...
  if (refreshFlag == INFO_REPAINT) {
    browse_spy.init(SPY_BROWSE_X1, SPY_BROWSE_Y1 + SPY_BROWSE_HEIGHT + 1,
                    SPY_BROWSE_X2, SPY_BROWSE_Y2 - 20, 0, 21, spy_filter(),
                    put_spy_rec, 1);
...
static void disp_total() {
  ...
  vga_back.d3_panel_up(SPY_BROWSE_X1, SPY_BROWSE_Y2 - 18, SPY_BROWSE_X2,
                       SPY_BROWSE_Y2);
...
static void put_spy_rec(int recNo, int x, int y, int refreshFlag) {
  ...
  if (spyPtr->is_civilian)
    ...
    titledName = text_unit.str_unit_titled(
        spyPtr->race_id, race_res[spyPtr->race_id]->get_name(spyPtr->name_id),
  ...
  case SPY_FIRM:
    ...
    placeStr =
        firm_res.get_build(firm_array[spyPtr->spy_place_para]->firm_build_id)
            ->name;
