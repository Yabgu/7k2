...
//								without checking day
//interavals. 							   (default: 0)
//
void Unit::process_ai() {
  if (unit_mode) // let functions in OUN_MODE.CPP process units in specific
                 // modes
    return;
  ...
  if (spy_recno && true_nation_recno() != nation_recno) {
    ...
    if (is_visible() &&
        misc.random(365 * FRAMES_PER_DAY) ==
            0) // if the unit stay outside for one year, it will get caught
    {
      resign(COMMAND_AI);
      return;
    }

    if (!spy_array[spy_recno]
             ->notify_cloaked_nation_flag) // if notify_cloaked_nation_flag is
                                           // 1, the nation will take it as its
                                           // own spies
      return;
  }
  ...
  if (cur_order.mode == UNIT_ATTACK && nation_recno) {
    if (!base_obj_array.is_deleted(cur_order.para)) {
      ...
      if (baseObj->cast_to_Town() &&
          baseObj->cast_to_Town()->nation_recno == 0 &&
  ...
  if (home_camp_firm_recno) {
    ...
    } else if ((firmMonsterFortress = firm_array[home_camp_firm_recno]
                                          ->cast_to_FirmMonsterFortress())) {
  ...
  if (is_all_stop()) {
    if (race_id && rank_id == RANK_KING) {
      ...
      // ######### begin Gilbert 24/3 ########//
      // BUGHERE : monster weapon was UNIT_CLASS_MONSTER but now
      // UNIT_CLASS_MONS_WEAPON
      if (unitClass == UNIT_CLASS_WEAPON || unitClass == UNIT_CLASS_MONS_WEAPON)
      // ######### end Gilbert 24/3 ########//
      {
        if (info.game_date % 15 ==
            sprite_recno %
                15) // don't call too often as the action may fail and it takes
                    // a while to call the function each time
        {
      ...
      //------ if this unit is a monster ------//
        ...
        //--- if previous attempts for new action failed, don't call
        //think_normal_human_action() so frequently then ---//

        if (ai_no_suitable_action) {
          if (info.game_date % 15 !=
              sprite_recno %
                  15) // don't call too often as the action may fail and it
                      // takes a while to call the function each time
            return;
        }

        if (!think_normal_monster_action()) {
          ai_no_suitable_action =
              true; // set this flag so think_normal_human_action() won't be
                    // called continously
      ...
      //------ if this unit is a human ------//
        ...
        if (!think_normal_human_action()) {
          ...
          //					if( !leader_unit_recno )		// only when the
          //unit is not led by a commander 						resign(COMMAND_AI); 					else
...
int Unit::think_stop_chase() {
  ...
  int aiChaseDistance = 10 + nation_array[nation_recno]->pref_military_courage /
                                 20; // chase distance: 10 to 15

  int curDistance =
      misc.points_distance(targetUnit->next_x_loc(), targetUnit->next_y_loc(),
...
void Unit::ai_move_to_nearby_town(int shouldAssign) {
  ...
  for (int i = town_array.size(); i > 0;
       i--) // can't use ai_town_array[] because this function will be called by
            // Unit::betray() when a unit defected to the player's kingdom
  {
    ...
    curDistance = misc.points_distance(curXLoc, curYLoc, townPtr->center_x,
                                       townPtr->center_y);

    if (curDistance <
        10) // no need to move if the unit is already close enough to the town.
      return;
...
void Unit::ai_move_to_nearby_firm(int firmId, int firmId2) {
  ...
  for (int i = firm_array.size(); i > 0;
       i--) // can't use ai_firm_array[] because this function will be called by
            // Unit::betray() when a unit defected to the player's kingdom
  {
    ...
    curRating = world.distance_rating(curXLoc, curYLoc, firmPtr->center_x,
                                      firmPtr->center_y);
...
int Unit::ai_escape_fire() {
  ...
  for (int i = 2; i < checkLimit; i++) {
    ...
    if (checkXLoc < 0 || checkXLoc >= MAX_WORLD_X_LOC || checkYLoc < 0 ||
        checkYLoc >= MAX_WORLD_Y_LOC)
      continue;
...
int Unit::think_king_action() { return think_leader_action(); }
...
int Unit::think_general_action() {
  ...
  //--- if the skill of the general and the number of soldiers he commands is
  //not large enough to justify building a new camp ---//
...
int Unit::think_leader_action() {
  ...
  if (rank_id == RANK_KING) // if this unit is the king, always assign it to a
                            // camp regardless of whether the king is a better
                            // commander than the existing one
    bestRating = -1000;
  ...
  for (int i = nationPtr->ai_camp_count - 1; i >= 0; i--) {
    ...
    int newLeadership =
        firmCamp->new_commander_leadership(race_id, skill_level());
    ...
    if (curRating > bestRating) {
      ...
      if (rank_id !=
          RANK_KING) // don't check this if the current unit is the king
      {
        actionRecno = nationPtr->is_action_exist(
            firmCamp->loc_x1, firmCamp->loc_y1, -1, -1,
            ACTION_AI_ASSIGN_OVERSEER, firmCamp->firm_id);

        if (actionRecno &&
            nationPtr->get_action(actionRecno)->processing_instance_count)
          continue;
      }
  ...
  if (team_info->member_count - 1 <= MAX_SOLDIER - bestCamp->soldier_count) {
    ...
    if (team_info->member_count > 0) {
      ...
      unit_group.assign(bestCamp->loc_x1, bestCamp->loc_y1, UNIT_LAND,
                        COMMAND_AI);
      return 1;
    }
  }
  ...
  return nationPtr->add_action(bestCamp->loc_x1, bestCamp->loc_y1, -1, -1,
                               ACTION_AI_ASSIGN_OVERSEER, bestCamp->firm_id, 1,
                               sprite_recno);
}
...
int Unit::think_normal_human_action() {
  ...
  if (nationPtr->ai_camp_count == 0 ||
      nationPtr->total_camp_unit_space(0) <=
          nationPtr->total_unit_count) // If there are more units than the
                                       // existing forts can hold
  {
    if (ai_build_firm())
      return 1;
  }

  //--- the following are actions that are only performed if the unit is not led
  //by a commander ---//

  if (leader_unit_recno &&
      unit_array[leader_unit_recno]
          ->is_visible()) // if the unit is led by a commander, let the
                          // commander makes the decision. If the leader has
                          // been assigned to a firm, don't consider it as a
                          // leader anymore
  {
    return 0;
  }
...
int Unit::think_assign_soldier_to_camp() {
  ...
  for (int i = 0; i < ownNation->ai_camp_count; i++) {
    ...
    if (firmCamp->is_soldier_full()) {
      ...
      if (firmCamp->majority_race() == race_id) {
        if (skill_level() < minSkill + 10)
          continue;
      } else //-- for different race, only assign if the skill is significantly
    ...
    curRating += world.distance_rating(curXLoc, curYLoc, firmCamp->center_x,
                                       firmCamp->center_y);
...
int Unit::think_assign_human_to_town() {
            ...
            // Unit::betray() when a unit defected to the player's kingdom
  {
    ...
    curRating = world.distance_rating(curXLoc, curYLoc, townPtr->center_x,
                                      townPtr->center_y);
  ...
  if (!hasTownInRegion) {
    //--- if region is too small don't consider this area, stay in the island
    //forever --//
    ...
    if (region_array.get_region_stat(regionId)
            ->camp_nation_count_array[nation_recno - 1] == 0) {
      //---- try to build one if this unit can ----//

      if (!leader_unit_recno) // if this unit is commanded by a leader, let the
                              // leader build the camp
        ai_build_firm();
    } else // if there is already a camp in this region, try to settle a new
           // town next to the camp
    {
      ai_settle_new_town();
    }

    return 1; // if we don't have any town in this region, return 1, so this
              // unit won't be resigned and so it can wait for other units to
              // set up camps and villages later ---//
  }
...
int Unit::think_build_fort() {
  ...
  if (!ownNation
           ->is_human()) // only build forts if the nation is a human nation
    return 0;
...
int Unit::think_reward() {
  ...
  if (spy_recno &&
      true_nation_recno() == nation_recno) // if this is a spy of ours
  {
  ...
  else if (!is_civilian()) // it is a military unit
  {
    ...
    {
      if (loyalty < UNIT_BETRAY_LOYALTY) // if this unit's loyalty is < betrayel
                                         // level, reward immediately
      {
        reward(nation_recno); // reward it immediatley if it's an overseer,
                              // don't check ai_should_spend()
        return 1;
      }
    ...
    neededLoyalty = MAX(UNIT_BETRAY_LOYALTY + 10,
                        neededLoyalty); // 10 points above the betray loyalty
                                        // level to prevent betrayal
...
void Unit::ai_leader_being_attacked(BaseObj *attackerObj) {
  ...
  if (rank_id == RANK_KING) {
    ...
    rc = skill_level() >=
         30 + (100 - nation_array[nation_recno]->pref_keep_general) /
                  2;       // 30 to 80
  ...
  if (rc) {
    if (info.game_date >
        team_info->ai_last_request_defense_date + callIntervalDays) {
...
int Unit::think_king_flee() {
  if (force_move_flag &&
      cur_action != SPRITE_IDLE) // the king is already fleeing now
    return 1;
  ...
  if ((team_info->member_count == 0 && !cur_order.ai_action_id) ||
      hit_points < max_hit_points() *
                       (150 - ownNation->pref_military_courage / 2) / 200) {
    //------------------------------------------//
    //
    ...
    if (cur_action == SPRITE_ATTACK) {
      for (int i = ownNation->ai_camp_count - 1; i >= 0; i--) {
        ...
        if (firmCamp->overseer_recno &&
            rank_id !=
                RANK_KING) // if there is already a commander in this camp.
                           // However if this is the king, than ingore this
          continue;
    ...
    if (bestCamp) {
      ...
      if (config.ai_aggressiveness > OPTION_LOW)
        force_move_flag = 1;
    } else // if the king is neither under attack or has a home camp, then call
           // the standard think_leader_action()
    {
      think_leader_action();
    }
...
int Unit::think_general_flee() {
  if (force_move_flag &&
      cur_action != SPRITE_IDLE) // the general is already fleeing now
    return 1;
  ...
  if ((team_info->member_count == 0 && !cur_order.ai_action_id) ||
      hit_points < max_hit_points() *
                       (75 + ownNation->pref_military_courage / 2) /
                       200) // 75 to 125 / 200
  {
    //------------------------------------------//
    //
    ...
    if (bestCamp) {
      if (bestCamp
              ->overseer_recno) // if there is already an overseer there, just
                                // move close to the camp for protection
      {
        ...
        assign(bestCamp->loc_x1, bestCamp->loc_y1);
    } else // if the general is neither under attack or has a home camp, then
           // call the standard think_leader_action()
    {
      think_leader_action();
    }
...
int Unit::ai_build_firm() {
  ...
  if (world.locate_space(
          xLoc, yLoc, xLoc, yLoc, firmInfo->loc_width + INTER_PLACE_SPACE * 2,
          firmInfo->loc_height +
              INTER_PLACE_SPACE *
                  2, // leave at least one location space around the building
          UNIT_LAND, curRegionId, 1)) {
    return ownNation->add_action(xLoc, yLoc, -1, -1, ACTION_AI_BUILD_FIRM,
                                 firmId, 1, sprite_recno);
  }
...
int Unit::ai_settle_new_town() {
  ...
  if (world.locate_space(xLoc, yLoc, bestCamp->loc_x2, bestCamp->loc_y2,
                         townWidth, townHeight, UNIT_LAND, curRegionId,
                         1)) // 1-build flag
  {
...
int Unit::ai_handle_seek_path_fail() {
  if (seek_path_fail_count <
      5 * SEEK_PATH_FAIL_INCREMENT) // wait unit it has failed many times
    return 0;
  ...
  if (rank_id == RANK_SOLDIER && !leader_unit_recno) {
    ...
    if (seek_path_fail_count >=
        (7 + skill_level() / 10) * SEEK_PATH_FAIL_INCREMENT)
      resignFlag = 1;
  } else {
    if (seek_path_fail_count >= 7 * SEEK_PATH_FAIL_INCREMENT)
      resignFlag = 1;
