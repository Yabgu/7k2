...
void Unit::process_ai()
{
    ...
    if (spy_recno && true_nation_recno() != nation_recno)
    {
        ...
        if (is_visible() &&
            misc.random(365 * FRAMES_PER_DAY) == 0) // if the unit stay outside for one year, it will get caught
        {
            resign(COMMAND_AI);
            return;
        }

        if (!spy_array[spy_recno]->notify_cloaked_nation_flag) // if notify_cloaked_nation_flag is
    ...
    if (race_id && rank_id != RANK_KING && info.game_date % 5 == sprite_recno % 5)
    {
        think_reward();
    }
    ...
    if (race_id && rank_id == RANK_GENERAL && info.game_date % 7 == sprite_recno % 7)
    {
    ...
    if (home_camp_firm_recno)
    {
        ...
        {
            rc = (unit_id == firmMonsterFortress->support_unit_id && !firmMonsterFortress->is_extra_builder_full()) ||
    ...
    if (is_all_stop())
    {
        ...
        else
        {
            ...
            // ######### end Gilbert 24/3 ########//
            {
                if (info.game_date % 15 == sprite_recno % 15) // don't call too often as the action may fail and it
            ...
            else if (unitClass == UNIT_CLASS_MONSTER || unitClass == UNIT_CLASS_INSECT ||
                     unitClass == UNIT_CLASS_ANIMAL)
            {
                ...
                {
                    ai_no_suitable_action = true; // set this flag so think_normal_human_action() won't be
                                                  // called continously
            ...
            else if (unitClass == UNIT_CLASS_HUMAN)
            {
                ...
                if (!think_normal_human_action())
                {
                    ...
                    //					if( !leader_unit_recno )		// only when
                    // the
                    // unit is not led by a commander
                    // resign(COMMAND_AI); else
...
int Unit::think_stop_chase()
{
    ...
    int curDistance = misc.points_distance(targetUnit->next_x_loc(), targetUnit->next_y_loc(), ai_original_target_x_loc,
                                           ai_original_target_y_loc);
...
void Unit::ai_move_to_nearby_town(int shouldAssign)
{
    ...
    for (int i = town_array.size(); i > 0; i--) // can't use ai_town_array[] because this function will be called by
...
void Unit::ai_move_to_nearby_firm(int firmId, int firmId2)
{
    ...
    for (int i = firm_array.size(); i > 0; i--) // can't use ai_firm_array[] because this function will be called by
                                                // Unit::betray() when a unit defected to the player's kingdom
    {
...
int Unit::think_general_action()
{
    ...
    else if (skill_level() + team_info->member_count * 4 < 40 + ownNation->pref_keep_general / 5) // 40 to 60
    {
        rc = 1;
    }
...
int Unit::think_leader_action()
{
    ...
    for (int i = nationPtr->ai_camp_count - 1; i >= 0; i--)
    {
        ...
        if (curRating > bestRating)
        {
            ...
            {
                actionRecno = nationPtr->is_action_exist(firmCamp->loc_x1, firmCamp->loc_y1, -1, -1,
    ...
    return nationPtr->add_action(bestCamp->loc_x1, bestCamp->loc_y1, -1, -1, ACTION_AI_ASSIGN_OVERSEER,
                                 bestCamp->firm_id, 1, sprite_recno);
}
...
int Unit::think_normal_human_action()
{
    ...
    if (nationPtr->ai_camp_count == 0 ||
        nationPtr->total_camp_unit_space(0) <= nationPtr->total_unit_count) // If there are more units than the
    ...
    if (leader_unit_recno && unit_array[leader_unit_recno]->is_visible()) // if the unit is led by a commander, let the
                                                                          // commander makes the decision. If the leader
                                                                          // has been assigned to a firm, don't consider
                                                                          // it as a leader anymore
    {
        return 0;
    }
...
int Unit::think_normal_monster_action()
{
    ...
    if (nationPtr->is_monster() && (nationPtr->ai_camp_count == 0 ||
...
void Unit::ai_leader_being_attacked(BaseObj *attackerObj)
{
    ...
    if (!attackerObj ||
        attackerObj->nation_recno == nation_recno) // this can happen when the unit has just changed nation
        return;
        ...
        hit_points < max_hit_points() * (150 - ownNation->pref_military_courage / 2) / 200)
    {
        ...
        if (cur_action == SPRITE_ATTACK)
        {
            for (int i = ownNation->ai_camp_count - 1; i >= 0; i--)
            {
                ...
                if (firmCamp->overseer_recno && rank_id != RANK_KING) // if there is already a commander in this camp.
...
int Unit::think_general_flee()
{
        ...
        hit_points < max_hit_points() * (75 + ownNation->pref_military_courage / 2) / 200) // 75 to 125 / 200
    {
        ...
        if (bestCamp)
        {
            if (bestCamp->overseer_recno) // if there is already an overseer there, just
...
int Unit::ai_build_firm()
{
    ...
    if (world.locate_space(xLoc, yLoc, xLoc, yLoc, firmInfo->loc_width + INTER_PLACE_SPACE * 2,
                           firmInfo->loc_height +
                               INTER_PLACE_SPACE * 2, // leave at least one location space around the building
                           UNIT_LAND, curRegionId, 1))
    {
        return ownNation->add_action(xLoc, yLoc, -1, -1, ACTION_AI_BUILD_FIRM, firmId, 1, sprite_recno);
    }
...
int Unit::ai_settle_new_town()
{
    ...
    if (world.locate_space(xLoc, yLoc, bestCamp->loc_x2, bestCamp->loc_y2, townWidth, townHeight, UNIT_LAND,
                           curRegionId,
                           1)) // 1-build flag
    {
