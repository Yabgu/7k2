...
void FirmMine::process_ai() {
  ...
  if (productivity > 0) // only do so if productivity > 0
  {
    ...
    if (nation_array[nation_recno]->ai_factory_count ==
        0) // if the nation doesn't have any factories yet, give building
           // factory a higher priority
    ...
    if (rc) {
      ...
      think_build_market(); // don't build it in FirmMine, when it builds a
                            // factory the factory will build a mine.
    }
  }
...
int FirmMine::think_build_market() {
  ...
  //-- only build a new market when the mine has a larger supply than the
  //demands from factories and market places
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    ...
    if (firmPtr->nation_recno ==
        nation_recno) // if it already has a raw material market, then no need
                      // to build a new one
      return 0;
  }
  ...
  if (!nationPtr->find_best_place_loc(FIRM_MARKET, loc_x1, loc_y1, buildXLoc,
                                      buildYLoc)) {
    no_neighbor_space = 1;
    return 0;
  }
...
int FirmMine::think_adjust_workforce() {
  ...
  int optimizedWorker =
      3 + MAX_WORKER - MAX_WORKER * (int)stock_qty / (int)max_stock_qty;
...
int FirmMine::ai_has_excess_worker() {
  //--- if the actual production is lower than the productivity, than the firm
  ...
  if (worker_count > 4) // at least keep 4 workers
  {
    return stock_qty > max_stock_qty * (float)0.9 &&
           production_30days() <
               productivity * 25; // take 25 days instead of 30 days so there
                                  // will be small chance of errors.
