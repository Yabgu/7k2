...
void FirmMine::process_ai()
{
    ...
    if (productivity > 0) // only do so if productivity > 0
    {
        ...
        if (nation_array[nation_recno]->ai_factory_count == 0) // if the nation doesn't have any factories yet, give
                                                               // building factory a higher priority
...
int FirmMine::think_build_market()
{
    ...
    for (int i = 0; i < linked_firm_count; i++)
    {
        ...
        if (firmPtr->nation_recno == nation_recno) // if it already has a raw material market, then no need
                                                   // to build a new one
            return 0;
    }
...
int FirmMine::ai_has_excess_worker()
{
    ...
    if (worker_count > 4) // at least keep 4 workers
    {
        return stock_qty > max_stock_qty * (float)0.9 &&
               production_30days() < productivity * 25; // take 25 days instead of 30 days so there
