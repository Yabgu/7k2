...
short *TownRes::cache_color_remap_table;
...
void TownRes::load_town_layout()
{
    ...
    for (i = 0; i < town_layout_count; i++)
    {
        ...
        // );
        townLayout->ground_bitmap_ptr =
            image_tpict.get_ptr(misc.nullify(townLayoutRec->ground_name, TownLayoutRec::GROUND_NAME_LEN));
        // ###### end Gilbert 9/9 ########//

        err_if(townLayout->slot_count > MAX_TOWN_LAYOUT_SLOT) err_now("Error: MAX_TOWN_LAYOUT_SLOT limit exceeded.");
...
void TownRes::load_town_slot()
{
    ...
    for (i = 0; i < town_slot_count; i++)
    {
        ...
        err_when(townSlot->build_type == TOWN_OBJECT_FARM && (townSlot->build_code < 1 || townSlot->build_code > 9));
    }
}
...
void TownRes::load_town_build()
{
    ...
    for (i = 0; i < town_build_count; i++)
    {
        ...
        townBuild->offset_x +=
            -townBuild->loc_width * LOCATE_WIDTH / 2 -
            (-townBuild->loc_width * ZOOM_LOC_X_WIDTH / 2 + -townBuild->loc_height * ZOOM_LOC_Y_WIDTH / 2);
        townBuild->offset_y +=
            -townBuild->loc_height * LOCATE_HEIGHT / 2 -
            (-townBuild->loc_width * ZOOM_LOC_X_HEIGHT / 2 + -townBuild->loc_height * ZOOM_LOC_Y_HEIGHT / 2);
    }
}
...
void TownRes::load_town_name()
{
    ...
    town_name_used_array =
        (char *)mem_add(sizeof(town_name_used_array[0]) * town_name_count); // store the used_count separately from
                                                                            // town_name_array to faciliate file saving
...
{
    err_if(recNo < 1 || recNo > town_layout_count) err_now("TownRes::get_layout()");
...
{
    err_if(recNo < 1 || recNo > town_build_type_count) err_now("TownRes::get_build_type()");
