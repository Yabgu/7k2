...
#define VIEW_MODE_BUTTON_HEIGHT 22 // 16
  ...
  static char viewModeArray[] = {MODE_NATION, MODE_TOWN,     MODE_ECONOMY,
                                 MODE_TRADE,  MODE_MILITARY, MODE_TECH,
                                 MODE_SPY,    MODE_RANK};
  for (i = 0; i < 8; i++, x += VIEW_MODE_BUTTON_X_SPACE) {
    if ((((i - 1) % 2) == 1) &&
        (current_display_mode.mode_id == MODE_ID_1024x768x16)) {
      x = 819;
      y = y + VIEW_MODE_BUTTON_HEIGHT;
    }

    if ((i == 4) &&
        (current_display_mode.mode_id == MODE_ID_800x600x16)) // the second row
    {
      x = 6;
      y = 29;
    }
    ...
    if (nation_array.player_recno == 0 &&
        i < 7) // when the player has lost the game, the only report available
               // is ranking report only
      continue;

    if (mouse.single_click(x, y, x + VIEW_MODE_BUTTON_WIDTH - 1,
                           y + VIEW_MODE_BUTTON_HEIGHT - 1)) {
      int newMode = viewModeArray[i];

      if (view_mode ==
          newMode) // when click on the same mode again, go to the normal mode
...
//
// [int] dispCampaignMsg - whether display a campaign message in the beginning
// of the game or not
//									(default:
//0)
//
...
void Sys::disp_map() {
  ...
  if (map_need_redraw) // requested by other modules to redraw the pre-drawn map
                       // background
  {
  ...
  if (!mapMatrix->filter_object_flag ||
      mapMatrix->filter_object_type == OBJ_FIRM)
    firm_array.draw_dot(mapMatrix->filter_object_para);

  if (!mapMatrix->filter_object_flag ||
      mapMatrix->filter_object_type == OBJ_TOWN)
  ...
  if (!mapMatrix->filter_object_flag ||
      mapMatrix->filter_object_type == OBJ_UNIT) {
...
void Sys::disp_zoom() {
  ...
  if (zoom_need_redraw) // requested by other modules to redraw the pre-drawn
                        // zoom background
  {
    long backupSeed = misc.get_random_seed();

    // #### begin Gilbert 8/2 #####//
    world.zoom_matrix->just_drawn_flag =
        0; // request world.zoom_matrix->disp() to call draw()
    // #### end Gilbert 8/2 #####//
...
void Sys::blt_virtual_buf() {
  ...
  if (!use_true_front)
    ...
    //--- in a debug sesion, vga_front is not the true front buffer, now copy it
    //to the true one ---//
...
void Sys::disp_frames_per_second() {
  ...
  if (game.game_mode == GAME_TUTORIAL) // don't display in tutorial mode as it
                                       // overlaps with the tutorial text
    return;
...
int Sys::change_display_mode(int modeId) {
  ...
  if (displayModeInfo) {
    ...
    if (vga.change_resolution(displayModeInfo->screen_width,
                              displayModeInfo->screen_height))
      rc = 1;
...
void Sys::capture_screen() {
  // NB: Increase this when allowing more decimals in the screenshot file, or
  // when changing the screenshot filename
  ...
  //   if( sys.debug_session )    // in debug session, the buffer is not locked,
  //   we need to lock it for capturing the screen
  if (use_true_front) // in triple buffer , the buffer is not locked, we need to
                      // lock it for capturing the screen
  {
