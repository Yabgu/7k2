...
#define VIEW_MODE_BUTTON_HEIGHT 22 // 16
    ...
    static char viewModeArray[] = {MODE_NATION,   MODE_TOWN, MODE_ECONOMY, MODE_TRADE,
                                   MODE_MILITARY, MODE_TECH, MODE_SPY,     MODE_RANK};
    for (i = 0; i < 8; i++, x += VIEW_MODE_BUTTON_X_SPACE)
    {
        ...
        if (nation_array.player_recno == 0 && i < 7) // when the player has lost the game, the only report available
...
void Sys::capture_screen()
{
    ...
    enum
    {
        MAX_SCREENSHOT_FILENAME_LENGTH = 8
    };
