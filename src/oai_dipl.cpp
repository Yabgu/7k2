...
void Nation::think_diplomacy() {
  //--- process incoming messages first, so we won't send out the same request
  //to nation which has already proposed the same thing ---//
  ...
  if (nation_array.is_deleted(
          nationRecno)) // the nation may have been deleted, if the nation
                        // accepts a purchase kingdom offer
    return;
  ...
  if (think_propose_alliance_treaty()) // try proposing alliance treaty first,
                                       // then try proposing friendly treaty
    return;
...
int Nation::should_diplomacy_retry(int talkId, int nationRecno) {
  ...
  if (talkId == TALK_DEMAND_TRIBUTE || talkId == TALK_DEMAND_AID ||
      talkId == TALK_EXCHANGE_TECH) {
    retryInterval = 60 + 60 * (100 - pref_diplomacy_retry) / 100; // 2-4 months
  } else {
    retryInterval = 90 + 270 * (100 - pref_diplomacy_retry) /
                             100; // 3 months to 12 months before next try
  }
  ...
  return info.game_date >
         get_relation(nationRecno)->last_talk_reject_date_array[talkId - 1] +
             retryInterval;
}
...
void Nation::ai_notify_reply(int talkMsgRecno) {
  ...
  if (talkMsg->reply_type == REPLY_REJECT)
    nationRelation->last_talk_reject_date_array[talkMsg->talk_id - 1] =
        info.game_date;
  else
  ...
  case TALK_REQUEST_MILITARY_AID:
    if (talkMsg->reply_type == REPLY_ACCEPT)
      relationChange = 0; // the AI never knows whether the player has really
                          // aided him in the war
    else
  ...
  case TALK_DEMAND_AID:
    if (talkMsg->reply_type == REPLY_ACCEPT) {
      ...
      relationChange =
          -(400 - pref_peacefulness) /
          10; // -30 to 40 points depending the peacefulness preference
    }
    break;

  case TALK_EXCHANGE_TECH:
    if (talkMsg->reply_type == REPLY_ACCEPT)
      relationChange = 5 + pref_research / 10; // +5 to +15
    else
      relationChange = -(5 + pref_research / 10); // -5 to -15
    break;
  ...
  if (relationChange < 0)
    relationChange -= relationChange * pref_forgiveness / 200;

  if (relationChange != 0)
    change_ai_relation_level(talkMsg->to_nation_recno, relationChange);

  //---- think about giving tribute to become more friendly with the nation so
  //it will accept our request next time ---//

  if (talkMsg->reply_type == REPLY_REJECT) {
    if (think_give_tribute_aid(talkMsg))
      return;

    //--- if our request was rejected, end treaty if the ai_nation_relation is
    //low enough ---//

    if (talkMsg->talk_id !=
            TALK_PROPOSE_ALLIANCE_TREATY && // the rejected request is not
                                            // alliance treaty
        nationRelation->status >= RELATION_FRIENDLY &&
        nationRelation->ai_relation_level <
            40 - pref_allying_tendency / 5) // 20 to 40
    {
      int talkId;
    ...
    //----- declare war if ai_relation_level==0 -----//
      ...
      if (config.ai_aggressiveness >= OPTION_HIGH || pref_peacefulness < 50) {
        talk_res.ai_send_talk_msg(talkMsg->to_nation_recno, nation_recno,
                                  TALK_DECLARE_WAR);
        ...
        if (config.ai_aggressiveness >= OPTION_VERY_HIGH ||
            (config.ai_aggressiveness >= OPTION_HIGH &&
             pref_peacefulness < 50)) {
          if (largest_town_recno) {
            think_capture_new_enemy_town(
                town_array[largest_town_recno],
                1); // 1-use forces from all camps to attack the target
          }
        }
      }
    }
...
int Nation::think_propose_friendly_treaty() {
  ...
  if (bestNationRecno) {
    talk_res.ai_send_talk_msg(bestNationRecno, nation_recno,
...
int Nation::think_end_treaty() {
  ...
  for (int i = 1; i <= nation_array.size(); i++) {
    ...
    if (nationRelation->ai_secret_attack ||
      ...
      if (!talk_res.can_send_msg(i, nation_recno,
                                 nationRelation->status == RELATION_FRIENDLY
...
int Nation::think_request_buy_food() {
  ...
  if (yearFoodChange > 0) {
    ...
    if (food >
        neededFoodLevel) // one to three times (based on pref_food_reserve) of
                         // the food needed in a year,
      return 0;
  }
  ...
  for (i = 1; i <= nation_array.size(); i++) {
    ...
    if (nationPtr->food < 500) // if the nation is short of food itself. The
                               // minimum request purchase qty is 500
      continue;
    ...
    if (nationPtr->yearly_food_change() < 0 && nationPtr->food < 1500) {
      continue;
    }
    ...
    curRating = relationStatus * 20 + (int)nationPtr->food / 100 +
  ...
  if (food < neededFoodLevel / 4) // if we need the food badly
  {
    ...
    if (bestNation->food > bestNation->all_population() *
                               UNIT_FOOD_YEAR_CONSUMPTION *
                               5 && // if the nation has plenty of food
        bestNation->cash < bestNation->fixed_expense_365days() /
                               2) // if the nation runs short of cash
    {
      buyPrice = 5;
    } else
      buyPrice = 10;
  ...
  talk_res.ai_send_talk_msg(bestNation->nation_recno, nation_recno,
                            TALK_REQUEST_BUY_FOOD, buyQty, buyPrice);
  return 1;
}
...
int Nation::think_declare_war() {
  ...
  for (i = 1; i <= nation_array.size(); i++) {
    ...
    if (!ai_should_spend(
            100 -
            trade_rating(
                i))) // if trade_rating is 0, importanceRating will be 100, if
                     // trade_rating is 100, importanceRating will be 0
      continue;
  ...
  if (bestTargetNation) {
    if (should_diplomacy_retry(TALK_DECLARE_WAR, bestTargetNation)) {
      talk_res.ai_send_talk_msg(bestTargetNation, nation_recno,
                                TALK_DECLARE_WAR);
      return 1;
    }
  }
...
int Nation::think_give_tribute_aid(TalkMsg *rejectedMsg) {
  ...
  if (info.game_date <
      nationRelation->never_accept_until_date_array[talkId - 1])
    return 0;
  ...
  if (rejectedTalkId == TALK_PROPOSE_TRADE_TREATY) {
    rc =
        ai_trade_with_rating(talkNationRecno) > 100 - pref_trading_tendency / 2;
  }
    ...
    int acceptRating =
        200 - pref_trading_tendency / 4 - pref_allying_tendency / 4;
  ...
  if (rc) {
    ...
    talk_res.ai_send_talk_msg(talkNationRecno, nation_recno, talkId,
                              tributeAmount);

    nationRelation->set_never_accept_until_date(
        talkId, 100); // don't offer tribute too often

    //------ request again after giving tribute ----//

    nationRelation->last_talk_reject_date_array[rejectedTalkId - 1] =
        0; // reset the rejected talk id.
    nationRelation->never_accept_until_date_array[rejectedTalkId - 1] =
        0; // reset the rejected talk id.

    talk_res.ai_send_talk_msg(talkNationRecno, nation_recno, rejectedTalkId,
                              rejectedMsg->talk_para1, rejectedMsg->talk_para2);
  }
...
int Nation::think_demand_tribute_aid() {
  if (info.game_date <
      info.game_start_date + 180 +
          nation_recno *
              50) // don't ask for tribute too soon, as in the beginning, the
  ...
  for (int i = totalNation; i > 0; i--) {
    ...
    if (talkId == TALK_DEMAND_TRIBUTE) {
      ...
      //---- if this is a Fryhtan nation, the tendency to request tribute is
      //higher -----//

      if (is_monster() &&
          nationPtr->is_human()) // and the target is a human nation
      {
        curRating *= 2;
      ...
      requestRating =
          20 + trade_rating(nationRecno) / 2 + (100 - pref_peacefulness) / 3;
      ...
      if (cash > fixedExpense * (50 + pref_cash_reserve) /
                     300 && // if the nation is runing short of cash, don't wait
                            // a while until next retry, retry immediately
    ...
    if (curRating > requestRating) {
      ...
      talk_res.ai_send_talk_msg(nationRecno, nation_recno, talkId,
                                tributeAmount);
...
int Nation::think_exchange_tech() {
  ...
  for (int i = totalNation; i > 0; i--) {
    ...
    for (techId = 1; techId <= tech_res.tech_count; techId++) {
      ...
      //)
      for (int j = 1; j <= techInfo->max_tech_level(nation_recno) ||
                      j <= techInfo->max_tech_level(nationRecno);
           j++)
      // ####### end Gilbert 24/12 ####//
      {
        if (!giveTechId) {
    ...
    //TALK_EXCHANGE_TECH, 			giveTechId, getTechId, giveTechLevel, getTechLevel );
    talk_res.ai_send_talk_msg(nationRecno, nation_recno, TALK_EXCHANGE_TECH,
                              giveTechId, getTechId, 0, giveTechLevel,
                              getTechLevel); // 0=don't force send
...
//------ End of function Nation::think_exchange_tech ------//
...
int Nation::think_request_surrender() {
  if (info.game_date <
      info.game_start_date +
          365 * 2) // don't offer to buy throne in the first 2 years of the game
    return 0;
  ...
  if (profit_365days() < 0 &&
      cash <
          20000) // don't ask if we are losing money and the cash isn't plenty
    return 0;
  ...
  for (i = 0; i < totalNation; i++) {
    ...
    talk_res.ai_send_talk_msg(
        nationRecno, nation_recno, TALK_REQUEST_SURRENDER,
        offerAmount / 10); // divide by 10 to cope with <short>'s upper limit
