...
void Nation::think_diplomacy()
{
    ...
    if (nation_array.is_deleted(nationRecno)) // the nation may have been deleted, if the nation
...
int Nation::should_diplomacy_retry(int talkId, int nationRecno)
{
    ...
    if (talkId == TALK_DEMAND_TRIBUTE || talkId == TALK_DEMAND_AID || talkId == TALK_EXCHANGE_TECH)
    {
    ...
    if (info.game_date < get_relation(nationRecno)->never_accept_until_date_array[talkId - 1])
    {
        return 0;
    }

    return info.game_date > get_relation(nationRecno)->last_talk_reject_date_array[talkId - 1] + retryInterval;
}
...
void Nation::ai_notify_reply(int talkMsgRecno)
{
    ...
    if (talkMsg->reply_type == REPLY_REJECT)
    {
        ...
        if (talkMsg->talk_id != TALK_PROPOSE_ALLIANCE_TREATY && // the rejected request is not
                                                                // alliance treaty
        ...
        else if (nationRelation->ai_relation_level == 0)
        {
            ...
            if (config.ai_aggressiveness >= OPTION_HIGH || pref_peacefulness < 50)
            {
                    ...
                    (config.ai_aggressiveness >= OPTION_HIGH && pref_peacefulness < 50))
                {
                    ...
                    {
                        think_capture_new_enemy_town(town_array[largest_town_recno],
...
int Nation::think_propose_friendly_treaty()
{
    ...
    for (int i = 1; i <= nation_array.size(); i++)
    {
        ...
        if (!nationRelation->has_contact || nationRelation->status >= RELATION_FRIENDLY)
            continue;
...
int Nation::think_end_treaty()
{
    ...
    for (int i = 1; i <= nation_array.size(); i++)
    {
        ...
        if (nationRelation->ai_secret_attack || (nationRelation->ai_relation_level < 30 && trade_rating(i) < 50))
        {
            //--- don't change terminate treaty too soon ---//

            if (info.game_date <
                nationRelation->last_change_status_date + 60 + pref_honesty / 2) // only after 60 to 110 days
                continue;

            //----------------------------------------//

            if (!talk_res.can_send_msg(i, nation_recno,
                                       nationRelation->status == RELATION_FRIENDLY ? TALK_END_FRIENDLY_TREATY
...
int Nation::think_request_buy_food()
{
    ...
    else
    {
        ...
        if (food > neededFoodLevel) // one to three times (based on pref_food_reserve) of
    ...
    for (i = 1; i <= nation_array.size(); i++)
    {
        ...
        curRating = relationStatus * 20 + (int)nationPtr->food / 100 + (int)nationPtr->yearly_food_change() / 10;
    ...
    {
        if (bestNation->food >
                bestNation->all_population() * UNIT_FOOD_YEAR_CONSUMPTION * 5 && // if the nation has plenty of food
...
int Nation::think_declare_war()
{
    ...
    for (i = 1; i <= nation_array.size(); i++)
    {
        ...
        if (!ai_should_spend(100 - trade_rating(i))) // if trade_rating is 0, importanceRating will be 100, if
        ...
        targetStrength = targetNation->military_rank_rating() + targetNation->population_rank_rating() / 2 +
...
int Nation::think_give_tribute_aid(TalkMsg *rejectedMsg)
{
    ...
    else if (rejectedTalkId == TALK_PROPOSE_FRIENDLY_TREATY || rejectedTalkId == TALK_PROPOSE_ALLIANCE_TREATY)
    {
    ...
    {
        rc = talkNation->military_rank_rating() > military_rank_rating() + (100 - pref_peacefulness) / 2;
    }
    ...
    if (rc)
    {
        ...
        talk_res.ai_send_talk_msg(talkNationRecno, nation_recno, rejectedTalkId, rejectedMsg->talk_para1,
                                  rejectedMsg->talk_para2);
    }
...
{
    if (info.game_date <
        info.game_start_date + 180 + nation_recno * 50) // don't ask for tribute too soon, as in the beginning, the
    ...
    for (int i = totalNation; i > 0; i--)
    {
        ...
        if (talkId == TALK_DEMAND_TRIBUTE)
        {
            ...
            requestRating = 20 + trade_rating(nationRecno) / 2 + (100 - pref_peacefulness) / 3;
        ...
        else
        {
            ...
            if (cash >
                    fixedExpense * (50 + pref_cash_reserve) / 300 && // if the nation is runing short of cash, don't
                                                                     // wait a while until next retry, retry immediately
...
int Nation::think_exchange_tech()
{
    ...
    for (int i = totalNation; i > 0; i--)
    {
        ...
        for (techId = 1; techId <= tech_res.tech_count; techId++)
        {
            ...
            //)
            for (int j = 1; j <= techInfo->max_tech_level(nation_recno) || j <= techInfo->max_tech_level(nationRecno);
                 j++)
            // ####### end Gilbert 24/12 ####//
            {
                if (!giveTechId)
        ...
        // );
        talk_res.ai_send_talk_msg(nationRecno, nation_recno, TALK_EXCHANGE_TECH, giveTechId, getTechId, 0,
                                  giveTechLevel,
...
int Nation::think_request_surrender()
{
    ...
    for (i = 0; i < totalNation; i++)
    {
        ...
        talk_res.ai_send_talk_msg(nationRecno, nation_recno, TALK_REQUEST_SURRENDER,
