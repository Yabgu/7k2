...
// ------ begin of function ScenarioEditor::~ScenarioEditor -------//
//
ScenarioEditor::~ScenarioEditor() { deinit(); }
// ------ end of function ScenarioEditor::~ScenarioEditor -------//
...
void ScenarioEditor::disp_menu_bar(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    button_menu_on.create_text(
        ZOOM_X1 + 5, ZOOM_Y1 + 5, ZOOM_X1 + 105, ZOOM_Y1 + 45,
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    for (i = 0; i < button_edit_mode.button_num; ++i) {
      ...
      button_edit_mode[i].create_text(
          x, y, x + 100, y + 40,
    ...
    button_blacken_map.create_text(
        x, y, x + 100, y + 40,
...
void ScenarioEditor::disp(int refreshFlag) {
  if (refreshFlag == INFO_REPAINT || edit_mode != last_edit_mode ||
...
//
void ScenarioEditor::draw_marker(int validAction, int markerX1, int markerY1,
                                 int markerX2, int markerY2) {
  // ------ draw marker -------//
  ...
  for (yLoc = markerY1; yLoc <= markerY2; ++yLoc) {
    for (xLoc = markerX1; xLoc <= markerX2; ++xLoc) {
      ...
      world.zoom_matrix->put_bitmapW_offset(
          ...
          maskBitmap.bitmap_ptr(), maskBitmap.left_edge, maskBitmap.top_edge, 0,
          0);
    }
  }
}
