...
//
int FirmMonsterTrain::think_del() { return 0; }
...
void FirmMonsterTrain::think_train_new() {
  ...
  // ####### begin Gilbert 23/2 #########//
  // avoid 0/0 case
  int unitPercent = unitInfo->nation_all_unit_count(nation_recno) == 0
                        ? 0
                        : 100 * unitInfo->nation_all_unit_count(nation_recno) /
                              nationPtr->total_unit_count;
  // ####### end Gilbert 23/2 #########//

  if (!nationPtr->ai_should_spend(
          80 - unitPercent,
          unitInfo->build_cost) // , unitInfo->build_live_points_cost)
                                // // do not pass live point cost as the cost is
                                // low enough to be ingored.
      || nationPtr->cash <= 0) {
    return;
  }
...
int FirmMonsterTrain::think_transfer_unit() {
  ...
  for (int j = 0; j < nationPtr->ai_camp_count; j++) {
    ...
    if (!firmCamp->is_soldier_full()) {
      curRating = world.distance_rating(center_x, center_y, firmCamp->center_x,
                                        firmCamp->center_y);
