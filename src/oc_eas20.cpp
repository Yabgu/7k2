...
int CampaignEastWest::stage_20_create_game()
{
    ...
    create_offensive_structure(STAGE_20_EASTERN_NATION_RECNO,
                               campaign_difficulty); // build 2 structures if the difficulty > 3
...
int CampaignEastWest::stage_20_create_firm()
{
    ...
    if (eastSituation == 1)
    {
        ...
        for (int i = 0; i < nationPtr->ai_camp_count; i++)
        {
            ...
            int addCount = MAX_SOLDIER - firmCamp->soldier_count -
                           misc.random(3); // don't always full up, sometimes leave a free slots empty
    ...
    if (eastSituation == 2)
    {
        ...
        int townRecno2 =
            random_pick_town_with_camp(STAGE_20_EASTERN_NATION_RECNO, 2); // only pick towns with <= 2 links
