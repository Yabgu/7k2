    ...
    for (int i = 0; i < MAX_FORT_ARCHER; ++i)
    {
        ...
        else if (i == current_archer_count)
        {
            ...
            if (subTowerHeight > 0)
            {
                ...
                vga.active_buf->put_bitmap_area(towerX, towerY, towerBuilding, 0, towerHeight - subTowerHeight,
    ...
    for (int inc = -1; inc <= 1; inc += 2)
    {
        ...
        for (int i = inc >= 0 ? 0 : raceCount - 1; i >= 0 && i < raceCount; i += inc)
        {
            ...
            if (inc < 0)
            {
                ...
                info.draw_unit_icon(x + SOLDIER_X_SPACING / 2, y, unitId, townPtr->nation_recno, windowX1, windowY1,
                                    windowX2, windowY2, 1);
            }
            else
            {
                ...
                if (info.draw_unit_icon(x + SOLDIER_X_SPACING / 2, y, unitId, townPtr->nation_recno, windowX1, windowY1,
...
static void disp_tower_count(ButtonCustom *button, int repaintBody)
{
    vga.active_buf->put_bitmap(button->x1, button->y1,
                               image_button.read(button->pushed_flag ? (char *)"NUM-D" : (char *)"NUM-U"));

    font_bible.center_put(button->x1, button->y1, button->x2, button->y2,
                          button->custom_para.ptr ? (char *)button->custom_para.ptr
