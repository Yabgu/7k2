...
void FirmFort::disp_camp_info(int dispY1, int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    button_tower_count.create(
        INFO_X1 + 13 + 3 * BUTTON_DISTANCE + 6, INFO_Y1 + 235 + 6,
        INFO_X1 + 13 + 3 * BUTTON_DISTANCE + 6 + TOWER_COUNT_BUTTON_WIDTH - 1,
        INFO_Y1 + 235 + 6 + TOWER_COUNT_BUTTON_HEIGHT - 1, disp_tower_count,
  ...
  for (int i = 0; i < MAX_FORT_ARCHER; ++i) {
    ...
    if (i < current_archer_count) {
      ...
      int subTowerHeight =
          train_archer_progress * towerHeight / MAX_TRAIN_ARCHER_PROGRESS;
      if (subTowerHeight > 0) {
        if (vga.active_buf->is_front)
          mouse.hide_area(towerX, towerY, towerX + towerWidth - 1,
                          towerY + towerHeight - 1);
        vga.active_buf->put_bitmap_area(towerX, towerY, towerBuilding, 0,
                                        towerHeight - subTowerHeight,
                                        towerWidth - 1, towerHeight - 1);
        if (vga.active_buf->is_front)
          mouse.show_area();
      }
    } else {
  ...
  help.set_help(INFO_X1 + 176, INFO_Y1 + 278, INFO_X1 + 176 + 39,
                INFO_Y1 + 278 + 48, "TOWERS");
}
...
void FirmFort::disp_train_menu(int refreshFlag) {
  ...
  for (int inc = -1; inc <= 1; inc += 2) {
    ...
    for (int i = inc >= 0 ? 0 : raceCount - 1; i >= 0 && i < raceCount;
      ...
      int x =
          INFO_X1 + 18 + ((raceId - 1) % SOLDIER_PER_ROW) * SOLDIER_X_SPACING;
      ...
      if (inc < 0) {
        ...
        info.draw_unit_icon(x + SOLDIER_X_SPACING / 2, y, unitId,
        ...
        if (info.draw_unit_icon(
                x + SOLDIER_X_SPACING / 2, y, unitId, townPtr->nation_recno,
        ...
        font_whbl.center_put(x, yLoyalty, x + SOLDIER_X_SPACING,
...
void FirmFort::detect_train_menu() {
  ...
  for (int i = 0; i < raceCount; ++i) {
    ...
    if (info.draw_unit_icon(x + SOLDIER_X_SPACING / 2, y, unitId,
                            townPtr->nation_recno, windowX1, windowY1, windowX2,
                            windowY2, 8) &
        8) {
