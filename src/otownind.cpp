...
void Town::think_independent_town() {
  ...
  if (rebel_recno) // if this is a rebel town, its AI will be executed in
                   // Rebel::think_town_action()
    return;
...
void Town::think_independent_set_link() {
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    ...
    if (firmPtr->cast_to_FirmCamp()) // a town cannot change its status with a
                                     // military camp
      continue;
...
int Town::think_independent_form_new_nation() {
  ...
  if (!nation_array
           .can_form_new_ai_nation()) // this function looks at config
                                      // parameters like config.new_town_emerge
    return 0;

  //----------------------------------------------//

  if (!can_independent_form_new_nation()) // this function just returns whether
                                          // this town can form a nation
    return 0;
...
int Town::form_new_nation() {
  ...
  int nationRecno = nation_array.new_nation(NATION_HUMAN, NATION_AI, race_id,
  ...
  int kingUnitRecno =
      unit_array.add_unit(unitId, nationRecno, RANK_KING, 100, xLoc, yLoc);
  ...
  nationPtr->set_king(kingUnitRecno,
  ...
  err_when(rebel_recno); // rebel has its own function in Rebel, this shouldn't
                         // be called

  set_nation(nationRecno); // set the town at last because set_nation() will
                           // delete the Town object
  ...
  for (int i = nation_array.size(); i > 0; i--) {
    ...
    nationPtr->get_relation(i)->set_never_accept_until_date(
        i, 365 * 2); // at least two years later
  }
  ...
  switch (misc.random(10)) {
    ...
    //			tech_res[ misc.random(tech_res.tech_count)+1
    //]->set_nation_tech_level(nationRecno, 1); 			break;
  ...
  case 4: // random leader units
    ...
    for (int i = 0; i < mobileCount && recruitable_pop(0) > 0;
         i++) // 0-don't recruit spies
    {
      int unitRecno =
          mobilize_town_people(1, 0); // 1-dec pop, 0-don't mobilize spies
...
int Town::think_independent_unit_join_nation() {
  ...
  independent_unit_join_nation_min_rating -=
      2; // make it easier to join nation everytime it's called
         // -2 each time, adding of 30 after a unit has been recruited and
         // calling it once every 2 months make it a normal rate of joining once
         // a year per town
  ...
  for (i = town_array.size(); i > 0; i--) {
    ...
    if (townPtr->race_id !=
        race_id) // the town's race must be the same as the current town's race
      continue;
    ...
    if (!townPtr->nation_recno ||
        nation_array.is_deleted(townPtr->nation_recno))
      continue;
    ...
    if (info.game_date <
        nationPtr->last_independent_unit_join_date +
            90) // don't join too frequently, at most 3 months a unit
      continue;
  ...
  independent_unit_join_nation_min_rating =
      bestRating + 100 + misc.random(30); // reset it to a higher rating
...
int Town::independent_unit_join_nation(int toTownRecno) {
  ...
  int unitRecno = mobilize_town_people(
      1,
      0); // 1-dec population after mobilizing the unit, 0-don't mobilize spies
