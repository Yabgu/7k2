...
#define LINE4_Y (INFO_Y1 + 80)
    ...
    if (site_type == SITE_RAW)
    {
        ...
        font_whbl.center_put(INFO_X1 + 20, LINE1_Y, INFO_X2 - 10, LINE1_Y + 20, text_firm.str_alchemy_raw(object_id));
    ...
    else if (site_type == SITE_SCROLL)
    {
        ...
        // " Seat of Power"; 		font_whbl.center_put( INFO_X1+20,
        // LINE3_Y-10, INFO_X2-10,
        // LINE3_Y+20-10, str ); 		str = "and Invoke "; 		str +=
        // unit_res[godInfo->unit_id]->name; 		font_whbl.center_put(
        // INFO_X1+20, LINE4_Y-10, INFO_X2-10, LINE4_Y+20-10, str );
...
#define MAX_COINS_TYPE 8
    ...
    world.zoom_matrix->put_bitmap_offset(curX, curY, curZ, bmpPtr, offsetX, offsetY, NULL, 0, 1 | (drawFlag << 2));
}
...
void Site::draw_selected()
{
    ...
    if (locPtr->is_firm() || locPtr->is_town()) // do not draw the selection frame if there is a firm
...
void SiteArray::draw_dot()
{
    ...
    for (i = 1; i <= size(); i++)
    {
...

        {
            ...
            rowWritePtr =
                vga_back.buf_ptr(world.map_matrix->calc_loc_pos_x(rawPtr->map_x_loc, rawPtr->map_y_loc) + MAP_X1,
...
void Site::disp_edit_mode(int refreshFlag)
{
    ...
    case SITE_RAW:
        ...
        x = edit_res_x2 = (edit_res_enable ? font_blue : font_whbl)
