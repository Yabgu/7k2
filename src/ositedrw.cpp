...
static short edit_res_x1, edit_res_y1, edit_res_x2, edit_res_y2,
    edit_res_enable;
...
#define LINE4_Y (INFO_Y1 + 80)
  ...
  if (site_type == SITE_RAW) {
    ...
    vga.active_buf->put_bitmap_trans(INFO_X1 + 13, INFO_Y1 + 129,
                                     raw_res.interface_raw_icon(object_id));
  }
  ...
  //--------- scroll of power --------//
    ...
    //		str += " Scroll of Power*";
    //		font_whbl.center_put( INFO_X1+20, LINE1_Y-10, INFO_X2-10,
    //LINE1_Y+20-10, str );
    font_whbl.center_put(
        INFO_X1 + 20, LINE1_Y - 10, INFO_X2 - 10, LINE1_Y + 20 - 10,
        text_firm.str_scroll_if_1(race_res[godInfo->race_id]->name));

    //		str = "Owners may build the";
    //		font_whbl.center_put( INFO_X1+20, LINE2_Y-10, INFO_X2-10,
    //LINE2_Y+20-10, str ); 		str = race_res[godInfo->race_id]->name; 		str += "
    //Seat of Power"; 		font_whbl.center_put( INFO_X1+20, LINE3_Y-10, INFO_X2-10,
    //LINE3_Y+20-10, str ); 		str = "and Invoke "; 		str +=
    //unit_res[godInfo->unit_id]->name; 		font_whbl.center_put( INFO_X1+20,
    //LINE4_Y-10, INFO_X2-10, LINE4_Y+20-10, str );

    font_whbl.center_put_paragraph(
        INFO_X1 + 20, LINE2_Y - 10, INFO_X2 - 10, LINE4_Y + 20 - 10,
        text_firm.str_scroll_if_2(
            race_res[godInfo->race_id]->name,
            firm_res
                .get_build(firm_res[FIRM_BASE]->get_build_id(godInfo->race_id))
                ->name,
  ...
  //----------- gold coins -----------//
    ...
    font_snds.put(INFO_X1 + 20, LINE1_Y,
                  text_firm.str_treasure_if_1()); // "Treasure" );

    font_snds.put(INFO_X1 + 20, LINE2_Y,
                  text_firm.str_treasure_if_2()); // "Worth" );
    ...
    //	font_snds.put( INFO_X1+20, LINE2_Y, item_res.item_name( object_id,
    //reserve_qty ), 0, -1, 1 ); 	font_snds.put( INFO_X1+20, LINE3_Y,
    //item_res.item_desc( object_id, reserve_qty ) );

    font_whbl.center_put(INFO_X1 + 20, LINE1_Y, INFO_X2 - 10, LINE1_Y + 20,
                         item_res.item_name(object_id, reserve_qty));
    font_whbl.center_put(INFO_X1 + 20, LINE2_Y, INFO_X2 - 10, LINE2_Y + 20,
                         item_res.item_desc(object_id, reserve_qty));
    image_gameif.put_back(INFO_X1 + 13, INFO_Y1 + 129, "ITEMBASE");
    char *iconBitmap = item_res.item_interface(object_id);
    if (iconBitmap) {
      vga.active_buf->put_bitmap_trans_decompress(INFO_X1 + 13, INFO_Y1 + 129,
                                                  iconBitmap);
    }

    //	image_gameif.put_front( INFO_X1+13, INFO_Y1+129,
    //item_info_array[object_id-1].bitmap_idx);
  } else if (site_type == SITE_WEAPON_BLUEPRINT) {
    TechInfo *techInfo = tech_res[object_id];

    font_snds.put(INFO_X1 + 20, LINE1_Y,
                  text_firm.str_blueprint_if_1()); // "Scroll of Weapon" );

    font_snds.put(INFO_X1 + 20, LINE2_Y,
                  text_firm.str_blueprint_if_2()); // "Technology" );
...
void Site::detect_info() {
  ...
  if (mouse.single_click(INFO_X1 + 16, INFO_Y1 + 10, INFO_X2 - 6,
                         INFO_Y1 + 87)) {
...
void Site::draw(int curX, int curY, int curZ, int drawFlag) {
  ...
  const int SITE_DRAW_OFFSET_X =
      -LOCATE_WIDTH / 2 - (-ZOOM_LOC_X_WIDTH / 2 + -ZOOM_LOC_Y_WIDTH / 2);
  const int SITE_DRAW_OFFSET_Y =
...
#define MAX_COINS_TYPE 8
  ...
  case SITE_ITEM: {
    ...
    if (bmpPtr) {
      ...
      offsetY = ZOOM_LOC_X_HEIGHT + ZOOM_LOC_Y_HEIGHT -
                ((Bitmap *)bmpPtr)->get_height();
    } else {
  ...
  world.zoom_matrix->put_bitmap_offset(curX, curY, curZ, bmpPtr, offsetX,
                                       offsetY, NULL, 0, 1 | (drawFlag << 2));
}
...
void Site::draw_selected() {
  ...
  if (locPtr->is_firm() ||
      locPtr->is_town()) // do not draw the selection frame if there is a firm
                         // or town built on the top of the site
    return;
...
void SiteArray::draw_dot() {
  ...
  for (i = 1; i <= size(); i++) {
...

    {
      ...
      rowWritePtr = vga_back.buf_ptr(world.map_matrix->calc_loc_pos_x(
                                         rawPtr->map_x_loc, rawPtr->map_y_loc) +
                                         MAP_X1,
                                     world.map_matrix->calc_loc_pos_y(
                                         rawPtr->map_x_loc, rawPtr->map_y_loc) +
                                         MAP_Y1);
    ...
    if (siteWidth == 1 && siteHeight == 1) {
      ...
      if (writePtr[vgaBufPitch] != unexploredColor)
        writePtr[vgaBufPitch] = siteColor;
    } else { // minimize register variable at one time
      register short *writePtr;

      for (y = 0; y < siteHeight;
...
void Site::disp_edit_mode(int refreshFlag) {
  ...
  case SITE_RAW:
    ...
    x = edit_res_x2 =
        (edit_res_enable ? font_blue : font_whbl)
            .put((edit_res_x1 = x + 20), (edit_res_y1 = y), reserve_qty, 0, x2);
  ...
  case SITE_GOLD_COIN: {
    ...
    edit_id_enable = 1;
    x = font_whbl.put(x1, y, text_firm.str_treasure_if_2(), 0,
                      x2); // "Worth" );
    x = edit_id_x2 = font_blue.put((edit_id_x1 = x + 20), (edit_id_y1 = y),
                                   misc.format(object_id, 2), 0, x2);
    edit_id_y2 = edit_id_y1 + font_blue.max_font_height;
    y += 20;

    edit_res_enable = 1;
    x = font_whbl.put(x1, y, text_editor.str_player_live_points(), 0,
                      x2); // "Life Points" );
    x = edit_res_x2 = font_blue.put((edit_res_x1 = x + 20), (edit_res_y1 = y),
                                    reserve_qty, 0, x2);
    edit_res_y2 = edit_res_y1 + font_blue.max_font_height;
    y += 20;
  } break;
  ...
  case SITE_ITEM:
    ...
    if (sys.debug_session) {
      edit_res_enable = 1;
      x = edit_res_x2 = font_blue.put((edit_res_x1 = x1), (edit_res_y1 = y),
                                      reserve_qty, 0, x2);
      edit_res_y2 = edit_res_y1 + font_blue.max_font_height;
    }
    y += 20;
...
int Site::detect_edit_mode() {
  ...
  if (edit_id_enable) {
    if (mouse.any_click(edit_id_x1, edit_id_y1, edit_id_x2, edit_id_y2,
                        LEFT_BUTTON)) {
  ...
  if (edit_res_enable) {
    if (mouse.any_click(edit_res_x1, edit_res_y1, edit_res_x2, edit_res_y2,
                        LEFT_BUTTON)) {
