...
//	[int] cap		 = set all letter to Cap letter (default : 0)
//
void Font::right_put(int x, int y, const char *textPtr, char clearBack,
                     char cap) {
...
int Font::text_height(int lineSpace) {
  return font_height * text_line_count + lineSpace * (text_line_count - 1);
}
...
void Font::put_paragraph(int x1, int y1, int x2, int y2, const char *textPtr,
  ...
  while (1) {
    ...
    if (newWord) {
      if (x - 1 > x2) // it is still okay if x-1==x2 because x is the next
                      // pixel, not the last displayed pixel
      {
                                             ...
                                             // word which will be printed
      {
        ...
        //----------- display char ------------//
          ...
          if (fontInfo->width > 0) {
            ...
            {
              //							if(
              //flag_hyper_field ) 								mainColor = HYPER_FIELD_COLOR;
    ...
    if (*textPtr == ' ' || *textPtr == '\n' || *textPtr == '\0' ||
        *textPtr == TEXT_END_CHAR) // not space
      newWord = 1;
    else {
      textChar = *((unsigned char *)textPtr); // textChar is <unsiged char>
...
void Font::center_put_paragraph(int x1, int y1, int x2, int y2,
                                const char *desStr, int lineSpace,
...
// Return : <int> lastX, the x coordination of the last pixel of last font
//
int Font::center_put(int x1, int y1, int x2, int y2, const char *desStr,
                     char clearBack, int cap) {
...
//                  (default : 1 )
//
void Font::put_field(int x1, int y1, const char *desStr, int x2, int value,
                     int format) {
...
//                   (default : 1 )
//
void Font::put_field(int x1, int y1, const char *desStr, int x2, double value,
                     int format) {
  vga_util.d3_panel_up(x1, y1, x2, y1 + font_height + 6);
...
// <char*> value  = value
//
void Font::put_field(int x1, int y1, const char *desStr, int x2,
                     const char *value) {
  vga_util.d3_panel_up(x1, y1, x2, y1 + font_height + 6);
...
// [char*] helpCode  = help code
//
void Font::field(int xDes, int y1, const char *desStr, int xValue,
                 const char *value, int xEnd, int refreshFlag,
                 const char *helpCode) {
  int x2;
...
void Font::center_put_to_buffer(char *dest, int destPitch, int x1, int y1,
                                int x2, int y2, const char *desStr) {
