...
void NationMonster::think_attack_hostile_nation() {
  ...
  if (misc.random(2) == 0) {
    ...
    for (int i = firm_array.size(); i > 0; i--) {
      ...
      if (firmPtr->nation_ptr()->get_relation_status(nation_recno) ==
          RELATION_HOSTILE) {
    ...
    for (int i = town_array.size(); i > 0; i--) {
      ...
      if (townPtr->nation_ptr()->get_relation_status(nation_recno) ==
          RELATION_HOSTILE) {
  ...
  ai_attack_target(targetBaseObj->obj_loc_x1(), targetBaseObj->obj_loc_y1(),
                   targetBaseObj->obj_power());
}
...
void NationMonster::think_buy_independent_lair() {
  ...
  for (int callLoop = 1; callLoop <= 2; callLoop++) {
    ...
    for (int i = firm_array.size(); i > 0; i--) {
      ...
      //--- only buy Fryhtans that we don't have, otherwise it is more cost
      //effective to build them on our own ---//
      ...
      if (callLoop == 1) {
        ...
        for (int j = 0; j < targetFirm->linked_firm_count; j++) {
          FirmLair *firmLair =
              firm_array[targetFirm->linked_firm_array[j]]->cast_to_FirmLair();
        ...
        for (int j = 0; j < ai_camp_count; j++) {
          ...
          curRating =
              world.distance_rating(targetFirm->center_x, targetFirm->center_y,
        ...
        if (bestLair) {
          ...
          if (!ai_should_spend(20 + pref_peacefulness / 2,
                               (float)bestLair->grant_to_non_own_lair_cost()))
            break;
...
int NationMonster::think_acquire_live_points() {
  ...
  for (i = town_array.size(); i > 0; i--) {
    ...
    if (townPtr->population <=
        (100 - pref_monster_aggressiveness) /
            20) // do not attack towns with too fewer people
      continue;
    ...
    if (townPtr->nation_recno) {
      ...
      } else // Netural to Alliance
      {
        ...
        static short delayDaysArray[] = {
            1000, 720, 365, 180, 90}; // very low, low, medium, high, very high

        if (info.game_date - info.game_start_date <
            delayDaysArray[config.ai_aggressiveness])
          continue;
        ...
        if (relationStatus == RELATION_NEUTRAL) // attack neutral
          ...
          rc = pref_honesty <
               50; // friendly or alliance, break treaty if honesty < 50
      }
    }
    ...
    if (rc) {
      ...
      if (townPtr->nation_recno &&
          nation_array[townPtr->nation_recno]->is_monster())
        curRating -= 50;
      ...
      if (townPtr->nation_recno &&
          !nation_array[townPtr->nation_recno]->is_ai()) {
  ...
  int defenseMode = live_points < 200; // if live points is too small, set
                                       // defense mode on to attack aggressively

  ai_attack_target(bestTown->loc_x1, bestTown->loc_y1, bestTownDefense,
                   defenseMode); // 1-set it to defense mode so that it can
                                 // attack aggressively
