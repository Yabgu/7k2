        ...
        for (int i = firm_array.size(); i > 0; i--)
        {
            ...
            else
            {
                ...
                for (int j = 0; j < ai_camp_count; j++)
                {
                    ...
                    curRating = world.distance_rating(targetFirm->center_x, targetFirm->center_y, firmLair->center_x,
                                                      firmLair->center_y);
...
int NationMonster::think_acquire_live_points()
{
    ...
    for (i = town_array.size(); i > 0; i--)
    {
        ...
        if (townPtr->population <=
            (100 - pref_monster_aggressiveness) / 20) // do not attack towns with too fewer people
            continue;
        ...
        else if (townPtr->nation_recno == 0 && townPtr->resistance(nation_recno) < MONSTER_COLLECT_TOWN_TRIBUTE_LOYALTY)
        {
            rc = 1;
        }
        ...
        if (rc)
        {
            ...
            curRating = 200 - curDefense / 10 +
                        world.distance_rating(nationCenterX, nationCenterY, townPtr->center_x, townPtr->center_y);
