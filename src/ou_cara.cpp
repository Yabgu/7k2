...
void UnitCaravan::set_stop(int stopId, int stopXLoc, int stopYLoc, char remoteAction)
{
    ...
    if (dest_stop_id)
    {
        if ((newStopFirmRecno = stop_array[dest_stop_id - 1].firm_recno) != oldStopFirmRecno)
        {
            ...
            err_when(firmPtr->firm_id != FIRM_MARKET && firmPtr->firm_id != FIRM_MINE &&
...
//------------- used to debug for multiplayer game ------------------//
    ...
    for (i = 0; i < MAX_STOP_FOR_CARAVAN; i++, nodePtr++)
    {
        ...
        if (!can_set_stop(nodePtr->firm_recno) || firmPtr->loc_x1 != nodePtr->firm_loc_x1 ||
...
//							if it is MAX_STOP_FOR_CARAVAN,
// this
// function will return 							the id. of the first
// valid stop.
//
...
int UnitCaravan::get_next_stop_id(int curStopId)
{
    ...
    else
    {
        ...
        if (!can_set_stop(stopPtr->firm_recno) || firmPtr->loc_x1 != stopPtr->firm_loc_x1 ||
...
void UnitCaravan::pre_process()
{
    ...
    if (is_visible() && stop_defined_num == 0 && nation_recno && world.get_loc(next_x_loc(), next_y_loc())->has_site())
    {
...
void UnitCaravan::caravan_on_way()
{
    ...
    if (cur_action == SPRITE_IDLE && journey_status != SURROUND_FIRM)
    {
        if (!firm_array.is_deleted(stopPtr->firm_recno))
        {
            ...
            if (nextXLoc >= firmPtr->loc_x1 - 1 && nextXLoc <= firmPtr->loc_x2 + 1 && nextYLoc >= firmPtr->loc_y1 - 1 &&
    ...
    if (journey_status == SURROUND_FIRM ||
        (nextXLoc == cur_order.loc_x && nextYLoc == cur_order.loc_y && cur_x == next_x &&
         cur_y == next_y && // move in a tile exactly
         (nextXLoc >= firmPtr->loc_x1 - 1 && nextXLoc <= firmPtr->loc_x2 + 1 && nextYLoc >= firmPtr->loc_y1 - 1 &&
          nextYLoc <= firmPtr->loc_y2 + 1))) // in the surrounding of the firm
    {
        ...
        if (nation_array[nation_recno]->get_relation(firmPtr->nation_recno)->trade_treaty)
        {
            ...
            for (int s = 1; s <= stop_defined_num; ++s)
            {
                ...
                if (!firm_array.is_deleted(stopFirmRecno) && firm_array[stopFirmRecno]->nation_recno == nation_recno)
                {
                    otherStopHasOwn = 1;
                }
            }
...
int UnitCaravan::appear_in_firm_surround(int &xLoc, int &yLoc, Firm *firmPtr)
{
    ...
    // surrounding 		7   x   3  14		of the firm and non-blocked, break
    // 6   5 4  ...		the test
    //---------------------------------------------------------//
    ...
    while (inside)
    {
        ...
        for (i = 0; i < limit; i++)
        {
            ...
            if (testXLoc < upperLeftBoundX || testXLoc > lowerRightBoundX || testYLoc < upperLeftBoundY ||
                testYLoc > lowerRightBoundY)
                continue;
    ...
    for (i = 0; i < MAX_STOP_FOR_CARAVAN; i++)
    {
        ...
        else
        {
            ...
            for (int x = 0; x < firmPtr->linked_firm_count; x++)
            {
                ...
                if (firmPtr2->nation_recno == nation_recno && firmPtr2->cast_to_FirmMarket())
                {
