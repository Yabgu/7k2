...
static char dummyCaravanEnableFlag[MAX_STOP_FOR_CARAVAN]
                                  [MAX_GOODS_SELECT_BUTTON];
...
static void update_caravan_stop_and_goods_to_dummy(UnitCaravan *caravanPtr) {
  ...
  for (int i = 0; i < MAX_STOP_FOR_CARAVAN; i++, stopPtr++) {
    ...
    dummyCaravanGoodsNum[i] =
        stopPtr->update_pick_up(dummyCaravanEnableFlag[i]);
  }
}
...
// <int> newPickUpType - set the pickup type of the specific stop. (0 -
// MAX_GOODS_SELECT_BUTTON-1) <int> remoteActoin  - remote action type
//
void UnitCaravan::set_stop_pick_up(int stopId, int newPickUpType,
                                   int remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <unit recno> <stop id> <new pick_up_type>
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_U_CARA_CHANGE_GOODS, 3 * sizeof(short));
    *shortPtr = sprite_recno;
    ...
    if (newPickUpType != AUTO_PICK_UP && newPickUpType != MANUAL_PICK_UP &&
        stop_array[stopId - 1].firm_id == FIRM_MARKET) {
...
//
void UnitCaravan::set_stop(int stopId, int stopXLoc, int stopYLoc,
                           char remoteAction) {
  //-------------------------------------------------------//
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_U_CARA_SET_STOP,
                                                         4 * sizeof(short));
    *shortPtr = sprite_recno;
    shortPtr[1] = stopId;
    shortPtr[2] = stopXLoc;
  ...
  if (stopPtr->firm_recno == firmPtr->firm_recno) {
    err_when(stopPtr->firm_loc_x1 != firmPtr->loc_x1 ||
  ...
  short oldStopFirmRecno =
      dest_stop_id ? stop_array[dest_stop_id - 1].firm_recno : 0;
  ...
  if (dest_stop_id) {
    if ((newStopFirmRecno = stop_array[dest_stop_id - 1].firm_recno) !=
        oldStopFirmRecno) {
      firmPtr = firm_array[newStopFirmRecno];
      err_when(firmPtr->firm_id != FIRM_MARKET &&
               firmPtr->firm_id != FIRM_MINE &&
               firmPtr->firm_id != FIRM_FACTORY);
...
void UnitCaravan::del_stop(int stopId, char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_U_CARA_DEL_STOP,
                                                         2 * sizeof(short));
    *shortPtr = sprite_recno;
    shortPtr[1] = stopId;
    return;
  ...
  //------ stop is deleted before receiving this message, thus, ignore invalid
  //message -----//
...
//------------- used to debug for multiplayer game ------------------//
  ...
  if (stop_defined_num == 1) {
    memset(insertNodePtr + 1, 0,
           sizeof(CaravanStop) * (MAX_STOP_FOR_CARAVAN - 1));
    dest_stop_id = 1;
    return;
  }
  ...
  if (stop_defined_num < MAX_STOP_FOR_CARAVAN)
    memset(stop_array + stop_defined_num, 0,
           sizeof(CaravanStop) * (MAX_STOP_FOR_CARAVAN - stop_defined_num));
  ...
  for (debugCount = 0; debugCount < stop_defined_num; debugCount++)
    err_when(
        stop_array[debugCount].firm_recno &&
        stop_array[debugCount].firm_recno ==
            stop_array[(debugCount + 1) % MAX_STOP_FOR_CARAVAN].firm_recno);
#endif

  //-----------------------------------------------------------------------------------------//
  // There should be at least one stop in the list.  Otherwise, clear all the
  // stops
  //-----------------------------------------------------------------------------------------//
  int ourFirmExist = 0;
  ...
  for (i = 0, dest_stop_id = 0, nodePtr = stop_array; i < stop_defined_num;
       i++, nodePtr++) {
    if (nodePtr->firm_recno == nextStopRecno) {
      ...
      firmPtr = firm_array[nodePtr->firm_recno];
      dist = misc.points_distance(xLoc, yLoc, firmPtr->center_x,
                                  firmPtr->center_y);
...
//---------//
void UnitCaravan::update_stop_and_goods_info() { update_stop_list(); }
...
int UnitCaravan::can_set_stop(int firmRecno) {
  ...
  case FIRM_MARKET:
    return nation_array[nation_recno]
        ->get_relation(firmPtr->nation_recno)
        ->trade_treaty;
...
//							if it is MAX_STOP_FOR_CARAVAN, this
//function will return 							the id. of the first valid stop.
//
...
void UnitCaravan::pre_process() {
  ...
  if (cur_action ==
      SPRITE_DIE) // if the unit is set to die mode in pre_process()
    return;
...
#define SURROUND_FIRM_WAIT_FACTOR 10
  ...
  if (stop_defined_num == 1) {
    ...
    if (firmXLoc1 != stopPtr->firm_loc_x1 ||
        firmYLoc1 != stopPtr->firm_loc_y1 ||
        (firmId != FIRM_MINE && firmId != FIRM_FACTORY &&
         firmId != FIRM_MARKET)) {
      update_stop_list();
      return;
    }
    ...
    if (curXLoc < firmXLoc1 - 1 || curXLoc > firmXLoc2 + 1 ||
        curYLoc < firmYLoc1 - 1 || curYLoc > firmYLoc2 + 1) {
...
//												NO_STOP_DEFINED
//if no valid stop 												SURROUND_FIRM if only one stop
//
void UnitCaravan::caravan_in_firm() {
  ...
  if (firm_array.is_deleted(entered_firm_recno)) {
    ...
    unit_array.disappear_in_firm(
        sprite_recno); // caravan also die if the market is deleted
    return;
  }
  ...
  if (locPtr->can_move(mobile_type))
    init_sprite(xLoc,
                yLoc); // appear in the location the unit disappeared before
  else {
    ...
    if (appear_in_firm_surround(xLoc, yLoc, firmPtr)) {
      ...
      wait_count = MAX_CARAVAN_WAIT_TERM *
                   10; //********* BUGHERE, continue to wait or ....
      return;
    }
  }
  ...
  entered_firm_recno = 0; // since entered_firm_recno is used to store the
                          // current market recno, reset before searching
  ...
  if (cur_action == SPRITE_IDLE && journey_status != SURROUND_FIRM) {
    if (!firm_array.is_deleted(stopPtr->firm_recno)) {
      ...
      if (nextXLoc >= firmPtr->loc_x1 - 1 && nextXLoc <= firmPtr->loc_x2 + 1 &&
          nextYLoc >= firmPtr->loc_y1 - 1 &&
          nextYLoc <= firmPtr->loc_y2 + 1) // hard code 1 for carvan size 1x1
        journey_status = SURROUND_FIRM;

      if (nextXLoc == cur_order.loc_x && nextYLoc == cur_order.loc_y &&
          !ignore_power_nation)
  ...
  if (journey_status == SURROUND_FIRM ||
      (nextXLoc == cur_order.loc_x && nextYLoc == cur_order.loc_y &&
       ...
       (nextXLoc >= firmPtr->loc_x1 - 1 && nextXLoc <= firmPtr->loc_x2 + 1 &&
        nextYLoc >= firmPtr->loc_y1 - 1 &&
        nextYLoc <= firmPtr->loc_y2 + 1))) // in the surrounding of the firm
  {
    ...
    stop_order();
    deinit_sprite(1); // the caravan enters the market now. 1-keep it selected
                      // if it is currently selected

    err_when(cur_x != -1);
    cur_x--; // set cur_x to -2, such that invisible but still process
             // pre_process()
    ...
    if (cur_action != SPRITE_MOVE) {
      ...
      // note: if return value is 0, cannot reach the firm.
      // //*********BUGHERE
      //----------------------------------------------------//
      move(firmPtr->loc_x1, firmPtr->loc_y1, true);
      journey_status = ON_WAY_TO_FIRM;
...
int UnitCaravan::appear_in_firm_surround(int &xLoc, int &yLoc, Firm *firmPtr) {
  int upperLeftBoundX =
      firmPtr->loc_x1 - 1; // the surrounding coordinates of the firm
  ...
  //		8   1   2  13		shown, if the location is the
  //surrounding 		7   x   3  14		of the firm and non-blocked, break 		6   5
  //4  ...		the test
  //---------------------------------------------------------//
  ...
  while (inside) {
    ...
    testYLoc = yLoc - count;
    for (i = 0; i < limit; i++) {
      if (testXLoc < 0 || testYLoc >= MAX_WORLD_X_LOC || testYLoc < 0 ||
          testYLoc >= MAX_WORLD_Y_LOC)
        continue;

      if (testXLoc < upperLeftBoundX || testXLoc > lowerRightBoundX ||
          testYLoc < upperLeftBoundY || testYLoc > lowerRightBoundY)
        continue;
...
short UnitCaravan::duplicate_caravan(char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_U_CARA_DUPLICATE, sizeof(short));
    *shortPtr = sprite_recno;
    return 0;
  }
  ...
  for (int i = 0; i < MAX_STOP_FOR_CARAVAN; i++, stopPtr++) {
    ...
    if (market && market->nation_recno == nation_recno) {
      ...
      if (unitRecno) {
        ...
        newCaravanPtr->default_market_trade_mode =
            caravanPtr->default_market_trade_mode;
        newStopPtr->pick_up_type = oldStopPtr->pick_up_type;
        for (materialInEveryStop = 0; materialInEveryStop < MAX_PICK_UP_GOODS;
             materialInEveryStop++)
          newStopPtr->pick_up_array[materialInEveryStop] =
              oldStopPtr->pick_up_array[materialInEveryStop];
        ...
        //				for( ; (j != i && j !=
        //i+MAX_STOP_FOR_CARAVAN-1) || !loopBreak ; ++j )
        //				{
        ...
        int stopCount;
        for (stopCount = MAX_STOP_FOR_CARAVAN; stopCount > 0;
             --stopCount, ++j) {
          if (j >= MAX_STOP_FOR_CARAVAN)
          ...
          if (!firm_array.is_deleted(oldStopPtr->firm_recno)) {
            ...
            newCaravanPtr->set_stop(k + 1, firmPtr->loc_x1, firmPtr->loc_y1,
                                    COMMAND_AUTO);

            newStopPtr->pick_up_type = oldStopPtr->pick_up_type;
            for (materialInEveryStop = 0;
                 materialInEveryStop < MAX_PICK_UP_GOODS; materialInEveryStop++)
              newStopPtr->pick_up_array[materialInEveryStop] =
                  oldStopPtr->pick_up_array[materialInEveryStop];

            //	newCaravanPtr->set_stop_pick_up(j+1, oldStopPtr->pick_up_type,
            //COMMAND_AUTO);
