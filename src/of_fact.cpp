...
//
void FirmFactory::init_derived() { auto_set_product(); }
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    //----------- if the firm is a market place ----------//
      ...
      for (j = 0; j < MAX_MARKET_GOODS; j++) {
        ...
        //--- if this raw material in this market hasn't been used by any
        //factories yet, then select it ---//
...
void FirmFactory::change_production(char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_FACTORY_CHG_PROD,
                                                         2 * sizeof(short));
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    if (firmPtr->firm_id == FIRM_MINE) {
      ...
      if (firmMine->next_output_firm_recno == firm_recno &&
        ...
        if (firmMine->nation_recno !=
            nation_recno) // make sure it has the cash to pay for the raw
                          // materials
        ...
        if (inputQty > 0) {
          ...
          if (firmMine->nation_recno != nation_recno)
            nationPtr->import_goods(IMPORT_RAW, firmMine->nation_recno,
                                    inputQty * RAW_PRICE);
        }
      }
    }
      ...
      if (firmMarket->next_output_firm_recno == firm_recno) {
        ...
        for (j = 0; j < MAX_MARKET_GOODS; j++, marketGoods++) {
          if (marketGoods->raw_id == product_raw_id &&
            ...
            inputQty =
                MIN(marketGoods->stock_qty, max_raw_stock_qty - raw_stock_qty);

            if (firmMarket->nation_recno !=
                nation_recno) // make sure it has the cash to pay for the raw
                              // materials
              inputQty = MIN(inputQty, nationPtr->cash / RAW_PRICE);
            ...
            if (inputQty > 0) {
              ...
              if (firmMarket->nation_recno != nation_recno)
                nationPtr->import_goods(IMPORT_RAW, firmMarket->nation_recno,
                                        inputQty * RAW_PRICE);
            }
          }
        }
...
void FirmFactory::manufacture(float maxMftQty) {
  ...
  if (nation_recno)
    yieldRate += 0.25f * tech_res[TECH_MANUFACTURING]->get_nation_tech_level(
                             nation_recno);
