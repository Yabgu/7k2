...
void FirmFactory::auto_set_product()
{
    ...
    for (i = 0; i < linked_firm_count; i++)
    {
        ...
        firmDistance = misc.points_distance(firmPtr->center_x, firmPtr->center_y, center_x, center_y);
...
void FirmFactory::input_raw()
{
    ...
    for (i = 0; i < linked_firm_count; i++)
    {
        ...
        if (firmPtr->firm_id == FIRM_MINE)
        {
            ...
            if (firmMine->next_output_firm_recno == firm_recno && firmMine->raw_id == product_raw_id &&
                firmMine->stock_qty > 0)
            {
                inputQty = MIN(firmMine->stock_qty, max_raw_stock_qty - raw_stock_qty);

                if (firmMine->nation_recno != nation_recno) // make sure it has the cash to pay for the raw
                                                            // materials
        ...
        if (firmPtr->firm_id == FIRM_MARKET)
        {
            ...
            if (firmMarket->next_output_firm_recno == firm_recno)
            {
                ...
                {
                    if (marketGoods->raw_id == product_raw_id && marketGoods->stock_qty > 0)
                    {
                        inputQty = MIN(marketGoods->stock_qty, max_raw_stock_qty - raw_stock_qty);

                        if (firmMarket->nation_recno != nation_recno) // make sure it has the cash to pay for the raw
                                                                      // materials
                            inputQty = MIN(inputQty, nationPtr->cash / RAW_PRICE);
