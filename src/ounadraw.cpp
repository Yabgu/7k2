...
// ---//
//
// [int] filterUnitClassId - if given, only display units whose type is
// filterUnitClassId
//
void UnitArray::draw_dot(int filterUnitClassId) {
  ...
  for (i = 1; i <= arraySize; i++) {
    ...
    if (!unitPtr->selected_flag) // if the unit is currently selected, ignore
                                 // the filters
    {
      if (filterUnitClassId &&
          unit_res[unitPtr->unit_id]->unit_class != filterUnitClassId)
        continue;
      ...
      if (world.map_matrix->filter_nation_flag &&
          world.map_matrix->filter_nation_recno != unitPtr->nation_recno)
        continue;
    }
    ...
    if (lineColor != TRANSPARENT_CODE) {
      ...
      if (unitPtr->cur_x_loc() != unitPtr->go_x_loc() ||
          unitPtr->cur_y_loc() != unitPtr->go_y_loc()) {
        lineToX = world.map_matrix->calc_loc_pos_x(unitPtr->go_x_loc(),
                                                   unitPtr->go_y_loc()) +
                  MAP_X1;
        lineToY = world.map_matrix->calc_loc_pos_y(unitPtr->go_x_loc(),
                                                   unitPtr->go_y_loc()) +
                  MAP_Y1;
        lineFromX = world.map_matrix->calc_loc_pos_x(unitPtr->next_x_loc(),
                                                     unitPtr->next_y_loc()) +
                    MAP_X1;
        lineFromY = world.map_matrix->calc_loc_pos_y(unitPtr->next_x_loc(),
                                                     unitPtr->next_y_loc()) +
                    MAP_Y1;
      ...
      if (unitPtr->cur_path && unitPtr->cur_path_result_id > 0) {
        ...
        for (j = unitPtr->cur_path_result_id; j > 0; j--) {
          ...
          lineFromX =
              world.map_matrix->calc_loc_pos_x(locFromX, locFromY) + MAP_X1;
          lineFromY =
              world.map_matrix->calc_loc_pos_y(locFromX, locFromY) + MAP_Y1;
    ...
    { // minimize register variable at one time
      ...
      if (world.map_matrix->map_mode != MAP_MODE_TRADE) {
        ...
        if (firm_array.selected_recno &&
            ...
            (((UnitCaravan *)unitPtr)->stop_array[0].firm_recno ==
                 firm_array.selected_recno ||
             ((UnitCaravan *)unitPtr)->stop_array[1].firm_recno ==
                 firm_array.selected_recno)) {
          nationColor = slowExcitedColorArray[unitPtr->nation_recno];
        } else {
          nationColor = nationColorArray[unitPtr->nation_recno];
      ...
      mapX = world.map_matrix->calc_loc_pos_x(unitPtr->cur_x_loc(),
                                              unitPtr->cur_y_loc()) +
             MAP_X1;
      mapY = world.map_matrix->calc_loc_pos_y(unitPtr->cur_x_loc(),
                                              unitPtr->cur_y_loc()) +
             MAP_Y1;
...
void UnitArray::draw_profile() {
  ...
  if (misc.get_time() >= lastDrawTime + 1000) {
    ...
    last_unit_process_town_defender_profile_time =
        unit_process_town_defender_profile_time;
    unit_process_town_defender_profile_time = 0L;

    last_unit_process_camp_defender_profile_time =
        unit_process_camp_defender_profile_time;
    ...
    last_unit_execute_build_firm_profile_time =
        unit_execute_build_firm_profile_time;
    unit_execute_build_firm_profile_time = 0L;

    last_unit_execute_settle_town_profile_time =
        unit_execute_settle_town_profile_time;
    ...
    last_unit_transform_fortress_profile_time =
        unit_transform_fortress_profile_time;
    unit_transform_fortress_profile_time = 0L;

    /*
    last_unit_check_build_firm_profile_time =
    unit_check_build_firm_profile_time; unit_check_build_firm_profile_time = 0L;
    */
    lastDrawTime = misc.get_time();
  }
  ...
  put_profile(y += 15, "Defend Town",
              last_unit_process_town_defender_profile_time);
  put_profile(y += 15, "Defend Camp",
              last_unit_process_camp_defender_profile_time);
  ...
  //	put_profile(y+=15, "Check Build"	,
  //last_unit_check_build_firm_profile_time);
  put_profile(y += 15, "Settle Town",
              last_unit_execute_settle_town_profile_time);
