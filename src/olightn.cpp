...
void Lightning::init(double fromX, double fromY, double toX, double toY, char energy)
{
    x = fromX;
    y = fromY;
    destx = toX;
...
void Lightning::draw_step(VgaBuf *vgabuf)
{
    ...
    if (!goal())
    {
        ...
        if (energy_level > 4)
        {
            if (prex >= bound_x1 + 2 && (int)x >= bound_x1 + 2 && prex <= bound_x2 - 2 && (int)x <= bound_x2 - 2 &&
                prey >= bound_y1 + 2 && (int)y >= bound_y1 + 2 && prey <= bound_y2 - 2 && (int)y <= bound_y2 - 2)
            {
                ...
                magic.draw_light_beam(&vga_back, prex, prey, x, y, 10, 4, 0, 80, 255, 255, 255, 255,
                                      vga.pixel_format_flag, 0);
        ...
        {
            if (prex >= bound_x1 + 1 && (int)x >= bound_x1 + 1 && prex <= bound_x2 - 1 && (int)x <= bound_x2 - 1 &&
                prey >= bound_y1 + 1 && (int)y >= bound_y1 + 1 && prey <= bound_y2 - 1 && (int)y <= bound_y2 - 1)
            {
                ...
                magic.draw_light_beam(&vga_back, prex, prey, x, y, 8, 2, 0, 80, 255, 255, 255, 255,
                                      vga.pixel_format_flag, 0);
            }
        }
        else
        {
            if (prex >= bound_x1 && (int)x >= bound_x1 && prex <= bound_x2 && (int)x <= bound_x2 && prey >= bound_y1 &&
                (int)y >= bound_y1 && prey <= bound_y2 && (int)y <= bound_y2)
            {
                //	vgabuf->line(prex, prey, (int) x, (int) y, OUTLIGHTNCOLOR);
                magic.draw_light_beam(&vga_back, prex, prey, x, y, 3, 1, 0, 80, 255, 255, 255, 255,
                                      vga.pixel_format_flag, 0);
            }
        }
    }
