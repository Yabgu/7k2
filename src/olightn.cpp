...
double Lightning::dist(double dx, double dy) { return sqrt(dx * dx + dy * dy); }
...
int Lightning::goal() { return (dist(destx - x, desty - y) < v); }
...
void Lightning::move_particle() {
  ...
  double randomAngle =
      ((rand_seed() & 255) / 128.0 - 1.0) * wide + attractionAngle;
  ...
  if (!goal()) {
    ...
    if (energy_level > 4) {
      if (prex >= bound_x1 + 2 && (int)x >= bound_x1 + 2 &&
        ...
        magic.draw_light_beam(&vga_back, prex, prey, x, y, 10, 4, 0, 80, 255,
                              255, 255, 255, vga.pixel_format_flag, 0);
        magic.draw_circle2(&vga_back, x, y, 25, 0, 0, 0, 80, 255,
                           vga.pixel_format_flag, 0);
        magic.draw_circle2(&vga_back, x, y, 8, 0, 0, 255, 255, 255,
                           vga.pixel_format_flag, 0);
      }
      ...
      if (prex >= bound_x1 + 1 && (int)x >= bound_x1 + 1 &&
        ...
        magic.draw_light_beam(&vga_back, prex, prey, x, y, 8, 2, 0, 80, 255,
                              255, 255, 255, vga.pixel_format_flag, 0);
      }
    } else {
      if (prex >= bound_x1 && (int)x >= bound_x1 && prex <= bound_x2 &&
          (int)x <= bound_x2 && prey >= bound_y1 && (int)y >= bound_y1 &&
          prey <= bound_y2 && (int)y <= bound_y2) {
        //	vgabuf->line(prex, prey, (int) x, (int) y, OUTLIGHTNCOLOR);
        magic.draw_light_beam(&vga_back, prex, prey, x, y, 3, 1, 0, 80, 255,
                              255, 255, 255, vga.pixel_format_flag, 0);
      }
    }
  }
