...
void NewsArray::disp()
{
    ...
    case MODE_NATION:
        rc = info.nation_report_mode == NATION_REPORT_TALK || info.nation_report_mode == NATION_REPORT_CHAT;
        break;
    }
...
int NewsArray::detect()
{
    ...
    if (sys.view_mode == MODE_NORMAL ||
        (sys.view_mode == MODE_NATION &&
         (info.nation_report_mode == NATION_REPORT_TALK || info.nation_report_mode == NATION_REPORT_CHAT)))
    {
...
int NewsArray::put(int detectAction)
{
    ...
    for (i = dispCount; i > 0; i--)
    {
        ...
        {
            err_when(talk_res.is_talk_msg_deleted(newsPtr->short_para1)); // it shouldn't have been deleted as when it's
...
#define NEWS_ICON_WIDTH 17
    ...
    if (detectAction)
    {
        ...
        if (rc)
        {
            if (id == NEWS_DIPLOMACY)
            {
                err_when(talk_res.is_talk_msg_deleted(short_para1)); // it shouldn't have been deleted as when it's
    ...
    if (id == NEWS_DIPLOMACY)
    {
        err_when(talk_res.is_talk_msg_deleted(short_para1)); // it shouldn't have been deleted as when it's deleted in
        ...
        if (talkMsgPtr->reply_type == REPLY_WAITING || talkMsgPtr->reply_type == REPLY_NOT_NEEDED)
        {
    ...
    news_font_ptr->put_paragraph(NEWS_X1 + NEWS_ICON_WIDTH + dateWidth, y, NEWS_X2, y + newsHeight, msgStr,
                                 NEWS_LINE_SPACE);
