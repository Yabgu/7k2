...
void NewsArray::default_setting() {
  ...
  news_who_option = NEWS_DISP_PLAYER; // default display news of groups
...
//
void NewsArray::set_font(Font *fontPtr) { news_font_ptr = fontPtr; }
...
int NewsArray::put(int detectAction) {
  ...
  for (i = size(); i >= MAX(1, last_clear_recno + 1); i--) {
    ...
    if (config.disp_news_flag == OPTION_DISPLAY_MAJOR_NEWS ||
        game.game_mode ==
            GAME_TUTORIAL) // only display major news in the tutorial
    {
      if (!newsPtr->is_major())
        continue;
    }
  ...
  for (i = dispCount; i > 0; i--) {
    ...
    if (newsPtr->id == NEWS_DIPLOMACY) {
      err_when(talk_res.is_talk_msg_deleted(
          newsPtr->short_para1)); // it shouldn't have been deleted as when it's
                                  // deleted in TalkRes(), it will be removed
                                  // from news_array immediately
    ...
    if (newsPtr->put(y, detectAction,
                     newsHeight)) // return 1 if reply diplomatic message
    {
  ...
  if (!detectAction) {
    if (nation_array
            .player_recno) // no news long if the player has been defeated
      image_icon.put_back(NEWS_X2 + 3, NEWS_Y2 - 17,
                          "NEWS_LOG"); // news log report
  ...
  //------- detect buttons -------//
    ...
    if (nation_array.player_recno) {
      help.set_help(NEWS_X2 + 3, NEWS_Y2 - 18, NEWS_X2 + 17, NEWS_Y2 - 4,
                    "NEWSLOG");

      if (mouse.single_click(NEWS_X2 + 3, NEWS_Y2 - 18, NEWS_X2 + 17,
                             NEWS_Y2 - 4))
    ...
    if (dispCount > 0) // clear news button
    {
      help.set_help(NEWS_X2 + 3 - 1, NEWS_Y2 - 34, NEWS_X2 + 16, NEWS_Y2 - 20,
                    "CLRNEWS");

      if (mouse.single_click(NEWS_X2 + 3 - 1, NEWS_Y2 - 34, NEWS_X2 + 16,
                             NEWS_Y2 - 20))
        clear_news_disp();
    }
  }
...
//
void NewsArray::clear_news_disp() { last_clear_recno = size(); }
//------- End of function NewsArray::clear_news_disp -----//

//------ Begin of function News::put -----//
//
// <int> y 				 - the y display location of the news
// message <int> detectAction - whether it's a detect action <int&> newsHeight
// - for returning the height of the news message displayed
//
// return: <int> 1 - if detect pressing on the news
//					  0 - not detected or it's a display
//action
//
...
#define NEWS_ICON_WIDTH 17
  ...
  int dateWidth = news_font_ptr->text_width(dateStr) +
                  5; // 5 is the space between the date and the text

  newsHeight = NEWS_HEIGHT;

  if (NEWS_ICON_WIDTH + dateWidth + news_font_ptr->text_width(msgStr) >
      NEWS_X2 - NEWS_X1 + 1) {
  ...
  // vga.active_buf->adjust_brightness(ZOOM_X1, y-3, NEWS_X2+1, y+newsHeight-4,
  // 6);
  vga.active_buf->bar_alpha(ZOOM_X1, y - 3, NEWS_X2 + 1, y + newsHeight - 4, 1,
                            V_BLACK);
  ...
  if (detectAction) {
    ...
    if (rc) {
      if (id == NEWS_DIPLOMACY) {
        err_when(talk_res.is_talk_msg_deleted(
            short_para1)); // it shouldn't have been deleted as when it's
                           // deleted in TalkRes(), it will be removed from
                           // news_array immediately

        TalkMsg *talkMsgPtr = talk_res.get_talk_msg(short_para1);

        if (talkMsgPtr->reply_type ==
            REPLY_WAITING) // currently waiting a reply from the player
        {
        ...
        int fromNationRecno =
            info.remote_chat_array[short_para1 - 1].from_nation_recno;
        ...
        vga.use_front();
        world.go_loc(loc_x, loc_y,
                     1); // 1-select object on the location if there is any
        return 1;
      }
    }
  ...
  if (id == NEWS_DIPLOMACY) {
    err_when(talk_res.is_talk_msg_deleted(
        short_para1)); // it shouldn't have been deleted as when it's deleted in
                       // TalkRes(), it will be removed from news_array
                       // immediately
  ...
  news_font_ptr->put_paragraph(NEWS_X1 + NEWS_ICON_WIDTH + dateWidth, y,
                               NEWS_X2, y + newsHeight, msgStr,
                               NEWS_LINE_SPACE);
...
News *NewsArray::operator[](int recNo) {
  ...
  err_if(!msgPtr || msgPtr->news_date == 0) err_now("NewsArray[] is deleted");
