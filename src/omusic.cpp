...
static const char *music_file[] = {
    "DEMO.WAV", // opening
    "DEMO.WAV", "DEMO.WAV", "DEMO.WAV", "DEMO.WAV", "DEMO.WAV", "DEMO.WAV",
    "DEMO.WAV", "DEMO.WAV", "DEMO.WAV", "DEMO.WAV", "DEMO.WAV",
};
...
static const char *music_file[] = {
    "CBRIEF.WAV", // opening
    "CFRYWIN.WAV",  "MENU.WAV",     "LOSE.WAV",     "WIN.WAV",
    "BKMUSIC1.WAV", "BKMUSIC2.WAV", "BKMUSIC3.WAV", "BKMUSIC4.WAV",
    "BKMUSIC5.WAV", "BKMUSIC6.WAV", "BKMUSIC7.WAV",
};
...
int Music::play(int songId, int playType) {
  ...
  if (playType & MUSIC_CD_THEN_WAV) {
    ...
    if (audio.cd_init_flag &&
        audio.play_cd(songId + 1,
                      config.cd_music_volume)) // skip the first data track
    {
      play_type = playType;
      song_id = songId;
      music_channel = 1;
    ...
    if (audio.wav_init_flag) {
      ...
      if (!DIR_MUSIC[0] || !misc.is_file_exist(waveFileStr) ||
          !audio.wav_init_flag)
        return 0;
...
int Music::is_playing(int songId) {
  ...
  if (play_type & MUSIC_PLAY_CD) {
    ...
    if (play_type & MUSIC_PLAY_LOOPED) {
      ...
      return audio.is_long_wav_playing(music_channel) &&
...
// return no. of songs
int Music::max_song() { return sizeof(music_file) / sizeof(char *); }
// -------- end of function Music::max_song --------//

// -------- begin of function Music::random_bgm_track --------//
int Music::random_bgm_track(int excludeSong) {
  int s = LOW_RANDOM_SONG +
          misc.get_time() % (HIGH_RANDOM_SONG - LOW_RANDOM_SONG + 1);
  if (s == excludeSong) {
...
void Music::yield() {
  ...
  if (config.music_flag) {
    if (!is_playing())
      play(random_bgm_track(oldSongId),
