...
void FirmCamp::assign_overseer(int newOverseerRecno)
{
    ...
    else
    {
        ...
        // created.  i.e. there
        //		is no overseer or soldier in the firm, so just assign the new
        // overseer
        // in the firm
        //----------------------------------------------------------------------------------------//
...
void FirmCamp::assign_soldier(int soldierUnitRecno)
{
    ...
    if (unitPtr->true_nation_recno() != nation_recno && soldier_count == MAX_SOLDIER)
    {
        return;
    }
    ...
    if (soldier_count == MAX_SOLDIER)
    {
        ...
        unitXLoc = unitPtr->next_x_loc(); // save the location for later init_sprite() if
    ...
    if (nation_recno)
        unit_res[unitPtr->unit_id]->inc_nation_unit_count(
            nation_recno); // disapper_in_firm() will decrease the unit count,
                           // which is not necessary, so we must increase it
                           // first to offset the later decrease
...
int FirmCamp::mobilize_soldier(int soldierId, char remoteAction)
{
    ...
    unitRecno = create_soldier_unit(thisSoldier); // if he is a spy, create_soldier_unit
...
int FirmCamp::create_soldier_unit(Soldier &thisSoldier)
{
    ...
    if (thisSoldier.spy_recno)
    {
        ...
        unitPtr->is_ai = spyPtr->cloaked_nation_recno && nation_array[spyPtr->cloaked_nation_recno]->is_ai();
...
void FirmCamp::free_all_people()
{
    if (!sys.signal_exit_flag)
    {
        assign_overseer(0);   // this function must be called before restore_world_matrix(),
...
bool FirmCamp::can_accept_assign(int unitRecno, int actionNationRecno)
{
    ...
    if (!unitInfo->class_info.has_combat_level && unitInfo->unit_class != UNIT_CLASS_WEAPON &&
...
int FirmCamp::invoke_defense(BaseObj *attackerObj, int netAttackerPower)
{
    ...
    last_attack_nation_recno = attackerObj->nation_recno; // last_attack_nation_recno is a BaseObj var, it is
...
int FirmCamp::swap_item(int fromSoldierId, int toSoldierId, int verifyItemId, char remoteAction)
{
    ...
    if (srcItem.id && !item_res.can_pick(srcItem.id, (toSoldierId == 0 ? overseer_recno : 0),
                                         (toSoldierId != 0 ? soldier_array[toSoldierId - 1].unit_id : 0)))
        return 0;
    if (destItem.id && !item_res.can_pick(destItem.id, (fromSoldierId == 0 ? overseer_recno : 0),
