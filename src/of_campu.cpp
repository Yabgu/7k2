...
//			space for the old overseer to initialize and appear in
//the 			map (the space the new overseer occupied).
//
//			** If the newOverseerRecno==0, there may be no space for
//			creating the old overseer.  Then, the old overseer will
//be 			deleted. **
//			** else there must be space for creating the old
//overseer, 			at least the space occupied by the new overseer **
//
void FirmCamp::assign_overseer(int newOverseerRecno) {
  if (!newOverseerRecno && !overseer_recno)
    return;

  //--- if the new overseer's nation is not the same as the firm's nation, don't
  //assign ---//

  if (newOverseerRecno &&
      unit_array[newOverseerRecno]->nation_recno != nation_recno)
    return;
  ...
  if (!newOverseerRecno) {
    ...
    SpriteInfo *spriteInfo =
        sprite_res[unit_res[oldUnitPtr->unit_id]->sprite_id];
    int xLoc = loc_x1;
    int yLoc = loc_y1;

    if (!locate_space(is_being_deleted, xLoc, yLoc, loc_x2, loc_y2,
    ...
    //----------------------------------------------------------------------------------------//
    // There should be at least one location (occupied by the new overseer) for
    // creating the old
    //	overseer.
    //
    // 1) If a town is already created, the new overseer settle down there, free
    // its space for
    //		creating the new overseer.
    // 2) If the overseer and the soldiers live in the firm, no town will be
    // created.  Thus, the
    //		space occupied by the old overseer is free for creating the new
    //overseer.
    // 3) If the overseer and the soldiers need live in town, and a town is
    // created.  i.e. there
    //		is no overseer or soldier in the firm, so just assign the new overseer
    //in the firm
    //----------------------------------------------------------------------------------------//
...
void FirmCamp::assign_soldier(int soldierUnitRecno) {
  err_when(!soldier_array); // this function shouldn't be called if this firm
                            // does not need soldier
  ...
  //---- if all soldier space are full, resign the worst soldier to release one
  //soldier space for the overseer ----//
  ...
  if (soldier_count == MAX_SOLDIER) {
    ...
    for (int i = 0; i < MAX_SOLDIER; i++) {
      if (soldier_array[i]
              .is_under_training()) // don't consider soldiers under training
        continue;
    ...
    if (!worstSoldierId) // if every soldier is under training, we cannot resign
                         // any
      return;
    ...
    unitXLoc =
        unitPtr->next_x_loc(); // save the location for later init_sprite() if
                               // the assign settle action failed
  ...
  if (soldierPtr->hit_points ==
      0) // 0.? will become 0 in (float) to (int) conversion
  ...
  if (unitPtr->spy_recno) {
    ...
    unitPtr->spy_recno =
        0; // reset it now so Unit::deinit() won't delete the Spy in spy_array
  }

  //--------- the unit disappear in firm -----//

  if (nation_recno)
    unit_res[unitPtr->unit_id]->inc_nation_unit_count(
        nation_recno); // disapper_in_firm() will decrease the unit count, which
                       // is not necessary, so we must increase it first to
                       // offset the later decrease
...
int FirmCamp::mobilize_overseer() {
  ...
  if (spaceFound) {
    ...
    unit_array.del(
        overseer_recno); // delete it when there is no space for the unit
    return 0;
  }
...
int FirmCamp::mobilize_soldier(int soldierId, char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_FIRM_MOBL_WORKER,
                                                         2 * sizeof(short));
  ...
  unitRecno =
      create_soldier_unit(thisSoldier); // if he is a spy, create_soldier_unit
                                        // wil call set_place(SPY_MOBILE)
...
void FirmCamp::resign_all_soldier(int disappearFlag) {
  for (int i = soldier_count; i > 0; i--) {
    ...
    if (!soldier_array[i - 1].is_under_training()) {
      if (!resign_soldier(i)) // if the unit cannot be mobilized (can be because
                              // it's under training), kill it
        kill_soldier(i);
    } else {
...
int FirmCamp::create_soldier_unit(Soldier &thisSoldier) {
  ...
  int unitRecno = create_unit(
      unitId); // this soldier no longer has a job as it has been resigned
  ...
  // ###### end Gilbert 24/3 #######//
  {
    unitPtr->set_weapon_version(
        thisSoldier.extra_para); // restore nation contribution
  } else if (unitPtr->race_id) {
  ...
  if (thisSoldier.name_id &&
      thisSoldier.race_id) // if this soldier is formerly an unit who has a name
  {
    // ###### begin Gilbert 1/3 #####//
    unitPtr->set_name(thisSoldier.name_id); // then give the name to the unit
    thisSoldier
        .free_name(); // free up the name from monster_res or race_res first
                      // ###### end Gilbert 1/3 #####//
  }
  ...
  //--- decrease the unit count as it is doubled counted when a new unit is
  //created ---//
  ...
  if (!sys.signal_exit_flag) {
    assign_overseer(
        0); // this function must be called before restore_world_matrix(),
            // otherwise the power area can't be completely reset
    resign_all_soldier(); // the workers in the firm will be killed if there is
                          // no space for creating the workers
  }

  kill_overseer(); // if the overseer or soldiers cannot be mobilized due to
                   // lack of space or that some soldiers are still under
                   // training, kill them
  kill_all_soldier();
...
bool FirmCamp::can_accept_assign(int unitRecno, int actionNationRecno) {
  ...
  if (unitPtr->is_civilian() ||
      unitPtr->mobile_type != UNIT_LAND) // only take land units
    return false;

  UnitInfo *unitInfo = unit_res[unitPtr->unit_id];

  // ###### begin Gilbert 24/3 ########//
  //	if( unitInfo->unit_class != UNIT_CLASS_HUMAN &&		// it must be a
  //human or a weapon 		 unitInfo->unit_class != UNIT_CLASS_WEAPON &&
...
int FirmCamp::invoke_defense(BaseObj *attackerObj, int netAttackerPower) {
  ...
  last_attack_nation_recno =
      attackerObj
          ->nation_recno; // last_attack_nation_recno is a BaseObj var, it is
                          // for defender unit to decide which units to attack
  ...
  for (int i = 1; i <= soldier_count; ++i) {
    ...
    if (soldierPtr->source_town_recno) {
      if (town_array.is_deleted(soldierPtr->source_town_recno) ||
          town_array[soldierPtr->source_town_recno]->race_id !=
              soldierPtr->race_id ||
          town_array[soldierPtr->source_town_recno]->nation_recno !=
              nation_recno) {
        soldierPtr->source_town_recno = 0;
      }
    }
...
void FirmCamp::promote_soldier(int soldierId, char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_CAMP_PROMOTE,
                                                         2 * sizeof(short));
...
//
int FirmCamp::swap_item(int fromSoldierId, int toSoldierId, int verifyItemId,
                        char remoteAction) {
  ...
  Item &srcItem = fromSoldierId == 0 ? unit_array[overseer_recno]->item
                                     : soldier_array[fromSoldierId - 1].item;
  Item &destItem = toSoldierId == 0 ? unit_array[overseer_recno]->item
                                    : soldier_array[toSoldierId - 1].item;
  ...
  if (srcItem.id &&
      !item_res.can_pick(
          srcItem.id, (toSoldierId == 0 ? overseer_recno : 0),
          (toSoldierId != 0 ? soldier_array[toSoldierId - 1].unit_id : 0)))
    return 0;
  if (destItem.id &&
      !item_res.can_pick(
          destItem.id, (fromSoldierId == 0 ? overseer_recno : 0),
          (fromSoldierId != 0 ? soldier_array[fromSoldierId - 1].unit_id : 0)))
    return 0;
