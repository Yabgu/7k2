...
// int FirmMonsterTrain::cur_train_type;			// parameter unitPara to
// recruit_train, determine to train civilian spy or infantry spy int
// FirmMonsterTrain::required_linked_firm[TRAIN_UNIT_TYPE_MAX] = { 0, FIRM_FORT,
// FIRM_SPECIAL };
  ...
  case FIRM_MONS_TRAIN_MENU_MAIN:
    ...
    if (train_type_count == 1) {
      ...
      info.draw_unit_icon(INFO_X1 + 40, INFO_Y1 + 52, unitId, nation_recno,
                          INFO_X1 + 15, INFO_Y1 + 5, INFO_X1 + 68, INFO_Y1 + 84,
                          1);
      font_snds.center_put(INFO_X1 + 88, INFO_Y1 + 15, INFO_X1 + 217,
                           INFO_Y1 + 74, unit_res[unitId]->name, 0, 0);
    }
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    if ((train_type_count == 1) && (train_unit_id[0] == UNIT_BEE))
      button_patrol.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 235, 'A',
                           "PATROL-B");
    else if ((train_type_count == 1) && (train_unit_id[0] == UNIT_TERMITE))
      button_patrol.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 235, 'A',
                           "PATROL-T");
    else
      button_patrol.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 235, 'A',
    ...
    for (i = 0; i < train_type_count; ++i) {
      ...
      {
        if ((train_type_count == 1) && (train_unit_id[0] == UNIT_BEE))
          button_train[i].create(INFO_X1 + 13 + i * BUTTON_DISTANCE,
                                 INFO_Y1 + 281, 'A', "TRAIN-B");
        else if ((train_type_count == 1) && (train_unit_id[0] == UNIT_TERMITE))
          button_train[i].create(INFO_X1 + 13 + i * BUTTON_DISTANCE,
                                 INFO_Y1 + 281, 'A', "TRAIN-T");
        else
          button_train[i].create(INFO_X1 + 13 + i * BUTTON_DISTANCE,
                                 INFO_Y1 + 281, 'A', "F_TRAIN");
      } else {
        if ((train_type_count == 1) && (train_unit_id[0] == UNIT_BEE))
          button_train[i].create(INFO_X1 + 13 + (7 - i) * BUTTON_DISTANCE,
                                 INFO_Y1 + 235, 'A', "TRAIN-B");
        else if ((train_type_count == 1) && (train_unit_id[0] == UNIT_TERMITE))
          button_train[i].create(INFO_X1 + 13 + (7 - i) * BUTTON_DISTANCE,
                                 INFO_Y1 + 235, 'A', "TRAIN-T");
        else
          button_train[i].create(INFO_X1 + 13 + (7 - i) * BUTTON_DISTANCE,
                                 INFO_Y1 + 235, 'A', "F_TRAIN");
      }
...
void FirmMonsterTrain::detect_firm_info() {
  ...
  if (button_mobilize.detect() && selected_trainee_id > 0 &&
      selected_trainee_id <= trainee_count) {
    // ####### begin Gilbert 26/1 #######//
    ...
    for (int i = inc >= 0 ? 1 : trainee_count; i > 0 && i <= trainee_count;
      ...
      if (inc < 0) {
        ...
        info.draw_unit_icon(
            ...
            (traineePtr->is_under_training())
                ? (((20 - traineePtr->combat_level()) << 6) + 33)
                : 1);
        // ####### end Gilbert 18/11 ######//
      } else {
        // second round is ascending to draw the frame
...
void FirmMonsterTrain::disp_trainee_info(int dispY1, int refreshFlag) {
  ...
  if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count) {
    ...
    if (unit_res[traineePtr->unit_id]->class_info.has_loyalty) {
      ...
      //traineePtr->loyalty, nation_recno, refreshFlag );
      info.disp_loyalty(
          x, y + 12,
          INFO_X2 - 99 -
...
void FirmMonsterTrain::detect_trainee_info() {
  ...
  if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count) {
    ...
    //		else if( traineePtr->spy_skill > 0 && mouse.single_click( x, y+28,
    //x+79, y+41 ) )
    //		{
    //			disp_combat_or_skill = 3;		// detect spying
    //skill
    //		}
