...
// int FirmMonsterTrain::cur_train_type;			// parameter unitPara
// to recruit_train, determine to train civilian spy or infantry spy int
...
void FirmMonsterTrain::put_info(int refreshFlag)
{
    ...
    case FIRM_MONS_TRAIN_MENU_MAIN:
        ...
        if (train_type_count == 1)
        {
            ...
            info.draw_unit_icon(INFO_X1 + 40, INFO_Y1 + 52, unitId, nation_recno, INFO_X1 + 15, INFO_Y1 + 5,
...
void FirmMonsterTrain::detect_firm_info()
{
    ...
    if (button_mobilize.detect() && selected_trainee_id > 0 && selected_trainee_id <= trainee_count)
    {
        // ####### begin Gilbert 26/1 #######//
    ...
    for (int inc = -1; inc <= 1; inc += 2)
    {
        ...
        for (int i = inc >= 0 ? 1 : trainee_count; i > 0 && i <= trainee_count; i += inc)
        {
            ...
            if (inc < 0)
            {
                ...
                info.draw_unit_icon(
                    x + TRAINEE_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1, windowX2, windowY2,
...
void FirmMonsterTrain::detect_trainee_info()
{
    ...
    if (selected_trainee_id > 0 && selected_trainee_id <= trainee_count)
    {
        ...
        //		else if( traineePtr->spy_skill > 0 && mouse.single_click( x,
        // y+28,
        // x+79, y+41 ) )
        //		{
