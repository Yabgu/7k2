...
enum
{
    STAGE_6_MONSTER_NATION_RECNO = 2
};
...
void CampaignEastWest::stage_6_init_vars()
{
    ...
    monster_reinforcement_count = reinforceCountArray[campaign_difficulty - 1]; // this is the number of units, not the
                                                                                // number of times of reinforcement
...
void CampaignEastWest::stage_6_next_day()
{
    ...
    if (!nation_array.is_deleted(STAGE_6_MONSTER_NATION_RECNO))
    {
        ...
        if (hero_unit_recno)
        {
            ...
            if (heroUnit->is_civilian() || heroUnit->nation_recno != PLAYER_NATION_RECNO)
            {
                hero_unit_recno = 0;
                return;
            }
...
void CampaignEastWest::stage_6_create_hero()
{
    ...
    hero_unit_recno = hero_res.create_hero(xLoc, yLoc, 0, (~nation_array)->race_id, 80, 80 + misc.random(20), -1, 0,
