...
int CampaignEastWest::stage_6_create_game() {
  ...
  create_troop(STAGE_6_MONSTER_NATION_RECNO, campaign_difficulty * 7,
               40 + campaign_difficulty * 5);
...
void CampaignEastWest::stage_6_init_vars() {
  ...
  monster_reinforcement_count =
      reinforceCountArray[campaign_difficulty -
                          1]; // this is the number of units, not the number of
                              // times of reinforcement

  next_monster_reinforcement_date =
      info.game_date + (MAX_CAMPAIGN_DIFFICULTY - campaign_difficulty + 1) * 30;
...
int CampaignEastWest::stage_6_create_town() {
  ...
  for (int i = townStateArray.size(); i > 0 && addedCount < townAddCount; i--) {
    ...
    if (create_nation_town(PLAYER_NATION_RECNO, xLoc, yLoc, 1, 0,
                           soldierAddCount)) // 0-the race of the town need not
                                             // to be the same as the nation
      addedCount++;
  }
...
void CampaignEastWest::stage_6_create_hero() {
  ...
  if (!world.locate_space(xLoc, yLoc, xLoc, yLoc, 2,
                          2)) // locate a space where a new unit can be created
  {
  ...
  hero_unit_recno = hero_res.create_hero(
      xLoc, yLoc, 0, (~nation_array)->race_id, 80, 80 + misc.random(20), -1, 0,
      1); // last 0-the hero must be a fictional one as opposite to a historic
          // one.	Last 1-use reserve
  ...
  if (hero_unit_recno) {
    ...
    game.add_lose_condition(E_KILL_UNIT, 1, hero_unit_recno,
                            0); // 1-this condition only applies to the player.
  }
}
  ...
  if (game.result_win_condition_id >= 0) {
    ...
    if (!unit_array.is_deleted(hero_unit_recno)) {
      ...
      if (unit_array.is_deleted(ownNation->king_unit_recno)) {
        old_king_name[0] = '\0';
        ownNation->set_king(hero_unit_recno,
                            0); // 0 - this is not the first king
        ...
        strncpy(old_king_name, oldKingUnit->unit_name(0),
                CUSTOM_NAME_LEN); // 0-without title
