...
void ScenarioEditor::disp_players_main(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    browse_nation.init(INFO_X1, INFO_Y1, INFO_X2, INFO_Y1 + 168, -1, 32,
                       MAX_NATION, disp_nation_rec);
  ...
  //		font_whbl.put( font_whbl.put( INFO_X1+20, y,
  //text_editor.str_king_name()) // "King Name" ) 		+10, y,
  //nation_array[brush_player_recno]->king_name() ) ;

  if (!nation_array.is_deleted(brush_player_recno)) {
    vga.active_buf->d3_panel_up(INFO_X1 + 5, y, INFO_X2 - 5,
                                y + 8 + font_cara_w.max_font_height);
    font_cara_w.center_put(INFO_X1 + 5, y, INFO_X2 - 5,
                           y + 8 + font_cara_w.max_font_height,
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    //		button_adjustment[0].create_text( INFO_X1+152, y, INFO_X1+214, y+18,
    //text_editor.str_fine_coarse(0) ); // "Fine" );
    //		button_adjustment[1].create_text( INFO_X1+152, y+20, INFO_X1+214,
    //y+38, text_editor.str_fine_coarse(1) ); // "Medium" );
    //		button_adjustment[2].create_text( INFO_X1+152, y+40, INFO_X1+214,
    //y+58, text_editor.str_fine_coarse(2) ); // "Coarse" );
  }
  ...
  if (!nation_array.is_deleted(brush_player_recno)) {
    ...
    font_whbl.put(INFO_X1 + 15, y,
                  text_editor.str_player_treasure()); //"Treasure" );
    font_whbl.right_put(INFO_X1 + 140 - 4, y,
    ...
    font_whbl.put(INFO_X1 + 15, y + 22,
                  text_editor.str_player_food()); // "Food" );
    font_whbl.right_put(INFO_X1 + 140 - 4, y + 22,
                        misc.format((int)nationPtr->food));
    button_food_up.paint();
    button_food_down.paint();

    // --------- reputation -------//

    font_whbl.put(INFO_X1 + 15, y + 44,
                  text_editor.str_player_reputation()); // "Reputation" );
    font_whbl.right_put(INFO_X1 + 140 - 4, y + 44,
                        misc.format((int)nationPtr->reputation));
    button_reput_up.paint();
    button_reput_down.paint();

    // --------- live points ---------//

    font_whbl.put(INFO_X1 + 15, y + 66,
                  text_editor.str_player_live_points()); // "Life Points" );
    font_whbl.right_put(INFO_X1 + 140 - 4, y + 66,
                        misc.format((int)nationPtr->live_points));
  ...
  y += 87;
  if (refreshFlag == INFO_REPAINT) {
    button_adjustment[0].create_text(
        INFO_X1 + 147, y + 18, INFO_X1 + 216, y + 38,
        text_editor.str_fine_coarse(0)); // "Fine" );
    button_adjustment[1].create_text(
        INFO_X1 + 76, y + 18, INFO_X1 + 145, y + 38,
        text_editor.str_fine_coarse(1)); // "Medium" );
    button_adjustment[2].create_text(
        INFO_X1 + 5, y + 18, INFO_X1 + 74, y + 38,
        text_editor.str_fine_coarse(2)); // "Coarse" );
  }
  ...
  if (refreshFlag == INFO_REPAINT) {
    //		button_player_type[0].create_text( INFO_X1+50, y, INFO_X1+100, y+18,
    //"Either" ); 		button_player_type[1].create_text( INFO_X1+50, y, INFO_X1+100,
    //y+18, "Computer" );
  }
  if (!nation_array.is_deleted(brush_player_recno)) {
...
void ScenarioEditor::detect_players_main() {
  ...
  if (mouse.single_click(INFO_X1 + 5, y, INFO_X2 - 5,
                         y + 8 + font_cara_w.max_font_height)) {
    ...
    backGroundBuf.resize(0, 0, (INFO_X2 - 5) - (INFO_X1 + 5 + lrMargin) + 1,
                         y2 - (y + yOffs) + 1);
    vga.active_buf->read_bitmapW(
        INFO_X1 + 5 + lrMargin, y + yOffs, INFO_X2 - 5, y2,
    ...
    nation_name_field.init(INFO_X1 + 5 + lrMargin, y + yOffs,
                           INFO_X2 - 5 - lrMargin, nationStr, FIELD_LEN,
                           &font_cara_w, 1, 1);
  ...
  //	{
  //		nation_array[brush_player_recno]->nation_type = rc ? NATION_AI :
  //NATION_REMOTE;
  //	}
}
...
//
int ScenarioEditor::detect_players_view() { return 0; }
...
static void disp_nation_rec(int recNo, int x, int y, int refreshFlag) {
  if (nation_array.is_deleted(recNo)) {
    ...
    font_snds.put(x + 16, y + 2,
                  text_editor.str_player_disable()); // "(Disabled)" );
  } else {
    ...
    if (nationPtr->is_human())
      font_snds.put(x + 132, y + 2, race_res[nationPtr->race_id]->name, 0,
                    x + 198);
    else
      font_snds.put(x + 132, y + 2, monster_res[nationPtr->monster_id()]->name,
                    0, x + 198);
    ...
    //		font_snds.put( x+1, y+2, (int)nation_array[recNo]->is_ai() ?
    //"Computer" : "Either" );
  }
}
