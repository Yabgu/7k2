...
//							  also align the frame's position
// to
// locations. 							  (default:0)
//
...
void MouseCursor::process(int curX, int curY)
{
    ...
    if (hide_all_flag || (hide_area_flag && is_touch(hide_x1, hide_y1, hide_x2, hide_y2)))
    {
        cursor_shown = 0;
    }
    else
    {
        ...
        if (cur_x1 < 0 || cur_x2 >= VGA_WIDTH || cur_y1 < 0 || cur_y2 >= VGA_HEIGHT)
        {
            ...
            vga_front.read_bitmapW((newX1 = MAX(cur_x1, 0)), (newY1 = MAX(cur_y1, 0)),
                                   (newX2 = MIN(cur_x2, VGA_WIDTH - 1)), (newY2 = MIN(cur_y2, VGA_HEIGHT - 1)),
                                   save_scr->bitmap_ptr());

            vga_front.put_bitmap_area_trans(cur_x1, cur_y1, icon_ptr, MAX(0, cur_x1) - cur_x1, MAX(0, cur_y1) - cur_y1,
    ...
    if (showOldRect && showNewRect && (oldX1 != newX1 || oldY1 != newY1 || oldX2 != newX2 || oldY2 != newY2))
    {
...
void MouseCursor::disp_back_buf(int bltX1, int bltY1, int bltX2, int bltY2)
{
    ...
    if (is_touch(bltX1, bltY1, bltX2, bltY2))
    {
        ...
        // x1-MAX(cur_x1,0), y1-MAX(cur_y1,0), save_back_scr->bitmap_ptr() );		//
        // +4 is the width & height info
        IMGbltW(save_scr->buf_ptr(), sizeof(short) * (MIN(cur_x2, VGA_WIDTH - 1) - MAX(cur_x1, 0) + 1),
        ...
        {
            vga_back.put_bitmap_area_trans(cur_x1, cur_y1, icon_ptr, MAX(bltX1, cur_x1) - cur_x1,
...
void MouseCursor::before_flip()
{
    if (cursor_shown)
    {
        ...
        if (cur_x1 < 0 || cur_x2 >= VGA_WIDTH || cur_y1 < 0 || cur_y2 >= VGA_HEIGHT)
        {
            vga_back.read_bitmapW(MAX(cur_x1, 0), MAX(cur_y1, 0), MIN(cur_x2, VGA_WIDTH - 1),
                                  MIN(cur_y2, VGA_HEIGHT - 1), save_back_scr->bitmap_ptr());

            vga_back.put_bitmap_area_trans(cur_x1, cur_y1, icon_ptr, MAX(0, cur_x1) - cur_x1, MAX(0, cur_y1) - cur_y1,
                                           MIN(VGA_WIDTH - 1, cur_x2) - cur_x1, MIN(VGA_HEIGHT - 1, cur_y2) - cur_y1);
        }
        else //----- if the whole cursor is on the screen -----//
        {
