...
MouseCursor::MouseCursor() { memset(this, 0, sizeof(MouseCursor)); }
//----------- End of function MouseCursor::MouseCursor ------//

//---------- Begin of function MouseCursor::~MouseCursor --------//

MouseCursor::~MouseCursor() { deinit(); }
//----------- End of function MouseCursor::~MouseCursor ------//
...
void MouseCursor::deinit() {
  if (init_flag) {
    ...
    if (merge_buf) // buffer for merging save screen from the front and back
                   // buffers
    {
      delete merge_buf;
      merge_buf = NULL;
...
void MouseCursor::load_cursor_info() {
  ...
  for (i = 0; i < cursor_count; i++) {
    ...
    cursorInfo->human_hot_spot_x =
        misc.atoi(cursorRec->human_hot_spot_x, cursorRec->HOT_SPOT_LEN);
    cursorInfo->human_hot_spot_y =
        misc.atoi(cursorRec->human_hot_spot_y, cursorRec->HOT_SPOT_LEN);

    memcpy(&bitmapOffset, cursorRec->monster_bitmap_ptr, sizeof(uint32_t));
    cursorInfo->monster_bitmap_ptr =
        monster_res_bitmap.read_imported(bitmapOffset);
    cursorInfo->monster_hot_spot_x =
        misc.atoi(cursorRec->monster_hot_spot_x, cursorRec->HOT_SPOT_LEN);
    cursorInfo->monster_hot_spot_y =
        misc.atoi(cursorRec->monster_hot_spot_y, cursorRec->HOT_SPOT_LEN);
  }
}
...
void MouseCursor::set_icon(int cursorId) {
  ...
  if (!hideAllFlag) // if the mouse has been hiden before, don't hide and show
                    // it
  {
...
//							  also align the frame's position to
//locations. 							  (default:0)
//
...
void MouseCursor::set_frame_border(int borderX1, int borderY1, int borderX2,
                                   int borderY2) {
...
void MouseCursor::process(int curX, int curY) {
  ...
  {
    vga_front.put_bitmapW_fast((oldX1 = MAX(cur_x1, 0)),
                               (oldY1 = MAX(cur_y1, 0)),
                               save_scr->bitmap_ptr());
    showOldRect = 1;
  ...
  if (hide_all_flag ||
      (hide_area_flag && is_touch(hide_x1, hide_y1, hide_x2, hide_y2))) {
    cursor_shown = 0;
  } else {
    //---- if the cursor is across the screen border -----//

    if (cur_x1 < 0 || cur_x2 >= VGA_WIDTH || cur_y1 < 0 ||
        cur_y2 >= VGA_HEIGHT) {
      // ####### begin Gilbert 29/1 ######//
      vga_front.read_bitmapW((newX1 = MAX(cur_x1, 0)), (newY1 = MAX(cur_y1, 0)),
                             (newX2 = MIN(cur_x2, VGA_WIDTH - 1)),
                             (newY2 = MIN(cur_y2, VGA_HEIGHT - 1)),
                             save_scr->bitmap_ptr());

      vga_front.put_bitmap_area_trans(
          cur_x1, cur_y1, icon_ptr, MAX(0, cur_x1) - cur_x1,
          MAX(0, cur_y1) - cur_y1, MIN(VGA_WIDTH - 1, cur_x2) - cur_x1,
          MIN(VGA_HEIGHT - 1, cur_y2) - cur_y1);

      showNewRect = 1;
      // ####### end Gilbert 29/1 ######//
    } else //----- if the whole cursor is on the screen -----//
    {
      vga_front.read_bitmapW(cur_x1, cur_y1, cur_x2, cur_y2,
                             save_scr->bitmap_ptr());
      vga_front.put_bitmap_trans(cur_x1, cur_y1,
                                 icon_ptr); // must use PutIcon instead of
                                            // PutArea for background masking
...
void MouseCursor::disp_frame(VgaBuf *vgaBufPtr) {
  ...
  vgaBufPtr->read_bitmapW(frame_x1, frame_y1, frame_x2, frame_y1,
                          frame_top_save_scr);
  vgaBufPtr->read_bitmapW(frame_x1, frame_y2, frame_x2, frame_y2,
                          frame_bottom_save_scr);
  vgaBufPtr->read_bitmapW(frame_x1, frame_y1, frame_x1, frame_y2,
                          frame_left_save_scr);
  vgaBufPtr->read_bitmapW(frame_x2, frame_y1, frame_x2, frame_y2,
                          frame_right_save_scr);

  //---------- draw the rectagular frame now -----------//

  vgaBufPtr->rect(frame_x1, frame_y1, frame_x2, frame_y2, 1,
                  OWN_SELECT_FRAME_COLOR);
...
void MouseCursor::disp_back_buf(int bltX1, int bltY1, int bltX2, int bltY2) {
  ...
  if (is_touch(bltX1, bltY1, bltX2, bltY2)) {
    ...
    // IMGbltW( save_scr->buf_ptr(), MIN(cur_x2,VGA_WIDTH-1) -MAX(cur_x1,0)+1,
    // x1-MAX(cur_x1,0), y1-MAX(cur_y1,0), save_back_scr->bitmap_ptr() );		// +4
    // is the width & height info
    IMGbltW(save_scr->buf_ptr(),
            sizeof(short) * (MIN(cur_x2, VGA_WIDTH - 1) - MAX(cur_x1, 0) + 1),
            x1 - MAX(cur_x1, 0), y1 - MAX(cur_y1, 0),
            save_back_scr->bitmap_ptr()); // +4 is the width & height info

    //--------- display the mouse cursor now -----------//

    if (cur_x1 < bltX1 || cur_x2 > bltX2 || cur_y1 < bltY1 || cur_y2 > bltY2) {
      vga_back.put_bitmap_area_trans(
          cur_x1, cur_y1, icon_ptr, MAX(bltX1, cur_x1) - cur_x1,
          MAX(bltY1, cur_y1) - cur_y1, MIN(bltX2, cur_x2) - cur_x1,
          MIN(bltY2, cur_y2) - cur_y1);
    }
...
void MouseCursor::before_flip() {
  if (cursor_shown) {
    ...
    if (cur_x1 < 0 || cur_x2 >= VGA_WIDTH || cur_y1 < 0 ||
        cur_y2 >= VGA_HEIGHT) {
      vga_back.read_bitmapW(
          MAX(cur_x1, 0), MAX(cur_y1, 0), MIN(cur_x2, VGA_WIDTH - 1),
          MIN(cur_y2, VGA_HEIGHT - 1), save_back_scr->bitmap_ptr());
    ...
    {
      vga_back.read_bitmapW(cur_x1, cur_y1, cur_x2, cur_y2,
                            save_back_scr->bitmap_ptr());
      vga_back.put_bitmap_trans(cur_x1, cur_y1,
                                icon_ptr); // must use PutIcon instead of
                                           // PutArea for background masking
    }
  }
...
void MouseCursor::after_flip() {
  if (cursor_shown) {
    ...
    vga_back.put_bitmapW_fast(MAX(cur_x1, 0), MAX(cur_y1, 0),
                              save_back_scr->bitmap_ptr());
  }
}
...
int MouseCursor::is_touch(int x1, int y1, int x2, int y2) {
  return ((cur_y1 <= y1 && cur_y2 >= y1) || (y1 <= cur_y1 && y2 >= cur_y1)) &&
         ((cur_x1 <= x1 && cur_x2 >= x1) || (x1 <= cur_x1 && x2 >= cur_x1));
}
