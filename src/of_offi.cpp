...
void FirmOffensive::disp_main_menu(int refreshFlag)
{
    ...
    if (cast_delay > 0)
    {
        ...
        if (ratio < 11)
            vga.active_buf->put_bitmap_trans(INFO_X1 + 80 + 20 + offsetX, INFO_Y1 + 49 + offsetY,
        ...
        vga.active_buf->read_bitmapW(INFO_X1 + 80 + 20 + offsetX, INFO_Y1 + 49 + offsetY,
                                     INFO_X1 + 94 + 20 + size + offsetX, INFO_Y1 + 80 + offsetY, hitPointBitmap);
        vga.active_buf->put_bitmap_trans(INFO_X1 + 80 + 20 + offsetX, INFO_Y1 + 49 + offsetY,
        ...
        if (world.zoom_matrix->get_detect_location(mouse.cur_x, mouse.cur_y, &pointedXLoc, &pointedYLoc, &mobileType))
        {
            ...
            case MAGIC_GOD_POWER:
                font_snds.center_put_paragraph(
                    ...
                    text_firm.str_cast_cost_cash(calc_cash_cost(menu_mode_para, pointedXLoc, pointedYLoc, 0)), 2, 0, 0);
                break;
