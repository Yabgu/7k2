...
void FirmOffensive::detect_main_menu() {
  ...
  {
  case MAGIC_FREEZE_LASER:
    help.set_help(INFO_X1 + 13, INFO_Y1 + 261, INFO_X1 + 13 + 44,
                  INFO_Y1 + 280 + 48, "MAGIC7");
    break;

  case MAGIC_GOD_POWER:
    help.set_help(INFO_X1 + 13, INFO_Y1 + 261, INFO_X1 + 13 + 44,
                  INFO_Y1 + 280 + 48, "MAGIC6");
    break;

  case MAGIC_LASER:
    help.set_help(INFO_X1 + 13, INFO_Y1 + 261, INFO_X1 + 13 + 44,
                  INFO_Y1 + 280 + 48, "MAGIC5");
    break;
...
void FirmOffensive::disp_magic_menu(int refreshFlag) {
  ...
  //	font_snds.center_put_paragraph( INFO_X1 +20 , INFO_Y1 +60, INFO_X2 -10,
  //INFO_Y1 +80,
  //		tech_res[magic_info_array[menu_mode_para-1].tech_id]->tech_des(), 2,
  //0, 0);
    ...
    if (world.zoom_matrix->get_detect_location(mouse.cur_x, mouse.cur_y,
      ...
      case MAGIC_GOD_POWER:
        font_snds.center_put_paragraph(
            ...
            text_firm.str_cast_cost_cash(
                calc_cash_cost(menu_mode_para, pointedXLoc, pointedYLoc, 0)),
            2, 0, 0);
        break;
      ...
      case MAGIC_TELEPORT:
        font_snds.center_put_paragraph(
            INFO_X1 + 20, INFO_Y1 + 80, INFO_X2 - 10, INFO_Y1 + 120,
            text_firm.str_teleport_cost_cash(calc_cash_cost(
                menu_mode_para, pointedXLoc, pointedYLoc, teleport_unit_recno)),
            2, 0, 0);
        break;
      }
...
image_icon.read("BUILDUP") ); font_bld.center_put(button->x1, button->y1-2,
button->x2, button->y2-2, magicName ); vga.active_buf->bar_alpha(button->x1+1,
button->y1+1, button->x2-1, button->y2-1, 1, 0);
        }
...
0); font_bld.center_put(button->x1+1, button->y1-1, button->x2+1, button->y2-1,
magicName );
        }
        else if (mouse.in_area(button->x1, button->y1, button->x2, button->y2))
        {
                vga.active_buf->put_bitmap(  button->x1, button->y1,
image_icon.read("BUILDSEL") ); font_bld.center_put(button->x1, button->y1-2,
button->x2, button->y2-2, magicName );
        }
        else
        {
                vga.active_buf->put_bitmap(  button->x1, button->y1,
