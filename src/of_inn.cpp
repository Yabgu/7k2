...
//
void FirmInn::deinit_derived() { Firm::deinit_derived(); }
...
void FirmInn::free_all_people() {
  ...
  if (!sys.signal_exit_flag) {
    for (int i = inn_unit_count; i > 0; --i) {
      if (inn_unit_array[i - 1].spy_recno &&
          (transfer_inn_unit(i) || hire(i, 1))) {
...
void FirmInn::assign_unit(int unitRecno) {
  ...
  if (inn_unit_count == MAX_INN_UNIT || count_soldier() >= MAX_INN_UNIT / 2)
    return;
...
// [int] spyEscape      whether this is an instance which a spy escape from the
// inn, instead of hiring a unit. i.e. no hire cost required
//								(default: 0)
// [short] exitFirmRecno  exit inn's firm recno if not same as this inn (see
// transfer_inn_unit)
//                      (default :0)
//
int FirmInn::hire(short recNo, int spyEscape, short exitFirmRecno) {
  ...
  if (innUnit->spy_recno &&
      (spyEscape // no expense if mobilizing spy
       || innUnit->true_nation_recno() ==
              nation_recno)) // no expense if it is own spy
  {
    // free, no need to play money
    unitRecno = (exitFirmRecno ? firm_array[exitFirmRecno] : this)
                    ->create_unit(innUnit->unit_id);
    if (!unitRecno)
  ...
  if (innUnit->spy_recno &&
      (spyEscape || innUnit->true_nation_recno() == nation_recno)) {
  ...
  // ######## begin Gilbert 24/2 #######//
  unitPtr->unique_id =
      innUnit->unique_id; // set unique before add_spy/set_spy_place
  // ######## end Gilbert 24/2 #######//
  ...
  if (innUnit->spy_skill > 0) {
    ...
    spy_array[unitPtr->spy_recno]->spy_loyalty =
        unitLoyalty; // this is the spy's true loyalty towards you
  }
...
void FirmInn::next_day() {
  ...
  int updateInterveal =
      10 +
      info.year_passed *
          2; // there will be less and less units to hire as the game passes
...
void FirmInn::update_add_hire_list() {
  ...
  if (inn_unit_count <
      MAX_INN_UNIT - 1) // -1 so that there is always one free slot for spies
  {
...
int FirmInn::should_add_inn_unit() {
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    firmInn = (FirmInn *)
        firm_array[linked_firm_array[i]]; // links between inns are stored in
                                          // linked_firm_array[] for quick scan
                                          // only
...
void FirmInn::add_inn_unit(int raceId) {
  ...
  if (!spyFlag) // a military unit
  {
    ...
    {
      unitRecno = hero_res.create_hero(
          -1, -1, 0, 0, 0, 0, 1); // -1 - create the unit as an invisible unit
    } // last 1 - the heroes are for hire, check the HeroInfo::for_hire before
      // adding it
    else //---- just create a unit, not a hero  -----//
    {
      ...
      unitRecno = hero_res.create_powerful_unit(
          -1, -1, 0, unitId,
          heroPower); // -1 - create the unit as an invisible unit
    }
    ...
    unit_array.del(unitRecno); // delete the unit as it was created temporarily
                               // for setting skill and item values
  } else                       // a spy
  {
    ...
    case 1: // infantry spy
      ...
      innUnit->skill.init(
          innUnit->unit_id, 20 + misc.random(30),
          20 +
              misc.random(
                  30)); // a spy also have leadership and combat ability, but
                        // lower than units with leadership as their main skills
      innUnit->spy_skill = 30 + misc.random(40);
      break;
    case 2: // special unit spy
      innUnit->unit_id = (char)raceInfo->special_unit_id;
      innUnit->skill.init(
          innUnit->unit_id, 30 + misc.random(30),
          10 +
              misc.random(
                  20)); // a spy also have leadership and combat ability, but
                        // lower than units with leadership as their main skills
      innUnit->spy_skill = 30 + misc.random(40);
      break;
    default:
...
void FirmInn::del_inn_unit(int recNo) {
  ...
  if (firm_array.selected_recno == firm_recno) {
    if (recoverSelected) {
      for (--selected_unit_id;
           selected_unit_id > 0 && memcmp(&inn_unit_array[selected_unit_id - 1],
                                          &selectedInnUnit, sizeof(InnUnit));
           --selected_unit_id)
        ;
    }
  }
...
int FirmInn::transfer_inn_unit(int recNo) {
  ...
  for (firmCount = firm_array.size(); firmCount > 0; --firmCount, ++firmRecno) {
    ...
    if (spyPtr && firmInn->nation_recno == spyPtr->true_nation_recno) {
      ...
      continue; // spy tend not to go back to own firmInn, unless no other
                // choice
    }
...
bool FirmInn::can_accept_assign(int unitRecno, int actionNationRecno) {
  ...
  return unitPtr->spy_recno &&
         !unitPtr->is_civilian() && // it must be a military unit
         unitPtr->nation_recno ==
             0; // it must be an independent unit and it must be a spy
}
...
int FirmInn::buy_item(short recNo, int checkingFlag) {
  ...
  if (innUnit->true_nation_recno() != nation_recno &&
      nationPtr->cash < innUnit->item.cost())
    return 0;

  if (!checkingFlag) {
    // drop the item outside

    if (site_array.add_site(loc_x2, loc_y2, SITE_ITEM, innUnit->item.id,
                            innUnit->item.para) ||
        site_array.add_site(loc_x2, loc_y1, SITE_ITEM, innUnit->item.id,
                            innUnit->item.para) ||
        site_array.add_site(loc_x1, loc_y2, SITE_ITEM, innUnit->item.id,
                            innUnit->item.para) ||
        site_array.add_site(loc_x1, loc_y1, SITE_ITEM, innUnit->item.id,
                            innUnit->item.para)) {
      // reduce nation cash

      if (innUnit->true_nation_recno() != nation_recno) {
        nationPtr->add_expense(EXPENSE_HIRE_UNIT, (float)innUnit->item.cost(),
                               0);
      }
