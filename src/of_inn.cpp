...
int FirmInn::hire(short recNo, int spyEscape, short exitFirmRecno)
{
    ...
    if (innUnit->spy_recno && (spyEscape || innUnit->true_nation_recno() == nation_recno))
    {
...
int FirmInn::should_add_inn_unit()
{
    ...
    {
        firmInn = (FirmInn *)firm_array[linked_firm_array[i]]; // links between inns are stored in
...
void FirmInn::add_inn_unit(int raceId)
{
    ...
    if (!spyFlag) // a military unit
    {
        ...
        else //---- just create a unit, not a hero  -----//
        {
            ...
            unitRecno = hero_res.create_powerful_unit(-1, -1, 0, unitId,
    ...
    else // a spy
    {
        ...
        case 1: // infantry spy
            ...
            innUnit->skill.init(innUnit->unit_id, 20 + misc.random(30),
                                20 + misc.random(30)); // a spy also have leadership and combat ability, but
        ...
        case 2: // special unit spy
            ...
            innUnit->skill.init(innUnit->unit_id, 30 + misc.random(30),
                                10 + misc.random(20)); // a spy also have leadership and combat ability, but
                                                       // lower than units with leadership as their main skills
            innUnit->spy_skill = 30 + misc.random(40);
            break;
        default:
...
{
    hire_cost = hero_res.hire_cost(combat_level(), skill.max_combat_level, skill_level(), spy_skill, &item);
}
...
void FirmInn::del_inn_unit(int recNo)
{
    ...
    if (firm_array.selected_recno == firm_recno)
    {
        if (recoverSelected)
        {
            for (--selected_unit_id; selected_unit_id > 0 &&
                                     memcmp(&inn_unit_array[selected_unit_id - 1], &selectedInnUnit, sizeof(InnUnit));
                 --selected_unit_id)
                ;
        }
    }
