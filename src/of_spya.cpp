...
void FirmSpy::think_train_new()
{
    ...
    if (nationPtr->total_population < 30 - nationPtr->pref_spy / 10) // don't use spies if the population is too low, we
                                                                     // need to use have people to grow population
        return;
    ...
    if (nationPtr->expense_365days(EXPENSE_SPY) > nationPtr->expense_365days() * (50 + nationPtr->pref_spy) / 400)
    {
        return;
    }
...
int FirmSpy::has_spy(int raceId, int isCivilian)
{
    ...
    {
        if (trainee_array[i].race_id == raceId && unit_res[trainee_array[i].unit_id]->is_civilian == isCivilian)
        {
            return 1;
        }
    }
...
void FirmSpy::think_spy_action()
{
    ...
    for (int i = 0; i < trainee_count; i++)
    {
        ...
        if (nationPtr->add_action(placePtr->loc_x1, placePtr->loc_y1, -1, -1, ACTION_AI_ASSIGN_SPY,
                                  placePtr->nation_recno, 1, unitRecno))
        {
            return;
        }
    }
