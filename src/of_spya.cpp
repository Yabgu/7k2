...
void FirmSpy::think_train_new() {
  ...
  if (nationPtr->total_population <
      30 - nationPtr->pref_spy /
               10) // don't use spies if the population is too low, we need to
                   // use have people to grow population
    return;

  if (nationPtr->total_spy_count > nationPtr->total_population *
                                       (10 + nationPtr->pref_spy / 5) /
                                       100) // 10% to 30%
    return;
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    if (misc.random(2) == 0) {
      ...
      if (can_train_from_town(townPtr->town_recno,
                              required_linked_firm[TRAIN_UNIT_CIVILIAN])) {
        int rc, counterSpy;
        ...
        if (counterSpy)
          rc =
              nationPtr->think_assign_spy_own_town(townPtr->race_id, region_id);
        else
          rc = nationPtr->think_assign_spy_target_town(townPtr->race_id,
                                                       region_id);
      ...
      int trainType =
          misc.random(2) == 0 ? TRAIN_UNIT_INFANTRY : TRAIN_UNIT_SPECIAL;

      if (can_train_from_town(townPtr->town_recno,
                              required_linked_firm[trainType])) {
        if (nationPtr->think_assign_spy_target_camp(townPtr->race_id,
                                                    region_id)) {
...
void FirmSpy::think_spy_action() {
  ...
  //--- if the nation's reputation is negative, then only do not produce spies
  //too often ---//
  ...
  for (int i = 0; i < trainee_count; i++) {
    ...
    if (unit_res[traineePtr->unit_id]->is_civilian) {
      ...
      if (counterSpy)
        townRecno = nationPtr->think_assign_spy_own_town(traineePtr->race_id,
                                                         region_id);
      else
        townRecno = nationPtr->think_assign_spy_target_town(traineePtr->race_id,
                                                            region_id);
      ...
      int firmRecno = nationPtr->think_assign_spy_target_camp(
          traineePtr->race_id, region_id);
    ...
    if (nationPtr->add_action(placePtr->loc_x1, placePtr->loc_y1, -1, -1,
                              ACTION_AI_ASSIGN_SPY, placePtr->nation_recno, 1,
                              unitRecno)) {
      return;
    }
  }
