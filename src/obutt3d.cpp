...
void Button3D::create(int pX1, int pY1, char buttonStyle,
                      const char *buttonName, char elasticFlag,
                      char defIsPushed) {
  init_flag = 1;
...
void Button3D::create(int pX1, int pY1, const char *upButtonName,
                      const char *downButtonName, char elasticFlag,
                      char defIsPushed) {
  init_flag = 1;
  ...
  if (button_type == BUTTON_TYPE_SHELL && icon_ptr) {
    ...
    if (enable_flag)
      Vga::active_buf->put_bitmap_trans_decompress(
          tx, ty, (char *)icon_ptr); // 0 means not clear background
    else                             // button disabled
    {
      // ##### begin Gilbert 24/12 #######//
      /*
      //--- put it on the back buffer, darken it and blt it back to the front
      buffer ---//

      if( !vga.use_back_buf )
              vga_back.read_bitmapW( x1, y1, x1+BUTTON_ACTION_WIDTH-1,
      y1+BUTTON_ACTION_HEIGHT-1, save_back_buf );

      //------ display and darken ----//

      vga_back.put_bitmap_trans_decompress( tx, ty, (char*) icon_ptr );   // 0
      means not clear background

      vga_back.adjust_brightness( x1, y1, x1+BUTTON_ACTION_WIDTH-1,
      y1+BUTTON_ACTION_HEIGHT-1, -5 );
      ...
      if( !vga.use_back_buf )
      {
              vga_util.blt_buf( x1, y1, x1+BUTTON_ACTION_WIDTH-1,
      y1+BUTTON_ACTION_HEIGHT-1, 0 ); vga_back.put_bitmapW( x1, y1,
      save_back_buf );
      }
      */
      ...
      Vga::active_buf->put_bitmap_trans_remap_decompress(
          tx, ty, (char *)icon_ptr,
          (short *)vga.vga_color_table->get_table(
              -MAX_BRIGHTNESS_ADJUST_DEGREE * 3 / 4));
...
//          0 - if not
//
int Button3D::detect(unsigned keyCode1, unsigned keyCode2, int detectRight,
                     int suspendPop) {
  int rc = 0;
