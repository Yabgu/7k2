...
void TalkMsg::process_accepted_reply()
{
    ...
    case TALK_DECLARE_WAR:
        toRelation->started_war_on_us_count++; // how many times this nation has started a
    ...
    case TALK_REQUEST_SURRENDER: {
        float offeredAmt = (float)talk_para1 * 10; // * 10 is to restore its original value. It has been divided by 10
    ...
    while (1)
    {
        ...
        if (talkMsgRecno && talkMsg.talk_id != TALK_DECLARE_WAR) // Exception handling - Declare War
...
#define TALK_MSG_VALID_DAYS 30
    ...
    case TALK_EXCHANGE_TECH:
        ...
        if (tech_res[talk_para1]->get_nation_tech_level(to_nation_recno) >= talk_para3 ||
            tech_res[talk_para2]->get_nation_tech_level(from_nation_recno) >= talk_para4)
        {
            return 0;
        }
        break;
...
char *TalkMsg::from_king_name()
{
    ...
    str = text_unit.str_unit_titled(nationPtr->race_id, nationPtr->king_name(), RANK_KING);
