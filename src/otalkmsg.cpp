...
static char talk_msg_reply_needed_array[] = {
    1, 1, 1, 0, 0, 0, 1, 1, 1, 1, 1, 0, 1, 1, 1, 1, 1, 1, 0,
};

//------- Begin of function TalkMsg::TalkMsg --------//
//
TalkMsg::TalkMsg() { memset(this, 0, sizeof(TalkMsg)); }
...
void TalkMsg::process_accepted_reply() {
  ...
  case TALK_END_ALLIANCE_TREATY:
    ...
    fromRelation->set_never_accept_until_date(TALK_PROPOSE_FRIENDLY_TREATY,
                                              300);
    toRelation->set_never_accept_until_date(TALK_PROPOSE_FRIENDLY_TREATY, 300);

    fromRelation->set_never_accept_until_date(TALK_PROPOSE_ALLIANCE_TREATY,
                                              300);
  ...
  case TALK_REQUEST_DECLARE_WAR:
    if (fromNation->get_relation_status(talk_para1) ==
        RELATION_HOSTILE) // the requesting nation must be at war with the enemy
    {
  ...
  case TALK_EXCHANGE_TECH:
    fromNation->give_tech(to_nation_recno, talk_para1,
                          talk_para3); // give this tech to the target nation
    toNation->give_tech(from_nation_recno, talk_para2,
                        talk_para4); // receive this tech from the target nation
    break;

  case TALK_REQUEST_SURRENDER: {
    float offeredAmt =
        (float)talk_para1 *
        10; // * 10 is to restore its original value. It has been divided by 10
            // to cope with the upper limit of <short>
  ...
  //---- if the nation accepts a message that is requesting help, then decrease
  //its good_relation_duration_rating, so it won't accept so easily next time
  //---//
  ...
  while (1) {
    ...
    int talkMsgRecno = talk_res.is_talk_msg_exist(
        &talkMsg, 0); // don't check talk_para1 & talk_para2

    if (talkMsgRecno &&
        talkMsg.talk_id != TALK_DECLARE_WAR) // Exception handling - Declare War
                                             // as both nations can send one
                                             // when a spy creates an incident
...
#define TALK_MSG_VALID_DAYS 30
  ...
  case TALK_REQUEST_TRADE_EMBARGO:
    ...
    //-- or if the requested nation already doesn't have a trade treaty with the
    //nation --//
  ...
  case TALK_REQUEST_DECLARE_WAR:
    ...
    if (fromNation->get_relation_status(talk_para1) != RELATION_HOSTILE)
      return 0;

    //-- or if the requested nation has become hostile with the nation --//

    if (toNation->get_relation_status(talk_para1) == RELATION_HOSTILE)
      return 0;
  ...
  case TALK_EXCHANGE_TECH:
    ...
    if (tech_res[talk_para1]->get_nation_tech_level(to_nation_recno) >=
            talk_para3 ||
        tech_res[talk_para2]->get_nation_tech_level(from_nation_recno) >=
            talk_para4) {
      return 0;
    }
    break;
...
char *TalkMsg::from_nation_name() {
  ...
  if (talk_res.msg_add_nation_color) {
    ...
    colorCodeStr[1] = FIRST_NATION_COLOR_CODE_IN_TEXT +
                      nation_array[from_nation_recno]->color_scheme_id;
...
char *TalkMsg::to_nation_name() {
  ...
  if (talk_res.msg_add_nation_color) {
    ...
    colorCodeStr[1] = FIRST_NATION_COLOR_CODE_IN_TEXT +
                      nation_array[to_nation_recno]->color_scheme_id;
...
char *TalkMsg::from_king_name() {
  ...
  Nation *nationPtr = nation_array[from_nation_recno];
  str = text_unit.str_unit_titled(nationPtr->race_id, nationPtr->king_name(),
                                  RANK_KING);
...
char *TalkMsg::to_king_name() {
  ...
  Nation *nationPtr = nation_array[to_nation_recno];
  str = text_unit.str_unit_titled(nationPtr->race_id, nationPtr->king_name(),
                                  RANK_KING);
...
// ------ begin of function TalkMsg::third_nation_name -------//

char *TalkMsg::third_nation_name(int nationRecno) {
  static String str;
  ...
  if (!nation_array.is_deleted(nationRecno)) {
    ...
    //		if( talk_res.msg_add_nation_color )	// always show color
    {
      char colorCodeStr[] = " 0";
    ...
    return nation_array.get_custom_nation_name(nationRecno);
  }
}
// ------ end of function TalkMsg::third_nation_name -------//
