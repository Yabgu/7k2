...
void FirmWar::process_queue() {
  ...
  if (nation_recno) {
    ...
    while (build_queue_count > 0 &&
           unit_res[build_queue_array[0]]->get_nation_tech_level(
               nation_recno) == 0) {
      misc.del_array_rec(build_queue_array, build_queue_count,
...
void FirmWar::process_build() {
  ...
  if (nation_recno) {
    bonusRate +=
        tech_res[TECH_WAR_MACHINE]->get_nation_tech_level(nation_recno) * 25;
    // ##### begin Gilbert 16/2 ######//
    bonusRate += god_res[GOD_INDIAN]->nation_prayer_count(nation_recno) * 30 /
                 MAX_WORKER; // 30% increase
                             // ##### end Gilbert 16/2 ######//
  }
