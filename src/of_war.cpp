...
void FirmWar::process_queue()
{
    ...
    if (nation_recno)
    {
        ...
        while (build_queue_count > 0 && unit_res[build_queue_array[0]]->get_nation_tech_level(nation_recno) == 0)
        {
...
void FirmWar::process_build()
{
    ...
    if (nation_recno)
    {
        ...
        // ##### begin Gilbert 16/2 ######//
        bonusRate +=
            god_res[GOD_INDIAN]->nation_prayer_count(nation_recno) * 30 / MAX_WORKER; // 30% increase
                                                                                      // ##### end Gilbert 16/2 ######//
    }
