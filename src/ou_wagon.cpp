    ...
    for (int i = spy_count - 1; i >= 0; --i)
    {
        ...
        if (spyPtr->true_nation_recno != spyPtr->cloaked_nation_recno)
        {
            ...
            if (!sys.signal_exit_flag &&
                spyPtr->true_nation_recno == nation_array.player_recno) // display when the player's spy is revealed
                                                                        // or the player has revealed an enemy spy
            {
...
//
// <short> spyRecno			-1=the first spy on the array, >0 any
// specific spy recno
//                         (default:-1)
