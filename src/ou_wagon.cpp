...
//
UnitWagon::UnitWagon() { init_derived(); }
...
//
// <short> spyRecno			-1=the first spy on the array, >0 any specific
// spy recno
//                         (default:-1)
...
short UnitWagon::del_spy(short spyRecno) {
  ...
  if (spyRecno < 0) {
    ...
    misc.del_array_rec(spy_recno, MAX_WAGON_POPULATION, sizeof(spy_recno[0]),
                       1);
  } else if (spyRecno > 0) {
    int i;
...
int UnitWagon::explore_for_player() {
  ...
  for (int i = 0; i < spy_count; ++i) {
    if (spy_array[spy_recno[i]]->true_nation_recno ==
...
short UnitWagon::mobilize_civilian(short spyRecno) {
  ...
  for (int r = 1; r < 16; ++r) {
    ...
    for (int y = y2; y >= y1; --y) {
      ...
      for (; x >= x1; --x, --locPtr) {
        if (world.check_unit_space(x, y, x + civSpriteInfo->loc_width - 1,
                                   y + civSpriteInfo->loc_height - 1)) {
          err_when(civUnitRecno);
          civUnitRecno = unit_array.add_unit(civUnitId, nation_recno,
                                             RANK_SOLDIER, loyalty, x, y);
          if (civUnitRecno)
            break;
        }
      }
