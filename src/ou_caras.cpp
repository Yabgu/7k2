...
void TradeStop::pick_up_set_auto() {
  //	memset(pick_up_array, 0, sizeof(char)*MAX_PICK_UP_GOODS);		// so
  //it can reserve
  pick_up_type = AUTO_PICK_UP;
}
//----------- End of function TradeStop::pick_up_set_auto -----------//

//--------- Begin of function TradeStop::pick_up_set_manual ---------//
void TradeStop::pick_up_set_manual() {
  //	memset(pick_up_array, 0, sizeof(char)*MAX_PICK_UP_GOODS);		// so
  //it can reserve

  // scan the first pickup
  int i;
  for (i = 0; i < MAX_PICK_UP_GOODS && !pick_up_array[i]; ++i)
    ;
  if (i >= MAX_PICK_UP_GOODS)
    pick_up_type = NO_PICK_UP;
  else
    pick_up_type = i + 1;
}
//----------- End of function TradeStop::pick_up_set_manual -----------//
...
int CaravanStop::update_pick_up(char *enableFlag) {
  ...
  int goodsNum =
      0; // represent the number of cargo displayed in the menu for this stop
  int firstGoodsId = 0;
  ...
  case FIRM_MARKET:
    ...
    selectiveMode =
        (pick_up_type != AUTO_PICK_UP && pick_up_type != NO_PICK_UP);
    for (i = 1; i <= MAX_MARKET_GOODS; i++, marketGoodsPtr++) {
      if ((id = marketGoodsPtr->raw_id)) {
        id += PICK_UP_RAW_FIRST - 1;
        goodsNum++;
        enableFlag[id] = 1;
        if (firstGoodsId == 0)
          firstGoodsId = id;
...
void CaravanStop::mp_pick_up_toggle(int pos)
{
        ...
        switch(firmPtr->firm_id)
        {
                case FIRM_MINE:
                                id =
((FirmMine*)firmPtr)->raw_id+PICK_UP_RAW_FIRST-1; if(id==pos) cargoExist = 1;
                                break;

                case FIRM_FACTORY:
                                id =
((FirmFactory*)firmPtr)->product_raw_id+PICK_UP_PRODUCT_FIRST-1; if(id==pos)
                                        cargoExist = 1;
                                break;

                case FIRM_MARKET:
                                marketGoodsPtr = ((FirmMarket*)
firmPtr)->market_goods_array; for(i=1; i<=MAX_MARKET_GOODS; i++,
marketGoodsPtr++)
                                {
...
marketGoodsPtr->product_raw_id)) // 1-3
                                        {
                                                id += PICK_UP_PRODUCT_FIRST-1;
                                                if(id==pos)
                                                {
