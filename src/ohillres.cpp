...
void HillRes::load_hill_block_info() {
  ...
  // Database *dbHill = game_set.open_db(HILL_DB);	// only one database can be
  // opened at a time
  Database *dbHill = &hillDbObj;

  hill_block_count = (short)dbHill->rec_count();
  hill_block_info_array =
      (HillBlockInfo *)mem_add(sizeof(HillBlockInfo) * hill_block_count);
  ...
  for (i = 0; i < hill_block_count; i++) {
    ...
    hillBlockInfo->pattern_id =
        (char)misc.atoi(hillBlockRec->pattern_id, hillBlockRec->PATTERN_ID_LEN);
    if (hillBlockInfo->pattern_id > max_pattern_id)
      max_pattern_id = hillBlockInfo->pattern_id;
    hillBlockInfo->sub_pattern_id = (char)misc.atoi(
        hillBlockRec->sub_pattern_id, hillBlockRec->SUB_PATTERN_ID_LEN);
    ...
    hillBlockInfo->priority =
        (char)misc.atoi(hillBlockRec->priority, hillBlockRec->PRIORITY_LEN);
    hillBlockInfo->bitmap_type = hillBlockRec->bitmap_type;
    hillBlockInfo->offset_x =
        misc.atoi(hillBlockRec->offset_x, hillBlockRec->OFFSET_LEN);
    hillBlockInfo->offset_y =
        misc.atoi(hillBlockRec->offset_y, hillBlockRec->OFFSET_LEN);
...
short HillRes::locate(char patternId, char subPattern, char searchPriority,
                      char specialFlag) {
...
short HillRes::scan(char patternId, char searchPriority, char specialFlag,
                    char findFirst) {
  err_when(patternId < 1 || patternId > max_pattern_id);
...
void HillBlockInfo::draw(int xLoc, int yLoc, int layerMask) {
  ...
  /*
          ...
          if( x1 <= -bitmap_width() || x1 >= ZOOM_WIDTH )	// out of the
     view area, not even a slight part of it appears in the view area return;
          ...
          {
                  vga_back.put_bitmap_area_trans_decompress( x1+ZOOM_X1,
...
void HillBlockInfo::draw_at(int absX1, int absY1, int layerMask) {
  ...
  if( x1 <= -bitmap_width() || x1 >= ZOOM_WIDTH )	// out of the view area,
  not even a slight part of it appears in the view area return;
  ...
  bitmap_ptr, MAX(0,x1)-x1, MAX(0,y1)-y1, MIN(ZOOM_WIDTH-1,x2)-x1,
