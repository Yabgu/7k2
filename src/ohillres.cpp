...
void HillRes::load_hill_block_info()
{
    ...
    // Database *dbHill = game_set.open_db(HILL_DB);	// only one database can
    // be opened at a time
    ...
    int patternMarked = 0;
    for (i = 0, hillBlockInfo = hill_block_info_array; i < hill_block_count; ++i, ++hillBlockInfo)
    {
...
{
    err_if(hillBlockId < 1 || hillBlockId > hill_block_count) err_now("HillRes::operator[]");
...
short HillRes::locate(char patternId, char subPattern, char searchPriority, char specialFlag)
{
    ...
    for (short j = startBlockIdx; j < endBlockIdx; ++j)
    {
        ...
        if (hillBlockInfo->pattern_id == patternId && hillBlockInfo->sub_pattern_id == subPattern &&
            hillBlockInfo->priority == searchPriority && hillBlockInfo->special_flag == specialFlag)
        {
            return j;
        }
    }
...
short HillRes::scan(char patternId, char searchPriority, char specialFlag, char findFirst)
{
    ...
    {
        HillBlockInfo *hillBlockInfo = hill_block_info_array + j - 1;
        if (hillBlockInfo->pattern_id == patternId && hillBlockInfo->priority == searchPriority &&
