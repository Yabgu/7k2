...
TownArray::TownArray()
    : DynArrayB(sizeof(Town *), 10, DEFAULT_REUSE_INTERVAL_DAYS) {}
//--------- End of function TownArray::TownArary ----------//

//------- Begin of function TownArray::~TownArray ----------//
//
TownArray::~TownArray() { deinit(); }
...
void TownArray::process() {
  ...
  if (sys.day_frame_count == 0 &&
      info.game_date % 15 == 0) // distribute demand every 15 days
    distribute_demand();
...
void TownArray::think_new_independent_town() {
  ...
  if (independentTownCount >=
      10) // only when the no. of independent town is less than 10
    return;

  //--- if the total population of all nations combined > 1000, then no new
  //independent town will emerge ---//
  ...
  for (i = 0; i < MAX_RACE; i++) {
    ...
    if (race_wander_pop_array[raceId - 1] >=
        10) // one of the race must have at least 10 people
      break;
  }
  ...
  townPtr->init_pop(addPop, townResistance, 0,
                    1); // 0-the add pop do not have jobs, 1-first init
...
//			 rangeX2, rangeY2   If not given, it can be on any
//location.
//
//
...
//                0 - free space found
//
int TownArray::think_town_loc(int maxTries, int &retXLoc, int &retYLoc,
                              int rangeX1, int rangeY1, int rangeX2,
                              int rangeY2) {
...
#define BUILD_TOWN_LOC_HEIGHT 16
  ...
  for (i = 0; i < maxTries; i++) {
    if (rangeX1 >= 0) {
      xLoc =
          rangeX1 + misc.random(rangeX2 - rangeX1 + 1 - BUILD_TOWN_LOC_WIDTH);
      yLoc = 2 + rangeY1 +
             misc.random(rangeY2 - rangeY1 + 1 - BUILD_TOWN_LOC_HEIGHT -
                         2); // do not build on the upper most location as the
                             // flag will go beyond the view area
    } else {
    ...
    if (!world.can_build_town(xLoc + INTER_PLACE_SPACE,
    ...
    for (townRecno = town_array.size(); townRecno > 0; townRecno--) {
      ...
      if (misc.points_distance(
              xLoc + 1, yLoc + 1,
              townPtr->center_x, // xLoc+1 and yLoc+1 to take the center
    ...
    /*
       ...
       --------//
       ...
       )
                    {
                            ...
                            if( misc.points_distance(xLoc+1, yLoc+1,
       firmPtr->center_x, firmPtr->center_y) < MONSTER_ATTACK_NEIGHBOR_RANGE )
                            {
                                    break;
                            }
                    }

                    if( firmRecno > 0 )     // if it's too close to monster
       firms continue;
    */
    //----------------------------------------//
  ...
  for (i = 1; i <= size(); i++) {
    ...
    { // minimize register variable at one time
      ...
      if (town_array.selected_recno !=
          i) // if the town is currently selected, ignore all filters
      {
        //				if( filterRaceId && townPtr->race_id !=
        //filterRaceId ) 					continue;

        // filter selected nation
        if (world.map_matrix->filter_nation_flag &&
            world.map_matrix->filter_nation_recno != townPtr->nation_recno)
          continue;
      }
      ...
      if (world.map_matrix->map_mode != MAP_MODE_TRADE) {
        ...
        if (firm_array.selected_recno &&
            ...
            townPtr->get_linked_firm_status(firm_array.selected_recno) ==
                LINK_EE) {
          nationColor = slowExcitedColorArray[townNation];
        } else {
          nationColor = nationColorArray[townNation];
      ...
      rowWritePtr = vga_back.buf_ptr(
          world.map_matrix->calc_loc_pos_x(townPtr->loc_x1, townPtr->loc_y1) +
              MAP_X1,
          world.map_matrix->calc_loc_pos_y(townPtr->loc_x1, townPtr->loc_y1) +
              MAP_Y1);
    }
    ...
    { // minimize register variable at one time
      ...
      for (y = 0; y < STD_TOWN_LOC_HEIGHT;
           (rowWritePtr += y & 1 ? vgaBufPitch : -1), ++y) {
        writePtr = rowWritePtr;
        for (x = 0; x < STD_TOWN_LOC_WIDTH;
             (writePtr += vgaBufPitch + 1), x += 2) {
...
int TownArray::find_nearest_town(int xLoc, int yLoc, int nationRecno) {
  ...
  for (i = size(); i > 0; i--) {
    ...
    curDistance =
        misc.points_distance(xLoc, yLoc, townPtr->center_x, townPtr->center_y);
...
//					  0 - settling failed. too close to a town of
//another nation.
//
