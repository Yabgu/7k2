...
//                0 - free space found
//
int TownArray::think_town_loc(int maxTries, int &retXLoc, int &retYLoc, int rangeX1, int rangeY1, int rangeX2,
                              int rangeY2)
{
        ...
        if (rangeX1 >= 0)
        {
            ...
            yLoc = 2 + rangeY1 +
                   misc.random(rangeY2 - rangeY1 + 1 - BUILD_TOWN_LOC_HEIGHT -
                               2); // do not build on the upper most location
                                   // as the flag will go beyond the view area
        }
        else
        {
    ...
    for (i = 1; i <= size(); i++)
    {
        ...
        { // minimize register variable at one time
            ...
            else
            {
                ...
                Firm *firmPtr;
                if (firm_array.selected_recno && (firmPtr = firm_array[firm_array.selected_recno]) &&
            ...
            rowWritePtr = vga_back.buf_ptr(world.map_matrix->calc_loc_pos_x(townPtr->loc_x1, townPtr->loc_y1) + MAP_X1,
...
//					  0 - settling failed. too close to a town
// of
// another nation.
//
