...
void SpinnerSmall::create(int x1, int y1, int x2, int y2, int vbrowseHeight, int minRec, int recCount,
                          SpinDispFP funcPtr, int recNo)
{
    min_rec_num = minRec;
...
void SpinnerSmall::paint(int recNo)
{
    ...
    vga.active_buf->d3_panel_down(ox1, oy1, ox2 - SCROLL_BUTTON_WIDTH, oy2, D3_PANEL_THICK, 1);
...
int SpinnerSmall::detect(int noPaint)
{
    if (!none_record && disp_rec_func)
    {
        ...
        if (button_vbrowse.detect() || mouse.single_click(ix1, iy1, ix2, iy2))
        {
            vbrowse_enable_flag = !vbrowse_enable_flag;
            if (vbrowse_enable_flag)
            {
                vbrowse.init(ox1, oy2, ox2 - SCROLL_BUTTON_WIDTH + SCROLL_BAR_WIDTH, oy2 + vbrowse_height, -1, 20,
                             total_rec_num, disp_vbrowse);
