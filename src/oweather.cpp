...
void Weather::init()
{
    ...
    init_date(info.game_year, info.game_month, info.game_day, config.latitude, quakeFreq);
...
void Weather::next_day()
{
    ...
    else
    {
        ...
        if (cur_cloud_str > 4)
        {
            ...
            // cur_cloud_str-4)
            if ((char)rand_seed(10) < cur_cloud_str - 2) // higher chance of windy, higher chance to make a tornado
    ...
    // && rand_seed(10)==0 )
    if (tornado_count > 20 && base_temp() >= 25 && wind_speed() >= 40 && rand_seed(10) < 2)
    {
...
{
    return base_temp() - (cur_cloud_str < 1 ? 0 : (cur_cloud_str < 4 ? 2 : 4)) + (cur_cloud_type & HOT_WAVE ? 8 : 0) -
