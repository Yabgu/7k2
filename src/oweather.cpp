...
//---------- Begin of function Weather::init ----------//
//
// The main init function called by game.init().
//
void Weather::init() {
  int quakeFreq;
  ...
  if (config.random_event_frequency) {
    quakeFreq =
        2000 - config.random_event_frequency * 400 + info.random_seed % 300;
  } else {
    quakeFreq = 0x0fffffff;
  }

  init_date(info.game_year, info.game_month, info.game_day, config.latitude,
            quakeFreq);
...
//---------- End of function Weather::init ----------//

//---------- Begin of function Weather::init_date ----------//
//
void Weather::init_date(short year, short month, short day, short latitude,
                        int quakeFreq) {
...
void Weather::next_day() {
  ...
  if (day_to_quake) {
    ...
    if (is_quake()) {
      ...
      quake_center_x = rand_seed(0x10000) * MAX_MAP_WIDTH / 0x10000;
      quake_center_y = rand_seed(0x10000) * MAX_MAP_HEIGHT / 0x10000;
    }
  } else
  ...
  if (cur_cloud_len > 0) {
    ...
    if (cur_cloud_str > 4) {
      ...
      //			if( cur_cloud_str >= 6 && (char) rand_seed(10) <
      //cur_cloud_str-4)
      if ((char)rand_seed(10) <
          cur_cloud_str -
              2) // higher chance of windy, higher chance to make a tornado
  ...
  // && rand_seed(10)==0 )
  if (tornado_count > 20 && base_temp() >= 25 && wind_speed() >= 40 &&
      rand_seed(10) < 2) {
...
short Weather::base_temp() {
  return ((short)(avg_temp + temp_amp * sin(season_phase / 365.0 * 2 * PI)));
}
...
short Weather::temp_c() {
  return base_temp() - (cur_cloud_str < 1 ? 0 : (cur_cloud_str < 4 ? 2 : 4)) +
         (cur_cloud_type & HOT_WAVE ? 8 : 0) -
         (cur_cloud_type & COLD_WAVE ? 10 : 0);
}
//---------- End of function Weather::temp_c ----------//

//---------- Begin of function Weather::temp_f ----------//
//
short Weather::temp_f() { return ((short)(base_temp() / 5.0 * 9.0 + 32.0)); }
...
char Weather::is_lightning() {
  if (this == &weather && magic_weather.lightning_day > 0)
...
char Weather::is_quake() {
  return ((this == &weather && magic_weather.is_quake) || day_to_quake == 0);
}
//---------- End of function Weather::is_quake ----------//

//---------- Begin of function Weather::quake_x ----------//
//
short Weather::quake_x() {
  if (this == &weather && magic_weather.is_quake)
    return magic_weather.quake_x;
  else
    return quake_center_x;
}
//---------- End of function Weather::quake_x ----------//

//---------- Begin of function Weather::quake_y ----------//
//
short Weather::quake_y() {
  if (this == &weather && magic_weather.is_quake)
    return magic_weather.quake_y;
  else
    return quake_center_y;
}
//---------- End of function Weather::quake_y ----------//

//---------- Begin of function Weather::quake_radius ----------//
//
short Weather::quake_radius() {
  if (this == &weather && magic_weather.is_quake)
    return magic_weather.quake_radius;
  else
    return 500; // infinite
}
//---------- End of function Weather::quake_radius ----------//
...
char Weather::has_tornado() { return tornado_count == 0; }
...
short MagicWeather::wind_speed() { return wind_spd; }
//--------- End of function MagicWeather::wind_speed ----------//

//--------- Begin of function MagicWeather::wind_direct ----------//
short MagicWeather::wind_direct() { return wind_dir; }
//--------- End of function MagicWeather::wind_direct ----------//

//--------- Begin of function MagicWeather::wind_direct_rad ----------//
double MagicWeather::wind_direct_rad() { return wind_dir * PI / 180.0; }
//--------- End of function MagicWeather::wind_direct_rad ----------//

//--------- Begin of function MagicWeather::rain_scale ----------//
short MagicWeather::rain_scale() { return rain_str; }
//--------- End of function MagicWeather::rain_scale ----------//

//--------- Begin of function MagicWeather::cast_quake ----------//
//
void MagicWeather::cast_quake(short x, short y, short radius) {
  is_quake = 2; // last for day, cast_quake is between a day, so is_quake can
                // last for longer than one day to cause damage
  quake_x = x;
  quake_y = y;
  quake_radius = radius;
}
//--------- End of function MagicWeather::cast_quake ----------//
