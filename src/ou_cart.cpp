            ...
            for (y = y1; y <= y2; ++y)
            {
                for (x = x1; x <= x2; ++x)
                {
                    ...
                    if (baseObjRecno)
                    {
                        ...
                        if (unit_res[unit_id]->class_info.has_weapon_version && get_weapon_version() > 2)
                        {
                            // at level 3, 1.5 times damage
                            explodeDamage *= (get_weapon_version()) * 0.5f;
                        }

                        baseObj->being_attack_hit(this, (float)EXPLODE_DAMAGE * cart_attack_attribute.effectiveness(
