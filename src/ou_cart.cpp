...
UnitExpCart::UnitExpCart() : Unit() { triggered = 0; }
      ...
      for (y = y1; y <= y2; ++y) {
        for (x = x1; x <= x2; ++x) {
          ...
          if (baseObjRecno) {
            ...
            baseObj->being_attack_hit(this,
                                      (float)EXPLODE_DAMAGE *
                                          cart_attack_attribute.effectiveness(
                                              baseObj->defense_attribute));
            // ##### end Gilbert 6/11 ######//
          }
        }
      }
...
int UnitExpCart::is_camouflage(int viewerNation) {
  ...
  if (cur_action != SPRITE_DIE && !is_nation(viewerNation) &&
