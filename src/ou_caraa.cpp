...
int UnitCaravan::think_resign()
{
    ...
    if (info.game_date > last_load_goods_date + 365 &&
        info.game_date % 30 == sprite_recno % 30) // don't call too often as the action may fail and
    ...
    if (info.game_date % 30 == sprite_recno % 30) // don't call too often as the action may fail and it
                                                  // takes a while to call the function each time
    {
        for (int i = stop_defined_num; i > 0; i--)
        {
            ...
            if (firm_array.is_deleted(firmRecno) || firm_array[firmRecno]->firm_id != FIRM_MARKET)
            {
...
int UnitCaravan::think_del_stop()
{
    ...
    for (i = stop_defined_num; i > 0; i--)
    {
        ...
        if (firmPtr->firm_id != FIRM_MARKET || nationPtr->get_relation(firmPtr->nation_recno)->trade_treaty ==
...
void UnitCaravan::think_set_pick_up_type2(int fromStopId, int toStopId)
{
    ...
    for (i = PICK_UP_PRODUCT_FIRST; i <= PICK_UP_PRODUCT_LAST; i++)
    {
        ...
        marketGoods = fromMarket->market_product_array(i - PICK_UP_PRODUCT_FIRST + 1);
