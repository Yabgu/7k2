...
void UnitCaravan::init_derived() { last_load_goods_date = info.game_date; }
...
void UnitCaravan::process_ai() {
  ...
  // ##### begin Gilbert 9/11 ######//
  // if( info.game_date%30 == sprite_recno%30 );		// extra
  // semi-colon
...
int UnitCaravan::think_resign() {
  ...
  if (info.game_date > last_load_goods_date + 365 &&
      info.game_date % 30 ==
          sprite_recno % 30) // don't call too often as the action may fail and
                             // it takes a while to call the function each time
  {
  ...
  if (info.game_date % 30 ==
      sprite_recno % 30) // don't call too often as the action may fail and it
...
int UnitCaravan::think_del_stop() {
  ...
  for (i = stop_defined_num; i > 0; i--) {
    ...
    if (firmPtr->firm_id != FIRM_MARKET ||
        nationPtr->get_relation(firmPtr->nation_recno)->trade_treaty ==
            0) // if the treaty trade has been terminated, delete the stop
    {
      del_stop(i, COMMAND_AI);
      return 1;
    }
    ...
    FirmMarket *firmMarket =
        (FirmMarket *)firm_array[stop_array[i - 1].firm_recno];
    ...
    if (nationPtr->get_relation_status(nationRecno) == RELATION_HOSTILE) {
      del_stop(i, COMMAND_AI);
      return 1;
    }
  }
...
void UnitCaravan::think_set_pick_up_type() {
  if (!is_visible()) // cannot change pickup type if the caravan is inside a
                     // market place.
    return;
  ...
  //--- think in both directions as a market can sell both raw materials and
  //finished goods ---//
...
void UnitCaravan::think_set_pick_up_type2(int fromStopId, int toStopId) {
  FirmMarket *fromMarket =
      (FirmMarket *)firm_array[stop_array[fromStopId - 1].firm_recno];
  FirmMarket *toMarket =
      (FirmMarket *)firm_array[stop_array[toStopId - 1].firm_recno];
  ...
  for (i = 0; i < MAX_MARKET_GOODS; i++, marketGoods++) {
    ...
    //-- when the from market has the product and the to market does not have
    //the product, then trade this good --//
  ...
  for (i = PICK_UP_RAW_FIRST; i <= PICK_UP_RAW_LAST; i++) {
    ...
    marketGoods = fromMarket->market_raw_array(i - PICK_UP_RAW_FIRST + 1);
  ...
  for (i = PICK_UP_PRODUCT_FIRST; i <= PICK_UP_PRODUCT_LAST; i++) {
    ...
    marketGoods =
        fromMarket->market_product_array(i - PICK_UP_PRODUCT_FIRST + 1);
