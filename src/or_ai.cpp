...
static const char *action_mode_str_array[] = {
    "Build firm", "Assign overseer", "Assign worker",  "Assign spy",
    "Scout",      "Build town",      "Settle to town", "Process diplomatic message",
    /*
            "Defend",
            "Sea Travel 1",
            "Sea Travel 2",
...
void Info::disp_ai_action(int refreshFlag)
{
    ...
    browse_action.init(ACTION_BROWSE_X1, ACTION_BROWSE_Y1 + 20, ACTION_BROWSE_X2, ACTION_BROWSE_Y2, 0, 22,
                       cur_nation_ptr->action_count(), put_action_rec, 1);

    browse_action.open(browse_ai_action_recno); // if refreshFlag is INFO_UPDATE, keep the
    ...
    browse_attack.init(ATTACK_BROWSE_X1, ATTACK_BROWSE_Y1 + 20, ATTACK_BROWSE_X2, ATTACK_BROWSE_Y2, 0, 22,
                       cur_nation_ptr->attack_camp_count, put_attack_rec, 1);

    browse_attack.open(browse_ai_attack_recno); // if refreshFlag is INFO_UPDATE, keep the
                                                // original top_rec_no of the browser
...
static void put_action_rec(int recNo, int x, int y, int refreshFlag)
{
    ...
    if (actionNode->action_mode >= ACTION_AI_BUILD_FIRM && actionNode->action_mode <= ACTION_AI_ASSIGN_SOLDIER)
    {
    ...
    /*
       font_bld.put( x+220, y, actionNode->action_x_loc );		// short
       month string font_bld.put( x+340, y, actionNode->action_y_loc );
       font_bld.put( x+410, y, actionNode->ref_x_loc ); font_bld.put( x+490, y,
...
static void disp_other_info()
{
    ...
    font_bld.put_field(x, y += 16, "ai_capture_enemy_town_plan_date", x2,
                       date.date_str(cur_nation_ptr->ai_capture_enemy_town_plan_date));
    font_bld.put_field(x, y += 16, "ai_capture_enemy_town_start_attack_date", x2,
                       date.date_str(cur_nation_ptr->ai_capture_enemy_town_start_attack_date));

    font_bld.put_field(x, y += 16, "ai_attack_target_nation_recno", x2, cur_nation_ptr->ai_attack_target_nation_recno,
                       1);
