...
void MouseSDL::hide_area(int x1, int y1, int x2, int y2)
{
    ...
    if (misc.is_touch(x1, y1, x2, y2, curX, curY, curX + mouse_cursor.icon_width - 1,
...
void MouseSDL::show_area()
{
    ...
    if (misc.is_touch(mouse_cursor.hide_x1, mouse_cursor.hide_y1, mouse_cursor.hide_x2, mouse_cursor.hide_y2, curX,
                      curY, curX + mouse_cursor.icon_width - 1, curY + mouse_cursor.icon_height - 1))
    {
...
// click_buffer[LEFT_BUTTON] 			if( RIGHT_BUTTON or
// RIGHT_BUTTON_RELEASE, read click_buffer[RIGHT_BUTTON]
...
int MouseSDL::get_event()
{
    ...
    do
    {
        ...
        case RIGHT_BUTTON:
            ...
            if ( // eptr->event_type == LEFT_BUTTON	&&		// left button has
                 // double click
...
int MouseSDL::single_click(int x1, int y1, int x2, int y2, int buttonId)
{
    ...
    if (buttonId == 0 || buttonId == 2) // left button
    {
        ...
        if (mouse_event_type == LEFT_BUTTON && cptr->count == 1 && cptr->x >= x1 && cptr->y >= y1 && cptr->x <= x2 &&
            cptr->y <= y2)
        {
    ...
    if (buttonId == 1 || buttonId == 2) // right button
    {
        ...
        if (mouse_event_type == RIGHT_BUTTON && cptr->count == 1 && cptr->x >= x1 && cptr->y >= y1 && cptr->x <= x2 &&
            cptr->y <= y2)
        {
            if (game.game_mode != GAME_TUTORIAL ||
...
int MouseSDL::double_click(int x1, int y1, int x2, int y2, int buttonId)
{
    ...
    if (buttonId == 0 || buttonId == 2) // left button
    {
        ...
        if (mouse_event_type == LEFT_BUTTON && cptr->count == 2 && cptr->x >= x1 && cptr->y >= y1 && cptr->x <= x2 &&
            cptr->y <= y2)
        {
            if (game.game_mode != GAME_TUTORIAL ||
                (game.game_mode == GAME_TUTORIAL && tutor.allow_mouse_click(cptr->x, cptr->y, 1, 1)))
    ...
    if (buttonId == 1 || buttonId == 2) // right button
    {
        ...
        if (mouse_event_type == RIGHT_BUTTON && cptr->count == 2 && cptr->x >= x1 && cptr->y >= y1 && cptr->x <= x2 &&
            cptr->y <= y2)
        {
            if (game.game_mode != GAME_TUTORIAL ||
                (game.game_mode == GAME_TUTORIAL && tutor.allow_mouse_click(cptr->x, cptr->y, 1, 2)))
...
int MouseSDL::release_click(int x1, int y1, int x2, int y2, int buttonId)
{
    ...
    if (buttonId == 0 || buttonId == 2) // left button
    {
        ...
        if (mouse_event_type == LEFT_BUTTON_RELEASE && cptr->release_x >= x1 && cptr->release_y >= y1 &&
    ...
    if (buttonId == 1 || buttonId == 2) // right button
    {
        ...
        if (mouse_event_type == RIGHT_BUTTON_RELEASE && cptr->release_x >= x1 && cptr->release_y >= y1 &&
            cptr->release_x <= x2 && cptr->release_y <= y2)
        {
            if (game.game_mode != GAME_TUTORIAL ||
...
int MouseSDL::is_key(unsigned scanCode, unsigned short skeyState, unsigned short charValue, unsigned flags)
{
    ...
    if (flags & K_TRANSLATE_KEY)
    {
        ...
        {
            // letter
            outChar = skeyState & CAP_LOCK_STATE_MASK ? (skeyState & SHIFT_KEY_MASK ? priChar : capitalChar)
    ...
    int retFlag2 = (charValue == 0) || outChar == charValue || ((flags & K_IGNORE_SHIFT) && shiftChar == charValue) ||
