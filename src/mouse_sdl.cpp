...
static int update_x1, update_y1, update_x2,
    update_y2; // coordination of the last double-buffer update area
...
void MouseSDL::hide_area(int x1, int y1, int x2, int y2) {
  ...
  if (mouse_cursor.hide_area_flag !=
      1) // only process for the first call of hide_area()
    return;
  ...
  if (misc.is_touch(x1, y1, x2, y2, curX, curY,
    ...
    if (handle_flicking) {
      ...
      err_when((update_x2 - update_x1 + 1) * (update_y2 - update_y1 + 1) *
                   sizeof(short) >
               VGA_UPDATE_BUF_SIZE);

      //---- save the update area of the back buf to a temp buffer ----//

      vga_back.read_bitmapW(update_x1, update_y1, update_x2, update_y2,
                            vga_update_buf);

      //--- copy the update area from the front buf to the back buf ---//

      vga_back.blt_buf_fast(&vga_front, update_x1, update_y1, update_x2,
                            update_y2);
...
void MouseSDL::show_area() {
  ...
  if (mouse_cursor.hide_area_flag !=
      0) // only process for the first call of hide_area()
    return;
  ...
  if (misc.is_touch(mouse_cursor.hide_x1, mouse_cursor.hide_y1,
                    mouse_cursor.hide_x2, mouse_cursor.hide_y2, curX, curY,
                    curX + mouse_cursor.icon_width - 1,
                    curY + mouse_cursor.icon_height - 1)) {
    ...
    if (handle_flicking) {
      ...
      vga_front.blt_buf_fast(&vga_back, update_x1, update_y1, update_x2,
                             update_y2);
...
void MouseSDL::add_event(MouseEvent *mouseEvent) {
  //---- call the game object to see if the mouse cursor icon needs to be
  //changed, or if the nation selection square needs to be activated ----//
...
//			if( LEFT_BUTTON or LEFT_BUTTON_RELEASE, read
//click_buffer[LEFT_BUTTON] 			if( RIGHT_BUTTON or RIGHT_BUTTON_RELEASE, read
//click_buffer[RIGHT_BUTTON]
...
int MouseSDL::get_event() {
  ...
  do {
    ...
    case RIGHT_BUTTON:
      ...
      if ( // eptr->event_type == LEFT_BUTTON	&&		// left button has double
           // click
    ...
    case KEY_RELEASE:
      ...
      // rc = 1;		// key release is not an event to outside, so
      // don't set rc
      break;
...
void MouseSDL::set_boundary(int x1, int y1, int x2, int y2, int boundType) {
  ...
  case 1: // rhombus boundary
  {
    ...
    // let vector U is from origin to upper right corner of Rectangle
    // (bound_x1,y1,x2,y2) let vector V is from origin to upper left coner of
    // the Rectangle a, b is width and height of the Rectangle U = (a/2)I +
    // (b/2)J V = (-a/2)I + (b/2)J
    // => aI = U-V		 bJ = U+V
...
int MouseSDL::single_click(int x1, int y1, int x2, int y2, int buttonId) {
  ...
  if (buttonId == 0 || buttonId == 2) // left button
  {
    ...
    if (mouse_event_type == LEFT_BUTTON && cptr->count == 1 && cptr->x >= x1 &&
        cptr->y >= y1 && cptr->x <= x2 && cptr->y <= y2) {
      if (game.game_mode != GAME_TUTORIAL ||
          (game.game_mode == GAME_TUTORIAL &&
           tutor.allow_mouse_click(cptr->x, cptr->y, 1, 1)))
        return 1;
    }
  }
  ...
  if (buttonId == 1 || buttonId == 2) // right button
  {
    ...
    if (mouse_event_type == RIGHT_BUTTON && cptr->count == 1 && cptr->x >= x1 &&
        cptr->y >= y1 && cptr->x <= x2 && cptr->y <= y2) {
      if (game.game_mode != GAME_TUTORIAL ||
          (game.game_mode == GAME_TUTORIAL &&
           tutor.allow_mouse_click(cptr->x, cptr->y, 1, 2)))
        return 2;
    }
  }
...
int MouseSDL::double_click(int x1, int y1, int x2, int y2, int buttonId) {
  ...
  if (buttonId == 0 || buttonId == 2) // left button
  {
    ...
    if (mouse_event_type == LEFT_BUTTON && cptr->count == 2 && cptr->x >= x1 &&
        cptr->y >= y1 && cptr->x <= x2 && cptr->y <= y2) {
      if (game.game_mode != GAME_TUTORIAL ||
          (game.game_mode == GAME_TUTORIAL &&
           tutor.allow_mouse_click(cptr->x, cptr->y, 1, 1)))
  ...
  if (buttonId == 1 || buttonId == 2) // right button
  {
    ...
    if (mouse_event_type == RIGHT_BUTTON && cptr->count == 2 && cptr->x >= x1 &&
        cptr->y >= y1 && cptr->x <= x2 && cptr->y <= y2) {
      if (game.game_mode != GAME_TUTORIAL ||
          (game.game_mode == GAME_TUTORIAL &&
           tutor.allow_mouse_click(cptr->x, cptr->y, 1, 2)))
...
int MouseSDL::release_click(int x1, int y1, int x2, int y2, int buttonId) {
  ...
  if (buttonId == 0 || buttonId == 2) // left button
  {
    ...
    if (mouse_event_type == LEFT_BUTTON_RELEASE && cptr->release_x >= x1 &&
        cptr->release_y >= y1 && cptr->release_x <= x2 &&
        cptr->release_y <= y2) {
      if (game.game_mode != GAME_TUTORIAL ||
          (game.game_mode == GAME_TUTORIAL &&
           tutor.allow_mouse_click(cur_x, cur_y, 2, 1)))
        return 1;
    }
  }
  ...
  if (buttonId == 1 || buttonId == 2) // right button
  {
    ...
    if (mouse_event_type == RIGHT_BUTTON_RELEASE && cptr->release_x >= x1 &&
        cptr->release_y >= y1 && cptr->release_x <= x2 &&
        cptr->release_y <= y2) {
      if (game.game_mode != GAME_TUTORIAL ||
          (game.game_mode == GAME_TUTORIAL &&
           tutor.allow_mouse_click(cur_x, cur_y, 2, 2)))
        return 2;
    }
  }
...
int MouseSDL::poll_event() {
  ...
  while (SDL_PeepEvents(&event, 1, SDL_GETEVENT, SDL_KEYDOWN, SDL_MOUSEWHEEL) >
    ...
    case SDL_KEYDOWN: {
      int bypass = 0;
      int mod = event.key.keysym.mod & (KMOD_CTRL | KMOD_SHIFT | KMOD_ALT);
...
int MouseSDL::wait_press(int timeOutSecond) {
  ...
  if (sys.paused_flag ||
      !sys.active_flag) // return immediately if switched task
    return 0;
  // ####### end Gilbert 7/7 #######//

  while (mouse.left_press || mouse.any_click() ||
         mouse.key_code) // avoid repeat clicking
  {
    sys.yield();
    mouse.get_event();
  }
...
long MouseSDL::micky_to_displacement(DWORD w) {
  long d = (long)w;
  // long a = abs(d);
  // return a >= double_speed_threshold ? (a >= triple_speed_threshold ? 3 * d :
  // 2 * d) : d;
...
// true if since LEFT_BUTTON (RIGHT_BUTTON) and before LEFT_BUTTON_RELEASE
// (RIGHT_BUTTON_RELEASE)
//
// remark : press_button_time[buttonId] indicates time when button is pressed
int MouseSDL::is_holding(int buttonId) { return holding_button[buttonId]; }
...
// call mouse.is_key(mouse.scan_code, mouse.event_skey_state, 'a', K_CHAR_KEY |
// K_IS_ALT)
//
// pass 0 as charValue to disable checking in charValue
// e.g pressed key is 'a'
// mouse.is_key(mouse.scan_code, mouse.event_skey_state, (WORD) 0, K_CHAR_KEY)
// returns 'a' but if key pressed is alt-a the same function call returns 0 use
// mouse.is_key(mouse.scan_code, mouse.event_skey_state, (WORD) 0, K_CHAR_KEY |
// K_IS_ALT ) instead
//
int MouseSDL::is_key(unsigned scanCode, unsigned short skeyState,
                     unsigned short charValue, unsigned flags) {
  unsigned short priChar = 0, shiftChar = 0, capitalChar = 0;
  unsigned onNumPad = 0;

  switch (scanCode) {
  case SDLK_ESCAPE:
    priChar = shiftChar = capitalChar = KEY_ESC;
    break;
  case SDLK_1:
    priChar = capitalChar = '1';
    shiftChar = '!';
    break;
  case SDLK_2:
    priChar = capitalChar = '2';
    shiftChar = '@';
    break;
  case SDLK_3:
    priChar = capitalChar = '3';
    shiftChar = '#';
    break;
  case SDLK_4:
    priChar = capitalChar = '4';
    shiftChar = '$';
    break;
  case SDLK_5:
    priChar = capitalChar = '5';
    shiftChar = '%';
    break;
  case SDLK_6:
    priChar = capitalChar = '6';
    shiftChar = '^';
    break;
  case SDLK_7:
    priChar = capitalChar = '7';
    shiftChar = '&';
    break;
  case SDLK_8:
    priChar = capitalChar = '8';
    shiftChar = '*';
    break;
  case SDLK_9:
    priChar = capitalChar = '9';
    shiftChar = '(';
    break;
  case SDLK_0:
    priChar = capitalChar = '0';
    shiftChar = ')';
    break;
  case SDLK_MINUS:
    priChar = capitalChar = '-';
  ...
  case SDLK_EQUALS:
    ...
    break;
  case SDLK_BACKSPACE:
    priChar = capitalChar = shiftChar = KEY_BACK_SPACE;
    break; // backspace
  case SDLK_TAB:
    priChar = capitalChar = shiftChar = KEY_TAB;
    break;
  case SDLK_q:
    priChar = 'q';
    capitalChar = shiftChar = 'Q';
    break;
  case SDLK_w:
    priChar = 'w';
    capitalChar = shiftChar = 'W';
    break;
  case SDLK_e:
    priChar = 'e';
    capitalChar = shiftChar = 'E';
    break;
  case SDLK_r:
    priChar = 'r';
    capitalChar = shiftChar = 'R';
    break;
  case SDLK_t:
    priChar = 't';
    capitalChar = shiftChar = 'T';
    break;
  case SDLK_y:
    priChar = 'y';
    capitalChar = shiftChar = 'Y';
    break;
  case SDLK_u:
    priChar = 'u';
    capitalChar = shiftChar = 'U';
    break;
  case SDLK_i:
    priChar = 'i';
    capitalChar = shiftChar = 'I';
    break;
  case SDLK_o:
    priChar = 'o';
    capitalChar = shiftChar = 'O';
    break;
  case SDLK_p:
    priChar = 'p';
    capitalChar = shiftChar = 'P';
    break;
  ...
  case SDLK_RETURN:
    priChar = capitalChar = shiftChar = KEY_RETURN;
    break;
  case SDLK_a:
    priChar = 'a';
    capitalChar = shiftChar = 'A';
    break;
  case SDLK_s:
    priChar = 's';
    capitalChar = shiftChar = 'S';
    break;
  case SDLK_d:
    priChar = 'd';
    capitalChar = shiftChar = 'D';
    break;
  case SDLK_f:
    priChar = 'f';
    capitalChar = shiftChar = 'F';
    break;
  case SDLK_g:
    priChar = 'g';
    capitalChar = shiftChar = 'G';
    break;
  case SDLK_h:
    priChar = 'h';
    capitalChar = shiftChar = 'H';
    break;
  case SDLK_j:
    priChar = 'j';
    capitalChar = shiftChar = 'J';
    break;
  case SDLK_k:
    priChar = 'k';
    capitalChar = shiftChar = 'K';
    break;
  case SDLK_l:
    priChar = 'l';
    capitalChar = shiftChar = 'L';
    break;
  case SDLK_SEMICOLON:
    priChar = capitalChar = ';';
    shiftChar = ':';
  ...
  case SDLK_z:
    priChar = 'z';
    capitalChar = shiftChar = 'Z';
    break;
  case SDLK_x:
    priChar = 'x';
    capitalChar = shiftChar = 'X';
    break;
  case SDLK_c:
    priChar = 'c';
    capitalChar = shiftChar = 'C';
    break;
  case SDLK_v:
    priChar = 'v';
    capitalChar = shiftChar = 'V';
    break;
  case SDLK_b:
    priChar = 'b';
    capitalChar = shiftChar = 'B';
    break;
  case SDLK_n:
    priChar = 'n';
    capitalChar = shiftChar = 'N';
    break;
  case SDLK_m:
    priChar = 'm';
    capitalChar = shiftChar = 'M';
    break;
  case SDLK_COMMA:
    priChar = capitalChar = ',';
    shiftChar = '<';
  ...
  case SDLK_KP_7:
    priChar = shiftChar = capitalChar = '7';
    onNumPad = 1;
    break;
  case SDLK_KP_8:
    priChar = shiftChar = capitalChar = '8';
    onNumPad = 1;
    break;
  case SDLK_KP_9:
    priChar = shiftChar = capitalChar = '9';
    onNumPad = 1;
    break;
  case SDLK_KP_4:
    priChar = shiftChar = capitalChar = '4';
    onNumPad = 1;
    break;
  case SDLK_KP_5:
    priChar = shiftChar = capitalChar = '5';
    onNumPad = 1;
    break;
  case SDLK_KP_6:
    priChar = shiftChar = capitalChar = '6';
    onNumPad = 1;
    break;
  case SDLK_KP_1:
    priChar = shiftChar = capitalChar = '1';
    onNumPad = 1;
    break;
  case SDLK_KP_2:
    priChar = shiftChar = capitalChar = '2';
    onNumPad = 1;
    break;
  case SDLK_KP_3:
    priChar = shiftChar = capitalChar = '3';
    onNumPad = 1;
    break;
  case SDLK_KP_0:
    priChar = shiftChar = capitalChar = '0';
    onNumPad = 1;
    break;
  case SDLK_KP_PERIOD:
    priChar = shiftChar = capitalChar = '.';
    onNumPad = 1;
    break;

  // function keys
  case SDLK_F1:
    priChar = shiftChar = capitalChar = KEY_F1;
    break;
  case SDLK_F2:
    priChar = shiftChar = capitalChar = KEY_F2;
    break;
  case SDLK_F3:
    priChar = shiftChar = capitalChar = KEY_F3;
    break;
  case SDLK_F4:
    priChar = shiftChar = capitalChar = KEY_F4;
    break;
  case SDLK_F5:
    priChar = shiftChar = capitalChar = KEY_F5;
    break;
  case SDLK_F6:
    priChar = shiftChar = capitalChar = KEY_F6;
    break;
  case SDLK_F7:
    priChar = shiftChar = capitalChar = KEY_F7;
    break;
  case SDLK_F8:
    priChar = shiftChar = capitalChar = KEY_F8;
    break;
  case SDLK_F9:
    priChar = shiftChar = capitalChar = KEY_F9;
    break;
  case SDLK_F10:
    priChar = shiftChar = capitalChar = KEY_F10;
    break;
  case SDLK_F11:
    priChar = shiftChar = capitalChar = KEY_F11;
    break;
  case SDLK_F12:
    priChar = shiftChar = capitalChar = KEY_F12;
    break;

  // arrow keys
  case SDLK_HOME:
    priChar = shiftChar = capitalChar = KEY_HOME;
    break;
  case SDLK_UP:
    priChar = shiftChar = capitalChar = KEY_UP;
    break;
  case SDLK_PAGEUP:
    priChar = shiftChar = capitalChar = KEY_PGUP;
    break;
  case SDLK_LEFT:
    priChar = shiftChar = capitalChar = KEY_LEFT;
    break;
  case SDLK_RIGHT:
    priChar = shiftChar = capitalChar = KEY_RIGHT;
    break;
  case SDLK_END:
    priChar = shiftChar = capitalChar = KEY_END;
    break;
  case SDLK_DOWN:
    priChar = shiftChar = capitalChar = KEY_DOWN;
    break;
  case SDLK_PAGEDOWN:
    priChar = shiftChar = capitalChar = KEY_PGDN;
    break;
  case SDLK_INSERT:
    priChar = shiftChar = capitalChar = KEY_INS;
    break;
  case SDLK_DELETE:
    priChar = shiftChar = capitalChar = KEY_DEL;
    break;
  ...
  case SDLK_UNDERSCORE:
    priChar = shiftChar = capitalChar = '_';
    break;
  default:
    MSG("unhandled key %x\n", scanCode);
  }
  ...
  int retFlag2 = (charValue == 0) || outChar == charValue ||
                 ...
                 ((flags & K_CASE_INSENSITIVE) &&
                  outChar == (unsigned short)tolower(charValue));
...
// ------ Begin of MouseSDL::is_key -------//
int MouseSDL::is_key(unsigned scanCode, unsigned short skeyState, char *keyStr,
                     unsigned flags) {
  ...
  switch (scanCode) {
  case SDLK_F1:
    numLockChar = priChar = "F1";
    break;
  case SDLK_F2:
    numLockChar = priChar = "F2";
    break;
  case SDLK_F3:
    numLockChar = priChar = "F3";
    break;
  case SDLK_F4:
    numLockChar = priChar = "F4";
    break;
  case SDLK_F5:
    numLockChar = priChar = "F5";
    break;
  case SDLK_F6:
    numLockChar = priChar = "F6";
    break;
  case SDLK_F7:
    numLockChar = priChar = "F7";
    break;
  case SDLK_F8:
    numLockChar = priChar = "F8";
    break;
  case SDLK_F9:
    numLockChar = priChar = "F9";
    break;
  case SDLK_F10:
    numLockChar = priChar = "F10";
    break;
  case SDLK_F11:
    numLockChar = priChar = "F11";
    break;
  case SDLK_F12:
    numLockChar = priChar = "F12";
    break;
  ...
  case SDLK_HOME:
    priChar = numLockChar = "HOME";
    break;
  case SDLK_UP:
    priChar = numLockChar = "UP";
    break;
  case SDLK_PAGEUP:
    priChar = numLockChar = "PAGE UP";
    break;
  case SDLK_LEFT:
    priChar = numLockChar = "LEFT";
    break;
  case SDLK_RIGHT:
    priChar = numLockChar = "RIGHT";
    break;
  case SDLK_END:
    priChar = numLockChar = "END";
    break;
  case SDLK_DOWN:
    priChar = numLockChar = "DOWN";
    break;
  case SDLK_PAGEDOWN:
    priChar = numLockChar = "PAGE DOWN";
    break;
  case SDLK_INSERT:
    priChar = numLockChar = "INSERT";
    break;
  case SDLK_DELETE:
    priChar = numLockChar = "DELETE";
    break;

  // 104-key only
  case SDLK_LGUI:
    priChar = numLockChar = "LEFT WINDOW";
    break;
  case SDLK_RGUI:
    priChar = numLockChar = "RIGHT WINDOW";
    break;
  case SDLK_MENU:
    priChar = numLockChar = "APP MENU";
    break;
  }
