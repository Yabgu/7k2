...
void SlideBar::init_scroll(short x1, short y1, short x2, short y2, int viewSize,
                           SlideBarFP dispFunc) {
  scrn_x1 = x1;
  scrn_y1 = y1;
  scrn_x2 = x2;
...
// set view_size to 0
void SlideBar::init_slide(short x1, short y1, short x2, short y2,
                          short barWidth, SlideBarFP dispFunc) {
  scrn_x1 = x1;
  scrn_y1 = y1;
  scrn_x2 = x2;
  scrn_y2 = y2;
...
int SlideBar::is_empty() { return max_recno - min_recno + scroll_type <= 0; }
...
short SlideBar::rect_left() {
  ...
  if (is_empty())
    ...
    return bound(scrn_x1 + (view_recno - min_recno) *
                               (scrn_x2 - scrn_x1 + 1 - scrn_bar_width) /
                               (max_recno - min_recno + scroll_type),
                 scrn_x1, scrn_x2);
}

short SlideBar::rect_right() {
  // slide bar style need not call rect_right(), it should be rect_left() +
  // scrn_bar_width - 1 as view_size = 0
  err_when(view_recno < min_recno);
  // err_when( view_recno > max_view_recno() );
  if (is_empty())
    return scrn_x2; // empty
  else
    return bound(scrn_x1 + scrn_bar_width - 1 +
                     (view_recno + view_size - min_recno) *
                         (scrn_x2 - scrn_x1 + 1 - scrn_bar_width) /
                         (max_recno - min_recno + scroll_type),
                 scrn_x1, scrn_x2);
}

// reverse function, pass the proposed rect_left to return the corresponding
// view_recno
...
void SlideVBar::init_scroll(short x1, short y1, short x2, short y2,
                            int viewSize, SlideVBarFP dispFunc) {
  scrn_x1 = x1;
  scrn_y1 = y1;
  scrn_x2 = x2;
  scrn_y2 = y2;
...
// set view_size to 0
void SlideVBar::init_slide(short x1, short y1, short x2, short y2,
                           short barHeight, SlideVBarFP dispFunc) {
  scrn_x1 = x1;
  scrn_y1 = y1;
  scrn_x2 = x2;
  scrn_y2 = y2;
...
int SlideVBar::is_empty() { return max_recno - min_recno + scroll_type <= 0; }
...
short SlideVBar::rect_top() {
  ...
  if (is_empty())
    ...
    return bound(scrn_y1 + (view_recno - min_recno) *
                               (scrn_y2 - scrn_y1 + 1 - scrn_bar_height) /
                               (max_recno - min_recno + scroll_type),
                 scrn_y1, scrn_y2);
}

short SlideVBar::rect_bottom() {
  // slide bar style need not call rect_bottom(), it should be rect_top() +
  // scrn_bar_height - 1 as view_size = 0
  err_when(view_recno < min_recno);
  // err_when( view_recno > max_view_recno() );
  if (is_empty())
    return scrn_y2; // empty
  else
    return bound(scrn_y1 + scrn_bar_height - 1 +
                     (view_recno + view_size - min_recno) *
                         (scrn_y2 - scrn_y1 + 1 - scrn_bar_height) /
                         (max_recno - min_recno + scroll_type),
                 scrn_y1, scrn_y2);
...
// reverse function, pass the proposed rect_top to return the corresponding
// view_recno
