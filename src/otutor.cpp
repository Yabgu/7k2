...
Tutor::Tutor() { memset(this, 0, sizeof(Tutor)); }
...
void Tutor::init() {
  ...
  text_block_array = (TutorTextBlock *)mem_add(
      sizeof(TutorTextBlock) *
      (MAX_TUTOR_TEXT_BLOCK + MAX_TUTOR_OBJECTIVE + MAX_TUTOR_OBJECTIVE));
  ...
  /*	//----- open tutorial introduction resource file -------//
          ...
          res_tutor_intro.init(str, 0);  	// 0-don't read all into
     buffer*/
...
void Tutor::load(int tutorId) {
  ...
  while (!fileTxt.is_eof()) {
    ...
    if (strcmpi(tokenStr, "Button") == 0) {
      ...
      strncpy(tutorTextBlock->button_code, tokenStr,
              tutorTextBlock->BUTTON_CODE_LEN);
    ...
    if (strcmpi(tokenStr, "Title") == 0) {
      ...
      while (1) {
        ...
        if (strcmpi(tokenStr, "**") == 0)
          break;
        else if (strcmpi(tokenStr, "*") == 0) {
          tokenStr = fileTxt.get_token(1);
          tutorTextBlock->objective_no =
              (tokenStr[0] - '0') * 10 + tokenStr[1] - '0';
          break;
        }
      ...
      //	tutorTextBlock->title_code[tutorTextBlock->TITLE_CODE_LEN] =
      //NULL;
    } else {
      tutorTextBlock->title_code[0] = '\0';
    }
    ...
    readLen =
        fileTxt.read_paragraph(textPtr, tutor_text_buf_size - totalReadLen);
  ...
  //------ Open the file and allocate buffer -----//
  FileTxt fileTxt2(
      filePtr2, dataSize2); // initialize fileTxt with an existing file stream
  if (dataSize2 > tutor_intro_buf_size) {
    tutor_intro_buf = mem_resize(
        tutor_intro_buf, dataSize2); // allocate a buffer larger than we need
                                     // for the largest size possible
  ...
  while (!fileTxt2.is_eof()) {
    ...
    readLen =
        fileTxt2.read_paragraph(textPtr, tutor_intro_buf_size - totalReadLen);
  ...
  //------ Open the file and allocate buffer -----//
  FileTxt fileTxt3(
      filePtr3, dataSize3); // initialize fileTxt with an existing file stream
  if (dataSize3 > tutor_result_buf_size) {
    tutor_result_buf = mem_resize(
        tutor_result_buf, dataSize3); // allocate a buffer larger than we need
                                      // for the largest size possible
  ...
  while (!fileTxt3.is_eof()) {
    ...
    readLen =
        fileTxt3.read_paragraph(textPtr, tutor_result_buf_size - totalReadLen);
...
char *Tutor::get_intro(int tutorId) {
  /*	//------- get the tutor msgs from the resource file -------//
          ...
          int   dataSize;
          File* filePtr = res_tutor_intro.get_file( tutor[tutorId]->code,
     dataSize);
          ...
          FileTxt fileTxt( filePtr, dataSize );  // initialize fileTxt with an
     existing file stream

          if( dataSize > tutor_intro_buf_size )
          {
                  tutor_intro_buf      = mem_resize( tutor_intro_buf, dataSize
     );       // allocate a buffer larger than we need for the largest size
     possible tutor_intro_buf_size = dataSize;
          }
...
//
// <int> inGameCall - whether it's called from the main menu or from in-game
// tutorial.
//
...
void Tutor::run(int tutorId, int inGameCall) {
  ...
  info.goal_deadline =
      date.julian(date.year(info.game_date) + config.goal_year_limit,
                  date.month(info.game_date), date.day(info.game_date));
...
void Tutor::play_speech(int tutorId, int objectiveRecno) {
  ...
  // ##### begin Gilbert 25/9 ######//
  cur_speech_wav_id = audio.play_long_wav(
...
int Tutor::detect() {
  /*	if( button_new_tutor.detect() )
          {
          stop_speech();
                  select_run_tutor(1);		// select and run tutorial,
     1-called from in-game, not from the main menu return 1;
          }
  ...
  bypass_allow_mouse_click = 1;
  bypass_allow_button_click = 1;
  if (close_box_pressed == 0 && cur_text_block_id < text_block_count &&
      button_next.detect()) {
    bypass_allow_mouse_click = 0;
    bypass_allow_button_click = 0;
    next_text_block();
  ...
  if (close_box == 1) {
    ...
    bypass_allow_mouse_click = 1;
    bypass_allow_button_click = 1;
    if (close_box_pressed == 0 && i == text_block_count &&
        button_close.detect()) {
      bypass_allow_mouse_click = 0;
      bypass_allow_button_click = 0;
      close_box_pressed = 1;
  ...
  if (mouse.any_click(minize_detect_area_x1, minize_detect_area_y1,
                      minize_detect_area_x2, minize_detect_area_y2,
                      LEFT_BUTTON)) {
...
void Tutor::disp_msg(char dispFlag, char darkness, char *gameMsg, ...) {
  ...
  if (!dispFlag) {
    ...
    vga_back.bar_alpha(TUTOR_X1, TUTOR_Y1, TUTOR_X2, TUTOR_Y1 + 10 + textHeight,
                       darkness, V_BLACK);
    ...
    {
      level = 0;
      for (j = TUTOR_Y1 + 10 + textHeight + barWidth;
           j > TUTOR_Y1 + 10 + textHeight; j--, level++) {
        rand_seed();
        if ((seed % barWidth) < level) {
          vga_back.bar_alpha(i, j, i, j, darkness, V_BLACK);
        }
    ...
    //	vga_util.d3_panel_down( TUTOR_X1, TUTOR_Y1, TUTOR_X2,
    //TUTOR_Y1+10+textHeight );
    font_mid.center_put_paragraph(TUTOR_X1 + 10, TUTOR_Y1 + 5, TUTOR_X2 - 10,
                                  TUTOR_Y1 + 5 + textHeight, strBuf, 4, 0, 0);
  }
...
void Tutor::disp_guideline() {
  ...
  if (!text_disable_flag) {
    ...
    vga_back.bar_alpha(TUTOR_BAR_X1, TUTOR_BAR_Y1, TUTOR_BAR_X2, TUTOR_BAR_Y2,
                       1, V_BLACK);
    ...
    if (tutorTextBlock->button_code[0]) {
      button_sample.paint(TUTOR_BUTTON_X1, TUTOR_BUTTON_Y1, 'A',
                          tutorTextBlock->button_code);
      textX2 = TUTOR_BUTTON_X1 - 16;
    }

    //-------- display title text --------//
    if (tutorTextBlock->title_code[0]) {
      font_tut.center_put_paragraph(TUTOR_BAR_X1 - 10, TUTOR_BAR_Y1 + 2,
                                    TUTOR_BAR_X2 - 20, TUTOR_BAR_Y2 + 2,
  ...
  font_bld.text_width(text_tutorial.str_guidelines(), -1,
                      TUTOR_X2 - 20 - TUTOR_X1);
  ...
  font_bld.center_put_paragraph(TUTOR_X1 + 10, y1, TUTOR_X2 - 10,
                                y1 + textHeight, text_tutorial.str_guidelines(),
                                4, 0, 0);
  y1 += textHeight + 5;
  ...
  //	font_tut.center_put_paragraph( TUTOR_X1+10, TUTOR_Y1+10, TUTOR_X2-10,
  //TUTOR_Y2-10, 									tutorTextBlock->text_ptr, 4, 0 ,0 );
  ...
  font_tut.put(
      TUTOR_BAR_X2 - 50, TUTOR_BAR_Y1 + 8,
...
void Tutor::disp_hints() {
  ...
  TutorTextBlock *tutorTextBlock2 =
      text_block_array + text_block_count + current_objective - 1;
  ...
  for (i = 0; i < MAX_IN_GAME_BOX_PER_OBJECTIVE; i++) {
    ...
    if (tutor_box[i].base_object_recno_no &&
      ...
      if (base_obj->cast_to_Unit()) {
        tutor_box[i].screen_loc_centre_x = world.zoom_matrix->calc_zoom_x(
            ((Unit *)base_obj)->cur_x * LOCATE_WIDTH,
            ((Unit *)base_obj)->cur_y * LOCATE_HEIGHT, 0);
        tutor_box[i].screen_loc_centre_y = world.zoom_matrix->calc_zoom_y(
            ((Unit *)base_obj)->cur_x * LOCATE_WIDTH,
            ((Unit *)base_obj)->cur_y * LOCATE_HEIGHT, 0);
      } else if (base_obj->cast_to_Firm()) {
        tutor_box[i].screen_loc_centre_x = world.zoom_matrix->calc_zoom_x(
            ((Firm *)base_obj)->center_x * LOCATE_WIDTH,
        ...
        tutor_box[i].screen_loc_centre_x = world.zoom_matrix->calc_zoom_x(
            ((Town *)base_obj)->center_x * LOCATE_WIDTH,
      ...
      x = tutor_box[i].screen_loc_centre_x - (boxWidth >> 1) +
          tutor_box[i].pointer_offset_x;
  ...
  if (tutorTextBlock->title_code[0]) {
    ...
    if (strcmp(tutorTextBlock2->intro_ptr, "") != 0) {
      font_bld.text_width(tutorTextBlock->title_code, -1,
                          TUTOR_X2 - 20 - TUTOR_X1);
      textHeight = font_bld.text_height() + 20;
      ...
      font_bld.center_put_paragraph(TUTOR_X1 + 10, y1, TUTOR_X2 - 10,
                                    y1 + textHeight, tutorTextBlock->title_code,
                                    4);
    } else {
      font_mid.text_width(tutorTextBlock->title_code, -1,
                          TUTOR_X2 - 20 - TUTOR_X1);
      ...
      int darkness = 1;
      seed = 35;
      vga_back.bar_alpha(TUTOR_X1, y1, TUTOR_X2, y1 + textHeight + 20, darkness,
                         V_BLACK);
      ...
      font_mid.center_put_paragraph(TUTOR_X1 + 10, y1, TUTOR_X2 - 10,
                                    y1 + textHeight, tutorTextBlock->title_code,
                                    4);
    }
    ...
    //		str += totalTextBlock;
    font_bld.center_put(
        TUTOR_X1 + 10, y1 + textHeight, TUTOR_X2 - 10, y1 + textHeight + 20,
  ...
  if (strcmp(tutorTextBlock2->intro_ptr, "") != 0) {
    ...
    font_bld.center_put_paragraph(TUTOR_X1 + 10, y1 + 5, TUTOR_X2 - 10,
                                  y1 + 5 + textHeight, tutorTextBlock->text_ptr,
                                  4, 0, 0);
  } else {
    ...
    int darkness = 1;
    seed = 35;
    vga_back.bar_alpha(TUTOR_X1, y1, TUTOR_X2, y1 + 10 + textHeight, darkness,
                       V_BLACK);
    ...
    {
      unsigned level = 0;
      for (int j = y1 + 10 + textHeight + barWidth; j > y1 + 10 + textHeight;
           j--, level++) {
        rand_seed();
        if ((seed % barWidth) < level) {
          vga_back.bar_alpha(i, j, i, j, darkness, V_BLACK);
    ...
    font_mid.center_put_paragraph(TUTOR_X1 + 10, y1 + 5, TUTOR_X2 - 10,
                                  y1 + 5 + textHeight, tutorTextBlock->text_ptr,
                                  4, 0, 0);
  ...
  if (tutorTextBlock->button_code[0]) {
    button_sample.paint(TUTOR_BUTTON_X1, y1 + 10 + textHeight, 'A',
                        tutorTextBlock->button_code);
    textHeight += 50;
  }
  ...
  //	button_continue.create_text(TUTOR_X1, y1+45, TUTOR_X2, y1+75,
  //text_game_menu.str_continue() ); // "Continue" );

  button_close.set_font(&font_bld);
  if (continue_box)
    button_close.create_text(
        TUTOR_X1, y1 + 80, TUTOR_X2, y1 + 110,
        text_tutorial.str_close_hints()); // "Close Hints" );
  else
    button_close.create_text(
        TUTOR_X1, y1 + 45, TUTOR_X2, y1 + 75,
        text_tutorial.str_close_hints()); //"Close Hints" );
