...
void Tutor::init()
{
    ...
    text_block_array = (TutorTextBlock *)mem_add(sizeof(TutorTextBlock) *
...
void Tutor::load(int tutorId)
{
    ...
    {
        tutor_intro_buf = mem_resize(tutor_intro_buf, dataSize2); // allocate a buffer larger than we need
    ...
    {
        tutor_result_buf = mem_resize(tutor_result_buf, dataSize3); // allocate a buffer larger than we need
                                                                    // for the largest size possible
...
void Tutor::run(int tutorId, int inGameCall)
{
    ...
    info.goal_deadline = date.julian(date.year(info.game_date) + config.goal_year_limit, date.month(info.game_date),
...
void Tutor::play_speech(int tutorId, int objectiveRecno)
{
    ...
    // ##### begin Gilbert 25/9 ######//
    cur_speech_wav_id = audio.play_long_wav(str, DEF_REL_VOLUME); // cur_speech_wav_id is the WAV id that is needed
...
int Tutor::detect()
{
    ...
    if (mouse.any_click(minize_detect_area_x1, minize_detect_area_y1, minize_detect_area_x2, minize_detect_area_y2,
                        LEFT_BUTTON))
    {
...
void Tutor::disp_msg(char dispFlag, char darkness, char *gameMsg, ...)
{
    ...
    else
    {
        ...
        font_mid.center_put_paragraph(TUTOR_X1 + 10, TUTOR_Y1 + 5, TUTOR_X2 - 10, TUTOR_Y1 + 5 + textHeight, strBuf, 4,
                                      0, 0);
    }
...
void Tutor::disp_guideline()
{
    ...
    font_bld.center_put_paragraph(TUTOR_X1 + 10, y1, TUTOR_X2 - 10, y1 + textHeight, text_tutorial.str_guidelines(), 4,
                                  0, 0);
    y1 += textHeight + 5;
    ...
    font_tut.put_paragraph(TUTOR_X1 + 10, y1, TUTOR_X2 - 10, TUTOR_Y2 - 10, tutorTextBlock->text_ptr, 4);

    //	font_tut.center_put_paragraph( TUTOR_X1+10, TUTOR_Y1+10, TUTOR_X2-10,
    // TUTOR_Y2-10, 									tutorTextBlock->text_ptr, 4,
    // 0 ,0
    // );
...
void Tutor::disp_hints()
{
    ...
    for (i = 0; i < MAX_IN_GAME_BOX_PER_OBJECTIVE; i++)
    {
        ...
        int x, y;
        if (!tutor_box[i].base_object_recno_no && tutor_box[i].screen_loc_centre_x == -1 &&
            tutor_box[i].screen_loc_centre_y == -1)
            continue;

        if (tutor_box[i].base_object_recno_no && base_obj_array.is_deleted(tutor_box[i].base_object_recno_no))
            continue;
        ...
        if (tutor_box[i].base_object_recno_no && !base_obj_array.is_deleted(tutor_box[i].base_object_recno_no))
        {
            ...
            if (base_obj->cast_to_Unit())
            {
                tutor_box[i].screen_loc_centre_x = world.zoom_matrix->calc_zoom_x(
                    ((Unit *)base_obj)->cur_x * LOCATE_WIDTH, ((Unit *)base_obj)->cur_y * LOCATE_HEIGHT, 0);
            ...
            {
                tutor_box[i].screen_loc_centre_x = world.zoom_matrix->calc_zoom_x(
                    ((Firm *)base_obj)->center_x * LOCATE_WIDTH, ((Firm *)base_obj)->center_y * LOCATE_HEIGHT,
            ...
            {
                tutor_box[i].screen_loc_centre_x = world.zoom_matrix->calc_zoom_x(
                    ((Town *)base_obj)->center_x * LOCATE_WIDTH, ((Town *)base_obj)->center_y * LOCATE_HEIGHT,
            ...
            vga_back.line(x + (boxWidth >> 1), y - (textHeight >> 1), tutor_box[i].screen_loc_centre_x,
        ...
        font_bld.center_put_paragraph(x, y - textHeight, x + boxWidth - 1, y, tutor_box[i].text, 4, 0, 0);
    }
    ...
    if (tutorTextBlock->title_code[0])
    {
        ...
        if (strcmp(tutorTextBlock2->intro_ptr, "") != 0)
        {
            ...
            font_bld.center_put_paragraph(TUTOR_X1 + 10, y1, TUTOR_X2 - 10, y1 + textHeight, tutorTextBlock->title_code,
                                          4);
        }
        else
        {
            ...
            font_mid.center_put_paragraph(TUTOR_X1 + 10, y1, TUTOR_X2 - 10, y1 + textHeight, tutorTextBlock->title_code,
                                          4);
        }
    ...
    if (continue_box)
        button_close.create_text(TUTOR_X1, y1 + 80, TUTOR_X2, y1 + 110,
                                 text_tutorial.str_close_hints()); // "Close Hints" );
    else
        button_close.create_text(TUTOR_X1, y1 + 45, TUTOR_X2, y1 + 75,
                                 text_tutorial.str_close_hints()); //"Close Hints" );
