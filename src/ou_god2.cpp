...
void UnitGod::think_roman_god() {
  ...
  for (int i = ownNation->ai_camp_count - 1; i >= 0; i--) {
    ...
    if (firmCamp->overseer_recno &&
        ...
        // &&	unitPtr->race_id == RACE_ROMAN			// only consider
        // ROMAN leader
        && unitPtr->is_human()
      ...
      if (unitPtr->skill_level() >= 40)
        curRating += 5000 - (unitPtr->skill_level() - 40) *
                                60; // strong leader need not be enhanced
      else
        curRating +=
            5000 - (40 - unitPtr->skill_level()) * 80; // don't add weak leader
      ...
      for (int j = firmCamp->soldier_count - 1; j >= 0; j--) {
        ...
        if (soldierPtr->race_id == RACE_ROMAN)
          curRating +=
              (unitPtr->combat_level() - soldierPtr->combat_level()) * 2;
        else
...
void UnitGod::think_greek_god() {
  ...
  for (int i = generalCount; i >= 0; --i) {
    ...
    int dist = misc.points_distance(next_x_loc(), next_y_loc(),
                                    generalPtr->next_x_loc(),
                                    generalPtr->next_y_loc());
    weight -= dist / 8;
  ...
  if (bestWeight >= 20 && bestWeight + ai_para >= 40) {
    ...
    // else go to there
    if (misc.points_distance(next_x_loc(), next_y_loc(), targetXLoc,
                             targetYLoc) <= 20) {
      if (!isTargetLoc) {
        // go the unit
        ai_para = 0;
        go_cast_power(targetXLoc, targetYLoc,
                      COMMAND_AI); // 1 - cast power type
      } else {
...
void UnitGod::think_carth_god() {
  ...
  for (int i = 20; i > 0; --i) {
    ...
    int dist = misc.points_distance(next_x_loc(), next_y_loc(), (x1 + x2) / 2,
                                    (y1 + y2) / 2);
    weight -= dist / 8;
  ...
  if (bestWeight >= 10 && bestWeight + ai_para >= 20 &&
      bestWeight + ai_para >= misc.random(50)) {
    ai_para = 0;
    go_cast_power(targetXLoc, targetYLoc, COMMAND_AI); // 1 - cast power type
  } else {
    ai_para++;
  ...
  for (int i = base_obj_array.size(); i > 0; --i) {
    ...
    if (baseObj->cast_to_Town()) {
      ...
      if (townPtr->under_construction ||
          info.game_date - townPtr->setup_date <
              365) // don't care building built less than one year
        continue;
      ...
      if (firmPtr->under_construction ||
          info.game_date - firmPtr->setup_date <
              365) // don't care building built less than one year
        continue;
...
void UnitGod::think_mongol_god() {
  ...
  for (y = y1; y <= y2; ++y) {
    ...
    for (x = x1; x <= x2; ++x, ++locPtr) {
      ...
      if (unitRecno && !unit_array.is_deleted(unitRecno) &&
          (unitPtr = unit_array[unitRecno]) && unitPtr->nation_recno != 0 &&
          !unitPtr->is_nation(
              nation_recno) // instead of unitPtr->nation_recno != nation_recno
          && (unitPtr->cur_action == SPRITE_IDLE ||
              unitPtr->cur_action == SPRITE_ATTACK) &&
          unitPtr->rank_id != RANK_KING && unitPtr->race_id) {
  ...
  if (bestWeight >= CITIZEN_COMBAT_LEVEL * 3 && bestWeight + ai_para >= 100 &&
      bestWeight + ai_para > misc.random(300)) {
  ...
  for (unitRecno = unit_array.size(); unitRecno > 0; --unitRecno) {
    ...
    if (!unit_array.is_deleted(unitRecno) &&
        (unitPtr = unit_array[unitRecno]) && unitPtr->is_visible() &&
        unitPtr->mobile_type == UNIT_LAND && unitPtr->nation_recno != 0 &&
        !unitPtr->is_nation(
            nation_recno) // instead of unitPtr->nation_recno != nation_recno
        && unitPtr->cur_action == SPRITE_IDLE &&
        unitPtr->rank_id != RANK_KING && unitPtr->race_id) {
  ...
  if (bestWeight >= CITIZEN_COMBAT_LEVEL * 3 && bestWeight + ai_para >= 80 &&
      bestWeight + ai_para > misc.random(150)) {
    move(targetXLoc, targetYLoc);
...
void UnitGod::think_egyptian_god() {
  ...
  for (int i = town_array.size(); i > 0; i--) {
    ...
    if (townPtr->population >= MAX_TOWN_GROWTH_POPULATION / 2)
      curRating -= (townPtr->population - MAX_TOWN_GROWTH_POPULATION / 2) *
                   300 / MAX_TOWN_GROWTH_POPULATION;
    else
      curRating -= (MAX_TOWN_GROWTH_POPULATION / 2 - townPtr->population) *
                   300 / MAX_TOWN_GROWTH_POPULATION;
  ...
  for (int i = firmArraySize; i > 0; --i, ++firmRecno) {
    ...
    if (firmPtr->nation_recno != nation_recno &&
        (firmPtr->nation_recno == 0 ||
         nation_ptr()->get_relation_status(firmPtr->nation_recno) <=
             RELATION_NEUTRAL)) {
...
void UnitGod::think_indian_god() {
  ...
  if (think_god_attack_target(targetXLoc, targetYLoc)) {
    go_cast_power(targetXLoc + 1, targetYLoc + 1,
                  COMMAND_AI); // 2 - cast power type
  }
...
--------//
                ...
                if( firmPtr->nation_recno == nation_recno ||
!firmPtr->nation_recno ) continue;

                //-------- only attack enemies ----------//

                if( ownNation->get_relation(firmPtr->nation_recno)->status !=
        ...
        if (targetObj->cast_to_Unit()) {
          ...
          if ((teamInfo = targetUnit->team_info) ||
              !unit_array.is_deleted(targetUnit->leader_unit_recno) &&
                  (teamInfo =
                       unit_array[targetUnit->leader_unit_recno]->team_info)) {
          ...
          weight += (int)targetObj->hit_points // wall strength
                    + (targetObj->cast_to_Town()->jobless_population +
                       +targetObj->cast_to_Town()->town_defender_count) *
                          2; // defense strength
        }
...
void UnitGod::think_viking_god()
{
        ...
        if( think_god_attack_target(targetXLoc, targetYLoc) )
        {
                go_cast_power(targetXLoc+1, targetYLoc+1, 2, COMMAND_AI);
// 2 - cast power type
        }
}
//------- End of function UnitGod::think_viking_god --------//
...
void UnitGod::think_persian_god()
{
        ...
        for( int i=ownNation->ai_camp_count-1 ; i>=0 ; i-- )
        {
                ...
                curRating = 100 * (totalMaxHitPoints-totalHitPoints) /
totalMaxHitPoints;

                //---- if the king is the commander of this camp -----//

                if( firmPtr->overseer_recno &&
                         unit_array[firmPtr->overseer_recno]->rank_id ==
RANK_KING )
                {
                        curRating += 20;
                }
        ...
        if( bestRating )
        {
                go_cast_power(targetXLoc, targetYLoc, 1, COMMAND_AI);
// 1 - cast power type
        }
}
//------- End of function UnitGod::think_persian_god --------//
...
void UnitGod::think_japanese_god()
{
        ...
        if( misc.random(2)==0 )
        {
                for( int i=firm_array.size() ; i>0 ; i-- )
                {
                        ...
                        if(
ownNation->get_relation(firmPtr->nation_recno)->status > RELATION_TENSE )
                                continue;

                        //------ calculate the rating of the firm -------//

                        curRating =
((FirmCamp*)firmPtr)->total_combat_level()/10;
...
ownNation->get_relation(townPtr->nation_recno)->status > RELATION_TENSE )
                                continue;

                        //------ calculate the rating of the firm -------//

                        curRating = townPtr->population +
(100-townPtr->average_loyalty());
...
void UnitGod::think_maya_god()
{
        ...
        for( int i=ownNation->ai_camp_count-1 ; i>=0 ; i-- )
        {
                ...
                if( firmPtr->overseer_recno )
                {
                        ...
                        if( unitPtr->race_id == RACE_MAYA &&
unitPtr->skill.combat_level < 100 ) curRating += 10;
                }
                ...
                {
                        if( workerPtr->race_id == RACE_MAYA &&
workerPtr->combat_level < 100 ) curRating += 5;
                }
...
void UnitGod::think_egyptian_god()
{
        ...
        for( int i=town_array.size() ; i>0 ; i-- )
        {
                ...
                if( townPtr->population >= MAX_TOWN_GROWTH_POPULATION/2 )
                        curRating -= (townPtr->population -
MAX_TOWN_GROWTH_POPULATION/2) * 300 / MAX_TOWN_GROWTH_POPULATION; else curRating
-= (MAX_TOWN_GROWTH_POPULATION/2 - townPtr->population) * 300 /
MAX_TOWN_GROWTH_POPULATION;
...
// don't affect indepedent unit
                                && unitPtr->nation_recno != nation_recno
                                && (unitPtr->loyalty >= 20 && unitPtr->loyalty
<= 60 || unitPtr->loyalty <= 80 && unitPtr->target_loyalty < 30) )
                        {
                                switch(
ownNation->get_relation(unitPtr->nation_recno)->status )
                                {
                                case RELATION_HOSTILE:
                                        curRating += 3;
                                        break;
...
unchange break;
                                ...
                                case RELATION_FRIENDLY:
                                        curRating -= 1;
// actually friendly humans are not affected break;

                                case RELATION_ALLIANCE:
                                        curRating -= 1;
// actually allied humans are not affected break;
        ...
        if( curRating > 1 )
        {
                ...
                go_cast_power(next_x_loc(), next_y_loc(), 1, COMMAND_AI);
// 1 - cast power type
        }
        else
        {
                ...
                for( int unitRecno = unit_array.size(); unitRecno > 0;
--unitRecno )
                {
                        Unit *unitPtr;
                        if( !unit_array.is_deleted(unitRecno)
...
ownNation->get_relation(unitPtr->nation_recno)->status == RELATION_HOSTILE )
                        {
                                int cost = misc.points_distance(next_x_loc(),
next_y_loc(), unitPtr->next_x_loc(), unitPtr->next_y_loc()); if( cost <
bestUnitCost )
                                {
                                        bestUnitCost = cost;
...
xLoc, yLoc) <= god_res[god_id]->cast_power_range ) go_cast_power(xLoc, yLoc, 1,
COMMAND_AI);		// 1 - cast power type else move( xLoc, yLoc );
                }
...
void UnitGod::think_zulu_god()
{
        ...
        for( int i=ownNation->ai_camp_count-1 ; i>=0 ; i-- )
        {
                ...
                if( firmPtr->overseer_recno
                        ...
                        &&	unitPtr->race_id == RACE_ZULU
// only consider ZULU leader
                        && unitPtr->skill.skill_level <= 70)
                {
                        if( unitPtr->rank_id == RANK_KING )
                                curRating += 5000;		// weak king
need leadership very much

                        if( unitPtr->skill.skill_level >= 40 )
                                curRating += 5000 - (unitPtr->skill.skill_level
- 40) * 60;	// strong leader need not be enhanced else curRating += 5000 -
(40 - unitPtr->skill.skill_level) * 80;		// don't add weak leader
...
workerPtr++ )
                        {
                                if( workerPtr->race_id == RACE_ZULU )
                                        curRating +=
(unitPtr->skill.combat_level - workerPtr->combat_level)*2; else curRating +=
unitPtr->skill.combat_level - workerPtr->combat_level;
                        }
