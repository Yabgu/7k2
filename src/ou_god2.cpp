...
void UnitGod::think_norman_god()
{
    ...
    else if ((int)hit_points / 2 < max_hit_points() && nation_ptr()->peaceful_days() > 100 &&
...
void UnitGod::think_greek_god()
{
...

    {
        ...
        for (int y = y1; y <= y2; ++y)
        {
            ...
            for (int x = x1; x <= x2; ++x, ++locPtr)
            {
                int unitRecno;
                Unit *unitPtr;
                if ((unitRecno = locPtr->unit_recno(UNIT_LAND)) && (unitPtr = unit_array[unitRecno]) &&
                    unitPtr->nation_recno == nation_recno && unitPtr->cur_order.mode == UNIT_ATTACK &&
    ...
    for (int i = generalCount; i >= 0; --i)
    {
        ...
        // distance
        int dist = misc.points_distance(next_x_loc(), next_y_loc(), generalPtr->next_x_loc(), generalPtr->next_y_loc());
        weight -= dist / 8;
    ...
    if (ai_para > 10)
    {
        ...
        if (bestWeight < 10 && (int)hit_points / 2 < max_hit_points() && nation_ptr()->peaceful_days() > 100 &&
            misc.random(max_hit_points()) > 2 * (int)hit_points)
        {
            resign(COMMAND_AI);
            return;
    ...
    for (int i = base_obj_array.size(); i > 0; --i)
    {
        ...
        if (baseObj->cast_to_Town())
        {
            ...
            if (townPtr->under_construction ||
                info.game_date - townPtr->setup_date < 365) // don't care building built less than one year
                continue;
        ...
        else if (baseObj->cast_to_Firm())
        {
            ...
            if (firmPtr->under_construction ||
                info.game_date - firmPtr->setup_date < 365) // don't care building built less than one year
                continue;
...
void UnitGod::think_mongol_god()
{
    ...
    for (y = y1; y <= y2; ++y)
    {
        ...
        for (x = x1; x <= x2; ++x, ++locPtr)
        {
            ...
            Unit *unitPtr;
            if (unitRecno && !unit_array.is_deleted(unitRecno) && (unitPtr = unit_array[unitRecno]) &&
            ...
            {
                int weight = int(unitPtr->combat_level() + unitPtr->hit_points + 3 * unitPtr->skill_level());
                if (bestWeight < weight)
                {
                    bestWeight = weight;
    ...
    {
        Unit *unitPtr;
        if (!unit_array.is_deleted(unitRecno) && (unitPtr = unit_array[unitRecno]) && unitPtr->is_visible() &&
            unitPtr->mobile_type == UNIT_LAND && unitPtr->nation_recno != 0 &&
            !unitPtr->is_nation(nation_recno) // instead of unitPtr->nation_recno != nation_recno
            && unitPtr->cur_action == SPRITE_IDLE && unitPtr->rank_id != RANK_KING && unitPtr->race_id)
        {
            int weight = int(unitPtr->combat_level() + unitPtr->hit_points + 3 * unitPtr->skill_level());
...
int UnitGod::think_god_attack_target(int &targetXLoc, int &targetYLoc)
{
    ...
    for (int i = unit_array.size(); i > 0; --i, ++unitRecno)
    {
        ...
        if (unitPtr->cur_order.mode == UNIT_ATTACK && (targetObjRecno = unitPtr->cur_order.para) &&
