...
void Firm::being_killed(BaseObj *attackerObj) {
  ...
  if (attackerObj && attackerObj->nation_recno) {
    ...
    if (is_monster()) {
      ...
      //--- when destroying an enslaving Fryhtan lair, the independent towns
      //enslaved by it will reduce its resistance towards you by 30 points ---//
    ...
    float killMoney =
        (float)(firm_res[firm_id]->setup_cost + firm_recno % 100) /
