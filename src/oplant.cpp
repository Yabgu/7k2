...
//							  (either ZONE_TROPICAL
// or
// ZONE_TEMPERATE) 							  0 - any climate
// zone <int> teraType    - the terrain type where the plant is.
//							  0 - any terrain type
...
int PlantRes::scan(int climateZone, int teraType, int townAge)
{
    ...
    for (i = 0; i < plant_count; i++, plantInfo++)
    {
        ...
        {
            if (!teraType || plantInfo->tera_type[0] == teraType || plantInfo->tera_type[1] == teraType ||
                plantInfo->tera_type[2] == teraType)
            {
                ...
                for (j = 0; j < plantInfo->bitmap_count; j++, plantBitmap++)
                {
                    if (!townAge || plantBitmap->town_age == townAge ||
                        plantBitmap->town_age == '*') // * = wildcard type, could apply to any town age level
                    {
...
{
    err_if(bitmapId < 1 || bitmapId > plant_bitmap_count) err_now("PlantRes::get_bitmap");
...
void PlantBitmap::draw(int xLoc, int yLoc)
{
    ...
    world.zoom_matrix->put_bitmap_offset(
        ...
        world.interpolate_z(xLoc * LOCATE_WIDTH + innerX, yLoc * LOCATE_HEIGHT + innerY), bitmap_ptr, offset_x,
        offset_y, NULL, 0, 1);
}
...
void PlantBitmap::draw_at(int curX, int curY, int curZ, int constructionCompletionPercent)
{
    ...
    world.zoom_matrix->put_bitmap_offset(curX, curY, curZ, bitmap_ptr, -bitmapWidth / 2, -bitmapHeight + 15, remapTable,
                                         0, 1);
}
