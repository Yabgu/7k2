...
void PlantRes::load_plant_info() {
  ...
  for (i = 0; i < plant_count; i++) {
    ...
    // SXM
#endif
    plantInfo->climate_zone =
        misc.atoi(plantRec->climate_zone, plantRec->ZONE_LEN);
    ...
    if (plantRec->tera_type1[0] == 'T') // town plant
    {
      ...
      if (plantRec->tera_type1[0] != ' ')
        plantInfo->tera_type[0] =
            terrain_res.get_tera_type_id(plantRec->tera_type1);
      else
    ...
    if (plantRec->tera_type2[0] == 'T') // town plant
    {
      ...
      if (plantRec->tera_type2[0] != ' ')
        plantInfo->tera_type[1] =
            terrain_res.get_tera_type_id(plantRec->tera_type2);
      else
    ...
    if (plantRec->tera_type3[0] == 'T') // town plant
    {
      ...
      if (plantRec->tera_type3[0] != ' ')
        plantInfo->tera_type[2] =
            terrain_res.get_tera_type_id(plantRec->tera_type3);
      else
        plantInfo->tera_type[2] = 0;
    }

    plantInfo->first_bitmap =
        misc.atoi(plantRec->first_bitmap, plantRec->FIRST_BITMAP_LEN);
    plantInfo->bitmap_count =
        misc.atoi(plantRec->bitmap_count, plantRec->BITMAP_COUNT_LEN);
  }
}
...
void PlantRes::load_plant_bitmap() {
  ...
  plant_bitmap_array =
      (PlantBitmap *)mem_add(sizeof(PlantBitmap) * plant_bitmap_count);
  ...
  for (i = 0; i < plant_bitmap_count; i++) {
    ...
    plantBitmap->size =
        misc.atoi(plantBitmapRec->size, plantBitmapRec->SIZE_LEN);
    ...
    plantBitmap->offset_x =
        misc.atoi(plantBitmapRec->offset_x, plantBitmapRec->OFFSET_LEN);
    plantBitmap->offset_y =
        misc.atoi(plantBitmapRec->offset_y, plantBitmapRec->OFFSET_LEN);
...
//							  (either ZONE_TROPICAL or
//ZONE_TEMPERATE) 							  0 - any climate zone
...
int PlantRes::scan(int climateZone, int teraType, int townAge) {
  ...
  for (i = 0; i < plant_count; i++, plantInfo++) {
    if (!climateZone || (plantInfo->climate_zone & climateZone)) {
      if (!teraType || plantInfo->tera_type[0] == teraType ||
        ...
        for (j = 0; j < plantInfo->bitmap_count; j++, plantBitmap++) {
          if (!townAge || plantBitmap->town_age == townAge ||
              plantBitmap->town_age ==
...
PlantInfo *PlantRes::operator[](int plantId) {
  err_if(plantId < 1 || plantId > plant_count) err_now("PlantRes::operator[]");
...
void PlantBitmap::draw(int xLoc, int yLoc) {
  ...
  world.zoom_matrix->put_bitmap_offset(
      xLoc * LOCATE_WIDTH + innerX, yLoc * LOCATE_HEIGHT + innerX,
      world.interpolate_z(xLoc * LOCATE_WIDTH + innerX,
                          yLoc * LOCATE_HEIGHT + innerY),
      bitmap_ptr, offset_x, offset_y, NULL, 0, 1);
}
...
// Draw the current plant on the map
//
void PlantBitmap::draw_at(int curX, int curY, int curZ,
                          int constructionCompletionPercent) {
  short *remapTable;

  remapTable = (short *)vga.vga_color_table->get_table(
      constructionCompletionPercent * 20 / 100 - 20);
