...
void Blob2DW::resize(short destLeft, short destTop, short destWidth,
                     short destHeight) {
  if (width == 0 && height == 0) {
    ...
    IMGbar(ptr->get_ptr(), ptr->get_true_pitch(), 0, 0, destWidth - 1,
           destHeight - 1, BACKGROUND_COLOR);
    ...
    if (alloc_size < siz) {
      ptr =
          (BitmapW *)mem_resize(ptr, (alloc_size = siz + DEFAULT_BLOB2DW_INC));
    }
    ...
    if (destHeight > height && destWidth > 0) {
      ...
      // ##### begin Gilbert 14/10 #######//
      fill_area(destLeft, y2, destLeft + destWidth - 1,
                destTop + destHeight - 1, BACKGROUND_COLOR, 0);
      // ##### end Gilbert 14/10 #######//
    }
  } else if (left_edge <= destLeft && top_edge <= destTop &&
             left_edge + width >= destLeft + destWidth &&
             top_edge + height >= destTop + destHeight) {
    // clipping
...

    {
      ...
      IMGbar(newPtr->get_ptr(), newPtr->get_true_pitch(), 0, 0, destWidth - 1,
             destHeight - 1, BACKGROUND_COLOR);

      if (copyWidth > 0 && copyHeight > 0) {
        int yCount = 0;
        short *src =
            ptr->get_ptr(copyLeft - left_edge, yCount + copyTop - top_edge);
        short *dest =
            newPtr->get_ptr(copyLeft - destLeft, yCount + copyTop - destTop);

        for (; yCount < copyHeight;
             ++yCount, src += ptr->get_pitch(), dest += ptr->get_pitch()) {
          // unsigned char *src = (yCount+copyTop-top_edge)*width +
          // copyLeft-left_edge; unsigned char *dest =
          // (yCount+copyTop-destTop)*destWdith + copyLeft-destLeft;
    ...
    if (top_edge < copyTop && width > 0) {
      fill_area(left_edge, top_edge, left_edge + width - 1, copyTop,
                BACKGROUND_COLOR, 0);
    }

    // fill bottom
    if (top_edge + height > copyTop + copyHeight && width > 0) {
      fill_area(left_edge, copyTop + copyHeight, left_edge + width - 1,
                top_edge + height - 1, BACKGROUND_COLOR, 0);
    }
    ...
    if (left_edge + width > copyLeft + copyWidth && destHeight > 0) {
      fill_area(copyLeft + copyWidth, copyTop, left_edge + width,
                copyTop + copyHeight - 1, BACKGROUND_COLOR, 0);
    }
  }
...
short *Blob2DW::p(short x, short y, int autoExtend) {
  if (autoExtend) {
    ...
    if (x < left_edge || x >= left_edge + width || y < top_edge ||
        y >= top_edge + height)
      return NULL;
  }
...
void Blob2DW::auto_clip(short *pLeft, short *pTop, short *pWidth,
                        short *pHeight, int autoResize) {
  short x1, y1, x2, y2;
