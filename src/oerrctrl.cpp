...
// ---------- define constant -----------//
// 0 don't display log, 1 display exception, 2 display more detail, 3 display
// all
...
int ErrorControl::send(char ecPlayerId, void *dataPtr,
                       long unsigned int dataLen) {
  ...
  if (frameId < 0) {
    ...
    *((CRC_TYPE *)(ecMsg + sizeof(EcMsgHeader) + dataLen)) =
        crc8((unsigned char *)ecMsg, sizeof(EcMsgHeader) + dataLen);
...
void ErrorControl::set_player_lost(char ecPlayerId) {
  ...
  clear_acked_frame(); // some send_queue message may be waiting this player's
                       // ack
}
    ...
    if (simError && !crc8((unsigned char *)recvPtr, recvLen)) {
        ...
        if (is_waiting_receive(ecMsg.sender_id, ecMsg.frame_id)) {
          ...
          if (!is_recv_full()) {
            if (dp_id[ecMsg.sender_id - 1]) {
              ...
              ((EcMsgHeader *)replyMsg)
                  ->init(ACKNOW, self_ec_player_id, ecMsg.frame_id);
              *((CRC_TYPE *)(replyMsg + sizeof(EcMsgHeader))) =
                  crc8((unsigned char *)replyMsg, sizeof(EcMsgHeader));
              mp_ptr->send(dp_id[ecMsg.sender_id - 1], replyMsg,
                           sizeof(replyMsg));
              ...
              if (ecMsg.frame_id == wait_to_receive[ecMsg.sender_id - 1]) {
                ...
                for (; recv_flag[ecMsg.sender_id - 1][scanFrame];
                     inc_frame_id(scanFrame))
      ...
      case NEGACK:
        ...
        if (is_waiting_ack(ecMsg.sender_id, ecMsg.frame_id)) {
          ...
          // recalculate CRC
          *((CRC_TYPE *)(replyMsg + replyLen - CRC_LEN)) =
              crc8((unsigned char *)replyMsg, replyLen - CRC_LEN);
      ...
      if (recvLen > sizeof(EcMsgHeader) + CRC_LEN) {
        ...
        if (senderId) {
          ...
          char replyMsg[sizeof(EcMsgHeader) + CRC_LEN];
          ((EcMsgHeader *)replyMsg)
              ->init(NEGACK, self_ec_player_id, wait_to_receive[senderId - 1]);
          *((CRC_TYPE *)(replyMsg + sizeof(EcMsgHeader))) =
              crc8((unsigned char *)replyMsg, sizeof(EcMsgHeader));
...
int ErrorControl::is_send_empty() { return (send_head == send_tail); }
...
int ErrorControl::send_queue_space() {
  ...
  return send_tail >= send_head ? MAX_QUEUE - 1 - (send_tail - send_head)
                                : send_head - send_tail - 1;
}
...
int ErrorControl::is_recv_empty() { return (recv_head == recv_tail); }

int ErrorControl::is_recv_full() {
  return (recv_tail + 1 == recv_head ||
          recv_tail + 1 == recv_head + MAX_RECV_QUEUE);
}

int ErrorControl::recv_queue_space() {
  // the queue can hold at most MAX_RECV_QUEUE-1 item
  return recv_tail >= recv_head ? MAX_RECV_QUEUE - 1 - (recv_tail - recv_head)
                                : recv_head - recv_tail - 1;
}
...
int ErrorControl::is_waiting_ack(char ecPlayerId, char frameId) {
  // true if frameId is between send_head (inclusive) and send_tail
  // (non-inclusive)
...
int ErrorControl::need_re_send(char frameId, int promptFactor) {
  return ((misc.get_time() - send_time[frameId]) * promptFactor) >=
         re_send_after[frameId];
...
int ErrorControl::is_waiting_receive(char ecPlayerId, char frameId) {
  ...
  return is_between(wait_to_receive[ecPlayerId - 1], frameId,
                    (wait_to_receive[ecPlayerId - 1] + MAX_QUEUE / 2) %
                        MAX_QUEUE) &&
    ...
    if (!are_all_acked(f) && need_re_send(f, promptFactor)) {
      ...
      for (char ecPlayerId = 1; ecPlayerId <= MAX_PLAYER; ++ecPlayerId) {
        ...
        if (resendSuccess > 0) {
          ...
          // recalculate CRC
          *((CRC_TYPE *)(ecMsg + ecMsgLen - CRC_LEN)) =
              crc8((unsigned char *)ecMsg, ecMsgLen - CRC_LEN);
