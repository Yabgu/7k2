...
void Town::draw(int displayLayer)
{
    ...
    if (under_construction)
    {
        ...
        remapTable = (short *)vga.vga_color_table->get_table(
            ...
            (-MAX_BRIGHTNESS_ADJUST_DEGREE +
             MAX_BRIGHTNESS_ADJUST_DEGREE * 4 / 10)); // brightness change from 40% to 90%
                                                      // ##### end Gilbert 9/10 ######//
    }
    else
        remapTable = NULL;
    ...
    else if (displayLayer == 1)
    {
        ...
        if ((hit_points >= max_hit_points()) && target_wall_level > current_wall_level)
        {
            ...
            short *remapTable = (short *)vga.vga_color_table->get_table(
                MAX_BRIGHTNESS_ADJUST_DEGREE - (wall_construction_percent)*MAX_BRIGHTNESS_ADJUST_DEGREE / 100);

            TownWallBitmapInfo *townWallBitmapInfo =
                town_wall_res.search(race_id, current_wall_level + 1, 100, 100, 'B'); // wall back sides
        ...
        else if (current_wall_level > 0 && hit_points > 0.0f)
        {
            TownWallBitmapInfo *townWallBitmapInfo = town_wall_res.search(
                race_id, current_wall_level, hit_points, (float)max_hit_points(), 'B'); // wall back sides
                ...
                if (is_pay_tribute_to_monster())
                {
                    ...
                    for (int i = 0; i < linked_firm_count; i++)
                    {
                            ...
                            resistance(firmPtr->nation_recno) < MONSTER_COLLECT_TOWN_TRIBUTE_LOYALTY)
                        {
                            ...
                            world.zoom_matrix->put_bitmap_offset(sectionX1 + townSlot->base_x,
                                                                 sectionY1 + townSlot->base_y, altitude, flagBitmap, -9,
        ...
        if ((hit_points >= max_hit_points()) && target_wall_level > current_wall_level)
        {
            ...
            TownWallBitmapInfo *townWallBitmapInfo =
                town_wall_res.search(race_id, current_wall_level + 1, 100, 100, 'F'); // wall back sides
        ...
        {
            TownWallBitmapInfo *townWallBitmapInfo = town_wall_res.search(
                race_id, current_wall_level, hit_points, (float)max_hit_points(), 'F'); // wall back sides
...
void Town::draw_flag(int curX, int curY)
{
    ...
    // image_spict.get_ptr(flagName), colorRemapTable, 1);	// 1-the bitmap
    // is compressed
    char *flagBitmap = image_spict.get_ptr(flagName);
    world.zoom_matrix->put_bitmap_offset(curX, curY, altitude, flagBitmap, -9, -97, colorRemapTable, 0, 1);
}
...
void Town::draw_farm(int curX, int curY, int farmId, short *remapTable)
{
    ...
    world.zoom_matrix->put_bitmap(curX, curY, altitude, farmBitmap, remapTable, 0, 1);
}
...
{
    return world.zoom_matrix->calc_zoom_y(loc_x1 * LOCATE_WIDTH, loc_y1 * LOCATE_HEIGHT, altitude) >= 0 &&
           world.zoom_matrix->calc_zoom_x(loc_x1 * LOCATE_WIDTH, (loc_y2 + 1) * LOCATE_HEIGHT - 1, altitude) >= 0 &&
           world.zoom_matrix->calc_zoom_x((loc_x2 + 1) * LOCATE_WIDTH - 1, loc_y1 * LOCATE_HEIGHT, altitude) <=
               ZOOM_WIDTH &&
           world.zoom_matrix->calc_zoom_y((loc_x2 + 1) * LOCATE_WIDTH - 1, (loc_y2 + 1) * LOCATE_HEIGHT - 1,
...
int Town::draw_detect_link_line(int actionDetect)
{
    ...
    int srcX = ZOOM_X1 + world.zoom_matrix->calc_zoom_x((loc_x1 + loc_x2 + 1) * LOCATE_WIDTH / 2,
                                                        (loc_y1 + loc_y2 + 1) * LOCATE_HEIGHT / 2, altitude);
    int srcY = ZOOM_Y1 + world.zoom_matrix->calc_zoom_y((loc_x1 + loc_x2 + 1) * LOCATE_WIDTH / 2,
                                                        (loc_y1 + loc_y2 + 1) * LOCATE_HEIGHT / 2, altitude);
    ...
    for (i = 0; i < linked_firm_count; i++)
    {
        ...
        if (!actionDetect)
        {
            ...
            if (firmPtr->firm_id == FIRM_LAIR && firmPtr->nation_recno && nation_recno == 0 &&
                resistance(firmPtr->nation_recno) < MONSTER_COLLECT_TOWN_TRIBUTE_LOYALTY)
            {
                thickLineFlag = 1;
            }
        ...
        if (can_toggle_firm_link(linked_firm_array[i]))
            ...
            bitmapPtr = image_icon.read(firmPtr->active_link_town_recno == town_recno ? (char *)"LINK_ACT"
                                                                                      : (char *)"LINK_DEA");
        else
            continue;
    ...
    for (i = 0; i < linked_town_count; i++)
    {
        ...
        //*
        // ZOOM_LOC_HEIGHT ) / 2;
        townX = ZOOM_X1 + world.zoom_matrix->calc_zoom_x((townPtr->loc_x1 + townPtr->loc_x2 + 1) * LOCATE_WIDTH / 2,
                                                         (townPtr->loc_y1 + townPtr->loc_y2 + 1) * LOCATE_HEIGHT / 2,
                                                         townPtr->altitude);
        townY = ZOOM_Y1 + world.zoom_matrix->calc_zoom_y((townPtr->loc_x1 + townPtr->loc_x2 + 1) * LOCATE_WIDTH / 2,
                                                         (townPtr->loc_y1 + townPtr->loc_y2 + 1) * LOCATE_HEIGHT / 2,
                                                         townPtr->altitude);
        ...
        if (nation_recno == nation_array.player_recno && nation_recno == townPtr->nation_recno &&
...
int Town::can_toggle_firm_link(int firmRecno)
{
    ...
    for (int i = 0; i < linked_firm_count; i++)
    {
        ...
        //		{
        //			//-- you can only toggle a link to a camp if the camp is
        // yours
        //--// 			case FIRM_CAMP: 				return firmPtr->nation_recno
        //==
        // nation_recno;
        //			//--- town to market link is governed by trade treaty and cannot
        // be
...
void Town::auto_set_layout()
{
...

    {
        ...
        for (layoutId = 1; layoutId <= town_res.town_layout_count; layoutId++) // scan from the least densed layout to
                                                                               // the most densed layout
        {
    ...
    for (int i = 0; i < townLayout->slot_count; i++)
    {
        ...
        {
        case TOWN_OBJECT_PLANT:
            slot_object_id_array[i] = plant_res.scan(0, 'T', 0); // 'T'-town only, 1st 0-any zone area, 2nd
