...
void Town::draw(int displayLayer) {
  ...
  if (under_construction) {
    ...
    remapTable = (short *)vga.vga_color_table->get_table(
        construction_completion_percent * MAX_BRIGHTNESS_ADJUST_DEGREE / 2 /
            100 +
        (-MAX_BRIGHTNESS_ADJUST_DEGREE +
         MAX_BRIGHTNESS_ADJUST_DEGREE * 4 /
             10)); // brightness change from 40% to 90%
                   // ##### end Gilbert 9/10 ######//
  } else
    remapTable = NULL;
  ...
  if (displayLayer == 4) {
    ...
    world.zoom_matrix->put_bitmap_offset(
        sectionX1, sectionY1, altitude, townLayout->ground_bitmap_ptr,
        TOWN_ADJ_OFFSET_X, TOWN_ADJ_OFFSET_Y, remapTable, 0, 1);
    ...
    for (int i = 0; i < townLayout->slot_count; i++, townSlot++) {
      ...
      case TOWN_OBJECT_FARM:
        draw_farm(sectionX1 + townSlot->base_x, sectionY1 + townSlot->base_y,
                  townSlot->build_code, remapTable);
        break;
      }
    }
  }
    ...
    if ((hit_points >= max_hit_points()) &&
      ...
      int wall_construction_percent =
          upgrade_wall_progress * 100 / MAX_WALL_PROGRESS;

      short *remapTable = (short *)vga.vga_color_table->get_table(
          MAX_BRIGHTNESS_ADJUST_DEGREE -
          (wall_construction_percent)*MAX_BRIGHTNESS_ADJUST_DEGREE / 100);

      TownWallBitmapInfo *townWallBitmapInfo = town_wall_res.search(
          race_id, current_wall_level + 1, 100, 100, 'B'); // wall back sides

      if (townWallBitmapInfo) {
        world.zoom_matrix->put_bitmap_offset(
            loc_x1 * LOCATE_WIDTH, loc_y1 * LOCATE_HEIGHT, altitude,
            townWallBitmapInfo->bitmap_ptr, townWallBitmapInfo->offset_x,
            townWallBitmapInfo->offset_y, remapTable, 0, 5);
      }
    } else if (current_wall_level > 0 && hit_points > 0.0f) {
      TownWallBitmapInfo *townWallBitmapInfo =
          town_wall_res.search(race_id, current_wall_level, hit_points,
                               (float)max_hit_points(), 'B'); // wall back sides

      if (townWallBitmapInfo) {
        world.zoom_matrix->put_bitmap_offset(
            loc_x1 * LOCATE_WIDTH, loc_y1 * LOCATE_HEIGHT, altitude,
            townWallBitmapInfo->bitmap_ptr, townWallBitmapInfo->offset_x,
            townWallBitmapInfo->offset_y, NULL, 0, 1);
      }
    } else {
      TownWallBitmapInfo *townWallBitmapInfo =
          town_wall_res.search(race_id, 0, 1, 1, 'B'); // wall back sides
    ...
    for (int i = 0; i < townLayout->slot_count; i++, townSlot++) {
      ...
      case TOWN_OBJECT_HOUSE: {
        town_res.get_build(slot_object_id_array[i])
            ->draw(town_recno, sectionX1 + townSlot->base_x,
                   sectionY1 + townSlot->base_y,
      ...
      case TOWN_OBJECT_PLANT: {
        plant_res.get_bitmap(slot_object_id_array[i])
            ->draw_at(sectionX1 + townSlot->base_x,
                      sectionY1 + townSlot->base_y, altitude,
                      under_construction ? construction_completion_percent
                                         : 100);
        break;
      }
        // case TOWN_OBJECT_FARM:
        //	draw_farm(sectionX1+townSlot->base_x,
        //sectionY1+townSlot->base_y, townSlot->build_code ); 	break;
          ...
          for (int i = 0; i < linked_firm_count; i++) {
            ...
            if (firmPtr->firm_id == FIRM_LAIR && firmPtr->nation_recno &&
              ...
              short *colorRemapTable =
                  game.get_color_remap_table(firmPtr->nation_recno, 0);

              char *flagBitmap = image_spict.get_ptr(flagName);
              world.zoom_matrix->put_bitmap_offset(
                  sectionX1 + townSlot->base_x, sectionY1 + townSlot->base_y,
    ...
    if ((hit_points >= max_hit_points()) &&
      ...
      TownWallBitmapInfo *townWallBitmapInfo = town_wall_res.search(
          race_id, current_wall_level + 1, 100, 100, 'F'); // wall back sides
      ...
      TownWallBitmapInfo *townWallBitmapInfo =
          town_wall_res.search(race_id, current_wall_level, hit_points,
                               (float)max_hit_points(), 'F'); // wall back sides
      ...
      TownWallBitmapInfo *townWallBitmapInfo =
          town_wall_res.search(race_id, 0, 1, 1, 'F');
...
void Town::draw_flag(int curX, int curY) {
  ...
  //	world.zoom_matrix->put_bitmap_remap_clip(drawX, drawY,
  //image_spict.get_ptr(flagName), colorRemapTable, 1);	// 1-the bitmap is
  //compressed
  char *flagBitmap = image_spict.get_ptr(flagName);
  world.zoom_matrix->put_bitmap_offset(curX, curY, altitude, flagBitmap, -9,
                                       -97, colorRemapTable, 0, 1);
}
...
void Town::draw_farm(int curX, int curY, int farmId, short *remapTable) {
  ...
  //	world.zoom_matrix->put_bitmap_clip(drawX, drawY,
  //image_spict.get_ptr(farmName), 1);	// 1-the bitmap is compressed
  char *farmBitmap = image_spict.get_ptr(farmName);

  world.zoom_matrix->put_bitmap(curX, curY, altitude, farmBitmap, remapTable, 0,
                                1);
}
...
int Town::is_in_zoom_win() {
  return world.zoom_matrix->calc_zoom_y(
             loc_x1 * LOCATE_WIDTH, loc_y1 * LOCATE_HEIGHT, altitude) >= 0 &&
         world.zoom_matrix->calc_zoom_x(loc_x1 * LOCATE_WIDTH,
                                        (loc_y2 + 1) * LOCATE_HEIGHT - 1,
                                        altitude) >= 0 &&
         world.zoom_matrix->calc_zoom_x((loc_x2 + 1) * LOCATE_WIDTH - 1,
                                        loc_y1 * LOCATE_HEIGHT,
                                        altitude) <= ZOOM_WIDTH &&
         world.zoom_matrix->calc_zoom_y((loc_x2 + 1) * LOCATE_WIDTH - 1,
                                        (loc_y2 + 1) * LOCATE_HEIGHT - 1,
                                        altitude) <= ZOOM_HEIGHT;
}
...
int Town::draw_detect_link_line(int actionDetect) {
  ...
  int srcX = ZOOM_X1 + world.zoom_matrix->calc_zoom_x(
                           (loc_x1 + loc_x2 + 1) * LOCATE_WIDTH / 2,
                           (loc_y1 + loc_y2 + 1) * LOCATE_HEIGHT / 2, altitude);
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    // ##### begin Gilbert 4/3 #########//
    if (!actionDetect) {
      //--- if this is a Fryhtan Lair, draw links to submission towns with a
      //think line ---//
      ...
      if (thickLineFlag) {
        anim_line.thick_line(&vga_back, srcX, srcY, firmX, firmY,
                             linked_firm_enable_array[i] == LINK_EE, -1);
      } else {
        anim_line.draw_line(&vga_back, srcX, srcY, firmX, firmY,
                            linked_firm_enable_array[i] == LINK_EE, 0);
      }
    }
    // ##### end Gilbert 4/3 #########//
    ...
    if (can_toggle_firm_link(linked_firm_array[i]))
      bitmapPtr = power.get_link_icon(linked_firm_enable_array[i],
                                      nation_recno == firmPtr->nation_recno);
    else if (firmPtr->can_set_active_link(town_recno))
      bitmapPtr = image_icon.read(firmPtr->active_link_town_recno == town_recno
                                      ? (char *)"LINK_ACT"
                                      : (char *)"LINK_DEA");
    else
      continue;
    ...
    if (firmPtr->cast_to_FirmCamp())
      help.set_help(firmX - 16, firmY - 16,
                    firmX - 16 + ((Bitmap *)bitmapPtr)->get_width(),
                    firmY - 16 + ((Bitmap *)bitmapPtr)->get_height(),
                    "TOWNLOCK");
    else
      help.set_help(firmX - 16, firmY - 16,
                    firmX - 16 + ((Bitmap *)bitmapPtr)->get_width(),
    ...
    if (actionDetect) {
      int rc = world.zoom_matrix->detect_bitmap_clip(firmX - 16, firmY - 16,
                                                     bitmapPtr);

      if (rc)
        mouse
            .reset_click(); // reset queued mouse click for fast single clicking
  ...
  for (i = 0; i < linked_town_count; i++) {
    ...
    //		townX = ( ZOOM_X1 + (townPtr->loc_x1-world.zoom_matrix->top_x_loc) *
    //ZOOM_LOC_WIDTH
    //				  + ZOOM_X1 + (townPtr->loc_x2-world.zoom_matrix->top_x_loc+1) *
    //ZOOM_LOC_WIDTH ) / 2; 		townY = ( ZOOM_Y1 +
    //(townPtr->loc_y1-world.zoom_matrix->top_y_loc) * ZOOM_LOC_HEIGHT
    //				  + ZOOM_Y1 + (townPtr->loc_y2-world.zoom_matrix->top_y_loc+1) *
    //ZOOM_LOC_HEIGHT ) / 2;
    townX = ZOOM_X1 +
            world.zoom_matrix->calc_zoom_x(
    ...
    anim_line.draw_line(&vga_back, srcX, srcY, townX, townY,
                        linked_town_enable_array[i] == LINK_EE);

    //------- detect on the migration icon -------//

    if (nation_recno == nation_array.player_recno &&
        nation_recno == townPtr->nation_recno && can_migrate(townRecno)) {
      ...
      if (actionDetect) {
        // ##### begin Gilbert 3/3 #######//
        int rc;
        if ((rc = world.zoom_matrix->detect_bitmap_clip(townX - 23, townY - 25,
                                                        bitmapPtr))) {
          mouse.reset_click(); // reset queued mouse click for fast single
                               // clicking

          err_when(town_array[townRecno]->population > MAX_TOWN_POPULATION);

          // calculate number of people to move instead of while(population > 0)
          // migrate_to(...); because in multiplayer mode, population is not
          // decrease at the moment
          int movePop = 1;
          if (rc == 2) // right click
          {
            movePop = MIN(population,
                          MAX_WAGON_POPULATION); // move 10 as if moving wagon
            if (movePop >
                MAX_TOWN_POPULATION - town_array[townRecno]->population)
        ...
        // ##### end Gilbert 3/3 #######//
        help.set_help(townX - 23, townY - 25,
                      townX - 23 + ((Bitmap *)bitmapPtr)->get_width(),
                      townY - 25 + ((Bitmap *)bitmapPtr)->get_height(),
                      "MIGRATE");
      } else {
...
int Town::can_toggle_firm_link(int firmRecno) {
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    ...
    //--// 			case FIRM_CAMP: 				return firmPtr->nation_recno == nation_recno;
    //			//--- town to market link is governed by trade treaty and cannot be
    //toggled ---// 			case FIRM_MARKET: 				return 0;			//
    //!nation_array[nation_recno]->get_relation(firmPtr->nation_recno)->trade_treaty;
    //			default:
    //				return
    //firm_res[firmPtr->firm_id]->is_linkable_to_town;
    //		}
    ...
    if (firmPtr->cast_to_FirmCamp() || firmPtr->can_set_active_link(town_recno))
      ...
      return 0; // town to market link is governed by trade treaty and cannot be
                // toggled
    else
...
void Town::auto_set_layout() {
...

  {
    ...
    suitBuildCount =
        (population + POPULATION_PER_HOUSE - 1) / POPULATION_PER_HOUSE;
    if (suitBuildCount <= 0)
      suitBuildCount = 1;
    ...
    for (layoutId = 1; layoutId <= town_res.town_layout_count;
         layoutId++) // scan from the least densed layout to the most densed
                     // layout
    {
    ...
    if (suitLayoutCount <= 0)
      layoutId = town_res.town_layout_count; // if none layout is suitable, use
                                             // the largest layout
    else
  ...
  for (int i = 0; i < townLayout->slot_count; i++) {
    ...
    case TOWN_OBJECT_PLANT:
      slot_object_id_array[i] =
          plant_res.scan(0, 'T', 0); // 'T'-town only, 1st 0-any zone area, 2nd
                                     // 0-any terain type, 3rd-age level
      break;
    ...
    case TOWN_OBJECT_HOUSE:
      slot_object_id_array[i] =
          town_res.scan_build(townLayout->first_slot_recno + i, race_id);
...
//
void Town::left_click() { town_array.selected_recno = town_recno; }
