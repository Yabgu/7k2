...
void Firm::process_common_ai() {
  ...
  if (hit_points < max_hit_points() && !should_close_flag &&
      (!nation_recno || nation_ptr()->ai_should_spend(
                            50 + nation_ptr()->pref_repair_concern / 2))) {
...
//
int Firm::ai_should_close() { return 0; }
...
int Firm::think_build_factory(int rawId) {
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    ...
    //---- if the nation does not have enough workers, do not build a new
    //factory ---//
    ...
    for (int j = firmPtr->linked_firm_count - 1; j >= 0; j--) {
      ...
      //--- if this factory is producing enough goods to the market place, then
      //it means it is still quite efficient

      MarketGoods *marketGoods =
          firmMarket->cast_to_FirmMarket()->market_product_array(rawId);
  ...
  //--- only when we have checked it three times and all say it needs a factory,
  //we then build a factory ---//

  if (++ai_should_build_factory_count >= 3) {
    short buildXLoc, buildYLoc;

    if (!nationPtr->find_best_place_loc(FIRM_FACTORY, loc_x1, loc_y1, buildXLoc,
                                        buildYLoc)) {
      no_neighbor_space = 1;
      return 0;
    }
    ...
    nationPtr->add_action(buildXLoc, buildYLoc, loc_x1, loc_y1,
                          ACTION_AI_BUILD_FIRM, FIRM_FACTORY);
...
//
void Firm::think_linked_town_change_nation(int linkedTownRecno,
                                           int oldNationRecno,
                                           int newNationRecno) {}
...
void Firm::ai_firm_captured(int capturerNationRecno) {
  ...
  talk_res.ai_send_talk_msg(capturerNationRecno, nation_recno,
                            TALK_DECLARE_WAR);
}
