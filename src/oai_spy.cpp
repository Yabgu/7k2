...
int Nation::ai_assign_spy_to_town(int townRecno, int raceId) {
  ...
  int mobileOnly =
      ...
      nation_recno; // if assign to own towns/firms, only get mobile spies,
                    // don't get spies from existing towns/firms as that will
                    // result in a loop effect
...
int Nation::ai_assign_spy_to_firm(int firmRecno) {
  ...
  if (firmPtr->cast_to_FirmCamp()) {
    ...
    {
      if (firmCamp->is_soldier_full()) // use is_soldier_full() for own firms as
                                       // it take into account of units
                                       // patrolling outside
        return 0;
    } else {
    ...
    int mobileOnly =
        ...
        nation_recno; // if assign to own firms/firms, only get mobile spies,
                      // don't get spies from existing firms/firms as that will
                      // result in a loop effect

    return ai_assign_spy(firmCamp->loc_x1, firmCamp->loc_y1, raceId,
                         mobileOnly);
  }
...
// [int] spyRaceId			  - if specified, only spies of this
// race
//											 will be
//located. (default:0)
// [int] mobileOnly 				  - get mobile spies only.
// (default:0)
//
int Nation::ai_assign_spy(int targetXLoc, int targetYLoc, int spyRaceId,
                          int mobileOnly) {
  int unitRecno = 0;
  ...
  if ((!assignTargetObj->cast_to_Firm() ||
       !assignTargetObj->cast_to_Firm()->cast_to_FirmCamp()) &&
  ...
  Spy *spyPtr =
      ai_find_spy(targetXLoc, targetYLoc, spyRaceId, isCivilian, mobileOnly);
  ...
  if (!unitRecno)
    unitRecno =
        hire_unit(spyRaceId, isCivilian, true, targetXLoc,
                  targetYLoc); // true-hire a spy as opposite to a military unit

  //--- if cannot hire one, try to train one ----//

  if (!unitRecno) {
    train_spy(spyRaceId, isCivilian, targetXLoc,
              targetYLoc); // train spies takes time so we return 0 anyway
    return 0;
  }
  ...
  int actionRecno =
      add_action(targetXLoc, targetYLoc, -1, -1, ACTION_AI_ASSIGN_SPY,
                 assignTargetObj->nation_recno, 1, unitRecno);
...
// (default:0)
//
Spy *Nation::ai_find_spy(int targetXLoc, int isCivilian, int targetYLoc,
                         int spyRaceId, int mobileOnly) {
    ...
    if (spyPtr->spy_place == SPY_MOBILE) {
      ...
      if (spyPtr->spy_place == SPY_FIRM) {
        ...
        if (firmPtr->nation_recno !=
            nation_recno) // only get spies from our own firms
          continue;
        ...
        if (townPtr->nation_recno !=
            nation_recno) // only get spies from our own towns
          continue;
...
// action_para						- the cloak nation recno the spy
// should set to.
//
int Nation::ai_assign_spy(ActionNode *actionNode) {
  ...
  int isCivilian =
      world.get_loc(actionNode->action_x_loc, actionNode->action_y_loc)
          ->is_town();
  ...
  /*
          if( reputation < 0 )		// if the nation's reputation is
     negative, use sneak mode to avoid chance of being uncovered and further
     damage the reputation newFlag = misc.random( 2+(-(int)reputation)/5 )==0;
     // 2 to 22 else
                  newFlag = misc.random(4)==0;		// 25% chance of being 1

          //-- the spy should not notify when it is just outside an espionage
     college --//
          ...
          if( newFlag )
          {
                  for( int i=firm_array.size() ; i>0 ; i-- )
                  {
                          ...
                          {
                                  if( misc.points_distance( firmPtr->center_x,
     firmPtr->center_y, spyUnit->next_x_loc(), spyUnit->next_y_loc() ) < 20 )
                                  {
                                          newFlag = 0;
                                          break;
                                  }
  ...
  */
  ...
  if (!spyUnit->can_spy_change_nation()) // if the spy can't change nation recno
                                         // now
  {
...
int Nation::think_assign_spy_target_town(int raceId, int regionId) {
  ...
  for (int i = town_array.size(); i > 0; i--) {
    ...
    if (townPtr->population >
        MAX_TOWN_POPULATION -
            5) // -5 so that even if we assign too many spies to a town at the
               // same time, there will still room for them
      continue;
    ...
    if (townPtr->nation_recno) {
      if (townPtr->loyalty <
          MIN_NATION_DEFEND_LOYALTY) // no need to assign spies to these towns
                                     // as they are already very low
        continue;
    } else {
      if (townPtr->resistance(nation_recno) <
          MIN_INDEPENDENT_DEFEND_LOYALTY) // no need to assign spies to these
                                          // towns as they are already very low
        continue;
    }
...
int Nation::think_assign_spy_own_town(int raceId, int regionId) {
  ...
  for (int i = town_array.size(); i > 0; i--) {
    ...
    int curSpyLevel = spy_array.total_spy_skill_level(SPY_TOWN, townRecno,
                                                      nation_recno, spyCount);
...
int Nation::train_spy(int raceId, int isCivilian, short destX, short destY) {
  ...
  for (i = 0; i < ai_spy_count; i++) {
    ...
    curRating = world.distance_rating(destX, destY, firmSpy->center_x,
                                      firmSpy->center_y);
  ...
  for (i = 0; i < ai_spy_count; i++) {
    ...
    if (firmSpy->scan_best_recruitable_town(
            townRecnoArray, raceId, firmSpy->required_linked_firm[trainType])) {
