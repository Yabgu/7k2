...
int Nation::ai_assign_spy_to_town(int townRecno, int raceId)
{
    ...
    int mobileOnly = townPtr->nation_recno == nation_recno; // if assign to own towns/firms, only get mobile spies,
...
int Nation::ai_assign_spy_to_firm(int firmRecno)
{
    ...
    if (firmPtr->cast_to_FirmCamp())
    {
        ...
        int mobileOnly = firmCamp->nation_recno == nation_recno; // if assign to own firms/firms, only get mobile spies,
                                                                 // don't get spies from existing firms/firms as that
                                                                 // will result in a loop effect
...
int Nation::ai_assign_spy(int targetXLoc, int targetYLoc, int spyRaceId, int mobileOnly)
{
    ...
    if (!unitRecno)
        unitRecno = hire_unit(spyRaceId, isCivilian, true, targetXLoc,
    ...
    int actionRecno =
        add_action(targetXLoc, targetYLoc, -1, -1, ACTION_AI_ASSIGN_SPY, assignTargetObj->nation_recno, 1, unitRecno);
...
//
// <int> targetXLoc, targetYLoc - the target location
// [int] spyRaceId			  	  - if specified, only spies of
// this race
//											 will
// be
// located. (default:0)
...
// action_para						- the cloak nation recno the
// spy should set to.
//
...
int Nation::think_assign_spy_target_camp(int raceId, int regionId)
{
    ...
    for (int firmRecno = firm_array.size(); firmRecno > 0; firmRecno--)
    {
        ...
        if (firmCamp->overseer_recno == 0 || firmCamp->soldier_count == MAX_SOLDIER)
        {
            continue;
        }
...
int Nation::think_assign_spy_target_town(int raceId, int regionId)
{
    ...
    for (int i = town_array.size(); i > 0; i--)
    {
        ...
        if (townPtr->population > MAX_TOWN_POPULATION - 5) // -5 so that even if we assign too many spies to a town at
                                                           // the same time, there will still room for them
            continue;
        ...
        {
            if (townPtr->loyalty < MIN_NATION_DEFEND_LOYALTY) // no need to assign spies to these towns
