...
void Nation::think_military()
{
    ...
    if (!find_best_place_loc(FIRM_FORT, bestTownPtr->loc_x1, bestTownPtr->loc_y1, buildXLoc, buildYLoc))
    {
        bestTownPtr->no_neighbor_space = 1;
        return;
    }

    add_action(buildXLoc, buildYLoc, bestTownPtr->loc_x1, bestTownPtr->loc_y1, ACTION_AI_BUILD_FIRM, FIRM_FORT);
}
//--------- Begin of function Nation::think_military --------//
...
int Nation::ai_should_expand_military()
{
    ...
    for (i = ai_camp_count - 1; i >= 0; i--)
    {
        ...
        soldierCount = (firmCamp->overseer_recno > 0) + firmCamp->soldier_count + firmCamp->patrol_unit_count;
...
int Nation::ai_is_troop_need_new_camp()
{
    ...
    int neededSoldierSpace = total_human_count + total_weapon_count - ai_camp_count * MAX_SOLDIER;
