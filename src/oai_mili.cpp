...
void Nation::think_military() {
  ...
  //--------- queue building the fort now ---------//

  short buildXLoc, buildYLoc;

  if (!find_best_place_loc(FIRM_FORT, bestTownPtr->loc_x1, bestTownPtr->loc_y1,
                           buildXLoc, buildYLoc)) {
    bestTownPtr->no_neighbor_space = 1;
    return;
  }

  add_action(buildXLoc, buildYLoc, bestTownPtr->loc_x1, bestTownPtr->loc_y1,
...
int Nation::ai_should_expand_military() {
  ...
  if (ai_camp_count * 9 > total_jobless_population *
                              (50 + pref_military_development) /
                              150) // (50 to 150) / 150
    return 0;
  ...
  for (i = ai_camp_count - 1; i >= 0; i--) {
    ...
    soldierCount = (firmCamp->overseer_recno > 0) + firmCamp->soldier_count +
  ...
  return freeSpaceCount <
         9; // build build new ones when the existing ones are almost full
}
//--------- Begin of function Nation::ai_should_expand_military --------//
...
int Nation::ai_is_troop_need_new_camp() {
  ...
  int neededSoldierSpace =
      total_human_count + total_weapon_count - ai_camp_count * MAX_SOLDIER;
  int neededGeneralSpace = total_general_count - ai_camp_count;

  return neededSoldierSpace >=
             9 + 20 * (100 - pref_military_development) / 200 &&
...
int Nation::ai_has_too_many_camp() {
  return ai_camp_count * 9 > total_jobless_population *
                                 (150 + pref_military_development) /
                                 150; // (150 to 250) / 150
}
//--------- Begin of function Nation::ai_has_too_many_camp --------//
...
int Nation::think_close_camp() { return 0; }
//--------- Begin of function Nation::think_close_camp --------//
