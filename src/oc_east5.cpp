...
void CampaignEastWest::stage_5_init_vars() {
  ...
  next_monster_reinforcement_date =
      info.game_date + (MAX_CAMPAIGN_DIFFICULTY - campaign_difficulty + 2) * 30;
...
int CampaignEastWest::stage_5_create_town() {
  ...
  int townAddCount = 6 + misc.random(3); // add 6 to 8 player towns
  int addedCount = 0;
...
int CampaignEastWest::stage_5_create_firm() {
  ...
  for (int i = 0; i < addCount; i++) {
    ...
    if (has_situation) {
      rc = misc.random(cur_situation.economy_level +
                       cur_situation.military_level) <
    ...
    if (rc)
      ...
      rc = create_military_firm(townRecno,
                                30 + misc.random(30)); // power rating: 30 to 60
