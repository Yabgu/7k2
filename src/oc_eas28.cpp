...
void CampaignEastWest::stage_28_init_vars() {
  ...
  game.add_win_condition(E_REACH_POPULATION, 1, 0, goal_population,
                         goal_year_limit);
  game.add_win_condition(E_REACH_ECONOMY, 1, 0, goal_economic_score,
                         goal_year_limit);
...
void CampaignEastWest::stage_28_prelude() {
  ...
  disp_narrative(res_stage.read("28PRELUD"), goal_population,
                 goal_economic_score, goal_year_limit);
  stop_speech();
...
void CampaignEastWest::stage_28_next_day() {
  ...
  if (misc.random((12 - campaign_difficulty * 2 + nation_array.size() * 2) *
                  15) == 0) {
...
char *CampaignEastWest::stage_28_intro_text() {
  return substitute_text(res_stage.read("28INTRO"), goal_population,
                         goal_economic_score, goal_year_limit);
}
...
char *CampaignEastWest::stage_28_goal_text() {
  return substitute_text(res_stage.read("28GOAL"), goal_population,
                         goal_economic_score, goal_year_limit);
}
