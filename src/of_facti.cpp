...
static short edit_raw_x1, edit_raw_y1, edit_raw_x2, edit_raw_y2,
    edit_raw_enable;
static short edit_product_x1, edit_product_y1, edit_product_x2, edit_product_y2,
    edit_product_enable;
...
void FirmFactory::detect_info() {
  ...
  if( detect_worker_list() )		// detect this when: it's the player's
  firm or the player has spies in this firm
  {
...
void FirmFactory::disp_firm_info(int dispY1, int refreshFlag) {
  ...
  font_snds.center_put_paragraph(INFO_X1 + 10, INFO_Y1 + 5, INFO_X1 + 96,
                                 INFO_Y1 + 45,
                                 text_firm.str_factory_raw(product_raw_id));
  font_snds.center_put_paragraph(INFO_X2 - 86, INFO_Y1 + 5, INFO_X2,
                                 INFO_Y1 + 45,
  ...
  if (raw_stock_qty >= max_raw_stock_qty)
    vga.active_buf->put_bitmap(INFO_X1 + 33, INFO_Y1 + 40 + 15,
  ...
  if (stock_qty >= max_stock_qty)
    vga.active_buf->put_bitmap(INFO_X2 - 64 + 6, INFO_Y1 + 40 + 15,
                               image_icon.read("FULLVILL"));
  ...
  //	if ( is_operating() )
  {
    ...
    vga.active_buf->put_bitmap(INFO_X1 + 93, INFO_Y1 + 36,
                               image_icon.read(animate));
  } else
    vga.active_buf->put_bitmap(INFO_X1 + 93, INFO_Y1 + 36,
                               image_icon.read("GEAR-1"));

  if (product_raw_id == 1) {
    vga.active_buf->put_bitmap(INFO_X1 + 20, INFO_Y1 + 88,
                               image_icon.read("CLY-RW_L"));
    vga.active_buf->put_bitmap_trans(INFO_X1 + 164, INFO_Y1 + 83,
                                     image_icon.read("POTFACT"));
  } else if (product_raw_id == 2) {
    vga.active_buf->put_bitmap(INFO_X1 + 20, INFO_Y1 + 88,
                               image_icon.read("COP-RW_L"));
    vga.active_buf->put_bitmap_trans(INFO_X1 + 164, INFO_Y1 + 83,
                                     image_icon.read("COPGDFAC"));
  } else if (product_raw_id == 3) {
    vga.active_buf->put_bitmap(INFO_X1 + 20, INFO_Y1 + 88,
                               image_icon.read("IRN-RW_L"));
    vga.active_buf->put_bitmap_trans(INFO_X1 + 164, INFO_Y1 + 83,
                                     image_icon.read("IRNGDFAC"));
  } else
    err_here();

  //	str  = "Monthly";
  //	font_snds.center_put( INFO_X1+17, INFO_Y1+90, INFO_X2, INFO_Y1+115, str
  //); 	str  = "Production"; 	font_snds.center_put( INFO_X1+17, INFO_Y1+106,
  //INFO_X2, INFO_Y1+121, str );
  font_snds.center_put_paragraph(INFO_X1 + 72, INFO_Y1 + 85, INFO_X1 + 169,
                                 INFO_Y1 + 121,
                                 text_firm.str_monthly_production(), 0, 0, 0);

  //	str = (int) production_30days();
  font_snds.center_put(INFO_X1 + 17, INFO_Y1 + 117, INFO_X2,
                       INFO_Y1 + 137, // str );
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    // manual button
    button_auto_switch_group[0].create(
        INFO_X1 + 3 * BUTTON_DISTANCE + 13, INFO_Y1 + 281,
        INFO_X1 + 3 * BUTTON_DISTANCE + 56 - 1,
        INFO_Y1 + 281 + BUTTON_ACTION_HEIGHT - 1, i_disp_auto_switch_button,
        ButtonCustomPara(this, 0), 0);

    // auto button
    button_auto_switch_group[1].create(
        INFO_X1 + 2 * BUTTON_DISTANCE + 13, INFO_Y1 + 281,
        INFO_X1 + 2 * BUTTON_DISTANCE + 56 - 1,
        INFO_Y1 + 281 + BUTTON_ACTION_HEIGHT - 1, i_disp_auto_switch_button,
        ButtonCustomPara(this, 1), 0);
  }

  help.set_help(INFO_X1 + 3 * BUTTON_DISTANCE + 13, INFO_Y1 + 281,
                INFO_X1 + 3 * BUTTON_DISTANCE + 56 - 1,
                INFO_Y1 + 281 + BUTTON_ACTION_HEIGHT - 1, "MANUAL");
  help.set_help(INFO_X1 + 2 * BUTTON_DISTANCE + 13, INFO_Y1 + 281,
                INFO_X1 + 2 * BUTTON_DISTANCE + 56 - 1,
                INFO_Y1 + 281 + BUTTON_ACTION_HEIGHT - 1, "AUTO");
...
void FirmFactory::detect_firm_info() {
  if (is_own()) {
    ...
    if (button_auto_switch_group.detect() >= 0) {
      if (!remote.is_enable()) {
        auto_switch_flag = button_auto_switch_group[button_auto_switch_group()]
                               .custom_para.value;
      } else {
        // packet structure : <firm recno> <product id>
        short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_FACTORY_AUTO,
                                                             2 * sizeof(short));
        shortPtr[0] = firm_recno;
        shortPtr[1] = button_auto_switch_group[button_auto_switch_group()]
                          .custom_para.value;
      }
      se_ctrl.immediate_sound("TURN_ON");
    }
...
static void i_disp_auto_switch_button(ButtonCustom *button, int repaintBody) {
  ...
  font_whbl.center_put(button->x1, y, button->x2, y + lineHeight - 1,
                       button->custom_para.value
                           ? text_firm.str_factory_auto()
                           : text_firm.str_factory_manual());
...
void FirmFactory::disp_edit_mode(int &refreshFlag, int &y, int dispBG) {
  ...
  x += 20;
  x = edit_raw_x2 = font_blue.put((edit_raw_x1 = x), (edit_raw_y1 = y),
                                  (int)raw_stock_qty, 0, x2);
  ...
  x += 20;
  x = edit_product_x2 = font_blue.put(
      (edit_product_x1 = x), (edit_product_y1 = y), (int)stock_qty, 0, x2);
...
int FirmFactory::detect_edit_mode() {
  ...
  if (edit_raw_enable) {
    if (mouse.any_click(edit_raw_x1, edit_raw_y1, edit_raw_x2, edit_raw_y2,
                        LEFT_BUTTON)) {
  ...
  if (edit_product_enable) {
    if (mouse.any_click(edit_product_x1, edit_product_y1, edit_product_x2,
                        edit_product_y2, LEFT_BUTTON)) {
      stock_qty += 100.0f;
      if (stock_qty > max_stock_qty)
