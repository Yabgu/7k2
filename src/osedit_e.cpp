...
void ScenarioEditor::disp_edit_view()
{
    ...
    if (mouse.in_area(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2))
    {
        ...
        if (world.zoom_matrix->get_detect_location(mouse.cur_x, mouse.cur_y, &xLoc, &yLoc, &mobileType))
        {
            if (unit_array.selected_recno && !unit_array.is_deleted(unit_array.selected_recno))
            {
                ...
                if (world.check_unit_space(xLoc, yLoc, xLoc + unitPtr->sprite_info->loc_width - 1,
                                           yLoc + unitPtr->sprite_info->loc_height - 1, unitPtr->mobile_type))
                {
                    validAction = 1;
                }
                markerX1 = xLoc;
...
int ScenarioEditor::detect_edit_view()
{
    ...
    if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, 0))
    {
        ...
        if (world.zoom_matrix->get_detect_location(mouse.click_x(0), mouse.click_y(0), &xLoc, &yLoc, &mobileType))
        {
            if (unit_array.selected_recno && !unit_array.is_deleted(unit_array.selected_recno))
            {
                // ###### begin Gilbert 6/3 ########//
