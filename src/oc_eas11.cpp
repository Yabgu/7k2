...
enum
{
    STAGE_11_ALLIED_MONSTER_NATION_RECNO = 2,
...
int CampaignEastWest::stage_11_create_game()
{
    ...
    stage_11_init_relation(); // must init relationship first so that allied units
    ...
    create_troop(STAGE_11_ALLIED_MONSTER_NATION_RECNO, 55 - campaign_difficulty * 5,
                 50 + misc.random(20)); // 50-70 combat level
...
void CampaignEastWest::stage_11_init_vars()
{
    ...
    game.add_win_condition(E_DEFEAT_KINGDOM, 1,
                           STAGE_11_ENEMY_MONSTER_NATION_RECNO); // the objective is to destroy the
                                                                 // enemy kingdom
}
...
int CampaignEastWest::stage_11_create_lair()
{
    ...
    create_lair(cur_monster_nation_recno, STAGE_11_ENEMY_MONSTER_NATION_RECNO, lairAddCount, independentTownAddCount,
                specialFirmCount, combatLevel);
...
void CampaignEastWest::stage_11_process_game_result()
{
    ...
    if (game.result_win_condition_id >= 0)
    {
        ...
        //-- if the attacking monster is still alive and you are still allied with
        // it --//

        if (fryhtan_joint_attack_reward && !nation_array.is_deleted(STAGE_11_ALLIED_MONSTER_NATION_RECNO))
        {
            if (state_array.nation_state_count(cur_monster_nation_recno) > 1 && // if the allied monster only has one
                                                                                // state, it will not give land
                ...
                misc.random(10) < 7) // 70% chance that the allied Fryhtan will deliver its promise
            {
                ...
                if (!rc) // if the monster doesn't have any state next to the player
                         // nation, then pick any state
                ...
                // 			disp_narrative( res_stage.read("11POST1") );
                // //BUGHERE

                //--- set the relation with the attacking fryhtan kingdom back to
                // neutral after the battle ----//
            ...
            else
            {
                ...
                //				disp_narrative( res_stage.read("11POST2")
                //);
                ////BUGHERE
            }
        }
    ...
    else
    {
        ...
        misc.set_random_seed(saved_random_seed); // restore the random seed so that the game will
