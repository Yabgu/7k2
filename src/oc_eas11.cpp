...
int CampaignEastWest::stage_11_create_game() {
  ...
  create_troop(STAGE_11_ALLIED_MONSTER_NATION_RECNO,
               55 - campaign_difficulty * 5,
...
int CampaignEastWest::stage_11_create_lair() {
  ...
  create_lair(cur_monster_nation_recno, STAGE_11_ENEMY_MONSTER_NATION_RECNO,
...
void CampaignEastWest::stage_11_process_game_result() {
  ...
  if (game.result_win_condition_id >= 0) {
    ...
    if (fryhtan_joint_attack_reward &&
      ...
      if (state_array.nation_state_count(cur_monster_nation_recno) >
              1 && // if the allied monster only has one state, it will not give
                   // land
          (~nation_array)
                  ->get_relation_status(STAGE_11_ALLIED_MONSTER_NATION_RECNO) ==
              RELATION_ALLIANCE &&
          misc.random(10) <
              7) // 70% chance that the allied Fryhtan will deliver its promise
      {
        //---- the allied Frythan gives a state to the player ----//

        int rc = random_pick_attack_state(western_nation_recno,
        ...
        (~nation_array)
            ->set_relation_status(STAGE_11_ALLIED_MONSTER_NATION_RECNO,
                                  RELATION_NEUTRAL);
      } else {
