...
//
void SnowLayer::init(short h, short v, short speed, short amp, short r, double s, char animSpeed, short initPeriod)
{
    h_sep = h;
    v_sep = v;
...
void SnowLayer::draw_step(VgaBuf *vgabuf)
{
    ...
    short snowColor[3];
    snowColor[0] = vga_back.translate_color(SNOW_COLOUR1);
    snowColor[1] = vga_back.translate_color(SNOW_COLOUR2);
    snowColor[2] = vga_back.translate_color(SNOW_COLOUR3);
    ...
    case 0:
        for (sy = snow_y, cy = 0; sy <= bound_y2 && cy < period; sy += v_sep, ++cy)
        {
            for (sx = snow_x, cx = 0; sx <= bound_x2; sx += h_sep, ++cx)
            {
                if ((cx + cy) % 3 != blind_site)
                {
                    vgabuf->draw_pixel(sx, sy, snowColor[2]);
                }
            }
        }
        break;
