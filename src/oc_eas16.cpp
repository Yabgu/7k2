...
int CampaignEastWest::stage_16_create_game() {
  ...
  create_troop(PLAYER_NATION_RECNO,
               46 - campaign_difficulty *
                        6); // 80 - average combat level, an elite force

  create_troop(PLAYER_NATION_RECNO, 13 - campaign_difficulty, 0,
               tech_res[mega_tech_id]->unit_id, 1); // 1-use last location
...
int CampaignEastWest::stage_16_create_lair() {
  int lairAddCount =
      4 + campaign_difficulty + misc.random(2); // add 6 to 7 Fryhtan Lairs
  ...
  create_lair(cur_monster_nation_recno, STAGE_16_MONSTER_NATION_RECNO,
