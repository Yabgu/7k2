...
//                       Elastic button will pop up immediately when button
//                       release Non-elastic button will remain pushed until
//                       pop() is called (default : 1)
//
// [int]  defIsPushed  = default is_pushed : 1-Pushed, 0-Non-pushed
//                         (default : 0)
//
void Button::create_text(int pX1, int pY1, const char *textPtr, char pElastic,
                         char defIsPushed, char textCentrePut) {
  int pX2, pY2;
...
//
void Button::paint_text(int pX1, int pY1, char *textPtr, char pElastic,
                        char defIsPushed, char textCentrePut) {
  int pX2, pY2;
...
//
// <int>   buttonType = BUTTON_TEXT (1) - text button, BUTTON_BITMAP (2) - icon
// button <int>   x1, y1     = coordination of the button <int>   x2, y2     =
// coordination of the button <void*> bodyPtr    = icon pointer or pointer to
...
//-------- Begin of function Button::create -------//
//
// Syntax : create(<int>,<int>,<int>,<int>,<int>,<const
// char*>,[char],[char],[char])
//
// <int>   buttonType = BUTTON_TEXT (1) - text button, BUTTON_BITMAP (2) - icon
// button <int>   x1, y1     = coordination of the button <int>   x2, y2     =
// coordination of the button <const char*> text = text of the button [char]
// elastic    = Whether the button is elastic
//                      Elastic button will pop up immediately when button
//                      release Non-elastic button will remain pushed until
//                      pop() is called (default : 1)
// [char]  defIsPushed = default is_pushed : 1-Pushed, 0-Non-pushed
...
//
// <void*> bodyPtr    = text or icon pointer or pointer to user defined
// function,
//
void Button::set_body(void *bodyPtr) {
  if (button_type == BUTTON_TEXT) {
    int strLen = strlen(
        (char *)bodyPtr); // copy the string to class member buffer
                          // some string are temporary, we need it for repaint
    if (strLen > STR_BUF_LEN)
      strLen = STR_BUF_LEN;
...
void Button::paint(int defIsPushed, int repaintBody) {
  ...
  if (enable_flag) {
    ...
    if (!is_pushed) {
      if (use_texture_flag)
        ...
        // ##### end Gilbert 19/10 ######//
        Vga::active_buf->d3_panel_up(x1, y1, x2, y2, 1,
                                     repaintBody || button_type == 1);
      }
    } else {
      if (use_texture_flag) {
        ...
        Vga::active_buf->d3_panel_down(x1, y1, x2, y2, 1,
                                       repaintBody || button_type == 1);
        // ##### end Gilbert 19/10 ######//
      }
    }
  ...
  if (button_type == BUTTON_TEXT) // text button
  {
    if (!text_centre_put) {
      ...
      if (!is_pushed)
        font_ptr->center_put_paragraph(x1 + 2, y1, x2 - 2, y2, str_buf, 2, 0,
                                       0);
      else
        font_ptr->center_put_paragraph(x1 + 1 + 2, y1 + 1, x2 + 1, y2 + 1 - 2,
                                       str_buf, 2, 0, 0);
    }
  }
    ...
    if (body_ptr && repaintBody) {
      ...
      Vga::active_buf->put_bitmap(
          tx, ty, (char *)body_ptr); // 0 means not clear background
    }
...
//          0 - if not
//
int Button::detect(unsigned keyCode1, unsigned keyCode2, int detectRight,
                   int suspendPop) {
  int rc = 0;
...
#define INACTIVE_TIMEOUT_SECONDS 10 // 10 seconds
  ...
  while (!detect(KEY_RETURN, KEY_ESC) &&
         !mouse.any_click(1)) // 1-only right mouse button
  {
    sys.yield();
    mouse.get_event();
...
//----------- Begin of function ButtonGroup::~ButtonGroup -----------//
//
ButtonGroup::~ButtonGroup() { delete[] button_array; }
//-------------- End of function ButtonGroup::~ButtonGroup ----------//
