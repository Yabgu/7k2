...
//
// Think about initiating a new action to capture a new enemy town.
//
// <Town*> capturerTown - our town to capture enemy towns.
// [int]   useAllCamp   - whether use troops in all camps to attack the enemy
// town
//								  (default: 0)
//
int Nation::think_capture_new_enemy_town(Town *capturerTown, int useAllCamp) {
  ...
  if (ai_capture_enemy_town_recno) // no new action if we are still trying to
                                   // capture a town
    return 0;

  //---- only attack when we have enough money to support the war ----//

  if (cash < 1000 + 2000 * pref_cash_reserve /
                        100) // if the cash is really too low now
    return 0;
  ...
  {
    ai_capture_enemy_town_recno =
        targetTown->town_recno; // this nation is currently trying to capture
                                // this town
  ...
  else if (rc == -1) // -1 means no defense on the target town, no attacking is
                     // needed.
  {
    return start_capture(targetTown->town_recno,
                         2); // call AI functions in OAI_CAPT.CPP to capture the
                             // town, false-don't hire a new capturer, use an
                             // existing unit as the capturer
  }
...
//
// This is called when a capturing action is process.
//
void Nation::think_capturing_enemy_town() {
  ...
  if (town_array.is_deleted(ai_capture_enemy_town_recno) ||
      town_array[ai_capture_enemy_town_recno]->nation_recno ==
          nation_recno) // this town has been captured already
  {
  ...
  int mobileCombatLevel = mobile_defense_combat_level(
      targetTown->center_x, targetTown->center_y, targetTown->nation_recno, 0,
      hasWar); // 0-don't return immediately even if there is war around this
               // town
  ...
  if (!ai_capture_enemy_town_start_attack_date) {
    ...
    if (info.game_date >
        ai_capture_enemy_town_plan_date +
            90) // when 3 months have gone and there still hasn't been any
                // attack on the town, there must be something bad happened to
                // our troop, cancel the entire action
      ai_capture_enemy_town_recno = 0;
  ...
  if (info.game_date - ai_capture_enemy_town_start_attack_date >= 15) {
    ...
    if (mobileCombatLevel > 0 &&
        hasWar == 2) // we are still in war with the enemy
    {
      ai_attack_target(targetTown->center_x, targetTown->center_y,
                       mobileCombatLevel, 0,
                       1); // 1-just all move there and wait for the units to
                           // attack the enemies automatically
      return;
    }
  }
  ...
  if (hasWar != 2) {
    ...
    int rc = attack_enemy_town_defense(targetTown,
                                       ai_capture_enemy_town_use_all_camp);
    ...
    if (rc ==
        -1) // -1 means no defense on the target town, no attacking is needed.
    {
      start_capture(targetTown->town_recno,
                    2); // call AI functions in OAI_CAPT.CPP to capture the town
    }
...
//					  0 - we don't have sufficient troops for
//attacking the target. 					 -1 - no defense on the target town, no attacking is
//needed.
//
int Nation::attack_enemy_town_defense(Town *targetTown, int useAllCamp) {
  ...
  int hasWar;
  int townMobileCombatLevel =
      mobile_defense_combat_level(targetTown->center_x, targetTown->center_y,
                                  targetTown->nation_recno, 0, hasWar);
  ...
  if (bestTargetFirm) {
    ...
    if (targetNation
            ->is_at_war()) // use all camps force if the nation is at war
      useAllCamp = 1;

    return ai_attack_target(bestTargetFirm->loc_x1, bestTargetFirm->loc_y1,
                            totalDefenseCombatLevel, 0, 0, 0, 0, useAllCamp);
  } else {
    //--- if there are any mobile defense force around the town ----//

    if (townMobileCombatLevel > 0)
      return ai_attack_target(targetTown->center_x, targetTown->center_y,
                              totalDefenseCombatLevel, 0,
                              1); // 1-just all move there and wait for the
                                  // units to attack the enemies automatically
  }
...
Town *Nation::think_capture_enemy_town_target(Town *capturerTown) {
  ...
  for (townRecno = town_array.size(); townRecno > 0; townRecno--) {
    ...
    if (targetTown->has_linked_camp(
            nation_recno, 0)) // 0-count both camps with or without overseers
      continue;
    ...
    else if (nationRelation->ai_relation_level <
             10) // even if the relation is not hostile, if the
                 // ai_relation_level is < 10, attack anyway
      rc = 1;

    else if (nationRelation->status <= RELATION_NEUTRAL &&
             targetTown->nation_recno ==
                 nation_array.max_overall_nation_recno && // if this is our
                                                          // biggest enemy
    ...
    //----- if this town does not have any linked camps, capture this town
    //immediately -----//

    if (targetTown->has_linked_camp(
            targetTown->nation_recno,
            0)) // 0-count both camps with or without overseers
      return targetTown;
    ...
    if (!ai_should_spend_war(
            nation_array[targetTown->nation_recno]->military_rank_rating()))
      continue;

    //-------------------------------------------------------//

    int townCombatLevel = enemy_town_combat_level(
        targetTown, 1,
        hasWar); // 1-return a rating if there is war with the town
    ...
    curRating =
        world.distance_rating(capturerTown->center_x, capturerTown->center_y,
                              targetTown->center_x, targetTown->center_y);

    curRating -= townCombatLevel / 10;

    curRating -= (int)targetTown->loyalty;

    curRating += targetTown->population; // put a preference on capturing
                                         // villages with large population
    ...
    //-- AI Aggressive is set above Low, than the AI will try to capture the
    //player's town first ---//

    if (!targetTown->ai_town) {
      if (game.game_mode ==
          GAME_TUTORIAL) // next attack the player in a tutorial game
      {
        continue;
      } else {
    ...
    for (int i = targetTown->linked_firm_count - 1; i >= 0; i--) {
      ...
      if (firmPtr->firm_id == FIRM_MINE) {
        ...
        if (raw_count_array[((FirmMine *)firmPtr)->raw_id - 1] == 0)
          curRating += 150 * (int)((FirmMine *)firmPtr)->reserve_qty /
                       MAX_RAW_RESERVE_QTY;
      }
    }
...
//
int Nation::enemy_town_combat_level(Town *targetTown, int returnIfWar,
                                    int &hasWar) {
  int enemyCombatLevel = mobile_defense_combat_level(
      targetTown->center_x, targetTown->center_y, targetTown->nation_recno,
      returnIfWar,
      hasWar); // 0-don't return even there are wars around the town
  ...
  /*
          ...
          for( i=targetTown->linked_town_count-1 ; i>=0 ; i-- )
          {
                  ...
                  if( townPtr->nation_recno == targetTown->nation_recno )
     //**BUGHERE enemyCombatLevel += townPtr->jobless_population * 5;
          }
  */
...
///		        return -1 if there is war and returnIfWar is 1
//
int Nation::enemy_firm_combat_level(Firm *targetFirm, int returnIfWar,
                                    int &hasWar) {
  int enemyCombatLevel = mobile_defense_combat_level(
      targetFirm->center_x, targetFirm->center_y, targetFirm->nation_recno,
      returnIfWar,
      hasWar); // 0-don't return even there are wars around the town
  ...
  //--- calculate the combat level of enemy camps linked to this towns that are
  //linked to this mine ---//
...
// is war
//											 around the
//given area. 											 1 - if there is war 											 2 - if our nation is involved in the war
//
...
//              going on.
//
int Nation::mobile_defense_combat_level(int targetXLoc, int targetYLoc,
                                        int targetNationRecno, int returnIfWar,
                                        int &hasWar) {
  int scanRange = 5 + config.ai_aggressiveness * 2;

  int attackerPower = world.net_attacker_power_in_area(
      targetXLoc, targetYLoc, nation_recno, targetNationRecno, returnIfWar,
      hasWar, scanRange);

  if (attackerPower == -1)
    return -1;
  else
    return -attackerPower; // negative of net attacker power is net defender
                           // power
}
