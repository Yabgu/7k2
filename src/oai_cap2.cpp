...
int Nation::think_capture_new_enemy_town(Town *capturerTown, int useAllCamp)
{
    ...
    {
        ai_capture_enemy_town_recno = targetTown->town_recno; // this nation is currently trying to capture
                                                              // this town
...
void Nation::think_capturing_enemy_town()
{
    ...
    int mobileCombatLevel =
        mobile_defense_combat_level(targetTown->center_x, targetTown->center_y, targetTown->nation_recno, 0,
                                    hasWar); // 0-don't return immediately even if there is war around
                                             // this town
    ...
    if (!ai_capture_enemy_town_start_attack_date)
    {
        ...
        if (info.game_date >
            ai_capture_enemy_town_plan_date + 90) // when 3 months have gone and there still hasn't been
                                                  // any attack on the town, there must be something bad
                                                  // happened to our troop, cancel the entire action
            ai_capture_enemy_town_recno = 0;
    ...
    if (info.game_date - ai_capture_enemy_town_start_attack_date >= 15)
    {
        ...
        {
            ai_attack_target(targetTown->center_x, targetTown->center_y, mobileCombatLevel, 0,
...
//					  0 - we don't have sufficient troops
// for
// attacking the target. 					 -1 - no defense on the target
// town, no attacking is needed.
//
int Nation::attack_enemy_town_defense(Town *targetTown, int useAllCamp)
{
    ...
    for (int i = targetTown->linked_firm_count - 1; i >= 0; i--)
    {
        ...
        if (firmPtr->nation_recno == targetTown->nation_recno && firmPtr->cast_to_FirmCamp())
        {
    ...
    int townMobileCombatLevel =
        mobile_defense_combat_level(targetTown->center_x, targetTown->center_y, targetTown->nation_recno, 0, hasWar);
    ...
    if (bestTargetFirm)
    {
        ...
        return ai_attack_target(bestTargetFirm->loc_x1, bestTargetFirm->loc_y1, totalDefenseCombatLevel, 0, 0, 0, 0,
                                useAllCamp);
    }
    else
    {
        ...
        if (townMobileCombatLevel > 0)
            return ai_attack_target(targetTown->center_x, targetTown->center_y, totalDefenseCombatLevel, 0,
...
Town *Nation::think_capture_enemy_town_target(Town *capturerTown)
{
    ...
    for (townRecno = town_array.size(); townRecno > 0; townRecno--)
    {
        ...
        if (targetTown->nation_recno == 0 || targetTown->nation_recno == nation_recno)
        {
            continue;
        }
        ...
        else if (nationRelation->ai_relation_level < 10) // even if the relation is not hostile, if the
                                                         // ai_relation_level is < 10, attack anyway
            rc = 1;

        else if (nationRelation->status <= RELATION_NEUTRAL &&
                 targetTown->nation_recno == nation_array.max_overall_nation_recno && // if this is our
                                                                                      // biggest enemy
        ...
        if (targetTown->has_linked_camp(targetTown->nation_recno,
        ...
        int townCombatLevel = enemy_town_combat_level(targetTown, 1,
        ...
        curRating = world.distance_rating(capturerTown->center_x, capturerTown->center_y, targetTown->center_x,
                                          targetTown->center_y);
...
int Nation::enemy_town_combat_level(Town *targetTown, int returnIfWar, int &hasWar)
{
    int enemyCombatLevel =
        mobile_defense_combat_level(targetTown->center_x, targetTown->center_y, targetTown->nation_recno, returnIfWar,
...
{
    int enemyCombatLevel =
        mobile_defense_combat_level(targetFirm->center_x, targetFirm->center_y, targetFirm->nation_recno, returnIfWar,
                                    hasWar); // 0-don't return even there are wars around the town
    ...
    for (int i = targetFirm->linked_town_count - 1; i >= 0; i--)
    {
        ...
        for (int j = linkedTown->linked_firm_count - 1; j >= 0; j--)
        {
            ...
            if (firmPtr->nation_recno == targetNationRecno && firmPtr->cast_to_FirmCamp())
            {
                enemyCombatLevel += ((FirmCamp *)firmPtr)->total_combat_level();
            }
        }
...
// the
// given area. 											 1 - if
// there is war 2 - if our nation is involved in the war
//
...
//              going on.
//
int Nation::mobile_defense_combat_level(int targetXLoc, int targetYLoc, int targetNationRecno, int returnIfWar,
                                        int &hasWar)
{
    int scanRange = 5 + config.ai_aggressiveness * 2;

    int attackerPower = world.net_attacker_power_in_area(targetXLoc, targetYLoc, nation_recno, targetNationRecno,
                                                         returnIfWar, hasWar, scanRange);
