...
LocationBitmap::LocationBitmap() { init_flag = 0; }

LocationBitmap::~LocationBitmap() { deinit(); }

void LocationBitmap::init(int cachedColumn, int cachedRow, int viewColumn,
                          int viewRow) {
  deinit();
  ...
  bitmap_array =
      (Blob2DW **)mem_add(sizeof(Blob2DW *) * cachedRow * cachedColumn);
  ...
  adj_row =
      cachedRow - viewRow > 2 ? 2 : 0; // bias all extra rows to the bottom side
...
// if get_new_bitmap still return NULL, create a temporary Blob2DW to store the
// bitmap of that location otherwise fill that Blob2DW
