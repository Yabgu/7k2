...
void TechRes::load_tech_class() {
  ...
  for (int i = 0; i < tech_class_count; i++) {
    ...
    techClass->race_id =
        misc.atoi(techClassRec->race_id, techClassRec->RACE_ID_LEN);

#ifdef DEBUG
    // check code name of tech class
    char techClassCode[techClassRec->CODE_LEN + 1];
    misc.rtrim_fld(techClassCode, techClassRec->class_code,
                   techClassRec->CODE_LEN);
...
void TechRes::load_tech_info() {
  ...
  for (i = 0; i < tech_count; i++) {
    ...
    techInfo->common_max_tech_level =
        misc.atoi(techRec->max_tech_level, techRec->MAX_TECH_LEVEL_LEN);
    techInfo->complex_level =
        misc.atoi(techRec->complex_level, techRec->COMPLEX_LEVEL_LEN);

    techInfo->unit_id = misc.atoi(techRec->unit_id, techRec->ID_LEN);
    techInfo->firm_id = misc.atoi(techRec->firm_id, techRec->ID_LEN);

    techInfo->parent_tech_id =
        misc.atoi(techRec->parent_tech_id, techRec->ID_LEN);
    techInfo->parent_tech_level = misc.atoi(&techRec->parent_tech_level,
                                            sizeof(techRec->parent_tech_level));
    techInfo->parent_tech_id2 =
        misc.atoi(techRec->parent_tech_id2, techRec->ID_LEN);
    techInfo->parent_tech_level2 = misc.atoi(
        &techRec->parent_tech_level2, sizeof(techRec->parent_tech_level2));
    techInfo->advantage_race_id =
        misc.atoi(techRec->advantage_race_id, techRec->RACE_ID_LEN);

    err_when(techInfo->advantage_race_id <
             0); // total_tech_level_array support only human race so far
    err_when(techInfo->parent_tech_id && (techInfo->parent_tech_level < 1 ||
                                          techInfo->parent_tech_level > 9));
    err_when(techInfo->parent_tech_id2 && (techInfo->parent_tech_level2 < 1 ||
                                           techInfo->parent_tech_level2 > 9));
    ...
    for (int raceId = 0; raceId <= MAX_RACE; ++raceId) {
      ...
      if (techInfo->advantage_race_id &&
          raceId == techInfo->advantage_race_id) {
        total_tech_level_array[raceId]++; // but advantage_race_id is not set
                                          // here
      }
    }
  ...
  for (i = 0; i < tech_count; i++) {
    ...
    if (techInfo->class_id == TECH_CLASS_SPU_STRUCTURE ||
      ...
      techInfo->race_id =
          i + 1 - tech_class(techInfo->class_id)->first_tech_id + 1;

      err_when(techInfo->race_id < 1 || techInfo->race_id > MAX_RACE);
    } else if (tech_class(techInfo->class_id)
                   ->race_id) // copy from tech class if it has
    {
...
void TechRes::second_init() {
  if (game.is_campaign_mode()) // tech are not available in the beginning of a
                               // campaign.
  {
    ...
    for (int i = 0; i < tech_count; i++) {
      if (tech_info_array[i].class_id == TECH_CLASS_OFFENSIVE_STRUCTURE)
        tech_info_array[i].available_level =
            0; // cannot research offensive structure in single game
      else
        tech_info_array[i].available_level =
            tech_info_array[i].common_max_tech_level;
    }
  }
}
...
void TechRes::second_init()
{
        ...
        for( i = 1; i <= tech_count; ++i )
        {
                ...
                if( techInfo->common_max_tech_level > 1	// meaningful to have
higher levels
                        && !techInfo->race_id
// non-race specific
                        && !techInfo->unit_id && !techInfo->firm_id )	// not
related to unit/firm technology
                {
        ...
        for( i = 1; i <= tech_count; ++i )
        {
                ...
                for( int raceId = 0; raceId <= MAX_RACE; ++raceId )
                {
                        total_tech_level_array[raceId] +=
techInfo->common_max_tech_level; if( techInfo->advantage_race_id && raceId ==
techInfo->advantage_race_id )
                        {
                                total_tech_level_array[raceId]++;
// but advantage_race_id is not set here
                        }
                }
        }
...
TechInfo *TechRes::operator[](int techId) {
  err_if(techId < 1 || techId > tech_count) err_now("TechRes::operator[]");
...
void TechRes::init_nation_tech(int nationRecno) {
  ...
  for (i = 0; i < tech_res.tech_count; i++, techInfo++) {
    ...
    // ##### begin Gilbert 26/11 #####//
    // since set_nation_tech_level cannot decrease UnitInfo/FirmInfo's nation
    // tech level, call here to reset nation tech level
    if (techInfo->unit_id)
...
int TechInfo::is_parent_tech_invented(int nationRecno) {
  ...
  if (tech_id ==
      TECH_ALCHEMY_YIELD) // only available for research when all basic alchemy
  ...
  if (parent_tech_id) {
    if (tech_res[parent_tech_id]->get_nation_tech_level(nationRecno) <
        parent_tech_level)
      return 0;
  }

  if (parent_tech_id2) {
    if (tech_res[parent_tech_id2]->get_nation_tech_level(nationRecno) <
        parent_tech_level2)
      return 0;
  }
...
int TechInfo::can_research(int nationRecno, int checkLivePoint) {
  ...
  if (checkLivePoint &&
      nation_array[nationRecno]->live_points < live_points_cost())
    return 0;

  if (class_id == TECH_CLASS_MEGA_WEAPON) // cannot research mega weapon
    return 0;

  //--- if this tech is race specific, the nation must have a town of that race
  //in order to be able to research it ---//
...
void TechInfo::set_nation_tech_level(int nationRecno, int techLevel) {
  ...
  if (unit_id &&
      unit_res[unit_id]->get_nation_tech_level(nationRecno) < techLevel)
    unit_res[unit_id]->set_nation_tech_level(nationRecno, techLevel);

  if (firm_id &&
      firm_res[firm_id]->get_nation_tech_level(nationRecno) < techLevel)
  ...
  if (nationPtr->research_tech_id == tech_id) {
    ...
    if (nationPtr->is_ai()) // AI has its own function in thinking about which
...
void TechRes::inc_tech_class_level(int nationRecno, int techClass) {
  int curTechLevel;
  TechInfo *techInfo =
      tech_info_array + tech_class(techClass)->first_tech_id - 1;
...
//
void TechClass::random_init(int nationRecno, int minTechCount,
                            int maxTechCount) {
  ...
  for (int i = 0; i < addCount; i++) {
    ...
    for (int j = 0; j < tech_count; j++) {
      ...
      TechInfo *techInfo =
          tech_res.tech_info_array + first_tech_id + techSubSeq - 1;

      if (techInfo->can_research(nationRecno)) {
        techInfo->set_nation_tech_level(
            nationRecno, techInfo->get_nation_tech_level(nationRecno) + 1);
        break;
      }
    }
  }
...
//							  by this race of the given nation
//is counted.
//
...
//								  (default: 1)
//
int TechRes::researchable_tech_count(int nationRecno, int raceId,
                                     int checkLivePoint) {
  int techCount = 0;
...
short TechInfo::max_tech_level(int nationRecno) {
  // ######### begin Gilbert 14/3 #########//
  //	if( nationRecno && advantage_race_id &&
  //nation_array[nationRecno]->race_id == advantage_race_id ) 		return
  //common_max_tech_level + 1; 	else 		return common_max_tech_level;

  // ----- take care of the current tech level -------//
  // steal or exchange tech can bring non-advantage nation to exceed
  // common_max_tech_level

  if (nationRecno && advantage_race_id &&
      nation_array[nationRecno]->race_id == advantage_race_id)
...
int TechInfo::is_useful_to_nation(int nationRecno) {
  ...
  if (nationPtr->is_human()) {
    ...
    if (race_id > 0) {
      //--- if there is a town of this race, then this tech is useful to this
      //nation ---//
  ...
  } else //---- if this is a monster nation ----//
  {
    ...
    if (unitInfo->nation_unit_count_array[nationRecno - 1] +
            unitInfo->nation_general_count_array[nationRecno - 1] >
        0) {
      return 1;
    } else {
      return 0;
...
int TechRes::suggest_next_tech(int nationRecno, int finishedTechId) {
  ...
  int reachMaxLevel =
      finishedTechLevel >= finishedTechInfo->max_tech_level(nationRecno);

  int techId, techId2;

  // step 1 : find any tech which is dependent on finishedTechId and can start
  // research
  ...
  // if the other requirement is not fullfilled, reseach the other requirement (
  // and its requirement)
  ...
  for (techId = 1; techId <= tech_count; ++techId) {
    ...
    if (finishedTechId == techInfo->parent_tech_id &&
      ...
      if (techInfo->can_research(nationRecno)) {
        return techId;
      } else if ((techId2 = find_researchable_requirement(nationRecno,
                                                          techId)) != 0) {
        return techId2;
      }
    }
  ...
  if (techClassId) {
    ...
    // step 4 : other tech of the same class, but need other requirement
...
//
int TechRes::find_researchable_requirement(int nationRecno, int techId,
                                           int recurCount) {
  TechInfo *techInfo = operator[](techId);
  if (techInfo->can_research(nationRecno))
    return techId;
  ...
  if (techInfo->parent_tech_id && operator[](techInfo->parent_tech_id)
                                      ->can_research(nationRecno))
    return techInfo->parent_tech_id;

  // find parent 2

  if (techInfo->parent_tech_id2 && operator[](techInfo->parent_tech_id2)
                                       ->can_research(nationRecno))
  ...
  if (techInfo->parent_tech_id &&
      (techId2 = find_researchable_requirement(
           nationRecno, techInfo->parent_tech_id, recurCount + 1)) != 0)
    return techId2;
  ...
  if (techInfo->parent_tech_id2 &&
      (techId2 = find_researchable_requirement(
           nationRecno, techInfo->parent_tech_id2, recurCount + 1)) != 0)
    return techId2;
