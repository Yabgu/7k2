...
void TechRes::load_tech_info()
{
    ...
    for (i = 0; i < tech_count; i++)
    {
        ...
        if (techInfo->class_id == TECH_CLASS_SPU_STRUCTURE || techInfo->class_id == TECH_CLASS_SPU_BERSERKER)
        {
...
{
    err_if(techClassId < 1 || techClassId > tech_count) err_now("TechRes::tech_class");
...
int TechInfo::is_parent_tech_invented(int nationRecno)
{
    ...
    if (tech_id == TECH_ALCHEMY_YIELD) // only available for research when all basic alchemy
...
int TechInfo::can_research(int nationRecno, int checkLivePoint)
{
    ...
    return get_nation_tech_level(nationRecno) < max_tech_level(nationRecno) && is_parent_tech_invented(nationRecno);
}
...
short TechInfo::max_tech_level(int nationRecno)
{
    ...
    // common_max_tech_level + 1; 	else 		return
...
int TechInfo::is_useful_to_nation(int nationRecno)
{
    ...
    else //---- if this is a monster nation ----//
    {
        ...
        if (unitInfo->nation_unit_count_array[nationRecno - 1] + unitInfo->nation_general_count_array[nationRecno - 1] >
            0)
        {
            return 1;
        }
...
{
    if (parent_tech_id && tech_res[parent_tech_id]->get_nation_tech_level(nationRecno) == 0)
    {
    ...
    if (parent_tech_id2 && tech_res[parent_tech_id2]->get_nation_tech_level(nationRecno) == 0)
    {
...
int TechRes::suggest_next_tech(int nationRecno, int finishedTechId)
{
    ...
    for (techId = 1; techId <= tech_count; ++techId)
    {
        ...
        if (finishedTechId == techInfo->parent_tech_id && finishedTechLevel >= techInfo->parent_tech_level ||
            finishedTechId == techInfo->parent_tech_id2 && finishedTechLevel >= techInfo->parent_tech_level2)
        {
            if (techInfo->can_research(nationRecno))
            {
                return techId;
    ...
    {
        TechInfo *techInfo = operator[](techId);
        if (finishedTechId == techInfo->parent_tech_id && techInfo->parent_tech_id2 != 0 &&
        ...
        if (finishedTechId == techInfo->parent_tech_id2 && techInfo->parent_tech_id != 0 &&
...
int TechRes::find_researchable_requirement(int nationRecno, int techId, int recurCount)
{
    ...
    if (techInfo->parent_tech_id && operator[](techInfo->parent_tech_id)->can_research(nationRecno))
        return techInfo->parent_tech_id;

    // find parent 2

    if (techInfo->parent_tech_id2 && operator[](techInfo->parent_tech_id2)->can_research(nationRecno))
