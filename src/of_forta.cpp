...
int FirmFort::think_train_archer() {
  if (target_archer_count >
      current_archer_count) // if one is already being trained
    return 0;
...
int FirmFort::ai_recruit(int recruitCount) {
  ...
  //----- if the nation does not have enough food, recruit much less than it is
  //needed ----//
  ...
  if (!nationPtr->ai_has_enough_food()) {
    recruitCount =
        recruitCount * (100 - nationPtr->not_enough_food_index()) / 100;

    if (recruitCount < 1)
      return 0;
  }

  //-- just train 2 units at maximum at a time, to avoid holding up human
  //resources --//
  ...
  if (nationPtr->cash > 0) // training soldiers costs money
  {
    ...
    for (int tryLoop = 1; tryLoop <= 2; tryLoop++) {
      for (int i = 0; i < linked_town_count; i++) {
        ...
        if (townPtr->jobless_population > 0 && townPtr->loyalty < 40) {
          if (townPtr->accumulated_reward_penalty >
              30) // if the reward penalty is too high, do reward
            break;
        ...
        while (townPtr->can_train()) {
          ...
          recruit_soldier(townPtr->town_recno, false,
                          COMMAND_AI); // last 1-force pulling people from the
                                       // town to the firm
  ...
  if (recruitCount > 0)
    nation_array[nation_recno]->add_action(loc_x1, loc_y1, -1, -1,
                                           ACTION_AI_ASSIGN_SOLDIER, FIRM_CAMP,
                                           recruitCount);
