...
int FirmFort::think_train_archer()
{
    ...
    int importanceRating =
        nationPtr->pref_military_development / 3 + (ai_combat_level_needed() - total_combat_level()) / 10;

    if (!nationPtr->ai_should_spend(importanceRating, TRAIN_ARCHER_COST))
        return 0;

    if (target_archer_count == current_archer_count && target_archer_count < MAX_FORT_ARCHER)
    {
        target_archer_count++;
        return 1;
    }
...
int FirmFort::ai_recruit(int recruitCount)
{
    ...
    if (recruitCount > 0)
        nation_array[nation_recno]->add_action(loc_x1, loc_y1, -1, -1, ACTION_AI_ASSIGN_SOLDIER, FIRM_CAMP,
                                               recruitCount);
