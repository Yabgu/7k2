...
static short edit_need_worker_x1, edit_need_worker_y1, edit_need_worker_x2,
    edit_need_worker_y2, edit_need_worker_enable;
...
void FirmWork::disp_worker_list(int dispY1, int refreshFlag) {
  ...
  for (int i = 0; i < MAX_WORKER; i++) {
    if (i < needed_worker_count) {
      if (i < worker_count) {
        ...
        //				{
        //					vga.active_buf->rect( x-4, y+2, x+15,
        //y+32, 1, VGA_LIGHT_BLUE);
        //				}
        //## end trevor 18/11 ##//
      } else {
        if (firm_id == FIRM_BASE)
    ...
    if (i < slide.view_recno && slide.drag_flag == 1) {
      if (firm_id == FIRM_BASE)
        vga.active_buf->put_bitmap_trans(x - 1, y + 6,
                                         image_icon.read("PRAY-OUT"));
      else
        vga.active_buf->put_bitmap_trans(x - 1, y + 2,
                                         image_icon.read("WRKR-OUT"));
    }
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    //							  Y2 - Y1 = button
    //height
    slide.init_slide(INFO_X1 + 7, INFO_Y1 + 220, INFO_X1 + 217, INFO_Y1 + 251,
                     34, disp_slide_bar);
...
void FirmWork::detect_worker_list() {
  ...
  help.set_help(INFO_X1 + 7, INFO_Y1 + 220, INFO_X1 + 217, INFO_Y1 + 251,
                "SLIDER");
...
static void disp_slide_bar(SlideBar *slideBar, int) {
  ...
  vga.active_buf->put_bitmap_trans_remap_decompress(
      INFO_X1, INFO_Y1 + 213, nationPict,
      game.get_color_remap_table(nationRecno, 0));
  if (vga.active_buf->is_front)
    mouse.show_area();
}
...
void FirmWork::disp_edit_mode(int &refreshFlag, int &y, int dispBG) {
  ...
  x = edit_need_worker_x2 =
      font_blue.put((edit_need_worker_x1 = x), (edit_need_worker_y1 = y),
                    needed_worker_count, 0, x2);
...
int FirmWork::detect_edit_mode() {
  ...
  if (edit_need_worker_enable) {
    if (mouse.any_click(edit_need_worker_x1, edit_need_worker_y1,
                        edit_need_worker_x2, edit_need_worker_y2,
                        LEFT_BUTTON)) {
