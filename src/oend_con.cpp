...
EndConditionInfo EndConditionInfo::end_condition_info[END_CONDITION_COUNT] = {
    {E_DEFEAT_ALL_OTHER_KINGDOMS, E_DEFEAT_OWN_KINGDOM, EW_WIN_ONLY, EP_NONE,
     EP_NONE, ET_OPTIONAL, "Defeat all other Kingdoms"},
    {E_DEFEAT_OWN_KINGDOM, E_DEFEAT_ALL_OTHER_KINGDOMS, EW_LOSE_ONLY, EP_NONE,
     EP_NONE, ET_OPTIONAL, "Your Kingdom is defeated"},
    {E_DEFEAT_KINGDOM, -1, EW_WIN_ONLY, EP_NATION_RECNO, EP_NONE, ET_OPTIONAL,
     "Defeat a certain Kingdom"},
    {E_FORM_ALLY_WITH_KINGDOM, -1, EW_WIN_ONLY, EP_NATION_RECNO, EP_NONE,
     ET_OPTIONAL, "Ally with a certain Kingdom"},
    {E_DEFEAT_ALL_FRYHTAN, -1, EW_WIN_ONLY, EP_NONE, EP_NONE, ET_OPTIONAL,
     "Defeat all Fryhtan"},
    {E_DEFEAT_EXCEPT_ALLIANCE, -1, EW_WIN_ONLY, EP_NONE, EP_NONE, ET_NONE,
     "Defeat all except one allied Kingdom"},
    {E_REACH_POPULATION, -1, EW_WIN_ONLY, EP_NONE, EP_0_10000, ET_OPTIONAL,
     "Population exceed a number"},
    {E_REACH_FRYHTAN, -1, EW_WIN_ONLY, EP_NONE, EP_0_10000, ET_OPTIONAL,
     "Fryhtan population exceed a number"},
    {E_REACH_UNIT_COUNT, -1, EW_WIN_ONLY, EP_UNIT_ID, EP_0_1000, ET_OPTIONAL,
     "Number of a unit exceed a number"},
    {E_REACH_ECONOMY, -1, EW_WIN_ONLY, EP_NONE, EP_0_10000, ET_OPTIONAL,
     "Economic score exceeds a level"},
    {E_REACH_MILITARY, -1, EW_WIN_ONLY, EP_NONE, EP_0_10000, ET_OPTIONAL,
     "Military score exceeds a level"},
    {E_REACH_TOTAL, -1, EW_WIN_ONLY, EP_NONE, EP_0_10000, ET_OPTIONAL,
     "Total score exceeds a level"},
    {E_KILL_MONSTER_SCORE, -1, EW_WIN_ONLY, EP_NONE, EP_0_10000, ET_OPTIONAL,
     "Fryhtan battling score exceeds a level"},
    {E_TRAIN_10_GENERAL, -1, EW_WIN_ONLY, EP_NONE, EP_0_100, ET_OPTIONAL,
     "Train 10 Generals"},
    {E_KILL_UNIT, E_KEEP_UNIT_ALIVE, EW_WIN_LOSE, EP_UNIT_RECNO, EP_NONE,
     ET_OPTIONAL, "A certain unit is killed"},
    {E_KEEP_UNIT_ALIVE, E_KILL_UNIT, EW_WIN_LOSE, EP_UNIT_RECNO, EP_NONE,
     ET_COMPULSORY, "A certain unit survives"},
    {E_MEET_UNIT, -1, EW_WIN_ONLY, EP_UNIT_RECNO, EP_NONE, ET_OPTIONAL,
     "Meet a certain unit"},
    {E_CONQUER_ALL_TOWN, -1, EW_WIN_ONLY, EP_NONE, EP_NONE, ET_OPTIONAL,
     "Conquer all indepedent towns"},
    {E_CONQUER_TOWN, -1, EW_WIN_LOSE, EP_TOWN_RECNO, EP_NONE, ET_OPTIONAL,
     "Conquer a certain town"},
    {E_DESTROY_FIRM, -1, EW_WIN_LOSE, EP_FIRM_RECNO, EP_NONE, ET_OPTIONAL,
     "Destroy a certain building"},
    {E_GET_SCROLL, -1, EW_WIN_ONLY, EP_SITE_RECNO, EP_NONE, ET_OPTIONAL,
     "Get ancient scroll"},
    {E_ACQUIRE_ITEM, -1, EW_WIN_ONLY, EP_ITEM_ID, EP_NONE, ET_OPTIONAL,
     "Acquire a certain item"},
    {E_NATION_SURVIVE, -1, EW_WIN_LOSE, EP_NONE, EP_0_100, ET_NONE,
     "Your kingdom survives"},
    // ######### begin Gilbert 2/3 ########//
    {E_ACQUIRE_TECH, -1, EW_WIN_ONLY, EP_TECH_ID, EP_NONE, ET_OPTIONAL,
     "Acquire technology"},
    // ######### end Gilbert 2/3 ########//
    {E_CONQUER_ALL_FIRM, -1, EW_WIN_ONLY, EP_NONE, EP_NONE, ET_OPTIONAL,
     "Conquer all firms"},
    {E_CONQUER_FIRM, -1, EW_WIN_LOSE, EP_FIRM_RECNO, EP_NONE, ET_OPTIONAL,
     "Conquer a certain firm"},
...
//
void EndCondition::init(int condId, int playerOnly, int p1, int p2,
                        int yearLimit, int loseCond) {
  condition_id = condId;
  ...
  // win condition
  err_when(!loseCond &&
           !(EndConditionInfo::end_condition_info[condition_id].win_or_lose &
             EW_WIN_ONLY));
  // loss condition
  err_when(loseCond &&
           !(EndConditionInfo::end_condition_info[condition_id].win_or_lose &
...
void EndCondition::set_condition_str(char *s, int loseCond) {
  ...
  case E_DEFEAT_KINGDOM: // Defeat <nation name>
    str = text_end_condition.process(
        text_end_condition.get_format_str(35, "EC02"),
        nation_array.is_deleted(para1) ? "_"
                                       : nation_array[para1]->nation_name());
    break;
  case E_FORM_ALLY_WITH_KINGDOM: // Ally with <nation name>
    str = text_end_condition.process(
        text_end_condition.get_format_str(36, "EC03"),
        nation_array.is_deleted(para1) ? "_"
                                       : nation_array[para1]->nation_name());
    break;
  ...
  case E_REACH_POPULATION: // Population reaches <number>
    str = text_end_condition.process(
        text_end_condition.get_format_str(39, "EC06"), para2);
    break;
  case E_REACH_FRYHTAN: // Fryhtan population reaches <number>
    str = text_end_condition.process(
        text_end_condition.get_format_str(40, "EC07"), para2);
    break;
  case E_REACH_UNIT_COUNT: // Own <number> of <unit name>(s)
    str = text_end_condition.process(
        text_end_condition.get_format_str(41, "EC08"), unit_res[para1]->name,
        para2);
    break;
  case E_REACH_ECONOMY: // Economic score reaches <number>
    str = text_end_condition.process(
        text_end_condition.get_format_str(42, "EC09"), para2);
    break;
  case E_REACH_MILITARY: // Military score reaches <number>
    str = text_end_condition.process(
        text_end_condition.get_format_str(43, "EC10"), para2);
    break;
  case E_REACH_TOTAL: // Total score reaches <number>
    str = text_end_condition.process(
        text_end_condition.get_format_str(44, "EC11"), para2);
    break;
  case E_KILL_MONSTER_SCORE: // Fryhtan battling score reaches <number>
    str = text_end_condition.process(
        text_end_condition.get_format_str(45, "EC12"), para2);
    break;
  case E_TRAIN_10_GENERAL: // Train 10 Generals of Leadership <number> or above
    str = text_end_condition.process(
        text_end_condition.get_format_str(46, "EC13"), para2);
    break;
  case E_KILL_UNIT: // Kill <unit name> of <nation name> / <unit name> of
                    // <nation name> get killed
    ...
    if (!loseCond) {
      ...
      if (nationRecno == 0) {
        str = text_end_condition.process(
            text_end_condition.get_format_str(49, "cC14"),
            unit_array[para1]->unit_name());
      } else if (nationRecno == nation_array.player_recno) {
        str = text_end_condition.process(
            text_end_condition.get_format_str(48, "bC14"),
            unit_array[para1]->unit_name(),
            text_unit.str_nation((~nation_array)->race_id));
      } else {
        str = text_end_condition.process(
            text_end_condition.get_format_str(47, "aC14"),
            unit_array[para1]->unit_name(),
            nation_array[nationRecno]->nation_name());
      }
    } else {
      int nationRecno = unit_array[para1]->nation_recno;
      if (nationRecno == 0) {
        str = text_end_condition.process(
            text_end_condition.get_format_str(52, "fC14"),
            unit_array[para1]->unit_name());
      } else if (nationRecno == nation_array.player_recno) {
        str = text_end_condition.process(
            text_end_condition.get_format_str(51, "eC14"),
            unit_array[para1]->unit_name(),
            text_unit.str_nation((~nation_array)->race_id));
      } else {
        str = text_end_condition.process(
            text_end_condition.get_format_str(50, "dC14"),
            unit_array[para1]->unit_name(),
            nation_array[nationRecno]->nation_name());
      }
    }
    break;
  case E_KEEP_UNIT_ALIVE: // Rescue <unit name> of <nation name> / Failed to
                          // kill <unit name> of <nation name>
    // BUGHERE : para1 str may no longer exist
    err_when(unit_array.is_truly_deleted(para1));
    if (!loseCond) {
      int nationRecno = unit_array[para1]->nation_recno;
      if (nationRecno == 0) {
        str = text_end_condition.process(
            text_end_condition.get_format_str(55, "cC15"),
            unit_array[para1]->unit_name());
      } else if (nationRecno == nation_array.player_recno) {
        str = text_end_condition.process(
            text_end_condition.get_format_str(54, "bC15"),
            unit_array[para1]->unit_name(),
            text_unit.str_nation((~nation_array)->race_id));
      } else {
        str = text_end_condition.process(
            text_end_condition.get_format_str(53, "aC15"),
            unit_array[para1]->unit_name(),
            nation_array[nationRecno]->nation_name());
      }
    } else {
      ...
      if (nationRecno == 0) {
        str = text_end_condition.process(
            text_end_condition.get_format_str(58, "fC15"),
            unit_array[para1]->unit_name());
      } else if (nationRecno == nation_array.player_recno) {
        str = text_end_condition.process(
            text_end_condition.get_format_str(57, "eC15"),
            unit_array[para1]->unit_name(),
            text_unit.str_nation((~nation_array)->race_id));
      } else {
        str = text_end_condition.process(
            text_end_condition.get_format_str(56, "dC15"),
            unit_array[para1]->unit_name(),
            nation_array[nationRecno]->nation_name());
      }
    }
  ...
  case E_MEET_UNIT: // Meet <unit name> of <nation name>
    ...
    err_when(unit_array.is_truly_deleted(para1));
    {
      ...
      if (nationRecno == 0) {
        str = text_end_condition.process(
            text_end_condition.get_format_str(61, "cC16"),
            unit_array[para1]->unit_name());
      } else if (nationRecno == nation_array.player_recno) {
        str = text_end_condition.process(
            text_end_condition.get_format_str(60, "bC16"),
            unit_array[para1]->unit_name(),
            text_unit.str_nation((~nation_array)->race_id));
      } else {
        str = text_end_condition.process(
            text_end_condition.get_format_str(59, "aC16"),
            unit_array[para1]->unit_name(),
            nation_array[nationRecno]->nation_name());
      }
    }
  ...
  case E_CONQUER_TOWN: // Conquer town <town name>
    ...
    str = text_end_condition.process(
        text_end_condition.get_format_str(63, "EC18"),
  ...
  case E_DESTROY_FIRM: // Destroy <building name>
    ...
    str = text_end_condition.process(
        text_end_condition.get_format_str(64, "EC19"),
  ...
  case E_ACQUIRE_ITEM: // Acquire item <item name>
    str = text_end_condition.process(
        text_end_condition.get_format_str(66, "EC21"),
        item_res.item_name(para1, 0));
    break;
  case E_NATION_SURVIVE: // Your kingdom survives for <number> year(s) / Your
                         // kingdom cannot win in <number> year(s)
    if (!loseCond) {
      str = text_end_condition.process(
          text_end_condition.get_format_str(67, "EC22"),
          text_unit.str_nation((~nation_array)->race_id), para2);
    } else {
      str = text_end_condition.process(
          text_end_condition.get_format_str(71, "aC22"),
          text_unit.str_nation((~nation_array)->race_id), para2);
    }
    break;
  case E_ACQUIRE_TECH: // Acquire <technology name> technology
    str = text_end_condition.process(
        text_end_condition.get_format_str(68, "EC23"),
  ...
  case E_CONQUER_FIRM: // Conquer firm <firm name>
    ...
    err_when(firm_array.is_deleted(para1));
    str = text_end_condition.process(
        text_end_condition.get_format_str(70, "EC25"),
        firm_array[para1]->firm_name());
    break;
  ...
  if (time_limit) {
    ...
    str = text_end_condition.process(
        text_end_condition.get_format_str(77, "ECTM"), (char *)str,
        time_limit); // cast String type explicitly in variable argument
  }
...
int EndCondition::is_condition_true(int nationRecno) {
  ...
  case E_DEFEAT_OWN_KINGDOM:
    return !nationPtr || (nationPtr->king_unit_recno == 0 &&
  ...
  case E_DEFEAT_ALL_FRYHTAN: {
    ...
    for (i = firm_array.size(); i > 0; i--) {
      ...
      if (firmPtr->nation_recno == 0 &&
          firmPtr->firm_id == FIRM_LAIR) // independent lair
        return 0;
    }
        ...
        for (int y = scanY1; y <= scanY2; ++y) {
          ...
          for (int x = scanX1; x <= scanX2; ++x, ++locPtr) {
            ...
            if ((touchUnitRecno = locPtr->unit_recno(UNIT_AIR)) &&
                    unit_array[touchUnitRecno]->nation_recno ==
                        nationRecno // avoid two nation win together
                ||
  ...
  case E_CONQUER_ALL_TOWN: // Conquer all independent towns
  {
    for (int townRecno = town_array.size(); townRecno > 0; --townRecno) {
      // if there exist any non-deleted independent towns which is not salve
      // town, return FALSE
  ...
  case E_CONQUER_TOWN: // Conquer town <town name>
    if (town_array.is_deleted(para1))
      para1 = 0; // clear para1 so any new town of the same town recno will
  ...
  case E_GET_SCROLL: // Acquire ancient scroll
    if (site_array.is_deleted(para1))
      para1 = 0; // clear para1 so any new site of same recno will never meet
                 // the condition
  ...
  case E_NATION_SURVIVE:
    // BUGHERE : not ready
    return nationPtr && info.game_date - info.game_start_date > para2 * 365;
  ...
  case E_CONQUER_ALL_FIRM: // Conquer all independent firms
  {
    for (int firmRecno = firm_array.size(); firmRecno > 0; --firmRecno) {
      // if there exist any non-deleted firm which does not belong to the
      // player,
  ...
  case E_CONQUER_FIRM: // Conquer firm <firm name>
    if (firm_array.is_deleted(para1))
      para1 = 0; // clear para1 so any new firm of the same town recno will
                 // never meet the condition
    else
...
void EndCondition::set_result_str(char *s, int loseCond) {
  ...
  // even if non parameter, still process the format string to reduce %%1s to
  // %1s
  switch (condition_id) {
  case E_DEFEAT_ALL_OTHER_KINGDOMS: // has Defeated All Other Kingdoms
    str += text_end_condition.process(
        text_end_condition.get_format_str(78, "ER00"));
    break;
  case E_DEFEAT_OWN_KINGDOM: // has been Defeated
    str += text_end_condition.process(
        text_end_condition.get_format_str(79, "ER01"));
    break;
  case E_DEFEAT_KINGDOM: // has Defeated <nation name>
    str += text_end_condition.process(
        text_end_condition.get_format_str(80, "ER02"),
        nation_array[para1]->nation_name());
    break;
  case E_FORM_ALLY_WITH_KINGDOM: // has Allied with <nation name>
    str += text_end_condition.process(
        text_end_condition.get_format_str(81, "ER03"),
        nation_array[para1]->nation_name());
    break;
  case E_DEFEAT_ALL_FRYHTAN: // has Defeated all Fryhtan Lairs
    str += text_end_condition.process(
        text_end_condition.get_format_str(82, "ER04"));
    break;
  case E_DEFEAT_EXCEPT_ALLIANCE: // has Defeated all except one Allied Kingdom
    str += text_end_condition.process(
        text_end_condition.get_format_str(83, "ER05"));
    break;
  case E_REACH_POPULATION: // reaches Population of <number>
    str += text_end_condition.process(
        text_end_condition.get_format_str(84, "ER06"), para2);
    break;
  case E_REACH_FRYHTAN: // reaches Fryhtan Population of <number>
    str += text_end_condition.process(
        text_end_condition.get_format_str(85, "ER07"), para2);
    break;
  case E_REACH_UNIT_COUNT: // owns <number> <unit name>(s)
    str += text_end_condition.process(
        text_end_condition.get_format_str(86, "ER08"), unit_res[para1]->name,
        para2);
    break;
  case E_REACH_ECONOMY: // reaches Economy Score of <number>
    str += text_end_condition.process(
        text_end_condition.get_format_str(87, "ER09"), para2);
    break;
  case E_REACH_MILITARY: // reaches Military Score of <number>
    str += text_end_condition.process(
        text_end_condition.get_format_str(88, "ER10"), para2);
    break;
  case E_REACH_TOTAL: // reaches Total Score of <number>
    str += text_end_condition.process(
        text_end_condition.get_format_str(89, "ER11"), para2);
    break;
  case E_KILL_MONSTER_SCORE: // reaches Fryhtan Battling Score of <number>
    str += text_end_condition.process(
        text_end_condition.get_format_str(90, "ER12"), para2);
    break;
  case E_TRAIN_10_GENERAL: // has Trained 10 Generals of Leadership <number> or
                           // above
    str += text_end_condition.process(
        text_end_condition.get_format_str(91, "ER13"), para2);
    break;
  case E_KILL_UNIT: // has killed <unit name> of <nation name> / failed to
                    // protect <unit name> of <nation name>
    // BUGHERE : para1 str may no longer exist
    ...
    if (!loseCond) {
      ...
      if (nationRecno == 0) {
        str += text_end_condition.process(
            text_end_condition.get_format_str(94, "cR14"),
            unit_array[para1]->unit_name());
      } else if (nationRecno == nation_array.player_recno) {
        str += text_end_condition.process(
            text_end_condition.get_format_str(93, "bR14"),
            unit_array[para1]->unit_name(),
            text_unit.str_nation((~nation_array)->race_id));
      } else {
        str += text_end_condition.process(
            text_end_condition.get_format_str(92, "aR14"),
            unit_array[para1]->unit_name(),
            nation_array[nationRecno]->nation_name());
      }
    } else {
      ...
      if (nationRecno == 0) {
        str += text_end_condition.process(
            text_end_condition.get_format_str(97, "fR14"),
            unit_array[para1]->unit_name());
      } else if (nationRecno == nation_array.player_recno) {
        str += text_end_condition.process(
            text_end_condition.get_format_str(96, "eR14"),
            unit_array[para1]->unit_name(),
            text_unit.str_nation((~nation_array)->race_id));
      } else {
        str += text_end_condition.process(
            text_end_condition.get_format_str(95, "dR14"),
            unit_array[para1]->unit_name(),
            nation_array[nationRecno]->nation_name());
      }
    }
    break;
  case E_KEEP_UNIT_ALIVE: // has Protected <unit name> of <nation name> / has
                          // Failed to Kill <unit name> of <nation name>
    // BUGHERE : para1 str may no longer exist
    if (unit_array.is_truly_deleted(para1)) {
      err_here();
      return;
    ...
    if (!loseCond) {
      ...
      if (nationRecno == 0) {
        str += text_end_condition.process(
            text_end_condition.get_format_str(100, "cR15"),
            unit_array[para1]->unit_name());
      } else if (nationRecno == nation_array.player_recno) {
        str += text_end_condition.process(
            text_end_condition.get_format_str(99, "bR15"),
            unit_array[para1]->unit_name(),
            text_unit.str_nation((~nation_array)->race_id));
      } else {
        str += text_end_condition.process(
            text_end_condition.get_format_str(98, "aR15"),
            unit_array[para1]->unit_name(),
            nation_array[nationRecno]->nation_name());
      }
    } else {
      ...
      if (nationRecno == 0) {
        str += text_end_condition.process(
            text_end_condition.get_format_str(103, "fR15"),
            unit_array[para1]->unit_name());
      } else if (nationRecno == nation_array.player_recno) {
        str += text_end_condition.process(
            text_end_condition.get_format_str(102, "eR15"),
            unit_array[para1]->unit_name(),
            text_unit.str_nation((~nation_array)->race_id));
      } else {
        str += text_end_condition.process(
            text_end_condition.get_format_str(101, "dR15"),
            unit_array[para1]->unit_name(),
            nation_array[nationRecno]->nation_name());
      }
    }
  ...
  case E_MEET_UNIT: // has Met <unit name> of <nation name>
    ...
    }
    {
      ...
      if (nationRecno == 0) {
        str += text_end_condition.process(
            text_end_condition.get_format_str(106, "cR16"),
            unit_array[para1]->unit_name());
      } else if (nationRecno == nation_array.player_recno) {
        str += text_end_condition.process(
            text_end_condition.get_format_str(105, "bR16"),
            unit_array[para1]->unit_name(),
            text_unit.str_nation((~nation_array)->race_id));
      } else {
        str += text_end_condition.process(
            text_end_condition.get_format_str(104, "aR16"),
            unit_array[para1]->unit_name(),
            nation_array[nationRecno]->nation_name());
      }
    }
  ...
  case E_CONQUER_ALL_TOWN: // has Conquered All Independent Towns
    str += text_end_condition.process(
        text_end_condition.get_format_str(107, "ER17"));
    break;
  case E_CONQUER_TOWN: // has Conquered Town <town name>
    ...
    str += text_end_condition.process(
        text_end_condition.get_format_str(108, "ER18"),
        town_array[para1]->town_name());
    break;
  case E_DESTROY_FIRM: // has Destroyed <building name>
    ...
    str += text_end_condition.process(
        text_end_condition.get_format_str(109, "ER19"),
        firm_array[para1]->firm_name());
    break;
  case E_GET_SCROLL: // Acquire the Ancient Scroll
    str += text_end_condition.process(
        text_end_condition.get_format_str(110, "ER20"));
    break;
  case E_ACQUIRE_ITEM: // has Acquired Item <item name>
    str += text_end_condition.process(
        text_end_condition.get_format_str(111, "ER21"),
        item_res.item_name(para1, 0));
    break;
  case E_NATION_SURVIVE: // has Survived for <number> Year(s) / cannot Win in
                         // <number> Year(s)
    if (!loseCond) {
      str += text_end_condition.process(
          text_end_condition.get_format_str(112, "ER22"), para2);
    } else {
      str += text_end_condition.process(
          text_end_condition.get_format_str(116, "aR22"), para2);
    }
    break;
  case E_ACQUIRE_TECH:
    str += text_end_condition.process(
        text_end_condition.get_format_str(113, "ER23"),
        tech_res[para1]->tech_des());
    break;
  case E_CONQUER_ALL_FIRM: // has Conquered All Firms
    str += text_end_condition.process(
        text_end_condition.get_format_str(114, "ER24"));
    break;
  case E_CONQUER_FIRM: // has Conquered Firm <firm name>
    ...
    str += text_end_condition.process(
        text_end_condition.get_format_str(115, "ER25"),
        firm_array[para1]->firm_name());
    break;
...
//
int EndCondition::update_condition_on_mobilize(int newUnitRecno,
                                               long uniqueId) {
  // ------- set unique unit id ------//
...
int EndCondition::update_condition_site_taken(int siteRecno, int nationRecno) {
  ...
  if (EndConditionInfo::end_condition_info[condition_id].para1_type ==
          EP_SITE_RECNO &&
      para1 == siteRecno && !para1_nation_took) {
...
// -------//
//
int EndCondition::update_condition_item_acquired(short itemId,
                                                 int nationRecno) {
  // ------- set unique unit id ------//

  if (EndConditionInfo::end_condition_info[condition_id].para1_type ==
          EP_ITEM_ID &&
      para1 == itemId && !para1_nation_took) {
    para1_nation_took = nationRecno;
    return 1;
  }
