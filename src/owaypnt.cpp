...
WayPoint::WayPoint() : DynArray(sizeof(OneWayPoint), 20) {
  ...
  for (int i = cur_unit_group.size(); i > 0; i--) {
    cur_unit_group.get_unit(i)->has_way_point =
        true; // it will be reset to false in Unit::stop_move()
  }
}
...
//
void WayPoint::del(int delRecno) { linkout(delRecno); }
...
//					  0 - all way points are finished, delete this
//WayPoint.
//
int WayPoint::process() {
  ...
  for (i = cur_unit_group.size(); i > 0; i--) {
    ...
    //--- if the unit was ordered to do something else, remove it from
    //cur_unit_group. (has_way_point is reset in UnitGroup::right_click() )
    //---//
  ...
  //--- if all units in the group has stopped, order them to move to new
  //location ----//

  if (size() > 0 && cur_unit_group.size() > 0) {
    OneWayPoint *oneWayPoint = get(1);

    cur_unit_group.right_click(oneWayPoint->loc_x, oneWayPoint->loc_y,
                               COMMAND_AUTO, false, false, false);

    del(1); // remove the one way point after successfully order the units to
            // move

    return 1;
  } else //-- no more way points and all units have stopped, now remove the
         //WayPoint
  {
    return 0;
  }
...
void WayPoint::draw(int drawMode) {
  ...
  for (i = 0; i <= size(); i++) {
    if (i > 0) // when i==0, we use the original destXLoc & destYLoc set in the
               // above code
    {
    ...
    if (drawMode == 2) // draw on the zoom map
    {
      ...
      world.zoom_matrix->put_bitmap_clip(destX + chOffsetX, destY + chOffsetY,
                                         chPtr);
    }
