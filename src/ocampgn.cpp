...
Campaign::Campaign() { memset(this, 0, sizeof(Campaign)); }
...
void Campaign::init(int campaignId) {
  ...
  shadow_array = (short *)mem_resize(
      shadow_array, MAIN_TERRAIN_WIDTH * MAIN_TERRAIN_HEIGHT * sizeof(short));
  memset(shadow_array, 0,
...
void Campaign::set_cur_situation(int situationId) {
  ...
  memcpy(&cur_situation, campaign_situation_array + situationId - 1,
         sizeof(CampaignSituation));
...
void Campaign::init_new_campaign() {
  ...
  init_uniqueness();      // a Battle function, call functions like
                          // hero_res.init_for_hire()
  // ######## end Gilbert 9/4 ##########//
  ...
  for (i = 0; i < MAIN_TERRAIN_WIDTH; i++) {
    for (int j = 0; j < MAIN_TERRAIN_HEIGHT; j++) {
      ...
      if (height > prevHeight)
        shadow_array[j * MAIN_TERRAIN_WIDTH + i] =
...
unsigned long Campaign::random(unsigned long maxNum) {
  err_if(maxNum < 0 || maxNum > 0x7FFF) err_now("Campaign::random()");
...
//					  0 if failure, it occurs if out of usable
//colors.
//
...
//
// <bool> isMonster - true if we want to get monster id. instead of human race
// id.
//
...
void Campaign::state_change_nation(int stateRecno, int newNationRecno) {
  ...
  //---- check if the nation still has any states left, if not, remove the
  //nation ----//
  ...
  {
    err_when(oldNationRecno ==
             CAMPAIGN_PLAYER_NATION_RECNO); // this should not happen
...
//										will be excluded
//from the player state list. 										(default: 0)
//
// [int] nextToNationRecno - if this is true, then a monster nation
//										will be picked
//regardless of whether it is 										next to the player nation or not. 										(default:
//CAMPAIGN_PLAYER_NATION_RECNO)
//
// [int] mustNextTo - if this is 1, then the returning state must be next
//							 to nextToNationRecno (default:
//0)
//
int Campaign::random_pick_monster_campaign_nation(int excludedStateRecno,
                                                  int nextToNationRecno,
                                                  int mustNextTo) {
  ...
  if (monsterNationCount == 0 && !mustNextTo) {
    //-- if there are still monster nations but none of them is next to the
    //player's nation, then still attack them. --//
    ...
    if (anyMonsterNationCount > 0) {
      for (i = state_array.size(); i > 0; i--) {
        ...
        if (campaignNationRecno &&
            get_nation(campaignNationRecno)->is_monster())
          return i;
      }
    }
...
//												will
//be excluded from the player state list.
//
...
//								  is selected.
//
int Campaign::random_pick_attack_state(int attackNationRecno,
                                       int targetNationRecno,
                                       int excludedTargetStateRecno) {
...
int Campaign::random_pick_town_with_camp(int nationRecno, int maxLink) {
  ...
  for (int i = town_array.size(); i > 0;
       i--) // locate a town closest to the mine
  {
    ...
    if (townPtr->nation_recno == nationRecno && townPtr->has_linked_own_camp &&
...
void Campaign::set_king(int nationRecno, int kingUnitRecno, int firstKing) {
  ...
  if (nationPtr->campaign_nation_recno) // this nation can be one that does not
...
//
void Campaign::set_relation_status(int nationRecno1, int nationRecno2,
                                   int newStatus) {
...
//
// <int> x, y 				- the location in mountain_layout to set
// to 1. [int] totalX, totalY - if this is given, the x, y given is in this
// scale and
//								  it will be converted to the
//scale of mountain_layout.
//
...
void Campaign::set_campaign_difficulty(int campaignDifficulty) {
  ...
  config.ai_aggressiveness =
      campaign_difficulty -
      MIN_CAMPAIGN_DIFFICULTY; // config.ai_aggressive starts at 0 (Very Low)
}
