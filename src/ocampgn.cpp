...
void Campaign::init(int campaignId)
{
    ...
    shadow_array = (short *)mem_resize(shadow_array, MAIN_TERRAIN_WIDTH * MAIN_TERRAIN_HEIGHT * sizeof(short));
...
//					  0 if failure, it occurs if out of
// usable
// colors.
//
...
// next to the player nation or not. (default: CAMPAIGN_PLAYER_NATION_RECNO)
//
// [int] mustNextTo - if this is 1, then the returning state must be next
//							 to nextToNationRecno
//(default:
// 0)
//
...
int Campaign::random_pick_attack_state(int attackNationRecno, int targetNationRecno, int excludedTargetStateRecno)
{
    ...
    for (i = 1; i <= state_array.size(); i++)
    {
        ...
        if (attackNationRecno && stateInfo->campaign_nation_recno != attackNationRecno)
        {
            continue;
        }
        ...
        {
            if (stateInfo->is_adjacent(j) && state_array[j]->campaign_nation_recno == targetNationRecno &&
...
void Campaign::set_campaign_difficulty(int campaignDifficulty)
{
    ...
    config.ai_aggressiveness =
        campaign_difficulty - MIN_CAMPAIGN_DIFFICULTY; // config.ai_aggressive starts at 0 (Very Low)
}
