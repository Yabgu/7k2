...
void CampaignEastWest::stage_2_init_vars() {
  ...
  next_monster_reinforcement_date =
      info.game_date + (MAX_CAMPAIGN_DIFFICULTY - campaign_difficulty + 1) * 20;
  ...
  game.add_win_condition(
      E_KILL_MONSTER_SCORE, 1, 0,
      goal_kill_monster_score); // 1-this condition only applies to the player.
}
...
int CampaignEastWest::stage_2_create_town() {
  ...
  if (misc.random(2) == 0) // divided horizontally
  {
    ...
    for (int i = 0; i < TOWN_STATE_MAP_HEIGHT; i++) {
      ...
      set_on_mountain_layout(mountainX, i, TOWN_STATE_MAP_WIDTH,
                             TOWN_STATE_MAP_HEIGHT);
      set_on_mountain_layout(mountainX + 1, i, TOWN_STATE_MAP_WIDTH,
                             TOWN_STATE_MAP_HEIGHT);
      set_on_mountain_layout(mountainX + 2, i, TOWN_STATE_MAP_WIDTH,
                             TOWN_STATE_MAP_HEIGHT);
    }
  } else // divided vertically
  {
    ...
    for (int i = 0; i < TOWN_STATE_MAP_WIDTH; i++) {
      ...
      set_on_mountain_layout(i, mountainY, TOWN_STATE_MAP_WIDTH,
                             TOWN_STATE_MAP_HEIGHT);
      set_on_mountain_layout(i, mountainY + 1, TOWN_STATE_MAP_WIDTH,
                             TOWN_STATE_MAP_HEIGHT);
      set_on_mountain_layout(i, mountainY + 2, TOWN_STATE_MAP_WIDTH,
                             TOWN_STATE_MAP_HEIGHT);
    }
  }
  ...
  for (int i = townStateArray.size(); i > 0; i--) {
    ...
    if (stateInfo->center_x >= rangeX1 && stateInfo->center_x <= rangeX2 &&
        stateInfo->center_y >= rangeY1 && stateInfo->center_y <= rangeY2) {
...
void CampaignEastWest::stage_2_process_game_result() {
  ...
  if (game.result_win_condition_id >= 0) {
    ...
    //-- although the player survive, the Fryhtan will eventually win and take
    //over the state --//
...
#ifdef DEBUG
      err.run("Win id. %d unidentified, stage: %d",
              game.result_win_condition_id, stage_id);
#endif
    }
  } else {
