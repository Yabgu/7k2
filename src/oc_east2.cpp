...
enum
{
    STAGE_2_MONSTER_NATION_RECNO = 2
};
...
void CampaignEastWest::stage_2_init_vars()
{
    ...
    game.add_win_condition(E_KILL_MONSTER_SCORE, 1, 0,
...
int CampaignEastWest::stage_2_create_town()
{
    ...
    for (int i = townStateArray.size(); i > 0; i--)
    {
        ...
        if (stateInfo->center_x >= rangeX1 && stateInfo->center_x <= rangeX2 && stateInfo->center_y >= rangeY1 &&
        ...
        else
        {
            if (monsterLairAddCount > 0)
            {
                ...
                if (create_monster_lair(xLoc, yLoc, STAGE_2_MONSTER_NATION_RECNO, soldierCount, soldierCombatLevel))
                {
                    monsterLairAddCount--;
                }
            }
