...
#define RTRIM_ARRAY(a, s)                                                      \
  {                                                                            \
    if (s < sizeof(a) / sizeof(*a))                                            \
      memset(a + s, 0, sizeof(a) - s * sizeof(*a));                            \
  }
...
void Unit::clear_ptr() {
  ...
  // ######## begin Gilbert 8/3 #######//
  if (max_power <=
      0) // max_power is used for display money granted to the local player
    max_power = 0;
  // ######## end Gilbert 8/3 #######//
...
void UnitGod::clear_ptr() { Unit::clear_ptr(); }
...
void UnitMonster::clear_ptr() { Unit::clear_ptr(); }
...
void UnitExpCart::clear_ptr() { Unit::clear_ptr(); }
...
void UnitMarine::clear_ptr()
{
        ...
        // must do this step before clear_ptr(), attack_info_array is reset
there if( !attack_info_array ) memset(&ship_attack_info, 0,
sizeof(ship_attack_info));
...
void Place::clear_ptr() { BaseObj::clear_ptr(); }
...
void FirmLair::clear_ptr() { FirmCamp::clear_ptr(); }
...
void FirmMine::clear_ptr() { FirmWork::clear_ptr(); }
...
void FirmFactory::clear_ptr() { FirmWork::clear_ptr(); }
...
void FirmResearch::clear_ptr() { FirmWork::clear_ptr(); }
...
void FirmMarket::clear_ptr() {
  ...
  for (i = 0; i < sizeof(market_goods_array) / sizeof(*market_goods_array);
       ++i) {
    if (!market_goods_array[i].raw_id &&
        !market_goods_array[i].product_raw_id) {
...
void FirmSpy::clear_ptr() { FirmTrain::clear_ptr(); }
...
void FirmSpecial::clear_ptr() { FirmTrain::clear_ptr(); }
...
void FirmLishorr::clear_ptr() { Firm::clear_ptr(); }
//----------- End of function FirmLishorr::clear_ptr -----------//

//----------- Begin of function FirmOffensive2::crc8 -----------//
UCHAR FirmOffensive2::crc8() {
  FirmOffensive2 &dummyFirmOffensive2 =
      *(FirmOffensive2 *)temp_obj.firm_offensive2;
...
void FirmOffensive2::clear_ptr() { Firm::clear_ptr(); }
//----------- End of function FirmOffensive2::clear_ptr -----------//

//----------- Begin of function FirmMonsterTrain::crc8 -----------//
UCHAR FirmMonsterTrain::crc8() {
  FirmMonsterTrain &dummyFirmMonsterTrain =
      *(FirmMonsterTrain *)temp_obj.firm_monster_train;
...
UCHAR FirmMonsterAlchemy::crc8() {
  FirmMonsterAlchemy &dummyFirmMonsterAlchemy =
      *(FirmMonsterAlchemy *)temp_obj.firm_monster_alchemy;
  ...
  *((char **)&dummyFirmMonsterAlchemy) = NULL;
  ...
  UCHAR c =
      ::crc8((UCHAR *)&dummyFirmMonsterAlchemy, sizeof(FirmMonsterAlchemy));
  return c;
}
//----------- End of function FirmMonsterAlchemy::crc8 -----------//

//----------- Begin of function FirmMonsterAlchemy::clear_ptr -----------//
void FirmMonsterAlchemy::clear_ptr() { Firm::clear_ptr(); }
//----------- End of function FirmMonsterAlchemy::clear_ptr -----------//

//----------- Begin of function FirmMonsterFortress::crc8 -----------//
UCHAR FirmMonsterFortress::crc8() {
  FirmMonsterFortress &dummyFirmMonsterFortress =
      *(FirmMonsterFortress *)temp_obj.firm_monster_fortress;
  ...
  *((char **)&dummyFirmMonsterFortress) = NULL;
  ...
  UCHAR c =
      ::crc8((UCHAR *)&dummyFirmMonsterFortress, sizeof(FirmMonsterFortress));
  return c;
}
...
void NationBase::clear_ptr() {
  ...
  memset(nation_name_str, 0,
         sizeof(nation_name_str)); // garbage may exist after the '\0'
  next_frame_ready = 0;
...
void Bullet::clear_ptr() { Sprite::clear_ptr(); }
//----------- End of function Bullet::clear_ptr -----------//
...
void BulletHoming::clear_ptr() { Bullet::clear_ptr(); }
...
void BulletProgram::clear_ptr() { Bullet::clear_ptr(); }
...
void BulletFlame::clear_ptr() { Bullet::clear_ptr(); }
...
UCHAR Site::crc8() {
  ...
  if ((char *)&site_recno >
      (char *)this) // clear virtual pointer for the future
    *((char **)&dummySite) = NULL;
