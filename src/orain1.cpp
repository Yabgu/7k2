...
//
void RainDrop::init(Rain *, short fromX, short fromY, short toX, short toY,
                    short speed) {
  cur_x = fromX;
  cur_y = fromY;
  dest_x = toX;
...
//
int RainDrop::is_goal() { return (cur_y + fall_speed >= dest_y); }
...
void RainDrop::draw_step(VgaBuf *vgabuf) {
  ...
  preY = (preY + cur_y) / 2;
  // ####### begin Gilbert 8/5 ######//
  if (preX >= ZOOM_X1 && cur_x >= ZOOM_X1 && preX <= ZOOM_X2 &&
      cur_x <= ZOOM_X2 && preY >= ZOOM_Y1 && cur_y >= ZOOM_Y1 &&
