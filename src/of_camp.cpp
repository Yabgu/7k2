...
char FirmCamp::firm_menu_mode =
    FIRM_MENU_MAIN; // whether the firm is in spy menu mode
...
void FirmCamp::deinit() {
  int firmRecno =
      firm_recno; // save the firm_recno first for reset_unit_home_camp()
...
void FirmCamp::next_day() {
  ...
  err_when(overseer_recno &&
           unit_array[overseer_recno]->nation_recno != nation_recno);
}
  ...
  if (is_ai) {
    ...
    for (int i = 0; i < nationPtr->attack_camp_count; i++) {
      if (nationPtr->attack_camp_array[i].firm_recno == firmRecno) {
        misc.del_array_rec(nationPtr->attack_camp_array,
                           nationPtr->attack_camp_count, sizeof(AttackCamp),
                           i + 1);
...
void FirmCamp::validate_patrol_unit() {
  ...
  for (int i = patrol_unit_count; i > 0; i--) {
    ...
    if (unit_array.is_deleted(unitRecno) ||
      ...
      misc.del_array_rec(patrol_unit_array, patrol_unit_count,
                         sizeof(patrol_unit_array[0]), i);
...
void FirmCamp::change_nation(int newNationRecno) {
  ...
  for (int i = 0; i < soldier_count; i++)
    unit_res[soldier_array[i].unit_id]->unit_change_nation(
        newNationRecno, nation_recno, soldier_array[i].rank_id);
  ...
  for (int i = 1; i <= soldier_count; ++i) {
    ...
    if (soldierPtr->is_under_training()) {
      ...
      if (nation_recno &&
          god_res[GOD_ROMAN]->nation_prayer_count(nation_recno) > 0) {
        basicCombatLevel +=
            BASIC_COMBAT_TRAIN / 2; // roman can train to combat level 30
      }
      // ###### patch end Gilbert 16/2 ######//

      int totalBuildDays = unit_res[soldierPtr->unit_id]->build_days;
      soldierPtr->skill.inc_combat_level(((float)basicCombatLevel + 0.125f) /
                                         totalBuildDays);
      soldierPtr->skill.inc_skill_level(((float)BASIC_SKILL_TRAIN + 0.125f) /
                                        totalBuildDays);
...
void FirmCamp::advanced_train() {
  ...
  if (soldier_count > 0 && (int)overseerUnit->skill_level() <
                               MAX_SKILL_TRAIN) // do not use overseerSkill
  {
    //-- the more soldiers this commander has, the higher the leadership will
    //increase ---//

    incValue = 5 * soldier_count * (int)overseerUnit->hit_points /
    ...
    if (is_ai && config.ai_aggressiveness > OPTION_MODERATE)
      incValue += incValue * (config.ai_aggressiveness - OPTION_MODERATE) /
                  3; // 33% faster if aggressivness is high, 66% faster if
                     // aggressiveness is very high
  ...
  if (overseerUnit->combat_level() < MAX_COMBAT_TRAIN) {
    incValue = 10 * (int)overseerUnit->hit_points /
               overseerUnit->max_hit_points() *
               (100 + overseerUnit->skill.skill_potential * 2) / 100;
    ...
    if (is_ai && config.ai_aggressiveness > OPTION_MODERATE)
      incValue += incValue * (config.ai_aggressiveness - OPTION_MODERATE) /
                  5; // 20% faster if aggressivness is high, 40% faster if
                     // aggressiveness is very high
  ...
  for (int i = 0; i < soldier_count; i++) {
    ...
    if (soldierPtr->combat_level() < overseerSkill &&
      ...
      incValue = MAX(20, overseerSkill - (int)soldierPtr->combat_level()) *
                 soldierPtr->hit_points / soldierPtr->max_hit_points() *
                 (100 + soldierPtr->skill.skill_potential * 2 +
                  overseerUnit->item.ability(ITEM_ABILITY_TRAIN)) /
                 100;
...
void FirmCamp::patrol(short patrolType) {
  ...
  if (overseerRecno &&
      !overseer_recno) // has overseer and the overseer is mobilized
  {
    Unit *overseerUnit = unit_array[overseerRecno];

    if (overseerUnit->is_own()) {
      se_res.sound(overseerUnit->cur_x_loc(), overseerUnit->cur_y_loc(), 1, 'S',
                   overseerUnit->sprite_id, "SEL");
    }
    ...
    for (int i = 0; i < patrol_unit_count; i++) {
      ...
      if (patrol_unit_array[i] !=
          overseerRecno) // don't set the overseer himself
  ...
  if (nation_recno ==
      nation_array.player_recno) // for player's camp, patrol() can only be
...
int FirmCamp::patrol_all_soldier(int overseerRecno, char patrolType) {
  ...
  for (mobileSoldierId = 1; mobileSoldierId <= soldier_count;
       ++mobileSoldierId) {
  ...
  if (soldier_count > 1) {
    ...
    if (selected_soldier_id > 0 && selected_soldier_id <= soldier_count) {
      for (selected_soldier_id = soldier_count;
           ...
           memcmp(&selectedSoldier, &soldier_array[selected_soldier_id - 1],
                  sizeof(Soldier));
           --selected_soldier_id)
        ;
...
void FirmCamp::reward(int soldierId, int remoteAction) {
  if (remoteAction == COMMAND_PLAYER && remote.is_enable()) {
    if (!remoteAction && remote.is_enable()) {
      ...
      short *shortPtr = (short *)remote.new_send_queue_msg(MSG_FIRM_REWARD,
                                                           2 * sizeof(short));
      *shortPtr = firm_recno;
    ...
    if (soldierId == 0) {
      ...
      nation_array[nation_recno]->add_expense(EXPENSE_REWARD_UNIT,
                                              (float)REWARD_COST);
    }
  }
}
...
void FirmCamp::consume_food() {
  ...
  if (nation_array[nation_recno]->food > 0) {
    ...
    nation_array[nation_recno]->consume_food((float)humanUnitCount *
