...
void FirmCamp::deinit()
{
    ...
    reset_unit_home_camp(firmRecno); // this must be called at last as
                                     // Firm::deinit() will create new units.
...
void FirmCamp::reset_attack_camp(int firmRecno)
{
    ...
    if (is_ai)
    {
        ...
        for (int i = 0; i < nationPtr->attack_camp_count; i++)
        {
            ...
            {
                misc.del_array_rec(nationPtr->attack_camp_array, nationPtr->attack_camp_count, sizeof(AttackCamp),
                                   i + 1);
...
void FirmCamp::validate_patrol_unit()
{
    ...
    for (int i = patrol_unit_count; i > 0; i--)
    {
        ...
        if (unit_array.is_deleted(unitRecno) || (unitPtr = unit_array[unitRecno])->is_visible() == 0 ||
...
{
    if (game.game_mode == GAME_TUTORIAL && nation_recno != nation_array.player_recno)
        return;
...
void FirmCamp::advanced_train()
{
    ...
    if (soldier_count > 0 && (int)overseerUnit->skill_level() < MAX_SKILL_TRAIN) // do not use overseerSkill
    {
        //-- the more soldiers this commander has, the higher the leadership will
        // increase ---//

        incValue = 5 * soldier_count * (int)overseerUnit->hit_points / overseerUnit->max_hit_points() *
        ...
        if (is_ai && config.ai_aggressiveness > OPTION_MODERATE)
            incValue +=
                incValue * (config.ai_aggressiveness - OPTION_MODERATE) / 3; // 33% faster if aggressivness is high, 66%
                                                                             // faster if aggressiveness is very high
    ...
    if (overseerUnit->combat_level() < MAX_COMBAT_TRAIN)
    {
        ...
        //			&& god_res[GOD_CELTIC]->nation_prayer_count(nation_recno) >
        // 0
        //)
        ...
        if (is_ai && config.ai_aggressiveness > OPTION_MODERATE)
            incValue +=
                incValue * (config.ai_aggressiveness - OPTION_MODERATE) / 5; // 20% faster if aggressivness is high, 40%
                                                                             // faster if aggressiveness is very high
    ...
    for (int i = 0; i < soldier_count; i++)
    {
        ...
        err_when(soldierPtr->is_monster() && monster_res.name_used_array[soldierPtr->name_id - 1] < 1);
        ...
        if (soldierPtr->combat_level() < overseerSkill && soldierPtr->combat_level() < MAX_COMBAT_TRAIN)
        {
            incValue = MAX(20, overseerSkill - (int)soldierPtr->combat_level()) * soldierPtr->hit_points /
                       soldierPtr->max_hit_points() *
                       (100 + soldierPtr->skill.skill_potential * 2 + overseerUnit->item.ability(ITEM_ABILITY_TRAIN)) /
                       100;
            ...
            {
                incValue = incValue * (MAX_WORKER * 2) /
                           (MAX_WORKER * 2 + god_res[GOD_EGYPTIAN]->nation_prayer_count(nation_recno));
            }
            // ###### patch end Gilbert 23/12 #########//

            if (is_ai && config.ai_aggressiveness > OPTION_MODERATE)
                incValue += incValue * (config.ai_aggressiveness - OPTION_MODERATE) /
                            5; // 20% faster if aggressivness is high, 40%
                               // faster if aggressiveness is very high
        ...
        if (soldierPtr->skill.skill_potential > 0 && soldierPtr->skill_level() < MAX_SKILL_TRAIN)
        {
            incValue = (int)MAX(50, overseerSkill - soldierPtr->skill_level()) * soldierPtr->hit_points /
                       soldierPtr->max_hit_points() * soldierPtr->skill.skill_potential * 2 / 100;
...
void FirmCamp::patrol(short patrolType)
{
    ...
    err_when(is_ai && ai_capture_town_recno); // ai_capture_town_recno is set
                                              // after patrol() is called
    ...
    if ((patrolType == 0) ||
        (patrolType == 2 && overseerRecno &&
         unit_array[overseerRecno]->hit_points >= unit_array[overseerRecno]->max_hit_points() - 25))
        assign_overseer(0);
    else
    ...
    if (overseerRecno && !overseer_recno) // has overseer and the overseer is mobilized
    {
        ...
        {
            se_res.sound(overseerUnit->cur_x_loc(), overseerUnit->cur_y_loc(), 1, 'S', overseerUnit->sprite_id, "SEL");
        }
    ...
    if (nation_recno == nation_array.player_recno) // for player's camp, patrol() can only be
...
int FirmCamp::patrol_all_soldier(int overseerRecno, char patrolType)
{
    ...
    for (mobileSoldierId = 1; mobileSoldierId <= soldier_count; ++mobileSoldierId)
    {
        ...
        if (!unitRecno)
            return 0; // keep the rest soldiers as there is no space for creating the
                      // unit

        --mobileSoldierId; // if can mobilize successfully, mobileSoldierId is
                           // unchanged
        ...
        if (overseerRecno)
        {
            unitPtr->update_loyalty(); // the unit is just assigned to a new leader,
                                       // set its target loyalty
        ...
        else if (nation_recno == nation_array.player_recno)
            unitPtr->select(true);
    }
...
void FirmCamp::sort_soldier()
{
    if (soldier_count > 1)
    {
        //--- prepare soldier_id_array[] for later preserving the currently soldier
        // selected ---//
        ...
        if (selected_soldier_id > 0 && selected_soldier_id <= soldier_count)
        {
            for (selected_soldier_id = soldier_count;
                 ...
                 memcmp(&selectedSoldier, &soldier_array[selected_soldier_id - 1], sizeof(Soldier));
                 --selected_soldier_id)
                ;
...
int FirmCamp::should_show_info()
{
    ...
    {
        if (firm_menu_mode == FIRM_MENU_BRIBE_RESULT || firm_menu_mode == FIRM_MENU_ASSASSINATE_RESULT ||
            firm_menu_mode == FIRM_MENU_STEAL_TECH_RESULT || firm_menu_mode == FIRM_MENU_INCIDENT_RESULT)
        {
            return 1;
        }
    }
...
{
    if (nation_recno == nation_array.player_recno)
        news_array.firm_captured(firm_recno, newNationRecno,
                                 0); // 0 - the capturer is not a spy
...
void FirmCamp::pay_expense()
{
    ...
    for (int i = 1; i <= soldier_count; i++, soldierPtr++)
    {
        ...
        // #### end Gilbert 24/3 #####//
        {
            if (nationPtr->cash > 0)
            {
                nationPtr->add_expense(EXPENSE_WEAPON, (float)unit_res[soldierPtr->unit_id]->year_cost / 365, 1);
            }
            else
            {
                //-- if it's a weapon, decrease hit points if the nation cannot pay the
                // unit --//
