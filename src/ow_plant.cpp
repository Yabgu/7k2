  ...
  for (int y = yBase; y < max_y_loc; y += scanDensity)
    for (int x = xBase; x < max_x_loc; x += scanDensity) {
      ...
      if (l->is_plant() && misc.random(100) < pGrow &&
          (basePlantId = plant_res.plant_recno(bitmapId = l->plant_id())) !=
              0 &&
          bitmapId - plant_res[basePlantId]->first_bitmap <
              plant_res[basePlantId]->bitmap_count - 1) {
  ...
  for (int y = yBase; y < max_y_loc; y += scanDensity) {
    for (int x = xBase; x < max_x_loc; x += scanDensity) {
      ...
      if (l->is_plant() &&
          (basePlantId = plant_res.plant_recno(bitmapId = l->plant_id())) !=
              0 &&
          ((plantGrade = bitmapId - plant_res[basePlantId]->first_bitmap) >=
               3 ||
...
void World::plant_spread(int pSpread) {
  ...
  for (int y1 = y - 1; y1 <= y + 1; ++y1)
    for (int x1 = x - 1; x1 <= x + 1; ++x1) {
      ...
      // #### begin Gilbert 6/3 #######//
      if (!l->can_add_plant() ||
          terrain_res[l->terrain_id]->average_type != teraType)
        build_flag = 0;
      // #### end Gilbert 6/3 #######//
    }
...
//
// scanRadius = radius of the search area for killing a tree for subroutine
// plant_death()
//					remove a tree if the area has more than 3
//trees
//
      ...
      if (locPtr->is_plant()) {
        ...
        for (int scanY = y - scanRadius; scanY <= y + scanRadius; scanY++) {
          for (int scanX = x - scanRadius; scanX <= x + scanRadius; scanX++) {
            if ((scanX >= 0) && (scanY >= 0) && (scanX < max_x_loc) &&
                (scanY < max_y_loc)) {
              totalSpace++;
...
void World::plant_spray(short *plantArray, char area, char distance, short x,
                        short y) {
  // #### begin Ban 8/10 #######//
  int i, j, tempi, tempj;
  Location *newl;
  ...
  for (i = -area; i <= area; i++) {
    for (j = -area; j <= area; j++) {
      ...
      // ###### end Gilbert 9/11 #####//
      if (((x + tempi) < (max_x_loc - 1)) && ((y + tempj) < (max_y_loc - 1)) &&
          ((x + tempi) > 0) && ((y + tempj) > 0)) {
        newl = get_loc(x + tempi, y + tempj);
        short basePlantId =
            plantArray[misc.random(PLANT_ARRAY_SIZE)]; // get type of tree
        short plantSize = misc.random(
            plant_res[basePlantId]->bitmap_count); // get  size of tree
        teraType = terrain_res[newl->terrain_id]->average_type;
        if (newl && newl->can_add_plant() &&
          ...
          plant_count++;
                   ...
                   (newl->plant_id() -
                    plant_res[plant_res.plant_recno(newl->plant_id())]
                        ->first_bitmap) > plantSize) {
          newl->remove_plant();
          newl->set_plant(plant_res[basePlantId]->first_bitmap + plantSize,
                          rand_inner_x(), rand_inner_y());
          newl->set_fire_src(100);
