...
void World::plant_reprod(int pReprod, int scanDensity)
{
    ...
    for (int y = yBase; y < max_y_loc; y += scanDensity)
    {
        for (int x = xBase; x < max_x_loc; x += scanDensity)
        {
            ...
            if (l->is_plant() && (basePlantId = plant_res.plant_recno(bitmapId = l->plant_id())) != 0 &&
                 ...
                 plantGrade == plant_res[basePlantId]->bitmap_count - 1))
            {
                ...
                if (misc.random(100) < tempEffect * pReprod)
                {
                    ...
                    while (trial--)
                    {
                        ...
                        // #### end Gilbert 6/3 #######//
                        {
                            newl->set_plant(plant_res[basePlantId]->first_bitmap, rand_inner_x(), rand_inner_y());
...
// plant_death()
//					remove a tree if the area has more than
// 3
// trees
//
        ...
        //		for( int x = 0; x < max_x_loc; x ++)
        {
            ...
            if (locPtr->is_plant())
            {
                ...
                for (int scanY = y - scanRadius; scanY <= y + scanRadius; scanY++)
                {
                    ...
                    {
                        if ((scanX >= 0) && (scanY >= 0) && (scanX < max_x_loc) && (scanY < max_y_loc))
                        {
                            totalSpace++;
...
void World::plant_spray(short *plantArray, char area, char distance, short x, short y)
{
    ...
    for (i = -area; i <= area; i++)
    {
        for (j = -area; j <= area; j++)
        {
            ...
            // ###### end Gilbert 9/11 #####//
            if (((x + tempi) < (max_x_loc - 1)) && ((y + tempj) < (max_y_loc - 1)) && ((x + tempi) > 0) &&
                ((y + tempj) > 0))
            {
