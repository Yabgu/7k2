...
void Town::process_ai() {
  ...
  if (info.game_date % 30 == town_recno % 30 &&
      !no_neighbor_space && // if there is no space in the neighbor area for
                            // building a new firm.
      population > 1) {
  ...
  if (info.game_date % 30 == (town_recno + 2) % 30 &&
      !no_neighbor_space && // if there is no space in the neighbor area for
                            // building a new firm.
      population >= 5) {
    ...
    //--- the following functions will only be called when the nation has at
    //least a mine ---//

    if (site_array.untapped_raw_count > 0 &&
        ownNation->ai_mine_count ==
            0 && // don't build other structures if there are untapped raw sites
                 // and our nation still doesn't have any
...
void Town::think_defense() {
  int enemyUnitRecno = detect_enemy(
      3); // only when 3 units are detected, we consider them as enemy
  ...
  for (i = unit_array.size(); i > 0; i--) {
    ...
    if (unitPtr->nation_recno == nation_recno) {
      curDis = MAX(abs(unitPtr->cur_x_loc() - enemyXLoc),
                   abs(unitPtr->cur_y_loc() - enemyYLoc));
...
int Town::think_build_firm(int firmId, int maxFirm) {
  ...
  //-- only build one market place next to this town, check if there is any
  //existing one --//
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    err_when(!linked_firm_array[i] ||
             firm_array.is_deleted(linked_firm_array[i]));

    firmPtr = firm_array[linked_firm_array[i]];

    //---- if there is one firm of this type near the town already ----//

    if (firmPtr->firm_id == firmId && firmPtr->nation_recno == nation_recno) {
...
void Town::think_collect_tax() {
  ...
  if (should_ai_migrate()) // if the town should migrate, do collect tax,
                           // otherwise the loyalty will be too low for
  ...
  int minLoyalty =
      55 + 30 * nation_array[nation_recno]->pref_loyalty_concern / 100;

  if (yearProfit < 0)                  // we are losing money now
    minLoyalty -= (-yearProfit) / 100; // more aggressive in collecting tax if
  ...
  int achievableLoyalty =
      target_loyalty - 10; // -10 because it's an average, -10 will be safer

  if (achievableLoyalty >
      minLoyalty) // if the achievable loyalty is higher, then use it
...
void Town::think_reward() {
  ...
  //---- if accumulated_reward_penalty>0, don't grant unless the villagers are
  //near the rebel level ----//

  Nation *ownNation = nation_array[nation_recno];

  if (loyalty <
      REBEL_LOYALTY + 5 + ownNation->pref_loyalty_concern / 10) // 35 to 45
  {
...
int Town::think_ai_migrate() {
  if (info.game_date <
      setup_date +
          90) // don't move if this town has just been set up for less than 90
              // days. It may be a town set up by think_split_town()
    return 0;

  Nation *nationPtr = nation_array[nation_recno];

  //-- the higher the loyalty, the higher the chance all the unit can be
  //migrated --//

  int minMigrateLoyalty =
      35 +
      recruit_dec_loyalty(
          1, 0) // 1-just recruit one unit, 0-don't recruit now, just return the
                // decrease that will result from the recruit
      + nationPtr->pref_loyalty_concern / 10; // 35 to 45

  if (loyalty < minMigrateLoyalty) {
    //-- if the total population is low (we need people) and the cash is high
    //(we have money), then grant to increase loyalty for migration --//

    if (accumulated_reward_penalty == 0 &&
        target_loyalty <
            minMigrateLoyalty +
                5) // if the average target loyalty is also lower than
    {
      if (nationPtr->ai_should_spend(
              20 + nationPtr->pref_territorial_cohesiveness / 2)) // 20 to 70
        reward(COMMAND_AI);
    }
  ...
  //--- check if there are already units currently migrating to the destination
  //town ---//

  Town *destTownPtr = town_array[bestTownRecno];

  if (nationPtr->is_action_exist(destTownPtr->loc_x1, destTownPtr->loc_y1, -1,
                                 -1, ACTION_AI_SETTLE_TOWN, 0)) {
    return 0;
  }
  ...
  nationPtr->add_action(destTownPtr->loc_x1, destTownPtr->loc_y1, loc_x1,
                        loc_y1, ACTION_AI_SETTLE_TOWN, 0, migrateCount);
...
int Town::think_ai_migrate_to_town() {
  ...
  for (int i = 0; i < nationPtr->ai_town_count; i++, aiTownPtr++) {
    ...
    if (population > MAX_TOWN_POPULATION -
                         townPtr->population) // if the town does not have
                                              // enough space for the migration
      continue;

    //--------- compare the ratings ---------//

    //		curRating = 1000 * townPtr->race_pop_array[majorityRace-1] /
    //townPtr->population;	// *1000 so that this will have a much bigger weight
    //than the distance rating
    curRating = 1000;

    curRating += world.distance_rating(center_x, center_y, townPtr->center_x,
                                       townPtr->center_y);

    if (curRating > bestRating) {
      //--- if there is a considerable population of this race, then must
      //migrate to a town with the same race ---// if(
      // race_pop_array[majorityRace-1] >= 6 )
      //{
      //	if( townPtr->majority_race() != majorityRace )
      //// must be commented out otherwise low population town will never be
      //optimized 		continue;
      //}
...
int Town::should_ai_migrate() {
  ...
  if (is_base_town || nationPtr->ai_base_town_count ==
                          0) // don't migrate if this is a base town or there is
                             // no base town in this nation
    return 0;

  if (population - jobless_population >
      0) // if there are workers in this town, don't migrate the town people
    return 0;
...
int Town::protection_available() {
  ...
  for (int i = linked_firm_count - 1; i >= 0; i--) {
    ...
    if (firmPtr->cast_to_FirmCamp())
      protectionLevel +=
          10 + firmPtr->cast_to_FirmCamp()
                   ->total_combat_level(); // +10 for the existence of the camp
                                           // structure
  }
...
int Town::think_build_market() {
  if (info.game_date < setup_date + 90) // don't build the market too soon, as
                                        // it may need to migrate to other town
    return 0;
  ...
  if (no_neighbor_space) // if there is no space in the neighbor area for
                         // building a new firm.
    return 0;
  ...
  //-- only build one market place next to this mine, check if there is any
  //existing one --//
  ...
  if (!ownNation->find_best_place_loc(FIRM_MARKET, loc_x1, loc_y1, buildXLoc,
                                      buildYLoc)) {
    no_neighbor_space = 1;
    return 0;
  }
  ...
  ownNation->add_action(buildXLoc, buildYLoc, loc_x1, loc_y1,
                        ACTION_AI_BUILD_FIRM, FIRM_MARKET);
...
int Town::think_build_fort() {
  //----- check if any of the other camps protecting this town is still
  //recruiting soldiers, if so, wait until their recruitment is finished. So we
  //can measure the protection available accurately.
  ...
  for (i = linked_firm_count - 1; i >= 0; --i) {
    ...
    if (firmFort->under_construction || firmFort->ai_recruiting_soldier ||
        firmFort->soldier_count <
            MAX_SOLDIER) // if this fort is still trying to recruit soldiers
    {
      return 0;
    }
  ...
  //--- this is one of the few base towns the nation has, then a fort must be
  //built ---//
  ...
  if (!buildFlag) {
    ...
    if (!ownNation->ai_should_spend(70 + ownNation->pref_military_development /
                                             4)) // 70 to 95
      return 0;

    //---- only build fort if we need more protection than it is currently
    //available ----//
    ...
    if (!(protectionNeeded > 0 &&
          protectionAvailable == 0)) // if protection needed > 0, and protection
                                     // available is 0, we must build a fort now
    {
      int needUrgency =
          100 * (protectionNeeded - protectionAvailable) / protectionNeeded;

      if (nationPtr->total_jobless_population - MAX_WORKER <
          (100 - needUrgency) * (200 - nationPtr->pref_military_development) /
              200) {
        return 0;
      }
    }
    ...
    if (nationPtr->total_jobless_population >= 30) {
      ...
      buildFlag = 1;
    } else if (nationPtr->ai_has_should_close_camp(
                   region_id)) // there is fort that can be closed
    {
      buildFlag = 1;
    }
  ...
  for (i = linked_firm_count - 1; i >= 0; i--) {
    ...
    if (firmPtr->firm_id == FIRM_CAMP &&
        firmPtr->nation_recno == nation_recno && !firmPtr->upgrading_firm_id) {
      // ####### begin Gilbert 26/1 #######//
...
int Town::think_build_research() {
  ...
  //--- if there is not enough food and all advanced farming has been
  //researched, then do build new towers of science as that will further reduce
  //food production ---//
  ...
  if (jobless_population < MAX_WORKER * workerPercent / 200 ||
      nationPtr->total_jobless_population <
          MAX_WORKER * 2 * workerPercent / 200) {
    return 0;
  }

  //------- if we already have too many researchers -------//

  if (100 * nationPtr->total_firm_worker(FIRM_RESEARCH) /
  ...
  if (nationPtr->ai_research_count == 0 &&
      nationPtr->pref_spy) // if we haven't had any towers of science yet
    importanceRating += 50;
  ...
  if (totalTechLevel ==
      tech_res.total_tech_level(
          nation_recno)) // all technology have been researched
    return 0;
...
int Town::think_build_spy() {
  ...
  int spyTechCount =
      tech_res.tech_class(TECH_CLASS_SPYING)->total_tech_count(nation_recno);
  ...
  if (jobless_population < MAX_WORKER * (150 - nationPtr->pref_spy) / 200 ||
      nationPtr->total_jobless_population <
          MAX_WORKER * 2 * (150 - nationPtr->pref_spy) / 200) {
    return 0;
  }

  //--- if we already have an espionage collage linked to this town, don't build
  //a new one ---//
...
int Town::think_build_special() {
  ...
  if (jobless_population <
          MAX_WORKER * (150 - nationPtr->pref_military_development) / 200 ||
      nationPtr->total_jobless_population <
          MAX_WORKER * 2 * (150 - nationPtr->pref_military_development) / 200) {
    return 0;
  }

  //--- if we already have a special unit training structure linked to this
  //town, don't build a new one ---//
  ...
  if (!nationPtr->ai_should_spend(50 +
                                  nationPtr->pref_military_development / 2))
    return 0;
...
int Town::think_build_war_factory() {
  ...
  if (jobless_population <
          MAX_WORKER * (200 - nationPtr->pref_use_weapon) / 200 ||
      nationPtr->total_jobless_population <
          MAX_WORKER * 2 * (200 - nationPtr->pref_use_weapon) / 200) {
    return 0;
  }
  ...
  if (nationPtr->cash <
      20000 * (100 + nationPtr->pref_cash_reserve) /
          200) // don't build new weapons if we are currently losing money
  {
    if (nationPtr->true_profit_365days() < 0 &&
        nationPtr->ai_war_count >
            0) // if we don't have any war factory, we may want to build one
               // despite that we are losing money
    {
      return 0;
    }
  ...
  int maxWarFactory =
      (1 + totalWeaponTechLevel) * nationPtr->pref_use_weapon / 100;
...
int Town::think_build_base() {
  ...
  if (nationPtr->ai_research_count == 0 ||
      nationPtr->ai_market_count == 0) // don't build it unless we have the more
                                       // basic structures built already
    return 0;
  ...
  if (jobless_population <
          MAX_WORKER * (200 - nationPtr->pref_seat_of_power) / 200 ||
      nationPtr->total_jobless_population <
          MAX_WORKER * 2 * (200 - nationPtr->pref_seat_of_power) / 200) {
    return 0;
  }
  ...
  for (i = 0; i < nationPtr->ai_base_count; i++) {
    ...
    buildRatingArray[god_res[firmBase->god_id]->race_id - 1] =
        0; // only build one

    //		if( firmBase->prayer_count < MAX_WORKER )
    //			buildRatingArray[ god_res[firmBase->god_id]->race_id-1 ] =
    //0; 		else 			buildRatingArray[ god_res[firmBase->god_id]->race_id-1 ] -= 10;
    //// -10 for one existing instance
  }
...
int Town::ai_build_neighbor_firm(int firmId, int firmRaceId) {
  ...
  if (!nationPtr->find_best_place_loc(firmId, loc_x1, loc_y1, buildXLoc,
                                      buildYLoc)) {
    no_neighbor_space = 1;
    return 0;
  }
  ...
  nationPtr->add_action(buildXLoc, buildYLoc, loc_x1, loc_y1,
                        ACTION_AI_BUILD_FIRM, firmId, 1, 0, firmRaceId);
...
int Town::think_split_town() {
  if (jobless_population ==
      0) // cannot move if we don't have any mobilizable unit
    return 0;
  ...
  if (population < (MAX_TOWN_POPULATION - 15) +
                       nationPtr->pref_territorial_cohesiveness / 10)
    return 0;

  //---- check if there is already a town of this race with low population
  //linked to this town, then don't split a new one ---//
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    if (townPtr->nation_recno == nation_recno && townPtr->population < 20 &&
...
void Town::think_move_between_town() {
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    while (1) {
      ...
      if (ourMajorityRace != raceId) // if our major race is not this race, then
                                     // move the unit to the target town
      {
        rc = 0;
      } else //-- if this town's major race is the same as the target town --//
      {
        if (population - townPtr->population >
            10) // if this town's population is larger than the target town by
                // 10 people, then move
          rc = 1;
      }
...
int Town::ai_settle_new(int raceId) {
  ...
  if (!world.locate_space(
          xLoc, yLoc, loc_x2, loc_y2, townWidth,
          townHeight, // STD_TOWN_LOC_WIDTH+2 for space around the town
  ...
  if (misc.points_distance(center_x, center_y,
                           xLoc + (STD_TOWN_LOC_WIDTH - 1) / 2,
                           yLoc + (STD_TOWN_LOC_HEIGHT - 1) / 2) >
      world.effective_distance(0, 0)) {
    return 0;
  }
...
int Town::think_attack_nearby_enemy() {
  enum { SCAN_X_RANGE = 6, SCAN_Y_RANGE = 6 };
  ...
  for (yLoc = yLoc1; yLoc <= yLoc2; yLoc++) {
    ...
    for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++) {
      ...
      //--- if there is an enemy firm here ---//
        ...
        //// don't attack monster here, OAI_MONS.CPP will handle that 					continue;

        //------- if this is a firm of our enemy -------//

        if (nationPtr->get_relation_status(firmPtr->nation_recno) ==
            RELATION_HOSTILE) {
          ...
          enemyCombatLevel +=
              MAX(50, firmCombatLevel); // minimum 50 points for each firm
...
int Town::think_attack_linked_enemy() {
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    if (linked_firm_enable_array[i] !=
        LINK_EE) // don't attack if the link is not enabled
      continue;
    ...
    if (firmPtr->is_ai && ownNation->get_relation_status(
                              firmPtr->nation_recno) != RELATION_HOSTILE) {
      continue;
    }
    ...
    if (firmPtr->cast_to_FirmCamp()) {
      ...
      if (nationStatus >= RELATION_FRIENDLY) {
        ...
        //-- if the link is off and the nation's miliary strength is bigger than
        //us, don't attack --//
      ...
      if (tradeRating > 50 ||
          tradeRating + ownNation->ai_trade_with_rating(firmPtr->nation_recno) >
              100) {
        continue;
      }
    ...
    } else //--- if this is another type of firm ----//
    {
      ...
      if (ownNation->get_relation_status(firmPtr->nation_recno) !=
          RELATION_HOSTILE)
        continue;
    ...
    ownNation->ai_attack_target(firmPtr->loc_x1, firmPtr->loc_y1,
                                targetCombatLevel);
...
int Town::think_capture_enemy_town() {
  ...
  if (nationPtr->ai_capture_enemy_town_recno) // a capturing action is already
                                              // in process
    return 0;
...
int Town::new_base_town_status() {
  ...
  for (i = linked_town_count - 1; i >= 0; i--) {
    ...
    if (townPtr->nation_recno == nation_recno && townPtr->is_base_town) {
      return 1;
    }
  }

  //---- if there is not a single town meeting the above criteria, then set the
  //town with the largest population to be the base town. ----//
  ...
  if (ownNation->ai_base_town_count <= 1) {
    for (i = ownNation->ai_town_count - 1; i >= 0; i--) {
      ...
      if (townPtr->population >
          population) // if this town is not the largest town return 0.
        return 0;
    }
...
int Town::think_scout() {
  ...
  if (info.game_date >
      info.game_start_date + 50 +
          ownNation->pref_scout) // only in the first half year of the game
    return 0;
...
int Town::think_build_wall() {
  if (target_wall_level >
          current_wall_level || // if one is already being trained
  ...
  int importanceRating =
      nationPtr->pref_town_defense / 3 + population / 2 + linked_firm_count * 4;
