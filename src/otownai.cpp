...
void Town::process_ai()
{
    ...
    if (info.game_date % 30 == town_recno % 30 && !no_neighbor_space && // if there is no space in the neighbor area for
                                                                        // building a new firm.
        population > 1)
    {
    ...
    if (info.game_date % 30 == (town_recno + 2) % 30 && !no_neighbor_space && // if there is no space in the neighbor
                                                                              // area for building a new firm.
        population >= 5)
    {
        ...
        if (site_array.untapped_raw_count > 0 &&
            ownNation->ai_mine_count == 0 && // don't build other structures if there are untapped raw sites
...
{
    if (info.game_date < setup_date + 90) // don't move if this town has just been set up for less than 90
    ...
    int minMigrateLoyalty = 35 + recruit_dec_loyalty(1, 0) // 1-just recruit one unit, 0-don't recruit now, just return
                                                           // the decrease that will result from the recruit
    ...
    if (nationPtr->is_action_exist(destTownPtr->loc_x1, destTownPtr->loc_y1, -1, -1, ACTION_AI_SETTLE_TOWN, 0))
    {
        return 0;
    }
    ...
    nationPtr->add_action(destTownPtr->loc_x1, destTownPtr->loc_y1, loc_x1, loc_y1, ACTION_AI_SETTLE_TOWN, 0,
                          migrateCount);
...
int Town::think_ai_migrate_to_town()
{
    ...
    for (int i = 0; i < nationPtr->ai_town_count; i++, aiTownPtr++)
    {
        ...
        if (population > MAX_TOWN_POPULATION - townPtr->population) // if the town does not have
        ...
        // townPtr->population;	// *1000 so that this will have a much bigger
        // weight than the distance rating
        curRating = 1000;
...
int Town::should_ai_migrate()
{
    ...
    if (is_base_town || nationPtr->ai_base_town_count == 0) // don't migrate if this is a base town or there is
...
int Town::protection_available()
{
    ...
    for (int i = linked_firm_count - 1; i >= 0; i--)
    {
        ...
        if (firmPtr->cast_to_FirmCamp())
            protectionLevel += 10 + firmPtr->cast_to_FirmCamp()->total_combat_level(); // +10 for the existence of the
                                                                                       // camp structure
    }
...
int Town::think_build_fort()
{
    ...
    if (!buildFlag)
    {
        ...
        if (!(protectionNeeded > 0 && protectionAvailable == 0)) // if protection needed > 0, and protection
    ...
    for (i = linked_firm_count - 1; i >= 0; i--)
    {
        ...
        if (firmPtr->firm_id == FIRM_CAMP && firmPtr->nation_recno == nation_recno && !firmPtr->upgrading_firm_id)
        {
            // ####### begin Gilbert 26/1 #######//
...
void Town::update_product_supply()
{
    ...
    for (int i = linked_firm_count - 1; i >= 0; i--)
    {
        ...
        if (firmPtr->nation_recno != nation_recno || firmPtr->firm_id != FIRM_MARKET)
        {
            continue;
        }
...
int Town::think_build_research()
{
    ...
    if (!nationPtr->ai_has_enough_food() && tech_res[TECH_FARMING]->get_nation_tech_level(nation_recno) ==
...
int Town::think_build_war_factory()
{
    ...
    if (nationPtr->cash <
        20000 * (100 + nationPtr->pref_cash_reserve) / 200) // don't build new weapons if we are currently losing money
    {
        if (nationPtr->true_profit_365days() < 0 &&
            nationPtr->ai_war_count > 0) // if we don't have any war factory, we may want to build one
...
int Town::think_build_base()
{
    ...
    if (nationPtr->ai_research_count == 0 || nationPtr->ai_market_count == 0) // don't build it unless we have the more
    ...
    for (i = 0; i < nationPtr->ai_base_count; i++)
    {
        ...
        //=
        // 0; 		else 			buildRatingArray[ god_res[firmBase->god_id]->race_id-1 ] -=
        // 10;
...
int Town::think_split_town()
{
    ...
    for (int i = 0; i < linked_town_count; i++)
    {
        ...
        if (townPtr->nation_recno == nation_recno && townPtr->population < 20 && townPtr->race_id == race_id)
        {
            return 0;
        }
    }
...
int Town::ai_settle_new(int raceId)
{
    ...
    if (!world.locate_space(xLoc, yLoc, loc_x2, loc_y2, townWidth,
...
int Town::think_attack_linked_enemy()
{
    ...
    for (int i = 0; i < linked_firm_count; i++)
    {
        ...
        if (firmPtr->is_ai && ownNation->get_relation_status(firmPtr->nation_recno) != RELATION_HOSTILE)
        {
            continue;
        }
        ...
        if (firmPtr->cast_to_FirmCamp())
        {
            ...
            else if (nationStatus == RELATION_NEUTRAL)
            {
                ...
                if (nation_array[firmPtr->nation_recno]->military_rank_rating() > ownNation->military_rank_rating())
                {
                    continue;
                }
            }
            ...
            if (tradeRating > 50 || tradeRating + ownNation->ai_trade_with_rating(firmPtr->nation_recno) > 100)
            {
                continue;
            }
