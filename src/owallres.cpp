...
void WallRes::load_wall_info()
{
    ...
    Database *dbWall = game_set.open_db(WALL_DB); // only one database can be opened at a time,
                                                  // so we read FIRM.DBF first
...
void WallRes::draw_selected()
{
    ...
    /*
            ...
            {
                    vga_back.rect( x, y, x+ZOOM_LOC_WIDTH-1, y+ZOOM_LOC_HEIGHT-1,
       1, OWN_SELECT_FRAME_COLOR );
            }
            */
}
...
{
    err_if(wallId < 1 || wallId > max_wall_id) err_now("WallRes::operator[]");
...
void WallInfo::draw(int xLoc, int yLoc, short *remapTbl)
{
    return; // no wall in 7k2
            ...
            if( x2 < 0 || x1 >= ZOOM_WIDTH )	// out of the view area, not even a
            slight part of it appears in the view area         return;
            ...
            if( x1 < 0 || x2 >= ZOOM_WIDTH || y1 < 0 || y2 >= ZOOM_HEIGHT )
            {
                    ...
                    {
                            if( remapTbl)
                                    vga_back.put_bitmap_area_trans_remap( x1+ZOOM_X1,
            y1+ZOOM_Y1, bitmap_ptr,         MAX(0,x1)-x1, MAX(0,y1)-y1,
            MIN(ZOOM_WIDTH-1,x2)-x1,         MIN(ZOOM_HEIGHT-1,y2)-y1,         remapTbl);         else
            vga_back.put_bitmap_area_trans(         x1+ZOOM_X1, y1+ZOOM_Y1, bitmap_ptr,
            MAX(0,x1)-x1, MAX(0,y1)-y1,         MIN(ZOOM_WIDTH-1,x2)-x1,
            MIN(ZOOM_HEIGHT-1,y2)-y1);
                    }
                    else
                    {
                            if( remapTbl)
                                    vga_back.put_bitmap_area_remap( x1+ZOOM_X1,
            y1+ZOOM_Y1, bitmap_ptr,         MAX(0,x1)-x1, MAX(0,y1)-y1,
            MIN(ZOOM_WIDTH-1,x2)-x1,         MIN(ZOOM_HEIGHT-1,y2)-y1,         remapTbl);         else
            vga_back.put_bitmap_area(         x1+ZOOM_X1, y1+ZOOM_Y1, bitmap_ptr,         MAX(0,x1)-x1,
            MAX(0,y1)-y1,         MIN(ZOOM_WIDTH-1,x2)-x1, MIN(ZOOM_HEIGHT-1,y2)-y1);
                    }
            }
            ...
            else
            {
                    ...
                    {
                            if( remapTbl)
                                    vga_back.put_bitmap_trans_remap( x1+ZOOM_X1,
            y1+ZOOM_Y1, bitmap_ptr, remapTbl);         else         vga_back.put_bitmap_trans(
            x1+ZOOM_X1,         y1+ZOOM_Y1, bitmap_ptr);
                    }
                    else
                    {
                            if( remapTbl)
                                    vga_back.put_bitmap_remap( x1+ZOOM_X1, y1+ZOOM_Y1,
            bitmap_ptr, remapTbl );         else         vga_back.put_bitmap( x1+ZOOM_X1,
            y1+ZOOM_Y1,         bitmap_ptr);
                    }
            }
            */
}
...
void WallInfo::draw_at(int absBaseX, int absBaseY, short *remapTbl)
{
    ...
    if( x1 < 0 || x2 >= ZOOM_WIDTH || y1 < 0 || y2 >= ZOOM_HEIGHT )
    {
            ...
            if(is_trans())
            {
                    if( remapTbl)
                            vga_back.put_bitmap_area_trans_remap( x1+ZOOM_X1,
    y1+ZOOM_Y1, bitmap_ptr, MAX(0,x1)-x1, MAX(0,y1)-y1, MIN(ZOOM_WIDTH-1,x2)-x1,
    MIN(ZOOM_HEIGHT-1,y2)-y1, remapTbl); else vga_back.put_bitmap_area_trans(
    x1+ZOOM_X1, y1+ZOOM_Y1, bitmap_ptr, MAX(0,x1)-x1, MAX(0,y1)-y1,
    MIN(ZOOM_WIDTH-1,x2)-x1, MIN(ZOOM_HEIGHT-1,y2)-y1);
            }
            else
            {
                    if( remapTbl)
                            vga_back.put_bitmap_area_remap( x1+ZOOM_X1,
    y1+ZOOM_Y1, bitmap_ptr, MAX(0,x1)-x1, MAX(0,y1)-y1, MIN(ZOOM_WIDTH-1,x2)-x1,
    MIN(ZOOM_HEIGHT-1,y2)-y1, remapTbl); else vga_back.put_bitmap_area(
    x1+ZOOM_X1, y1+ZOOM_Y1, bitmap_ptr, MAX(0,x1)-x1, MAX(0,y1)-y1,
    MIN(ZOOM_WIDTH-1,x2)-x1, MIN(ZOOM_HEIGHT-1,y2)-y1);
    ...
    else
    {
            if(is_trans() )
            {
                    if( remapTbl)
                            vga_back.put_bitmap_trans_remap( x1+ZOOM_X1,
    y1+ZOOM_Y1, bitmap_ptr, remapTbl); else vga_back.put_bitmap_trans( x1+ZOOM_X1,
    y1+ZOOM_Y1, bitmap_ptr);
            }
            else
            {
                    if( remapTbl)
                            vga_back.put_bitmap_remap( x1+ZOOM_X1, y1+ZOOM_Y1,
    bitmap_ptr, remapTbl ); else vga_back.put_bitmap( x1+ZOOM_X1, y1+ZOOM_Y1,
    bitmap_ptr);
            }
    }
    */
