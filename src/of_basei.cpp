...
static short edit_pray_x1, edit_pray_y1, edit_pray_x2, edit_pray_y2,
    edit_pray_enable;
...
void FirmBase::disp_god_info(int dispY1, int refreshFlag) {
  ...
  if (ratio < 11)
    vga.active_buf->put_bitmap_trans(INFO_X1 + 80 - 13, INFO_Y1 + 49 + 48,
  ...
  vga.active_buf->read_bitmapW(INFO_X1 + 80 - 13, INFO_Y1 + 49 + 48,
                               INFO_X1 + 94 + size - 13, INFO_Y1 + 80 + 48,
                               hitPointBitmap);
  vga.active_buf->put_bitmap_trans(INFO_X1 + 80 - 13, INFO_Y1 + 49 + 48,
                                   image_spict.read("MTR_00"));
  vga.active_buf->put_bitmapW(INFO_X1 + 80 - 13, INFO_Y1 + 49 + 48,
                              hitPointBitmap);
  ...
  /*
          ...
          font_snds.center_put( INFO_X1 +80 -13, INFO_Y1 +46 +48, INFO_X1 +187
     -13, INFO_Y1 +63 +48, str );
  */
  ...
  font_snds.center_put_paragraph(INFO_X1 + 33, INFO_Y1, INFO_X2 - 22,
                                 INFO_Y1 + 115, baseDesc, -2, 0, 0);
}
...
//
void FirmBase::detect_info() { FirmWork::detect_info(); }
...
void FirmBase::disp_firm_info(int dispY1, int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    button_clear_text.create(INFO_X1 + 13 + 3 * BUTTON_DISTANCE, INFO_Y1 + 281,
                             'A', "PREVMENU");
  }
  ...
  button_clear_text.visible_flag =
      invoked_effect_text[0]; // displaying invoked effect text
...
void FirmBase::detect_firm_info() {
  ...
  if (button_invoke.detect() && can_invoke()) {
    if (remote.is_enable()) {
      ...
      short *shortPtr = (short *)remote.new_send_queue_msg(
          MSG_F_BASE_INVOKE_GOD, sizeof(short));
  ...
  //	help.set_help( INFO_X1+13+54, INFO_Y1+261, INFO_X1+13+53+44,
  //INFO_Y1+280+48, "GDINVOKE" ); 	help.set_help( INFO_X1+13+108, INFO_Y1+261,
  //INFO_X1+13+107+44, INFO_Y1+280+48, "BLESS" );
}
// -------- end of function FirmBase::disp_firm_info --------//
...
void FirmBase::disp_edit_mode(int &refreshFlag, int &y, int dispBG) {
  ...
  x += 20;
  x = edit_pray_x2 = font_blue.put((edit_pray_x1 = x), (edit_pray_y1 = y),
                                   (int)pray_points, 0, x2);
...
int FirmBase::detect_edit_mode() {
  ...
  if (edit_pray_enable) {
    if (mouse.any_click(edit_pray_x1, edit_pray_y1, edit_pray_x2, edit_pray_y2,
                        LEFT_BUTTON)) {
      pray_points += 50.0f;
