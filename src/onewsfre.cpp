...
int News::is_major()
{
    ...
    {
        //-- only rebellions happening in the player's citizens are considered as
        // major news --//
...
void News::migrate()
{
    ...
    //----------------------------------------------//
    //
    // Un <worker name>/paysan <racial> a quitté votre town de <town name>
    // pour le town de <town name> dans <nation name>.
    //
    // Un <worker name>/paysan <racial> a quitté le town de <town name>
    // dans <nation name> pour votre town de <town name>.
    //
    //----------------------------------------------//
...
void News::town_destroyed()
{
    ...
    //----------------------------------------------//
    //
    // Votre town de <name name> a été détruit par <kingdom name>.
    // Votre town de <name name> a été détruit par des rebelles.
    // Votre town de <name name> a été détruit par des Fryhtans.
    //
    //----------------------------------------------//
...
void News::town_abandoned()
{
    ...
    //----------------------------------------------//
    //
    // Votre town de <town name> a été déserté par
    // tous ses habitants.
    //
    //----------------------------------------------//
...
void News::town_surrendered()
{
    ...
    //----------------------------------------------//
    //
    // Le town de <town name> dans <King's Kingdom> s'est soumis
    // à vous.
    //
    // Le town indépendant de <town name> s'est soumis
    // à vous.
    //
    // Votre town de <town name> s'est soumis
    // à <King's Kingdom>.
    //
    //----------------------------------------------//
...
void News::your_spy_killed()
{
    ...
    // <town name> [ dans le <nation name> ].  (pas de nation name pour des
    // towns indépendants)
    //
    // Votre espion a été découvert et exécuté en mission dans une
    // <firm type name> près de <town name> dans le <nation name>.
    //
...
void News::enemy_spy_killed()
{
    ...
    //
    // Un espion de <kingdom> a été découvert et exécuté dans votre town de
    // <town name>.
    //
...
void News::lightning_damage()
{
    ...
    //
    // Votre <firm name> près de <town name> a été frappé(e)/détruit(e) par
    // la foudre.
    //
    // Votre town de <town name> a été frappé/détruit par la foudre.
    //
    //----------------------------------------------//
...
void News::firm_worn_out()
{
    ...
    str += " n'est plus en état de marche en raison du manque de fonds pour la "
           "maintenance.";
}
...
char *News::nation_name1()
{
    ...
    if (nation_recno1 < 0) // human player - custom name
        str += nation_array.get_custom_name(nation_recno1,
                                            1); // 1-first word of the name only
    else
        str += race_res[nation_race_id1]->get_single_name((WORD)nation_recno1);
...
char *News::nation_name2()
{
    ...
    if (nation_recno2 < 0) // human player - custom name
        str += nation_array.get_custom_name(nation_recno2,
                                            1); // 1-first word of the name only
    else
        str += race_res[nation_race_id2]->get_single_name((WORD)nation_recno2);
