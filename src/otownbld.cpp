...
//building.
//
// <int> selectedFlag - whether the town of this building is currently selected
// or not.
//
void TownBuild::draw(int townRecno, int curX, int curY,
                     int constructionCompletionPercent) {
  //------- get the color remap table for this sprite ------//

  Town *townPtr = town_array[townRecno];

  short *colorRemapTable = game.get_color_remap_table(
      townPtr->nation_recno, town_array.selected_recno == townRecno);

  colorRemapTable = town_res.calc_color_remap_table(
      colorRemapTable, (float)constructionCompletionPercent / 100);
