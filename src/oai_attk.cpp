...
//                                checking on it.
//                                (default: 0)
// [int] useAllCamp             - use all camps to attack even if defenseMode is
// 0
//                                (default: defenseMode, which is default to 0)
// [int] retainCombatLevel      - if this is 1, do not apply military projection
// or
...
//
int Nation::ai_attack_target(int targetXLoc, int targetYLoc,
                             int targetCombatLevel, int defenseMode,
                             int justMoveToFlag, int attackerMinCombatLevel,
                             int leadAttackCampRecno, int useAllCamp,
                             int retainCombatLevel) {
  // this will be called when the AI tries to capture the town and attack the
  // town's defense.
...
// }
  ...
  //--- if the AI is hard-wired to never declare war with the target nation,
  //then do nothing ---//

  int targetNationRecno = get_target_nation_recno(targetXLoc, targetYLoc);

  if (targetNationRecno && get_relation(targetNationRecno)
                               ->ai_never_consider[TALK_DECLARE_WAR - 1]) {
    return 0;
  }

  //--- order nearby mobile units who are on their way to home camps to join
  //this attack mission. ---//

  if (defenseMode)
    useAllCamp = 1;

  if (defenseMode) // only for defense mode, for attack mission, we should plan
                   // and organize it better
  {
    int originalTargetCombatLevel = targetCombatLevel;

    targetCombatLevel = ai_attack_order_nearby_mobile(targetXLoc, targetYLoc,
                                                      targetCombatLevel);

    if (targetCombatLevel <
        0) // the mobile force alone can finish all the enemies
      return originalTargetCombatLevel - targetCombatLevel;
  }

  //--- try to send troop with maxTargetCombatLevel, and don't send troop if
  //available combat level < minTargetCombatLevel ---//

  int maxTargetCombatLevel = targetCombatLevel *
                             (150 + pref_force_projection / 2) /
                             100; // 150% to 200%
  int minTargetCombatLevel;

  //--- if the target is a human kingdom, we need to send more troop is humans
  //are cunning ---//

  if (targetNationRecno && nation_array[targetNationRecno]->is_human() &&
      config.ai_aggressiveness >= OPTION_MODERATE) {
    maxTargetCombatLevel += maxTargetCombatLevel *
                            (config.ai_aggressiveness - OPTION_MODERATE + 1) /
                            3;
  }
...
#define MAX_SUITABLE_TOWN_CAMP 10 // no. of camps in a town
  ...
  if (leadAttackCampRecno) {
    ...
    attack_camp_array[attack_camp_count].combat_level =
        ((FirmCamp *)firm_array[leadAttackCampRecno])->total_combat_level();
  ...
  //---- if the military courage is low or the king is injured, don't send the
  //king out to battles ---//
  ...
  if (king_unit_recno) {
    ...
    if (kingUnit->unit_mode == UNIT_MODE_OVERSEE) {
      ...
      if (ai_camp_count >
          3 + (100 - ownNation->pref_military_courage) /
                  20) // don't use the king if we have other generals, the king
                      // won't be used if we have 3 to 8 camps. The higher the
                      // military courage is, the smaller will be the number of
                      // camps
        rc = 1;

      //--- if the military courage is low or the king is injured ---//

      else if (kingUnit->hit_points <
               230 -
                   ownNation->pref_military_courage) // 130 to 230, if over 200,
      ...
      if (rc) {
        ...
        if (kingFirmRecno &&
            kingUnit->hit_points >=
                150 - ownNation->pref_military_courage /
                          4) // if the king is not heavily injured
        {
          firmCamp = (FirmCamp *)firm_array[kingFirmRecno];

          if (firmCamp->soldier_count ==
              MAX_SOLDIER) // the king shouldn't go out alone
          {
            if (misc.points_distance(firmCamp->center_x, firmCamp->center_y,
                                     targetXLoc, targetYLoc) <=
                world.effective_distance(firmCamp->firm_id, 0)) {
              kingFirmRecno = 0;
            }
          }
        }
  ...
  for (i = 0; i < ai_camp_count; i++) {
    ...
    if (firmCamp->ai_capture_town_recno) // the base is trying to capture an
                                         // independent town
      continue;
    ...
    if (firmCamp->firm_recno == kingFirmRecno && ai_camp_count > 1 &&
        !useAllCamp)
      continue;
    ...
    if (overseerUnit->hit_points < overseerUnit->max_hit_points() &&
        overseerUnit->hit_points <
            100 - ownNation->pref_military_courage / 2) // 50 to 100
    {
      continue;
    }
    ...
    //
    // Add this camp if:
    // 1. we are in defense mode, and have to get all the forces available to
    // defend against the attack.
    // 2. this camp isn't linked to any of our towns.
    //
    //-------------------------------------//

    rc = useAllCamp || firmCamp->firm_id == FIRM_LAIR ||
         firmCamp->linked_town_count ==
             0; // don't use this camp as it may be in the process of capturing
                // an indepdendent town or an enemy town Fryhtan lairs do not
                // have any linked own towns, so we cannot scan suitable firms
                // from owned towns
    if (rc) {
      ...
      if (k == attack_camp_count &&
          attack_camp_count <
              MAX_SUITABLE_ATTACK_CAMP) // k==attack_camp_count means that the
                                        // camp hasn't been added yet
      {
  ...
  //---- locate for camps that are extra for protecting towns (there are basic
  //ones doing the protection job only) ----//
    ...
    for (i = 0; i < ai_town_count; i++) {
      ...
      for (j = townPtr->linked_firm_count - 1; j >= 0; j--) {
        ...
        if (firmPtr->cast_to_FirmCamp()) {
          ...
          if (firmCamp->patrol_unit_count >
              0) // if there are units patrolling out
            continue;

          if (firmCamp->ai_capture_town_recno) // the base is trying to capture
                                               // an independent town
            continue;
          ...
          if (townCampCount < MAX_SUITABLE_TOWN_CAMP) {
            ...
            townCampArray[townCampCount].combat_level =
                firmCamp->total_combat_level();
      ...
      if (totalCombatLevel > protectionNeeded) {
        //--- see if the protection is still enough if we put one of the camps
        //into the upcoming battle ---//

        for (int j = 0; j < townCampCount; j++) {
          if (totalCombatLevel - townCampArray[j].combat_level >
              protectionNeeded) {
            //--- if so, add this camp to the suitable camp list ---//

            if (attack_camp_count < MAX_SUITABLE_ATTACK_CAMP) {
              //--- this camp can be linked to a town previously processed
              //already (in this case, two towns linked to the same camp) ---//

              int k;
              for (k = 0; k < attack_camp_count; k++) {
                if (attack_camp_array[k].firm_recno ==
                    townCampArray[j].firm_recno)
                  break;
              }
              ...
              if (k == attack_camp_count) {
                err_when(
                    firm_array[townCampArray[j].firm_recno]->nation_recno !=
                    nation_recno);
                ...
                totalCombatLevel -=
                    townCampArray[j]
                        .combat_level; // reduce it from the total combat level
                                       // as its combat level has just been
  ...
  if (totalCombatLevel <
      minTargetCombatLevel) // if we are not strong enough to attack yet
  {
    attack_camp_count = 0;
    return 0;
  }
  ...
  for (i = 0; i < attack_camp_count; i++) {
    ...
    attack_camp_array[i].distance = misc.points_distance(
        firmPtr->center_x, firmPtr->center_y, targetXLoc, targetYLoc);
  ...
  qsort(&attack_camp_array, attack_camp_count, sizeof(attack_camp_array[0]),
        sort_attack_camp_function);

  //----- now take out the lowest rating ones -----//

  for (i = attack_camp_count - 1; i >= 0; i--) {
    if (totalCombatLevel - attack_camp_array[i].combat_level >
        maxTargetCombatLevel) {
      totalCombatLevel -= attack_camp_array[i].combat_level;
      attack_camp_count--;
    }
  }
...
//
void Nation::set_ai_attack_target(int targetXLoc, int targetYLoc,
                                  int defenseMode) {
  ...
  ai_attack_target_nation_recno =
      get_target_nation_recno(targetXLoc, targetYLoc);
      ...
      if (baseObj->nation_recno == ai_attack_target_nation_recno) {
        ...
        for (j = 0; j < ai_attack_target_count; j++) {
          if (baseObj->base_obj_recno ==
              world
                  .get_loc(ai_attack_target_x_loc[j], ai_attack_target_y_loc[j])
                  ->base_obj_recno('L')) {
            break;
          }
        }
        ...
        ai_attack_target_x_loc[ai_attack_target_count] =
            (baseObj->obj_loc_x1() + baseObj->obj_loc_x2()) / 2;
        ai_attack_target_y_loc[ai_attack_target_count] =
            (baseObj->obj_loc_y1() + baseObj->obj_loc_y2()) / 2;
...
//
int Nation::ai_attack_order_nearby_mobile(int targetXLoc, int targetYLoc,
  ...
  for (int i = 2; i < scanRange * scanRange; i++) {
    ...
    if (unitPtr->cur_order.mode == UNIT_STOP ||
      ...
      targetCombatLevel -=
          (int)unitPtr->hit_points; // reduce the target combat level
...
void Nation::ai_attack_target_sync() {
  ...
  int maxTravelDays =
      sprite_res[unit_res[basicUnitId]->sprite_id]->travel_days(maxDistance);
  ...
  for (i = 0; i < attack_camp_count; i++) {
    ...
    attack_camp_array[i].patrol_date =
        info.game_date + (maxTravelDays - travelDays);
  }
...
void Nation::ai_attack_target_execute(int directAttack) {
  ...
  while (ai_attack_target_nation_recno !=
         get_target_nation_recno(ai_attack_target_x_loc[0],
                                 ai_attack_target_y_loc[0])) {
    err_when(ai_attack_target_x_loc[0] < 0 ||
             ai_attack_target_x_loc[0] >= MAX_WORLD_X_LOC);
    err_when(ai_attack_target_y_loc[0] < 0 ||
             ai_attack_target_y_loc[0] >= MAX_WORLD_Y_LOC);

    //---- go to the next target ----//

    misc.del_array_rec(ai_attack_target_x_loc, ai_attack_target_count,
                       sizeof(ai_attack_target_x_loc[0]), 1);
    misc.del_array_rec(ai_attack_target_y_loc, ai_attack_target_count,
                       sizeof(ai_attack_target_y_loc[0]), 1);
  ...
  for (int i = attack_camp_count - 1; i >= 0; i--) {
    ...
    if (firmCamp->overseer_recno || firmCamp->soldier_count) {
             ...
             // units
      {
        ...
        if (ai_attack_target_nation_recno)
          talk_res.ai_send_talk_msg(ai_attack_target_nation_recno, nation_recno,
                                    TALK_DECLARE_WAR);

        //--- in defense mode, just move close to the target, the unit will
        //start attacking themselves as their relationship is hostile already
        //---//

        if (!directAttack) {
          unit_group.set(firmCamp->patrol_unit_array,
                         firmCamp->patrol_unit_count);
          unit_group.move(ai_attack_target_x_loc[0], ai_attack_target_y_loc[0],
                          false, COMMAND_AI); // false-not forced move
        } else {
          //-------- set should_attack on the target to 1 --------//

          enable_should_attack_on_target(ai_attack_target_x_loc[0],
                                         ai_attack_target_y_loc[0]);
          ...
          int baseObjRecno =
              world
                  .get_loc(ai_attack_target_x_loc[0], ai_attack_target_y_loc[0])
                  ->base_obj_recno();
    ...
    misc.del_array_rec(attack_camp_array, attack_camp_count, sizeof(AttackCamp),
                       i + 1);
    attack_camp_count--;
  }
}
...
int Nation::think_secret_attack() {
  ...
  for (int i = 1; i <= nation_array.size(); i++) {
    ...
    if (!nationRelation->ai_secret_attack) {
      ...
      if (tradeRating >
          (50 - pref_trading_tendency /
                    2)) // 0 to 50, 0 if trade tendency is 100, it is 0
      {
        continue;
      }
    }
    ...
    curRating = (ourMilitary - nationPtr->military_rank_rating()) * 2 +
                ...
                - tradeRating * 2 - get_relation(i)->ai_relation_level / 2 -
                pref_peacefulness / 2;
