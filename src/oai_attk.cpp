...
//
int Nation::ai_attack_target(int targetXLoc, int targetYLoc, int targetCombatLevel, int defenseMode, int justMoveToFlag,
                             int attackerMinCombatLevel, int leadAttackCampRecno, int useAllCamp, int retainCombatLevel)
{
...
// }
    ...
    if (targetNationRecno && get_relation(targetNationRecno)->ai_never_consider[TALK_DECLARE_WAR - 1])
    {
        return 0;
    }
    ...
    if (targetNationRecno && nation_array[targetNationRecno]->is_human() && config.ai_aggressiveness >= OPTION_MODERATE)
    {
...
#define MAX_SUITABLE_TOWN_CAMP 10 // no. of camps in a town
    ...
    if (king_unit_recno)
    {
        ...
        if (kingUnit->unit_mode == UNIT_MODE_OVERSEE)
        {
            ...
            if (ai_camp_count >
                3 + (100 - ownNation->pref_military_courage) / 20) // don't use the king if we have other generals,
                                                                   // the king won't be used if we have 3 to 8 camps.
                                                                   // The higher the military courage is, the smaller
                                                                   // will be the number of camps
                rc = 1;

            //--- if the military courage is low or the king is injured ---//

            else if (kingUnit->hit_points < 230 - ownNation->pref_military_courage) // 130 to 230, if over 200,
            ...
            if (rc)
            {
                ...
                if (kingFirmRecno && kingUnit->hit_points >= 150 - ownNation->pref_military_courage /
                                                                       4) // if the king is not heavily injured
                {
                    ...
                    {
                        if (misc.points_distance(firmCamp->center_x, firmCamp->center_y, targetXLoc, targetYLoc) <=
    ...
    for (i = 0; i < ai_camp_count; i++)
    {
        ...
        rc = useAllCamp || firmCamp->firm_id == FIRM_LAIR ||
             firmCamp->linked_town_count == 0; // don't use this camp as it may be in the process of capturing
        ...
        if (rc)
        {
            ...
            if (k == attack_camp_count &&
                attack_camp_count < MAX_SUITABLE_ATTACK_CAMP) // k==attack_camp_count means that the
                    ...
                    if (totalCombatLevel - townCampArray[j].combat_level > protectionNeeded)
                    {
                        ...
                        if (attack_camp_count < MAX_SUITABLE_ATTACK_CAMP)
                        {
                            ...
                            if (k == attack_camp_count)
                            {
                                ...
                                totalCombatLevel -=
                                    townCampArray[j].combat_level; // reduce it from the total combat level
    ...
    for (i = 0; i < attack_camp_count; i++)
    {
        ...
        attack_camp_array[i].distance =
            misc.points_distance(firmPtr->center_x, firmPtr->center_y, targetXLoc, targetYLoc);
...
int Nation::ai_attack_order_nearby_mobile(int targetXLoc, int targetYLoc, int targetCombatLevel)
{
    ...
    for (int i = 2; i < scanRange * scanRange; i++)
    {
        ...
        if (unitPtr->hit_points < unitPtr->max_hit_points() * (150 - pref_military_courage / 2) / 200)
        {
            continue;
        }
        ...
        if (unitPtr->cur_order.mode == UNIT_STOP || unitPtr->cur_order.mode == UNIT_ASSIGN)
        {
...
void Nation::ai_attack_target_execute(int directAttack)
{
    ...
    while (ai_attack_target_nation_recno !=
           get_target_nation_recno(ai_attack_target_x_loc[0], ai_attack_target_y_loc[0]))
    {
    ...
    for (int i = attack_camp_count - 1; i >= 0; i--)
    {
        ...
        if (firmCamp->overseer_recno || firmCamp->soldier_count)
        {
            ...
            if (firmCamp->patrol_unit_count > 0) // there could be chances that there are no some for mobilizing the
                                                 // units
            {
                ...
                if (!directAttack)
                {
                    ...
                    unit_group.move(ai_attack_target_x_loc[0], ai_attack_target_y_loc[0], false,
                                    COMMAND_AI); // false-not forced move
                }
                else
                {
                    //-------- set should_attack on the target to 1 --------//
                    ...
                    int baseObjRecno =
                        world.get_loc(ai_attack_target_x_loc[0], ai_attack_target_y_loc[0])->base_obj_recno();
...
void Nation::reset_ai_attack_target()
{
    ...
    for (int i = 0; i < attack_camp_count; i++)
    {
        ...
        err_when(!firmPtr->cast_to_FirmCamp() || firmPtr->nation_recno != nation_recno);
...
int Nation::think_secret_attack()
{
    ...
    for (int i = 1; i <= nation_array.size(); i++)
    {
        ...
        curRating = (ourMilitary - nationPtr->military_rank_rating()) * 2 +
                    ...
                    - tradeRating * 2 - get_relation(i)->ai_relation_level / 2 - pref_peacefulness / 2;
