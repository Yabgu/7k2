...
void ResTxt::init(char *resName, int alwaysOpenFlag)
{
    ...
    txt_buf_size = DEF_TXT_BUF_SIZE; // this size should be large enough for average text
                                     // unit, also we don't need realloc very frequently
...
char *ResTxt::get_body(int recNo)
{
    ...
    if (!always_open_flag) // if not always open, then only open during reading,
                           // and close it immediately after that
        file_txt.file_open(res_file_name);
