...
char Place::is_being_deleted =
    0; // true only when the firm is to be removed from the firm_array
...
//									   0 just remove the
//existing builder, do not assign new one.
//
...
int Place::set_builder(short newBuilderRecno) {
  err_when(under_construction &&
           newBuilderRecno == 0); // can't remove the construction worker when
                                  // the firm is under construction
  ...
  if (builder_recno) {
    ...
    if (unitPtr->is_visible()) // is visible if the unit is not inside the firm
                               // location
    {
      builder_region_id =
          world.get_region_id(unitPtr->cur_x_loc(), unitPtr->cur_y_loc());
      unitPtr->deinit_sprite();
    }

    err_when(unitPtr->unit_mode != 0 &&
             unitPtr->unit_mode !=
                 UNIT_MODE_REBEL); // UNIT_MODE_REBEL is exceptional as rebels
                                   // on settle down to a town
...
int Place::mobilize_builder(short unitRecno) {
  ...
  if (!locate_space(is_being_deleted, xLoc, yLoc, loc_x2, loc_y2,
                    ...
                    unitPtr->mobile_type, builder_region_id) &&
      !world.locate_space(xLoc, yLoc, loc_x2, loc_y2, spriteInfo->loc_width,
                          spriteInfo->loc_height, unitPtr->mobile_type,
                          builder_region_id)) {
...
int Place::locate_space(int removeFirm, int &xLoc, int &yLoc, int xLoc2,
                        int yLoc2, int width, int height, int mobileType,
                        int regionId) {
  int checkXLoc, checkYLoc;

  if (removeFirm) {
    //--- if the place is about to be removed, also count the space currently
    //occupied by the place ---//
    int x1 = xLoc;
    int y1 = yLoc;
    int x2 = xLoc2;
    int y2 = yLoc2;
      ...
      for (checkXLoc = x1; checkXLoc <= x2; checkXLoc++) {
        ...
        for (int y = 0; y < height && !occupiedFlag; y++) {
          for (int x = 0; x < width; x++) {
            if (!world.get_loc(checkXLoc + x, checkYLoc + y)
                     ->can_move(mobileType)) {
              occupiedFlag = 1;
              break;
            }
    ...
    if (!world.locate_space(checkXLoc, checkYLoc, xLoc2, yLoc2, width, height,
                            mobileType, regionId)) {
      return 0;
    } else {
      xLoc = checkXLoc;
...
int Place::create_unit(int unitId) {
  ...
  int unitRecno =
      unit_array.add_unit(unitId, unitNationRecno, RANK_SOLDIER, 0, xLoc, yLoc);
...
int Place::create_unit_locate_space(int unitId, int &xLoc, int &yLoc) {
  if (!is_being_deleted) // if the place is being deleted, create the units in
  ...
  for (int r = 0; r <= 20; r++) {
    ...
    if (locate_space(
            is_being_deleted, xLoc, yLoc, MIN(loc_x2 + r, world.max_x_loc - 1),
            MIN(loc_y2 + r, world.max_y_loc - 1), spriteInfo->loc_width,
...
int Place::can_create_unit(int unitId, int xLoc, int yLoc) {
  ...
  for (int y = 0; y < spriteInfo->loc_height; y++) {
    for (int x = 0; x < spriteInfo->loc_width; x++) {
      ...
      if (thisXLoc < 0 || thisXLoc >= MAX_WORLD_X_LOC || thisYLoc < 0 ||
          thisYLoc >= MAX_WORLD_Y_LOC) {
        return 0;
      }
...
void Place::being_attack_hit(BaseObj *attackerObj, float damagePoint) {
  ...
  //--- if this is a Killer Bee which can only attack one, multiply its demages
  //against building ---//

  if (attackerObj && attackerObj->cast_to_Unit() &&
      attackerObj->cast_to_Unit()->unit_id == UNIT_BEE)
    damagePoint *= 5;
  ...
  if (nation_recno && is_ai) {
    if (attackerObj && attackerObj->nation_recno ==
                           nation_recno) // this can happen when the unit has
                                         // just changed nation
      return;
...
void Place::invoke_camp_defense(BaseObj *attackerObj) {
  ...
  if (!nation_recno) {
    ...
    if (cast_to_Town() && cast_to_Town()->is_pay_tribute_to_monster()) {
      ...
      int netAttackerPower = world.net_attacker_power_in_area(
          center_x, center_y, attackerObj->nation_recno, nation_recno, 0,
          hasWar, EFFECTIVE_DEFEND_DISTANCE);

      //-- no need to call for reinforcement if the defense force is more
      //powerful than the attacking force --//
      ...
      for (int i = 0; i < thisTown->linked_firm_count; i++) {
        ...
        if (thisTown->tribute_to_lair(thisTown->linked_firm_array[i], 1)) {
          FirmLair *firmLair =
              firm_array[thisTown->linked_firm_array[i]]->cast_to_FirmLair();
  ...
  int netAttackerPower = world.net_attacker_power_in_area(
      center_x, center_y, attackerObj->nation_recno, nation_recno, 0, hasWar,
      EFFECTIVE_DEFEND_DISTANCE);

  //-- no need to call for reinforcement if the defense force is more powerful
  ...
  for (int i = firm_array.size(); i > 0; i--) {
    ...
    int defensePower = firmPtr->cast_to_FirmCamp()->invoke_defense(
        attackerObj, netAttackerPower);
