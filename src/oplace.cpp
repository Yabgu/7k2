...
{
    err_when(under_construction && newBuilderRecno == 0); // can't remove the construction worker when
    ...
    if (builder_recno)
    {
        ...
        err_when(unitPtr->unit_mode != 0 && unitPtr->unit_mode != UNIT_MODE_REBEL); // UNIT_MODE_REBEL is exceptional as
                                                                                    // rebels on settle down to a town
...
int Place::mobilize_builder(short unitRecno)
{
    ...
    if (!locate_space(is_being_deleted, xLoc, yLoc, loc_x2, loc_y2, spriteInfo->loc_width, spriteInfo->loc_height,
        ...
        !world.locate_space(xLoc, yLoc, loc_x2, loc_y2, spriteInfo->loc_width, spriteInfo->loc_height,
                            unitPtr->mobile_type, builder_region_id))
    {
    ...
    err_when(unitPtr->unit_mode != UNIT_MODE_CONSTRUCT_FIRM && unitPtr->unit_mode != UNIT_MODE_CONSTRUCT_TOWN);
...
int Place::locate_space(int removeFirm, int &xLoc, int &yLoc, int xLoc2, int yLoc2, int width, int height,
                        int mobileType, int regionId)
{
...
int Place::create_unit_locate_space(int unitId, int &xLoc, int &yLoc)
{
    ...
    for (int r = 0; r <= 20; r++)
    {
        ...
        if (locate_space(is_being_deleted, xLoc, yLoc, MIN(loc_x2 + r, world.max_x_loc - 1),
                         MIN(loc_y2 + r, world.max_y_loc - 1), spriteInfo->loc_width, spriteInfo->loc_height,
...
int Place::can_create_unit(int unitId, int xLoc, int yLoc)
{
    ...
    for (int y = 0; y < spriteInfo->loc_height; y++)
    {
        for (int x = 0; x < spriteInfo->loc_width; x++)
        {
            ...
            if (thisXLoc < 0 || thisXLoc >= MAX_WORLD_X_LOC || thisYLoc < 0 || thisYLoc >= MAX_WORLD_Y_LOC)
            {
                return 0;
            }
...
void Place::being_attack_hit(BaseObj *attackerObj, float damagePoint)
{
    ...
    {
        if (attackerObj && attackerObj->nation_recno == nation_recno) // this can happen when the unit has
                                                                      // just changed nation
            return;
...
void Place::invoke_camp_defense(BaseObj *attackerObj)
{
    ...
    if (!nation_recno)
    {
        ...
        if (cast_to_Town() && cast_to_Town()->is_pay_tribute_to_monster())
        {
            ...
            int netAttackerPower = world.net_attacker_power_in_area(center_x, center_y, attackerObj->nation_recno,
                                                                    nation_recno, 0, hasWar, EFFECTIVE_DEFEND_DISTANCE);
    ...
    int netAttackerPower = world.net_attacker_power_in_area(center_x, center_y, attackerObj->nation_recno, nation_recno,
                                                            0, hasWar, EFFECTIVE_DEFEND_DISTANCE);
    ...
    for (int i = firm_array.size(); i > 0; i--)
    {
        ...
        if (misc.points_distance(firmPtr->center_x, firmPtr->center_y, center_x, center_y) > EFFECTIVE_DEFEND_DISTANCE)
        {
            continue;
        }
