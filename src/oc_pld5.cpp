...
/*
Protect a seat of power from being destroyed by Fryhtan. This is the seat of
power your ancestor built and is of extreme importance to your kingdom and your
people. If this seat of power is destroyed, you will lose the game.
*/
...
void CampaignEastWest::plot_d5_create_game() {
  ...
  for (; trial > 0 && seatCount > 0; --trial) {
    ...
    if ((townRecno = random_pick_town_with_camp(
             nation_array.player_recno, 8))) // only pick town with <= 8 links
    {
      int firmRecno;
      ...
      if ((firmRecno =
               create_firm_next_to_place(town_array[townRecno], FIRM_BASE,
                                         town_array[townRecno]->race_id))) {
        // ---------- add lose condition --------//
