...
//--------- Begin of function NationArray::~NationArray ---------//
//
NationArray::~NationArray() { deinit(); }
//----------- End of function NationArray::~NationArray ---------//
...
void NationArray::init() {
  ...
  nation_color_array[0] =
      (char)V_WHITE; // if nation_recno==0, the color is white
  nation_power_color_array[0] =
      (char)VGA_GRAY + 10; // if nation_recno==0, the color is white
  nation_power_color_array[MAX_NATION + 1] =
      (char)VGA_GRAY +
      10; // if Location::power_nation_recno==MAX_NATION+1, this means there are
          // more than one nation have influence over this location.
...
// [unsigned long] dpPlayerId = DirectPlayer player id. (only for multiplayer
// game)
//
//
// return : <int> nationRecno = the recno. of the newly added nation
//
int NationArray::new_nation(int nationClassId, int nationType, int raceId,
                            int colorSchemeId, unsigned long dpPlayerId) {
  //--------------------------------------------------------//
  ...
  //--- store the colors of all nations into a single array for later fast
  //access ---//

  nation_color_array[nationPtr->nation_recno] = nationPtr->nation_color;
  nation_power_color_array[nationPtr->nation_recno] =
      nationPtr->nation_color; // use a lighter color for the nation power area
...
int NationArray::new_nation(NewNationPara &nationPara) {
  ...
  char nationType = nationPara.dp_player_id == remote.self_player_id()
                        ? NATION_OWN
                        : NATION_REMOTE;
  ...
  nationPtr->init(nationType, nationPara.race_id, nationPara.color_scheme,
                  nationPara.dp_player_id);

  if (nationPara.use_gem_stones) {
    nationPtr->init_cash(nationPara.use_gem_stones);
    game.total_gem_stones +=
        nationPara.use_gem_stones; // do not reset use_gemstones, as init_cash
                                   // is called again in battle
...
void NationArray::del_nation(int recNo) {
  ...
  Nation *nationPtr =
      nation_array[recNo]; // operator[] will check for deleted nation error,
                           // can't use operator() because it use
                           // firm_array()->nation_recno
  ...
  *((Nation **)get()) = NULL; // Nullify the pointer
  ...
  if (recNo == player_recno) {
    ...
    if (!sys.signal_exit_flag)
      sys.set_view_mode(MODE_NORMAL); // set the view mode to normal mode to
                                      // prevent possible problems
  }

  if (!sys.quick_exit_flag()) {
    //---------- update statistic ----------//
    update_statistic(); // as max_overall_nation_recno and others may be
                        // pointing to the deleted nation
  }
  // ####### end Gilbert 26/7 #########//
}
...
void NationArray::process() {
  ...
  for (i = size(); i > 0; i--) {
    ...
    if (i % FRAMES_PER_DAY ==
        int(sys.frame_count %
            FRAMES_PER_DAY)) // only process each firm once per day
    {
#ifdef DEBUG
...
int NationArray::random_unused_monster_type() {
  ...
  int pickedInstance = misc.random(MAX_MONSTER_TYPE - usedCount) + 1;
  int usedId = 0;
...
int NationArray::random_unused_color() {
  ...
  int pickedInstance =
      misc.random(4) +
      1; // always try to pick the first 4 which are more distinct whereas the
         // last 3 colors are very hard to distinguish
...
void NationArray::update_statistic() {
  ...
  memset(independent_town_count_race_array, 0,
         sizeof(independent_town_count_race_array)); // the no. of independent
                                                     // towns each race has
  ...
  for (i = town_array.size(); i > 0; i--) {
    ...
    if (townPtr->nation_recno) {
      ...
      //			{
      //				if( townPtr->race_pop_array[j] >= 6 )
      //// only count it if the pop of the race >= 6
  ...
  //--- update nation rating (this must be called after the above code, which
  //update vars like total_population ---//
  ...
  update_total_human_count(); //**BUGHERE there is bug with human count, this
                              //function is for fixing bug on-fly
...
void NationArray::update_military_rating() {
  ...
  for (i = firm_array.size(); i > 0; i--) {
    ...
    if (firmPtr->cast_to_FirmCamp()) {
      ...
      // 20 is the base military points for a unit, so the nation that has many
      // more units can be reflected in the military rating
    }
    // ###### patch begin 9/11 ########//
  ...
  for (i = unit_array.size(); i > 0; i--) {
    ...
    if (unit_res[unitPtr->unit_id]->unit_class == UNIT_CLASS_SHIP) {
      nation_array[unitPtr->nation_recno]->total_ship_combat_level +=
          (int)unitPtr->hit_points;
    }

    //----------------------------------//

    if (unitPtr->unit_mode ==
        UNIT_MODE_OVERSEE) // firm commanders are counted above with firm_array
      continue;
    ...
    //			addPoints += (unitInfo->weapon_power + unitPtr->get_weapon_version()
    //- 1) * 30;
    ...
    if (unitPtr->leader_unit_recno &&
        !unit_array.is_deleted(unitPtr->leader_unit_recno))
      addPoints += addPoints *
                   unit_array[unitPtr->leader_unit_recno]->skill_level() / 100;

    nationCombatLevelArray[unitPtr->nation_recno - 1] +=
        addPoints +
        20; // 20 is the base military points for a unit, so the nation that has
            // many more units can be reflected in the military rating
  }
...
void NationArray::update_total_human_count() {
  ...
  for (i = firm_array.size(); i > 0; i--) {
    ...
    if (firmPtr->cast_to_FirmTrain()) {
      ...
      totalHumanCountArray[firmTrain->nation_recno - 1] +=
          firmTrain->trainee_count;
    }
  }
        ...
        unitPtr->rank_id != RANK_KING) // does not count kings
    {
      if (unitPtr->unit_id == UNIT_WAGON)
        totalHumanCountArray[unitPtr->nation_recno - 1] +=
            unitPtr->cast_to_UnitWagon()->population;
      else
        totalHumanCountArray[unitPtr->nation_recno - 1]++;
    }
  }
...
//							NULL if there is no custom name for
//the specific nation.
//
...
void NationArray::set_custom_both_name(int nationRecno, char *nameStr) {
  ...
  set_custom_nation_name(nationRecno,
                         text_unit.str_unit_nation(nationPtr->race_id, str));
}
...
char NationArray::should_attack(short attackingNation, short attackedNation) {
  ...
  if (nation_array.is_deleted(attackingNation) ||
      nation_array.is_deleted(attackedNation))
    return 0;

  return this->operator[](attackingNation)
      ->get_relation_should_attack(attackedNation);
}
