...
void NationArray::init()
{
    ...
    nation_power_color_array[MAX_NATION + 1] =
        (char)VGA_GRAY + 10; // if Location::power_nation_recno==MAX_NATION+1, this means there are
...
int NationArray::new_nation(NewNationPara &nationPara)
{
    ...
    char nationType = nationPara.dp_player_id == remote.self_player_id() ? NATION_OWN : NATION_REMOTE;
    ...
    if (nationPara.use_gem_stones)
    {
        ...
        game.total_gem_stones += nationPara.use_gem_stones; // do not reset use_gemstones, as init_cash
...
void NationArray::del_nation(int recNo)
{
    ...
    Nation *nationPtr = nation_array[recNo]; // operator[] will check for deleted nation error,
...
int NationArray::random_unused_color()
{
    ...
    int pickedInstance = misc.random(4) + 1; // always try to pick the first 4 which are more distinct whereas the
...
{
    return config.new_nation_emerge && nation_array.ai_nation_count < config.ai_nation_count() &&
           nation_array.nation_count < MAX_NATION && info.game_date > last_del_nation_date + NEW_NATION_INTERVAL_DAYS &&
...
void NationArray::update_military_rating()
{
    ...
    for (i = firm_array.size(); i > 0; i--)
    {
        ...
        if (firmPtr->cast_to_FirmCamp())
        {
            ...
            nationCombatLevelArray[firmPtr->nation_recno - 1] +=
                firmCamp->total_combat_level() + (firmCamp->overseer_recno > 0 + firmCamp->soldier_count) * 20;
        ...
        else if (firmPtr->cast_to_FirmTrain())
        {
            ...
            nationCombatLevelArray[firmPtr->nation_recno - 1] +=
                firmTrain->total_military_combat_level() + firmTrain->total_military_count() * 20;
            // 20 is the base military points for a unit, so the nation that has many
            // more units can be reflected in the military rating
        }
        // ###### patch end 9/11 ########//
    ...
    for (i = unit_array.size(); i > 0; i--)
    {
        ...
        //			addPoints += (unitInfo->weapon_power +
        // unitPtr->get_weapon_version()
        //- 1) * 30;
        ...
        nationCombatLevelArray[unitPtr->nation_recno - 1] +=
            addPoints + 20; // 20 is the base military points for a unit, so the nation that has
...
void NationArray::update_total_human_count()
{
    ...
    for (i = unit_array.size(); i > 0; i--)
    {
        ...
        if (unitPtr->nation_recno && unitPtr->is_human() && unitPtr->rank_id != RANK_KING) // does not count kings
        {
...
//							NULL if there is no custom name
// for
// the specific nation.
//
