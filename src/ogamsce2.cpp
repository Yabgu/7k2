...
enum { SCROLL_X1 = 757, SCROLL_Y1 = 352, SCROLL_X2 = 770, SCROLL_Y2 = 492 };
...
enum {
  TEXT_AREA_X1 = SCROLL_SHEET_X1 + 30,
  TEXT_AREA_Y1 = 356,
...
// <char**> scenFileNameArray - an array of <char*> pointing to the file name of
// the
//										  available
//scenarios.
//
...
int Game::select_scenario(int scenCount, ScenInfo *scenInfoArray) {
  if (scenCount == 0) {
    box.msg(
        text_game_menu.str_none_scenario()); // "Scenario files not found." );
    return 0;
  }
  ...
  // create one button for each path
  {
    ...
    for (i = 0; i < MAX_SCENARIO_PATH; ++i, (x1 += groupButtonWidth)) {
      ...
      groupEnableButton[i].create_text(
          x1, SCROLL_SHEET_Y2 + 20, x1 + groupButtonWidth - 1,
          SCROLL_SHEET_Y2 + 20 + groupButtonHeight - 1,
  ...
  for (j = 0; j < MAX_SCENARIO_PATH && scenCountArray[j] == 0; ++j)
    if (j >= MAX_SCENARIO_PATH) // all scenario are empty (when auto.scn not
                                // considered)
    {
      box.msg(
          text_game_menu.str_none_scenario()); // "Scenario files not found." );
  ...
  briefModeButton.create_text(SCROLL_SHEET_X2 - 140, SCROLL_SHEET_Y2 + 42,
                              SCROLL_SHEET_X2 - 40, SCROLL_SHEET_Y2 + 62,
                              text_game_menu.str_brief_mode(briefMode), 0);
  ...
  int *scenIndex =
      (int *)blobScenIndex.p(); // gives the index in scenInfoArray, given 0 to
                                // currentScenCount-1
  ...
  int pageUpX1 =
      (LSCROLL_X2 + LSCROLL_X1 - ((Bitmap *)arrowBitmap)->get_width()) / 2;
  int pageUpY1 =
  ...
  int pageDownX1 =
      (RSCROLL_X2 + RSCROLL_X1 - ((Bitmap *)arrowBitmap)->get_width()) / 2;
  int pageDownY1 =
  ...
  //	scrollUp.create(menuX1+SCROLL_X1,menuY1+SCROLL_Y1-17, "SV-UP-U",
  //"SV-UP-D", 1, 0); 	scrollDown.create(menuX1+SCROLL_X1,menuY1+SCROLL_Y2+1,
  //"SV-DW-U", "SV-DW-D", 1, 0); 	startButton.create(menuX1+373, menuY1+529,
  //"START-U", "START-D",1, 0); 	cancelButton.create(menuX1+548, menuY1+529,
  //"CANCEL-U", "CANCEL-D", 1, 0);

  //	SlideVBar scrollBar;
  //	scrollBar.init_scroll(menuX1+SCROLL_X1, menuY1+SCROLL_Y1,
  //menuX1+SCROLL_X2, menuY1+SCROLL_Y2, 		MAX_BROWSE_DISP_REC,
  //disp_scroll_bar_func); 	scrollBar.set(minRecno, scenCount, minRecno);

  //	scrollBar.set_view_recno(browseRecno - MAX_BROWSE_DISP_REC/2);
  Button3D textScrollUp, textScrollDown;
  textScrollUp.create(TEXT_SCROLL_X1, TEXT_SCROLL_Y1 - 17, "SV-UP-U", "SV-UP-D",
                      1, 0);
  textScrollDown.create(TEXT_SCROLL_X1, TEXT_SCROLL_Y2 + 1, "SV-DW-U",
                        "SV-DW-D", 1, 0);
  ...
  *(textBuffer.reserve(1)) = '\0';
  ...
  const int ESTIMATED_LINE_IN_TEXT_AREA =
      ...
      (textFont.font_height + TEXT_LINE_SPACE);

  SlideVBar textScrollBar;
  textScrollBar.init_scroll(TEXT_SCROLL_X1, TEXT_SCROLL_Y1, TEXT_SCROLL_X2,
                            TEXT_SCROLL_Y2, ESTIMATED_LINE_IN_TEXT_AREA,
                            disp_scroll_bar_func);

  GetA playerNameField;
  playerNameField.init(364, 89, 632, config.player_name, config.PLAYER_NAME_LEN,
                       &font_bold_black, 0, 1);
  ...
  while (1) {
    ...
    if (refreshFlag) {
      if (refreshFlag & TUOPTION_PAGE) {
        ...
        font_bld.center_put(TITLE_TEXT_X2 - 100, TITLE_TEXT_Y1 - 25,
                            TITLE_TEXT_X2 - 50, TITLE_TEXT_Y2 - 8,
                            text_game_menu.str_level_in_score());
        font_bld.center_put(TITLE_TEXT_X2 - 50, TITLE_TEXT_Y1 - 25,
                            TITLE_TEXT_X2, TITLE_TEXT_Y2 - 8,
                            text_game_menu.str_bonus());
      ...
      if (refreshFlag & TUOPTION_ENUMERATE) {
        ...
        int selectedScenario =
            scenIndex[browseRecno - 1]; // need to set the new browseRecno
          ...
          if (j >= 0 && j < MAX_SCENARIO_PATH) {
            if (groupEnableButton[j].is_pushed) {
              ...
              if (i == selectedScenario)
                browseRecno = currentScenCount; // set new browseRecno to the
                                                // one before change
            }
          }
        }
        ...
        maxPage = (currentScenCount + itemPerPage[briefMode] - 1) /
                  itemPerPage[briefMode];
        if (maxPage <= 1)
      ...
      pathName =
          DIR_SCENARIO_PATH(scenInfoArray[scenIndex[browseRecno - 1]].dir_id);
      err_when(!*pathName);
      ...
      if ((refreshFlag & TUOPTION_PIC_AREA) && briefMode == 0) {
        ...
        if (browseRecno && misc.is_file_exist(str)) {
          ...
          // draw the background
          vga_util.blt_buf(PIC_AREA_X1, PIC_AREA_Y1, PIC_AREA_X2, PIC_AREA_Y2,
                           0);
        }
      }
      ...
      if (refreshFlag & TUOPTION_TEXT_BUFFER) {
        ...
        if (browseRecno && misc.is_file_exist(str)) {
          ...
          textFont.count_line(TEXT_AREA_X1, TEXT_AREA_Y1, TEXT_AREA_X2,
                              TEXT_AREA_Y2, textBuffer.queue_buf,
                              TEXT_LINE_SPACE, dispLines, totalLines);
      ...
      if (refreshFlag & TUOPTION_TEXT_AREA) {
        vga_util.blt_buf(TEXT_AREA_X1, TEXT_AREA_Y1, TEXT_AREA_X2, TEXT_AREA_Y2,
                         0);
        if (textBuffer.queue_buf[0] != '\0') {
          textFont.put_paragraph(
              TEXT_AREA_X1, TEXT_AREA_Y1, TEXT_AREA_X2, TEXT_AREA_Y2,
              textBuffer.queue_buf, TEXT_LINE_SPACE,
              textScrollBar.view_recno); // 4 - space between lines
        }
      }
      ...
      if (refreshFlag & TUOPTION_ALL_BROWSE) {
        ...
        for (i = itemPerPage[briefMode]; i > 0;
             --i, ++rec, (y1 += rowHeight), (y2 += rowHeight)) {
          ...
          if (rec >= 1 && rec <= currentScenCount) {
            ...
            if (scenInfo->scen_name[0]) {
              (browseRecno == rec ? font_bold_red : font_bold_black)
                  .center_put(x1, y1, x2 - 50, y2, scenInfo->scen_name);

              //---- display the scenario difficulty and bonus points ----//

              textFont.center_put(x2 - 100, y1, x2 - 50, y2,
                                  misc.format(scenInfo->goal_difficulty), 1);
              textFont.center_put(x2 - 50, y1, x2, y2,
                                  misc.format(scenInfo->goal_score_bonus), 1);
            } else // description not found, display file name
            {
              (browseRecno == rec ? font_bold_red : font_bold_black)
                  .center_put(x1, y1, x2 - 50, y2, scenInfo->file_name);
            }
          }
        }
      }
      ...
      if (page > 0) {
        ...
        //	vga.active_buf->put_bitmap_trans_decompress_hmirror( pageUpX1,
        //pageUpY1, arrowBitmap );
    ...
    if (mouse.any_click(LSCROLL_X1, LSCROLL_Y1, LSCROLL_X2, LSCROLL_Y2)) {
    ...
    } else if (mouse.any_click(RSCROLL_X1, RSCROLL_Y1, RSCROLL_X2,
                               RSCROLL_Y2)) {
      if (page < maxPage - 1) {
        ++page;
        if (itemPerPage[briefMode] == 1) // change browseRecno as well
      ...
      retFlag = 0;
      break; // break while(1)
    } else if (mouse.single_click(BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2,
                                  BUTTON2_Y2)) {
      // load button
      refreshFlag = TUOPTION_ALL;
    ...
    // detect scroll area
    {
      ...
      for (i = itemPerPage[briefMode]; i > 0;
        ...
        if (mouse.double_click(x1, y1, x2, y2)) {
          ...
          if (rec != browseRecno) {
            if (browseRecno - recBegin >= 0 &&
                browseRecno - recBegin < itemPerPage[briefMode])
              refreshFlag |= TUOPTION_BROWSE(browseRecno - recBegin);
            browseRecno = rec;
            refreshFlag |=
                TUOPTION_SCEN_DETAIL | TUOPTION_BROWSE(browseRecno - recBegin);
          }
        }
      }
    ...
    for (j = 0; j < MAX_SCENARIO_PATH; ++j) {
      ...
      if ((rc = groupEnableButton[j].detect(0, 0, 2))) // detect right click
      {
        ...
        if (rc == 2) {
          for (i = 0; i < MAX_SCENARIO_PATH && !groupEnableButton[i].is_pushed;
               ++i)
            ;
        ...
        if (rc ==
            1) // left click, disable all other buttons and enable this button
        {
          for (i = 0; i < MAX_SCENARIO_PATH; ++i) {
...
static void disp_scroll_bar_func(SlideVBar *scroll, int) {
  ...
  {
    vga_front.bar(scroll->scrn_x1, scroll->scrn_y1, scroll->scrn_x2,
                  rectTop - 1, V_BLACK);
  }
  if (scroll->scrn_y2 > rectBottom) // fill regin above the scroll button
  {
    vga_front.bar(scroll->scrn_x1, rectBottom + 1, scroll->scrn_x2,
                  scroll->scrn_y2, V_BLACK);
  }
}
