...
{
    return nation_recno == 0 && hero_id && hero_res[hero_id]->for_hire;
}
...
{
    return hero_res.hire_cost(combat_level(), skill.max_combat_level, skill_level(), 0, &item);
}
...
{
    return is_hero_for_hire() && nation_array[nationRecno]->is_human() && // only human nations can hire heroes
...
int Unit::hire_hero(int nationRecno)
{
    ...
    {
        if (unit_array.selected_recno == sprite_recno && nationRecno == nation_array.player_recno)
        {
            hire_hero_result = -1;
        }
        return 0;
