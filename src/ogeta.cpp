...
void GetA::init(int x1, int y1, int x2, char *field, unsigned length, Font *fontPtr, char align, char detectEsc)
{
    x = x1;
    y = y1;
    x_limit = x2;
...
unsigned GetA::detect_key()
{
    ...
    if (keyCode)
    {
        ...
        if (keyCode >= ' ' && keyCode <= 0xff)
        {
            ...
            {
                // insert character
                memmove(input_field + mark_begin() + 1, input_field + mark_end(), strlen(input_field) - mark_end() + 1);
        ...
        else if (keyCode == KEY_DEL)
        {
            if (is_select())
            {
                ...
                err_when(mark_cursor_pos < 0 || mark_cursor_pos > strlen(input_field));
                // erase selected area
                memmove(input_field + mark_begin(), input_field + mark_end(), strlen(input_field) - mark_end() + 1);
            ...
            else
            {
                ...
                {
                    err_when(cursor_pos < 0 || cursor_pos > strlen(input_field));
                    memmove(input_field + cursor_pos, input_field + cursor_pos + 1, strlen(input_field) - cursor_pos);
                    err_when(cursor_pos < 0 || cursor_pos > strlen(input_field));
                }
            }
            err_when(cursor_pos < 0 || cursor_pos > strlen(input_field));
        ...
        if (keyCode == KEY_RETURN || keyCode == KEY_UP || keyCode == KEY_DOWN || keyCode == KEY_TAB)
        {
            return keyCode;
        }
    }
...
void GetA::paint(int paintCursor)
{
    ...
    // ##### end Gilbert 22/2 ########//
    {
        ...
        {
        case 0:
            IMGbltWArea(bitmap->get_ptr(), bitmap->get_true_pitch(), 0, 0, (short *)back_ground_bitmap, 0, 0,
                        MIN(textWidth, backGroundWidth) - 1, MIN(textHeight, backGroundHeight) - 1);
            break;
        case 1: {
            int l = (x_limit - x + 1 - textWidth) / 2;
            if (l >= 0 && l < backGroundWidth)
                IMGbltWArea(bitmap->get_ptr(), bitmap->get_true_pitch(), -l, 0, (short *)back_ground_bitmap, l, 0,
                            MIN(l + textWidth, backGroundWidth) - 1, MIN(textHeight, backGroundHeight) - 1);
        }
        break;
        case -1: {
    ...
    else
    {
        ...
        //			IMGbar( bitmap->get_ptr(), bitmap->get_true_pitch(), 0,
        // 0,
    ...
    if (paintCursor && enable_flag && cursorX < x_limit)
    {
        ...
        if (leftX < textWidth)
        {
            IMGbar(bitmap->get_ptr(), bitmap->get_true_pitch(), leftX, font_ptr->height() - 1,
                   MIN(rightX, textWidth - 1), font_ptr->height() - 1, 0);
        }
    ...
    if (!Vga::use_back_buf)
    {
        ...
        case 0: // left justified
            ...
            else
            {
                ...
                if (textWidth < backGroundWidth && x + textWidth <= x_limit) // fill right
                    Vga::active_buf->put_bitmapW_area(x, y, back_ground_bitmap, textWidth, 0,
        ...
        case 1: // center justified
            ...
            else
            {
                ...
                {
                    Vga::active_buf->put_bitmapW_area(x, y, back_ground_bitmap, 0, 0, MIN(l - x, backGroundWidth) - 1,
                                                      backGroundHeight - 1);
                }
                if (l + textWidth <= x_limit && l + textWidth - x < backGroundWidth)
                {
                    Vga::active_buf->put_bitmapW_area(x, y, back_ground_bitmap, l + textWidth - x, 0,
...
int GetA::detect_click()
{
    ...
    if (!mouse_drag_flag)
    {
        ...
        if (clickCount == 1)
        {
            ...
            for (cursor_pos = strlen(input_field); cursor_pos > 0 && mouse.click_x() < cursor_x(cursor_pos);
                 --cursor_pos)
                ;
            err_when(cursor_pos < 0 || cursor_pos > strlen(input_field));
            clear_select();
    ...
    else
    {
        ...
        for (cursor_pos = strlen(input_field); cursor_pos > 0 && mouse.cur_x < cursor_x(cursor_pos); --cursor_pos)
            ;
        err_when(cursor_pos < 0 || cursor_pos > strlen(input_field));
        return 1;
    }
...
int GetAGroup::set_focus(int n, int paintFlag)
{
    ...
    {
    case -1:
        for (i = 0, newFocused = (oldFocused + 1) % geta_num; i < geta_num && !geta_array[newFocused].enable_flag;
    ...
    case -2:
        ...
        break;
    case -3:
        for (i = 0, newFocused = 0; i < geta_num && !geta_array[newFocused].enable_flag;
             ++i, newFocused = (newFocused + 1) % geta_num)
            ;
        break;
    case -4:
        for (i = 0, newFocused = geta_num - 1; i < geta_num && !geta_array[newFocused].enable_flag;
             ++i, newFocused = (newFocused + geta_num - 1) % geta_num)
            ;
        break;
    default:
