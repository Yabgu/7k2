...
void GetA::set_background(BitmapW *backPtr) { back_ground_bitmap = backPtr; }

int GetA::height() { return font_ptr->max_font_height; }
...
unsigned GetA::detect_key() {
  ...
  else if ((autoRepeatCount = mouse.key_auto_repeat_count()) >
  ...
  if (keyCode) {
    ...
    if (keyCode == KEY_RETURN || keyCode == KEY_UP || keyCode == KEY_DOWN ||
        keyCode == KEY_TAB) {
      return keyCode;
    }
  }
...
void GetA::paint(int paintCursor) {
  ...
  // ##### end Gilbert 22/2 ########//
  {
    ...
    switch (align_flag) {
    case 0:
      IMGbltWArea(bitmap->get_ptr(), bitmap->get_true_pitch(), 0, 0,
                  (short *)back_ground_bitmap, 0, 0,
                  MIN(textWidth, backGroundWidth) - 1,
                  MIN(textHeight, backGroundHeight) - 1);
      break;
    case 1: {
      int l = (x_limit - x + 1 - textWidth) / 2;
      if (l >= 0 && l < backGroundWidth)
        IMGbltWArea(bitmap->get_ptr(), bitmap->get_true_pitch(), -l, 0,
                    (short *)back_ground_bitmap, l, 0,
                    MIN(l + textWidth, backGroundWidth) - 1,
                    MIN(textHeight, backGroundHeight) - 1);
    } break;
    case -1: {
    ...
    switch (align_flag) {
    case 0: // left just
      vga_back.read_bitmapW(x, y, x + textWidth - 1, y + textHeight - 1,
                            bitmap);
      break;
    case 1: // centre just
    {
      int l = (x_limit - x + 1 - textWidth) / 2;
      vga_back.read_bitmapW(x + l, y, x + l + textWidth - 1, y + textHeight - 1,
                            bitmap);
    } break;
    case -1:
      vga_back.read_bitmapW(x_limit - textWidth + 1, y, x_limit,
                            y + textHeight - 1, bitmap);
      break;
    default:
      err_here();
    }
    ...
    //			IMGbar( bitmap->get_ptr(), bitmap->get_true_pitch(), 0, 0,
    //textWidth-1, textHeight-1, transparent_code_w);
    //		}
    // ##### end Gilbert 22/2 ########//
  }

  font_ptr->put_to_bufferW(bitmap->get_ptr(), bitmap->get_true_pitch(), 0, 0,
                           input_field);

  if (paintCursor && enable_flag && cursorX < x_limit) {
    // horizontal bar for selected area
    if (leftX < textWidth) {
      IMGbar(bitmap->get_ptr(), bitmap->get_true_pitch(), leftX,
             font_ptr->height() - 1, MIN(rightX, textWidth - 1),
             font_ptr->height() - 1, 0);
    }

    // vertical bar
    if (cursorX < textWidth)
      IMGbar(bitmap->get_ptr(), bitmap->get_true_pitch(), cursorX, 0, cursorX,
             font_ptr->height() - 1, 0);
  }
    ...
    case 0: // left justified
      if (!back_ground_bitmap) {
        if (x + textWidth <= x_limit) // fill right
          vga_util.blt_buf(x + textWidth, y, x_limit,
                           y + font_ptr->max_font_height - 1, 0);
        Vga::active_buf->put_bitmapW(x, y, bitmap);
      } else {
        short backGroundWidth = back_ground_bitmap->get_width();
        short backGroundHeight = back_ground_bitmap->get_height();
        if (textWidth < backGroundWidth &&
            x + textWidth <= x_limit) // fill right
          Vga::active_buf->put_bitmapW_area(
              x, y, back_ground_bitmap, textWidth, 0,
              MIN(x_limit - x, backGroundWidth - 1), backGroundHeight - 1);
        Vga::active_buf->put_bitmapW(x, y, bitmap);
      }
      break;
    ...
    case 1: // center justified
      if (!back_ground_bitmap) {
        ...
        if (x < l && l - x <= backGroundWidth) {
          Vga::active_buf->put_bitmapW_area(x, y, back_ground_bitmap, 0, 0,
                                            MIN(l - x, backGroundWidth) - 1,
                                            backGroundHeight - 1);
        }
        if (l + textWidth <= x_limit && l + textWidth - x < backGroundWidth) {
          Vga::active_buf->put_bitmapW_area(
              x, y, back_ground_bitmap, l + textWidth - x, 0,
              MIN(x_limit - x + 1, backGroundWidth) - 1, backGroundHeight - 1);
        }
        Vga::active_buf->put_bitmapW(l, y, bitmap);
      }
      break;
...
int GetA::cursor_x(int curPos) {
  ...
  case 1: // center justified
    return x + ((x_limit - x) - font_ptr->text_width(input_field)) / 2 +
           font_ptr->text_width(input_field, curPos);
  case -1: // right justified
    return x_limit - font_ptr->text_width(input_field) +
           font_ptr->text_width(input_field, curPos);
  default:
    err_here();
    return 0;
...
int GetA::detect_click() {
  ...
  if (!mouse_drag_flag) {
    int clickCount = mouse.any_click(x - font_ptr->max_font_width, y, x_limit,
                                     y + height() - 1);
    if (clickCount == 1) {
      // set cursor_pos
    ...
    for (cursor_pos = strlen(input_field);
         cursor_pos > 0 && mouse.cur_x < cursor_x(cursor_pos); --cursor_pos)
      ;
    err_when(cursor_pos < 0 || cursor_pos > strlen(input_field));
    return 1;
  }
...
unsigned GetA::mark_begin() { return MIN(cursor_pos, mark_cursor_pos); }

unsigned GetA::mark_end() { return MAX(cursor_pos, mark_cursor_pos); }
...
GetAGroup::~GetAGroup() { delete[] geta_array; }
...
int GetAGroup::operator()() { return focused_geta; }
