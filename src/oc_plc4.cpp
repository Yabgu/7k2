  ...
  for (radius = 6; spyCount > 0 && radius <= 20; (radius += 2)) {
    ...
    for (trial = 10 + radius; spyCount > 0 && trial > 0; --trial) {
      ...
      for (n = 1; n <= nation_array.size(); ++n) {
        if (nation_array.is_deleted(n) || n == nation_array.player_recno ||
      ...
      n = nationRecnoCount > 1 ? nationRecnoArray[misc.random(nationRecnoCount)]
                               : nationRecnoArray[0];

      // ----- create a unit of that race --------//

      int unitRecno =
          create_unit(nation_array.player_recno,
                      race_res[nation_array[n]->race_id]->infantry_unit_id,
                      RANK_SOLDIER, areaX1, areaY1, areaX2, areaY2);
      if (unitRecno) {
        unit_array[unitRecno]->spy_recno =
            spy_array.add_spy(unitRecno, 50 + misc.random(40));
        spyCount--;
      }
    }
  }
