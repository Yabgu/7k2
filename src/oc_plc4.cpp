...
void CampaignEastWest::plot_c4_create_game()
{
    ...
    for (radius = 6; spyCount > 0 && radius <= 20; (radius += 2))
    {
        ...
        for (trial = 10 + radius; spyCount > 0 && trial > 0; --trial)
        {
            ...
            {
                if (nation_array.is_deleted(n) || n == nation_array.player_recno || !nation_array[n]->is_human())
                    continue;
            ...
            int unitRecno = create_unit(nation_array.player_recno, race_res[nation_array[n]->race_id]->infantry_unit_id,
