...
FirmSpy::FirmSpy() {
  memset(sizeof(FirmTrain) + (char *)this, 0,
         sizeof(FirmSpy) - sizeof(FirmTrain));
}
...
void FirmSpy::recruit_trainee(int townRecno, int trainType, char remoteAction) {
  ...
  if (nation_recno &&
      nation_array[nation_recno]->cash < (float)TRAIN_SKILL_COST)
    return;
  ...
  //--- if all remaining units are spies, when you try to train one, all of them
  //will become mobilized ---//
  ...
  traineePtr->init(townPtr->race_id, (char)townPtr->loyalty, 0, trainType,
                   townRecno);
  // ######## begin Gilbert 28/6 #########//
