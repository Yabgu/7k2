...
void FirmCamp::process_ai() {
  ...
  //--- think about building a firm and employ independent citizens to lower
  //their resistance ---//
  ...
  if (info.game_date % 15 ==
      (firm_recno + 10) %
          15) // do not call too often as the penalty of accumulation is 10 days
    think_use_cash_to_capture();

  //------- assign overseer and soldiers -------//

  if (info.game_date % 15 ==
      firm_recno % 15) // do not call too often because when an AI action is
                       // queued, it will take a while to carry it out
    think_recruit();
  ...
  if (ai_capture_town_recno) {
    should_close_flag =
        0; // disable should_close_flag when trying to capture a firm,
           // should_close_flag is set in process_common_ai()

    if (nation_array[nation_recno]->attack_camp_count ==
            0 && // only when attack_camp_count==0, the attack mission is
                 // complete
        patrol_unit_count == 0) {
  ...
  int checkInterval =
      13 - nation_array[nation_recno]->pref_military_development / 10;
  ...
  if (info.game_date % intervalDays ==
      firm_recno %
          intervalDays) // do not call too often because when an AI action is
                        // queued, it will take a while to carry it out
    think_capture();
...
void FirmCamp::process_ai_capturing() {
  err_when(
      patrol_unit_count <=
      0); // this function shouldn't be called at all if patrol_unit_count==0
  err_when(patrol_unit_count > 9);

  if (!ai_capture_town_recno)
    return;

  if (think_capture_return()) // if the capturing units should now return to
                              // their base.
    return;
  ...
  //--- if the town is still not captured but all mobile town defenders are
  //destroyed, attack the town again ---//
  ...
  if (townPtr->town_defender_count == 0) {
    ...
    if (townPtr->nation_recno)
      nation_array[nation_recno]->set_relation_should_attack(
          townPtr->nation_recno, 1, COMMAND_AI);
...
void FirmCamp::ai_attack_town_defender(Unit *attackerUnit) {
  ...
  if (attackerUnit->cur_action == SPRITE_IDLE)
    ...
    //--- if this unit is currently attacking the town, ask it to attack a
    //defender unit ---//
  ...
  for (int i = 0; i < unitCount; i++) {
    ...
    if (targetUnit->unit_mode == UNIT_MODE_TOWN_DEFENDER &&
      ...
      if (targetUnit->nation_recno)
        nation_array[nation_recno]->set_relation_should_attack(
            targetUnit->nation_recno, 1, COMMAND_AI);
...
void FirmCamp::think_recruit() {
  ...
  //-- if this camp is empty, think about move a whole troop from a useless camp
  //(should_ai_close()==1)
  ...
  if (!overseer_recno && soldier_count == 0 &&
    ...
    //--- see if there are any useless camps around and pick the most suitable
    //one ---//
    ...
    for (int i = nationPtr->ai_camp_count - 1; i >= 0; i--) {
      ...
      if (firmCamp->should_close_flag &&
        ...
        curRating =
            100 - misc.points_distance(center_x, center_y, firmCamp->center_x,
                                       firmCamp->center_y);
    ...
    if (bestCampPtr) {
      ...
      if (bestCampPtr->patrol_unit_count ==
          0) // there could be chances that there are no some for mobilizing the
             // units
        return;
      ...
      memcpy(coming_unit_array, bestCampPtr->patrol_unit_array,
             sizeof(coming_unit_array));
      coming_unit_count = bestCampPtr->patrol_unit_count;

      //---- order the unit to move to this camp now ---//

      unit_group.set(bestCampPtr->patrol_unit_array,
                     bestCampPtr->patrol_unit_count);
  ...
  if (!overseer_recno)
    nationPtr->add_action(loc_x1, loc_y1, -1, -1, ACTION_AI_ASSIGN_OVERSEER,
                          FIRM_FORT);

  //---- calculate the number of soldiers needed to be recruited ----//

  int combatLevelNeeded = ai_combat_level_needed();
  int recruitCount = (combatLevelNeeded - total_combat_level()) /
                     50; // assuming each soldier has 50 points of protection
  ...
  if (overseer_recno == nationPtr->king_unit_recno) // recruit more soldiers if
                                                    // the commander is the king
  {
    extraRecruit += 10;
  ...
  if (extraRecruit <
      -MAX_SOLDIER) // set a minimum for extraRecruit, if it is too low, the
                    // recruitCount will be affected by it too much. We want to
                    // the combat level difference have an affect here
  ...
  if (extraRecruit > 0)
    ...
    recruitCount +=
        extraRecruit / 2; // the impact is small if extraDiff is negative
...
int FirmCamp::ai_recruit(int recruitCount) {
  ...
  recruitCount =
      MIN(recruitCount, MAX_SOLDIER - soldier_count - patrol_unit_count);
  ...
  if (recruitCount > 0)
    nation_array[nation_recno]->add_action(loc_x1, loc_y1, -1, -1,
                                           ACTION_AI_ASSIGN_SOLDIER, firm_id,
                                           recruitCount);
...
int FirmCamp::ai_combat_level_needed() {
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    if (townPtr->nation_recno == nation_recno) {
      if (townPtr->should_ai_migrate()) // no need for this if this town is
                                        // going to migrate
        continue;

      combatNeeded +=
          townPtr->population * 10; // 30 people need 300 combat levels

      if (townPtr->is_base_town)
        combatNeeded += townPtr->population *
                        10; // double the combat level need for base town
    }
  }
...
int FirmCamp::total_combat_level() {
  ...
  for (int i = 0; i < soldier_count; i++, soldierPtr++) {
    ...
    totalCombatLevel +=
        soldierPtr->hit_points; // use it points instead of combat level because
                                // hit_points represent both combat level and
                                // hit points left
    ...
    if (!unitInfo->class_info.has_combat_level) {
      ...
      totalCombatLevel +=
          weaponPower * 30; // extra_para keeps the weapon version
    }
    // ####### end Gilbert 24/3 #######//
  ...
  if (overseer_recno) {
    ...
    totalCombatLevel +=
        totalCombatLevel * (int)unitPtr->skill_level() /
        150; // divided by 150 instead of 100 because while the attacking
             // ability of the unit is affected by the general, the hit points
             // isn't, so we shouldn't do a direct multiplication.
...
int FirmCamp::ai_should_close() {
  ...
  return linked_town_count == 0 && linked_firm_count == 0 &&
         nationPtr->total_camp_unit_space(0) >=
             nationPtr->total_unit_count + MAX_SOLDIER;
}
...
void FirmCamp::think_capture() {
  if (is_attack_camp) // if this camp has been assigned to an attack mission
                      // already
    return;
  ...
  if (targetTown->nation_recno) {
    //--- if there are any defenses (camps and mobile units) on the target town,
    //destroy them all first -----//

    if (ownNation->attack_enemy_town_defense(targetTown) !=
        -1) // only proceed further when the result is -1, which means no
            // defense on the target town, no attacking is needed.
      return;
  }
  ...
  if (nation_recno)
    ...
    resistanceDiff =
        targetTown->resistance(nation_recno) - MIN_INDEPENDENT_DEFEND_LOYALTY;

  if (targetTown->population >=
      5) // if the pop count is lower than 5, ingore it
  {
    defenseCombatLevel +=
        targetTown->possible_defender_combat_level(nation_recno);
  }

  //--- try using spies if there are defense forces and the nation likes to use
  //spies ---//

  if (defenseCombatLevel >
      0) // && ownNation->pref_spy >= 50 && misc.random(3)==0 )		// 1/3
         // chance of using spies here, otherwise only use spies when we are not
         // strong enough to take over the village by force
  {
    if (targetTown->nation_recno ==
        0) // if the camp is trying to capture an independent town, the
           // leadership and race id. of the overseer matters.
    {
      if (think_assign_better_overseer(
              targetTown)) // a better general is being assigned to this firm,
                           // wait for it
        return;
      ...
      if (defenseCombatLevel > 100 + ownNation->pref_military_courage &&
          resistanceDiff > (100 - ownNation->pref_peacefulness) /
                               5) // depending on the peacefulness, the nation
                                  // won't attack if resistance > (0-20)
      {
        return;
      }
    } else {
      //--- don't attack if the target nation's military rating is higher than
      //ours ---//
  ...
  if (rc) {
    ...
    defense_flag = 0; // turn off the defense flag during capturing so the
                      // general is staying in the base to influence the town

    //--- as the current commander has been out to attack the town by
    //ai_attack_target(), we need to assign him back to the camp for influencing
    //the town and eventually capture it ---//
...
int FirmCamp::think_capture_target_town() {
  ...
  if (nation_array[nation_recno]->is_action_exist(
          loc_x1, loc_y1, -1, -1, ACTION_AI_ASSIGN_SOLDIER, firm_id))
    return 0;

  //-- decide which town to attack (only when the camp is linked to more than
  //one town ---//
  ...
  for (i = 0; i < linked_town_count; i++) {
    ...
    if (!townPtr->nation_recno) // only capture independent town
    {
      ...
      if (resistanceDec > 0 &&
          curResistance >
              25 - 25 * ownNation->pref_peacefulness /
                       100 && // for nation that has a high peacefulness
                              // preference they will wait for the loyalty to
                              // fall and try not to attack the town unless
                              // necessary
          townPtr->population >= 5) // if it's less than 5, don't count it, as
                                    // that it will be easy to attack
      {
    ...
    } else //-------- if it's a nation town ---------//
    {
      NationRelation *nationRelation =
          ownNation->get_relation(townPtr->nation_recno);
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    for (int j = 0; j < townPtr->linked_firm_count; j++) {
      ...
      if (firmPtr->firm_id == FIRM_LAIR &&
          ...
          && townPtr->resistance(firmPtr->nation_recno) <
                 MONSTER_COLLECT_TOWN_TRIBUTE_LOYALTY)
      // ####### end Gilbert 3/2 ########//
      {
        //--- if this is an enslaving town, we should attack it ---//

        if (nation_array[nation_recno]->ai_attack_target(
                firmPtr->loc_x1, firmPtr->loc_y1, firmPtr->obj_power(), 0, 0, 0,
                firm_recno, 1)) {
          return 1;
        }
      }
...
//
int FirmCamp::ai_capture_independent_town(Town *targetTown,
                                          int defenseCombatLevel) {
  ...
  int combatDiff =
      defenseCombatLevel * (150 + nationPtr->pref_force_projection / 2) / 100 -
      curCombatLevel; // try to recruit soldiers based on the force projection
                      // perference

  if (combatDiff > 0) {
    if (ai_recruit(combatDiff)) // try to recruit new soldiers to increase the
                                // combat ability of the troop
      return 0;
  }

  combatDiff =
      defenseCombatLevel * (200 - nationPtr->pref_military_courage / 2) / 100 -
      curCombatLevel;

  //---------- attack the target town now ----------//

  if (nation_array[nation_recno]->ai_attack_target(
          targetTown->loc_x1, targetTown->loc_y1, defenseCombatLevel, 0, 0, 0,
          firm_recno))
    return 1;
...
int FirmCamp::think_capture_return() {
  ...
  if (returnCamp) {
    ...
    ai_capture_town_recno =
        0; // turn it on again after the capturing plan is finished
    defense_flag = 1;
    return 1;
  }
...
int FirmCamp::think_assign_better_overseer(Town *targetTown) {
  ...
  //-- if the resistance of the majority race has already dropped to its lowest
  //possible --//
  ...
  if (targetTown->resistance(nation_recno) <=
    ...
    if (targetTown->resistance(nation_recno) > 30) {
      if (think_assign_better_overseer2(targetTown->town_recno,
                                        targetTown->race_id))
        return 1;
    }
  }

  //-- if the resistance of the 2nd majority race has already dropped to its
  //lowest possible --//
...
int FirmCamp::think_assign_better_overseer2(int targetTownRecno, int raceId) {
  ...
  int bestUnitRecno =
      ownNation->find_best_capturer(targetTownRecno, reduceResistance);

  if (!bestUnitRecno ||
      bestUnitRecno == overseer_recno) // if we already got the best one here
    return 0;

  err_when(unit_res[unit_array[bestUnitRecno]->unit_id]->is_civilian);

  //---- only assign new overseer if the new one's leadership is significantly
  //higher than the current one ----//

  if (overseer_recno && unit_array[bestUnitRecno]->skill_level() <
                            unit_array[overseer_recno]->skill_level() + 15) {
    return 0;
  }
  ...
  int actionRecno =
      ownNation->add_action(loc_x1, loc_y1, -1, -1, ACTION_AI_ASSIGN_OVERSEER,
                            firm_id, 1, bestUnitRecno);
...
int FirmCamp::ai_capture_enemy_town(Town *targetTown, int defenseCombatLevel) {
  ...
  //--- use all camps to attack if we have money and we are stronger than the
  //enemy ---//

  if (ourMilitary - enemyMilitary > 30 &&
      ownNation->ai_should_spend(ownNation->pref_military_courage / 2))
    useAllCamp = 1;
  ...
  return nation_array[nation_recno]->ai_attack_target(
      targetTown->loc_x1, targetTown->loc_y1, defenseCombatLevel, 0, 0, 0,
      firm_recno, useAllCamp);
}
...
int FirmCamp::think_capture_use_spy(Town *targetTown) {
  ...
  int spyCount;
  int curSpyLevel = spy_array.total_spy_skill_level(
      SPY_TOWN, targetTown->town_recno, nation_recno, spyCount);
...
// Think about using spies for capturing the target town.
//
int FirmCamp::think_capture_use_spy2(Town *targetTown, int raceId,
                                     int curSpyLevel) {
  Nation *ownNation = nation_array[nation_recno];
...
void FirmCamp::ai_update_link_status() {
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    //---- don't try to capture other nation's towns unless the AI is at war or
    //tense with the nation ----//

    if (townPtr->nation_recno &&
        ownNation->get_relation_status(townPtr->nation_recno) <=
            RELATION_TENSE) // hostile or tense
    {
      continue;
    }
...
int FirmCamp::ai_has_excess_soldier() {
  ...
  if (ai_capture_town_recno) // no if the camp is trying to capture an
                             // independent town
    return 0;
...
int FirmCamp::think_employ_to_capture() {
  ...
  if (nationPtr->pref_peacefulness > 50) {
    ...
    int importanceRating =
        50 + nationPtr->pref_research / 2 - nationPtr->ai_research_count * 10;

    if (nationPtr->ai_research_count ==
        0) // if we haven't had any towers of science yet
      importanceRating += 50;
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    //--- check if a unit is already on its way to build firm for capturing this
    //town ---//

    if (nationPtr->is_action_exist(
            -1, -1, townPtr->loc_x1, townPtr->loc_y1, ACTION_AI_BUILD_FIRM,
            FIRM_RESEARCH)) // 2-only check the reference location, ignore the
                            // action location
    {
      continue;
    }
    ...
    if (nationPtr->find_best_place_loc(FIRM_RESEARCH, townPtr->loc_x1,
                                       townPtr->loc_y1, buildXLoc, buildYLoc)) {
      nationPtr->add_action(buildXLoc, buildYLoc, townPtr->loc_x1,
                            townPtr->loc_y1, ACTION_AI_BUILD_FIRM,
                            FIRM_RESEARCH, 1, 0, nationPtr->race_id);
...
//
void FirmCamp::think_linked_town_change_nation(int linkedTownRecno,
                                               int oldNationRecno,
                                               int newNationRecno) {
  //-----------------------------------------------//
  //
  ...
  if (oldNationRecno == 0 && newNationRecno > 0 &&
      newNationRecno != nation_recno) {
    ...
    for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++) {
      ...
      if (locPtr->unit_recno(UNIT_LAND)) {
        ...
        if (unitPtr->cur_action == SPRITE_ATTACK &&
            nationPtr->get_relation_status(unitPtr->nation_recno) ==
                RELATION_HOSTILE) {
    ...
    for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++) {
      ...
      if (baseObj->nation_recno &&
          nationPtr->get_relation_status(baseObj->nation_recno) ==
              RELATION_HOSTILE) {
        if (nationPtr->ai_attack_target(
                baseObj->obj_loc_x1(), baseObj->obj_loc_y1(),
                baseObj->obj_power(), 0, 0, 0, firm_recno)) {
          return 1;
        }
      }
    }
...
void FirmCamp::think_change_town_link() {
  ...
  for (int i = linked_town_count - 1; i >= 0; i--) {
    ...
    int enableFlag = townPtr->nation_recno == 0 ||
                     ownNation->get_relation(townPtr->nation_recno)->status ==
                         RELATION_HOSTILE;
...
int FirmCamp::think_assign_better_commander() {
  ...
  int actionRecno = ownNation->is_action_exist(
      loc_x1, loc_y1, -1, -1, ACTION_AI_ASSIGN_OVERSEER, firm_id);
  ...
  if (actionRecno) {
    ...
    //--- however, if the action hasn't been processed, we still try to use the
    //approach here ---//
  }
  ...
  if (overseer_recno) {
    ...
    if (is_human())
      bestLeadership = curLeadership + 10 +
                       ownNation->pref_loyalty_concern /
                           10; // nations that have higher loyalty concern will
                               // not switch commander too frequently
    else
      bestLeadership = curLeadership + 5 +
                       ownNation->pref_loyalty_concern /
                           20; // monsters have less concern on this
  }
  ...
  if (bestFirmRecno)
    unitRecno = firm_array[bestFirmRecno]->cast_to_FirmCamp()->mobilize_soldier(
        bestSoldierId, COMMAND_AI);
  ...
  if (!unitRecno) {
    for (i = ownNation->ai_inn_count - 1; i >= 0 && !unitRecno; i--) {
      FirmInn *firmInn =
          firm_array[ownNation->ai_inn_array[i]]->cast_to_FirmInn();
  ...
  return ownNation->add_action(
      loc_x1, loc_y1, -1, -1, ACTION_AI_ASSIGN_OVERSEER, firm_id, 1, unitRecno);
}
...
int FirmCamp::cur_commander_leadership(int bestRaceId) {
  ...
  if (overseer_recno) {
    ...
    if (unitCommander->race_id == bestRaceId)
      ...
      commanderLeadership = unitCommander->skill_level() /
                            2; // divided by 2 if the race doesn't match
  }
...
int FirmCamp::new_commander_leadership(int newRaceId, int newSkillLevel) {
  ...
  if (overseer_recno) {
    if (newRaceId == bestRaceId)
      ...
      commanderLeadership =
          newSkillLevel / 2; // divided by 2 if the race doesn't match
  }
...
//
int FirmCamp::think_upgrade_to_fort() { return 0; }
