...
void FirmCamp::process_ai()
{
    ...
    if (info.game_date % 15 ==
        (firm_recno + 10) % 15) // do not call too often as the penalty of accumulation is 10 days
        think_use_cash_to_capture();

    //------- assign overseer and soldiers -------//

    if (info.game_date % 15 == firm_recno % 15) // do not call too often because when an AI action is
    ...
    {
        should_close_flag = 0; // disable should_close_flag when trying to capture a firm,
                               // should_close_flag is set in process_common_ai()

        if (nation_array[nation_recno]->attack_camp_count == 0 && // only when attack_camp_count==0, the attack mission
                                                                  // is complete
            patrol_unit_count == 0)
        {
    ...
    if (info.game_date % intervalDays == firm_recno % intervalDays) // do not call too often because when an AI action
                                                                    // is queued, it will take a while to carry it out
        think_capture();

    //------ think attack enslaving fryhtans ------//

    if (info.game_date % intervalDays ==
        (firm_recno + 5) % intervalDays) // do not call too often because when an AI action is
                                         // queued, it will take a while to carry it out
...
void FirmCamp::ai_attack_town_defender(Unit *attackerUnit)
{
    ...
    else if (attackerUnit->cur_order.mode == UNIT_ATTACK)
    {
        ...
        if (attackerUnit->cur_order.loc_x == townPtr->loc_x1 && attackerUnit->cur_order.loc_y == townPtr->loc_y1)
        {
            shouldAttackUnit = 1;
        }
    }
    ...
    for (int i = 0; i < unitCount; i++)
    {
        ...
        if (targetUnit->unit_mode == UNIT_MODE_TOWN_DEFENDER && targetUnit->unit_mode_para == ai_capture_town_recno)
        {
...
void FirmCamp::think_recruit()
{
    ...
    if (coming_unit_count > 0)
    {
        ...
        for (int i = 0; i < coming_unit_count; i++)
        {
            ...
            if (unitPtr->nation_recno == nation_recno && unitPtr->cur_order.mode == UNIT_ASSIGN)
            {
         ...
         nationPtr->firm_should_close_array[FIRM_FORT - 1] > 0))
    {
        ...
        for (int i = nationPtr->ai_camp_count - 1; i >= 0; i--)
        {
            ...
            if (firmCamp->should_close_flag && (firmCamp->overseer_recno || firmCamp->soldier_count > 0))
            {
                curRating = 100 - misc.points_distance(center_x, center_y, firmCamp->center_x, firmCamp->center_y);
        ...
        if (bestCampPtr)
        {
            ...
            if (bestCampPtr->patrol_unit_count == 0) // there could be chances that there are no some for mobilizing the
                                                     // units
                return;
    ...
    int recruitCount =
        (combatLevelNeeded - total_combat_level()) / 50; // assuming each soldier has 50 points of protection
    ...
    if (extraRecruit < -MAX_SOLDIER) // set a minimum for extraRecruit, if it is too low, the
...
int FirmCamp::total_combat_level()
{
    ...
    for (int i = 0; i < soldier_count; i++, soldierPtr++)
    {
        ...
        totalCombatLevel += soldierPtr->hit_points; // use it points instead of combat level because
    ...
    if (overseer_recno)
    {
        ...
        totalCombatLevel += totalCombatLevel * (int)unitPtr->skill_level() /
                            150; // divided by 150 instead of 100 because while the attacking
                                 // ability of the unit is affected by the general, the hit
                                 // points isn't, so we shouldn't do a direct multiplication.
...
void FirmCamp::think_capture()
{
    ...
    if (defenseCombatLevel > 0) // && ownNation->pref_spy >= 50 && misc.random(3)==0 )		// 1/3
                                // chance of using spies here, otherwise only use spies when we are not
                                // strong enough to take over the village by force
    {
        if (targetTown->nation_recno == 0) // if the camp is trying to capture an independent town, the
                                           // leadership and race id. of the overseer matters.
        {
            if (think_assign_better_overseer(targetTown)) // a better general is being assigned to this firm,
                                                          // wait for it
                return;
            ...
            if (defenseCombatLevel > 100 + ownNation->pref_military_courage &&
                resistanceDiff > (100 - ownNation->pref_peacefulness) / 5) // depending on the peacefulness, the nation
        ...
        else
        {
            ...
            if (nation_array[targetTown->nation_recno]->military_rank_rating() > ownNation->military_rank_rating())
            {
                return;
            }
        }
    ...
    if (rc)
    {
        ...
        // ai_attack_target(), we need to assign him back to the camp for
        // influencing the town and eventually capture it ---//
...
int FirmCamp::think_capture_target_town()
{
    ...
    for (i = 0; i < linked_town_count; i++)
    {
        ...
        if (!townPtr->nation_recno) // only capture independent town
        {
            ...
            if (resistanceDec > 0 &&
                curResistance >
                    25 - 25 * ownNation->pref_peacefulness / 100 && // for nation that has a high peacefulness
                                                                    // preference they will wait for the loyalty
                                                                    // to fall and try not to attack the town
                                                                    // unless necessary
...
int FirmCamp::think_attack_enslaving_monster()
{
    ...
    for (int i = 0; i < linked_town_count; i++)
    {
        ...
        for (int j = 0; j < townPtr->linked_firm_count; j++)
        {
            ...
            // ####### begin Gilbert 3/2 ########//
            if (firmPtr->firm_id == FIRM_LAIR && firmPtr->nation_recno // avoid townPtr->resistance() error
            ...
            // ####### end Gilbert 3/2 ########//
            {
                ...
                if (nation_array[nation_recno]->ai_attack_target(firmPtr->loc_x1, firmPtr->loc_y1, firmPtr->obj_power(),
                                                                 0, 0, 0, firm_recno, 1))
                {
                    return 1;
                }
...
int FirmCamp::ai_capture_independent_town(Town *targetTown, int defenseCombatLevel)
{
    ...
    combatDiff = defenseCombatLevel * (200 - nationPtr->pref_military_courage / 2) / 100 - curCombatLevel;

    //---------- attack the target town now ----------//

    if (nation_array[nation_recno]->ai_attack_target(targetTown->loc_x1, targetTown->loc_y1, defenseCombatLevel, 0, 0,
                                                     0, firm_recno))
        return 1;
...
int FirmCamp::think_assign_better_overseer(Town *targetTown)
{
    ...
    if (ownNation->is_action_exist(loc_x1, loc_y1, -1, -1, ACTION_AI_ASSIGN_OVERSEER, firm_id))
    {
    ...
    if (targetTown->resistance(nation_recno) <= (float)(targetTown->target_resistance(nation_recno) + 1))
    {
...
int FirmCamp::think_assign_better_overseer2(int targetTownRecno, int raceId)
{
    ...
    if (overseer_recno && unit_array[bestUnitRecno]->skill_level() < unit_array[overseer_recno]->skill_level() + 15)
    {
        return 0;
    }
    ...
    int actionRecno =
        ownNation->add_action(loc_x1, loc_y1, -1, -1, ACTION_AI_ASSIGN_OVERSEER, firm_id, 1, bestUnitRecno);
...
int FirmCamp::ai_capture_enemy_town(Town *targetTown, int defenseCombatLevel)
{
    ...
    else if (config.ai_aggressiveness >= OPTION_MODERATE && !targetNation->is_ai() && ourMilitary > enemyMilitary)
    {
        if (config.ai_aggressiveness >= OPTION_HIGH || ownNation->pref_peacefulness < 50)
        {
            useAllCamp = 1;
        }
    }

    return nation_array[nation_recno]->ai_attack_target(targetTown->loc_x1, targetTown->loc_y1, defenseCombatLevel, 0,
                                                        0, 0, firm_recno, useAllCamp);
}
...
int FirmCamp::think_employ_to_capture()
{
    ...
    for (int i = 0; i < linked_town_count; i++)
    {
        ...
        if (nationPtr->is_action_exist(-1, -1, townPtr->loc_x1, townPtr->loc_y1, ACTION_AI_BUILD_FIRM,
        ...
        {
            nationPtr->add_action(buildXLoc, buildYLoc, townPtr->loc_x1, townPtr->loc_y1, ACTION_AI_BUILD_FIRM,
...
int FirmCamp::think_attack_nearby_enemy()
{
    ...
    for (yLoc = yLoc1; yLoc <= yLoc2; yLoc++)
    {
        ...
        for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++)
        {
            ...
            if (baseObj->nation_recno && nationPtr->get_relation_status(baseObj->nation_recno) == RELATION_HOSTILE)
            {
                if (nationPtr->ai_attack_target(baseObj->obj_loc_x1(), baseObj->obj_loc_y1(), baseObj->obj_power(), 0,
                                                0, 0, firm_recno))
                {
                    return 1;
                }
...
void FirmCamp::think_change_town_link()
{
    ...
    for (int i = linked_town_count - 1; i >= 0; i--)
    {
        ...
        int enableFlag =
            townPtr->nation_recno == 0 || ownNation->get_relation(townPtr->nation_recno)->status == RELATION_HOSTILE;
...
int FirmCamp::think_assign_better_commander()
{
    ...
    int actionRecno = ownNation->is_action_exist(loc_x1, loc_y1, -1, -1, ACTION_AI_ASSIGN_OVERSEER, firm_id);
    ...
    if (overseer_recno)
    {
        ...
        if (is_human())
            bestLeadership =
                curLeadership + 10 + ownNation->pref_loyalty_concern / 10; // nations that have higher loyalty concern
                                                                           // will not switch commander too frequently
        else
            bestLeadership =
                curLeadership + 5 + ownNation->pref_loyalty_concern / 20; // monsters have less concern on this
    }
    ...
    if (!unitRecno)
    {
        for (i = ownNation->ai_inn_count - 1; i >= 0 && !unitRecno; i--)
        {
            ...
            for (int j = 0; j < firmInn->inn_unit_count; j++)
            {
                ...
                if (unit_res[innUnit->unit_id]->race_id == majority_race() && innUnit->skill_level() > bestLeadership)
                {
                    int importanceRating =
                        30 + (innUnit->skill_level() - curLeadership) * (150 + ownNation->pref_hire_unit) / 100;
    ...
    for (i = unit_array.size(); i > 0; i--)
    {
        ...
        {
            int importanceRating =
                30 + (unitPtr->skill_level() - curLeadership) * (150 + ownNation->pref_hire_unit) / 100;
