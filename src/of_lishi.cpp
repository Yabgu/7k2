...
void FirmLishorr::put_info(int refreshFlag)
{
    ...
    if (max_hit_points())
    {
        ...
        font_whbl.center_put(INFO_X1 + 169, INFO_Y1 + 45, INFO_X1 + 191, INFO_Y1 + 57,
                             misc.format((int)max_hit_points(), 4));
    }

    //	if (firm_id != FIRM_WILDE_LISHORR)
    //		font_whbl.center_put( INFO_X1 +12, INFO_Y1 +9, INFO_X2, INFO_Y1
    //+21, "Lishorr", 0, 1 ); 	else 		font_whbl.center_put( INFO_X1
    //+12, INFO_Y1 +9,
    // INFO_X2, INFO_Y1 +21, "Wilde Lishorr", 0, 1 );

    FirmBuild *firmBuild = firm_res.get_build(firm_build_id);
    font_whbl.center_put(INFO_X1 + 12, INFO_Y1 + 9, INFO_X2, INFO_Y1 + 21, firmBuild->name, 0, 1);
    ...
    FirmBitmap *firmBitmap = firm_res.get_bitmap(firmBuild->first_bitmap(firm_cur_frame[0]));
    if (firmBitmap)
    {
        ...
        //+firmBitmap->offset_x, 		INFO_Y1 +158 +firmBitmap->offset_y,
        // bitmapPtr,
        // colorRemapTable);
    }
}
