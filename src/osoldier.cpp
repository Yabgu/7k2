...
//
Soldier::Soldier() { memset(this, 0, sizeof(Soldier)); }
...
void Soldier::change_hit_points(int changePoints) {
  //	err_when( town_recno );		// for those live in town, their loyalty
  //are based on town people loyalty.
...
int Soldier::max_attack_range() {
  ...
  AttackInfo *attackInfo =
      unit_res.get_attack_info(unit_res[unit_id]->first_attack);
...
//									to. Since Soldier does
//not contain any info 									about the nation. This info has to be passed by 								   other
//functions.
//
