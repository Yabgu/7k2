    ...
    if (!remote.is_enable())
    {
        ...
        else
        {
            if (game.game_mode != GAME_TUTORIAL && !isLoadedGame)
            {
                ...
                tips_res.box_active = !player_profile.hide_tips || (mouse.skey_state & SHIFT_KEY_MASK);
...
//-*********** syn game test ***********-//
    ...
    if (!config.explore_whole_map && nation_array.player_recno && !nation_array.is_deleted(nation_array.player_recno))
    {
        for (short nationRecno = 1; nationRecno <= nation_array.size(); ++nationRecno)
        {
            if (nationRecno == nation_array.player_recno || nation_array.is_deleted(nationRecno))
                continue;
    ...
    while (1)
    {
        ...
        if (!paused_flag && active_flag ||
            use_true_front && remote.is_enable()) // if switch tasked, still run in multiplay mode
        // ####### end Gilbert 11/5 #######//
        {
#ifdef DEBUG
            ...
            if (!paused_flag && active_flag)
            {
                if (rc)
                {
                    ...
                    if (skipFrameCount >= skipFrameMax || exceededDispTime < targetFrameTime)
                    {
                        misc.lock_seed();
                        disp_frame();
                        misc.unlock_seed();
                        ...
                        if (firstUnreadyTime && misc.get_time() - firstUnreadyTime > 5000)
                        {
                            ...
                            for (int nationRecno = 1; nationRecno <= MAX_NATION; ++nationRecno)
                            {
                                    ...
                                    !nation_array.is_deleted(nationRecno))
                                {
                                    ...
                                    char colorCodeStr[] = " 0";
                                    colorCodeStr[1] =
                                        FIRST_NATION_COLOR_CODE_IN_TEXT + nation_array[nationRecno]->color_scheme_id;
                                    // x2 = font_news.put( x2, y, colorCodeStr );

                                    int x2 = font_news.put(x, y,
                                                           text_game_menu.str_mp_wait_player(
                                                               nation_array[nationRecno]->nation_name(), colorCodeStr));
...
void Sys::yield()
{
    ...
    if (remote.is_enable())
    {
        ...
        remote.process_specific_msg(MSG_SET_SPEED); // need to test it here for restoring the speed from
                                                    // frozen to normal
                                                    // ####### begin Gilbert 28/6 #######//
...
#define RESEND_AGAIN_TIME_OUT 1000 // keep resending if no responses
    ...
    else if (remote_send_success_flag && remote.has_send_frame(nation_array.player_recno, frame_count) &&
             (~nation_array)->next_frame_ready == 0)
    {
        ...
        if (remote_send_success_flag) // still failed, try again next time
        {
            ...
            remote.init_send_queue(frame_count + 1,
    ...
    for (nationRecno = nation_array.size(); nationRecno > 0; nationRecno--)
    {
        ...
        if (nationPtr->is_remote() && (remote.has_send_frame(nationRecno, frame_count) && !nationPtr->next_frame_ready))
        {
            if (unreadyPlayerFlag)
    ...
    for (int i = nation_array.size(); i > 0; i--)
    {
        ...
        nation_array[i]->next_frame_ready = 0; // -- instead of set to 0, set it may be 2 if it has just received an
                         ...
                         // game
    {
        if (next_frame_time < 1000) // the DWORD variable has been overflow
        {
            if (curTime < next_frame_time ||
                curTime >= 1000) // >= 1000 if the curTime has been overflow yet, wait for it
...
void Sys::process()
{
    ...
    frame_count++;
    is_sync_frame = frame_count % 3 == 0; // check if sychronization should take place at this
