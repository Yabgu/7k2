  ...
  if (!remote.is_enable()) {
    if (scenario_cheat_flag) {
      ...
      if (game.game_mode != GAME_TUTORIAL && !isLoadedGame) {
        ...
        tips_res.box_active =
            !player_profile.hide_tips || (mouse.skey_state & SHIFT_KEY_MASK);
...
void Sys::main_loop(int isLoadedGame) {
  ...
  if (!isLoadedGame) {
    day_frame_count =
        0; // for determining when the day counter should be increased.
    frame_count = 1;
  }
  ...
  last_frame_time =
      misc.get_time() +
      60000; // plus 60 seconds buffer for game loading/starting time
  is_sync_frame = 0;
...
//-*********** syn game test ***********-//
  ...
  if (!config.explore_whole_map && nation_array.player_recno &&
    ...
    for (short nationRecno = 1; nationRecno <= nation_array.size();
      ...
      if (relation->contact_msg_flag && !relation->has_contact) {
        ...
        short *shortPtr = (short *)remote.new_send_queue_msg(MSG_NATION_CONTACT,
                                                             2 * sizeof(short));
  ...
  while (1) {
    ...
    if (!paused_flag && active_flag ||
        use_true_front &&
            remote.is_enable()) // if switch tasked, still run in multiplay mode
    // ####### end Gilbert 11/5 #######//
    {
#ifdef DEBUG
      ...
      yield(); // could be improved, give back the control to Windows, so it can
               // do some OS management. Maybe call WaitMessage() here and set
               // up a timer to get messages regularly.
      ...
      DWORD markTime =
          misc.get_time(); // a time taken earlier than should_next_frame takes
      ...
      if (config.frame_speed > 0) // 0-frozen
      {
        if (remote.is_enable()) // && is_sync_frame )
        {
          ...
          misc.unlock_seed();
          rc =
              is_mp_sync(&unreadyPlayerFlag); // if all players are synchronized
          misc.lock_seed();
        } else
        ...
        if (rc) {
          ...
          if (remote.is_enable())
            misc.lock_seed(); // such that random seed is unchanged outside
                              // sys::process()
          LOG_END;

          // -------- compare objects' crc --------- //
          if (remote.is_enable() && (remote.sync_test_level & 2) &&
              (frame_count % (remote.get_process_frame_delay() + 3)) == 0) {
      ...
      if (!paused_flag && active_flag) {
        if (rc) {
          // ######## begin Gilbert 13/2 #######//
          // if the time after disp_frame exceeded the expected time limit
          // (1000/config.frame_speed), acculumate the exceeded time if the
          // acculumated time exceeded time of one frame
          // (1000/config.frame_speed), skip frame
          unsigned long targetFrameTime =
              config.frame_speed > 0 ? (1000 / config.frame_speed) : 1000;
          ...
          if (skipFrameCount >= skipFrameMax ||
            ...
            if (dispFrameTime - lastDispFrameTime >= targetFrameTime) {
              ...
              exceededDispTime +=
                  dispFrameTime - lastDispFrameTime - targetFrameTime;
            } else {
              exceededDispTime = 0;
            }
            skipFrameCount = 0;
          ...
          if (config.frame_speed == 0 ||
              markTime - lastDispFrameTime >=
                  DWORD(1000 / config.frame_speed) ||
      ...
      if (mp_save_flag && mp_save_frame == frame_count) {
        ...
        if (nation_array
                .player_recno) // only save then the player is still in the game
        {
          // game_file.save_game(
          // player_profile.save_game_path(remote.save_file_name) );
          game_file.save_game(game_file_array.save_default_dir,
...
#ifdef DEBUG
      unsigned long interLoopTime =
          startTime - lastEndTime; // we may also interested in this timing
...
void Sys::unpause() {
  ...
  mouse
      .update_skey_state(); // update ctrl/shift/alt key state after switch task
...
void Sys::yield() {
  ...
  if (remote.is_enable()) {
    ...
    remote.process_specific_msg(
        MSG_SET_SPEED); // need to test it here for restoring the speed from
                        // frozen to normal
                        // ####### begin Gilbert 28/6 #######//
...
#define RESEND_TIME_OUT                                                        \
  2000 // if the other machines still aren't ready after 2 seconds, send the
       // notification again
#define RESEND_AGAIN_TIME_OUT 1000 // keep resending if no responses
  ...
  if (!nation_array.player_recno) {
    ...
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_NEXT_FRAME, sizeof(short));

    shortPtr[0] = nation_array.player_recno; // short_para1 is the nation recno
                                             // of the current player

    //------------ queue MSG_QUEUE_TRAILER ----------//

    shortPtr =
        (short *)remote.new_send_queue_msg(MSG_QUEUE_TRAILER, sizeof(short));
    shortPtr[0] = nation_array.player_recno; // short_para1 is the nation recno
                                             // of the current player

    //------ copy all queued action to our own receive buffer to merge with
    //other player's action ----//

    remote.append_send_to_receive();

    //--- copy the whole queue to a buffer in case of resend request from other
    //players ---//
    ...
    remote_send_success_flag =
        remote
            .send_queue_now(); // if not sent successfully, try again next time
    ...
    if (remote_send_success_flag) // still failed, try again next time
    {
      ...
      remote.init_send_queue(
          frame_count + 1,
          nation_array.player_recno); // frame_count, initialize for next
                                      // frame's send queue
      // sent random seed
      char *p = (char *)remote.new_send_queue_msg(MSG_TELL_RANDOM_SEED,
  ...
  for (nationRecno = nation_array.size(); nationRecno > 0; nationRecno--) {
    ...
    if (nationPtr->is_remote() &&
        (remote.has_send_frame(nationRecno, frame_count) &&
         !nationPtr->next_frame_ready)) {
      if (unreadyPlayerFlag)
  ...
  for (int i = nation_array.size(); i > 0; i--) {
    ...
    nation_array[i]->next_frame_ready =
        0; // -- instead of set to 0, set it may be 2 if it has just received an
           // notifying signal for the further next frame from a player as it
           // also sent out a next frame ready msg to all other players
  }
...
int Sys::should_next_frame() {
  ...
  if (next_frame_time) // if next_frame_time==0, it's the first frame of the
                       // game
  {
    if (next_frame_time < 1000) // the DWORD variable has been overflow
    {
      if (curTime < next_frame_time ||
          curTime >=
              1000) // >= 1000 if the curTime has been overflow yet, wait for it
                    // to overflow so we can compare it when next_frame_time
        return 0;
...
void Sys::process() {
  ...
  frame_count++;
  is_sync_frame =
      frame_count % 3 == 0; // check if sychronization should take place at this
                            // frame (for handling one sync per n frames)
...
void Sys::set_speed(int frameSpeed, int remoteCall) {
  ...
  if (remote.is_enable() && !remoteCall) {
    ...
    *((short *)remoteMsg->data_buf) = frameSpeed;
    ...
    remote.send_free_msg(
        remoteMsg); // send out the message and free it after finishing sending
  }

  //---------- set the speed now ----------//

  if (config.frame_speed == 0) // if it's currently frozen, set last_frame_time
                               // to avoid incorrect timeout
    last_frame_time = misc.get_time();
