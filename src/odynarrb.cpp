...
void DynArrayB::linkout(int delPos)
{
    ...
    {
        empty_room_array = (EmptyRoom *)mem_resize(empty_room_array, (empty_room_num + EMPTY_ROOM_ALLOC_STEP) *
                                                                         sizeof(*empty_room_array));
...
template <typename Visitor> static void visit_dyn_array_b(Visitor *v, DynArrayB *dab)
{
    /* DynArray */
...
{
    if (!write_with_record_size(filePtr, this, &visit_dyn_array_b<FileWriterVisitor>, DYN_ARRAY_B_RECORD_SIZE))
        return 0;
...
int DynArrayB::read_file(File *filePtr)
{
    ...
    if (!read_with_record_size(filePtr, this, &visit_dyn_array_b<FileReaderVisitor>, DYN_ARRAY_B_RECORD_SIZE))
        return 0;
...
int DynArrayB::write_empty_room(File *filePtr)
{
    ...
    if (empty_room_count > 0)
    {
        if (!filePtr->file_write(empty_room_array, sizeof(EmptyRoom) * empty_room_count))
        {
            return 0;
        }
    }
...
int DynArrayB::read_empty_room(File *filePtr)
{
    ...
    if (empty_room_count > 0)
    {
        empty_room_array = (EmptyRoom *)mem_resize(empty_room_array, sizeof(EmptyRoom) * empty_room_count);

        if (!filePtr->file_read(empty_room_array, sizeof(*empty_room_array) * empty_room_count))
        {
            return 0;
        }
    }
