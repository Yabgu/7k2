...
void VgaBuf::tile(int x1,int y1,int x2,int y2,char* tileName)
{
   if( image_init_flag )
   {
      err_if( x1>x2 || y1>y2 || x1<0 || y1<0 || x2>=image_width ||
y2>=image_height ) err_now( "VgaBuf::tile (image)" );
...
// maxValue <int>   indiColor      = color of the indicator [int]   backColor =
// background color
...
//------------- Start of function VgaBuf::indicator --------------//
//
// <int>   barType        = bar style, bit 0 = disp curValue, bit 1 =  disp '/'
// and maxValue, bit 2 clear=mtr_b4 set=mtr_40, bit 3 = use another back buffer
