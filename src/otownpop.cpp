...
void Town::init_pop(int addPop, int addLoyalty, int hasJob, int firstInit)
{
    ...
    else
    {
        if (nation_recno)
        {
            loyalty = (loyalty * (population - addPopulation) + (float)addLoyalty * addPopulation) / population;
        ...
        else
        {
            ...
            {
                resistance_array[j] =
                    (resistance_array[j] * (population - addPopulation) + (float)addLoyalty * addPopulation) /
                    population;
...
void Town::unjob_town_people()
{
    ...
    for (i = 0; i < linked_firm_count; i++)
    {
        ...
        curRating += firmWork->worker_count + 10 +
                     100 * firmWork->worker_count / firmWork->needed_worker_count; // two factors are taken into account
                                                                                   // here: the underemployment rate of
                                                                                   // the firm and the number of workers
                                                                                   // actually working here.

        if (firmWork->productivity == 0) // there is no productivity in the firm, obviously due to some
...
void Town::population_grow()
{
    ...
    if (nation_recno)
        growthInc += population * (100 + quality_of_life / 2) / 100; // quality of life can increase the population
                                                                     // growth by a maximum of 50%.
    else
...
void Town::update_target_loyalty()
{
    ...
    targetLoyalty = 10 + bonus_loyalty +             // 10 base points + loyalty bonus which is set
    ...
    if (loyalty < target_loyalty)
    {
        ...
        if (race_id == RACE_EGYPTIAN && god_res[GOD_EGYPTIAN]->nation_prayer_count(nation_recno) > 0)
            loyaltyInc = loyaltyInc * (MAX_WORKER * 2) /
                         ((MAX_WORKER * 2) + god_res[GOD_EGYPTIAN]->nation_prayer_count(nation_recno));
...
void Town::update_target_resistance()
{
    ...
    for (i = 0; i < linked_firm_count; i++)
    {
        ...
        if (curValue == -1 || newValue < curValue) // need to do this comparsion as there may be more than
    ...
    for (j = 0; j < maxNation; j++)
    {
        ...
        if (target_resistance_array[j] >= 0)
        {
            ...
            if (resistance_array[j] > targetResistance) // only decrease, no increase for resistance
            {
                ...
                if (resistance_array[j] < targetResistance) // avoid resistance oscillate between
        ...
        if (resistance_array[j] < SURRENDER_RESISTANCE) // also values between consider 0 and 1 as zero as
...
void Town::move_pop(Town *destTown)
{
    ...
    if (jobless_population == 0)
    {
        for (int i = 0; i < linked_firm_count; i++)
        {
            ...
            int newDistance =
                misc.points_distance(firmWork->center_x, firmWork->center_y, destTown->center_x, destTown->center_y);
...
#define RESISTANCE_INCREASE                                                                                            \
    2 // if the defender defeat the attackers and return the town with victory, \
      // the resistance will further increase
        ...
        if (unitPtr->nation_recno == nation_recno && unitPtr->unit_mode_para == town_recno)
        {
    ...
    if (nation_recno == 0) // update the town people's combat level with this unit's combat level
    {
        if (wagonPtr)
            town_combat_level =
                ((int)town_combat_level * (population - unitPopulation) + unitPopulation * CITIZEN_COMBAT_LEVEL) /
                population;
        else
...
//						if 0, only a unit will be created and
// no
...
int Town::mobilize_town_people(int decPop, int mobileSpyFlag)
{
    ...
    int unitRecno = unit_array.add_unit(unitId, nation_recno, RANK_SOLDIER, (int)loyalty, xLoc, yLoc);
...
void Town::kill_town_people(int attackerNationRecno)
{
    ...
    if (nation_recno && attackerNationRecno)                     // your people's loyalty decreases
        ...
        nation_array[nation_recno]->civilian_killed(race_id, 0); // but only when your units are killed by enemies,
                                                                 // neutral disasters are not counted
    ...
    if (attackerNationRecno)
    {
        ...
        if (nationPtr->is_monster())
            nationPtr->change_live_points(CITIZEN_COMBAT_LEVEL * 2 *
                                          2); // 1st * 2 - combat level to hit points, 2nd * 2 -
                                              // killing humans gives twice as much live points
    }
}
...
int Town::recruit(bool recruitWagon, char remoteAction)
{
    ...
    if (recruitCount == 1) // recruit a single unit
    {
        ...
        if (spyArrayCount)
            ...
            unitRecno = mobilize_town_people(1, 0); // the 1st parameter is whether decreasing the population
...
//						loyalty to be decreased without
// actual
...
//							 if no, spies are not counted
// in
// the 							 total no. of units that can be
// recruited.
//
...
void Town::evenly_distribute_worker()
{
    ...
    for (i = 0; i < linked_firm_count; i++)
    {
        ...
        for (int j = 0; j < linked_firm_count; j++)
        {
            ...
            if (100 * (otherFirmWork->worker_count - 1) / otherFirmWork->needed_worker_count >= avgEmploymentRate ||
