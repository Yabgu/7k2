...
// [int] hasJob	  - whether the units will have jobs or not after moving into
// this town
//							 (default: 0)
...
void Town::init_pop(int addPop, int addLoyalty, int hasJob, int firstInit) {
  ...
  if (firstInit) // first initialization at the beginning of the game
  {
    if (nation_recno) {
      ...
      for (int j = 0; j < MAX_NATION;
           j++) // reset resistance for non-existing races
      {
...
//
void Town::dec_pop() {
  //--- if all people are employed, we first need to make one unemployed before
  //decreasing the population ---//

  if (jobless_population == 0)
    unjob_town_people(); // ask a unit in the town who is currently at work to
                         // quit his job

  err_when(
      population <
      jobless_population); // population must be larger than jobless_population
  ...
  if (population <= max_layout_pop - POPULATION_PER_HOUSE && population != 0) {
    auto_set_layout();
  }
}
...
void Town::unjob_town_people() {
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    // - high headcount, so that firms with low headcount have a smaller chance
    // of losing employees.
    //
    //------------------------------------//

    curRating = world.distance_rating(center_x, center_y, firmWork->center_x,
                                      firmWork->center_y);

    curRating +=
        firmWork->worker_count + 10 +
        100 * firmWork->worker_count /
            firmWork->needed_worker_count; // two factors are taken into account
                                           // here: the underemployment rate of
                                           // the firm and the number of workers
                                           // actually working here.

    if (firmWork->productivity ==
        0) // there is no productivity in the firm, obviously due to some
           // linkage or capacity problems, have a higher priority for resigning
           // workers from these firm
      curRating += 100;
...
void Town::population_grow() {
  ...
  if (population >= MAX_TOWN_GROWTH_POPULATION ||
      population >= MAX_TOWN_POPULATION)
    return;

  //-------- population growth by birth ---------//

  //-- the population growth in an independent town is slower than in a nation
  //town ---//
  ...
  if (nation_recno)
    growthInc += population * (100 + quality_of_life / 2) /
                 100; // quality of life can increase the population growth by a
                      // maximum of 50%.
  else
    growthInc +=
        population / 2; // slower population growth for independent towns
  ...
  {
    growthInc += growthInc *
                 god_res[GOD_EGYPTIAN]->nation_prayer_count(nation_recno) /
                 (MAX_WORKER * 3);
  }

  if (is_pay_tribute_to_monster()) // if the town is paying tributes to
                                   // monsters, the population growth is reduced
                                   // by half
    growthInc /= 2;
...
void Town::update_target_loyalty() {
  ...
  targetLoyalty = 10 +
                  bonus_loyalty + // 10 base points + loyalty bonus which is set
                                  // when a village change nation itself and the
                                  // bonus will decrease over time
  ...
  if (targetLoyalty <
      0) // targetLoyalty can be negative if there are hostile races conflicts
    targetLoyalty = 0;
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    if (firmCamp->nation_recno ==
        nation_recno) // if the command base belongs to the same nation
    {
      target_loyalty += thisInfluence * firmCamp->loyalty_adjustment() / 100;
    } else if (unitPtr->race_id ==
               race_id) // for enemy camps, only decrease same race peasants
    {
      // -------- if it has wall influence is reduced --------- //

      if (current_wall_level > 0)
        thisInfluence -=
            20 * current_wall_level * (int)hit_points / max_hit_points();
...
void Town::update_loyalty() {
  ...
  if (loyalty < target_loyalty) {
    //--- if this town is linked to enemy camps, but no own camps, no increase
    //in loyalty, only decrease ---//
    ...
    if (race_id == RACE_EGYPTIAN &&
        god_res[GOD_EGYPTIAN]->nation_prayer_count(nation_recno) > 0)
      loyaltyInc = loyaltyInc * (MAX_WORKER * 2) /
                   ((MAX_WORKER * 2) +
                    god_res[GOD_EGYPTIAN]->nation_prayer_count(nation_recno));
...
//
// <int>   nationRecno   	 - recno of the nation which the resistance is
// towards <float> resistanceChange - the amount of change in resistance.
//
...
// -Race of the general and the king (whether its race is the same as the town
// people's race) -Leadership of the general
//
...
void Town::update_target_resistance() {
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    // ------- effect of god --------//
    if (firmCamp->nation_recno &&
        god_res[GOD_MONGOL]->nation_prayer_count(firmCamp->nation_recno) > 0)
      newValue -= 20;
    // ###### end Gilbert 19/11 ######//
    ...
    if (curValue == -1 ||
        newValue <
            curValue) // need to do this comparsion as there may be more than
                      // one command bases of the same nation linked to this
                      // town, we use the one with the most influence.
      curValue = newValue;
  }
}
...
void Town::update_resistance() {
  ...
  for (j = 0; j < maxNation; j++) {
    ...
    if (target_resistance_array[j] >= 0) {
      ...
      if (resistance_array[j] >
          targetResistance) // only decrease, no increase for resistance
      {
        ...
        if (resistance_array[j] <
            targetResistance) // avoid resistance oscillate between
                              // taregtLoyalty-1 and taregtLoyalty+1
    ...
    if (resistance_array[j] <
        SURRENDER_RESISTANCE) // also values between consider 0 and 1 as zero as
                              // they are displayed as 0 in the interface
    {
      if (think_surrender())
        break;
    }
...
int Town::migrate_to(int destTownRecno, char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    ...
    // ##### begin Gilbert 7/10 ######//
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_TOWN_MIGRATE, 2 * sizeof(short));
    // ##### end Gilbert 7/10 ######//
...
void Town::move_pop(Town *destTown) {
  ...
  if (spy_count > 0 && spy_count == population) {
    ...
    spy_array[spyRecno]->spy_place_para =
        destTown->town_recno; // set the place_para of the spy

    spy_count--;
    destTown->spy_count++;
  }

  //--- if all people in town are employed, we need to migrate an employed unit
  //and update his employment record in the firm he works for ---//
  ...
  if (jobless_population == 0) {
    for (int i = 0; i < linked_firm_count; i++) {
      ...
      int newDistance =
          misc.points_distance(firmWork->center_x, firmWork->center_y,
  ...
  if (moveWorkerSuccess) // if the migrated unit can still work in the original
                         // firm, decrease the jobless population counter
    jobless_population++;
...
void Town::assign_unit(int unitRecno) {
  ...
  //--- if aftr assigning the wagon, the population exceeds the maximum, don't
  //assign ---//
...
#define RESISTANCE_INCREASE                                                    \
  2 // if the defender defeat the attackers and return the town with victory,
    // the resistance will further increase
    ...
    if (unitPtr->nation_recno == nation_recno &&
      ...
      //-- if the unit is a town defender, skill.skill_level is temporary used
      //for storing the loyalty that will be added back to the town if the
      //defender returns to the town
      ...
      if (nation_recno) // set the loyalty later for nation_recno > 0
      {
        ...
        {
          //--- if the town is already paying tribute to the nation, don't
          //increase the resistance to that nation ---//

          // ###### patch begin Gilbert 29/3 ######//
          if (last_attack_nation_recno == i + 1 // Unit::mobilize_defender set
                                                // the last_attack_nation_recno
              && !is_pay_tribute_to_monster(i + 1))
          // ###### patch end Gilbert 29/3 ######//
          {
            float newResistance =
                resistance_array[i] + loyaltyInc + RESISTANCE_INCREASE;
  ...
  if (nation_recno ==
      0) // update the town people's combat level with this unit's combat level
  {
    if (wagonPtr)
      town_combat_level =
          ((int)town_combat_level * (population - unitPopulation) +
           unitPopulation * CITIZEN_COMBAT_LEVEL) /
          population;
    else
      town_combat_level = ((int)town_combat_level * (population - 1) +
                           (int)unitPtr->combat_level()) /
                          population;
  }

  //--------- delete the unit --------//

  unit_array.disappear_in_town(unitRecno,
                               town_recno); // the unit disappear from the map
                                            // because it has moved into a town
}
...
//						if 0, only a unit will be created and no
...
int Town::mobilize_town_people(int decPop, int mobileSpyFlag) {
  ...
  int unitRecno = unit_array.add_unit(unitId, nation_recno, RANK_SOLDIER,
                                      (int)loyalty, xLoc, yLoc);
...
// return: <int> recno of the unit created.
//
int Town::mobilize_wagon(int decPop, int wagonPopulation, short spyArrayCount,
                         short *spyArray) {
...
void Town::kill_town_people(int attackerNationRecno) {
  ...
  //	{
  //		int spyRecno = spy_array.find_town_spy(town_recno,
  //misc.random(spy_count)+1 ); 		spy_array.del_spy(spyRecno);
  //	}
  // ###### end Gilbert 18/6 ########//

  //---- killing civilian people decreases loyalty -----//

  if (nation_recno && attackerNationRecno) // your people's loyalty decreases
                                           // because you cannot protect them.
    nation_array[nation_recno]->civilian_killed(
        race_id, 0); // but only when your units are killed by enemies, neutral
                     // disasters are not counted

  if (attackerNationRecno) //	the attacker's people's loyalty decreases
                           //because of the killing actions.
    nation_array[attackerNationRecno]->civilian_killed(
        race_id, 1); // the nation is the attacking one
  ...
  if (misc.random(population) < spy_count) {
    int spyRecno =
        spy_array.find_town_spy(town_recno, misc.random(spy_count) + 1);
  ...
  if (attackerNationRecno) {
    ...
    if (nationPtr->is_monster())
      nationPtr->change_live_points(
          CITIZEN_COMBAT_LEVEL * 2 *
          2); // 1st * 2 - combat level to hit points, 2nd * 2 - killing humans
              // gives twice as much live points
  }
}
...
void Town::update_quality_of_life() {
  ...
  quality_of_life =
      int((float)100 * totalPurchase / (townDemand * MAX_PRODUCT));
}
...
int Town::recruit(bool recruitWagon, char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    ...
    // ##### begin Gilbert 7/10 ######//
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_TOWN_RECRUIT, 2 * sizeof(short));
    shortPtr[0] = town_recno;
  ...
  int recruitCount = recruitable_pop(
      1); // 1-also recruit spy. Don't exclude them from recruitment
  ...
  if (spy_count > 0) {
    ...
    err_when(recruitCount >
             sizeof(counterSpyArray) / sizeof(counterSpyArray[0]));
  ...
  if (recruitCount == 1) // recruit a single unit
  {
    ...
    if (spyArrayCount)
      unitRecno = spy_array[spyArray[0]]->mobilize_town_spy(
          1); // 1-decrease population immediately
    else
      unitRecno = mobilize_town_people(
          1, 0); // the 1st parameter is whether decreasing the population
                 // immediately, if decrease immediately in recruit mode, not in
                 // training mode, 2nd para 0-don't mobilize spies
  } else {
    // mobilize wagon
    unitRecno = mobilize_wagon(1, recruitCount, spyArrayCount,
                               spyArray); // 1-decrease population immediately
  }
...
//								if it is > 1, they are
//recruited to form a wagon.
//
// <int> decNow - decrease now, if it is 0, just return the
//						loyalty to be decreased without actual
//decreasing. 						(default: 1)
//
...
//							 if no, spies are not counted in
//the 							 total no. of units that can be recruited.
//
int Town::recruitable_pop(int recruitSpy) {
  ...
  if (!recruitSpy) {
    ...
    if (recruitableCount ==
        -1) // it may have been reduced twice if the unit being trained is a spy
      recruitableCount = 0;
  }
...
void Town::evenly_distribute_worker() {
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    totalFirmWork++;
    totalEmploymentRate +=
        100 * firmWork->worker_count / firmWork->needed_worker_count;
  }
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    //--- if the employment rate in this firm is below average, try to increase
    //it by transfering workers from other firms to it ----//

    for (int j = 0; j < linked_firm_count; j++) {
      if (i == j)
        continue;

      FirmWork *otherFirmWork =
          firm_array[linked_firm_array[j]]->cast_to_FirmWork();

      if (!otherFirmWork || otherFirmWork->needed_worker_count == 0)
        continue;

      employmentRate = 100 * otherFirmWork->worker_count /
                       otherFirmWork->needed_worker_count;
      ...
      if (100 * (otherFirmWork->worker_count - 1) /
                  otherFirmWork->needed_worker_count >=
              avgEmploymentRate ||
...
bool Town::can_accept_assign(int unitRecno, int actionNationRecno) {
  ...
  return unitPtr->race_id == race_id && unitPtr->nation_recno == nation_recno;
}
