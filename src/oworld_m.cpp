...
MapMatrix::MapMatrix()
{
    init(MAP_X1, MAP_Y1, MAP_X2, MAP_Y2, MAP_WIDTH, MAP_HEIGHT, 0.5, 0.5, -0.5, 0.5,
         1,  // isometric view
...
void MapMatrix::init_para()
{
    ...
    button_filter_object.create(MAP_MODE_BUTTON_X1 + 56 + 6, MAP_MODE_BUTTON_Y1 + 8 + 19, "O-UP", "O-DN", 0);
    button_filter_object.set_help_code("OBJECT");
    button_filter_nation.create(MAP_MODE_BUTTON_X1 + 30 + 17, MAP_MODE_BUTTON_Y1 + 9, "N-UP", "N-DN", 0);
...
void MapMatrix::disp_mode_button(int putFront)
{
    ...
    if (putFront)
    {
        mouse.hide_area(MAP_MODE_BUTTON_X1, MAP_MODE_BUTTON_Y1, MAP_MODE_BUTTON_X1 + 84 - 1,
                        MAP_MODE_BUTTON_Y1 + 86 - 1);
        help.short_front_buf.hide_area(MAP_MODE_BUTTON_X1, MAP_MODE_BUTTON_Y1, MAP_MODE_BUTTON_X1 + 84 - 1,
                                       MAP_MODE_BUTTON_Y1 + 86 - 1);
        ...
        mouse.hide_area(MAP_MODE_BUTTON_X1 - 97, MAP_MODE_BUTTON_Y1 + 10, MAP_MODE_BUTTON_X1 - 97 + 34 - 1,
    ...
    {
        help.short_back_buf.hide_area(MAP_MODE_BUTTON_X1, MAP_MODE_BUTTON_Y1, MAP_MODE_BUTTON_X1 + 84 - 1,
                                      MAP_MODE_BUTTON_Y1 + 86 - 1);
...
int MapMatrix::detect()
{
    ...
    if (mouse.single_click(MAP_MODE_BUTTON_X1 - 97, MAP_MODE_BUTTON_Y1 + 10, MAP_MODE_BUTTON_X1 - 97 + 34 - 1,
                           MAP_MODE_BUTTON_Y1 + 10 + 34 - 1))
    {
        if (zoom_map_building_disappear)
            zoom_map_building_disappear = 0;
...
PutLineLower: // draw lower triangle
    ...
    //		for( yLoc = 0; yLoc < max_y_loc; (yLoc&1?++pixelY:--pixelX) ,
    //++yLoc
    //)
    //		{
    ...
    case MAP_MODE_TERRAIN: {
        ...
        for (yLoc = 0; yLoc < max_y_loc; (yLoc & 1 ? ++pixelY : --pixelX), ++yLoc)
        {
            ...
            for (; xLoc < max_x_loc; (xLoc += 2), (locPtr += 2), (writePtr += writePtrInc))
            {
                ...
                {
                    // ##### begin Gilbert 10/2 #######//
                    if (filter_object_flag || filter_nation_flag) // do not anything except filtered objects
                                                                  // when filters are on
                    ...
                    else if (locPtr->has_dirt())
                    {
                        ...
                        *writePtr = vga_back.translate_color(terrain_res.get_map_tile(
                            locPtr->road_terrain_id)[tileYOffset + (xLoc & TERRAIN_TILE_X_MASK)]);
                    else
                    {
                                            ...
                                            terrain_res[(locPtr - 2)->terrain_id]->average_type)
                        {
                            ...
                            *writePtr = vga_back.translate_color(terrain_res.get_map_tile(
                                locPtr->terrain_id)[tileYOffset + (xLoc & TERRAIN_TILE_X_MASK)]);
                        }
                        else
                        {
...
void MapMatrix::set_filter_para()
{
    ...
    if (filter_object_flag)
    {
        ...
        else if (town_array.selected_recno)
        {
            ...
            filter_object_para =
                town_array[town_array.selected_recno]->is_pay_tribute_to_monster(nation_array.player_recno);
        }
        else
...
void MapMatrix::disp()
{
    ...
    if ((filter_object_flag && !filter_object_locked) || (filter_nation_flag && !filter_nation_locked))
    {
        set_filter_para();
    }
...
int MapMatrix::detect_area()
{
    ...
    if (mouse.release_click() && mouse.click_x() >= image_x1 && mouse.click_x() <= image_x2 &&
