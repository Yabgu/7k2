...
MapMatrix::MapMatrix() {
  init(MAP_X1, MAP_Y1, MAP_X2, MAP_Y2, MAP_WIDTH, MAP_HEIGHT, 0.5, 0.5, -0.5,
       0.5,
       1,  // isometric view
...
void MapMatrix::init_para() {
  ...
  button_filter_object.create(MAP_MODE_BUTTON_X1 + 56 + 6,
                              MAP_MODE_BUTTON_Y1 + 8 + 19, "O-UP", "O-DN", 0);
  button_filter_object.set_help_code("OBJECT");
  button_filter_nation.create(MAP_MODE_BUTTON_X1 + 30 + 17,
                              MAP_MODE_BUTTON_Y1 + 9, "N-UP", "N-DN", 0);
...
void MapMatrix::paint() { disp_mode_button(); }
...
//						  0-display the buttons on the back
//buffer 						  (default: 0)
//
void MapMatrix::disp_mode_button(int putFront) {
  ...
  if (putFront) {
    ...
    help.short_front_buf.hide_area(MAP_MODE_BUTTON_X1, MAP_MODE_BUTTON_Y1,
                                   MAP_MODE_BUTTON_X1 + 84 - 1,
                                   MAP_MODE_BUTTON_Y1 + 86 - 1);
    image_button.put_front(MAP_MODE_BUTTON_X1, MAP_MODE_BUTTON_Y1, iconName, 1);

    if (button_filter_object.pushed_flag)
      vga_front.put_bitmap_trans_decompress(MAP_MODE_BUTTON_X1 + 55,
                                            MAP_MODE_BUTTON_Y1 + 7,
                                            image_button.read("O-LOCK"));

    if (button_filter_nation.pushed_flag)
      vga_front.put_bitmap_trans_decompress(MAP_MODE_BUTTON_X1 + 30,
                                            MAP_MODE_BUTTON_Y1 + 7,
    ...
    mouse.hide_area(MAP_MODE_BUTTON_X1 - 97, MAP_MODE_BUTTON_Y1 + 10,
                    MAP_MODE_BUTTON_X1 - 97 + 34 - 1,
                    MAP_MODE_BUTTON_Y1 + 10 + 34 - 1);
    help.short_front_buf.hide_area(
        MAP_MODE_BUTTON_X1 - 97, MAP_MODE_BUTTON_Y1 + 10,
        MAP_MODE_BUTTON_X1 - 97 + 34 - 1, MAP_MODE_BUTTON_Y1 + 10 + 34 - 1);
    image_button.put_front(MAP_MODE_BUTTON_X1 - 94, MAP_MODE_BUTTON_Y1 + 13,
                           iconName2, 1);
    help.short_front_buf.show_area();
    mouse.show_area();
  } else {
    ...
    if (button_filter_object.pushed_flag)
      vga_back.put_bitmap_trans_decompress(MAP_MODE_BUTTON_X1 + 55,
                                           MAP_MODE_BUTTON_Y1 + 7,
                                           image_button.read("O-LOCK"));

    if (button_filter_nation.pushed_flag)
      vga_back.put_bitmap_trans_decompress(MAP_MODE_BUTTON_X1 + 30,
                                           MAP_MODE_BUTTON_Y1 + 7,
                                           image_button.read("N-LOCK"));
    ...
    help.short_front_buf.hide_area(
        MAP_MODE_BUTTON_X1 - 97, MAP_MODE_BUTTON_Y1 + 10,
        MAP_MODE_BUTTON_X1 - 97 + 34 - 1, MAP_MODE_BUTTON_Y1 + 10 + 34 - 1);
    image_button.put_back(MAP_MODE_BUTTON_X1 - 94, MAP_MODE_BUTTON_Y1 + 13,
                          iconName2, 1);
    help.short_front_buf.show_area();
  }
}
...
int MapMatrix::detect() {
  ...
  for (int i = 0; i < MAP_MODE_COUNT;
       i++, x += MAP_MODE_BUTTON_WIDTH, y += MAP_MODE_BUTTON_HEIGHT) {
    if (mouse.single_click(x, y, x + MAP_MODE_BUTTON_WIDTH - 1,
                           y + MAP_MODE_BUTTON_HEIGHT - 1)) {
      toggle_map_mode(i);
      return 1;
    }
  }
...
PutLineLower: // draw lower triangle
  ...
  //			for( xLoc = 0; xLoc < max_x_loc; (xLoc += 2), (writePtr +=
  //writePtrInc), (locPtr+=2) )
  //			{
  //			}
  //		}
  //	}
  ...
  case MAP_MODE_TRADE:
  // ##### end Gilbert 2/11 #####//
  case MAP_MODE_SPOT: { // traversal of location in MapMatrix
    int pixelX = (image_x1 + image_x2 + 1) / 2;
    int pixelY = image_y1;
    int xLoc, yLoc;
    int writePtrInc = vga_back.buf_pitch() + 1;
      ...
      for (xLoc = 0; xLoc < max_x_loc;
        ...
        if (locPtr->explored()) {
          ...
          /*						else if( locPtr->has_dirt()
             ...
             rock_res.get_rock_info(dirtPtr->rock_recno); if
             (dirtInfo->rock_type == 'P') *writePtr = seaColor; else *writePtr =
             rockColor;
                                                          }

                                                          else if(
             locPtr->is_plant() || xLoc+1<max_x_loc && locPtr[1].is_plant() )
                                                                  *writePtr =
             plantColor;
          */
          else
...
void MapMatrix::set_filter_para() {
  ...
  if (filter_object_flag) {
    if (unit_array.selected_recno) {
      ...
      filter_object_para =
          unit_res[unit_array[unit_array.selected_recno]->unit_id]->unit_class;
      ...
      filter_object_para =
          town_array[town_array.selected_recno]->is_pay_tribute_to_monster(
...
void MapMatrix::draw_square() {
  ...
  /*
     ...
     world.zoom_matrix->get_base_y()) / LOCATE_WIDTH * loc_width; int y0 =
     image_y1+world.zoom_matrix->calc_cur_y( world.zoom_matrix->get_base_x(),
     world.zoom_matrix->get_base_y()) / LOCATE_HEIGHT * loc_height; int x1 =
     image_x1+world.zoom_matrix->calc_cur_x( world.zoom_matrix->get_base_x() +
     world.zoom_matrix->image_width, world.zoom_matrix->get_base_y()) /
     LOCATE_WIDTH * loc_width; int y1 = image_y1+world.zoom_matrix->calc_cur_y(
                  world.zoom_matrix->get_base_x() +
     world.zoom_matrix->image_height, world.zoom_matrix->get_base_y()) /
     LOCATE_HEIGHT * loc_height; int x2 =
     image_x1+world.zoom_matrix->calc_cur_x( world.zoom_matrix->get_base_x() +
     world.zoom_matrix->image_width, world.zoom_matrix->get_base_y() +
     world.zoom_matrix->image_height) / LOCATE_WIDTH * loc_width; int y2 =
     image_y1+world.zoom_matrix->calc_cur_y( world.zoom_matrix->get_base_x() +
     world.zoom_matrix->image_height, world.zoom_matrix->get_base_y() +
     world.zoom_matrix->image_height) / LOCATE_HEIGHT * loc_height; int x3 =
     image_x1+world.zoom_matrix->calc_cur_x( world.zoom_matrix->get_base_x(),
                  world.zoom_matrix->get_base_y() +
     world.zoom_matrix->image_height) / LOCATE_WIDTH * loc_width; int y3 =
     image_y1+world.zoom_matrix->calc_cur_y( world.zoom_matrix->get_base_x(),
                  world.zoom_matrix->get_base_y() +
     world.zoom_matrix->image_height) / LOCATE_HEIGHT * loc_height;
  */
  ...
  if (!cur_cargo_rect_type) {
    ...
    x2 = image_x1 + calc_loc_pos_x(cur_x_loc + cur_cargo_width,
                                   cur_y_loc + cur_cargo_height);
    y2 = image_x1 + calc_loc_pos_y(cur_x_loc + cur_cargo_width,
                                   cur_y_loc + cur_cargo_height);
  ...
  // however, in order to simplify the code, the following code is case
  // dependence in this case the matrix map must be 200 X 200 and the upper
  // triangle must with height = 100, width = 200 and the lower triangle must
  // with height = 99 and width = 198
  //
  // vga_back.rect( x1, y1, x2, y2, 1, VGA_YELLOW + squareFrameCount );
  if ((y1 < image_y1) || (y1 > (image_y2 - 1)))
    limitx = -1;
  else if ((y1 >= image_y1) && (y1 < (image_y1 + 100)))
    limitx = y1 - image_y1;
  else if ((y1 <= (image_y2 - 1)) && (y1 > (image_y2 - 100)))
  ...
  if ((x1 < image_x1) || (x1 > (image_x2 - 1)))
    limity = -1;
  else if ((x1 >= image_x1) && (x1 < (image_x1 + 100)))
    limity = x1 - image_x1;
  else if ((x1 <= (image_x2 - 1)) && (x1 > (image_x2 - 101)))
  ...
  if ((y2 < image_y1) || (y2 > (image_y2 - 1)))
    limitx = -1;
  else if ((y2 >= image_y1) && (y2 < (image_y1 + 100)))
    limitx = y2 - image_y1;
  else if ((y2 <= (image_y2 - 1)) && (y2 > (image_y2 - 100)))
  ...
  if ((x3 < image_x1) || (x3 > (image_x2 - 1)))
    limity = -1;
  else if ((x3 >= image_x1) && (x3 < (image_x1 + 100)))
    limity = x3 - image_x1;
  else if ((x3 <= (image_x2 - 1)) && (x3 > (image_x2 - 101)))
...
void MapMatrix::toggle_map_mode(int modeId) {
  /*
          if( map_mode == modeId )
          {
                  if( map_mode == MAP_MODE_POWER )		// clicking on a
     pressed button unclick the button power_mode = !power_mode;
          }
          else
          {
                  map_mode = modeId;
...
int MapMatrix::detect_area() {
  ...
  // ##### end Gilbert 30/3 #######//
  {
    ...
    if (get_detect_location(mouse.cur_x, mouse.cur_y, &xLoc, &yLoc, NULL)) {
      ...
      if (!valid_cur_box()) // valid_cur_box() return 1 if it has refreshed, 0
                            // if didn't
        disp();
  ...
  if (mouse.release_click() && mouse.click_x() >= image_x1 &&
      mouse.click_x() <= image_x2 && mouse.click_y() >= image_y1 &&
      mouse.click_y() <= image_y2) {
...
// return 1 if success
//
int MapMatrix::get_detect_location(int scrnX, int scrnY, int *locX, int *locY,
                                   char *mobileType) {
...
// return 1 if success
//
int MapMatrix::get_close_location(int scrnX, int scrnY, int *locX, int *locY,
                                  char *mobileType) {
  return 0;
}
//----------- End of function MapMatrix::get_detect_location ----------//
