...
int CampaignEastWest::stage_4_create_game() {
  ...
  create_troop(PLAYER_NATION_RECNO, 65 - campaign_difficulty * 10,
               65 - campaign_difficulty * 5);
  ...
  for (int i = town_array.size(); i > 0; i--) {
    ...
    if (townPtr->nation_recno == STAGE_4_EASTERN_NATION_RECNO &&
        misc.random(2) == 0) {
      world.unveil(townPtr->loc_x1, townPtr->loc_y1, townPtr->loc_x2,
                   townPtr->loc_y2);
...
void CampaignEastWest::stage_4_init_vars() {
  ...
  next_rebel_date =
      info.game_date + 90 +
      misc.random(60); // don't rebel until at least 3 months into the game
...
void CampaignEastWest::stage_4_prelude() {
  ...
  disp_narrative(
      res_stage.read("4PRELUDE")); // 0-the letter is from a human kingdom
  stop_speech();

  attacker_state_recno = random_pick_state(
      western_nation_recno,
      target_state_recno); // pick a state of the Western empire that is next to
                           // defected state.
...
int CampaignEastWest::stage_4_create_town() {
  ...
  return create_town(STAGE_4_EASTERN_NATION_RECNO, townAddCount,
                     hasFortPercent);
}
...
int CampaignEastWest::stage_4_create_firm() {
  ...
  int townRecno2 = random_pick_town_with_camp(
      STAGE_4_EASTERN_NATION_RECNO, 2); // only pick towns with <= 2 links
...
void CampaignEastWest::stage_4_next_day() {
  ...
  //--- towns of the eastern empire will defect to the player's nation as
  //player's nation grows stronger ---//
  ...
  if (info.game_date >= next_rebel_date &&
    ...
    int easternRating =
        nation_array[STAGE_4_EASTERN_NATION_RECNO]->overall_rating;

    if (maxRebelCount * playerRating / easternRating > rebel_count ||
        campaign_difficulty ==
            1) // If difficulty is 1, then don't check the rating
    {
      int townRecno =
          town_defect(STAGE_4_EASTERN_NATION_RECNO, PLAYER_NATION_RECNO);
...
void CampaignEastWest::stage_4_process_game_result() {
  ...
  //---- if the player lost the game ----//
    ...
    set_event(EVENT_RANDOM_POINT_1); // still go to random point 1 even the
