...
int CampaignEastWest::stage_4_create_game()
{
    ...
    for (int i = town_array.size(); i > 0; i--)
    {
        ...
        if (townPtr->nation_recno == STAGE_4_EASTERN_NATION_RECNO && misc.random(2) == 0)
        {
...
void CampaignEastWest::stage_4_prelude()
{
    ...
    attacker_state_recno = random_pick_state(western_nation_recno,
...
void CampaignEastWest::stage_4_next_day()
{
    ...
    if (info.game_date >= next_rebel_date && !nation_array.is_deleted(STAGE_4_EASTERN_NATION_RECNO))
    {
