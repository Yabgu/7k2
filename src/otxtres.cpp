...
void TextResource::load_res(const char *fileName)
{
    ...
    while (!f.is_eof())
    {
        ...
        if (main_text[offset] == CHECK_MARK_BEGIN && strchr(main_text + offset, CHECK_MARK_END))
        {
    ...
    if (textId > 0 && textId <= text_count)
    {
        ...
        if (!checkMark // skip mark checking
            || (formatText > main_text &&
                ...
                //			&& strncmp(formatText-1-CHECK_MARK_STRLEN,
                // checkMark,
                // CHECK_MARK_STRLEN) == 0) )
            ...
            else if (*readPtr >= '0' && *readPtr <= '9')
            {
                ...
                case 'f': // float %<n>[,f]f
                    if (argId > 0 && argId <= MAX_ARG)
                    {
                        ...
                        char *strPtr = format(floatArgs[argId - 1],
                ...
                case 'd': // integer %<n>[,f]d
                    if (argId > 0 && argId <= MAX_ARG)
                    {
                        ...
                        char *strPtr = format(intArgs[argId - 1],
                                              formatId ? formatId : 1); // default format is 1
                        // strcpy, but we need end of string
                        for (; *strPtr != '\0'; ++strPtr)
                        {
