...
// %%              a single percent
// %<n>s           string, <n> is from 1 to 9, the number in the argument list
// (eg. %1s)
...
// %<n>p{<s>|<p>}  use <s> singular form or <p> plural form, determined by
// integer argment <n>
//                 (e.g. %1p{box|boxes})
// %<n>q{<z>|<s>|<p>}  use <z> zero form, <s> singular form or <p> plural form,
// determined by integer argment <n>
...
// useage
// 1. font_news.put( ZOOM_X1+10, dy, test_text_res.process(
// test_text_res.get_format_str(1, "CHM1"), "a", "bb", "ccc") ); find format str
// in line 1, and check [CHM1] mark at the beginning of the that line the line
// appear in resource file may look like:
//
...
//	char *kingdom_name(char *playerName) { return process( get_format_str(
//3, "KNAME"), playerName); }
//};
...
char TextResource::short_str_buf[FREE_SHORT_STR_COUNT]
                                [TEXT_RES_SHORT_STRLEN + 1];
...
// ------- begin of function TextResource::~TextResource -------//
//
TextResource::~TextResource() { deinit(); }
// ------- end of function TextResource::~TextResource -------//
...
void TextResource::load_res(const char *fileName) {
  ...
  int guessTextCount =
      fileSize / 64 + 1; // initial guess, 64 is assumed char per line
  ...
  while (!f.is_eof()) {
    ...
    if (text_count >= guessTextCount) {
      ...
      text_offset_array =
          (int *)mem_resize(text_offset_array, sizeof(int) * guessTextCount);
    }
  ...
  if (textId > 0 && textId <= text_count) {
    ...
    if (!checkMark // skip mark checking
        || (formatText > main_text &&
            ...
            //			&& strncmp(formatText-1-CHECK_MARK_STRLEN, checkMark,
            //CHECK_MARK_STRLEN) == 0) )
            && *(uint32_t *)(formatText - 1 - CHECK_MARK_STRLEN) ==
                   *(uint32_t *)checkMark)) {
      return formatText;
    }
  }
...
char *TextResource::format(double inNum, int formatType) {
  ...
  if (intDigit > 29) // integer digits can't exceed 29
    err.run("TextResource::format(), inNum : %e, formatType : %d", inNum,
            formatType);
#endif
...
char *TextResource::roman_number(int inNum) {
  ...
  for (int i = 0; i < deciLen; ++i) {
    static const char *unitDigitArray[10] = {"",  "I",  "II",  "III",  "IV",
                                             "V", "VI", "VII", "VIII", "IX"};
    static const char *tenDigitArray[10] = {"",  "X",  "XX",  "XXX",  "XL",
                                            "L", "LX", "LXX", "LXXX", "XC"};
    static const char *hundredDigitArray[10] = {"",  "C",  "CC",  "CCC",  "CD",
                                                "D", "DC", "DCC", "DCCC", "CM"};
...
// cast to (char *) explicitly, variable argument cannot handle class object
// well.
        ...
        *writePtr = '%';
        ...
        case 'f': // float %<n>[,f]f
          if (argId > 0 && argId <= MAX_ARG) {
            ...
            char *strPtr =
                format(floatArgs[argId - 1],
                       formatId ? formatId : 1); // default format is 1
            // strcpy, but we need end of string
            for (; *strPtr != '\0'; ++strPtr) {
              *writePtr = *strPtr;
              ++writePtr;
        ...
        case 'p': // plural form %<n>p{<s>|<p>}
          if (argId > 0 && argId <= MAX_ARG) {
            ...
            if (intArgs[argId - 1] == 0 || intArgs[argId - 1] == 1) {
              ...
              // plural form
              if (readPtr[1] == '{' && (strPtr = strchr(readPtr + 2, '|'))) {
