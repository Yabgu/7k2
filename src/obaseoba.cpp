...
//
BaseObjArray::BaseObjArray()
    : DynArrayB(sizeof(BaseObj *), 500, DEFAULT_REUSE_INTERVAL_DAYS) {}
...
void BaseObjArray::del(int recNo) {
  ...
  err_when(!baseObjPtr); // don't delete it as it will be deleted at its derived
                         // class level
