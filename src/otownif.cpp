...
static Button3D button_spy, button_cancel, button_spy_mobilize, button_buildwall, button_spy_reward, button_spy_action,
...
static short browse_race_recno = 1,
             browse_race_town_recno = 0; // the town which the browse_race displays its info
...
static short edit_loyalty_x1[MAX_NATION], edit_loyalty_y1[MAX_NATION], edit_loyalty_x2[MAX_NATION],
    edit_loyalty_y2[MAX_NATION], edit_loyalty_enable[MAX_NATION];
...
void Town::disp_info(int refreshFlag)
{
    ...
    {
        if (last_menu_mode != town_menu_mode) //|| spy_menu_repaint)		// if changing menu mode
...
void Town::disp_main_menu(int refreshFlag)
{
    if (under_construction)
    {
        ...
        // INFO_Y1+165, "Under" ); 		font_vill.center_put( INFO_X1+99,
        // INFO_Y1+165, INFO_X1+217, INFO_Y1+185, "Construction" );
        // font_vill.space_width += 4;
        font_vill.center_put_paragraph(INFO_X1 + 99, INFO_Y1 + 152, INFO_X1 + 217, INFO_Y1 + 185,
                                       text_firm.str_under_construction(), -5, 0, 0);

        font_snds.center_put(INFO_X1 + 20, INFO_Y1 + 125, INFO_X1 + 125, INFO_Y1 + 145, town_name(), 0, 1);
        font_vill.center_put(INFO_X1 + 20, INFO_Y1 + 105, INFO_X1 + 125, INFO_Y1 + 125, text_firm.str_town_of());
    ...
    font_town.center_put(INFO_X1 + 11, INFO_Y1 + 11, INFO_X2, INFO_Y1 + 29, town_name(), 0, 1);
    ...
    font_snds.center_put(INFO_X1 + 11, INFO_Y1 + 36, INFO_X2, INFO_Y1 + 48,
                         (is_pay_tribute_to_monster() ? text_firm.str_race_slave_town(race_res[race_id]->adjective)
    ...
    // = sprite_info->res_bitmap.read_imported(spriteFrame->bitmap_offset);
    // short* colorRemapTable = sprite_info->get_color_remap_table(nation_recno,
    // 0); 		if
    //(!((stopArrayOffset[5-i] < 8 || sprite_info->turn_resolution <= 8) ?
    //(stopArrayOffset[5-i] & 7) >= 5 : (stopArrayOffset[5-i] & 7) >= 4))
    //		{
    //			if( !colorRemapTable )
    //				vga_back.put_bitmap_trans_decompress(INFO_X1
    //+offsetX[5-i] +spriteFrame->offset_x, INFO_Y1 +offsetY[5-i]
    //+spriteFrame->offset_y,
    // bitmapPtr); 			else
    // vga_back.put_bitmap_trans_remap_decompress(INFO_X1 +offsetX[5-i]
    ...
    font_vill.center_put(INFO_X1 + 155, INFO_Y1 + 145, INFO_X1 + 215, INFO_Y1 + 157, misc.format(jobless_population));
    ...
    if (population > 0)
    {
        ...
        int dispArrow = 0;
        if (curLoyalty == targetLoyalty || targetLoyalty == -1) // only display up and down arrow for independent
                                                                // town's resistance
            str = curLoyalty;
        else
        {
        ...
        tempx =
            font_vill.center_put(INFO_X1 + 111, INFO_Y1 + 187, INFO_X1 + 182, INFO_Y1 + 187 + font_vill.height(), str);
        if (dispArrow)
        {
    ...
    if (nation_array.player_recno) // player not die yet
    {
        ...
        if (is_own())
        {
            ...
            if ((button_builder.visible_flag = current_wall_level > 0 && hit_points < max_hit_points()))
            {
                // #### end Gilbert 24/2 ########//
            ...
            {
                vga.active_buf->bar_alpha(button_buildwall.x1, button_buildwall.y1,
                ...
                font_whbl.center_put(button_buildwall.x1, button_buildwall.y1,
                                     button_buildwall.x1 + BUTTON_ACTION_WIDTH - 1,
                                     button_buildwall.y1 + BUTTON_ACTION_HEIGHT - 21, text_firm.str_wall_repair());
                // str = (int)townPtr->hit_points *100 / (int)townPtr->max_hit_points();
                // str += "%";
                font_whbl.center_put(button_buildwall.x1, button_buildwall.y1 + 21,
                                     button_buildwall.x1 + BUTTON_ACTION_WIDTH - 1,
            ...
            else if (target_wall_level > current_wall_level)
            {
                ...
                font_whbl.center_put(button_buildwall.x1, button_buildwall.y1,
                                     button_buildwall.x1 + BUTTON_ACTION_WIDTH - 1,
                                     button_buildwall.y1 + BUTTON_ACTION_HEIGHT - 21, text_firm.str_wall_building());
...
void Town::disp_auto_loyalty()
{
    ...
    {
        vga.active_buf->bar_alpha(button_tax.x1, button_tax.y1 + 15, button_tax.x1 + BUTTON_ACTION_WIDTH - 1,
                                  button_tax.y1 + BUTTON_ACTION_HEIGHT - 16, 1, 0);
        font_whbl.center_put(button_tax.x1, button_tax.y1, button_tax.x1 + BUTTON_ACTION_WIDTH - 1,
                             button_tax.y1 + BUTTON_ACTION_HEIGHT - 1, misc.format(auto_collect_tax_loyalty));
    }
    if (auto_grant_loyalty)
    {
        vga.active_buf->bar_alpha(button_grant.x1, button_grant.y1 + 15, button_grant.x1 + BUTTON_ACTION_WIDTH - 1,
                                  button_grant.y1 + BUTTON_ACTION_HEIGHT - 16, 1, 0);
        font_whbl.center_put(button_grant.x1, button_grant.y1, button_grant.x1 + BUTTON_ACTION_WIDTH - 1,
                             button_grant.y1 + BUTTON_ACTION_HEIGHT - 1, misc.format(auto_grant_loyalty));
    }
}
...
void Town::detect_main_menu()
{
    ...
    if (mouse.single_click(INFO_X1 + 11, INFO_Y1 + 5, INFO_X1 + 217, INFO_Y1 + 83) &&
        (!is_own() || remote.is_enable() ||
         !mouse.single_click(INFO_X1 + 23, INFO_Y1 + 11, INFO_X1 + 205, INFO_Y1 + 28)))
    {
        world.go_loc(center_x, center_y);
        return;
    }

    if (mouse.single_click(INFO_X1 + 23, INFO_Y1 + 11, INFO_X1 + 205, INFO_Y1 + 28) && is_own() && !remote.is_enable())
    {
        input_town_name();
        return;
    }
...
void Town::disp_spy_menu(int refreshFlag)
{
    ...
    if (refreshFlag == INFO_REPAINT)
    {
        browse_spy.init(INFO_X1 + 13, INFO_Y1 + 40, INFO_X1 + 233, INFO_Y1 + 219, 0, 45, playerSpyCount, put_spy_rec);
...
void Town::disp_auto_menu(int modeCollectTax, int refreshFlag)
{
    ...
    // click) or all your Towns (Right - click) when the Loyalty reaches:";
    // else
    //		headingStr = "Automatically Grant Money to this Town (Left -
    // click)
    ...
    font_bld.center_put_paragraph(INFO_X1 + 17, INFO_Y1 + 20, INFO_X2 - 7, INFO_Y1 + 70,
                                  modeCollectTax ? text_firm.str_auto_tax_inst() : text_firm.str_auto_grant_inst(), 0,
                                  0, 0);

    for (i = 0; i < BUTTON_LOYALTY_COUNT; i++)
    {
        if (refreshFlag == INFO_REPAINT)
            button_loyalty_array[i].create(x + (i >> 2) * 94, y + (i % 4) * 23, x + (i >> 2) * 94 + 91,
    ...
    if (refreshFlag == INFO_REPAINT)
        button_loyalty_disabled.create(x + (i >> 2) * 94, y + (i % 4) * 23, x + (i >> 2) * 94 + 91,
                                       y + (i % 4) * 23 + 20, disp_auto_menu_button,
                                       ButtonCustomPara(game.get_color_remap_table(nation_recno, 0), i));
...
void Town::disp_man_power(int refreshFlag)
{
    ...
    font_snds.put_paragraph(INFO_X1 + 20, INFO_Y1 + 8, INFO_X2 - 5, INFO_Y2 - 5, text_firm.str_set_wall_crew());

    for (i = 0; i < 9; i++)
    {
        if (refreshFlag == INFO_REPAINT)
            button_loyalty_array[i].create(x, y + i * 19, x + 187, y + i * 19 + 17, disp_man_power_button,
                                           ButtonCustomPara(game.get_color_remap_table(nation_recno, 0), i));
        button_loyalty_array[i].paint();
    }
...
static int spy_filter(int recNo)
{
    ...
    for (int i = spy_array.size(); i >= 1; i--)
    {
        ...
        if (spyPtr->spy_place == SPY_TOWN && spyPtr->spy_place_para == Town::if_town_recno &&
            spyPtr->true_nation_recno == nation_array.player_recno)
        {
            recCount++;
        }
...
static void put_spy_rec(int recNo, int x, int y, int refreshFlag)
{
    ...
    info.draw_unit_icon(x + 10, y + 15, race_res[spyPtr->race_id]->civilian_unit_id, nationRecno, INFO_X1, INFO_Y1,
                        INFO_X2, INFO_Y2, 1);
...
int Town::disp_edit_mode(int refreshFlag)
{
    ...
    for (i = 1; i <= nation_array.size(); i++)
    {
        ...
        x = edit_loyalty_x2[i - 1] =
            font_blue.put((edit_loyalty_x1[i - 1] = x1 + 50), (edit_loyalty_y1[i - 1] = y), curLoyalty, 0, x2);
...
void Town::detect_edit_mode()
{
    ...
    {
        if (mouse.any_click(edit_population_x1, edit_population_y1, edit_population_x2, edit_population_y2,
                            LEFT_BUTTON))
        {
            if (population < MAX_TOWN_POPULATION)
            {
    ...
    for (i = 1; i <= nation_array.size(); i++)
    {
        ...
        {
            if (mouse.any_click(edit_loyalty_x1[i - 1], edit_loyalty_y1[i - 1], edit_loyalty_x2[i - 1],
                                edit_loyalty_y2[i - 1], LEFT_BUTTON))
            {
                if (nation_recno == i)
                {
                    loyalty += 10;
            ...
            if (mouse.any_click(edit_loyalty_x1[i - 1], edit_loyalty_y1[i - 1], edit_loyalty_x2[i - 1],
                                edit_loyalty_y2[i - 1], RIGHT_BUTTON))
            {
                if (nation_recno == i)
                {
                    loyalty -= 10;
