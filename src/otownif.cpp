...
static Button3D button_spy, button_cancel, button_spy_mobilize,
    button_buildwall, button_spy_reward, button_spy_action,
    button_spy_view_secret;
...
static short browse_race_recno = 1,
             browse_race_town_recno =
                 0; // the town which the browse_race displays its info
...
static short action_spy_recno; // recno of the spy that is doing the bribing or
                               // viewing secret reports of other nations
static short edit_population_x1, edit_population_y1, edit_population_x2,
    edit_population_y2, edit_population_enable;
static short edit_loyalty_x1[MAX_NATION], edit_loyalty_y1[MAX_NATION],
    edit_loyalty_x2[MAX_NATION], edit_loyalty_y2[MAX_NATION],
    edit_loyalty_enable[MAX_NATION];
...
void Town::disp_info(int refreshFlag) {
  ...
  //	(refreshFlag==INFO_REPAINT && !disable_refresh) )
  {
    ...
    if (last_menu_mode !=
        town_menu_mode) //|| spy_menu_repaint)		// if changing menu mode
                        //pass repaint to sub-menu
    {
...
void Town::disp_main_menu(int refreshFlag) {
  if (under_construction) {
    ...
    //		font_vill.center_put( INFO_X1+113, INFO_Y1+152, INFO_X1+205,
    //INFO_Y1+165, "Under" ); 		font_vill.center_put( INFO_X1+99, INFO_Y1+165,
    //INFO_X1+217, INFO_Y1+185, "Construction" ); 		font_vill.space_width += 4;
    font_vill.center_put_paragraph(
        INFO_X1 + 99, INFO_Y1 + 152, INFO_X1 + 217, INFO_Y1 + 185,
        text_firm.str_under_construction(), -5, 0, 0);

    font_snds.center_put(INFO_X1 + 20, INFO_Y1 + 125, INFO_X1 + 125,
                         INFO_Y1 + 145, town_name(), 0, 1);
    font_vill.center_put(INFO_X1 + 20, INFO_Y1 + 105, INFO_X1 + 125,
                         INFO_Y1 + 125, text_firm.str_town_of());

    // str = construction_completion_percent;
    // str += "%";
    font_vill.center_put(INFO_X1 + 20, INFO_Y1 + 139, INFO_X1 + 125,
                         INFO_Y1 + 171,
    ...
    if (nation_recno && nation_array.player_recno == nation_recno) {
      if (refreshFlag == INFO_REPAINT) {
        ...
        if ((current_wall_level > 0) && (hit_points < max_hit_points())) {
          button_builder.create(INFO_X1 + 13, INFO_Y1 - 96, "REPAIRU",
                                "REPAIRD", 0);
  ...
  //		str += " Slave";
  //	str += " Town";
  //	tempx = font_snds.center_put( INFO_X1 +11, INFO_Y1 +36 , INFO_X2,
  //INFO_Y1 +48, str );

  // ##### patch begin Gilbert 23/10 #######//
  // before use race name instead of adjective
  font_snds.center_put(
      INFO_X1 + 11, INFO_Y1 + 36, INFO_X2, INFO_Y1 + 48,
  ...
  if (current_wall_level > 0) {
    ...
    help.set_help(INFO_X1 + 173, INFO_Y1 - 62, INFO_X1 + 202, INFO_Y1 - 29,
                  "T_ENENGY");
  } else
  ...
  if (population == MAX_TOWN_POPULATION)
    vga.active_buf->put_bitmap(INFO_X1 + 35, INFO_Y1 + 117,
                               image_icon.read("FULLVILL"));
  else
  ...
  for (int i = ((dispPopulation + 9) / 10); i > 0; i--) {
    info.draw_unit_icon(INFO_X1 + offsetX[i - 1], INFO_Y1 + offsetY[i - 1],
                        race_res[race_id]->civilian_unit_id, nation_recno,
                        INFO_X1 + 12, INFO_Y1 + 163, INFO_X1 + 110,
                        INFO_Y1 + 227, 1);
  }

  help.set_help(INFO_X1 + 16, INFO_Y1 + 96, INFO_X1 + 96, INFO_Y1 + 138,
                "TPOP");
  ...
  //	{
  //		SpriteStop *stopAction = sprite_info->stop_array
  //+stopArrayOffset[5-i]; 		SpriteFrame *spriteFrame =
  //sprite_frame_res[stopAction->frame_recno]; 		char* bitmapPtr =
  //sprite_info->res_bitmap.read_imported(spriteFrame->bitmap_offset); 		short*
  //colorRemapTable = sprite_info->get_color_remap_table(nation_recno, 0); 		if
  //(!((stopArrayOffset[5-i] < 8 || sprite_info->turn_resolution <= 8) ?
  //(stopArrayOffset[5-i] & 7) >= 5 : (stopArrayOffset[5-i] & 7) >= 4))
  //		{
  //			if( !colorRemapTable )
  //				vga_back.put_bitmap_trans_decompress(INFO_X1 +offsetX[5-i]
  //+spriteFrame->offset_x, INFO_Y1 +offsetY[5-i] +spriteFrame->offset_y,
  //bitmapPtr); 			else 				vga_back.put_bitmap_trans_remap_decompress(INFO_X1
  //+offsetX[5-i] +spriteFrame->offset_x, INFO_Y1 +offsetY[5-i]
  //+spriteFrame->offset_y, bitmapPtr, colorRemapTable);
  //		}
  //		else
  //		{
  //			if( !colorRemapTable )
  //				vga_back.put_bitmap_trans_decompress_hmirror(INFO_X1
  //+offsetX[5-i] +spriteFrame->offset_x, INFO_Y1 +offsetY[5-i]
  //+spriteFrame->offset_y, bitmapPtr); 			else
  //				vga_back.put_bitmap_trans_remap_decompress_hmirror(INFO_X1
  //+offsetX[5-i] +spriteFrame->offset_x, INFO_Y1 +offsetY[5-i]
  //+spriteFrame->offset_y, bitmapPtr, colorRemapTable);
  //		}
  //	}
  ...
  // str = "Peasant";
  font_vilb.put(INFO_X1 + 160, INFO_Y1 + 127, text_firm.str_town_peasant(), 0,
                INFO_X2 - 6);

  // str = jobless_population;
  font_vill.center_put(INFO_X1 + 155, INFO_Y1 + 145, INFO_X1 + 215,
                       INFO_Y1 + 157, misc.format(jobless_population));
  help.set_help(INFO_X1 + 155, INFO_Y1 + 130, INFO_X1 + 213, INFO_Y1 + 163,
                "TPEASANT");
  ...
  font_vill.center_put(INFO_X1 + 70, INFO_Y1 + 145, INFO_X1 + 160,
                       INFO_Y1 + 157,
                       misc.format(population - jobless_population));
  help.set_help(INFO_X1 + 84, INFO_Y1 + 130, INFO_X1 + 144, INFO_Y1 + 164,
                "TWORKER");
  ...
  if (population > 0) {
    ...
    if (nation_recno) {
      ...
      help.set_help(INFO_X1 + 112, INFO_Y1 + 172, INFO_X1 + 181, INFO_Y1 + 206,
                    "TLOYALTY");
    } else {
      // str = "Resistance";
      ...
      //	vga.active_buf->put_bitmap( INFO_X1 +113, INFO_Y1 +172,
      //image_icon.read( "RESIST" ));

      if (config.show_ai_info) {
        if (info.default_viewing_nation_recno) {
          curLoyalty = (int)resistance(info.default_viewing_nation_recno);
          targetLoyalty =
              target_resistance_array[info.default_viewing_nation_recno - 1];
        } else {
          curLoyalty = 0;
          targetLoyalty = 0;
        }
        ...
        if (nation_array.player_recno) {
          ...
          targetLoyalty =
              target_resistance_array[nation_array.player_recno - 1];
        } else {
          curLoyalty = 0;
          targetLoyalty = 0;
        }
      ...
      if (targetLoyalty >
          curLoyalty)       // resistance only decrease, won't increase
        targetLoyalty = -1; // don't display the decrease target
    }
    int dispArrow = 0;
    if (curLoyalty == targetLoyalty ||
        targetLoyalty == -1) // only display up and down arrow for independent
                             // town's resistance
      str = curLoyalty;
    else {
      str = curLoyalty;
    ...
    tempx = font_vill.center_put(INFO_X1 + 111, INFO_Y1 + 187, INFO_X1 + 182,
                                 INFO_Y1 + 187 + font_vill.height(), str);
    if (dispArrow) {
      tempx = tempx - font_vill.text_width(misc.format(targetLoyalty)) - 12;
      if (targetLoyalty > curLoyalty)
        vga.active_buf->put_bitmap_trans(tempx, INFO_Y1 + 190,
                                         image_icon.read("ARROWVUP"));
      else
        //	if( targetLoyalty < curLoyalty )
        vga.active_buf->put_bitmap_trans(tempx, INFO_Y1 + 190,
                                         image_icon.read("ARROWVDN"));
    }
  }
  // ------- display button ------- //
  int x1 = INFO_X1 + 13;
  ...
  if (nation_array.player_recno) // player not die yet
  {
    if (refreshFlag == INFO_REPAINT) {
      ...
      button_builder.create(INFO_X1 + 13, INFO_Y1 - 96, "REPAIRU", "REPAIRD",
                            0);
      button_builder.set_help_code("REPAIR");
    ...
    // if( nation_recno && nation_array.player_recno == nation_recno )
    if (is_own()) {
      if ((button_destruct.visible_flag =
               population == 0)) // can destruct only empty town
      {
        button_destruct.enable_flag = 1;
        button_destruct.paint();
      }
      ...
      if ((button_builder.visible_flag =
        ...
        if (repair_flag && hit_points < max_hit_points()) {
          // set pushed_flag to 1 as although repair_flag will not reset after
          // selecting other things, pushed flag will do
          button_builder.pushed_flag = repair_flag;
          button_builder.update_bitmap(((sys.frame_count % 5) < 3)
                                           ? (char *)"REPAIRU"
                                           : (char *)"REPAIRD");
        } else {
          button_builder.pushed_flag = repair_flag;
          button_builder.update_bitmap(repair_flag ? (char *)"REPAIRU"
                                                   : (char *)"REPAIRD");
        }
      }
      ...
      button_buildwall.enable_flag =
          ((population > 0) && (current_wall_level < 2)) ? 1 : 0;
      if (target_wall_level >= 1)
      ...
      if (hit_points < max_hit_points() && repair_flag) {
        vga.active_buf->bar_alpha(
            button_buildwall.x1, button_buildwall.y1,
            button_buildwall.x1 + BUTTON_ACTION_WIDTH - 1,
            button_buildwall.y1 + BUTTON_ACTION_HEIGHT - 1, 1, 0);
        // str = "Repair";
        font_whbl.center_put(button_buildwall.x1, button_buildwall.y1,
                             button_buildwall.x1 + BUTTON_ACTION_WIDTH - 1,
                             button_buildwall.y1 + BUTTON_ACTION_HEIGHT - 21,
                             text_firm.str_wall_repair());
        // str = (int)townPtr->hit_points *100 / (int)townPtr->max_hit_points();
        // str += "%";
        font_whbl.center_put(
            button_buildwall.x1, button_buildwall.y1 + 21,
            button_buildwall.x1 + BUTTON_ACTION_WIDTH - 1,
            button_buildwall.y1 + BUTTON_ACTION_HEIGHT - 1,
  ...
  if (nation_array.player_recno) {
    ...
    if (totalTribute) {
      ...
      font_whbl.put_paragraph(BUTTON_X1 + 10, BUTTON_Y1 + 20, BUTTON_X2 - 10,
                              BUTTON_Y2 - 20,
...
void Town::disp_auto_loyalty() {
  if (auto_collect_tax_loyalty) {
    vga.active_buf->bar_alpha(button_tax.x1, button_tax.y1 + 15,
                              button_tax.x1 + BUTTON_ACTION_WIDTH - 1,
                              button_tax.y1 + BUTTON_ACTION_HEIGHT - 16, 1, 0);
    font_whbl.center_put(button_tax.x1, button_tax.y1,
                         button_tax.x1 + BUTTON_ACTION_WIDTH - 1,
                         button_tax.y1 + BUTTON_ACTION_HEIGHT - 1,
                         misc.format(auto_collect_tax_loyalty));
  }
  if (auto_grant_loyalty) {
    vga.active_buf->bar_alpha(button_grant.x1, button_grant.y1 + 15,
                              button_grant.x1 + BUTTON_ACTION_WIDTH - 1,
                              button_grant.y1 + BUTTON_ACTION_HEIGHT - 16, 1,
                              0);
    font_whbl.center_put(button_grant.x1, button_grant.y1,
                         button_grant.x1 + BUTTON_ACTION_WIDTH - 1,
                         button_grant.y1 + BUTTON_ACTION_HEIGHT - 1,
...
void Town::detect_main_menu() {
  ...
  if (mouse.single_click(INFO_X1 + 11, INFO_Y1 + 5, INFO_X1 + 217,
                         INFO_Y1 + 83) &&
      (!is_own() || remote.is_enable() ||
       !mouse.single_click(INFO_X1 + 23, INFO_Y1 + 11, INFO_X1 + 205,
                           INFO_Y1 + 28))) {
    world.go_loc(center_x, center_y);
    return;
  }

  if (mouse.single_click(INFO_X1 + 23, INFO_Y1 + 11, INFO_X1 + 205,
                         INFO_Y1 + 28) &&
      is_own() && !remote.is_enable()) {
    input_town_name();
    return;
  ...
  if (nation_array.player_recno) {
    ...
    if ((rc = button_builder.detect(0, 0, 1))) {
      ...
      {
        nation_array[nation_recno]->repair_all_building(
            button_builder.pushed_flag ? 1 : 0, COMMAND_PLAYER);
      }
      // ###### end Gilbert 21/4 ########//
    }
    ...
    if ((rc = button_destruct.detect(0, 0, 1))) {
      ...
      if (!remote.is_enable()) {
        ...
        short *shortPtr =
            (short *)remote.new_send_queue_msg(MSG_TOWN_SELL, sizeof(short));
        *shortPtr = town_recno;
      }
      // ##### end Gilbert 7/10 #######//
    }
  ...
  help.set_help(INFO_X1 + 23, INFO_Y1 + 11, INFO_X1 + 205, INFO_Y1 + 28,
                "TOWNNAME");
}
...
void Town::disp_spy_menu(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    browse_spy.init(INFO_X1 + 13, INFO_Y1 + 40, INFO_X1 + 233, INFO_Y1 + 219, 0,
                    45, playerSpyCount, put_spy_rec);

    //	browse_spy.init( RACE_BROWSE_X1, RACE_BROWSE_Y1, RACE_BROWSE_X2,
    //RACE_BROWSE_Y2, 							0, 25, playerSpyCount, put_spy_rec );
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    button_cancel.create(INFO_X1 + 13 + 3 * BUTTON_DISTANCE, y2, 'A',
                         "PREVMENU");
  }
  ...
  if (nation_recno !=
      nation_array.player_recno) // if the spy is in another nation's town
  {
...
void Town::detect_spy_menu() {
    ...
    if (!remote.is_enable()) {
      if (spyPtr->mobilize_town_spy()) {
        spyPtr->notify_cloaked_nation_flag =
            0; // reset it so the player can control it
               //			disp_spy_menu( INFO_REPAINT );
      }
    } else {
      // packet structure <spy recno>
      short *shortPtr =
          (short *)remote.new_send_queue_msg(MSG_SPY_LEAVE_TOWN, sizeof(short));
    ...
    if (button_spy_action.detect()) // set action mode
    {
      ...
      if (!remote.is_enable()) {
        ...
        short *shortPtr = (short *)remote.new_send_queue_msg(
            MSG_SPY_CYCLE_ACTION, sizeof(short));
        *shortPtr = spyPtr->spy_recno;
      }
    }
  }
  ...
  if (button_cancel
          .detect()) //|| mouse.any_click(1) )		// right click to cancel
  {
    // info.disp();
    town_menu_mode = TOWN_MENU_MAIN;
...
void Town::disp_auto_menu(int modeCollectTax, int refreshFlag) {
  ...
  //	if( modeCollectTax )
  //		headingStr = "Automatically Collect Tax from this Town (Left -
  //click) or all your Towns (Right - click) when the Loyalty reaches:"; 	else
  //		headingStr = "Automatically Grant Money to this Town (Left - click)
  //or all your Towns (Right - click) when the Loyalty drops below:"; 	str =
  //headingStr;
  font_bld.center_put_paragraph(
      INFO_X1 + 17, INFO_Y1 + 20, INFO_X2 - 7, INFO_Y1 + 70,
      modeCollectTax ? text_firm.str_auto_tax_inst()
                     : text_firm.str_auto_grant_inst(),
      0, 0, 0);

  for (i = 0; i < BUTTON_LOYALTY_COUNT; i++) {
    if (refreshFlag == INFO_REPAINT)
      button_loyalty_array[i].create(
          x + (i >> 2) * 94, y + (i % 4) * 23, x + (i >> 2) * 94 + 91,
          y + (i % 4) * 23 + 20, disp_auto_menu_button,
  ...
  if (refreshFlag == INFO_REPAINT)
    button_loyalty_disabled.create(
        x + (i >> 2) * 94, y + (i % 4) * 23, x + (i >> 2) * 94 + 91,
        y + (i % 4) * 23 + 20, disp_auto_menu_button,
        ButtonCustomPara(game.get_color_remap_table(nation_recno, 0), i));
...
void Town::disp_man_power(int refreshFlag) {
  ...
  //	headingStr = "Please assign the number of jobless civilians (0 - 8) to
  //build the town wall :"; 	str = headingStr;
  ...
  for (i = 0; i < 9; i++) {
    if (refreshFlag == INFO_REPAINT)
      button_loyalty_array[i].create(
          x, y + i * 19, x + 187, y + i * 19 + 17, disp_man_power_button,
          ButtonCustomPara(game.get_color_remap_table(nation_recno, 0), i));
    button_loyalty_array[i].paint();
  }
...
static void disp_man_power_button(ButtonCustom *button, int) {
  ...
  if (button->pushed_flag) {
    ...
    //		vga_util.blt_buf(button->x1, button->y1, button->x2, button->y2,
    //0);
    font_snds.center_put(button->x1 + 1, button->y1 + 2, button->x2 + 1,
                         button->y2 + 2, text_firm.str_no_of_worker(paintInfo));
  } else if (mouse.in_area(button->x1, button->y1, button->x2, button->y2)) {
    vga.active_buf->put_bitmap(button->x1, button->y1,
  ...
  if (rc == 1) {
    ...
    if (modeCollectTax) {
      if (!remote.is_enable()) {
        ...
        short *shortPtr = (short *)remote.new_send_queue_msg(MSG_TOWN_AUTO_TAX,
                                                             2 * sizeof(short));
        *shortPtr = town_recno;
      ...
      if (!remote.is_enable()) {
        ...
        // packet structure <town recno> <loyalty level>
        short *shortPtr = (short *)remote.new_send_queue_msg(
            MSG_TOWN_AUTO_GRANT, 2 * sizeof(short));
        *shortPtr = town_recno;
        shortPtr[1] = loyaltyLevel;
      }
    }
...
static void put_spy_rec(int recNo, int x, int y, int refreshFlag) {
  ...
  info.draw_unit_icon(x + 10, y + 15,
                      race_res[spyPtr->race_id]->civilian_unit_id, nationRecno,
  ...
  //		help.set_unit_help( unitId, 0, x+1, y+1, x+RACE_ICON_WIDTH+4,
  //y+RACE_ICON_HEIGHT+4 );
  //	}
...
#define SAVE_GAME_DESC_LEN 15
  ...
  descTextBox.init(boxX1 + 26, boxY1 + 12, boxX2 - 22, gameDesc,
                   SAVE_GAME_DESC_LEN, &font_town, 1, 0);
  ...
  button_ok.create_text(x, y, x + buttonWidth - 1, y + buttonHeight - 1,
                        text_basic.str_box_ok()); // "Ok" );
  x = (boxX1 + 2 * boxX2) / 3 - buttonWidth / 2 + 10;
  button_cancel.create_text(x, y, x + buttonWidth - 1, y + buttonHeight - 1,
                            text_basic.str_box_cancel()); // "Cancel" );
...
int Town::disp_edit_mode(int refreshFlag) {
  ...
  x = edit_population_x2 =
      font_blue.put((edit_population_x1 = x + 20), (edit_population_y1 = y),
                    population, 0, x2);
  ...
  for (i = 1; i <= nation_array.size(); i++) {
    ...
    if (nation_recno == i) {
      font_whbl.put(x1 + 15, y,
                    text_firm.str_loyalty_to(nationPtr->nation_name()), 0, x2);
      curLoyalty = (int)loyalty;
      targetLoyalty = (int)target_loyalty;
    } else {
      font_whbl.put(x1 + 15, y,
                    text_firm.str_resistance_to(nationPtr->nation_name()), 0,
                    x2);
    ...
    x = edit_loyalty_x2[i - 1] =
        font_blue.put((edit_loyalty_x1[i - 1] = x1 + 50),
...
void Town::detect_edit_mode() {
  ...
  if (edit_population_enable) {
    if (mouse.any_click(edit_population_x1, edit_population_y1,
                        edit_population_x2, edit_population_y2, LEFT_BUTTON)) {
      if (population < MAX_TOWN_POPULATION) {
        // ##### begin Gilbert 6/7 #######//
    ...
    if (mouse.any_click(edit_population_x1, edit_population_y1,
                        edit_population_x2, edit_population_y2, RIGHT_BUTTON)) {
      // ####### begin Gilbert 6/7 #####//
  ...
  for (i = 1; i <= nation_array.size(); i++) {
    if (edit_loyalty_enable[i - 1]) {
      if (mouse.any_click(edit_loyalty_x1[i - 1], edit_loyalty_y1[i - 1],
                          edit_loyalty_x2[i - 1], edit_loyalty_y2[i - 1],
                          LEFT_BUTTON)) {
        if (nation_recno == i) {
          loyalty += 10;
          if (loyalty > 100)
