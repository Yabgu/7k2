...
// next to.
// action_para					   - firm id.
// action_para2				   - firm race id. (for FirmBase and
// FirmSPU only)
//										  (for other
// firm
...
int Nation::ai_build_firm(ActionNode *actionNode)
{
    err_when(actionNode->action_x_loc < 0 ||
             actionNode->action_y_loc < 0); // they must be given when this function is called
    ...
    else
    {
        ...
        if (!builderUnit && firm_res[firmId]->is_military) // if the building is a military
...
int Nation::ai_assign_soldier(ActionNode *actionNode)
{
    ...
    if (MAX_SOLDIER - firmCamp->soldier_count <
        actionNode->instance_count) // if the firm now has more soldiers, reduce the
                                    // number needed to be assigned to the firm
    {
        ...
        actionNode->instance_count =
            MAX(actionNode->processing_instance_count + 1, MAX_SOLDIER - firmCamp->soldier_count);
    }
...
int Nation::ai_settle_town(ActionNode *actionNode)
{
    ...
    if (!townPtr->can_recruit_peasant())
    {
        ...
        int minRecruitLoyalty = MIN_RECRUIT_LOYALTY + townPtr->recruit_dec_loyalty(1, 0); // 1-recruit one unit only
    ...
    if (unitPtr->cur_order.mode == UNIT_ASSIGN)
    {
        ...
        if (recruitCount > 1)
            actionNode->instance_count =
                MAX(actionNode->processing_instance_count + 1, actionNode->instance_count - recruitCount + 1);
