...
//next to.
// action_para					   - firm id.
// action_para2				   - firm race id. (for FirmBase and FirmSPU
// only)
//										  (for other firm
//types, it doesn't matter as the king's nationality determines the style of
//buildings)
//
int Nation::ai_build_firm(ActionNode *actionNode) {
  err_when(actionNode->action_x_loc < 0 ||
           actionNode->action_y_loc <
               0); // they must be given when this function is called
  ...
  if (actionNode->unit_recno)
    ...
    builderUnit =
        ai_get_unit(true, raceId, actionNode); // true - get a civilian unit

    if (!builderUnit &&
        firm_res[firmId]->is_military) // if the building is a military
                                       // building, try location a military unit
  ...
  builderUnit->build_firm(actionNode->action_x_loc, actionNode->action_y_loc,
                          firmId, COMMAND_AI);
...
int Nation::ai_build_town(ActionNode *actionNode) {
  ...
  if (actionNode->unit_recno)
    ...
    builderUnit =
        ai_get_unit(1, raceId, actionNode); // 1-the unit needs to be a civilian
  ...
  builderUnit->settle_town(actionNode->action_x_loc, actionNode->action_y_loc,
                           COMMAND_AI);
...
//
// action_x_loc, action_y_loc - location of the firm to which the overseer
// should be assigned. ref_x_loc,    ref_y_loc    - not used action_para
// - firm id. [action_para2]				   - race id of the
// overseer
//										  (if not given,
//the majority race will be used.)
//
int Nation::ai_assign_overseer(ActionNode *actionNode) {
  ...
  if (!check_firm_ready(actionNode->action_x_loc, actionNode->action_y_loc,
                        firmId)) // return 0 to cancel action
  ...
  Unit *overseerUnit =
      ai_get_unit(0, raceId, actionNode); // 0-the unit is not a civilian unit
...
int Nation::ai_assign_soldier(ActionNode *actionNode) {
  ...
  if (MAX_SOLDIER - firmCamp->soldier_count <
      actionNode
          ->instance_count) // if the firm now has more soldiers, reduce the
                            // number needed to be assigned to the firm
  {
    err_when(actionNode->processing_instance_count >=
             actionNode->instance_count);
    actionNode->instance_count = MAX(actionNode->processing_instance_count + 1,
                                     MAX_SOLDIER - firmCamp->soldier_count);
  }
  ...
  if (!world.get_loc(actionNode->action_x_loc, actionNode->action_y_loc)
           ->is_firm()) // firm exists, so assign
    return -1;
...
int Nation::ai_settle_town(ActionNode *actionNode) {
  ...
  Location *locPtr =
      world.get_loc(actionNode->ref_x_loc, actionNode->ref_y_loc);
  ...
  if (!townPtr->can_recruit_peasant()) {
    //--- if there is not camp, that means we cannot grant villagers to increase
    //loyalty, then do nothing ---//
    ...
    if (townPtr->loyalty < minRecruitLoyalty) {
      ...
      if (!townPtr->can_recruit_peasant()) // if still cannot be recruited,
                                           // return 0 now
        return 0;
    }
  ...
  if (actionNode->instance_count - actionNode->processing_instance_count > 1) {
    recruitCount =
        actionNode->instance_count - actionNode->processing_instance_count;
  ...
  int unitRecno =
      townPtr->recruit(recruitWagon, COMMAND_AI); // 0-not recruit wagon
  ...
  unitPtr->assign(actionNode->action_x_loc,
                  actionNode->action_y_loc); // assign to the town
  ...
  if (unitPtr->cur_order.mode == UNIT_ASSIGN) {
    ...
    if (recruitCount > 1)
      actionNode->instance_count =
          MAX(actionNode->processing_instance_count + 1,
              actionNode->instance_count - recruitCount + 1);
