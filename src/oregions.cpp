...
void RegionArray::init_region_stat() {
  ...
  for (i = 1; i <= region_info_count; i++) {
    ...
    if (regionInfo->region_size >=
            MIN_STAT_REGION_SIZE && // regions are sorted by their sizes
  ...
  err_when(regionStatId !=
           region_stat_count + 1); // no all regionStat get their region_id
...
void RegionStat::init() {
  ...
  for (int seaRegionId = 1; seaRegionId <= region_array.region_info_count;
       seaRegionId++) {
    ...
    for (int i = 1; i <= region_array.region_stat_count; i++, regionStat++) {
      ...
      if (region_array.is_adjacent(seaRegionId, regionStat->region_id)) {
        reachable_region_array[reachable_region_count].sea_region_id =
            seaRegionId;
...
void RegionStat::update_stat() {
  ...
  memcpy(reachableRegionArray, reachable_region_array,
         sizeof(reachable_region_array));
  ...
  memcpy(reachable_region_array, reachableRegionArray,
         sizeof(reachable_region_array));
  ...
  for (i = town_array.size(); i > 0; i--) {
    ...
    if (townPtr->nation_recno) {
      ...
      nation_jobless_population_array[townPtr->nation_recno - 1] +=
  ...
  for (i = 0; i < nation_array.size(); i++) {
    ...
    if (firm_nation_count_array[i] > 0 || town_nation_count_array[i] > 0 ||
