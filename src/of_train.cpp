...
//
//
void FirmTrain::recruit_trainee(int townRecno, int trainType,
                                char remoteAction) {
  ...
  if (town_array.is_deleted(townRecno) ||
      ...
      (firm_res[firm_id]->same_race_worker &&
       town_array[townRecno]->race_id != race_id))
    return;
  ...
  // (spySeq=misc.random(townPtr->recruitable_pop(1))) < townPtr->spy_count )
  // use town population instead of recruitable_pop(1) because assume some spy
  // is working, spy_count include those working

  if (townPtr->spy_count > 0 &&
      (spySeq = misc.random(townPtr->population)) < townPtr->spy_count)
    spyRecno = spy_array.find_town_spy(townRecno, 1 + spySeq);
  else
    spyRecno = 0;

  traineePtr->init(townPtr->race_id, (char)townPtr->loyalty, spyRecno,
                   trainType, townRecno);
...
int FirmTrain::mobilize_trainee(int traineeId, char remoteAction) {
  ...
  err_when(!trainee_array); // this function shouldn't be called if this firm
                            // does not need trainee
  ...
  if (thisTrainee.is_under_training) {
    if (thisTrainee.is_human()) {
      ...
      //			thisTrainee.unit_id =
      //race_res[thisTrainee.race_id]->civilian_unit_id; 			if( nation_recno )
      //				unit_res[thisTrainee.unit_id]->inc_nation_unit_count(nation_recno);
      //			Skill civilSkill;
      //			civilSkill.init(thisTrainee.unit_id,
      //CITIZEN_COMBAT_LEVEL, 0); 			thisTrainee.skill = civilSkill;

      thisTrainee.skill.set_combat_level(CITIZEN_COMBAT_LEVEL);
      thisTrainee.spy_skill = 0; // reset spy skill
      thisTrainee.change_unit_id(
          race_res[thisTrainee.race_id]->civilian_unit_id, nation_recno);
    } else {
      err_here();
    }
  }
  ...
  unitRecno =
      create_trainee_unit(thisTrainee); // if he is a spy, create_trainee_unit
    ...
    for (int i = trainee_count; i > 0; --i) {
      ...
      if (!trainee_array[i - 1].is_under_training) {
        if (!mobilize_trainee(i, COMMAND_AUTO))
          kill_trainee(i); // if the unit cannot be mobilized (can be because
...
int FirmTrain::create_trainee_unit(Trainee &thisTrainee) {
  ...
  //--------- decrease the unit count first because create_unit() will increase
  //it ---------//
  ...
  int unitRecno = create_unit(
      unitId); // this trainee no longer has a job as it has been resigned
  ...
  //	{
  //		unitPtr->set_weapon_version( thisTrainee.extra_para );	// restore
  //nation contribution
  //	}
  ...
  //	if( thisTrainee.name_id && thisTrainee.race_id )		// if this
  //trainee is formerly an unit who has a name
  ...
  if (thisTrainee.spy_recno) {
    ...
    unitPtr->spy_recno =
        spy_array.add_spy(unitRecno, (int)thisTrainee.spy_skill);
  }
...
int FirmTrain::can_recruit(int townRecno, int trainType) {
  ...
  return nation_array[nation_recno]->cash >= 0 && trainee_count < MAX_TRAINEE &&
...
void FirmTrain::change_nation(int newNationRecno) {
  ...
  for (int i = 0; i < trainee_count; i++)
    unit_res[trainee_array[i].unit_id]->unit_change_nation(
        newNationRecno, nation_recno, RANK_SOLDIER);
...
void FirmTrain::cancel_train_trainee(int traineeId, char remoteAction) {
  ...
  if (sourceTown) {
    ...
    if (traineePtr->spy_recno > 0) {
      ...
      traineePtr->spy_recno =
          0; // reset it so kill_trainee() won't delete the spy
    }
  ...
  for (int i = 1; i <= trainee_count; ++i) {
    ...
    if (traineePtr->source_town_recno) {
      if (town_array.is_deleted(traineePtr->source_town_recno) ||
          town_array[traineePtr->source_town_recno]->race_id !=
              traineePtr->race_id ||
          town_array[traineePtr->source_town_recno]->nation_recno !=
              nation_recno) {
