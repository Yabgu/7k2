...
int FirmTrain::mobilize_trainee(int traineeId, char remoteAction)
{
    ...
    unitRecno = create_trainee_unit(thisTrainee); // if he is a spy, create_trainee_unit
...
int FirmTrain::create_trainee_unit(Trainee &thisTrainee)
{
    ...
    //	{
    //		unitPtr->set_weapon_version( thisTrainee.extra_para );	//
    // restore
    // nation contribution
    //	}
    ...
    //	if( thisTrainee.name_id && thisTrainee.race_id )		// if
    // this
...
int FirmTrain::can_recruit(int townRecno, int trainType)
{
    ...
    return nation_array[nation_recno]->cash >= 0 && trainee_count < MAX_TRAINEE && town_array[townRecno]->can_train();
}
...
void FirmTrain::cancel_train_trainee(int traineeId, char remoteAction)
{
    ...
    if (traineePtr->source_town_recno && !town_array.is_deleted(traineePtr->source_town_recno) &&
        (townPtr = town_array[traineePtr->source_town_recno]) && townPtr->race_id == traineePtr->race_id &&
    ...
    if (!sourceTown)
    {
        for (int i = 0; i < linked_town_count; ++i)
        {
            ...
            if (townPtr->race_id == traineePtr->race_id && townPtr->nation_recno == nation_recno &&
