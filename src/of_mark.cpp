            ...
            if (firmMine->raw_id)
            {
                for (i = 0, marketGoods = market_goods_array; i < MAX_MARKET_GOODS; i++, marketGoods++)
                {
                    ...
                    if (marketGoods->raw_id == firmMine->raw_id)
                    {
                        ...
                        if (firmMine->next_output_firm_recno == firm_recno && firmMine->stock_qty > 0 &&
            ...
            if (firmFactory->product_raw_id)
            {
                for (i = 0, marketGoods = market_goods_array; i < MAX_MARKET_GOODS; i++, marketGoods++)
                {
                    if (marketGoods->product_raw_id == firmFactory->product_raw_id)
                    {
                        ...
                        if (firmFactory->next_output_firm_recno == firm_recno && firmFactory->stock_qty > 0 &&
                            marketGoods->stock_qty < max_stock_qty)
                        {
        ...
        if (marketGoods->product_raw_id && marketGoods->stock_qty > 0)
        {
            ...
            if (is_ai && config.ai_aggressiveness > OPTION_MODERATE)
                sales += sales * (config.ai_aggressiveness - OPTION_MODERATE) /
                         4; // 25% more cash if AI aggressivness is high,
                            // 50% more if aggressiveness is very high
...
void FirmMarket::update_trade_link()
{
    ...
    for (int i = 0; i < linked_town_count; i++)
    {
        ...
        {
            tradeTreaty =
                ownNation->get_relation(townPtr->nation_recno)->trade_treaty || townPtr->nation_recno == nation_recno;
        }
