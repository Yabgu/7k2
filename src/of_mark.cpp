...
void FirmMarket::next_day() {
  ...
  if (info.game_date % PROCESS_GOODS_INTERVAL ==
      firm_recno % PROCESS_GOODS_INTERVAL) {
...
//					 >0 - the number of new caravans we can
//hire.
//
int FirmMarket::can_hire_caravan() {
  ...
  int caravanCount =
      unit_res[UNIT_CARAVAN]->nation_unit_count_array[nation_recno - 1];
...
short FirmMarket::hire_caravan(char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_MARKET_HIRE_CARA,
                                                         sizeof(short));
    *shortPtr = firm_recno;
    return 0;
  }
  ...
  for (t = 0; t < linked_firm_count; t++) {
    ...
    if (firmPtr->firm_id == FIRM_MINE) {
      ...
      if (firmMine->raw_id) {
        for (i = 0, marketGoods = market_goods_array; i < MAX_MARKET_GOODS;
             i++, marketGoods++) {
          //--- only assign a slot to the product if it comes from a firm of our
          //own ---//
          ...
          if (marketGoods->raw_id == firmMine->raw_id) {
            ...
            if (firmMine->next_output_firm_recno == firm_recno &&
                firmMine->stock_qty > 0 &&
                marketGoods->stock_qty < max_stock_qty) {
              ...
              if (firmPtr->nation_recno != nation_recno)
                nationPtr->import_goods(IMPORT_RAW, firmPtr->nation_recno,
                                        inputQty * RAW_PRICE);
            } else if (marketGoods->stock_qty == max_stock_qty) {
              marketGoods->cur_month_supply++; // add it so the other functions
                                               // can know that this market has
                                               // direct supply links
            }
        ...
        if (i == MAX_MARKET_GOODS && firmMine->stock_qty > 0 &&
            !queued_firm_recno)
    ...
    //--------- if it's a factory ------------//
      ...
      if (firmFactory->product_raw_id) {
        for (i = 0, marketGoods = market_goods_array; i < MAX_MARKET_GOODS;
          ...
          if (marketGoods->product_raw_id == firmFactory->product_raw_id) {
            ...
            if (firmFactory->next_output_firm_recno == firm_recno &&
              ...
              if (firmPtr->nation_recno != nation_recno)
                nationPtr->import_goods(IMPORT_PRODUCT, firmPtr->nation_recno,
                                        inputQty * PRODUCT_PRICE);
            } else if (marketGoods->stock_qty == max_stock_qty) {
              marketGoods->cur_month_supply++; // add it so the other functions
                                               // can know that this market has
                                               // direct supply links
        ...
        if (i == MAX_MARKET_GOODS && firmFactory->stock_qty > 0 &&
            !queued_firm_recno)
          queued_firm_recno = firmPtr->firm_recno;
      }
    }
...
void FirmMarket::sell_goods() {
  ...
  for (i = 0, marketGoods = market_goods_array; i < MAX_MARKET_GOODS;
    ...
    if (marketGoods->product_raw_id && marketGoods->stock_qty > 0) {
      ...
      // ##### begin Gilbert 16/2 #######//
      if (nation_recno &&
          god_res[GOD_CHINESE]->nation_prayer_count(nation_recno) > 0)
      ...
      if (is_ai && config.ai_aggressiveness > OPTION_MODERATE)
        sales += sales * (config.ai_aggressiveness - OPTION_MODERATE) /
                 4; // 25% more cash if AI aggressivness is high, 50% more if
  ...
  for (i = 0, marketGoods = market_goods_array; i < MAX_MARKET_GOODS;
    ...
    if (marketGoods->product_raw_id || marketGoods->raw_id) {
      if (marketGoods->sales_365days() == 0 &&
          marketGoods->supply_30days() == 0 && marketGoods->stock_qty == 0) {
...
int FirmMarket::stock_value_index() {
  ...
  return 100 * (int)totalValue /
         (MAX_MARKET_GOODS * PRODUCT_PRICE * MAX_MARKET_STOCK);
}
...
//		if( productId )
//			market_product_array[productId-1] = market_goods_array
+ i;
//	}
...
void FirmMarket::update_trade_link() {
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    if (townPtr->nation_recno) {
      tradeTreaty =
          ownNation->get_relation(townPtr->nation_recno)->trade_treaty ||
          townPtr->nation_recno == nation_recno;
    } else // if this is an independent town, it buys goods from the market if
           // its resistance towards the nation is < 30
    {
      tradeTreaty =
    ...
    if (linked_town_enable_array[i] != (tradeTreaty ? LINK_EE : LINK_DD))
      toggle_town_link(i + 1, tradeTreaty, COMMAND_AUTO,
                       1); // 1-toggle both side
  }
}
