...
void Campaign::select_royal_menu(CampaignMember *king, CampaignMember *royalList, int &royalCount)
{
    ...
    int needSelectFryhtan = can_use_fryhtan && surrendered_fryhtan_id &&
                            (attacker_state_recno && state_array[attacker_state_recno]->campaign_nation_recno ==
                                                         CAMPAIGN_PLAYER_NATION_RECNO ||
                             attacker2_state_recno && state_array[attacker2_state_recno]->campaign_nation_recno ==
                                                          CAMPAIGN_PLAYER_NATION_RECNO);
        ...
        while (1)
        {
            ...
            if (refreshFlag)
            {
                ...
                if (refreshFlag & DROPTION_UNITS)
                {
                    for (int r = king ? 0 : 1; r <= (UNIT_ROW * UNIT_MAX_ROW); ++r)
                    {
                        ...
                        if (unit)
                        {
                            ...
                            // draw unit info
                            info.draw_unit_icon(x1 + UNIT_ICON_OFFSET_X, y1 + UNIT_ICON_OFFSET_Y, unit->unit_id,
                            ...
                            if (unit->item.id)
                            {
                                vga.active_buf->put_bitmap_trans(x1 + ITEM_OFFSET_X, y1 + ITEM_OFFSET_Y,
                            ...
                            if (r == 0 || selectedFlagArray[r - 1] > 0) // king is always selected
                            {
                                vga.active_buf->put_bitmap_trans_decompress(
                                    x1 + TICK_OFFSET_X + 4, y1 + TICK_OFFSET_Y + 4, image_button.read("TICK"));
                                vga.active_buf->d3_panel_down(x1 + TICK_OFFSET_X, y1 + TICK_OFFSET_Y,
                                                              x1 + TICK_OFFSET_X + tickWidth - 1,
                                                              y1 + TICK_OFFSET_Y + tickHeight - 1, 4, 0);
                            }
                            else if (selectedFlagArray[r - 1] == 0)
                                vga.active_buf->d3_panel_up(x1 + TICK_OFFSET_X, y1 + TICK_OFFSET_Y,
                                                            x1 + TICK_OFFSET_X + tickWidth - 1,
                ...
                if (refreshFlag & DROPTION_UNIT_INFO)
                {
                    ...
                    if (selectedRecno == 0 && king || selectedRecno > 0 && selectedRecno <= royalCount)
                    {
                        CampaignMember *unit;
                        if (selectedRecno == 0)
                            unit = king;
            ...
            if (needSelectFryhtan &&
                (askFryhtanForceFirst || mouse.single_click(BUTTON3_X1, BUTTON3_Y1, BUTTON3_X2, BUTTON3_Y2)))
            {
                ...
                this_battle_use_fryhtan = !box.ask(
                    ...
                    // mission:", humanForceStr, fryhtanForceStr );		// use ! to
                    // reverse the order
            ...
            for (int r = king ? 0 : 1; r <= royalCount; ++r)
            {
                ...
                if (r > 0 && mouse.any_click(x1 + TICK_OFFSET_X, y1 + TICK_OFFSET_Y, x1 + TICK_OFFSET_X + tickWidth - 1,
                                             y1 + TICK_OFFSET_Y + tickHeight - 1))
                {
                ...
                // right click at item area
                                                          ...
                                                          x1 + ITEM_OFFSET_X + ITEM_UNIT_ICON_WIDTH - 1,
                                                          y1 + ITEM_OFFSET_Y + ITEM_UNIT_ICON_HEIGHT - 1, 1))
                {
                    // delete item
            ...
            if (selectedRecno > 0 && selectedRecno <= royalCount &&
                (mouse.any_click(BUTTON9_X1, BUTTON9_Y1, BUTTON9_X2, BUTTON9_Y2) || mouse.key_code == KEY_DEL))
            {
...
void Campaign::pick_royal_to_battle()
{
    ...
    if (stage_id == CampaignEastWest::STAGE_TRAIN_LEADERS) // this scenario should not allow
    ...
    select_royal_menu(cNation->king.unit_id ? &cNation->king : NULL, cNation->noble, cNation->noble_count);
}
