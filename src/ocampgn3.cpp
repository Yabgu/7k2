...
void Campaign::catch_nation_info() {
  for (int i = nation_array.size(); i > 0; i--) {
    ...
    memcpy(cNation->know_base_array, nationPtr->know_base_array,
           sizeof(nationPtr->know_base_array));

    //----------- copy tech info ------------//

    int j;
    for (j = 1; j <= tech_res.tech_count; j++) {
      cNation->tech_level_array[j - 1] =
          tech_res[j]->nation_tech_level_array[i - 1];
      cNation->research_progress_array[j - 1] =
          tech_res[j]->nation_research_progress_array[i - 1];
    }
    ...
    for (j = 1; j <= nation_array.size(); j++) {
      ...
      cNation->relation_array[t - 1].should_attack =
          nationPtr->relation_array[j - 1].should_attack;
      cNation->relation_array[t - 1].trade_treaty =
          nationPtr->relation_array[j - 1].trade_treaty;
      cNation->relation_array[t - 1].status =
          nationPtr->relation_array[j - 1].status;
    }
  }
}
...
void Campaign::fetch_nation_info() {
  for (int i = nation_array.size(); i > 0; i--) {
    ...
    memcpy(nationPtr->know_base_array, cNation->know_base_array,
           sizeof(nationPtr->know_base_array));
    ...
    for (j = 1; j <= nation_array.size(); j++) {
      ...
      nationPtr->set_relation_should_attack(
          j, cNation->relation_array[t - 1].should_attack, COMMAND_AUTO);
      nationPtr->set_trade_treaty(j,
                                  cNation->relation_array[t - 1].trade_treaty);
  ...
  for (i = 1; i <= unit_array.size(); ++i) {
    ...
    if (royalCount < royalMax) {
      ...
      if (unitPtr->spy_recno &&
          unitPtr->true_nation_recno() == unitPtr->nation_recno)
    ...
    if (firmPtr->cast_to_FirmCamp()) {
      ...
      for (int j = 0; j < campPtr->soldier_count; ++j) {
        ...
        if (soldierPtr->spy_recno &&
            soldierPtr->true_nation_recno(i) == firmPtr->nation_recno)
...
void CampaignMember::update_unit(Soldier *soldierPtr, Spy *spyPtr,
                                 int firmRecno) {
  sprite_recno = 0;
...
int CampaignMember::create_unit(int nationRecno, int xLoc, int yLoc) {
  int unitRecno =
      unit_array.add_unit(unit_id, nationRecno, rank_id, loyalty, xLoc, yLoc);
  ...
  if (unitRecno) {
    ...
    sprite_recno =
        unitPtr->sprite_recno; // mark this unit has been put to the game
  }
...
//
char *CampaignMember::unit_name(int nationRecno, int withTitle,
                                int firstNameOnly) {
  static String str;
...
//
void Campaign::select_royal_menu(CampaignMember *king,
                                 CampaignMember *royalList, int &royalCount) {
  ...
  //	char *helpText = text_campaign.str_select_royal_help_1(); // "Check the
  //units that you wish to use in this scenario. You may Click on an Artifact
  //and move it to another unit.";
  int selectedRecno = 0;
  ...
  Blob backupMember(sizeof(CampaignMember) *
                    (royalCount + 1)); // add a bit more
  ...
  int needSelectFryhtan =
      can_use_fryhtan && surrendered_fryhtan_id &&
      (attacker_state_recno &&
           state_array[attacker_state_recno]->campaign_nation_recno ==
               CAMPAIGN_PLAYER_NATION_RECNO ||
       attacker2_state_recno &&
           state_array[attacker2_state_recno]->campaign_nation_recno ==
               CAMPAIGN_PLAYER_NATION_RECNO);
    ...
    while (1) {
      ...
      if (refreshFlag) {
        if (refreshFlag & DROPTION_PAGE) {
          ...
          font_bold_black.center_put(SCROLL_SHEET_X1 + 10, 81,
                                     SCROLL_SHEET_X1 + 210, 116,
                                     text_campaign.str_royal_units());
          //"Royal Units");
          font_cara.put_paragraph(
              SCROLL_SHEET_X1 + 210, 81, SCROLL_SHEET_X2 - 10, 116,
...

          {
            ...
            vga.active_buf->draw_d3_up_border(frameX1, frameY1, frameX2,
                                              frameY2);
          }
        ...
        if (refreshFlag & DROPTION_UNITS) {
          for (int r = king ? 0 : 1; r <= (UNIT_ROW * UNIT_MAX_ROW); ++r) {
            ...
            if (unit) {
              ...
              // draw unit info
              info.draw_unit_icon(x1 + UNIT_ICON_OFFSET_X,
                                  y1 + UNIT_ICON_OFFSET_Y, unit->unit_id,
                                  -colorSchemeId, x1, y1, x2, y2, 1);

              // display item

              if (unit->item.id) {
                vga.active_buf->put_bitmap_trans(
                    x1 + ITEM_OFFSET_X, y1 + ITEM_OFFSET_Y,
                    item_res.item_unit_interface(unit->item.id));
              }

              // display combat level
              font_whbl.right_put(x1 + 26, y1 + 2,
                                  misc.format(unit->combat_level()));

              // display leadership
              if (unit->skill_level() > 0) {
                font_whbl.right_put(x1 + 26, y1 + 16,
                                    misc.format(unit->skill_level()));
              }

              // display spy skill

              if (unit->is_spy) {
                font_whbl.right_put(x1 + 26, y1 + 30,
                                    misc.format(unit->spy_skill));
              }

              // frame if selected

              if (r == 0 ||
                  selectedFlagArray[r - 1] > 0) // king is always selected
              {
                vga.active_buf->put_bitmap_trans_decompress(
                    x1 + TICK_OFFSET_X + 4, y1 + TICK_OFFSET_Y + 4,
                    image_button.read("TICK"));
                vga.active_buf->d3_panel_down(
                    x1 + TICK_OFFSET_X, y1 + TICK_OFFSET_Y,
                    x1 + TICK_OFFSET_X + tickWidth - 1,
                    y1 + TICK_OFFSET_Y + tickHeight - 1, 4, 0);
              } else if (selectedFlagArray[r - 1] == 0)
                vga.active_buf->d3_panel_up(
                    x1 + TICK_OFFSET_X, y1 + TICK_OFFSET_Y,
                    x1 + TICK_OFFSET_X + tickWidth - 1,
                    y1 + TICK_OFFSET_Y + tickHeight - 1);
              else {
                vga.active_buf->d3_panel_up(
                    x1 + TICK_OFFSET_X, y1 + TICK_OFFSET_Y,
                    x1 + TICK_OFFSET_X + tickWidth - 1,
                    y1 + TICK_OFFSET_Y + tickHeight - 1);
                font_cara.center_put(x1 + TICK_OFFSET_X, y1 + TICK_OFFSET_Y,
                                     x1 + TICK_OFFSET_X + tickWidth - 1,
                                     y1 + TICK_OFFSET_Y + tickHeight - 1, "X");
              }
        ...
        if (refreshFlag & DROPTION_UNIT_INFO) {
          vga.active_buf->d3_panel_down(UNIT_INFO_X1, UNIT_INFO_Y1,
                                        UNIT_INFO_X2, UNIT_INFO_Y2);
          ...
          if (selectedRecno == 0 && king ||
            ...
            font_cara.put(UNIT_INFO_X1 + 6, UNIT_INFO_Y1 + 6,
                          unit_res[unit->unit_id]->name);
            font_cara.put(UNIT_INFO_X1 + 6, UNIT_INFO_Y1 + 22,
                          unit->unit_name(CAMPAIGN_PLAYER_NATION_RECNO));
            int x2 = font_cara.put(UNIT_INFO_X1 + 6, UNIT_INFO_Y1 + 38,
                                   unit->item.item_name());
            if (unit->item.id) {
            ...
            font_cara.put(UNIT_INFO_X1 + 300, UNIT_INFO_Y1 + 6,
                          text_unit.str_combat_level()); // "Combat Level" );
            font_cara.put(UNIT_INFO_X1 + 450, UNIT_INFO_Y1 + 6,
                          unit->combat_level());

            if (unit->skill_level() > 0) {
              font_cara.put(UNIT_INFO_X1 + 300, UNIT_INFO_Y1 + 22,
                            text_unit.str_leadership()); //"Leadership" );
              font_cara.put(UNIT_INFO_X1 + 450, UNIT_INFO_Y1 + 22,
                            unit->skill_level());
            }
            if (unit->is_spy) {
              font_cara.put(UNIT_INFO_X1 + 300, UNIT_INFO_Y1 + 38,
                            text_unit.str_spy_skill()); // "Spying" );
              font_cara.put(UNIT_INFO_X1 + 450, UNIT_INFO_Y1 + 38,
                            unit->spy_skill);
            }
          }
        }
        ...
        if (refreshFlag & DROPTION_FRYHTAN_FORCE) {
          if (needSelectFryhtan) {
            ...
            font_thin_black.center_put_paragraph(
                BUTTON3_X1 + 1, BUTTON3_Y1, BUTTON3_X2, BUTTON3_Y2,
                this_battle_use_fryhtan ? text_campaign.str_fryhtan_force()
      ...
      if (needSelectFryhtan && (askFryhtanForceFirst ||
                                mouse.single_click(BUTTON3_X1, BUTTON3_Y1,
                                                   BUTTON3_X2, BUTTON3_Y2))) {
        //--- if the player has a fryhtan troop to use and the player is an
        //attacker in the next battle, then allow the player to select the type
        //of troop to send. ---//

        this_battle_use_fryhtan = !box.ask(
            // "Choose the type of force that you wish to field in the next
            // mission:", humanForceStr, fryhtanForceStr );		// use ! to reverse
            // the order
            text_campaign.str_ask_fryhtan(), text_campaign.str_human_force(),
        ...
        if (askFryhtanForceFirst) {
          ...
          continue; // skip other mouse click, especially when
                    // askFryhtanForceFirst is true
        }
      }
      ...
      for (int r = king ? 0 : 1; r <= royalCount; ++r) {
        ...
        if (r > 0 && mouse.any_click(x1 + TICK_OFFSET_X, y1 + TICK_OFFSET_Y,
                                     x1 + TICK_OFFSET_X + tickWidth - 1,
                                     y1 + TICK_OFFSET_Y + tickHeight - 1)) {
        ...
        // click at item area
                                 ...
                                 x1 + ITEM_OFFSET_X + ITEM_UNIT_ICON_WIDTH - 1,
                                 y1 + ITEM_OFFSET_Y + ITEM_UNIT_ICON_HEIGHT -
                                     1) ||
          ...
          if (freeSlot.id) {
            mouse_cursor.define_custom_icon(
                item_res.item_unit_interface(freeSlot.id), 0, 0);
        ...
        else if (unit->item.id &&
                 mouse.any_click(x1 + ITEM_OFFSET_X, y1 + ITEM_OFFSET_X,
                                 x1 + ITEM_OFFSET_X + ITEM_UNIT_ICON_WIDTH - 1,
                                 y1 + ITEM_OFFSET_Y + ITEM_UNIT_ICON_HEIGHT - 1,
                                 1)) {
          // delete item
      ...
      if (mouse.single_click(BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2, BUTTON4_Y2)) {
        ...
        memcpy(royalList, backupMember.p(),
               sizeof(CampaignMember) * backupCount);
      ...
      if (selectedRecno > 0 && selectedRecno <= royalCount &&
        ...
        misc.del_array_rec(royalList, royalCount, sizeof(royalList[0]),
                           selectedRecno);
        misc.del_array_rec(selectedFlagArray, royalCount,
                           sizeof(selectedFlagArray[0]), selectedRecno);
        --royalCount;
      ...
      if (mouse.single_click(BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2)) {
        ...
        if (selectedCount > CURRENT_MAX_ROYAL_UNIT) {
          ...
          // box.msg("You cannot keep more than 20 royal units, delete some of
          // them");
          box.msg(text_campaign.str_max_royal_in_store(MAX_NOBLE));
        } else {
          // confirm to clear item

          // if( box.ask( freeSlot.id ? text_campaign.str_confirm_drop_item()
          // "Drop this item?" : "Are you sure?" ) )
          if (box.ask(freeSlot.id ? text_campaign.str_confirm_drop_item()
                                  : text_campaign.str_confirm_end_royal())) {
            freeSlot.clear();
            break;
          }
        }
...
void Campaign::pick_royal_to_battle() {
  ...
  if (stage_id ==
      CampaignEastWest::STAGE_TRAIN_LEADERS) // this scenario should not allow
                                             // using royal units otherwise it
  ...
  select_royal_menu(cNation->king.unit_id ? &cNation->king : NULL,
