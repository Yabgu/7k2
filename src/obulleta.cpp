...
//		always return 1
//
int BulletArray::bullet_path_possible(short startXLoc, short startYLoc,
  ...
  while (curStep++ < totalStep) {
    ...
    if (!emitter) {
      ...
      if (!locPtr->walkable(3) &&
          (locPtr->unit_recno(emitter->mobile_type) != emitter->sprite_recno))
        return 0;
    }
  }
...
// return 0 if not
//
int BulletArray::add_bullet_possible(short startXLoc, short startYLoc,
  ...
  if (targetWidth == 1 && targetHeight == 1) {
    if (bullet_path_possible(startXLoc, startYLoc, attackerMobileType,
                             targetXLoc, targetYLoc, targetMobileType,
  ...
  //----------------------------------------------------------------------//
  if (bullet_path_possible(startXLoc, startYLoc, attackerMobileType,
                           targetXLoc + xOffset, targetYLoc + yOffset,
                           targetMobileType, bulletSpeed, bulletSpriteId,
                           emitter)) {
  ...
  short found =
      0; // found a location to attack the target and the path is not blocked
  short end = 0;
  ...
  while (!found) {
    ...
    leftX += leftXOffset;
    leftY += leftYOffset;
    if (leftX >= 0 && leftX < targetWidth && leftY >= 0 &&
        leftY < targetHeight) {
      if (bullet_path_possible(startXLoc, startYLoc, attackerMobileType,
                               targetXLoc + leftX, targetYLoc + leftY,
                               targetMobileType, bulletSpeed, bulletSpriteId,
                               emitter)) {
    ...
    rightX += rightXOffset;
    rightY += rightYOffset;
    if (rightX >= 0 && rightX < targetWidth && rightY >= 0 &&
        rightY < targetHeight) {
      if (bullet_path_possible(startXLoc, startYLoc, attackerMobileType,
                               targetXLoc + rightX, targetYLoc + rightY,
                               targetMobileType, bulletSpeed, bulletSpriteId,
                               emitter)) {
...
short BulletArray::add_bullet(Firm *parentFirm, Unit *targetUnit) {
  ...
  bulletPtr->init(BULLET_BY_FIRM, parentFirm->firm_recno,
                  targetUnit->next_x_loc(), targetUnit->next_y_loc(),
...
short BulletArray::add_bullet(Unit *parentUnit, BaseObj *targetObj) {
  ...
  if (attackXLoc >= targetXLoc &&
      attackXLoc < targetXLoc + targetObj->obj_loc_width() &&
      attackYLoc >= targetYLoc &&
    ...
    //-------------------------------------------------------//
    short bulletId = (parentUnit->attack_info_array + parentUnit->cur_attack)
                         ->bullet_sprite_id;
    Bullet *bulletPtr;
    create_bullet(bulletId, &bulletPtr);
    bulletPtr->init(BULLET_BY_UNIT, parentUnit->sprite_recno, attackXLoc,
                    attackYLoc, targetObj->obj_mobile_type());
    err_when(parentUnit->cur_dir != parentUnit->final_dir);
    parentUnit->get_dir(parentUnit->next_x_loc(), parentUnit->next_y_loc(),
                        attackXLoc, attackYLoc);
    return bulletPtr->sprite_recno;
  }
