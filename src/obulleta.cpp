...
//		always return 1
//
int BulletArray::bullet_path_possible(short startXLoc, short startYLoc, char attackerMobileType, short destXLoc,
                                      short destYLoc, char targetMobileType, char bulletSpeed, short bulletSpriteId,
                                      Unit *emitter)
{
    // ##### begin Gilbert 6/5 ######//
    return 1;
...
// return 0 if not
//
int BulletArray::add_bullet_possible(short startXLoc, short startYLoc, char attackerMobileType, short targetXLoc,
                                     short targetYLoc, char targetMobileType, short targetWidth, short targetHeight,
                                     short &resultXLoc, short &resultYLoc, char bulletSpeed, short bulletSpriteId,
                                     Unit *emitter)
{
    ...
    {
        if (bullet_path_possible(startXLoc, startYLoc, attackerMobileType, targetXLoc, targetYLoc, targetMobileType,
                                 bulletSpeed, bulletSpriteId, emitter))
        {
    ...
    short atEdge = 0;                   // i.e. the attacking point is at the corner or edge of the
                                        // target 1 for at the edge, 0 for at corner
    ...
    //----------------------------------------------------------------------//
    if (bullet_path_possible(startXLoc, startYLoc, attackerMobileType, targetXLoc + xOffset, targetYLoc + yOffset,
                             targetMobileType, bulletSpeed, bulletSpriteId, emitter))
    {
    ...
    while (!found)
    {
        ...
        {
            if (bullet_path_possible(startXLoc, startYLoc, attackerMobileType, targetXLoc + leftX, targetYLoc + leftY,
                                     targetMobileType, bulletSpeed, bulletSpriteId, emitter))
            {
        ...
        {
            if (bullet_path_possible(startXLoc, startYLoc, attackerMobileType, targetXLoc + rightX, targetYLoc + rightY,
                                     targetMobileType, bulletSpeed, bulletSpriteId, emitter))
            {
...
short BulletArray::add_bullet(Firm *parentFirm, Unit *targetUnit)
{
    ...
    bulletPtr->init(BULLET_BY_FIRM, parentFirm->firm_recno, targetUnit->next_x_loc(), targetUnit->next_y_loc(),
                    targetUnit->mobile_type);
...
short BulletArray::add_bullet(Place *parentPlace, Place *targetPlace)
{
    ...
    /*	if (parentPlace == targetPlace &&
                    (parentPlace->bullet_id() == 0 ||
       sprite_res[parentPlace->bullet_id()]->sprite_sub_type != 'M'))
            {
    ...
    {
        bulletPtr->init(BULLET_BY_FIRM, parentPlace->cast_to_Firm()->firm_recno, targetPlace->center_x,
                        targetPlace->center_y, UNIT_LAND);
    }
    else if (parentPlace->cast_to_Town())
    {
        bulletPtr->init(BULLET_BY_TOWN, parentPlace->cast_to_Town()->town_recno, targetPlace->center_x,
                        targetPlace->center_y, UNIT_LAND);
    }
    return bulletPtr->sprite_recno;
...
short BulletArray::add_bullet(Unit *parentUnit, BaseObj *targetObj)
{
    ...
    if (attackXLoc >= targetXLoc && attackXLoc < targetXLoc + targetObj->obj_loc_width() && attackYLoc >= targetYLoc &&
        attackYLoc < targetYLoc + targetObj->obj_loc_height())
    {
        //-------------------------------------------------------//
...
short BulletArray::add_bullet(Town *parentTown, Unit *targetUnit)
{
    ...
    bulletPtr->init(BULLET_BY_TOWN, parentTown->town_recno, targetUnit->next_x_loc(), targetUnit->next_y_loc(),
                    targetUnit->mobile_type);
    return bulletPtr->sprite_recno;
}
//----------- End of function BulletArray::add_bullet -----------//
