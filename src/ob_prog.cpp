...
void BulletProgram::draw(int, int drawFlag)
{
        ...
        if (cur_action == SPRITE_MOVE)
        {
            if (config.building_size == 1)
                magic.draw_magic_ten(
                    &vga_back, world.zoom_matrix->calc_zoom_x(origin_x, origin_y, z_init),
    ...
    else if (str == "MAGIC2")
    {
        ...
        {
            if (config.building_size == 1)
                magic.draw_magic_eleven(&vga_back, world.zoom_matrix->calc_zoom_x(origin_x, origin_y, z_init) - 42,
                                        world.zoom_matrix->calc_zoom_y(origin_x, origin_y, z_init) - 100,
                                        vga.pixel_format_flag, cur_frame);
            else
                magic.draw_magic_eleven(&vga_back, world.zoom_matrix->calc_zoom_x(origin_x, origin_y, z_init) - 21,
    ...
    else if (str == "MAGIC3")
    {
        ...
        {
            if (config.building_size == 1)
                magic.draw_magic_twelve(&vga_back, world.zoom_matrix->calc_zoom_x(origin_x, origin_y, z_init),
                                        world.zoom_matrix->calc_zoom_y(origin_x, origin_y, z_init),
    ...
    else if (str == "MAGIC5")
    {
        ...
        {
            if (config.building_size == 1)
                magic.draw_magic_thirteen(&vga_back, world.zoom_matrix->calc_zoom_x(origin_x, origin_y, z_init),
                                          world.zoom_matrix->calc_zoom_y(origin_x, origin_y, z_init) - 60,
                                          vga.pixel_format_flag, cur_frame);
            else
                magic.draw_magic_thirteen(&vga_back, world.zoom_matrix->calc_zoom_x(origin_x, origin_y, z_init) - 4,
