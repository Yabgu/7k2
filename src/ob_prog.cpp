...
//
void BulletProgram::init(char parentType, short parentRecno, short targetXLoc,
                         short targetYLoc, char targetMobileType) {
  Bullet::init(parentType, parentRecno, targetXLoc, targetYLoc,
               targetMobileType);
  cur_step = 0;
...
char BulletProgram::display_layer() { return 8; }
...
void BulletProgram::draw(int, int drawFlag) {
  ...
  if (str == "LIGHTING") {
    ...
    if (cur_action == SPRITE_MOVE) {
      magic.draw_magic_two(
          &vga_back,
          world.zoom_matrix->calc_zoom_x(target_x_loc * LOCATE_WIDTH,
                                         target_y_loc * LOCATE_HEIGHT, z_dest),
          world.zoom_matrix->calc_zoom_y(target_x_loc * LOCATE_WIDTH,
                                         target_y_loc * LOCATE_HEIGHT, z_dest) -
              400,
          world.zoom_matrix->calc_zoom_x(target_x_loc * LOCATE_WIDTH,
                                         target_y_loc * LOCATE_HEIGHT, z_dest),
      ...
      if (config.building_size == 1)
        magic.draw_magic_twelve_die(&vga_back,
                                    ...
                                    world.zoom_matrix->calc_zoom_y(
                                        ...
                                        target_y_loc * LOCATE_HEIGHT, z_dest),
                                    vga.pixel_format_flag, cur_frame,
                                    nation_recno, target_x_loc + target_y_loc);
      else
        magic.draw_magic_twelve_die(&vga_back,
                                    world.zoom_matrix->calc_zoom_x(
                                        target_x_loc * LOCATE_WIDTH,
  ...
  /*	else
          if ( str == "FIRMDIE" )
          {
                  // the following assume the target and shooter is a firm of
     same recno
                  // so lots of checking are missing
                  if ( cur_action == SPRITE_DIE &&
     !firm_array.is_deleted(parent_recno) )
                  {
                          Firm* firmPtr = firm_array[parent_recno];
                          if (firmPtr)
                                  magic.draw_magic_firm_die(&vga_back,
                                          world.zoom_matrix->calc_zoom_x(origin_x,
     origin_y, z_init), world.zoom_matrix->calc_zoom_y(origin_x, origin_y,
     z_init), vga.pixel_format_flag, cur_frame);
                  }
          //	else
          //		err_here();
          }*/
