    ...
    if (get_loc(x1, westWallFirst)->can_build_wall()) {
      ...
      if (x1 >= GATE_WIDTH - 1 && y2 - y1 >= GATE_LENGTH) {
        ...
        for (trial = y2 - y1 - GATE_LENGTH, westGateY = y1 + (trial + 1) / 2;
          ...
          if (can_build_area(x1 - GATE_WIDTH + 1, westGateY, x1,
                             westGateY + GATE_LENGTH - 1)) {
            //-------- put west gate --------- //
    ...
    if (get_loc(x2, eastWallFirst)->can_build_wall()) {
      ...
      if (x2 <= max_x_loc - GATE_WIDTH + 1 && y2 - y1 >= GATE_LENGTH) {
        ...
        for (trial = y2 - y1 - GATE_LENGTH, eastGateY = y1 + (trial + 1) / 2;
          ...
          if (can_build_area(x2, eastGateY, x2 + GATE_WIDTH - 1,
                             eastGateY + GATE_LENGTH - 1)) {
            //----------- put east gate ----------//
      ...
      if (y1 >= GATE_WIDTH - 1 && x2 - x1 >= GATE_LENGTH) {
        ...
        for (trial = x2 - x1 - GATE_LENGTH, northGateX = x1 + (trial + 1) / 2;
          ...
          if (can_build_area(northGateX, y1 - GATE_WIDTH + 1,
            ...
            //--------------- put north gate ----------------//
            build_north_gate(northGateX, y1 - GATE_WIDTH + 1, townRecno,
                             initHp);
            break;
          }
          increment = -increment;
    ...
    if ((locPtr = get_loc(southWallFirst, y2))->can_build_wall() ||
      ...
      if (y2 <= max_y_loc - GATE_WIDTH + 1 && x2 - x1 >= GATE_LENGTH) {
        ...
        for (trial = x2 - x1 - GATE_LENGTH, southGateX = x1 + (trial + 1) / 2;
          ...
          if (can_build_area(southGateX, y2, southGateX + GATE_LENGTH - 1,
                             y2 + GATE_WIDTH - 1)) {
            //----------- put south gate -------------//
...
void World::build_west_wall(short x1, short y1, short y2, short townRecno,
  ...
  while (startY <= y2) {
    ...
    {
      //-------- found segment end -----------//
      for (endY = startY + 1; endY <= y2 && get_loc(x1, endY)->can_build_wall();
           ++endY)
        ;
      --endY;
...
void World::build_north_wall(short x1, short x2, short y1, short townRecno,
  ...
  while (startX <= x2) {
    ...
    {
      //--------- found segment end ----------//
      for (endX = startX + 1; endX <= x2 && get_loc(endX, y1)->can_build_wall();
           ++endX)
        ;
      --endX;
...
void World::build_south_gate(short x1, short y1, short townRecno,
                             short initHp) {
  //---------- put two gate towers ----------//
  Location *locPtr = get_loc(x1, y1);
...
int World::form_wall(short x, short y, short maxRecur) {
  ...
  case 1:
    ...
    break;
  case -1:
    newWallTile = newWallRubble + TOWER_DES1 -
                  TOWER_CON1; // e.g. TOWER_CON1 -> TOWER_DES1
    break;
  case 2:
...
#define SPACING 12
  ...
  for (int trial = 7; trial > 0; --trial) {
    for (y = build_wall_y_seq[next_build_wall_seq]; y < max_y_loc;
         y += SPACING) {
      for (x = build_wall_x_seq[next_build_wall_seq]; x < max_x_loc;
           x += SPACING) {
        Location *locPtr = get_loc(x, y);
        // ######## begin Gilbert 7/3 ##########//
        if (locPtr->had_wall()) {
...
//
void World::build_wall_tile(int xLoc, int yLoc, short nationRecno,
                            char remoteAction) {
  ...
  if (can_build_wall(xLoc, yLoc, nationRecno)) {
    if (!remoteAction && remote.is_enable()) {
      ...
      short *shortPtr =
          (short *)remote.new_send_queue_msg(MSG_WALL_BUILD, 3 * sizeof(short));
...
// see also ZoomMatrix->draw_build_marker
//
void World::destruct_wall_tile(int xLoc, int yLoc, short nationRecno,
                               char remoteAction) {
  Location *locPtr = get_loc(xLoc, yLoc);

  if (locPtr->is_wall_creating() &&
      can_destruct_wall(xLoc, yLoc, nationRecno)) {
    if (!remoteAction && remote.is_enable()) {
      // packet structure : <nation recno> <xLoc> <yLoc>
      short *shortPtr = (short *)remote.new_send_queue_msg(MSG_WALL_DESTRUCT,
                                                           3 * sizeof(short));
      shortPtr[0] = nationRecno;
      shortPtr[1] = xLoc;
      shortPtr[2] = yLoc;
