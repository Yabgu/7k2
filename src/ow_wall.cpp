...
void World::build_wall_section(short x1, short y1, short x2, short y2, short townRecno, short initHp)
{
    //--------- build west wall -----------//
    Location *locPtr;
    short westWallFirst;
    for (westWallFirst = y1; westWallFirst < y2; ++westWallFirst)
    {
        if (get_loc(x1, westWallFirst)->can_build_wall())
        {
            ...
            if (x1 >= GATE_WIDTH - 1 && y2 - y1 >= GATE_LENGTH)
            {
                ...
                for (trial = y2 - y1 - GATE_LENGTH, westGateY = y1 + (trial + 1) / 2; trial > 0;
                     --trial, westGateY += increment)
                {
                    if (can_build_area(x1 - GATE_WIDTH + 1, westGateY, x1, westGateY + GATE_LENGTH - 1))
                    {
                        //-------- put west gate --------- //
    ...
    for (eastWallFirst = y1; eastWallFirst < y2; ++eastWallFirst)
    {
        if (get_loc(x2, eastWallFirst)->can_build_wall())
        {
            ...
            if (x2 <= max_x_loc - GATE_WIDTH + 1 && y2 - y1 >= GATE_LENGTH)
            {
                ...
                for (trial = y2 - y1 - GATE_LENGTH, eastGateY = y1 + (trial + 1) / 2; trial > 0;
                     --trial, eastGateY += increment)
                {
    ...
    for (northWallFirst = x1; northWallFirst < x2; ++northWallFirst)
    {
            ...
            (locPtr->is_wall() && locPtr->wall_town_recno() == townRecno))
        {
            ...
            if (y1 >= GATE_WIDTH - 1 && x2 - x1 >= GATE_LENGTH)
            {
                ...
                for (trial = x2 - x1 - GATE_LENGTH, northGateX = x1 + (trial + 1) / 2; trial > 0;
                     --trial, northGateX += increment)
                {
                    if (can_build_area(northGateX, y1 - GATE_WIDTH + 1, northGateX + GATE_LENGTH - 1, y1))
                    {
                        //--------------- put north gate ----------------//
    ...
    for (southWallFirst = x1; southWallFirst < x2; ++southWallFirst)
    {
            ...
            (locPtr->is_wall() && locPtr->wall_town_recno() == townRecno))
        {
            ...
            if (y2 <= max_y_loc - GATE_WIDTH + 1 && x2 - x1 >= GATE_LENGTH)
            {
                ...
                for (trial = x2 - x1 - GATE_LENGTH, southGateX = x1 + (trial + 1) / 2; trial > 0;
                     --trial, southGateX += increment)
                {
                    if (can_build_area(southGateX, y2, southGateX + GATE_LENGTH - 1, y2 + GATE_WIDTH - 1))
                    {
                        //----------- put south gate -------------//
...
void World::build_west_wall(short x1, short y1, short y2, short townRecno, short initHp)
{
...
void World::open_west_gate(short x2, short y1, short townRecno)
{
    ...
    if (locPtr->is_wall() && locPtr->wall_id() == WGATE_N && locPtr->wall_town_recno() == townRecno)
    {
...
void World::build_east_wall(short x1, short y1, short y2, short townRecno, short initHp)
{
    //--------- find segments of buildable terrain ---------//
    short startY = y1, endY = y1;
    while (startY <= y2)
...
void World::open_east_gate(short x1, short y1, short townRecno)
{
    ...
    if (locPtr->is_wall() && locPtr->wall_id() == EGATE_N && locPtr->wall_town_recno() == townRecno)
    {
        for (short y = 3; y < GATE_LENGTH - 3; ++y)
            for (short x = 0; x < GATE_WIDTH; ++x)
...
void World::build_north_gate(short x1, short y1, short townRecno, short initHp)
{
    //---------- put two gate towers -----------//
    Location *locPtr = get_loc(x1, y1);
...
void World::build_north_wall(short x1, short x2, short y1, short townRecno, short initHp)
{
    //---------- find segments of buildable terrain ---------//
...
void World::open_north_gate(short x1, short y2, short townRecno)
{
    ...
    if (locPtr->is_wall() && locPtr->wall_id() == NGATE_W && locPtr->wall_town_recno() == townRecno)
    {
...
void World::build_south_wall(short x1, short x2, short y1, short townRecno, short initHp)
{
    //----------- find segments of buildable terrain -------------//
    short startX = x1, endX = x1;
    while (startX <= x2)
...
void World::open_south_gate(short x1, short y1, short townRecno)
{
    ...
    if (locPtr->is_wall() && locPtr->wall_id() == SGATE_W && locPtr->wall_town_recno() == townRecno)
    {
        for (short x = 3; x < GATE_LENGTH - 3; ++x)
            for (short y = 0; y < GATE_WIDTH; ++y)
                get_loc(x1 + x, y1 + y)->remove_wall();
...
int World::form_wall(short x, short y, short maxRecur)
{
    ...
    case 5: // north and south
        ...
        if (y > 0 && get_loc(x, y - 1)->is_wall())
        {
            ...
            if (northWallTile != NSWALL && northWallTile != NSWALL_SHADOW && !is_wall_rubble(northWallTile))
    ...
    case 10: // west and east
        ...
        if (x > 0 && get_loc(x - 1, y)->is_wall())
        {
            ...
            if (westWallTile != EWWALL && westWallTile != EWWALL_SHADOW && !is_wall_rubble(westWallTile))
...
void World::build_wall_tile(int xLoc, int yLoc, short nationRecno, char remoteAction)
{
    ...
    if (can_build_wall(xLoc, yLoc, nationRecno))
    {
        ...
        else
        {
            ...
            locPtr->set_fire_src(-50);
            //			nation_array[nationRecno]->add_expense(
            //(float)BUILD_WALL_COST );
        }
    }
    else if (locPtr->is_wall_destructing() && can_destruct_wall(xLoc, yLoc, nationRecno))
    {
        if (!remoteAction && remote.is_enable())
        {
            // packet structure : <nation recno> <xLoc> <yLoc>
        ...
        else
        {
            ...
            //				BUILD_WALL_COST *
            //(100-locPtr->wall_abs_hit_point()) / 100 );
        }
    }
}
