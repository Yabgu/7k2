...
//							  if so, then only hero
// with
// for_hire == forHire 							  will be
// created. (default: -1, ignore for_hire) [int] historicHero - if this is
// given, the hero's historic_hero var
...
// unit, this flag decides whether randomly select
// or not (default: 0 not randomly select)
// (1 select infantry unit) (2 select special unit)
//
// return: <int> the recno of the unit created.
//
int HeroRes::create_hero(int xLoc, int yLoc, int nationRecno, int raceId, int heroPower, int unitLoyalty, int forHire,
...

{
    ...
    int unitRecno = create_powerful_unit(xLoc, yLoc, nationRecno, unitId, heroPower, unitLoyalty);
...
// return: <int> the recno of the unit created.
//
int HeroRes::create_powerful_unit(int xLoc, int yLoc, int nationRecno, int unitId, int heroPower, int unitLoyalty,
                                  int historicHero)
{
    ...
    if (heroPower > 35) // only add items if the hero power is larger than 35
    {
        ...
        unitPtr->item.init_random(MIN_RARITY + misc.random(itemRarity - MIN_RARITY) + 1, itemRarity, unitPtr->unit_id,
                                  &unitPtr->skill);
...
#define MAX_INDEPENDENT_HERO 20 // the game will only have a maximum of 20 heroes at a time
    ...
    if (humanNationCount > 0 &&
            misc.random(90 + yearPassed * 2) >= humanNationCount            // more human kingdom higher chance.
...
int HeroRes::generate_hero()
{
    ...
    if (world.locate_space(xLoc, yLoc, townPtr->loc_x2, townPtr->loc_y2, 1, 1, UNIT_LAND, townPtr->region_id))
    {
...
int HeroRes::induce_hero_join_nation(int nationRecno, int inducePower)
{
    ...
    for (int i = unit_array.size(); i > 0; i--)
    {
        ...
        if (nationPtr->reputation > 0)
            ...
            heroPower -= (int)nationPtr->reputation; // negative reputation results in higher heroPower,
...
//
int HeroRes::hire_cost(int combatLevel, int maxCombatLevel, int skillLevel, int spySkill, Item *itemPtr)
{
    long hireCost = 0;
    ...
    if (skillLevel > 0) // if this unit is one with leadership skill
    {
        ...
        for (int i = 10; i < skillLevel; i += 20) // increase the hiring cost with bigger steps when the skill
                                                  // level gets higher
            hireCost += i / 5;
    }
    ...
    if (spySkill > 0) // if this unit is a spy
    {
        ...
        for (int i = 10; i < spySkill; i += 20) // increase the hiring cost with bigger steps when the skill
                                                // level gets higher
            hireCost += i / 5;
    }
