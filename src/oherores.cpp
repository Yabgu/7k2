...
void HeroRes::load_hero_info() {
  ...
  for (i = 0; i < hero_count; i++) {
    ...
    heroInfo->historic_hero =
        1; // this is a historic hero as opposite to a fictional one
  }
}
...
void HeroRes::generate_fictional_hero() {
  // ##### begin Gilbert 14/3 ########//
  // ----- backup seed so all machine have same set of fictional hero (for
  // multi-player game) ----//
  ...
  for (int raceId = 1; raceId <= MAX_RACE; raceId++) {
    ...
    for (i = heroCount; i < HERO_COUNT_PER_RACE; i++) {
      ...
      // ##### begin Gilbert 1/3 #####//
      err_when(hero_count - oldHeroCount - 1 < 0 ||
               hero_count - oldHeroCount - 1 >= MAX_HERO);
      nameIdArray[hero_count - oldHeroCount - 1] = nameId;
      // ##### end Gilbert 1/3 #####//

      strncpy(heroInfo->name, race_res[raceId]->get_name(nameId),
              heroInfo->NAME_LEN);
  ...
  for (int j = oldHeroCount + 1; j <= hero_count; ++j) {
    ...
    if (heroInfo->race_id && nameIdArray[j - oldHeroCount - 1]) {
      race_res[heroInfo->race_id]->free_name_id(
          nameIdArray[j - oldHeroCount - 1]);
    }
  }
  // ######## end Gilbert 1/3 ######//
...
void HeroRes::second_init() {
  ...
  //--- if this is a campaign, and if any of the royal units are heroes, we
  //should set the appeared_flag of HeroRes ---//
...
HeroInfo *HeroRes::operator[](int heroId) {
  err_if(heroId < 1 || heroId > hero_count) err_now("HeroRes::operator[]");
...
//							  if so, then only hero with
//for_hire == forHire 							  will be created. 							  (default: -1, ignore for_hire)
// [int] historicHero - if this is given, the hero's historic_hero var
//								must be the same as the given
//one. (default: -1)
// [int] useReserve   - use the reserved hero, for campaign only. We need to
// reserve a hero for Stage 6 - Hero Debut
//							 	(default: 0)
//	[int] randomClass	 - if the hero can be both infantry or special
//unit, this flag decides whether randomly select 							 	or not (default: 0 not
//randomly select) 								(1 select infantry unit) 								(2 select special unit)
//
// return: <int> the recno of the unit created.
//
int HeroRes::create_hero(int xLoc, int yLoc, int nationRecno, int raceId,
                         int heroPower, int unitLoyalty, int forHire,
                         int historicHero, int useReserve, int randomClass)

{
  ...
  if (game.is_campaign_mode()) {
    if (useReserve) // use the reserved hero, for campaign only. We need to
                    // reserve a hero for Stage 6 - Hero Debut
      heroInfo =
          hero_res[filterHeroArray[filterHeroCount - 1]]; // use the last one
    else
      heroInfo = hero_res[filterHeroArray[misc.random(
          filterHeroCount - 1)]]; // -1 exclude the last one
  } else
...
// return: <int> the recno of the unit created.
//
int HeroRes::create_powerful_unit(int xLoc, int yLoc, int nationRecno,
                                  int unitId, int heroPower, int unitLoyalty,
                                  int historicHero) {
  ...
  int unitRecno = unit_array.add_unit(unitId, nationRecno, RANK_SOLDIER,
                                      unitLoyalty, xLoc, yLoc);
  ...
  if (heroPower > 35) // only add items if the hero power is larger than 35
  {
    ...
    unitPtr->item.init_random(MIN_RARITY +
                                  misc.random(itemRarity - MIN_RARITY) + 1,
...
#define MAX_INDEPENDENT_HERO                                                   \
  20 // the game will only have a maximum of 20 heroes at a time
  ...
  if (humanNationCount > 0 &&
          misc.random(90 + yearPassed * 2) >=
              humanNationCount // more human kingdom higher chance.
      || humanNationCount <= 0 &&
             misc.random(120 + yearPassed * 2) !=
                 0) // If no human kingdom, still a small chance
    return;
...
int HeroRes::generate_hero() {
  ...
  if (world.locate_space(xLoc, yLoc, townPtr->loc_x2, townPtr->loc_y2, 1, 1,
                         UNIT_LAND, townPtr->region_id)) {
...
int HeroRes::induce_hero_join_nation(int nationRecno, int inducePower) {
  ...
  for (int i = unit_array.size(); i > 0; i--) {
    ...
    if (nationPtr->reputation > 0)
      ...
      heroPower -=
          (int)nationPtr
              ->reputation; // negative reputation results in higher heroPower,
                            // which has a negative effect on getting the hero
...
//
int HeroRes::hire_cost(int combatLevel, int maxCombatLevel, int skillLevel,
                       int spySkill, Item *itemPtr) {
  long hireCost = 0;
  ...
  if (skillLevel > 0) // if this unit is one with leadership skill
  {
    ...
    for (int i = 10; i < skillLevel;
         i += 20) // increase the hiring cost with bigger steps when the skill
                  // level gets higher
      hireCost += i / 5;
  }
