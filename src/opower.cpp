...
void Power::mouse_handler()
{
    ...
    if (!enable_flag || win_opened || Box::opened_flag || sys.signal_exit_flag) // a window or box is opened upon the
                                                                                // current interface
    {
        return;
    }
    ...
    if (mouse.cur_x >= ZOOM_X1 && mouse.cur_x <= ZOOM_X2 && mouse.cur_y >= ZOOM_Y1 && mouse.cur_y <= ZOOM_Y2 &&
        (pointingLoc = test_detect(mouse.cur_x, mouse.cur_y)) != NULL)
    {
        ...
        mouse_cursor.set_icon(
            choose_cursor(mouse.cur_x, mouse.cur_y, selectedType, selectedRecno, pointingType, pointingRecno));
    }
    else
    {
        mouse_cursor.set_icon(
            choose_cursor(mouse.cur_x, mouse.cur_y, selectedType, selectedRecno, SCREEN_OBJECT_NONE, 0));
    }
    ...
    if (mouse.is_mouse_event())
    {
        if (mouse.mouse_event_type == LEFT_BUTTON &&
            (game.game_mode != GAME_TUTORIAL ||
             (game.game_mode == GAME_TUTORIAL &&
              tutor.allow_mouse_click(mouse.click_x(LEFT_BUTTON), mouse.click_y(LEFT_BUTTON), 1, 1))))
        {
            ...
            //// if the mouse is inside the zoom area
            /// mouseY >= MAP_Y1 && mouseY <=
            // MAP_Y2 )
            //			{
            //				int curXLoc;
            //				int curYLoc;
        ...
        //------- the selection action is complete --------//
                 ...
                 (game.game_mode != GAME_TUTORIAL ||
                  (game.game_mode == GAME_TUTORIAL &&
                   tutor.allow_mouse_click(mouse.release_x(LEFT_BUTTON), mouse.release_y(LEFT_BUTTON), 2, 1))))
        {
            int mouseX = mouse.click_x(LEFT_BUTTON);
            int mouseY = mouse.click_y(LEFT_BUTTON);
            ...
            if (mouse_cursor.frame_flag)
            {
                ...
                rc = detect_select(mouse_cursor.frame_x1, mouse_cursor.frame_y1, mouse_cursor.frame_x2,
                                   mouse_cursor.frame_y2, 0, mouse.event_skey_state & SHIFT_KEY_MASK);
            }
        }
    }
    else
...
int Power::detect_action()
{
    ...
    else if (world.map_matrix->get_detect_location(mouseX, mouseY, &curXLoc, &curYLoc,
...
Location *Power::test_detect(int scrnX, int scrnY, char *mobileType, int *retXLoc, int *retYLoc)
{
    ...
    {
        Unit *lockedUnit;
        if ((mouse.skey_state & CONTROL_KEY_MASK) && !unit_array.is_deleted(locked_unit_recno) &&
            (lockedUnit = unit_array[locked_unit_recno]) && lockedUnit->is_visible() && !lockedUnit->is_stealth())
        {
            // empty
    ...
    if (scrnX < world.zoom_matrix->image_x1 ||
        scrnX > world.zoom_matrix->image_x2 || // if the mouse is inside the zoom area
...
#define DETECT_SPREAD 4
        ...
        (detectSpread && world.zoom_matrix->get_close_location(scrnX, scrnY, &curXLoc, &curYLoc, &tempMobileType)))
    {
        ...
        for (; rowYLoc <= selYLoc2 && rowXLoc <= selXLoc2; rowYLoc < selYLoc2 ? ++rowYLoc : ++rowXLoc)
        {
            ...
            for (; xLoc <= selXLoc2 && yLoc >= selYLoc1; ++xLoc, --yLoc)
            {
                ...
                if (locPtr->unit_recno(UNIT_AIR) &&
                    !unit_array.is_deleted(locPtr->air_cargo_recno)) // if the mouse does not click
                ...
                if (unitPtr && !unitPtr->is_stealth())
                {
                    ...
                    if (absX >= unitPtr->abs_x1 && absY >= unitPtr->abs_y1 && absX <= unitPtr->abs_x2 &&
                        absY <= unitPtr->abs_y2)
                    {
                        int curZ = unitPtr->get_z();
                        int logAbsX = world.zoom_matrix->calc_abs_x(unitPtr->cur_x, unitPtr->cur_y, curZ);
                        int logAbsY =
                            world.zoom_matrix->calc_abs_y(unitPtr->cur_x, unitPtr->cur_y, curZ) +
        ...
        for (; rowYLoc <= selYLoc2 && rowXLoc <= selXLoc2; rowYLoc < selYLoc2 ? ++rowYLoc : ++rowXLoc)
        {
            ...
            for (; xLoc <= selXLoc2 && yLoc >= selYLoc1; ++xLoc, --yLoc)
            {
                ...
                if ((locPtr->unit_recno(UNIT_LAND) || locPtr->unit_recno(UNIT_SEA)) &&
                    !unit_array.is_deleted(locPtr->cargo_recno)) // if the mouse does not click directly on
                                                                 // a sprite
                {
        ...
        if (locPtr->is_town() && !town_array[locPtr->town_recno()]->is_stealth() ||
            ...
            locPtr->has_site() && !site_array[locPtr->site_recno()]->is_stealth() || locPtr->is_wall())
        {
...
int Power::detect_select(int selX1, int selY1, int selX2, int selY2, int recallGroup, int shiftSelect)
{
    ...
    int absSelX1 =
        selX1 - world.zoom_matrix->image_x1 + world.zoom_matrix->get_base_x(); // the mouse cursor's absolute position
    ...
    Location *testDetectLoc =
        selectOneOnly ? test_detect(selX2, selY2, &pMobileType, &testDetectXLoc, &testDetectYLoc) : NULL;
    int rc = 0;
    ...
    if (selectOneOnly)
    {
        ...
        if (locPtr->unit_recno(pMobileType) && !unit_array.is_deleted(locPtr->unit_recno(pMobileType)))
        {
            ...
            if (recallGroup)
            {
                ...
                {
                    // ###### begin Gilbert 1/2 ##### //
                    if (!shiftSelect || !unit_array.selected_recno) // if selecting firm/town... still
                                                                    // reset selection too
                                                                    // ###### end Gilbert 1/2 ##### //
                    ...
                    if (se_res.mark_select_object_time())
                    {
                        ...
                        se_res.sound(headUnit->cur_x_loc(), headUnit->cur_y_loc(), 1, 'S', headUnit->sprite_id, "SEL");
                    }
                }
            }
            else
            {
                // ###### begin Gilbert 1/2 ##### //
                if (!shiftSelect || !unit_array.selected_recno) // if selecting firm/town... still reset
                                                                // selection too
                                                                // ###### end Gilbert 1/2 ##### //
                    power.reset_selection();
                ...
                {
                    se_res.sound(unitPtr->cur_x_loc(), unitPtr->cur_y_loc(), 1, 'S', unitPtr->sprite_id, "SEL");
                }
            }
        }
        ...
        if (!recallGroup && pMobileType != UNIT_AIR)
        {
            ...
            if (locPtr->is_firm() && !firm_array.is_deleted(locPtr->firm_recno()))
            {
                ...
                if (se_res.mark_select_object_time())
                {
                    se_res.sound(firmPtr->center_x, firmPtr->center_y, 1, 'F', firmPtr->firm_id,
                                 firmPtr->under_construction ? (char *)"SELU" : (char *)"SEL");
                    se_res.sound(firmPtr->center_x, firmPtr->center_y, 1, 'B', firmPtr->firm_build_id,
                                 firmPtr->under_construction ? (char *)"SELU" : (char *)"SEL");
                }
            }
    ...
    else //------- if select more than one unit ---------//
    {
        ...
        {
            if (!unit_array.is_deleted(unitRecno) && (unitPtr = unit_array[unitRecno])->is_visible() &&
                unitPtr->is_own())
            {
                unitPtr->update_abs_pos();

                if (misc.is_touch(absSelX1, absSelY1, absSelX2, absSelY2, unitPtr->abs_x1, unitPtr->abs_y1,
                                  unitPtr->abs_x2, unitPtr->abs_y2))
                {
                    // ####### begin Gilbert 4/2 ##########//
                    if (!rc && (!shiftSelect || !unit_array.selected_recno ||
                                !unit_array[unit_array.selected_recno]->is_own())) // if selecting firm/town... still
                                                                                   // reset selection too
                    // ####### end Gilbert 4/2 ##########//
                    {
                        power.reset_selection();
...
int Power::execute_command(int mouseX, int mouseY)
{
    ...
    case COMMAND_SETTLE:
        ...
        if (locPtr->is_town() &&
            town_array[locPtr->town_recno()]->can_accept_assign(cmdUnit->sprite_recno, nation_array.player_recno))
        {
            unit_group.set_to_selected();
    ...
    case COMMAND_SET_CARAVAN_STOP: {
        ...
        if (locPtr->is_firm())
        {
            ...
            if (unitCaravan->can_set_stop(firmPtr->firm_recno))
            {
                ...
                unitCaravan->set_stop(command_para, curXLoc, curYLoc,
