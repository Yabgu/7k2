...
Power::Power() { memset(this, 0, sizeof(Power)); }
...
void Power::mouse_handler() {
  ...
  if (!enable_flag || win_opened || Box::opened_flag ||
      sys.signal_exit_flag) // a window or box is opened upon the current
                            // interface
  {
    return;
  }

  //---- if there is an object at where the mouse cursor is pointing at, change
  //the cursor shape ----//
  ...
  if (mouse.cur_x >= ZOOM_X1 && mouse.cur_x <= ZOOM_X2 &&
    ...
    short pointingRecno;
    ScreenObjectType pointingType =
        find_pointing_type(pointingLoc, &pointingRecno);

    mouse_cursor.set_icon(choose_cursor(mouse.cur_x, mouse.cur_y, selectedType,
                                        selectedRecno, pointingType,
                                        pointingRecno));
  } else {
    mouse_cursor.set_icon(choose_cursor(mouse.cur_x, mouse.cur_y, selectedType,
                                        selectedRecno, SCREEN_OBJECT_NONE, 0));
  }
  ...
  if (mouse.is_mouse_event()) {
    if (mouse.mouse_event_type == LEFT_BUTTON &&
        (game.game_mode != GAME_TUTORIAL ||
         (game.game_mode == GAME_TUTORIAL &&
          tutor.allow_mouse_click(mouse.click_x(LEFT_BUTTON),
                                  mouse.click_y(LEFT_BUTTON), 1, 1)))) {
      int mouseX = mouse.click_x(LEFT_BUTTON);
      int mouseY = mouse.click_y(LEFT_BUTTON);

      if (!mouse_cursor.frame_flag) {
        if (mouseX >= ZOOM_X1 &&
            mouseX <= ZOOM_X2 && // if the mouse is inside the zoom area
      ...
      //			if( mouseX >= MAP_X1 && mouseX <= MAP_X2 &&
      //// if the mouse is inside the zoom area 				 mouseY >= MAP_Y1 && mouseY <=
      //MAP_Y2 )
      //			{
      //				int curXLoc;
      //				int curYLoc;
      ...
      //
      //				if(
      //world.map_matrix->get_detect_location(mouseX, mouseY, &curXLoc,
      //&curYLoc, &curMobileType) )
      //				{
      //					mouse.set_boundary(MAP_X1,
      //MAP_Y1, MAP_X2, MAP_Y2, 1);		// rhombus boundary
      //				}
      //			}
    ...
    //------- the selection action is complete --------//
             ...
             (game.game_mode != GAME_TUTORIAL ||
              (game.game_mode == GAME_TUTORIAL &&
               tutor.allow_mouse_click(mouse.release_x(LEFT_BUTTON),
                                       mouse.release_y(LEFT_BUTTON), 2, 1)))) {
      int mouseX = mouse.click_x(LEFT_BUTTON);
      int mouseY = mouse.click_y(LEFT_BUTTON);
      ...
      if (mouse_cursor.frame_flag) {
        ...
        rc = detect_select(mouse_cursor.frame_x1, mouse_cursor.frame_y1,
                           mouse_cursor.frame_x2, mouse_cursor.frame_y2, 0,
...
int Power::detect_action() {
  if (!mouse.has_mouse_event || mouse.mouse_event_type != RIGHT_BUTTON ||
  ...
  if (game.game_mode == GAME_TUTORIAL &&
      !tutor.allow_mouse_click(mouseX, mouseY, 2, 2))
    return 0;
  ...
  if (mouseX >= ZOOM_X1 &&
      ...
      (test_detect(mouseX, mouseY, &curMobileType, &curXLoc, &curYLoc) ||
       world.zoom_matrix->get_detect_location(mouseX, mouseY, &curXLoc,
                                              &curYLoc, &curMobileType))) {
  }

  //---------- if click on the map window -----------//

  else if (world.map_matrix->get_detect_location(
               mouseX, mouseY, &curXLoc, &curYLoc,
               &curMobileType)) // if the mouse is inside the zoom area
  {
  }
  ...
  if (unit_array.selected_recno &&
    ...
    // ####### begin Gilbert 20/11 #######//
    unit_group.right_click(
        curXLoc, curYLoc, COMMAND_PLAYER,
        (mouse.event_skey_state & ALT_KEY_MASK) != 0, // alt to add way point
        (mouse.event_skey_state & SHIFT_KEY_MASK) !=
            0,                                // shift key to force attack
...
// test_detect keep on returning the location of the unit, even outside the
// screen unit ctrl key release or unit disappeared
//
//
Location *Power::test_detect(int scrnX, int scrnY, char *mobileType,
                             int *retXLoc, int *retYLoc) {
  ...
  if (locked_unit_recno) {
    ...
    if ((mouse.skey_state & CONTROL_KEY_MASK) &&
        ...
        lockedUnit->is_visible() && !lockedUnit->is_stealth()) {
      // empty
      *mobileType = lockedUnit->mobile_type;
      return world.get_loc((*retXLoc = lockedUnit->next_x_loc()),
                           (*retYLoc = lockedUnit->next_y_loc()));
    } else {
      locked_unit_recno = 0;
    }
  }
  ...
  if (scrnX < world.zoom_matrix->image_x1 ||
      scrnX >
          world.zoom_matrix->image_x2 || // if the mouse is inside the zoom area
      scrnY < world.zoom_matrix->image_y1 ||
      scrnY > world.zoom_matrix->image_y2) {
    return NULL;
  }
...
#define DETECT_SPREAD 4
  ...
  if (world.zoom_matrix->get_detect_location(scrnX, scrnY, &curXLoc, &curYLoc,
                                             &tempMobileType) ||
      (detectSpread &&
       world.zoom_matrix->get_close_location(scrnX, scrnY, &curXLoc, &curYLoc,
                                             &tempMobileType))) {
    int selXLoc1 =
        curXLoc - detectSpread; // expand 4 tiles in case of big sprite
    if (selXLoc1 < 0)
      selXLoc1 = 0;
    ...
    int xLoc, yLoc;
    int absX =
        scrnX - world.zoom_matrix->image_x1 + world.zoom_matrix->get_base_x();
    int absY =
        scrnY - world.zoom_matrix->image_y1 + world.zoom_matrix->get_base_y();
    ...
    for (; rowYLoc <= selYLoc2 && rowXLoc <= selXLoc2;
         rowYLoc < selYLoc2 ? ++rowYLoc : ++rowXLoc) {
      xLoc = rowXLoc;
      yLoc = rowYLoc;
      for (; xLoc <= selXLoc2 && yLoc >= selYLoc1; ++xLoc, --yLoc) {
        ...
        if (locPtr->unit_recno(UNIT_AIR) &&
            !unit_array.is_deleted(
                locPtr->air_cargo_recno)) // if the mouse does not click
        ...
        if (unitPtr && !unitPtr->is_stealth()) {
          ...
          if (absX >= unitPtr->abs_x1 && absY >= unitPtr->abs_y1 &&
            ...
            int logAbsX = world.zoom_matrix->calc_abs_x(unitPtr->cur_x,
                                                        unitPtr->cur_y, curZ);
            int logAbsY = world.zoom_matrix->calc_abs_y(unitPtr->cur_x,
                                                        unitPtr->cur_y, curZ) +
                          (unitPtr->loc_width * ZOOM_LOC_X_HEIGHT +
                           unitPtr->loc_height * ZOOM_LOC_Y_HEIGHT) /
                              2 +
            ...
            int fromTop = abs(absX - logAbsX) * 2 + abs(absY - logAbsY);
    ...
    for (; rowYLoc <= selYLoc2 && rowXLoc <= selXLoc2;
      ...
      for (; xLoc <= selXLoc2 && yLoc >= selYLoc1; ++xLoc, --yLoc) {
        ...
        if ((locPtr->unit_recno(UNIT_LAND) || locPtr->unit_recno(UNIT_SEA)) &&
            !unit_array.is_deleted(
                locPtr->cargo_recno)) // if the mouse does not click directly on
                                      // a sprite
        {
...
//						0 	- no unit or firm detected in the
//rectd area.
//
int Power::detect_select(int selX1, int selY1, int selX2, int selY2,
                         int recallGroup, int shiftSelect) {
  if (selX1 > selX2) {
    int temp = selX2;
    selX2 = selX1;
    selX1 = temp;
  ...
  int absSelX1 =
      selX1 - world.zoom_matrix->image_x1 +
      world.zoom_matrix->get_base_x(); // the mouse cursor's absolute position
                                       // on the whole world map
  int absSelY1 =
      selY1 - world.zoom_matrix->image_y1 + world.zoom_matrix->get_base_y();
  ...
  Location *testDetectLoc = selectOneOnly
                                ? test_detect(selX2, selY2, &pMobileType,
                                              &testDetectXLoc, &testDetectYLoc)
                                : NULL;
  int rc = 0;
  ...
  if (selectOneOnly) {
    ...
    if (locPtr->unit_recno(pMobileType) &&
        !unit_array.is_deleted(locPtr->unit_recno(pMobileType))) {
      ...
      if (recallGroup) {
        if (unitPtr->is_own() && unitPtr->in_team()) {
          // ###### begin Gilbert 1/2 ##### //
          if (!shiftSelect ||
              !unit_array.selected_recno) // if selecting firm/town... still
                                          // reset selection too
                                          // ###### end Gilbert 1/2 ##### //
        ...
        // ###### begin Gilbert 1/2 ##### //
        if (!shiftSelect ||
            !unit_array.selected_recno) // if selecting firm/town... still reset
                                        // selection too
                                        // ###### end Gilbert 1/2 ##### //
          power.reset_selection();
    ...
    if (!recallGroup && pMobileType != UNIT_AIR) {
      ...
      if (locPtr->is_firm() && !firm_array.is_deleted(locPtr->firm_recno())) {
        ...
        if (se_res.mark_select_object_time()) {
          se_res.sound(
              firmPtr->center_x, firmPtr->center_y, 1, 'F', firmPtr->firm_id,
              firmPtr->under_construction ? (char *)"SELU" : (char *)"SEL");
          se_res.sound(firmPtr->center_x, firmPtr->center_y, 1, 'B',
                       firmPtr->firm_build_id,
                       firmPtr->under_construction ? (char *)"SELU"
                                                   : (char *)"SEL");
        }
      }
      ...
      //--------- handle town selection -------//
               ...
               !town_array.is_deleted(locPtr->town_recno())) {
        ...
        rc = 1;
        if (se_res.mark_select_object_time()) {
          se_res.sound(townPtr->center_x, townPtr->center_y, 1, 'T', 0, "SEL");
        }
      }
    ...
    for (int unitRecno = unit_array.size(); unitRecno > 0; --unitRecno) {
      if (!unit_array.is_deleted(unitRecno) &&
        ...
        if (misc.is_touch(absSelX1, absSelY1, absSelX2, absSelY2,
                          unitPtr->abs_x1, unitPtr->abs_y1, unitPtr->abs_x2,
                          unitPtr->abs_y2)) {
          // ####### begin Gilbert 4/2 ##########//
          if (!rc && (!shiftSelect || !unit_array.selected_recno ||
                      !unit_array[unit_array.selected_recno]
                           ->is_own())) // if selecting firm/town... still reset
                                        // selection too
          // ####### end Gilbert 4/2 ##########//
          {
            power.reset_selection();
          ...
          } else if (se_res.mark_select_object_time() && !selectSound) {
            Unit *headUnit = unit_array[unit_array.selected_recno];
            se_res.sound(headUnit->cur_x_loc(), headUnit->cur_y_loc(), 1, 'S',
                         headUnit->sprite_id, "SEL");
            selectSound = 1;
          ...
          // unitPtr->left_click(shiftSelect);
          unitPtr->left_click(
              0); // do not pass shiftSelect, otherwise it will be toggle select
          // ##### end Gilbert 4/2 ######//
          rc = 1;
        }
      }
...
//
void Power::issue_command(int commandId, int commandUnitRecno,
                          int commandPara) {
...
int Power::execute_command(int mouseX, int mouseY) {
  ...
  if (!world.zoom_matrix->get_detect_location(mouseX, mouseY, &curXLoc,
                                              &curYLoc, &curMobileType))
    return 0;
  ...
  err_when(command_id >= COMMAND_UNIT_FIRST && command_id < COMMAND_UNIT_LAST &&
           !cmdUnit);
  err_when(command_id >= COMMAND_FIRM_FIRST && command_id < COMMAND_FIRM_LAST &&
           !cmdFirm);
  ...
  case COMMAND_SETTLE:
    ...
    //-- if there is an existing town on the targetted location, group assign
    //the selected units to it. Otherwise, order the selected unit to build a
    //town first. --//
    ...
    if (locPtr->is_town() &&
      ...
      unit_group.set_to_selected();
      unit_group.assign(curXLoc, curYLoc, UNIT_LAND, COMMAND_PLAYER,
                        nation_array.player_recno);
    } else {
  ...
  case COMMAND_SET_CARAVAN_STOP: {
    ...
    if (locPtr->is_firm()) {
      ...
      if (unitCaravan->can_set_stop(firmPtr->firm_recno)) {
        ...
        unitCaravan->set_stop(
            command_para, curXLoc, curYLoc,
            COMMAND_PLAYER); // command_para is the id. of the stop
      }
    }
  ...
  case COMMAND_USE_ITEM_ON_TOWN: {
    ...
    if ((targetBaseObjRecno =
             world.get_loc(curXLoc, curYLoc)->base_obj_recno()) &&
      ...
      if (itemPtr->id == command_para // make sure the same item
        ...
        if (!remote.is_enable()) {
          ...
          short *shortPtr = (short *)remote.new_send_queue_msg(
              MSG_UNIT_USE_ITEM, 3 * sizeof(short));
  ...
  case COMMAND_USE_ITEM_ON_SPACE: {
    Item *itemPtr = &cmdUnit->item;
    if (itemPtr->id == command_para // make sure the same item
        && itemPtr->can_use_manually() > 0 && locPtr->explored()) {
      if (!remote.is_enable()) {
  ...
  case COMMAND_MAGIC_TELEPORT_PRE:
    if (cmdFirm->cast_to_FirmMagic()) {
      ...
      if (locPtr->has_any_unit() &&
          (pointedUnitRecno = locPtr->get_any_unit(pointedMobileType)) &&
  ...
  case COMMAND_MAGIC_TELEPORT_POST:
    if (cmdFirm->cast_to_FirmMagic()) {
      cmdFirm->cast_to_FirmMagic()->cast_magic(
          command_para, curXLoc, curYLoc,
