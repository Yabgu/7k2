...
//
void Bullet::init(char parentType, short parentRecno, short targetXLoc,
  ...
  if (parentUnit) {
    ...
    AttackInfo *attackInfo =
        parentUnit->attack_info_array + parentUnit->cur_attack;
    ...
    /*	if (parentFirm->center_x == targetXLoc && parentFirm->center_y ==
       targetYLoc && (parentFirm->bullet_id() == 0 ||
       sprite_res[parentFirm->bullet_id()]->sprite_sub_type != 'M'))
            {
                    attack_damage  = 0;
                    damage_radius  = 1;
                    fire_radius    = 0;
  ...
  sprite_info
      ->load_bitmap_res(); // the sprite bitmap will be freed by ~Sprite(), so
                           // we don't have to add ~Bullet() to free it.
  ...
  if (parentUnit) {
    ...
    if (parentFirm->cast_to_FirmLishorr() ||
        parentFirm->cast_to_FirmOffensive2()) {
      set_dir(parentFirm->center_x, parentFirm->center_y, targetXLoc,
              targetYLoc);
      ...
      /*	if (byPassFlag == 1)
              {
                      origin_x = cur_x = (parentFirm->loc_x1 +
                              sys.frame_count %(parentFirm->loc_x2
         -parentFirm->loc_x1 +1)) *LOCATE_WIDTH; origin_y = cur_y =
         (parentFirm->loc_y1 + (sys.frame_count +
         parentFirm->firm_recno)%(parentFirm->loc_y2 -parentFirm->loc_y1 +1))
         *LOCATE_HEIGHT;
              }
              else*/
      {
  ...
  go_x = target_x_loc * LOCATE_WIDTH + LOCATE_WIDTH / 2 +
         ZoomMatrix::calc_cur_x(correctAbsX, correctAbsY);
  go_y = target_y_loc * LOCATE_HEIGHT + LOCATE_HEIGHT / 2 +
         ZoomMatrix::calc_cur_y(correctAbsX, correctAbsY);
  ...
  err_when(abs(correctAbsX -
               ZoomMatrix::calc_abs_x(correctCurX, correctCurY, 0)) > 1);
  err_when(abs(correctAbsY -
               ZoomMatrix::calc_abs_y(correctCurX, correctCurY, 0)) > 1);
#endif
  ...
  z_dest = world.interpolate_z(target_x_loc * LOCATE_WIDTH,
...
void Bullet::process_move() {
  ...
  if (++cur_step > total_step) {
    ...
    // if it has die frame, adjust cur_x, cur_y to be align with the
    // target_x_loc, target_y_loc
      ...
      if (fire_radius == 1) {
        ...
        for (y = y1; y <= y2; ++y) {
          ...
          for (x = x1; x <= x2; ++x, ++locPtr) {
            ...
            if (locPtr->fire_src() > 0)
              locPtr->set_fire_src(
                  1); // such that the fire will be put out quickly
          }
        }
      }
    }
...
void Bullet::hit_target(short x, short y) {
  ...
  attackDamage +=
      attackDamage *
      (4 - misc.abs_direction_diff(cur_dir & 7, targetUnit->cur_dir & 7)) / 8;
  // ######## end Gilbert 1/6 ##########//
  ...
  if (targetUnit->is_guarding()) {
    ...
    case SPRITE_MOVE:
      ...
      //			if( (targetUnit->cur_dir & 7)== ((cur_dir + 4 )
      //& 7)
      //				|| (targetUnit->cur_dir & 7)== ((cur_dir
      //+ 3 ) & 7)
      //				|| (targetUnit->cur_dir & 7)== ((cur_dir
      //+ 5 ) & 7) )
      if (misc.abs_direction_diff(targetUnit->cur_dir & 7, cur_dir & 7) >= 3) {
        // ###### begin Gilbert 22/10 ###### //
        // attackDamage = attackDamage > (float)10/ATTACK_SLOW_DOWN ?
        // attackDamage - (float)10/ATTACK_SLOW_DOWN : 0; do not divide by
        // ATTACK_SLOW_DOWN
        attackDamage -= 10.0f;
        // ###### end Gilbert 22/10 ###### //
        se_res.sound(targetUnit->cur_x_loc(), targetUnit->cur_y_loc(), 1, 'S',
  ...
  // if target is japanese SPU, reduce damage
  if (targetUnit->unit_id == UNIT_JAPANESE_SPU ||
...
void Bullet::hit_building(short x, short y) {
  ...
  if (locPtr->is_firm()) {
    ...
    if (!firmPtr ||
        !nation_array.should_attack(nation_recno, firmPtr->nation_recno))
      return;
    ...
    if (!townPtr ||
        !nation_array.should_attack(nation_recno, townPtr->nation_recno))
      return;
  ...
  //		targetObj->being_attack_hit(unit_array[parent_recno],
  //attackDamage 	else 		targetObj->being_attack_hit(NULL, attackDamage);

  targetObj->being_attack_hit(base_obj_array.is_deleted(parent_base_obj_recno)
                                  ? NULL
                                  : base_obj_array[parent_base_obj_recno],
                              attackDamage);
  // ##### end Gilbert 6/11 ######//
}
...
int Bullet::check_hit() {
  ...
  for (int c = 0; c < SCAN_RANGE * SCAN_RANGE; ++c) {
    ...
    if (x >= 0 && x < world.max_x_loc && y >= 0 && y < world.max_y_loc) {
      ...
      short *baseObjHitPtr;
      for (baseObjHitPtr = baseObjHit + baseObjHitCount - 1;
           baseObjHitPtr >= baseObjHit; --baseObjHitPtr) {
      ...
      if (baseObjHitPtr < baseObjHit) // not found
      {
        ...
        if (base_obj_array[baseObjRecno]->cast_to_Unit()) {
          hit_target(x, y); // note: no error checking here because mobile_type
                            // should be taken into account
        }
        // ### patch begin Gilbert 16/2 #######//
        else {
          if (c == 0) // only center attack building
            hit_building(x, y);
        }
        // ### patch end Gilbert 16/2 #######//
        // ####### begin Gilbert 3/6 ########//
        if (baseObjHitCount >= SCAN_RANGE * SCAN_RANGE)
          break; // hit too many objects that baseObjHit array cannot contain
                 // ####### end Gilbert 3/6 ########//
      }
    }
  }
      ...
      if (!unit_array.is_deleted(unitRecno)) {
        ...
        if (attenuated_damage(unitPtr) > 0) {
          if (unitPtr->is_special_ability_effective() == SPAB_REFLECT_ARROW &&
              attack_damage > 1.0f // reflect will decrease damage, so to avoid
                                   // infinte reflect
              && reflect(unitPtr->base_obj_recno)) {
            int effectId = sprite_res.search_sprite("REFLECT");
            if (effectId)
              Effect::create(effectId, cur_x, cur_y, SPRITE_DIE, cur_dir & 7, 2,
                             0);
            return 0;
          }
          ...
          case SPRITE_READY_TO_MOVE:
            if (unitPtr->can_stand_guard() && !unitPtr->is_guarding()) {
              unitPtr->set_dir((cur_dir + 4) &
                               7); // opposite direction of arrow
              unitPtr->set_guard_on();
            }
            break;
          case SPRITE_MOVE:
            if (unitPtr->can_move_guard() &&
                !unitPtr->is_guarding()
                //								&& (
                ...
                //									)
                //								)
                && misc.abs_direction_diff(unitPtr->cur_dir & 7, cur_dir & 7) >=
...
float Bullet::attenuated_damage(BaseObj *objPtr) {
  //	short d = misc.points_distance(curX, curY, target_x_loc * LOCATE_WIDTH,
  //target_y_loc * LOCATE_HEIGHT);
  short d;
  ...
  if (objPtr->cast_to_Unit()) {
    ...
    d = misc.area_distance(
        ...
        (unitPtr->loc_width - 1) * LOCATE_WIDTH +
            unitPtr->cur_x, // size of target is 1 loc smaller
        (unitPtr->loc_height - 1) * LOCATE_HEIGHT + unitPtr->cur_y,
        target_x_loc * LOCATE_WIDTH,
        target_y_loc * LOCATE_HEIGHT, // assume bullet is point size
    ...
    d = misc.area_distance(
        ...
        target_y_loc * LOCATE_HEIGHT, target_x_loc * LOCATE_WIDTH,
        target_y_loc * LOCATE_HEIGHT);
  }

  // damage drops from attack_damage to attack_damage/2, as range drops from 0
  // to damage_radius
