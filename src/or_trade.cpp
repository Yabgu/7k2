...
void Info::disp_trade(int refreshFlag) {
  ...
  vga_back.d3_panel_up(CARAVAN_BROWSE_X1, CARAVAN_BROWSE_Y1, CARAVAN_BROWSE_X2,
                       CARAVAN_BROWSE_Y1 + 20);
  ...
  font_bld.put(x + 340, y,
               text_reports.str_goods_carried()); //"Goods Carried" );

  if (refreshFlag == INFO_REPAINT) {
    // ###### begin Gilbert 19/10 #######//
    browse_caravan.init(CARAVAN_BROWSE_X1, CARAVAN_BROWSE_Y1 + 22,
                        CARAVAN_BROWSE_X2, CARAVAN_BROWSE_Y2 - 20, 0, 18,
                        report_array.size(), put_caravan_rec, 1);
    // ###### end Gilbert 19/10 #######//
  ...
  /*
          ...
          vga_back.d3_panel_up(SHIP_BROWSE_X1, SHIP_BROWSE_Y1, SHIP_BROWSE_X2,
  SHIP_BROWSE_Y1+20 );
          ...
          if( refreshFlag == INFO_REPAINT )
          {
                  browse_ship.init( SHIP_BROWSE_X1, SHIP_BROWSE_Y1+22,
  SHIP_BROWSE_X2, SHIP_BROWSE_Y2-20, 0, 16, report_array2.size(), put_ship_rec,
  1 );
...
void Info::detect_trade() {
  ...
  /*
          if( browse_ship.detect() )
          {
                  ...
                  {
                          Unit* unitPtr = unit_array[
     get_report_data2(browse_ship_recno) ];

                          world.go_loc(unitPtr->next_x_loc(),
     unitPtr->next_y_loc(), 1);
                  }
          }
  */
}
...
static void disp_total() {
  ...
  vga_back.d3_panel_up(CARAVAN_BROWSE_X1, CARAVAN_BROWSE_Y2 - 18,
                       CARAVAN_BROWSE_X2, CARAVAN_BROWSE_Y2);
  ...
  /*
          ...
          vga_back.d3_panel_up(SHIP_BROWSE_X1, SHIP_BROWSE_Y2-18,
     SHIP_BROWSE_X2, SHIP_BROWSE_Y2 );
  ...
  default:
    for (i = PICK_UP_PRODUCT_FIRST; i <= PICK_UP_PRODUCT_LAST; i++) {
      if (tradeStop->pick_up_array[i - 1]) {
        vga_back.put_bitmap(
            x, y, raw_res.small_product_icon(i - PICK_UP_PRODUCT_FIRST + 1));
        x += 12;
    ...
    for (i = PICK_UP_RAW_FIRST; i <= PICK_UP_RAW_LAST; i++) {
      if (tradeStop->pick_up_array[i - 1]) {
        vga_back.put_bitmap(x, y,
                            raw_res.small_raw_icon(i - PICK_UP_RAW_FIRST + 1));
        x += 12;
