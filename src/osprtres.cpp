...
void SpriteRes::load_sprite_info()
{
    ...
    short *first_dir_recno_array =
        (short *)mem_add(sizeof(short) * sprite_info_count); // allocate temporary arrays for temporary storage
    ...
    for (i = 0; i < sprite_info_count; i++)
    {
        ...
        spriteInfo->max_height = misc.atoi(spriteRec->max_height, spriteRec->SPRITE_MAX_HEIGHT_LEN);

        spriteInfo->max_height +=
            -spriteInfo->loc_width * LOCATE_WIDTH / 2 -
            (-spriteInfo->loc_width * ZOOM_LOC_X_WIDTH / 2 + -spriteInfo->loc_height * ZOOM_LOC_Y_WIDTH / 2);

        spriteInfo->max_bitmap_height = misc.atoi(spriteRec->max_bitmap_height, spriteRec->SPRITE_MAX_HEIGHT_LEN);
        ...
        spriteInfo->bullet_delay = misc.atoi(spriteRec->bullet_delay, spriteRec->DAMAGE_LEN);

        if (spriteRec->remap_bitmap_flag == '\0' || spriteRec->remap_bitmap_flag == ' ' ||
        ...
        for (j = 0, actionRecno = first_dir_recno_array[i]; j < dir_count_array[i]; j++, actionRecno++)
        {
            ...
            if (spriteActionRec->action[0] == 'M')
            {
                ...
                spriteMove->first_frame_recno =
                    misc.atoi(spriteActionRec->first_frame_recno, spriteActionRec->RECNO_LEN);
            ...
            else if (spriteActionRec->action[0] == 'A')
            {
                ...
                spriteAttack->first_frame_recno =
                    misc.atoi(spriteActionRec->first_frame_recno, spriteActionRec->RECNO_LEN);
            ...
            {
                spriteInfo->die.first_frame_recno =
                    misc.atoi(spriteActionRec->first_frame_recno, spriteActionRec->RECNO_LEN);
            ...
            else if (spriteActionRec->action[0] == 'G')
            {
                if (spriteActionRec->action[1] == 'M')
                {
                    ...
                    spriteGuardMove->first_frame_recno =
                        misc.atoi(spriteActionRec->first_frame_recno, spriteActionRec->RECNO_LEN);
                ...
                else
                {
                    ...
                    spriteGuardStop->first_frame_recno =
                        misc.atoi(spriteActionRec->first_frame_recno, spriteActionRec->RECNO_LEN);
...
int SpriteInfo::travel_days(int travelDistance)
{
    ...
    return travelFrames / FRAMES_PER_DAY * 110 / 100; // + 10% for circumstances that the units are blocked and needed
