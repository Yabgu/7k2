...
void SpriteRes::load_sprite_info() {
  ...
  short *first_dir_recno_array = (short *)mem_add(
      sizeof(short) *
      sprite_info_count); // allocate temporary arrays for temporary storage
  ...
  for (i = 0; i < sprite_info_count; i++) {
    ...
    misc.rtrim_fld(spriteInfo->sprite_code, spriteRec->sprite_code,
                   spriteRec->CODE_LEN);
    ...
    spriteInfo->turn_resolution =
        misc.atoi(spriteRec->turn_resolution, spriteRec->TURN_RES_LEN);
    spriteInfo->loc_width =
        misc.atoi(spriteRec->loc_width, spriteRec->SPRITE_PARA_LEN);
    spriteInfo->loc_height =
        misc.atoi(spriteRec->loc_height, spriteRec->SPRITE_PARA_LEN);
    spriteInfo->max_height =
        misc.atoi(spriteRec->max_height, spriteRec->SPRITE_MAX_HEIGHT_LEN);

    spriteInfo->max_height += -spriteInfo->loc_width * LOCATE_WIDTH / 2 -
                              (-spriteInfo->loc_width * ZOOM_LOC_X_WIDTH / 2 +
                               -spriteInfo->loc_height * ZOOM_LOC_Y_WIDTH / 2);

    spriteInfo->max_bitmap_height = misc.atoi(spriteRec->max_bitmap_height,
                                              spriteRec->SPRITE_MAX_HEIGHT_LEN);

    spriteInfo->speed = misc.atoi(spriteRec->speed, spriteRec->SPRITE_PARA_LEN);
    spriteInfo->max_speed =
        misc.atoi(spriteRec->speed, spriteRec->SPRITE_PARA_LEN);
    spriteInfo->frames_per_step =
        misc.atoi(spriteRec->frames_per_step, spriteRec->SPRITE_PARA_LEN);

    spriteInfo->max_rain_slowdown =
        misc.atoi(spriteRec->max_rain_slowdown, spriteRec->SPRITE_PARA_LEN);
    spriteInfo->max_snow_slowdown =
        misc.atoi(spriteRec->max_snow_slowdown, spriteRec->SPRITE_PARA_LEN);
    spriteInfo->lightning_damage =
        misc.atoi(spriteRec->lightning_damage, spriteRec->DAMAGE_LEN);

    spriteInfo->bullet_delay =
        misc.atoi(spriteRec->bullet_delay, spriteRec->DAMAGE_LEN);
    ...
    if (spriteRec->pal_file_name[0] == ' ' ||
        spriteRec->pal_file_name[0] == '\0') {
      spriteInfo->pal_ptr = NULL;
    } else {
      uint32_t paletteOffset;
      memcpy(&paletteOffset, spriteRec->pal_offset, sizeof(uint32_t));
      spriteInfo->pal_ptr = 8 + (BYTE *)res_pal.read_imported(
                                    paletteOffset); // skip 8 bytes header
    }
    spriteInfo->palw_ptr = NULL;

    first_dir_recno_array[i] =
        misc.atoi(spriteRec->first_move_recno, spriteRec->RECNO_LEN);
  ...
  for (i = 0; i < sprite_info_count; i++) {
    ...
    for (j = 0, actionRecno = first_dir_recno_array[i]; j < dir_count_array[i];
         j++, actionRecno++) {
      ...
      if (spriteActionRec->action[0] == 'M') {
        ...
        spriteMove->first_frame_recno = misc.atoi(
            spriteActionRec->first_frame_recno, spriteActionRec->RECNO_LEN);
        spriteMove->frame_count =
            misc.atoi(spriteActionRec->frame_count, spriteActionRec->COUNT_LEN);

        //--- the first movement frame is the default stop frame ---//

        if (spriteInfo->stop_array[dirId].frame_recno == 0) {
          spriteInfo->stop_array[dirId].frame_recno =
              spriteMove->first_frame_recno;
      ...
      //-------- attacking motion or weapon motion --------//
        ...
        err_when(spriteActionRec->action[1] < '1' ||
                 spriteActionRec->action[1] > '9');

        spriteAttack =
            spriteInfo->attack_array[spriteActionRec->action[1] - '1'] + dirId;

        spriteAttack->first_frame_recno = misc.atoi(
            spriteActionRec->first_frame_recno, spriteActionRec->RECNO_LEN);
        spriteAttack->frame_count =
            misc.atoi(spriteActionRec->frame_count, spriteActionRec->COUNT_LEN);
      }
      ...
      //--------- stop bitmap ---------//
        ...
        spriteInfo->stop_array[dirId].frame_recno = misc.atoi(
            spriteActionRec->first_frame_recno, spriteActionRec->RECNO_LEN);
        spriteInfo->stop_array[dirId].frame_count =
            misc.atoi(spriteActionRec->frame_count, spriteActionRec->COUNT_LEN);
      }
      ...
      //-------- dying motion ---------//
        ...
        spriteInfo->die.first_frame_recno = misc.atoi(
            spriteActionRec->first_frame_recno, spriteActionRec->RECNO_LEN);
        spriteInfo->die.frame_count =
            misc.atoi(spriteActionRec->frame_count, spriteActionRec->COUNT_LEN);
      }
      ...
      //--------- guarding motion --------//
        ...
        if (spriteActionRec->action[1] == 'M') {
          // moving guard
          SpriteGuardMove *spriteGuardMove =
              spriteInfo->guard_move_array + dirId;
          spriteGuardMove->first_frame_recno = misc.atoi(
              spriteActionRec->first_frame_recno, spriteActionRec->RECNO_LEN);
          spriteGuardMove->frame_count = misc.atoi(spriteActionRec->frame_count,
                                                   spriteActionRec->COUNT_LEN);

          // set can_guard_flag
          spriteInfo->can_guard_flag |= 2;
        } else {
          // standing guard
          SpriteGuardStop *spriteGuardStop =
              spriteInfo->guard_stop_array + dirId;
          spriteGuardStop->first_frame_recno = misc.atoi(
              spriteActionRec->first_frame_recno, spriteActionRec->RECNO_LEN);
          spriteGuardStop->frame_count = misc.atoi(spriteActionRec->frame_count,
                                                   spriteActionRec->COUNT_LEN);
...
void SpriteRes::load_sub_sprite_info() {
  ...
  sub_sprite_info_array =
      (SubSpriteInfo *)mem_add(sizeof(SubSpriteInfo) * subSpriteCount);
  ...
  for (int i = 0; i < subSpriteCount; i++) {
    ...
    subSpriteInfo->sprite_id =
        misc.atoi(subSpriteRec->sub_sprite_id, subSpriteRec->RECNO_LEN);
    err_when(subSpriteInfo->sprite_id > sprite_info_count);
    subSpriteInfo->sprite_info =
        sprite_info_array + subSpriteInfo->sprite_id - 1;
    subSpriteInfo->offset_x =
        misc.atoi(subSpriteRec->offset_x, subSpriteRec->OFFSET_LEN);
    subSpriteInfo->offset_y =
        misc.atoi(subSpriteRec->offset_y, subSpriteRec->OFFSET_LEN);
    ...
    SpriteInfo *parentSprite =
        sprite_res[misc.atoi(subSpriteRec->sprite_id, subSpriteRec->RECNO_LEN)];
...
int SpriteInfo::travel_days(int travelDistance) {
  ...
  return travelFrames / FRAMES_PER_DAY * 110 /
         100; // + 10% for circumstances that the units are blocked and needed
              // to wait and turning, etc.
}
