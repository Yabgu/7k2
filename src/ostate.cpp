...
//
StateArray::StateArray() { memset(this, 0, sizeof(StateArray)); }
//-------- end of function StateArray::StateArray ----------//

//-------- begin of function StateArray::~StateArray ----------//
//
StateArray::~StateArray() { deinit(); }
//-------- end of function StateArray::StateArray ----------//
...
void StateArray::generate(int stateCount) {
  ...
  do {
    ...
    for (s = 1; s <= state_count; ++s) {
      ...
      while (--trial >= 0) {
        ...
        // check separation
        for (int t = 1; t < s; ++t) {
          if (misc.diagonal_distance(
                  xLoc,
                  yLoc, operator[](t)->center_x, operator[](t)->center_y) *
                  2 <
              avgSeparation)
            closeFlag = 1;
        }
      ...
      state->strength_x1 = (max_x_loc + random(max_x_loc)) *
                           (max_y_loc + random(max_y_loc)) *
                           (max_y_loc + random(max_y_loc)) / 8;
      state->strength_y1 = (max_x_loc + random(max_x_loc)) *
                           (max_x_loc + random(max_x_loc)) *
                           (max_y_loc + random(max_y_loc)) / 8;

      //			state->strength_x2 =
      //(max_y_loc+random(max_y_loc)) * (max_y_loc+random(max_y_loc)) / 4;
      //			state->strength_y2 =
      //(max_x_loc+random(max_x_loc)) * (max_x_loc+random(max_x_loc)) / 4;
      //			state->strength_xy =
      //(max_x_loc+random(max_x_loc)) * (max_y_loc+random(max_y_loc)) / 4;
      state->strength_x2 = (max_y_loc + random(max_y_loc / 2)) *
                           (max_y_loc + random(max_y_loc / 2)) / 4;
      state->strength_y2 = (max_x_loc + random(max_x_loc / 2)) *
                           (max_x_loc + random(max_x_loc / 2)) / 4;
      state->strength_xy = (max_x_loc + random(max_x_loc / 2)) *
                           (max_y_loc + random(max_y_loc / 2)) /
                           8; // this coeff is smaller
    }
...

  {
    ...
    for (s = 1; s <= state_count; ++s) {
      ...
      if (centerX >= 0 && centerX < max_x_loc && centerY >= 0 &&
          centerY < max_y_loc && get_loc(centerX, centerY)->state_recno == s) {
...
int StateArray::is_adjacent_to_nation(int state1, int campaignNationRecno) {
  ...
  for (int i = size(); i > 0; i--) {
    ...
    if (stateInfo->is_adjacent(i) && operator[](i)->campaign_nation_recno ==
                                         campaignNationRecno) {
      return i;
    }
  }
...
double StateInfo::distance_func(float x, float y) {
  ...
  return (strength_x2 * dx + strength_x1) * dx +
         (strength_y2 * dy + strength_y1) * dy + strength_xy * dx * dy;
}
...
int StateInfo::is_near_border() {
  return west_bound_count + north_bound_count + east_bound_count +
         south_bound_count;
}
...
void StateArray::fill_state(int x, int y) {
  ...
  for (right = x + 1; right < max_x_loc && !get_loc(right, y)->state_recno;
       ++right) {
