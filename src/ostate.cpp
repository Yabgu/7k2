...
void StateArray::generate(int stateCount)
{
    ...
    do
    {
        ...
        for (s = 1; s <= state_count; ++s)
        {
            ...
            while (--trial >= 0)
            {
                ...
                for (int t = 1; t < s; ++t)
                {
                    if (misc.diagonal_distance(xLoc, yLoc, operator[](t)->center_x, operator[](t)->center_y) * 2 <
                        avgSeparation)
                        closeFlag = 1;
                }
                if (!closeFlag)
            ...
            state->strength_x1 =
                (max_x_loc + random(max_x_loc)) * (max_y_loc + random(max_y_loc)) * (max_y_loc + random(max_y_loc)) / 8;
            state->strength_y1 =
                (max_x_loc + random(max_x_loc)) * (max_x_loc + random(max_x_loc)) * (max_y_loc + random(max_y_loc)) / 8;
            ...
            state->strength_xy =
                (max_x_loc + random(max_x_loc / 2)) * (max_y_loc + random(max_y_loc / 2)) / 8; // this coeff is smaller
        }
...

    {
        ...
        for (s = 1; s <= state_count; ++s)
        {
            ...
            if (centerX >= 0 && centerX < max_x_loc && centerY >= 0 && centerY < max_y_loc &&
...
int StateArray::is_adjacent_to_nation(int state1, int campaignNationRecno)
{
    ...
    for (int i = size(); i > 0; i--)
    {
        ...
        if (stateInfo->is_adjacent(i) && operator[](i)->campaign_nation_recno == campaignNationRecno)
        {
            return i;
        }
    }
...
int StateInfo::add_game_nation()
{
    ...
    int nationRecno = nation_array.new_nation(cNation->is_human() ? NATION_HUMAN : NATION_MONSTER,
...
double StateInfo::distance_func(float x, float y)
{
    ...
    return (strength_x2 * dx + strength_x1) * dx + (strength_y2 * dy + strength_y1) * dy + strength_xy * dx * dy;
}
...
int StateArray::are_nation_adjacent(int cNation1, int cNation2)
{
    for (int i = size(); i > 0; i--)
    {
        ...
        if (stateInfo->campaign_nation_recno == cNation1 && is_adjacent_to_nation(i, cNation2))
        {
            return 1;
        }
    }
