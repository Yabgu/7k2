...
// <int>	raceId			 -	the race the selected unit
// should have
//										(0 for
// any
// races)
...
// <char&>	resultFlag		-	describle how to find the
// skilled unit
//										0 - for unable
// to
// train unit, 										1 - for
// existing skilled unit 2 - for unit hired from inn
// 3 - for training unit in town (training is required)
//
// [int]	   actionId - the action id. of the action which
//							  the unit should do after it
// has
// finished training.
//
...
Unit *Nation::ai_find_unit(int isCivilian, int raceId, short destX, short destY, char &resultFlag, int actionId)
{
    ...
    else
    {
        ...
        if (!isCivilian) // if the wanted unit is a military, we can try to hire it
                         // from the inn
        {
            unitRecno = hire_unit(raceId, isCivilian, false, destX,
...
Unit *Nation::ai_find_unit_from_camp(int raceId, short destX, short destY)
{
    ...
    for (int i = 0; i < ai_camp_count; i++)
    {
        ...
        {
            if (!firmCamp->soldier_array[j].is_under_training() && firmCamp->soldier_array[j].race_id == raceId)
            {
                break;
            }
        }
...
int Nation::hire_unit(int raceId, int isCivilian, int hireSpy, short destX, short destY)
{
    ...
    for (i = 0; i < ai_inn_count; i++)
    {
        ...
        for (j = innUnitCount; j > 0; j--, innUnit--)
        {
            ...
            curRating = (int)innUnit->skill_level() - (100 - 100 * curFirmDist / MAX_WORLD_X_LOC);
...
//								  (if not given, the
// majority
// race of the destination 								   firm will
// be used.)
//
// return: <int> recno of the unit recruited.
//
int Nation::recruit_in_firm_soldier(Firm *destFirmPtr, int preferedRaceId)
{
    ...
    for (i = 0; i < ai_camp_count; i++)
    {
        ...
        curRating = excessCombatLevel / 3 + world.distance_rating(firmCamp->center_x, firmCamp->center_y,
