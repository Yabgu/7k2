...
// <int>	raceId			 -	the race the selected unit should
// have
//										(0 for any
//races)
// <ActionNode*> actionNode - the ActionNode of the action that needs this
// skilled unit.
//
...
Unit *Nation::ai_get_unit(int isCivilian, int raceId, ActionNode *actionNode) {
  ...
  if (actionNode->unit_recno) // a unit has started training previously
  {
    ...
    //---- for harbor, we have to get the land region id. instead of the sea
    //region id. ----//
    /*
                             ...
                             actionNode->action_para==FIRM_HARBOR )
                    {
                            ...
                            for( yLoc=actionNode->action_y_loc ;
       yLoc<actionNode->action_y_loc+3 ; yLoc++ )
                            {
                                    for( xLoc=actionNode->action_x_loc ;
       xLoc<actionNode->action_x_loc+3 ; xLoc++ )
                                    {
                                            if( region_array[
       world.get_region_id(xLoc,yLoc) ]->region_type == REGION_LAND )
                                            {
                                                    rc=1;
                                                    break;
                                            }
    ...
    */
    ...
    wantedUnit = ai_find_unit(isCivilian, raceId, xLoc, yLoc, resultFlag,
  ...
  if (!wantedUnit->is_visible()) {
    //		actionNode->next_retry_date = info.game_date + TOTAL_TRAIN_DAYS +
    //1;
    return NULL; // continue processing this action after this date, this is
                 // used when training a unit for construction
  }
...
//
// <int>	isCivilian		-	whether the unit is a civilian
// unit <int>		raceId			-	the race the selected
// unit should have
//										(0 for any
//races)
// <short>	destX, destY	-	location the unit move to
// <char&>	resultFlag		-	describle how to find the skilled
// unit
//										0 - for unable to
//train unit, 										1 - for existing skilled unit 										2 - for unit hired from inn 										3 - for
//training unit in town (training is required)
//
// [int]	   actionId - the action id. of the action which
//							  the unit should do after it has
//finished training.
//
// return the unit pointer pointed to the skilled unit
//
Unit *Nation::ai_find_unit(int isCivilian, int raceId, short destX, short destY,
                           char &resultFlag, int actionId) {
  ...
  for (int i = unit_array.size(); i > 0; i--) {
    ...
    if (unitPtr->is_visible()) {
      ...
      if (unitPtr->cur_action != SPRITE_ATTACK &&
          !unitPtr->cur_order.ai_action_id) {
        curDist = misc.points_distance(unitPtr->next_x_loc(),
                                       unitPtr->next_y_loc(), destX, destY);
  ...
  if (wantedUnit) {
    ...
    if (!isCivilian) // if the wanted unit is a military, we can try to hire it
                     // from the inn
    {
      unitRecno = hire_unit(
          raceId, isCivilian, false, destX,
          destY); // this function will try going with hiring units that are
                  // better than training your own ones. false-don't hire a spy,
                  // just hire a normal military unit
...
int Nation::ai_should_hire_unit(int importanceRating) {
  ...
  return ai_should_spend(importanceRating + pref_hire_unit / 5 -
                         10); // -10 to +10 depending on pref_hire_unit
}
...
Unit *Nation::ai_find_unit_from_camp(int raceId, short destX, short destY) {
  ...
  for (int i = 0; i < ai_camp_count; i++) {
    ...
    if (j >= 0) {
      curRating = world.distance_rating(destX, destY, firmCamp->center_x,
                                        firmCamp->center_y);
...
//
int Nation::hire_unit(int raceId, int isCivilian, int hireSpy, short destX,
                      short destY) {
  ...
  for (i = 0; i < ai_inn_count; i++) {
    ...
    curFirmDist = misc.points_distance(firmPtr->center_x, firmPtr->center_y,
                                       destX, destY);
...
//	<short>	destY	  - the y location the unit will move to
//
// <int&>   recruitTownRecno - the recno of the town where this unit is
// recruited.
//
// return: <int> recno of the unit recruited.
//
int Nation::recruit_peasant(int raceId, short destX, short destY,
                            int &recruitTownRecno) {
  ...
  for (i = 0; i < ai_town_count; i++) {
    ...
    if (!townPtr->jobless_population || // no jobless population or currently a
                                        // unit is being trained
    ...
    curDist = misc.points_distance(townPtr->center_x, townPtr->center_y, destX,
                                   destY);
...
//								  (if not given, the majority
//race of the destination 								   firm will be used.)
//
// return: <int> recno of the unit recruited.
//
int Nation::recruit_in_firm_soldier(Firm *destFirmPtr, int preferedRaceId) {
  err_when(destFirmPtr->cast_to_FirmCamp() ==
           NULL); // it must either be a Camp or a Fort
  ...
  for (i = 0; i < ai_camp_count; i++) {
    ...
    int excessCombatLevel =
        firmCamp->total_combat_level() - firmCamp->ai_combat_level_needed();
