...
int Nation::total_alliance_military() {
  ...
  for (int i = nation_array.size(); i > 0; i--) {
    ...
    case RELATION_ALLIANCE:
      ...
      //				totalPower +=
      //nation_array[i]->military_rank_rating() / 2;	//50% 				break;
    }
  }
...
int Nation::total_enemy_military() {
  ...
  for (int i = nation_array.size(); i > 0; i--) {
    ...
    /*
       ...
       j==nation_recno ) continue;

                                    if( get_relation_status(j) !=
       RELATION_HOSTILE )   // only if this is one of our enemies. continue;

                                    //--- check if it is allied or friendly to
       any of our enemies ---//

                                    relationStatus2 =
       nationPtr->get_relation_status(j);

                                    if( relationStatus2 > enemyRelationStatus )
       // Friendly will replace none, Alliance will replace Friendly
                                            enemyRelationStatus =
       relationStatus2;
                            }

                            if( enemyRelationStatus == RELATION_ALLIANCE )
                                    totalPower +=
       nationPtr->military_rank_rating() * 3 / 4;	 // 75%

                            else if( enemyRelationStatus == RELATION_FRIENDLY )
                                    totalPower +=
       nationPtr->military_rank_rating() / 2;		 // 50%
                    }
    */
  }
...
int Nation::total_enemy_count() {
  ...
  for (int i = nation_array.size(); i > 0; i--) {
    ...
    if (get_relation_status(i) == RELATION_HOSTILE)
      totalEnemy++;
  }
...
void Nation::think_deal_with_one_enemy(int enemyNationRecno) {
  ...
  for (int i = 1; i <= nation_array.size(); i++) {
    ...
    if (nationPtr->is_human() !=
        is_human()) // only ally with nation of the same type against the enemy
      continue;

    nationRelation = nationPtr->get_relation(nation_recno);

    //--- if this nation is already allied to us, request it to declare war with
    //the enemy ---//

    if (nationRelation->status == RELATION_ALLIANCE &&
        nationPtr->get_relation_status(enemyNationRecno) != RELATION_HOSTILE) {
      if (should_diplomacy_retry(TALK_REQUEST_DECLARE_WAR, i)) {
        talk_res.ai_send_talk_msg(i, nation_recno, TALK_REQUEST_DECLARE_WAR,
                                  enemyNationRecno);
        continue;
      }
    }
    ...
    if (nationPtr->get_relation_status(enemyNationRecno) < RELATION_FRIENDLY) {
      //--- and this nation is neutral or friendly with us ---//

      if (nationRelation->status >= RELATION_NEUTRAL &&
        ...
        if (should_diplomacy_retry(TALK_REQUEST_TRADE_EMBARGO, i))
          talk_res.ai_send_talk_msg(i, nation_recno, TALK_REQUEST_TRADE_EMBARGO,
                                    enemyNationRecno);
      }
      ...
      if (nationRelation->status != RELATION_HOSTILE) {
        ...
        if (!nationRelation->trade_treaty) {
          if (should_diplomacy_retry(TALK_PROPOSE_TRADE_TREATY, i))
            talk_res.ai_send_talk_msg(i, nation_recno,
                                      TALK_PROPOSE_TRADE_TREATY);
        } else //--- if we already have a trade treaty with this nation ---//
        {
          // if this nation is already friendly to us, propose an alliance
          // treaty now --//
          ...
          //-- if the nation has significiant trade with us, propose a friendly
          //treaty now --//

          else if (nationPtr->trade_rating(nation_recno) > 10 ||
                   nationPtr->ai_trade_with_rating(nation_recno) >=
                       50) // or if the product complement each other very well
          {
...
int Nation::think_surrender() {
  ...
  Nation *nationPtr;
  int curRating, bestRating = (70 - pref_surrender), bestNationRecno = 0;

  if (!king_unit_recno) // if there is no successor to the king, the nation will
                        // tend more to surrender
    bestRating = -100;
  ...
  for (int i = nation_array.size(); i > 0; i--) {
    ...
    if (nationPtr->cash <=
        300) // don't surrender to an ecnomically handicapped nation
      continue;

    if (nationPtr->is_human() !=
        is_human()) // never surrender to different nationalty / Fryhtan species
      continue;

    if (get_relation(i)->ai_never_consider[TALK_SURRENDER - 1])
      continue;

    curRating = ai_surrender_to_rating(i);

    //--- if the nation will tend to surrender if there is only a small number
    //of units left ---//
...
int Nation::think_unite_against_big_enemy() {
  if (info.game_date - info.game_start_date <
      365 * 3 * (100 + pref_military_development) /
          100) // only do this after 3 to 6 years into the game
  {
    return 0;
  }
  ...
  //---- find the overall rank rating of the second most powerful computer
  //kingdom ---//
  ...
  if (get_relation_status(secondBestNationRecno) < RELATION_NEUTRAL)
    return 0;

  //--- if all AI kingdoms are way behind the human players, unite to against
  //the human player ---//
...
int Nation::think_eliminate_enemy_town(int enemyNationRecno) {
  ...
  for (int i = town_array.size(); i > 0; i--) {
    ...
    if (is_human() && base_town_count_in_region(townPtr->region_id) == 0)
      continue;

    //----- take into account of the mobile units around this town -----//

    int mobileCombatLevel = mobile_defense_combat_level(
        townPtr->center_x, townPtr->center_y, townPtr->nation_recno, 1, hasWar);

    if (mobileCombatLevel ==
        -1) // do not attack this town because a battle is already going on
      continue;
    ...
    return ai_attack_target(townPtr->loc_x1, townPtr->loc_y1,
                            mobileCombatLevel + townCombatLevel);
  }
...
int Nation::think_eliminate_enemy_firm(int enemyNationRecno) {
  ...
  for (int i = firm_array.size(); i > 0; i--) {
    ...
    if (is_human() && base_town_count_in_region(firmPtr->region_id) == 0)
      continue;

    //----- take into account of the mobile units around this town -----//

    int mobileCombatLevel = mobile_defense_combat_level(
        firmPtr->center_x, firmPtr->center_y, firmPtr->nation_recno, 1, hasWar);
    ...
    //other civilian firms 			firmCombatLevel =
    //((FirmCamp*)firmPtr)->total_combat_level(); 		else 			firmCombatLevel =
    //firmPtr->worker_count * 10;		// civilian firms have very low combat
    //level
    if (firmPtr->cast_to_FirmCamp()) // other civilian firms
      firmCombatLevel = firmPtr->cast_to_FirmCamp()->total_combat_level();
    else
      firmCombatLevel = 0; // civilian firms cannot mobilize to fight

    return ai_attack_target(firmPtr->loc_x1, firmPtr->loc_y1,
                            mobileCombatLevel + firmCombatLevel);
  }
...
int Nation::think_eliminate_enemy_unit(int enemyNationRecno) {
  ...
  for (int i = unit_array.size(); i > 0; i--) {
    ...
    if (!unitPtr->is_visible() ||
        unitPtr->mobile_type != UNIT_LAND) // only deal with land units now
      continue;

    //--- only attack if we have any base town in the enemy unit's region ---//

    if (is_human() && base_town_count_in_region(unitPtr->region_id()) == 0)
      continue;

    //----- take into account of the mobile units around this town -----//

    int mobileCombatLevel = mobile_defense_combat_level(
        unitPtr->next_x_loc(), unitPtr->next_y_loc(), unitPtr->nation_recno, 1,
        hasWar);
    ...
    return ai_attack_target(unitPtr->next_x_loc(), unitPtr->next_y_loc(),
                            mobileCombatLevel + (int)unitPtr->obj_power());
  }
...
int Nation::think_ally_against_big_enemy() {
  if (info.game_date <
      info.game_start_date + 365 +
          nation_recno *
              70) // don't ask for tribute too soon, as in the beginning, the
                  // ranking are all the same for all nations
    return 0;
  ...
  //-- if AI aggressiveness > high, only deal against the player, but not other
  //kingdoms ---//
  ...
  //--- increase the ai_relation_level towards other nations except the enemy so
  //we can ally against the enemy ---//
  ...
  for (i = nation_array.size(); i > 0; i--) {
    ...
    int thisIncLevel =
        incRelationLevel * (100 - get_relation(i)->ai_relation_level) / 100;
  ...
  if (trade_rating(enemyNationRecno) + ai_trade_with_rating(enemyNationRecno) >
      100 - pref_trading_tendency / 3) {
    return 0;
  }
  ...
  if (nationRelation->ai_relation_level > 30) {
    ...
    if (should_diplomacy_retry(talkId, enemyNationRecno)) {
      ...
      talk_res.ai_send_talk_msg(enemyNationRecno, nation_recno, talkId,
                                aidAmount);
    }
...
//
int Nation::ai_should_attack_friendly(int friendlyNationRecno,
                                      int attackTemptation) {
  ...
  if (info.game_date < nationRelation->last_change_status_date + 60 +
                           pref_honesty / 2) // only after 60 to 110 days
    return 0;

  //------------------------------------------------//

  int resistanceRating =
      friendlyNation->military_rank_rating() - military_rank_rating();
...
void Nation::ai_end_treaty(int nationRecno) {
  ...
  if (nationRelation->status == RELATION_FRIENDLY) {
    talk_res.ai_send_talk_msg(nationRecno, nation_recno,
                              TALK_END_FRIENDLY_TREATY, 0, 0,
                              1); // 1-force send
  } else if (nationRelation->status == RELATION_ALLIANCE) {
    talk_res.ai_send_talk_msg(nationRecno, nation_recno,
                              TALK_END_ALLIANCE_TREATY, 0, 0, 1);
  }

  err_when(nationRelation->status >=
           RELATION_FRIENDLY); // when the status is still friendly or alliance
}
...
int Nation::ai_has_enough_food() {
  //----- if the food change is negative ----//

  if (yearly_food_change() < 0 &&
      100 * -yearly_food_change() / food > 50 - pref_food_reserve / 3) {
    return 0;
  }
...
//----- Begin of function Nation::not_enough_food_index -----//
//
// Return a value from 0 to 100 indicating how serious the lack of food is.
//
int Nation::not_enough_food_index() {
  if (ai_has_enough_food()) // if the AI has enough food.
    return 0;

  err_when(yearly_food_change() >
           0); // ai_has_enough_food() should return 1 if food change is > 0
...
int Nation::think_attack_enemy_firm(int enemyNationRecno, int firmId) {
  ...
  for (int i = firm_array.size(); i > 0; i--) {
    ...
    curRating = world.distance_rating(firmPtr->center_x, firmPtr->center_y,
                                      ourLargestTown->center_x,
                                      ourLargestTown->center_y);
...
int Nation::think_against_mine_monopoly() {
  ...
  if (config.ai_aggressiveness <
      OPTION_HIGH) // only attack if aggressiveness >= high
    return 0;
  ...
  //-- for high aggressiveness, it will check cash before attack, for very high
  //aggressiveness, it won't check cash before attack ---//

  if (config.ai_aggressiveness <
      OPTION_VERY_HIGH) // only attack if aggressiveness >= high
  {
    if (cash > 2000 + 1000 * pref_cash_reserve /
                          100) // only attack if we run short of cash
      return 0;
  }
  ...
  for (i = nation_array.size(); i > 0; i--) {
    ...
    curRating =
        mineCount * 100 - get_relation(i)->ai_relation_level - trade_rating(i);
  ...
  //--- if the relationship with this nation is still good, don't attack yet,
  //ask for aid first ---//
  ...
  if (nationRelation->ai_relation_level > 30) {
    ...
    if (should_diplomacy_retry(talkId, targetNationRecno)) {
      ...
      talk_res.ai_send_talk_msg(targetNationRecno, nation_recno, talkId,
                                aidAmount);
    }
  ...
  for (i = firm_array.size(); i > 0; i--) {
    ...
    return ai_attack_target(firmPtr->loc_x1, firmPtr->loc_y1, targetCombatLevel,
                            0, 0, 0, 0, 1); // 1-use all camps
  }
