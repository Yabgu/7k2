...
int Nation::total_alliance_military()
{
    ...
    for (int i = nation_array.size(); i > 0; i--)
    {
        ...
        case RELATION_ALLIANCE:
            ...
            //				totalPower +=
            // nation_array[i]->military_rank_rating() / 2;	//50%
            // break;
        }
    }
...
void Nation::think_deal_with_one_enemy(int enemyNationRecno)
{
    ...
    for (int i = 1; i <= nation_array.size(); i++)
    {
        ...
        if (nationPtr->is_human() != is_human()) // only ally with nation of the same type against the enemy
            continue;
        ...
        if (nationPtr->get_relation_status(enemyNationRecno) < RELATION_FRIENDLY)
        {
            ...
            if (nationRelation->status >= RELATION_NEUTRAL && nationPtr->get_relation(enemyNationRecno)->trade_treaty)
            {
...
int Nation::think_surrender()
{
    ...
    for (int i = nation_array.size(); i > 0; i--)
    {
        ...
        if (nationPtr->is_human() != is_human()) // never surrender to different nationalty / Fryhtan species
            continue;
...
int Nation::think_eliminate_enemy_town(int enemyNationRecno)
{
    ...
    for (int i = town_array.size(); i > 0; i--)
    {
        ...
        int mobileCombatLevel =
            mobile_defense_combat_level(townPtr->center_x, townPtr->center_y, townPtr->nation_recno, 1, hasWar);
...
int Nation::think_eliminate_enemy_firm(int enemyNationRecno)
{
    ...
    for (int i = firm_array.size(); i > 0; i--)
    {
        ...
        int mobileCombatLevel =
            mobile_defense_combat_level(firmPtr->center_x, firmPtr->center_y, firmPtr->nation_recno, 1, hasWar);
        ...
        //=
        // firmPtr->worker_count * 10;		// civilian firms have very low
        // combat level
...
int Nation::think_eliminate_enemy_unit(int enemyNationRecno)
{
    ...
    for (int i = unit_array.size(); i > 0; i--)
    {
        ...
        int mobileCombatLevel =
            mobile_defense_combat_level(unitPtr->next_x_loc(), unitPtr->next_y_loc(), unitPtr->nation_recno, 1, hasWar);
...
{
    if (info.game_date <
        info.game_start_date + 365 + nation_recno * 70) // don't ask for tribute too soon, as in the beginning, the
...
void Nation::ai_end_treaty(int nationRecno)
{
    ...
    if (nationRelation->status == RELATION_FRIENDLY)
    {
        talk_res.ai_send_talk_msg(nationRecno, nation_recno, TALK_END_FRIENDLY_TREATY, 0, 0,
                                  1); // 1-force send
    }
    ...
    err_when(nationRelation->status >= RELATION_FRIENDLY); // when the status is still friendly or alliance
}
...
int Nation::ai_has_enough_food()
{
    ...
    if (yearly_food_change() < 0 && 100 * -yearly_food_change() / food > 50 - pref_food_reserve / 3)
    {
        return 0;
    }
    ...
    return food > 2000 + 2000 * pref_food_reserve / 100 || yearly_food_change() > 0;
}
...
int Nation::not_enough_food_index()
{
    ...
    err_when(yearly_food_change() > 0); // ai_has_enough_food() should return 1 if food change is > 0
...
int Nation::think_attack_enemy_firm(int enemyNationRecno, int firmId)
{
    ...
    for (int i = firm_array.size(); i > 0; i--)
    {
        ...
        if (firmPtr->firm_id != firmId || firmPtr->nation_recno != enemyNationRecno)
        {
            continue;
        }
        ...
        curRating = world.distance_rating(firmPtr->center_x, firmPtr->center_y, ourLargestTown->center_x,
    ...
    return ai_attack_target(targetFirm->loc_x1, targetFirm->loc_y1, targetCombatLevel, 0, 0, 0, 0, useAllCamp);
}
...
int Nation::think_against_mine_monopoly()
{
    ...
    for (i = nation_array.size(); i > 0; i--)
    {
        ...
        curRating = mineCount * 100 - get_relation(i)->ai_relation_level - trade_rating(i);
    ...
    for (i = firm_array.size(); i > 0; i--)
    {
        ...
        if (firmPtr->firm_id != FIRM_MINE || firmPtr->nation_recno != targetNationRecno ||
        ...
        return ai_attack_target(firmPtr->loc_x1, firmPtr->loc_y1, targetCombatLevel, 0, 0, 0, 0, 1); // 1-use all camps
    }
