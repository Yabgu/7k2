...
void TextResFirm::init() { TextResource::init(DIR_RES "T_FIRM.RES"); }

// text id 1 - 12
// [BAG1]When taxes are gathered from Norman subjects, their loyalty will
// decrease by 7 points instead of 10 points. [BAG2]Viking soldiers will now
// gain power-attack skills 25 points earlier than normal. [BAG3]Your skill of
// your Deruvids will now increase at a much greater rate. [BAG4]All Roman
// military units will now be trained with 10 more points of combat skill.
// [BAG5]Advanced walls will automatically be built around all of your towns.
// Damaged walls will automatically be repaired at no cost. [BAG6]Your farmers
// will now produce 30%% more food than those of other kingdoms, enabling you to
// support more soldiers and workers. [BAG7]You will now receive a 20%% greater
// profit from all sales in your markets. [BAG8]In explored areas you will now
// be able to see all enemies who have been ordered to attack you, as well as
// their intended paths. [BAG9]Resistance to you in all independent towns has
// now dropped by 20 points. [BAGA]The rate of population growth in all of your
// Egyptian towns is now 30%% greater. [BAGB]When other Kingdoms break treaties
// with you, they will suffer twice the normal penalties. [BAGC]Your research
// projects will now be completed in 30% less time.
...
char *TextResFirm::str_bribe_success() { return get_format_str(50, "CMBY"); }

// text id 51
// [CMBN]Bribing Failed.
char *TextResFirm::str_bribe_fail() { return get_format_str(51, "CMBN"); }

// text id 52
// [CMBK]Your Spy Was Caught And Executed.
char *TextResFirm::str_bribe_execute() { return get_format_str(52, "CMBK"); }
...
// text id 55
// [YES_]Yes
char *TextResFirm::str_yes() { return get_format_str(55, "YES_"); }

// text id 56
// [NO__]No
char *TextResFirm::str_no() { return get_format_str(56, "NO__"); }

// text id 57
// [CMAS]Assassination Succeeded.
char *TextResFirm::str_assassin_success() { return get_format_str(57, "CMAS"); }

// text id 58
// [CMAE]Your Spy Escaped.
char *TextResFirm::str_assassin_escape() { return get_format_str(58, "CMAE"); }
...
char *TextResFirm::str_assassin_fail() { return get_format_str(60, "CMAF"); }

// text id 61
// [CMTA]Do you really want to steal technology?
char *TextResFirm::str_ask_steal_tech() { return get_format_str(61, "CMTA"); }
...
char *TextResFirm::str_steal_tech_fail() { return get_format_str(64, "CMTN"); }
...
char *TextResFirm::str_incident_choose() { return get_format_str(67, "CMW1"); }

// text id 68
// [CMWA]Do you really want to create an incident?
char *TextResFirm::str_ask_incident() { return get_format_str(68, "CMWA"); }
...
char *TextResFirm::str_incident_success() { return get_format_str(70, "CMWY"); }

// text id 71
// [CMWF]Create Incident Failed.
char *TextResFirm::str_incident_fail() { return get_format_str(71, "CMWF"); }

// text id 72
// [CMWX]Your Spy Was Caught And Executed.
char *TextResFirm::str_incident_execute() { return get_format_str(72, "CMWX"); }
...
// text id 88
// [FAAU]Auto
char *TextResFirm::str_factory_auto() { return get_format_str(88, "FAAU"); }

// text id 89
// [FAMA]Manual
char *TextResFirm::str_factory_manual() { return get_format_str(89, "FAMA"); }
...
char *TextResFirm::str_no_trade() { return get_format_str(91, "MANT"); }
...
// text id 96
// [MIUR]Unmined Raw
char *TextResFirm::str_unmined_raw() { return get_format_str(96, "MIUR"); }

// text id 97
// [MIMR]Mined Raw
char *TextResFirm::str_mined_raw() { return get_format_str(97, "MIMR"); }
...
char *TextResFirm::str_no_research() { return get_format_str(112, "TSNO"); }
...
char *TextResFirm::str_no_build_weapon() { return get_format_str(115, "WFNW"); }
...
char *TextResFirm::str_alchemy_idle() { return get_format_str(123, "ALID"); }
...
// [FX4N]
// [FX4#]
char *TextResFirm::str_firm_name(int firmBuildId, char *closeTownName,
                                 int instanceId) {
  ...
  if (closeTownName) {
    ...
    case FIRM_FORT:
      if (instanceId <= 1)
        return process(get_format_str(128, "FORN"), firmBuild->name,
                       closeTownName);
      else
        return process(get_format_str(129, "FOR#"), firmBuild->name,
                       closeTownName, instanceId);
    default:
      if (instanceId <= 1)
        return process(get_format_str(126, "FIRN"), firmBuild->name,
                       closeTownName);
      else
        return process(get_format_str(127, "FIR#"), firmBuild->name,
                       closeTownName, instanceId);
    }
  }
...
char *TextResFirm::str_spy_steal_info() { return get_format_str(135, "STIF"); }
...
char *TextResFirm::str_scroll_if_1(char *raceName) {
  return process(get_format_str(140, "SCRL"), raceName);
}

// text id 141
// [SCR2]Owners may build the %1s %2s and Invoke %3s
char *TextResFirm::str_scroll_if_2(char *raceName, char *firmBuildName,
                                   char *godName) {
  return process(get_format_str(141, "SCR2"), raceName, firmBuildName, godName);
}

// text id 142
// [SGOL]Treasure
char *TextResFirm::str_treasure_if_1() { return get_format_str(142, "SGOL"); }

// text id 143
// [SGOW]Worth
char *TextResFirm::str_treasure_if_2() { return get_format_str(143, "SGOW"); }

// text id 144
// [SOW1]Scroll of Weapon
char *TextResFirm::str_blueprint_if_1() { return get_format_str(144, "SOW1"); }

// text id 145
// [SOW2]Technology
char *TextResFirm::str_blueprint_if_2() { return get_format_str(145, "SOW2"); }

// text id 151
// [TWNF]Town of
char *TextResFirm::str_town_of() { return get_format_str(151, "TWNF"); }

// text id 152
// [<race>RTWN]%1s Town
char *TextResFirm::str_race_town(char *raceName) {
  return process(get_format_str(152, "RTWN"), raceName);
}

// text id 153
// [<race>STWN]%1s Slave Town
char *TextResFirm::str_race_slave_town(char *raceName) {
  return process(get_format_str(153, "STWN"), raceName);
}

// text id 154
// [NWAL]No Town Wall
char *TextResFirm::str_no_town_wall() { return get_format_str(154, "NWAL"); }

// text id 155
// [TPOP]Population
char *TextResFirm::str_town_population() { return get_format_str(155, "TPOP"); }

// text id 156
// [TWRK]Worker
char *TextResFirm::str_town_worker() { return get_format_str(156, "TWRK"); }

// text id 157
// [TPES]Peasant
char *TextResFirm::str_town_peasant() { return get_format_str(157, "TPES"); }

// text id 158
// [RSST]Resistance
char *TextResFirm::str_resistance() { return get_format_str(158, "RSST"); }

// text id 159
// [(wall repair)WRPR]Repair
char *TextResFirm::str_wall_repair() { return get_format_str(159, "WRPR"); }

// text id 160
// [(wall building)WBLD]Building
char *TextResFirm::str_wall_building() { return get_format_str(160, "WBLD"); }

// text id 161
// [SLVT]Annual tribute paid to you by this town: %1,2d
char *TextResFirm::str_slave_tribute(int amount) {
  return process(get_format_str(161, "SLVT"), amount);
}

// text id 162
// [TSKL]Skill
char *TextResFirm::str_town_spy_skill() { return get_format_str(162, "TSKL"); }
...
// text id 164
// [AUTT]Automatically Collect Tax from this Town (Left - click) or all your
// Towns (Right - click) when the Loyalty reaches:
char *TextResFirm::str_auto_tax_inst() { return get_format_str(164, "AUTT"); }

// text id 165
// [AUTG]Automatically Grant Money to this Town (Left - click) or all your Towns
// (Right - click) when the Loyalty drops below:
char *TextResFirm::str_auto_grant_inst() { return get_format_str(165, "AUTG"); }

// text id 166
// [(not used)WMAN]Please assign the number of jobless civilians (0 - 8) to
// build the town wall :
char *TextResFirm::str_set_wall_crew() { return get_format_str(166, "WMAN"); }

// text id 167
// [DISA]DISABLE
char *TextResFirm::str_disable_auto() { return get_format_str(167, "DISA"); }
...
char *TextResFirm::str_loyalty_to(char *nationName) {
  return process(get_format_str(169, "LOYT"), nationName);
}

// text id 170
// [RSST]Resistance to %1s
char *TextResFirm::str_resistance_to(char *nationName) {
  return process(get_format_str(170, "RSST"), nationName);
}

// text id 171
// [LINC]Left click to increase
char *TextResFirm::str_left_inc() { return get_format_str(171, "LINC"); }

// text id 172
// [RINC]Right click to increase
char *TextResFirm::str_right_dec() { return get_format_str(172, "RINC"); }

// text id 173
// [TIIF]%1q{|Peasant: |Peasants: }%1z %2q{|Worker: |Workers: }%2z
char *TextResFirm::str_town_info_pop(int peasant, int worker) {
  return process(get_format_str(173, "TIIF"), peasant, worker);
}

// text id 174
// [YSPY](Your %1p{Spy|Spyies})
char *TextResFirm::str_spy_inside(int spyCount) {
  return process(get_format_str(174, "YSPY"), spyCount);
}

// text id 175
// [NOCM]No Commander
char *TextResFirm::str_no_commander() { return get_format_str(175, "NOCM"); }
...
char *TextResFirm::str_mine_info(int rawId) {
  if (rawId) {
    return process(get_format_str(176, "MINR"), str_factory_raw(rawId));
  } else
...
char *TextResFirm::str_mine_no_raw() { return get_format_str(178, "MINN"); }
...
char *TextResFirm::str_factory_info(int rawId) {
  if (rawId)
    return process(get_format_str(179, "FACR"), str_factory_ware(rawId));
  else
...
// text id 181
// [PRAY]Pray Points
char *TextResFirm::str_pray_points() { return get_format_str(181, "PRAY"); }

// text id 182
// [ENER]Energy
char *TextResFirm::str_cast_energy() { return get_format_str(182, "ENER"); }
