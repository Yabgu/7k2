...
FirmMonsterAlchemy::FirmMonsterAlchemy() {
  memset((char *)this + sizeof(Firm), 0,
         sizeof(FirmMonsterAlchemy) - sizeof(Firm));
...
void FirmMonsterAlchemy::next_day() {
  ...
  if (info.game_date % PROCESS_GOODS_INTERVAL ==
      firm_recno %
          PROCESS_GOODS_INTERVAL) // produce raw materials once every 3 days
  {
    produce_raw();
  }
}
...
int FirmMonsterAlchemy::can_converse(int newTechId) {
  ...
  if (newTechId &&
      tech_res[newTechId]->get_nation_tech_level(nation_recno) > 0) {
    switch (newTechId) {
...
void FirmMonsterAlchemy::change_conversion(int newTechId, char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_ALCH_CONVERSE,
                                                         2 * sizeof(short));
...
void FirmMonsterAlchemy::produce_raw() {
  ...
  float consumeQty =
      MAX_CONSUME // 50% to 100% of maximum consume as the technology increases
      * (techInfo->common_max_tech_level +
         techInfo->get_nation_tech_level(nation_recno)) /
  ...
  if (rawQty < 0.0f) {
    ...
    site_array[site_recno]->reserve_qty =
        (int)reserve_qty; // update the reserve_qty in site_array
  }
  ...
  if (site_recno && reserve_qty == 0) {
    site_array.untapped_raw_count++; // have to restore its first as del_site()
                                     // will decrease uptapped_raw_count
