...
void TalkMsg::request_declare_war() {
  ...
  //
  // Senden:
  // <King>'s Königreich erbittet Ihre Kriegserklärung gegen <King B>'s
  // Königreich. Sie bitten <King>'s Königreich um Kriegserklärung gegen <King
  // B>'s Königreich.
  //
  // Antwort:
  // <King>'s Königreich akzeptiert/verweigert Kriegserklärung gegen <King B>'s
  // Königreich. Sie akzeptieren/verweigern Kriegserklärung gegen <King B>'s
  // Königreich.
  //
  //---------------------------------------------//
...
void TalkMsg::demand_tribute(int isAid) {
  ...
  // Hilfe/Tribut an Sie.	     //CAUTION: No more alternating 'give/pay', just
  // only: 'Zahlung'!
  //
...
void TalkMsg::demand_tech() {
  ...
  char friendRelation = (nation_array[from_nation_recno]
                             ->get_relation_status(to_nation_recno)
                             ->is_friendly());
...
void TalkMsg::request_military_aid() {
  ...
  //
  // Sie akzeptieren die Entsendung von militärischer Soforthilfe zu <King>'s
  // Königreich. Sie verweigern die Entsendung der militärischen Soforthilfe zu
  // <King>'s Königreich.
  //
  //---------------------------------------------//
  ...
  if (reply_type == REPLY_WAITING || !should_disp_reply) {
    ...
    if (viewing_nation_recno == from_nation_recno) {
      ...
      if (reply_type == REPLY_ACCEPT)
        str +=
            " akzeptiert, sofort die erbetene militärische Hilfe zu entsenden.";
      else
        str += " verweigert die militärische Soforthilfe.";
    } else {
      if (reply_type == REPLY_ACCEPT)
        str =
            "Sie akzeptieren die Entsendung von militärischer Soforthilfe zu ";
      else
  ...
  if (reply_type == REPLY_WAITING || !should_disp_reply) {
    ...
    if (viewing_nation_recno == from_nation_recno) {
      ...
      if (reply_type == REPLY_ACCEPT)
        ...
        str += " verweigert, sich selbst zu entehren, indem er seinen Thron "
               "verkauft!";
    } else {
      if (reply_type == REPLY_ACCEPT) {
        ...
        str = "Sie verweigen es, sich selbst zu entehren, indem Sie Ihren "
              "Thron an ";
        str += from_nation_name();
        str += " verkaufen.";
      }
