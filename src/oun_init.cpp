...
//                              (if startXLoc < 0, this is a unit for hire, and
//                              is not a unit of the game yet. init_sprite()
//                              won't be called for this unit) (default: -1, -1)
//
// Note: sprite_recno must be initialized first before calling Unit::init()
//
void Unit::init(int unitId, int nationRecno, int rankId, int unitLoyalty,
                int startXLoc, int startYLoc) {
  //------------ set basic vars -------------//

  nation_recno = (char)nationRecno;
  rank_id = rankId; // rank_id must be initialized before init_unit_id() as
                    // init_unit_id() may overwrite it
  ...
  } else //---- init non-human unit series no. ----//
  {
    if (nation_recno)
      name_id =
          ++nation_array[nation_recno]->last_unit_name_id_array[unit_id - 1];
    else
      name_id = 0;
  }
  ...
  if (is_die_after_attack())
    ...
    // waste his life
    behavior_mode =
        UNIT_DEFENSIVE; // bee die after attack, so don't waste his life
  else
...
void Unit::init_sprite(int startXLoc, int startYLoc) {
  Sprite::init(unit_res[unit_id]->sprite_id, startXLoc,
               startYLoc); // BUGHERE use unit_id temporarily
  ...
  err_if(!sprite_recno) // sprite_recno must be initialized first before calling
                        // UnitB::init()
      err_here();

  for (h = 0, y = startYLoc; h < loc_height; h++, y++) {
    for (w = 0, x = startXLoc; w < loc_width; w++, x++) {
      err_if(!world.get_loc(x, y)->can_move(
          mobile_type)) // it must be 0 to put the sprite in this location
          err_here();
...
void Unit::deinit() {
  ...
  if (!sys.signal_exit_flag && nation_recno) {
    if (rank_id ==
        RANK_KING) // check nation_recno because monster kings will die also.
    {
      king_die();
      err_when(unit_array.is_truly_deleted(sprite_recno));
  ...
  if (unit_mode == UNIT_MODE_ON_SHIP) {
    err_here();
    /*
    if( !unit_array.is_deleted(unit_mode_para) )    // the ship may have been
    destroyed at the same time. Actually when the ship is destroyed, all units
    onboard are killed and this function is called.
    {
  ...
  //----- if this is a ship in the harbor -----//
    ...
    err_here();
    /*
    if( !firm_array.is_deleted(unit_mode_para) )    // the ship may have been
    destroyed at the same time. Actually when the ship is destroyed, all firms
    onboard are killed and this function is called.
    {
  ...
  else if (selected_flag) // caravan may be deinit_sprite(1) and keep selected,
                          // now unselect
  {
    int curSelected =
        unit_array.selected_recno ==
        sprite_recno; // if it is currently selected unit, reset selected_recno
    deselect();
...
void Unit::deinit_sprite(int keepSelected) {
  ...
  for (h = 0, y = next_y_loc(); h < sprite_info->loc_height; h++, y++) {
    for (w = 0, x = next_x_loc(); w < sprite_info->loc_width; w++, x++) {
      err_if(world.get_loc(x, y)->unit_recno(mobile_type) !=
             sprite_recno) // it must be 0 to put the sprite in this location
          err_here();
  ...
  /*
     //----- this is a monster unit defending its town ------//
          ...
          else if( unit_mode==UNIT_MODE_MONSTER && unit_mode_para )
          {
                  ...
                  FirmMonster* firmMonster = (FirmMonster*)
     firm_array[unit_mode_para];
    ...
    for (int i = unit_array.size(); i > 0; i--) {
      ...
      if (unit_array[i]->selected_flag) {
        if (unit_array.selected_recno == 0 ||
            unit_array[i]->rank_id >
                unit_array[unit_array.selected_recno]->rank_id) {
