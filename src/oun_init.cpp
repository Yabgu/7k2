...
void Unit::init(int unitId, int nationRecno, int rankId, int unitLoyalty, int startXLoc, int startYLoc)
{
    ...
    if (is_die_after_attack())
        // behavior_mode = UNIT_STAND_GROUND;		// bee die after attack, so
...
void Unit::init_attack_info()
{
    ...
    if (unitInfo->class_info.has_weapon_version && (techLevel = get_weapon_version()) > 0)
    // ####### end Gilbert 24/3 ########//
    {
        switch (unit_id)
        {
...
void Unit::deinit()
{
    ...
    {
        int curSelected =
            unit_array.selected_recno == sprite_recno; // if it is currently selected unit, reset selected_recno
        deselect();
