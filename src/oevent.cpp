...
int RandomEvent::execute_event()
{
        ...
        if (!nation_array.is_deleted(nationRecno))
        {
            ...
            for (techId = 1; techId <= MAX_TECH; ++techId)
            {
                if (tech_res[techId]->can_research(nationRecno) &&
                    nationPtr->research_tech_id != techId) // if the nation is currently researching this tech,
                                                           // don't consider it
                {
    ...
    case RANDOM_EVENT_UNIT_DIE: {
        ...
        for (int t = unit_array.size(); t > 0; --t, ++unitRecno)
        {
            ...
            Unit *unitPtr;
            if (!unit_array.is_deleted(unitRecno) && (unitPtr = unit_array[unitRecno]) && unitPtr->nation_recno
#ifdef FIXED_NATION
            ...
            if (firmPtr->cast_to_FirmCamp())
            {
                ...
                for (int s = camp->soldier_count; s > 0; --s)
                {
                    ...
                    if ((soldierPtr->is_human() &&
                             random(soldierPtr->max_hit_points() * 2) > (int)soldierPtr->hit_points ||
                         soldierPtr->is_monster() &&
                             random(soldierPtr->max_hit_points() * 3) > (int)soldierPtr->hit_points * 2) &&
    ...
    case RANDOM_EVENT_ITEM_DISCOVER: {
        ...
        for (int t = town_array.size(); t > 0; --t, ++townRecno)
        {
            ...
            for (int i = 3 + random(2); i > 0; --i)
            {
                ...
                if (itemId && site_array.add_site(townPtr->loc_x2 - i, townPtr->loc_y2 + 1, SITE_ITEM, itemId,
