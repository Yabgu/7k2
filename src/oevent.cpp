...
int RandomEvent::execute_event() {
  ...
  case RANDOM_EVENT_FINISH_TECH: {
    ...
    for (int t = nation_array.size(); t > 0; --t, ++nationRecno) {
      ...
      if (!nation_array.is_deleted(nationRecno) &&
        ...
        if (nationPtr->is_own()) {
          news_array.event_finish_tech(
              techId, tech_res[techId]->get_nation_tech_level(nationRecno));
        }
  ...
  case RANDOM_EVENT_NEW_TECH: {
    int nationRecno =
        random(nation_array.size()) + 1; // no need to try other nation
#ifdef FIXED_NATION
    nationRecno = FIXED_NATION;
#endif
    for (int t = nation_array.size(); t > 0; --t, ++nationRecno) {
  ...
  case RANDOM_EVENT_FOOD_DESTROYED: {
    ...
    Nation *nationPtr;
    if (!nation_array.is_deleted(nationRecno) &&
        (nationPtr = nation_array[nationRecno]) && nationPtr->food > 500.0f) {
      int foodLost = 0;
  ...
  case RANDOM_EVENT_UNIT_DIE: {
    ...
    for (int t = unit_array.size(); t > 0; --t, ++unitRecno) {
      ...
      if (!unit_array.is_deleted(unitRecno) &&
          (unitPtr = unit_array[unitRecno]) && unitPtr->nation_recno
#ifdef FIXED_NATION
          && unitPtr->nation_recno == FIXED_NATION
#endif
          && (eventId == RANDOM_EVENT_GENERAL_DIE &&
                  unitPtr->rank_id == RANK_GENERAL ||
              eventId == RANDOM_EVENT_UNIT_DIE &&
                  unitPtr->rank_id == RANK_SOLDIER) &&
          (unitPtr->is_human() && unitPtr->unit_id != UNIT_WAGON ||
           unitPtr->is_monster() &&
               random(2) < 1) // monster has 1/2 chance survive
          && !game.is_unit_in_condition(
                 unitPtr->unique_id)) // unit affecting win/lose can't die
      {
        if (unitPtr->is_visible()) // mobile
        {
          ...
          last_event_para1 =
              unitPtr->race_id; // do not remember unit recno, it will be killed
          last_event_para2 =
              unitPtr->hero_id ? -unitPtr->hero_id : unitPtr->name_id;
          ...
          if (firmPtr->cast_to_FirmCamp()) {
            ...
            last_event_para1 =
                unitPtr
                    ->race_id; // do not remember unit recno, it will be killed
            last_event_para2 =
                unitPtr->hero_id ? -unitPtr->hero_id : unitPtr->name_id;
      ...
      if (firmPtr->cast_to_FirmCamp()) {
        ...
        for (int s = camp->soldier_count; s > 0; --s) {
          ...
          if ((soldierPtr->is_human() &&
                   random(soldierPtr->max_hit_points() * 2) >
                       (int)soldierPtr->hit_points ||
               soldierPtr->is_monster() &&
                   random(soldierPtr->max_hit_points() * 3) >
                       (int)soldierPtr->hit_points * 2) &&
              !game.is_unit_in_condition(
                  soldierPtr->unique_id)) // unit affecting win/lose can't die
          {
            camp->kill_soldier(s);
            peopleKill++;
          }
        ...
        for (int s = mfort->archer_count; s > 0; --s) {
          ...
          if (archerPtr->rank_id != RANK_KING && archerPtr->race_id &&
              random(archerPtr->max_hit_points() * 3) >
                  (int)archerPtr->hit_points * 2) {
  ...
  case RANDOM_EVENT_UNIT_JOIN: {
    ...
    if (nation_array.packed_size() > 0) {
      nationRecno =
          nation_array.packed_recno(random(nation_array.packed_size()) + 1);
#ifdef FIXED_NATION
      nationRecno = FIXED_NATION;
#endif
      if (!nation_array.is_deleted(nationRecno)) {
        ...
        int unitId;
        int combatLevel =
            BASIC_SKILL_TRAIN + random(MAX_SKILL_TRAIN - BASIC_SKILL_TRAIN);
        int skillLevel =
            BASIC_COMBAT_TRAIN + random(MAX_COMBAT_TRAIN - BASIC_COMBAT_TRAIN);
        if (raceId <= MAX_RACE) {
          switch (random(3)) {
          case 0:
        ...
        if (unitRecno) {
          ...
          //unitPtr->is_blocked() )
          //						{
          //							// cannot reach town, resign
          //it 							unitPtr->resign( COMMAND_AUTO );
          //						}
          //						else
          {
  ...
  case RANDOM_EVENT_ITEM_DISCOVER: {
    ...
    for (int t = town_array.size(); t > 0; --t, ++townRecno) {
      ...
      for (int i = 3 + random(2); i > 0; --i) {
        int itemId = item_res.random_item_id(
            MIN_RARITY, MIN_RARITY + random(MAX_RARITY - MIN_RARITY + 1));
        if (itemId &&
            site_array.add_site(townPtr->loc_x2 - i, townPtr->loc_y2 + 1,
                                SITE_ITEM, itemId,
                                item_res.random_para(itemId, random(0x7fff)))) {
          itemCreated++;
        }
      }
  ...
  case RANDOM_EVENT_TOWN_PEOPLE_KILLED: {
    ...
    for (int t = town_array.size(); t > 0; --t, ++townRecno) {
      ...
      if (townPtr->population < oldPop) {
        ...
        if (townPtr->is_own()) {
          news_array.event_town_people_killed(townRecno,
                                              oldPop - townPtr->population);
        }
        ...
        last_event_para1 = townRecno;
        last_event_para2 =
            oldPop - townPtr->population; // number of people died
        break;
      }
    }
  } break;
...
int RandomEvent::create_boundary_unit(int unitId, int nationRecno) {
  ...
  for (int t = 16; t > 0; --t, ++p) {
    ...
    int unitRecno;
    if (world.check_unit_space(outerX1, outerY1, outerX2, outerY2,
                               unitInfo->mobile_type) &&
        (unitRecno = unit_array.add_unit(unitId, nationRecno, RANK_SOLDIER, 100,
                                         x, y))) {
      return unitRecno;
    }
  }
