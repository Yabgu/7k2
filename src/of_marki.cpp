...
static Point section_point_array[] = {{8, 110}, {8, 130}, {8, 150}, {30, 110}, {30, 130}, {30, 150},

                                      {2, 65},  {2, 75},  {2, 85},  {15, 65},  {15, 75},  {15, 85}};

static Point slot_point_array[] = {{0, 0}, {6, 0}, {0, 6}, {6, 6}, {12, 0}, {12, 6}, {-2, -1}, {4, -1}, {-4, -7},

                                   {0, 0}, {3, 0}, {0, 3}, {3, 3}, {6, 0},  {6, 3},  {-1, -1}, {2, -1}, {-2, -3}};
...
void FirmMarket::detect_market_info()
{
    ...
    for (; i < MAX_MARKET_GOODS; ++i, ++marketGoods)
    {
        ...
        else if (marketGoods->product_raw_id)
        {
            ...
            //			char productName[20];
            //			strcpy(productName,
            // raw_res[marketGoods->product_raw_id]->name);
            // strcat(productName, " Products"); 			str =
            // translate.process(productName); #else 			str  =
            // raw_res[marketGoods->product_raw_id]->name; 			str +=
            // translate.process(" Products"); #endif
...
void FirmMarket::draw(int displayLayer)
{
    ...
    if (displayLayer == 1)
    {
        ...
        for (i = 0, marketGoods = market_goods_array; i < MAX_MARKET_GOODS; i++, marketGoods++)
        {
            ...
            {
                if (config.building_size == 1)
                    world.zoom_matrix->put_bitmap(x + slot_point_array[j].x, y + slot_point_array[j].y, altitude,
                                                  iconPtr, NULL, 0, 0);
                else
    ...
    for (int i = 0; i < MAX_MARKET_GOODS; ++i)
    {
        ...
        if (edit_stock_enable[i])
        {
            ...
            if (x < INFO_X1 + 120)
                x = INFO_X1 + 120;
            x = edit_stock_x2[i] =
                font_blue.put((edit_stock_x1[i] = x), (edit_stock_y1[i] = y), (int)marketGoods->stock_qty, 0, x2);
