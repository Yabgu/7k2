...
static Point section_point_array[] = {
    {8, 110}, {8, 130}, {8, 150}, {30, 110}, {30, 130}, {30, 150},

    {2, 65},  {2, 75},  {2, 85},  {15, 65},  {15, 75},  {15, 85}};

static Point slot_point_array[] = {
    {0, 0},  {6, 0},   {0, 6},  {6, 6},   {12, 0},
    {12, 6}, {-2, -1}, {4, -1}, {-4, -7},
...
void FirmMarket::put_info(int refreshFlag) {
  ...
  //--- only display market info if the player is allowed to trade with this
  //market ---//
  ...
  /*
          ...
          //--- only display market info if the player is allowed to trade with
     this market ---//
          ...
          disp_income(INFO_Y1+209, refreshFlag );	  // 1-display income
     figure

          if( refreshFlag == INFO_REPAINT )
                  button_hire_caravan.paint( INFO_X1, INFO_Y1+236, 'A',
     "HIRECARA" );
...
void FirmMarket::detect_info() {
  ...
  /*
          ...
          if( nation_recno != nation_array.player_recno )		// the
     following controls are only available for player's firms return;
     ...
     market_goods_array+i;
                          ...
                          {
                                  // message structure : <firm recno> <cell no
     0-3> short *shortPtr = (short
     *)remote.new_send_queue_msg(MSG_F_MARKET_SCRAP, sizeof(short)+sizeof(short)
     ); shortPtr[0] = firm_recno; shortPtr[1] = i;
                          }
  ...
  return Firm::should_show_info() ||
         (nation_array.player_recno &&
          nation_array[nation_recno]
              ->get_relation(nation_array.player_recno)
              ->trade_treaty);
}
...
void FirmMarket::disp_market_info(int dispY1, int refreshFlag) {
  if (!should_show_info()) {
    font_snds.center_put_paragraph(
        INFO_X1 + 16, INFO_Y1 + 8, INFO_X2 - 10, INFO_Y1 + 42,
        text_firm.str_no_trade(), // "You're not permitted to trade with this
                                  // market.",
        1, 0, 0);
    return;
  }
  ...
  for (; i < MAX_MARKET_GOODS; ++i, ++marketGoods) {
    ...
    if (marketGoods->raw_id) {
      //			bitmapPtr =
      //raw_res.large_raw_icon(marketGoods->raw_id);
    } else if (marketGoods->product_raw_id) {
      //			bitmapPtr =
      //raw_res.large_product_icon(marketGoods->product_raw_id);
    } else {
      // empty
      continue;
    }
    ...
    if (!show_sale_debug_info) {
      ...
      if (yearSales > 0) {
        ...
        vga.active_buf->put_bitmap_trans(x - 4, y,
                                         image_gameif.read(bitmapName));
      }
    } else {
      font_snds.put(x, y + 16, "Sales");
      font_snds.right_put(x + 56, y + 16,
                          misc.format((int)marketGoods->sales_365days()));
    }

    // ------- display quantity -----//

    if (!show_sale_debug_info) {
      font_snds.center_put(x, y + 20, x + 56, y + 36,
                           misc.format((int)marketGoods->stock_qty));
    } else {
    ...
    if (show_sale_debug_info) {
      ...
      font_snds.right_put(x + 56, y + 32,
                          misc.format((int)marketGoods->month_demand));
    }
    ...
    if (marketGoods->stock_qty >= MAX_MARKET_STOCK - 10) {
      ...
      vga.active_buf->put_bitmap(x + 8,
                                 y + 48 + RAW_LARGE_ICON_HEIGHT - 1 -
                                     ((Bitmap *)fullBitmap)->get_height(),
                                 fullBitmap);
    }
  }
...
market ---//
  ...
  if( nation_array.player_recno )
          noTrade =
nation_array[nation_recno]->get_relation(nation_array.player_recno)->trade_treaty==0;
  else
  ...
  if( noTrade )
  {
          if( refreshFlag == INFO_REPAINT )
          {
                  ...
                  font_san.center_put( INFO_X1, dispY1+3 , INFO_X2, dispY1+25,
"You're not permitted to" ); font_san.center_put( INFO_X1, dispY1+23, INFO_X2,
dispY1+51, "trade with this market." );
          }
  ...
  for( i=0, marketGoods=market_goods_array ; i<MAX_MARKET_GOODS ; i++,
marketGoods++, y+=53 )
  {
          if( refreshFlag == INFO_REPAINT )
          ...
          else if( marketGoods->product_raw_id )
          {
                  ...
                  char productName[20];
                  strcpy(productName,
raw_res[marketGoods->product_raw_id]->name); strcat(productName, " Products");
...
#endif
                  bitmapPtr =
raw_res.small_product_icon(marketGoods->product_raw_id);
          }
          else
          {
          ...
          if( refreshFlag == INFO_REPAINT )
          {
                  ...
                  {
                          button_clear_stock[i].paint_text( INFO_X2-46, y+2,
INFO_X2-3, y+19, "Clear" );		// Clear Stock
          ...
          font_san.field( x, ty, "Stock", x+60, str, x+119, refreshFlag,
"MK_STOCK" );

          font_san.field( x, ty+16, "Sales", x+60, (int)
marketGoods->sales_365days(), 2, x+104, refreshFlag, "MK_SALES" );

          x+=105;

          // ####### patch begin Gilbert 16/3 #########//
          //font_san.field( x, ty+16, "Demand", x+70, (int)
marketGoods->month_demand, 1,
          //					 INFO_X2-2, refreshFlag, "MK_DEMAN"
); font_san.field( x, ty+16, "Demand", x+67, (int) marketGoods->month_demand, 1,
                                                   INFO_X2-1, refreshFlag,
"MK_DEMAN" );
...
void FirmMarket::detect_market_info() {
  ...
  for (; i < MAX_MARKET_GOODS; ++i, ++marketGoods) {
    ...
    if (marketGoods->raw_id) {
      ...
      //			char productName[20];
      //			strcpy(productName,
      //raw_res[marketGoods->product_raw_id]->name); 			strcat(productName, "
      //Products"); 			str = translate.process(productName); #else 			str  =
      //raw_res[marketGoods->product_raw_id]->name; 			str += translate.process("
      //Products"); #endif
...
void FirmMarket::disp_income(int dispY1, int refreshFlag) {
  ...
  font_snds.put(INFO_X1 + 26, INFO_Y1 + 21,
                text_firm.str_yearly_income((int)income_365days()));
}
...
void FirmMarket::draw(int displayLayer) {
  ...
  if (displayLayer == 1) {
    ...
    for (i = 0, marketGoods = market_goods_array; i < MAX_MARKET_GOODS;
         i++, marketGoods++) {
      if (marketGoods->raw_id)
      ...
      for (j = 0; j < cargoCount; j++) {
        if (config.building_size == 1)
          world.zoom_matrix->put_bitmap(x + slot_point_array[j].x,
                                        y + slot_point_array[j].y, altitude,
                                        iconPtr, NULL, 0, 0);
        else
          world.zoom_matrix->put_bitmap(x + slot_point_array[j + 9].x,
                                        y + slot_point_array[j + 9].y, altitude,
                                        iconPtr, NULL, 0, 0);
      }
    }
  }
...
void FirmMarket::disp_edit_mode(int &refreshFlag, int &y, int dispBG) {
  ...
  for (int i = 0; i < MAX_MARKET_GOODS; ++i) {
    ...
    edit_stock_enable[i] =
        marketGoods->raw_id != 0 || marketGoods->product_raw_id != 0;

    if (edit_stock_enable[i]) {
      if (marketGoods->product_raw_id)
        x = font_whbl.put(
            x1, y, text_firm.str_factory_ware(marketGoods->product_raw_id), 0,
            x2);
      else if (marketGoods->raw_id)
        x = font_whbl.put(x1, y, text_firm.str_factory_raw(marketGoods->raw_id),
                          0, x2);
      else {
        err_here();
      }
      ...
      if (x < INFO_X1 + 120)
        x = INFO_X1 + 120;
      x = edit_stock_x2[i] =
          font_blue.put((edit_stock_x1[i] = x), (edit_stock_y1[i] = y),
                        (int)marketGoods->stock_qty, 0, x2);
...
int FirmMarket::detect_edit_mode() {
  ...
  for (int i = 0; i < MAX_MARKET_GOODS; ++i) {
    MarketGoods *marketGoods = &market_goods_array[i];
    if (edit_stock_enable[i]) {
      if (mouse.any_click(edit_stock_x1[i], edit_stock_y1[i], edit_stock_x2[i],
                          edit_stock_y2[i], LEFT_BUTTON)) {
