...
void FirmWork::pay_expense() {
  ...
  nation_array[nation_recno]->add_expense(
      EXPENSE_FIRM, (float)WORKER_YEAR_SALARY * worker_count / 365, 1);
...
#define RESISTANCE_DECREASE_PER_WORKER                                         \
  float(0.2) // resistance decrease per month every 15 days
...
void FirmWork::recruit_worker() {
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    err_when(townPtr->nation_recno != nation_recno &&
             townPtr->nation_recno != 0); // only recruit from own towns

    if (townPtr->jobless_population == 0)
      continue;

    int townDistance = misc.points_distance(
        townPtr->center_x, townPtr->center_y, center_x, center_y);

    //-------- calculate the recruit rating --------//

    recruitRating = 100 * townPtr->jobless_population /
                        townPtr->population            // relative value
...
//
void FirmWork::set_needed_worker_count(int neededWorkerCount,
                                       char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_FIRM_SET_WORKER_COUNT, 2 * sizeof(short));
    *shortPtr = firm_recno;
  ...
  //--- if the new needed worker count is larger than the existing number of
  //workers ---//

  if (worker_count > neededWorkerCount) {
    //------ resign local worker -------//

    for (int i = worker_count - 1; worker_count > neededWorkerCount && i >= 0;
         i--) {
...
void FirmWork::resign_worker(int workerId) {
  err_when(!worker_array); // this function shouldn't be called if this firm
                           // does not need worker
...
void FirmWork::calc_productivity() {
  ...
  productivity =
      100 * worker_count / MAX_WORKER; // the maximum productivity is 100
}
