...
void InGameMenu::enter(char untilExitFlag)
{
    ...
    if (game.is_campaign_mode())
    {
        ...
        if (game.campaign()->stage_id == 6 && ((CampaignEastWest *)game.campaign())->hero_unit_recno == 0)
        {
    ...
    if (game.game_mode == GAME_TUTORIAL)
    {
        ...
        if (tutor.selectively_detect_flag != 5 && tutor.selectively_detect_flag != 0 &&
...
int InGameMenu::detect()
{
    ...
    font_bold_red.center_put(x1, y + ySpacing * b, x2, y + ySpacing * b + font_bold_black.max_font_height, str);
    ...
    case ING_RETIRE_GAME:              // retire
        ...
        {
            if (box.ask(text_game_menu.str_ask_retire(game.is_campaign_mode()), text_game_menu.str_no_yes(1),
                        text_game_menu.str_no_yes(0), 175, 320))
            {
                if (remote.is_enable())
                {
    ...
    case ING_CAMPAIGN_CHAT: {
        if (game.is_campaign_mode() && box.ask(text_game_menu.str_ask_cheat_win(), text_game_menu.str_no_yes(1),
                                               text_game_menu.str_no_yes(0), 120, 320))
        {
    ...
    case ING_QUIT_TO_MAIN_MENU: // quit to main menu
    {
        ...
        if (!nation_array.player_recno || box.ask(text_game_menu.str_ask_quit_main(), text_game_menu.str_no_yes(1),
                                                  text_game_menu.str_no_yes(0), boxX1, 350))
        {
    ...
    case ING_QUIT_TO_WINDOWS: // quit to Windows
        if (!nation_array.player_recno || box.ask(text_game_menu.str_ask_quit_game(), text_game_menu.str_no_yes(1),
                                                  text_game_menu.str_no_yes(0), 130, 400))
        {
            if (remote.is_enable() && nation_array.player_recno)
            {
                // BUGHERE : message will not be sent out
