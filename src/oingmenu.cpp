...
enum { GAME_MENU_WIDTH = 548, GAME_MENU_HEIGHT = 532 };
...
unsigned InGameMenu::menu_hot_key[InGameMenu::GAME_OPTION_COUNT] = {
    'o', 0, 's', 'l', 0, 0, 0, 0, 0, 0, KEY_ESC};
...
void InGameMenu::enter(char untilExitFlag) {
  ...
  if (game.is_campaign_mode()) {
    ...
    game_menu_option_flag[ING_RESTART_GAME] = 0; // disable restart
    //		menu_option_str[ING_RETIRE_GAME] = menu_option_4_str[0];		//
    //retire
    game_menu_option_flag[ING_MISSION_BRIEFING] =
        0; // disable missing brief in non campaign mode
    game_menu_option_flag[ING_CAMPAIGN_CHAT] =
        0; // disable campaign chat in non campaign mode
  }
...
void InGameMenu::disp(int needRepaint) {
  ...
  if (refresh_flag) {
    ...
    //         );
    mouse.hide();
    vga.active_buf->put_bitmap_trans_remap(bx, by, background_bitmap,
                                           color_remap_table);
    ...
    for (int b = 0; b < GAME_OPTION_COUNT; ++b) {
      if (game_menu_option_flag[b]) {
        ...
        font_bold_black.center_put(
            x1, y + ySpacing * b, x2,
            y + ySpacing * b + font_bold_black.max_font_height, str);
      }
    }
...
int InGameMenu::detect() {
  ...
  for (b = 0; b < GAME_OPTION_COUNT; ++b) {
    if (game_menu_option_flag[b] == 1 &&
        (menu_hot_key[b] && mouse.key_code == menu_hot_key[b] ||
         mouse.single_click(x1, y + ySpacing * b, x2,
                            y + ySpacing * b +
                                font_bold_black.max_font_height)))
      break;
  ...
  font_bold_red.center_put(x1, y + ySpacing * b, x2,
                           y + ySpacing * b + font_bold_black.max_font_height,
                           str);
  ...
  case ING_CAMPAIGN_CHAT: {
    if (game.is_campaign_mode() &&
        box.ask(text_game_menu.str_ask_cheat_win(),
                text_game_menu.str_no_yes(1), text_game_menu.str_no_yes(0), 120,
                320)) {
  ...
  case ING_QUIT_TO_MAIN_MENU: // quit to main menu
  {
    ...
    if (!nation_array.player_recno ||
      ...
      if (remote.is_enable() && nation_array.player_recno) {
        // BUGHERE : message will not be sent out
        short *shortPtr = (short *)remote.new_send_queue_msg(MSG_PLAYER_QUIT,
                                                             2 * sizeof(short));
        shortPtr[0] = nation_array.player_recno;
  ...
  case ING_QUIT_TO_WINDOWS: // quit to Windows
    if (!nation_array.player_recno ||
        box.ask(text_game_menu.str_ask_quit_game(),
                text_game_menu.str_no_yes(1), text_game_menu.str_no_yes(0), 130,
                400)) {
      if (remote.is_enable() && nation_array.player_recno) {
        // BUGHERE : message will not be sent out
        short *shortPtr = (short *)remote.new_send_queue_msg(MSG_PLAYER_QUIT,
