...
static void scroll_buffer(short *imageBuf, int truePitch, int srcX1, int srcY1, int srcX2, int srcY2, int shiftX,
                          int shiftY, int fillColor = -1);
...
ZoomMatrix::ZoomMatrix()
    : land_disp_sort_array(sizeof(DisplaySort), 100), air_disp_sort_array(sizeof(DisplaySort), 50),
      land_top_disp_sort_array(sizeof(DisplaySort), 40), land_bottom_disp_sort_array(sizeof(DisplaySort), 20)
{
    init(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, ZOOM_WIDTH, ZOOM_HEIGHT, ZOOM_LOC_X_WIDTH, ZOOM_LOC_X_HEIGHT,
...
void ZoomMatrix::draw()
{
    ...
    dispPower = (world.map_matrix->map_mode == MAP_MODE_POWER && world.map_matrix->power_mode) ||
                power.command_id == COMMAND_BUILD_FIRM || power.command_id == COMMAND_SETTLE;
    ...
    else if (misc.is_touch(update_abs_x1, update_abs_y1, update_abs_x2, update_abs_y2, last_abs_x1, last_abs_y1,
                           last_abs_x2, last_abs_y2))
    {
        // scrolling
        scroll_buffer(((BitmapW *)save_image_buf)->get_ptr(), ((BitmapW *)save_image_buf)->get_true_pitch(), 0, 0,
    ...
    else
    {
        ...
        //*)save_image_buf)->get_true_pitch(),
        // IMAGE_BUF_LEFT_MARGIN,
        ...
        if (disp_rect_type == 0 && (top_x_loc < 0 || top_x_loc + disp_x_loc > max_x_loc || top_y_loc < 0 ||
                                    top_y_loc + disp_y_loc > max_y_loc) // rectangular
            || disp_rect_type == RHOMBUS_LOCATION &&
                   (top_x_loc - disp_x_loc < 0 || top_x_loc + disp_y_loc >= max_x_loc || top_y_loc - disp_y_loc < 0 ||
                    top_y_loc + disp_x_loc + disp_y_loc >= max_y_loc)) // rhombus, this condition is prudent to
                                                                       // prefer fill black
        {
            IMGbar(((BitmapW *)save_image_buf)->get_ptr(), ((BitmapW *)save_image_buf)->get_true_pitch(), 0, 0,
                   ((BitmapW *)save_image_buf)->get_width() - 1, ((BitmapW *)save_image_buf)->get_height() - 1,
    ...
    int cursorXLoc;
    int cursorYLoc;
    int cursorMarked = get_detect_location(mouse.cur_x, mouse.cur_y, &cursorXLoc, &cursorYLoc, NULL);
#endif
    ...
    do // outer loop
    {
        ...
        do
        {
            ...
            if (xLoc >= 0 && xLoc < max_x_loc && yLoc >= 0 && yLoc < max_y_loc)
            {
                ...
                int locAbsX1 =
                    calc_abs_x(xLoc * LOCATE_WIDTH, (yLoc + 1) * LOCATE_HEIGHT, locCorner.bottom_left->get_altitude());
                int locAbsY1 =
                    calc_abs_y(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, locCorner.top_left->get_altitude()) +
                    locCorner.loc_ptr->min_y;
                int locAbsX2 =
                    calc_abs_x((xLoc + 1) * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, locCorner.top_right->get_altitude()) -
                    1;
                ...
                if (misc.is_touch(locAbsX1, locAbsY1, locAbsX2, locAbsY2, update_abs_x1, update_abs_y1, update_abs_x2,
                                  update_abs_y2) &&
                    (!misc.is_touch(locAbsX1, locAbsY1, locAbsX2, locAbsY2, last_abs_x1, last_abs_y1, last_abs_x2,
                                    last_abs_y2) ||
                    ...
                    if ((*lastDrawStatePtr = newDrawState) != FULL_DARK_MASK_ID)
                    {
                        ...
                        if (locCorner.loc_ptr->is_road())
                        {
                            ...
                            else
                            {
                                ...
                                textureBitmap =
                                    terrain_res.mask_texture(roadInfo->bitmap_ptr, textureBitmap,
                        ...
                        //#ifdef DEBUG
                        //						if( !cursorMarked || cursorXLoc != xLoc
                        //||
                        // cursorYLoc
                        //!= yLoc ) #endif
                        if (newDrawState == FULL_BRIGHT_MASK_ID || newDrawState == HALF_BRIGHT_MASK_ID)
                        {
                ...
                {
                    int scrnX0 =
                        ZOOM_X1 + calc_zoom_x(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, locCorner.top_left->altitude);
                    int scrnY0 =
                        ZOOM_Y1 + calc_zoom_y(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, locCorner.top_left->altitude);
                    int scrnX1 = ZOOM_X1 + calc_zoom_x((xLoc + 1) * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT,
                                                       locCorner.top_right->altitude);
                    int scrnY1 = ZOOM_Y1 + calc_zoom_y((xLoc + 1) * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT,
                                                       locCorner.top_right->altitude);
                    int scrnX2 = ZOOM_X1 + calc_zoom_x(xLoc * LOCATE_WIDTH, (yLoc + 1) * LOCATE_HEIGHT,
                                                       locCorner.bottom_left->altitude);
                    int scrnY2 = ZOOM_Y1 + calc_zoom_y(xLoc * LOCATE_WIDTH, (yLoc + 1) * LOCATE_HEIGHT,
                                                       locCorner.bottom_left->altitude);
    ...
    vga_back.put_bitmapW_area(image_x1 - IMAGE_BUF_LEFT_MARGIN, image_y1 - IMAGE_BUF_TOP_MARGIN, save_image_buf,
                              IMAGE_BUF_LEFT_MARGIN, IMAGE_BUF_TOP_MARGIN, image_width + IMAGE_BUF_LEFT_MARGIN - 1,
...
void ZoomMatrix::draw_magic_effects()
{
    ...
    //		magic.draw_magic_two(&vga_back, ZOOM_X2/2, ZOOM_Y1+50,
    // ZOOM_X2/2,
    // ZOOM_Y2-150, 15,
    // vga.pixel_format_flag, init_magic2);
    if (magic.effect_testing_flag == 3)
        magic.draw_magic_three(&vga_back, ZOOM_X2 / 2, ZOOM_Y1 + 50, ZOOM_X2 / 2, ZOOM_Y2 - 100, vga.pixel_format_flag,
                               init_magic2);
    if (magic.effect_testing_flag == 4)
        magic.draw_magic_four(&vga_back, ZOOM_X2 / 2, ZOOM_Y1 + 70, ZOOM_X2 / 2, ZOOM_Y2 - 70, vga.pixel_format_flag,
                              init_magic2);
    ...
    if (magic.effect_testing_flag == 6)
        magic.draw_magic_six(&vga_back, ZOOM_X2 / 2, ZOOM_Y1 + 50, ZOOM_X2 / 2, ZOOM_Y2 - 100, vga.pixel_format_flag,
                             init_magic2);
    if (magic.effect_testing_flag == 7)
        magic.draw_magic_seven(&vga_back, ZOOM_X2 / 2, ZOOM_Y1 + 50, ZOOM_X2 / 2, ZOOM_Y2 - 100, vga.pixel_format_flag,
                               init_magic2);
    if (magic.effect_testing_flag == 8)
        magic.draw_magic_eight(&vga_back, ZOOM_X2 / 2, ZOOM_Y1 + 50, ZOOM_X2 / 2, ZOOM_Y2 - 100, vga.pixel_format_flag,
                               init_magic2);
}
...
void ZoomMatrix::draw_weather_effects()
{
    ...
    if (snowScale > 0 && init_snow == 0)
    {
        ...
        snow.init(weather.wind_speed() * sin(weather.wind_direct_rad()) / 200.0, snowScale + 2);
    ...
    */
    ...
    else if (power.command_id == COMMAND_MAGIC_TELEPORT_POST)
    {
        ...
        if (!unit_array.is_deleted(FirmMagic::teleport_unit_recno))
        {
            ...
            locHeight = unitPtr->loc_height;
            validAction =
                world.check_unit_space(xLoc, yLoc, xLoc + locWidth - 1, yLoc + locHeight - 1, unitPtr->mobile_type);
        }
        else
        {
            validAction = 0;
    ...
    for (yLoc = locY1; yLoc <= locY2; ++yLoc)
    {
        for (xLoc = locX1; xLoc <= locX2; ++xLoc)
        {
            ...
            if (xLoc >= innerX1 && xLoc <= innerX2 && yLoc >= innerY1 && yLoc <= innerY2)
            {
...
void ZoomMatrix::disp_text()
{
    ...
    for (i = town_array.size(); i > 0; i--)
    {
        ...
        //		if( !world.get_loc(townPtr->center_x,
        // townPtr->center_y)->explored() ) 			continue;
        ...
        if (dispExtendInfo)
        {
            ...
            if (townPtr->is_own() || townPtr->nation_recno == 0)
            {
                ...
                put_center_text(x, y, z,
                                text_firm.str_town_info_pop(townPtr->jobless_population, townPtr->worker_population()));
                z -= 16;
            }
        }
...
//				str = firmPtr->firm_name();
        ...
        if (firmPtr->cast_to_FirmCamp())
        {
            ...
            if (firmCamp->overseer_recno && (spyRecno = unit_array[firmCamp->overseer_recno]->spy_recno) &&
            ...
            if (dispExtendInfo)
            {
                ...
                // ###### end Gilbert 10/2 #####//
                {
                    ...
                    // monster_res[unitPtr->monster_id()]->name),
                    // unitPtr->rank_id
                    // )
                    // );
                    put_center_text(
                        ...
                        text_unit.str_race_title(unitPtr->race_id,
                                                 (unitPtr->is_human() ? race_res[unitPtr->race_id]->adjective
        ...
        if (firmPtr->cast_to_FirmFactory())
        {
            ...
            if (dispExtendInfo)
            {
                ...
                else
                {
                    ...
                    //						str = "Producing ";
                    //						str +=
                    // raw_res[firmFactory->product_raw_id]->name; 						str
                    // += " Ware";
                    //					}
                    //					else
                    //						str = "No Ware Under
...
void ZoomMatrix::put_center_text(int x, int y, int z, const char *str, char black, Font *font_ptr)
{
    ...
    if (scrnX1 <= image_x2 && scrnX2 >= image_x1 && scrnY1 <= image_y2 && scrnY2 >= image_y1)
    {
        ...
        put_bitmapW_offset(x, y, z, tempBuffer.bitmap_ptr(), tempBuffer.left_edge, tempBuffer.top_edge, 0, 0);
    }
}
...
#define DRAW_LEFT_RIGHT 6
    ...
    int maxRowCount = image_height / (ZOOM_LOC_X_HEIGHT + ZOOM_LOC_Y_HEIGHT) * 2 + DRAW_DOWNSIDE * 2;
    int maxColCount = image_width / (ZOOM_LOC_X_WIDTH - ZOOM_LOC_Y_WIDTH) * 2 + DRAW_LEFT_RIGHT * 2;
                ...
                if (locPtr->unit_recno(UNIT_AIR))
                {
                    ...
                    if (!unitPtr->is_stealth() && unitPtr->next_x_loc() == xLoc && unitPtr->next_y_loc() == yLoc)
                    {
                        ...
                        displaySort.object_y2 =
                            calc_abs_y(unitPtr->cur_x + LOCATE_WIDTH / 2, unitPtr->cur_y + LOCATE_HEIGHT / 2, 0);
#else
#error
#endif
                        // ##### begin Gilbert 19/9 #######//
                        // if(pLoc == locPtr && pMobileType == UNIT_AIR)	// BUGHERE :
                        // this part may fails if sprite size > 1x1
...
#error
                        ...
                        if (hidden_by_place(unitPtr->cur_x_loc(), unitPtr->cur_y_loc(), unitPtr->sprite_info->loc_width,
...
#elif (SORT_OBJECT_METHOD == 1)
                        displaySort.object_y2 =
                            calc_abs_y((firmPtr->loc_x1 + firmPtr->loc_x2 + 1) * LOCATE_WIDTH / 2,
...
#elif (SORT_OBJECT_METHOD == 1)
                        displaySort.object_y2 =
                            calc_abs_y((townPtr->loc_x1 + townPtr->loc_x2 + 1) * LOCATE_WIDTH / 2,
...
#error
                ...
                else if (locPtr->is_plant())
                {
                    ...
                    if (!config.blacken_map || locPtr->explored())
                    {
                        ...
                        // BUGHERE : drawY ???
#if (SORT_OBJECT_METHOD == 0)
                        displaySort.object_y2 = yLoc * LOCATE_HEIGHT + innerY - LOCATE_HEIGHT / 2 +
                                                plantBitmap->offset_y + plantBitmap->bitmap_height - 1;
#elif (SORT_OBJECT_METHOD == 1)
                        displaySort.object_y2 =
                            calc_abs_y(xLoc * LOCATE_WIDTH + innerX, yLoc * LOCATE_HEIGHT + innerY, 0);
#else
#error
#endif
                ...
                else if (locPtr->is_rock())
                {
                    ...
                    if (!rockPtr->is_stealth() && xLoc == rockPtr->loc_x && yLoc == rockPtr->loc_y)
                    {
                        ...
                        displaySort.object_y2 =
                            calc_abs_y((rockPtr->loc_x * 2 + rockInfo->loc_width + 1) * LOCATE_WIDTH / 2,
...
#error
                ...
                if (locPtr->has_site() && locPtr->walkable(3)) // don't display if a building/object has
...
#elif (SORT_OBJECT_METHOD == 1)
                        //						displaySort.object_y2 =
                        // calc_abs_y(xLoc+LOCATE_WIDTH/2,
                        // yLoc+LOCATE_HEIGHT/2, 0);
                        displaySort.object_y2 =
                            calc_abs_y((sitePtr->map_x_loc + sitePtr->map_x2 + 1) * LOCATE_WIDTH / 2,
...
#error
                ...
                else if (locPtr->has_dirt())
                {
                    ...
                    if (!dirtPtr->is_stealth() && xLoc == dirtPtr->loc_x && yLoc == dirtPtr->loc_y)
                    {
                        ...
                        displaySort.object_y2 =
                            calc_abs_y((dirtPtr->loc_x * 2 + rockInfo->loc_width + 1) * LOCATE_WIDTH / 2,
...
#error
    ...
    for (i = bullet_array.size(); i > 0; i--)
    {
        ...
        if ((colCount < -DRAW_LEFT_RIGHT * 2 || colCount > maxColCount || rowCount < -DRAW_UPSIDE * 2 ||
...
#elif (SORT_OBJECT_METHOD == 1)
            displaySort.object_y2 =
                calc_abs_y(bulletPtr->cur_x + LOCATE_WIDTH / 2, bulletPtr->cur_y + LOCATE_HEIGHT / 2, 0);
#else
#error
#endif
        }
        ...
        case LAND_DISP_LAYER_MASK:
            ...
            // ---- calculate draw_flags ------//
            displaySort.draw_flags = hidden_by_place(bulletPtr->cur_x_loc(), bulletPtr->cur_y_loc(), 1, 1);
    ...
    for (i = tornado_array.size(); i > 0; i--)
    {
        ...
        if (tornadoPtr->cur_x_loc() < 0 || tornadoPtr->cur_y_loc() < 0 || tornadoPtr->cur_x_loc() >= MAX_WORLD_X_LOC ||
...
#elif (SORT_OBJECT_METHOD == 1)
        displaySort.object_y2 =
            calc_abs_y(tornadoPtr->cur_x + LOCATE_WIDTH / 2, tornadoPtr->cur_y + LOCATE_HEIGHT / 2, 0);
#else
#error
#endif
        air_disp_sort_array.linkin(&displaySort);
...
#elif (SORT_OBJECT_METHOD == 1)
        displaySort.object_y2 =
            calc_abs_y(effectPtr->cur_x + LOCATE_WIDTH / 2, effectPtr->cur_y + LOCATE_HEIGHT / 2, 0);
#else
#error
#endif
...
void ZoomMatrix::draw_objects_now(DynArray *unitArray, int displayLayer, char flags)
{
    ...
    int dispPower = (world.map_matrix->map_mode == MAP_MODE_POWER && world.map_matrix->power_mode == 1) ||
                    power.command_id == COMMAND_BUILD_FIRM || power.command_id == COMMAND_SETTLE;
    ...
    for (i = 1; i <= dispCount; i++)
    {
        ...
        case OBJECT_POINTED_UNIT: {
            ...
            if (displaySortPtr->draw_flags & OBJECT_DRAW_POINTED || unitPtr->selected_flag)
            {
                int animLineColorSeries =
                    2 + (unitPtr->selected_flag | displaySortPtr->draw_flags & OBJECT_DRAW_POINTED);
                ...
                {
                    // draw a back line
                    anim_line.draw_line(&vga_back, rectX0, rectY0, rectX0, rectY0 + unitPtr->sprite_info->max_height, 1,
                                        animLineColorSeries);
                }
                ...
                unitPtr->draw(displaySortPtr->object_type == OBJECT_POINTED_UNIT, displaySortPtr->draw_flags & flags);

                if (displaySortPtr->draw_flags & OBJECT_DRAW_POINTED)
                {
                    // draw a back line
                    anim_line.draw_line(&vga_back, rectX1, rectY1, rectX1, rectY1 + unitPtr->sprite_info->max_height, 1,
                                        animLineColorSeries);
                    anim_line.draw_line(&vga_back, rectX2, rectY2, rectX2, rectY2 + unitPtr->sprite_info->max_height, 1,
                                        animLineColorSeries);
                    anim_line.draw_line(&vga_back, rectX3, rectY3, rectX3, rectY3 + unitPtr->sprite_info->max_height, 1,
                                        animLineColorSeries);

                    anim_line.draw_line(&vga_back, rectX0, rectY0 + unitPtr->sprite_info->max_height, rectX1,
                                        rectY1 + unitPtr->sprite_info->max_height, 1, animLineColorSeries);
                    anim_line.draw_line(&vga_back, rectX1, rectY1 + unitPtr->sprite_info->max_height, rectX2,
                                        rectY2 + unitPtr->sprite_info->max_height, 1, animLineColorSeries);
                    anim_line.draw_line(&vga_back, rectX2, rectY2 + unitPtr->sprite_info->max_height, rectX3,
                                        rectY3 + unitPtr->sprite_info->max_height, 1, animLineColorSeries);
                    anim_line.draw_line(&vga_back, rectX3, rectY3 + unitPtr->sprite_info->max_height, rectX0,
                                        rectY0 + unitPtr->sprite_info->max_height, 1, animLineColorSeries);
                }
            }
            else
            {
        ...
        case OBJECT_BULLET:
            bullet_array[displaySortPtr->object_recno]->draw(0, displaySortPtr->draw_flags & flags);
            break;
        ...
        case OBJECT_FIRM: {
            ...
            {
                int animLineColorSeries = 2 + ((firm_array.selected_recno == displaySortPtr->object_recno) |
        ...
        case OBJECT_TOWN: {
            ...
            {
                int animLineColorSeries = 2 + ((town_array.selected_recno == displaySortPtr->object_recno) |
                                               displaySortPtr->draw_flags & OBJECT_DRAW_POINTED);
        ...
        case OBJECT_PLANT:
            plant_res.get_bitmap(displaySortPtr->object_recno)->draw(displaySortPtr->x_loc, displaySortPtr->y_loc);
            break;
        ...
        case OBJECT_FIRE: {
            ...
            int innerX =
                LOCATE_WIDTH / 2 + (((displaySortPtr->x_loc + 11) * (displaySortPtr->y_loc + 13)) % (LOCATE_WIDTH / 2));
            int innerY = LOCATE_HEIGHT / 2 +
                         (((displaySortPtr->x_loc + 13) * (displaySortPtr->y_loc + 17)) % (LOCATE_HEIGHT / 2));
            ...
            /*
                    ...
                    {
                            BYTE palBuf[256][3];
                            palFile.file_seek(8);     				// bypass the
               header info palFile.file_read(palBuf, sizeof(palBuf));
               palFile.file_close();
            ...
            */
            ...
            put_bitmap_offset(x1, y1, world.interpolate_z(x1, y1), (char *)flame[f].bitmap, Flame::offset_x(f),
                              Flame::offset_y(f), flame[f].color_remap_table, 0, 0);
            //	put_bitmap_offset( x1, y1, world.interpolate_z(x1, y1), (char
            //*)flame[f].bitmap, 		Flame::offset_x(f), Flame::offset_y(f),
            // NULL, 0 , 0);
        }
        break;
        ...
        case OBJECT_WALL: {
            ...
            wall_res[displaySortPtr->object_recno & 0xff]->draw_at(displaySortPtr->x_loc, displaySortPtr->y_loc,
                                                                   remapTable);
        }
        break;
        case OBJECT_TORNADO:
            tornado_array[displaySortPtr->object_recno]->draw(0, displaySortPtr->draw_flags & flags);
            break;
        ...
        case OBJECT_EFFECT:
            effect_array[displaySortPtr->object_recno]->draw(0, displaySortPtr->draw_flags & flags);
            break;
...
//
// <int>   x, y 			  - the location of the bitmap, in the
// current screen coordination <char*> bitmapPtr 	  - bitmap ptr [int]
...
void ZoomMatrix::put_bitmap_clip(int x, int y, char *bitmapPtr, int compressedFlag)
{
    ...
    if (x < ZOOM_X1 || x2 > ZOOM_X2 || y < ZOOM_Y1 || y2 > ZOOM_Y2)
    {
        if (compressedFlag)
        {
            vga_back.put_bitmap_area_trans_decompress(x, y, bitmapPtr, MAX(ZOOM_X1, x) - x, MAX(ZOOM_Y1, y) - y,
                                                      MIN(ZOOM_X2, x2) - x, MIN(ZOOM_Y2, y2) - y);
        }
        else
        {
            vga_back.put_bitmap_area_trans(x, y, bitmapPtr, MAX(ZOOM_X1, x) - x, MAX(ZOOM_Y1, y) - y,
                                           MIN(ZOOM_X2, x2) - x, MIN(ZOOM_Y2, y2) - y);
        }
    }
...
int ZoomMatrix::detect_bitmap_clip(int x, int y, char *bitmapPtr)
{
    ...
    // MIN(ZOOM_Y2,y2), 2 );
    return mouse.any_click(MAX(ZOOM_X1, x), MAX(ZOOM_Y1, y), MIN(ZOOM_X2, x2), MIN(ZOOM_Y2, y2), 0)
               ? 1
...
void ZoomMatrix::put_bitmap_remap_clip(int x, int y, char *bitmapPtr, short *colorRemapTable, int compressedFlag)
{
    ...
    if (x < ZOOM_X1 || x2 > ZOOM_X2 || y < ZOOM_Y1 || y2 > ZOOM_Y2)
    {
        if (compressedFlag)
        {
            if (colorRemapTable)
            {
                vga_back.put_bitmap_area_trans_remap_decompress(x, y, bitmapPtr, MAX(ZOOM_X1, x) - x,
                                                                MAX(ZOOM_Y1, y) - y, MIN(ZOOM_X2, x2) - x,
                                                                MIN(ZOOM_Y2, y2) - y, colorRemapTable);
            }
            else
            {
                vga_back.put_bitmap_area_trans_decompress(x, y, bitmapPtr, MAX(ZOOM_X1, x) - x, MAX(ZOOM_Y1, y) - y,
        ...
        else
        {
            if (colorRemapTable)
            {
                vga_back.put_bitmap_area_trans_remap(x, y, bitmapPtr, MAX(ZOOM_X1, x) - x, MAX(ZOOM_Y1, y) - y,
...
{
#if (LOCATE_WIDTH == LOCATE_HEIGHT && ZOOM_LOC_X_WIDTH == LOCATE_WIDTH && ZOOM_LOC_Y_WIDTH == -LOCATE_HEIGHT &&        \
     ZOOM_Z_WIDTH == 0)
...
int ZoomMatrix::calc_zoom_y(int curX, int curY, int curZ)
{
    ...
    return ((curX - (top_x_loc << LOCATE_WIDTH_SHIFT) + curY - (top_y_loc << LOCATE_HEIGHT_SHIFT)) >> 1) - curZ -
...
int ZoomMatrix::calc_abs_x(int curX, int curY, int curZ)
{
    ...
    return ((curX * ZOOM_LOC_X_WIDTH * LOCATE_HEIGHT + curY * ZOOM_LOC_Y_WIDTH * LOCATE_WIDTH)
            // / (LOCATE_WIDTH *  LOCATE_HEIGTH)
...
int ZoomMatrix::calc_abs_y(int curX, int curY, int curZ)
{
    ...
    return ((curX * ZOOM_LOC_X_HEIGHT * LOCATE_HEIGHT + curY * ZOOM_LOC_Y_HEIGHT * LOCATE_WIDTH) >>
            (LOCATE_WIDTH_SHIFT + LOCATE_HEIGHT_SHIFT)) +
           curZ * ZOOM_Z_HEIGHT;
#endif
}
...
{
    return top_x_loc * ZOOM_LOC_X_WIDTH + top_y_loc * ZOOM_LOC_Y_WIDTH + (ZOOM_LOC_X_WIDTH + ZOOM_LOC_Y_WIDTH) / 2;
    // last term is because top_x_loc, and top_y_loc points to the
...
{
    return top_x_loc * ZOOM_LOC_X_HEIGHT + top_y_loc * ZOOM_LOC_Y_HEIGHT + (ZOOM_LOC_X_HEIGHT + ZOOM_LOC_Y_HEIGHT) / 2;
    // last term is because top_x_loc, and top_y_loc points to the
    // center of a Location
}
...
int ZoomMatrix::get_detect_location(int scrnX, int scrnY, int *locX, int *locY, char *mobileType)
{
    ...
    } while (locAX < max_x_loc && locAY < max_y_loc || locBX < max_x_loc && locBY < max_y_loc);
...
int ZoomMatrix::get_close_location(int scrnX, int scrnY, int *locX, int *locY, char *mobileType)
{
    ...
    int highAbsX = calc_abs_x((highX + 1) * LOCATE_WIDTH, (highY + 1) * LOCATE_HEIGHT,
...
//
void ZoomMatrix::put_bitmap_offset(int curX, int curY, int curZ, char *bitmapPtr, int offsetX, int offsetY,
                                   short *colorRemapTable, int mirror, int compressed)
{
    ...
    else
    {
        ...
        case 0: // uncompressed
            ...
            {
                if (!colorRemapTable)
                    vga_back.put_bitmap_area_trans(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
                else
                    vga_back.put_bitmap_area_trans_remap(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2,
                                                         colorRemapTable);
            }
            else
            {
                if (!colorRemapTable)
                    vga_back.put_bitmap_area_trans_hmirror(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
                else
                    vga_back.put_bitmap_area_trans_remap_decompress_hmirror(x1, y1, bitmapPtr, srcX1, srcY1, srcX2,
                                                                            srcY2, colorRemapTable);
            }
            break;
        case 1: // compressed
            if (!mirror)
            {
                if (!colorRemapTable)
                    vga_back.put_bitmap_area_trans_decompress(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
                else
                    vga_back.put_bitmap_area_trans_remap_decompress(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2,
                                                                    colorRemapTable);
            }
            else
            {
                if (!colorRemapTable)
                    vga_back.put_bitmap_area_trans_decompress_hmirror(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
                else
                    vga_back.put_bitmap_area_trans_remap_decompress_hmirror(x1, y1, bitmapPtr, srcX1, srcY1, srcX2,
                                                                            srcY2, colorRemapTable);
            }
        ...
        case 2: // uncompressed, blending
            ...
            if (!mirror)
            {
                vga_back.put_bitmap_blend_area(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
            }
            else
            {
                vga_back.put_bitmap_blend_area_hmirror(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
            }
            break;
        ...
        case 1 | 4: // compressed
            ...
            {
                if (!colorRemapTable)
                    vga_back.put_bitmap_area_half_decompress(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
                else
                    vga_back.put_bitmap_area_half_remap_decompress(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2,
                                                                   colorRemapTable);
            }
            else
            {
                if (!colorRemapTable)
                    vga_back.put_bitmap_area_half_decompress_hmirror(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
                else
                    vga_back.put_bitmap_area_half_remap_decompress_hmirror(x1, y1, bitmapPtr, srcX1, srcY1, srcX2,
                                                                           srcY2, colorRemapTable);
            }
            break;
        case 2 | 4: // uncompressed, blending
            ...
            if (!mirror)
            {
                vga_back.put_bitmap_weak_blend_area(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
            }
            else
            {
                vga_back.put_bitmap_weak_blend_area_hmirror(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
            }
            break;
        default:
            err_here();
...
//
void ZoomMatrix::put_bitmapW_offset(int curX, int curY, int curZ, short *bitmapPtr, int offsetX, int offsetY,
                                    int mirror, int compressed)
{
    ...
    else
    {
        ...
        case 0: // uncompressed
            if (!mirror)
            {
                vga_back.put_bitmapW_area_trans(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
            }
            else
            {
                err_here();
        ...
        case 0 | 8: // uncompressed, draw black mask
            if (!mirror)
            {
                vga_back.put_bitmapW_area_trans_blacken(x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
            }
            else
            {
                err_here();
...
#define UVMULTIPLIER_SHIFT 2
    ...
    long vz = ((c30 * u + c20 * UVMULTIPLIER) * u + c10 * UVMULTIPLIER * UVMULTIPLIER) * u /
                  (UVMULTIPLIER * UVMULTIPLIER * UVMULTIPLIER) +
              ((c03 * v + c02 * UVMULTIPLIER) * v + c01 * UVMULTIPLIER * UVMULTIPLIER) * v /
                  (UVMULTIPLIER * UVMULTIPLIER * UVMULTIPLIER) +
              ((c21 * u + c12 * v) + c11 * UVMULTIPLIER) * u * v / (UVMULTIPLIER * UVMULTIPLIER * UVMULTIPLIER) + c00;
    // du1z = 3*c30*u^2 + 2*c21*u*v + c22*v^2 + 2*c20*u + c11*v + c10
    long vdu1zM2 = // dz/du at (u,v) multiplied by 2 * C_MULTIPLIER
        (((c30 * u * 3 + c20 * 2 * UVMULTIPLIER) * u + (c21 * u * 2 + c12 * v + c11 * UVMULTIPLIER) * v) /
             (UVMULTIPLIER * UVMULTIPLIER) +
         c10) *
        2;
    // du2z = 6*c30*u + 2*c21*v + 2*c20
...
static int test_inside_polygon(int x, int y, POINT *vertex, int verCount)
{
    ...
    {
        s = (y - vertex->y) * (vertex[1].x - vertex->x) - (x - vertex->x) * (vertex[1].y - vertex->y);
        if (s >= 0)
            ++pos;
        if (s <= 0)
            --neg;
...
static void scroll_buffer(short *imageBuf, int truePitch, int winX1, int winY1, int winX2, int winY2, int shiftX,
                          int shiftY, int fillColor)
{
...
void ZoomMatrix::disp_rally_point()
{
    ...
    if (flag)
    {
        ...
        short chOffsetX = -29;
        short chOffsetY = -45;
        put_bitmap_offset(curX, curY, curZ, chPtr, chOffsetX, chOffsetY, NULL, 0, 0);
    }
}
