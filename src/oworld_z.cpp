...
static void scroll_buffer(short *imageBuf, int truePitch, int srcX1, int srcY1,
                          int srcX2, int srcY2, int shiftX, int shiftY,
                          int fillColor = -1);
...
static int hidden_by_place(int xLoc, int yLoc, int spriteWidth,
                           int spriteHeight);
...
static void draw_unit_path_on_zoom_map(int displayLayer) {
  ...
  for (i = unit_array.size(); i > 0; --i) {
    ...
    if ((config.show_unit_path & 1) && unitPtr->selected_flag &&
        (config.show_ai_info || !playerNation ||
         unitPtr->is_nation(playerNation)) &&
    ...
    // ------ show unit targeting our nation  ----------//
             ...
             unitPtr->cur_action !=
                 SPRITE_ATTACK // if already attack, not need to show path
    ...
    if (unitPtr->cur_x != unitPtr->go_x || unitPtr->cur_y != unitPtr->go_y) {
      world.zoom_matrix->get_zoom_pos(
          unitPtr->cur_x_loc(), unitPtr->cur_y_loc(), lineFromX, lineFromY);
      world.zoom_matrix->get_zoom_pos(unitPtr->go_x_loc(), unitPtr->go_y_loc(),
                                      lineToX, lineToY);

      anim_line.draw_line(&vga_back, lineFromX, lineFromY, lineToX, lineToY, 1,
                          animSeries);
    }
    ...
    if (unitPtr->cur_path) {
      ...
      for (j = unitPtr->cur_path_result_id; j > 0; j--) {
        ...
        world.zoom_matrix->get_zoom_pos(locFromX, locFromY, lineFromX,
                                        lineFromY);
...
ZoomMatrix::ZoomMatrix()
    : land_disp_sort_array(sizeof(DisplaySort), 100),
  ...
  init(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, ZOOM_WIDTH, ZOOM_HEIGHT,
  ...
  int saveHeight =
      image_height + IMAGE_BUF_TOP_MARGIN + IMAGE_BUF_BOTTOM_MARGIN;
...
void ZoomMatrix::init_var() {
  ...
  center_x = calc_cur_x((image_x2 - image_x1) / 2, (image_y2 - image_y1) / 2) /
             LOCATE_WIDTH;
  center_y = calc_cur_y((image_x2 - image_x1) / 2, (image_y2 - image_y1) / 2) /
             LOCATE_HEIGHT;

  last_draw_state =
      (BYTE *)mem_resize(last_draw_state, max_x_loc * max_y_loc * sizeof(BYTE));
...
void ZoomMatrix::draw() {
  ...
  if (update_abs_x1 == last_abs_x1 && update_abs_y1 == last_abs_y1) {
    // no scrolling
  } else if (misc.is_touch(update_abs_x1, update_abs_y1, update_abs_x2,
                           update_abs_y2, last_abs_x1, last_abs_y1, last_abs_x2,
                           last_abs_y2)) {
    // scrolling
    scroll_buffer(((BitmapW *)save_image_buf)->get_ptr(),
                  ((BitmapW *)save_image_buf)->get_true_pitch(), 0, 0,
                  ((BitmapW *)save_image_buf)->get_width() - 1,
                  ((BitmapW *)save_image_buf)->get_height() - 1, // full buffer
                  last_abs_x1 - update_abs_x1, last_abs_y1 - update_abs_y1,
                  vga_back.translate_color(V_BLACK));
  } else {
    // complete new screen
    // fill black
    // vga_back.bar(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, V_BLACK);
    //		IMGbar( ((BitmapW *)save_image_buf)->get_ptr() , ((BitmapW
    //*)save_image_buf)->get_true_pitch(), 			IMAGE_BUF_LEFT_MARGIN,
    //IMAGE_BUF_TOP_MARGIN, 			image_width+IMAGE_BUF_LEFT_MARGIN-1,
    //image_height+IMAGE_BUF_TOP_MARGIN-1, 0 );

    // skip filling if window is complete inside map
    if (disp_rect_type == 0 &&
            (top_x_loc < 0 || top_x_loc + disp_x_loc > max_x_loc ||
             top_y_loc < 0 || top_y_loc + disp_y_loc > max_y_loc) // rectangular
        || disp_rect_type == RHOMBUS_LOCATION &&
               (top_x_loc - disp_x_loc < 0 ||
                top_x_loc + disp_y_loc >= max_x_loc ||
                top_y_loc - disp_y_loc < 0 ||
                top_y_loc + disp_x_loc + disp_y_loc >=
                    max_y_loc)) // rhombus, this condition is prudent to prefer
                                // fill black
    {
      IMGbar(((BitmapW *)save_image_buf)->get_ptr(),
             ((BitmapW *)save_image_buf)->get_true_pitch(), 0, 0,
             ((BitmapW *)save_image_buf)->get_width() - 1,
             ((BitmapW *)save_image_buf)->get_height() - 1,
             vga_back.translate_color(V_BLACK));
    }
  }
  ...
  int cursorXLoc;
  int cursorYLoc;
  int cursorMarked = get_detect_location(mouse.cur_x, mouse.cur_y, &cursorXLoc,
                                         &cursorYLoc, NULL);
#endif
  ...
  do // outer loop
  {
    ...
    do {
      ...
      if (xLoc >= 0 && xLoc < max_x_loc && yLoc >= 0 && yLoc < max_y_loc) {
        ...
        int locAbsX1 =
            calc_abs_x(xLoc * LOCATE_WIDTH, (yLoc + 1) * LOCATE_HEIGHT,
                       locCorner.bottom_left->get_altitude());
        int locAbsY1 = calc_abs_y(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT,
                                  locCorner.top_left->get_altitude()) +
                       locCorner.loc_ptr->min_y;
        int locAbsX2 =
            calc_abs_x((xLoc + 1) * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT,
                       locCorner.top_right->get_altitude()) -
            1;
        int locAbsY2 =
            locAbsY1 - locCorner.loc_ptr->min_y + locCorner.loc_ptr->max_y;

        BYTE newDrawState;
        // see also omatrix2.cpp LocationCorner::render
        // ####### begin Gilbert 17/5 #######//
        if (!config.blacken_map ||
            (config.explore_whole_map && !config.fog_of_war))
        ...
        if (misc.is_touch(locAbsX1, locAbsY1, locAbsX2, locAbsY2, update_abs_x1,
                          update_abs_y1, update_abs_x2, update_abs_y2) &&
            (!misc.is_touch(locAbsX1, locAbsY1, locAbsX2, locAbsY2, last_abs_x1,
                            last_abs_y1, last_abs_x2, last_abs_y2) ||
             topRow + rowCount < last_top_row ||
             topRow + rowCount > last_bottom_row ||
             leftCol + colCount < last_left_col ||
             leftCol + colCount > last_right_col ||
             *lastDrawStatePtr != newDrawState))
        // so locations are simply not scanned in the loop last time, but inside
        // last_abs_x/y
        {
          if ((*lastDrawStatePtr = newDrawState) != FULL_DARK_MASK_ID) {
            ...
            TerrainInfo *terrainInfo =
                terrain_res[locCorner.loc_ptr->terrain_id];
            unsigned char cellId = (yLoc & 15) << 4 | (xLoc & 15);
            char *textureBitmap = NULL;

            if (locCorner.loc_ptr->is_road()) {
              TerrainInfo *roadInfo =
                  terrain_res[locCorner.loc_ptr->road_terrain_id];
              if (roadInfo->lower_type == roadInfo->higher_type)
                textureBitmap =
                    terrain_res.get_texture(roadInfo->lower_type, cellId)
                        ->bitmap_ptr;
              else {
                if (terrainInfo->lower_type == terrainInfo->higher_type) {
                  textureBitmap =
                      terrain_res.get_texture(terrainInfo->lower_type, cellId)
                          ->bitmap_ptr;
                } else {
                  textureBitmap = terrain_res.mask_texture(
                      terrainInfo->bitmap_ptr, // mask
                      terrain_res.get_texture(terrainInfo->lower_type, cellId)
                          ->bitmap_ptr,
                      terrain_res.get_texture(terrainInfo->higher_type, cellId)
                          ->bitmap_ptr);
                }

                terrainInfo = terrain_res[locCorner.loc_ptr->road_terrain_id];
                textureBitmap = terrain_res.mask_texture(
                    roadInfo->bitmap_ptr, textureBitmap,
            ...
            //#ifdef DEBUG
            //						if( !cursorMarked || cursorXLoc != xLoc || cursorYLoc
            //!= yLoc ) #endif
            if (newDrawState == FULL_BRIGHT_MASK_ID ||
                newDrawState == HALF_BRIGHT_MASK_ID) {
              locCorner.render(
                  (BitmapW *)save_image_buf, textureBitmap,
                  calc_zoom_x(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, 0) +
                      IMAGE_BUF_LEFT_MARGIN, // do not pass z
                  calc_zoom_y(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, 0) +
                      IMAGE_BUF_TOP_MARGIN,
                  newDrawState);
            } else {
              locCorner.render_fog(
                  (BitmapW *)save_image_buf, textureBitmap,
                  calc_zoom_x(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, 0) +
                      IMAGE_BUF_LEFT_MARGIN, // do not pass z
                  calc_zoom_y(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, 0) +
            ...
            locCorner.render_special(
                (BitmapW *)save_image_buf, 1, NULL, V_BLACK, V_BLACK,
                calc_zoom_x(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, 0) +
                    IMAGE_BUF_LEFT_MARGIN, // do not pass z
                calc_zoom_y(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, 0) +
                    IMAGE_BUF_TOP_MARGIN);
        ...
        zoomY1 = calc_zoom_y(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT,
                             locCorner.top_left->altitude) +
                 locCorner.loc_ptr->min_y; // not get_altitude
        ...
        if (sys.testing_session && debug2_enable_flag) {
          int scrnX0 =
              ZOOM_X1 + calc_zoom_x(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT,
                                    locCorner.top_left->altitude);
          int scrnY0 =
              ZOOM_Y1 + calc_zoom_y(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT,
                                    locCorner.top_left->altitude);
          int scrnX1 = ZOOM_X1 + calc_zoom_x((xLoc + 1) * LOCATE_WIDTH,
                                             yLoc * LOCATE_HEIGHT,
                                             locCorner.top_right->altitude);
          int scrnY1 = ZOOM_Y1 + calc_zoom_y((xLoc + 1) * LOCATE_WIDTH,
                                             yLoc * LOCATE_HEIGHT,
                                             locCorner.top_right->altitude);
          int scrnX2 = ZOOM_X1 + calc_zoom_x(xLoc * LOCATE_WIDTH,
                                             (yLoc + 1) * LOCATE_HEIGHT,
                                             locCorner.bottom_left->altitude);
          int scrnY2 = ZOOM_Y1 + calc_zoom_y(xLoc * LOCATE_WIDTH,
                                             (yLoc + 1) * LOCATE_HEIGHT,
                                             locCorner.bottom_left->altitude);
      ...
      if (zoomY1 < minZoomY1)
        minZoomY1 =
            zoomY1; // if any of this row is inside the screen, draw next row
    ...
    if (colCount > maxColCount)
      maxColCount = colCount;
           ...
           ZOOM_HEIGHT); // if all square outside ZOOM_Y2, stop drawing
  ...
  vga_back.put_bitmapW_area(image_x1 - IMAGE_BUF_LEFT_MARGIN,
                            image_y1 - IMAGE_BUF_TOP_MARGIN, save_image_buf,
                            IMAGE_BUF_LEFT_MARGIN, IMAGE_BUF_TOP_MARGIN,
                            image_width + IMAGE_BUF_LEFT_MARGIN - 1,
                            image_height + IMAGE_BUF_TOP_MARGIN - 1);
...
void ZoomMatrix::disp() {
  ...
  if (save_image_buf) {
    ...
    // just_drawn_flag = 1, don't redraw because it have been saved to
    // save_image_buf
...
void ZoomMatrix::draw_white_site() {
  /*
          ...
          int maxXLoc = top_x_loc + disp_x_loc;        // divide by 2 for
     world_info int maxYLoc = top_y_loc + disp_y_loc;
     ...
     )
          {
                  ...
                  for( x=image_x1,xLoc=top_x_loc ; xLoc<maxXLoc ; xLoc++,
     x+=loc_width, locPtr++ )
                  {
                          if(locPtr->unit_recno(UNIT_LAND) ||
     locPtr->unit_recno(UNIT_SEA) || locPtr->unit_recno(UNIT_AIR)) vga_back.bar(
     x, y, x+31, y+31, V_WHITE );
                  }
          }
          */
}
...
void ZoomMatrix::draw_magic_effects() {
  ...
  if (magic.effect_testing_flag == 1)
    magic.draw_magic_one(&vga_back, ZOOM_X2 / 2, ZOOM_Y2 / 2,
                         vga.pixel_format_flag, init_magic2);
  //	if (magic.effect_testing_flag == 2)
  //		magic.draw_magic_two(&vga_back, ZOOM_X2/2, ZOOM_Y1+50, ZOOM_X2/2,
  //ZOOM_Y2-150, 15, 								vga.pixel_format_flag, init_magic2);
  if (magic.effect_testing_flag == 3)
    magic.draw_magic_three(&vga_back, ZOOM_X2 / 2, ZOOM_Y1 + 50, ZOOM_X2 / 2,
                           ZOOM_Y2 - 100, vga.pixel_format_flag, init_magic2);
  if (magic.effect_testing_flag == 4)
    magic.draw_magic_four(&vga_back, ZOOM_X2 / 2, ZOOM_Y1 + 70, ZOOM_X2 / 2,
                          ZOOM_Y2 - 70, vga.pixel_format_flag, init_magic2);
  if (magic.effect_testing_flag == 5)
    magic.draw_magic_five(&vga_back, ZOOM_X2 / 2, ZOOM_Y2 / 2 - 60, ZOOM_X2 / 2,
                          ZOOM_Y2 / 2 + 100, vga.pixel_format_flag,
                          init_magic2);
  if (magic.effect_testing_flag == 6)
    magic.draw_magic_six(&vga_back, ZOOM_X2 / 2, ZOOM_Y1 + 50, ZOOM_X2 / 2,
                         ZOOM_Y2 - 100, vga.pixel_format_flag, init_magic2);
  if (magic.effect_testing_flag == 7)
    magic.draw_magic_seven(&vga_back, ZOOM_X2 / 2, ZOOM_Y1 + 50, ZOOM_X2 / 2,
                           ZOOM_Y2 - 100, vga.pixel_format_flag, init_magic2);
  if (magic.effect_testing_flag == 8)
    magic.draw_magic_eight(&vga_back, ZOOM_X2 / 2, ZOOM_Y1 + 50, ZOOM_X2 / 2,
                           ZOOM_Y2 - 100, vga.pixel_format_flag, init_magic2);
}
...
void ZoomMatrix::draw_weather_effects() {
  ...
  if (weather.is_quake()) {
    if (vibration == -1) {
      ...
      vibration =
          weather.quake_rate(top_x_loc + center_x, top_y_loc + center_y) * 16 /
          100;
  ...
  if (newRainScale != init_rain) {
    ...
    if (newRainScale) {
      rain.start_rain(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, newRainScale,
                      weather.wind_speed() * sin(weather.wind_direct_rad()) /
                          100.0);
      ...
      if (rain_channel_id == 0) // from no rain to rain
      {
        if (config.sound_effect_flag && config.rain_audio) {
          //	rain_channel_id =
          //audio.play_loop_wav(DIR_SOUND"RAIN.WAV",11008*2,
          //RelVolume(relVolume,0));
          RelVolume r(relVolume, 0);
          rain_channel_id =
              audio.play_loop_wav(DIR_SOUND "RAIN.WAV", 0, DsVolume(r));
        }
      } else {
        // changing rain
        if (config.sound_effect_flag && config.rain_audio) {
  ...
  if (world.lightning_signal >= 105 && world.lightning_signal <= 108) {
    if (!init_lightning) {
      ...
      // play sound
      if (world.lightning_signal == 108 && config.sound_effect_flag &&
          config.lightning_audio) {
      ...
      lightning_x1 =
          Lightning::bound_x1 + 20 +
          short(mRandom % (Lightning::bound_x2 - Lightning::bound_x1 - 40));
      lightning_y1 = Lightning::bound_y1 - 50;
      lightning_x2 =
          Lightning::bound_x1 + 10 +
          short(mRandom % (Lightning::bound_x2 - Lightning::bound_x1 - 20));
      lightning_y2 =
          (Lightning::bound_y1 + Lightning::bound_y2) / 2 +
  ...
  } else if (config.lightning_visual && init_lightning >= 104 &&
             init_lightning <= 107) {
    newBrightness = maxBrightness;
  } else if (config.lightning_visual && init_lightning >= 101 &&
             init_lightning <= 103) {
  ...
  if (windSpeed >= 20) {
    ...
    if (wind_channel_id == 0) {
      if (config.sound_effect_flag && config.wind_audio) {
        // ###### begin Gilbert 6/8 #######//
        // wind_channel_id = audio.play_loop_wav(DIR_SOUND"WIND.WAV",0,
        // relVolume); wind_channel_id =
        // audio.play_loop_wav(DIR_SOUND"WIND.WAV",25088*2,
        // RelVolume(relVolume,0));  // 25088 samples, 8-bit stereo, so *2
        RelVolume r(relVolume, 0);
        wind_channel_id =
            audio.play_loop_wav(DIR_SOUND "WIND.WAV", 0, DsVolume(r));
        // ###### end Gilbert 6/8 #######//
      }
    } else {
...
void ZoomMatrix::draw_build_marker() {
  if (!(mouse.cur_x >= ZOOM_X1 &&
        mouse.cur_x <= ZOOM_X2 && // if the mouse is inside the zoom area
  ...
  if (power.command_id == COMMAND_BUILD_FIRM) {
    ...
    validAction = world.can_build_firm(xLoc, yLoc, power.command_para,
    ...
    validAction =
        (~nation_array)->cash >= firmInfo->setup_cost &&
        (~nation_array)->live_points >= firmInfo->setup_live_points_cost;
    // ###### begin Gilbert 22/4 ########//
    if (validAction) {
      validAction = FirmMonsterFortress::can_build_fortress(
          unit_array.selected_recno, xLoc, yLoc, NULL);
    }
    // ###### end Gilbert 22/4 ########//
  }
  ...
  /*
          ...
          else if( power.command_id == COMMAND_BUILD_WALL )
          {
                  ...
                  validAction = world.can_build_wall(xLoc, yLoc,
     nation_array.player_recno)
                          || ( locPtr->is_wall() &&
     locPtr->is_wall_destructing() && world.can_destruct_wall(xLoc, yLoc,
     nation_array.player_recno) );
          }
          ...
          else if( power.command_id == COMMAND_DESTRUCT_WALL )
          {
                  ...
                  validAction = world.can_destruct_wall(xLoc, yLoc,
     nation_array.player_recno)
  ...
  */
  ...
  // ###### end Gilbert 5/3 ########//
    ...
    FirmMagic *firmMagic =
        firm_array[power.command_firm_recno]->cast_to_FirmMagic();
    if (!firmMagic)
      return;
    ...
    if (power.command_para == MAGIC_DESTROY_BUILDING) // magic id
    {
      ...
      if (locPtr->is_firm()) {
        ...
        validAction = nation_array.should_attack(nation_array.player_recno,
    ...
    if (!unit_array.is_deleted(FirmMagic::teleport_unit_recno)) {
      ...
      locHeight = unitPtr->loc_height;
      validAction =
          world.check_unit_space(xLoc, yLoc, xLoc + locWidth - 1,
                                 yLoc + locHeight - 1, unitPtr->mobile_type);
    } else {
      validAction = 0;
      locWidth = locHeight = 1;
    }
  ...
  for (yLoc = locY1; yLoc <= locY2; ++yLoc) {
    for (xLoc = locX1; xLoc <= locX2; ++xLoc) {
      ...
      if (xLoc >= innerX1 && xLoc <= innerX2 && yLoc >= innerY1 &&
          yLoc <= innerY2) {
      ...
      put_bitmapW_offset(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT,
                         lc.top_left->altitude, maskBitmap.bitmap_ptr(),
                         maskBitmap.left_edge, maskBitmap.top_edge, 0, 0);
    }
  }
  // ###### end Gilbert 3/11 #####//
...
#define GOD_CAST_RANGE_COLOR V_WHITE
  ...
  MIN(y2,ZOOM_Y2), GOD_CAST_RANGE_COLOR );
                  }
          }
  }
  */
...
void ZoomMatrix::disp_text() {
  ...
  for (i = town_array.size(); i > 0; i--) {
    ...
    //-- if config.disp_extend_info==1, only show extended info when the mouse
    //is over the object ---//
    ...
    if (zoomX < -50 || zoomX > image_width + 50 || zoomY < 0 ||
        zoomY > image_height + 36)
      continue;
    ...
    if (dispExtendInfo) {
      ...
      if (townPtr->is_pay_tribute_to_monster())
        put_center_text(x, y, z,
                        text_firm.str_race_slave_town(
                            race_res[townPtr->race_id]->adjective));
      else
        put_center_text(
            x, y, z,
            text_firm.str_race_town(race_res[townPtr->race_id]->adjective));
      ...
      if (townPtr->is_own() || townPtr->nation_recno == 0) {
        ...
        put_center_text(
            x, y, z,
            text_firm.str_town_info_pop(townPtr->jobless_population,
                                        townPtr->worker_population()));
        z -= 16;
      }
    }
...
#if (defined(CHINESE))
      put_center_text((curX1 + curX2) / 2 + 16, (curY1 + curY2) / 2 + 16, curZ,
                      firmPtr->firm_name());
#else
      put_center_text((curX1 + curX2) / 2, (curY1 + curY2) / 2, curZ,
                      firmPtr->firm_name());
#endif
    }
    ...
    if (firmPtr->cast_to_FirmCamp()) {
      ...
      for (int i = 0; i < firmCamp->soldier_count; ++i) {
        if ((spyRecno = firmCamp->soldier_array[i].spy_recno) &&
            spy_array[spyRecno]->true_nation_recno ==
                nation_array.player_recno) {
          ++spyCount;
          break;
        }
      ...
      if (dispExtendInfo) {
        ...
        // ###### end Gilbert 10/2 #####//
        {
          ...
          //					put_center_text( x, y, z,
          //						text_unit.str_race_title(
          //unitPtr->race_id, 						(unitPtr->is_human() ?
          //race_res[unitPtr->race_id]->adjective :
          //monster_res[unitPtr->monster_id()]->name), 						unitPtr->rank_id ) );
          put_center_text(
              x, y, z,
              text_unit.str_race_title(
                  unitPtr->race_id,
                  (unitPtr->is_human()
                       ? race_res[unitPtr->race_id]->adjective
                       : monster_res[unitPtr->monster_id()]->adjective),
                  unitPtr->rank_id));
          ...
          put_center_text(x, y, z,
                          text_firm.str_no_commander()); //"No Commander" );
          z -= 16;
        }
        /*
                                str  = "Soldiers: ";
                                ...
                                if( firmCamp->cast_to_FirmFort() &&
                                         firmCamp->cast_to_FirmFort()->current_archer_count
           > 0 )
                                {
                                        str += " Archers: ";
                                        str +=
    ...
    if (firmPtr->cast_to_FirmFactory()) {
      ...
      if (dispExtendInfo) {
        if (firmFactory->under_construction) {
          ...
          //						str = "Producing ";
          //						str +=
          //raw_res[firmFactory->product_raw_id]->name; 						str += " Ware";
          //					}
          //					else
          //						str = "No Ware Under
          //Production";
          put_center_text(
              x, y, z, text_firm.str_factory_info(firmFactory->product_raw_id));
          z -= 16;
        }
      }
    }
...
//
void ZoomMatrix::put_center_text(int x, int y, int z, const char *str,
                                 char black, Font *font_ptr) {
  ...
  font_ptr->put_to_bufferW(tempBuffer.ptr->get_ptr(),
                           tempBuffer.buf_true_pitch(), 0, 0, str);
  //	font_news.put_to_bufferW(tempBuffer.ptr->get_ptr(),
  //tempBuffer.buf_true_pitch(), 0, 0, str);
  ...
  if (scrnX1 <= image_x2 && scrnX2 >= image_x1 && scrnY1 <= image_y2 &&
      scrnY2 >= image_y1) {
    // clip
    ...
    put_bitmapW_offset(x, y, z, tempBuffer.bitmap_ptr(), tempBuffer.left_edge,
                       tempBuffer.top_edge, 0, 0);
  }
}
...
void ZoomMatrix::blacken_unexplored() {
  ...
  for( y = topLoc; y <= bottomLoc; ++y, scrnY += ZOOM_LOC_HEIGHT)
  {
          ...
          for( x = leftLoc; x <= rightLoc; ++x, scrnX += ZOOM_LOC_WIDTH )
          {
                  ...
                  {
                          northRow = (northRow << 1) |
  ((++northRowLoc)->explored() ? 1 : 0); thisRow  = (thisRow  << 1) |
  ((++thisRowLoc )->explored() ? 1 : 0); southRow = (southRow << 1) |
  ((++southRowLoc)->explored() ? 1 : 0);
                  }
                  else
                  {
                          // replicate bit 1
                  ...
                  {
                          explored_mask.draw(scrnX, scrnY, northRow, thisRow,
  southRow);
                  }
                  else
                  {
...
void ZoomMatrix::blacken_fog_of_war() {
  ...
  ++thisRowLoc )
                  {
                          ...
                          {
                                  vga_back.bar(scrnX, scrnY,
  scrnX+ZOOM_LOC_WIDTH-1, scrnY+ZOOM_LOC_HEIGHT-1, 0);
                          }
                          else
                          {
                                  ...
                                  if( v < MAX_VISIT_LEVEL-7)
                                  {
                                          ...
                                          vga_back.pixelize_32x32(scrnX+1,
  scrnY, 0); vga_back.pixelize_32x32(scrnX, scrnY+1, 0);
                                  }
                                  // for visibility >= MAX_VISIT_LEVEL, draw
  nothing
                          }
                  }
          }
  ...
  else
  {
          ...
          for( y = topLoc; y <= bottomLoc; ++y, scrnY += ZOOM_LOC_HEIGHT)
          {
                  thisRowLoc = get_loc(leftLoc, y);
                  northRowLoc = y > 0 ? get_loc(leftLoc, y-1) : thisRowLoc;
                  southRowLoc = y+1 < max_y_loc ? get_loc(leftLoc, y+1):
  thisRowLoc;
  ...
  )
                  {
                          ...
                          // if on the east of the map, simply replicate the
  eastest square
                          ...
                          midNorthRow[2] = MIN( MIN(northRow[2], northRow[1]),
  midThisRow[2] ); midNorthRow[1] = MIN( northRow[1], thisRow[1]);
                          midNorthRow[0] = MIN( MIN(northRow[0], northRow[1]),
  midThisRow[0] ); midSouthRow[2] = MIN( MIN(southRow[2], southRow[1]),
  midThisRow[2] ); midSouthRow[1] = MIN( southRow[1], thisRow[1]);
                          midSouthRow[0] = MIN( MIN(southRow[0], southRow[1]),
  midThisRow[0] ); unsigned char midMean = ((int) thisRow[0] + thisRow[2] +
                                  northRow[0] + northRow[1] + northRow[2] +
                                  southRow[0] + southRow[1] + southRow[2] ) /8;
                          midThisRow[1] = MIN(thisRow[1], midMean );

                          vga_back.fog_remap(scrnX, scrnY, (char
  **)explored_mask.brightness_table->get_table_array(), midNorthRow, midThisRow,
  midSouthRow);
                  }
          }
  }
...
#define DRAW_LEFT_RIGHT 6
  ...
  //---- add the objects on the zoom area to land_disp_sort_array in a sorted
  //display order ---//
  ...
  for (rowCount = -DRAW_UPSIDE * 2; rowCount <= maxRowCount;
       (rowCount & 1 ? ++rowXLoc : ++rowYLoc), rowCount++) {
    int xLoc = rowXLoc;
    int yLoc = rowYLoc;

    for (colCount = -DRAW_LEFT_RIGHT * 2; colCount <= maxColCount;
         ++xLoc, --yLoc, colCount += 2) {
      if (xLoc >= 0 && xLoc < max_x_loc && yLoc >= 0 && yLoc < max_y_loc) {
        LocationCorners lc;
        get_loc_corner(xLoc, yLoc, &lc);
        ...
        if (locPtr->unit_recno(UNIT_AIR)) {
          ...
          if (!unitPtr->is_stealth() && unitPtr->next_x_loc() == xLoc &&
              unitPtr->next_y_loc() == yLoc) {
            ...
            displaySort.object_y2 =
                calc_abs_y(unitPtr->cur_x + LOCATE_WIDTH / 2,
                           unitPtr->cur_y + LOCATE_HEIGHT / 2, 0);
#else
#error
#endif
            // ##### begin Gilbert 19/9 #######//
            // if(pLoc == locPtr && pMobileType == UNIT_AIR)	// BUGHERE : this
            // part may fails if sprite size > 1x1
            if (pLoc &&
                pLoc->unit_recno(pMobileType) == displaySort.object_recno) {
...
#error
#endif
            // if( pLoc == locPtr && (pMobileType == UNIT_LAND || pMobileType ==
            // UNIT_SEA)) // BUGHERE : this part may fails if sprite size > 1x1
            if (pLoc && pLoc->unit_recno(pMobileType) &&
                pLoc->unit_recno(pMobileType) == displaySort.object_recno) {
              displaySort.object_type = OBJECT_POINTED_UNIT;
              displaySort.draw_flags |= OBJECT_DRAW_POINTED; // outline flag
            }
        ...
        //--------- if there is a firm on the location --------//
          ...
          if (!firmPtr->is_stealth() && xLoc == firmPtr->loc_x1 &&
              yLoc == firmPtr->loc_y1) {
            memset(&displaySort, 0, sizeof(displaySort));
...
#error
#endif
            // ##### begin Gilbert 19/9 ######//
            if (pLoc && pLoc->is_firm() &&
                pLoc->firm_recno() == displaySort.object_recno) {
        ...
        //------ if there is a town section on the location -----//
          ...
          if (!townPtr->is_stealth() && xLoc == townPtr->loc_x1 &&
              yLoc == townPtr->loc_y1) {
            memset(&displaySort, 0, sizeof(displaySort));
...
#error
#endif
            // ##### begin Gilbert 19/9 ######//
            if (pLoc && pLoc->is_town() &&
                pLoc->town_recno() == displaySort.object_recno) {
              displaySort.draw_flags |= OBJECT_DRAW_POINTED;
            }
            // ##### end Gilbert 19/9 ######//
        ...
        //------ if there is a plant on the location -----//
          ...
          // if( !config.blacken_map || world.is_explored(xLoc, yLoc, xLoc,
          // yLoc) )
          if (!config.blacken_map || locPtr->explored()) {
            ...
            // BUGHERE : drawY ???
#if (SORT_OBJECT_METHOD == 0)
            displaySort.object_y2 = yLoc * LOCATE_HEIGHT + innerY -
                                    LOCATE_HEIGHT / 2 + plantBitmap->offset_y +
                                    plantBitmap->bitmap_height - 1;
#elif (SORT_OBJECT_METHOD == 1)
            displaySort.object_y2 = calc_abs_y(
                xLoc * LOCATE_WIDTH + innerX, yLoc * LOCATE_HEIGHT + innerY, 0);
#else
#error
#endif
        ...
        //------ if there is a wall on the location -------//
          ...
          {
                  // -------- non-gate square
                  displaySort.object_y2 = yLoc* ZOOM_LOC_HEIGHT
          +ZOOM_LOC_HEIGHT-1; displaySort.x_loc = xLoc * ZOOM_LOC_WIDTH;
          ...
          else
          {
                  // -------- gate square ---------//
                  displaySort.object_y2 = yLoc * ZOOM_LOC_HEIGHT +
          wallInfo->offset_y + wallInfo->bitmap_height() -1; displaySort.x_loc =
          xLoc + wallInfo->loc_off_x; displaySort.y_loc = yLoc +
          wallInfo->loc_off_y; if( xLoc == MAX( displaySort.x_loc, zoomXLoc1) &&
                           yLoc == MAX( displaySort.y_loc, zoomYLoc1) )
                  {
                          displaySort.x_loc = xLoc * ZOOM_LOC_WIDTH +
          wallInfo->offset_x; displaySort.y_loc = yLoc * ZOOM_LOC_HEIGHT +
          wallInfo->offset_y; land_disp_sort_array.linkin(&displaySort);
                  }
          }
          */
        } else if (locPtr->has_hill() &&
                   hill_res[locPtr->hill_id1()]->layer & 2) {
          /*
          memset(&displaySort, 0, sizeof(displaySort));
          ...
          */
          ...
          if (!rockPtr->is_stealth() && xLoc == rockPtr->loc_x &&
              yLoc == rockPtr->loc_y) {
            memset(&displaySort, 0, sizeof(displaySort));
            ...
            displaySort.object_y2 =
                calc_abs_y((rockPtr->loc_x * 2 + rockInfo->loc_width + 1) *
                               LOCATE_WIDTH / 2,
                           (rockPtr->loc_y * 2 + rockInfo->loc_height + 1) *
                               LOCATE_HEIGHT / 2,
                           0);
#else
#error
        ...
        if (locPtr->has_site() &&
            locPtr->walkable(3)) // don't display if a building/object has
                                 // already been built on the location
        {
          Site *sitePtr = site_array[locPtr->site_recno()];

          if (!sitePtr->is_stealth() && xLoc == sitePtr->map_x_loc &&
              yLoc == sitePtr->map_y_loc) {
            memset(&displaySort, 0, sizeof(displaySort));
...
#elif (SORT_OBJECT_METHOD == 1)
            //						displaySort.object_y2 =
            //calc_abs_y(xLoc+LOCATE_WIDTH/2, 							yLoc+LOCATE_HEIGHT/2, 0);
            displaySort.object_y2 = calc_abs_y(
                (sitePtr->map_x_loc + sitePtr->map_x2 + 1) * LOCATE_WIDTH / 2,
                (sitePtr->map_y_loc + sitePtr->map_y2 + 1) * LOCATE_HEIGHT / 2,
                0);
#else
#error
#endif
            if (pLoc && pLoc->has_site() &&
                pLoc->site_recno() == displaySort.object_recno) {
              displaySort.draw_flags |= OBJECT_DRAW_POINTED;
            }

            if (sitePtr->map_x2 == sitePtr->map_x_loc &&
                sitePtr->map_y2 == sitePtr->map_y_loc &&
        ...
        // ###### begin Gilbert 21/9 ######## //
          ...
          if (!dirtPtr->is_stealth() && xLoc == dirtPtr->loc_x &&
              yLoc == dirtPtr->loc_y) {
            memset(&displaySort, 0, sizeof(displaySort));
            ...
            displaySort.object_y2 =
                calc_abs_y((dirtPtr->loc_x * 2 + rockInfo->loc_width + 1) *
                               LOCATE_WIDTH / 2,
                           (dirtPtr->loc_y * 2 + rockInfo->loc_height + 1) *
                               LOCATE_HEIGHT / 2,
                           0);
#else
#error
...
#endif
        {
          ...
          if (!config.blacken_map || locPtr->explored()) {
            ...
            displaySort.x_loc = xLoc;
            displaySort.y_loc = yLoc;
            int innerX = LOCATE_WIDTH / 2 +
                         (((xLoc + 11) * (yLoc + 13)) % (LOCATE_WIDTH / 2));
            int innerY = LOCATE_HEIGHT / 2 +
                         (((xLoc + 13) * (yLoc + 17)) % (LOCATE_HEIGHT / 2));
#if (SORT_OBJECT_METHOD == 0)
...
#error
  ...
  for (i = bullet_array.size(); i > 0; i--) {
    ...
    colCount = (bulletPtr->cur_x_loc() - top_x_loc) -
               (bulletPtr->cur_y_loc() - top_y_loc);
    rowCount = (bulletPtr->cur_x_loc() - top_x_loc) +
               (bulletPtr->cur_y_loc() - top_y_loc);
...
#elif (SORT_OBJECT_METHOD == 1)
      displaySort.object_y2 =
          calc_abs_y(bulletPtr->cur_x + LOCATE_WIDTH / 2,
...
#error
    ...
    case LAND_DISP_LAYER_MASK:
      ...
      // ---- calculate draw_flags ------//
      displaySort.draw_flags =
          hidden_by_place(bulletPtr->cur_x_loc(), bulletPtr->cur_y_loc(), 1, 1);
  ...
  for (i = tornado_array.size(); i > 0; i--) {
    ...
    if (tornadoPtr->cur_x_loc() < 0 || tornadoPtr->cur_y_loc() < 0 ||
        tornadoPtr->cur_x_loc() >= MAX_WORLD_X_LOC ||
        tornadoPtr->cur_y_loc() >= MAX_WORLD_Y_LOC)
      continue;

    if (tornadoPtr->is_stealth()) // check inside map before call is_stealth();
      continue;

    colCount = (tornadoPtr->cur_x_loc() - top_x_loc) -
               (tornadoPtr->cur_y_loc() - top_y_loc);
    rowCount = (tornadoPtr->cur_x_loc() - top_x_loc) +
               (tornadoPtr->cur_y_loc() - top_y_loc);
...
#elif (SORT_OBJECT_METHOD == 1)
    displaySort.object_y2 =
        calc_abs_y(tornadoPtr->cur_x + LOCATE_WIDTH / 2,
...
#error
  ...
  for (i = effect_array.size(); i > 0; i--) {
    ...
    colCount = (effectPtr->cur_x_loc() - top_x_loc) -
               (effectPtr->cur_y_loc() - top_y_loc);
    rowCount = (effectPtr->cur_x_loc() - top_x_loc) +
               (effectPtr->cur_y_loc() - top_y_loc);
...
#elif (SORT_OBJECT_METHOD == 1)
      displaySort.object_y2 = calc_abs_y(
          (firmDiePtr->parent_loc_x1 + firmDiePtr->parent_loc_x2 + 1) *
                  LOCATE_WIDTH / 2 +
              LOCATE_WIDTH / 4,
          (firmDiePtr->parent_loc_y1 + firmDiePtr->parent_loc_y2 + 1) *
                  LOCATE_HEIGHT / 2 +
              LOCATE_HEIGHT / 4,
          0);
#else
#error
  ...
  land_disp_sort_array.zap(
      0);                     // 0-don't resize the array, keep its current size
  ...
  if (dispFire > 0) {
    ...
    if (fire_channel_id == 0) {
      ...
      //	fire_channel_id = audio.play_loop_wav( DIR_SOUND"FIRE.WAV",8447
      //*2, RelVolume(relVolume,0));
      RelVolume r(relVolume, 0);
      fire_channel_id =
          audio.play_loop_wav(DIR_SOUND "FIRE.WAV", 0, DsVolume(r));
...
static RGBColor identity_color_function(RGBColor col, int, int) { return col; }
...
void ZoomMatrix::draw_objects_now(DynArray *unitArray, int displayLayer,
  ...
  for (i = 1; i <= dispCount; i++) {
    ...
    if (flags &&
        !(displaySortPtr->draw_flags &
          flags)) // draw on flags || (displaySortPtr->draw_flags & flags)
      continue;
    ...
    case OBJECT_POINTED_UNIT: {
      ...
      if (displaySortPtr->draw_flags & OBJECT_DRAW_POINTED ||
        ...
        int animLineColorSeries =
            2 + (unitPtr->selected_flag |
                 displaySortPtr->draw_flags & OBJECT_DRAW_POINTED);
        short curX1 = unitPtr->cur_x;
        short curY1 = unitPtr->cur_y;
        short curX2 =
            curX1 + unitPtr->sprite_info->loc_width * LOCATE_WIDTH - 1;
        short curY2 =
            curY1 + unitPtr->sprite_info->loc_height * LOCATE_HEIGHT - 1;
        ...
        if (displaySortPtr->draw_flags & OBJECT_DRAW_POINTED) {
          // draw a back line
          anim_line.draw_line(&vga_back, rectX0, rectY0, rectX0,
                              rectY0 + unitPtr->sprite_info->max_height, 1,
                              animLineColorSeries);
        }

        anim_line.draw_line(&vga_back, rectX0, rectY0, rectX1, rectY1, 1,
                            animLineColorSeries);
        anim_line.draw_line(&vga_back, rectX1, rectY1, rectX2, rectY2, 1,
                            animLineColorSeries);
        anim_line.draw_line(&vga_back, rectX2, rectY2, rectX3, rectY3, 1,
                            animLineColorSeries);
        anim_line.draw_line(&vga_back, rectX3, rectY3, rectX0, rectY0, 1,
                            animLineColorSeries);
        ...
        if (displaySortPtr->draw_flags & OBJECT_DRAW_POINTED) {
          // draw a back line
          anim_line.draw_line(&vga_back, rectX1, rectY1, rectX1,
                              rectY1 + unitPtr->sprite_info->max_height, 1,
                              animLineColorSeries);
          anim_line.draw_line(&vga_back, rectX2, rectY2, rectX2,
                              rectY2 + unitPtr->sprite_info->max_height, 1,
                              animLineColorSeries);
          anim_line.draw_line(&vga_back, rectX3, rectY3, rectX3,
                              rectY3 + unitPtr->sprite_info->max_height, 1,
                              animLineColorSeries);

          anim_line.draw_line(&vga_back, rectX0,
                              rectY0 + unitPtr->sprite_info->max_height, rectX1,
                              rectY1 + unitPtr->sprite_info->max_height, 1,
                              animLineColorSeries);
          anim_line.draw_line(&vga_back, rectX1,
                              rectY1 + unitPtr->sprite_info->max_height, rectX2,
                              rectY2 + unitPtr->sprite_info->max_height, 1,
                              animLineColorSeries);
          anim_line.draw_line(&vga_back, rectX2,
                              rectY2 + unitPtr->sprite_info->max_height, rectX3,
                              rectY3 + unitPtr->sprite_info->max_height, 1,
                              animLineColorSeries);
          anim_line.draw_line(&vga_back, rectX3,
                              rectY3 + unitPtr->sprite_info->max_height, rectX0,
                              rectY0 + unitPtr->sprite_info->max_height, 1,
                              animLineColorSeries);
        }
      } else {
    ...
    case OBJECT_BULLET:
      bullet_array[displaySortPtr->object_recno]->draw(
          0, displaySortPtr->draw_flags & flags);
      break;
    ...
    case OBJECT_FIRM: {
      ...
      if (displaySortPtr->draw_flags & OBJECT_DRAW_POINTED ||
        ...
        int animLineColorSeries =
            2 + ((firm_array.selected_recno == displaySortPtr->object_recno) |
                 displaySortPtr->draw_flags & OBJECT_DRAW_POINTED);
    ...
    case OBJECT_TOWN: {
      ...
      if (displaySortPtr->draw_flags & OBJECT_DRAW_POINTED ||
        ...
        int animLineColorSeries =
            2 + ((town_array.selected_recno == displaySortPtr->object_recno) |
                 displaySortPtr->draw_flags & OBJECT_DRAW_POINTED);
    ...
    case OBJECT_FIRE: {
      ...
      int innerX =
          LOCATE_WIDTH / 2 +
          (((displaySortPtr->x_loc + 11) * (displaySortPtr->y_loc + 13)) %
           (LOCATE_WIDTH / 2));
      int innerY =
          LOCATE_HEIGHT / 2 +
          (((displaySortPtr->x_loc + 13) * (displaySortPtr->y_loc + 17)) %
           (LOCATE_HEIGHT / 2));
      ...
      /*
              ...
              {
                      BYTE palBuf[256][3];
                      palFile.file_seek(8);     				// bypass the header
         info palFile.file_read(palBuf, sizeof(palBuf)); palFile.file_close();
                      ...
                      colorTable.generate_table_fast( 1, palBufDesc,
         identity_color_function ); colorRemapTable = (short
         *)colorTable.get_table(0);

              }
      */

      put_bitmap_offset(x1, y1, world.interpolate_z(x1, y1),
                        (char *)flame[f].bitmap, Flame::offset_x(f),
                        Flame::offset_y(f), flame[f].color_remap_table, 0, 0);
      //	put_bitmap_offset( x1, y1, world.interpolate_z(x1, y1), (char
      //*)flame[f].bitmap, 		Flame::offset_x(f), Flame::offset_y(f), 		NULL, 0 , 0);
    } break;
    ...
    case OBJECT_HILL: {
      ...
      if( dispPower && nationRecno > 0)
      {
              ...
              if( x1 >= 0 && y1 >= 0 && x1 < ZOOM_WIDTH - (ZOOM_LOC_WIDTH-1) &&
      y1 < ZOOM_HEIGHT - (ZOOM_LOC_HEIGHT-1))
              {
    ...
    case OBJECT_SITE: {
      ...
      if (displaySortPtr->draw_flags & OBJECT_DRAW_POINTED ||
          site_array.selected_recno == displaySortPtr->object_recno) {
        int animLineColorSeries =
            2 + ((site_array.selected_recno == displaySortPtr->object_recno) |
                 displaySortPtr->draw_flags & OBJECT_DRAW_POINTED);
      ...
      /*	int i =
         site_array[displaySortPtr->object_recno]->animation_count; if (i <100)
              {
                      site_array[displaySortPtr->object_recno]->animation_count
         --; if (i > 0) i = 10 - i; else i = 10 + i; if (i == 0)
                                      site_array[displaySortPtr->object_recno]->animation_count
         = 100; i = i << 4;
              }
              else
      ...
      sitePtr->draw(displaySortPtr->x_loc * LOCATE_WIDTH,
                    displaySortPtr->y_loc * LOCATE_HEIGHT, sitePtr->altitude,
                    displaySortPtr->draw_flags & OBJECT_DRAW_HIDDEN);
      break;
    }
    }
  }
...
//
// <int>   x, y 			  - the location of the bitmap, in the current
// screen coordination <char*> bitmapPtr 	  - bitmap ptr [int]
// compressedFlag - whether the bitmap is compressed or not
//									 (default:
//0)
//
void ZoomMatrix::put_bitmap_clip(int x, int y, char *bitmapPtr,
  ...
  if (x < ZOOM_X1 || x2 > ZOOM_X2 || y < ZOOM_Y1 || y2 > ZOOM_Y2) {
    if (compressedFlag) {
      vga_back.put_bitmap_area_trans_decompress(
          x, y, bitmapPtr, MAX(ZOOM_X1, x) - x, MAX(ZOOM_Y1, y) - y,
          MIN(ZOOM_X2, x2) - x, MIN(ZOOM_Y2, y2) - y);
    } else {
      vga_back.put_bitmap_area_trans(x, y, bitmapPtr, MAX(ZOOM_X1, x) - x,
                                     MAX(ZOOM_Y1, y) - y, MIN(ZOOM_X2, x2) - x,
                                     MIN(ZOOM_Y2, y2) - y);
    }
  }
...
int ZoomMatrix::detect_bitmap_clip(int x, int y, char *bitmapPtr) {
  ...
  // return mouse.single_click( MAX(ZOOM_X1,x), MAX(ZOOM_Y1,y), MIN(ZOOM_X2,x2),
  // MIN(ZOOM_Y2,y2), 2 );
  return mouse.any_click(MAX(ZOOM_X1, x), MAX(ZOOM_Y1, y), MIN(ZOOM_X2, x2),
                         MIN(ZOOM_Y2, y2), 0)
             ? 1
             : mouse.any_click(MAX(ZOOM_X1, x), MAX(ZOOM_Y1, y),
                               MIN(ZOOM_X2, x2), MIN(ZOOM_Y2, y2), 1)
                   ? 2
                   : 0;
}
...
//0)
//
void ZoomMatrix::put_bitmap_remap_clip(int x, int y, char *bitmapPtr,
  ...
  if (x < ZOOM_X1 || x2 > ZOOM_X2 || y < ZOOM_Y1 || y2 > ZOOM_Y2) {
    if (compressedFlag) {
      if (colorRemapTable) {
        vga_back.put_bitmap_area_trans_remap_decompress(
            x, y, bitmapPtr, MAX(ZOOM_X1, x) - x, MAX(ZOOM_Y1, y) - y,
            MIN(ZOOM_X2, x2) - x, MIN(ZOOM_Y2, y2) - y, colorRemapTable);
      } else {
        vga_back.put_bitmap_area_trans_decompress(
            x, y, bitmapPtr, MAX(ZOOM_X1, x) - x, MAX(ZOOM_Y1, y) - y,
            MIN(ZOOM_X2, x2) - x, MIN(ZOOM_Y2, y2) - y);
      ...
      if (colorRemapTable) {
        vga_back.put_bitmap_area_trans_remap(
            x, y, bitmapPtr, MAX(ZOOM_X1, x) - x, MAX(ZOOM_Y1, y) - y,
            MIN(ZOOM_X2, x2) - x, MIN(ZOOM_Y2, y2) - y, colorRemapTable);
      } else {
        vga_back.put_bitmap_area_trans(
            x, y, bitmapPtr, MAX(ZOOM_X1, x) - x, MAX(ZOOM_Y1, y) - y,
            MIN(ZOOM_X2, x2) - x, MIN(ZOOM_Y2, y2) - y);
      }
    }
  ...
  //---- the whole sprite is inside the view area ------//
    ...
    if (compressedFlag) {
      if (colorRemapTable)
        vga_back.put_bitmap_trans_remap_decompress(x, y, bitmapPtr,
                                                   colorRemapTable);
      else
        vga_back.put_bitmap_trans_decompress(x, y, bitmapPtr);
    } else {
...
int ZoomMatrix::calc_zoom_x(int curX, int curY, int curZ) {
#if (LOCATE_WIDTH == LOCATE_HEIGHT && ZOOM_LOC_X_WIDTH == LOCATE_WIDTH &&      \
     ZOOM_LOC_Y_WIDTH == -LOCATE_HEIGHT && ZOOM_Z_WIDTH == 0)

  // optimized for the current value

  return (curX - (top_x_loc << LOCATE_WIDTH_SHIFT)) -
         (curY - (top_y_loc << LOCATE_HEIGHT_SHIFT)) -
  ...
  return (((curX - top_x_loc * LOCATE_WIDTH) * ZOOM_LOC_X_WIDTH *
               LOCATE_HEIGHT +
           (curY - top_y_loc * LOCATE_HEIGHT) * ZOOM_LOC_Y_WIDTH *
               LOCATE_WIDTH) /
          (LOCATE_WIDTH * LOCATE_HEIGTH)) +
         curZ * ZOOM_Z_WIDTH - (ZOOM_LOC_X_WIDTH + ZOOM_LOC_Y_WIDTH) / 2;
...
#if (LOCATE_WIDTH == LOCATE_HEIGHT && ZOOM_LOC_X_HEIGHT * 2 == LOCATE_WIDTH && \
     ZOOM_LOC_Y_HEIGHT * 2 == LOCATE_HEIGHT && ZOOM_Z_HEIGHT == -1)

  // optimized for the current value

  return ((curX - (top_x_loc << LOCATE_WIDTH_SHIFT) + curY -
           (top_y_loc << LOCATE_HEIGHT_SHIFT)) >>
          1) -
         curZ - (ZOOM_LOC_X_HEIGHT + ZOOM_LOC_Y_HEIGHT) / 2;
#else

  // general form

  return (((curX - top_x_loc * LOCATE_WIDTH) * ZOOM_LOC_X_HEIGHT *
               LOCATE_HEIGHT +
           (curY - top_y_loc * LOCATE_HEIGHT) * ZOOM_LOC_Y_HEIGHT *
               LOCATE_WIDTH) /
          (LOCATE_WIDTH * LOCATE_HEIGHT)) +
         curZ * ZOOM_Z_HEIGHT - (ZOOM_LOC_X_HEIGHT + ZOOM_LOC_Y_HEIGHT) / 2;
  // last term is because top_x_loc, and top_y_loc points to the
  // center of a Location on the top left of the window
#endif
}
...
int ZoomMatrix::get_base_x() {
  return top_x_loc * ZOOM_LOC_X_WIDTH + top_y_loc * ZOOM_LOC_Y_WIDTH +
         (ZOOM_LOC_X_WIDTH + ZOOM_LOC_Y_WIDTH) / 2;
  // last term is because top_x_loc, and top_y_loc points to the
...
int ZoomMatrix::get_base_y() {
  return top_x_loc * ZOOM_LOC_X_HEIGHT + top_y_loc * ZOOM_LOC_Y_HEIGHT +
...
int ZoomMatrix::calc_cur_x(int absX, int absY, int curZ) {
  return ((absX - curZ * ZOOM_Z_WIDTH) * (LOCATE_WIDTH * ZOOM_LOC_Y_HEIGHT) -
         ...
         (ZOOM_LOC_X_WIDTH * ZOOM_LOC_Y_HEIGHT -
          ZOOM_LOC_X_HEIGHT * ZOOM_LOC_Y_WIDTH);
}
...
int ZoomMatrix::calc_cur_y(int absX, int absY, int curZ) {
  return ((absY - curZ * ZOOM_Z_HEIGHT) * (LOCATE_HEIGHT * ZOOM_LOC_X_WIDTH) -
         ...
         (ZOOM_LOC_Y_HEIGHT * ZOOM_LOC_X_WIDTH -
          ZOOM_LOC_Y_WIDTH * ZOOM_LOC_X_HEIGHT);
}
//------ End of function ZoomMatrix::calc_cur_x ---------//
...
// return 1 if success
//
int ZoomMatrix::get_detect_location(int scrnX, int scrnY, int *locX, int *locY,
                                    char *mobileType) {

  if (scrnX < image_x1 || scrnX > image_x2 || scrnY < image_y1 ||
      scrnY > image_y2)
    return 0;
  ...
  do {
    if (locAX < max_x_loc && locAY < max_y_loc) {
      ...
      int curZ;
      vertex[0].x =
          calc_abs_x(curAX, curAY, (curZ = lc.top_left->get_altitude()));
      vertex[0].y = calc_abs_y(curAX, curAY, curZ);
      vertex[1].x =
          calc_abs_x(curAXNext, curAY, (curZ = lc.top_right->get_altitude()));
      vertex[1].y = calc_abs_y(curAXNext, curAY, curZ);
      vertex[3].x =
          calc_abs_x(curAX, curAYNext, (curZ = lc.bottom_left->get_altitude()));
      vertex[3].y = calc_abs_y(curAX, curAYNext, curZ);
      vertex[2].x = calc_abs_x(curAXNext, curAYNext,
                               (curZ = lc.bottom_right->get_altitude()));
      ...
      if (absY < minY) {
        ...
        if (absY <= maxY && test_inside_polygon(absX, absY, vertex, 4)) {
          ...
          *locY = locAY;
          ...
          err_when(*locX < 0 || *locX > max_x_loc || *locY < 0 ||
                   *locY > max_y_loc);
          return 1;
        }
        curAX += LOCATE_WIDTH;
    ...
    if (locBX < max_x_loc && locBY < max_y_loc) {
      ...
      int curZ;
      vertex[0].x =
          calc_abs_x(curBX, curBY, (curZ = lc.top_left->get_altitude()));
      vertex[0].y = calc_abs_y(curBX, curBY, curZ);
      vertex[1].x =
          calc_abs_x(curBXNext, curBY, (curZ = lc.top_right->get_altitude()));
      vertex[1].y = calc_abs_y(curBXNext, curBY, curZ);
      vertex[3].x =
          calc_abs_x(curBX, curBYNext, (curZ = lc.bottom_left->get_altitude()));
      vertex[3].y = calc_abs_y(curBX, curBYNext, curZ);
      vertex[2].x = calc_abs_x(curBXNext, curBYNext,
                               (curZ = lc.bottom_right->get_altitude()));
...
// return 1 if success
//
int ZoomMatrix::get_close_location(int scrnX, int scrnY, int *locX, int *locY,
                                   char *mobileType) {
  if (scrnX < image_x1 || scrnX > image_x2 || scrnY < image_y1 ||
      scrnY > image_y2)
    return 0;
  ...
  if (absX >= 0) {
    ...
    int absYTopRight = calc_abs_y(max_x_loc * LOCATE_WIDTH, 0,
                                  get_corner(max_x_loc, 0)->get_altitude());
    ...
    int absYBottomLeft = calc_abs_y(0, max_y_loc * LOCATE_HEIGHT,
                                    get_corner(0, max_y_loc)->get_altitude());
  ...
  int lowAbsX = calc_abs_x(lowX * LOCATE_WIDTH, lowY * LOCATE_HEIGHT,
                           get_corner(lowX, lowY)->get_altitude());
  int highAbsX =
      calc_abs_x((highX + 1) * LOCATE_WIDTH, (highY + 1) * LOCATE_HEIGHT,
                 get_corner(highX + 1, highY + 1)->get_altitude());
  int estLocX = ((highAbsX - absX) * lowX + (absX - lowAbsX) * highX) /
                (highAbsX - lowAbsX);
  int estLocY = ((highAbsX - absX) * lowY + (absX - lowAbsX) * highY) /
                (highAbsX - lowAbsX);
  if (estLocX < lowX)
    estLocX = lowX;
  if (estLocX > highX)
...
// [int] compressed           0=uncompressed, 1=compressed,
// 2=uncompressed+alpha, bit 2= translucent
//
void ZoomMatrix::put_bitmap_offset(int curX, int curY, int curZ,
                                   char *bitmapPtr, int offsetX, int offsetY,
                                   short *colorRemapTable, int mirror,
                                   int compressed) {
  ...
  if (x1 >= 0 && x2 < image_width && y1 >= 0 && y2 < image_height) {
    ...
    case 0: // uncompressed
      if (!mirror) {
        ...
        if (!colorRemapTable)
          ...
          // vga_back.put_bitmap_trans_remap_hmirror(x1, y1, bitmapPtr,
          // colorRemapTable);
          vga_back.put_bitmap_trans_remap_decompress_hmirror(x1, y1, bitmapPtr,
                                                             colorRemapTable);
      }
      break;
    case 1: // compressed
      if (!mirror) {
        if (!colorRemapTable)
          ...
          vga_back.put_bitmap_trans_remap_decompress(x1, y1, bitmapPtr,
                                                     colorRemapTable);
      } else {
        if (!colorRemapTable)
    ...
    case 1 | 4: // compressed
      if (!mirror) {
        if (!colorRemapTable)
          ...
          vga_back.put_bitmap_half_remap_decompress(x1, y1, bitmapPtr,
                                                    colorRemapTable);
      } else {
        if (!colorRemapTable)
          vga_back.put_bitmap_half_decompress_hmirror(x1, y1, bitmapPtr);
        else
          vga_back.put_bitmap_half_remap_decompress_hmirror(x1, y1, bitmapPtr,
                                                            colorRemapTable);
      }
      break;
    ...
    case 0: // uncompressed
      if (!mirror) {
        if (!colorRemapTable)
          vga_back.put_bitmap_area_trans(x1, y1, bitmapPtr, srcX1, srcY1, srcX2,
                                         srcY2);
        else
          vga_back.put_bitmap_area_trans_remap(x1, y1, bitmapPtr, srcX1, srcY1,
                                               srcX2, srcY2, colorRemapTable);
      } else {
        if (!colorRemapTable)
          vga_back.put_bitmap_area_trans_hmirror(x1, y1, bitmapPtr, srcX1,
                                                 srcY1, srcX2, srcY2);
        else
          vga_back.put_bitmap_area_trans_remap_decompress_hmirror(
              x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2, colorRemapTable);
      }
      break;
    case 1: // compressed
      if (!mirror) {
        if (!colorRemapTable)
          vga_back.put_bitmap_area_trans_decompress(x1, y1, bitmapPtr, srcX1,
                                                    srcY1, srcX2, srcY2);
        else
          vga_back.put_bitmap_area_trans_remap_decompress(
              x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2, colorRemapTable);
        ...
        if (!colorRemapTable)
          vga_back.put_bitmap_area_trans_decompress_hmirror(
              x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
        else
          vga_back.put_bitmap_area_trans_remap_decompress_hmirror(
              x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2, colorRemapTable);
      }
    ...
    case 2: // uncompressed, blending
      // ignore coloRemapTable
      if (!mirror) {
        vga_back.put_bitmap_blend_area(x1, y1, bitmapPtr, srcX1, srcY1, srcX2,
                                       srcY2);
      } else {
        vga_back.put_bitmap_blend_area_hmirror(x1, y1, bitmapPtr, srcX1, srcY1,
                                               srcX2, srcY2);
      }
      break;
    ...
    case 1 | 4: // compressed
      if (!mirror) {
        if (!colorRemapTable)
          vga_back.put_bitmap_area_half_decompress(x1, y1, bitmapPtr, srcX1,
                                                   srcY1, srcX2, srcY2);
        else
          vga_back.put_bitmap_area_half_remap_decompress(
              x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2, colorRemapTable);
      } else {
        if (!colorRemapTable)
          vga_back.put_bitmap_area_half_decompress_hmirror(
              x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2);
        else
          vga_back.put_bitmap_area_half_remap_decompress_hmirror(
              x1, y1, bitmapPtr, srcX1, srcY1, srcX2, srcY2, colorRemapTable);
      }
      break;
    case 2 | 4: // uncompressed, blending
      // ignore coloRemapTable
      if (!mirror) {
        vga_back.put_bitmap_weak_blend_area(x1, y1, bitmapPtr, srcX1, srcY1,
                                            srcX2, srcY2);
      } else {
        vga_back.put_bitmap_weak_blend_area_hmirror(x1, y1, bitmapPtr, srcX1,
                                                    srcY1, srcX2, srcY2);
      }
      break;
    default:
...
void ZoomMatrix::put_bitmapW_offset(int curX, int curY, int curZ,
  ...
  if (x1 >= 0 && x2 < image_width && y1 >= 0 && y2 < image_height) {
    ...
    case 0: // uncompressed
      if (!mirror) {
        vga_back.put_bitmapW_area_trans(x1, y1, bitmapPtr, srcX1, srcY1, srcX2,
                                        srcY2);
      } else {
        err_here();
    ...
    case 0 | 8: // uncompressed, draw black mask
      if (!mirror) {
        vga_back.put_bitmapW_area_trans_blacken(x1, y1, bitmapPtr, srcX1, srcY1,
                                                srcX2, srcY2);
      } else {
        err_here();
      }
...
static void draw_cubic_plane(int x, int y, LocationCorners lc, UCHAR color)
{
        ...
        long c20 = ( 3 * (z1-z0) - duz1w - duz0w - duz0w ) * (C_MULTIPLIER /
LOCATE_WIDTH / LOCATE_WIDTH); long c02 = ( 3 * (z2-z0) - dvz2h - dvz0h - dvz0h )
* (C_MULTIPLIER / LOCATE_WIDTH / LOCATE_WIDTH); long c30 = (duz1w + duz0w -
(z1-z0) - (z1-z0)) * (C_MULTIPLIER / LOCATE_WIDTH / LOCATE_WIDTH /
LOCATE_WIDTH); long c21 = (duz3w + duz0w - duz1w - duz2w) * (C_MULTIPLIER / 2 /
LOCATE_WIDTH / LOCATE_WIDTH / LOCATE_HEIGHT); long c12 = (dvz3h + dvz0h - dvz1h
- dvz2h) * (C_MULTIPLIER / 2 / LOCATE_WIDTH / LOCATE_HEIGHT / LOCATE_HEIGHT);
        long c03 = (dvz2h + dvz0h - (z2-z0) - (z2-z0)) * (C_MULTIPLIER /
LOCATE_HEIGHT / LOCATE_HEIGHT / LOCATE_HEIGHT); long c11 = ( z3 + z0 - z1 - z2 )
* (C_MULTIPLIER / LOCATE_WIDTH / LOCATE_HEIGHT)
        ...
        for( rowU = 3, v = 1; v < LOCATE_HEIGHT * UVMULTIPLIER;
                ...
                ++lineCount, lineCountMod4 = lineCountMod4 < lineCountPeriod-1 ?
lineCountMod4+1 : 0)
        {
                long u = rowU;
                ...
                long z =
                        ((c30*u + c20*UVMULTIPLIER)*u +
c10*UVMULTIPLIER*UVMULTIPLIER) * u / (UVMULTIPLIER*UVMULTIPLIER*UVMULTIPLIER) +
                        ((c03*v + c02*UVMULTIPLIER)*v +
c01*UVMULTIPLIER*UVMULTIPLIER) * v / (UVMULTIPLIER*UVMULTIPLIER*UVMULTIPLIER) +
                        ((c21*u + c12*v) + c11*UVMULTIPLIER)*u*v /
(UVMULTIPLIER*UVMULTIPLIER*UVMULTIPLIER)
                        + c00;
                // du1z = 3*c30*u^2 + 2*c21*u*v + c22*v^2 + 2*c20*u + c11*v +
c10 long du1zM2 =		// dz/du at (u,v) multiplied by 2 * C_MULTIPLIER
                        (((c30*u*3 + c20*2*UVMULTIPLIER ) * u +
                        (c21*u*2 + c12*v + c11*UVMULTIPLIER) * v )
                        / ( UVMULTIPLIER * UVMULTIPLIER)
                        + c10) * 2;
                // du2z = 6*c30*u + 2*c21*v + 2*c20
                long du2zM2 =         // d2z/du2 at (u,v) multiplied by 2 *
C_MULTIPLIER
                        ...
                        u += 2 * UVMULTIPLIER, x += 2, ++y)
                {
                        ...
                        // must update z, du1z then du3z from low to high
derivative
...
static void draw_cubic_plane(int zoomX, int zoomY, LocationCorners lc,
                             UCHAR color) {
...
#define UVMULTIPLIER_SHIFT 2
  ...
  long vz =
      ((c30 * u + c20 * UVMULTIPLIER) * u + c10 * UVMULTIPLIER * UVMULTIPLIER) *
          u / (UVMULTIPLIER * UVMULTIPLIER * UVMULTIPLIER) +
      ((c03 * v + c02 * UVMULTIPLIER) * v + c01 * UVMULTIPLIER * UVMULTIPLIER) *
          v / (UVMULTIPLIER * UVMULTIPLIER * UVMULTIPLIER) +
      ((c21 * u + c12 * v) + c11 * UVMULTIPLIER) * u * v /
          (UVMULTIPLIER * UVMULTIPLIER * UVMULTIPLIER) +
      c00;
...
draw_cubic_plane_1:
        ...
        // long du1zM2 =		// dz/du at (u,v) multiplied by 2 *
        // C_MULTIPLIER
        // (((c30*u*3 + c20*2*UVMULTIPLIER ) * u +
        // (c21*u*2 + c12*v + c11*UVMULTIPLIER) * v )
        // / ( UVMULTIPLIER * UVMULTIPLIER)
...
static int hidden_by_place(int xLoc, int yLoc, int spriteWidth,
  ...
  for (int c = 1; c <= 3; ++c) {
    ...
    int dPixel = 0;
    for (; d <= dMax;
         ++d, ++x, ++y, dPixel -= (ZOOM_LOC_Y_HEIGHT + ZOOM_LOC_X_HEIGHT)) {
      ...
      if (locPtr->is_firm()) {
        ...
        Firm *firmPtr = firm_array[locPtr->firm_recno()];
        short minOffsetY =
            firm_res.get_build(firmPtr->firm_build_id)->min_offset_y;
        ...
        Rock *rockPtr = rock_array[locPtr->rock_array_recno()];
        short rockBitmapRecno =
            rock_res.get_bitmap_recno(rockPtr->rock_recno, rockPtr->cur_frame);
        RockBitmapInfo *rockBitmapInfo =
            rock_res.get_bitmap_info(rockBitmapRecno);
...
// <int> winX1, winY1, winX2, winY2    - a scroll window, any scrolling cannot
// scroll out this window
//                                     - usually the whole buffer
// <int> shiftX, shiftY                - scroll size (in number of pixels),
// positive means move right/down [int]fillColor                      - color to
// fill the shifted area, negative to disable (default : -1)
static void scroll_buffer(short *imageBuf, int truePitch, int winX1, int winY1,
                          int winX2, int winY2, int shiftX, int shiftY,
                          int fillColor) {
  ...
  IMGscrollW(imageBuf, truePitch, srcX1, srcY1, srcX2, srcY2, srcX1 + shiftX,
             srcY1 + shiftY);
...
void ZoomMatrix::disp_rally_point() {
  ...
  if (firm_array.selected_recno) {
    ...
    if (firmPtr->rally_enable_flag) {
      if (firmPtr->rally_dest_base_obj_recno &&
          !base_obj_array.is_deleted(firmPtr->rally_dest_base_obj_recno)) {
        // ------- if destination is a place --------//

        BaseObj *destObj = base_obj_array[firmPtr->rally_dest_base_obj_recno];
        flag = 1;
        curX = (destObj->obj_loc_x1() + destObj->obj_loc_x2() + 1) *
               LOCATE_WIDTH / 2;
        curY = (destObj->obj_loc_y1() + destObj->obj_loc_y2() + 1) *
               LOCATE_HEIGHT / 2;
  ...
  if (flag) {
    ...
    short chOffsetX = -29;
    short chOffsetY = -45;
    put_bitmap_offset(curX, curY, curZ, chPtr, chOffsetX, chOffsetY, NULL, 0,
                      0);
  }
}
