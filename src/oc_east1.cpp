...
int CampaignEastWest::stage_1_create_game() {
  ...
  create_troop(STAGE_1_MONSTER_NATION_RECNO,
               unitCountArray[campaign_difficulty - 1]);

  if (unit_group.size() >
      0) // when a troop is created successfully in create_troop(), unit_group
         // will keep the created units
  {
    ...
    int unitRecno = create_unit(STAGE_1_MONSTER_NATION_RECNO, unitPtr->unit_id,
                                RANK_SOLDIER, xLoc, yLoc, xLoc, yLoc);
  }
...
void CampaignEastWest::stage_1_prelude() {
  ...
  int rc =
      random_pick_attack_state(cur_monster_nation_recno, western_nation_recno);

  err_when(!rc);

  play_speech("PRE-01.WAV");
  disp_narrative(
      res_stage.read("1PRELUDE")); // 1-the letter is from a Fryhtan kingdom
  stop_speech();
...
int CampaignEastWest::stage_1_create_town() {
  ...
  for (int i = townStateArray.size(); i > 0 && addedCount < townAddCount; i--) {
    ...
    if (create_nation_town(PLAYER_NATION_RECNO, xLoc, yLoc, 1, 1,
                           soldierAddCount)) // 1-the race of the town should be
                                             // the same as the nation
      addedCount++;
  }
...
void CampaignEastWest::stage_1_disp_end_game_msg() {
  if (game.result_win_condition_id >= 0) {
    disp_in_game_msg(res_stage.read("1WIN"),
                     (int)(~nation_array)->kill_monster_score);
  } else {
