...
enum
{
    STAGE_1_MONSTER_NATION_RECNO = 2
};
...
int CampaignEastWest::stage_1_create_game()
{
    ...
    if (unit_group.size() > 0) // when a troop is created successfully in create_troop(), unit_group
                               // will keep the created units
    {
        ...
        int unitRecno =
            create_unit(STAGE_1_MONSTER_NATION_RECNO, unitPtr->unit_id, RANK_SOLDIER, xLoc, yLoc, xLoc, yLoc);
    }
