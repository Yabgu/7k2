...
void CampaignEastWest::init_new_campaign_derived() {
  memset(sizeof(Campaign) + (char *)this, 0,
         sizeof(CampaignEastWest) - sizeof(Campaign));
  ...
  hero_res
      .disable_hero_appear(); // no heroes will randomly appear in this campaign
  hero_debut_state_count = 1; // The hero debut event will happen when the
                              // number of player states drops to this number
...
void CampaignEastWest::create_nation() {
  ...
  get_nation(western_nation_recno)
      ->set_custom_name(
          text_campaign.str_western_empire()); // "Western Empire" );
  get_nation(eastern_nation_recno)
      ->set_custom_name(
          text_campaign.str_eastern_empire()); // "Eastern Empire" );
}
...
void CampaignEastWest::set_state_nation() {
  ...
  for (i = state_array.size(); i > 0; i--) {
    ...
    //--- there is a 33% chance that the new state is an extended area of one of
    //the existing Fryhtan nations as opposite to the state of a new nation
    //----//
  ...
  //--- if western empire has no states, change one of the eastern to the
  //western ---//
  ...
  while (state_array.nation_state_count(western_nation_recno) < 2) {
    ...
    for (i = state_array.size(); i > 0; i--) {
      ...
      if (stateInfo->campaign_nation_recno == eastern_nation_recno &&
        ...
        //---- if the western empire already has a state, then the new state
        //must be next to the existing border ----//
    ...
    if (!changeStateRecno &&
        state_array.nation_state_count(western_nation_recno) >
            0) // if western nation already has a state
      break;
  ...
  err_when(state_array.nation_state_count(western_nation_recno) ==
           0); // at least 2 states for the Western Empire
}
...
void CampaignEastWest::process_game_result() {
  ...
  if (game.result_win_condition_id >= 0) {
    ...
    cheat_to_win_flag = 0;
  } else //--- if the player loses, reset win_to_unveil_tech_id as the tech will
         //only be unveil in the next game if the player won this game. ----//
  {
  ...
  catch_nation_info(); // copy nation relation from game nation to campaign
                       // nation
}
...
void CampaignEastWest::del_nation(int nationRecno) {
  ...
  if (nationRecno == new_kingdom_nation_recno) {
    ...
    //--- if there are other independent kingdoms remaining, then set
    //new_kingdom_nation_recno to it ---//

    for (int i = nation_count(); i > 0; i--) {
      if (is_nation_deleted(i))
        continue;

      if (get_nation(i)->is_human() && i != western_nation_recno &&
          i != eastern_nation_recno) {
