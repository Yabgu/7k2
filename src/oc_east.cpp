...
void CampaignEastWest::set_state_nation()
{
    ...
    for (i = state_array.size(); i > 0; i--)
    {
        ...
        if (stateInfo->center_x > MAX_STATE_MAP_WIDTH * 70 / 100 || !stateInfo->is_near_border())
        {
            continue;
        }
        ...
        if (misc.random(3) == 0 || monsterNationCount == maxMonsterNation)
        {
            for (int j = state_array.size(); j > 0; j--)
            {
                ...
                if (stateInfo2->is_adjacent(stateRecno) && stateInfo2->campaign_nation_recno &&
    ...
    while (state_array.nation_state_count(western_nation_recno) < 2)
    {
        ...
        for (i = state_array.size(); i > 0; i--)
        {
            ...
            if (stateInfo->campaign_nation_recno == eastern_nation_recno && stateInfo->center_x < smallestX)
            {
        ...
        if (!changeStateRecno &&
            state_array.nation_state_count(western_nation_recno) > 0) // if western nation already has a state
            break;
...
char *CampaignEastWest::plot_text()
{
    ...
    {
        return substitute_text(res_plot.read("PLOTA5"), get_nation(cur_monster_nation_recno)->race_name(),
...
void CampaignEastWest::del_nation(int nationRecno)
{
    ...
    if (nationRecno == new_kingdom_nation_recno)
    {
        ...
        for (int i = nation_count(); i > 0; i--)
        {
            ...
            if (get_nation(i)->is_human() && i != western_nation_recno && i != eastern_nation_recno)
            {
