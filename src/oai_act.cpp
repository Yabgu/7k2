...
int Nation::process_action(int priorityActionRecno, int processActionMode)
{
    ...
    for (actionRecno = 1;
         actionRecno <= action_count() && (thisSessionProcessCount < maxSessionProcessCount || processActionMode) &&
         ...
         actionRecno++)
    {
        ...
        if (actionNode->retry_count == 0) // the actionNode may still exist even when retry_count==0, waiting
...
if (delFlag &&
    actionNode->processing_instance_count == actionNode->processed_instance_count) // if processing_count >
                                                                                   // processed_count, do not remove
                                                                                   // this ActionNode, as there are some
                                                                                   // unit using this actionNode, when
                                                                                   // they finish or fail the action,
...
//									   (default:
// 1)
// [int]   unitRecno			 - recno of the unit responsible for
// this action
//										if not given,
// an
...
//					  0 - if the action is not added as it
// is
// already in action_array.
//
int Nation::add_action(short xLoc, short yLoc, short refXLoc, short refYLoc, int actionMode, int actionPara,
                       int instanceCount, int unitRecno, int actionPara2, short *groupUnitArray)
{
    ...
    actionNode.retry_count = STD_ACTION_RETRY_COUNT; // number of term to wait before discarding this
                                                     // action
...
//
int Nation::is_action_exist(short actionXLoc, short actionYLoc, short refXLoc, short refYLoc, int actionMode,
                            int actionPara, int unitRecno)
{
    int i;
    ActionNode *actionNode;
    ...
    for (i = action_count(); i > 0; i--)
    {
        ...
        {
            if (unitRecno && unitRecno != actionNode->unit_recno) // it requests to match the unit recno and
    ...
    for (i = action_count(); i > 0; i--)
    {
        ...
        if (actionNode->action_mode == actionMode && actionNode->action_para == actionPara)
        {
            ...
            err_when(actionNode->action_x_loc < 0 || actionNode->action_y_loc < 0 ||
                     actionNode->action_x_loc >= MAX_WORLD_X_LOC || actionNode->action_y_loc >= MAX_WORLD_Y_LOC);
...
int Nation::is_build_action_exist(int firmId, int xLoc, int yLoc)
{
    ...
    for (i = action_count(); i > 0; i--)
    {
        ...
        {
            if (misc.points_distance(actionNode->action_x_loc, actionNode->action_y_loc, xLoc, yLoc) <= effectiveDis)
            {
                return 1;
            }
        }
...
void Nation::action_finished(WORD aiActionId, short unitRecno, int actionFailure)
{
    ...
    if (actionRecno == 0) // not found, this can happen when an action is removed because the
