...
//								 	    be processed first.
//Otherwise it will process 							  		    actions in the array in a sequential order.
//
// [int] processActionMode   - if this is given, only message of this type
//										 will be processed
//and all messages in the queued of this 										 type will be processed.
//
// Note: priorityActionRecno and processActionMode couldn't be used at
//		   the same time.
//
// return: <int> 1 - all messages of the specific action mode are processed or
// all messages are processed
//							or the priority message has been
//processed.
//
int Nation::process_action(int priorityActionRecno, int processActionMode) {
  ...
  int divider =
      1 -
      config
          .ai_aggressiveness; // the more nations there, the less process count
  ...
  for (actionRecno = 1;
       ...
       (thisSessionProcessCount < maxSessionProcessCount ||
        processActionMode) &&
       !doneFlag; // if processActionMode has been specific, then all messages
                  // in the queue of this type will be processed
       actionRecno++) {
    //----- priority action ------//

    if (priorityActionRecno) {
      actionRecno = priorityActionRecno;
      doneFlag = 1; // mark it done, so if the function "continue" to the next
                    // loop, the function will end
    }
    ...
    if (actionNode->action_mode == ACTION_AI_PROCESS_TALK_MSG &&
      ...
      if (misc.random(10) >
          0) // 1/10 chance of processing the diplomatic messages
        continue;
    }
    ...
    if (actionNode->processing_instance_count == actionNode->instance_count) {
      ...
      if (info.game_date > actionNode->add_date + 30 * 6) {
        ...
        actionRecno--; // stay in this array position as the current one has
                       // been deleted, the following one replace the current
                       // one's position
      }
    ...
    err_when(actionNode->processing_instance_count >
             actionNode->instance_count);

    if (info.game_date < actionNode->next_retry_date &&
        !priorityActionRecno) // priorityAction bypass retry date checking
      continue;

    if (actionNode->retry_count ==
        0) // the actionNode may still exist even when retry_count==0, waiting
           // for processed_count to reach processing_count
      continue;

    //---- if the assigned unit has been deleted, cancel the action ----//

    if (actionNode->unit_recno &&
        unit_array.is_deleted(actionNode->unit_recno)) {
      del_action(actionRecno);
      actionRecno--; // stay in this array position as the current one has been
                     // deleted, the following one replace the current one's
                     // position
      continue;
    }
...
if (nation_array.is_deleted(
        nationRecno)) // diplomatic option can result in surrendering
  return 0;
...
if (rc == 1) // the action has been processed, but not sure whether it is
             // complete or not
{
...
{
  if (actionNode->unit_recno) // if the action is associated with a unit, delete
                              // the action if it fails as the associated the
                              // unit will do anything before the next try and
                              // the action will become invalid
  {
    delFlag = 1;
  } else {
    actionNode->next_retry_date =
        info.game_date + 7; // try again one week later
...
if (delFlag &&
    actionNode->processing_instance_count ==
        actionNode
            ->processed_instance_count) // if processing_count >
                                        // processed_count, do not remove this
                                        // ActionNode, as there are some unit
                                        // using this actionNode, when they
                                        // finish or fail the action,
                                        // processed_count will increase and
                                        // processing_count will reach
                                        // processed_count
{
  del_action(actionRecno);
  actionRecno--; // stay in this array position as the current one has been
...
// <short> refXLoc, refYLoc - reference location (optional, not all action types
// need this) <int>   actionMode		 - action mode <int> actionPara
// - action parameter
// [int]	  instanceCount	 - no. of instances of this action should be
// carried out
//									   (default:
//1)
// [int]   unitRecno			 - recno of the unit responsible for this
// action
//										if not given, an
//appropriate unit will be found.
// [int]    actionPara2		 - action para2
// [short*] groupUnitArray  - array of unit recno in the group for the action
//										the no. of units
//in the array is stored in instance_count 										(default: NULL)
//
// return: <int> recno of the action added in action_array
//					  0 - if the action is not added as it is
//already in action_array.
//
int Nation::add_action(short xLoc, short yLoc, short refXLoc, short refYLoc,
  ...
  if (is_action_exist(xLoc, yLoc, refXLoc, refYLoc, actionMode, actionPara,
                      unitRecno))
    return 0;
  ...
  actionNode.ref_x_loc =
      refXLoc; // the refective location of this action make to
  actionNode.ref_y_loc = refYLoc;
  actionNode.retry_count =
      STD_ACTION_RETRY_COUNT; // number of term to wait before discarding this
                              // action
  actionNode.instance_count =
      instanceCount; // num of this action being processed in the waiting queue
  ...
  if (groupUnitArray) {
    ...
    memcpy(actionNode.group_unit_array, groupUnitArray,
           instanceCount * sizeof(groupUnitArray[0]));

    immediateProcess = 1; // have to execute this command immediately as the
                          // unit in unit_array[] may change
    actionNode.retry_count =
        1; // only try once as the unit in unit_array[] may change
  }
  ...
  if (unitRecno) {
    ...
    err_when(
        actionMode ==
            ACTION_AI_ASSIGN_OVERSEER && // an overseer must be a military unit
...
//
void Nation::del_action(int actionRecno) { action_array.linkout(actionRecno); }
//---------- End of function Nation::del_action --------//

//--------- Begin of function Nation::get_action --------//
//
ActionNode *Nation::get_action(int actionRecno) {
  err_when(actionRecno < 1 || actionRecno > action_array.size());

  ActionNode *actionNode = (ActionNode *)action_array.get(actionRecno);

  err_when(actionNode->action_mode == 0); // it has been marked deleted

  return actionNode;
}
//---------- End of function Nation::get_action --------//

//--------- Begin of function Nation::is_action_deleted --------//
//
int Nation::is_action_deleted(int actionRecno) {
  err_when(actionRecno < 1 || actionRecno > action_array.size());

  ActionNode *actionNode = (ActionNode *)action_array.get(actionRecno);

  return actionNode->action_mode == 0;
}
//---------- End of function Nation::is_action_deleted --------//

//--------- Begin of function Nation::is_action_exist --------//
//
// Note: it only check either action?Loc or ref?Loc. If actionXLoc is -1
//			it will check ref?Loc, otherwise it will check
//action?Loc.
//
...
// [int]	  unitRecno              - unit recno to match with, only useful
// for actions under processing.
//
int Nation::is_action_exist(short actionXLoc, short actionYLoc, short refXLoc,
                            short refYLoc, int actionMode, int actionPara,
                            int unitRecno) {
  int i;
  ActionNode *actionNode;
  ...
  for (i = action_count(); i > 0; i--) {
    ...
    if (actionNode->action_mode == actionMode &&
      ...
      if (unitRecno &&
          unitRecno !=
              actionNode->unit_recno) // it requests to match the unit recno and
                                      // it is not matched here
        continue;

      if (actionXLoc >= 0) {
        if (actionNode->action_x_loc == actionXLoc &&
            actionNode->action_y_loc == actionYLoc)
          return i;
      } else {
        if (actionNode->ref_x_loc == refXLoc &&
            actionNode->ref_y_loc == refYLoc)
          return i;
      }
    }
...
//								action with destination in
//this 							   region will be checked.
//
int Nation::is_action_exist(int actionMode, int actionPara, int regionId) {
  ...
  for (i = action_count(); i > 0; i--) {
    ...
    if (actionNode->action_mode == actionMode &&
      ...
      err_when(actionNode->action_x_loc < 0 || actionNode->action_y_loc < 0 ||
...
int Nation::is_build_action_exist(int firmId, int xLoc, int yLoc) {
  ...
  for (i = action_count(); i > 0; i--) {
    ...
    if (actionNode->action_mode == ACTION_AI_BUILD_FIRM &&
      ...
      if (misc.points_distance(actionNode->action_x_loc,
                               actionNode->action_y_loc, xLoc,
                               yLoc) <= effectiveDis) {
        return 1;
      }
    }
...
//
void Nation::action_finished(WORD aiActionId, short unitRecno,
                             int actionFailure) {
  ...
  if (actionRecno ==
      0) // not found, this can happen when an action is removed because the
         // unit cannot finish its task after too long, while the action_id
         // still remain the Unit::cur_order.
  {
    return;
  }
  ...
  //	int shouldStop=1;
  /*
          if( actionNode->action_mode == ACTION_AI_SEA_TRAVEL )		// don't
     reset the unit's ai_action_id in ACTION_AI_SEA_TRAVEL mode as if we reset
     it, the ship will take new action and won't wait for the units to go
     aboard.
          {
                   if( !unit_array.is_deleted(unitRecno) &&
                            unit_res[ unit_array[unitRecno]->unit_id
     ]->unit_class == UNIT_CLASS_SHIP )
                   {
  ...
  */
  ...
  err_when(actionNode->processed_instance_count >
           actionNode->processing_instance_count);
  ...
  //---- if the action is failed and all the outstanding units are finished, del
  //the action ---//
...
void Nation::action_failure(WORD aiActionId, short unitRecno) {
  //-- if the unit is a ship, ignore it as it will be called by stop2() when it
  //stops, but hasn't yet finished its action --//
  /*
          ...
          {
                  int actionMode =
     get_action_based_on_id(aiActionId)->action_mode;
  ...
  /*
          ...
          switch( actionNode->action_mode )
          {
                  case ACTION_AI_SEA_TRAVEL:
                          ...
                          actionNode->instance_count = 1;	// only move one
     ship, it was previously set to the no. units to aboard the ship actionRecno
     = add_action(actionNode, 1);			// 1-immediate process
     flag break;

                  case ACTION_AI_SEA_TRAVEL2:
                          actionNode->action_mode = ACTION_AI_SEA_TRAVEL3;
                          actionRecno = add_action(actionNode, 1);
     // 1-immediate process flag break;
          }
