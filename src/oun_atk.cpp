...
void Unit::execute_attack() {
  ...
  if (!validate_attack_target()) {
    if (has_pushed_order()) {
      //--- if the unit's order is to attack a town, look for defenders to
      //attack first before going back to the town ---//
      ...
      if (pushed_order.mode == UNIT_ATTACK) {
        if (!base_obj_array.is_deleted(pushed_order.para)) {
          ...
          if (baseObj->cast_to_Town() &&
              baseObj->cast_to_Town()->town_defender_count > 0) {
      ...
      // ####### begin Gilbert 31/5 ########//
      // if it aggressive attack from a stop action, pushed order become a move
      // back to the original location perform another aggressive attack first
      // before return
      else if (pushed_order.mode == UNIT_STOP) {
        if (think_aggressive_attack(0, 1))
          return;
        else // proceed move action
        {
          // usually force_move_flag is false
          if (pushed_order.para == -2 // see pushed_order
              && pushed_order.loc_x >= 0 &&
              pushed_order.loc_x < MAX_WORLD_X_LOC && pushed_order.loc_y >= 0 &&
              pushed_order.loc_y < MAX_WORLD_Y_LOC) {
            pop_order(); // pop the order first
            move(cur_order.loc_x, cur_order.loc_y, force_move_flag);
          }
          return;
        }
      }
      ...
      if (in_ai_attack_mission)
        ai_attack_next_target();
      else
        stop_order();
    }
  ...
  if (cur_action != SPRITE_ATTACK) {
    if (cur_action == SPRITE_WAIT) {
      // ####### begin Gilbert 28/4 #######//
      if (wait_state > (retry_state <= MAX_UNIT_WAIT_STATE
                            ? retry_state + MAX_UNIT_WAIT_STATE_FIRST
                            : MAX_UNIT_WAIT_STATE)) {
        wait_state = 0;
    ...
    // ####### end Gilbert 3/5 #######//
    {
      ...
      if( attackDistance <= attack_range() )
      {
              ...
              stop_move();
              if(attack_range() >1 )
                      set_attack_dir(next_x_loc(), next_y_loc(),
      range_attack_x_loc, range_attack_y_loc); else
              {
                      int closeAttackLocX = targetObj->obj_loc_x1();
                      int closeAttackLocY = targetObj->obj_loc_y1();
                      close_attack_location(targetObj, closeAttackLocX,
      closeAttackLocY); set_attack_dir(next_x_loc(), next_y_loc(),
      closeAttackLocX, closeAttackLocY);
              }
              if(is_dir_correct() || (mobile_type == UNIT_AIR &&
      !attackDistance)) attack_now(targetObj, attackDistance); else set_turn();
              return;
      }
      */
      ...
      if (attackDistance <= attackInfo->attack_range) {
        ...
        if (attackInfo->bullet_sprite_id &&
            !bullet_array.add_bullet_possible(
                cur_x_loc(), cur_y_loc(), mobile_type, targetObj->obj_loc_x1(),
                targetObj->obj_loc_y1(), targetObj->obj_mobile_type(),
                targetObj->obj_loc_width(), targetObj->obj_loc_height(),
                range_attack_x_loc, range_attack_y_loc,
          ...
          err_when(targetObj->obj_loc_x1() + attack_loc_offset_x +
                       obj_loc_width() - 1 >=
                   MAX_WORLD_X_LOC);
          err_when(targetObj->obj_loc_y1() + attack_loc_offset_y < 0);
          err_when(targetObj->obj_loc_y1() + attack_loc_offset_y +
                       obj_loc_height() - 1 >=
                   MAX_WORLD_Y_LOC);
          err_when(
              (attack_loc_offset_x || attack_loc_offset_y) &&
              (!bullet_array.add_bullet_possible(
                  targetObj->obj_loc_x1() + attack_loc_offset_x,
                  targetObj->obj_loc_y1() + attack_loc_offset_y, mobile_type,
                  targetObj->obj_loc_x1(), targetObj->obj_loc_y1(),
                  targetObj->obj_mobile_type(), targetObj->obj_loc_width(),
                  targetObj->obj_loc_height(), range_attack_x_loc,
                  range_attack_y_loc, attackInfo->bullet_speed,
                  attackInfo->bullet_sprite_id)));

          if (attack_loc_offset_x || attack_loc_offset_y)
            move_to(targetObj->obj_loc_x1() + attack_loc_offset_x,
                    targetObj->obj_loc_y1() + attack_loc_offset_y);
          else
            stop_move();
        } else
      ...
      else if (cur_action == SPRITE_IDLE) // if the sprite is currently idle,
                                          // search path and move again
      {
        attack(cur_order.para, false);
      }
    }
...
//
// <int>  targetRecno      		- recno of the target (the target should be
// a BaseObj class) <bool> resetOrder             - reset the current and the
// pushed order if there is any
//											  true if
//this is a player action or a high level AI action 											  false if this is a low level
//AI reaction and aggression action
//
void Unit::attack(short targetRecno, bool resetOrder) {
  ...
  //----- if the target is outside the attack range, the unit must move close to
  //the target -----//
  /*
          if( attackDistance > attack_range() )
          {
                  ...
                  if(attack_loc_offset_x || attack_loc_offset_y)
                  {
                          if(!can_move(targetXLoc+attack_loc_offset_x,
     targetYLoc+attack_loc_offset_y)) find_new_location_for_attack(targetObj);

                          err_when(targetXLoc+attack_loc_offset_x < 0);
                          err_when(targetXLoc+attack_loc_offset_x+obj_loc_width()-1
     >= MAX_WORLD_X_LOC); err_when(targetYLoc+attack_loc_offset_y < 0);
                          err_when(targetYLoc+attack_loc_offset_y+obj_loc_height()-1
     >= MAX_WORLD_Y_LOC);

                          if((cur_action != SPRITE_ATTACK) &&
     (attack_loc_offset_x || attack_loc_offset_y) && ((next_x_loc() !=
     targetXLoc+attack_loc_offset_x) || (next_y_loc() !=
     targetYLoc+attack_loc_offset_y))) move_to(targetXLoc+attack_loc_offset_x,
     targetYLoc+attack_loc_offset_y);
                          //	move(targetXLoc, targetYLoc);
                          else if(cur_action != SPRITE_ATTACK)
                                  stop_move();
                  }
                  else//when there is not a pre-assigned location for attacking
     the target
                  {
     ...
     >= MAX_WORLD_Y_LOC);
                          ...
                          if(attack_loc_offset_x || attack_loc_offset_y)
                                  move_to(targetXLoc+attack_loc_offset_x,
     targetYLoc+attack_loc_offset_y);
                          //	move(targetXLoc, targetYLoc);
                          else
                                  stop_move();
  ...
  */
  if (attackDistance > max_attack_range()) {
    //-- don't call move_to() too frequently. If the previous path search fails,
    //wait 10 days before doing a new one.
...
int Unit::set_attack(short targetRecno, bool resetOrder) {
  ...
  if (!validate_attack_target()) // this function must be called after setting
                                 // the order.
  {
    stop_cur_order();
    return 0;
  ...
  //							targetObj->obj_loc_width(),
  //targetObj->obj_loc_height());
  int attackDistance = area_distance(targetObj);
  // #### end Gilbert 4/11 ######//
...
void Unit::attack_now(BaseObj *targetObj, int attackDistance) {
  // ###### begin Gilbert 16/12 ##########//
  //	err_when(abs(cur_x-next_x)>sprite_info->speed ||
  //abs(cur_y-next_y)>sprite_info->speed);
  err_when(abs(cur_x - next_x) > sprite_speed() ||
           abs(cur_y - next_y) > sprite_speed());
  // ###### end Gilbert 16/12 ##########//
  ...
  //---- if the target is next to this unit and current attack mode is range
  //attack, set the attack mode to close attack ---//
  if ((attackDistance == 1 && attack_range() > 1) ||
      (attackDistance > 1 &&
       attack_range() == 1)) // often false condition is checked first
    choose_best_attack_mode(
        attackDistance,
        targetObj->obj_mobile_type()); // may change to close attack
  ...
  if (attack_range() > 1) {
    ...
    if (!bullet_array.add_bullet_possible(
            curXLoc, curYLoc, mobile_type, targetObj->obj_loc_x1(),
            targetObj->obj_loc_y1(), targetObj->obj_mobile_type(),
            targetObj->obj_loc_width(), targetObj->obj_loc_height(),
            range_attack_x_loc, range_attack_y_loc,
            cur_attack_info()->bullet_speed,
            cur_attack_info()->bullet_sprite_id, this)) {
      find_new_position_for_range_attack(targetObj);

      err_when(targetObj->obj_loc_x1() + attack_loc_offset_x < 0);
      err_when(targetObj->obj_loc_x1() + attack_loc_offset_x + obj_loc_width() -
                   1 >=
               MAX_WORLD_X_LOC);
      err_when(targetObj->obj_loc_y1() + attack_loc_offset_y < 0);
      err_when(targetObj->obj_loc_y1() + attack_loc_offset_y +
                   obj_loc_height() - 1 >=
               MAX_WORLD_Y_LOC);
      err_when((attack_loc_offset_x || attack_loc_offset_y) &&
               (!bullet_array.add_bullet_possible(
                   targetObj->obj_loc_x1() + attack_loc_offset_x,
                   targetObj->obj_loc_y1() + attack_loc_offset_y, mobile_type,
                   targetObj->obj_loc_x1(), targetObj->obj_loc_y1(),
...
//					  0 - other statuses - either waiting for next
//attack 					      or is attacking.
//
int Unit::process_attack() {
  ...
  //----- if the sprite has come to the frame of hitting the target of releasing
  //a bullet -----//
  ...
  if ((cur_frame == cur_sprite_attack()->frame_count - 1 &&
       attackInfo->effect_out_frame <
           1) // add the effect at the second last frame if no effect_out_frame
              // specified
  ...
  /*
          ...
          if((unit_id == UNIT_JAGUAR || unit_id == UNIT_GORILLA) &&
     attackInfo->consume_power)  //frighten people
          {
                  Location* locPtr;
                  for(locX = MAX(0, next_x_loc()-panicRange);
     locX<=MIN(MAX_WORLD_X_LOC-1, next_x_loc()+panicRange); locX++)
                  {
                          for(locY = MAX(0, next_y_loc()-panicRange); locY<=
     MIN(MAX_WORLD_Y_LOC-1, next_y_loc()+panicRange); locY++)
                          {
                                  if(locX == next_x_loc() && locY ==
     next_y_loc() ) continue; locPtr = world.get_loc(locX, locY); if(panicRecno
     = locPtr->unit_recno(UNIT_LAND))
                                  {
                                          panicUnitId =
     (unitPtr=unit_array[panicRecno])->unit_id; if(panicUnitId ==
     UNIT_NORMAN_SPU || panicUnitId == UNIT_ROMAN_SPU || panicUnitId ==
     UNIT_GREEK_SPU
                                                  || (panicUnitId ==
     UNIT_CARTH_SPU && unit_id == UNIT_GORILLA) || panicUnitId ==
     UNIT_MONGOL_SPU || panicUnitId == UNIT_EGYPTIAN_SPU
                                                  || (panicUnitId ==
     UNIT_INDIAN_SPU && unit_id == UNIT_GORILLA))
                                          {
                                                  //panic !!
                                                  int panicShiftX =
     1+misc.random(5); int panicShiftY = 1+misc.random(5);

                                                  if(next_x_loc()-locX > 0)
     //horror at the right side ! panicShiftX = -panicShiftX;		//move
     towards the left if(next_y_loc()-locY > 0) //horror at the bottom side !
                                                          panicShiftY =
     -panicShiftY;		//escape to the top

                                                  if(unitPtr->cur_order.mode ==
     UNIT_ATTACK || unitPtr->cur_action == SPRITE_ATTACK) unitPtr->stop_order();
     //stop attacking at once and escape int escapeLocX = MAX(0,
     locX+panicShiftX); escapeLocX = MIN(MAX_WORLD_X_LOC-1, escapeLocX); int
     escapeLocY = MAX(0, locY+panicShiftY); escapeLocY = MIN(MAX_WORLD_Y_LOC-1,
     escapeLocY);

                                                  unitPtr->move_to(escapeLocX,
     escapeLocY);
                                          }
                                  }
                                  else
  ...
  */
  ...
  if (attack_range() > 1) // range attack
  {
    ...
    if (cur_frame == attackInfo->bullet_out_frame) {
      ...
      //				think_use_special_ability();
      //// call only in next_day
      // ####### end Gilbert 9/5 #########//
      ...
      if (bullet_array.add_bullet_possible(
              ...
              targetObj->obj_loc_y1(), targetObj->obj_mobile_type(),
              targetObj->obj_loc_width(), targetObj->obj_loc_height(),
              range_attack_x_loc, range_attack_y_loc, attackInfo->bullet_speed,
              attackInfo->bullet_sprite_id)) {
        //				set_attack_dir(curXLoc, curYLoc,
        //range_attack_x_loc, range_attack_y_loc);
        ...
        if (attackInfo->consume_power == 0 &&
          ...
          scanner.init(next_x_loc(), next_y_loc(), loc_width,
                       attackInfo->attack_range);
          ...
          for (scanner.restart(); !scanner.is_finish(); scanner.next()) {
            ...
            if (scanUnit->nation_recno == targetObj->nation_recno &&
                scanUnit != this &&
                scanUnit->is_special_ability_effective() !=
                    SPAB_REFLECT_ARROW // don't shoot at target with reflect on
                && get_dir(cur_x, cur_y, scanUnit->cur_x, scanUnit->cur_y) ==
                       cur_dir &&
                can_attack_target(scanUnit->base_obj_recno) &&
                bullet_array.add_bullet_possible(
                    next_x_loc(), next_y_loc(), mobile_type,
                    scanUnit->next_x_loc(), scanUnit->next_y_loc(),
                    scanUnit->mobile_type, scanUnit->loc_width,
                    scanUnit->loc_height, range_attack_x_loc,
                    range_attack_y_loc, attackInfo->bullet_speed,
                    attackInfo->bullet_sprite_id, this)) {
        ...
        // spray shuriken units in 2 square around the target
        if (unit_id == UNIT_JAPANESE_SPU && attackInfo->consume_power == 0 &&
            nation_recno &&
            tech_res[TECH_SPU_BERSERK(RACE_JAPANESE)]->get_nation_tech_level(
                nation_recno) &&
          ...
          WorldScanTraverser scanner;
          scanner.init(targetObj->obj_loc_x1(), targetObj->obj_loc_y1(),
                       targetObj->obj_loc_width(), 2);
          scanner.sort_by_distance(targetObj);

          // sort first, because filter is slower

          int maxTarget = 2;
          for (scanner.restart(); !scanner.is_finish() && maxTarget > 0;
               scanner.next()) {
            Unit *scanUnit = unit_array[scanner.get_unit()];
            if (scanUnit->nation_recno == targetObj->nation_recno &&
                scanUnit != this &&
                ...
                && get_dir(cur_x, cur_y, scanUnit->cur_x, scanUnit->cur_y) ==
                       cur_dir &&
                area_distance(scanUnit) <= attackInfo->attack_range &&
                can_attack_target(scanUnit->base_obj_recno) &&
                bullet_array.add_bullet_possible(
                    next_x_loc(), next_y_loc(), mobile_type,
                    scanUnit->next_x_loc(), scanUnit->next_y_loc(),
  ...
  } else // close attack
  {
    ...
    if (cur_frame == cur_sprite_attack()->frame_count) {
        ...
        for (locY = y1; locY <= y2; ++locY) {
          ...
          for (locX = x1; locX <= x2; ++locX, ++locPtr) {
            ...
            if (panicRecno && panicRecno != sprite_recno // exclude itself
                && unit_array[panicRecno]->base_obj_recno !=
                       cur_order.para) // also exclude attacking target
            {
              ...
              if (panicFlag) {
                ...
                if (unitPtr->cur_order.mode == UNIT_ATTACK ||
                    unitPtr->cur_action == SPRITE_ATTACK)
      ...
      float attackDamage =
          actual_damage() * attackInfo->attack_attribute.effectiveness(
                                targetObj->defense_attribute);
      ...
      if ((targetUnit = targetObj->cast_to_Unit())) {
        ...
        attackDamage += attackDamage *
                        (4 - misc.abs_direction_diff(cur_dir & 7,
                                                     targetUnit->cur_dir & 7)) /
                        8;
        // ####### end Gilbert 1/6 ##### //
      }
      ...
      if (is_special_ability_effective() == SPAB_FREEZE) {
        ...
        for (scanner.restart(); !scanner.is_finish(); scanner.next()) {
          Unit *scanUnit = unit_array[scanner.get_unit()];
          if ((!targetUnit ||
               targetUnit != scanUnit) // not the target attacking
              && scanUnit->nation_recno == targetObj->nation_recno &&
              misc.random(scanUnit->combat_level()) <
                  misc.random(combat_level() /
                              2)) // saving depend on combat level
          {
            // ###### begin Gilbert 10/4 ##########//
...
int Unit::validate_attack_target() {
  ...
  if (targetObj->obj_type == OBJ_FIRM || targetObj->obj_type == OBJ_TOWN) {
    if (targetObj->obj_loc_x1() !=
            cur_order.loc_x || // a town or a firm cannot move
        targetObj->obj_loc_y1() != cur_order.loc_y) {
      return 0;
    }
  } else if (targetObj->obj_type == OBJ_UNIT) {
    //-- if Unit::name_id does not match the saved extra_para, that means this
    ...
    if (has_pushed_order()) {
      ...
      if (original_target_x_loc >= 0 &&
        ...
        //--- only chase in aggressive mode or in special mode like defending
        //and rebel ----//

        enum { AGGRESSIVE_CHASE_RANGE = 5 };

        int curDistance = misc.points_distance(
            targetObj->obj_loc_x1(), targetObj->obj_loc_y1(),
      ...
      //-------------------------------------------//
        ...
        // ##### begin Gilbert 15/3 ########//
        // int attackDistance = cal_distance( targetObj->obj_loc_x1(),
        // targetObj->obj_loc_y1(),
        //							targetObj->obj_loc_width(),
        //targetObj->obj_loc_height() );
        int attackDistance = area_distance(targetObj);
        // ##### end Gilbert 15/3 ########//
...
int Unit::can_attack_target(int targetObjRecno) {
          ...
          if( unit_id == UNIT_BEE )
          {
                  ...
                  if( targetBaseObj->cast_to_Unit() )
                  {
                          ...
                          if( !targetUnit->race_id
                                  ...
                                  || unit_res[targetUnit->unit_id]->unit_class
     == UNIT_CLASS_WEAPON
                                  || unit_res[targetUnit->unit_id]->unit_class
     == UNIT_CLASS_GOD) )
                          {
  ...
  */
  ...
  Unit *targetUnit;
  if ((targetUnit = targetObj->cast_to_Unit())) {
    if (targetUnit->unit_id == UNIT_CARAVAN || targetUnit->cast_to_UnitGod()) {
      return 0;
    }
  }
  /// ######### end Gilbert 24/4 #########//
  ...
  if (nation_recno) {
    ...
    // ###### begin Gilbert 29/4 #########//
    // return
    // nation_array[nation_recno]->get_relation_should_attack(targetNationRecno);
  ...
  //----- if this is an independent unit -----//
    ...
    if (unit_mode == UNIT_MODE_TOWN_DEFENDER) {
      ...
      return town_array[unit_mode_para]->last_attack_nation_recno ==
             targetNationRecno;
    } else if (unit_mode == UNIT_MODE_CAMP_DEFENDER) {
      if (firm_array.is_deleted(unit_mode_para))
        return 0;

      return firm_array[unit_mode_para]->last_attack_nation_recno ==
             targetNationRecno;
...
void Unit::being_attack_hit(BaseObj *attackerObj, float damagePoint) {
  ...
  if (damagePoint > 0.0f && (item.ability(ITEM_ABILITY_ABSORB_HIT) ||
                             item.ability(ITEM_ABILITY_INVINCIBLE))) {
    damagePoint = 0.0f;
    item.use_now();
  }
  ...
  if (attackerObj && attackerObj->cast_to_Unit()) {
    ...
    if (is_monster()) {
      ...
      //			||
      //!unit_array.is_deleted(attackUnit->leader_unit_recno)
      //// check leader's item
      //				&& (itemPtr =
      //&unit_array[attackUnit->leader_unit_recno]->item)
      //				&& (antiMonsterAbility =
      //itemPtr->ability(ITEM_ABILITY_ANTI_MONSTER))
      //				&& itemPtr->para == monster_id() )
      // #### end Gilbert 26/1 #######//
      {
  ...
  if (cast_to_UnitMonster() && attackerObj && attackerObj->nation_recno) {
    nation_array[attackerObj->nation_recno]->change_reputation(
        REPUTATION_INCREASE_PER_ATTACK_MONSTER);
  }

  //--- set rebel hostile nation if this unit is a rebel ---//

  if (unit_mode == UNIT_MODE_REBEL && attackerObj &&
      attackerObj->nation_recno) {
    if (!rebel_array.is_deleted(unit_mode_para))
      rebel_array[unit_mode_para]->set_hostile_nation(
          attackerObj->nation_recno);
  }
  ...
  //--- if a member in a troop is under attack, ask for other troop members to
  //help ---//

  if (attackerObj &&
      !base_obj_array.is_deleted(
          attackerObj->base_obj_recno) // the object may be dying
      && !force_move_flag
      // ####### begin Gilbert 15/4 #######//
      // && info.game_date%2 == sprite_recno%2 )	// remarked to maximize
      // the effect
      // && last_attack_date != lastAttackDateBackup )		// once per day
      // && last_attack_date - lastAttackDateBackup >= 2 )	// almost mean
      // only the first attack in a battle
      && info.game_date - last_ask_attack_date >=
             3) // no less than 3 days after last ask help attack
                // ####### end Gilbert 20/5 #######//
  {
    // even no people are asked to fight, still mark the last_ask_attack_date to
    // reduce process time

    last_ask_attack_date = info.game_date;

    //		if( (leader_unit_recno || (team_info &&
    //team_info->member_count>1))
  ...
  if (attackerObj && attackerObj->obj_is_visible()) {
    if (unit_mode == UNIT_MODE_TOWN_DEFENDER &&
        !town_array.is_deleted(unit_mode_para) &&
        attackerObj->area_distance(town_array[unit_mode_para]) <=
            TOWN_TARGET_RANGE) {
      town_array[unit_mode_para]->return_fire(attackerObj);
    }

    if (unit_mode == UNIT_MODE_CAMP_DEFENDER &&
        !firm_array.is_deleted(unit_mode_para) &&
        attackerObj->area_distance(firm_array[unit_mode_para]) <=
            FORT_BULLET_RANGE) {
  ...
  if (attackerObj && attackerObj->cast_to_Unit()) {
    ...
    if (race_id // target can reduce loyalty
        && (loyaltyDecProbability =
                attackUnit->item.ability(ITEM_ABILITY_DEC_LOYALTY)) &&
  ...
  // ------ process special ability --------//
  // if( is_human() && combat_level() >= MAX_COMBAT_BATTLE )
  //		think_use_special_ability();			// call only in
  //next_day
  // ####### end Gilbert 9/5 #########//
}
...
void Unit::auto_react_attack(BaseObj *attackerObj) {
  ...
  if ((force_move_flag || behavior_mode == UNIT_STAND_GROUND) &&
      cur_action != SPRITE_IDLE) // if the unit is stuck and cannot move, then
                                 // counter-attack
    return;

  //	if( attackerObj->obj_type != OBJ_UNIT )		// only auto attack
  //units, do not attack offensive firms 		return;
  if (!attackerObj->is_visible()) // only auto attack units, do not attack
                                  // offensive firms
    return;
  ...
  if (get_behavior_mode() != UNIT_AGGRESSIVE && cur_action != SPRITE_ATTACK &&
      cur_action != SPRITE_IDLE) {
    return;
  }

  //---- in stand ground mode, only attack unit next to the current unit, don't
  //move to attack if the enemy is using range attack ---//

  if (get_behavior_mode() == UNIT_STAND_GROUND) {
    // ######## begin Gilbert 15/3 #########//
    //		int attackDistance = cal_distance( attackerObj->obj_loc_x1(),
    //attackerObj->obj_loc_y1(), 									attackerObj->obj_loc_width(),
    //attackerObj->obj_loc_height() );
  ...
  if (attackerObj->cast_to_Town() && attackerObj->nation_recno == 0 &&
      nation_recno != 0 &&
      attackerObj->cast_to_Town()->resistance(nation_recno) < 1.0f) {
    return;
  }
  // ###### end Gilbert 15/4 #########//
  ...
  // ###### begin Gilbert 20/5 #######//
  if ( // cur_action == SPRITE_ATTACK &&		// two people attack one
       // target, response to one only
    ...
    //		if( attackingObj->cast_to_Unit()			// already attack an unit, don't
    //change target
    //			|| !attackerObj->cast_to_Unit() )	// not attacking an unit, but not
    //attacked by a unit, don't change target as well 			return;

    if (attackerObj->cast_to_Unit() &&
        (!attackingObj
              ->cast_to_Unit() // if attacking non-unit switch to attack unit
         || area_distance(attackerObj) <=
                    maxAttackRange // if attacking unit is out of range but new
                                   // target is within range
                && area_distance(attackingObj) - 1 > maxAttackRange)) {
      // empty
    } else {
      return;
    }
  ...
  if (!has_pushed_order()) // if there is already an order pushed into the
                           // stack, that means the unit is already preforming
                           // an ad hoc action (likely already defending), then
                           // just replace the original ad hoc action with the
                           // current one.
    push_order();
  ...
  if (cur_order.mode !=
      UNIT_ATTACK) // if the attack action failed, resume order
    pop_order();
}
...
int Unit::ask_team_help_attack(BaseObj *attackerObj) {
  ...
  //--- if the attacking unit is our unit (this can happen if the unit is
  //porcupine) ---//
  ...
  if (leader_unit_recno &&
      !unit_array.is_deleted(
          leader_unit_recno)) // if the current unit is a soldier, get its
                              // leader's recno
...
#define LEADER_IN_DANAGER_HIT_POINT 30
    ...
    for (i = teamInfo->member_count - 1; i >= 0; i--) {
      ...
      if (unitPtr->cur_order.mode == UNIT_ATTACK &&
          ...
          (unitsTarget = base_obj_array[unitPtr->cur_order.para])
              ->is_visible()) {
      ...
      if (leaderInDanger &&
              targetDistance <= maxAttackRange // behavior is bad unless unitPtr
          ...
          || unitsTarget && unitsTargetDistance - 1 > maxAttackRange &&
                 targetDistance <= maxAttackRange) {
        // -------- add to suit unit recno -------//

        suitUnitRecno[suitCount] = unitRecno;
        suitUnitDistance[suitCount] = targetDistance > maxAttackRange
                                          ? targetDistance - maxAttackRange
                                          : 0;
        suitCount++;
  ...
  */
  ...
  for (scanner.restart(); !scanner.is_finish(); scanner.next()) {
    ...
    if (unitPtr->is_all_stop()
        ...
        // if chasing target but new target is already in range, preempt
        || unitsTarget && unitsTargetDistance - 1 > maxAttackRange &&
               targetDistance <= maxAttackRange) {
      // ####### begin Gilbert 5/5 ##########//
      if (!unitPtr->has_pushed_order())
      ...
      ++helpCount;
    } else if (unitPtr->is_all_stop()) {
      unitPtr->think_aggressive_attack(targetDistance, 1);
...
void Unit::being_killed(BaseObj *attackerObj) {
  // ###### begin Gilbert 17/6 #########//
  // if add news message for cloaked spy
  if (spy_recno && is_own() && nation_recno != nation_array.player_recno) {
  ...
  //--- if a monster All-high is killed, the action will induce heroes to join
  //the nation that killed the monster ---//

  if (attackerObj && attackerObj->nation_recno && rank_id == RANK_KING &&
      is_monster()) {
...
void Unit::update_being_killed_statistic(BaseObj *attackerObj) {
  ...
  } else if (is_monster() && nation_recno > 0) {
    if (curNation)
  ...
  // -------- fryhtan unit ---------//
    ...
    if (attackerNation) {
      ...
      //--- killing Fryhtan kings and generals increase the attacking nation's
      //reputation ---//
  ...
  //--------- if it's a non-human unit ---------//
    ...
    case UNIT_CLASS_CARAVAN:
      ...
      if (attackerNation)
        attackerNation->civilian_killed(
            0, 1); // 1-is the nation is the attacking one
      break;
      // ####### begin Gilbert 24/3 #######//
...
void Unit::being_killed_leave_money(BaseObj *attackerObj) {
  ...
  //		|| !unit_res[unit_id]->unit_class == UNIT_CLASS_MONS_WEAPON )
  if (0) {
    killMoney = max_hit_points() / 2;
  } else {
    switch (rank_id) {
    case RANK_KING:
  ...
  // killMoney = killMoney * (80 + unique_id % 40) / 100;		// -20 to +20
  // randomness
  killMoney = killMoney * (80 + (unsigned long)unique_id % 40) /
              100; // -20 to +20 randomness
  // ###### patch end Gilbert 29/3 ########//

  cur_power = killMoney;
  max_power =
      (attackerObj->nation_recno == nation_array.player_recno) ? -10 : 0;

  nation_array[attackerObj->nation_recno]->add_income(INCOME_TREASURE,
                                                      (float)killMoney);
...
void Unit::being_killed_subordinate_betray(BaseObj *attackerObj) {
  ...
  int netAttackerPower = world.net_attacker_power_in_area(
      next_x_loc(), next_y_loc(), attackerObj->nation_recno, nation_recno, 0,
      hasWar, EFFECTIVE_LEADING_DISTANCE);

  //--- the nation with most units in the surrounding area ----//

  int mostPopulatedNationRecno =
      most_populated_nation_nearby(EFFECTIVE_LEADING_DISTANCE);

  //------------------------------------------------------//

  err_when(!team_info);

  int betrayAllowCount =
      1 + misc.random(3); // betray only 1 to 3 units at maximum
  ...
  for (int i = 0; i < team_info->member_count; i++) {
    ...
    if (!unitPtr->is_visible() ||
        !unitPtr->race_id) // only living beings will betray
      continue;
    ...
    if (misc.points_distance(unitPtr->next_x_loc(), unitPtr->next_y_loc(),
                             next_x_loc(),
                             next_y_loc()) > EFFECTIVE_LEADING_DISTANCE) {
      continue;
    }
    ...
    //----------------------------------------------//
      ...
      betrayChance =
          40 - 50 * (int)unitPtr->hit_points / (int)unitPtr->max_hit_points() -
          (int)unitPtr->loyalty / 2;
...
int Unit::most_populated_nation_nearby(int scanRange) {
  ...
  if (!bestNationId)
    bestNationId =
        misc.random(nation_array.size()) +
        1; // if there is no human units nearby (perhaps just using weapons)
...
void Unit::target_move(BaseObj *targetObj) {
  ...
  // ####### begin Gilbert 16/4 ########//
  // ------ prevent attack() or any other function changing
  // original_target_x_loc
  ...
  // ###### end Gilbert 15/3 ########//
  {
    ...
    if (!update_attack_path_dist()) {
      if (cur_action == SPRITE_MOVE || cur_action == SPRITE_WAIT ||
  ...
  {
    //-----------------------------------------------------------------------------//
    // although the target has moved, the unit can still attack it. no need to
    // move
    //-----------------------------------------------------------------------------//

    // ####### begin Gilbert 16/12 ########//
    //		if(abs(cur_x-next_x)>=sprite_info->speed ||
    //abs(cur_y-next_y)>=sprite_info->speed) 			return;	// return as moving
    if (abs(cur_x - next_x) >= sprite_speed() ||
        abs(cur_y - next_y) >= sprite_speed())
    ...
    if (cur_action == SPRITE_ATTACK) {
      ...
      if ((attackDistance == 1 && attack_range() > 1) ||
          (attackDistance > 1 && attack_range() == 1)) // may change attack mode
      {
    ...
    /*
    if((attackDistance==1 && attack_range() > 1) || (attackDistance > 1 &&
attack_range() == 1)) // may change attack mode
            choose_best_attack_mode(attackDistance,
targetObj->obj_mobile_type());

    if(attack_range() > 1) // range attack
    {
            //------------------ do range attack ----------------------//

            if(bullet_array.add_bullet_possible(curXLoc, curYLoc, mobile_type,
targetXLoc, targetYLoc, targetObj->obj_mobile_type(),
targetObj->obj_loc_width(), targetObj->obj_loc_height(), range_attack_x_loc,
range_attack_y_loc, cur_attack_info()->bullet_speed,
cur_attack_info()->bullet_sprite_id))	// range attack possible
            {
                    set_cur(next_x, next_y);
                    err_when(range_attack_x_loc==-1 || range_attack_y_loc==-1);

                    set_attack_dir(curXLoc, curYLoc, range_attack_x_loc,
range_attack_y_loc); stop_move();

//				if(is_dir_correct())
//					bullet_array.add_bullet(this,
targetObj);
//				cycle_eqv_attack();
                    //cur_frame  = 1;
  ...
  err_when(original_target_x_loc != oriTargetXLoc ||
           original_target_y_loc != oriTargetYLoc);
}
...
//
// If both attack_loc_offset_x and attack_loc_offset_y is 0 after running this
// function, that means no suitable location is found for the unit to attack.
//
void Unit::find_new_location_for_attack(BaseObj *targetObj) {
  ...
  if ((testLocY = targetLocY - obj_loc_height()) >= 0) // has top edge
  {
    ...
    while (w >= 0) {
      if (targetLocX >= w) { // has left top corner
        index = -w;
        break;
      }
      w--;
  ...
  if ((testLocX = targetLocX + targetWidth) <
      MAX_WORLD_X_LOC - obj_loc_width()) // has right edge
  {
    h = obj_loc_height();
  ...
  if ((testLocY = targetLocY + targetHeight) <
      MAX_WORLD_Y_LOC - obj_loc_height()) // has bottom edge
  {
    w = obj_loc_width();
...
void Unit::find_new_position_for_range_attack(BaseObj *targetObj) {
  ...
  err_when(targetXLoc < 0 || targetXLoc >= MAX_WORLD_X_LOC || targetYLoc < 0 ||
           targetYLoc >= MAX_WORLD_Y_LOC);

  int xLoc1 = MAX(targetXLoc - attackRange, 0);
  int yLoc1 = MAX(targetYLoc - attackRange, 0);
  int xLoc2 =
      MIN(targetXLoc + targetWidth - 1 + attackRange, MAX_WORLD_X_LOC - 1);
  int yLoc2 =
      MIN(targetYLoc + targetHeight - 1 + attackRange, MAX_WORLD_Y_LOC - 1);
  int checkXLoc, checkYLoc;

  // #### begin Gilbert 4/11 ######//
  // 	if((cal_distance(targetXLoc, targetYLoc, targetWidth, targetHeight) <=
  // attackRange)
  if (area_distance(targetObj) <= attackRange
      // #### end Gilbert 4/11 ######//
      && bullet_array.add_bullet_possible(
             curXLoc, curYLoc, mobile_type, targetXLoc, targetYLoc,
             targetObj->obj_mobile_type(), targetWidth, targetHeight,
             range_attack_x_loc, range_attack_y_loc, attackInfo->bullet_speed,
             attackInfo->bullet_sprite_id)) // inside the attack range
  {
  ...
  for (checkXLoc = xLoc1; checkXLoc <= xLoc2; checkXLoc++) {
    if (can_move(checkXLoc, yLoc1) &&
        bullet_array.add_bullet_possible(
            checkXLoc, yLoc1, mobile_type, targetXLoc, targetYLoc,
            targetObj->obj_mobile_type(), targetWidth, targetHeight,
            range_attack_x_loc, range_attack_y_loc, attackInfo->bullet_speed,
            attackInfo->bullet_sprite_id)) {
    ...
    if (can_move(checkXLoc, yLoc2) &&
        bullet_array.add_bullet_possible(
            checkXLoc, yLoc2, mobile_type, targetXLoc, targetYLoc,
            targetObj->obj_mobile_type(), targetWidth, targetHeight,
            range_attack_x_loc, range_attack_y_loc, attackInfo->bullet_speed,
            attackInfo->bullet_sprite_id)) {
      attack_loc_offset_x = checkXLoc - targetXLoc;
  ...
  for (checkYLoc = yLoc1 + 1; checkYLoc < yLoc2; checkYLoc++) {
    ...
    if (can_move(xLoc1, checkYLoc) &&
        bullet_array.add_bullet_possible(
            xLoc1, checkYLoc, mobile_type, targetXLoc, targetYLoc,
            targetObj->obj_mobile_type(), targetWidth, targetHeight,
            range_attack_x_loc, range_attack_y_loc, attackInfo->bullet_speed,
            attackInfo->bullet_sprite_id)) {
    ...
    if (can_move(xLoc2, checkYLoc) &&
        bullet_array.add_bullet_possible(
            xLoc2, checkYLoc, mobile_type, targetXLoc, targetYLoc,
            targetObj->obj_mobile_type(), targetWidth, targetHeight,
            range_attack_x_loc, range_attack_y_loc, attackInfo->bullet_speed,
            attackInfo->bullet_sprite_id)) {
...
//---------------End of function
//Unit::find_new_position_range_attack----------------//
...
//
void Unit::set_attack_dir(short curX, short curY, short targetX,
                          short targetY) {
...
void Unit::close_attack_location(BaseObj *targetObj, int &closeAttackLocX,
                                 int &closeAttackLocY) {
  int targetWidth = targetObj->obj_loc_width();
  int targetHeight = targetObj->obj_loc_height();
  int targetLocX = targetObj->obj_loc_x1();
  ...
  if (next_x_loc() <= targetLocX)
    ...
    xOffset =
        next_x_loc() - targetLocX; // in the middle(vertical) of the target
  ...
  if (next_y_loc() <= targetLocY)
    ...
    yOffset =
        next_y_loc() - targetLocY; // in the middle(horizontal) of the target

  closeAttackLocX = targetLocX + xOffset;
  closeAttackLocY = targetLocY + yOffset;
}

//--------- Begin of function Unit::ai_attack_next_target --------//
//
// If this unit has been assigned to an ai_attack mission,
// check if there is any other targets to attack.
//
int Unit::ai_attack_next_target() {
  int curXLoc, curYLoc;

  if (cur_order.mode == UNIT_ATTACK) {
    curXLoc = cur_order.loc_x; // the last target's location
    curYLoc = cur_order.loc_y;
  } else {
    curXLoc = cur_x_loc(); // the unit's current location
    curYLoc = cur_y_loc();
  }
  ...
  for (int i = 1; i <= scanRange * scanRange; i++) {
    ...
    if (baseObjRecno) {
      ...
      if (baseObj->nation_recno && can_attack_target(baseObjRecno) &&
          nationPtr->get_relation_status(baseObj->nation_recno) ==
              RELATION_HOSTILE) {
        attack(baseObjRecno, 1);
        return 1;
      }
    }
  }

  //--- no new targets found, reset it from the mission mode ---//

  in_ai_attack_mission = 0;
  return 0;
}
//---------- End of function Unit::ai_attack_next_target ---------//
