...
void Unit::execute_attack()
{
            ...
            else if (pushed_order.mode == UNIT_STOP)
            {
                ...
                else // proceed move action
                {
                    ...
                    if (pushed_order.para == -2 // see pushed_order
                        && pushed_order.loc_x >= 0 && pushed_order.loc_x < MAX_WORLD_X_LOC && pushed_order.loc_y >= 0 &&
    ...
    if (targetObj->obj_loc_x1() != cur_order.loc_x || targetObj->obj_loc_y1() != cur_order.loc_y)
    {
    ...
    if (cur_action != SPRITE_ATTACK)
    {
        if (cur_action == SPRITE_WAIT)
        {
            // ####### begin Gilbert 28/4 #######//
            if (wait_state >
                (retry_state <= MAX_UNIT_WAIT_STATE ? retry_state + MAX_UNIT_WAIT_STATE_FIRST : MAX_UNIT_WAIT_STATE))
            {
                wait_state = 0;
            ...
            */
            ...
            if (attackDistance <= attackInfo->attack_range)
            {
                ...
                if (attackInfo->bullet_sprite_id &&
                    !bullet_array.add_bullet_possible(cur_x_loc(), cur_y_loc(), mobile_type, targetObj->obj_loc_x1(),
                                                      ...
                                                      attackInfo->bullet_sprite_id, this))
                {
                    ...
                    err_when((attack_loc_offset_x || attack_loc_offset_y) &&
                             (!bullet_array.add_bullet_possible(
                                 ...
                                 targetObj->obj_loc_y1() + attack_loc_offset_y, mobile_type, targetObj->obj_loc_x1(),
                                 targetObj->obj_loc_y1(), targetObj->obj_mobile_type(), targetObj->obj_loc_width(),
                                 targetObj->obj_loc_height(), range_attack_x_loc, range_attack_y_loc,
...
//
// <int>  targetRecno      		- recno of the target (the target should
// be a BaseObj class) <bool> resetOrder             - reset the current and the
// pushed order if there is any
//											  true
// if
// this is a player action or a high level AI action
...
void Unit::attack_now(BaseObj *targetObj, int attackDistance)
{
    ...
    if ((attackDistance == 1 && attack_range() > 1) ||
        (attackDistance > 1 && attack_range() == 1)) // often false condition is checked first
    ...
    if (attack_range() > 1)
    {
        ...
        if (!bullet_array.add_bullet_possible(curXLoc, curYLoc, mobile_type, targetObj->obj_loc_x1(),
                                              targetObj->obj_loc_y1(), targetObj->obj_mobile_type(),
                                              targetObj->obj_loc_width(), targetObj->obj_loc_height(),
                                              range_attack_x_loc, range_attack_y_loc, cur_attack_info()->bullet_speed,
                                              cur_attack_info()->bullet_sprite_id, this))
        {
            ...
            err_when((attack_loc_offset_x || attack_loc_offset_y) &&
                     (!bullet_array.add_bullet_possible(
                         targetObj->obj_loc_x1() + attack_loc_offset_x, targetObj->obj_loc_y1() + attack_loc_offset_y,
                         mobile_type, targetObj->obj_loc_x1(), targetObj->obj_loc_y1(), targetObj->obj_mobile_type(),
                         targetObj->obj_loc_width(), targetObj->obj_loc_height(), range_attack_x_loc,
                         range_attack_y_loc, attackInfo->bullet_speed, attackInfo->bullet_sprite_id)));
...
//					  0 - other statuses - either waiting for
// next
...
int Unit::process_attack()
{
    ...
    if ((cur_frame == cur_sprite_attack()->frame_count - 1 &&
         attackInfo->effect_out_frame < 1) // add the effect at the second last frame if no effect_out_frame
                                           // specified
    ...
    */
    ...
    if (attack_range() > 1) // range attack
    {
        ...
        if (cur_frame == attackInfo->bullet_out_frame)
        {
            ...
            if (bullet_array.add_bullet_possible(
                    curXLoc, curYLoc, mobile_type, targetObj->obj_loc_x1(), targetObj->obj_loc_y1(),
                    ...
                    range_attack_x_loc, range_attack_y_loc, attackInfo->bullet_speed, attackInfo->bullet_sprite_id))
            {
                ...
                if (attackInfo->consume_power == 0 && is_special_ability_effective() == SPAB_MIRAGE_ARROW)
                {
                    ...
                    for (scanner.restart(); !scanner.is_finish(); scanner.next())
                    {
                        ...
                        if (scanUnit->nation_recno == targetObj->nation_recno && scanUnit != this &&
                            ...
                            bullet_array.add_bullet_possible(
                                next_x_loc(), next_y_loc(), mobile_type, scanUnit->next_x_loc(), scanUnit->next_y_loc(),
                                scanUnit->mobile_type, scanUnit->loc_width, scanUnit->loc_height, range_attack_x_loc,
                                range_attack_y_loc, attackInfo->bullet_speed, attackInfo->bullet_sprite_id, this))
                        {
                ...
                if (unit_id == UNIT_JAPANESE_SPU && attackInfo->consume_power == 0 && nation_recno &&
                    tech_res[TECH_SPU_BERSERK(RACE_JAPANESE)]->get_nation_tech_level(nation_recno) &&
                    targetObj->cast_to_Unit())
                {
                    ...
                    WorldScanTraverser scanner;
                    scanner.init(targetObj->obj_loc_x1(), targetObj->obj_loc_y1(), targetObj->obj_loc_width(), 2);
                    scanner.sort_by_distance(targetObj);

                    // sort first, because filter is slower

                    int maxTarget = 2;
                    for (scanner.restart(); !scanner.is_finish() && maxTarget > 0; scanner.next())
                    {
                        Unit *scanUnit = unit_array[scanner.get_unit()];
                        if (scanUnit->nation_recno == targetObj->nation_recno && scanUnit != this &&
                            scanUnit->is_special_ability_effective() !=
    ...
    else // close attack
    {
        ...
        if (cur_frame == cur_sprite_attack()->frame_count)
        {
            ...
            float attackDamage =
                actual_damage() * attackInfo->attack_attribute.effectiveness(targetObj->defense_attribute);
                ...
                for (scanner.restart(); !scanner.is_finish(); scanner.next())
                {
                    ...
                    if ((!targetUnit || targetUnit != scanUnit) // not the target attacking
                        ...
                        misc.random(scanUnit->combat_level()) <
                            misc.random(combat_level() / 2)) // saving depend on combat level
                    {
                        // ###### begin Gilbert 10/4 ##########//
...
int Unit::validate_attack_target()
{
    ...
    else if (targetObj->obj_type == OBJ_UNIT)
    {
        ...
        if (has_pushed_order())
        {
            ...
            if (original_target_x_loc >= 0 && (get_behavior_mode() == UNIT_AGGRESSIVE || unit_mode))
            {
                ...
                enum
                {
                    AGGRESSIVE_CHASE_RANGE = 5
                };

                int curDistance = misc.points_distance(targetObj->obj_loc_x1(), targetObj->obj_loc_y1(),
...
int Unit::can_attack()
{
    return attack_count > 0 && (!is_civilian() || unit_mode == UNIT_MODE_TOWN_DEFENDER || unit_mode == UNIT_MODE_REBEL);
}
...
int Unit::can_attack_target(int targetObjRecno)
{
    ...
    */
    ...
    if (nation_recno)
    {
        ...
        if (targetUnit && targetUnit->unit_id == UNIT_EXPLOSIVE_CART && nation_recno == targetNationRecno)
        {
            return 1;
        }
...
void Unit::being_attack_hit(BaseObj *attackerObj, float damagePoint)
{
    ...
    if (damagePoint > 0.0f && (item.ability(ITEM_ABILITY_ABSORB_HIT) || item.ability(ITEM_ABILITY_INVINCIBLE)))
    {
        damagePoint = 0.0f;
        item.use_now();
    }
    ...
    if (attackerObj && attackerObj->cast_to_Unit())
    {
        ...
        if (is_monster())
        {
            ...
            // #### begin Gilbert 26/1 #######//
            if ((antiMonsterAbility = itemPtr->ability(ITEM_ABILITY_ANTI_MONSTER)) && itemPtr->para == monster_id())
            //			||
    ...
    if (unit_mode == UNIT_MODE_REBEL && attackerObj && attackerObj->nation_recno)
    {
        if (!rebel_array.is_deleted(unit_mode_para))
    ...
    if (attackerObj && !base_obj_array.is_deleted(attackerObj->base_obj_recno) // the object may be dying
        && !force_move_flag
        // ####### begin Gilbert 15/4 #######//
                                                       ...
                                                       // ####### end Gilbert 20/5 #######//
    {
        ...
        //			&& ask_team_help_attack(attackerObj) >=
        // MAX_ATTACK_HELPER
        //)
        //		{
        //			// empty
    ...
    if (attackerObj && attackerObj->cast_to_Unit())
    {
        ...
        if ((freezeProbability = attackUnit->item.ability(ITEM_ABILITY_FREEZE)) && misc.random(100) < freezeProbability)
        {
...
void Unit::auto_react_attack(BaseObj *attackerObj)
{
    ...
    if (get_behavior_mode() != UNIT_AGGRESSIVE && cur_action != SPRITE_ATTACK && cur_action != SPRITE_IDLE)
    {
        return;
    }
    ...
    if (attackerObj->cast_to_Town() && attackerObj->nation_recno == 0 && nation_recno != 0 &&
    ...
    if ( // cur_action == SPRITE_ATTACK &&		// two people attack one
        ...
        cur_order.mode == UNIT_ATTACK && !base_obj_array.is_deleted(cur_order.para))
    {
        BaseObj *attackingObj = base_obj_array[cur_order.para];

        int maxAttackRange = max_attack_range();

        //		if( attackingObj->cast_to_Unit()			// already attack an unit,
        // don't
        // change target
        //			|| !attackerObj->cast_to_Unit() )	// not attacking an unit, but
        // not
        // attacked by a unit, don't change target as well 			return;

        if (attackerObj->cast_to_Unit() &&
            (!attackingObj->cast_to_Unit()                   // if attacking non-unit switch to attack unit
             || area_distance(attackerObj) <= maxAttackRange // if attacking unit is out of range but new
...
int Unit::ask_team_help_attack(BaseObj *attackerObj)
{
    ...
    if (leader_unit_recno && !unit_array.is_deleted(leader_unit_recno)) // if the current unit is a soldier, get its
                                                                        // leader's recno
...
#define LEADER_IN_DANAGER_HIT_POINT 30
        ...
        int leaderInDanger =
            (leaderUnitRecno == sprite_recno) && 100 * hit_points / max_hit_points() < LEADER_IN_DANAGER_HIT_POINT;
        ...
        for (i = teamInfo->member_count - 1; i >= 0; i--)
        {
            ...
            if (unitPtr->get_behavior_mode() == UNIT_STAND_GROUND && targetDistance > maxAttackRange)
                continue;

            BaseObj *unitsTarget;
            int unitsTargetDistance;

            if (unitPtr->cur_order.mode == UNIT_ATTACK && !base_obj_array.is_deleted(unitPtr->cur_order.para) &&
            ...
            if (leaderInDanger && targetDistance <= maxAttackRange // behavior is bad unless unitPtr
...
void Unit::being_killed(BaseObj *attackerObj)
{
    ...
    if (attackerObj && attackerObj->nation_recno && rank_id == RANK_KING && is_monster())
    {
...
void Unit::being_killed_leave_money(BaseObj *attackerObj)
{
    ...
    // killMoney = killMoney * (80 + unique_id % 40) / 100;		// -20 to
    // +20 randomness
...
void Unit::being_killed_subordinate_betray(BaseObj *attackerObj)
{
    ...
    int netAttackerPower = world.net_attacker_power_in_area(next_x_loc(), next_y_loc(), attackerObj->nation_recno,
                                                            nation_recno, 0, hasWar, EFFECTIVE_LEADING_DISTANCE);
    ...
    for (int i = 0; i < team_info->member_count; i++)
    {
        ...
        if (misc.points_distance(unitPtr->next_x_loc(), unitPtr->next_y_loc(), next_x_loc(), next_y_loc()) >
            EFFECTIVE_LEADING_DISTANCE)
        {
            continue;
        }
        ...
        int betrayChance = 50 + netAttackerPower / 20 - 50 * (int)unitPtr->hit_points / (int)unitPtr->max_hit_points() -
        ...
        {
            betrayChance =
                40 - 50 * (int)unitPtr->hit_points / (int)unitPtr->max_hit_points() - (int)unitPtr->loyalty / 2;
...
int Unit::most_populated_nation_nearby(int scanRange)
{
    ...
    if (!bestNationId)
        bestNationId =
            misc.random(nation_array.size()) + 1; // if there is no human units nearby (perhaps just using weapons)
...
void Unit::target_move(BaseObj *targetObj)
{
    ...
    else // attackDistance <= max_attack_range()
    {
        ...
        // ####### begin Gilbert 16/12 ########//
        //		if(abs(cur_x-next_x)>=sprite_info->speed ||
        // abs(cur_y-next_y)>=sprite_info->speed) 			return;	// return as
        // moving
        if (abs(cur_x - next_x) >= sprite_speed() || abs(cur_y - next_y) >= sprite_speed())
...
void Unit::find_new_position_for_range_attack(BaseObj *targetObj)
{
    ...
    if (area_distance(targetObj) <= attackRange
        ...
        && bullet_array.add_bullet_possible(curXLoc, curYLoc, mobile_type, targetXLoc, targetYLoc,
                                            targetObj->obj_mobile_type(), targetWidth, targetHeight, range_attack_x_loc,
