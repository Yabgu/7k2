...
static StateArray town_state_array; // a state array for setting the locations
                                    // of nation towns.

//-------- Begin of function Battle::Battle --------//

Battle::Battle() { reset_placement_range(); }
...
void Battle::run(NewNationPara *mpGame, int mpPlayerCount) {
  ...
  race_res.second_init(); // it is called here because we want to reset
                          // name_used_array[] everytime
  hero_res.second_init(); // the second_init() of hero is called here instead of
                          // in Battle::second_init because hero_res will reset
                          // all appeared_flag excepts royal unit heroes
  ...
  if (game.is_campaign_mode()) {
    ...
    //			world.gen_mountains(0, 50);		// 50% the amount of mountains in a
    //campaign game is 50% less than a normal game.
  } else {
    init_uniqueness();
    ...
    //-- reset the cash and live points as it may be modified when creating game
    //objects --//
  ...
  if (!game.is_campaign_mode()) {
    ...
    if (config.goal_population_flag) {
      game.add_win_condition(E_REACH_POPULATION, 0, 0, config.goal_population,
                             0);
      game.add_win_condition(E_REACH_FRYHTAN, 0, 0, config.goal_population, 0);
    }

    if (config.goal_economic_score_flag) {
      game.add_win_condition(E_REACH_ECONOMY, 0, 0, config.goal_economic_score,
                             0);
    }
  ...
  // Set speed to normal. When hosting, broadcast the speed to the clients. As a
  // client, set to default speed initially.
  ...
  config.king_undie_flag =
      sys.debug_session && !mpGame &&
      !game.is_campaign_mode(); // sys.testing_session && !mpGame;
#else
...
void Battle::init_uniqueness() {
  ...
  //	race_res.second_init();		// race_res is initialized here instead
  //of in race_res because campaign will use it (before a game is started and
  //game.init() is called.)
...
void Battle::set_placement_range(int rangeX1, int rangeY1, int rangeX2,
                                 int rangeY2) {
...
void Battle::create_pregame_object(NewNationPara *mpGame, int mpPlayerCount) {
  ...
  if (!mpGame) {
    int nationRecno = nation_array.new_nation(
        NATION_HUMAN, NATION_OWN, config.race_id, config.player_nation_color);
  ...
  //--- if there is no space for creating new town/firm or unit, delete the
  //unprocessed nations ---//

  if (noSpaceFlag) {
    for (; nationRecno <= nation_array.size(); nationRecno++)
      nation_array.del_nation(
          nationRecno); // no space for creating a town for the nation, so we
                        // have to delete the nation
  }
  ...
  // ###### begin Gilbert 2/7 ######//
  // if( config.ai_monster_nation_count > 0 )			// only create independent
  // Fryhtan lairs if the number of Fryhtan kingdoms is set to larger than 0
...
void Battle::create_ai_nation() {
  ...
  for (i = 0; i < aiHumanCount; i++) {
    ...
    nation_array.new_nation(
        NATION_HUMAN, NATION_AI, raceId,
        nation_array
            .random_unused_color()); // 2nd parameter = the race id., 3rd
  ...
  for (i = 0; i < aiMonsterCount; i++) {
    ...
    nation_array.new_nation(
        NATION_MONSTER, NATION_AI, raceId,
        nation_array
            .random_unused_color()); // 2nd parameter = the race id., 3rd
                                     // parameters = color scheme id.
  }
...
//					  0 - failed, because of no available
//space
//
int Battle::create_one_human_asset(Nation *nationPtr) {
  ...
  if (config.random_start_up && !game.is_campaign_mode()) {
    ...
    int createCount =
        (60 - townPtr->population) / 3; // the less population the villager has
                                        // the more mobile units will be created
    ...
    for (int i = 0; i < createCount; i++) {
      ...
      if (!create_unit(nationPtr->nation_recno, unitId, rankId, townPtr->loc_x1,
                       townPtr->loc_y1, townPtr->loc_x2, townPtr->loc_y2)) {
        break;
      }
    }
  }
...
int Battle::create_human_town_unit(Nation *nationPtr) {
  ...
  for (i = 10; i > 0; i--) // try a maximum of 10 times
  {
    ...
    firmRecno = firm_array.generate_firm(
        townPtr->loc_x1 + STD_TOWN_LOC_WIDTH + INTER_PLACE_SPACE,
        townPtr->loc_y1, nationPtr->nation_recno, FIRM_FORT,
        race_res[nationPtr->race_id]->code);
  ...
  memset(firmFort->last_archer_fire_frame, 0,
         sizeof(firmFort->last_archer_fire_frame));
...
int Battle::create_human_town(Nation *nationPtr) {
  ...
  for (int i = town_state_array.size(); i > 0; i--) {
    ...
    if (world.can_build_town(xLoc, yLoc)) {
      townRecno =
          create_town(nationPtr->nation_recno, nationPtr->race_id, xLoc, yLoc);
  ...
  if (!townRecno)
    townRecno = create_town_random_loc(nationPtr->nation_recno,
                                       nationPtr->race_id, xLoc, yLoc);
...
int Battle::create_one_monster_asset(Nation *nationPtr) {
  ...
  int maxTries = (placement_range_x2 - placement_range_x1) *
                 (placement_range_y2 - placement_range_y1);
  FirmInfo *firmInfo = firm_res[FIRM_LAIR];

  if (!world.locate_space_random(
          xLoc, yLoc, placement_range_x2, placement_range_y2,
          firmInfo->loc_width + interPlaceSpace * 2,
          firmInfo->loc_height + interPlaceSpace * 2, maxTries)) {
    return 0;
  }
  ...
  int firmRecno =
      create_firm(xLoc + interPlaceSpace, yLoc + interPlaceSpace,
  ...
  int addCount = 300 / ((int)unit_res[unitId]->hit_points * 20 /
                        100); // 20 is the startup combat level
...
void Battle::create_independent_town(int addCount) {
  //--- only a maximum of 7 different races for independent towns, the 7 races
  //are determined by the races of the kingdoms in the game ----//
...
#define MAX_INDEPENDENT_TOWN_RACE_TYPE 7
  ...
  for (i = 0; i < 1000 && addCount > 0;
       i++) // 1000 is the maximum number of tries
  {
    if (++raceId > MAX_RACE)
      raceId = 1;
...
void Battle::create_road() {
  if (town_array.size() > 2) {
    ...
    roadMap.init(MAX_WORLD_X_LOC, MAX_WORLD_Y_LOC,
                 terrain_res.max_height(TERRAIN_DARK_GRASS));
    ...
    for (town1 = 1; town1 < town_array.size(); ++town1) {
      ...
      for (townRecno = town1 + 1; townRecno <= town_array.size(); ++townRecno) {
        ...
        int townDist =
            abs(town_array[townRecno]->center_x - town_array[town1]->center_x) +
      ...
      world.gen_road(roadMap, town_array[town1]->center_x,
                     town_array[town1]->center_y, town_array[town2]->center_x,
                     town_array[town2]->center_y);
    }
...
//
int Battle::create_unit(int nationRecno, int unitId, int rankId, int nextXLoc1,
                        int nextYLoc1, int nextXLoc2, int nextYLoc2) {
  ...
  if (nextXLoc1 >= 0) {
    ...
    world.check_unit_space_avoid_mountain =
        1; // set this for world.locate_space()_
    ...
    if (!world.locate_space_random(xLoc, yLoc, placement_range_x2,
                                   placement_range_y2, spriteInfo->loc_width,
                                   spriteInfo->loc_height, maxTries, 0, 0,
                                   teraMask)) {
      return 0;
    }
  }
  ...
  if (game.is_campaign_mode() && rankId == RANK_KING && nationRecno &&
    ...
    unitRecno = unit_array.add_unit(unitId, nationRecno, rankId, unitLoyalty,
                                    xLoc, yLoc);
  }
  // ###### end Gilbert 9/4 #########//
...
// [int] initPop 		= town population, if not specified, it will be
// randomly determined
//
...
int Battle::create_town(int nationRecno, int raceId, int xLoc, int yLoc,
  ...
  if (nationRecno) {
    ...
    townPtr->init_pop(initPop, 100, 0,
                      1); // 100-startup loyalty, last 1-first initialization at
...
//
// [int] initPop = town population, if not specified, it will be randomly
// determined
//
// return: <int> townRecno - >0  the recno of the town created
//                           ==0 no town created
//
int Battle::create_town_random_loc(int nationRecno, int raceId, int &xLoc,
                                   int &yLoc, int initPop) {
  //------- locate for a space to build the town ------//

  int maxTries = (placement_range_x2 - placement_range_x1) *
                 (placement_range_y2 - placement_range_y1);

  if (!town_array.think_town_loc(maxTries, xLoc, yLoc, placement_range_x1,
                                 placement_range_y1, placement_range_x2,
                                 placement_range_y2)) {
    return 0;
  }
...
//                           ==0 no town created
//
int Battle::create_town_next_to_place(Place *placePtr, int nationRecno,
                                      int raceId, int initPop) {
  ...
  if (!callNationPtr->find_best_place_loc(0, placePtr->loc_x1, placePtr->loc_y1,
                                          buildXLoc, buildYLoc))
    return 0;
...
//
int Battle::create_firm(int xLoc, int yLoc, int nationRecno, int firmId,
                        int firmRaceId) {
  //----- get the build code -----//
...
int Battle::create_firm_next_to_place(Place *placePtr, int firmId,
                                      int firmRaceId, int nationRecno,
                                      short distance) {
  short buildXLoc, buildYLoc;
  ...
  if (!nationPtr->find_best_place_loc(firmId, placePtr->loc_x1,
                                      placePtr->loc_y1, buildXLoc, buildYLoc,
                                      distance))
    return 0;
  ...
  int effectiveDis = world.effective_distance(
      firmId,
      (placePtr->cast_to_Firm() ? placePtr->cast_to_Firm()->firm_id : 0));

  if (misc.points_distance(placePtr->center_x, placePtr->center_y,
                           buildXLoc + firmInfo->loc_width / 2,
                           buildYLoc + firmInfo->loc_height / 2) >
      effectiveDis) {
    return 0;
  }
  ...
  return firm_array.generate_firm(buildXLoc, buildYLoc, nationRecno, firmId,
                                  buildCode);
}
...
void Battle::create_monster_lair(int nationRecno, int addCount) {
  ...
  for (int i = 0; i < addCount; i++) {
    ...
    if (!world.locate_space_random(
            xLoc, yLoc, placement_range_x2, placement_range_y2,
            firmInfo->loc_width + LAIR_INTER_SPACE * 2,
            firmInfo->loc_height + LAIR_INTER_SPACE * 2,
            maxTries)) // 5 space locations surrounding the building
    {
      break;
    }
    ...
    int firmRecno = firm_array.generate_firm(
        xLoc + LAIR_INTER_SPACE, yLoc + LAIR_INTER_SPACE, nationRecno,
    ...
    if (!world.locate_space(xLoc, yLoc, xLoc + firmInfo->loc_width - 1,
                            yLoc + firmInfo->loc_height - 1,
                            spriteInfo->loc_width, spriteInfo->loc_height)) {
    ...
    if (nationRecno == 0 ||
        nation_array[nationRecno]
            ->king_unit_recno) // if it is an independent nation or the nation
                               // unit already has a king, then set this unit to
                               // general
      rankId = RANK_GENERAL;
    else
      rankId = RANK_KING;
    ...
    int unitRecno =
        unit_array.add_unit(unitId, nationRecno, rankId, 0, xLoc, yLoc);
...
//                0 - not all are space
//
int Battle::is_space(int xLoc1, int yLoc1, int xLoc2, int yLoc2,
                     char mobileType) {
  int xLoc, yLoc;
  Location *locPtr;
  ...
  /*
     ...
     ---//
          ...
          if( i>town_array.size() )		// the player has no town
          {
                  ...
                  short xLoc, yLoc;
                  unitKing->get_cur_loc(xLoc, yLoc);		// get the
     unit's location no matter whether the unit is in a building or an open
     space
...
void Battle::inverse_placement_range() {
  ...
  if (placement_range_y1 == 0 && placement_range_y2 == MAX_WORLD_Y_LOC - 1) {
    ...
    {
      placement_range_x1 =
          placement_range_x2; // now we shift it to the right side
      placement_range_x2 = MAX_WORLD_X_LOC - 1;
    } else // if it is on the right side
    {
      placement_range_x2 -=
          placement_range_x1; // now we shift it to the left side
      placement_range_x1 = 0;
    }
  } else //----- if it is vertically split -------//
  {
    ...
    {
      placement_range_y1 =
          placement_range_y2; // now we shift it to the bottom side
      placement_range_y2 = MAX_WORLD_Y_LOC - 1;
    } else // if it is on the right side
    {
      placement_range_y2 -=
          placement_range_y1; // now we shift it to the top side
      placement_range_y1 = 0;
    }
  }
}
...
void Battle::create_random_item() {
  ...
  for (int i = 0; i < addCount; i++) {
    ...
    if (!world.locate_space_random(
            xLoc, yLoc, placement_range_x2, placement_range_y2, 3, 3,
            maxTries)) // locate a space of 3x3 and add the item on the center
    {
      break;
    }
    ...
    site_array.add_site(xLoc + 1, yLoc + 1, SITE_ITEM, randomItem.id,
                        randomItem.para);
    // ##### end Gilbert 24/12 #######//
  }
}
