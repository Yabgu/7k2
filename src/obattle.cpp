...
void Battle::run(NewNationPara *mpGame, int mpPlayerCount)
{
    ...
    if (game.is_campaign_mode())
    {
        ...
        //			world.gen_mountains(0, 50);		// 50% the amount of mountains in
        // a
    ...
    err_when(nation_array.player_recno && (~nation_array)->nation_recno != nation_array.player_recno);
    ...
    config.king_undie_flag =
        sys.debug_session && !mpGame && !game.is_campaign_mode(); // sys.testing_session && !mpGame;
#else
...
void Battle::create_pregame_object(NewNationPara *mpGame, int mpPlayerCount)
{
    ...
    if (!mpGame)
    {
        int nationRecno = nation_array.new_nation(NATION_HUMAN, NATION_OWN, config.race_id, config.player_nation_color);
    ...
    if (noSpaceFlag)
    {
        for (; nationRecno <= nation_array.size(); nationRecno++)
            nation_array.del_nation(nationRecno); // no space for creating a town for the nation, so we
    ...
    // ###### begin Gilbert 2/7 ######//
    // if( config.ai_monster_nation_count > 0 )			// only create
    // independent Fryhtan lairs if the number of Fryhtan kingdoms is set to
    // larger than 0
...
void Battle::create_ai_nation()
{
    ...
    for (i = 0; i < aiHumanCount; i++)
    {
        ...
        nation_array.new_nation(NATION_HUMAN, NATION_AI, raceId,
    ...
    for (i = 0; i < aiMonsterCount; i++)
    {
        ...
        nation_array.new_nation(NATION_MONSTER, NATION_AI, raceId,
                                nation_array.random_unused_color()); // 2nd parameter = the race id., 3rd
                                                                     // parameters = color scheme id.
    }
...
int Battle::create_one_human_asset(Nation *nationPtr)
{
    ...
    if (config.random_start_up && !game.is_campaign_mode())
    {
        ...
        int createCount = (60 - townPtr->population) / 3; // the less population the villager has
        ...
        for (int i = 0; i < createCount; i++)
        {
            ...
            if (!create_unit(nationPtr->nation_recno, unitId, rankId, townPtr->loc_x1, townPtr->loc_y1, townPtr->loc_x2,
                             townPtr->loc_y2))
            {
                break;
            }
...
int Battle::create_human_town_unit(Nation *nationPtr)
{
    ...
    for (i = 10; i > 0; i--) // try a maximum of 10 times
    {
        ...
        firmRecno = firm_array.generate_firm(townPtr->loc_x1 + STD_TOWN_LOC_WIDTH + INTER_PLACE_SPACE, townPtr->loc_y1,
                                             nationPtr->nation_recno, FIRM_FORT, race_res[nationPtr->race_id]->code);
...
int Battle::create_human_town(Nation *nationPtr)
{
    ...
    for (int i = town_state_array.size(); i > 0; i--)
    {
        ...
        if (xLoc < placement_range_x1 || yLoc < placement_range_y1 || xLoc > placement_range_x2 ||
            yLoc > placement_range_y2)
        {
            continue;
        }
...
int Battle::create_one_monster_asset(Nation *nationPtr)
{
    ...
    int firmRecno = create_firm(xLoc + interPlaceSpace, yLoc + interPlaceSpace, nationPtr->nation_recno, FIRM_LAIR,
                                nationPtr->race_id);
...
//
int Battle::create_unit(int nationRecno, int unitId, int rankId, int nextXLoc1, int nextYLoc1, int nextXLoc2,
                        int nextYLoc2)
{
    ...
    if (nextXLoc1 >= 0)
    {
        ...
        if (!world.locate_space(xLoc, yLoc, nextXLoc2, nextYLoc2, spriteInfo->loc_width, spriteInfo->loc_height))
        {
    ...
    else
    {
        ...
        if (!world.locate_space_random(xLoc, yLoc, placement_range_x2, placement_range_y2, spriteInfo->loc_width,
                                       spriteInfo->loc_height, maxTries, 0, 0, teraMask))
        {
            return 0;
        }
    }
    ...
    if (game.is_campaign_mode() && rankId == RANK_KING && nationRecno &&
        ...
        (cNationPtr = game.campaign()->get_nation(cNationRecno)) && cNationPtr->king.unit_id)
    {
...
int Battle::create_town(int nationRecno, int raceId, int xLoc, int yLoc, int initPop)
{
    ...
    else
    {
        ...
        townPtr->init_pop(initPop, townResistance, 0,
...
int Battle::create_town_random_loc(int nationRecno, int raceId, int &xLoc, int &yLoc, int initPop)
{
    ...
    if (!town_array.think_town_loc(maxTries, xLoc, yLoc, placement_range_x1, placement_range_y1, placement_range_x2,
                                   placement_range_y2))
    {
        return 0;
    }
...
//								  new place.
//
int Battle::create_firm_next_to_place(Place *placePtr, int firmId, int firmRaceId, int nationRecno, short distance)
{
    short buildXLoc, buildYLoc;
    ...
    int effectiveDis =
        world.effective_distance(firmId, (placePtr->cast_to_Firm() ? placePtr->cast_to_Firm()->firm_id : 0));

    if (misc.points_distance(placePtr->center_x, placePtr->center_y, buildXLoc + firmInfo->loc_width / 2,
                             buildYLoc + firmInfo->loc_height / 2) > effectiveDis)
    {
        return 0;
    }
...
void Battle::create_monster_lair(int nationRecno, int addCount)
{
    ...
    for (int i = 0; i < addCount; i++)
    {
        ...
        if (nationRecno == 0 || nation_array[nationRecno]->king_unit_recno) // if it is an independent nation or the
                                                                            // nation unit already has a king, then set
                                                                            // this unit to general
            rankId = RANK_GENERAL;
        else
            rankId = RANK_KING;
...
void Battle::create_random_item()
{
    ...
    for (int i = 0; i < addCount; i++)
    {
        ...
        if (!world.locate_space_random(xLoc, yLoc, placement_range_x2, placement_range_y2, 3, 3,
