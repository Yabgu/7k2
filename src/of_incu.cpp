...
short FirmIncubator::incubate_unit_id[MAX_INCUBATE_TYPE] = {
    // ###### begin Gilbert 26/1 #######//
    UNIT_FR_BALLISTA, UNIT_F_CATAPULT, UNIT_F_CANNON,
    // ###### end Gilbert 26/1 #######//
};
...
void FirmIncubator::remove_queue(int unitId) {
  for (int i = build_queue_count - 1; i >= 0; i--) {
    if (build_queue_array[i] == unitId) {
      ...
      misc.del_array_rec(build_queue_array, build_queue_count,
                         sizeof(build_queue_array[0]), i + 1);
...
//
void FirmIncubator::cancel_build_unit() { build_unit_id = 0; }
...
void FirmIncubator::process_build() {
  ...
  if (build_progress_days > totalBuildDays) {
    ...
    if (is_own())
      se_res.far_sound(center_x, center_y, 1, 'F', firm_id, "FINS", 'S',
                       unit_res[build_unit_id]->sprite_id);
...
void FirmIncubator::process_queue() {
  ...
  if (nationPtr->cash < unit_res[build_unit_id]->build_cost ||
      nationPtr->live_points <
          unit_res[build_unit_id]->build_live_points_cost) {
    build_unit_id = 0;
    return;
  }

  nationPtr->add_expense(EXPENSE_WEAPON, unit_res[build_unit_id]->build_cost,
                         1);
  nationPtr->change_live_points(
      float(-unit_res[build_unit_id]->build_live_points_cost));

  err_when(build_queue_count > MAX_BUILD_QUEUE);

  misc.del_array_rec(build_queue_array, build_queue_count,
                     sizeof(build_queue_array[0]), 1);
