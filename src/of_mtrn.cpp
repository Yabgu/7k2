...
FirmMonsterTrain::FirmMonsterTrain(int trainUnitId) {
  memset(sizeof(Firm) + (char *)this, 0,
         sizeof(FirmMonsterTrain) - sizeof(Firm));
...
int FirmMonsterTrain::can_recruit(int trainUnitId) {
  ...
  if (!nation_recno ||
      unit_res[trainUnitId]->get_nation_tech_level(nation_recno) == 0)
    return 0;
...
void FirmMonsterTrain::basic_train() {
  for (int i = 1; i <= trainee_count; ++i) {
    ...
    if (monsterTrainee->is_under_training()) {
      float incPoints = (float)BASIC_COMBAT_TRAIN /
                        unit_res[monsterTrainee->unit_id]->build_days;
...
int FirmMonsterTrain::mobilize_trainee(int traineeId, char remoteAction) {
  ...
  misc.del_array_rec(trainee_array, trainee_count, sizeof(MonsterTrainee),
                     traineeId);
...
int FirmMonsterTrain::create_trainee_unit(MonsterTrainee &thisTrainee) {
  ...
  int unitRecno =
      create_unit(thisTrainee.unit_id); // this MonsterTrainee no longer has a
                                        // job as it has been resigned
  ...
  unitPtr->set_combat_level(-1); // -1 for current combat level so as to update
                                 // unit attribute of that combat level
...
void FirmMonsterTrain::change_nation(int newNationRecno) {
  ...
  for (int i = 0; i < trainee_count; i++) {
    if (!trainee_array[i]
             .is_under_training()) // inc_nation_unit_count when training finish
