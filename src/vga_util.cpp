...
void VgaUtil::blt_buf(int x1, int y1, int x2, int y2, int putBackCursor) {
  ...
  if (putBackCursor) {
    mouse_cursor.hide_area_flag =
        0; // do not call mouse.hide_area() which will double paint the cursor
  ...
  if (putBackCursor)
    mouse_cursor.hide_area_flag =
        0; // do not call mouse.show_area() which will double paint the cursor
  else
    mouse.show_area();
...
//                        (default: 0)
//
void VgaUtil::d3_panel_up(int x1, int y1, int x2, int y2, int vgaFrontOnly,
                          int drawBorderOnly) {
  ...
  if (!drawBorderOnly) {
    ...
    if (Vga::opaque_flag)
      ...
      // vgaBuf->bar_alpha(x1+1, y1+1, x2-1, y2-1, IF_UP_BRIGHTNESS_ADJUST/2-1,
      // V_WHITE);
      vgaBuf->bar_alpha(x1 + 1, y1 + 1, x2 - 1, y2 - 1,
                        IF_UP_BRIGHTNESS_ADJUST / 2 - 1, VgaBuf::color_light);
    // ##### end Gilbert 19/10 #####//
  }
  ...
  if (!vgaFrontOnly && !vga.use_back_buf) // only blt the back to the front is
                                          // the active buffer is the front
...
//                        (default: 0)
//
void VgaUtil::d3_panel_down(int x1, int y1, int x2, int y2, int vgaFrontOnly,
                            int drawBorderOnly) {
  err_when(x1 > x2 || y1 > y2 || x1 < 0 || y1 < 0 || x2 >= VGA_WIDTH ||
           y2 >= VGA_HEIGHT);
  ...
  if (!drawBorderOnly) {
    ...
    if (Vga::opaque_flag)
      ...
      // vgaBuf->bar_alpha(x1+1, y1+1, x2-1, y2-1,
      // IF_DOWN_BRIGHTNESS_ADJUST/2-1, V_WHITE);
      vgaBuf->bar_alpha(x1 + 1, y1 + 1, x2 - 1, y2 - 1,
                        IF_DOWN_BRIGHTNESS_ADJUST / 2 - 1, VgaBuf::color_dark);
    // ##### end Gilbert 19/10 ######//
  }
...
//                        (default: 0)
//
void VgaUtil::d3_panel2_up(int x1, int y1, int x2, int y2, int vgaFrontOnly,
                           int drawBorderOnly) {
  err_when(x1 > x2 || y1 > y2 || x1 < 0 || y1 < 0 || x2 >= VGA_WIDTH ||
           y2 >= VGA_HEIGHT);
  ...
  if (!drawBorderOnly)
    // vgaBuf->adjust_brightness(x1+2, y1+2, x2-3, y2-3,
    // IF_UP_BRIGHTNESS_ADJUST);
    vgaBuf->bar_alpha(x1 + 2, y1 + 2, x2 - 3, y2 - 3,
                      IF_UP_BRIGHTNESS_ADJUST / 2 - 1, V_WHITE);
...
//                        (default: 0)
//
void VgaUtil::d3_panel2_down(int x1, int y1, int x2, int y2, int vgaFrontOnly,
                             int drawBorderOnly) {
  err_when(x1 > x2 || y1 > y2 || x1 < 0 || y1 < 0 || x2 >= VGA_WIDTH ||
           y2 >= VGA_HEIGHT);
...
void VgaUtil::separator(int x1, int y1, int x2, int y2) {
  ...
  if (y1 + 1 == y2) // horizontal line
  {
    ...
    vga_front.bar_alpha(x1, y1, x2, y1, IF_UP_BRIGHTNESS_ADJUST / 2 - 1,
                        V_WHITE);
    // vga_front.adjust_brightness(x1, y2, x2, y2, IF_DOWN_BRIGHTNESS_ADJUST);
    vga_front.bar_alpha(x1, y2, x2, y2, IF_DOWN_BRIGHTNESS_ADJUST / 2 - 1,
                        V_WHITE);
  } else {
    // vga_front.adjust_brightness(x1, y1, x1, y2, IF_UP_BRIGHTNESS_ADJUST);
    vga_front.bar_alpha(x1, y1, x1, y2, IF_UP_BRIGHTNESS_ADJUST / 2 - 1,
                        V_WHITE);
    // vga_front.adjust_brightness(x2, y1, x2, y2, IF_DOWN_BRIGHTNESS_ADJUST);
    vga_front.bar_alpha(x2, y1, x2, y2, IF_DOWN_BRIGHTNESS_ADJUST / 2 - 1,
                        V_WHITE);
  }
}
...
void VgaUtil::disp_image_file(const char *fileName, int x1, int y1) {
  ...
  if (misc.is_file_exist(str)) {
    ...
    // if( !back_up_pal )		// only save the palette when there isn't one saved
    // already
