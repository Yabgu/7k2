...
// <1st message length (short), not including this 2 bytes> <1st message
// content> <2nd message length> <2nd message content> ...
...
void RemoteQueue::traverse_next()
{
        ...
        queue_ptr += *(unsigned short *)queue_ptr + sizeof(unsigned short);
// skip the message length as well
}
...
int RemoteQueue::validate_queue(int start) {
  ...
  for (rqt.traverse_set_start(start); !rqt.traverse_finish();
       rqt.traverse_next()) {
    RemoteMsg *remoteMsgPtr = rqt.get_remote_msg();
    if (remoteMsgPtr->id) {
      if (remoteMsgPtr->id < FIRST_REMOTE_MSG_ID ||
          remoteMsgPtr->id > LAST_REMOTE_MSG_ID)
        return 0;
    }
...
void RemoteQueueTraverse::traverse_set_start(int start) { offset = start; }
