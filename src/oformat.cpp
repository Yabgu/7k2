...
void UnitGroup::line_formation(int direction)
{
    ...
    if (!check && !direction)
    {
        ...
        {
            unitPtr = get_unit(t);
            for (i = 0, temperoryLocationForInDirectionX = unitPtr->next_x_loc(); i < unitPtr->obj_loc_width();
                 i++, temperoryLocationForInDirectionX++)
                for (j = 0, temperoryLocationForInDirectionY = unitPtr->next_y_loc(); j < unitPtr->obj_loc_height();
                     j++, temperoryLocationForInDirectionY++)
                    world.get_loc(temperoryLocationForInDirectionX, temperoryLocationForInDirectionY)
    ...
    for (tSelected = 0; tSelected < nSelected; tSelected++)
    {
        ...
        for (i = 0, temperoryLocationForInDirectionX = sortedArray[tSelected]->next_x_loc(); i < sizeArrayX[tSelected];
             i++, temperoryLocationForInDirectionX++)
            for (j = 0, temperoryLocationForInDirectionY = sortedArray[tSelected]->next_y_loc();
    ...
    for (tSelected = 0; tSelected < nSelected; tSelected++)
    {
        ...
        else if (leaderAtMiddleWithSpanAlongPositiveXAndPositiveY)
        {
            if (tSelected % 2)
            {
                ...
                sortedArray[tSelected]->move((oldLocX2 = oldLocX2 - sizeArrayX[tSelected]),
...
void UnitGroup::circle_formation(int direction)
{
    ...
    int minLocY = MAX_WORLD_Y_LOC;
    int nRange = 0,
        nClose = 0; // number of units with range attack and close attack respectively
    Unit *general = NULL;
    //-------------------------------------//
    for (t = 1; t <= nSelected; t++)
    {
    ...
    {
        if (ir == 0)
            rangeAttackUnits[ir]->move(centerX - rangeAttackUnits[ir]->obj_loc_width(),
    ...
    for (tSelected = 0; tSelected < nOther; tSelected++)
    {
        ...
        unitsArray[tSelected]->move((int)(centerX - cosineArray[tSelected]), (int)(centerY + sineArray[tSelected]));
...
void UnitGroup::arc_formation(int direction)
{
    ...
    if (!direction || direction == LEADER_AT_TOP || direction == LEADER_AT_BOTTOM || direction == LEADER_AT_RIGHT ||
    ...
    else if (slantPosYPosX)
    {
        ...
        if (nRange)
        {
            ...
            {
                if (!ir)
                    rangeAttackUnits[ir]->move((int)(circleCenterX + sineAdd45ArrayR[0]),
    ...
    else if (slantPosYNegX)
    {
        ...
        if (nRange)
        {
            ...
            {
                if (!ir)
                    rangeAttackUnits[ir]->move((int)(circleCenterX + sineSub45ArrayR[0]),
...
void UnitGroup::staggered_formation(int direction)
{
    ...
    for (tSelected = 0; tSelected < nSelected; tSelected++)
    {
        if (leaderAtMiddleSpanX)
        {
            if (tSelected % 2)
            {
                sortedArray[tSelected]->move(tempOldX1,
            ...
            {
                tempOldX2 -= sizeArrayX[tSelected];
                sortedArray[tSelected]->move(tempOldX2,
                                             (locY + multiplySign * maximumSizeY * (((tSelected + 1) / 2) % 2)));
            }
        }
        else if (leaderAtMiddleSpanY)
        {
            if (tSelected % 2)
            {
                sortedArray[tSelected]->move((locX + multiplySign * maximumSizeX * (((tSelected + 1) / 2) % 2)),
                                             tempOldY1);
                tempOldY1 += sizeArrayY[tSelected];
            }
            else
...
void UnitGroup::square_formation(int direction)
{
    ...
    if (!direction || direction == LEADER_AT_TOP)
    {
        ...
        while (check && tSelected < nSelected)
        {
            ...
            for (i = 0, theSecondTemperoryX = temperoryX; i < sortedArray[tSelected]->obj_loc_width();
                 i++, theSecondTemperoryX++)
                for (j = 0, theSecondTemperoryY = temperoryY; j < sortedArray[tSelected]->obj_loc_height();
    ...
    for (tSelected = 0; tSelected < nSelected; tSelected++)
    {
        ...
        // setting the occupancy of the lcoations originally occupied by the units
        // to be true
        for (i = 0, tempLocX = sortedArray[tSelected]->next_x_loc(); i < sortedArray[tSelected]->obj_loc_width();
             i++, tempLocX++)
            for (j = 0, tempLocY = sortedArray[tSelected]->next_y_loc(); j < sortedArray[tSelected]->obj_loc_height();
                 j++, tempLocY++)
                world.get_loc(tempLocX, tempLocY)->set_unit(sortedArray[tSelected]->sprite_recno, mobile_type);
    }
    ...
    for (tSelected = 0; tSelected < nSelected; tSelected++)
    {
        if (!slant)
        {
            if (yMultiplicativeFactor == xMultiplicativeFactor)
                sortedArray[tSelected]->move((locX + (xMultiplicativeFactor) * (tSelected % numberOfLines)),
        ...
        else
        {
            ...
            sortedArray[tSelected]->move(firstX + xMultiplicativeFactor * (tSelected % numberOfLines) * maximumSizeX,
                                         firstY + yMultiplicativeFactor * (tSelected % numberOfLines) * maximumSizeY);
        }
