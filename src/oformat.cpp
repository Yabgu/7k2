...
void UnitGroup::vanguard_formation(int direction) {
  ...
  for (t = 1; t <= nSelected; t++) {
    ...
    //--set the occupancy of the place where is originally occupied by the troop
    //to be false--//
  ...
  for (i = 0; i < nClose; i++) {
    for (j = 0; j < i; j++) {
      if (closeAttackUnits[j]->max_attack_range() >
          closeAttackUnits[i]->max_attack_range()) {
  ...
  for (ic = 0; ic < nClose; ic++) {
    ...
    err_when(ic > 0 && closeAttackUnits[ic]->max_attack_range() <
                           closeAttackUnits[ic - 1]->max_attack_range());
  }
  ...
  if (!direction || direction == LEADER_AT_TOP) {
    ...
    while (check && ic < nClose) {
      if (ic % 2) {
        ...
        temperoryX =
            (tempOldX2 = tempOldX2 - closeAttackUnits[ic]->obj_loc_width());
    ...
    if (nRange) {
      ...
      while (check && ir < nRange) {
        if (ir % 2) {
          ...
          temperoryX =
              (tempOldX2 = tempOldX2 - rangeAttackUnits[ir]->obj_loc_width());
          temperoryY = tempOldY2;
  ...
  if ((!direction && !check) || direction == LEADER_AT_BOTTOM) {
    ...
    while (check && ic < nClose) {
      if (ic % 2) {
        ...
        tempOldX1 += (closeAttackUnits[ic]->obj_loc_width());
        temperoryY =
            (tempOldY1 = tempOldY1 - closeAttackUnits[ic]->obj_loc_height());
      } else {
        temperoryX =
            (tempOldX2 = tempOldX2 - closeAttackUnits[ic]->obj_loc_width());
        temperoryY =
            (tempOldY2 = tempOldY2 - closeAttackUnits[ic]->obj_loc_height());
      }
      if (!closeAttackUnits[ic]->can_move(temperoryX, temperoryY))
        check = false;
      ic++;
    ...
    if (nRange) {
      ...
      while (check && ir < nRange) {
        if (ir % 2) {
          ...
          tempOldX1 += (rangeAttackUnits[ir]->obj_loc_width());
          temperoryY =
              (tempOldY1 = tempOldY1 - rangeAttackUnits[ir]->obj_loc_height());
        } else {
          temperoryX =
              (tempOldX2 = tempOldX2 - rangeAttackUnits[ir]->obj_loc_width());
          temperoryY =
              (tempOldY2 = tempOldY2 - rangeAttackUnits[ir]->obj_loc_height());
        }
        if (!rangeAttackUnits[ir]->can_move(temperoryX, temperoryY))
          check = false;
        ir++;
  ...
  if ((!direction && !check) || direction == LEADER_AT_RIGHT) {
    ...
    while (check && ic < nClose) {
      if (ic % 2) {
        temperoryX =
            (tempOldX1 = tempOldX1 - closeAttackUnits[ic]->obj_loc_width());
        temperoryY = tempOldY1;
        tempOldY1 += (closeAttackUnits[ic]->obj_loc_height());
      } else {
        temperoryX =
    ...
    if (nRange) {
      ...
      while (check && ir < nRange) {
        if (ir % 2) {
          temperoryX =
              (tempOldX1 = tempOldX1 - rangeAttackUnits[ir]->obj_loc_width());
          temperoryY = tempOldY1;
          tempOldY1 += (rangeAttackUnits[ir]->obj_loc_height());
        } else {
          temperoryX =
  ...
  if (!check && !direction) {
    for (t = 1; t <= nSelected; t++) {
      unitPtr = get_unit(t);
      for (i = 0, tempLocX = unitPtr->next_x_loc();
           i < unitPtr->obj_loc_width(); i++, tempLocX++)
        for (j = 0, tempLocY = unitPtr->next_y_loc();
             j < unitPtr->obj_loc_height(); j++, tempLocY++)
          world.get_loc(tempLocX, tempLocY)
              ->set_unit(unitPtr->sprite_recno, mobile_type);
    }
    if (rangeAttackUnits)
  ...
  for (t = 1; t <= nSelected; t++) {
    ...
    // setting the occupancy of the lcoations originally occupied by the units
    // to be true
    for (i = 0, tempLocX = unitPtr->next_x_loc(); i < unitPtr->obj_loc_width();
         i++, tempLocX++)
      for (j = 0, tempLocY = unitPtr->next_y_loc();
           j < unitPtr->obj_loc_height(); j++, tempLocY++)
        world.get_loc(tempLocX, tempLocY)
...
// This function calculate the locations and set the unit to move to form a line
// formation if longline is true, long line formation will be form. if longline
// is false, short line formation will be form.
void UnitGroup::line_formation(int direction) {
  //--------------Define local variables----------------//
  int nSelected = 0;
  int locX, locY, i, j, t, temperoryLocationForInDirectionX,
      temperoryLocationForInDirectionY, tSelected = 0;
  int temperoryX, temperoryY;
  ...
  for (i = 0; i < nSelected; i++) {
    for (j = 0; j < i; j++) {
      if (sortedArray[j]->max_attack_range() >
          sortedArray[i]->max_attack_range()) {
  ...
  for (i = 0; i < nSelected; i++) {
    ...
    err_when(i > 0 && sortedArray[i]->max_attack_range() <
                          sortedArray[i - 1]->max_attack_range());
  }
  ...
  if (!direction || direction == LEADER_AT_LEFT ||
    ...
    for (i = 0, theSecondTemperoryX = locX; i < sizeArrayX[0];
         i++, theSecondTemperoryX++)
      for (j = 0, theSecondTemperoryY = locY; j < sizeArrayY[0];
           j++, theSecondTemperoryY++)
        if (!world.get_loc(theSecondTemperoryX, theSecondTemperoryY)
                 ->can_move(mobile_type))
          check = false;
    ...
    while (check && tSelected < nSelected) {
      ...
      temperoryY = locY;
      for (i = 0, theSecondTemperoryX = temperoryX; i < sizeArrayX[tSelected];
           i++, theSecondTemperoryX++)
        for (j = 0, theSecondTemperoryY = temperoryY; j < sizeArrayY[tSelected];
             j++, theSecondTemperoryY++)
          if (!world.get_loc(theSecondTemperoryX, theSecondTemperoryY)
                   ->can_move(mobile_type))
            check = false;
  ...
  if ((!check && !direction) || direction == LEADER_AT_TOP_LEFT ||
      direction == LEADER_AT_BOTTOM_RIGHT) {
    locX = centerX;
    locY = centerY;
    if ((diff = locX + lengthRight + 1 - MAX_WORLD_X_LOC) > 0)
      locX -= diff;
  ...
  if ((!check && !direction) || direction == LEADER_AT_TOP_RIGHT ||
      direction == LEADER_AT_BOTTOM_LEFT) {
    locX = centerX;
    locY = centerY + 1;
    if ((diff = locX + lengthRight + 1 - MAX_WORLD_X_LOC) > 0)
      locX -= diff;
  ...
  if (!check && !direction) {
    ...
    for (t = 1; t <= nSelected; t++) {
      unitPtr = get_unit(t);
      for (i = 0, temperoryLocationForInDirectionX = unitPtr->next_x_loc();
           i < unitPtr->obj_loc_width();
           i++, temperoryLocationForInDirectionX++)
        for (j = 0, temperoryLocationForInDirectionY = unitPtr->next_y_loc();
             j < unitPtr->obj_loc_height();
             j++, temperoryLocationForInDirectionY++)
          world
              .get_loc(temperoryLocationForInDirectionX,
                       temperoryLocationForInDirectionY)
              ->set_unit(unitPtr->sprite_recno, mobile_type);
    }
    return;
  }
  ...
  for (tSelected = 0; tSelected < nSelected; tSelected++) {
    if (sortedArray[tSelected]->team_info &&
        !sortedArray[tSelected]->team_info->formation_direction)
      sortedArray[tSelected]->team_info->formation_direction = finalDirection;

    for (i = 0,
        temperoryLocationForInDirectionX = sortedArray[tSelected]->next_x_loc();
         i < sizeArrayX[tSelected]; i++, temperoryLocationForInDirectionX++)
      for (j = 0, temperoryLocationForInDirectionY =
                      sortedArray[tSelected]->next_y_loc();
           j < sizeArrayY[tSelected]; j++, temperoryLocationForInDirectionY++)
        world
            .get_loc(temperoryLocationForInDirectionX,
                     temperoryLocationForInDirectionY)
            ->set_unit(sortedArray[tSelected]->sprite_recno, mobile_type);
  }
  ...
  for (tSelected = 0; tSelected < nSelected; tSelected++) {
    if (leaderAtMiddleWithSpanAlongXOK) {
      if (tSelected % 2) {
        ...
        sortedArray[tSelected]->move(
            (oldLocX2 = oldLocX2 - sizeArrayX[tSelected]), locY);
      ...
      if (tSelected % 2) {
        ...
        sortedArray[tSelected]->move(
            locX, (oldLocY2 = oldLocY2 - sizeArrayY[tSelected]));
      ...
      if (tSelected % 2) {
        ...
        sortedArray[tSelected]->move(
            (oldLocX2 = oldLocX2 - sizeArrayX[tSelected]),
            oldLocY2 = oldLocY2 - sizeArrayY[tSelected]);
    } else if (leaderAtMiddleWithSpanAlongPositiveXAndNegativeY) {
      if (tSelected % 2) {
        sortedArray[tSelected]->move(
            oldLocX1, (oldLocY1 = oldLocY1 - sizeArrayY[tSelected]));
        oldLocX1 += sizeArrayX[tSelected];
      } else {
        sortedArray[tSelected]->move(
            (oldLocX2 = oldLocX2 - sizeArrayX[tSelected]), oldLocY2);
...
// This function calculates the location and set the unit to move to form a
// circle formation
void UnitGroup::circle_formation(int direction) {
  ...
  int nRange = 0,
      nClose =
          0; // number of units with range attack and close attack respectively
  Unit *general = NULL;
  //-------------------------------------//
  for (t = 1; t <= nSelected; t++) {
    unitPtr = get_unit(t);
  ...
  // set unit of range attack to the middle of the circle, but the number cannot
  // exceed 4, otherwise the formation will be very ugly
  if (nRange > 4)
    nRange = 4;
  ...
  if (nOther < 4)
    ...
    radius = ((nOther - 1) / 5 + 2) *
             maximumSizeY; // this should be a jolly good radius
  ...
  for (ir = 0; ir < nRange; ir++) {
    if (ir == 0)
      rangeAttackUnits[ir]->move(
          centerX - rangeAttackUnits[ir]->obj_loc_width(),
          centerY - rangeAttackUnits[ir]->obj_loc_height());
    if (ir == 1)
      rangeAttackUnits[ir]->move(centerX + generalSizeX,
                                 centerY -
                                     rangeAttackUnits[ir]->obj_loc_height());
    if (ir == 2)
      rangeAttackUnits[ir]->move(centerX -
                                     rangeAttackUnits[ir]->obj_loc_width(),
                                 centerY + generalSizeY);
    if (ir == 3)
      rangeAttackUnits[ir]->move(centerX + generalSizeX,
                                 centerY + generalSizeY);
...
void UnitGroup::arc_formation(int direction) {
  ...
  if (!direction || direction == LEADER_AT_TOP ||
      direction == LEADER_AT_BOTTOM || direction == LEADER_AT_RIGHT ||
      direction == LEADER_AT_LEFT) {
  ...
  if (!direction || direction == LEADER_AT_TOP_LEFT ||
      direction == LEADER_AT_TOP_RIGHT || direction == LEADER_AT_BOTTOM_LEFT ||
      direction == LEADER_AT_BOTTOM_RIGHT) {
  ...
  if (!direction || direction == LEADER_AT_BOTTOM) {
    ...
    // circleCenterY =
    // (int)((float)centerY-((float)radius)*0.5f-sinPiOverTenTimesRadius*0.5f)+1;
    ...
    if ((diff = (int)(circleCenterX + radiusC + maximumSizeX + 1 -
                      MAX_WORLD_X_LOC)) > 0) {
      circleCenterX -= diff;
      locX = circleCenterX;
      centerX -= diff;
  ...
  if ((!check && !direction) || direction == LEADER_AT_BOTTOM_RIGHT) {
    //		circleCenterX =
    //(int)((float)centerX-(((float)radius)*0.5f+sinPiOverTenTimesRadius*0.5f)*sinPiOverFour)+3;
    //		circleCenterY =
    //(int)((float)centerY-(((float)radius)*0.5f+sinPiOverTenTimesRadius*0.5f)*sinPiOverFour)+3;
    ...
    if ((diff = circleCenterX + radiusC + maximumSizeX + 1 - MAX_WORLD_X_LOC) >
        0) {
      circleCenterX -= diff;
    ...
    if ((diff = circleCenterY + radiusC + maximumSizeY + 1 - MAX_WORLD_Y_LOC) >
        0) {
      circleCenterY -= diff;
  ...
  if ((!check && !direction) || direction == LEADER_AT_BOTTOM_LEFT) {
    //		circleCenterX =
    //(int)((float)centerX+(((float)radius)*0.5f+sinPiOverTenTimesRadius*0.5f)*sinPiOverFour)-1;
    //		circleCenterY =
    //(int)((float)centerY-(((float)radius)*0.5f+sinPiOverTenTimesRadius*0.5f)*sinPiOverFour)+3;
    if (!nRange) {
      locX = centerX - 2 * maximumSizeX;
    ...
    if ((diff = circleCenterX + (int)sineSub45ArrayC[nClose / 2] +
                maximumSizeX + 1 - MAX_WORLD_X_LOC) > 0) {
      circleCenterX -= diff;
  ...
  if ((!check && !direction) || direction == LEADER_AT_TOP_RIGHT) {
    ...
    //(int)((float)centerX-(((float)radius)*0.5f+sinPiOverTenTimesRadius*0.5f)*sinPiOverFour)+3;
    //		circleCenterY =
    //(int)((float)centerY+(((float)radius)*0.5f+sinPiOverTenTimesRadius*0.5f)*sinPiOverFour)-1;
    ...
    if ((diff = circleCenterY + (int)cosineSub45ArrayC[nClose / 2] +
                maximumSizeY + 1 - MAX_WORLD_Y_LOC) > 0) {
      circleCenterY -= diff;
      locY = circleCenterY - (int)cosineAdd45ArrayC[0];
      centerY -= diff;
    }
  ...
  if ((!direction && !check) || direction == LEADER_AT_LEFT) {
    //		circleCenterX =
    //(int)((float)centerX+((float)radius)*0.5f+sinPiOverTenTimesRadius*0.5f)+1;
  ...
  if (yDirection) {
    ...
    for (ic = 0; ic < nClose; ic++) {
      closeAttackUnits[ic]->move(
          ((int)(circleCenterX + theSign * sineArrayC[arrayIndex])),
          ((int)(circleCenterY +
                 yMultiplicativeFactor * cosineArrayC[arrayIndex])));
    ...
    if (nRange) {
      ...
      for (ir = 0; ir < nRange; ir++) {
        rangeAttackUnits[ir]->move(
            ((int)(circleCenterX + theSign * sineArrayR[arrayIndex])),
            ((int)(circleCenterY +
                   yMultiplicativeFactor * cosineArrayR[arrayIndex])));
    ...
    for (ic = 0; ic < nClose; ic++) {
      closeAttackUnits[ic]->move(
          ((int)(circleCenterX +
                 xMultiplicativeFactor * cosineArrayC[arrayIndex])),
    ...
    if (nRange) {
      ...
      for (ir = 0; ir < nRange; ir++) {
        rangeAttackUnits[ir]->move(
            ((int)(circleCenterX +
                   xMultiplicativeFactor * cosineArrayR[arrayIndex])),
    ...
    for (ic = 0; ic < nClose; ic++) {
      if (!ic)
        closeAttackUnits[ic]->move((int)(circleCenterX + sineAdd45ArrayC[0]),
                                   (int)(circleCenterY + cosineAdd45ArrayC[0]));
      else if (theSign > 0)
        closeAttackUnits[ic]->move(
            (int)(circleCenterX + sineAdd45ArrayC[arrayIndex]),
            (int)(circleCenterY + cosineAdd45ArrayC[arrayIndex]));
      else
        closeAttackUnits[ic]->move(
            (int)(circleCenterX - sineSub45ArrayC[arrayIndex]),
    ...
    if (nRange) {
      ...
      for (ir = 0; ir < nRange; ir++) {
        if (!ir)
          rangeAttackUnits[ir]->move(
              (int)(circleCenterX + sineAdd45ArrayR[0]),
              (int)(circleCenterY + cosineAdd45ArrayR[0]));
        else if (theSign > 0)
          rangeAttackUnits[ir]->move(
              (int)(circleCenterX + sineAdd45ArrayR[arrayIndex]),
              (int)(circleCenterY + cosineAdd45ArrayR[arrayIndex]));
        else
          rangeAttackUnits[ir]->move(
              (int)(circleCenterX - sineSub45ArrayR[arrayIndex]),
    ...
    for (ic = 0; ic < nClose; ic++) {
      if (!ic)
        closeAttackUnits[ic]->move((int)(circleCenterX + sineSub45ArrayC[0]),
                                   (int)(circleCenterY + cosineSub45ArrayC[0]));
      else if (theSign > 0)
        closeAttackUnits[ic]->move(
            (int)(circleCenterX + sineSub45ArrayC[arrayIndex]),
            (int)(circleCenterY + cosineSub45ArrayC[arrayIndex]));
    ...
    if (nRange) {
      ...
      for (ir = 0; ir < nRange; ir++) {
        if (!ir)
          rangeAttackUnits[ir]->move(
              (int)(circleCenterX + sineSub45ArrayR[0]),
              (int)(circleCenterY + cosineSub45ArrayR[0]));
        else if (theSign > 0)
          rangeAttackUnits[ir]->move(
              (int)(circleCenterX + sineSub45ArrayR[arrayIndex]),
              (int)(circleCenterY + cosineSub45ArrayR[arrayIndex]));
...
// This function calculates the position and set the unit to move to the
// locations to form a staggered line formation
void UnitGroup::staggered_formation(int direction) {
  ...
  //---FIRST, we must determine the direction in which the formation should be
  //oriented---//
  ...
  if ((!direction && !check) || direction == LEADER_AT_BOTTOM_LEFT) {
    ...
    if ((diff = locX + ((nSelected + 2) / 4 + 1) * maximumSizeX + 1 -
                MAX_WORLD_X_LOC) > 0)
      locX -= diff;
    if ((diff = locY + ((nSelected - 1) / 4 + 1) * maximumSizeY + 1 -
                MAX_WORLD_Y_LOC) > 0)
      locY -= diff;
  ...
  if ((!direction && !check) || direction == LEADER_AT_TOP_RIGHT) {
    ...
    if ((diff = locX + ((nSelected + 3) / 4 + 1) * maximumSizeX + 1 -
                MAX_WORLD_X_LOC) > 0)
      locX -= diff;
    if ((diff = locY + ((nSelected + 1) / 4 + 1) * maximumSizeY + 1 -
                MAX_WORLD_Y_LOC) > 0)
  ...
  if ((!direction && !check) || direction == LEADER_AT_BOTTOM_RIGHT) {
    ...
    if ((diff = locX + ((nSelected + 2) / 4 + 1) * maximumSizeX + 1 -
                MAX_WORLD_X_LOC) > 0)
      locX -= diff;
    if ((diff = locY + ((nSelected + 3) / 4 + 1) * maximumSizeY + 1 -
                MAX_WORLD_Y_LOC) > 0)
      locY -= diff;
  ...
  if ((!direction && !check) || direction == LEADER_AT_TOP_LEFT) {
    ...
    if ((diff = locX + ((nSelected / 4) + 1) * maximumSizeX + 1 -
                MAX_WORLD_X_LOC) > 0)
      locX -= diff;
    if ((diff = locY + ((nSelected + 1) / 4 + 1) * maximumSizeY + 1 -
                MAX_WORLD_Y_LOC) > 0)
  ...
  for (tSelected = 0; tSelected < nSelected; tSelected++) {
    ...
    // setting the occupancy of the lcoations originally occupied by the units
    // to be true
    for (i = 0, tempLocX = sortedArray[tSelected]->next_x_loc();
         i < sizeArrayX[tSelected]; i++, tempLocX++)
      for (j = 0, tempLocY = sortedArray[tSelected]->next_y_loc();
           j < sizeArrayY[tSelected]; j++, tempLocY++)
        world.get_loc(tempLocX, tempLocY)
            ->set_unit(sortedArray[tSelected]->sprite_recno, mobile_type);
  }

  //---set the units to move to the location assigned in order to form a
  //formation---//
  tSelected = 0;
  ...
  for (tSelected = 0; tSelected < nSelected; tSelected++) {
    if (leaderAtMiddleSpanX) {
      if (tSelected % 2) {
        sortedArray[tSelected]->move(
            tempOldX1,
            (locY + multiplySign * maximumSizeY * (((tSelected + 1) / 2) % 2)));
        tempOldX1 += sizeArrayX[tSelected];
      } else {
        tempOldX2 -= sizeArrayX[tSelected];
        sortedArray[tSelected]->move(
      ...
      if (tSelected % 2) {
        sortedArray[tSelected]->move(
            (locX + multiplySign * maximumSizeX * (((tSelected + 1) / 2) % 2)),
            tempOldY1);
        tempOldY1 += sizeArrayY[tSelected];
      } else {
        tempOldY2 -= sizeArrayY[tSelected];
    ...
    sortedArray[tSelected]->set_in_formation();
    //----set the occupancy of the locations originally occupied by the troop to
    //be true--//
  }
...
void UnitGroup::square_formation(int direction) {
  ...
  int centerX = ((maxLocX + minLocX) % 2) ? (maxLocX + minLocX + 1) / 2
                                          : (maxLocX + minLocX) / 2;
  int centerY = ((maxLocY + minLocY) % 2) ? (maxLocY + minLocY + 1) / 2
                                          : (maxLocY + minLocY) / 2;
  // sort units by attack_range, since we want close attack units at the front
  Unit *tempUnit;
  for (i = 0; i < (nSelected - (int)(generalIsHere)); i++) {
    for (j = 0; j < i; j++) {
      if (sortedArrayTmp[j]->max_attack_range() >
          sortedArrayTmp[i]->max_attack_range()) {
  ...
  if (!direction || direction == LEADER_AT_TOP) {
    ...
    for (i = 0, theSecondTemperoryX = locX; i < sortedArray[0]->obj_loc_width();
         i++, theSecondTemperoryX++)
      for (j = 0, theSecondTemperoryY = locY;
           j < sortedArray[0]->obj_loc_height(); j++, theSecondTemperoryY++)
        if (!world.get_loc(theSecondTemperoryX, theSecondTemperoryY)
                 ->can_move(mobile_type))
          check = false;
    ...
    while (check && tSelected < nSelected) {
      ...
      for (i = 0, theSecondTemperoryX = temperoryX;
           i < sortedArray[tSelected]->obj_loc_width();
           i++, theSecondTemperoryX++)
        for (j = 0, theSecondTemperoryY = temperoryY;
             j < sortedArray[tSelected]->obj_loc_height();
             j++, theSecondTemperoryY++)
          if (!world.get_loc(theSecondTemperoryX, theSecondTemperoryY)
                   ->can_move(mobile_type))
            check = false;
  ...
  if ((!direction && !check) || direction == LEADER_AT_TOP_RIGHT) {
    ...
    if ((diff = locX - (numberOfLines - 1) * maximumSizeX) < 0)
      locX -= diff;
    if ((diff = locY + (numberOfLines + numberOfRows - 1) * maximumSizeY + 1 -
                MAX_WORLD_Y_LOC) > 0)
      locY -= diff;
    if (locY < 0)
      locY = 0;
  ...
  if ((!direction && !check) || direction == LEADER_AT_TOP_LEFT) {
    ...
    if ((diff = locX + (numberOfLines + numberOfRows - 1) * maximumSizeX + 1 -
                MAX_WORLD_X_LOC) > 0)
      locX -= diff;
    if (locX < 0)
      locX -= diff;
  ...
  for (tSelected = 0; tSelected < nSelected; tSelected++) {
    ...
    // to be true
    for (i = 0, tempLocX = sortedArray[tSelected]->next_x_loc();
         i < sortedArray[tSelected]->obj_loc_width(); i++, tempLocX++)
      for (j = 0, tempLocY = sortedArray[tSelected]->next_y_loc();
           j < sortedArray[tSelected]->obj_loc_height(); j++, tempLocY++)
        world.get_loc(tempLocX, tempLocY)
            ->set_unit(sortedArray[tSelected]->sprite_recno, mobile_type);
  }
  ...
  for (tSelected = 0; tSelected < nSelected; tSelected++) {
    if (!slant) {
      if (yMultiplicativeFactor == xMultiplicativeFactor)
        sortedArray[tSelected]->move(
            (locX + (xMultiplicativeFactor) * (tSelected % numberOfLines)),
            (locY + (yMultiplicativeFactor) * (tSelected / numberOfLines)));
      else
        sortedArray[tSelected]->move(
            (locX + (xMultiplicativeFactor) * (tSelected / numberOfLines)),
            (locY + (yMultiplicativeFactor) * (tSelected % numberOfLines)));
      ...
      sortedArray[tSelected]->move(
          firstX + xMultiplicativeFactor * (tSelected % numberOfLines) *
                       maximumSizeX,
          firstY + yMultiplicativeFactor * (tSelected % numberOfLines) *
                       maximumSizeY);
    }
...
void UnitGroup::square_preformation(int direction) {
  ...
  if (!direction || direction == LEADER_AT_TOP) {
    ...
    if ((diff = locX + numberOfLines * formationSpaceX + 1 - MAX_WORLD_X_LOC) >
        0)
      locX -= diff;
    if (locX < 0)
      locX = 0;
    if ((diff = locY + numberOfRows * formationSpaceY + 1 - MAX_WORLD_Y_LOC) >
        0)
      locY -= diff;
    if (locY < 0)
      locY = 0;
  ...
  if ((!direction && !check) || direction == LEADER_AT_RIGHT) {
    ...
    if ((diff = locX - (numberOfRows - 1) * formationSpaceX) < 0)
      locX -= diff;
    if ((diff = locY + numberOfLines * formationSpaceY + 1 - MAX_WORLD_Y_LOC) >
        0)
      locY -= diff;
    if (locY < 0)
      locY -= diff;
  ...
  if ((!direction && !check) || direction == LEADER_AT_LEFT) {
    ...
    if ((diff = locX + numberOfRows * formationSpaceX + 1 - MAX_WORLD_X_LOC) >
        0)
      locX -= diff;
    if (locX < 0)
      locX = 0;
...
void UnitGroup::sandwich_formation(int direction) {
  ...
  if ((!direction && !check) || direction == LEADER_AT_TOP_RIGHT) {
    ...
    widthRight = (widthRightForClose > widthRightForRange) ? widthRightForClose
                                                           : widthRightForRange;
    widthLeft = (widthLeftForClose > widthLeftForRange) ? widthLeftForClose
                                                        : widthLeftForRange;
    heightRight = (heightRightForClose > heightRightForRange)
                      ? heightRightForClose
                      : (heightRightForRange);
    heightLeft = (heightLeftForClose > heightLeftForRange)
                     ? heightLeftForClose
                     : (heightLeftForRange);
