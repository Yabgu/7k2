...
void UnitGod::disp_hit_point() {
  ...
  if (max_hit_points()) {
    ...
    if (ratio < 11)
      vga.active_buf->put_bitmap_trans(INFO_X1 + 80 - 15, INFO_Y1 + 49 + 128,
    ...
    vga.active_buf->read_bitmapW(INFO_X1 + 80 - 15, INFO_Y1 + 49 + 128,
                                 INFO_X1 + 94 + size - 15, INFO_Y1 + 80 + 128,
                                 hitPointBitmap);
    vga.active_buf->put_bitmap_trans(INFO_X1 + 80 - 15, INFO_Y1 + 49 + 128,
                                     image_spict.read("MTR_00"));
    vga.active_buf->put_bitmapW(INFO_X1 + 80 - 15, INFO_Y1 + 49 + 128,
                                hitPointBitmap);
    mem_del(hitPointBitmap);

    str = (int)hitPoints;
    font_snds.center_put(INFO_X1 + 43, INFO_Y1 + 190, INFO_X1 + 65,
                         INFO_Y1 + 202, str);
    str = (int)max_hit_points();
    font_snds.center_put(INFO_X1 + 172, INFO_Y1 + 190, INFO_X1 + 194,
                         INFO_Y1 + 202, str);
  }
}
