...
void Spy::process_ai() {
  if (spy_recno % 30 ==
      info.game_date % 30) // think about changing actions once 30 days
    think_reward();
...
void Spy::think_town_spy() {
  ...
  if (townPtr->nation_recno == 0) {
    ...
    //--- if the resistance has already drop low enough, the spy no longer needs
    //to be in the town ---//

    if (townPtr->resistance(true_nation_recno) <
        MIN_INDEPENDENT_DEFEND_LOYALTY) {
      mobilize_town_spy();
    }
  } else {
    ...
    if (townPtr->loyalty <
        50 - trueNation->pref_loyalty_concern /
                 10) // pref_loyalty_concern actually does apply to here, we
                     // just use a preference var so that the decision making
                     // process will vary between nations
    {
      set_action_mode(SPY_SOW_DISSENT);
    } else {
...
void Spy::think_firm_spy() {
  ...
  else if (misc.random(3) ==
           0) // 1/10 chance to set it to idle to prevent from being caught
  {
    set_action_mode(SPY_IDLE);
  }
  //	else if( misc.random(2)==0 &&
  //				can_sabotage() && firmPtr->is_operating() &&
  //firmPtr->get_productivity() >= 20.0f )
  //	{
...
int Spy::think_mobile_spy_new_action() {
  ...
  if (!is_civilian) {
    int firmRecno =
        trueNation->think_assign_spy_target_camp(race_id, spyRegionId);

    if (firmRecno) {
      Firm *firmPtr = firm_array[firmRecno];

      return add_assign_spy_action(firmPtr->loc_x1, firmPtr->loc_y1,
  ...
  if (is_civilian) {
    int townRecno =
        trueNation->think_assign_spy_target_town(race_id, spyRegionId);

    if (townRecno) {
      Town *townPtr = town_array[townRecno];

      return add_assign_spy_action(townPtr->loc_x1, townPtr->loc_y1,
  ...
  if (trueNation->total_spy_count > trueNation->total_population *
                                        (10 + trueNation->pref_spy / 5) /
                                        100) // 10% to 30%
  {
    dropIdentity = 1;
  }
  ...
  //--- the expense of spies should not be too large ---//
           ...
           trueNation->expense_365days() * (50 + trueNation->pref_counter_spy) /
               400) {
    dropIdentity = 1;
  }
...
//
int Spy::add_assign_spy_action(int destXLoc, int destYLoc,
                               int cloakedNationRecno) {
  err_when(spy_place != SPY_MOBILE);
  err_when(unit_array.is_truly_deleted(spy_place_para));

  return nation_array[true_nation_recno]->add_action(
      destXLoc, destYLoc, -1, -1, ACTION_AI_ASSIGN_SPY, cloakedNationRecno, 1,
      spy_place_para);
}
...
int Spy::ai_spy_being_attacked(BaseObj *attackerObj) {
  if (!attackerObj) // if the attacker is dead already (it's the bullet that
                    // hits the spy)
    return 0;
  ...
  if (attackerObj->true_nation_recno() == true_nation_recno) {
    if (spy_skill > 50 - trueNation->pref_spy / 10 ||
        spyUnit->hit_points <
            spyUnit->max_hit_points() *
                (100 - trueNation->pref_military_courage / 2) / 100) {
    ...
    if (trueNation->get_relation_status(attackerObj->nation_recno) !=
            RELATION_HOSTILE &&
        trueNation->get_relation_status(cloaked_nation_recno) ==
            RELATION_HOSTILE) {
      if (spy_skill > 50 - trueNation->pref_spy / 10 ||
          spyUnit->hit_points <
              spyUnit->max_hit_points() *
...
int Spy::think_bribe() {
  ...
  //---- see if the overseer can be bribe (kings and your own spies can't be
  //bribed) ----//
  ...
  int succeedChange = firmPtr->spy_bribe_succeed_chance(
      0, spy_recno, 0); // first 0 - $0 bribe amount, 3rd 0 - bribe the overseer

  //-- then based on it, figure out how much we have to offer to bribe
  //successfully --//
  ...
  {
    err_when(firm_array[newSpy->spy_place_para]->nation_recno !=
             true_nation_recno);

    newSpy->drop_spy_identity(); // drop the spy identity of the newly bribed
                                 // spy if the capture is successful, this will
                                 // save the spying costs
  }
...
int Spy::think_assassinate() {
  ...
  if (!get_assassinate_rating(
          firmPtr->overseer_recno, attackRating, defenseRating,
          otherDefenderCount)) // return 0 if assassination is not possible
    return 0;
  ...
  if (attackRating - defenseRating +
          misc.random(20 + trueNation->pref_spy / 4) >
      100) {
...
int Spy::think_create_incident() {
  ...
  //--- determine the nation to be involved with the target nation in the
  //incident ---//
  ...
  for (int i = nation_array.size(); i > 0; i--) {
    ...
    curRating = get_incident_rating(
        targetNation->nation_recno,
        i); // get the probability of successfully creating an incident
