...
void Spy::think_town_spy()
{
    ...
    else
    {
        ...
        if (townPtr->loyalty <
            50 - trueNation->pref_loyalty_concern / 10) // pref_loyalty_concern actually does apply to here, we
...
int Spy::think_mobile_spy()
{
    ...
    if (unitPtr->is_all_stop() && (!notify_cloaked_nation_flag || cloaked_nation_recno == 0))
    {
...
int Spy::think_mobile_spy_new_action()
{
    ...
    if (trueNation->total_spy_count >
        trueNation->total_population * (10 + trueNation->pref_spy / 5) / 100) // 10% to 30%
    {
        dropIdentity = 1;
    }
...
int Spy::add_assign_spy_action(int destXLoc, int destYLoc, int cloakedNationRecno)
{
    ...
    return nation_array[true_nation_recno]->add_action(destXLoc, destYLoc, -1, -1, ACTION_AI_ASSIGN_SPY,
                                                       cloakedNationRecno, 1, spy_place_para);
}
...
int Spy::think_bribe()
{
    ...
    if (spy_skill < MIN(50, overseerUnit->skill_level()) || !ownNation->ai_should_spend(30))
    {
        return 0;
    }
    ...
    int succeedChange =
        firmPtr->spy_bribe_succeed_chance(0, spy_recno, 0); // first 0 - $0 bribe amount, 3rd 0 - bribe the overseer
...
int Spy::think_assassinate()
{
    ...
    if (!get_assassinate_rating(firmPtr->overseer_recno, attackRating, defenseRating,
...
int Spy::think_steal_tech()
{
    ...
    if (!firmPtr || firmPtr->nation_recno == true_nation_recno || firmPtr->nation_recno == 0)
    {
        return 0;
    }
...
int Spy::think_create_incident()
{
    ...
    for (int i = nation_array.size(); i > 0; i--)
    {
        ...
        if (nationPtr->nation_recno == targetNation->nation_recno || nationPtr->nation_recno == true_nation_recno)
        {
            continue;
        }
        ...
        curRating = get_incident_rating(targetNation->nation_recno,
