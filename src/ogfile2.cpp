...
// HeroRes				hero_res;
//
// UnitArray			unit_array(100); 			// with
// derived classes BulletArray			bullet_array(100); SiteArray
...
template <typename Visitor> static void visit_talk_msg(Visitor *v, TalkMsg *tm)
{
...
template <typename Visitor> static void visit_talk_choice(Visitor *v, TalkChoice *tc)
{
...
template <typename Visitor> static void visit_talk_res(Visitor *v, TalkRes *tr)
{
...
{
    if (!write_with_record_size(filePtr, this, &visit_talk_res<FileWriterVisitor>, TALK_RES_RECORD_SIZE))
        return 0;
...
int TalkRes::read_file(File *filePtr)
{
    ...
    if (!read_with_record_size(filePtr, this, &visit_talk_res<FileReaderVisitor>, TALK_RES_RECORD_SIZE))
        return 0;
...
int Game::write_file(File *filePtr)
{
    ...
    if (is_campaign_mode())
    {
        ...
        int writeSize = campaign_class_size(cur_campaign->campaign_id) - sizeof(void *);

        if (!filePtr->file_write((char *)cur_campaign + sizeof(void *),
...
int Game::read_file(File *filePtr)
{
    ...
    if (is_campaign_mode())
    {
        ...
        int readSize = campaign_class_size(cur_campaign->campaign_id) - sizeof(void *);

        if (!filePtr->file_read((char *)cur_campaign + sizeof(void *),
...
int CampaignEastWest::write_derived_file(File *filePtr)
{
    ...
    {
        for (i = 0; plot_create_game_FP_array[i] != NULL && plot_create_game_FP_array[i] != plot_create_game_FP; ++i)
            ;
    ...
    {
        for (i = 0; plot_next_day_FP_array[i] != NULL && plot_next_day_FP_array[i] != plot_next_day_FP; ++i)
            ;
...
int World::read_file(File *filePtr)
{
    ...
    //) ) 		return 0; 	corner_matrix = (Corner*) mem_resize(
    // corner_matrix, 		(max_x_loc+1)*(max_y_loc+1)*sizeof(Corner) ); 	if(
    //! filePtr->file_read(corner_matrix,
    //(max_x_loc+1)*(max_y_loc+1)*sizeof(Corner) ) ) 		return 0;
    ...
    if (writeDecompressedMatrix)
    {
        if (writeCompressedMatrix)
        {
            ...
            Blob tempLoc(
                MAX(max_x_loc * max_y_loc * sizeof(Location), (max_x_loc + 1) * (max_y_loc + 1) * sizeof(Corner)));
            // save for comparision
...
{
    return (filePtr->file_write(this, sizeof(Plasma)) &&
