...
// HeroRes				hero_res;
//
// UnitArray			unit_array(100); 			// with derived
// classes BulletArray			bullet_array(100); SiteArray
// site_array; TownArray			town_array; NationArray
// nation_array; FirmArray			firm_array;
// // with derived classes TornadoArray		tornado_array(10); RebelArray
// rebel_array; SpyArray				spy_array;
...
// Power					power
// Weather				weather,
// weather_forecast[MAX_WEATHER_FORECAST]; MagicWeather		magic_weather
// News					news
// World					world
// Tutor					tutor
//### begin alex 23/9 ###//
...
int GameFile::write_file(File *filePtr) {
  ...
  if (game_file_array.demo_format)
    filePtr->file_put_short(
        -GAME_VERSION); // negative no. means shareware version
  else
...
int GameFile::read_file(File *filePtr) {
  ...
  if (filePtr->file_get_short() / 100 != (game_file_array.demo_format
                                              ? -(GAME_VERSION / 100)
                                              : GAME_VERSION / 100))
    return -1;
  ...
  if (!firm_die_array.read_file(filePtr))
    ...
    for (int i = 1; i <= firm_die_array.size(); i++) {
      if (!firm_die_array.is_deleted(i)) {
        ...
        FirmBuild *firmBuild =
            firm_die_res.get_build(firmDiePtr->firm_build_id);
...
int RaceRes::write_file(File *filePtr) {
  ...
  return filePtr->file_write(name_used_array,
                             sizeof(name_used_array[0]) * name_count);
}
...
int RaceRes::read_file(File *filePtr) {
  ...
  return filePtr->file_read(name_used_array,
                            sizeof(name_used_array[0]) * name_count);
}
...
int HeroRes::read_file(File *filePtr) {
  ...
  hero_info_array =
      (HeroInfo *)mem_resize(hero_info_array, hero_count * sizeof(HeroInfo));
...
int UnitRes::write_file(File *filePtr) {
  ...
  for (int i = 1; i <= unit_res.unit_info_count; i++, unitInfo++) {
    if (!filePtr->file_write(unitInfo->nation_tech_level_array,
                             sizeof(unitInfo->nation_tech_level_array)))
      return 0;

    if (!filePtr->file_write(unitInfo->nation_unit_count_array,
                             sizeof(unitInfo->nation_unit_count_array)))
      return 0;

    if (!filePtr->file_write(unitInfo->nation_general_count_array,
...
int UnitRes::read_file(File *filePtr) {
  ...
  for (int i = 1; i <= unit_res.unit_info_count; i++, unitInfo++) {
    if (!filePtr->file_read(unitInfo->nation_tech_level_array,
                            sizeof(unitInfo->nation_tech_level_array)))
      return 0;

    if (!filePtr->file_read(unitInfo->nation_unit_count_array,
                            sizeof(unitInfo->nation_unit_count_array)))
      return 0;

    if (!filePtr->file_read(unitInfo->nation_general_count_array,
                            sizeof(unitInfo->nation_general_count_array)))
      return 0;
  }
...
int FirmRes::read_file(File *filePtr) {
  ...
  //----- save the firm names, so that it won't be overwritten by the saved game
  //file ----//
  ...
  for (int i = 0; i < firm_count; i++) {
    memcpy(firm_info_array[i].name, oldFirmInfoArray[i].name,
           FirmInfo::NAME_LEN + 1);
    memcpy(firm_info_array[i].short_name, oldFirmInfoArray[i].short_name,
           FirmInfo::SHORT_NAME_LEN + 1);
    memcpy(firm_info_array[i].overseer_title,
           oldFirmInfoArray[i].overseer_title, FirmInfo::TITLE_LEN + 1);
    memcpy(firm_info_array[i].worker_title, oldFirmInfoArray[i].worker_title,
           FirmInfo::TITLE_LEN + 1);
...
int TownRes::write_file(File *filePtr) {
  int rc = filePtr->file_write(
      town_name_used_array, sizeof(town_name_used_array[0]) * town_name_count);
  return (rc & filePtr->file_write(town_name_array,
                                   sizeof(TownName) * town_name_count));
}
...
int TownRes::read_file(File *filePtr) {
  int rc = filePtr->file_read(
      town_name_used_array, sizeof(town_name_used_array[0]) * town_name_count);
  return (rc & filePtr->file_read(town_name_array,
                                  sizeof(TownName) * town_name_count));
}
...
int TechRes::write_file(File *filePtr) {
  if (!filePtr->file_write(tech_class_array,
                           tech_class_count * sizeof(TechClass)))
    return 0;
  ...
  // ####### begin Gilbert 23/12 #########//
  if (!filePtr->file_write(total_tech_level_array,
                           sizeof(total_tech_level_array)))
    return 0;
  // ####### end Gilbert 23/12 #########//
...
int TechRes::read_file(File *filePtr) {
  ...
  TechClass *backupTechClass =
      (TechClass *)mem_add(tech_class_count * sizeof(TechClass));
  memcpy(backupTechClass, tech_class_array,
         tech_class_count * sizeof(TechClass));
  if (!filePtr->file_read(tech_class_array,
                          tech_class_count * sizeof(TechClass))) {
  ...
  // ####### begin Gilbert 23/12 #########//
  if (!filePtr->file_read(total_tech_level_array,
                          sizeof(total_tech_level_array)))
    return 0;
  // ####### end Gilbert 23/12 #########//
...
int Game::write_file(File *filePtr) {
  ...
  if (is_campaign_mode()) {
    ...
    int writeSize =
        campaign_class_size(cur_campaign->campaign_id) - sizeof(void *);

    if (!filePtr->file_write(
            (char *)cur_campaign + sizeof(void *),
            writeSize)) // do not write the virtual function tabel pointer
      return 0;
...
int Game::read_file(File *filePtr) {
  ...
  if (is_campaign_mode()) {
    ...
    if (!filePtr->file_read(
            (char *)cur_campaign + sizeof(void *),
            readSize)) // do not write the virtual function tabel pointer
      return 0;
    ...
    for (int i = 0; i < MAIN_TERRAIN_WIDTH; i++) {
      for (int j = 0; j < MAIN_TERRAIN_HEIGHT; j++) {
        ...
        if (height > prevHeight)
          cur_campaign->shadow_array[j * MAIN_TERRAIN_WIDTH + i] =
              32; // 16 * abs(height-prevHeight) / 8;
        else
          cur_campaign->shadow_array[j * MAIN_TERRAIN_WIDTH + i] =
              32 - 16 * abs(height - prevHeight) / 8;
        prevHeight = height;
...
int CampaignEastWest::write_derived_file(File *filePtr) {
  ...
  if (!plot_create_game_FP) {
    ...
    for (i = 0; plot_create_game_FP_array[i] != NULL &&
                plot_create_game_FP_array[i] != plot_create_game_FP;
         ++i)
      ;
  ...
  if (!plot_next_day_FP) {
    ...
    for (i = 0; plot_next_day_FP_array[i] != NULL &&
                plot_next_day_FP_array[i] != plot_next_day_FP;
         ++i)
      ;
...
int World::write_file(File *filePtr) {
  ...
  //	if( !filePtr->file_write(loc_matrix,
  //max_x_loc*max_y_loc*sizeof(Location) ) ) 		return 0; 	if(
  //!filePtr->file_write(corner_matrix,
  //(max_x_loc+1)*(max_y_loc+1)*sizeof(Corner) ) ) 		return 0;
  ...
  if (writeCompressedMatrix) {
    ...
    for (i = max_x_loc * max_y_loc; i > 0;
         --i, srcBytePtr += sizeof(Location), ++destBytePtr) {
      *destBytePtr = *srcBytePtr;
    }
    writeLen =
        rle.compress((unsigned char *)buffer.p(), max_x_loc * max_y_loc, 0xff);
    if (writeLen > cBuffer.size)
      cBuffer.resize(writeLen + 16);
    writeLen = rle.compress((unsigned char *)buffer.p(), max_x_loc * max_y_loc,
                            (unsigned char *)cBuffer.p(), 0xff);
    ...
    writeLen =
        rle.compress((unsigned char *)buffer.p(), max_x_loc * max_y_loc, 0x80);
    if (writeLen > cBuffer.size)
      cBuffer.resize(writeLen + 16);
    writeLen = rle.compress((unsigned char *)buffer.p(), max_x_loc * max_y_loc,
                            (unsigned char *)cBuffer.p(), 0x80);
    filePtr->file_put_long(writeLen);
    if (!filePtr->file_write(cBuffer.p(), writeLen))
      return 0;
  ...
  if (writeDecompressedMatrix) {
    ...
    if (!filePtr->file_write(loc_matrix,
      ...
      return 0;
    if (!filePtr->file_write(corner_matrix, (max_x_loc + 1) * (max_y_loc + 1) *
                                                sizeof(Corner)))
      return 0;
  }
...
int World::read_file(File *filePtr) {
  ...
  //) ) 		return 0; 	corner_matrix = (Corner*) mem_resize( corner_matrix,
  //		(max_x_loc+1)*(max_y_loc+1)*sizeof(Corner) );
  //	if( !filePtr->file_read(corner_matrix,
  //(max_x_loc+1)*(max_y_loc+1)*sizeof(Corner) ) ) 		return 0;
  ...
  if (writeCompressedMatrix) {
    ...
    if (!filePtr->file_read(cBuffer.p(), readLen))
      return 0;
    if (rle.expand((unsigned char *)cBuffer.p(), (unsigned char *)buffer.p(),
                   0xff) != max_x_loc * max_y_loc)
      return 0;
    srcBytePtr = (unsigned char *)buffer.p();
    destBytePtr = (unsigned char *)&loc_matrix->loc_flag;
    for (i = max_x_loc * max_y_loc; i > 0;
         --i, destBytePtr += sizeof(Location), ++srcBytePtr) {
      *destBytePtr = *srcBytePtr;
    }
  ...
  if (writeDecompressedMatrix) {
    if (writeCompressedMatrix) {
      ...
      Blob tempLoc(MAX(max_x_loc * max_y_loc * sizeof(Location),
                       (max_x_loc + 1) * (max_y_loc + 1) * sizeof(Corner)));
      // save for comparision
      if (filePtr->file_read(tempLoc.p(),
                             max_x_loc * max_y_loc * sizeof(Location))) {
        if (memcmp(tempLoc.p(), loc_matrix,
                   max_x_loc * max_y_loc * sizeof(Location)))
          return 0;
      }
      if (filePtr->file_read(tempLoc.p(), (max_x_loc + 1) * (max_y_loc + 1) *
                                              sizeof(Corner))) {
        if (memcmp(tempLoc.p(), corner_matrix,
                   (max_x_loc + 1) * (max_y_loc + 1) * sizeof(Corner)))
          return 0;
      }
    } else {
      // load decompressed matrix
      if (!filePtr->file_read(loc_matrix,
                              max_x_loc * max_y_loc * sizeof(Location)))
        return 0;
      if (!filePtr->file_read(corner_matrix, (max_x_loc + 1) * (max_y_loc + 1) *
                                                 sizeof(Corner)))
        return 0;
    }
  }
...
int Plasma::read_file(File *filePtr) {
  ...
  return (
      filePtr->file_read(matrix, (max_x + 1) * (max_y + 1) * sizeof(short)));
}
// -------- end of function Plasma::write_file ---------//
