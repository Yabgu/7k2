...
void ScenarioEditor::disp_victory_main(int refreshFlag) {
  ...
  case SE_VICTORY_MAIN: // victory main menu
    if (refreshFlag == INFO_REPAINT) {
      ...
      button_win_or_lose[0].create_text(
          INFO_X1 + 15, INFO_Y1 + 5, INFO_X1 + 110, INFO_Y1 + 25,
          text_editor.str_victory(), 0); // "Victory"
      button_win_or_lose[1].create_text(INFO_X1 + 115, INFO_Y1 + 5,
                                        INFO_X1 + 210, INFO_Y1 + 25,
    ...
    {
      if (refreshFlag == INFO_REPAINT) {
        browse_win_condition.init(INFO_X1, INFO_Y1 + 32, INFO_X2, INFO_Y1 + 240,
                                  -1, 36, game.win_condition_count,
                                  disp_win_condition);
        browse_win_condition.open(win_condition_browse_recno);
        button_add_win_condition.create_text(INFO_X1 + 20, INFO_Y1 + 252,
                                             INFO_X1 + 100, INFO_Y1 + 272,
                                             text_editor.str_add()); //"Add");
        button_del_win_condition.create_text(
            INFO_X1 + 120, INFO_Y1 + 252, INFO_X1 + 200, INFO_Y1 + 272,
            text_editor.str_delete()); // "Delete");
        button_set_win_require_all_condition.create_text(
            INFO_X1 + 12, INFO_Y1 + 275, INFO_X2 - 12, INFO_Y1 + 295,
            text_editor.str_require_all(), 0); //"Require All");
      }
      browse_win_condition.paint();
      browse_win_condition.refresh();

      button_add_win_condition.enable_flag =
          game.win_condition_count < MAX_WIN_CONDITION;
      ...
      if ((button_set_win_require_all_condition.visible_flag =
               game.win_condition_count > 1)) {
        button_set_win_require_all_condition.enable_flag = 1;
        button_set_win_require_all_condition.is_pushed =
            game.win_require_all_condition != 0;
        if (!button_set_win_require_all_condition.is_pushed)
          button_set_win_require_all_condition.set_body(
              text_editor.str_require_any()); //"Any Condition is true" );
        else
          button_set_win_require_all_condition.set_body(
              text_editor.str_require_all()); //"All Conditions are true" );
        button_set_win_require_all_condition.paint();
      }
    } else // lose
    {
      if (refreshFlag == INFO_REPAINT) {
        browse_lose_condition.init(
            INFO_X1, INFO_Y1 + 32, INFO_X2, INFO_Y1 + 240, -1, 36,
            game.lose_condition_count, disp_lose_condition);
        browse_lose_condition.open(lose_condition_browse_recno);
        button_add_lose_condition.create_text(INFO_X1 + 20, INFO_Y1 + 252,
                                              INFO_X1 + 100, INFO_Y1 + 272,
                                              text_editor.str_add()); //"Add");
        button_del_lose_condition.create_text(
            INFO_X1 + 120, INFO_Y1 + 252, INFO_X1 + 200, INFO_Y1 + 272,
            text_editor.str_delete()); // "Delete");
      }
      ...
      button_add_lose_condition.enable_flag =
          game.lose_condition_count < MAX_LOSE_CONDITION;
    ...
    if (game.win_deadline) {
      int x2 = font_whbl.put(INFO_X1 + 12, INFO_Y1 + 302,
                             text_editor.str_deadline());
      x2 = font_whbl.put(x2 + 6, INFO_Y1 + 302,
                         date.date_str(game.win_deadline, 1)); // 1-short date

      x2 += 20;
      if (refreshFlag == INFO_REPAINT) {
        button_deadline_up.create_text(x2, INFO_Y1 + 302, x2 + 10,
                                       INFO_Y1 + 311, "+");
        button_deadline_down.create_text(x2, INFO_Y1 + 312, x2 + 10,
                                         INFO_Y1 + 321, "-");
      }
  ...
  case SE_VICTORY_WIN_COND: // select winning condition
    if (refreshFlag == INFO_REPAINT) {
      ...
      // ####### end Gilbert 22/2 ########//
      button_next.create_text(INFO_X1 + 20, INFO_Y1 + 230, INFO_X1 + 100,
                              INFO_Y1 + 250,
                              text_editor.str_next()); // "Next" );
      button_cancel.create_text(INFO_X1 + 120, INFO_Y1 + 230, INFO_X1 + 200,
                                INFO_Y1 + 250,
                                text_editor.str_cancel()); // "Cancel" );
    }
    browse_win_id.paint();
    // ####### begin Gilbert 22/2 ########//
    ...
    if (browse_win_id.recno() > 0 &&
        browse_win_id.recno() <= browse_win_id.total_rec()) {
      font_whbl.put_paragraph(
          INFO_X1 + 8, INFO_Y1 + 260, INFO_X2 - 8, INFO_Y1 + 315,
          EndConditionInfo::end_condition_info
              [filtered_win_condition[browse_win_id.recno() - 1]]
                  .name());
    }
    break;
  ...
  case SE_VICTORY_LOSE_COND: // select losing condition
    ...
    button_cancel.paint();
    // display the full text of winning condition
    if (browse_lose_id.recno() > 0 &&
        browse_lose_id.recno() <= browse_lose_id.total_rec()) {
      font_whbl.put_paragraph(
          INFO_X1 + 8, INFO_Y1 + 260, INFO_X2 - 8, INFO_Y1 + 315,
          EndConditionInfo::end_condition_info
              [filtered_lose_condition[browse_lose_id.recno() - 1]]
                  .name());
    }
    break;
  ...
  case SE_VICTORY_SCALAR: // input a scalar quantity
    if (refreshFlag == INFO_REPAINT) {
      button_scalar_up10.create_text(INFO_X1 + 130, INFO_Y1 + 10,
                                     INFO_X1 + 150 - 1, INFO_Y1 + 25, "++");
      button_scalar_up.create_text(INFO_X1 + 150, INFO_Y1 + 10,
                                   INFO_X1 + 170 - 1, INFO_Y1 + 25, "+");
      button_scalar_down.create_text(INFO_X1 + 170, INFO_Y1 + 10,
                                     INFO_X1 + 190 - 1, INFO_Y1 + 25, "-");
      button_scalar_down10.create_text(INFO_X1 + 190, INFO_Y1 + 10,
                                       INFO_X1 + 210 - 1, INFO_Y1 + 25, "--");
      button_next.create_text(INFO_X1 + 20, INFO_Y1 + 30, INFO_X1 + 100,
                              INFO_Y1 + 50, text_editor.str_next()); //"Next" );
      button_cancel.create_text(INFO_X1 + 120, INFO_Y1 + 30, INFO_X1 + 200,
                                INFO_Y1 + 50,
                                text_editor.str_cancel()); //"Cancel" );
    }
    font_whbl.right_put(INFO_X1 + 120, INFO_Y1 + 12,
                        misc.format(selected_scalar));
  ...
  case SE_VICTORY_YEAR_LIMIT: // input years limit
    if (refreshFlag == INFO_REPAINT) {
      button_year_up.create_text(INFO_X1 + 150, INFO_Y1 + 10, INFO_X1 + 170,
                                 INFO_Y1 + 25, "+");
      button_year_down.create_text(INFO_X1 + 180, INFO_Y1 + 10, INFO_X1 + 200,
                                   INFO_Y1 + 25, "-");
      button_next.create_text(INFO_X1 + 20, INFO_Y1 + 30, INFO_X1 + 100,
                              INFO_Y1 + 50, text_editor.str_next()); //"Next" );
      button_cancel.create_text(INFO_X1 + 120, INFO_Y1 + 30, INFO_X1 + 200,
    ...
    if (selected_year_limit) {
      //			int x2 =	font_whbl.put( INFO_X1+80,
      //INFO_Y1+12, selected_year_limit ); 			font_whbl.put( x2, INFO_Y1+12,
      //selected_year_limit > 1 ? "years" : "year" );
      font_whbl.put(INFO_X1 + 80, INFO_Y1 + 12,
                    text_editor.str_time_limit(selected_year_limit));
    } else {
      font_whbl.right_put(INFO_X1 + 145, INFO_Y1 + 12,
                          text_editor.str_no_time_limit()); //"No time limit" );
    }
  ...
  case SE_VICTORY_MESSAGE: {
    font_whbl.put_paragraph(INFO_X1 + 15, INFO_Y1 + 5, INFO_X2 - 5,
                            INFO_Y1 + 100, message_str);
    if (refreshFlag == INFO_REPAINT) {
      if (message_sub_mode == 0) {
        button_next.create_text(INFO_X1 + 20, INFO_Y1 + 110,
                                text_editor.str_back()); //"Back");
      } else {
        button_next.create_text(INFO_X1 + 20, INFO_Y1 + 110, INFO_X1 + 100,
                                INFO_Y1 + 130,
                                text_editor.str_next()); //"Next" );
        button_cancel.create_text(INFO_X1 + 120, INFO_Y1 + 110, INFO_X1 + 200,
                                  INFO_Y1 + 130,
                                  text_editor.str_cancel()); // "Cancel" );
      }
    }
...
void ScenarioEditor::detect_victory_main() {
  ...
  case SE_VICTORY_MAIN: // victory mode main menu
    ...
    if (win_or_lose == 0) // win
    {
      ...
      if (button_add_win_condition.detect()) {
        edit_sub_menu_mode[edit_mode] =
            SE_VICTORY_WIN_COND; // switch to selecting winning condition
        return;
      }

      if (button_del_win_condition
              .detect()) // remove selected winning condition
      {
        if (browse_win_condition.recno() > 1 &&
            browse_win_condition.recno() <= game.win_condition_count) {
          ...
          browse_win_condition.open(win_condition_browse_recno,
                                    game.win_condition_count);
        }
      }

      if (button_set_win_require_all_condition.detect()) {
        game.win_require_all_condition =
            button_set_win_require_all_condition.is_pushed;
      }
    } else // lose
    {
      ...
      if (button_add_lose_condition
              .detect()) // switch to selecting losing condition
      {
        edit_sub_menu_mode[edit_mode] = SE_VICTORY_LOSE_COND;
        return;
      }

      if (button_del_lose_condition
              .detect()) // remove selected losing condition
      {
        if (browse_lose_condition.recno() > 1 &&
            browse_lose_condition.recno() <= game.lose_condition_count) {
  ...
  case SE_VICTORY_WIN_COND: // select a winning condition
    ...
    // ###### end Gilbert 22/2 #########//
    if (button_next.detect() && !browse_win_id.none_record &&
        browse_win_id.recno()) {
  ...
  case SE_VICTORY_LOSE_COND: // select a losing condition
    ...
    if (button_next.detect() && !browse_lose_id.none_record &&
        browse_lose_id.recno()) {
      selected_lose_condition =
          filtered_lose_condition[browse_lose_id.recno() - 1];
...
//
int ScenarioEditor::detect_victory_view() { return 0; }
...
static void disp_win_condition(int recNo, int x, int y, int refreshFlag) {
  ...
  font_snds.put_paragraph(
      x + 6, y + 2, x + 195, y + 34,
      game.win_condition[recNo - 1].cond_str); // 0=win condition
  // ###### end Gilbert 22/2 #######//
}
...
static void disp_lose_condition(int recNo, int x, int y, int refreshFlag) {
  ...
  font_snds.put_paragraph(
      x + 6, y + 2, x + 195, y + 34,
      game.lose_condition[recNo - 1].cond_str); // 1=lose condition
  // ###### end Gilbert 22/2 #######//
}
...
static void disp_win_cond_id(int recNo, int x, int y, int refreshFlag) {
  font_snds.put(
      x + 6, y + 2,
      EndConditionInfo::end_condition_info[filtered_win_condition[recNo - 1]]
          .name(),
      0, x + 194);
}
...
static void disp_lose_cond_id(int recNo, int x, int y, int refreshFlag) {
  font_snds.put(
      x + 6, y + 2,
      EndConditionInfo::end_condition_info[filtered_lose_condition[recNo - 1]]
          .name(),
      0, x + 194);
}
...
//
static int proceed_victory_menu() {
  err_when(selected_win_condition &&
           selected_lose_condition); // cannot be both non-zero
  EndConditionInfo &condInfo =
      EndConditionInfo::end_condition_info[selected_win_condition +
                                           selected_lose_condition];
  ...
  // check next menu
    ...
    case EP_0_100:
      ...
      scenario_editor.edit_sub_menu_mode[scenario_editor.edit_mode] =
          SE_VICTORY_SCALAR;
      break;
    ...
    case EP_NATION_RECNO: // get select object
      message_str =
          text_editor.str_select_nation(); //"Select target nation by selecting
                                           //any object of that nation";
      message_sub_mode = 1;
      scenario_editor.edit_sub_menu_mode[scenario_editor.edit_mode] =
          SE_VICTORY_MESSAGE;
      break;
    ...
    case EP_FRIEND_UNIT_RECNO:
      message_str =
          text_editor.str_select_friendly_unit(); // "Select a friendly unit";
      message_sub_mode = 1;
      scenario_editor.edit_sub_menu_mode[scenario_editor.edit_mode] =
          SE_VICTORY_MESSAGE;
      break;
    ...
    case EP_FRIEND_TOWN_RECNO:
      message_str =
          text_editor.str_select_friendly_town(); // "Select a friendly town";
      message_sub_mode = 1;
      scenario_editor.edit_sub_menu_mode[scenario_editor.edit_mode] =
          SE_VICTORY_MESSAGE;
      break;
    ...
    case EP_FRIEND_FIRM_RECNO:
      message_str =
          text_editor
              .str_select_friendly_firm(); // "Select a friendly building";
      message_sub_mode = 1;
      scenario_editor.edit_sub_menu_mode[scenario_editor.edit_mode] =
          SE_VICTORY_MESSAGE;
      break;
...
//
static void add_condition() {
  // parameter are: selected_win/lose_condition, selected_recno,
  // selected_scalar, game_time_limit
  ...
  case EP_FRIEND_TOWN_RECNO:
    if (town_array.selected_recno &&
        town_array[town_array.selected_recno]->is_own())
      selected_recno = town_array.selected_recno;
    else
      failed = 1;
    break;
  ...
  case EP_FRIEND_FIRM_RECNO:
    if (firm_array.selected_recno &&
        firm_array[firm_array.selected_recno]->is_own())
      selected_recno = firm_array.selected_recno;
    else
      failed = 1;
    break;
  ...
  if (selected_win_condition) {
    ...
    game.add_win_condition(selected_win_condition, 1, selected_recno,
                           selected_scalar, selected_year_limit);
  } else if (selected_lose_condition) {
    if (game.lose_condition_count >= MAX_LOSE_CONDITION)
      return;

    game.add_lose_condition(selected_lose_condition, 1, selected_recno,
                            selected_scalar, selected_year_limit);
  }
}
