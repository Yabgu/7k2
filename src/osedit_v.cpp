...
void ScenarioEditor::disp_victory_main(int refreshFlag)
{
    ...
    case SE_VICTORY_MAIN: // victory main menu
        ...
        if (win_or_lose == 0) // win
        {
            if (refreshFlag == INFO_REPAINT)
            {
                browse_win_condition.init(INFO_X1, INFO_Y1 + 32, INFO_X2, INFO_Y1 + 240, -1, 36,
                                          game.win_condition_count, disp_win_condition);
                browse_win_condition.open(win_condition_browse_recno);
                button_add_win_condition.create_text(INFO_X1 + 20, INFO_Y1 + 252, INFO_X1 + 100, INFO_Y1 + 272,
                                                     text_editor.str_add()); //"Add");
                button_del_win_condition.create_text(INFO_X1 + 120, INFO_Y1 + 252, INFO_X1 + 200, INFO_Y1 + 272,
                                                     text_editor.str_delete()); // "Delete");
                button_set_win_require_all_condition.create_text(INFO_X1 + 12, INFO_Y1 + 275, INFO_X2 - 12,
                                                                 INFO_Y1 + 295, text_editor.str_require_all(),
                                                                 0); //"Require All");
            }
        ...
        else // lose
        {
            if (refreshFlag == INFO_REPAINT)
            {
                browse_lose_condition.init(INFO_X1, INFO_Y1 + 32, INFO_X2, INFO_Y1 + 240, -1, 36,
                                           game.lose_condition_count, disp_lose_condition);
                browse_lose_condition.open(lose_condition_browse_recno);
                button_add_lose_condition.create_text(INFO_X1 + 20, INFO_Y1 + 252, INFO_X1 + 100, INFO_Y1 + 272,
                                                      text_editor.str_add()); //"Add");
                button_del_lose_condition.create_text(INFO_X1 + 120, INFO_Y1 + 252, INFO_X1 + 200, INFO_Y1 + 272,
                                                      text_editor.str_delete()); // "Delete");
            }
    ...
    case SE_VICTORY_WIN_COND: // select winning condition
        if (refreshFlag == INFO_REPAINT)
        {
            browse_win_id.init(INFO_X1, INFO_Y1 + 6, INFO_X2, INFO_Y1 + 220, -1, 20, filtered_win_condition_count,
                               disp_win_cond_id);
            // ####### begin Gilbert 22/2 ########//
            browse_win_id.open(win_id_browse_recno);
            // ####### end Gilbert 22/2 ########//
            button_next.create_text(INFO_X1 + 20, INFO_Y1 + 230, INFO_X1 + 100, INFO_Y1 + 250,
                                    text_editor.str_next()); // "Next" );
            button_cancel.create_text(INFO_X1 + 120, INFO_Y1 + 230, INFO_X1 + 200, INFO_Y1 + 250,
    ...
    case SE_VICTORY_LOSE_COND: // select losing condition
        if (refreshFlag == INFO_REPAINT)
        {
            browse_lose_id.init(INFO_X1, INFO_Y1 + 6, INFO_X2, INFO_Y1 + 220, -1, 20, filtered_lose_condition_count,
                                disp_lose_cond_id);
            // ####### begin Gilbert 22/2 ########//
    ...
    case SE_VICTORY_SEL_UNIT_ID: // select unit id
        if (refreshFlag == INFO_REPAINT)
        {
            browse_unit_id.init(INFO_X1, INFO_Y1 + 6, INFO_X2, INFO_Y1 + 220, -1, 20, unit_res.unit_info_count,
                                disp_unit_id);
            // ##### begin Gilbert 22/2 #######//
    ...
    case SE_VICTORY_SEL_ITEM_ID:
        if (refreshFlag == INFO_REPAINT)
        {
            browse_item_id.init(INFO_X1, INFO_Y1 + 6, INFO_X2, INFO_Y1 + 220, -1, 20, ITEM_TYPE_COUNT, disp_item_id);
            // ##### begin Gilbert 22/2 #######//
    ...
    case SE_VICTORY_SEL_TECH_ID:
        if (refreshFlag == INFO_REPAINT)
        {
            browse_tech_id.init(INFO_X1, INFO_Y1 + 6, INFO_X2, INFO_Y1 + 220, -1, 20, MAX_TECH, disp_tech_id);
    ...
    case SE_VICTORY_SCALAR: // input a scalar quantity
        if (refreshFlag == INFO_REPAINT)
        {
            ...
            button_next.create_text(INFO_X1 + 20, INFO_Y1 + 30, INFO_X1 + 100, INFO_Y1 + 50,
                                    text_editor.str_next()); //"Next" );
            button_cancel.create_text(INFO_X1 + 120, INFO_Y1 + 30, INFO_X1 + 200, INFO_Y1 + 50,
                                      text_editor.str_cancel()); //"Cancel" );
        }
    ...
    case SE_VICTORY_YEAR_LIMIT: // input years limit
        ...
        if (selected_year_limit)
        {
            ...
            // INFO_Y1+12, selected_year_limit ); 			font_whbl.put(
            // x2, INFO_Y1+12, selected_year_limit > 1 ? "years" : "year" );
    ...
    case SE_VICTORY_MESSAGE: {
        ...
        if (refreshFlag == INFO_REPAINT)
        {
            ...
            {
                button_next.create_text(INFO_X1 + 20, INFO_Y1 + 110, INFO_X1 + 100, INFO_Y1 + 130,
                                        text_editor.str_next()); //"Next" );
                button_cancel.create_text(INFO_X1 + 120, INFO_Y1 + 110, INFO_X1 + 200, INFO_Y1 + 130,
                                          text_editor.str_cancel()); // "Cancel" );
            }
        }
        button_next.paint();
...
{
    // ###### begin Gilbert 22/2 #######//
    font_snds.put_paragraph(x + 6, y + 2, x + 195, y + 34,
...
static void disp_win_cond_id(int recNo, int x, int y, int refreshFlag)
{
    font_snds.put(x + 6, y + 2, EndConditionInfo::end_condition_info[filtered_win_condition[recNo - 1]].name(), 0,
                  x + 194);
}
...
static void disp_lose_cond_id(int recNo, int x, int y, int refreshFlag)
{
    font_snds.put(x + 6, y + 2, EndConditionInfo::end_condition_info[filtered_lose_condition[recNo - 1]].name(), 0,
                  x + 194);
}
...
static int proceed_victory_menu()
{
    ...
    else if (current_para >= 1 && current_para <= 2)
    {
        ...
        case EP_NATION_RECNO:                              // get select object
            message_str = text_editor.str_select_nation(); //"Select target nation by selecting
...
static void add_condition()
{
    ...
    if (selected_win_condition)
    {
        ...
        game.add_win_condition(selected_win_condition, 1, selected_recno, selected_scalar, selected_year_limit);
    }
    else if (selected_lose_condition)
    {
        ...
        game.add_lose_condition(selected_lose_condition, 1, selected_recno, selected_scalar, selected_year_limit);
    }
}
