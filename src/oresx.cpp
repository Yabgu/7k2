...
// <int>   readAll		= whether read all data into the buffer or read one
// each time
//									(default:0)
...
//						0 - failed
//
int ResourceIdx::read_into_user_buf(char *dataName, char *userBuf,
                                    int userBufSize, int userStartReadPos) {
  ...
  int rc =
      get_data(indexId) != NULL; // ==NULL if actual data size > buffer size
...
char *ResourceIdx::get_data(int indexId) {
  ...
  indexId--; // make it start from 0, instead of starting from 1 for easy array
             // accessing
  ...
  if (user_data_buf) {
    if (user_start_read_pos > 0) {
      file_seek(user_start_read_pos,
                SEEK_CUR); // skip the width and height info
...
File *ResourceIdx::get_file(const char *dataName, int &dataSize) {
  ...
  //			dataSize = index_buf[i+1].pointer -
  //index_buf[i].pointer; 			return this;
  //		}
  //	}
...
//the buffer.
//
void ResourceIdx::set_user_buf(char *userDataBuf, int bufSize,
                               int userStartReadPos) {
...
void ResourceIdx::build_sorted_index() {
  ...
  memcpy(sorted_index_buf, index_buf,
         rec_count * sizeof(ResIndex)); // last dummy record is not copied
