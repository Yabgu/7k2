...
short Magic::add_blend3(short *point, short R, short G, short B, int mode,
                        int par) {
  short prev_r;
  short prev_g;
  short prev_b;
...
void Magic::init(int x1, int y1, int x2, int y2) {
  ...
  *(check_table + 0 + 0) = (unsigned short)0x001F;
  *(check_table + 6 + 0) = (unsigned short)0x001F;
  *(check_table + 0 + 1) = (unsigned short)0x03E0;
  *(check_table + 6 + 1) = (unsigned short)0x07E0;
  *(check_table + 0 + 2) = (unsigned short)0x7C00;
  *(check_table + 6 + 2) = (unsigned short)0xF800;
  *(check_table + 0 + 3) = (unsigned short)0x001F;
  *(check_table + 6 + 3) = (unsigned short)0x003F;
  *(check_table + 0 + 4) = (unsigned short)0;
  *(check_table + 6 + 4) = (unsigned short)0;
  *(check_table + 0 + 5) = (unsigned short)10;
  *(check_table + 6 + 5) = (unsigned short)11;

  *(check_table + 12 + 0) = (unsigned short)0x7C00;
  *(check_table + 18 + 0) = (unsigned short)0xF800;
  *(check_table + 12 + 1) = (unsigned short)0x03E0;
  *(check_table + 18 + 1) = (unsigned short)0x07E0;
  *(check_table + 12 + 2) = (unsigned short)0x001F;
  *(check_table + 18 + 2) = (unsigned short)0x001F;
  *(check_table + 12 + 3) = (unsigned short)0x001F;
  *(check_table + 18 + 3) = (unsigned short)0x003F;
  *(check_table + 12 + 4) = (unsigned short)10;
  *(check_table + 18 + 4) = (unsigned short)11;
  *(check_table + 12 + 5) = (unsigned short)0;
  *(check_table + 18 + 5) = (unsigned short)0;
...
double Magic::dist(double dx, double dy) { return sqrt(dx * dx + dy * dy); }
...
void Magic::straight_light_beam(VgaBuf *vgabuf, int x1, int y1, int x2, int y2,
                                int thickness, int lightness, short R1,
                                short G1, short B1, short R2, short G2,
                                short B2, int mode, char angle) {
  ...
  int line =
      (y2 - y1) > 0 ? (2 * vgabuf->buf_pitch()) : -(2 * vgabuf->buf_pitch());
...
END:	mov eax, thickness
  ...
  if (d_x > d_y) {
    ...
    for (k = -thickness; k <= thickness; k++) {
      ...
      limit = d_x;
      if (angle > 1) {
        line = (y2 - y1 - k) > 0 ? (2 * vgabuf->buf_pitch())
                                 : -(2 * vgabuf->buf_pitch());
      ...
      if (angle == 1) {
        ...
        point =
            vgabuf->buf_ptr(x1 + (x2 - x1) * abs(k) / thickness / 2, k + y1);
      }
...
FINISH12:			add esi, x_inc   // point = point + x_inc;
...
#else
        {
          ...
          prev_r = *edi;
          short ax =
              (static_cast<unsigned short>(*point & prev_r) >> cl) + temp_r;
          if (!(ax > 0x1F)) {
            prev_r = ax << cl;
          }
          prev_g = *(edi + 1);
    ...
    for (k = -thickness; k <= thickness; k++) {
      ...
      if (angle == 1) {
        ...
        point =
            vgabuf->buf_ptr(k + x1, y1 + (y2 - y1) * abs(k) / thickness / 2);
      }
      ratio = 15 - (abs(k) - lightness) * 15 / diff;
      temp_r = (R1 * ratio) >> 4;
      temp_g = (G1 * ratio) >> 4;
...
END:	mov eax, thickness
  ...
  if (d_x > d_y) {
    ...
    for (k = -thickness; k <= thickness; k++) {
      ...
      // clipping the Y axis and calculate the new starting and ending y
      // coordinates no need to clip the x axis because the draw_light_beam()
      // routine has done this part avoid dividing by zero error
      if (slope == 0)
        slope = 1;
      ...
      if (angle > 1) {
        ...
        // clipping the Y axis and calculate the new starting and ending y
        // coordinates no need to clip the x axis because the draw_light_beam()
        // routine has done this avoid dividing by zero error
        if (slope == 0)
          slope = 1;
        ...
        line = (tempY2 - tempY1) > 0 ? (2 * vgabuf->buf_pitch())
                                     : -(2 * vgabuf->buf_pitch());
        limit = abs(tempX2 - tempX1);
        // avoid dividing by zero error
        if (limit != 0)
      ...
      if (angle == 1) {
        //	limit = d_x - d_x * abs(k)/thickness;
        //	point = vgabuf->buf_ptr(x1 + (x2-x1) *abs(k) /thickness /2,k +
        //y1);
        limit = limit - limit * abs(k) / thickness;
        point = vgabuf->buf_ptr(
            tempX1 + (tempX2 - tempX1) * abs(k) / thickness / 2, tempY1);
      }
      // Ready Variables for line drawing
      ratio = 15 - (abs(k) - lightness) * 15 / diff;
      temp_r = (R1 * ratio) >> 4;
...
FINISH12:			add esi, x_inc   // point = point + x_inc;
    ...
    for (k = -thickness; k <= thickness; k++) {
      ...
      if (angle == 1) {
        limit = limit - limit * abs(k) / thickness;
        point = vgabuf->buf_ptr(tempX1, tempY1 + (tempY2 - tempY1) * abs(k) /
                                                     thickness / 2);
      }
      ratio = 15 - (abs(k) - lightness) * 15 / diff;
      temp_r = (R1 * ratio) >> 4;
...
void Magic::draw_light_beam(VgaBuf *vgabuf, int x1, int y1, int x2, int y2,
                            int thickness, int lightness, short R1, short G1,
                            short B1, short R2, short G2, short B2, int mode,
                            char angle, char fast) {
  int dx = abs(x2 - x1);
  int dy = abs(y2 - y1);
  int b_x1 = bound_x1;
  int b_x2 = bound_x2;
  ...
  if ((x1 < b_x1 && x2 < b_x1) || (x1 > b_x2 && x2 > b_x2) ||
    ...
    return;
  if (x1 >= b_x1 && x1 <= b_x2 && x2 >= b_x1 && x2 <= b_x2 && y1 >= b_y1 &&
      y1 <= b_y2 && y2 >= b_y1 && y2 <= b_y2)

    if (fast)
      straight_light_beam(vgabuf, x1, y1, x2, y2, thickness, lightness, R1, G1,
                          B1, R2, G2, B2, mode, angle);
    else
      straight_light_beam2(vgabuf, x1, y1, x2, y2, thickness, lightness, R1, G1,
                           B1, R2, G2, B2, mode, angle);
  else {
    if (y1 == y2) {
      // horizontal line
      ...
      if (x2 < b_x1)
        ...
        x2 = b_x2;
      if (fast)
        straight_light_beam(vgabuf, x1, y1, x2, y2, thickness, lightness, R1,
                            G1, B1, R2, G2, B2, mode, angle);
      else
        straight_light_beam2(vgabuf, x1, y1, x2, y2, thickness, lightness, R1,
                             G1, B1, R2, G2, B2, mode, angle);
    } else if (x1 == x2) {
      // vertical line
      if (x1 < b_x1 || x1 > b_x2)
...
void Magic::generate_particle(int x1, int y1, int x2, int y2, int dirx,
                              int diry, int type, int var, int mode, int number,
                              int birth_time, int lifecount, int lifeLimit,
                              int lifeOffset) {
  int i, diff;
  ...
  if (type == 0) {
    ...
    for (i = 0; i < number; i++) {
      ...
      if (diff > 0) {
        ...
        if (temp_ptr->dir_y <= 0)
          temp_ptr->pos_y =
              temp_y + temp_ptr->dir_y * lifeLimit + diff / lifeLimit;
        else
          temp_ptr->pos_y =
              temp_y + temp_ptr->dir_y * lifeLimit + (diff >> 1) / lifeLimit;
      } else {
    ...
    for (i = 0; i < number; i++) {
      ...
      if (mode == 0)
        temp_ptr->color =
            0xFFFF >> (5 + (temp_ptr->life + lifeOffset) * 10 /
                               20); //	temp_ptr->prev_color = 0xFFFF;
                                    //temp_ptr->delta_color = 25 +temp_r1/4;
      else if (mode == 1)
        temp_ptr->color =
            0xFFFF >> (5 + (temp_ptr->life + lifeOffset) * 11 /
                               20); //	temp_ptr->prev_color = 0xFFFF;
                                    //temp_ptr->delta_color = 25 +temp_r1/4;
      else if (mode == 2)
        temp_ptr->color =
...
void Magic::draw_particle(int x1, int y1, int x2, int y2, int dirx, int diry,
                          int type, int var, int size, VgaBuf *vgabuf, int mode,
                          int number, int birth_time, int lifecount,
                          int life_limit, int lifeOffset) {
  Particle *particle;
  int i;
  seed = (unsigned)(x1 + x2 + y1 + y2);
  (void)rand_seed();
  h_particle = (Particle *)mem_add(sizeof(Particle) * number);
  generate_particle(x1, y1, x2, y2, dirx, diry, type, var, mode, number,
                    birth_time, lifecount, life_limit, lifeOffset);
  ...
  if (type == 0) // direction particle
  {
    for (i = 0; i < number; i++) {
      ...
      if ((particle->life <= life_limit) && (particle->life >= 0))
        draw_circle(vgabuf, particle->pos_x, particle->pos_y, 5, 1, 1, 200, 100,
                    100, mode, 0);
    }
  } else if (type == 1) // explosion particle
  {
    for (i = 0; i < number; i++) {
      particle = h_particle + i;
  ...
  } else if (type == 2) // explosion circle particle with size variation
  {
    for (i = 0; i < number; i++) {
      particle = h_particle + i;
      if ((particle->life <= (life_limit << 1)) && (particle->life >= 0))
        draw_circle(vgabuf, particle->pos_x, particle->pos_y, size, 1, 1, 200,
                    100, 100, mode, 0);
    }
  } else if (type == 3) // explosion hole particle with size variation
  {
    for (i = 0; i < number; i++) {
      particle = h_particle + i;
      if ((particle->life <= (life_limit << 1)) && (particle->life >= 0))
  ...
  } else // direction line
  {
    ...
    for (i = 0; i < number; i++) {
      ...
      if ((particle->life <= life_limit) && (particle->life >= 0) &&
        ...
        IMGline(vgabuf->buf_ptr(), vgabuf->buf_true_pitch(),
                vgabuf->buf_width(), vgabuf->buf_height(), particle->prev_x,
                particle->prev_y, particle->pos_x, particle->pos_y,
                particle->color);
    }
  }
  mem_del(h_particle);
...
void Magic::draw_circle(VgaBuf *vgabuf, int x1, int y1, int radius, char r_x,
                        char r_y, short R1, short G1, short B1, int mode,
                        int dir) {
...
#ifdef ASM_FOR_MSVC
        _asm {
      mov edi, base1 add edi, mode_offset mov esi,
          point // ok
              mov dx,
          [esi]
          // RED:
          mov cl,
          [edi + 8] mov bx, [edi] mov prev_r,
          bx // assign R_OVER value
              mov ax,
          dx and ax, bx shr ax, cl add ax, temp_r cmp ax, 001fh jg GREEN shl ax,
          cl // assign R_UNDER value
              mov prev_r,
          ax GREEN : mov bx,
                     [edi + 2] mov prev_g,
                     bx // assign G_OVER value
                         mov ax,
                     dx and ax,
                     bx shr ax,
                     5 add ax,
                     temp_g cmp ax,
                     [edi + 6] jg BLUE shl ax,
                     5 // assign G_UNDER value
                     mov prev_g,
                     ax BLUE : mov cl,
                               [edi + 10] mov bx,
                               [edi + 4] mov prev_b,
                               bx // assign B_OVER value
                                   mov ax,
                               dx and ax,
                               bx shr ax,
                               cl add ax,
                               temp_b cmp ax,
                               001fh jg END shl ax,
                               cl // assign B_UNDER value
                                   mov prev_b,
                               ax END
          : mov ax,
            prev_r //*point = prev_r | prev_g | prev_b;
                or ax,
            prev_g or ax,
            prev_b mov[esi],
            ax inc esi // point ++;
            ...
            ebx inc eax mov mag,
            eax mov edi,
            dis_state // if (mag < *(color_table +dis_state))
                mov ebx,
            base3 shl edi,
            2 add ebx,
            edi mov esi,
            ...
            edi shr edi,
            1 cmp eax,
            [ebx] jl D_MINS cmp eax,
            [ebx + 4] // if (mag > *(color_table +dis_state +1))
            jle D_PLUS add esi,
            4 // dis_state ++;
            add edi,
            2 D_MINS : sub esi,
                       2 // dis_state --;
                       dec edi mov ax,
                       [esi + 2] // temp_r = *(color_table + 7 +dis_state +1);
                       mov temp_r,
                       ax mov ax,
                       [esi + 16] // temp_g = *(color_table +14 +dis_state +1);
                       mov temp_g,
                       ax mov ax,
                       [esi + 30] // temp_b = *(color_table +21 +dis_state +1);
                       mov temp_b,
                       ax mov dis_state,
                       edi D_PLUS:
    }
...
//			0-> ellipse colour intensity decreases towards the
//center
//
// no return values
//
void Magic::draw_circle2(VgaBuf *vgabuf, int x1, int y1, int radius, char r_x,
                         char r_y, short R1, short G1, short B1, int mode,
                         int dir) {
  int i, j, line = 2 * vgabuf->buf_pitch();
  int mag, temp_mag, up_level;
...
FINISH:
  ...
  *(reinterpret_cast<unsigned int *>(base3 + 10)) = eax;
  ...
  if (r_y > 0)
    temp_mag =
        temp_mag +
        ((((radius - abs(start_y)) * (1 + radius + abs(start_y))) >> 1) << 1) -
        ((((radius - abs(start_y)) * (1 + radius + abs(start_y))) >> 1) >>
         (r_y - 1));

  if (r_x > 0)
    temp_mag =
        temp_mag +
        ((((radius - abs(start_x)) * (1 + radius + abs(start_x))) >> 1) << 1) -
        ((((radius - abs(start_x)) * (1 + radius + abs(start_x))) >> 1) >>
         (r_x - 1));
...
TD_PLUS:
  ...
  /*
          ...
          _asm
          {
                  // Convert R1, B1 and G1 from 0-255 to 0-31 or 0-63 depending
     on the pixel format
                  //R1 = R1 *31 /255
                  mov ax, R1
                  shr ax, 3
          ...
          for(y =-16; y <17; y++)
          {
                  for(x =-16; x <17; x++)
                  {
                          _asm
                          {
                                  ...
                                  add esi, base2
                                  mov ax, [esi+ 2]		//temp_r =
     *(color_table + 7 +dis_state +1); mov temp_r, ax mov ax, [esi+38]
     //temp_g = *(color_table +14 +dis_state +1); mov temp_g, ax mov ax,
     [esi+74]		//temp_b = *(color_table +21 +dis_state +1); mov temp_b,
     ax
                          }
                          //draw one pixel
                          ...
                          for(j =0; j <(rY>>4); j++)
                          {
                                  for(i =0; i <(rX>>4); i++)
                                  {
                                          ...
                                          //RED:
                                                  ...
                                                  mov bx, [edi]
                                                  mov prev_r, bx
     // assign R_OVER value mov ax, dx and ax, bx shr ax, cl add ax, temp_r cmp
     ax, 001fh jg  GREEN
                                                  shl ax, cl
     // assign R_UNDER value mov prev_r, ax GREEN: mov bx, [edi +2] mov prev_g,
     bx			// assign G_OVER value mov ax, dx and ax, bx shr ax, 5
                                                  add ax, temp_g
                                                  cmp ax, [edi +6]
                                                  jg  BLUE
                                                  shl ax, 5
     // assign G_UNDER value mov prev_g, ax BLUE: mov cl, [edi +10] mov bx, [edi
     +4] mov prev_b, bx			// assign B_OVER value mov ax, dx and
     ax, bx shr ax, cl add ax, temp_b cmp ax, 001fh jg  END shl ax, cl
     // assign B_UNDER value mov prev_b, ax END: mov ax, prev_r
     //*point = prev_r | prev_g | prev_b; or  ax, prev_g or  ax, prev_b mov
     [esi], ax
                                          }
                                          point ++;
                                  }
...
void Magic::generate_lighting_particle(int x1, int y1, int x2, int y2,
                                       int number, int lifecount,
  ...
  for (i = 0; i < (number - 1); i++) {
    ...
    if (temp_r1 < limit) {
      ...
      temp_ptr->dir_x =
          (temp_ry - (step_y[1] << 3)) >> 5; //(temp_r1 - 8) * step_y / 32
      temp_ptr->dir_y =
          ((step_x[1] << 3) - temp_rx) >> 5; //(temp_r1 - 8) *-step_x / 32
      temp_ptr->pos_x =
          temp_ptr->prev_x + (temp_rx >> 3) +
          temp_ptr->dir_x * lifecount; // assigning current_position
      temp_ptr->pos_y =
          temp_ptr->prev_y + (temp_ry >> 3) +
          temp_ptr->dir_y * lifecount; // temp_r1 *(y2-y1) /(number *8) +
                                       // temp_ptr->dir_y *lifecount
    }
...
// ----------- End of function Magic::generate_particle ----------//
// ----------- Begin of function Magic::draw_particle ----------//
void Magic::draw_lighting(int x1, int y1, int x2, int y2, int thickness,
                          int lightness, short R1, short G1, short B1, short R2,
                          short G2, short B2, VgaBuf *vgabuf, int var, int mode,
                          int number, int lifecount, int level) {
  Lighting_Particle *particle, *particle1, *h_l_particle;
  int i;
  int temp, temp1;
  short R3 = (R1 + (R2 >> 1)) >> 1, G3 = (G1 + (G2 >> 1)) >> 1,
        B3 = (B1 + (B2 >> 1)) >> 1;
  seed = (unsigned)(x1 + x2 + y1 + y2 + var);
  (void)rand_seed();
  h_l_particle =
      (Lighting_Particle *)mem_add(sizeof(Lighting_Particle) * (number + 2));
  generate_lighting_particle(x1, y1, x2, y2, number, lifecount, h_l_particle);
  for (i = 0; i < number; i++) {
    particle = h_l_particle + i;
    draw_light_beam(vgabuf, particle->prev_x, particle->prev_y, particle->pos_x,
                    particle->pos_y, thickness, lightness, R1, G1, B1, R2, G2,
                    B2, mode, 00);
    draw_circle(vgabuf, particle->pos_x, particle->pos_y, thickness, 1, 1, R3,
                G3, B3, mode, 0);
  }
  ...
  if (level > 0) {
    for (i = 0; i < number; i++) {
      ...
      draw_lighting(particle->prev_x, particle->prev_y, particle1->pos_x,
                    particle1->pos_y, thickness, lightness, R1, G1, B1, R2, G2,
                    B2, vgabuf, level, mode, temp, lifecount, temp1);
    }
  }
...
void Magic::draw_magic_one(VgaBuf *vgabuf, int x1, int y1, int mode,
                           int count) {
  if (count == 6) {
    draw_light_beam(vgabuf, x1 - 100, y1 - 200, x1, y1, 10, 2, 255, 80, 0, 255,
                    255, 255, mode, 0);
    draw_light_beam(vgabuf, x1 + 100, y1 - 200, x1, y1, 10, 2, 255, 80, 0, 255,
                    255, 255, mode, 0);
    draw_light_beam(vgabuf, x1 - 150, y1 + 100, x1, y1, 10, 2, 255, 80, 0, 255,
                    255, 255, mode, 0);
    draw_light_beam(vgabuf, x1 + 150, y1 + 100, x1, y1, 10, 2, 255, 80, 0, 255,
                    255, 255, mode, 0);
    ...
    draw_circle(vgabuf, x1 + 150, y1 + 100, 25, 1, 1, 255, 200, 0, mode, 1);
    draw_particle(x1 - 100, y1 - 200, x1 - 100, y1 - 200, 0, 0, 0, 6, 0, vgabuf,
                  mode, 100, 2, 0, 20);
    draw_particle(x1 + 100, y1 - 200, x1 + 100, y1 - 200, 0, 0, 0, 6, 0, vgabuf,
                  mode, 100, 2, 0, 20);
    draw_particle(x1 - 150, y1 + 100, x1 - 150, y1 + 100, 0, 0, 0, 6, 0, vgabuf,
                  mode, 100, 2, 0, 20);
    draw_particle(x1 + 150, y1 + 100, x1 + 150, y1 + 100, 0, 0, 0, 6, 0, vgabuf,
                  mode, 100, 2, 0, 20);
  }
  ...
  if ((count > 5) && (count < 16))
    draw_circle(vgabuf, x1, y1, (count - 4) << 3, 0, 1, 255 - 25 * (count - 6),
                255 - 25 * (count - 6), 10, mode, 1);
  if ((count > 0) && (count < 6)) {
    draw_circle(vgabuf, x1 - 100, y1 - 200, 17, 1, 1, 255, 200, 0, mode, 1);
    draw_circle(vgabuf, x1 + 100, y1 - 200, 17, 1, 1, 255, 200, 0, mode, 1);
    draw_circle(vgabuf, x1 - 150, y1 + 100, 25, 1, 1, 255, 200, 0, mode, 1);
  ...
  if ((count > 6) && (count < 21))
    draw_particle(x1 + 10, y1 + 10, x1 - 10, y1 - 10, 0, 0, 1, 10, 0, vgabuf,
                  mode, 100, 2, count - 6, 6);
}
// ----------- End of function Magic::draw_magic_one -----//
// ----------- Begin of function Magic::draw_magic_two -----//
void Magic::draw_magic_two(VgaBuf *vgabuf, int x1, int y1, int x2, int y2,
                           int diff, int mode, int curStep, int totalStep) {
  ...
  //	if ((count == 3) || (count == 6))
  //		vgabuf->bar_alpha( bound_x1, bound_y1, bound_x2, bound_y2, 1,
  //V_BLACK );
  ...
  if ((count > 3) && (count < 6)) {
    draw_lighting(x1, y1, x2 + diff, y2 + diff, 8, 2, 127, 127, 00, 255, 255,
                  255, vgabuf, (count - 3) * 27, mode, 10, 3, 1);
  ...
  if ((count > 6) && (count < 9)) {
    draw_lighting(x1, y1, x2 - diff, y2 - diff, 8, 2, 127, 127, 00, 255, 255,
                  255, vgabuf, (count - 6) * 117, mode, 10, 3, 1);
    draw_circle(vgabuf, x2 - diff, y2 - diff, 40, 1, 1, 255, 255, 255, mode, 0);
  }
  if ((count > 0) && (count < 4)) {
    //		draw_light_beam(&vga_back, x1, y1, x1, y1-count*15,
    //						10, 4, 127, 127, 0, 255, 255, 255, vga.pixel_format_flag,
    //02); 		draw_light_beam(&vga_back, x1, y1, x1+count*15, y1, 						10, 4, 127, 127,
    //0, 255, 255, 255, vga.pixel_format_flag, 02); 		draw_light_beam(&vga_back,
    //x1, y1, x1, y1+count*15, 						10, 4, 127, 127, 0, 255, 255, 255,
    //vga.pixel_format_flag, 02); 		draw_light_beam(&vga_back, x1, y1,
    //x1-count*15, y1, 						10, 4, 127, 127, 0, 255, 255, 255, vga.pixel_format_flag,
    //02);
    draw_circle(vgabuf, x1, y1, (10 * count), 1, 1, 255, 255, 255, mode, 0);
    draw_circle(vgabuf, x1, y1 + 5 - (10 * count), (12 * count), 1, 1, 255, 255,
                255, mode, 1);
  }
  if ((count > 3) && (count < 10)) {
    //		draw_light_beam(&vga_back, x1, y1, x1, y1-45,
    //						10, 4, 127, 127, 0, 255, 255, 255, vga.pixel_format_flag,
    //02); 		draw_light_beam(&vga_back, x1, y1, x1+45, y1, 						10, 4, 127, 127, 0,
    //255, 255, 255, vga.pixel_format_flag, 02); 		draw_light_beam(&vga_back, x1,
    //y1, x1, y1+45, 						10, 4, 127, 127, 0, 255, 255, 255, vga.pixel_format_flag,
    //02); 		draw_light_beam(&vga_back, x1, y1, x1-45, y1, 						10, 4, 127, 127, 0,
    //255, 255, 255, vga.pixel_format_flag, 02);
    draw_circle(vgabuf, x1, y1, 50, 1, 1, 255, 255, 255, mode, 0);
    draw_circle(vgabuf, x1, y1 - 35, 50, 1, 1, 255, 255, 255, mode, 1);
  }
  if ((count > 3) && (count < 18))
    draw_particle(x2 + diff + 10, y2 + diff + 10, x2 + diff - 10,
                  y2 + diff - 10, 0, 0, 1, 10, 0, vgabuf, mode, 100, 2,
                  count - 3, 6);
  if ((count > 6) && (count < 21))
    draw_particle(x2 - diff + 10, y2 - diff + 10, x2 - diff - 10,
                  y2 - diff - 10, 0, 0, 1, 10, 0, vgabuf, mode, 100, 2,
...
void Magic::draw_magic_three(VgaBuf *vgabuf, int x1, int y1, int x2, int y2,
                             int mode, int count) {
  if ((count > 0) && (count < 7)) {
  ...
  if (count == 1) {
    draw_light_beam(vgabuf, x1, y1, x2, y2, 25, 0, 127, 127, 0, 127, 127, 127,
                    mode, 2);
    draw_circle(vgabuf, x2, y2, 35, 1, 1, 255, 255, 255, mode, 0);
  }
  if ((count > 0) && (count < 7)) {
    draw_circle(vgabuf, (x1 - x2) / 3 + x2, (y1 - y2) / 3 + y2,
                (count + 1) << 3, 0, 1, 255 - 50 * (count - 1),
                255 - 50 * (count - 1), 10, mode, 1);
    draw_circle(vgabuf, (x1 - x2) * 2 / 3 + x2, (y1 - y2) * 2 / 3 + y2,
                (count + 2) << 3, 0, 1, 255 - 50 * (count - 1),
                255 - 50 * (count - 1), 10, mode, 1);
  }
  ...
  if ((count > 5) && (count < 12)) {
    draw_circle(vgabuf, (x1 - x2) / 3 + x2, (y1 - y2) / 3 + y2,
                (count - 4) << 3, 0, 1, 255 - 50 * (count - 6),
                255 - 50 * (count - 6), 10, mode, 1);
    draw_circle(vgabuf, (x1 - x2) * 2 / 3 + x2, (y1 - y2) * 2 / 3 + y2,
                (count - 3) << 3, 0, 1, 255 - 50 * (count - 6),
                255 - 50 * (count - 6), 10, mode, 1);
  }
  x1 = x1 + 200;
  ...
  if ((count > 10) && (count < 17)) {
    draw_circle(vgabuf, (x1 - x2) / 3 + x2, (y1 - y2) / 3 + y2,
                (count - 9) << 3, 0, 1, 255 - 50 * (count - 11),
                255 - 50 * (count - 11), 10, mode, 1);
    draw_circle(vgabuf, (x1 - x2) * 2 / 3 + x2, (y1 - y2) * 2 / 3 + y2,
                (count - 8) << 3, 0, 1, 255 - 50 * (count - 11),
                255 - 50 * (count - 11), 10, mode, 1);
  }
  if ((count > 0) && (count < 21))
    draw_particle(x2 + 5, y2 + 5, x2 - 5, y2 - 5, 0, 0, 1, 10, 0, vgabuf, mode,
                  100, 2, count, 9);
  if ((count > 5) && (count < 21))
    draw_particle(x2 + 5, y2 + 5, x2 - 5, y2 - 5, 0, 0, 1, 10, 0, vgabuf, mode,
                  100, 2, count - 5, 7);
...
void Magic::draw_magic_four(VgaBuf *vgabuf, int x1, int y1, int x2, int y2,
                            int mode, int count) {
  int step_x = (x2 - x1) >> 3;
  int step_y = (y2 - y1) >> 3;
  ...
  if ((count > 2) && (count < 11))
    draw_particle(x1 + 5, y1 + 5, x1 - 5, y1 - 5, -step_x, -step_y, 0, -avg, 0,
                  vgabuf, mode, 100, 2, 3, 15);
  if ((count > 9) && (count < 21))
    draw_circle(vgabuf, x2, y2, (count - 8) << 3, 0, 1, 255 - 22 * (count - 10),
                10, 255 - 22 * (count - 10), mode, 1);
  if ((count > 9) && (count < 21))
    draw_particle(x2 + 5, y2 + 5, x2 - 5, y2 - 5, -step_x >> 1, -step_y >> 1, 1,
                  -avg, 0, vgabuf, mode, 100, 2, count - 9, 5);
}
// ----------- End of function Magic::draw_magic_four -----//
// ----------- Begin of function Magic::draw_magic_five -----//
void Magic::draw_magic_five(VgaBuf *vgabuf, int x1, int y1, int x2, int y2,
                            int mode, int count) {
  int step_x = (x1 - x2) >> 2;
  int step_y = (y1 - y2) >> 2;
  int avg = (step_x + step_y) >> 1;
  draw_particle(x2 + 8, y2 + 8, x2 - 10, y2 - 10, step_x >> 1, step_y >> 1, 2,
                avg, 40 - (count << 1), vgabuf, mode, 15, 2, count - 1, 8);
  draw_particle(x2 + 10, y2 + 10, x2 - 8, y2 - 8, step_x >> 1, step_y >> 1, 3,
                avg, 20, vgabuf, mode, 5, 2, count - 1, 8);
}
// ----------- End of function Magic::draw_magic_five -----//
// ----------- Begin of function Magic::draw_magic_six -----//
void Magic::draw_magic_six(VgaBuf *vgabuf, int x1, int y1, int x2, int y2,
                           int mode, int count) {
  if ((count > 10) && (count < 15)) {
    draw_circle(vgabuf, x1, y1, 25 + (count << 3), 1, 1, 55 + 5 * (count - 10),
                255 - (count - 10) * 50, 255 - (count - 10) * 50, mode, 0);
    draw_circle(vgabuf, x1, y1, 15 + (count << 3), 0, 1, 35 + 5 * (count - 10),
                0, 0, mode, 1);
  }
  if ((count > 0) && (count < 11)) {
    draw_light_beam(vgabuf, x1, y1, (x2 - x1) * count / 10 + x1,
                    (y2 - y1) * count / 10 + y1, 25, 0, 127, 127, 0, 127, 127,
                    127, mode, 2);
    draw_circle(vgabuf, x1, y1, 55, 1, 1, 255, 255, 255, mode, 0);
    draw_circle(vgabuf, x1, y1, 35, 0, 1, 255, 0, 0, mode, 1);
  }
  if (count == 10)
    draw_circle(vgabuf, x2, y2, 35, 1, 1, 255, 255, 255, mode, 0);
  if ((count > 3) && (count < 8))
    draw_circle(vgabuf, (x1 - x2) * 5 / 6 + x2, (y1 - y2) * 5 / 6 + y2,
                (count + 1) << 3, 0, 1, 255 - 70 * (count - 4),
                255 - 70 * (count - 4), 10, mode, 1);
  if ((count > 4) && (count < 9))
    draw_circle(vgabuf, (x1 - x2) * 4 / 6 + x2, (y1 - y2) * 4 / 6 + y2,
                (count - 1) << 3, 0, 1, 255 - 70 * (count - 5),
                255 - 70 * (count - 5), 10, mode, 1);
  if ((count > 5) && (count < 10))
    draw_circle(vgabuf, (x1 - x2) * 3 / 6 + x2, (y1 - y2) * 3 / 6 + y2,
                (count - 3) << 3, 0, 1, 255 - 70 * (count - 6),
                255 - 70 * (count - 6), 10, mode, 1);
  if ((count > 7) && (count < 12))
    draw_circle(vgabuf, (x1 - x2) * 2 / 6 + x2, (y1 - y2) * 2 / 6 + y2,
                (count - 6) << 3, 0, 1, 255 - 70 * (count - 8),
                255 - 70 * (count - 8), 10, mode, 1);
  if ((count > 8) && (count < 13))
    draw_circle(vgabuf, (x1 - x2) * 1 / 6 + x2, (y1 - y2) * 1 / 6 + y2,
                (count - 8) << 3, 0, 1, 255 - 70 * (count - 9),
                255 - 70 * (count - 9), 10, mode, 1);
  if ((count > 10) && (count < 21))
    draw_particle(x2 + 5, y2 + 5, x2 - 5, y2 - 5, 0, 0, 1, 10, 0, vgabuf, mode,
                  100, 2, count - 10, 5);
}
...
void Magic::draw_magic_seven(VgaBuf *vgabuf, int x1, int y1, int x2, int y2,
                             int mode, int count) {
  //	IMGline(vgabuf->buf_ptr(), vgabuf->buf_true_pitch(),
  //vgabuf->buf_width(), 						vgabuf->buf_height(), x1-20, y1, x1+20, y1, 0xFFFF);
  ...
  for (int i = 0; i < totalNum; i++) {
    ...
    if ((frame > 3) && (frame < 6)) {
      draw_circle(vgabuf, curX, curY, 15, 1, 1, 255, 255, 255, mode, 0);
      draw_circle(vgabuf, curX, curY, 10, 1, 1, color[col][0], color[col][1],
                  color[col][2], mode, 1);
    }
    if ((frame > 5) && (frame < 8)) {
      draw_circle(vgabuf, curX, curY, 15, 1, 1, color[col][0], color[col][1],
                  color[col][2], mode, 0);
      draw_circle(vgabuf, curX, curY, 15, 1, 1, color[col][0], color[col][1],
                  color[col][2], mode, 1);
      draw_light_beam(vgabuf, curX, curY, curX - 20, curY, 3, 1, 255, 255, 255,
                      255, 255, 255, mode, 2);
      draw_light_beam(vgabuf, curX, curY, curX + 20, curY, 3, 1, 255, 255, 255,
                      255, 255, 255, mode, 2);
    }
    ...
    if ((frame > 7) && (frame < 10)) {
      draw_circle(vgabuf, curX, curY, 40, 1, 1, color[col][0], color[col][1],
                  color[col][2], mode, 0);
      draw_circle(vgabuf, curX, curY, 10, 1, 1, color[col][0], color[col][1],
                  color[col][2], mode, 0);
      draw_light_beam(vgabuf, curX, curY, curX - 20, curY, 3, 1, 255, 255, 255,
    ...
    if ((frame > 9) && (frame < 12)) {
      ...
      draw_circle(vgabuf, curX + 8, curY + 8, 12, 1, 1, color[col][0],
                  color[col][1], color[col][2], mode, 0);
      draw_light_beam(vgabuf, curX, curY, curX - 30, curY, 5, 1, 255, 255, 255,
                      color[col][0], color[col][1], color[col][2], mode, 2);
      draw_light_beam(vgabuf, curX, curY, curX + 30, curY, 5, 1, 255, 255, 255,
                      color[col][0], color[col][1], color[col][2], mode, 2);
      draw_light_beam(vgabuf, curX, curY, curX, curY - 30, 5, 1, 255, 255, 255,
                      color[col][0], color[col][1], color[col][2], mode, 2);
      draw_light_beam(vgabuf, curX, curY, curX, curY + 30, 5, 1, 255, 255, 255,
                      color[col][0], color[col][1], color[col][2], mode, 2);
    }
    ...
    if ((frame > 11) && (frame < 14)) {
      draw_circle(vgabuf, curX, curY, 20, 1, 1, color[col][0], color[col][1],
                  color[col][2], mode, 1);
      draw_circle(vgabuf, curX, curY, 30, 1, 1, 255, 255, 255, mode, 0);
      //	draw_circle(vgabuf, curX+8, curY+8, 12, 1, 1, color[col][0],
      //color[col][1], color[col][2], mode, 0);
      draw_circle(vgabuf, curX + 8, curY + 8, 12, 1, 1, 255, 255, 255, mode, 0);
      draw_light_beam(vgabuf, curX, curY, curX - 40, curY, 5, 0, 255, 255, 255,
                      color[col][0], color[col][1], color[col][2], mode, 2);
      draw_light_beam(vgabuf, curX, curY, curX + 40, curY, 5, 0, 255, 255, 255,
                      color[col][0], color[col][1], color[col][2], mode, 2);
      draw_light_beam(vgabuf, curX, curY, curX, curY - 40, 5, 0, 255, 255, 255,
                      color[col][0], color[col][1], color[col][2], mode, 2);
      draw_light_beam(vgabuf, curX, curY, curX, curY + 40, 5, 0, 255, 255, 255,
                      color[col][0], color[col][1], color[col][2], mode, 2);
    }
...
void Magic::draw_magic_eight(VgaBuf *vgabuf, int x1, int y1, int x2, int y2,
                             int mode, int count) {
  //	draw_circle2(vgabuf, x1-100, y1,		 100, 100, 255, 055, 055, mode,
  //1); 	draw_circle2(vgabuf, x1-100, y1+200, 100, 100, 255, 055, 055, mode, 1);
  ...
  draw_circle2(vgabuf, x1, y1, 80, config.frame_speed / 3, 0, 255, 0, 0, mode,
               1);
  draw_circle2(vgabuf, x1, y1, 80, 0, config.frame_speed / 3, 0, 255, 0, mode,
               1);
  draw_circle2(vgabuf, x1, y1, 80, 0, 0, 0, 0, 255, mode, 1);

  draw_circle2(vgabuf, x1, y1 + 300, 80, config.frame_speed / 3, 0, 255, 0, 0,
               mode, 1);
  draw_circle2(vgabuf, x1, y1 + 300, 80, 0, config.frame_speed / 3, 0, 255, 0,
               mode, 1);
  draw_circle2(vgabuf, x1, y1 + 300, 80, 0, 0, 0, 0, 255, mode, 1);
}

// ----------- Begin of function Magic::draw_magic_nine -----//
void Magic::draw_magic_nine(VgaBuf *vgabuf, int x1, int y1, int x2, int y2,
                            int mode, int curStep, int totalStep) {
  // shift all coordinates to the ZOOM display
  x1 = x1 + bound_x1;
  y1 = y1 + bound_y1;
  x2 = x2 + bound_x1;
  ...
  if (count == 3) {
    draw_lighting(x1, y1, x2, y2, 4, 1, 30, 30, 00, 180, 180, 180, vgabuf, 27,
                  mode, 12, 3, 2);
    draw_circle(vgabuf, x2, y2, 10, 1, 1, 255, 255, 255, mode, 0);
  }
  if ((count > 3) && (count < 6)) {
    draw_lighting(x1, y1, x2, y2, 4, 1, 127, 127, 00, 255, 255, 255, vgabuf,
                  (count + 3) * 27, mode, 12, 3, 2);
    draw_circle(vgabuf, x2, y2, 20, 1, 1, 255, 255, 255, mode, 0);
  }
  if ((count > 0) && (count < 4)) {
    draw_light_beam(vgabuf, x1, y1, x1, y1 - count * 10, 5, 2, 127, 127, 0, 255,
                    255, 255, vga.pixel_format_flag, 02);
    draw_light_beam(vgabuf, x1, y1, x1 + count * 10, y1, 5, 2, 127, 127, 0, 255,
                    255, 255, vga.pixel_format_flag, 02);
    draw_light_beam(vgabuf, x1, y1, x1, y1 + count * 10, 5, 2, 127, 127, 0, 255,
                    255, 255, vga.pixel_format_flag, 02);
    draw_light_beam(vgabuf, x1, y1, x1 - count * 10, y1, 5, 2, 127, 127, 0, 255,
...
void Magic::draw_magic_nine_die(VgaBuf *vgabuf, int x1, int y1, int mode,
                                int count) {
  x1 = x1 + bound_x1;
  y1 = y1 + bound_y1;
...
void Magic::draw_magic_ten(VgaBuf *vgabuf, int x1, int y1, int x2, int y2,
                           int mode, int curStep, int totalStep, int delay) {
  // shift all coordinates to the ZOOM display
  x1 = x1 + bound_x1;
  y1 = y1 + bound_y1;
  x2 = x2 + bound_x1;
  ...
  if ((count > 0) && (count <= delay))
    draw_particle(x1 - 5, y1 - 120, x1 + 5, y1 - 110, 0, 0, 0, 6, 0, vgabuf,
                  mode, 100, 2, 9 - curStep, 20);

  if ((count == 6) || (count == 7))
    draw_circle(vgabuf, x1, y1 - 405, 40, 1, 1, 255, 255, 255, mode, 0);

  if ((count > 5) && (count < 16))
    draw_circle(vgabuf, x1, y1 - 405, (count - 4) << 3, 1, 1,
                255 - 25 * (count - 6), 255 - 25 * (count - 6), 10, mode, 1);

  if (count == 6)
    draw_lighting(x1, y1 - 110, x1 + 5, y1 - 400, 8, 2, 30, 30, 00, 180, 180,
                  180, vgabuf, 27, mode, 10, 3, 1);

  if ((count > 6) && (count < 9)) {
    draw_lighting(x1, y1 - 110, x1 + 5, y1 - 400, 8, 2, 127, 127, 00, 255, 255,
                  255, vgabuf, (count - 6) * 27, mode, 10, 3, 1);
    draw_circle(vgabuf, x1, y1 - 405, 40, 1, 1, 255, 255, 255, mode, 0);
  }
  if (count == 9) {
    draw_lighting(x1, y1 - 110, x1 - 5, y1 - 400, 8, 2, 30, 30, 00, 180, 180,
                  180, vgabuf, 117, mode, 10, 3, 1);
    draw_circle(vgabuf, x1, y1 - 405, 30, 1, 1, 255, 255, 255, mode, 0);
  }
  if ((count > 9) && (count < 12)) {
    draw_lighting(x1, y1 - 110, x1 - 5, y1 - 400, 8, 2, 127, 127, 00, 255, 255,
                  255, vgabuf, (count - 9) * 117, mode, 10, 3, 1);
    draw_circle(vgabuf, x1, y1 - 405, 40, 1, 1, 255, 255, 255, mode, 0);
  }

  if ((count > 1) && (count < 5))
    draw_circle(vgabuf, x1, y1 - 115, (10 * (count)), 1, 1, 255, 255, 255, mode,
                0);
  ...
  if ((count > 6) && (count < 13)) {
    draw_light_beam(&vga_back, x1, y1 - 405, x1, y1 - 45 - 405, 10, 4, 127, 127,
                    0, 255, 255, 255, vga.pixel_format_flag, 02);
    draw_light_beam(&vga_back, x1, y1 - 405, x1 + 45, y1 - 405, 10, 4, 127, 127,
                    0, 255, 255, 255, vga.pixel_format_flag, 02);
    draw_light_beam(&vga_back, x1, y1 - 405, x1, y1 + 45 - 405, 10, 4, 127, 127,
                    0, 255, 255, 255, vga.pixel_format_flag, 02);
    draw_light_beam(&vga_back, x1, y1 - 405, x1 - 45, y1 - 405, 10, 4, 127, 127,
  ...
  if ((count > 11) && (count < 16)) {
    draw_light_beam(vgabuf, x1, y1 - 400, x2, y2, 25, 0, 127, 127, 0, 127, 127,
                    127, mode, 2, 0);
    draw_circle(vgabuf, x2, y2, 53 - 10 * (count - 11), 1, 1, 255, 0, 255, mode,
                1);
    draw_circle(vgabuf, x2, y2, 40 - 10 * (count - 11), 1, 1, 255, 255, 255,
                mode, 0);
    draw_lighting(x2 - 50 + 10 * (count - 11), y2, x2 - 10 + 2 * (count - 11),
                  y2, 4, 1, 127, 0, 127, 255, 255, 255, vgabuf, count, mode, 5,
                  2, 0);
    draw_circle(vgabuf, x2 - 50 + 10 * (count - 11), y2, 10 - 2 * (count - 11),
                1, 1, 255, 255, 255, mode, 0);
    draw_lighting(x2 + 50 - 10 * (count - 11), y2, x2 + 10 - 2 * (count - 11),
                  y2, 4, 1, 127, 0, 127, 255, 255, 255, vgabuf, count, mode, 5,
                  2, 0);
    draw_circle(vgabuf, x2 + 50 - 10 * (count - 11), y2, 10 - 2 * (count - 11),
                1, 1, 255, 255, 255, mode, 0);
    draw_lighting(x2, y2 - 50 + 10 * (count - 11), x2,
                  y2 - 10 + 2 * (count - 11), 4, 1, 127, 0, 127, 255, 255, 255,
                  vgabuf, count, mode, 5, 2, 0);
    draw_circle(vgabuf, x2, y2 - 50 + 10 * (count - 11), 10 - 2 * (count - 11),
                1, 1, 255, 255, 255, mode, 0);
    draw_lighting(x2, y2 + 50 - 10 * (count - 11), x2,
                  y2 + 10 - 2 * (count - 11), 4, 1, 127, 0, 127, 255, 255, 255,
                  vgabuf, count, mode, 5, 2, 0);
...
void Magic::draw_magic_eleven(VgaBuf *vgabuf, int x1, int y1, int mode,
                              int curStep) {
  x1 = x1 + bound_x1;
  y1 = y1 + bound_y1;
  ...
  draw_circle2(vgabuf, x1, y1, 40 * curStep, 1, 0, (curStep << 4) >> intensity,
               255 >> intensity, (255 - (curStep << 4)) >> intensity, mode, 1);
}
// ----------- End of function Magic::draw_magic_eleven -----//

// ----------- Begin of function Magic::draw_magic_twelve -----//
void Magic::draw_magic_twelve(VgaBuf *vgabuf, int x1, int y1, int mode,
                              int curStep, int nationRecno) {
  x1 = x1 + bound_x1;
  y1 = y1 + bound_y1;
  ...
  if ((curStep > 4) && (curStep < 17)) {
    ...
    if (locx1 <= bound_x2 && locx2 >= bound_x1 && locy1 <= bound_y2 &&
        locy2 >= bound_y1) {
      ...
      vgabuf->put_bitmap_area_trans_remap_decompress(
          locx1, locy1, (char *)bitmapPtr, srcX1, srcY1, srcX2, srcY2,
          colorRemapTable);
    }
  }
  ...
  if ((curStep > 1) && (curStep < 3)) {
    draw_circle(vgabuf, x1, y1, 45, 1, 1, 255, 255, 255, mode, 0);
    draw_circle(vgabuf, x1, y1, 30, 1, 1, color[col][0], color[col][1],
                color[col][2], mode, 1);
  }
  if ((curStep > 2) && (curStep < 4)) {
    draw_circle(vgabuf, x1, y1, 45, 1, 1, color[col][0], color[col][1],
                color[col][2], mode, 0);
    draw_circle(vgabuf, x1, y1, 45, 1, 1, color[col][0], color[col][1],
                color[col][2], mode, 1);
    draw_light_beam(vgabuf, x1, y1, x1 - 60, y1, 5, 1, 255, 255, 255, 255, 255,
                    255, mode, 2);
    draw_light_beam(vgabuf, x1, y1, x1 + 60, y1, 5, 1, 255, 255, 255, 255, 255,
                    255, mode, 2);
  }
  ...
  if ((curStep > 3) && (curStep < 5)) {
    draw_circle(vgabuf, x1, y1, 120, 1, 1, color[col][0], color[col][1],
                color[col][2], mode, 0);
    draw_circle(vgabuf, x1, y1, 30, 1, 1, color[col][0], color[col][1],
                color[col][2], mode, 0);
    draw_light_beam(vgabuf, x1, y1, x1 - 60, y1, 5, 1, 255, 255, 255, 255, 255,
  ...
  if ((curStep > 4) && (curStep < 7)) {
    draw_circle(vgabuf, x1, y1, 60, 1, 1, color[col][0], color[col][1],
                color[col][2], mode, 1);
    draw_circle(vgabuf, x1, y1, 120, 1, 1, color[col][0], color[col][1],
                color[col][2], mode, 0);
    draw_circle(vgabuf, x1 + 24, y1 + 24, 36, 1, 1, color[col][0],
                color[col][1], color[col][2], mode, 0);
    draw_light_beam(vgabuf, x1, y1, x1 - 90, y1, 8, 1, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
    draw_light_beam(vgabuf, x1, y1, x1 + 90, y1, 8, 1, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
    draw_light_beam(vgabuf, x1, y1, x1, y1 - 90, 8, 1, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
    draw_light_beam(vgabuf, x1, y1, x1, y1 + 90, 8, 1, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
  }
  ...
  if ((curStep > 6) && (curStep < 9)) {
    ...
    draw_light_beam(vgabuf, x1, y1, x1 - 120, y1, 8, 0, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
    draw_light_beam(vgabuf, x1, y1, x1 + 120, y1, 8, 0, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
    draw_light_beam(vgabuf, x1, y1, x1, y1 - 120, 8, 0, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
    draw_light_beam(vgabuf, x1, y1, x1, y1 + 120, 8, 0, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
  }
  ...
  if ((curStep > 8) && (curStep < 11)) {
    draw_circle(vgabuf, x1, y1, 70, 1, 1, color[col][0], color[col][1],
                color[col][2], mode, 1);
    draw_circle(vgabuf, x1, y1, 60, 1, 1, 255, 255, 255, mode, 0);
    draw_circle(vgabuf, x1 + 16, y1 + 16, 24, 1, 1, 255, 255, 255, mode, 0);
    draw_light_beam(vgabuf, x1, y1, x1 - 80, y1, 5, 0, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
    draw_light_beam(vgabuf, x1, y1, x1 + 80, y1, 5, 0, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
    draw_light_beam(vgabuf, x1, y1, x1, y1 - 80, 5, 0, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
    draw_light_beam(vgabuf, x1, y1, x1, y1 + 80, 5, 0, 255, 255, 255,
                    color[col][0], color[col][1], color[col][2], mode, 2);
  }
  ...
  if ((curStep > 10) && (curStep < 17)) {
    draw_circle(vgabuf, x1, y1, 170 - curStep * 10, 1, 1, color[col][0],
                color[col][1], color[col][2], mode, 1);
    draw_light_beam(vgabuf, x1, y1, x1 - (curStep * 20 - 180), y1, 3, 1, 255,
                    255, 255, 255, 255, 255, mode, 2);
    draw_light_beam(vgabuf, x1, y1, x1 + (curStep * 20 - 180), y1, 3, 1, 255,
                    255, 255, 255, 255, 255, mode, 2);
  }

  //	draw_circle2(vgabuf, x1, y1, 40 * curStep, 0, 0,
  //(curStep<<4)>>intensity, 255>>intensity, 		(255-(curStep<<4))>>intensity,
  //mode, 1);
}
// ----------- End of function Magic::draw_magic_twelve -----//

// ----------- Begin of function Magic::draw_magic_twelve_die -----//
void Magic::draw_magic_twelve_die(VgaBuf *vgabuf, int x1, int y1, int mode,
                                  int curStep, int nationRecno, int random) {
  x1 = x1 + bound_x1;
  y1 = y1 + bound_y1;
  ...
  if ((curStep > 0) && (curStep < 6))
    draw_circle2(vgabuf, x1, y1, (5 - curStep) * 35, 1, 0, 50 * curStep,
                 50 * curStep, 50 * curStep, mode, 1);
  ...
  if ((curStep > 7) && (curStep < 17))
    draw_circle2(vgabuf, x1, y1, 40 * (curStep - 7), 1, 0,
                 (255 - (curStep << 4)) >> intensity, 255 >> intensity,
                 (curStep << 4) >> intensity, mode, 1);
}
// ----------- End of function Magic::draw_magic_twelve_die -----//

// ----------- Begin of function Magic::draw_magic_thirteen -----//
void Magic::draw_magic_thirteen(VgaBuf *vgabuf, int x1, int y1, int mode,
                                int curStep) {
  x1 = x1 + bound_x1;
  y1 = y1 + bound_y1;

  int locX[15] = {240, 300, 240, 0, -240, -300, -240, 0,
                  240, 300, 240, 0, -240, -300, -240};
  int locY[15] = {140, 0, -160, -200, -160, 0, 140, 200,
                  140, 0, -160, -200, -160, 0, 140};
  int step = curStep;
  int intensity = (curStep >> 2);

  if ((curStep > 0) && (curStep < 10))
    draw_circle2(vgabuf, x1, y1, 40 * step, 1, 0,
                 (255 - 25 * step) >> intensity, (255 - 25 * step) >> intensity,
                 255 >> intensity, mode, 1);
  ...
  draw_particle(x1 + 10, y1 + 10, x1 - 10, y1 - 10, 0, 0, 2, 10, 10, vgabuf,
                mode, 50, 2, curStep, 10, 4);
  draw_particle(x1 + 10, y1 + 10, x1 - 10, y1 - 10, 0, 0, 4, 10, 0, vgabuf,
                mode, 50, 2, curStep, 10, 4);

  draw_particle(x1 + 10, y1 + 10, x1 - 10, y1 - 10, 0, 0, 0, 7, 10, vgabuf,
                mode, 50, 2, curStep, 10, 4);

  if ((curStep >= 0) && (curStep < 15)) {
    if ((curStep % 2) == 0) {
      draw_light_beam(vgabuf, x1, y1, x1 + locX[curStep >> 1],
                      y1 + locY[curStep >> 1], 10, 2, 0, 80, 255, 255, 255, 255,
                      mode, 0);
      draw_circle2(vgabuf, x1 + locX[curStep >> 1], y1 + locY[curStep >> 1], 30,
                   0, 0, 0, 80, 255, mode, 0);
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if ((curStep >= j) && (curStep <= j + 5)) {
      draw_particle(x1 + locX[j >> 1] - 5, y1 + locY[j >> 1] - 5,
                    x1 + locX[j >> 1] + 5, y1 + locY[j >> 1] + 5, 0, 0, 1, 10,
                    0, vgabuf, mode, 50, 2, curStep - j, 6);
    }
  }
}
...
void Magic::draw_magic_forteen(VgaBuf *vgabuf, int x1, int y1, int mode,
                               int curStep, int curDir) {
  x1 = x1 + bound_x1;
  y1 = y1 + bound_y1 - 20;

  draw_circle2(vgabuf, x1, y1, 60, 0, 1, 255 >> (curStep >> 1),
               255 >> (curStep >> 1), 255 >> (curStep >> 1), mode, 0);
  y1 += 32;
  switch (curDir) {
  case 0:
    draw_light_beam(vgabuf, x1 - 10, y1 - (curStep << 3), x1 + 10,
                    y1 + 5 - (curStep << 3), 10, 0, 0, 80, 255, 255, 255, 255,
                    mode, 0);
    break;
  case 1:
    draw_light_beam(vgabuf, x1 + 7, y1 - 8 - (curStep << 3), x1 + 13,
                    y1 - 8 - (curStep << 3), 10, 0, 0, 80, 255, 255, 255, 255,
                    mode, 0);
    break;
  case 2:
    draw_light_beam(vgabuf, x1 - 10, y1 + 5 - 16 - (curStep << 3), x1 + 10,
                    y1 - 16 - (curStep << 3), 10, 0, 0, 80, 255, 255, 255, 255,
                    mode, 0);
    break;
  case 3:
    draw_light_beam(vgabuf, x1 - 10, y1 - 16 - (curStep << 3), x1 + 10,
                    y1 - 16 - (curStep << 3), 10, 0, 0, 80, 255, 255, 255, 255,
                    mode, 0);
    break;
  case 4:
    draw_light_beam(vgabuf, x1 - 10, y1 - 16 - (curStep << 3), x1 + 10,
                    y1 + 5 - 16 - (curStep << 3), 10, 0, 0, 80, 255, 255, 255,
                    255, mode, 0);
    break;
  case 5:
    draw_light_beam(vgabuf, x1 - 7, y1 - 8 - (curStep << 3), x1 - 13,
    ...
    break;
  case 6:
    draw_light_beam(vgabuf, x1 - 10, y1 + 5 - (curStep << 3), x1 + 10,
                    y1 - (curStep << 3), 10, 0, 0, 80, 255, 255, 255, 255, mode,
                    0);
    break;
  case 7:
    draw_light_beam(vgabuf, x1 - 10, y1 - (curStep << 3), x1 + 10,
...
void Magic::draw_magic_fifteen(VgaBuf *vgabuf, int x1, int y1, int mode,
                               int curStep) {
  x1 = x1 + bound_x1;
  y1 = y1 + bound_y1;
  draw_circle(vgabuf, x1, y1, curStep << 6, 0, 1, 0, 0, 510 >> curStep, mode,
              1);
}
// ----------- End of function Magic::draw_magic_fifteen -----//

// ----------- Begin of function Magic::draw_magic_firm_die -----//
void Magic::draw_magic_firm_die(VgaBuf *vgabuf, int x1, int y1, int mode,
                                int curStep) {
  x1 = x1 + bound_x1;
  y1 = y1 + bound_y1;

  draw_particle(x1 + 10, y1 + 10, x1 - 10, y1 - 10, 0, 0, 0, 10, 0, vgabuf,
                mode, 50, 2, curStep, 10, 4);
  draw_particle(x1 + 10, y1 + 10, x1 - 10, y1 - 10, 0, 0, 4, 10, 0, vgabuf,
                mode, 50, 2, curStep, 10, 4);
}
