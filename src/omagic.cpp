...
void Magic::straight_light_beam(VgaBuf *vgabuf, int x1, int y1, int x2, int y2, int thickness, int lightness, short R1,
                                short G1, short B1, short R2, short G2, short B2, int mode, char angle)
{
...
void Magic::straight_light_beam2(VgaBuf *vgabuf, int x1, int y1, int x2, int y2, int thickness, int lightness, short R1,
                                 short G1, short B1, short R2, short G2, short B2, int mode, char angle)
{
...
void Magic::draw_light_beam(VgaBuf *vgabuf, int x1, int y1, int x2, int y2, int thickness, int lightness, short R1,
                            short G1, short B1, short R2, short G2, short B2, int mode, char angle, char fast)
{
    int dx = abs(x2 - x1);
    int dy = abs(y2 - y1);
    int b_x1 = bound_x1;
    ...
    if ((x1 < b_x1 && x2 < b_x1) || (x1 > b_x2 && x2 > b_x2) || (y1 < b_y1 && y2 < b_y1) || (y1 > b_y2 && y2 > b_y2))
        return;
    if (x1 >= b_x1 && x1 <= b_x2 && x2 >= b_x1 && x2 <= b_x2 && y1 >= b_y1 && y1 <= b_y2 && y2 >= b_y1 && y2 <= b_y2)
                              ...
                              int number, int birth_time, int lifecount, int lifeLimit, int lifeOffset)
{
    ...
    else if (type < 4)
    {
        for (i = 0; i < number; i++)
        {
            ...
            if (diff > 0)
            {
                temp_ptr->pos_x = temp_x + temp_ptr->dir_x * lifeLimit + (temp_ptr->dir_x >> 2) * diff;
                diff = abs(temp_ptr->dir_y) * diff * diff;
                if (temp_ptr->dir_y <= 0)
                    temp_ptr->pos_y = temp_y + temp_ptr->dir_y * lifeLimit + diff / lifeLimit;
                else
                    temp_ptr->pos_y = temp_y + temp_ptr->dir_y * lifeLimit + (diff >> 1) / lifeLimit;
            }
            else
            {
    ...
    else if (type == 4)
    {
        for (i = 0; i < number; i++)
        {
            ...
            if (mode == 0)
                temp_ptr->color =
                    0xFFFF >> (5 + (temp_ptr->life + lifeOffset) * 10 / 20); //	temp_ptr->prev_color = 0xFFFF;
                                                                             // temp_ptr->delta_color = 25 +temp_r1/4;
            else if (mode == 1)
                temp_ptr->color =
                    0xFFFF >> (5 + (temp_ptr->life + lifeOffset) * 11 / 20); //	temp_ptr->prev_color = 0xFFFF;
                                                                             // temp_ptr->delta_color = 25 +temp_r1/4;
            else if (mode == 2)
                temp_ptr->color =
                    0xFFFF << (5 + (temp_ptr->life + lifeOffset) * 10 / 20); //	temp_ptr->prev_color = 0xFFFF;
...
void Magic::draw_particle(int x1, int y1, int x2, int y2, int dirx, int diry, int type, int var, int size,
                          VgaBuf *vgabuf, int mode, int number, int birth_time, int lifecount, int life_limit,
                          int lifeOffset)
{
    Particle *particle;
    int i;
    ...
    generate_particle(x1, y1, x2, y2, dirx, diry, type, var, mode, number, birth_time, lifecount, life_limit,
                      lifeOffset);
    ...
    else // direction line
    {
        ...
        {
            particle = h_particle + i;
            if ((particle->life <= life_limit) && (particle->life >= 0) && (particle->pos_x <= bound_x2) &&
                (particle->pos_x >= bound_x1) && (particle->prev_x <= bound_x2) && (particle->prev_x >= bound_x1) &&
                (particle->pos_y <= bound_y2) && (particle->pos_y >= bound_y1) && (particle->prev_y <= bound_y2) &&
                (particle->prev_y >= bound_y1))
                IMGline(vgabuf->buf_ptr(), vgabuf->buf_true_pitch(), vgabuf->buf_width(), vgabuf->buf_height(),
                        particle->prev_x, particle->prev_y, particle->pos_x, particle->pos_y, particle->color);
        }
    }
    mem_del(h_particle);
    h_particle = NULL;
...
void Magic::draw_circle(VgaBuf *vgabuf, int x1, int y1, int radius, char r_x, char r_y, short R1, short G1, short B1,
                        int mode, int dir)
{
...
// no return values
//
void Magic::draw_circle2(VgaBuf *vgabuf, int x1, int y1, int radius, char r_x, char r_y, short R1, short G1, short B1,
                         int mode, int dir)
{
    int i, j, line = 2 * vgabuf->buf_pitch();
    int mag, temp_mag, up_level;
                                       ...
                                       Lighting_Particle *h_l_particle)
{
    ...
    for (i = 0; i < (number - 1); i++)
    {
        ...
        if (temp_r1 < limit)
        {
            ...
            temp_ptr->pos_x =
                temp_ptr->prev_x + (temp_rx >> 3) + temp_ptr->dir_x * lifecount; // assigning current_position
            temp_ptr->pos_y =
                temp_ptr->prev_y + (temp_ry >> 3) + temp_ptr->dir_y * lifecount; // temp_r1 *(y2-y1) /(number *8) +
                          ...
                          int level)
{
    ...
    for (i = 0; i < number; i++)
    {
        ...
        draw_light_beam(vgabuf, particle->prev_x, particle->prev_y, particle->pos_x, particle->pos_y, thickness,
                        lightness, R1, G1, B1, R2, G2, B2, mode, 00);
    ...
    if (level > 0)
    {
        for (i = 0; i < number; i++)
        {
            ...
            draw_lighting(particle->prev_x, particle->prev_y, particle1->pos_x, particle1->pos_y, thickness, lightness,
                          R1, G1, B1, R2, G2, B2, vgabuf, level, mode, temp, lifecount, temp1);
        }
    }
...
void Magic::draw_magic_one(VgaBuf *vgabuf, int x1, int y1, int mode, int count)
{
    if (count == 6)
    {
        draw_light_beam(vgabuf, x1 - 100, y1 - 200, x1, y1, 10, 2, 255, 80, 0, 255, 255, 255, mode, 0);
        draw_light_beam(vgabuf, x1 + 100, y1 - 200, x1, y1, 10, 2, 255, 80, 0, 255, 255, 255, mode, 0);
        draw_light_beam(vgabuf, x1 - 150, y1 + 100, x1, y1, 10, 2, 255, 80, 0, 255, 255, 255, mode, 0);
        draw_light_beam(vgabuf, x1 + 150, y1 + 100, x1, y1, 10, 2, 255, 80, 0, 255, 255, 255, mode, 0);
    ...
    if ((count > 5) && (count < 16))
        draw_circle(vgabuf, x1, y1, (count - 4) << 3, 0, 1, 255 - 25 * (count - 6), 255 - 25 * (count - 6), 10, mode,
                    1);
    if ((count > 0) && (count < 6))
    {
        draw_circle(vgabuf, x1 - 100, y1 - 200, 17, 1, 1, 255, 200, 0, mode, 1);
...
void Magic::draw_magic_two(VgaBuf *vgabuf, int x1, int y1, int x2, int y2, int diff, int mode, int curStep,
                           int totalStep)
{
    ...
    if (count == 3)
    {
        draw_lighting(x1, y1, x2 + diff, y2 + diff, 8, 2, 30, 30, 00, 180, 180, 180, vgabuf, 27, mode, 10, 3, 1);
        draw_circle(vgabuf, x2 + diff, y2 + diff, 30, 1, 1, 255, 255, 255, mode, 0);
    }
    if ((count > 3) && (count < 6))
    {
        draw_lighting(x1, y1, x2 + diff, y2 + diff, 8, 2, 127, 127, 00, 255, 255, 255, vgabuf, (count - 3) * 27, mode,
                      10, 3, 1);
        draw_circle(vgabuf, x2 + diff, y2 + diff, 40, 1, 1, 255, 255, 255, mode, 0);
    }
    if (count == 6)
    {
        draw_lighting(x1, y1, x2 - diff, y2 - diff, 8, 2, 30, 30, 00, 180, 180, 180, vgabuf, 117, mode, 10, 3, 1);
        draw_circle(vgabuf, x2 - diff, y2 - diff, 30, 1, 1, 255, 255, 255, mode, 0);
    }
    if ((count > 6) && (count < 9))
    {
        draw_lighting(x1, y1, x2 - diff, y2 - diff, 8, 2, 127, 127, 00, 255, 255, 255, vgabuf, (count - 6) * 117, mode,
                      10, 3, 1);
        draw_circle(vgabuf, x2 - diff, y2 - diff, 40, 1, 1, 255, 255, 255, mode, 0);
    }
    if ((count > 0) && (count < 4))
    {
        ...
        //						10, 4, 127, 127, 0, 255, 255, 255,
        // vga.pixel_format_flag,
        // 02); 		draw_light_beam(&vga_back, x1, y1, x1+count*15, y1, 10, 4, 127,
        // 127, 0, 255, 255, 255, vga.pixel_format_flag, 02);
        // draw_light_beam(&vga_back, x1, y1, x1,
        // y1+count*15, 						10, 4, 127, 127, 0, 255, 255,
        // 255,
        // vga.pixel_format_flag, 02); 		draw_light_beam(&vga_back, x1,
        // y1,
        // x1-count*15, y1, 						10, 4, 127, 127, 0, 255, 255,
        // 255, vga.pixel_format_flag, 02);
    ...
    if ((count > 3) && (count < 10))
    {
        ...
        //						10, 4, 127, 127, 0, 255, 255, 255,
        // vga.pixel_format_flag,
        // 02); 		draw_light_beam(&vga_back, x1, y1, x1+45, y1,
        // 10, 4, 127, 127, 0, 255, 255, 255, vga.pixel_format_flag, 02);
        // draw_light_beam(&vga_back, x1, y1, x1, y1+45, 10, 4, 127, 127, 0, 255,
        // 255, 255, vga.pixel_format_flag, 02); 		draw_light_beam(&vga_back,
        // x1,
        // y1, x1-45, y1, 						10, 4, 127, 127, 0, 255, 255,
        // 255, vga.pixel_format_flag, 02);
        draw_circle(vgabuf, x1, y1, 50, 1, 1, 255, 255, 255, mode, 0);
        draw_circle(vgabuf, x1, y1 - 35, 50, 1, 1, 255, 255, 255, mode, 1);
    }
    if ((count > 3) && (count < 18))
        draw_particle(x2 + diff + 10, y2 + diff + 10, x2 + diff - 10, y2 + diff - 10, 0, 0, 1, 10, 0, vgabuf, mode, 100,
                      2, count - 3, 6);
    if ((count > 6) && (count < 21))
        draw_particle(x2 - diff + 10, y2 - diff + 10, x2 - diff - 10, y2 - diff - 10, 0, 0, 1, 10, 0, vgabuf, mode, 100,
                      2, count - 6, 6);
...
void Magic::draw_magic_three(VgaBuf *vgabuf, int x1, int y1, int x2, int y2, int mode, int count)
{
    ...
    if ((count > 0) && (count < 7))
    {
        draw_circle(vgabuf, (x1 - x2) / 3 + x2, (y1 - y2) / 3 + y2, (count + 1) << 3, 0, 1, 255 - 50 * (count - 1),
                    255 - 50 * (count - 1), 10, mode, 1);
        draw_circle(vgabuf, (x1 - x2) * 2 / 3 + x2, (y1 - y2) * 2 / 3 + y2, (count + 2) << 3, 0, 1,
                    255 - 50 * (count - 1), 255 - 50 * (count - 1), 10, mode, 1);
    }
    ...
    if ((count > 5) && (count < 12))
    {
        draw_circle(vgabuf, (x1 - x2) / 3 + x2, (y1 - y2) / 3 + y2, (count - 4) << 3, 0, 1, 255 - 50 * (count - 6),
                    255 - 50 * (count - 6), 10, mode, 1);
        draw_circle(vgabuf, (x1 - x2) * 2 / 3 + x2, (y1 - y2) * 2 / 3 + y2, (count - 3) << 3, 0, 1,
                    255 - 50 * (count - 6), 255 - 50 * (count - 6), 10, mode, 1);
    }
    x1 = x1 + 200;
    ...
    if ((count > 10) && (count < 17))
    {
        draw_circle(vgabuf, (x1 - x2) / 3 + x2, (y1 - y2) / 3 + y2, (count - 9) << 3, 0, 1, 255 - 50 * (count - 11),
                    255 - 50 * (count - 11), 10, mode, 1);
        draw_circle(vgabuf, (x1 - x2) * 2 / 3 + x2, (y1 - y2) * 2 / 3 + y2, (count - 8) << 3, 0, 1,
                    255 - 50 * (count - 11), 255 - 50 * (count - 11), 10, mode, 1);
    }
    if ((count > 0) && (count < 21))
...
void Magic::draw_magic_four(VgaBuf *vgabuf, int x1, int y1, int x2, int y2, int mode, int count)
{
    ...
    if ((count > 0) && (count < 3))
    {
        ...
        draw_circle(vgabuf, x1, y1, 40, 1, 1, 255, 255, 255, mode, 0);
        draw_lighting(x1 - 50, y1, x1 - 10, y1, 4, 1, 127, 0, 127, 255, 255, 255, vgabuf, count, mode, 5, 2, 0);
        draw_circle(vgabuf, x1 - 50, y1, 10, 1, 1, 255, 255, 255, mode, 0);
        draw_lighting(x1 + 50, y1, x1 + 10, y1, 4, 1, 127, 0, 127, 255, 255, 255, vgabuf, count, mode, 5, 2, 0);
        draw_circle(vgabuf, x1 + 50, y1, 10, 1, 1, 255, 255, 255, mode, 0);
        draw_lighting(x1, y1 - 50, x1, y1 - 10, 4, 1, 127, 0, 127, 255, 255, 255, vgabuf, count, mode, 5, 2, 0);
    ...
    if ((count > 2) && (count < 10))
    {
        draw_lighting(x1 - 33, y1, x1 - 10, y1, 4, 1, 127, 0, 127, 255, 255, 255, vgabuf, count, mode, 5, 2, 0);
    ...
    if ((count > 9) && (count < 21))
        draw_circle(vgabuf, x2, y2, (count - 8) << 3, 0, 1, 255 - 22 * (count - 10), 10, 255 - 22 * (count - 10), mode,
                    1);
    if ((count > 9) && (count < 21))
        draw_particle(x2 + 5, y2 + 5, x2 - 5, y2 - 5, -step_x >> 1, -step_y >> 1, 1, -avg, 0, vgabuf, mode, 100, 2,
                      count - 9, 5);
}
...
void Magic::draw_magic_five(VgaBuf *vgabuf, int x1, int y1, int x2, int y2, int mode, int count)
{
    ...
    draw_particle(x2 + 8, y2 + 8, x2 - 10, y2 - 10, step_x >> 1, step_y >> 1, 2, avg, 40 - (count << 1), vgabuf, mode,
                  15, 2, count - 1, 8);
    draw_particle(x2 + 10, y2 + 10, x2 - 8, y2 - 8, step_x >> 1, step_y >> 1, 3, avg, 20, vgabuf, mode, 5, 2, count - 1,
                  8);
}
...
void Magic::draw_magic_six(VgaBuf *vgabuf, int x1, int y1, int x2, int y2, int mode, int count)
{
    ...
    {
        draw_circle(vgabuf, x1, y1, 25 + (count << 3), 1, 1, 55 + 5 * (count - 10), 255 - (count - 10) * 50,
                    255 - (count - 10) * 50, mode, 0);
        draw_circle(vgabuf, x1, y1, 15 + (count << 3), 0, 1, 35 + 5 * (count - 10), 0, 0, mode, 1);
    }
    if ((count > 0) && (count < 11))
    {
        draw_light_beam(vgabuf, x1, y1, (x2 - x1) * count / 10 + x1, (y2 - y1) * count / 10 + y1, 25, 0, 127, 127, 0,
                        127, 127, 127, mode, 2);
        draw_circle(vgabuf, x1, y1, 55, 1, 1, 255, 255, 255, mode, 0);
        draw_circle(vgabuf, x1, y1, 35, 0, 1, 255, 0, 0, mode, 1);
    }
    if (count == 10)
        draw_circle(vgabuf, x2, y2, 35, 1, 1, 255, 255, 255, mode, 0);
    if ((count > 3) && (count < 8))
        draw_circle(vgabuf, (x1 - x2) * 5 / 6 + x2, (y1 - y2) * 5 / 6 + y2, (count + 1) << 3, 0, 1,
                    255 - 70 * (count - 4), 255 - 70 * (count - 4), 10, mode, 1);
    if ((count > 4) && (count < 9))
        draw_circle(vgabuf, (x1 - x2) * 4 / 6 + x2, (y1 - y2) * 4 / 6 + y2, (count - 1) << 3, 0, 1,
                    255 - 70 * (count - 5), 255 - 70 * (count - 5), 10, mode, 1);
    if ((count > 5) && (count < 10))
        draw_circle(vgabuf, (x1 - x2) * 3 / 6 + x2, (y1 - y2) * 3 / 6 + y2, (count - 3) << 3, 0, 1,
                    255 - 70 * (count - 6), 255 - 70 * (count - 6), 10, mode, 1);
    if ((count > 7) && (count < 12))
        draw_circle(vgabuf, (x1 - x2) * 2 / 6 + x2, (y1 - y2) * 2 / 6 + y2, (count - 6) << 3, 0, 1,
                    255 - 70 * (count - 8), 255 - 70 * (count - 8), 10, mode, 1);
    if ((count > 8) && (count < 13))
        draw_circle(vgabuf, (x1 - x2) * 1 / 6 + x2, (y1 - y2) * 1 / 6 + y2, (count - 8) << 3, 0, 1,
                    255 - 70 * (count - 9), 255 - 70 * (count - 9), 10, mode, 1);
    if ((count > 10) && (count < 21))
...
void Magic::draw_magic_seven(VgaBuf *vgabuf, int x1, int y1, int x2, int y2, int mode, int count)
{
    ...
    // vgabuf->buf_width(), 						vgabuf->buf_height(), x1-20, y1,
    // x1+20, y1, 0xFFFF); 	draw_circle(vgabuf, x1, y1, 30, 1, 1, 155, 155,
    // 255, mode, 0);
    ...
    for (int i = 0; i < totalNum; i++)
    {
        ...
        if ((frame > 9) && (frame < 12))
        {
            ...
            draw_light_beam(vgabuf, curX, curY, curX - 30, curY, 5, 1, 255, 255, 255, color[col][0], color[col][1],
                            color[col][2], mode, 2);
            draw_light_beam(vgabuf, curX, curY, curX + 30, curY, 5, 1, 255, 255, 255, color[col][0], color[col][1],
                            color[col][2], mode, 2);
            draw_light_beam(vgabuf, curX, curY, curX, curY - 30, 5, 1, 255, 255, 255, color[col][0], color[col][1],
                            color[col][2], mode, 2);
            draw_light_beam(vgabuf, curX, curY, curX, curY + 30, 5, 1, 255, 255, 255, color[col][0], color[col][1],
                            color[col][2], mode, 2);
        }
        ...
        if ((frame > 11) && (frame < 14))
        {
            ...
            draw_light_beam(vgabuf, curX, curY, curX - 40, curY, 5, 0, 255, 255, 255, color[col][0], color[col][1],
                            color[col][2], mode, 2);
            draw_light_beam(vgabuf, curX, curY, curX + 40, curY, 5, 0, 255, 255, 255, color[col][0], color[col][1],
                            color[col][2], mode, 2);
            draw_light_beam(vgabuf, curX, curY, curX, curY - 40, 5, 0, 255, 255, 255, color[col][0], color[col][1],
                            color[col][2], mode, 2);
            draw_light_beam(vgabuf, curX, curY, curX, curY + 40, 5, 0, 255, 255, 255, color[col][0], color[col][1],
                            color[col][2], mode, 2);
        }
...
{
    //	draw_circle2(vgabuf, x1-100, y1,		 100, 100, 255, 055, 055,
    // mode,
    // 1); 	draw_circle2(vgabuf, x1-100, y1+200, 100, 100, 255, 055, 055, mode,
    // 1);
...
void Magic::draw_magic_nine(VgaBuf *vgabuf, int x1, int y1, int x2, int y2, int mode, int curStep, int totalStep)
{
    ...
    if (count == 3)
    {
        draw_lighting(x1, y1, x2, y2, 4, 1, 30, 30, 00, 180, 180, 180, vgabuf, 27, mode, 12, 3, 2);
        draw_circle(vgabuf, x2, y2, 10, 1, 1, 255, 255, 255, mode, 0);
    }
    if ((count > 3) && (count < 6))
    {
        draw_lighting(x1, y1, x2, y2, 4, 1, 127, 127, 00, 255, 255, 255, vgabuf, (count + 3) * 27, mode, 12, 3, 2);
        draw_circle(vgabuf, x2, y2, 20, 1, 1, 255, 255, 255, mode, 0);
    }
    if ((count > 0) && (count < 4))
    {
        draw_light_beam(vgabuf, x1, y1, x1, y1 - count * 10, 5, 2, 127, 127, 0, 255, 255, 255, vga.pixel_format_flag,
                        02);
        draw_light_beam(vgabuf, x1, y1, x1 + count * 10, y1, 5, 2, 127, 127, 0, 255, 255, 255, vga.pixel_format_flag,
                        02);
        draw_light_beam(vgabuf, x1, y1, x1, y1 + count * 10, 5, 2, 127, 127, 0, 255, 255, 255, vga.pixel_format_flag,
                        02);
        draw_light_beam(vgabuf, x1, y1, x1 - count * 10, y1, 5, 2, 127, 127, 0, 255, 255, 255, vga.pixel_format_flag,
...
void Magic::draw_magic_ten(VgaBuf *vgabuf, int x1, int y1, int x2, int y2, int mode, int curStep, int totalStep,
                           int delay)
{
    // shift all coordinates to the ZOOM display
    x1 = x1 + bound_x1;
    ...
    if ((count > 5) && (count < 16))
        draw_circle(vgabuf, x1, y1 - 405, (count - 4) << 3, 1, 1, 255 - 25 * (count - 6), 255 - 25 * (count - 6), 10,
                    mode, 1);
    ...
    if ((count > 6) && (count < 9))
    {
        draw_lighting(x1, y1 - 110, x1 + 5, y1 - 400, 8, 2, 127, 127, 00, 255, 255, 255, vgabuf, (count - 6) * 27, mode,
                      10, 3, 1);
        draw_circle(vgabuf, x1, y1 - 405, 40, 1, 1, 255, 255, 255, mode, 0);
    }
    if (count == 9)
    ...
    if ((count > 9) && (count < 12))
    {
        draw_lighting(x1, y1 - 110, x1 - 5, y1 - 400, 8, 2, 127, 127, 00, 255, 255, 255, vgabuf, (count - 9) * 117,
                      mode, 10, 3, 1);
        draw_circle(vgabuf, x1, y1 - 405, 40, 1, 1, 255, 255, 255, mode, 0);
    }
    ...
    if ((count > 6) && (count < 13))
    {
        draw_light_beam(&vga_back, x1, y1 - 405, x1, y1 - 45 - 405, 10, 4, 127, 127, 0, 255, 255, 255,
                        vga.pixel_format_flag, 02);
        draw_light_beam(&vga_back, x1, y1 - 405, x1 + 45, y1 - 405, 10, 4, 127, 127, 0, 255, 255, 255,
                        vga.pixel_format_flag, 02);
        draw_light_beam(&vga_back, x1, y1 - 405, x1, y1 + 45 - 405, 10, 4, 127, 127, 0, 255, 255, 255,
                        vga.pixel_format_flag, 02);
        draw_light_beam(&vga_back, x1, y1 - 405, x1 - 45, y1 - 405, 10, 4, 127, 127, 0, 255, 255, 255,
...
void Magic::draw_magic_twelve(VgaBuf *vgabuf, int x1, int y1, int mode, int curStep, int nationRecno)
{
    ...
    if ((curStep > 4) && (curStep < 17))
    {
        ...
        if (locx1 <= bound_x2 && locx2 >= bound_x1 && locy1 <= bound_y2 && locy2 >= bound_y1)
        {
            ...
            vgabuf->put_bitmap_area_trans_remap_decompress(locx1, locy1, (char *)bitmapPtr, srcX1, srcY1, srcX2, srcY2,
                                                           colorRemapTable);
        }
    }
    ...
    if ((curStep > 4) && (curStep < 7))
    {
        ...
        draw_light_beam(vgabuf, x1, y1, x1 - 90, y1, 8, 1, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
        draw_light_beam(vgabuf, x1, y1, x1 + 90, y1, 8, 1, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
        draw_light_beam(vgabuf, x1, y1, x1, y1 - 90, 8, 1, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
        draw_light_beam(vgabuf, x1, y1, x1, y1 + 90, 8, 1, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
    }
    ...
    if ((curStep > 6) && (curStep < 9))
    {
        ...
        draw_light_beam(vgabuf, x1, y1, x1 - 120, y1, 8, 0, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
        draw_light_beam(vgabuf, x1, y1, x1 + 120, y1, 8, 0, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
        draw_light_beam(vgabuf, x1, y1, x1, y1 - 120, 8, 0, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
        draw_light_beam(vgabuf, x1, y1, x1, y1 + 120, 8, 0, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
    }
    ...
    if ((curStep > 8) && (curStep < 11))
    {
        ...
        draw_light_beam(vgabuf, x1, y1, x1 - 80, y1, 5, 0, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
        draw_light_beam(vgabuf, x1, y1, x1 + 80, y1, 5, 0, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
        draw_light_beam(vgabuf, x1, y1, x1, y1 - 80, 5, 0, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
        draw_light_beam(vgabuf, x1, y1, x1, y1 + 80, 5, 0, 255, 255, 255, color[col][0], color[col][1], color[col][2],
                        mode, 2);
    }
    ...
    //(curStep<<4)>>intensity, 255>>intensity,
    //(255-(curStep<<4))>>intensity,
    // mode, 1);
}
...
void Magic::draw_magic_twelve_die(VgaBuf *vgabuf, int x1, int y1, int mode, int curStep, int nationRecno, int random)
{
    ...
    if ((curStep > 2) && (curStep < 8))
        draw_circle2(vgabuf, x1, y1, (7 - curStep) * 35, 1, 0, 50 * curStep - 100, 50 * curStep - 100,
                     50 * curStep - 100, mode, 1);

    int intensity = ((curStep - 4) >> 2);
    if ((curStep > 7) && (curStep < 17))
        draw_circle2(vgabuf, x1, y1, 40 * (curStep - 7), 1, 0, (255 - (curStep << 4)) >> intensity, 255 >> intensity,
...
void Magic::draw_magic_thirteen(VgaBuf *vgabuf, int x1, int y1, int mode, int curStep)
{
    ...
    if ((curStep > 0) && (curStep < 10))
        draw_circle2(vgabuf, x1, y1, 40 * step, 1, 0, (255 - 25 * step) >> intensity, (255 - 25 * step) >> intensity,
    ...
    if ((curStep >= 0) && (curStep < 15))
    {
        if ((curStep % 2) == 0)
        {
            draw_light_beam(vgabuf, x1, y1, x1 + locX[curStep >> 1], y1 + locY[curStep >> 1], 10, 2, 0, 80, 255, 255,
                            255, 255, mode, 0);
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        {
            draw_particle(x1 + locX[j >> 1] - 5, y1 + locY[j >> 1] - 5, x1 + locX[j >> 1] + 5, y1 + locY[j >> 1] + 5, 0,
                          0, 1, 10, 0, vgabuf, mode, 50, 2, curStep - j, 6);
        }
    }
}
...
void Magic::draw_magic_forteen(VgaBuf *vgabuf, int x1, int y1, int mode, int curStep, int curDir)
{
    ...
    draw_circle2(vgabuf, x1, y1, 60, 0, 1, 255 >> (curStep >> 1), 255 >> (curStep >> 1), 255 >> (curStep >> 1), mode,
                 0);
    y1 += 32;
    switch (curDir)
    {
    case 0:
        draw_light_beam(vgabuf, x1 - 10, y1 - (curStep << 3), x1 + 10, y1 + 5 - (curStep << 3), 10, 0, 0, 80, 255, 255,
                        255, 255, mode, 0);
        break;
    case 1:
        draw_light_beam(vgabuf, x1 + 7, y1 - 8 - (curStep << 3), x1 + 13, y1 - 8 - (curStep << 3), 10, 0, 0, 80, 255,
                        255, 255, 255, mode, 0);
        break;
    case 2:
        draw_light_beam(vgabuf, x1 - 10, y1 + 5 - 16 - (curStep << 3), x1 + 10, y1 - 16 - (curStep << 3), 10, 0, 0, 80,
                        255, 255, 255, 255, mode, 0);
        break;
    case 3:
        draw_light_beam(vgabuf, x1 - 10, y1 - 16 - (curStep << 3), x1 + 10, y1 - 16 - (curStep << 3), 10, 0, 0, 80, 255,
                        255, 255, 255, mode, 0);
        break;
    case 4:
        draw_light_beam(vgabuf, x1 - 10, y1 - 16 - (curStep << 3), x1 + 10, y1 + 5 - 16 - (curStep << 3), 10, 0, 0, 80,
                        255, 255, 255, 255, mode, 0);
        break;
    case 5:
        draw_light_beam(vgabuf, x1 - 7, y1 - 8 - (curStep << 3), x1 - 13, y1 - 8 - (curStep << 3), 10, 0, 0, 80, 255,
        ...
        break;
    case 6:
        draw_light_beam(vgabuf, x1 - 10, y1 + 5 - (curStep << 3), x1 + 10, y1 - (curStep << 3), 10, 0, 0, 80, 255, 255,
                        255, 255, mode, 0);
        break;
    case 7:
        draw_light_beam(vgabuf, x1 - 10, y1 - (curStep << 3), x1 + 10, y1 - (curStep << 3), 10, 0, 0, 80, 255, 255, 255,
                        255, mode, 0);
        break;
    }
}
