...
void CampaignEastWest::stage_27_init_vars() {
  ...
  game.reset_win_condition(); // so that only achieving E_REACH_POPULATION will
                              // win the game.

  game.add_win_condition(E_REACH_POPULATION, 1, 0, goal_population,
                         goal_year_limit);
}
...
int CampaignEastWest::stage_27_create_town() {
  Battle::create_independent_town(
      4 + misc.random(3)); // 4-6 independent towns at the beginning
...
void CampaignEastWest::stage_27_create_egyptian_rebel() {
  int nationRecno = nation_array.new_nation(
      NATION_HUMAN, NATION_AI, RACE_EGYPTIAN,
...
char *CampaignEastWest::stage_27_intro_text() {
  return substitute_text(res_stage.read("27INTRO"), goal_population,
                         goal_year_limit);
}
...
char *CampaignEastWest::stage_27_goal_text() {
  return substitute_text(res_stage.read("27GOAL"), goal_population,
                         goal_year_limit);
}
