...
void Sprite::sprite_move(int desX, int desY)
{
    ...
    */
    ...
    set_next(cur_x + stepMagn * move_x_pixel_array[final_dir], cur_y + stepMagn * move_y_pixel_array[final_dir],
             -stepMagn);
...
void Sprite::process_move()
{
    ...
    if (next_x != go_x || next_y != go_y) // if next_x==go_x & next_y==go_y, reach destination
    ...
    err_when(cur_x - next_x != 0 && cur_y - next_y != 0 &&
             abs(next_x - cur_x) != abs(next_y - cur_y)); // is a better checking if speed in all
                                                          // direction is equal
#ifdef DEBUG
