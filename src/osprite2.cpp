...
void Sprite::sprite_move(int desX, int desY) {
  ...
  err_when(desX < 0 || desY < 0 || desX >= MAX_WORLD_X_LOC * LOCATE_WIDTH ||
           desY >= MAX_WORLD_Y_LOC * LOCATE_HEIGHT);
  ...
  {
    err_if(abs(vectorX) != abs(vectorY)) err_here();
  }
#endif
  ...
  */
  ...
  set_next(cur_x + stepMagn * move_x_pixel_array[final_dir],
           cur_y + stepMagn * move_y_pixel_array[final_dir], -stepMagn);
...
#define TURN_REQUIRE_AMOUNT 60
  ...
  if ((cur_dir + HALF_SPRITE_DIR_TYPE) % MAX_SPRITE_DIR_TYPE ==
      final_dir) // opposite direction
  {
    cur_dir += (final_dir % 2 ? 1 : -1);
    cur_dir = cur_dir % MAX_SPRITE_DIR_TYPE;
  } else {
    err_when(cur_dir == final_dir || ((cur_dir + HALF_SPRITE_DIR_TYPE) %
                                      MAX_SPRITE_DIR_TYPE) == final_dir);

    UCHAR dirDiff =
        (final_dir - cur_dir + MAX_SPRITE_DIR_TYPE) % MAX_SPRITE_DIR_TYPE;
...
void Sprite::process_move() {
  ...
  err_when(abs(cur_x - next_x) > LOCATE_WIDTH * debugStepMagn1 ||
           abs(cur_y - next_y) > LOCATE_HEIGHT * debugStepMagn1);
#endif
  ...
  if (cur_x == go_x && cur_y == go_y) {
    ...
    for (h = 0, y = next_y_loc(); h < sprite_info->loc_height && !blocked;
         h++, y++) {
      for (w = 0, x = next_x_loc(); w < sprite_info->loc_width && !blocked;
           w++, x++)
        blocked = world.get_loc(x, y)->unit_recno(mobile_type) != sprite_recno;
    }
    err_if(blocked) err_here();
#endif
    cur_frame = 1;
    return;
  }
  ...
  static short vector_x_array[] = {
      0, 1, 1, 1, 0, -1, -1, -1}; // default vectors, temporary only
  ...
  if (final_dir & 1) // diagonal direction slower
  {
    if (stepX >= 8) {
      stepY =
          (stepX -= (unsigned)stepX / 4); // slightly fast for unsigned division
    } else if (stepX >= 3) {
      --stepX;
      --stepY;
    }
  ...
  if (next_x != go_x ||
      next_y != go_y) // if next_x==go_x & next_y==go_y, reach destination
                      // already, don't move further.
  {
    if (abs(cur_x - next_x) <= stepX && abs(cur_y - next_y) <= stepY) {
      int stepMagn = move_step_magn();

      if (!set_next(next_x + stepMagn * move_x_pixel_array[final_dir],
                    next_y + stepMagn * move_y_pixel_array[final_dir],
                    -stepMagn))
        return;
    }
  }
  ...
  short vectorX = vector_x_array[final_dir] *
                  stepX; // cur_dir may be changed in the above set_next() call
  ...
  err_when(cur_x - next_x != 0 && cur_y - next_y != 0 &&
           abs(next_x - cur_x) !=
               abs(next_y - cur_y)); // is a better checking if speed in all
                                     // direction is equal
#ifdef DEBUG
...
int Sprite::process_attack() {
  ...
  if (++cur_frame > spriteAttack->frame_count) {
    ...
    set_remain_attack_delay(); // set it to the max and start the backward
                               // counting
    return 1;
  }
...
int Sprite::set_next(int nextX, int nextY, int para, int blockedChecked) {
  err_here(); //**BUGHERE, for now, we don't have any pure Sprites, all
              //functions should call Unit::set_next() instead of
              //Sprite::set_next()
...
//
void Sprite::set_guard_off() { guard_count = 0; }
