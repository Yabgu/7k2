...
void FirmLair::think_build_special() {
  //--- if an action is already queued for building a special structure, don't
  //build new ones ---//
...
int FirmLair::should_build_special(int firmId) {
  ...
  //--- if we can build one now and we don't have one currently, then build
  //immediately ---//
  ...
  int importanceRating = 40 +
                         nation_array[nation_recno]->pref_monster_special / 5 -
                         ownFirmCount * 10;

  return nation_array[nation_recno]->ai_should_spend(
      importanceRating, firmInfo->setup_cost, firmInfo->setup_live_points_cost);
}
...
int FirmLair::think_build_kharsuf_special() {
  ...
  int arrayCount = 0;
  short *array =
      nationPtr->update_ai_firm_array(FIRM_LISHORR, 0, 0, arrayCount);
  for (int i = 0; i < arrayCount; i++, array++) {
    ...
    int netPower =
        world.net_attacker_power_in_area(firmPtr->center_x, firmPtr->center_y,
  ...
  //--- if no new Lishorr is built because none of the firms' power is below
  //ai_last_build_lishorr_net_power

  nationPtr->ai_last_build_lishorr_net_power +=
      100; // increase it 100 at a time, so the next time more firms will meet
           // the condition
...
int FirmLair::think_build_bregma_special() {
  ...
  //--- if any of the existing Alchemy firm is lowerly utilized, don't build new
  //ones ---//

  Nation *nationPtr = nation_array[nation_recno];

  for (int i = 0; i < nationPtr->ai_mons_train_count; i++) {
    FirmMonsterTrain *firmPtr = firm_array[nationPtr->ai_mons_train_array[i]]
                                    ->cast_to_FirmMonsterTrain();
...
int FirmLair::think_build_kerassos_special() {
  ...
  for (int i = 0; i < nationPtr->ai_mons_animal_count; i++) {
    FirmAnimal *firmPtr =
        firm_array[nationPtr->ai_mons_animal_array[i]]->cast_to_FirmAnimal();

    err_when(!firmPtr);

    if (firmPtr->animal_unit_count >= 2) // don't build new ones if existing
                                         // ones have sufficient animal stocks
      return 0;
  }
...
int FirmLair::think_build_minotauros_special() {
  ...
  for (int i = 0; i < nationPtr->ai_mons_alchemy_count; i++) {
    FirmMonsterAlchemy *firmPtr =
        firm_array[nationPtr->ai_mons_alchemy_array[i]]
            ->cast_to_FirmMonsterAlchemy();
...
int FirmLair::think_build_grokken_special() { return 0; }
...
int FirmLair::think_build_ezpinez_special() {
  ...
  //--- if any of the existing Incubator firm is lowerly utilized, don't build
  //new ones ---//

  Nation *nationPtr = nation_array[nation_recno];

  for (int i = 0; i < nationPtr->ai_incubator_count; i++) {
    FirmIncubator *firmPtr =
        firm_array[nationPtr->ai_incubator_array[i]]->cast_to_FirmIncubator();
...
int FirmLair::think_build_exovum_special() {
  ...
  for (int i = 0; i < nationPtr->ai_mons_magic_count; i++) {
    FirmMagic *firmPtr =
        firm_array[nationPtr->ai_mons_magic_array[i]]->cast_to_FirmMagic();
...
int FirmLair::think_kharsuf_buy_tech() {
  static short techIdArray[] = {TECH_LISHORR, TECH_WILDE_LISHORR,
...
//
int FirmLair::think_tech_to_buy(short *techIdArray, short *techPrefArray,
                                int techCount) {
  Nation *ownNation = nation_array[nation_recno];
  ...
  for (i = 0; i < techCount; i++)
    totalTechLevel +=
        tech_res[techIdArray[i]]->get_nation_tech_level(nation_recno);
  ...
  for (i = 0; i < techCount; i++) {
    ...
    if (ownNation->military_rating + ownNation->pref_monster_special / 10 +
                buyPref / 5 >
            40 + totalTechLevel * 10 ||
        have_excess_live_point(techId,
                               1000 + totalTechLevel * 100 - buyPref * 4)) {
...
int FirmLair::have_excess_live_point(int techId, int excessLivePoints) {
  ...
  return ownNation->live_points >
         tech_res[techId]->live_points_cost() +
             excessLivePoints * (75 + ownNation->pref_live_points_reserve / 2) /
                 100;
}
...
int FirmLair::think_exovum_buy_tech() {
  static short techIdArray[] = {TECH_MAGIC_DAMAGE_BUILDING,
                                TECH_MAGIC_FREEZE_UNIT, TECH_MAGIC_DAMAGE_AREA,
                                TECH_MAGIC_TELEPORT};
...
int FirmLair::think_minotauros_buy_tech() {
  static short techIdArray[] = {TECH_LIVE_TO_GOLD, TECH_GOLD_TO_LIVE,
                                TECH_RAW_TO_LIVE, TECH_RAW_TO_GOLD,
                                TECH_ALCHEMY_YIELD};
