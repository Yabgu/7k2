...
void FirmLair::think_build_special()
{
    ...
    for (i = ownNation->action_count(); i > 0; i--)
    {
        ...
        if (actionNode->action_mode == ACTION_AI_BUILD_FIRM && actionNode->action_para >= FIRST_MONSTER_SPECIAL_FIRM &&
...
int FirmLair::should_build_special(int firmId)
{
    ...
    int importanceRating = 40 + nation_array[nation_recno]->pref_monster_special / 5 - ownFirmCount * 10;

    return nation_array[nation_recno]->ai_should_spend(importanceRating, firmInfo->setup_cost,
...
int FirmLair::think_build_kharsuf_special()
{
    ...
    for (int i = 0; i < arrayCount; i++, array++)
    {
        ...
        int netPower = world.net_attacker_power_in_area(firmPtr->center_x, firmPtr->center_y, nation_recno, 0, 0,
                                                        hasWar, scanRange);
    ...
    nationPtr->ai_last_build_lishorr_net_power += 100; // increase it 100 at a time, so the next time more firms will
                                                       // meet the condition
...
int FirmLair::think_build_bregma_special()
{
    ...
    for (int i = 0; i < nationPtr->ai_mons_train_count; i++)
    {
        ...
        if (firmPtr->firm_id == firmId && firmPtr->trainee_count < MAX_MONSTER_TRAINEE)
        {
            return 0;
        }
    }
...
int FirmLair::think_build_minotauros_special()
{
    ...
    if (tech_res[TECH_RAW_TO_LIVE] || tech_res[TECH_RAW_TO_GOLD])
    {
        ...
        int rawId = nationPtr->seek_mine(xLoc, yLoc, refXLoc,
...
int FirmLair::have_excess_live_point(int techId, int excessLivePoints)
{
    ...
    return ownNation->live_points > tech_res[techId]->live_points_cost() +
                                        excessLivePoints * (75 + ownNation->pref_live_points_reserve / 2) / 100;
}
...
{
    static short techIdArray[] = {TECH_MAGIC_DAMAGE_BUILDING, TECH_MAGIC_FREEZE_UNIT, TECH_MAGIC_DAMAGE_AREA,
                                  TECH_MAGIC_TELEPORT};
...
{
    static short techIdArray[] = {TECH_LIVE_TO_GOLD, TECH_GOLD_TO_LIVE, TECH_RAW_TO_LIVE, TECH_RAW_TO_GOLD,
                                  TECH_ALCHEMY_YIELD};
