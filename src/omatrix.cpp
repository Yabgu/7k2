...
void Matrix::init(int winX1, int winY1, int winX2, int winY2, int areaWidth,
                  int areaHeight, float locXWidth, float locXHeight,
                  float locYWidth, float locYHeight, char dispRecType,
                  int saveAreaToBuf) {
  ...
  int shiftTmp;
  for (shiftTmp = determinant, determinant_shift = 0; shiftTmp > 1;
       ++determinant_shift, shiftTmp >>= 1)
    ;
  ...
  if (saveAreaToBuf) {
    // ##### begin Gilbert 20/10 ######//
    save_image_buf =
        (short *)BitmapW::mem_add_bitmap(image_width, image_height);
    // ##### end Gilbert 20/10 ######//
  } else
    save_image_buf = NULL;
...
void Matrix::init_var() {
  if (!disp_rect_type) {
    disp_x_loc = (image_width << LOC_XY_MUL_SHIFT) /
                 loc_x_width; // loc_x_width is multiplied by LOC_XY_MUL_SHIFT
    center_x = disp_x_loc / 2;
    if (disp_x_loc > max_x_loc)
      disp_x_loc = max_x_loc;
    if (center_x > max_x_loc)
      center_x = max_x_loc;
    disp_y_loc = (image_height << LOC_XY_MUL_SHIFT) /
                 loc_y_height; // loc_x_width is multiplied by LOC_XY_MUL_SHIFT
    ...
    disp_x_loc =
        (image_width << (LOC_XY_MUL_SHIFT + 1)) / (loc_x_width - loc_y_width);
    // #### begin Gilbert 1/2 ########//
    // disp_y_loc = (image_width << (LOC_XY_MUL_SHIFT+1)) / (loc_x_height +
    // loc_y_height);
    disp_y_loc = (image_height << (LOC_XY_MUL_SHIFT + 1)) /
                 (loc_x_height + loc_y_height);
    // #### end Gilbert 1/2 ########//
...
void Matrix::draw() {
  ...
  if (!disp_rect_type) {
    ...
    for (y = image_y1, yLoc = top_y_loc; yLoc < maxYLoc;
         yLoc++, y += loc_y_height) {
      if ((i++) % 10 == 0)
        sys.yield();
      ...
      for (x = image_x1, xLoc = top_x_loc; xLoc < maxXLoc;
           xLoc++, x += loc_x_width, locPtr++)
  ...
  post_draw(); // if they have anything to draw after the basic terrain
               // (e.g.firms)

  //---------------------------------------------------//

  if (save_image_buf) {
    vga_back.read_bitmapW(image_x1, image_y1, image_x2, image_y2,
                          save_image_buf);
    just_drawn_flag = 1;
  }
}
...
//
void Matrix::refresh() {
  if (save_image_buf) // redraw the background, so disp() won't use the old
                      // background
    draw();
...
int Matrix::valid_cur_box(int callRefresh) {
  ...
  if (!disp_rect_type) {
    ...
    // such that middle bottom of display rectangle within (0, 0, max_x_loc-1,
    // max_y_loc-1) find the vector from top left to middle bottom of the
    // display area in x,y location plane
...
void Matrix::valid_disp_area(int fitInBox) {
  ...
  //--- if the current highlighted location is outside the display area, then
  //reposition it ----//
...
// return 1 if success
//
int Matrix::get_detect_location(int scrnX, int scrnY, int *locX, int *locY,
                                char *mobileType) {
  if (scrnX >= image_x1 && scrnX <= image_x2 && scrnY >= image_y1 &&
    ...
    // try not to use divide becuase result division on a negative number is not
    // desired
    if (LOC_XY_MUL_SHIFT >= determinant_shift) {
      dummyX = top_x_loc + ((relX * loc_y_height - relY * loc_y_width)
                            << (LOC_XY_MUL_SHIFT - determinant_shift));
      dummyY = top_y_loc + ((relY * loc_x_width - relX * loc_x_height)
                            << (LOC_XY_MUL_SHIFT - determinant_shift));
    } else {
      dummyX = top_x_loc + ((relX * loc_y_height - relY * loc_y_width) >>
                            (determinant_shift - LOC_XY_MUL_SHIFT));
      dummyY = top_y_loc + ((relY * loc_x_width - relX * loc_x_height) >>
                            (determinant_shift - LOC_XY_MUL_SHIFT));
    }

    if (dummyX >= 0 && dummyX < max_x_loc && dummyY >= 0 &&
        dummyY < max_y_loc) {
      *locX = dummyX;
      *locY = dummyY;
      return 1;
...
// return 1 if success
//
int Matrix::get_close_location(int scrnX, int scrnY, int *locX, int *locY,
                               char *mobileType) {
  return 0;
}
...
int Matrix::calc_loc_pos_x(int xLoc, int yLoc) {
  return ((xLoc - top_x_loc) * loc_x_width +
          (yLoc - top_y_loc) * loc_y_width) >>
         LOC_XY_MUL_SHIFT;
}

int Matrix::calc_loc_pos_y(int xLoc, int yLoc) {
  return ((xLoc - top_x_loc) * loc_x_height +
          (yLoc - top_y_loc) * loc_y_height) >>
         LOC_XY_MUL_SHIFT;
}
