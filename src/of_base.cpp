...
void FirmBase::invoke_god()
{
    ...
    if (effectId)
        ...
        Effect::create(effectId, unitPtr->next_x_loc() * LOCATE_WIDTH, unitPtr->next_y_loc() * LOCATE_HEIGHT,
                       SPRITE_IDLE, 1, 8, 0);
}
...
int FirmBase::can_invoke()
{
    ...
    return !god_unit_recno &&                   // one base can only support one god
           pray_points >= MAX_PRAY_POINTS / 10; // there must be at least 10% of the maximum
