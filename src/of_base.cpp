  ...
  for (int i = 1; i <= god_res.god_count; i++) {
    if (god_res[i]->race_id == race_id &&
        (god_res[i]->is_nation_know(nation_recno) ||
         nationPtr->know_base_array[god_res[i]->race_id - 1])) {
      god_id = i;
      break;
    }
  }
...
void FirmBase::next_day() {
  ...
  } else if (is_own() && (info.game_date + firm_recno) % 60 == 0)
...
void FirmBase::invoke_god() {
  ...
  if (effectId)
    //	Effect::create(effectId, center_x *LOCATE_WIDTH, center_y
    //*LOCATE_HEIGHT, SPRITE_IDLE, 1, 8, 0);
    Effect::create(effectId, unitPtr->next_x_loc() * LOCATE_WIDTH,
                   unitPtr->next_y_loc() * LOCATE_HEIGHT, SPRITE_IDLE, 1, 8, 0);
}
...
int FirmBase::can_invoke() {
  ...
  return !god_unit_recno && // one base can only support one god
         pray_points >=
             MAX_PRAY_POINTS / 10; // there must be at least 10% of the maximum
                                   // pray points to cast a creature
}
