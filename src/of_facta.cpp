...
int FirmFactory::think_build_market() {
  ...
  //-- only build one market place next to this factory, check if there is any
  //existing one --//
...
int FirmFactory::think_change_production() {
  ...
  if (info.game_date <
      setup_date + 30) // only change production after the factory has been
                       // running for at least one month
    return 0;
...
#define MIN_FACTORY_IMPORT_STOCK_QTY 20
    ...
    if (firmPtr->firm_id == FIRM_MINE) {
      ...
      if (firmMine->stock_qty >= MIN_FACTORY_IMPORT_STOCK_QTY) {
        ...
        if (curRating > bestRating) {
          if (firmPtr->nation_recno == nation_recno ||
              !bestIsOwn) // try to get raw materials from own firms first
          {
  ...
  if (bestProductId) {
    ...
    if (info.game_date > setup_date + 60) {
      ai_del_firm(); // delete the firm if there is no raw materials available
                     // after it has been built for over 2 months
      return 1;
    }
  }
