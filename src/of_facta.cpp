...
int FirmFactory::think_change_production()
{
    ...
    if (info.game_date < setup_date + 30) // only change production after the factory has been
...
#define MIN_FACTORY_IMPORT_STOCK_QTY 20
    ...
    for (int i = 0; i < linked_firm_count; i++)
    {
        ...
        else if (firmPtr->firm_id == FIRM_MARKET)
        {
            ...
            {
                if (marketGoods->raw_id && marketGoods->stock_qty >= MIN_FACTORY_IMPORT_STOCK_QTY)
                {
