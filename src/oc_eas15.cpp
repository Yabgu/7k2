...
enum
{
    STAGE_15_MONSTER_NATION_RECNO = 2
};
...
void CampaignEastWest::stage_15_init_vars()
{
    ...
    game.reset_win_condition(); // so that only achieving E_ACQUIRE_TECH will win
                                // the game.
...
int CampaignEastWest::stage_15_create_lair()
{
    ...
    create_lair(cur_monster_nation_recno, STAGE_15_MONSTER_NATION_RECNO, lairAddCount, independentTownAddCount,
                specialFirmCount);
...
{
    //--- set the tech level here, as the setting will be overiden in
    // fetch_nation_info ---//
