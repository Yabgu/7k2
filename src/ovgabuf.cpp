...
{
    err_if(buf_locked) err_now("VgaBuf::lock_buf() error, buffer already locked.");
...
void VgaBuf::save_area_common_buf(int x1, int y1, int x2, int y2)
{
    ...
    err_if(saveSize > COMMON_DATA_BUF_SIZE) err_now("VgaBuf::save_area_common_buf()");
...
void VgaBuf::put_large_bitmap(int x1, int y1, File *filePtr, short *colorRemapTable)
{
    ...
    {
        int bufferLine =
            (COMMON_DATA_BUF_SIZE - 2 * sizeof(short)) / pictWidth; // max. no. of lines can be in the buffer
