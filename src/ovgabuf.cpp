...
VgaBuf::VgaBuf() { memset(this, 0, sizeof(VgaBuf)); }
//-------- End of function VgaBuf::VgaBuf ----------//

//-------- Begin of function VgaBuf::~VgaBuf ----------//

VgaBuf::~VgaBuf() { deinit(); }
//-------- End of function VgaBuf::~VgaBuf ----------//
...
void VgaBuf::put_bitmap_trans(int x, int y, char *bitmapPtr) {
  ...
  IMGbltTransRemap(cur_buf_ptr, cur_pitch, x, y, bitmapPtr,
                   default_remap_table);
...
// Put a bitmap on the surface buffer and hide the mouse cursor while displaying
//
void VgaBuf::put_bitmap_remap(int x, int y, char *bitmapPtr,
                              short *colorTable) {
  err_when(!buf_locked);
...
void VgaBuf::put_bitmapW(int x, int y, short *bitmapWBuf) {
  ...
  if (is_front)
    mouse.hide_area(x, y, x + ((BitmapW *)bitmapWBuf)->get_width() - 1,
                    y + ((BitmapW *)bitmapWBuf)->get_height() - 1);
...
short *VgaBuf::save_area(int x1, int y1, int x2, int y2, short *saveScr) {
  ...
  long newSize =
      sizeof(short) * 4 + BitmapW::size((x2 - x1 + 1), (y2 - y1 + 1));
...
void VgaBuf::put_large_bitmap(int x1, int y1, File *filePtr,
  ...
  {
    int bufferLine = (COMMON_DATA_BUF_SIZE - 2 * sizeof(short)) /
                     pictWidth; // max. no. of lines can be in the buffer
    ...
    while (y1 <= y2) {
      ...
      filePtr->file_read(((Bitmap *)sys.common_data_buf)->bitmap,
                         (unsigned)pictWidth * (ty - y1 + 1));

      if (is_front)
        mouse.hide_area(x1, y1, x2,
                        ty); // if the mouse cursor is in that area, hide it
...
//
// Put a picture on the screen, when a picture's size is > 64K
// Pass a file pointer to put_large_bitmap_trans() for continously reading the
// file
//
// Syntax
//
...
void VgaBuf::put_large_bitmap_trans(int x1, int y1, File *filePtr,
                                    short *colorRemapTable) {
  if (filePtr == NULL)
    return;
  ...
  if (Bitmap::size(pictWidth, pictHeight) <= COMMON_DATA_BUF_SIZE) {
    ...
    put_bitmap_trans_remap(x1, y1, (char *)sys.common_data_buf,
                           colorRemapTable);
...
void VgaBuf::put_large_bitmapW(int x1, int y1, File *filePtr) {
  ...
  } else //----- if the picture size > 64K, read in line by line -----//
  {
    int bufferLine = (COMMON_DATA_BUF_SIZE - 2 * sizeof(short)) / pictWidth /
                     sizeof(short); // max. no. of lines can be in the buffer
    int ty = y1 + bufferLine - 1;
    ...
    while (y1 <= y2) {
      ...
      filePtr->file_read(((BitmapW *)sys.common_data_buf)->bitmap,
                         (unsigned)pictWidth * (ty - y1 + 1) * sizeof(short));
