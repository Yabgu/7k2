...
SurfaceSDL::SurfaceSDL(SDL_Surface *s) : surface(s), save_buf(NULL) {
  cur_pitch = s->pitch;
}
...
//
BOOL SurfaceSDL::is_buf_lost() { return 0; }
...
//
void SurfaceSDL::blt_buf_area(SurfaceSDL *srcBuf, int x1, int y1, int x2,
                              int y2) {
  SDL_Rect bltRect;
...
// Blit an area of the source surface to local destination surface.
//
void SurfaceSDL::blt_virtual_buf_area(SurfaceSDL *srcBuf, int x1, int y1,
                                      int x2, int y2) {
  SDL_Rect bltRect;
...
SDL_Surface *SurfaceSDL::get_surface() { return surface; }
