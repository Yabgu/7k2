...
#define SLOT_Y2(n) (by + SLOT_Y_SPACING * (n) + 110 + SLOT_HEIGHT - 1)
    ...
    if (!pre_game)
    {
        ...
        delButton.create_text(bx + BUTTON7_X1, by + BUTTON7_Y1, bx + BUTTON7_X2, by + BUTTON7_Y2,
                              text_game_menu.str_delete_save());
        cancelButton.create_text(bx + BUTTON4_X1, by + BUTTON4_Y1, bx + BUTTON4_X2, by + BUTTON4_Y2,
    ...
    while (1)
    {
        ...
        if (refreshFlag)
        {
            if (refreshFlag & LSOPTION_PAGE)
            {
                ...
                if (action_mode == 1)
                    font_bold_black.center_put(bx + SCROLL_SHEET_X1, by + SCROLL_SHEET_Y1, bx + SCROLL_SHEET_X2,
                                               SLOT_Y1(0),
                                               text_game_menu.str_save_game()); // "Save Game" );
                else if (action_mode == 2)
                    font_bold_black.center_put(bx + SCROLL_SHEET_X1, by + SCROLL_SHEET_Y1, bx + SCROLL_SHEET_X2,
                                               SLOT_Y1(0),
                                               text_game_menu.str_load_game()); // "Load Game" );
                ...
                if (pre_game)
                {
                    if (action_mode == 1)
                    {
                        font_thin_black.center_put(bx + BUTTON2_X1, by + BUTTON2_Y1, bx + BUTTON2_X2, by + BUTTON2_Y2,
                                                   text_game_menu.str_save()); // "Save" );
                        font_thin_black.center_put(bx + BUTTON3_X1, by + BUTTON3_Y1, bx + BUTTON3_X2, by + BUTTON3_Y2,
                    ...
                    font_thin_black.center_put(bx + BUTTON4_X1, by + BUTTON4_Y1, bx + BUTTON4_X2, by + BUTTON4_Y2,
                                               text_game_menu.str_cancel()); // "Cancel" );
                    font_thin_black.center_put(bx + BUTTON7_X1, by + BUTTON7_Y1, bx + BUTTON7_X2, by + BUTTON7_Y2,
                                               text_game_menu.str_delete_save()); // "Delete" );
                }
                else
                {
                    startButton.paint();
                ...
                for (i = 0; i < MAX_BROWSE_DISP_REC; ++i)
                {
                    ...
                    vga.active_buf->read_bitmapW(SLOT_X1(i), SLOT_Y1(i), SLOT_X2(i), SLOT_Y2(i),
                                                 browseArea[i].bitmap_ptr());
                }

                // capture page up area
                pageUpArea.clear();
                pageUpArea.resize(pageUpX1, pageUpY1, ((Bitmap *)arrowBitmap)->get_width(),
                                  ((Bitmap *)arrowBitmap)->get_height());
                vga.active_buf->read_bitmapW(pageUpX1, pageUpY1, pageUpX2, pageUpY2, pageUpArea.bitmap_ptr());
                pageDownArea.clear();
                pageDownArea.resize(pageDownX1, pageDownY1, ((Bitmap *)arrowBitmap)->get_width(),
                                    ((Bitmap *)arrowBitmap)->get_height());
            ...
            for (i = 0; i < MAX_BROWSE_DISP_REC; ++i)
            {
                if (refreshFlag & LSOPTION_SLOT(i))
                {
                    ...
                    if (rec == 0)
                    {
                        font_bold_black.center_put(browseSlotX1, browseSlotY1, browseSlotX2, browseSlotY2,
        ...
        if ((pre_game ? mouse.single_click(bx + BUTTON4_X1, by + BUTTON4_Y1, bx + BUTTON4_X2, by + BUTTON4_Y2)
                      : cancelButton.detect()) ||
            mouse.key_code == KEY_ESC ||
            mouse.any_click(RIGHT_BUTTON) > 0) // also when ESC key is pressed or right button
        {
            ...
            break; // break while(1)
                 ...
                 (pre_game ? mouse.single_click(bx + BUTTON2_X1, by + BUTTON2_Y1, bx + BUTTON2_X2, by + BUTTON2_Y2)
                           : startButton.detect()))
        {
            // save / load button
            refreshFlag = LSOPTION_ALL;
            ...
            if (retFlag != 0)
            {
                ...
                break;
                 ...
                 (pre_game ? mouse.single_click(bx + BUTTON3_X1, by + BUTTON3_Y1, bx + BUTTON3_X2, by + BUTTON3_Y2)
                           : saveNewButton.detect()))
        {
            ...
            if (retFlag != 0)
            {
                ...
                break;
                 ...
                 (pre_game ? mouse.single_click(bx + BUTTON7_X1, by + BUTTON7_Y1, bx + BUTTON7_X2, by + BUTTON7_Y2)
                           : delButton.detect()))
        {
            ...
            if (browse_recno != 0) // cannot del save game slot
            {
                ...
                //				if( browse_top_recno >
                // size()-MAX_BROWSE_DISP_REC+1)
                // browse_top_recno =
                // size()-MAX_BROWSE_DISP_REC+1; 				if(
                // browse_top_recno < minRecno )
                //					browse_top_recno = minRecno;
                ...
                // recalculate page
                maxPage =
                    (size() - minRecno + 1 + MAX_BROWSE_DISP_REC - 1) / MAX_BROWSE_DISP_REC; // divide but round up
                if (page >= maxPage)
                {
                    page = maxPage - 1;
                    if (page < 0)
...
void GameFile::disp_info(int x, int y)
{
    ...
    misc.extract_file_name(shortFileName,
...
int GameFileArray::process_action(int saveNew)
{
    ...
    if (action_mode == 1)
    {
        ...
        {
            if (!box.ask(text_game_menu.str_ask_overwrite())) // "It will overwrite the existing
                                                              // saved game, proceed ?" ) )
                return 0;
...
int GameFileArray::save_new_game(const char *fileName)
{
    ...
    if (fileName) // has a filename before extension
    {
        ...
        // strcpy( gameFile.file_name, fileName );		// now use
        // gameFile.file_name instead
    }
    else
    {
        load_all_game_header(save_default_dir, save_default_ext);
        ...
        gameFile.set_file_name(save_default_dir,
                               strchr(save_default_ext,
...
int GameFile::ask_desc()
{
    ...
    keyFieldArea.resize(descTextBox.x, descTextBox.y, descTextBox.x_limit - descTextBox.x + 1,
    ...
    while (1)
    {
        ...
        if (refreshFlag)
        {
            ...
            font_bold_black.put(boxX1 + 16, boxY1 + 20,
            ...
            vga_front.read_bitmapW(keyFieldArea.left_edge, keyFieldArea.top_edge,
                                   ...
                                   keyFieldArea.top_edge + keyFieldArea.height - 1, keyFieldArea.bitmap_ptr());
