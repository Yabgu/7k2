...
#define MAX_BROWSE_DISP_REC                                                    \
  4 // max. no. of records can be displayed in the saved game browser
...
void GameFileArray::init(const char *path, const char *extStr) {
  ...
  if (!has_read_hall_of_fame) // only read once, GameFileArray::init() is called
                              // every time the load/save game menu is brought
                              // up.
  {
    read_hall_of_fame();
...
int GameFileArray::menu(int actionMode, int *recno) {
  ...
  pre_game = 0;
  if (actionMode == 3) // actionMode = 3 means load game from main menu, so
                       // action_mode is still two
  {
    action_mode = 2;
    pre_game = 1;
  }
  ...
  if (action_mode == 2 && size() == 0) {
    box.msg(text_game_menu
                .str_none_save_game()); // "You haven't saved any games yet." );
    return 0;
  }
...
#define SLOT_Y2(n) (by + SLOT_Y_SPACING * (n) + 110 + SLOT_HEIGHT - 1)
  ...
  int pageUpX1 =
      bx + (LSCROLL_X2 + LSCROLL_X1 - ((Bitmap *)arrowBitmap)->get_width()) / 2;
  int pageUpY1 =
      by +
      (LSCROLL_Y2 + LSCROLL_Y1 - ((Bitmap *)arrowBitmap)->get_height()) / 2;
  int pageUpX2 = pageUpX1 + ((Bitmap *)arrowBitmap)->get_width() - 1;
  int pageUpY2 = pageUpY1 + ((Bitmap *)arrowBitmap)->get_height() - 1;
  int pageDownX1 =
      bx + (RSCROLL_X2 + RSCROLL_X1 - ((Bitmap *)arrowBitmap)->get_width()) / 2;
  int pageDownY1 =
      by +
      (RSCROLL_Y2 + RSCROLL_Y1 - ((Bitmap *)arrowBitmap)->get_height()) / 2;
  ...
  Blob2DW
      browseArea[MAX_BROWSE_DISP_REC]; // save bitmap of save game slot areas
  ...
  if (!pre_game) {
    startButton.create_text(bx + BUTTON2_X1, by + BUTTON2_Y1, bx + BUTTON2_X2,
                            by + BUTTON2_Y2,
                            action_mode == 1 ? text_game_menu.str_save()
                                             : text_game_menu.str_load());
    if (action_mode == 1)
      saveNewButton.create_text(bx + BUTTON3_X1, by + BUTTON3_Y1,
                                bx + BUTTON3_X2, by + BUTTON3_Y2,
                                text_game_menu.str_save_new());
    delButton.create_text(bx + BUTTON7_X1, by + BUTTON7_Y1, bx + BUTTON7_X2,
                          by + BUTTON7_Y2, text_game_menu.str_delete_save());
    cancelButton.create_text(bx + BUTTON4_X1, by + BUTTON4_Y1, bx + BUTTON4_X2,
                             by + BUTTON4_Y2, text_game_menu.str_cancel());
  }

  //	int i;
  int page, maxPage;
  maxPage = (size() - minRecno + 1 + MAX_BROWSE_DISP_REC - 1) /
            MAX_BROWSE_DISP_REC; // divide but round up
  ...
  while (1) {
    ...
    if (refreshFlag) {
      if (refreshFlag & LSOPTION_PAGE) {
        ...
        if (pre_game) {
          ...
          str += "CHOOSE2.ICN";
          imageFile.file_open(str);
          vga.active_buf->put_large_bitmap_trans(bx, by, &imageFile,
                                                 colorRemapTable);
        }
        ...
        if (action_mode == 1)
          font_bold_black.center_put(
              bx + SCROLL_SHEET_X1, by + SCROLL_SHEET_Y1, bx + SCROLL_SHEET_X2,
              SLOT_Y1(0), text_game_menu.str_save_game()); // "Save Game" );
        else if (action_mode == 2)
          font_bold_black.center_put(
              bx + SCROLL_SHEET_X1, by + SCROLL_SHEET_Y1, bx + SCROLL_SHEET_X2,
              SLOT_Y1(0), text_game_menu.str_load_game()); // "Load Game" );

        // display save, save new/ load, delete cancel button

        if (pre_game) {
          if (action_mode == 1) {
            font_thin_black.center_put(bx + BUTTON2_X1, by + BUTTON2_Y1,
                                       bx + BUTTON2_X2, by + BUTTON2_Y2,
                                       text_game_menu.str_save()); // "Save" );
            font_thin_black.center_put(
                bx + BUTTON3_X1, by + BUTTON3_Y1, bx + BUTTON3_X2,
                by + BUTTON3_Y2,
                text_game_menu.str_save_new()); // "Save New" );
          } else if (action_mode == 2) {
            font_thin_black.center_put(bx + BUTTON2_X1, by + BUTTON2_Y1,
                                       bx + BUTTON2_X2, by + BUTTON2_Y2,
                                       text_game_menu.str_load()); // "Load" );
          }
          font_thin_black.center_put(
              bx + BUTTON4_X1, by + BUTTON4_Y1, bx + BUTTON4_X2,
              by + BUTTON4_Y2, text_game_menu.str_cancel()); // "Cancel" );
          font_thin_black.center_put(
              bx + BUTTON7_X1, by + BUTTON7_Y1, bx + BUTTON7_X2,
              by + BUTTON7_Y2, text_game_menu.str_delete_save()); // "Delete" );
        } else {
          startButton.paint();
          if (action_mode == 1)
            saveNewButton.paint();
        ...
        for (i = 0; i < MAX_BROWSE_DISP_REC; ++i) {
          ...
          vga.active_buf->read_bitmapW(SLOT_X1(i), SLOT_Y1(i), SLOT_X2(i),
                                       SLOT_Y2(i), browseArea[i].bitmap_ptr());
        }

        // capture page up area
        pageUpArea.clear();
        pageUpArea.resize(pageUpX1, pageUpY1,
                          ((Bitmap *)arrowBitmap)->get_width(),
                          ((Bitmap *)arrowBitmap)->get_height());
        vga.active_buf->read_bitmapW(pageUpX1, pageUpY1, pageUpX2, pageUpY2,
                                     pageUpArea.bitmap_ptr());
        pageDownArea.clear();
        pageDownArea.resize(pageDownX1, pageDownY1,
                            ((Bitmap *)arrowBitmap)->get_width(),
                            ((Bitmap *)arrowBitmap)->get_height());
        vga.active_buf->read_bitmapW(pageDownX1, pageDownY1, pageDownX2,
                                     pageDownY2, pageDownArea.bitmap_ptr());

        // capture page no area
        pageNoArea.clear();
        pageNoArea.resize(pageNoX1, pageNoY1, pageNoX2 - pageNoX1 + 1,
                          pageNoY2 - pageNoY1 + 1);
        vga.active_buf->read_bitmapW(pageNoX1, pageNoY1, pageNoX2, pageNoY2,
                                     pageNoArea.bitmap_ptr());
      ...
      for (i = 0; i < MAX_BROWSE_DISP_REC; ++i) {
        if (refreshFlag & LSOPTION_SLOT(i)) {
          ...
          mouse.hide_area(browseSlotX1, browseSlotY1, browseSlotX2,
                          browseSlotY2);

          // draw save bitmap area
          vga_front.put_bitmapW_fast(browseSlotX1, browseSlotY1,
                                     browseArea[i].bitmap_ptr());

          // draw slot content
          int rec = page * MAX_BROWSE_DISP_REC + i + minRecno;
          if (rec == 0) {
            font_bold_black.center_put(
                browseSlotX1, browseSlotY1, browseSlotX2, browseSlotY2,
                text_game_menu.str_empty_game_slot()); // "Empty Game Slot" );
          ...
          if (rec == browse_recno) {
            // draw black frame
            vga_front.rect(browseSlotX1, browseSlotY1, browseSlotX2,
                           browseSlotY2, 2, V_BLACK);
          }
      ...
      if (refreshFlag & LSOPTION_PAGE_NO) {
        ...
        //				str += "/";
        //				str += maxPage;
        vga.active_buf->put_bitmapW(pageNoX1, pageNoY1,
                                    pageNoArea.bitmap_ptr());
        font_snds.center_put(pageNoX1, pageNoY1, pageNoX2, pageNoY2,
                             text_game_menu.str_page_str(page + 1, maxPage));
        mouse.show_area();
        ...
        if (page > 0)
          ...
          vga.active_buf->put_bitmapW(pageUpX1, pageUpY1,
                                      pageUpArea.bitmap_ptr());
        mouse.show_area();

        mouse.hide_area(pageDownX1, pageDownY1, pageDownX2, pageDownY2);
        if (page < maxPage - 1)
          vga.active_buf->put_bitmap_trans_decompress(pageDownX1, pageDownY1,
                                                      arrowBitmap);
        else
          vga.active_buf->put_bitmapW(pageDownX1, pageDownY1,
                                      pageDownArea.bitmap_ptr());
        mouse.show_area();
      }
    ...
    for (i = 0; i < MAX_BROWSE_DISP_REC; ++i) {
      ...
      if (mouse.double_click(browseSlotX1, browseSlotY1, browseSlotX2,
                             browseSlotY2)) {
      ...
      } else if (mouse.single_click(browseSlotX1, browseSlotY1, browseSlotX2,
                                    browseSlotY2)) {
        // click on game slot
        if (browse_recno != rec) {
          // refresh old slot
    ...
    if (page > 0 && mouse.any_click(bx + LSCROLL_X1, by + LSCROLL_Y1,
                                    bx + LSCROLL_X2, by + LSCROLL_Y2)) {
      page--;
      refreshFlag |= LSOPTION_ALL_SLOTS | LSOPTION_SCROLL | LSOPTION_PAGE_NO;
    }

    if (page < maxPage - 1 &&
        mouse.any_click(bx + RSCROLL_X1, by + RSCROLL_Y1, bx + RSCROLL_X2,
                        by + RSCROLL_Y2)) {
      page++;
      refreshFlag |= LSOPTION_ALL_SLOTS | LSOPTION_SCROLL | LSOPTION_PAGE_NO;
    }
    ...
    if ((pre_game ? mouse.single_click(bx + BUTTON4_X1, by + BUTTON4_Y1,
                                       bx + BUTTON4_X2, by + BUTTON4_Y2)
                  : cancelButton.detect()) ||
        mouse.key_code == KEY_ESC ||
        mouse.any_click(RIGHT_BUTTON) >
            0) // also when ESC key is pressed or right button
    {
      ...
      break; // break while(1)
               ...
               (pre_game ? mouse.single_click(bx + BUTTON2_X1, by + BUTTON2_Y1,
                                              bx + BUTTON2_X2, by + BUTTON2_Y2)
                         : startButton.detect())) {
      // save / load button
      refreshFlag = LSOPTION_ALL;
      ...
      if (retFlag != 0) {
        ...
        break;
               ...
               (pre_game ? mouse.single_click(bx + BUTTON3_X1, by + BUTTON3_Y1,
                                              bx + BUTTON3_X2, by + BUTTON3_Y2)
                         : saveNewButton.detect())) {
      // save new button
      refreshFlag = LSOPTION_ALL;
      ...
      if (retFlag != 0) {
        ...
        break;
               ...
               (pre_game ? mouse.single_click(bx + BUTTON7_X1, by + BUTTON7_Y1,
                                              bx + BUTTON7_X2, by + BUTTON7_Y2)
                         : delButton.detect())) {
      ...
      if (browse_recno != 0) // cannot del save game slot
      {
        ...
        //				if( browse_top_recno >
        //size()-MAX_BROWSE_DISP_REC+1) 					browse_top_recno =
        //size()-MAX_BROWSE_DISP_REC+1; 				if( browse_top_recno < minRecno )
        //					browse_top_recno = minRecno;
        ...
        if (action_mode == 2 && size() == 0) {
          box.msg(text_game_menu.str_none_save_game()); // "You haven't saved
                                                        // any games yet." );
          return 0;
        }
      } else {
        box.msg(text_game_menu
                    .str_cannot_delete_slot()); // "Cannot delete this slot");
      }
    }
  ...
  /*
          ...
          for( int i=browse_top_recno ; i<=lastRec ; i++, y+=BROWSE_REC_HEIGHT )
          {
                  if( i==0 )
                  {
                          ...
                          font_bible.center_put( x, y, x+BROWSE_REC_WIDTH-1,
     y+BROWSE_REC_HEIGHT-1, "Empty Game Slot" );
                  }
                  else
                  {
                  ...
                  if( i == browse_recno )
                  {
                          ...
                          vga_front.put_bitmap_trans_decompress( x, y,
...
void GameFile::disp_info(int x, int y) {
  //	if( race_id > 0 )
  //		vga_front.put_bitmap(x+10, y+10,	unit_res[
  //race_res[race_id]->infantry_unit_id ]->king_icon_ptr); 	else
  //		vga_front.put_bitmap(x+10, y+10,	unit_res[
  //monster_res[-race_id]->unit_id ]->king_icon_ptr); 	x+=60;
  ...
  font_bold_black.put(x + 24, y + 10,
                      text_game_menu.str_king_title(race_id, player_name));
  ...
  misc.extract_file_name(
      shortFileName,
      file_name); // get the file name only from a full path string
                  //	str += shortFileName;
  ...
  //	str += date.date_str(date.julian(sysTime.wYear,
  //sysTime.wMonth,sysTime.wDay), 1); 	str += " "; 	str += date.time_str(
  //sysTime.wHour * 100 + sysTime.wMinute ); 	font_small.put( x+380, y+32, str );
  x2 = font_small.put(x + 380, y + 32, text_game_menu.str_file_date());
  x2 = font_small.put(
      x2, y + 32,
      date.date_str(date.julian(sysTime.wYear, sysTime.wMonth, sysTime.wDay),
                    1));
  x2 = font_small.put(x2 + 6, y + 32,
                      date.time_str(sysTime.wHour * 100 + sysTime.wMinute));
}
...
int GameFileArray::process_action(int saveNew) {
  ...
  if (action_mode == 1) {
    ...
    {
      if (!box.ask(text_game_menu
                       .str_ask_overwrite())) // "It will overwrite the existing
                                              // saved game, proceed ?" ) )
        return 0;
  ...
  //----------- load game -------------//
    ...
    if (!sys.debug_session &&
        gameFile->scenario_cheat_flag < sys.scenario_cheat_flag) {
      box.msg(
          text_game_menu
              .str_editor_cant_load()); // "Cannot load in scenario editor" );
      return 0;
    }
    ...
    if (!pre_game && (game.is_campaign_mode() || gameFile->campaign_id)) {
      ...
      sys.signal_exit_flag =
          2; // quit to main menu and it will load game automatically
      return 1;
    }
    // ###### end Gilbert 9/4 ########//
...
int GameFileArray::save_new_game(const char *fileName) {
  ...
  if (fileName) // has a filename before extension
  {
    ...
    if (!strchr(fileName, '.')) {
      strcat(gameFile.file_name,
             strchr(save_default_ext, '.')); // remove '*' of "*.SAV"
    }

    //----- check for overwriting an existing file ----//

    for (gameFileRecno = 1; gameFileRecno <= game_file_array.size();
         gameFileRecno++) {
      gameFilePtr = game_file_array[gameFileRecno];

      // ###### begin Gilbert 31/10 ######//
      if (stricmp(gameFilePtr->file_name, gameFile.file_name) ==
          0) // if this file name already exist
      // ###### end Gilbert 31/10 ######//
      {
        addFlag = 0;
        break;
    ...
    // strcpy( gameFile.file_name, fileName );		// now use gameFile.file_name
    // instead
  } else {
    load_all_game_header(save_default_dir, save_default_ext);
    ...
    gameFile.set_file_name(
        save_default_dir,
        strchr(
            save_default_ext,
            '.')); // give it a new game_file_name based on current group name
  }
  ...
  if (gameFile.save_game(save_default_dir, NULL,
                         NULL)) // use gameFile.file_name
  {
    strcpy(last_file_name, gameFile.file_name);
...
int GameFileArray::write_hall_of_fame() {
  ...
  if (!misc.path_cat(full_path, sys.dir_config, HALL_OF_FAME_FILE_NAME,
                     MAX_PATH)) {
  ...
  if (rc)
    rc =
        (file.file_write(hall_fame_array[0],
                         sizeof(HallFame) * HALL_FAME_NUM) &&
...
int GameFileArray::read_hall_of_fame() {
  ...
  if (rc)
    rc =
        (file.file_read(hall_fame_array[0], sizeof(HallFame) * HALL_FAME_NUM) &&
  ...
  for (i = 1; i <= gameDir.size(); i++) {
    ...
    if (file.file_open(str, 1, 1) // last 1=allow varying read & write size
      ...
      strcpy(gameFile.file_name,
             file.file_name); // in case that the name may be different
#ifndef NO_WINDOWS
...
int GameFile::ask_desc() {
  ...
  GetA descTextBox;
  descTextBox.init(boxX1 + 16, boxY1 + 50, boxX2 - 12, gameDesc,
                   SAVE_GAME_DESC_LEN, &font_san, 0, 0);

  keyFieldArea.resize(descTextBox.x, descTextBox.y,
                      descTextBox.x_limit - descTextBox.x + 1,
                      descTextBox.font_ptr->max_font_height);
  ...
  while (1) {
    ...
    if (refreshFlag) {
      ...
      font_bold_black.put(
          boxX1 + 16, boxY1 + 20,
          text_game_menu.str_ask_desc_save()); // "Description of save game");

      // ------ set get field background -------//

      mouse.hide_area(boxX1, boxY1, boxX2, boxY2);
      vga_front.read_bitmapW(keyFieldArea.left_edge, keyFieldArea.top_edge,
                             keyFieldArea.left_edge + keyFieldArea.width - 1,
                             keyFieldArea.top_edge + keyFieldArea.height - 1,
...
static int last_game_recno() {
  ...
  for (i = game_file_array.size(); i > 0; i--) {
    if (strcmp(game_file_array[i]->file_name, game_file_array.last_file_name) ==
        0)
      return i;
  }
...
static void disp_scroll_bar_func(SlideVBar *scroll, int) {
  ...
  vga_front.bar(scroll->scrn_x1, rectTop, scroll->scrn_x2, rectBottom,
                VGA_YELLOW + 1);
  if (rectBottom - rectTop > 6) {
    vga_front.d3_panel_up(scroll->scrn_x1, rectTop, scroll->scrn_x2, rectBottom,
                          2, 0);
  }
}
