...
void CampaignEastWest::plot_a7_create_game() {
  ...
  plot_nation_recno1 = nation_array.new_nation(
      NATION_HUMAN, NATION_AI, raceId, nation_array.random_unused_color());

  Nation *rebelNation = nation_array[plot_nation_recno1];

  // ----- give tech --------//

  tech_res[TECH_SPU_STRUCTURE(raceId)]->set_nation_tech_level(
      plot_nation_recno1, 1);
  tech_res[TECH_SPU_BERSERK(raceId)]->set_nation_tech_level(plot_nation_recno1,
                                                            1);
    ...
    if (firmPtr->cast_to_FirmFort()) {
      ...
      for (int s = firmFort->soldier_count; s > 0; --s) {
        ...
        if (soldierPtr->race_id == raceId) {
          soldierPtr->change_unit_id(
              race_res[soldierPtr->race_id]->special_unit_id,
      ...
      int s;
      for (s = MAX_TRAINEE - 2 - misc.random(2);
           s > 0 && firmSpecial->scan_best_recruitable_town(bestTown) > 0;
           --s) {
        firmSpecial->recruit_trainee(bestTown[0], TRAIN_UNIT_SPECIAL,
                                     COMMAND_AUTO);
      }
  ...
  rebelNation->get_relation(plot_enemy_nation_recno)
      ->set_ai_never_consider(neverConsiderArray1);

  //--- hard-wire the rebel's relation with the player ---//

  static short neverConsiderArray2[] = {TALK_END_ALLIANCE_TREATY,
                                        TALK_END_TRADE_TREATY, TALK_DECLARE_WAR,
                                        TALK_SURRENDER, 0};

  rebelNation->get_relation(nation_array.player_recno)
      ->set_ai_never_consider(neverConsiderArray2);

  // ------- rebel nation is allied with the player initially --------//

  rebelNation->set_relation_status(nation_array.player_recno,
                                   RELATION_ALLIANCE);
}
...
void CampaignEastWest::plot_a7_next_day() {
  ...
  //--- the rebel will propose an alliance treaty to the player if reputation >=
  //50 -----//
  ...
  if (playerNation->reputation >= 50.f &&
    ...
    talk_res.ai_send_talk_msg(playerNation->nation_recno, plot_nation_recno1,
                              TALK_SURRENDER);
