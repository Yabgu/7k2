...
void CampaignEastWest::plot_a7_create_game()
{
    ...
    for (int firmRecno = 1; firmRecno <= firm_array.size(); ++firmRecno)
    {
        ...
        else if (firmPtr->cast_to_FirmSpecial())
        {
            ...
            int s;
            for (s = MAX_TRAINEE - 2 - misc.random(2); s > 0 && firmSpecial->scan_best_recruitable_town(bestTown) > 0;
                 --s)
            {
    ...
    static short neverConsiderArray2[] = {TALK_END_ALLIANCE_TREATY, TALK_END_TRADE_TREATY, TALK_DECLARE_WAR,
                                          TALK_SURRENDER, 0};
...
void CampaignEastWest::plot_a7_next_day()
{
    ...
    if (playerNation->reputation >= 50.f && nationRelation->status == RELATION_ALLIANCE)
    {
