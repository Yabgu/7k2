...
void World::spread_fire(Weather &w) {
  ...
  // -------- normalize wind_speed between -WIND_SPREADFIRE*SPREAD_RATE to
  // +WIND_SPREADFIRE*SPREAD_RATE -------
  int windCos = int(w.wind_speed() * cos(w.wind_direct_rad()) / 100.0 *
                    SPREAD_RATE * WIND_SPREADFIRE);
  int windSin = int(w.wind_speed() * sin(w.wind_direct_rad()) / 100.0 *
                    SPREAD_RATE * WIND_SPREADFIRE);
  char rainSnowReduction = 0;

  rainSnowReduction =
      (w.rain_scale() > 0 || w.snow_scale() > 0)
          ? RAIN_SNOW_REDUCTION + (w.rain_scale() + w.snow_scale()) / 4
          : 0;
  ...
  for (y = scan_fire_y; y < max_y_loc; y += SCAN_FIRE_DIST) {
    ...
    for (x = scan_fire_x; x < max_x_loc;
         x += SCAN_FIRE_DIST, locPtr += SCAN_FIRE_DIST) {
      ...
      if (fireValue > 0) {
        ...
        // ------- burn units ---------//
        else if (locPtr->unit_recno(UNIT_LAND)) {
          targetUnit = unit_array[locPtr->unit_recno(UNIT_LAND)];

          // ######## begin Gilbert 17/3 ###### //
          targetUnit->being_attack_hit(NULL, 2.0f * flameDamage);
          // ######## end Gilbert 17/3 ###### //
          ...
          // ######## end Gilbert 17/3 ###### //
                   ...
                   firm_res[firm_array[locPtr->firm_recno()]->firm_id]
                       ->buildable) {
...
//### begin alex 6/8 ###//
          ...
          if (targetFirm->hit_points <= 0) {
            ...
            // ###### begin Gilbert 29/5 ########//
            se_res.sound(targetFirm->center_x, targetFirm->center_y, 1, 'F',
                         targetFirm->firm_id, "DIE");
            // ###### end Gilbert 29/5 ########//
        ...
        if (SPREAD_RATE > 0) {
          ...
          if (y < max_y_loc - 1 &&
              (sidePtr = get_loc(x, y + 1))->fire_src() > 0 &&
              sidePtr->fire_str() <= 0) {
            sidePtr->add_fire_str(bound_zero(char(SPREAD_RATE - windCos)));
          }
          ...
          if (x < max_x_loc - 1 &&
              (sidePtr = get_loc(x + 1, y))->fire_src() > 0 &&
              sidePtr->fire_str() <= 0) {
