...
void World::spread_fire(Weather &w)
{
    ...
    rainSnowReduction =
        (w.rain_scale() > 0 || w.snow_scale() > 0) ? RAIN_SNOW_REDUCTION + (w.rain_scale() + w.snow_scale()) / 4 : 0;
...
//### begin alex 6/8 ###//
                    ...
                    if (targetFirm->hit_points <= 0)
                    {
                        ...
                        // ###### begin Gilbert 29/5 ########//
                        se_res.sound(targetFirm->center_x, targetFirm->center_y, 1, 'F', targetFirm->firm_id, "DIE");
                        // ###### end Gilbert 29/5 ########//
                ...
                if (SPREAD_RATE > 0)
                {
                    ...
                    if (y < max_y_loc - 1 && (sidePtr = get_loc(x, y + 1))->fire_src() > 0 && sidePtr->fire_str() <= 0)
                    {
                        sidePtr->add_fire_str(bound_zero(char(SPREAD_RATE - windCos)));
                    }

                    // spread of east square
                    if (x < max_x_loc - 1 && (sidePtr = get_loc(x + 1, y))->fire_src() > 0 && sidePtr->fire_str() <= 0)
                    {
            ...
            else
            {
                ...
                if (flammability >= -30 && flammability < 50 && misc.random(100) < RESTORE_RATE)
                    flammability++;
            }
