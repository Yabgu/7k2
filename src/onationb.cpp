...
void NationBase::deinit()
{
    ...
    for (i = spy_array.size(); i > 0; i--)
    {
        ...
        if (spyPtr->true_nation_recno == nation_recno) // drop spy identities of spies in towns, firms and mobile
                                                       // ones
        ...
        err_when(spyPtr->true_nation_recno == nation_recno || // there should be no more spies associated
                                                              // with this nation
...
void NationBase::init_relation(int relationNationRecno)
{
    ...
    if (is_ai() && nation_array[relationNationRecno]->is_ai()) // AI has contact with each other in the
                                                               // beginning of the game.
        nationRelation->has_contact = 1;
    else
        nationRelation->has_contact =
            relationNationRecno == nation_recno || config.explore_whole_map; // if the map is blackened out, no
    ...
    nationRelation->ai_relation_level =
        RELATION_NEUTRAL * RELATION_LEVEL_PER_STATUS + misc.random(40) - 20; // -20 to +20 randomness
...
void NationBase::close_all_firm()
{
    ...
    {
        if (!firm_array.is_deleted(i) && firm_array[i]->nation_recno == nation_recno)
        {
...
void NationBase::hand_over_to(int handoverNationRecno)
{
    ...
    for (i = unit_array.size(); i > 0; i--)
    {
        if (unit_array.is_deleted(i))
        {
            ...
            if (!unit_array.is_truly_deleted(i) && unit_array[i]->nation_recno == nation_recno)
            {
                unit_array.die(i);
            }
    ...
    for (i = town_array.size(); i > 0; i--)
    {
        if (!town_array.is_deleted(i) && town_array[i]->nation_recno == nation_recno)
        {
    ...
    {
        if (!bullet_array.is_deleted(i) && bullet_array[i]->nation_recno == nation_recno)
        {
    ...
    for (i = spy_array.size(); i > 0; i--)
    {
        ...
        err_when(spyPtr->cloaked_nation_recno == nation_recno); // there should be no units in this nation after the
...
void NationBase::set_king(int kingUnitRecno, int firstKing)
{
    ...
    if (firstKing)
    {
        ...
        if (!nationName[0])
        {
            //			String str;
            //			str  =
            // nation_array.get_custom_king_name(nation_recno);
            // if(kingUnit->race_id > 0) 				str += "'s
            // Kingdom"; 			else str
            // +=
            // "'s Kwyzan";
            ...
            firstWord[i] = '\0';
            // ########## end Gilbert 26/6 ########//
            nation_array.set_custom_nation_name(nation_recno, text_unit.str_unit_nation(kingUnit->race_id, firstWord));
        }
    }
...
char *NationBase::nation_name()
{
    ...
    ////
    // 1-get the first word of the name only
    // nation_name_str[NATION_NAME_LEN]=NULL;
    //#elif(defined(FRENCH))
    ...
    // 1-get the first word of the name only
    // nation_name_str[NATION_NAME_LEN]=NULL;
    //	strncat( nation_name_str, "'s ", NATION_NAME_LEN );		// 1-get
    // the
    // first word of the name only 	nation_name_str[NATION_NAME_LEN]=NULL;
...
void NationBase::set_relation_status(short nationRecno, char newStatus, char recursiveCall)
{
    ...
    */
    ...
    if (!recursiveCall && nationRelation->status == RELATION_TENSE && newStatus == RELATION_HOSTILE &&
        info.game_date < nationRelation->last_change_status_date + 5) // 5 days after the cease-fire, the nation will
                                                                      // remain cease-fire
    {
        return;
    }
    ...
    if (newRelationLevel < nationRelation->ai_relation_level) // only set it when the new value is
    ...
    if (newStatus == RELATION_ALLIANCE || newStatus == RELATION_FRIENDLY || newStatus == RELATION_TENSE)
    {
...
//							  			  with
// this
// nation. If it's an alliance treaty,
...
void NationBase::being_attacked(BaseObj *attackerObj)
{
    ...
    if (nationRelation->status != RELATION_HOSTILE)
    {
        ...
        nationRelation->started_war_on_us_count++; // how many times this nation has started a
                                                   // war with us, the more the times the
                                                   // worse this nation is.

        if (nationRelation->status == RELATION_ALLIANCE || nationRelation->status == RELATION_FRIENDLY)
        {
            attackNation->end_treaty(nation_recno,
...
int NationBase::trade_rating(int nationRecno)
{
    ...
    int tradeRating2 =
        50 * (int)nation_array[nationRecno]->get_relation(nation_recno)->last_year_import[IMPORT_TOTAL] /
...
int NationBase::total_tech_level(int unitClass)
{
    ...
    for (int i = 1; i <= tech_res.tech_count; i++)
    {
        ...
        if (techLevel > 0)
        {
            if (!unitClass || (techInfo->unit_id && unit_res[techInfo->unit_id]->unit_class == unitClass))
            {
...
//								  0 - all races, when
// a
// Caravan is killed, 0 will 								  be
// passed, the loyalty of all races will be decreased.
//
// <int> isAttacker     - 1 if the nation is the offensive attacker.
//								  0 if the nation that
// suffers
// the atttack.
//
// [int] killedCount	- the number of civilian units killed.
//							  this can be higher than 1
// for
// wagons. 							  (default: 1)
//
...
void NationBase::succeed_king(int kingUnitRecno)
{
    ...
    if (newKing->spy_recno)
    {
        ...
        err_when(newKing->rank_id == RANK_KING && newKing->spy_recno); // it can't still be a spy when it asscends as
                                                                       // the king
    }
}
...
int NationBase::has_king_successor_candidate()
{
    ...
    for (i = unit_array.size(); i > 0; i--)
    {
        ...
        if (unitPtr->nation_recno == nation_recno && unitPtr->can_succeed_king() && unitPtr->rank_id != RANK_KING &&
            !unitPtr->is_civilian())
        {
            return 1;
        }
...
int NationBase::base_town_count_in_region(int regionId)
{
    ...
    if (regionStatId)
    {
        return region_array.get_region_stat2(regionStatId)->base_town_nation_count_array[nation_recno - 1];
    }
...
{
    return (int)cash / 300 + (int)true_income_365days() / 2 + (int)true_profit_365days();
}
...
int NationBase::get_overall_rating()
{
    ...
    {
        return 10 * population_rating / 500 + 45 * military_rating / 200 + 45 * economic_rating / 10000;
    }
}
...
void NationBase::repair_all_building(int flag, char remoteAction)
{
    ...
    for (int firmRecno = firm_array.size(); firmRecno > 0; firmRecno--)
    {
        ...
        if (flag && !firmPtr->repair_flag && firmPtr->hit_points < firmPtr->max_hit_points() ||
...
void NationBase::set_auto_retreat(int hitPointPercentage)
{
    ...
    for (int unitRecno = unit_array.size(); unitRecno > 0; unitRecno--)
    {
        ...
        if (unitPtr->nation_recno == nation_recno && unitPtr->home_camp_firm_recno && unitPtr->rank_id == RANK_GENERAL)
        {
...
//											for
// setting
// ai_never_consider[]. The
// latest element of the array is 0, indicating its end.
//
...
float NationBase::research_progress()
{
    ...
    return (float)tech_res[research_tech_id]->nation_research_progress_array[nation_recno - 1] +
