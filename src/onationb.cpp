...
static int succeed_king_loyalty_change(int thisRaceId, int newKingRaceId,
                                       int oldKingRaceId);

//--------- Begin of function NationBase::NationBase --------//

NationBase::NationBase() { memset(this, 0, sizeof(NationBase)); }
...
NationBase::~NationBase() {
  ...
  err_when(nation_recno); // deinit() must be called first before this
...
//
void NationBase::init(int nationType, int raceId, int colorSchemeId,
                      DWORD playerId) {
  //------------- set vars ---------------//
  ...
  //### begin alex 23/9 ###//
  if (remote.is_enable() && nation_recno && !is_ai() &&
      misc.is_file_exist("TECHGOD.SYS")) {
...
void NationBase::deinit() {
  ...
  for (i = spy_array.size(); i > 0; i--) {
    ...
    if (spyPtr->true_nation_recno ==
        nation_recno) // drop spy identities of spies in towns, firms and mobile
                      // ones
    ...
    err_when(spyPtr->true_nation_recno ==
                 nation_recno || // there should be no more spies associated
                                 // with this nation
  ...
  if (!sys.signal_exit_flag) {
    if (info.default_viewing_nation_recno == nation_recno) {
      ...
      sys.set_view_mode(MODE_NORMAL); // it will set viewing_nation_recno to
                                      // default_viewing_nation_recno
...
void NationBase::init_relation(int relationNationRecno) {
  ...
  set_relation_should_attack(relationNationRecno,
                             relationNationRecno != nation_recno, COMMAND_AUTO);

  if (is_ai() && nation_array[relationNationRecno]
                     ->is_ai()) // AI has contact with each other in the
                                // beginning of the game.
    nationRelation->has_contact = 1;
  else
    nationRelation->has_contact =
        relationNationRecno == nation_recno ||
        config.explore_whole_map; // if the map is blackened out, no contact in
                                  // the beginning

  nationRelation->trade_treaty = relationNationRecno == nation_recno;

  nationRelation->status = RELATION_NEUTRAL;
  nationRelation->ai_relation_level =
      RELATION_NEUTRAL * RELATION_LEVEL_PER_STATUS + misc.random(40) -
      20; // -20 to +20 randomness
  nationRelation->last_change_status_date = info.game_date;

  if (relationNationRecno == nation_recno) // own nation
    relation_status_array[relationNationRecno - 1] =
        RELATION_ALLIANCE; // for facilitating searching
  else
...
void NationBase::deinit_all_unit() {
  ...
  for (i = unit_array.size(); i > 0; i--) {
    if (unit_array.is_deleted(i)) {
      unitPtr = (Unit *)unit_array.get_ptr(
          i); // this unit is dying, so is_deleted() return 1;
  ...
  for (i = unit_array.size(); i > 0; i--) {
    ...
    err_when(unitPtr->nation_recno ==
             nation_recno); // they should have been all deleted.
  }
}
...
void NationBase::hand_over_to(int handoverNationRecno) {
  ...
  //--- hand over units (should hand over units first as you cannot change a
  //firm's nation without changing the nation of the overseer there ---//
  ...
  for (i = spy_array.size(); i > 0; i--) {
    ...
    err_when(spyPtr->cloaked_nation_recno ==
             nation_recno); // there should be no units in this nation after the
                            // above deinitialization
...
void NationBase::set_king(int kingUnitRecno, int firstKing) {
  ...
  kingUnit->stop_order(); // clear the existing order, as there might be an
                          // assigning to firm/town order. But kings cannot be
                          // assigned to towns or firms as workers.

  //---------- king related vars ----------//

  const char *kingName = nation_array.get_custom_king_name(nation_recno);

  //-- if the custom king name hasn't been set or the first king has died and
  //now there is a new king succeeding the throne, set a new custom king name
  //--//
  ...
  if (firstKing) {
    ...
    if (!nationName[0]) {
      //			String str;
      //			str  =
      //nation_array.get_custom_king_name(nation_recno); 			if(kingUnit->race_id >
      //0) 				str += "'s Kingdom"; 			else 				str += "'s Kwyzan";
      //			nation_array.set_custom_nation_name(nation_recno,
      //str);
      ...
      for (; *nameStr == ' '; ++nameStr)
        ; // trim leading space
      for (i = 0; *nameStr && *nameStr != ' ' && i < config.PLAYER_NAME_LEN;
           ++i, ++nameStr)
        firstWord[i] = *nameStr;
      firstWord[i] = '\0';
      // ########## end Gilbert 26/6 ########//
      nation_array.set_custom_nation_name(
          nation_recno,
...
char *NationBase::nation_name() {
  ...
  //	strncat( nation_name_str, king_name(1), NATION_NAME_LEN );		//
  //1-get the first word of the name only 	nation_name_str[NATION_NAME_LEN]=NULL;
  //#elif(defined(FRENCH))
  ...
  //#else
  //	// German and US
  //	strncpy( nation_name_str, king_name(1), NATION_NAME_LEN );		//
  //1-get the first word of the name only 	nation_name_str[NATION_NAME_LEN]=NULL;
  //	strncat( nation_name_str, "'s ", NATION_NAME_LEN );		// 1-get the
  //first word of the name only 	nation_name_str[NATION_NAME_LEN]=NULL; 	strncat(
  //nation_name_str, translate.process("Kingdom"), NATION_NAME_LEN );
  //	nation_name_str[NATION_NAME_LEN]=NULL;
  //#endif

  strncpy(nation_name_str, text_unit.str_unit_nation(race_id, king_name(1)),
          NATION_NAME_LEN); // 1-get the first word of the name only
  nation_name_str[NATION_NAME_LEN] = '\0';

  return nation_name_str; // each name needs to have its own var as multiple
                          // nation names will be displayed at the same time in
                          // diplomatic talk choices
}
...
const char *NationBase::king_name(int firstWordOnly) {
  const char *kingName =
      nation_array.get_custom_king_name(nation_recno, firstWordOnly);
  ...
  if (king_unit_recno) {
    ...
    if (unitPtr->is_human())
      return race_res[unitPtr->race_id]->get_name(unitPtr->name_id,
                                                  firstWordOnly ? 1 : 0);
    else
...
char *NationBase::cash_str() {
  ...
  if (curProfit) {
    ...
    str += misc.format(abs(curProfit),
                       4); // format type 4 - no thousand separators
    str += ")";
  }
...
char *NationBase::food_str() {
  ...
  if (foodChange) {
    ...
    str += misc.format(abs(foodChange),
                       4); // format type 4 - no thousand separators
    str += ")";
  }
...
void NationBase::next_day() {
  ...
  if (!king_unit_recno) {
    if (info.game_date % 3 ==
        nation_recno % 3) // decrease 1 loyalty point every 3 days
  ...
  err_when(get_relation_status(nation_recno) !=
           RELATION_ALLIANCE); // relation to own nation must be alliance

  //------- validate attack_camp_array --------//

  for (int i = ((Nation *)this)->attack_camp_count - 1; i >= 0; i--) {
    firm_array[((Nation *)this)->attack_camp_array[i].firm_recno]; // error if
                                                                   // it is
                                                                   // deleted
  }
                           ...
                           unitPtr->rank_id != RANK_KING )
                  {
                          if( unitPtr->is_human() )
                          {
                                  if( unitPtr->unit_id == UNIT_WAGON )
                                          totalHumanCount1 +=
  unitPtr->cast_to_UnitWagon()->population; else totalHumanCount1++;
                          }
                          else
                                  totalMonsterCount++;
                  }
  ...
  firmCamp->soldier_array[j].is_monster() ) totalMonsterCount++;
                          }
                  }
...
void NationBase::next_month() {
  ...
  for (int i = 1; i <= nation_array.size(); i++) {
    ...
    //--- update good_relation_duration_rating ---//
      ...
      nationRelation->good_relation_duration_rating +=
          (float)0.2; // this is the monthly increase
  ...
  if (reputation < 15)
    change_reputation(god_res[GOD_VIKING]->nation_prayer_count(nation_recno) *
                      (15.0 / 12 / MAX_WORKER));
...
//0)
//
void NationBase::add_expense(int expenseType, float expenseAmt,
                             int fixedExpense) {
...
void NationBase::add_cheat(float cheatAmount) {
  ...
  } else // cheat less obviously, randomly add to one of the account at a random
         // amount //
  {
...
//
void NationBase::import_goods(int importType, int importNationRecno,
                              float importAmt) {
...
//                      	(default: 0)
//
void NationBase::set_relation_status(short nationRecno, char newStatus,
                                     char recursiveCall) {
  ...
  /*
          #ifdef DEBUG
          if( nation_array[nationRecno]->is_ai() && is_ai() )	// AI must
     terminate the treaty first before declaring war
          {
  ...
  */
  ...
  if (!recursiveCall && nationRelation->status == RELATION_TENSE &&
      newStatus == RELATION_HOSTILE &&
      info.game_date < nationRelation->last_change_status_date +
                           5) // 5 days after the cease-fire, the nation will
                              // remain cease-fire
  {
    return;
  }
  ...
  if (newRelationLevel <
      nationRelation->ai_relation_level) // only set it when the new value is
  ...
  /*
          ...
          if( newStatus == RELATION_ALLIANCE )
          {
                  ...
                  for( i=nation_array.size() ; i>0 ; i-- )
                  {
                          ...
                          //-- if we have contact with this nation and our ally
     doesn't, share the contact with it --//
...
void NationBase::set_relation_passable(short nationRecno, char status) {
  relation_passable_array[nationRecno - 1] =
      (relation_status_array[nationRecno - 1] >= status);
}
...
void NationBase::set_relation_should_attack(short nationRecno, char newValue,
                                            char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_NATION_SET_SHOULD_ATTACK, 3 * sizeof(short));
...
//							  			  with this
//nation. If it's an alliance treaty, 							  			  allied nations with break treaty with 							  			  this
//nation.
//
...
void NationBase::end_treaty(int withNationRecno, int newStatus) {
  ...
  if (newStatus <= RELATION_NEUTRAL) {
    ...
    withNation->get_relation(nation_recno)->good_relation_duration_rating =
        (float)0;
  }
...
void NationBase::set_trade_treaty(int nationRecno, char treatyFlag) {
  ...
  nation_array[nationRecno]->get_relation(nation_recno)->trade_treaty =
      treatyFlag;
}
...
void NationBase::being_attacked(BaseObj *attackerObj) {
  ...
  //--- if it is an accidential attack (e.g. bullets attack with spreading
  //damages) ---//
  ...
  if (nationRelation->status != RELATION_HOSTILE) {
    //--- if this nation (the one being attacked) has a higher than 0
    //reputation, the attacker's reputation will decrease ---//

    if (reputation > 0)
      attackNation->change_reputation(-reputation * 40 / 100);

    nationRelation
        ->started_war_on_us_count++; // how many times this nation has started a
                                     // war with us, the more the times the
                                     // worse this nation is.

    if (nationRelation->status == RELATION_ALLIANCE ||
        nationRelation->status == RELATION_FRIENDLY) {
      attackNation->end_treaty(
          nation_recno,
          RELATION_HOSTILE); // the attacking nation abruptly terminates the
                             // treaty with us, not we terminate the treaty with
                             // them, so attackNation->end_treaty() should be
                             // called instead of end_treaty()
    } else {
...
int NationBase::trade_rating(int nationRecno) {
  ...
  int tradeRating2 =
      50 *
          (int)nation_array[nationRecno]
              ->get_relation(nation_recno)
              ->last_year_import[IMPORT_TOTAL] /
          (int)(last_year_income + 1) +
      50 * (int)get_relation(nationRecno)->last_year_import[IMPORT_TOTAL] /
          (int)(last_year_expense + 1);
...
//								  0 - all races, when a
//Caravan is killed, 0 will 								  be passed, the loyalty of all races will be
//decreased.
//
// <int> isAttacker     - 1 if the nation is the offensive attacker.
//								  0 if the nation that suffers
//the atttack.
//
// [int] killedCount	- the number of civilian units killed.
//							  this can be higher than 1 for
//wagons. 							  (default: 1)
//
void NationBase::civilian_killed(int civilianRaceId, int isAttacker,
  ...
  if (isAttacker) {
    ...
    if (civilianRaceId == 0) // a caravan
      ...
      change_reputation(
          (float)-1 *
          killedCount); // a mobile civilian unit, a citizen in town, or a wagon
  }
  /*
          else
          {
                  change_all_people_loyalty((float)-0.6 * killedCount,
     civilianRaceId);

                  if( civilianRaceId==0 )				// a
     caravan change_reputation(-(float)2 * killedCount); else
...
void NationBase::succeed_king(int kingUnitRecno) {
  ...
  newKingLeadership =
      MAX(20, newKingLeadership); // give the king a minimum level of leadership
  ...
  for (i = unit_array.size(); i > 0; i--) {
    ...
    unitPtr->change_loyalty(
        loyaltyChange + succeed_king_loyalty_change(unitPtr->race_id,
                                                    newKing->race_id, race_id));
  }
  ...
  for (i = firm_array.size(); i > 0; i--) {
    ...
    if (firmPtr->cast_to_FirmCamp()) {
      ...
      for (int j = firmCamp->soldier_count - 1; j >= 0; j--, soldierPtr++) {
        ...
        soldierPtr->change_loyalty(
            loyaltyChange + succeed_king_loyalty_change(soldierPtr->race_id,
                                                        newKing->race_id,
                                                        race_id));
      }
    }
  ...
  for (i = town_array.size(); i > 0; i--) {
    ...
    if (townPtr->population > 0) {
      townPtr->change_loyalty((float)loyaltyChange +
                              succeed_king_loyalty_change(
                                  townPtr->race_id, newKing->race_id, race_id));
    }
  }

  //--- remove all messages using the old king's name as only one king's name
  ...
  if (newKing->spy_recno) {
    ...
    err_when(newKing->rank_id == RANK_KING &&
             newKing->spy_recno); // it can't still be a spy when it asscends as
                                  // the king
  }
}
...
int NationBase::has_king_successor_candidate() {
  ...
  for (i = unit_array.size(); i > 0; i--) {
    ...
    if (unitPtr->nation_recno == nation_recno && unitPtr->can_succeed_king() &&
        unitPtr->rank_id != RANK_KING && !unitPtr->is_civilian()) {
      return 1;
    }
  }
...
//
// [int] checkOnlyFlag : if true, do not call game.game_end (called by
// game.game_end)
//
...
int NationBase::base_town_count_in_region(int regionId) {
  ...
  } else if (region_array[regionId]->region_size <
...
//
int NationBase::get_population_rating() { return all_population(); }
...
int NationBase::get_economic_rating() {
  return (int)cash / 300 + (int)true_income_365days() / 2 +
...
int NationBase::get_overall_rating() {
  ...
  if (is_human()) {
    return 33 * population_rating / 500 + 33 * military_rating / 200 +
           33 * economic_rating / 10000;
  } else {
    return 10 * population_rating / 500 + 45 * military_rating / 200 +
...
int NationBase::goal_destroy_monster_achieved() {
  ...
  /*
                   ...
                   unit_res.mobile_monster_count == 0 )
          {
                  ...
                  //-- if this nation is the one that has destroyed most
     monsters, it wins, otherwise it loses --//
...
int NationBase::goal_economic_score_achieved() {
  ...
  info.set_rank_data(
      0); // 0-set all nations, not just those that have contact with us
...
void NationBase::repair_all_building(int flag, char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_NATION_REPAIR_ALL_BUILDING, 2 * sizeof(short));
  ...
  for (int townRecno = town_array.size(); townRecno > 0; townRecno--) {
    ...
    if (flag && !townPtr->repair_flag && townPtr->current_wall_level > 0 &&
            townPtr->hit_points < townPtr->max_hit_points() ||
...
void NationBase::set_auto_retreat(int hitPointPercentage) {
  ...
  for (int unitRecno = unit_array.size(); unitRecno > 0; unitRecno--) {
    ...
    if (unitPtr->nation_recno == nation_recno &&
        unitPtr->home_camp_firm_recno && unitPtr->rank_id == RANK_GENERAL) {
...
//											for setting
//ai_never_consider[]. The 											latest element of the array is 0, 											indicating its end.
//
...
void NationRelation::set_never_accept_until_date(int talkId, int dayCount) {
  int newDate = info.game_date +
                dayCount * (70 + misc.random(60)) / 100; // -30 to +30 random

  never_accept_until_date_array[talkId - 1] =
      MAX(never_accept_until_date_array[talkId - 1], newDate);
}
...
void NationBase::start_research(int techId, char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_NATION_RESEARCH,
                                                         2 * sizeof(short));
...
int NationBase::progress_research(float newProgress) {
  ...
  newProgress +=
      research_progress(); // newProgress is now the new research_progres

  if (newProgress >= 100) {
    set_research_progress((newProgress = 0.0f)); // reset the progress

    int newTechLevel =
        tech_res[research_tech_id]->get_nation_tech_level(nation_recno) + 1;
    ...
    tech_res[research_tech_id]->set_nation_tech_level(nation_recno,
                                                      newTechLevel);
...
float NationBase::research_progress() {
  ...
  return (float)tech_res[research_tech_id]
             ->nation_research_progress_array[nation_recno - 1] +
         fraction_research_progress;
}
...
void NationBase::set_research_progress(float newProgress) {
  ...
  tech_res[research_tech_id]->nation_research_progress_array[nation_recno - 1] =
      intProgress;
