...
void RawRes::load_all_info()
{
    ...
    for (i = 0; i < raw_count; i++)
    {
        ...
        rawInfo->map_icon_offset_x +=
            ...
            (-rawInfo->map_loc_width * ZOOM_LOC_X_WIDTH / 2 + -rawInfo->map_loc_height * ZOOM_LOC_Y_WIDTH / 2);
        rawInfo->map_icon_offset_y +=
            -rawInfo->map_loc_height * LOCATE_HEIGHT / 2 -
            (-rawInfo->map_loc_width * ZOOM_LOC_X_HEIGHT / 2 + -rawInfo->map_loc_height * ZOOM_LOC_Y_HEIGHT / 2);
    }
}
...
void RawRes::update_supply_firm()
{
    ...
    for (short firmRecno = firm_array.size(); firmRecno > 0; firmRecno--)
    {
        ...
        if (firmPtr->firm_id == FIRM_FACTORY)
        {
            ...
            if (firmFactory->product_raw_id && firmFactory->stock_qty > firmFactory->max_stock_qty / 5)
            {
        ...
        if (firmPtr->firm_id == FIRM_MINE)
        {
            ...
            if (firmMine->raw_id && firmMine->stock_qty > firmMine->max_stock_qty / 5)
            {
...
void RawRes::put_small_raw_icon(int x, int y, int rawId)
{
    ...
    help.set_custom_help(x, y, x + RAW_SMALL_ICON_WIDTH - 1, y + RAW_SMALL_ICON_HEIGHT - 1, operator[](rawId)->name);
}
