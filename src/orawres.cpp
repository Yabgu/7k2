...
void RawRes::load_all_info() {
  ...
  for (i = 0; i < raw_count; i++) {
    ...
    rawInfo->map_loc_width =
        (char)misc.atoi(rawRec->map_loc_width, rawRec->LOC_LEN);
    rawInfo->map_loc_height =
        (char)misc.atoi(rawRec->map_loc_height, rawRec->LOC_LEN);
    rawInfo->map_icon_offset_x =
        misc.atoi(rawRec->map_icon_offset_x, rawRec->OFFSET_LEN);
    rawInfo->map_icon_offset_y =
        misc.atoi(rawRec->map_icon_offset_y, rawRec->OFFSET_LEN);
    ...
    rawInfo->map_icon_offset_x +=
        -rawInfo->map_loc_width * LOCATE_WIDTH / 2 -
        (-rawInfo->map_loc_width * ZOOM_LOC_X_WIDTH / 2 +
         -rawInfo->map_loc_height * ZOOM_LOC_Y_WIDTH / 2);
    rawInfo->map_icon_offset_y +=
        -rawInfo->map_loc_height * LOCATE_HEIGHT / 2 -
        (-rawInfo->map_loc_width * ZOOM_LOC_X_HEIGHT / 2 +
         -rawInfo->map_loc_height * ZOOM_LOC_Y_HEIGHT / 2);
  }
}
  ...
  for (short firmRecno = firm_array.size(); firmRecno > 0; firmRecno--) {
    ...
    if (firmPtr->firm_id == FIRM_FACTORY) {
      ...
      if (firmFactory->product_raw_id &&
        ...
        raw_res[firmFactory->product_raw_id]->add_product_supply_firm(
            firmRecno);
      }
    }
      ...
      for (int i = 0; i < MAX_MARKET_GOODS; i++, marketGoods++) {
        if (marketGoods->stock_qty > MAX_MARKET_STOCK / 5) {
          if (marketGoods->product_raw_id)
            raw_res[marketGoods->product_raw_id]->add_product_supply_firm(
                firmRecno);
...
RawInfo::RawInfo()
    : raw_supply_firm_array(sizeof(short), 70),
      product_supply_firm_array(sizeof(short), 70) {}
...
RawInfo *RawRes::operator[](int rawId) {
  err_if(rawId < 1 || rawId > raw_count) err_now("RawRes::operator[]");
...
void RawRes::put_small_raw_icon(int x, int y, int rawId) {
  ...
  help.set_custom_help(x, y, x + RAW_SMALL_ICON_WIDTH - 1,
                       y + RAW_SMALL_ICON_HEIGHT - 1, operator[](rawId)->name);
}
