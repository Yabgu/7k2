...
void FirmWar::disp_build_menu(int refreshFlag)
{
    ...
    TechClass *techClass = tech_res.tech_class(war_tech_class); // war_tech_class is set the which
    ...
    for (int techId = techClass->first_tech_id; techId < techClass->first_tech_id + techClass->tech_count; ++techId)
    {
        ...
        if (refreshFlag == INFO_REPAINT)
        {
            ...
            button_queue_weapon[added_count].create(x1 + 20, y1 + 5, x1 + 49, y1 + 34, i_disp_queue_button,
                                                    ButtonCustomPara(this, unitId));
            button_queue_weapon[added_count].set_help_code("WEAPNUM");

            button_weapon[added_count].create(x1, y1 + 35, x1 + 67, y1 + 114, i_disp_build_button,
...
void FirmWar::disp_firm_info(int dispY1, int refreshFlag)
{
    ...
    if (is_own())
    {
        ...
        button_select_build2.visible_flag =
            nation_recno && tech_res.tech_class(TECH_CLASS_MEGA_WEAPON)->total_tech_count(nation_recno) > 0;
#endif
...
void FirmWar::disp_edit_mode(int &refreshFlag, int &y, int dispBG)
{
    ...
    if (edit_progress_enable)
    {
        ...
        x += 20;
        x = edit_progress_x2 =
            font_blue.put((edit_progress_x1 = x), (edit_progress_y1 = y), (int)build_progress_days, 0, x2);
