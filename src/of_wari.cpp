...
void FirmWar::put_info(int refreshFlag) {
  if (refreshFlag == INFO_REPAINT)
    ...
    if (last_menu_mode !=
        war_menu_mode) // if changing menu mode pass repaint to sub-menu
    {
...
void FirmWar::disp_war_info(int dispY1, int refreshFlag) {
  ...
  // #### begin Gilbert 5/10 ######//
  if (refreshFlag == INFO_REPAINT) {
    button_cancel_build.create(INFO_X1 + 182, INFO_Y1 + 5, "WRFC_X-U",
                               "WRFC_X-D");
  ...
  if (!build_unit_id) {
    ...
    font_snds.put_paragraph(INFO_X1 + 25, INFO_Y1 + 13, INFO_X1 + 140,
                            INFO_Y1 + 46, text_firm.str_no_build_weapon());
    ...
    //			nation_recno, INFO_X1+12, INFO_Y1+55, INFO_X2, INFO_Y2,
    //1);
    ...
    str = unitInfo->name;
    // ##### begin Gilbert 24/3 #####//
    // if( unitInfo->unit_class == UNIT_CLASS_WEAPON )		// add version
    // no.
    ...
    font_snds.center_put(INFO_X1 + 18, INFO_Y1 + 13, INFO_X2, INFO_Y1 + 25, str,
                         0, 1);

    //-- unitInfo->build_days is the no. of days take to build the firm when
    //productivity is 100, the actually no. of days will be longer if
    //productivity is less than 100 --//
    // ##### begin Gilbert 30/12 #####//
    float buildProgressDays = build_progress_days;
    //										  + (float)
    //(sys.frame_count-last_process_build_frame_no) / FRAMES_PER_DAY
    //										  * (float) (worker_count*6+productivity/2) /
    //100;
    // ##### end Gilbert 30/12 #####//
...
void FirmWar::disp_build_menu(int refreshFlag) {
  ...
  TechClass *techClass =
      tech_res.tech_class(war_tech_class); // war_tech_class is set the which
  ...
  for (int techId = techClass->first_tech_id;
    ...
    if (refreshFlag == INFO_REPAINT) {
      ...
      button_queue_weapon[added_count].create(x1 + 20, y1 + 5, x1 + 49, y1 + 34,
                                              i_disp_queue_button,
                                              ButtonCustomPara(this, unitId));
      button_queue_weapon[added_count].set_help_code("WEAPNUM");

      button_weapon[added_count].create(
          x1, y1 + 35, x1 + 67, y1 + 114, i_disp_build_button,
          ButtonCustomPara(&button_queue_weapon[added_count], unitId));
    ...
    if (rc > 0) {
      if (rc == 1) // left button
      {
        if (remote.is_enable()) {
          ...
          short *shortPtr = (short *)remote.new_send_queue_msg(
              MSG_F_WAR_BUILD_WEAPON, 2 * sizeof(short));
      ...
      } else // right button - remove queue
      {
        if (remote.is_enable()) {
          // packet structure : <firm recno> <unit Id>
          short *shortPtr = (short *)remote.new_send_queue_msg(
              MSG_F_WAR_CANCEL_WEAPON, 2 * sizeof(short));
          shortPtr[0] = firm_recno;
          shortPtr[1] = unitId;
        } else
          remove_queue(unitId);
...
static void i_disp_build_button(ButtonCustom *button, int repaintBody) {
  ...
  info.draw_unit_icon(x1 + 34, y1 + 46, unitId, nationRecno, INFO_X1, INFO_Y1,
                      INFO_X2, INFO_Y2, button->pushed_flag ? 4 : 1);
  ...
  font_whbl.center_put(x1 - 3, y2 - 22, x1 + 67 + 3, y2 - 2,
                       misc.format(unit_res[unitId]->build_cost, 2));
...
static void i_disp_queue_button(ButtonCustom *button, int repaintBody) {
  ...
  vga.active_buf->put_bitmap(x1, y1,
                             image_button.read(button->pushed_flag
                                                   ? (char *)"NUM-D"
                                                   : (char *)"NUM-U"));
...
void FirmWar::disp_firm_info(int dispY1, int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    button_select_build2.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 281,
                                'A', "MAKEWEAP");

    // ###### begin Gilbert 4/5 ########//
    button_go_rally.create(INFO_X1 + 13 + BUTTON_DISTANCE * 2, INFO_Y1 + 281,
                           'A', "RALLY-GO");
    button_clear_rally.create(INFO_X1 + 13 + BUTTON_DISTANCE * 3, INFO_Y1 + 281,
                              'A', "RALLY-NO");
    // ###### end Gilbert 4/5 ########//
  }
  ...
  if ((int)productivity > 0 && build_unit_id) {
    ...
    vga.active_buf->put_bitmap(INFO_X1 + 140, INFO_Y1 + 61,
...
#else
    button_select_build2.visible_flag =
        nation_recno && tech_res.tech_class(TECH_CLASS_MEGA_WEAPON)
                                ->total_tech_count(nation_recno) > 0;
#endif
    ...
    if (build_unit_id) {
      if (button_cancel_build.detect()) {
        ...
        if (!remote.is_enable())
          ...
          short *shortPtr = (short *)remote.new_send_queue_msg(
              MSG_F_WAR_SKIP_WEAPON, sizeof(short));
          shortPtr[0] = firm_recno;
        }
      }
    }
...
void FirmWar::disp_edit_mode(int &refreshFlag, int &y, int dispBG) {
  ...
  if (edit_progress_enable) {
    ...
    x += 20;
    x = edit_progress_x2 =
        font_blue.put((edit_progress_x1 = x), (edit_progress_y1 = y),
                      (int)build_progress_days, 0, x2);
