...
int Nation::ai_defend(BaseObj *attackerObj) {
  // ####### begin Gilbert 25/2 #######//
  if (!attackerObj || !attackerObj->is_visible()) // object may be invisible
    // ####### end Gilbert 25/2 #######//
    return 0;

  //--- don't call for defense too frequently, only call once 7 days (since this
  //function will be called every time our king/firm/town is attacked, so this
  ...
  int enemyCombatLevel = mobile_defense_combat_level(
      attackerXLoc, attackerYLoc, attackerObj->nation_recno, 0,
      hasWar); // 0-don't return immediately even if there is war around this
               // town

  //-- the value returned is enemy strength minus your own strength, so if it's
  //positive, it means that your enemy is stronger than you, otherwise you're
  //stronger than your enemy --//

  int attackCombatLevel = ai_attack_target(
      attackerXLoc, attackerYLoc, enemyCombatLevel, 1); // 1-defense mode
