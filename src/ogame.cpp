...
int Game::init(int loadGameCall) {
  ...
  *(strstr(tpictFile, "?")) = '0' + config.terrain_set;
  image_tpict.init(tpictFile, 1,
                   0); // config.terrain_set dependent, must load before
                       // town_res.init and terrain_res.init
  terrain_res.init();
  plant_res.init();

  sprite_res.init(); // sprite resource object must been initialized after
                     // game_set as it relies on game_set for info.
  ...
  if (!loadGameCall) {
    if (game.is_campaign_mode())
      info.init(game.campaign()->game_year, game.campaign()->game_month,
                game.campaign()->game_day);
    else
      info.init(INFO_DEFAULT_YEAR, INFO_DEFAULT_MONTH,
                INFO_DEFAULT_DAY); // it reads in the panel texture and copy it
                                   // to vga_back's buffer
  }
...
#define REMAP_KEY_COUNT 8
  ...
  static ColorRemapMethod remap_method_array[] = {
      ...
      {0xA4, 0xC4}, {0xA8, 0xC8}, {0xAC, 0xCC}, {0xB0, 0xD0},
      {0xB4, 0xD4}, {0xB8, 0xD8}, {0xBC, 0xDC},
  };
  ...
  static BYTE customized_color_code[] = {
      186, 186, 186, // Gray
      164, 164, 164, 141, 141, 141, 119, 119, 119, 97,  97,
      97,  75,  75,  75,  52,  52,  52,  30,  30,  30,

      255, 119, 92, // Red
      244, 85,  77,  225, 62,  62,  210, 43,  43,  181, 34,
      34,  150, 17,  17,  115, 13,  10,  77,  6,   4,

      127, 145, 254, // Blue
      100, 119, 231, 79,  95,  212, 61,  76,  188, 48,  58,
      159, 33,  41,  130, 17,  23,  103, 2,   5,   77,

      107, 198, 67, // Green
      86,  184, 47,  74,  160, 47,  61,  140, 45,  49,  121,
      44,  37,  100, 43,  25,  78,  43,  13,  54,  39,

      239, 132, 236, // Purple
      222, 111, 217, 206, 89,  201, 194, 57,  188, 155, 53,
      150, 123, 41,  121, 94,  27,  93,  66,  13,  65,

      255, 177, 0, // Orange
      251, 158, 6,   232, 135, 5,   211, 117, 4,   195, 102,
      8,   166, 83,  2,   151, 66,  8,   130, 52,  13,

      241, 232, 44, // Yellow
      223, 210, 40,  196, 185, 37,  176, 160, 39,  163, 151,
      36,  147, 131, 36,  121, 108, 32,  96,  83,  27,

      214, 153, 107, // Brown
      198, 128, 85,  177, 103, 66,  154, 81,  55,  139, 80,
      66,  112, 62,  55,  82,  46,  45,  50,  25,  25,
  ...
  for (i = 0; i <= MAX_COLOR_SCHEME;
       i++, colorRemap++) // MAX_COLOR_SCHEME+1, +1 for independent units
  {
    ...
    for (j = 0; j < 8; j++)
      colorRemap->color_table[FIRST_REMAP_KEY + j] =
          (short)vga.make_pixel(customized_color_code[i * 24 + j * 3],
                                customized_color_code[i * 24 + j * 3 + 1],
                                customized_color_code[i * 24 + j * 3 + 2]);

    //      for( j=0 ; j<4 ; j++ )
    //			colorRemap->color_table[FIRST_REMAP_KEY+j] = vga.translate_color(
    //(remap_method_array[i].primary_color+j) );

    //      for( j=0 ; j<4 ; j++ )
    //			colorRemap->color_table[FIRST_REMAP_KEY+4+j] = vga.translate_color(
    //(remap_method_array[i].secondary_color+j) );
  }
}
...
short *Game::get_color_remap_table(int nationRecno, int selectedFlag) {
  ...
  else if (nationRecno == 0 ||
           nation_array.is_deleted(
               nationRecno)) // independent units
                             // ####### end gilbert 12/3 #######//
...
#define CYCLE_FRAME_INTERVAL 3
  ...
  if (selectedFlag) {
    ...
    /*
          ...
          {
             cycleId = sys.frame_count / CYCLE_FRAME_INTERVAL %
       (CYCLE_COLOR_COUNT*2-2);

             if( cycleId >= CYCLE_COLOR_COUNT )              // cycle in
       reserved order cycleId = CYCLE_COLOR_COUNT*2-2 - cycleId;
          }
    */
