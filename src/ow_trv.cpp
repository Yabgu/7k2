    ...
    for (int x = x1; x <= x2; ++x, ++locPtr) {
      ...
      if (!unit_array.is_deleted(unitRecno)) // reject if dying
      {
        ...
        if (unitPtr->loc_width == 1 ||
            x == MAX(unitPtr->obj_loc_x1(), x1) &&
                y == MAX(unitPtr->obj_loc_y1(), y1)) // add once
        {
          append(unitRecno);
        }
      }
...
void WorldScanTraverser::append(short unitRecno) {
  if (scan_count >= alloc_count) {
    ...
    if (unit_recno_array == default_recno_array) {
      ...
      unit_recno_array =
          (ScannedObj *)mem_add(alloc_count * sizeof(*unit_recno_array));
      memcpy(unit_recno_array, default_recno_array,
             scan_count * sizeof(*unit_recno_array));
    } else {
      unit_recno_array = (ScannedObj *)mem_resize(
          unit_recno_array, alloc_count * sizeof(*unit_recno_array));
    }
  }
...
//
int WorldScanTraverser::is_finish() { return traverse_count >= scan_count; }
...
//
void WorldScanTraverser::next() { ++traverse_count; }
// ----- end of function WorldScanTraverser::next -------//

// ----- begin of function WorldScanTraverser::restart -------//
//
void WorldScanTraverser::restart() { traverse_count = 0; }
...
void WorldScanTraverser::remove() {
  ...
  //	if( scan_count > 0 )
  //	{
  //		misc.del_array_rec( unit_recno_array, scan_count,
  //sizeof(*unit_recno_array), traverse_count+1 );
  //		--scan_count;
  //		--traverse_count;		// move backward so next() will advance
  //traverse_count
  //	}
...
void WorldScanTraverser::sort_by_distance(int xLoc, int yLoc, int locSize) {
  ...
  qsort(unit_recno_array, scan_count, sizeof(*unit_recno_array),
        sort_unit_by_dist1);
}
...
static int sort_unit_by_dist1(const void *shortPtr1, const void *shortPtr2) {
  ...
  return misc.area_distance(sort_center_x1, sort_center_y1, sort_center_x2,
                            sort_center_y2, unitA->obj_loc_x1(),
                            unitA->obj_loc_y1(), unitA->obj_loc_x2(),
                            unitA->obj_loc_y2()) -
         misc.area_distance(sort_center_x1, sort_center_y1, sort_center_x2,
                            sort_center_y2, unitB->obj_loc_x1(),
                            unitB->obj_loc_y1(), unitB->obj_loc_x2(),
                            unitB->obj_loc_y2());
}
// ------ begin of static function sort_unit_by_dist1 -------//
...
static int sort_unit_by_dist2(const void *shortPtr1, const void *shortPtr2) {
  ...
  return sort_center_obj->area_distance(unitA) -
         sort_center_obj->area_distance(unitB);
}
...
static int sort_unit_by_weight(const void *shortPtr1, const void *shortPtr2) {
  return ((ScannedObj *)shortPtr1)->sort_weight -
         ((ScannedObj *)shortPtr2)->sort_weight;
}
// ------ begin of static function sort_unit_by_weight -------//
