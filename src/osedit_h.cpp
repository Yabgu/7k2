...
void ScenarioEditor::init_human_mode() {
  ...
  human_unit_id_array =
      (short *)mem_add(unit_res.unit_info_count * sizeof(short));
  ...
  human_firm_group_array = (short *)mem_add(
      (race_res.race_count + firm_res.firm_group_count) * sizeof(short));
...
void ScenarioEditor::disp_human_main(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    button_unit_or_struct[0].create_text(
        INFO_X1 + 15, INFO_Y1 + 5, INFO_X1 + 110, INFO_Y1 + 25,
        text_editor.str_unit(), 0); //"Unit", 0 );
    button_unit_or_struct[1].create_text(
        INFO_X1 + 115, INFO_Y1 + 5, INFO_X1 + 210, INFO_Y1 + 25,
        text_editor.str_structure(), 0); //"Structure", 0 );
  }

  font_zoom.put(INFO_X1 + 10, INFO_Y2 - 28, text_editor.str_double_left_add(),
                0, INFO_X2 - 5);
  font_zoom.put(INFO_X1 + 10, INFO_Y2 - 14, text_editor.str_double_right_del(),
                0, INFO_X2 - 5);
...

  {
    ...
    for (int nationRecno = 0; nationRecno <= MAX_NATION; ++nationRecno) {
      if (refreshFlag == INFO_REPAINT) {
        button_player_recno[nationRecno].create(
            INFO_X1 + 16 + nationRecno * 25, INFO_Y1 + 30,
            INFO_X1 + 16 + (nationRecno + 1) * 25 - 2, INFO_Y1 + 57,
            i_disp_nation_button, ButtonCustomPara(NULL, nationRecno), 0);
      }
      button_player_recno[nationRecno].visible_flag =
          button_player_recno[nationRecno].enable_flag =
              !nationRecno ||
              !nation_array.is_deleted(nationRecno); // nation 0 always enable
    }
  }
  ...
  if (!unit_or_struct) {
    ...
    if (refreshFlag == INFO_REPAINT) {
      // init firm browser
      vbrowse_firm_build.init(INFO_X1 + 5, INFO_Y1 + 80, INFO_X2 - 5,
                              INFO_Y2 - 30, -1, 20, human_firm_group_count,
                              disp_human_firm_group);
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    //			button_race_group[r].create_text( INFO_X1+5+r*raceLength,
    //INFO_Y1+58, 				INFO_X1+5+(r+1)*raceLength-1, INFO_Y1+78, raceAbbr );
    //		}
    spinner_race_group.create(INFO_X1 + 80, INFO_Y1 + 58, INFO_X1 + 210,
                              INFO_Y1 + 78, 247, 0, 1 + MAX_RACE,
                              i_disp_race_spinner, human_race_filter);
  }
...
void ScenarioEditor::disp_human_view() {
  ...
  if (mouse.in_area(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2)) {
    ...
    Location *locPtr =
        power.test_detect(mouse.cur_x, mouse.cur_y, &mobileType, &xLoc, &yLoc);
    ...
    // construction marker
    ...
    else if (world.zoom_matrix->get_detect_location(mouse.cur_x, mouse.cur_y,
                                                    &xLoc, &yLoc, NULL)) {
      ...
      if (unit_or_struct == 0 && !vbrowse_unit_id.none_record) {
        ...
        if (world.check_unit_space(xLoc, yLoc, xLoc + spriteInfo->loc_width - 1,
                                   yLoc + spriteInfo->loc_height - 1,
        ...
        err_when(human_firm_group_array[human_firm_group_browse_recno - 1] ==
                 0);
        if (human_firm_group_array[human_firm_group_browse_recno - 1] < 0) {
          // put town of race
          // -human_firm_group_array[human_firm_group_browse_recno] (xLoc, yLoc)
          ...
          // put firm group
          // human_firm_group_array[human_firm_group_browse_recno] on (xLoc,
          // yLoc)

          FirmGroup *firmGroup = firm_res.get_group(
              human_firm_group_array[human_firm_group_browse_recno - 1]);
...
int ScenarioEditor::detect_human_view() {
  ...
  {
    int xLoc, yLoc;
    if (world.zoom_matrix->get_detect_location(
            mouse.click_x(0), mouse.click_y(0), &xLoc, &yLoc, NULL)) {
      if (unit_or_struct == 0 && !vbrowse_unit_id.none_record) {
        // put unit human_unit_id_array[human_unit_id_browse_recno-1]
        // on (xLoc, yLoc)
        int unitId = human_unit_id_array[human_unit_id_browse_recno - 1];
        ...
        if (world.check_unit_space(xLoc, yLoc, xLoc + spriteInfo->loc_width - 1,
                                   yLoc + spriteInfo->loc_height - 1,
                                   unitInfo->mobile_type)) {
          int unitRecno = unit_array.add_unit(unitId, brush_player_recno,
                                              RANK_SOLDIER, 100, xLoc, yLoc);
        ...
        if (human_firm_group_array[human_firm_group_browse_recno - 1] < 0) {
          ...
          if (world.can_build_town(xLoc, yLoc, 0)) {
            int raceId =
                -human_firm_group_array[human_firm_group_browse_recno - 1];
            int townRecno = town_array.generate_town(brush_player_recno, raceId,
                                                     xLoc, yLoc);
            if (townRecno) {
              ...
              if (brush_player_recno)
                ...
                townPtr->init_pop(0, town_array.independent_town_resistance(),
                                  0, 1);
          ...
          // put firm group
          // human_firm_group_array[human_firm_group_browse_recno-1] on (xLoc,
          // yLoc)
          ...
          if (world.can_build_firm(xLoc, yLoc, firmGroup->firm_id, 0)) {
            if (brush_player_recno == 0) {
              box.msg(text_editor
                          .str_no_independent()); // "Cannot add any Independent
                                                  // structures" );
            } else {
              if (firmGroup->firm_id ==
                  FIRM_BASE) // if it's a seat of power, give the nation a
                             // scroll of power first
                god_res[firmGroup->race_id]->enable_know(brush_player_recno);

              firm_array.generate_firm(xLoc, yLoc, brush_player_recno,
                                       firmGroup->firm_id,
                                       firmGroup->race_code);
            }
          }
  ...
  if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2,
                         1)) // right click to remove
  {
    int xLoc, yLoc;
    char mobileType;
    Location *locPtr = power.test_detect(mouse.click_x(1), mouse.click_y(1),
                                         &mobileType, &xLoc, &yLoc);
    if (locPtr) {
...
void ScenarioEditor::collect_human_unit(int raceId) {
  ...
  if (human_unit_id_browse_recno >= 1 &&
      human_unit_id_browse_recno <= human_unit_id_count) {
    ...
    case UNIT_CLASS_SHIP:
      if (!raceId || unitInfo->race_id == raceId) {
        ...
        if (i == oldSelectedUnitId)
          human_unit_id_browse_recno =
              human_unit_id_count; // after human_unit_id_count incremented
      }
      break;
...
void ScenarioEditor::collect_human_firm(int raceId) {
  ...
  if (human_firm_group_browse_recno >= 1 &&
      human_firm_group_browse_recno <= human_firm_group_count) {
    oldSelectedFirmGroup =
        human_firm_group_array[human_firm_group_browse_recno - 1];
  }
  ...
  for (i = 1; i <= race_res.race_count; ++i) {
    if (!raceId || i == raceId) {
      human_firm_group_array[human_firm_group_count++] =
          -i; // insert record for towns
      if (oldSelectedFirmGroup == -i) {
        human_firm_group_browse_recno =
            human_firm_group_count; // increased human_firm_group_count
      }
    }
  }
...
static void disp_human_unit_id(int recNo, int x, int y, int refreshFlag) {
  if (recNo > 0 && recNo <= scenario_editor.human_unit_id_count)
    font_san.put(x + 6, y + 2,
                 unit_res[scenario_editor.human_unit_id_array[recNo - 1]]->name,
                 0, x + browse_unit_width - 1);
}
...
static void disp_human_firm_group(int recNo, int x, int y, int refreshFlag) {
  if (recNo > 0 && recNo <= scenario_editor.human_firm_group_count) {
    ...
    {
      FirmGroup *firmGroup =
          firm_res.get_group(scenario_editor.human_firm_group_array[recNo - 1]);
#if (defined(GERMAN))
...
static void i_disp_nation_button(ButtonCustom *button, int) {
  ...
  char nationColor = nationRecno && !nation_array.is_deleted(nationRecno)
                         ? nation_array.nation_color_array[nationRecno]
                         : nation_array.nation_color_array[0];
