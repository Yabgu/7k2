...
void ScenarioEditor::disp_human_main(int refreshFlag)
{
...

    {
        ...
        for (int nationRecno = 0; nationRecno <= MAX_NATION; ++nationRecno)
        {
            ...
            button_player_recno[nationRecno].visible_flag = button_player_recno[nationRecno].enable_flag =
    ...
    if (!unit_or_struct)
    {
        ...
        {
            // init unit browser
            vbrowse_unit_id.init(INFO_X1 + 5, INFO_Y1 + 80, INFO_X2 - 5, INFO_Y2 - 30, -1, 20, human_unit_id_count,
                                 disp_human_unit_id);
    ...
    else
    {
        ...
        {
            // init firm browser
            vbrowse_firm_build.init(INFO_X1 + 5, INFO_Y1 + 80, INFO_X2 - 5, INFO_Y2 - 30, -1, 20,
                                    human_firm_group_count, disp_human_firm_group);
    ...
    if (refreshFlag == INFO_REPAINT)
    {
        ...
        //			button_race_group[r].create_text(
        // INFO_X1+5+r*raceLength,
        // INFO_Y1+58, 				INFO_X1+5+(r+1)*raceLength-1, INFO_Y1+78, raceAbbr
        // );
        //		}
        spinner_race_group.create(INFO_X1 + 80, INFO_Y1 + 58, INFO_X1 + 210, INFO_Y1 + 78, 247, 0, 1 + MAX_RACE,
        ...
        else if (world.zoom_matrix->get_detect_location(mouse.cur_x, mouse.cur_y, &xLoc, &yLoc, NULL))
        {
            ...
            if (unit_or_struct == 0 && !vbrowse_unit_id.none_record)
            {
                ...
                if (world.check_unit_space(xLoc, yLoc, xLoc + spriteInfo->loc_width - 1,
                                           yLoc + spriteInfo->loc_height - 1, unitInfo->mobile_type))
                {
                    validAction = 1;
                }
                markerX1 = xLoc;
            ...
            else if (unit_or_struct == 1 && !vbrowse_firm_build.none_record)
            {
                ...
                else
                {
                    ...
                    FirmGroup *firmGroup =
                        firm_res.get_group(human_firm_group_array[human_firm_group_browse_recno - 1]);
...
int ScenarioEditor::detect_human_view()
{
    ...
    {
        int xLoc, yLoc;
        if (world.zoom_matrix->get_detect_location(mouse.click_x(0), mouse.click_y(0), &xLoc, &yLoc, NULL))
        {
            if (unit_or_struct == 0 && !vbrowse_unit_id.none_record)
            {
                // put unit human_unit_id_array[human_unit_id_browse_recno-1]
            ...
            else if (unit_or_struct == 1 && !vbrowse_firm_build.none_record)
            {
                ...
                else
                {
                    ...
                    if (world.can_build_firm(xLoc, yLoc, firmGroup->firm_id, 0))
                    {
                        ...
                        {
                            box.msg(text_editor.str_no_independent()); // "Cannot add any Independent
                                                                       // structures" );
                        }
                        else
                        {
                            if (firmGroup->firm_id == FIRM_BASE) // if it's a seat of power, give the nation a
                                                                 // scroll of power first
                                god_res[firmGroup->race_id]->enable_know(brush_player_recno);

                            firm_array.generate_firm(xLoc, yLoc, brush_player_recno, firmGroup->firm_id,
                                                     firmGroup->race_code);
                        }
                    }
                }
