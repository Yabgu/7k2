...
// <int> disable			 = 0 for old scroll browser
//									1 will disable
// the
// scroll bar
...
// <int> totalRec        = total no. of records (for generating scroll bar
// indicator)
//
...
//                         (default : MIN_SPACE)
//
void VBrowse::init(int inX1, int inY1, int inX2, int inY2, int disable, int recWidth, int recHeight, int totalRec,
                   BrowDispFP dispFunc, int dispFrame, int minInterSpace, int dispFrameRect)
{
    x1 = inX1;
    y1 = inY1;
    x2 = inX2;
    ...
    err_if(x_max_rec <= 0 || y_max_rec <= 0) err_now("VBrowse::init() error : disp_max_rec <= 0");
...
void VBrowse::init_var(int totalRec, int recNo)
{
    ...
    if (top_rec_no + disp_max_rec - 1 > total_rec_num) // if there is an empty area at the lower part of the
                                                       // browser
    {
    ...
    scroll_bar.init(1, x2 - SCROLL_BAR_WIDTH, y1, x2, y2, disp_max_rec,
                    (disp_frame && x_max_rec == 1 ? 1 : disp_max_rec), // only use record as scroller unit when
                                                                       // it's a vertical browser with frame
                    total_rec_num, 1);
}
...
void VBrowse::close()
{
    ...
    Vga::active_buf->d3_panel_up(x1 + 1, y1 + 1, x2 - SCROLL_BAR_WIDTH - 3,
                                 y2 - 1); // the place a large block on the list content box
}
...
void VBrowse::paint()
{
    Vga::active_buf->d3_panel_down(x1, y1, x2 - SCROLL_BAR_WIDTH - 3,
                                   y2); // the list content box
...
void VBrowse::refresh(int newRecNo, int newTotalRec)
{
    ...
    if (newRecNo >= 0 || newTotalRec >= 0) // if current recno and total recno has no change, don't
                                           // do the recalculation
    {
        ...
        if (rec_no == 0 && newTotalRec > 0) // if originally rec_no==0, and total_rec is also 0, when the new
                                            // total_rec is > 0, the rec_no should also be adjusted
            rec_no = 1;
...
int VBrowse::detect()
{
    ...
    if (detect_pull_flag && disp_frame && x_max_rec == 1) // only pull when the vbrowser is vertical, horizontal is
                                                          // technically possible, but not used practically
    {
        if (detect_pull())
            return rec_no;
    }
    ...
    if (disp_frame)
    {
        ...
        {
            recNo = top_rec_no + (mouse.click_y() - iy1) / (rec_height + rec_y_space) * x_max_rec +
        ...
        {
            recNo = top_rec_no + (mouse.cur_y - iy1) / (rec_height + rec_y_space) * x_max_rec +
        ...
        if (recNo > 0 && recNo <= top_rec_no - 1 + disp_max_rec) // if it is not in the empty browser area
        {
            ...
            if (double_click) // even if current record no. doesn't change, return
                              // recno if double clicked
                return rec_no;
        }
    }
    ...
    if ((rc = scroll_bar.detect()) > 0)
    {
        ...
        if (disp_frame && x_max_rec == 1) // only use record as scroller unit when it's a vertical browser
                                          // with frame, refer to scroll_bar.init() in VBrowse::init_var()
        {
            if (rec_no < top_rec_no)
            {
                top_rec_no = rec_no;
...
int VBrowse::detect_pull()
{
    ...
    if ((x_max_rec == 1 && mouse.press_area(x1, 0, x2, y1)) || (y_max_rec == 1 && mouse.press_area(0, y1, x1, y2)))
    {
        if (rec_no > 1)
        {
            disp_one(rec_no, CLEAR_HIGH);
...
int VBrowse::detect_right()
{
    ...
    if (mouse.single_click(ix1, iy1, ix2, iy2, 1)) // 1-right mouse button
    {
        ...
        recNo = top_rec_no + (mouse.click_y(1) - iy1) / (rec_height + rec_y_space) * x_max_rec +
...
{
    int recNo;
    Vga::active_buf->bar(ix1, iy1, ix2, iy2,
                         VBROWSE_COLOR_BACK); // clear background

    int scrollRecno = (disp_frame && x_max_rec == 1) ? rec_no : top_rec_no;

    scroll_bar.refresh(scrollRecno, 1, disp_max_rec, (disp_frame && x_max_rec == 1 ? 1 : disp_max_rec), total_rec_num);
...
void VBrowse::disp_one(int recNo, int dispType)
{
    ...
    if (disp_frame && dispType == DISP_HIGH && (!mode))
    {
        ...
        scroll_bar.refresh(scrollRecno, 0, disp_max_rec, (disp_frame && x_max_rec == 1 ? 1 : disp_max_rec),
                           total_rec_num);
    }
}
