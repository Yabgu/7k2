...
char VBrowse::press_record =
    0; // for detecting pulling records upwards and downloads
...
//                         (default : MIN_SPACE)
//
void VBrowse::init(int inX1, int inY1, int inX2, int inY2, int disable,
                   int recWidth, int recHeight, int totalRec,
                   BrowDispFP dispFunc, int dispFrame, int minInterSpace,
                   int dispFrameRect) {
  x1 = inX1;
  y1 = inY1;
  x2 = inX2;
  ...
  if (x_max_rec > 1)
    rec_x_space = minInterSpace + (ix2 - ix1 + minInterSpace + 1) %
                                      (rec_width + minInterSpace) /
                                      (x_max_rec - 1);
  else
    rec_x_space = minInterSpace;

  if (y_max_rec > 1)
    rec_y_space = minInterSpace + (iy2 - iy1 + minInterSpace + 1) %
                                      (rec_height + minInterSpace) /
                                      (y_max_rec - 1);
  else
...
void VBrowse::init_var(int totalRec, int recNo) {
  ...
  if (top_rec_no + disp_max_rec - 1 >
      total_rec_num) // if there is an empty area at the lower part of the
                     // browser
  {
  ...
  scroll_bar.init(1, x2 - SCROLL_BAR_WIDTH, y1, x2, y2, disp_max_rec,
                  (disp_frame && x_max_rec == 1
                       ? 1
                       : disp_max_rec), // only use record as scroller unit when
...
void VBrowse::close() {
  ...
  Vga::active_buf->d3_panel_up(
      x1 + 1, y1 + 1, x2 - SCROLL_BAR_WIDTH - 3,
...
void VBrowse::paint() {
  ...
  Vga::active_buf->bar(x2 - SCROLL_BAR_WIDTH - 2, y1, x2 - SCROLL_BAR_WIDTH - 1,
                       y2, Vga::active_buf->color_up);
...
void VBrowse::refresh(int newRecNo, int newTotalRec) {
  ...
  if (newRecNo >= 0 ||
      newTotalRec >= 0) // if current recno and total recno has no change, don't
                        // do the recalculation
  {
    ...
    if (rec_no == 0 &&
        newTotalRec >
            0) // if originally rec_no==0, and total_rec is also 0, when the new
...
void VBrowse::update() {
  ...
  for (recNo = top_rec_no;
       recNo <= total_rec_num && recNo < top_rec_no + disp_max_rec; recNo++) {
...
int VBrowse::detect() {
  ...
  if (detect_pull_flag && disp_frame &&
      x_max_rec == 1) // only pull when the vbrowser is vertical, horizontal is
  ...
  if (disp_frame) {
    ...
    if (recNo > 0 &&
        recNo <= top_rec_no - 1 +
                     disp_max_rec) // if it is not in the empty browser area
    {
  ...
  if ((rc = scroll_bar.detect()) > 0) {
    ...
    if (disp_frame &&
        x_max_rec ==
            1) // only use record as scroller unit when it's a vertical browser
...
void VBrowse::disp_one(int recNo, int dispType) {
  ...
  if (disp_frame && dispType == CLEAR_HIGH)
    Vga::active_buf->rect(x - 2, y - 2, x + rec_width + 1, y + rec_height + 1,
                          2, VBROWSE_COLOR_BACK);
