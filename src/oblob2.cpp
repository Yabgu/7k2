...
void Blob2D::resize(short destLeft, short destTop, short destWidth,
                    short destHeight) {
  if (width == 0 && height == 0) {
    ...
    if (destHeight > height && destWidth > 0) {
      ...
      fill_area(destLeft, top_edge + height, destLeft + destWidth - 1,
...
void Blob2D::fill_area(short x1, short y1, short x2, short y2,
                       unsigned char color, int autoExtend) {
  if (autoExtend) {
...
void Blob2D::auto_clip(short *pLeft, short *pTop, short *pWidth, short *pHeight,
                       int autoResize) {
  short x1, y1, x2, y2;
