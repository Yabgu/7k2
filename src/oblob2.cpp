...
void Blob2D::resize(short destLeft, short destTop, short destWidth, short destHeight)
{
    ...
    else if (left_edge == destLeft && top_edge == destTop && width == destWidth)
    {
        ...
        if (destHeight > height && destWidth > 0)
        {
            ...
            fill_area(destLeft, top_edge + height, destLeft + destWidth - 1, destTop + destHeight - 1, BACKGROUND_COLOR,
                      0);
        }
    }
    else if (left_edge <= destLeft && top_edge <= destTop && left_edge + width >= destLeft + destWidth &&
    ...
    else
    {
        ...
        {
            fill_area(left_edge, copyTop + copyHeight, left_edge + width - 1, top_edge + height - 1, BACKGROUND_COLOR,
                      0);
        }
        ...
        {
            fill_area(left_edge, copyTop, copyLeft - 1, copyTop + copyHeight - 1, BACKGROUND_COLOR, 0);
        }

        // fill right
        if (left_edge + width > copyLeft + copyWidth && destHeight > 0)
        {
            fill_area(copyLeft + copyWidth, copyTop, left_edge + width, copyTop + copyHeight - 1, BACKGROUND_COLOR, 0);
        }
    }
...
unsigned char *Blob2D::p(short x, short y, int autoExtend)
{
    ...
    {
        if (x < left_edge || x >= left_edge + width || y < top_edge || y >= top_edge + height)
            return NULL;
    }
...
void Blob2D::fill_area(short x1, short y1, short x2, short y2, unsigned char color, int autoExtend)
{
    if (autoExtend)
    {
