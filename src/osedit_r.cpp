...
void ScenarioEditor::disp_diplomacy_main(int refreshFlag) {
  ...
  vga.active_buf->d3_panel_down(
      FIELD_X1 - PANEL_WIDTH - 4, FIELD_Y1 - PANEL_WIDTH - 4,
  ...
  String str;
  for (p = 1; p <= nation_array.size();
       ++p, (y += Y_SPACING), (labelY += Y_SPACING)) {
    // put nation p
  ...
  for (p = RELATION_HOSTILE; p <= RELATION_ALLIANCE; ++p) {
    ...
    font_zoom.put(x + 17, y, text_talk.str_relation_status(p), 0,
                  MIN(x + 108, INFO_X2 - 5));
    if (col < 1) // 2 column
    {
      x += 110;
  ...
  // put trade treaty status
  {
    ...
    font_zoom.put(x + 17, y, text_talk.str_trade_treaty(), 0,
                  MIN(x + 108, INFO_X2 - 5));
  }
...
void ScenarioEditor::detect_diplomacy_main() {
  ...
  for (m = 1; m <= nation_array.size(); ++m, y += Y_SPACING) {
    ...
    for (n = 1; n <= nation_array.size(); ++n, x += X_SPACING) {
      ...
      } else if (mouse.any_click(x, y, x + X_SPACING - 1, y + Y_SPACING - 1,
                                 1)) {
...
//
int ScenarioEditor::detect_diplomacy_view() { return 0; }
