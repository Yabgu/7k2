...
// static void disp_worker_hit_points(int x1, int y1, int x2, int hitPoints, int
// maxHitPoints);
...
void Firm::disp_info_both(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT ||
      ...
      last_should_show_info != should_show_info() || last_is_own != is_own()) {
    last_under_construction = under_construction;
  ...
  if (under_construction) {
    ...
    // font_vill.center_put( INFO_X1+113, INFO_Y1+152, INFO_X1+205, INFO_Y1+165,
    // "Under" ); font_vill.center_put( INFO_X1+99, INFO_Y1+165, INFO_X1+217,
    // INFO_Y1+185, "Construction" );
    font_vill.center_put_paragraph(
        INFO_X1 + 99, INFO_Y1 + 152, INFO_X1 + 217, INFO_Y1 + 185,
        text_firm.str_under_construction(), -5, 0, 0);
    font_vill.center_put_paragraph(INFO_X1 + 20, INFO_Y1 + 103, INFO_X1 + 125,
                                   INFO_Y1 + 147, firm_name(), -5, 0, 0);
    ...
    font_vill.center_put(INFO_X1 + 20, INFO_Y1 + 139, INFO_X1 + 125,
                         INFO_Y1 + 171, misc.format(ratio, 3));
  } else
    ...
    //); 			font_san.field( INFO_X1+10, INFO_Y2-20, " ", INFO_X1+20, firm_recno, 1,
    //INFO_X2-10, refreshFlag); 			font_san.field( INFO_X1+40, INFO_Y2-20, " ",
    //INFO_X1+50, loc_x1, 1, INFO_X2-10, refreshFlag); 			font_san.field(
    //INFO_X1+70, INFO_Y2-20, " ", INFO_X1+80, loc_y1, 1, INFO_X2-10,
    //refreshFlag); 			font_san.field( INFO_X1+100, INFO_Y2-20, " ", INFO_X1+110,
    //ai_link_checked, 1, INFO_X2-10, refreshFlag);
    //		}
#endif
...
void Firm::disp_basic_info(int dispY1, int refreshFlag) {
  ...
  if (firm_id != FIRM_WILDE_LISHORR && firm_id != FIRM_LISHORR &&
  ...
  // ------- create button ------- //
  int showRepairIcon = builder_recno && !under_construction &&
                       should_show_info() && hit_points < max_hit_points();
  ...
  if (should_show_info()) {
    if ((button_builder.visible_flag = !under_construction &&
...
int Firm::detect_basic_info() {
  ...
  //							  unit_array[builder_recno]->is_own();
  //// this is your unit in your firm or it is a spy of yours in an enemy firm
    ...
    if (under_construction) {
      if (!remote.is_enable())
        ...
        short *shortPtr = (short *)remote.new_send_queue_msg(
            MSG_FIRM_MOBL_BUILDER, sizeof(short));
        *shortPtr = firm_recno;
      }
    } else {
      // ####### begin Gilbert 21/4 ##########//
  ...
  if (button_destruct
          .detect()) // if press del during chat, the firm destructed too
  // ####### end Gilbert 27/1 #######//
  {
    if (under_construction)
  ...
  help.set_help(INFO_X1 + 16, INFO_Y1 - 28, INFO_X1 + 45, INFO_Y1 + 1,
                "COLCODE");
...
workerPtr->small_icon_ptr());
                        ...
                        if( selected_worker_id == i+1 )
                                ...
                                vga_front.rect( x, y, x+27, y+23, 2,
vga_front.color_up );

                        //------ display hit points bar --------//

                        disp_worker_hit_points( x+2, y+24, x+25,
...
workerPtr->combat_level, 1, x+49); else font_san.disp(x+30, y+6,
workerPtr->skill_level, 1, x+49);
                        }
                        ...
                        if( mouse.in_area(x, y, x+27, y+23) )
                                help.set_unit_help( workerPtr->unit_id, 0, x, y,
x+27, y+23 );
                }
                else
                {
                        if( last_race_id_array[i] != 0 || last_unit_id_array[i]
!= 0 )
                        {
...
int Firm::detect_worker_list()
{
        ...
        for( i=0 ; i<worker_count ; i++ )
        {
                ...
                switch( mouse.single_click(x, y, x+27, y+23, 2) )
                {
                        ...
                        case 2:
                                if( is_own() )		// only if this is our
own firm
                                {
                                        //--- if the town where the unit lives
belongs to the nation of this firm ---//
...
maxHitPoints)
{
        ...
        {
                if( maxHitPoints <= hit_bar_max_array[i] ||
i==HIT_BAR_TYPE_COUNT-1 )
                {
        ...
        vga_front.bar( x1, y1, x1+barWidth-1, y1, hitBarColor +
HIT_BAR_DARK_BORDER ); vga_front.bar( x1+barWidth, y1, x1+barWidth, y1, V_BLACK
);
...
void Firm::disp_worker_info(int dispY1, int refreshFlag)
{
        ...
        if( refreshFlag == INFO_REPAINT )
        {
                if( firm_id == FIRM_CAMP )		// the command base has
one more field vga_util.d3_panel_up( INFO_X1, dispY1, INFO_X2, dispY1+71 ); else
                        vga_util.d3_panel_up( INFO_X1, dispY1, INFO_X2,
dispY1+55 );
        }
...
FirmInfo::live_in_town is 1
                {
                        Town* townPtr = town_array[workerPtr->town_recno];
                        font_san.field( x, y, "Residence", x+100,
townPtr->town_name(), INFO_X2-2, refreshFlag); y+=16;

                        if( town_array[workerPtr->town_recno]->nation_recno ==
nation_recno && workerPtr->race_id )
                        {
                                info.disp_loyalty( x, y, x+60,
workerPtr->loyalty(), workerPtr->target_loyalty(firm_recno), nation_recno,
refreshFlag );
                        }
                }
                else
// FirmInfo::live_in_town is 0
                {
...
refreshFlag );
                ...
                font_san.field( x, y,
Skill::skill_str_array[workerPtr->skill_id-1], x+100, str, INFO_X2-2,
refreshFlag);
                ...
                if( firm_id == FIRM_CAMP )
                {
                        ...
                        font_san.field( x, y, "Combat", x+100, str, INFO_X2-2,
refreshFlag); y+=16;
                        ...
                        font_san.field( x, y, "Hit Points", x+100, str,
INFO_X2-2, refreshFlag);
                }
        }
}
...
void Firm::disp_overseer_info(int dispY1, int refreshFlag)
{
        if( refreshFlag == INFO_REPAINT )
        {
                if( firm_id == FIRM_CAMP )
                        vga_util.d3_panel_up( INFO_X1, dispY1, INFO_X2,
dispY1+71 ); else vga_util.d3_panel_up( INFO_X1, dispY1, INFO_X2, dispY1+55 );
        }
        ...
        {
                info.disp_loyalty( x, y, x+100, unitPtr->loyalty,
unitPtr->target_loyalty, nation_recno, refreshFlag ); y+=16;
        }
        ...
        if( firm_id==FIRM_CAMP )		// only display combat level in
camps and don't display it in seat of power
        {
                font_san.field( x, y, "Combat" , x+100,
unitPtr->skill.combat_level, 1, INFO_X2-2, refreshFlag); y+=16;
        }
...
//
// beware int& refreshFlag, so it can inform subclass disp_edit_mode to change
// to INFO_REPAINT
//
