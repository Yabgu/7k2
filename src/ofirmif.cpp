...
void Firm::disp_info_both(int refreshFlag)
{
    ...
    if (refreshFlag == INFO_REPAINT || last_under_construction != under_construction ||
    ...
    if (under_construction)
    {
        ...
        font_vill.center_put_paragraph(INFO_X1 + 20, INFO_Y1 + 103, INFO_X1 + 125, INFO_Y1 + 147, firm_name(), -5, 0,
                                       0);
        ...
        if (ratio > 100)
            ratio = 100;
        font_vill.center_put(INFO_X1 + 20, INFO_Y1 + 139, INFO_X1 + 125, INFO_Y1 + 171, misc.format(ratio, 3));
    }
    else
        ...
        //""
        //); 			font_san.field( INFO_X1+10, INFO_Y2-20, " ", INFO_X1+20,
        // firm_recno, 1,
        // INFO_X2-10, refreshFlag); 			font_san.field( INFO_X1+40, INFO_Y2-20, "
        // ", INFO_X1+50, loc_x1, 1, INFO_X2-10, refreshFlag);
        // font_san.field( INFO_X1+70, INFO_Y2-20, " ", INFO_X1+80, loc_y1, 1,
        // INFO_X2-10,
        // refreshFlag); 			font_san.field( INFO_X1+100, INFO_Y2-20, " ",
        // INFO_X1+110, ai_link_checked, 1, INFO_X2-10, refreshFlag);
        //		}
#endif
...
void Firm::disp_basic_info(int dispY1, int refreshFlag)
{
    ...
    if (firm_id != FIRM_WILDE_LISHORR && firm_id != FIRM_LISHORR && firm_id != FIRM_OFFENSIVE_BUILDING_3)
    {
        //		if ( firm_id == FIRM_FORT )
        //			str = firm_name(0);
        //		else
        //			str = firm_name();
        font_snds.center_put(INFO_X1 + 15, INFO_Y1 - 19, INFO_X2, INFO_Y1 - 4, firm_name(), 0, 1);
        disp_hit_point(0);
    ...
    if (should_show_info())
    {
        if ((button_builder.visible_flag =
                 !under_construction && hit_points < max_hit_points() && firm_id != FIRM_LISHORR))
        {
