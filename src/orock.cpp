...
void Rock::process() {
  if (--delay_remain <= 0) {
    cur_frame =
        rock_res.choose_next(rock_recno, cur_frame, random(ROCK_ALT_PATH));
    //	delay_remain =
    //rock_res.get_anim_info(rock_res.get_anim_recno(rock_recno,
    //cur_frame))->delay;
    RockBitmapInfo *rockBitmapInfo = rock_res.get_bitmap_info(
...
void Rock::draw() {
  ...
  rock_res.draw(
      rock_recno, loc_x, loc_y,
      world
          .get_corner(loc_x + rockInfo->loc_width, loc_y + rockInfo->loc_height)
          ->get_altitude(),
      cur_frame);
}
...
//{
//	rock_res.draw_block(rock_recno, xLoc, yLoc, xLoc-loc_x, yLoc-loc_y,
//cur_frame);
//}
...
int Rock::is_stealth() {
  return config.blacken_map &&
         !world.is_explored_quick(loc_x, loc_y, loc_x2(), loc_y2());
}
...
RockBitmapInfo *Rock::rock_bitmap_info() {
  err_when(!rock_recno);
  err_when(cur_frame <= 0 ||
           cur_frame > rock_res.get_rock_info(rock_recno)->max_frame);
...
//
RockArray::RockArray(int initArraySize)
    : DynArrayB(sizeof(Rock), initArraySize, DEFAULT_REUSE_INTERVAL_DAYS) {}
// ------- end of function RockArray::RockArray -------//

// ------- begin of function RockArray::init -------//
//
void RockArray::init() { zap(); }
