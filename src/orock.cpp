...
void Rock::init(short rockRecno, short xLoc, short yLoc)
{
    ...
    RockBitmapInfo *rockBitmapInfo = rock_res.get_bitmap_info(rock_res.get_bitmap_recno(rockRecno, cur_frame));
...
void Rock::process()
{
    if (--delay_remain <= 0)
    {
        ...
        // cur_frame))->delay;
        RockBitmapInfo *rockBitmapInfo = rock_res.get_bitmap_info(rock_res.get_bitmap_recno(rock_recno, cur_frame));
...
void Rock::draw()
{
    ...
    rock_res.draw(rock_recno, loc_x, loc_y,
                  world.get_corner(loc_x + rockInfo->loc_width, loc_y + rockInfo->loc_height)->get_altitude(),
                  cur_frame);
}
...
RockBitmapInfo *Rock::rock_bitmap_info()
{
    ...
    return rock_res.get_bitmap_info(rock_res.get_bitmap_recno(rock_recno, cur_frame));
}
