...
static short edit_reserve_x1, edit_reserve_y1, edit_reserve_x2, edit_reserve_y2,
    edit_reserve_enable;
static short edit_stock_x1, edit_stock_y1, edit_stock_x2, edit_stock_y2,
    edit_stock_enable;
...
void FirmMine::disp_mine_info(int dispY1, int refreshFlag)
{
        ...
        if( !raw_id )
        {
                font_san.center_put( INFO_X1, dispY1, INFO_X2, dispY1+70, "No
Natural Resources" ); return;
        ...
        font_san.field( x, y, "Monthly Production", x+126, (int)
production_30days(), 1, INFO_X2-2, refreshFlag, "MN_PROD"); y+=16;

        str  = (int) stock_qty;
        str += " / ";
        str += (int) max_stock_qty;

        font_san.field( x, y, "Mined Stock", x+126, str, INFO_X2-2, refreshFlag,
"MN_STOCK"); y+=16;

        font_san.field( x, y, "Untapped Reserve", x+126, (int) reserve_qty, 1,
INFO_X2-2, refreshFlag, "MN_UNTAP");
}
...
void FirmMine::disp_firm_info(int dispY1, int refreshFlag) {
  ...
  if (raw_id == 0) {
    ...
    font_snds.put_paragraph(INFO_X1 + 20, INFO_Y1 + 38, INFO_X2 - 5,
                            INFO_Y2 - 5, text_firm.str_mine_no_raw());
  } else {
    //		font_snds.center_put( INFO_X1+10, INFO_Y1+5, INFO_X1+96,
    //INFO_Y1+30, 			"Unmined Raw", 0, 1 );
    font_snds.put(INFO_X1 + 14, INFO_Y1 + 12, text_firm.str_unmined_raw(), 0,
                  -1, 1);

    //		font_snds.center_put( INFO_X2-86+12, INFO_Y1+5, INFO_X2,
    //INFO_Y1+30, 			text_firm.str_mined_raw(), // "Mined Raw", 			0, 1 );
    font_snds.right_put(INFO_X2 - 10, INFO_Y1 + 12, text_firm.str_mined_raw(),
                        0, 1);
    ...
    if (raw_id == RAW_CLAY) {
      ...
      vga.active_buf->put_bitmap_trans(INFO_X1 + 164, INFO_Y1 + 83,
                                       image_icon.read("CLY-RW_R"));
    } else if (raw_id == RAW_COPPER) {
      vga.active_buf->put_bitmap(INFO_X1 + 20, INFO_Y1 + 88,
                                 image_icon.read("COP-RW_L"));
      vga.active_buf->put_bitmap_trans(INFO_X1 + 164, INFO_Y1 + 83,
                                       image_icon.read("COP-RW_R"));
    } else if (raw_id == RAW_IRON) {
      vga.active_buf->put_bitmap(INFO_X1 + 20, INFO_Y1 + 88,
                                 image_icon.read("IRN-RW_L"));
      vga.active_buf->put_bitmap_trans(INFO_X1 + 164, INFO_Y1 + 83,
                                       image_icon.read("IRN-RW_R"));
    } else
      err_here();

    //		str  = "Monthly";
    //		font_snds.center_put( INFO_X1+17, INFO_Y1+90, INFO_X2, INFO_Y1+115,
    //str ); 		str  = "Production"; 		font_snds.center_put( INFO_X1+17, INFO_Y1+106,
...
void FirmMine::disp_edit_mode(int &refreshFlag, int &y, int dispBG) {
  ...
  if (edit_reserve_enable) {
    ...
    x += 20;
    x = edit_reserve_x2 = font_blue.put(
        (edit_reserve_x1 = x), (edit_reserve_y1 = y), (int)reserve_qty, 0, x2);
  ...
  if (edit_stock_enable) {
    ...
    x += 20;
    x = edit_stock_x2 = font_blue.put((edit_stock_x1 = x), (edit_stock_y1 = y),
                                      (int)stock_qty, 0, x2);
...
int FirmMine::detect_edit_mode() {
  ...
  if (edit_reserve_enable && !site_array.is_deleted(site_recno)) {
    ...
    if (mouse.any_click(edit_reserve_x1, edit_reserve_y1, edit_reserve_x2,
                        edit_reserve_y2, LEFT_BUTTON)) {
      reserve_qty += 1000.0f;
  ...
  if (edit_stock_enable) {
    if (mouse.any_click(edit_stock_x1, edit_stock_y1, edit_stock_x2,
                        edit_stock_y2, LEFT_BUTTON)) {
      stock_qty += 100.0f;
      if (stock_qty > max_stock_qty)
