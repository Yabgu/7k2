...
{
    if (cast_delay < MAX_CAST_MAGIC_DELAY && (GAME_VERSION <= 101 || hit_points >= max_hit_points()))
    {
    ...
    case MAGIC_DESTROY_BUILDING: {
        ...
        if (locPtr->is_firm())
        {
            ...
            if (nation_array.should_attack(nation_recno, firmPtr->nation_recno))
            {
                ...
                {
                    news_array.magic_tower_attack(base_obj_recno, magicId, firmPtr->base_obj_recno, firmPtr->center_x,
                                                  firmPtr->center_y);
                }
        ...
        else if (locPtr->is_town())
        {
            ...
            if (nation_array.should_attack(nation_recno, townPtr->nation_recno))
            {
                ...
                if (!is_own()) // inform all other player
                {
                    news_array.magic_tower_attack(base_obj_recno, magicId, townPtr->base_obj_recno, townPtr->center_x,
                                                  townPtr->center_y);
                }
                ...
                if (locPtr->unit_recno(UNIT_LAND))
                {
                    ...
                    if (nation_array.should_attack(nation_recno, unitPtr->nation_recno))
                    {
                        ...
                        else
                        {
                            ...
                            unitPtr->being_attack_hit(this,
                                                      (float)magicInfo->effect / unitPtr->loc_width /
                                                          unitPtr->loc_width); // trigger being attack

                            if (locPtr->can_set_fire() && locPtr->fire_str() < 30)
                                locPtr->set_fire_str(30);
                            if (locPtr->fire_src() > 0)
                                locPtr->set_fire_src(1); // such that the fire will be put out
                                                         // quickly sound effect
                            PosVolume posv(x, y);
                            RelVolume relVolume(posv);
                            if (relVolume.rel_vol < 100)
                                relVolume.rel_vol = 100;
                ...
                if (locPtr->is_firm())
                {
                    ...
                    if (nation_array.should_attack(nation_recno, firmPtr->nation_recno))
                    {
                        ...
                        else
                        {
                            ...
                            se_ctrl.request("OFF-1", relVolume);
                            firmPtr->hit_points -=
                                (magicInfo->effect << 1) / firmPtr->loc_width() / firmPtr->loc_width();
                            firmPtr->being_attack_hit(this, 0.0f); // trigger being attack
                        }
                    }
                }
...
int FirmMagic::calc_live_point_cost(int magicId, int targetXLoc, int targetYLoc, int targetUnitRecno)
{
    ...
    int dist1 = misc.area_distance(loc_x1, loc_y1, loc_x2, loc_y2, targetXLoc, targetYLoc, targetXLoc, targetYLoc);
    // ####### begin Gilbert 21/3 #######//
    int dist2 = 0;
    if (targetUnitRecno && !unit_array.is_deleted(targetUnitRecno) && unit_array[targetUnitRecno]->is_visible())
    {
...
int FirmMagic::transfer_energy(int targetFirmRecno, char remoteAction)
{
    ...
    if (firm_array.is_deleted(targetFirmRecno) || !(targetFirm = firm_array[targetFirmRecno]->cast_to_FirmMagic()) ||
        targetFirm->under_construction || nation_recno != targetFirm->nation_recno // same nation
    ...
    {
        // sufficient case
        targetFirm->cast_delay = MAX_CAST_MAGIC_DELAY; // don't use delayDec * efficiency, avoid round
                                                       // off error
        cast_delay -= delayDec;
    }
    else
    ...
    Effect::create(transferEffectId, targetFirm->center_x * LOCATE_WIDTH, targetFirm->center_y * LOCATE_HEIGHT,
                   SPRITE_IDLE, 1, 8, 0);
