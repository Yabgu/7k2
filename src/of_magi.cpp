...
MagicInfo FirmMagic::magic_info_array[MAGIC_COUNT] = {
    //   magic id                tech_id/                     life
    //   point/cost/delay/effect/range/speed/radius/cost per range/
    {MAGIC_DESTROY_BUILDING, TECH_MAGIC_DAMAGE_BUILDING, 30, 0, 61, 50, 20, 30,
     0, 5, 0}, // 50% of max hp
               // ##### patch begin Gilbert 13/7 ######//
    {MAGIC_FREEZE_UNIT, TECH_MAGIC_FREEZE_UNIT, 30, 0, 30,
     FRAMES_PER_DAY * 12, // 15,
     20, 30, 3, 4, 0},    // 20 frames
                          // ##### patch end Gilbert 13/7 ######//
    {MAGIC_DAMAGE_AREA, TECH_MAGIC_DAMAGE_AREA, 100, 0, 10, 100, 8, 30, 3, 6,
     0}, // 20 hit points
    {MAGIC_TELEPORT, TECH_MAGIC_TELEPORT, 15, 0, 5, 0, 50, 2, 0, 1,
     0},                                                       // no effect
    {MAGIC_FREEZE_LASER, 0, 0, 120, 50, 200, 0, 30, 10, 0, 0}, //
    {MAGIC_GOD_POWER, 0, 0, 140, 50, 100, 20, 30, 3, 0, 5},    //
    {MAGIC_LASER, 0, 0, 70, 50, FRAMES_PER_DAY * 15, 0, 30, 8, 0, 0}, //
};
...
//
void FirmMagic::init(int xLoc, int yLoc, int nationRecno, int firmId,
  ...
  //	use of magic_id_upper_limit and magic_id_lower_limit mainly in
  //of_magia.cpp
  // and this cpp. Although of_magii.cpp also use them, different building
  // always has their own interface cpp file
...
void FirmMagic::process() {
  if (delayed_magic_count > 0) {
    if (--delayed_magic_count == 0) {
      process_delayed_magic(delayed_magic_id, delayed_magic_x_loc,
                            delayed_magic_y_loc, delayed_magic_unit_recno);
    }
  }
}
...
int FirmMagic::can_cast(int magicId) {
  ...
  if (magic_info_array[magicId - 1].tech_id &&
      tech_res[magic_info_array[magicId - 1].tech_id]->get_nation_tech_level(
          nation_recno) == 0)
    return 0;
  // ######## end Gilbert 4/5 #########//
...
//
int FirmMagic::cast_magic(int magicId, int targetXLoc, int targetYLoc,
                          int targetUnitRecno, char remoteAction) {
  if (cast_delay < MAX_CAST_MAGIC_DELAY)
    return 0;
  ...
  int livePointCost =
      calc_live_point_cost(magicId, targetXLoc, targetYLoc, targetUnitRecno);
  int cashCost =
      calc_cash_cost(magicId, targetXLoc, targetYLoc, targetUnitRecno);
  ...
  if (!remoteAction && remote.is_enable()) {
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_F_MAGIC_CAST, 5 * sizeof(short));
  ...
  case MAGIC_DESTROY_BUILDING: {
    ...
    if (locPtr->is_firm()) {
      ...
      if (nation_array.should_attack(nation_recno, firmPtr->nation_recno)) {
        ...
        {
          news_array.magic_tower_attack(base_obj_recno, magicId,
                                        firmPtr->base_obj_recno,
...
//
int FirmMagic::process_delayed_magic(int magicId, int targetXLoc,
                                     int targetYLoc, int targetUnitRecno) {
  int rc = 0;
  ...
  case MAGIC_DESTROY_BUILDING: {
    ...
    if (locPtr->is_firm()) {
      ...
      if (nation_array.should_attack(nation_recno, firmPtr->nation_recno)) {
        firmPtr->hit_points -=
            float(firmPtr->max_hit_points() * magicInfo->effect / 100);
      ...
      if (nation_array.should_attack(nation_recno, townPtr->nation_recno)) {
        ...
        if (townPtr->hit_points == 0.0f) {
          // no change
        } else if (townPtr->hit_points >=
                   effectHit) // wall is stronger than damage
        {
          err_when(townPtr->max_hit_points() ==
                   0); // notice townPtr->hit_points may be 0 when no wall

          townPtr->hit_points -= effectHit;
          effectCent = 0;                      // people are safe
        } else if (townPtr->hit_points > 0.0f) // wall is not as strong, but
                                               // still can reduce the damage
        {
          err_when(effectHit == 0); // divide by zero

          effectCent -= int(townPtr->hit_points * effectCent / effectHit);
          townPtr->hit_points = 0.0f;
          // 0.01f to pass the alreadyKilled checking in
          // BaseObj::being_attack_hit
        }
      ...
      for (int x = x1; x <= x2; ++x) {
        ...
        if (locPtr->unit_recno(UNIT_LAND)) {
          ...
          if (nation_array.should_attack(nation_recno, unitPtr->nation_recno)) {
            ...
            unitPtr->being_attack_hit(
                this, (float)magicInfo->effect); // trigger being attack
        ...
        if (locPtr->unit_recno(UNIT_LAND)) {
          ...
          if (nation_array.should_attack(nation_recno, unitPtr->nation_recno)) {
            if (delay_again_flag == 1) {
              ...
              //	unitPtr->hit_points  -=
              //magicInfo->effect/unitPtr->loc_width/unitPtr->loc_width;
              unitPtr->being_attack_hit(
                  this, (float)magicInfo->effect / unitPtr->loc_width /
                            unitPtr->loc_width); // trigger being attack
          ...
          if (nation_array.should_attack(nation_recno, firmPtr->nation_recno)) {
            if (delay_again_flag == 1) {
              ...
              if (relVolume.rel_vol < 100)
                relVolume.rel_vol = 100;
              se_ctrl.request("OFF-1", relVolume);
              firmPtr->hit_points -= (magicInfo->effect << 1) /
                                     firmPtr->loc_width() /
                                     firmPtr->loc_width();
              firmPtr->being_attack_hit(this, 0.0f); // trigger being attack
            }
...
//
int FirmMagic::calc_live_point_cost(int magicId, int targetXLoc, int targetYLoc,
                                    int targetUnitRecno) {
  err_when(magicId <= 0 || magicId > MAGIC_COUNT);
  ...
  int dist1 = misc.area_distance(loc_x1, loc_y1, loc_x2, loc_y2, targetXLoc,
                                 targetYLoc, targetXLoc, targetYLoc);
  // ####### begin Gilbert 21/3 #######//
  int dist2 = 0;
  if (targetUnitRecno && !unit_array.is_deleted(targetUnitRecno) &&
...
//
int FirmMagic::calc_cash_cost(int magicId, int targetXLoc, int targetYLoc,
                              int targetUnitRecno) {
  err_when(magicId <= 0 || magicId > MAGIC_COUNT);
...
//
short FirmMagic::bullet_init_z() { return altitude + 32; }
...
int FirmMagic::transfer_energy(int targetFirmRecno, char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_MAGIC_TRANSFER,
                                                         2 * sizeof(short));
  ...
  if (efficiency <
      1 / MAX_CAST_MAGIC_DELAY) // use all energy cannot increase 1 pt of energy
    return 1;
  ...
  int delayDec =
      int((MAX_CAST_MAGIC_DELAY - targetFirm->cast_delay) / efficiency);
  if (cast_delay >= delayDec) {
    // sufficient case
    targetFirm->cast_delay =
        MAX_CAST_MAGIC_DELAY; // don't use delayDec * efficiency, avoid round
                              // off error
    cast_delay -= delayDec;
  } else {
    // insufficient case
    delayDec = cast_delay;
    targetFirm->cast_delay += int(
        delayDec *
        efficiency); // don't use delayDec * efficiency, avoid round off error
    cast_delay = 0;
  }

  // add effect
  static int transferEffectId = sprite_res.search_sprite("AN_EFF1");
  Effect::create(transferEffectId, targetFirm->center_x * LOCATE_WIDTH,
                 targetFirm->center_y * LOCATE_HEIGHT, SPRITE_IDLE, 1, 8, 0);
