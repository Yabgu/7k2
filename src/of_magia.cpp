...
void FirmMagic::think_cast_magic()
{
    ...
    int random = misc.random(magic_id_upper_limit - magic_id_lower_limit + 1) + magic_id_lower_limit;
...
void FirmMagic::think_cast_to_area(int magicId)
{
    ...
    for (int i = unit_array.size(); i > 0; i--)
    {
        ...
        if (unitPtr->nation_recno == nation_recno && unitPtr->last_attack_date >= info.game_date - 1)
        {
