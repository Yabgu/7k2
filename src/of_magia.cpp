...
//
int FirmMagic::think_del() { return 0; }
...
void FirmMagic::think_cast_magic() {
  ...
  if (cast_delay < MAX_CAST_MAGIC_DELAY || // only cast magic when the nation is
  ...
  for (int loopId = 1; loopId <= 2; loopId++) {
    ...
    for (int i = firm_array.size(); i > 0; i--) {
      ...
      if (loopId == 1) {
        ...
        curRating =
            (int)(100 - 100 * firmPtr->hit_points / firmPtr->max_hit_points());
    ...
    if (bestFirm) {
      cast_magic(magicId, bestFirm->center_x, bestFirm->center_y, 0,
                 COMMAND_AI);
      return;
    }
  }
    ...
    if (unitPtr->nation_recno == nation_recno &&
      ...
      for (scanner.restart(); !scanner.is_finish(); scanner.next()) {
        ...
        if (targetUnit->nation_recno != nation_recno &&
            nation_array.should_attack(nation_recno,
...
void FirmMagic::think_cast_to_top(int magicId) {
  ...
  WorldScanTraverser scanner;
  scanner.init(loc_x1, loc_y1, loc_width(),
               magic_info_array[magicId - 1].cast_radius);
  err_when(magic_info_array[magicId - 1].cast_range !=
           0); // assume magic is around the building
  ...
  for (scanner.restart(); !scanner.is_finish(); scanner.next()) {
    ...
    if (unitPtr->nation_recno == nation_recno) {
      ownUnit++;
    } else if (nation_array.should_attack(nation_recno,
  ...
  if (enemyUnit >
          10 * hit_points / max_hit_points() +
              misc.random(10) // large number of enemy, or going to die, cast
      || enemyUnit >= 5 && ownUnit >= 5 &&
             (ownUnit - enemyUnit - 5) < misc.random(20)) // close force, cast
  {
