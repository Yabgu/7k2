...
void Info::disp_tech(int refreshFlag)
{
...
#if (defined(CHINESE))
    font_bld.center_put_paragraph(x + 223, TECH_BROWSE_Y1 + 6, x + 329, TECH_BROWSE_Y1 + 32,
                                  text_reports.str_tech_present_version(), 0, 1, 0);
    font_bld.center_put_paragraph(x + 316, TECH_BROWSE_Y1 + 6, x + 439, TECH_BROWSE_Y1 + 32,
                                  text_reports.str_tech_researching_version(), 0, 1, 0);
    font_bld.center_put_paragraph(x + 413, TECH_BROWSE_Y1 + 6, x + 540, TECH_BROWSE_Y1 + 32,
    ...
    font_bld.center_put_paragraph(x + 240, TECH_BROWSE_Y1, x + 329, TECH_BROWSE_Y1 + 32,
                                  text_reports.str_tech_present_version(), 0, 1, 0);

    // font_bld.center_put_paragraph( x+330, TECH_BROWSE_Y1, x+439,
    // TECH_BROWSE_Y1+32, "Researching Version", 0, 1, 0);
    font_bld.center_put_paragraph(x + 330, TECH_BROWSE_Y1, x + 439, TECH_BROWSE_Y1 + 32,
                                  text_reports.str_tech_researching_version(), 0, 1, 0);
    ...
    font_bld.center_put_paragraph(x + 440, TECH_BROWSE_Y1, x + 540, TECH_BROWSE_Y1 + 32,
                                  text_reports.str_tech_research_progress(), 0, 1, 0);
#endif

    if (refreshFlag == INFO_REPAINT)
    {
        browse_tech.init(TECH_BROWSE_X1, TECH_BROWSE_Y1 + 34, TECH_BROWSE_X2, TECH_BROWSE_Y2, 0, 22, tech_filter(),
                         put_tech_rec, 1);
    ...
    if (refreshFlag == INFO_REPAINT)
    {
        browse_god.init(GOD_BROWSE_X1, GOD_BROWSE_Y1 + 22, GOD_BROWSE_X2, GOD_BROWSE_Y2, 0, 22, god_filter(),
                        put_god_rec, 1);
...
static int tech_filter(int recNo)
{
    ...
    for (int techId = tech_res.tech_count; techId > 0; techId--)
    {
        ...
        if (tech_res[techId]->class_id == TECH_CLASS_SPYING && config.spy_methodology)
        {
            continue;
        }
...
static void put_tech_rec(int recNo, int x, int y, int refreshFlag)
{
    ...
    if (curLevel >= 1 &&
        techInfo->max_tech_level(0) > 1) // only need to display version for tech with more than one level
    ...
    if (nationPtr->research_tech_id == techId)
    {
        ...
        vga_back.indicator(x + 441, y - 1, x + 509, y + 13,
                           nation_array[info.viewing_nation_recno]->research_progress(), (float)100, VGA_LIGHT_BLUE,
                           V_BLACK);
        // ######## end Gilbert 17/3 #########//
    }
}
