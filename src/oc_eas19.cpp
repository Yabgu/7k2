...
int CampaignEastWest::stage_19_create_game() {
  ...
  if (misc.random(2) == 0)
    create_offensive_structure(
        STAGE_19_EASTERN_NATION_RECNO,
        1 + campaign_difficulty /
                2); // build 2 structures if the difficulty > 3
...
int CampaignEastWest::stage_19_create_town() {
  ...
  return create_town(STAGE_19_EASTERN_NATION_RECNO, townAddCount,
                     hasFortPercent);
}
...
int CampaignEastWest::stage_19_create_firm() {
  ...
  int townRecno2 = random_pick_town_with_camp(
      STAGE_19_EASTERN_NATION_RECNO, 2); // only pick towns with <= 2 links
