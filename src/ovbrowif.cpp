...
void VBrowseIF::init(int inX1, int inY1, int inX2, int inY2, int recWidth,
                     int recHeight, int totalRec, BrowDispFP dispFunc,
                     int dispFrame, int minInterSpace, int dispFrameRect) {
  VBrowse::init(inX1, inY1, inX2, inY2, 0, recWidth, recHeight, totalRec,
                dispFunc, dispFrame, minInterSpace, dispFrameRect);
...
void VBrowseIF::paint() {
  vga_util.d3_panel_down(x1, y1, x2 - SCROLL_BAR_WIDTH - 3, y2,
                         vga_front_only); // the list content box
...
void VBrowseIF::refresh(int newRecNo, int newTotalRec) {
  if (!vga_front_only && !vga.use_back_buf)
    vga_util.blt_buf(x1, y1, x2 - SCROLL_BAR_WIDTH - 3, y2,
                     0); // the list content box
...
void VBrowseIF::disp_one(int recNo, int dispType) {
  ...
  if (disp_frame && dispType == DISP_HIGH) {
    ...
    Vga::active_buf->draw_d3_up_border(x - 2, y - 2, x + rec_width + 1,
                                       y + rec_height + 1);
