...
void FirmLair::process_ai() {
  ...
  if (info.game_date % 7 == firm_recno % 7 ||
      info.game_start_date ==
          info.game_date) // recruit immediately in the beginning of the game
    think_recruit();
  ...
  if (info.game_date % 30 == (firm_recno + 10) % 30)
  ...
  //----- think about attacking camps/lairs newly built to compete with this
  //lair to control a town -----//
  ...
  if (info.game_date % 60 == (firm_recno + 15) % 60 ||
      info.game_start_date ==
          info.game_date) // recruit immediately in the beginning of the game
  {
    if (!think_repress_new_town()) {
      //---- build new lairs if existing are full ----//

      Nation *nationPtr = nation_array[nation_recno];

      if (nationPtr->total_camp_unit_space(0) <= nationPtr->total_unit_count) {
        ai_build_lair(this);
      }
    }
  }

  //----- think about buying technology -----//

  if (info.game_date % 30 == (firm_recno + 20) % 30)
    think_buy_tech();

  //----- think about building special structures -----//

  if (info.game_date % 45 == (firm_recno + 20) % 45)
    think_build_special();
}
...
int FirmLair::ai_should_close() {
  ...
  return nationPtr->total_camp_unit_space(0) >=
             nationPtr->total_unit_count +
                 MAX_SOLDIER && // we can eliminate a firm and can still keep
                                // all units
         linked_town_count == 0 &&
         nationPtr->ai_camp_count >
             1; // if the nation just has one camp, don't close it.
}
//----------- End of function FirmLair::ai_should_close ----------//

//------ Begin of function FirmLair::think_repress_new_town -------//

int FirmLair::think_repress_new_town() {
  ...
  if (!overseer_recno ||
      soldier_count <
          1 + (should_close_flag ? 0 : nationPtr->pref_force_projection / 30)) {
    return 0;
  }

  //	if( !nationPtr->ai_should_spend( 35 + nationPtr->pref_collect_tax/3,
  //firm_res[FIRM_LAIR]->setup_cost) ) 		return 0;

  //------- see if we still have some lairs in the process of subduing existing
  //linked towns ----//

  int outstandingLair = 0;
  int maxOutstandingLair;

  if (nationPtr->ai_camp_count >= 7 - nationPtr->pref_military_development / 30)
    maxOutstandingLair = 2; // if we already have this number of outstanding
                            // lairs, do not build new ones.
  else
  ...
  for (i = 0; i < nationPtr->ai_camp_count; i++) {
    FirmLair *firmLair =
        firm_array[nationPtr->ai_camp_array[i]]->cast_to_FirmLair();

    if (firmLair) {
      if (firmLair->under_construction ||
          firmLair->has_resisting_linked_town()) {
        if (++outstandingLair == maxOutstandingLair)
          return 0;
      }
    }
  }
  ...
  for (i = town_array.size(); i > 0; i--) {
    ...
    curRating -=
        townPtr->linked_camp_count(false) *
        50; // false-don't just count those with an oversee, count all camps

    if (curRating > bestRating) {
      //--- check if a unit is already on its way to build firm for capturing
      //this town ---//

      if (!nationPtr->is_action_exist(
              -1, -1, townPtr->loc_x1, townPtr->loc_y1, ACTION_AI_BUILD_FIRM,
              FIRM_LAIR)) // 2-only check the reference location, ignore the
                          // action location
      {
...
//------ Begin of function FirmLair::ai_build_lair -------//
//
// Order a unit in this lair to build a new lair next to the given location.
//
// <Place*> placePtr - build a lair linked to this place.
//
void FirmLair::ai_build_lair(Place *placePtr) {
  ...
  //----- check if somebody is already on the way to build a lair ----//

  Nation *nationPtr = nation_array[nation_recno];

  if (nationPtr->is_action_exist(-1, -1, placePtr->loc_x1, placePtr->loc_y1,
                                 ACTION_AI_BUILD_FIRM, FIRM_LAIR)) {
    return;
  }

  //----- mobilize a soldier to build the firm -----//
  ...
  if (!nationPtr->find_best_place_loc(FIRM_LAIR, placePtr->loc_x1,
                                      placePtr->loc_y1, buildXLoc, buildYLoc)) {
    placePtr->no_neighbor_space = 1;
    return;
  }

  //---------- add the action to the queue now ----------//

  nationPtr->add_action(buildXLoc, buildYLoc, placePtr->loc_x1,
                        placePtr->loc_y1, ACTION_AI_BUILD_FIRM, FIRM_LAIR, 1,
                        unitRecno);
}
//------- End of function FirmLair::ai_build_lair ---------//
...
//							      combat level this firm
//needs. 									if not given, it is ai_combat_level_needed() - total_combat_level()
//
void FirmLair::think_recruit(int combatLevelDiff) {
  ...
  //--- if we cannot further recruit units because of the lack of money or live
  //points ---//

  if (!can_recruit())
    return;

  //---- don't recruit further if there are more than one unit under training
  //currently ---//
  ...
  //----------------------------------------------//
  //
  // If there are special firms that we can build but we
  // don't have the live points to build them and we haven't
  // built any of them yet, then don't spend live points
  // until we have build one.
  //
  //----------------------------------------------//
  ...
  for (int firmId = FIRST_MONSTER_SPECIAL_FIRM;
    ...
    if (firmInfo->nation_can_build(nation_recno, race_id,
                                   0) && // 0-don't check cash
        firmInfo->get_nation_firm_count(nation_recno) == 0) {
      importanceRating -= 40 + nationPtr->pref_monster_special / 4;
      break;
    }
  }

  //--- if the commander has high leadership, have more soldiers under his
  //training ---//

  if (overseer_recno) {
    int addRating = unit_array[overseer_recno]->skill_level() * 2 / 3;

    if (unit_array[overseer_recno]->rank_id == RANK_KING)
      addRating += addRating / 2;

    importanceRating += addRating;
  }

  int unitId = monster_res[monster_id()]->unit_id;

  if (!nationPtr->ai_should_spend(importanceRating,
                                  unit_res[unitId]->build_cost))
    return;
...
int FirmLair::ai_get_soldier_from_other_lair() {
  ...
  for (int i = 0; i < nationPtr->ai_camp_count; i++) {
    ...
    if (firmLair
            ->has_resisting_linked_town()) // if this firm is dealing with
                                           // resistanting towns itself, don't
                                           // bother to get units from it
      continue;
    ...
    if (firmLair->soldier_count <=
        soldier_count +
            patrol_unit_count) // only if the firm has more soldier than this
      continue;

    curRating =
        (firmLair->soldier_count - soldier_count - patrol_unit_count) * 10 +
        world.distance_rating(center_x, center_y, firmLair->center_x,
                              firmLair->center_y);
  ...
  if (bestFirm) {
    ...
    if (!unitRecno) {
      ...
      if (readySoldierCount > 0) {
        unitRecno = bestFirm->mobilize_soldier(
            misc.random(readySoldierCount) + 1, COMMAND_AI);
      }
    }
...
void FirmLair::think_attack_linked_town() {
  ...
  int scanRange = world.effective_distance(
      0, FIRM_LAIR); // 0-town. The effective distance between a town and a
                     // Fryhtan Lair
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    if (townPtr->nation_recno == 0 &&
      ...
      int defenseCombatLevel =
          townPtr->possible_defender_combat_level(nation_recno);

      if (nation_array[nation_recno]->ai_attack_target(
              townPtr->loc_x1, townPtr->loc_y1, defenseCombatLevel, 0, 0, 0,
              firm_recno, 1)) {
        return;
      }
      /*
         ...
         (100+ownNation->pref_force_projection*2/3) / 100 )
                              {
                                      rc = 1;
                              }
                              else
         ...
         between the defenders and our units ----//

                                      if( townPtr->town_defender_count > 0 &&
                                               world.net_attacker_power_in_area(townPtr->center_x,
         townPtr->center_y, nation_recno, 0, 1, hasWar, scanRange) == -1 )
         // -1 means a battle is already going on
                                      {
                                              rc = 1;
                                      }
                                      else
                                      {
                                              recruitCombatLevel =
         defenseCombatLevel-firmCombatLevel;		// need to recruit new
         soldiers
                                      }
                              }
                              ...
                              {
                                      if(
         nation_array[nation_recno]->ai_attack_target(townPtr->loc_x1,
                                               townPtr->loc_y1,
         defenseCombatLevel, 0, 0, 0, firm_recno, 0, 1) )		//
         1-retain target combat level, don't apply miliary projection preference
                                      {
                                              return;
                                      }
                              }
...
int FirmLair::has_resisting_linked_town() {
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    if (townPtr->nation_recno == 0 && townPtr->resistance(nation_recno) >=
                                          MONSTER_COLLECT_TOWN_TRIBUTE_LOYALTY)
      return 1;
  }
...
int FirmLair::ai_combat_level_needed() {
  ...
  for (int i = 0; i < linked_town_count; i++) {
    ...
    //------ if the town has other lairs linked to it, the needed combat level
    //is determined by the number of firms of other kingdoms
    ...
    for (int j = 0; j < townPtr->linked_firm_count; j++) {
      ...
      if (firmPtr->nation_recno) {
        int relationStatus =
            nationPtr->get_relation_status(firmPtr->nation_recno);
...
int FirmLair::think_attack_competing_camp() {
      ...
      for (int j = 0; j < townPtr->linked_firm_count; j++) {
        ...
        if (firmPtr->nation_recno != nation_recno && firmPtr->nation_recno &&
          ...
          if (nationPtr->pref_monster_aggressiveness > 50 ||
            ...
            curRating =
                nationPtr->get_relation_status(firmPtr->nation_recno) * 150 +
  ...
  if (bestTargetFirm) {
    if (nationPtr->ai_attack_target(
            bestTargetFirm->loc_x1, bestTargetFirm->loc_y1,
            bestTargetFirm->total_combat_level(), 0, 0, 0, firm_recno, 0,
            1)) // 1-retain target combat level, don't apply miliary projection
                // preference
    {
      return 1;
    }
