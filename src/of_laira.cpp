...
void FirmLair::process_ai()
{
    ...
    if (info.game_date % 7 == firm_recno % 7 ||
        info.game_start_date == info.game_date) // recruit immediately in the beginning of the game
        think_recruit();
...
int FirmLair::ai_should_close()
{
    ...
    return nationPtr->total_camp_unit_space(0) >=
               nationPtr->total_unit_count + MAX_SOLDIER && // we can eliminate a firm and can still keep
                                                            // all units
           linked_town_count == 0 &&
           nationPtr->ai_camp_count > 1; // if the nation just has one camp, don't close it.
}
...
int FirmLair::think_repress_new_town()
{
    ...
    if (!overseer_recno || soldier_count < 1 + (should_close_flag ? 0 : nationPtr->pref_force_projection / 30))
    {
        return 0;
    }
    ...
    for (i = town_array.size(); i > 0; i--)
    {
        ...
        curRating -=
            townPtr->linked_camp_count(false) * 50; // false-don't just count those with an oversee, count all camps
...
void FirmLair::ai_build_lair(Place *placePtr)
{
    ...
    if (nationPtr->is_action_exist(-1, -1, placePtr->loc_x1, placePtr->loc_y1, ACTION_AI_BUILD_FIRM, FIRM_LAIR))
    {
        return;
    }
    ...
    if (!nationPtr->find_best_place_loc(FIRM_LAIR, placePtr->loc_x1, placePtr->loc_y1, buildXLoc, buildYLoc))
    {
    ...
    nationPtr->add_action(buildXLoc, buildYLoc, placePtr->loc_x1, placePtr->loc_y1, ACTION_AI_BUILD_FIRM, FIRM_LAIR, 1,
                          unitRecno);
}
...
//							      combat level this
// firm
// needs. 									if not given, it
// is ai_combat_level_needed()
// - total_combat_level()
//
void FirmLair::think_recruit(int combatLevelDiff)
{
    ...
    int importanceRating = 70 * (MAX_SOLDIER - soldier_count) / MAX_SOLDIER + MAX(combatLevelDiff, 0) / 5;
...
int FirmLair::ai_get_soldier_from_other_lair()
{
    ...
    for (int i = 0; i < nationPtr->ai_camp_count; i++)
    {
        ...
        if (firmLair->has_resisting_linked_town()) // if this firm is dealing with
        ...
        curRating = (firmLair->soldier_count - soldier_count - patrol_unit_count) * 10 +
                    world.distance_rating(center_x, center_y, firmLair->center_x, firmLair->center_y);
    ...
    if (bestFirm)
    {
        ...
        if (bestFirm->should_close_flag && bestFirm->overseer_recno && misc.random(3) == 0) // and some randomness
        {
...
void FirmLair::think_attack_linked_town()
{
    ...
    int scanRange = world.effective_distance(0, FIRM_LAIR); // 0-town. The effective distance between a town and a
                                                            // Fryhtan Lair
    ...
    for (int i = 0; i < linked_town_count; i++)
    {
        ...
        if (townPtr->nation_recno == 0 && townPtr->resistance(nation_recno) >= MONSTER_COLLECT_TOWN_TRIBUTE_LOYALTY)
        {
            ...
            if (nation_array[nation_recno]->ai_attack_target(townPtr->loc_x1, townPtr->loc_y1, defenseCombatLevel, 0, 0,
                                                             0, firm_recno, 1))
            {
                return;
            }
...
int FirmLair::think_attack_competing_camp()
{
    ...
    for (int i = 0; i < linked_town_count; i++)
    {
        ...
        if (townPtr->nation_recno == 0 && townPtr->resistance(nation_recno) >= MONSTER_COLLECT_TOWN_TRIBUTE_LOYALTY)
        {
            ...
            for (int j = 0; j < townPtr->linked_firm_count; j++)
            {
                ...
                if (firmPtr->nation_recno != nation_recno && firmPtr->nation_recno && firmPtr->setup_date > setup_date)
                {
                    int relationStatus = nationPtr->get_relation_status(firmPtr->nation_recno);
                    ...
                    if (nationPtr->pref_monster_aggressiveness > 50 || relationStatus == RELATION_HOSTILE ||
                        (relationStatus <= RELATION_NEUTRAL && nation_array[firmPtr->nation_recno]->is_human()))
                    {
                        curRating =
                            nationPtr->get_relation_status(firmPtr->nation_recno) * 150 + firmPtr->total_combat_level();
    ...
    if (bestTargetFirm)
    {
        if (nationPtr->ai_attack_target(bestTargetFirm->loc_x1, bestTargetFirm->loc_y1,
