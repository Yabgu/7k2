...
void World::generate_map(int loadGame) {
  ...
  loc_matrix = (Location *)mem_resize(
      loc_matrix, MAX_WORLD_X_LOC * MAX_WORLD_Y_LOC * sizeof(Location));
  corner_matrix = (Corner *)mem_resize(
      corner_matrix,
      (MAX_WORLD_X_LOC + 1) * (MAX_WORLD_Y_LOC + 1) * sizeof(Corner));
  ...
  if (!loadGame) {
    ...
    //--- if campaign's mountain layout is enabled, we leave it to the campaign
    //functions to generate the mountains ---//

    if (!game.is_campaign_mode())
      gen_mountains();

    else if (!game.campaign()->mountain_layout_enabled)
      world.gen_mountains(0, 50); // 50% the amount of mountains in a campaign
                                  // game is 50% less than a normal game.
...
void World::set_tera_id(Plasma &plasma) {
  ...
  for (int y = 0; y < max_y_loc; ++y) {
    for (int x = 0; x < max_x_loc; ++x) {
      ...
      seType =
          TerrainRes::terrain_height(plasma.get_pix(x + 1, y + 1), &seSubType);

      if ((get_loc(x, y)->terrain_id =
               terrain_res.scan(nwType, nwSubType, neType, neSubType, swType,
                                swSubType, seType, seSubType, 0, 1, 0)) == 0) {
        err.run("Error World::set_tera_id, Cannot find terrain type %d:%d, "
                "%d:%d, %d:%d, %d:%d",
                nwType, nwSubType, neType, neSubType, swType, swSubType, seType,
                seSubType);
      }
    }
  }
...
void World::set_loc_flags() {
  ...
  for (int xLoc = 0; xLoc < MAX_WORLD_X_LOC;
       xLoc++) // set the top and bottom edges
  {
    get_loc(xLoc, 0)->set_power_off();
    get_loc(xLoc, MAX_WORLD_Y_LOC - 1)->set_power_off();
  }

  for (int yLoc = 0; yLoc < MAX_WORLD_Y_LOC;
       yLoc++) // set the left and right edges
  {
  ...
  if (config.explore_whole_map) {
    for (i = 0; i < totalLoc; i++, locPtr++) {
      ...
      //				else
      //					set_surr_power_off(i%MAX_WORLD_X_LOC,
      //i/MAX_WORLD_X_LOC);
      //			}
...
void World::substitute_pattern() {
  ...
  for (short y = 0; y < max_y_loc; ++y) {
    for (short x = 0; x < max_x_loc; ++x) {
      ...
      SubFound = terrain_res.search_pattern(terrain_res[terrainId]->pattern_id,
      ...
      for (int i = 0; i < SubFound; ++i) {
        ...
        if (flag) {
          ...
          for (terrainSubInfo = candSub[i]; terrainSubInfo != NULL;
            ...
            if (!(get_loc(tx, ty)->terrain_id = terrain_res.scan(
                      oldTerrain->average_type,
...
void World::set_region_id() {
  ...
  for (y = 0; y < max_y_loc; ++y) {
    ...
    for (x = 0; x < max_x_loc; ++x, ++locPtr) {
      ...
      if (y > 0) {
        if (x > 0 &&
            (adjRegionId = get_loc(x - 1, y - 1)->region_id) < thisRegionId)
      ...
      if (x > 0 && (adjRegionId = get_loc(x - 1, y)->region_id) < thisRegionId)
        region_array.set_adjacent(thisRegionId, adjRegionId);
      if (x < max_x_loc - 1 &&
          (adjRegionId = get_loc(x + 1, y)->region_id) < thisRegionId)
        region_array.set_adjacent(thisRegionId, adjRegionId);
...
void World::fill_region(short x, short y) {
  ...
  for (left = x; left >= 0 && !get_loc(left, y)->region_id &&
                 get_loc(left, y)->region_type() == walkable;
       --left) {
    get_loc(left, y)->region_id = regionId;
  }
  ++left;

  for (right = x + 1; right < max_x_loc && !get_loc(right, y)->region_id &&
                      get_loc(right, y)->region_type() == walkable;
       ++right) {
  ...
  if (y < max_y_loc) {
    for (x = left > 0 ? left - 1 : 0; x <= right + 1 && x < max_x_loc; ++x) {
      if (!get_loc(x, y)->region_id &&
          get_loc(x, y)->region_type() == walkable) {
        fill_region(x, y);
      }
    }
  }
...
void World::set_tera_height(Plasma &plasma) {
  ...
  for (y = 0; y <= max_y_loc; ++y) {
    for (x = 0; x <= max_x_loc; ++x) {
      ...
      if (x <= 0) {
        err_when(x < 0);
        gradientX = float(4 * plasma.get_pix(x + 1, y) -
                          plasma.get_pix(x + 2, y) - 3 * plasma.get_pix(x, y)) /
                    (2 * LOCATE_WIDTH);
      } else if (x >= max_x_loc) {
        err_when(x > max_x_loc);
        gradientX =
            float(plasma.get_pix(x - 2, y) - 4 * plasma.get_pix(x - 1, y) +
                  3 * plasma.get_pix(x, y)) /
            (2 * LOCATE_WIDTH);
      } else {
      ...
      if (y <= 0) {
        err_when(y < 0);
        gradientY = float(4 * plasma.get_pix(x, y + 1) -
                          plasma.get_pix(x, y + 2) - 3 * plasma.get_pix(x, y)) /
                    (2 * LOCATE_HEIGHT);
      } else if (y >= max_y_loc) {
        err_when(y > max_y_loc);
        gradientY =
            float(plasma.get_pix(x, y - 2) - 4 * plasma.get_pix(x, y - 1) +
                  3 * plasma.get_pix(x, y)) /
            (2 * LOCATE_HEIGHT);
      } else {
  ...
  for (y = 0; y < max_y_loc; ++y) {
    for (x = 0; x < max_x_loc; ++x) {
      ...
      lc.loc_ptr->c30 = (duz1w + duz0w - (z1 - z0) * 2)
                        << (C_MULTIPLIER_SHIFT - LOCATE_WIDTH_SHIFT -
                            LOCATE_WIDTH_SHIFT - LOCATE_WIDTH_SHIFT);
      lc.loc_ptr->c21 = (duz3w + duz0w - duz1w - duz2w)
                        << (C_MULTIPLIER_SHIFT - 1 - LOCATE_WIDTH_SHIFT -
                            LOCATE_WIDTH_SHIFT - LOCATE_HEIGHT_SHIFT);
      lc.loc_ptr->c12 = (dvz3h + dvz0h - dvz1h - dvz2h)
                        << (C_MULTIPLIER_SHIFT - 1 - LOCATE_WIDTH_SHIFT -
                            LOCATE_HEIGHT_SHIFT - LOCATE_HEIGHT_SHIFT);
      lc.loc_ptr->c03 = (dvz2h + dvz0h - (z2 - z0) * 2)
                        << (C_MULTIPLIER_SHIFT - LOCATE_HEIGHT_SHIFT -
                            LOCATE_HEIGHT_SHIFT - LOCATE_HEIGHT_SHIFT);
      lc.loc_ptr->c20 =
          ((z1 - z0) * 3 - duz1w - duz0w * 2)
          << (C_MULTIPLIER_SHIFT - LOCATE_WIDTH_SHIFT - LOCATE_WIDTH_SHIFT);
      lc.loc_ptr->c11 =
          ((z3 + z0 - z1 - z2)
           << (C_MULTIPLIER_SHIFT - LOCATE_WIDTH_SHIFT - LOCATE_HEIGHT_SHIFT)) -
          (lc.loc_ptr->c21 << LOCATE_WIDTH_SHIFT) -
          (lc.loc_ptr->c12 << LOCATE_HEIGHT_SHIFT);
      lc.loc_ptr->c02 =
          ((z2 - z0) * 3 - dvz2h - dvz0h * 2)
          << (C_MULTIPLIER_SHIFT - LOCATE_HEIGHT_SHIFT - LOCATE_HEIGHT_SHIFT);
      ...
      // find the height of the rendered bitmap
      {
        ...
        // int refHeight = ZoomMatrix::calc_abs_y(0, 0,
        // lc.loc_ptr->evaluate_z(0, 0));
        int refHeight =
            ZoomMatrix::calc_abs_y(0, 0, lc.loc_ptr->c00 >> C_MULTIPLIER_SHIFT);
...
void World::add_fringe( Plasma& heightMap, Plasma &fringeMap )
{
        ...
        for( y = 1; y < heightMap.max_y; ++y )
        {
                for( x = 1; x < heightMap.max_x; ++x )
                {
                        ...
                        if( f < 128 &&
                                ( fringeMap.get_pix(x-1,y) >= 128 ||
fringeMap.get_pix(x+1,y) >= 128
                                || fringeMap.get_pix(x,y-1) >= 128 ||
fringeMap.get_pix(x,y+1) >= 128) )
                        {
                                short h;
...
void World::smooth_altitude(Plasma &plasma) {
  ...
  const int d = 6;
  static short coeff[d + 1] = {8, 2, 0, 0,
                               0, 1, 5}; // weight at (x-0), (x-1), (x-2) ...
  {
    for (short y = 0; y <= plasma.max_y; ++y) {
      for (short x = 0; x <= plasma.max_x; ++x) {
        ...
        if (x >= d && y >= d && dh1 <= 16 && dh2 <= 16 &&
            dh3 <= 16) // no filtering needed
          continue;
...
void World::set_road_id(Plasma &roadMap) {
  ...
  for (y = 0; y < max_y_loc; ++y) {
    for (x = 0; x < max_x_loc; ++x) {
      ...
      neType =
          TerrainRes::terrain_height(roadMap.get_pix(x + 1, y), &neSubType);
      swType =
          TerrainRes::terrain_height(roadMap.get_pix(x, y + 1), &swSubType);
      seType =
          TerrainRes::terrain_height(roadMap.get_pix(x + 1, y + 1), &seSubType);

      if (nwType != TERRAIN_ROAD && neType != TERRAIN_ROAD &&
          swType != TERRAIN_ROAD && seType != TERRAIN_ROAD) {
        get_loc(x, y)->road_terrain_id = 0;
        continue; // no road in this tile
      }

      if ((get_loc(x, y)->road_terrain_id =
               terrain_res.scan(nwType, nwSubType, neType, neSubType, swType,
                                swSubType, seType, seSubType, 0, 1, 0)) == 0) {
        err.run("Error World::gen_road, Cannot find terrain type %d:%d, %d:%d, "
                "%d:%d, %d:%d",
                nwType, nwSubType, neType, neSubType, swType, swSubType, seType,
                seSubType);
      }
