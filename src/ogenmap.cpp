...
void World::generate_map2()
{
    ...
    {
        err_when(locPtr->terrain_id < 1 || locPtr->terrain_id > terrain_res.terrain_count);
    }
...
void World::set_tera_id(Plasma &plasma)
{
    ...
    for (int y = 0; y < max_y_loc; ++y)
    {
        for (int x = 0; x < max_x_loc; ++x)
        {
            ...
            if ((get_loc(x, y)->terrain_id = terrain_res.scan(nwType, nwSubType, neType, neSubType, swType, swSubType,
                                                              seType, seSubType, 0, 1, 0)) == 0)
            {
...
void World::substitute_pattern()
{
    ...
    for (short y = 0; y < max_y_loc; ++y)
    {
        for (short x = 0; x < max_x_loc; ++x)
        {
            ...
            SubFound = terrain_res.search_pattern(terrain_res[terrainId]->pattern_id, candSub, resultArraySize);
            for (int i = 0; i < SubFound; ++i)
            {
                ...
                for (terrainSubInfo = candSub[i]; terrainSubInfo != NULL; terrainSubInfo = terrainSubInfo->next_step)
                {
                    if (tx < 0 || tx >= max_x_loc || ty < 0 || ty >= max_y_loc ||
                        terrain_res[get_loc(tx, ty)->terrain_id]->pattern_id != terrainSubInfo->old_pattern_id)
                    {
                        flag = 0;
                        break;
                    }
                ...
                if (flag)
                {
                         ...
                         terrainSubInfo = terrainSubInfo->next_step)
                    {
                        ...
                        if (!(get_loc(tx, ty)->terrain_id = terrain_res.scan(
                                  oldTerrain->average_type, oldTerrain->secondary_type + terrainSubInfo->sec_adj,
...
void World::fill_region(short x, short y)
{
    ...
    for (right = x + 1;
         right < max_x_loc && !get_loc(right, y)->region_id && get_loc(right, y)->region_type() == walkable; ++right)
    {
...
void World::set_tera_height(Plasma &plasma)
{
    ...
    for (y = 0; y <= max_y_loc; ++y)
    {
        for (x = 0; x <= max_x_loc; ++x)
        {
            ...
            {
                gradientX = float(plasma.get_pix(x + 1, y) - plasma.get_pix(x - 1, y)) / (2 * LOCATE_WIDTH);
            }
            ...
            {
                gradientY = float(plasma.get_pix(x, y + 1) - plasma.get_pix(x, y - 1)) / (2 * LOCATE_HEIGHT);
            }
    ...
    for (y = 0; y < max_y_loc; ++y)
    {
        for (x = 0; x < max_x_loc; ++x)
        {
            ...
            lc.loc_ptr->c30 = (duz1w + duz0w - (z1 - z0) * 2)
                              << (C_MULTIPLIER_SHIFT - LOCATE_WIDTH_SHIFT - LOCATE_WIDTH_SHIFT - LOCATE_WIDTH_SHIFT);
            lc.loc_ptr->c21 = (duz3w + duz0w - duz1w - duz2w) << (C_MULTIPLIER_SHIFT - 1 - LOCATE_WIDTH_SHIFT -
                                                                  LOCATE_WIDTH_SHIFT - LOCATE_HEIGHT_SHIFT);
            lc.loc_ptr->c12 = (dvz3h + dvz0h - dvz1h - dvz2h) << (C_MULTIPLIER_SHIFT - 1 - LOCATE_WIDTH_SHIFT -
                                                                  LOCATE_HEIGHT_SHIFT - LOCATE_HEIGHT_SHIFT);
            lc.loc_ptr->c03 = (dvz2h + dvz0h - (z2 - z0) * 2)
                              << (C_MULTIPLIER_SHIFT - LOCATE_HEIGHT_SHIFT - LOCATE_HEIGHT_SHIFT - LOCATE_HEIGHT_SHIFT);
            lc.loc_ptr->c20 = ((z1 - z0) * 3 - duz1w - duz0w * 2)
            ...
            lc.loc_ptr->c02 = ((z2 - z0) * 3 - dvz2h - dvz0h * 2)
...
void World::set_road_id(Plasma &roadMap)
{
    ...
    for (y = 0; y < max_y_loc; ++y)
    {
        for (x = 0; x < max_x_loc; ++x)
        {
            ...
            if (nwType != TERRAIN_ROAD && neType != TERRAIN_ROAD && swType != TERRAIN_ROAD && seType != TERRAIN_ROAD)
            {
                get_loc(x, y)->road_terrain_id = 0;
                continue; // no road in this tile
            }

            if ((get_loc(x, y)->road_terrain_id = terrain_res.scan(nwType, nwSubType, neType, neSubType, swType,
