...
VgaSDL::VgaSDL() : window(NULL), renderer(NULL), screen(NULL), target(NULL), texture(NULL)
{
...
{
    SDL_Surface *surface = SDL_CreateRGBSurface(0, VGA_WIDTH, VGA_HEIGHT, VGA_BPP, 0, 0, 0, 0);
...
{
    texture = SDL_CreateTexture(renderer, window_pixel_format, SDL_TEXTUREACCESS_STREAMING, VGA_WIDTH, VGA_HEIGHT);
    if (!texture)
    {
        return false;
    }
