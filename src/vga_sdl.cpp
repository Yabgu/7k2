...
VgaSDL::VgaSDL()
    : window(NULL), renderer(NULL), screen(NULL), target(NULL), texture(NULL) {
...
//
void VgaSDL::destroy_window() { SDL_DestroyWindow(window); }
...
int VgaSDL::init() {
  ...
  if (sys.use_true_front) // if we are currently in triple buffer mode, don't
                          // lock the front buffer otherwise the system will
                          // hang up
  {
    init_front(&vga_true_front);
    init_back(&vga_front); // create in video memory
    vga_front.is_front =
        1; // set it to 1, overriding the setting in init_back()
    init_back(&vga_back);
  } else {
...
int VgaSDL::init_back(VgaBuf *b, int width, int height) {
  SDL_Surface *surface =
      SDL_CreateRGBSurface(0, VGA_WIDTH, VGA_HEIGHT, VGA_BPP, 0, 0, 0, 0);
...
//
bool VgaSDL::is_inited() { return screen != NULL; }
...
void VgaSDL::init_color_table() {
  ...
  vga_color_table->generate_table_fast(MAX_BRIGHTNESS_ADJUST_DEGREE, palDesc,
...
void VgaSDL::handle_messages() {
  ...
  while (SDL_PeepEvents(&event, 1, SDL_GETEVENT, SDL_QUIT, SDL_WINDOWEVENT) >
         0) {
...
bool VgaSDL::create_texture() {
  texture =
      SDL_CreateTexture(renderer, window_pixel_format,
                        SDL_TEXTUREACCESS_STREAMING, VGA_WIDTH, VGA_HEIGHT);
  if (!texture) {
    return false;
  } else {
    return true;
