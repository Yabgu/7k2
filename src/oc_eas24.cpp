...
enum
{
    STAGE_24_EASTERN_NATION_RECNO = 2,
...
int CampaignEastWest::stage_24_create_game()
{
    ...
    stage_24_init_relation(); // must init relationship first so that allied units
    ...
    create_troop(STAGE_24_EASTERN_NATION_RECNO, 60 - campaign_difficulty * 6, 0, 0,
                 1); // last 1-create new units on the location next to the
                     // last bunch units created
...
void CampaignEastWest::stage_24_init_vars()
{
    ...
    game.add_win_condition(E_DEFEAT_KINGDOM, 1,
                           STAGE_24_MONSTER_NATION_RECNO); // the objective is to destroy the enemy
                                                           // kingdom
}
...
int CampaignEastWest::stage_24_create_lair()
{
    ...
    create_lair(cur_monster_nation_recno, STAGE_24_MONSTER_NATION_RECNO, lairAddCount, independentTownAddCount,
                specialFirmCount);
...
void CampaignEastWest::stage_24_process_game_result()
{
    ...
    if (game.result_win_condition_id >= 0)
    {
        ...
        if (++east_joint_attack_fryhtan_win_count % 2 == 1 ||
            nation_array.is_deleted(STAGE_24_EASTERN_NATION_RECNO)) // or the allied kingdom has been
                                                                    // destroyed
        {
