...
void FirmMonsterFortress::detect_info() {
  ...
  if (should_show_info()) {
    detect_camp_info();
    // ##### patch begin Gilbert 7/8 #####//
    if (firm_array.is_deleted(
            firmRecno)) // in case button_transform_grokken button is pressed
      return;
    // ##### patch end Gilbert 7/8 #####//
...
void FirmMonsterFortress::disp_camp_info(int dispY1, int refreshFlag) {
  if (is_own()) {
    ...
    if (refreshFlag == INFO_REPAINT) {
      ...
      button_transform_grokken.create(x1 + BUTTON_DISTANCE, y1, 'A',
                                      "FORTRS-2");
    ...
    if (nation_array[nation_recno]->cash >= REWARD_COST &&
        ...
        unit_array[archer_unit_recno[selected_archer_id - 1]]->rank_id !=
            RANK_KING)
    ...
    info.draw_unit_icon(INFO_X1 + 40, INFO_Y1 + 52, support_unit_id,
                        nation_recno, INFO_X1 + 15, INFO_Y1 + 5, INFO_X1 + 68,
                        INFO_Y1 + 84, 1);
    ...
    font_snds.center_put(INFO_X1 + 88, INFO_Y1 + 15, INFO_X1 + 217,
                         INFO_Y1 + 74, str, 0, 0);
  }
}
...
void FirmMonsterFortress::detect_camp_info() {
  if (is_own()) {
    ...
    if (selected_archer_id > 0 && selected_archer_id <= archer_count &&
      ...
      if (!remote.is_enable()) {
        unit_array[archer_unit_recno[selected_archer_id - 1]]->reward(
            nation_array.player_recno);
      } else {
        // packet structure : <rewarding nation> <unit count> <unit recno> ...
        short *shortPtr = (short *)remote.new_send_queue_msg(MSG_UNIT_REWARD,
                                                             3 * sizeof(short));
        // reserve two short more in case selected_recno is slightly smalled
        // then the actual selected
...
//
void FirmMonsterFortress::disp_soldier_list(int dispY1, int refreshFlag,
                                            int dispSpyMenu) {
  ...
  for (int inc = -1; inc <= 1; inc += 2) {
    ...
    for (int i = inc >= 0 ? 1 : archer_count; i >= 1 && i <= archer_count;
         i += inc) {
      // display soldier i
      ...
      if (inc < 0) {
        // first round is descending draw to icon
        info.draw_unit_icon(x + ARHCER_X_SPACING / 2, y, unitId, nation_recno,
                            windowX1, windowY1, windowX2, windowY2, 1);
      } else {
        // second round is ascending to draw the frame

        if (info.draw_unit_icon(
                x + ARHCER_X_SPACING / 2, y, unitId, nation_recno, windowX1,
                windowY1, windowX2, windowY2,
                inAreaFlag | (i == selected_archer_id ? 3 : 0)) &
            4) {
        ...
        if (!dispSpyMenu) {
          ...
          {
            vga.active_buf->put_bitmap_trans(x + ARHCER_X_SPACING / 2 - 8,
          ...
          // display spy skill
          err_when(!ownSpy);
          font_whbl.center_put(x, yHp, x + ARHCER_X_SPACING,
                               yHp + font->max_font_height,
        ...
        disp_soldier_hit_points(x, yHp + font->max_font_height + 2,
                                x + ARHCER_X_SPACING * 7 / 8 - 1, hp, maxHp);
      }
    }
  }
}
...
int FirmMonsterFortress::detect_soldier_list(int selectSpyMenu) {
  ...
  for (int i = 1; i <= archer_count; ++i) {
    ...
    int rc = info.draw_unit_icon(
        x + ARHCER_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1,
...
void FirmMonsterFortress::disp_soldier_info(int dispY1, int refreshFlag) {
  ...
  if (selected_archer_id > 0 && selected_archer_id <= archer_count) {
    ...
    if (archerPtr->rank_id != RANK_KING && nation_recno &&
        unitInfo->class_info.has_loyalty) {
      if (archerPtr->loyalty != archerPtr->target_loyalty)
        info.disp_loyalty(
            x, y + 12,
            INFO_X2 - 99 -
                font_snds.text_width(misc.format(archerPtr->loyalty, 4)) -
                font_snds.text_width(
                    misc.format(archerPtr->target_loyalty, 4)) -
                font_snds.text_width("11"),
            archerPtr->loyalty, archerPtr->target_loyalty, nation_recno,
    ...
    if (unitInfo->class_info.has_combat_level) {
      ...
      font_snds.right_put(INFO_X2 - 10, y + 12,
                          misc.format(archerPtr->combat_level(), 4));
    }
    ...
    if (unitInfo->class_info.has_skill_level && archerPtr->skill_level() > 0) {
      ...
      font_snds.right_put(INFO_X2 - 10, y + 26,
                          misc.format(archerPtr->skill_level(), 4));
    }
...
void FirmMonsterFortress::disp_edit_mode(int &refreshFlag, int &y, int dispBG) {
  ...
  if (selected_archer_id > 0 && selected_archer_id <= archer_count &&
    ...
    if (unitInfo->class_info.has_combat_level) {
      ...
      x = edit_combat_x2 =
          font_blue.put((edit_combat_x1 = x + 20), (edit_combat_y1 = y),
                        archerPtr->skill.actual_combat_level(NULL), 0, x2);
      edit_combat_y2 = edit_combat_y1 + font_blue.max_font_height;
      edit_combat_enable = 1;
      if (archerPtr->combat_level() !=
          archerPtr->skill.actual_combat_level(NULL)) {
        x = font_whbl.put(x, y, "(", 0, x2);
      ...
      if (sys.debug_session) // maximum combat level is a higher secret
      {
        ...
        x = edit_max_combat_x2 =
            font_blue.put((edit_max_combat_x1 = x), (edit_max_combat_y1 = y),
                          archerPtr->skill.max_combat_level, 0, x2);
    ...
    if (unitInfo->class_info.has_skill_level) {
      ...
      x = edit_skill_x2 =
          font_blue.put((edit_skill_x1 = x + 20), (edit_skill_y1 = y),
                        archerPtr->skill.actual_skill_level(NULL), 0, x2);
      edit_skill_y2 = edit_skill_y1 + font_blue.max_font_height - 1;
      edit_skill_enable = 1;
      if (archerPtr->skill_level() !=
          archerPtr->skill.actual_skill_level(NULL)) {
        x = font_whbl.put(x, y, "(", 0, x2);
      ...
      if (sys.debug_session) // maximum skill level is a higher secret
      {
        ...
        x = edit_max_skill_x2 =
            font_blue.put((edit_max_skill_x1 = x), (edit_max_skill_y1 = y),
                          archerPtr->skill.max_skill_level, 0, x2);
    ...
    if (archerPtr->spy_recno) {
      ...
      x = edit_spy_x2 =
          font_blue.put(edit_spy_x1 = x + 20, edit_spy_y1 = y,
                        spy_array[archerPtr->spy_recno]->spy_skill, 0, x2);
