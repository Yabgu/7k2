...
void FirmMonsterFortress::disp_camp_info(int dispY1, int refreshFlag)
{
    if (is_own())
    {
        ...
        if (nation_array[nation_recno]->cash >= REWARD_COST && selected_archer_id > 0 &&
            selected_archer_id <= archer_count &&
        ...
        info.draw_unit_icon(INFO_X1 + 40, INFO_Y1 + 52, support_unit_id, nation_recno, INFO_X1 + 15, INFO_Y1 + 5,
                            INFO_X1 + 68, INFO_Y1 + 84, 1);
...
void FirmMonsterFortress::detect_camp_info()
{
    if (is_own())
    {
        ...
        if (selected_archer_id > 0 && selected_archer_id <= archer_count && button_reward.detect())
        {
            if (!remote.is_enable())
            {
...
void FirmMonsterFortress::disp_soldier_list(int dispY1, int refreshFlag, int dispSpyMenu)
{
    ...
    for (int inc = -1; inc <= 1; inc += 2)
    {
        ...
        for (int i = inc >= 0 ? 1 : archer_count; i >= 1 && i <= archer_count; i += inc)
        {
            ...
            {
                // first round is descending draw to icon
                info.draw_unit_icon(x + ARHCER_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1, windowX2,
                                    windowY2, 1);
            }
            else
            {
                ...
                if (info.draw_unit_icon(x + ARHCER_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1, windowX2,
                                        windowY2, inAreaFlag | (i == selected_archer_id ? 3 : 0)) &
                    4)
                {
                ...
                if (!dispSpyMenu)
                {
                    ...
                    {
                        vga.active_buf->put_bitmap_trans(x + ARHCER_X_SPACING / 2 - 8, yHp - 5,
                    ...
                    if (itemId)
                    {
                        ...
                        if (iconPtr)
                            vga.active_buf->put_bitmap_trans(
                                x + ARHCER_X_SPACING / 2 - ((Bitmap *)iconPtr)->get_width() / 2, yHp + 53, iconPtr);
                    }
                }
                else
                {
                ...
                disp_soldier_hit_points(x, yHp + font->max_font_height + 2, x + ARHCER_X_SPACING * 7 / 8 - 1, hp,
                                        maxHp);
            }
        }
    }
...
int FirmMonsterFortress::detect_soldier_list(int selectSpyMenu)
{
    ...
    for (int i = 1; i <= archer_count; ++i)
    {
        ...
        int rc = info.draw_unit_icon(x + ARHCER_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1, windowX2,
                                     windowY2,
        ...
        if (archerPtr->rank_id != RANK_KING && nation_recno && unitInfo->class_info.has_loyalty)
        {
            if (archerPtr->loyalty != archerPtr->target_loyalty)
                info.disp_loyalty(x, y + 12,
                                  INFO_X2 - 99 - font_snds.text_width(misc.format(archerPtr->loyalty, 4)) -
                                      font_snds.text_width(misc.format(archerPtr->target_loyalty, 4)) -
                                      font_snds.text_width("11"),
                                  archerPtr->loyalty, archerPtr->target_loyalty, nation_recno, refreshFlag,
...
void FirmMonsterFortress::disp_edit_mode(int &refreshFlag, int &y, int dispBG)
{
        ...
        !unit_array.is_deleted(archer_unit_recno[selected_archer_id - 1]))
    {
        ...
        if (unitInfo->class_info.has_loyalty) // && rank_id != RANK_KING )
        {
            ...
            x = edit_loyalty_x2 =
                font_blue.put((edit_loyalty_x1 = x + 20), (edit_loyalty_y1 = y), archerPtr->loyalty, 0, x2);
        ...
        if (archerPtr->spy_recno)
        {
            ...
            x = edit_spy_x2 =
                font_blue.put(edit_spy_x1 = x + 20, edit_spy_y1 = y, spy_array[archerPtr->spy_recno]->spy_skill, 0, x2);
