...
static void init_scenario_var(ScenInfo *scenInfo)
{
    ...
    Nation *playerNation = ~nation_array; // we reset a whole block of vars in NationBase which are
    ...
    int resetSize = (char *)playerNation->relation_array - (char *)(&(playerNation->cur_year_profit));

    memset(&(playerNation->cur_year_profit), 0, resetSize);

    //------- reset statistic vars -------//

    resetSize = (char *)(&(playerNation->own_firm_destroyed)) - (char *)(&(playerNation->enemy_soldier_killed)) +
    ...
    */
    ...
    /*	// ####### begin Gilbert 5/7 #########//
       ...
       condition for( i = 0; i < game.lose_condition_count; i++ )
                    game.lose_condition[i].update_strings(1);		//
       1=lose condition
            // ####### end Gilbert 5/7 #########//*/
}
