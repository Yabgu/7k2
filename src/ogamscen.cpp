...
//
// [int] countScenarioOnly = 0 = display menu and select scenario, 1 = return
// number of scenario (default:0) return : <int> 1 - ok, a scenario is selected
// and run
//                0 - cancel
//
  ...
  for (dirId = 0; dirId < MAX_SCENARIO_PATH; ++dirId) {
    if (DIR_SCENARIO_PATH(dirId)[0]) {
      ...
      if (gameDir.size() > 0) {
        ...
        scenInfoArray = (ScenInfo *)mem_resize(
            scenInfoArray, sizeof(ScenInfo) * (scenInfoSize + gameDir.size()));
        for (int i = 1; i <= gameDir.size(); ++i, ++scenInfoSize) {
          ...
          if (!misc.is_file_exist(str)) {
            ...
            fileTxtScen.read_line(scenInfoArray[scenInfoSize].scen_name,
                                  ScenInfo::SCEN_NAME_LEN);

            //---- get the difficulty level and score bonus ----//

            fileTxtScen.get_token(); // skip "Difficulty:"
            scenInfoArray[scenInfoSize].goal_difficulty =
                (short)fileTxtScen.get_num();

            fileTxtScen.get_token(); // skip "Bonus:"
            scenInfoArray[scenInfoSize].goal_score_bonus =
                (short)fileTxtScen.get_num();
          }
        }
      }
  ...
  if (misc.is_file_exist(str)) {
    ...
    if (game_file.load_game(NULL, str) > 0) {
      ...
      if (nation_array.player_recno)
        nation_array.set_custom_both_name(nation_array.player_recno,
                                          playerName);
...
static void init_scenario_var(ScenInfo *scenInfo) {
  ...
  Nation *playerNation =
      ~nation_array; // we reset a whole block of vars in NationBase which are
                     // related to financial info
  ...
  */
  ...
  /*	// ####### begin Gilbert 5/7 #########//
          ...
          for( i = 0; i < game.win_condition_count; i++ )
                  game.win_condition[i].update_strings(0);		// 0=win
     condition for( i = 0; i < game.lose_condition_count; i++ )
                  game.lose_condition[i].update_strings(1);		// 1=lose
     condition
          // ####### end Gilbert 5/7 #########//*/
}
...
int sort_scenario_func(const void *arg1, const void *arg2) {
  return ((ScenInfo *)arg1)->goal_difficulty -
         ((ScenInfo *)arg2)->goal_difficulty;
}
//-------- End of function init_scenario_var ----------//
