...
static short move_x_pixel_array[] = {0, LOCATE_WIDTH,  LOCATE_WIDTH,  LOCATE_WIDTH,
                                     0, -LOCATE_WIDTH, -LOCATE_WIDTH, -LOCATE_WIDTH};
static short move_y_pixel_array[] = {-LOCATE_HEIGHT, -LOCATE_HEIGHT, 0, LOCATE_HEIGHT,
...
void UnitB::move_to(int destXLoc, int destYLoc, bool raiseDest)
{
    ...
    // ######## begin Gilbert 31/5 ##########//
    path_finder.find_path(next_x_loc(), next_y_loc(), destXLoc, destYLoc, rangeX1, rangeY1, rangeX2, rangeY2, 1,
                          raiseDest, 0, stopAtRange);
    // ######## end Gilbert 31/5 ##########//
    ...
    else //---- path searching failed ----//
    {
        ...
        //		if( seek_path_fail_count < 100 )		// prevent
        // numeric
    ...
    if (!cur_path)
    {
        if (handlePowerNationFlag)
        {
            ...
            //			// ######## begin Gilbert 27/4 #######//
            //			pathFinderStartTime = misc.get_time() -
            // pathFinderStartTime; 			pathfind_profile_time +=
            // pathFinderStartTime;
            //			pathfind_count++;
            ...
            if (!cur_path)
            {
                ...
                if (seek_path_fail_count <= 100 * SEEK_PATH_FAIL_INCREMENT)
                    seek_path_fail_count += SEEK_PATH_FAIL_INCREMENT; // 10 is the single increment unit. It
    ...
    if (cur_path && cur_action != SPRITE_MOVE) // if it is previously moving, don't call
...
void UnitB::process_move()
{
    ...
    {
        err_when(cur_x != next_x || cur_y != next_y); // if the unit reach its destination, then
                                                      // cur_? == next_? == go_?
...
void UnitB::process_blocked()
{
    if (number_of_times_being_blocked++ > MAX_UNIT_BLOCKED_TIME ||
        ...
        ((abs(move_to_loc_x - next_x_loc()) <= obj_loc_width()) &&
         (abs(move_to_loc_y - next_y_loc()) <= obj_loc_height()) && !can_move(move_to_loc_x, move_to_loc_y) &&
...
int UnitB::set_next(int newNextX, int newNextY, int para, int ignoreBlockCheck)
{
    ...
    if (!ignoreBlockCheck)
    {
        ...
        if (isBlocked)
        {
            ...
            if (misc.is_touch(rangeX1, rangeY1, rangeX2, rangeY2, newNextXLoc, newNextYLoc, newNextXLoc + loc_width - 1,
...
void UnitB::process_wait()
{
    ...
    if (can_move((cur_x + stepMagn * move_x_pixel_array[final_dir]) / LOCATE_WIDTH,
...
void UnitB::process_idle()
{
    ...
    if (is_blocked() && cur_order.mode != UNIT_ATTACK)
        process_blocked();
             ...
             is_blocked() && ((next_x_loc() != move_to_loc_x) || (next_y_loc() != move_to_loc_y)) &&
...
void UnitB::get_object_range(int objXLoc, int objYLoc, int &rangeX1, int &rangeY1, int &rangeX2, int &rangeY2)
{
    ...
    //	int* objLocFlag = (int*)
    // mem_add_clear(loc_width*loc_height*sizeof(int)); 	int* objCargoRecno =
    // (int*) mem_add_clear(loc_width*loc_height*sizeof(int));
    ...
    // 26/4) 			if(!objLocFlag[ind] && objCargoRecno[ind] == sprite_recno)
    // //it is myself 				objCargoRecno[ind] = 0;
    // js++;
    //		}
    //		is++;
    //	}
...
//	err_when(!checkCargo && !checkFlag);
    ...
    if (loc_width == 1 && loc_height == 1)
    {
        ...
        for (rangeY1 = ((objYLoc >= 1) ? (objYLoc - 1) : 0); rangeY1 >= 0; rangeY1--)
        {
            if ((world.get_loc(objXLoc, rangeY1)->loc_flag) != objLocFlag ||
                (mobile_type == UNIT_AIR ? (world.get_loc(objXLoc, rangeY1)->air_cargo_recno != objCargoRecno)
                                         : (world.get_loc(objXLoc, rangeY1)->cargo_recno != objCargoRecno)))
            {
        ...
        for (rangeY2 = ((objYLoc < MAX_WORLD_Y_LOC - 1) ? (objYLoc + 1) : (MAX_WORLD_Y_LOC - 1));
             rangeY2 < MAX_WORLD_Y_LOC; rangeY2++)
        {
            if ((world.get_loc(objXLoc, rangeY2)->loc_flag) != objLocFlag ||
                (mobile_type == UNIT_AIR ? (world.get_loc(objXLoc, rangeY2)->air_cargo_recno != objCargoRecno)
                                         : (world.get_loc(objXLoc, rangeY2)->cargo_recno != objCargoRecno)))
            {
        ...
        for (rangeX1 = ((objXLoc >= 1) ? (objXLoc - 1) : 0); rangeX1 >= 0; rangeX1--)
        {
            if ((world.get_loc(rangeX1, objYLoc)->loc_flag) != objLocFlag ||
                (mobile_type == UNIT_AIR ? (world.get_loc(rangeX1, objYLoc)->air_cargo_recno != objCargoRecno)
                                         : (world.get_loc(rangeX1, objYLoc)->cargo_recno != objCargoRecno)))
            {
        ...
        for (rangeX2 = ((objXLoc < MAX_WORLD_X_LOC - 1) ? (objXLoc + 1) : (MAX_WORLD_X_LOC - 1));
             rangeX2 < MAX_WORLD_X_LOC; rangeX2++)
        {
            if ((world.get_loc(rangeX2, objYLoc)->loc_flag) != objLocFlag ||
                (mobile_type == UNIT_AIR ? (world.get_loc(rangeX2, objYLoc)->air_cargo_recno != objCargoRecno)
                                         : (world.get_loc(rangeX2, objYLoc)->cargo_recno != objCargoRecno)))
            {
         ...
         // and 1x1 ( 3x3 may not work)
    {
            ...
            if (objCargoRecnoTL && (objLocFlagTL & LOCATE_BLOCK_MASK))
            {
                for (rangeY1 = ((objYLoc >= 1) ? (objYLoc - 1) : 0); rangeY1 >= 0; rangeY1--)
                {
                    if ((world.get_loc(objXLoc, rangeY1)->loc_flag) != objLocFlagTL ||
                        (mobile_type == UNIT_AIR ? (world.get_loc(objXLoc, rangeY1)->air_cargo_recno != objCargoRecnoTL)
                                                 : (world.get_loc(objXLoc, rangeY1)->cargo_recno != objCargoRecnoTL)))
                    {
                        rangeY1++;
                        break;
                    }
                ...
                for (tmpRangeY1 = ((objYLoc >= 1) ? (objYLoc - 1) : 0); tmpRangeY1 >= 0; tmpRangeY1--)
                {
                    if ((world.get_loc(objXLoc, tmpRangeY1)->loc_flag) != objLocFlagTL ||
                        (mobile_type == UNIT_AIR
                             ? (world.get_loc(objXLoc, tmpRangeY1)->air_cargo_recno != objCargoRecnoTL)
                             : (world.get_loc(objXLoc, tmpRangeY1)->cargo_recno != objCargoRecnoTL)))
                    {
                        tmpRangeY1++;
                        break;
                    }
            ...
            if (objCargoRecnoTR && (objLocFlagTR & LOCATE_BLOCK_MASK))
            {
                for (tmpRangeY2 = ((objYLoc >= 1) ? (objYLoc - 1) : 0); tmpRangeY2 >= 0; tmpRangeY2--)
                {
                    if ((world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)->loc_flag) != objLocFlagTR ||
                        (mobile_type == UNIT_AIR
                             ? (world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)->air_cargo_recno != objCargoRecnoTR)
                             : (world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)->cargo_recno != objCargoRecnoTR)))
                    {
                        tmpRangeY2++;
                        break;
                    }
        ...
        if (objCargoRecnoBL == objCargoRecnoBR && objLocFlagBL == objLocFlagBR)
        {
            ...
            {
                for (rangeY2 = ((objYLoc + loc_height < MAX_WORLD_Y_LOC) ? objYLoc + loc_height : MAX_WORLD_Y_LOC - 1);
                     rangeY2 < MAX_WORLD_Y_LOC; rangeY2++)
                {
                    if ((world.get_loc(objXLoc, rangeY2)->loc_flag) != objLocFlagBL ||
                        (mobile_type == UNIT_AIR ? (world.get_loc(objXLoc, rangeY2)->air_cargo_recno != objCargoRecnoBL)
                                                 : (world.get_loc(objXLoc, rangeY2)->cargo_recno != objCargoRecnoBL)))
                    {
                        rangeY2--;
                        break;
                    }
        ...
        else
        {
            ...
            {
                for (tmpRangeY1 =
                         ((objYLoc + loc_height < MAX_WORLD_Y_LOC) ? objYLoc + loc_height : MAX_WORLD_Y_LOC - 1);
                     tmpRangeY1 < MAX_WORLD_Y_LOC; tmpRangeY1++)
                {
                    if ((world.get_loc(objXLoc, tmpRangeY1)->loc_flag) != objLocFlagBL ||
                        (mobile_type == UNIT_AIR
                             ? (world.get_loc(objXLoc, tmpRangeY1)->air_cargo_recno != objCargoRecnoBL)
                             : (world.get_loc(objXLoc, tmpRangeY1)->cargo_recno != objCargoRecnoBL)))
                    {
                        tmpRangeY1--;
                        break;
                    }
            ...
            if (objCargoRecnoBR && (objLocFlagBR & LOCATE_BLOCK_MASK))
            {
                     ...
                     tmpRangeY2 < MAX_WORLD_Y_LOC; tmpRangeY2++)
                {
                    if ((world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)->loc_flag) != objLocFlagBR ||
                        (mobile_type == UNIT_AIR
                             ? (world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)->air_cargo_recno != objCargoRecnoBR)
                             : (world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)->cargo_recno != objCargoRecnoBR)))
                    {
                        tmpRangeY2--;
                        break;
                    }
            ...
            if (objCargoRecnoTL && (objLocFlagTL & LOCATE_BLOCK_MASK))
            {
                for (rangeX1 = ((objXLoc >= 1) ? (objXLoc - 1) : 0); rangeX1 >= 0; rangeX1--)
                {
                    if ((world.get_loc(rangeX1, objYLoc)->loc_flag) != objLocFlagTL ||
                        (mobile_type == UNIT_AIR ? (world.get_loc(rangeX1, objYLoc)->air_cargo_recno != objCargoRecnoTL)
                                                 : (world.get_loc(rangeX1, objYLoc)->cargo_recno != objCargoRecnoTL)))
                    {
                        rangeX1++;
                        break;
                    }
            ...
            if (objCargoRecnoTL && (objLocFlagTL & LOCATE_BLOCK_MASK))
            {
                for (tmpRangeX1 = ((objXLoc >= 1) ? (objXLoc - 1) : 0); tmpRangeX1 >= 0; tmpRangeX1--)
                {
                    if ((world.get_loc(tmpRangeX1, objYLoc)->loc_flag) != objLocFlagTL ||
                        (mobile_type == UNIT_AIR
                             ? (world.get_loc(tmpRangeX1, objYLoc)->air_cargo_recno != objCargoRecnoTL)
                             : (world.get_loc(tmpRangeX1, objYLoc)->cargo_recno != objCargoRecnoTL)))
                    {
                        tmpRangeX1++;
                        break;
                    }
            ...
            if (objCargoRecnoBL && (objLocFlagBL & LOCATE_BLOCK_MASK))
            {
                for (tmpRangeX2 = ((objXLoc >= 1) ? (objXLoc - 1) : 0); tmpRangeX2 >= 0; tmpRangeX2--)
                {
                    if ((world.get_loc(tmpRangeX2, objYLoc + loc_height - 1)->loc_flag) != objLocFlagBL ||
                        (mobile_type == UNIT_AIR
                             ? (world.get_loc(tmpRangeX2, objYLoc + loc_height - 1)->air_cargo_recno != objCargoRecnoBL)
                             : (world.get_loc(tmpRangeX2, objYLoc + loc_height - 1)->cargo_recno != objCargoRecnoBL)))
                    {
                        tmpRangeX2++;
                        break;
                    }
        ...
        if (objCargoRecnoTR == objCargoRecnoBR && objLocFlagTR == objLocFlagBR)
        {
            if (objCargoRecnoTR && (objLocFlagTR & LOCATE_BLOCK_MASK))
            {
                for (rangeX2 = ((objXLoc + loc_width < MAX_WORLD_X_LOC) ? objXLoc + loc_width : MAX_WORLD_X_LOC - 1);
                     rangeX2 < MAX_WORLD_X_LOC; rangeX2++)
                {
                    if ((world.get_loc(rangeX2, objYLoc)->loc_flag) != objLocFlagTR ||
                        (mobile_type == UNIT_AIR ? (world.get_loc(rangeX2, objYLoc)->air_cargo_recno != objCargoRecnoTR)
                                                 : (world.get_loc(rangeX2, objYLoc)->cargo_recno != objCargoRecnoTR)))
                    {
                        rangeX2--;
                        break;
                    }
        ...
        else
        {
            ...
            if (objCargoRecnoTR && (objLocFlagTR & LOCATE_BLOCK_MASK))
            {
                for (tmpRangeX1 = ((objXLoc + loc_width < MAX_WORLD_X_LOC) ? objXLoc + loc_width : MAX_WORLD_X_LOC - 1);
                     tmpRangeX1 < MAX_WORLD_X_LOC; tmpRangeX1++)
                {
                    if ((world.get_loc(tmpRangeX1, objYLoc)->loc_flag) != objLocFlagTR ||
                        (mobile_type == UNIT_AIR
                             ? (world.get_loc(tmpRangeX1, objYLoc)->air_cargo_recno != objCargoRecnoTR)
                             : (world.get_loc(tmpRangeX1, objYLoc)->cargo_recno != objCargoRecnoTR)))
                    {
                        tmpRangeX1--;
                        break;
                    }
            ...
            if (objCargoRecnoBR && (objLocFlagBR & LOCATE_BLOCK_MASK))
            {
                for (tmpRangeX2 = ((objXLoc + loc_width < MAX_WORLD_X_LOC) ? objXLoc + loc_width : MAX_WORLD_X_LOC - 1);
                     tmpRangeX2 < MAX_WORLD_X_LOC; tmpRangeX2++)
                {
                    if (world.get_loc(tmpRangeX2, objYLoc + loc_height - 1)->loc_flag != objLocFlagBR ||
                        (mobile_type == UNIT_AIR
                             ? (world.get_loc(tmpRangeX2, objYLoc)->air_cargo_recno != objCargoRecnoBR)
                             : (world.get_loc(tmpRangeX2, objYLoc)->cargo_recno != objCargoRecnoBR)))
                    {
                        tmpRangeX2--;
                        break;
                    }
...
int UnitB::checking_who_occupy_the_place_i_want_to_go(int checkingLocX, int checkingLocY)
{
    ...
    if (checkingLocX >= MAX_WORLD_X_LOC || checkingLocX < 0 || checkingLocY >= MAX_WORLD_Y_LOC || checkingLocY < 0)
        return 0;
    ...
    while (!occupyingUnitRecordNumber && is < obj_loc_width())
    {
        ...
        while (!occupyingUnitRecordNumber && js < obj_loc_height())
        {
            ...
            occupyingUnitRecordNumber =
                world.get_loc(temperoryCheckingLocX, temperoryCheckingLocY)->unit_recno(mobile_type);
            if (occupyingUnitRecordNumber &&
                ((occupyingUnitRecordNumber == sprite_recno) ||
                 (theOccupyingStuff = unit_array[occupyingUnitRecordNumber])->nation_recno != nation_recno ||
                 theOccupyingStuff->cur_action == SPRITE_MOVE || theOccupyingStuff->cur_action == SPRITE_WAIT ||
...
bool UnitB::can_move(int locX, int locY)
{
    ...
    //(locPtr->unit_recno(obj_mobile_type()) != sprite_recno)))
    // return false;
    //		}
    //	}
    ...
    else
    {
        ...
        for (j = 0; j < loc_height; ++j)
        {
            ...
            {
                if (!locPtr->can_move(mobile_type) && locPtr->unit_recno(mobile_type) != sprite_recno)
                    return false;
            }
        }
    }
...
bool UnitB::can_move2(int locX, int locY)
{
    ...
    //			locPtr = world.get_loc(locX+i, locY+j);
    //			if(locX+i>= MAX_WORLD_X_LOC || locX+i <0 || locY+j >=
    // MAX_WORLD_Y_LOC || locY+j < 0 ) 				return false;
