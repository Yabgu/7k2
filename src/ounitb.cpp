...
static short move_x_pixel_array[] = {
    0, LOCATE_WIDTH,  LOCATE_WIDTH,  LOCATE_WIDTH,
    0, -LOCATE_WIDTH, -LOCATE_WIDTH, -LOCATE_WIDTH};
static short move_y_pixel_array[] = {
    -LOCATE_HEIGHT, -LOCATE_HEIGHT, 0, LOCATE_HEIGHT,
    LOCATE_HEIGHT,  LOCATE_HEIGHT,  0, -LOCATE_HEIGHT};
...
void UnitB::move_to(int destXLoc, int destYLoc, bool raiseDest) {
  ...
  //	err_when(destXLoc < 0 || destXLoc >= MAX_WORLD_X_LOC || destYLoc < 0 ||
  //destYLoc >= MAX_WORLD_Y_LOC);
  ...
  //--- save the destination for later possible retries in searching when it is
  //blocked ----//
  ...
  int stopAtRange =
      cur_order.mode == UNIT_ATTACK ? cast_to_Unit()->attack_range() - 1 : 0;
  ...
  if (nation_recno) {
    ...
    int srcLocPowerNation =
        world.get_loc(next_x_loc(), next_y_loc())->power_nation_recno;
    int destLocPowerNation =
        world.get_loc(destXLoc, destYLoc)->power_nation_recno;

    //--- only if unit stands on a passable location and the destination is
    //passable, we handle power nation ---//

    if ((!srcLocPowerNation ||
         nationPtr->get_relation_passable(srcLocPowerNation)) &&
        (!destLocPowerNation ||
         nationPtr->get_relation_passable(destLocPowerNation))) {
      path_finder.set_handle_power_nation(1,
                                          nationPtr->relation_passable_array);
  ...
  // ######## begin Gilbert 31/5 ##########//
  path_finder.find_path(next_x_loc(), next_y_loc(), destXLoc, destYLoc, rangeX1,
                        rangeY1, rangeX2, rangeY2, 1, raiseDest, 0,
                        stopAtRange);
  // ######## end Gilbert 31/5 ##########//
  ...
  } else                      //---- path searching failed ----//
  {
    ...
    //		if( seek_path_fail_count < 100 )		// prevent numeric
    //overflow 			seek_path_fail_count++;
  }
  ...
  cur_path =
      path_finder.retrieve(pathNodeCount, totalSteps); // retrieves the path
  ...
  if (!cur_path) {
    if (handlePowerNationFlag) {
      ...
      // ##### end Gilber 27/4 ########//
      // ###### begin Gilbert 12/5 ########//
      // path_finder.set_attribute(loc_width, loc_height, mobile_type,
      // PFIND_IGNORE_UNIT );
      // ###### end Gilbert 12/5 ########//
      path_finder.set_handle_power_nation(0);
      ...
      path_finder.find_path(next_x_loc(), next_y_loc(), destXLoc, destYLoc,
                            rangeX1, rangeY1, rangeX2, rangeY2, 1, raiseDest, 0,
                            stopAtRange);
      // ######## end Gilbert 31/5 ##########//
      ...
      //			// ######## begin Gilbert 27/4 #######//
      //			pathFinderStartTime = misc.get_time() -
      //pathFinderStartTime; 			pathfind_profile_time += pathFinderStartTime;
      //			pathfind_count++;
      //			if( pathFinderStartTime >
      //longest_pathfind_profile_time )
      //			{
      //				longest_pathfind_profile_time =
      //pathFinderStartTime; 				longest_pathfind_unit_recno = sprite_recno;
      //			}
      //			// ######## begin Gilbert 27/4 #######//
      //#endif
      if (!cur_path) {
        ...
        if (seek_path_fail_count <= 100 * SEEK_PATH_FAIL_INCREMENT)
          seek_path_fail_count +=
              SEEK_PATH_FAIL_INCREMENT; // 10 is the single increment unit. It
                                        // will be reduce by 1 each time in
                                        // Unit::attack()
      }
    } else {
      if (!handle_blocking())
  ...
  if (cur_path &&
      cur_action != SPRITE_MOVE) // if it is previously moving, don't call
                                 // next_move() as it has still yet to finish
                                 // its remaining pixel move to snap to grids
  {
...
void UnitB::process_move() {
  ...
  if (cur_x == go_x && cur_y == go_y) {
    err_when(cur_x != next_x ||
             cur_y != next_y); // if the unit reach its destination, then
                               // cur_? == next_? == go_?
    ...
    if (cur_path_result_id > 0) // 0 means it has completed all its move
    {
      ...
      if (cur_action != SPRITE_MOVE) // if next_move() is not successful, the
  ...
  err_when(cur_x - next_x != 0 && cur_y - next_y != 0 &&
           abs(next_x - cur_x) != abs(next_y - cur_y));

  //------- call Sprite::process_move --------//

  Sprite::process_move();

  err_when(cur_x < 0 || cur_y < 0 || cur_x >= MAX_WORLD_X_LOC * LOCATE_WIDTH ||
           cur_y >= MAX_WORLD_Y_LOC * LOCATE_HEIGHT);
...
void UnitB::process_blocked() {
  if (number_of_times_being_blocked++ > MAX_UNIT_BLOCKED_TIME ||
      ...
      ((abs(move_to_loc_x - next_x_loc()) <= obj_loc_width()) &&
       ...
       !checking_who_occupy_the_place_i_want_to_go(move_to_loc_x,
                                                   move_to_loc_y))) {
    set_no_longer_blocked();
    stop_move();
  } else
...
void UnitB::next_move() {
  ...
  err_when(pathResult->loc_x == cur_x_loc() &&
           pathResult->loc_y == cur_y_loc());

  sprite_move(pathResult->loc_x * LOCATE_WIDTH,
              pathResult->loc_y * LOCATE_HEIGHT);
...
void UnitB::set_idle() {
  err_when(cur_x < 0);
  err_when(cur_x != next_x || cur_y != next_y || next_x != go_x ||
           next_y != go_y);
...
// <int> para						- used to count the
// result_path_dist
// <int> ignoreBlockCheck		- whether ignore blocking check or not
//
int UnitB::set_next(int newNextX, int newNextY, int para,
                    int ignoreBlockCheck) {
  ...
  if (!ignoreBlockCheck) {
    ...
    if (isBlocked) {
      ...
      get_object_range(move_to_loc_x, move_to_loc_y, rangeX1, rangeY1, rangeX2,
                       rangeY2);

      if (misc.is_touch(rangeX1, rangeY1, rangeX2, rangeY2, newNextXLoc,
                        newNextYLoc, newNextXLoc + loc_width - 1,
                        newNextYLoc + loc_height - 1)) {
        stop_move();
      } else //------- otherwise wait ---------//
      {
        set_wait();
  ...
  if (steps_remaining > 0) // steps_remaining can be decreased to negative as
                           // set_next() is called when stopping a unit, which
                           // is an addition to the steps it is supposed to move
    steps_remaining--;

  // --------- explore map ---------//

  if (cast_to_Unit())
    cast_to_Unit()->explore_on_move(curNextXLoc, curNextYLoc, newNextXLoc,
                                    newNextYLoc);
...
void UnitB::stop_move() {
  ...
  cur_path_result_id = 0; // reset it so that if the mode is SPRITE_MOVE, it
                          // won't use the old path
}
...
void UnitB::process_wait() {
  ...
  if (can_move(
          (cur_x + stepMagn * move_x_pixel_array[final_dir]) / LOCATE_WIDTH,
          (cur_y + stepMagn * move_y_pixel_array[final_dir]) / LOCATE_WIDTH)) {
    wait_state = 0;
    cur_action = SPRITE_MOVE;
    return;
  } else // find a new path if the thing occupying the next tile no longer move
  {
    /*		int occUnitNum;
                    Unit* occStuff;
                    occUnitNum =
       checking_who_occupy_my_next_tile((cur_x+stepMagn*move_x_pixel_array[final_dir])/LOCATE_WIDTH,
       (cur_y+stepMagn*move_y_pixel_array[final_dir])/LOCATE_WIDTH);
                    ...
                    if( (!occUnitNum || (occUnitNum &&
       (occStuff=unit_array[occUnitNum])
                             && (occStuff->cur_action != SPRITE_MOVE
                             && occStuff->cur_action != SPRITE_WAIT)))
                             || (SEARCH_NEW_PATH_PROBABILITY > misc.random(100)
       && sprite_speed() <= occStuff->sprite_speed() && !is_in_formation()) )
                    {
                            wait_state = 0;
                            move_to(move_to_loc_x, move_to_loc_y);
                    //	retry_state++;
...
void UnitB::process_idle() {
  ...
  if (is_blocked() && cur_order.mode != UNIT_ATTACK)
    process_blocked();
  else if (/*is_in_formation() && */
           is_blocked() &&
           ((next_x_loc() != move_to_loc_x) ||
            (next_y_loc() != move_to_loc_y)) &&
           can_move(move_to_loc_x, move_to_loc_y))
...
//
void UnitB::get_object_range(int objXLoc, int objYLoc, int &rangeX1,
                             int &rangeY1, int &rangeX2, int &rangeY2) {
  ...
  //	int* objLocFlag = (int*)
  //mem_add_clear(loc_width*loc_height*sizeof(int)); 	int* objCargoRecno = (int*)
  //mem_add_clear(loc_width*loc_height*sizeof(int));
  ...
  //			// BUGHERE : objLocFlag is not longer zero (Gilbert
  //26/4) 			if(!objLocFlag[ind] && objCargoRecno[ind] == sprite_recno) //it is
  //myself 				objCargoRecno[ind] = 0; 			js++;
  //		}
  //		is++;
  //	}
...
//	err_when(!checkCargo && !checkFlag);
  ...
  if (loc_width == 1 && loc_height == 1) {
    ...
    short objCargoRecno =
        mobile_type == UNIT_AIR ? locPtr->air_cargo_recno : locPtr->cargo_recno;

    for (rangeY1 = ((objYLoc >= 1) ? (objYLoc - 1) : 0); rangeY1 >= 0;
         rangeY1--) {
      if ((world.get_loc(objXLoc, rangeY1)->loc_flag) != objLocFlag ||
          (mobile_type == UNIT_AIR
               ? (world.get_loc(objXLoc, rangeY1)->air_cargo_recno !=
                  objCargoRecno)
               : (world.get_loc(objXLoc, rangeY1)->cargo_recno !=
                  objCargoRecno))) {
    ...
    for (rangeY2 = ((objYLoc < MAX_WORLD_Y_LOC - 1) ? (objYLoc + 1)
                                                    : (MAX_WORLD_Y_LOC - 1));
         rangeY2 < MAX_WORLD_Y_LOC; rangeY2++) {
      if ((world.get_loc(objXLoc, rangeY2)->loc_flag) != objLocFlag ||
          (mobile_type == UNIT_AIR
               ? (world.get_loc(objXLoc, rangeY2)->air_cargo_recno !=
                  objCargoRecno)
               : (world.get_loc(objXLoc, rangeY2)->cargo_recno !=
                  objCargoRecno))) {
    ...
    for (rangeX1 = ((objXLoc >= 1) ? (objXLoc - 1) : 0); rangeX1 >= 0;
         rangeX1--) {
      if ((world.get_loc(rangeX1, objYLoc)->loc_flag) != objLocFlag ||
          (mobile_type == UNIT_AIR
               ? (world.get_loc(rangeX1, objYLoc)->air_cargo_recno !=
                  objCargoRecno)
               : (world.get_loc(rangeX1, objYLoc)->cargo_recno !=
                  objCargoRecno))) {
    ...
    for (rangeX2 = ((objXLoc < MAX_WORLD_X_LOC - 1) ? (objXLoc + 1)
                                                    : (MAX_WORLD_X_LOC - 1));
         rangeX2 < MAX_WORLD_X_LOC; rangeX2++) {
      if ((world.get_loc(rangeX2, objYLoc)->loc_flag) != objLocFlag ||
          (mobile_type == UNIT_AIR
               ? (world.get_loc(rangeX2, objYLoc)->air_cargo_recno !=
                  objCargoRecno)
               : (world.get_loc(rangeX2, objYLoc)->cargo_recno !=
                  objCargoRecno))) {
        rangeX2--; // go back to the last one which is valid
        break;
      }
    }
  } else // 2 x 2  BUGHERE: only for 2x2 but not for other size other than 2x2
         // and 1x1 ( 3x3 may not work)
  {
    unsigned short objLocFlagTL, objLocFlagTR, objLocFlagBL,
        objLocFlagBR; // top left, top right, bottom ...
    short objCargoRecnoTL, objCargoRecnoTR, objCargoRecnoBL,
        objCargoRecnoBR; // top left, top right, bottom ...
    ...
    if (objXLoc + loc_width <= MAX_WORLD_X_LOC) {
      ...
      objCargoRecnoTR = mobile_type == UNIT_AIR ? locPtr->air_cargo_recno
                                                : locPtr->cargo_recno;
    } else {
    ...
    if (objYLoc + loc_height <= MAX_WORLD_Y_LOC) {
      ...
      objCargoRecnoBL = mobile_type == UNIT_AIR ? locPtr->air_cargo_recno
                                                : locPtr->cargo_recno;
    } else {
    ...
    // get flag and cargo recno of bottom left corner
    if (objXLoc + loc_width <= MAX_WORLD_X_LOC &&
        objYLoc + loc_height <= MAX_WORLD_Y_LOC) {
      locPtr = world.get_loc(objXLoc + loc_width - 1, objYLoc + loc_height - 1);
      objLocFlagBR = locPtr->loc_flag;
      objCargoRecnoBR = mobile_type == UNIT_AIR ? locPtr->air_cargo_recno
                                                : locPtr->cargo_recno;
    } else {
    ...
    if (objCargoRecnoTL == objCargoRecnoTR && objLocFlagTL == objLocFlagTR) {
      ...
      if (objCargoRecnoTL && (objLocFlagTL & LOCATE_BLOCK_MASK)) {
        for (tmpRangeY1 = ((objYLoc >= 1) ? (objYLoc - 1) : 0); tmpRangeY1 >= 0;
             tmpRangeY1--) {
          if ((world.get_loc(objXLoc, tmpRangeY1)->loc_flag) != objLocFlagTL ||
              (mobile_type == UNIT_AIR
                   ? (world.get_loc(objXLoc, tmpRangeY1)->air_cargo_recno !=
                      objCargoRecnoTL)
                   : (world.get_loc(objXLoc, tmpRangeY1)->cargo_recno !=
                      objCargoRecnoTL))) {
            tmpRangeY1++;
            break;
          }
      ...
      if (objCargoRecnoTR && (objLocFlagTR & LOCATE_BLOCK_MASK)) {
        for (tmpRangeY2 = ((objYLoc >= 1) ? (objYLoc - 1) : 0); tmpRangeY2 >= 0;
             tmpRangeY2--) {
          if ((world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)->loc_flag) !=
                  objLocFlagTR ||
              (mobile_type == UNIT_AIR
                   ? (world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)
                          ->air_cargo_recno != objCargoRecnoTR)
                   : (world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)
                          ->cargo_recno != objCargoRecnoTR))) {
            tmpRangeY2++;
            break;
          }
        }
    ...
    if (objCargoRecnoBL == objCargoRecnoBR && objLocFlagBL == objLocFlagBR) {
      if (objCargoRecnoBL && (objLocFlagBL & LOCATE_BLOCK_MASK)) {
        for (rangeY2 = ((objYLoc + loc_height < MAX_WORLD_Y_LOC)
                            ? objYLoc + loc_height
                            : MAX_WORLD_Y_LOC - 1);
             rangeY2 < MAX_WORLD_Y_LOC; rangeY2++) {
      ...
      int tmpRangeY1, tmpRangeY2;
      if (objCargoRecnoBL && (objLocFlagBL & LOCATE_BLOCK_MASK)) {
        for (tmpRangeY1 = ((objYLoc + loc_height < MAX_WORLD_Y_LOC)
                               ? objYLoc + loc_height
                               : MAX_WORLD_Y_LOC - 1);
             tmpRangeY1 < MAX_WORLD_Y_LOC; tmpRangeY1++) {
      ...
      if (objCargoRecnoBR && (objLocFlagBR & LOCATE_BLOCK_MASK)) {
        for (tmpRangeY2 = ((objYLoc + loc_height < MAX_WORLD_Y_LOC)
                               ? objYLoc + loc_height
                               : MAX_WORLD_Y_LOC - 1);
             tmpRangeY2 < MAX_WORLD_Y_LOC; tmpRangeY2++) {
          if ((world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)->loc_flag) !=
                  objLocFlagBR ||
              (mobile_type == UNIT_AIR
                   ? (world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)
                          ->air_cargo_recno != objCargoRecnoBR)
                   : (world.get_loc(objXLoc + loc_width - 1, tmpRangeY2)
                          ->cargo_recno != objCargoRecnoBR))) {
            tmpRangeY2--;
            break;
          }
        }
    ...
    if (objCargoRecnoTL == objCargoRecnoBL && objLocFlagTL == objLocFlagBL) {
      ...
      if (objCargoRecnoTL && (objLocFlagTL & LOCATE_BLOCK_MASK)) {
        for (tmpRangeX1 = ((objXLoc >= 1) ? (objXLoc - 1) : 0); tmpRangeX1 >= 0;
             tmpRangeX1--) {
          if ((world.get_loc(tmpRangeX1, objYLoc)->loc_flag) != objLocFlagTL ||
              (mobile_type == UNIT_AIR
                   ? (world.get_loc(tmpRangeX1, objYLoc)->air_cargo_recno !=
                      objCargoRecnoTL)
                   : (world.get_loc(tmpRangeX1, objYLoc)->cargo_recno !=
                      objCargoRecnoTL))) {
            tmpRangeX1++;
            break;
          }
      ...
      if (objCargoRecnoBL && (objLocFlagBL & LOCATE_BLOCK_MASK)) {
        for (tmpRangeX2 = ((objXLoc >= 1) ? (objXLoc - 1) : 0); tmpRangeX2 >= 0;
             tmpRangeX2--) {
          if ((world.get_loc(tmpRangeX2, objYLoc + loc_height - 1)->loc_flag) !=
                  objLocFlagBL ||
              (mobile_type == UNIT_AIR
                   ? (world.get_loc(tmpRangeX2, objYLoc + loc_height - 1)
                          ->air_cargo_recno != objCargoRecnoBL)
                   : (world.get_loc(tmpRangeX2, objYLoc + loc_height - 1)
                          ->cargo_recno != objCargoRecnoBL))) {
            tmpRangeX2++;
            break;
          }
        }
    ...
    if (objCargoRecnoTR == objCargoRecnoBR && objLocFlagTR == objLocFlagBR) {
      if (objCargoRecnoTR && (objLocFlagTR & LOCATE_BLOCK_MASK)) {
        for (rangeX2 = ((objXLoc + loc_width < MAX_WORLD_X_LOC)
                            ? objXLoc + loc_width
                            : MAX_WORLD_X_LOC - 1);
             rangeX2 < MAX_WORLD_X_LOC; rangeX2++) {
      ...
      int tmpRangeX1, tmpRangeX2;
      if (objCargoRecnoTR && (objLocFlagTR & LOCATE_BLOCK_MASK)) {
        for (tmpRangeX1 = ((objXLoc + loc_width < MAX_WORLD_X_LOC)
                               ? objXLoc + loc_width
                               : MAX_WORLD_X_LOC - 1);
             tmpRangeX1 < MAX_WORLD_X_LOC; tmpRangeX1++) {
      ...
      if (objCargoRecnoBR && (objLocFlagBR & LOCATE_BLOCK_MASK)) {
        for (tmpRangeX2 = ((objXLoc + loc_width < MAX_WORLD_X_LOC)
                               ? objXLoc + loc_width
                               : MAX_WORLD_X_LOC - 1);
             tmpRangeX2 < MAX_WORLD_X_LOC; tmpRangeX2++) {
          if (world.get_loc(tmpRangeX2, objYLoc + loc_height - 1)->loc_flag !=
                  objLocFlagBR ||
              (mobile_type == UNIT_AIR
                   ? (world.get_loc(tmpRangeX2, objYLoc)->air_cargo_recno !=
                      objCargoRecnoBR)
                   : (world.get_loc(tmpRangeX2, objYLoc)->cargo_recno !=
                      objCargoRecnoBR))) {
            tmpRangeX2--;
            break;
          }
...
bool UnitB::handle_blocking() {
  ...
  // setting the unit himself as not occupying the tile, otherwise it will found
  // out that he himself occupying the tile
  ...
  // would like to check first the direction of the destination, it makes more
  // sense
  int diffX = destX - lX;
  int diffY = destY - lY;
  ...
  while (!handleSuccessfully && directionAngle < (firstTimeAngle + 360)) {
    occupyingUnitRecordNumber =
        checking_who_occupy_the_place_i_want_to_go(checkingLocX, checkingLocY);
    ...
    // do it only when it is of the same nationality and is a unit and is not a
    // troop let the blocking unit to move to the direction of the blocked unit
    // want to go to

    if (theOccupyingObject &&
        theOccupyingObject->nation_recno == nation_recno) {
      ...
      // if the blocking unit successfully walk away, the blocked unit can then
      // try to find a new path to the destination
      if (theOccupyingObject->can_move(theMoveToPlaceX, theMoveToPlaceY) ||
          theOccupyingObject->checking_who_occupy_the_place_i_want_to_go(
              theMoveToPlaceX, theMoveToPlaceY)) {
        theOccupyingObject->move_to(theMoveToPlaceX, theMoveToPlaceY, false);
        if (theOccupyingObject->cur_action == SPRITE_MOVE ||
            theOccupyingObject->cur_action == SPRITE_WAIT) {
          stop_move();
...
//------------Begin of function
//Unit::checking_who_occupy_the_place_i_want_to_go----------//
//
// This function checks who occupy the place that the unit wanted to go to
//
// <int> checkingLocX and checkingLocY ----the coordinate of the tile that the
// unit wanted to go
//
// return type <int> ----- the unit record number of the unit that occupys the
// tile is returned
//
int UnitB::checking_who_occupy_the_place_i_want_to_go(int checkingLocX,
                                                      int checkingLocY) {
  ...
  // only found those unit that are of the same nation of the said unit and is
  // not moving
  // it is because only this type of units should walk away and give out the
  // position if the occupying unit is moving, no need to consider since the
  // tile will soon be freed
  ...
  while (!occupyingUnitRecordNumber && is < obj_loc_width()) {
    ...
    while (!occupyingUnitRecordNumber && js < obj_loc_height()) {
      if (temperoryCheckingLocY >= MAX_WORLD_Y_LOC ||
          temperoryCheckingLocY < 0) {
        js++;
      ...
      occupyingUnitRecordNumber =
          world.get_loc(temperoryCheckingLocX, temperoryCheckingLocY)
              ->unit_recno(mobile_type);
      if (occupyingUnitRecordNumber &&
          ((occupyingUnitRecordNumber == sprite_recno) ||
           (theOccupyingStuff = unit_array[occupyingUnitRecordNumber])
                   ->nation_recno != nation_recno ||
...
bool UnitB::is_dead() { return hit_points <= 0 || cur_action == SPRITE_DIE; }
...
//					return false if the location cannot be
//walk
bool UnitB::can_move(int locX, int locY) {
  ...
  //			if(locX+i>= MAX_WORLD_X_LOC || locX+i <0 || locY+j >=
  //MAX_WORLD_Y_LOC || locY+j < 0 || (!locPtr->can_move(obj_mobile_type()) &&
  //(locPtr->unit_recno(obj_mobile_type()) != sprite_recno))) 				return false;
  //		}
  //	}

  if (loc_width == 1) {
    err_when(loc_height != 1);
    if (locX < 0 || locY < 0 || locX >= MAX_WORLD_X_LOC ||
        locY >= MAX_WORLD_Y_LOC)
      return false;
    ...
    int i, j;
    if (locX < 0 || locY < 0 || locX + loc_width > MAX_WORLD_X_LOC ||
        locY + loc_height > MAX_WORLD_Y_LOC)
      return false;
...
//
// check a specific location can be walk, taking care that if that location is
// occupied by a walking or waiting unit, that location will later on be freed.
//
// <int> locX - x-coordinate
// <int> locY - y-coordinate
//
...
bool UnitB::can_move2(int locX, int locY) {
  ...
  //			locPtr = world.get_loc(locX+i, locY+j);
  //			if(locX+i>= MAX_WORLD_X_LOC || locX+i <0 || locY+j >=
  //MAX_WORLD_Y_LOC || locY+j < 0 ) 				return false; 			else
  //if(!locPtr->can_move(mobile_type))
  //			{
  ...
  //					if(unitPtr->cur_action == SPRITE_WAIT ||
  //unitPtr->cur_action == SPRITE_MOVE) 						continue; 					else 						return false;
  //				}
  //			}
  //		}
  //	}
...
//------------ Begin of function
//UnitB::checking_who_occupy_my_next_tile-------//
int UnitB::checking_who_occupy_my_next_tile(int checkingLocX,
                                            int checkingLocY) {
  int is, js, occupyingUnitRecordNumber = 0;
  int temperoryCheckingLocX, temperoryCheckingLocY;
