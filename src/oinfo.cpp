...
#define FIRST_REMAP_KEY                                                        \
  0xE0 // the source color code of the colors to be remapped
...
static short remap_location_array[] = {
    ...
    0xA4, 0xC4, 0xA8, 0xC8, 0xAC, 0xCC, 0xB0,
    0xD0, 0xB4, 0xD4, 0xB8, 0xD8, 0xBC, 0xDC,
};
...
//
Info::Info()
    : report_array(sizeof(short), 50), report_array2(sizeof(short), 50),
...
void Info::init(int yyyy, int mm, int dd) {
  ...
  goal_deadline =
      date.julian(date.year(game_start_date) + config.goal_year_limit,
                  date.month(game_start_date), date.day(game_start_date));
  ...
  err_when(
      MAX_REMOTE_CHAT_STR <
      DISP_NEWS_COUNT); // it must not fewer then the maximum number of news
                        // that can be displayed on the screen at a time.
}
...
//									random seed. otherwise a
//random seed will be 								   picked. 								   (default:0)
//
...
void Info::disp_panel() {
  ...
  if (!info_background_bitmap)
    info_background_bitmap = (short *)mem_add(
        BitmapW::size(INFO_X2 - INFO_X1 + 1, INFO_Y2 - INFO_Y1 + 1));
  vga_back.read_bitmapW(INFO_X1, INFO_Y1, INFO_X2, INFO_Y2,
                        info_background_bitmap);

  if (!heading_background_bitmap)
    heading_background_bitmap = (short *)mem_add(BitmapW::size(
        TOP_MENU_X2 - TOP_MENU_X1 + 1, TOP_MENU_Y2 - TOP_MENU_Y1 + 1));
  vga_back.read_bitmapW(TOP_MENU_X1, TOP_MENU_Y1, TOP_MENU_X2, TOP_MENU_Y2,
                        heading_background_bitmap);

  //	if (current_display_mode.mode_id == MODE_ID_1024x768x16)
  //	{
  //		if( !heading_background_bitmap2 )
  //			heading_background_bitmap2 = (short *)mem_add( BitmapW::size(31,
  //101) ); 		vga_back.read_bitmapW( REPU_BUTTON_X1, REPU_BUTTON_Y1,
  //REPU_BUTTON_X1 +100, REPU_BUTTON_Y1 +30, heading_background_bitmap2 );
  //	}
}
...
//
void Info::disp() {
  //	mouse.handle_flicking = 0;		// since it will be called by Firm
  //detect functions directly which may have set mouse.handle_flicking to 1
  //first, so we need to cancel it here
  ...
  //	vga.active_buf->put_bitmap_trans_remap(INFO_X1+5, INFO_Y1-255,
  //nationPict, game.get_color_remap_table(nation_array.player_recno, 0) );

  vga.active_buf->put_bitmap_trans(INFO_X1 + 3, INFO_Y1 - 125,
                                   image_gameif.read("REMSCR"));

  //	if (spy_execute)
  //	{
  //		vga.active_buf->put_bitmap( INFO_X1+17, INFO_Y1+96,
  //image_gameif.read("EXECUTE") ); 		font_whbl.center_put( INFO_X1 +11, INFO_Y1
  //+100, INFO_X1 +219, INFO_Y1 +130, "You have just executed a spy");
  //	}
  ...
  // ##### begin Gilbert 1/2 ######//
  //	if( scenario_editor.is_enable() && scenario_editor.edit_mode !=
  //SEDIT_MODE_NONE)
  //	{
  ...
  // ####### begin Gilbert 23/2 ######//
  // if in scenario editor mode, call repaint to both normal interface and
  // editor interface such that normal interface is called with INFO REPAINT
  // when changing selected object in editor's mode
...
void Info::update() {
  ...
  // ##### begin Gilbert 13/10 ######//
  if (scenario_editor.is_enable() &&
      scenario_editor.edit_mode != SEDIT_MODE_NONE) {
...
#ifdef DEBUG
    if (town_array.selected_recno || site_array.selected_recno ||
...
#ifdef DEBUG
    if (firm_array.selected_recno || site_array.selected_recno ||
        unit_array.selected_recno) {
      err.msg("invalid selected_recno");
    }
#endif
...
#ifdef DEBUG
    if (firm_array.selected_recno || town_array.selected_recno ||
        unit_array.selected_recno) {
      err.msg("invalid selected_recno");
    }
#endif
...
void Info::disp_heading() {
  ...
  //	if (current_display_mode.mode_id == MODE_ID_1024x768x16)
  //		vga_back.put_bitmapW(REPU_BUTTON_X1, REPU_BUTTON_Y1,
  //heading_background_bitmap2);
  vga.use_back();
#endif
  ...
  if (game_speed_info_display_delay > 0 && config.frame_speed > 0) {
    ...
    str += misc.format(i, 4);
    font_news.center_put(ZOOM_X1, ZOOM_Y1, ZOOM_X2,
                         ZOOM_Y2 - (font_news.height() << 1), str);
  ...
  else if (game_pause_info_display_delay > 0 && config.frame_speed > 0) {
    image_spict.put_back(((ZOOM_X1 + ZOOM_X2) >> 1) - 109,
  ...
  if (display_hot_key > 2 && !option_menu.is_active()) {
    ...
    FileTxt fileTxt(
        filePtr, dataSize); // initialize fileTxt with an existing file stream
    if (dataSize > hot_key_buf_size) {
      hot_key_buf = mem_resize(hot_key_buf,
                               dataSize); // allocate a buffer larger than we
    ...
    //	font_mid.center_put_paragraph( 0, 0, VGA_WIDTH, VGA_HEIGHT, textPtr, 2,
    //0, 0 );
    Campaign::put_center_text(107 + ZOOM_X1, ZOOM_Y1, textPtr, 0, &font_whbl,
                              0);
  ...
  font_snds.disp(DATE_X1, DATE_Y1, date.date_str(game_date, 1),
                 DATE_X1 + DATE_LENGTH, 1);
  ...
  if (!nation_array.player_recno) // the player has lost the game
  {
    ...
    if (current_display_mode.mode_id == MODE_ID_800x600x16) {
      font_snds.disp(FOOD_X1, FOOD_Y1, "",
                     FOOD_X1 + FOOD_LENGTH); // clear the display
    ...
    if (current_display_mode.mode_id == MODE_ID_800x600x16)
      font_snds.disp(REPU_BUTTON_X1 + 30, REPU_BUTTON_Y1 + 12, "",
                     REPU_BUTTON_X1 + 100);
    else if (current_display_mode.mode_id == MODE_ID_1024x768x16) {
      if (config.race_id > 0) {
        font_snds.disp(REPU_BUTTON_X1 + 12, REPU_BUTTON_Y1 + 27, "",
                       REPU_BUTTON_X1 + 50);
        font_snds.disp(REPU_BUTTON_X1 + 2, REPU_BUTTON_Y1 + 39, "",
                       REPU_BUTTON_X1 + 50);
      } else {
        font_snds.disp(REPU_BUTTON_X1 + 5, REPU_BUTTON_Y1 + 27, "",
                       REPU_BUTTON_X1 + 50);
        font_snds.disp(REPU_BUTTON_X1 - 2, REPU_BUTTON_Y1 + 39, "",
                       REPU_BUTTON_X1 + 50);
      }
    ...
    if (current_display_mode.mode_id == MODE_ID_800x600x16) {
      ...
      if (strPtr[2] == '-')
        ...
        font_snds.disp(FOOD_X1 - 7, FOOD_Y1 + 12, strPtr,
                       FOOD_X1 + FOOD_LENGTH);
    } else
      err_here();
    ...
    if (current_display_mode.mode_id == MODE_ID_800x600x16) {
      ...
      if (strPtr[2] == '-')
        ...
        font_snds.disp(CASH_X1 - 7, CASH_Y1 + 12, strPtr,
                       CASH_X1 + CASH_LENGTH);
    } else
      err_here();
    ...
    if (config.race_id > 0) {
      if (current_display_mode.mode_id == MODE_ID_800x600x16)
        thumbBmp = image_icon.get_ptr(changeAmt >= 0 ? (char *)"REPU_UP"
                                                     : (char *)"REPU_DW");
      else if (current_display_mode.mode_id == MODE_ID_1024x768x16)
        thumbBmp = image_icon.get_ptr(changeAmt >= 0 ? (char *)"REUP1024"
                                                     : (char *)"REDW1024");
      else
        err_here();
      vga.active_buf->put_bitmap(REPU_BUTTON_X1, REPU_BUTTON_Y1, thumbBmp);
    ...
    if (dispValue >= 0) {
      str = misc.format((int)dispValue,
                        4); // format type 4 - no thousand separators
      if (current_display_mode.mode_id == MODE_ID_800x600x16) {
        tempx = font_snds.disp(REPU_BUTTON_X1 + 30, REPU_BUTTON_Y1 + 12, str,
                               REPU_BUTTON_X1 + 120);
      } else if (current_display_mode.mode_id == MODE_ID_1024x768x16) {
        if (config.race_id > 0)
          font_snds.disp(REPU_BUTTON_X1 + 12, REPU_BUTTON_Y1 + 27, str,
                         REPU_BUTTON_X1 + 50);
        else
          font_snds.disp(REPU_BUTTON_X1 + 5, REPU_BUTTON_Y1 + 27, str,
                         REPU_BUTTON_X1 + 50);
      } else
        err_here();
    } else {
      ...
      str += misc.format((int)-dispValue,
                         4); // format type 4 - no thousand separators
      if (current_display_mode.mode_id == MODE_ID_800x600x16) {
        tempx = font_red.disp(REPU_BUTTON_X1 + 30, REPU_BUTTON_Y1 + 12, str,
                              REPU_BUTTON_X1 + 120);
      } else if (current_display_mode.mode_id == MODE_ID_1024x768x16) {
        if (config.race_id > 0)
          font_red.disp(REPU_BUTTON_X1 + 12, REPU_BUTTON_Y1 + 27, str,
                        REPU_BUTTON_X1 + 50);
        else
          font_red.disp(REPU_BUTTON_X1 + 5, REPU_BUTTON_Y1 + 27, str,
                        REPU_BUTTON_X1 + 50);
      } else
        err_here();
    }
    ...
    if (dispChange) {
      ...
      if (str[2] == '-') {
        if (current_display_mode.mode_id == MODE_ID_800x600x16) {
          ...
          if (config.race_id > 0)
            font_red.disp(REPU_BUTTON_X1 + 2, REPU_BUTTON_Y1 + 39, str,
                          REPU_BUTTON_X1 + 50);
          else
            font_red.disp(REPU_BUTTON_X1 - 2, REPU_BUTTON_Y1 + 39, str,
                          REPU_BUTTON_X1 + 50);
        ...
        if (current_display_mode.mode_id == MODE_ID_800x600x16) {
          ...
          if (config.race_id > 0)
            font_snds.disp(REPU_BUTTON_X1 + 2, REPU_BUTTON_Y1 + 39, str,
                           REPU_BUTTON_X1 + 50);
          else
            font_snds.disp(REPU_BUTTON_X1 - 2, REPU_BUTTON_Y1 + 39, str,
                           REPU_BUTTON_X1 + 50);
...
void Info::process_viewing_spy() {
  ...
  if (spy_array.is_deleted(viewing_spy_recno)) // the spy is dead
  {
    ...
    if (spyPtr->spy_place_nation_recno() != info.viewing_nation_recno) {
      ...
      /*
              //--- on average, a spy will get caught in 5 to 15 days when
         viewing the secret of its enemy ---//

              if( misc2.random(5+spyPtr->spy_skill/5) )		// use misc2 to avoid
...
char *Info::game_duration_str() {
  ...
  //-- For scenarios (whose goal_difficulty are > 0 ), the actual game start
  //date is no info.game_start_date, we must calculate it

  if (info.goal_difficulty) {
    gameStartDate = date.julian(
        date.year(info.goal_deadline) - config.goal_year_limit,
        date.month(info.goal_deadline), date.day(info.goal_deadline));
  } else {
...
void Info::save_game_scr() {
  ...
  if (0 < ZOOM_Y1) {
    ...
    save_buf_1b = vga_back.save_area(
        0, 0, VGA_WIDTH - 1,
        ZOOM_Y1 - 1); // save the back buffer also as the top area of the back
                      // buf is used for font display
  } else {
  ...
  if (ZOOM_Y2 < VGA_HEIGHT - 1) {
    save_buf_2 =
        vga_front.save_area(0, ZOOM_Y2 + 1, VGA_WIDTH - 1, VGA_HEIGHT - 1);
#ifdef USE_FLIP
    save_buf_2b =
        vga_back.save_area(0, ZOOM_Y2 + 1, VGA_WIDTH - 1, VGA_HEIGHT - 1);
#endif
  } else {
    save_buf_2 = NULL;
    save_buf_2b = NULL;
  ...
  if (ZOOM_X2 < VGA_WIDTH - 1) {
    save_buf_4 =
        vga_front.save_area(ZOOM_X2 + 1, ZOOM_Y1, VGA_WIDTH - 1, ZOOM_Y2);
#ifdef USE_FLIP
    save_buf_4b =
        vga_back.save_area(ZOOM_X2 + 1, ZOOM_Y1, VGA_WIDTH - 1, ZOOM_Y2);
#endif
  } else {
    save_buf_4 = NULL;
    save_buf_4b = NULL;
...
//
int Info::disp_loyalty(int x, int y, int x2, int curLoyalty, int targetLoyalty,
                       int nationRecno, int refreshFlag, int useBlueFont) {
  int endX;

  if (x < x2) // if x==x2, don't display the field name.
    //		(useBlueFont?font_blu2:font_snds).put( x, y, "Loyalty", 1, x2-1
    //);
    (useBlueFont ? font_blu2 : font_snds)
        .put(x, y, text_unit.str_loyalty(), 1, x2 - 1);

  endX = font_snds.put(x2, y, curLoyalty, 1); // display current loyalty

  if (nation_array[nationRecno]->cash <=
      0) // if the nation no longer has money to pay the unit
    targetLoyalty = 0;

  if (targetLoyalty > curLoyalty) {
    // ##### begin Gilbert 22/12 #####//
    vga.active_buf->put_bitmap_trans(endX + 3, y + 2,
                                     image_icon.read("ARROWUP"));
    // ##### end Gilbert 22/12 #####//
    endX += 10;
...
// <int> unitId       unit id
// <int> nationRecno  nation recno of the unit, to get color table, (negative
// for color scheme id) <int> windowX1,Y1,X2,Y2		clipping window <int>
// para			 0=draw icon only
//                    bit 0=draw icon
...
//							 other upper bits used as the level
//of darkness
//
...
int Info::draw_unit_icon(int x, int y, int unitId, int nationRecno,
                         int windowX1, int windowY1, int windowX2, int windowY2,
                         int para) {
  ...
  if (x1 >= windowX1 && x2 <= windowX2 && y1 >= windowY1 && y2 <= windowY2) {
    ...
    if (para & 1) {
      ...
      if (para & 2) {
        ...
        for (j = 0; j < 4; j++)
          colorRemapTable[FIRST_REMAP_KEY + j] =
              colorRemapTable[remap_location_array[colorSchemeId * 2] + j];
        for (j = 0; j < 4; j++)
          colorRemapTable[FIRST_REMAP_KEY + 4 + j] =
              colorRemapTable[remap_location_array[colorSchemeId * 2 + 1] + j];
      } else if (para & 32) {
        // ------- darken the icon --------//
        colorRemapTable =
            (short *)vga.vga_color_table->get_table(-((para >> 6) + 5));
        for (j = 0; j < 8; j++)
          colour_buffer[j] = colorRemapTable[FIRST_REMAP_KEY + j];

        //	for( j=0 ; j<8 ; j++ )
        //		colorRemapTable[FIRST_REMAP_KEY+j] = (short)
        //vga.make_pixel(customized_color_code[colorSchemeId*24+j*3],
        //			customized_color_code[colorSchemeId*24+j*3+1],
        //customized_color_code[colorSchemeId*24+j*3+2]);
        for (j = 0; j < 4; j++)
          colorRemapTable[FIRST_REMAP_KEY + j] =
              colorRemapTable[remap_location_array[colorSchemeId * 2] + j];
        for (j = 0; j < 4; j++)
      ...
      vga.active_buf->put_bitmap_trans_remap_decompress(
          x1, y1, (char *)bitmapPtr, colorRemapTable);
      if (para & 32 || para & 2) {
        for (j = 0; j < 8; j++)
    ...
    if (para & 6) {
      ...
      int p = para & 6;
      if ((para & 4) &&
          !mouse.in_area(x1, y1, x2, y2)) // if not on cursor, clear bit1
      {
        rc &= ~4;
        p &= ~4;
      }
    ...
    if (para & 8 &&
        (mouse.any_click(x1, y1, x2, y2, LEFT_BUTTON) == 0 ||
         (game.game_mode == GAME_TUTORIAL &&
          !tutor.allow_unit_icon_click(x1, y1, unitInfo, para & 32, 1)))) {
      rc &= ~8; // clear bit 3
    }
    ...
    if (para & 16 &&
        (mouse.any_click(x1, y1, x2, y2, RIGHT_BUTTON) == 0 ||
         (game.game_mode == GAME_TUTORIAL &&
          !tutor.allow_unit_icon_click(x1, y1, unitInfo, para & 32, 2))))
    ...
    if (para & 1) {
      ...
      vga.active_buf->put_bitmap_area_trans_remap_decompress(
          x1, y1, (char *)bitmapPtr, srcX1, srcY1, srcX2, srcY2,
          colorRemapTable);
    ...
    if (para & 6) {
      ...
      if (p) // draw rectangle
      {
        if (x1 >= windowX1) // draw left side
          vga.active_buf->bar(x1, y1 + srcY1, x1, y1 + srcY2,
                              colorArray[p >> 1]);
        if (x2 <= windowX2) // draw right side
          vga.active_buf->bar(x2, y1 + srcY1, x2, y1 + srcY2,
                              colorArray[p >> 1]);
        if (y1 >= windowY1) // draw top side
          vga.active_buf->bar(x1 + srcX1, y1, x1 + srcX2, y1,
                              colorArray[p >> 1]);
        if (y2 <= windowY2) // draw bottom side
          vga.active_buf->bar(x1 + srcX1, y2, x1 + srcX2, y2,
                              colorArray[p >> 1]);
      }
    }
    ...
    if (para & 8 &&
        (mouse.any_click(x1 + srcX1, y1 + srcY1, x1 + srcX2, y1 + srcY2,
                         LEFT_BUTTON) == 0 ||
         (game.game_mode == GAME_TUTORIAL &&
          !tutor.allow_unit_icon_click(x1, y1, unitInfo, para & 32, 1)))) {
      rc &= ~8; // clear bit 3
