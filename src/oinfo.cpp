...
static short remap_location_array[] = {
    0xBC,
    0xDC, // the first remap table is for independent units
    0xA0,
    0xC0, // following are eight remap table for each color code
    0xA4, 0xC4, 0xA8, 0xC8, 0xAC, 0xCC, 0xB0, 0xD0, 0xB4, 0xD4, 0xB8, 0xD8, 0xBC, 0xDC,
};
...
//
Info::Info()
    : report_array(sizeof(short), 50), report_array2(sizeof(short), 50), talk_msg_disp_array(sizeof(TalkMsgDisp), 50)
{
...
void Info::init(int yyyy, int mm, int dd)
{
    ...
    goal_deadline = date.julian(date.year(game_start_date) + config.goal_year_limit, date.month(game_start_date),
    ...
    err_when(MAX_REMOTE_CHAT_STR < DISP_NEWS_COUNT); // it must not fewer then the maximum number of news
...
void Info::disp_panel()
{
    ...
    if (!heading_background_bitmap)
        heading_background_bitmap =
            (short *)mem_add(BitmapW::size(TOP_MENU_X2 - TOP_MENU_X1 + 1, TOP_MENU_Y2 - TOP_MENU_Y1 + 1));
    ...
    //	{
    //		if( !heading_background_bitmap2 )
    //			heading_background_bitmap2 = (short *)mem_add(
    // BitmapW::size(31,
...
void Info::disp()
{
    //	mouse.handle_flicking = 0;		// since it will be called by
    // Firm
    ...
    // image_gameif.read("EXECUTE") ); 		font_whbl.center_put( INFO_X1 +11,
    // INFO_Y1
...
void Info::disp_heading()
{
    ...
    if (config.frame_speed == 0)
        image_spict.put_back(((ZOOM_X1 + ZOOM_X2) >> 1) - 109, ((ZOOM_Y1 + ZOOM_Y2) >> 1) + 10, "GPAUSE", 1);
    else if (game_pause_info_display_delay > 0 && config.frame_speed > 0)
    {
        image_spict.put_back(((ZOOM_X1 + ZOOM_X2) >> 1) - 109, ((ZOOM_Y1 + ZOOM_Y2) >> 1) + 10, "GRESUME", 1);
        ...
        else
        {
            /*
               ...
               viewing the secret of its enemy ---//
                    ...
                    if( misc2.random(5+spyPtr->spy_skill/5) )		// use misc2 to
               avoid multiplayer sync problem
                    {
...
void Info::save_game_scr()
{
    ...
    if (0 < ZOOM_Y1)
    {
        ...
        save_buf_1b = vga_back.save_area(0, 0, VGA_WIDTH - 1,
...
//
int Info::draw_unit_icon(int x, int y, int unitId, int nationRecno, int windowX1, int windowY1, int windowX2,
                         int windowY2, int para)
{
    ...
    else
    {
        ...
        if (para & 1)
        {
            ...
            vga.active_buf->put_bitmap_area_trans_remap_decompress(x1, y1, (char *)bitmapPtr, srcX1, srcY1, srcX2,
