...
//
SpriteArray::SpriteArray(int initArraySize)
    : DynArrayB(sizeof(Sprite *), initArraySize, DEFAULT_REUSE_INTERVAL_DAYS) {}
//----------- End of function SpriteArray::SpriteArray ---------//

//--------- Begin of function SpriteArray::~SpriteArray ---------//
//
SpriteArray::~SpriteArray() { deinit(); }
//----------- End of function SpriteArray::~SpriteArray ---------//
...
//
// Note: it does not call Sprite::init_recno() as it is supposed to be used by
// disp_sprite_array
...
void SpriteArray::add_sorted(Sprite *newSprite) {
  ...
  while (r > l) {
    ...
    testY = operator[](x + 1)
                ->abs_y2; // the one to be compared with the adding one.
...
#define SYS_YIELD_INTERVAL 20
  ...
  int sysYieldCount = size() - size() % SYS_YIELD_INTERVAL;
  ...
  for (int i = 1; i <= size(); i++) {
    ...
    if (spritePtr->is_freezed()) {
      spritePtr->freeze_frame_left--;
      continue;
    }
    ...
    if (spritePtr->cur_x == -1) // cur_x == -1 if the unit has removed from the
                                // map and gone into a firm
      continue;

    //------- process sprite --------//

    err_when(!spritePtr->sprite_info->need_turning &&
             spritePtr->cur_dir != spritePtr->final_dir);

#ifdef DEBUG
    unsigned long profileStartTime = misc.get_time();
#endif

    spritePtr->pre_process(); // it's actually calling Unit::pre_process() and
                              // other derived Unit classes
    ...
    if (is_deleted(i)) // in case pre_process() kills the current Sprite
      continue;

    if (spritePtr->cur_x <
        0) // if( spritePtr->cur_x == -1 || spritePtr->cur_x==-2)
      continue;
    ...
    case SPRITE_READY_TO_MOVE:
      spritePtr->cur_action =
          SPRITE_IDLE; // to avoid problems of insensitive of mouse cursor
      spritePtr->process_idle();
      break;
