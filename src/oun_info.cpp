...
void Unit::set_combat_level(int combatLevel) {
  ...
  if (combat_level() <= MAX_COMBAT_TRAIN)
    ...
    max_power =
        combat_level() / 10 + (MAX_COMBAT_TRAIN + 50 - MAX_COMBAT_TRAIN / 10);
...
void Unit::set_rank(int rankId) {
  ...
  err_when(unit_mode != UNIT_MODE_REBEL && rankId >= RANK_GENERAL &&
           is_civilian());
  ...
  else if (rankId < rank_id &&
           rank_id != RANK_KING) // no decrease in loyalty if a spy king hands
                                 // his nation to his parent nation and become a
                                 // general again
  ...
  if (nation_recno) {
    ...
    if (rank_id == RANK_KING && rankId != RANK_KING)
      unitInfo->inc_nation_unit_count(
          nation_recno); // since kings are not included in nation_unit_count,
                         // when it is no longer a king, we need to re-increase
                         // it.

    //------ if promote a unit to a king ------//

    if (rank_id != RANK_KING && rankId == RANK_KING)
      unitInfo->dec_nation_unit_count(
          nation_recno); // since kings are not included in nation_unit_count,
  ...
  if (rank_id == RANK_GENERAL && rankId == RANK_SOLDIER) {
    ...
    for (int i = unit_array.size(); i > 0; i--) {
      Unit *unitPtr =
          (Unit *)unit_array.get_ptr(i); // don't use is_deleted() as it filters
                                         // out units that are currently dying
  ...
  //----- if this is a soldier being promoted to a general -----//
    ...
    if (leader_unit_recno) {
      if (!unit_array.is_deleted(
              leader_unit_recno)) // the leader unit may have been killed at the
                                  // same time
      {
...
void Unit::set_spy(int spyRecno) { spy_recno = spyRecno; }
...
const char *Unit::unit_name(int withTitle, int firstNameOnly) {
  ...
  if (race_id && unit_id != UNIT_WAGON) {
    ...
    return text_unit.str_non_race_unit_name(
        ...
        (unitInfo->unit_class != UNIT_CLASS_GOD
             ? name_id
             : 0), // don't put serial no. to god
        kingAnimal);
  }
...
//
int Unit::is_own() { return UnitB::is_own() || is_own_spy(); }
...
//
int Unit::is_civilian() { return unit_res[unit_id]->is_civilian; }
...
int Unit::is_camouflage(int viewerNation) {
  ...
  if (spy_recno && (spyPtr = spy_array[spy_recno]) &&
      spyPtr->true_nation_recno != viewerNation && spyPtr->camouflage_count > 0)
    return 1;
  else if (item.ability(ITEM_ABILITY_INVISIBLE) && !is_nation(viewerNation))
    return 1; // do not call item.use_now(), not appropriate. Let next day to
              // decrease item.para
  // ##### patch begin Gilbert 14/7 #####//
  //	else if( unit_id == UNIT_JAPANESE_SPU && !is_nation(viewerNation) &&
  //rank_id != RANK_KING ) 		return 1;
...
int Unit::is_stealth() {
  // ####### begin Gilbert 9/2 #######//
  //	return (config.fog_of_war && world.get_loc(next_x_loc(),
  //next_y_loc())->visibility() < unit_res[unit_id]->stealth)
  //		|| spy_recno && is_camouflage( nation_array.player_recno );
  //// check spy_recno to reduce calling to is_camouflage()
  ...
  if (config.blacken_map) {
    if (config.fog_of_war) {
      if (loc_width == 1) {
        return world.get_loc(next_x_loc(), next_y_loc())->visibility() <
               unit_res[unit_id]->stealth;
      } else {
        int x1 = next_x_loc();
        int y1 = next_y_loc();
...
int Unit::commander_power() {
  ...
  if (unit_mode == UNIT_MODE_OVERSEE) {
    ...
    if (firmPtr->cast_to_FirmCamp()) {
      ...
      commanderPower +=
          firmPtr->cast_to_FirmCamp()->soldier_count * 3; // 0 to 24
...
int Unit::obj_power() {
  ...
  if (!unitInfo->class_info.has_combat_level) {
    // return (int) hit_points + (unitInfo->weapon_power + get_weapon_version()
    // - 1) * 15;
...
int Unit::can_resign() {
  // ###### begin Gilbert 16/5 ########//
  return rank_id != RANK_KING && !game.is_unit_in_condition(unique_id);
  // ###### end Gilbert 16/5 ########//
}
...
int Unit::can_succeed_king() {
  ...
  if (nationPtr->is_human()) {
    //-- only the nation is a human nation, only human military unit can succeed
    //the king --//
