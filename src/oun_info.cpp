...
void Unit::set_rank(int rankId)
{
    ...
    else if (rankId < rank_id && rank_id != RANK_KING) // no decrease in loyalty if a spy king hands
    ...
    if (nation_recno)
    {
        ...
        if (rank_id == RANK_KING && rankId != RANK_KING)
            unitInfo->inc_nation_unit_count(nation_recno); // since kings are not included in nation_unit_count,
        ...
        if (rank_id != RANK_KING && rankId == RANK_KING)
            unitInfo->dec_nation_unit_count(nation_recno); // since kings are not included in nation_unit_count,
    ...
    if (rank_id == RANK_GENERAL && rankId == RANK_SOLDIER)
    {
        ...
        {
            Unit *unitPtr = (Unit *)unit_array.get_ptr(i); // don't use is_deleted() as it filters
    ...
    else if (rank_id == RANK_SOLDIER && rankId == RANK_GENERAL)
    {
        ...
        {
            if (!unit_array.is_deleted(leader_unit_recno)) // the leader unit may have been killed at the
                                                           // same time
            {
...
const char *Unit::unit_name(int withTitle, int firstNameOnly)
{
    ...
    else
    {
        ...
        return text_unit.str_non_race_unit_name(
            unitInfo->name, (unitInfo->class_info.has_weapon_version ? get_weapon_version() : 0),
...
int Unit::is_camouflage(int viewerNation)
{
    ...
    if (spy_recno && (spyPtr = spy_array[spy_recno]) && spyPtr->true_nation_recno != viewerNation &&
        spyPtr->camouflage_count > 0)
        return 1;
...
int Unit::is_stealth()
{
    ...
    if (config.blacken_map)
    {
        ...
        else if (!config.explore_whole_map)
        {
            ...
            {
                return !world.is_explored(next_x_loc(), next_y_loc(), next_x_loc() + loc_width - 1,
...
int Unit::get_cur_loc(short &xLoc, short &yLoc)
{
    ...
    else if (unit_mode == UNIT_MODE_OVERSEE || unit_mode == UNIT_MODE_CONSTRUCT_FIRM ||
