...
static short table_start_up_cash[8] = {OPTION_VERY_HIGH, OPTION_VERY_HIGH, OPTION_HIGH, OPTION_MODERATE,
                                       OPTION_MODERATE,  OPTION_MODERATE,  OPTION_LOW,  OPTION_LOW};
static short table_ai_start_up_cash[8] = {OPTION_LOW,      OPTION_LOW,  OPTION_MODERATE,  OPTION_MODERATE,
                                          OPTION_MODERATE, OPTION_HIGH, OPTION_VERY_HIGH, OPTION_VERY_HIGH};
static char table_ai_aggressiveness[8] = {OPTION_NONE,     OPTION_NONE, OPTION_LOW,  OPTION_MODERATE,
...
static char table_independent_town_resistance[8] = {OPTION_LOW,      OPTION_LOW,  OPTION_MODERATE, OPTION_MODERATE,
                                                    OPTION_MODERATE, OPTION_HIGH, OPTION_HIGH,     OPTION_HIGH};
static char table_random_event_frequency[8] = {OPTION_NONE, OPTION_NONE,     OPTION_LOW,      OPTION_LOW,
                                               OPTION_LOW,  OPTION_MODERATE, OPTION_MODERATE, OPTION_MODERATE};
static char table_new_nation_emerge[8] = {0, 0, 0, 1, 1, 1, 1, 1};
static char table_monster_type[8] = {OPTION_MONSTER_INTELLIGENT, OPTION_MONSTER_INTELLIGENT, OPTION_MONSTER_INTELLIGENT,
                                     OPTION_MONSTER_INTELLIGENT, OPTION_MONSTER_INTELLIGENT, OPTION_MONSTER_INTELLIGENT,
...
int Config::single_player_difficulty(int raceId)
{
    ...
    //	else
    //	{
    //		if( start_up_has_mine_nearby )		// stronger human,
    // weaker
    // fryhtan 			score += 2;
    //	}
...
int Config::validate()
{
    ...
    if (ai_human_nation_count < 0 || ai_monster_nation_count < 0 || ai_nation_count() >= MAX_NATION)
        invalidPoint = 0x100;
    ...
    if (start_up_raw_site < 1 || start_up_raw_site > 7)
        invalidPoint = 0x105;
    if (difficulty_level != OPTION_CUSTOM && (difficulty_level < OPTION_EASIEST || difficulty_level > OPTION_HARDEST))
        invalidPoint = 0x106;
    ...
    // show_debug_info;
    // fast_build;							// fast
    // building everything disable_ai_flag; king_undie_flag;
    ...
    // not checked
    // show_unit_path;				// bit 0 show unit path on ZoomMatrix, bit 1
    // for MapMatrix
    ...
    // char			pan_control;                            // mono speaker
    // should disable pan_control
    ...
    // blacken_map;				// whether the map is blackened at the
    // first place
