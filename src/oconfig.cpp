...
static short table_start_up_cash[8] = {
    OPTION_VERY_HIGH, OPTION_VERY_HIGH, OPTION_HIGH, OPTION_MODERATE,
    OPTION_MODERATE,  OPTION_MODERATE,  OPTION_LOW,  OPTION_LOW};
static short table_ai_start_up_cash[8] = {
    OPTION_LOW,      OPTION_LOW,  OPTION_MODERATE,  OPTION_MODERATE,
    OPTION_MODERATE, OPTION_HIGH, OPTION_VERY_HIGH, OPTION_VERY_HIGH};
static char table_ai_aggressiveness[8] = {
    OPTION_NONE,     OPTION_NONE, OPTION_LOW,  OPTION_MODERATE,
    OPTION_MODERATE, OPTION_HIGH, OPTION_HIGH, OPTION_VERY_HIGH};
static short table_start_up_independent_town[8] = {30, 30, 30, 15,
                                                   15, 15, 7,  7};
...
static char table_independent_town_resistance[8] = {
    OPTION_LOW,      OPTION_LOW,  OPTION_MODERATE, OPTION_MODERATE,
    OPTION_MODERATE, OPTION_HIGH, OPTION_HIGH,     OPTION_HIGH};
static char table_random_event_frequency[8] = {
    OPTION_NONE, OPTION_NONE,     OPTION_LOW,      OPTION_LOW,
    OPTION_LOW,  OPTION_MODERATE, OPTION_MODERATE, OPTION_MODERATE};
static char table_new_nation_emerge[8] = {0, 0, 0, 1, 1, 1, 1, 1};
static char table_monster_type[8] = {
    OPTION_MONSTER_INTELLIGENT, OPTION_MONSTER_INTELLIGENT,
    OPTION_MONSTER_INTELLIGENT, OPTION_MONSTER_INTELLIGENT,
    OPTION_MONSTER_INTELLIGENT, OPTION_MONSTER_INTELLIGENT,
    OPTION_MONSTER_INTELLIGENT, OPTION_MONSTER_INTELLIGENT};
...
int Config::single_player_difficulty(int raceId) {
  ...
  if (raceId >= 0)
    ...
    score += start_up_raw_site *
             2; // more site human are stronger, fryhtan is harder to play
  ...
  //	else
  //	{
  //		if( start_up_has_mine_nearby )		// stronger human, weaker
  //fryhtan 			score += 2;
  //	}
...
int Config::validate() {
  ...
  if (independent_town_resistance < OPTION_LOW ||
      independent_town_resistance > OPTION_HIGH)
    invalidPoint = 0x123;
  if (random_event_frequency < OPTION_NONE ||
      random_event_frequency > OPTION_HIGH)
    invalidPoint = 0x124;
  new_nation_emerge = new_nation_emerge ? 1 : 0;
  if (monster_type < OPTION_MONSTER_NONE ||
      monster_type > OPTION_MONSTER_INTELLIGENT)
    invalidPoint = 0x125;
  ...
  // show_ai_info;
  // show_debug_info;
  // fast_build;							// fast building
  // everything disable_ai_flag; king_undie_flag;
  // // for testing game only
  ...
  if (!memchr(player_name, '\0', PLAYER_NAME_LEN + 1))
    invalidPoint = 0x201;
  if (player_nation_color < 0 ||
      player_nation_color > MAX_COLOR_SCHEME) // allow 0
    invalidPoint = 0x202;
  // expired_flag;
  ...
  if (opaque_report < 0 || opaque_report > 1)
    invalidPoint = 0x300;
  if (disp_news_flag != OPTION_DISPLAY_MAJOR_NEWS &&
      disp_news_flag != OPTION_DISPLAY_ALL_NEWS)
    invalidPoint = 0x301;
  ...
  if (disp_extend_info < 0 || disp_extend_info > 2)
    invalidPoint = 0x306;
  show_all_unit_icon =
      show_all_unit_icon ? 1 : 0; // 0:show icon when pointed, 1:always
  // not checked
  // show_unit_path;				// bit 0 show unit path on ZoomMatrix, bit 1 for
  // MapMatrix
  ...
  // char			pan_control;                            // mono speaker should
  // disable pan_control
  ...
  // blacken_map;				// whether the map is blackened at the first
  // place
  ...
  if (display_mode_id != MODE_ID_800x600x16 &&
      display_mode_id != MODE_ID_1024x768x16)
    invalidPoint = 0x3c0;
