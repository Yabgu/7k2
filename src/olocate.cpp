...
//{
// return terrain_res[terrain_id]->average_type == TERRAIN_DARK_DIRT;
// //**BUGHERE, to be changed to TERRAIN_HILL when the no. of terrain type has
// been reduced to 4 from 7
//	return 0;
//}
...
//
// <int> setTimeOut   call set_wall_timeout to refuse building wall at the same
// place
...
void Location::remove_unit(int mobileType) {
  if (mobileType == UNIT_AIR) {
    ...
    err_when(loc_flag &
             LOCATE_BLOCK_MASK); // if there is a still object on the location
    cargo_recno = 0;
  }
}
...
void Location::set_power_on() { loc_flag &= ~LOCATE_POWER_OFF; }
//-------- End of function Location::set_power_on --------//

//-------- Begin of function Location::set_power_off --------//
void Location::set_power_off() { loc_flag |= LOCATE_POWER_OFF; }
//-------- End of function Location::set_power_off --------//

//-------- Begin of function Location::is_power_off --------//
int Location::is_power_off() { return (loc_flag & LOCATE_POWER_OFF); }
...
int LocationCorners::interpolate_z(int curX, int curY) {
  ...
  return ((int)top_left->get_altitude() * (LOCATE_WIDTH - xPixel) *
              (LOCATE_HEIGHT - yPixel) +
