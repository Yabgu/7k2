...
void FirmMonsterAlchemy::disp_firm_info(int dispY1, int refreshFlag) {
  ...
  if (should_show_info()) {
    ...
    if (raw_id) {
      ...
      //			str += " Deposit";
      font_whbl.center_put(INFO_X1 + 20, INFO_Y1 + 20, INFO_X2 - 10,
                           INFO_Y1 + 40, text_firm.str_alchemy_raw(raw_id));

      // str = "Estimated Reserves ";
      // str += misc.format((int)reserve_qty, 1);
      font_whbl.center_put(INFO_X1 + 20, INFO_Y1 + 40, INFO_X2 - 10,
                           INFO_Y1 + 60,
                           text_firm.str_estimated_resource((int)reserve_qty));

      vga.active_buf->put_bitmap_trans(INFO_X1 + 13, INFO_Y1 + 129,
                                       raw_res.interface_raw_icon(raw_id));
    }
    ...
    if (!conversion_tech_id) {
      font_whbl.center_put(
          INFO_X1 + 20, INFO_Y1 + 100, INFO_X2 - 10, INFO_Y1 + 120,
      ...
      font_whbl.center_put(INFO_X1 + 20, INFO_Y1 + 120, INFO_X2 - 10,
                           INFO_Y1 + 140, str);

      //	x2 = font_snds.put( INFO_X1+20, INFO_Y1+118, "Monthly
      //Production: "); 	x2 = font_snds.put( x2, INFO_Y1+118, (int)
      //production_30days() );
    }
  }
  ...
  if (is_own()) {
    if (button_change.detect()) {
      ...
      do {
        switch (newTechId) {
        case 0:
          newTechId = TECH_LIVE_TO_GOLD;
          break;
        case TECH_LIVE_TO_GOLD:
          newTechId = TECH_GOLD_TO_LIVE;
          break;
        case TECH_GOLD_TO_LIVE:
          newTechId = TECH_RAW_TO_LIVE;
          break;
        case TECH_RAW_TO_LIVE:
          newTechId = TECH_RAW_TO_GOLD;
          break;
