...
TipsRes::TipsRes() { memset(this, 0, sizeof(TipsRes)); }

TipsRes::~TipsRes() { deinit(); }
...
void TipsRes::load_info() {
  ...
  while (!fileTxt.is_eof()) {
    ...
    if (tokenStr[0] == '@') // start of a class
    {
      ...
      {
        TipsClass *tipsClassPtr =
            (TipsClass *)tempTipsClass.reserve(sizeof(TipsClass));
  ...
  err_when((unsigned)tempTipsInfo.length() !=
           tips_info_count * sizeof(TipsInfo)); // assume 1 byte alignment
  ...
  err_when((unsigned)tempTipsClass.length() !=
           tips_class_count * sizeof(TipsClass)); // assume 1 byte alignment
}
...
void TipsRes::random_select() {
  ...
  if (count > 0) {
    // int chosen = misc.random(count) + 1;		// change random seed cuase bug in
    // game.init
    ...
    for (c = 1; c <= tips_class_count; ++c) {
      if (get_class(c)->is_active) {
        ...
        if (count >= chosen) {
          ...
          // BUGHERE : not so sure
          current_tips = tipsClass->first_tips + tipsClass->tips_count - 1 +
                         chosen - count;
          break;
        }
      }
...
void TipsRes::next() {
  ...
  if (++current_tips >= get_class(currentClass)->first_tips +
                            get_class(currentClass)->tips_count) {
...
void TipsRes::prev() {
  ...
  if (--current_tips < get_class(currentClass)->first_tips) {
    ...
    if (cc <= 0) // not found
    {
      ...
      current_tips = get_class(currentClass)->first_tips +
                     get_class(currentClass)->tips_count - 1;
      err_when(current_tips < 1 || current_tips > tips_info_count);
    }
  }
}
...
void TipsRes::disp(int x, int y) {
  ...
  font_cara_w.text_width(textPtr, -1,
                         TIPS_TEXT_WIDTH -
                             TIPS_BORDER_WIDTH * 2); // left and right side
  ...
  font_cara_w.put_paragraph(x + TIPS_BORDER_WIDTH, y + TIPS_BORDER_HEIGHT,
                            x + TIPS_TEXT_WIDTH - TIPS_BORDER_WIDTH - 1,
                            y + boxHeight - TIPS_BORDER_HEIGHT +
                                1, // reserve higher
                            textPtr);
  ...
  if (mouse.any_click(xb, yb, xb + TIPS_BUTTON_WIDTH - 1,
                      yb + TIPS_BUTTON_HEIGHT - 1))
    vga_util.d3_panel_down(xb, yb, xb + TIPS_BUTTON_WIDTH - 1,
                           yb + TIPS_BUTTON_HEIGHT - 1);
  else
    vga_util.d3_panel_up(xb, yb, xb + TIPS_BUTTON_WIDTH - 1,
                         yb + TIPS_BUTTON_HEIGHT - 1);
  font_bld.center_put(xb, yb - 4, xb + TIPS_BUTTON_WIDTH - 1,
                      yb + TIPS_BUTTON_HEIGHT - 1,
  ...
  if (mouse.any_click(xb, yb, xb + TIPS_BUTTON_WIDTH - 1,
                      yb + TIPS_BUTTON_HEIGHT - 1))
    vga_util.d3_panel_down(xb + TICK_WIDTH, yb, xb + TIPS_BUTTON_WIDTH - 1,
                           yb + TIPS_BUTTON_HEIGHT - 1);
  else
    vga_util.d3_panel_up(xb + TICK_WIDTH, yb, xb + TIPS_BUTTON_WIDTH - 1,
                         yb + TIPS_BUTTON_HEIGHT - 1);

  if (!player_profile.hide_tips) {
    vga.active_buf->d3_panel_up(xb, yb, xb + TICK_WIDTH - 1,
                                yb + TICK_HEIGHT - 1);
  } else {
    vga.active_buf->put_bitmap_trans_decompress(xb + 4, yb + 4,
                                                image_button.read("TICK"));
    vga.active_buf->d3_panel_down(xb, yb, xb + TICK_WIDTH - 1,
                                  yb + TICK_WIDTH - 1, 4, 0);
  }

  font_bld.center_put(xb + TICK_WIDTH, yb - 4, xb + TIPS_BUTTON_WIDTH - 1,
                      yb + TIPS_BUTTON_HEIGHT - 1,
...
void TipsClass::set_active() { is_active = 1; }
