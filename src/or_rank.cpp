...
#define X_SHIFT 0
#endif
  font_bld.put(x + 332 + X_SHIFT, y + 7,
...
#undef X_SHIFT
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    //		browse_nation.init( NATION_BROWSE_X1, NATION_BROWSE_Y1+34,
    //NATION_BROWSE_X2, NATION_BROWSE_Y2, 								  0, 22, info.nation_filter(),
    //put_nation_rec, 1 );
    browse_nation.init(NATION_BROWSE_X1, NATION_BROWSE_Y1 + 34,
                       NATION_BROWSE_X2, NATION_BROWSE_Y2, 0, 19,
  ...
  if (!game.game_has_ended) // if the ending screen has already appeared once,
...
static void disp_score() {
  ...
  vga_util.d3_panel_down(NATION_SCORE_X1, NATION_SCORE_Y1, NATION_SCORE_X2,
                         NATION_SCORE_Y2);
  ...
  for (int i = 0; i < MAX_RANK_TYPE; i++, y += 16) {
    ...
    font_bld.put(x, y,
                 text_reports.str_score_type(i + 1)); // rankStrArray[i] );
  ...
  font_bld.put(x, y + 2,
               text_reports.str_overall_score()); // "Overall Score" );
  ...
  //--- the following is only displayed in standalone game (and not in
  //campaigns) ---//
  ...
  if (game.is_campaign_mode()) {
    ...
    str = text_reports
              .str_campaign_difficulty(); // "Campaign Difficulty Level: ";
    ...
    //			str += " year.";
    font_bld.put(x, y + 20,
                 text_reports.str_campaign_year_passed(
...
static int disp_goal(int refreshFlag) {
  ...
  if (game.is_campaign_mode()) {
    ...
    /*
                    font_bld.count_line( NATION_GOAL_X1+6, NATION_GOAL_Y1+6,
       NATION_GOAL_X2-12, NATION_GOAL_Y2 - 6, game.campaign()->full_goal_text(),
                                                                            2,
       dispLines, totalLines);
    */
    dispLines = 0;
    totalLines = 4;

    if (NATION_GOAL_Y1 + textHeight + 6 > NATION_GOAL_Y2) {
      //	vga_util.d3_panel_down( NATION_GOAL_X1, NATION_GOAL_Y1,
      //NATION_GOAL_X2, NATION_GOAL_Y1 + textHeight +6 );
      //		font_bld.put_paragraph( NATION_GOAL_X1+6,
      //NATION_GOAL_Y1+6, 	 	NATION_GOAL_X2-6, NATION_GOAL_Y2 - 6,
      //game.campaign()->full_goal_text() );

      if (refreshFlag == INFO_REPAINT) {
        browse_goal.init(NATION_GOAL_X1, NATION_GOAL_Y1, NATION_GOAL_X2,
                         NATION_GOAL_Y2, 0, 22, totalLines - dispLines + 1,
                         put_goal_rec, 1, 3, 0);
        ...
        font_bld.put_paragraph(NATION_GOAL_X1 + 6, NATION_GOAL_Y1 + 6,
                               NATION_GOAL_X2 - 22, NATION_GOAL_Y2 - 6,
                               game.campaign()->full_goal_text(), 2,
                               browse_goal_recno);
      }
      y = NATION_GOAL_Y2;
    } else {
      y = MIN(NATION_GOAL_Y1 + textHeight + font_bld.height() + 6,
              NATION_GOAL_Y2);

      vga_util.d3_panel_down(NATION_GOAL_X1, NATION_GOAL_Y1, NATION_GOAL_X2,
                             y); // one line space
      font_bld.put_paragraph(NATION_GOAL_X1 + 6, NATION_GOAL_Y1 + 6,
                             NATION_GOAL_X2 - 6, NATION_GOAL_Y1 + textHeight,
                             game.campaign()->full_goal_text());
    }
  } else //-- if it is just a standalone game ----//
  {
    ...
    if (game.lose_condition_count > 0 &&
        game.lose_condition[0].condition_id == E_DEFEAT_OWN_KINGDOM)
      firstLoseCond = 1;
    ...
    if (game.win_condition_count > 1 || game.win_deadline) {
      ...
      if (game.win_deadline) {
        if (game.win_condition_count == 1)
          str = text_reports.str_goal_single_timed(date.date_str(
              game.win_deadline, 1)); // "GOAL: Achieve in n years.
        else if (game.win_require_all_condition)
          str = text_reports.str_goal_and_timed(date.date_str(
              game.win_deadline,
              1)); // "GOAL: Achieve ALL of the Following in n years";
        else
          str = text_reports.str_goal_or_timed(date.date_str(
              game.win_deadline,
              1)); // "GOAL: Achieve One of the Following in n years";
      } else {
        if (game.win_require_all_condition)
          str = text_reports
                    .str_goal_and(); // "GOAL: Achieve ALL of the Following";
        else
          str = text_reports
                    .str_goal_or(); // "GOAL: Achieve One of the Following";
      }
      ...
      for (int i = 0; i < game.win_condition_count && lineCount > 0; ++i) {
        font_bld.put(x, y, game.win_condition[i].cond_str, 0,
                     NATION_GOAL_X2 - 6);
        y += lineSpacing;
        --lineCount;
      }
    ...
    for (int i = firstLoseCond; i < game.lose_condition_count && lineCount > 0;
         ++i) {
      int x2 = x;
      if (i == firstLoseCond)
        x2 = font_bld.put(x2, y, text_reports.str_lose_());
      font_bld.put(x2, y, game.lose_condition[i].cond_str, 0,
                   NATION_GOAL_X2 - 6);
      y += lineSpacing;
      --lineCount;
    }
...
static void put_nation_rec(int recNo, int x, int y, int refreshFlag) {
  ...
  font_bld.put(x + 495, y,
               info.get_rank_pos_str(0, nationRecno)); // 0-overall ranking
}
...
static void put_goal_rec(int recNo, int x, int y, int refreshFlag) {
  /*	font_bld.put_paragraph( NATION_GOAL_X1+6, NATION_GOAL_Y1+6,
                          NATION_GOAL_X2-12, NATION_GOAL_Y2 - 6,
  game.campaign()->full_goal_text(), 2, 1);
  /*	int	  nationRecno = info.nation_filter(recNo);
          Nation* nationPtr   = nation_array[nationRecno];
          ...
          if( nationPtr->is_human() )			// only display
  reputation for humans font_bld.put( x+435, y, info.get_rank_pos_str(4,
  nationRecno) );

          font_bld.put( x+500, y, info.get_rank_pos_str(0, nationRecno) );
  // 0-overall ranking*/
}
...
//								  that have contact with the
//viewing 								  nation is counted. Otherwise all nations 								  are counted.
//
void Info::set_rank_data(int onlyHasContact) {
  ...
  if (nation_array.player_recno &&
      !nation_array.is_deleted(info.viewing_nation_recno))
    viewingNation = nation_array[info.viewing_nation_recno];
...
char *Info::get_rank_pos_str(int rankType, int nationRecno) {
  ...
  for (int i = 1; i <= nation_array.size(); i++) {
    ...
    if (rankData > curNationRankData) // if another nation's value is higher
                                      // than the given nation's value
      rankPos++;
  }
...
int Info::get_rank_score(int rankType, int nationRecno) {
  ...
  int rankScore =
      100 * nation_rank_data_array[rankType - 1][nationRecno - 1] / maxValue;
