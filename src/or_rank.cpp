...
#undef X_SHIFT
    ...
    if (refreshFlag == INFO_REPAINT)
    {
        ...
        // NATION_BROWSE_X2, NATION_BROWSE_Y2, 								  0,
        // 22, info.nation_filter(), put_nation_rec, 1 );
        browse_nation.init(NATION_BROWSE_X1, NATION_BROWSE_Y1 + 34, NATION_BROWSE_X2, NATION_BROWSE_Y2, 0, 19,
...
static void disp_score()
{
    ...
    if (game.is_campaign_mode())
    {
        ...
        //			str += " year.";
        font_bld.put(x, y + 20,
                     text_reports.str_campaign_year_passed(date.date_str(info.game_start_date), info.year_passed));
        return;
    }
    ...
    int difficultyRating =
        nation_array[viewNationRecno]->race_id >= 0 ? config.human_difficulty_rating : config.monster_difficulty_rating;
...
static int disp_goal(int refreshFlag)
{
    ...
    if (game.is_campaign_mode())
    {
        font_bld.text_width(game.campaign()->full_goal_text(), -1, NATION_GOAL_X2 - NATION_GOAL_X1 - 12);
        ...
        */
        ...
        if (NATION_GOAL_Y1 + textHeight + 6 > NATION_GOAL_Y2)
        {
            ...
            if (refreshFlag == INFO_REPAINT)
            {
                browse_goal.init(NATION_GOAL_X1, NATION_GOAL_Y1, NATION_GOAL_X2, NATION_GOAL_Y2, 0, 22,
                                 totalLines - dispLines + 1, put_goal_rec, 1, 3, 0);
            ...
            else
            {
                ...
                font_bld.put_paragraph(NATION_GOAL_X1 + 6, NATION_GOAL_Y1 + 6, NATION_GOAL_X2 - 22, NATION_GOAL_Y2 - 6,
                                       game.campaign()->full_goal_text(), 2, browse_goal_recno);
            }
            y = NATION_GOAL_Y2;
        }
        else
        {
            ...
            vga_util.d3_panel_down(NATION_GOAL_X1, NATION_GOAL_Y1, NATION_GOAL_X2,
                                   y); // one line space
            font_bld.put_paragraph(NATION_GOAL_X1 + 6, NATION_GOAL_Y1 + 6, NATION_GOAL_X2 - 6,
                                   NATION_GOAL_Y1 + textHeight, game.campaign()->full_goal_text());
        }
    }
        ...
        if (game.win_condition_count > 1 || game.win_deadline)
        {
            ...
            if (game.win_deadline)
            {
                if (game.win_condition_count == 1)
                    str = text_reports.str_goal_single_timed(
                        date.date_str(game.win_deadline, 1)); // "GOAL: Achieve in n years.
                else if (game.win_require_all_condition)
                    str = text_reports.str_goal_and_timed(
                        date.date_str(game.win_deadline,
                                      1)); // "GOAL: Achieve ALL of the Following in n years";
                else
                    str = text_reports.str_goal_or_timed(
                        date.date_str(game.win_deadline,
...
// the
// viewing 								  nation is counted.
// Otherwise all nations are counted.
//
