...
//								(default: 0)
void ScrollBar::init(int itype, int ix1, int iy1, int ix2, int iy2, int pageSkipRec, int dispMaxRec, int totalRec,
...
void ScrollBar::refresh(int topRecNo, int forceRefresh, int pageSkipRec, int dispMaxRec, int totalRec)
{
    ...
    if ((newY = rec_to_y(topRecNo)) != indicator_y || total_rec_num != oldTotalRec || forceRefresh)
    {
        indicator_y = newY;
...
int ScrollBar::detect()
{
    ...
    if (type == VERTICAL && mouse.any_click(x1 + 2, y1 + 14, x2 - 2, y2 - 14) ||
        type == HORIZONTAL && mouse.any_click(x1 + 14, y1 + 2, x2 - 14, y2 - 2) || mouse.key_code == KEY_PGUP ||
        mouse.key_code == KEY_PGDN)
    {
        if (type == VERTICAL)
            pos = mouse.click_y();
        else
            ...
            else
            {
                if (mouse.left_press && lastMouseY != mouse.cur_x)
                {
                    ...
                    if (if_flag)
                    {
                        ...
                        Vga::active_buf->draw_d3_up_border(indicator_y, y1 + 2, indicator_y + indicator_height - 1,
                                                           y2 - 2);
                    }
                    else
                    {
