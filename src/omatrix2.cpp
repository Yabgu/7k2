...
static long full_slope_brightness_table[4 * MAX_BRIGHTNESS_ADJUST_DEGREE + 1] =
    {0x0500,  0x0500,  0x0500,  0x0500,  0x0500,  0x0500,           0x0500,
     0x0500,  0x0400,  0x0400,  0x0400,  0x0400,  0x0400,           0x0400,
     0x0400,  0x0400,  0x0300,  0x0300,  0x0300,  0x0300,           0x0300,
     0x0300,  0x0300,  0x0300,  0x0200,  0x0200,  0x0200,           0x0200,
     0x0200,  0x0200,  0x0200,  0x0200,  0x0100,  0x0100,           0x0100,
     0x0100,  0x0100,  0x0100,  0x0000,  0x0000,  0x0000,           0x0000,
     0x0000,  -0x0100, -0x0100, -0x0100, -0x0100, -0x0200,          -0x0200,
     -0x0200, -0x0300, -0x0300, -0x0400, -0x0400, -0x0500,          -0x0500,
     -0x0600, -0x0700, -0x0700, -0x0800, -0x0900, -0x0a00,          -0x0a00,
     -0x0b00, -0x0c00, -0x0d00, -0x0e00, -0x0f00, -0x1000,          -0x1100,
     -0x1100, -0x1200, -0x1300, -0x1400, -0x1500, -0x1600,          -0x1700,
     -0x1800, -0x1900, -0x1a00, -0x1a00, -0x1b00, -0x1c00,          -0x1d00,
     -0x1e00, -0x1e00, -0x1f00, -0x1f00, -0x2000, -0x2000,          -0x2000,
...
static long fog_slope_brightness_table[4 * MAX_BRIGHTNESS_ADJUST_DEGREE + 1] = {
    ...
    -0x1200, -0x1300, -0x1400, -0x1500, -0x1600, -0x1700, -0x1700, -0x1800,
...
static long *slope_tables[3] = {dark_slope_brightness_table,
                                fog_slope_brightness_table,
                                full_slope_brightness_table};
...
void LocationCorners::render(Blob2DW *blob, char *texturePtr, BYTE drawState) {
  blob->resize(ZOOM_LOC_Y_WIDTH, loc_ptr->min_y,
               ZOOM_LOC_X_WIDTH - ZOOM_LOC_Y_WIDTH,
  ...
  if (last_draw_state != drawState) {
    ...
    case FULL_DARK_MASK_ID:
      memcpy(slope_brightness_table, dark_slope_brightness_table,
             sizeof(slope_brightness_table));
      break;
    case HALF_BRIGHT_MASK_ID:
      memcpy(slope_brightness_table, fog_slope_brightness_table,
             sizeof(slope_brightness_table));
      break;
    case FULL_BRIGHT_MASK_ID:
      memcpy(slope_brightness_table, full_slope_brightness_table,
             sizeof(slope_brightness_table));
      break;
    default:
  ...
  short *bufPtr = blob->ptr->bitmap +
                  (-ZOOM_Z_HEIGHT * top_left->altitude - blob->top_edge) *
                      blob->ptr->get_pitch() -
                  blob->left_edge;
...
location_render_2b:
      ...
      seaColor = vga.translate_color(SEA_COLOR) *
                 0x10001; // duplicate low word to high word
    ...
    for (vCount = 0; vCount < LOCATE_HEIGHT / 2; ++vCount) {
      ...
      for (uCount = 0; uCount < LOCATE_WIDTH / 2; ++uCount) {
        ...
        *(reinterpret_cast<int *>(
            (tmpBufPtr - (edx / 2) + (bufTruePitch / 2)))) = color2;
...
void LocationCorners::render(BitmapW *bitmapPtr, char *texturePtr, int offsetX,
                             int offsetY, BYTE drawState) {
  // try to freeze the blob, don't allow it to resize
  ...
  if (!(loc_ptr->loc_flag & LOCATE_SEA_COAST)) {
    ...
    for (vCount = 0; vCount < LOCATE_HEIGHT / 2; ++vCount) {
      ...
      for (uCount = 0; uCount < LOCATE_WIDTH / 2; ++uCount) {
        ...
        eax = (eax & 0xFFFF0000) |
              *(reinterpret_cast<unsigned short *>(texturePtr));
        ebx = (ebx & 0xFFFFFF00) |
              *(reinterpret_cast<unsigned char *>(texturePtr) + 1);
        eax = rotr(eax, 16);
        eax = (eax & 0xFFFF0000) | *(remapTables + ebx);
        eax = rotl(eax, 16);
        ebx = (ebx & 0xFFFFFF00) |
              *(reinterpret_cast<unsigned char *>(texturePtr));
...
void LocationCorners::render_fog(BitmapW *bitmapPtr, char *texturePtr,
                                 int offsetX, int offsetY, char *fogMask) {
  c00 = loc_ptr->c00;
  c10 = loc_ptr->c10;
  c01 = loc_ptr->c01;
  c20 = loc_ptr->c20;
