...
void FirmMarket::process_ai() {
  ...
  if (info.game_date <
      last_import_new_goods_date + 60) // don't new imports until it's 60 days
                                       // after the last one was imported
    return;

  if (can_hire_caravan()) {
    Nation *ownNation = nation_array[nation_recno];

    int thinkInterval = 10 + (100 - ownNation->pref_trading_tendency) /
                                 5; // think once every 10 to 30 days
    ...
    if (is_market_linked_to_town()) {
      ...
      if (info.game_date % 60 == (firm_recno + 20) % 60) {
        ...
        if (!last_import_new_goods_date ||
            info.game_date > last_import_new_goods_date +
                                 180) // only think increase existing supply 180
                                      // days after importing a new one
        {
...
void FirmMarket::think_market_build_factory() {
  ...
  //---- think about building factories to manufacture goods using raw materials
  //in the market place ---//

  MarketGoods *marketGoods = market_goods_array;
  Firm *firmPtr;

  ai_should_build_factory_count =
      2; // always set it to 2, so think_build_factory() will start to build a
         // market as soon as there is a need
  ...
  for (int i = 0; i < MAX_MARKET_GOODS; i++, marketGoods++) {
    ...
    //----- check if the raw materials are from a local mine, if so don't build
    //a factory, we only build a factory to manufacture goods using raw
    //materials from a remote town.
...
void FirmMarket::ai_update_link_status() {
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    rc = firmPtr->firm_id == FIRM_MINE ||
         firmPtr->firm_id == FIRM_FACTORY; // for output raw materials to the
                                           // factory to manufacture

    toggle_firm_link(i + 1, rc, COMMAND_AI); // enable the link
  }

  //----- always enable links to towns as there is no downsides for selling
  //goods to the villagers ----//

  for (i = 0; i < linked_town_count; i++) {
    err_when(!linked_town_array[i] ||
             town_array.is_deleted(linked_town_array[i]));
...
int FirmMarket::think_del() {
  ...
  if (stock_value_index() >= 10) {
    ...
    if (info.game_date < no_linked_town_since_date + 180 +
                             180 * ownNation->pref_trading_tendency / 100) {
      return 0;
    }
  }
...
int FirmMarket::think_import_new_product() {
  ...
  int i, j;
  Town *townPtr;
  short needProductSupplyPop[MAX_PRODUCT]; // the total population in the towns
                                           // linked to the market that needs
  ...
  if (is_market_linked_to_town(
          1)) // 1-only count towns that are our own and are base towns
  {
    if (!no_neighbor_space &&
        nationPtr->total_jobless_population >= MAX_WORKER * 2 &&
        can_hire_caravan() >=
            2) // if there is a shortage of caravan supplies, use it for
               // transporting finished products instead of raw materials
    {
  ...
  for (int i = 0; i < MAX_MARKET_GOODS; i++, marketGoods++) {
    if (marketGoods->product_raw_id) {
      if (marketGoods->stock_qty < MAX_MARKET_STOCK / 10 &&
          marketGoods->month_demand * 0.8 >
              marketGoods->supply_30days()) // the supply falls behind the
...
int FirmMarket::think_import_specific_product(int productId) {
  ...
  for (i = rawInfo->product_supply_firm_array.size(); i > 0; i--) {
    ...
    if (firmPtr->firm_id == FIRM_FACTORY) {
      if (firmPtr->nation_recno !=
          nation_recno) // can import goods from own factories only
        continue;
      ...
      if (firmFactory->product_raw_id == productId)
        stockLevel =
            100 * (int)firmFactory->stock_qty / (int)firmFactory->max_stock_qty;
    }

    //---- think about inputing goods from this market ----//

    else if (firmPtr->firm_id == FIRM_MARKET) {
      //--- if this is a foreign sale market, don't import from it (e.g. Nation
      //A's market built near Nation B's village -----//
      ...
      if (firmPtr->nation_recno != nation_recno) // if this is not our market
      {
        ...
        if (j < 0) // if this market is not linked to its own town (then it must
                   // be a foreign market)
          continue;
      }
      ...
      if (nation_array[firmPtr->nation_recno]
              ->get_relation(nation_recno)
              ->trade_treaty == 0)
        continue;

      MarketGoods *marketGoods =
          ((FirmMarket *)firmPtr)->market_product_array(productId);
    ...
    if (firmPtr->nation_recno == nation_recno) {
      if (stockLevel <
          10) // for our own market, the stock level requirement is lower
        continue;

      curRating = 50;
    } else {
      if (stockLevel < 20) // for other player's market, only import when the
                           // stock level is high enough
        continue;
    ...
    curRating += stockLevel / 2 + world.distance_rating(center_x, center_y,
                                                        firmPtr->center_x,
                                                        firmPtr->center_y);
  ...
  if (bestFirmPtr)
    return ai_create_new_trade(bestFirmPtr, 0,
                               PICK_UP_PRODUCT_FIRST + productId - 1);
...
int FirmMarket::think_export_product() {
    ...
    if (marketGoods->product_raw_id) {
      if (marketGoods->stock_qty > MAX_MARKET_STOCK * 3 / 4 &&
          marketGoods->month_demand <
              marketGoods->supply_30days() /
                  2) // the supply is at least double of the demand
      {
  ...
  for (int townRecno = town_array.size(); townRecno > 0; townRecno--) {
    ...
    if (townPtr->population <
        20 - (10 * nationPtr->pref_trading_tendency /
              100)) // 10 to 20 as the minimum population for considering trade
      continue;

    if (townPtr->has_product_supply[exportProductId -
                                    1]) // if the town already has the supply of
                                        // product, return now
      continue;
    ...
    if (townPtr->no_neighbor_space) // if there is no space in the neighbor area
                                    // for building a new firm.
      continue;

    if (misc.points_distance(center_x, center_y, townPtr->center_x, center_y) >
        MAX_WORLD_X_LOC / 4) // don't consider if it is too far away
      continue;

    if (townPtr->town_recno &&
        nationPtr->get_relation_status(townPtr->town_recno) <
            RELATION_FRIENDLY) // only build markets to friendly nation's town
    {
      continue;
    }
...
int FirmMarket::think_mft_specific_product(int rawId) {
  ...
  for (i = rawInfo->raw_supply_firm_array.size(); i > 0; i--) {
    ...
    //-- if this is our own supply, don't import the raw material, but import
    //the finished goods instead. --//
    ...
    if (firmPtr->firm_id == FIRM_MARKET) {
      ...
      for (j = firmPtr->linked_firm_count - 1; j >= 0; j--) {
        ...
        if (linkedFirm->firm_id == FIRM_MINE &&
            linkedFirm->nation_recno == firmPtr->nation_recno) {
          if (((FirmMine *)linkedFirm)->raw_id == rawId)
            break;
        }
      }

      if (j < 0) // this market does not have any direct supplies, so don't pick
                 // up goods from it
        continue;
    ...
    NationRelation *nationRelation =
        nationPtr->get_relation(firmPtr->nation_recno);

    curRating = stockLevel - 100 *
                                 misc.points_distance(center_x, center_y,
                                                      firmPtr->center_x,
                                                      firmPtr->center_y) /
                                 MAX_WORLD_X_LOC;
...
//
int FirmMarket::ai_create_new_trade(Firm *firmPtr, int stop1PickUpType,
                                    int stop2PickUpType) {
  ...
  for (int i = ownNation->ai_caravan_count - 1; i >= 0; i--) {
    ...
    if (stop1PickUpType &&
        !unitCaravan->has_pick_up_type(stop1Id, stop1PickUpType)) {
      if (unitCaravan
              ->is_visible()) // can't set stop when the caravan is in a firm
        unitCaravan->set_stop_pick_up(stop1Id, stop1PickUpType, COMMAND_AI);
      rc = 1;
    }

    if (stop2PickUpType &&
        !unitCaravan->has_pick_up_type(stop2Id, stop2PickUpType)) {
      if (unitCaravan
              ->is_visible()) // can't set stop when the caravan is in a firm
  ...
  if (caravanInRouteCount >=
      2) // don't have more than 2 caravans on a single route
    return 0;
  ...
  err_when(unitCaravan->stop_array[0].firm_recno ==
           firmPtr->firm_recno); // cannot set both stops to the same firm
