...
void FirmMarket::process_ai()
{
    ...
    if (info.game_date < last_import_new_goods_date + 60) // don't new imports until it's 60 days
        ...
        if (is_market_linked_to_town())
        {
            ...
            if (info.game_date % 60 == (firm_recno + 20) % 60)
            {
                ...
                if (!last_import_new_goods_date ||
                    info.game_date > last_import_new_goods_date + 180) // only think increase existing supply 180
...
void FirmMarket::think_market_build_factory()
{
    ...
    ai_should_build_factory_count = 2; // always set it to 2, so think_build_factory() will start to build a
    ...
    for (int i = 0; i < MAX_MARKET_GOODS; i++, marketGoods++)
    {
        ...
        for (j = 0; j < linked_firm_count; j++)
        {
            ...
            if (firmPtr->firm_id == FIRM_MINE && firmPtr->nation_recno == nation_recno &&
...
void FirmMarket::ai_update_link_status()
{
    ...
    for (i = 0; i < linked_firm_count; i++)
    {
        ...
        rc = firmPtr->firm_id == FIRM_MINE || firmPtr->firm_id == FIRM_FACTORY; // for output raw materials to the
...
int FirmMarket::think_del()
{
    ...
    if (stock_value_index() >= 10)
    {
        ...
        if (info.game_date < no_linked_town_since_date + 180 + 180 * ownNation->pref_trading_tendency / 100)
        {
            return 0;
        }
    }
...
int FirmMarket::think_import_new_product()
{
    ...
    {
        if (!no_neighbor_space && nationPtr->total_jobless_population >= MAX_WORKER * 2 &&
            can_hire_caravan() >= 2) // if there is a shortage of caravan supplies, use it for
    ...
    for (int i = 0; i < MAX_MARKET_GOODS; i++, marketGoods++)
    {
        if (marketGoods->product_raw_id)
        {
            if (marketGoods->stock_qty < MAX_MARKET_STOCK / 10 &&
                marketGoods->month_demand * 0.8 > marketGoods->supply_30days()) // the supply falls behind the
...
int FirmMarket::think_import_specific_product(int productId)
{
    ...
    for (i = rawInfo->product_supply_firm_array.size(); i > 0; i--)
    {
        ...
        else if (firmPtr->firm_id == FIRM_MARKET)
        {
            ...
            MarketGoods *marketGoods = ((FirmMarket *)firmPtr)->market_product_array(productId);
...
int FirmMarket::think_export_product()
{
        ...
        if (marketGoods->product_raw_id)
        {
            if (marketGoods->stock_qty > MAX_MARKET_STOCK * 3 / 4 &&
                marketGoods->month_demand <
                    marketGoods->supply_30days() / 2) // the supply is at least double of the demand
            {
    ...
    for (int townRecno = town_array.size(); townRecno > 0; townRecno--)
    {
        ...
        if (townPtr->population < 20 - (10 * nationPtr->pref_trading_tendency /
                                        100)) // 10 to 20 as the minimum population for considering trade
            continue;

        if (townPtr->has_product_supply[exportProductId - 1]) // if the town already has the supply of
                                                              // product, return now
            continue;
        ...
        if (townPtr->town_recno && nationPtr->get_relation_status(townPtr->town_recno) <
...
int FirmMarket::think_mft_specific_product(int rawId)
{
    ...
    for (i = rawInfo->raw_supply_firm_array.size(); i > 0; i--)
    {
        ...
        curRating = stockLevel - 100 * misc.points_distance(center_x, center_y, firmPtr->center_x, firmPtr->center_y) /
                                     MAX_WORLD_X_LOC;
