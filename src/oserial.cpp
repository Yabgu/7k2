...
// ------ begin of function SerialRepository::~SerialRepository --------//
//
SerialRepository::~SerialRepository() { deinit(); }
// ------ end of function SerialRepository::~SerialRepository --------//
...
int SerialRepository::save() {
  ...
  rc = file.file_write(history_array,
                       sizeof(history_array[0]) * cur_history_size);
...
int SerialRepository::load() {
  ...
  history_array =
      (long *)mem_resize(history_array, sizeof(history_array[0]) * max_history);
  ...
  rc = file.file_read(history_array,
                      sizeof(history_array[0]) * cur_history_size);
...
void SerialRepository::add(long num) {
  ...
  if (cur_history_size >= max_history) {
    memmove(history_array, history_array + 1,
            (max_history - 1) * sizeof(history_array[0]));
...
bool SerialRepository::remove(long num) {
  ...
  for (register long i = cur_history_size - 1; i >= 0; --i) {
    if (history_array[i] == num) {
      misc.del_array_rec(history_array, cur_history_size,
                         sizeof(history_array[0]), i + 1);
      --cur_history_size;
      flag = true;
    }
  }
