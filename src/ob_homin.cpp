...
BulletHoming::BulletHoming() : Bullet() { target_base_obj_recno = 0; }
// --------- end of function BulletHoming::BulletHoming --------//

// --------- begin of function BulletHoming::~BulletHoming --------//
BulletHoming::~BulletHoming() { deinit(); }
// --------- end of function BulletHoming::BulletHoming --------//

// --------- begin of function BulletHoming::init --------//
void BulletHoming::init(char parentType, short parentRecno, short targetXLoc,
  ...
  if (baseObjPtr->cast_to_Unit()) {
    ...
    max_step =
        char((attackInfo->attack_range * LOCATE_WIDTH + speed - 1) / speed);
  } else if (baseObjPtr->cast_to_Firm() &&
             baseObjPtr->cast_to_Firm()->cast_to_FirmFort()) {
  ...
  if (base_obj_array.is_deleted(target_base_obj_recno)) {
    ...
    if (unitPtr && unitPtr->is_visible()) {
      ...
      if (total_step > max_step) {
        ...
        target_x_loc = (cur_x + adjX) + (int)(go_x - (cur_x + adjX)) /
                                            (actualStep - total_step) /
                                            LOCATE_WIDTH;
        target_x_loc = (cur_y + adjY) + (int)(go_y - (cur_y + adjY)) /
                                            (actualStep - total_step) /
                                            LOCATE_HEIGHT;
      }
    }
