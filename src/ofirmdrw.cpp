...
static Point slot_point_array[] = {
    {6, 6}, {6, 14}, {6, 22}, {14, 6}, {14, 14}, {14, 22}, {22, 6}, {22, 14}, {22, 22},
};
...
void Firm::draw(int displayLayer)
{
        ...
        for (yLoc = loc_y1; yLoc <= loc_y2; ++yLoc)
        {
            for (xLoc = loc_x1; xLoc <= loc_x2; ++xLoc)
            {
                ...
                world.zoom_matrix->put_bitmapW_offset(xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, lc.top_left->altitude,
                                                      maskBitmap.bitmap_ptr(), maskBitmap.left_edge,
                                                      maskBitmap.top_edge, 0, 0);
            }
        }
    ...
    {
        colorRemapTable = firm_res.calc_color_remap_table(colorRemapTable, hit_points / max_hit_points());
    }
    else
    {
        ...
        (!under_construction || construction_frame() >= firmBuild->under_construction_bitmap_count - 1))
    {
        ...
        firm_res.get_bitmap(firmBuild->ground_bitmap_recno)
            ->draw_at(loc_x1 * LOCATE_WIDTH, loc_y1 * LOCATE_HEIGHT, altitude, colorRemapTable, displayLayer,
                      firmBuild);
        // ###### end Gilbert 21/9 ########//
    }
...
#define FLAG_HEIGHT 25
    ...
    if (under_construction)
    {
        ...
        firmBitmap = firm_res.get_bitmap(firmBuild->under_construction_bitmap_recno + buildFraction);
    }
    ...
    // LOCATE_HEIGHT, 		altitude, firmBitmap->bitmap_ptr,
    // firmBitmap->offset_x,
    ...
    if (firmBitmap && firmBitmap->display_layer != 4)
    {
        ...
        // altitude, 			colorRemapTable, displayLayer, cast_to_FirmLishorr() ?
        // firmBuild
        //- cast_to_FirmLishorr()->animation_offset : firmBuild);
        firmBitmap->draw_at(loc_x1 * LOCATE_WIDTH, loc_y1 * LOCATE_HEIGHT, altitude, colorRemapTable, displayLayer,
                            firmBuild);
    }
    ...
    {
        char *flagBitmapPtr = image_spict.get_ptr("FLAG-S0");
        world.zoom_matrix->put_bitmap_offset((loc_x2 + 1) * LOCATE_WIDTH - 1, loc_y1 * LOCATE_HEIGHT,
                                             altitude, // world.get_corner(loc_x2+1, loc_y1)->get_altitude(),
                                             flagBitmapPtr, -FLAG_WIDTH / 2, -FLAG_HEIGHT, stdRemapTable, 0, 1);
        world.zoom_matrix->put_bitmap_offset(loc_x1 * LOCATE_WIDTH, (loc_y2 + 1) * LOCATE_HEIGHT - 1,
                                             altitude, // world.get_corner(loc_x1, loc_y2+1)->get_altitude(),
                                             flagBitmapPtr, -FLAG_WIDTH / 2, -FLAG_HEIGHT, stdRemapTable, 0, 1);
        world.zoom_matrix->put_bitmap_offset((loc_x2 + 1) * LOCATE_WIDTH - 1, (loc_y2 + 1) * LOCATE_HEIGHT - 1,
        ...
        if (firmCurFrame[i - 1] > 1)
        {
            ...
            for (j = 0; j < bitmapCount; j++)
            {
                ...
                if (firmBitmap && firmBitmap->display_layer != 4)
                    ...
                    {
                        firmBitmap->draw_at(loc_x1 * LOCATE_WIDTH, loc_y1 * LOCATE_HEIGHT, altitude, colorRemapTable,
                                            displayLayer, firmBuild);
                    }
            }
        }
    }
...
int Firm::draw_detect_link_line(int actionDetect)
{
    ...
    for (i = 0; i < linked_town_count; i++)
    {
        ...
        {
            bitmapPtr = image_icon.read(active_link_town_recno == linked_town_array[i] ? (char *)"LINK_ACT"
                                                                                       : (char *)"LINK_DEA");
