...
static Point slot_point_array[] = {
    {6, 6},   {6, 14}, {6, 22},  {14, 6},  {14, 14},
    {14, 22}, {22, 6}, {22, 14}, {22, 22},
};
...
void Firm::draw(int displayLayer) {
  ...
  if (world.map_matrix->zoom_map_building_disappear) {
    ...
    for (yLoc = loc_y1; yLoc <= loc_y2; ++yLoc) {
      for (xLoc = loc_x1; xLoc <= loc_x2; ++xLoc) {
        ...
        world.zoom_matrix->put_bitmapW_offset(
            xLoc * LOCATE_WIDTH, yLoc * LOCATE_HEIGHT, lc.top_left->altitude,
            maskBitmap.bitmap_ptr(), maskBitmap.left_edge, maskBitmap.top_edge,
            0, 0);
      }
    }

    world.zoom_matrix->put_center_text((curX1 + curX2) / 2, (curY1 + curY2) / 2,
                                       curZ, firm_name());
  ...
  short *colorRemapTable = firmBuild->get_color_remap_table(
      nation_recno, firm_array.selected_recno == firm_recno);
  if (under_construction) {
    colorRemapTable = firm_res.calc_color_remap_table(
        colorRemapTable, hit_points / max_hit_points());
  } else {
  ...
  if (firmBuild->ground_bitmap_recno &&
    ...
    firm_res.get_bitmap(firmBuild->ground_bitmap_recno)
        ->draw_at(loc_x1 * LOCATE_WIDTH, loc_y1 * LOCATE_HEIGHT, altitude,
...
void Firm::draw_full_size(int displayLayer) {
  ...
  short *stdRemapTable = game.get_color_remap_table(
      nation_recno, firm_array.selected_recno == firm_recno);
  // ##### begin Gilbert 21/9 ######//
  // short *colorRemapTable = firmBuild->get_color_remap_table(nation_recno,
  // firm_array.selected_recno == firm_recno);
  short *colorRemapTable =
      firm_res
          .cache_color_remap_table; // set in firm_res.calc_color_remap_table
  // ##### end Gilbert 21/9 ######//
...
#define FLAG_HEIGHT 25
    ...
    world.zoom_matrix->put_bitmap_offset(
        ...
        flagBitmapPtr, -FLAG_WIDTH / 2, -FLAG_HEIGHT, stdRemapTable, 0, 1);
  }
  ...
  if (under_construction) {
    ...
    firmBitmap = firm_res.get_bitmap(
        firmBuild->under_construction_bitmap_recno + buildFraction);
  }
    ...
    firmBitmap =
        firm_res.get_bitmap(firmBuild->first_bitmap(firm_cur_frame[0]));
  ...
  //	world.zoom_matrix->put_bitmap_offset(loc_x1 * LOCATE_WIDTH, loc_y1 *
  //LOCATE_HEIGHT, 		altitude, firmBitmap->bitmap_ptr, 		firmBitmap->offset_x,
  //firmBitmap->offset_y, 		colorRemapTable, 0, 1);

  if (firmBitmap && firmBitmap->display_layer != 4) {
    //		firmBitmap->draw_at( loc_x1*LOCATE_WIDTH, loc_y1*LOCATE_HEIGHT,
    //altitude, 			colorRemapTable, displayLayer, cast_to_FirmLishorr() ? firmBuild
    //- cast_to_FirmLishorr()->animation_offset : firmBuild);
...
#define FLAG_HEIGHT 25
    ...
    world.zoom_matrix->put_bitmap_offset(
        ...
        -FLAG_WIDTH / 2, -FLAG_HEIGHT, stdRemapTable, 0, 1);
  }
      ...
      for (j = 0; j < bitmapCount; j++) {
        ...
        if (firmBitmap && firmBitmap->display_layer != 4)
          if (firmBitmap->ani_part == i) {
            firmBitmap->draw_at(loc_x1 * LOCATE_WIDTH, loc_y1 * LOCATE_HEIGHT,
                                altitude, colorRemapTable, displayLayer,
                                firmBuild);
          }
      }
    }
...
int Firm::draw_detect_link_line(int actionDetect) {
  if (firm_id == FIRM_INN) // FirmInn's link is only for scan for neighbor inns
                           // quickly, the link line is not displayed
    return 0;
  ...
  for (i = 0; i < linked_firm_count; i++) {
    ...
    if (!actionDetect) {
      anim_line.draw_line(&vga_back, srcX, srcY, firmX, firmY,
                          linked_firm_enable_array[i] == LINK_EE);
    }
    ...
    if (actionDetect) {
      if (is_own() && world.zoom_matrix->detect_bitmap_clip(
                          firmX - 16, firmY - 16, bitmapPtr))
      //	if( is_own() && world.zoom_matrix->detect_bitmap_clip( firmX-11,
      //firmY-11, bitmapPtr ) )
      {
      ...
      if (nation_recno == nation_array.player_recno)
        //	world.zoom_matrix->put_bitmap_clip( firmX-11, firmY-11,
        //bitmapPtr );
  ...
  for (i = 0; i < linked_town_count; i++) {
    ...
    if (!actionDetect) {
      ...
      if (firm_id == FIRM_LAIR && nation_recno && townPtr->nation_recno == 0 &&
      ...
      if (!flag) {
        anim_line.draw_line(&vga_back, srcX, srcY, townX, townY,
                            linked_town_enable_array[i] == LINK_EE, 0);
      } else {
        anim_line.thick_line(&vga_back, srcX, srcY, townX, townY,
                             linked_town_enable_array[i] == LINK_EE, -1);
      }
    }
    ...
    if (can_toggle_town_link(linked_town_array[i]))
      bitmapPtr = power.get_link_icon(linked_town_enable_array[i],
                                      nation_recno == townPtr->nation_recno);
    else if (can_set_active_link(linked_town_array[i])) {
      bitmapPtr = image_icon.read(active_link_town_recno == linked_town_array[i]
                                      ? (char *)"LINK_ACT"
                                      : (char *)"LINK_DEA");
    ...
    if (!cast_to_FirmCamp())
      help.set_help(townX - 16, townY - 16,
                    townX - 16 + ((Bitmap *)bitmapPtr)->get_width(),
                    townY - 16 + ((Bitmap *)bitmapPtr)->get_height(),
                    "WORKLOCK");

    if (actionDetect) {
      //	int rc = world.zoom_matrix->detect_bitmap_clip( townX-11,
      //townY-11, bitmapPtr );
      int rc = world.zoom_matrix->detect_bitmap_clip(townX - 16, townY - 16,
                                                     bitmapPtr);
...
int Firm::is_in_zoom_win() {
  ...
  if( x1 <= -firmBuild->max_bitmap_width || x1 >= ZOOM_WIDTH )	// out of the
  view area, not even a slight part of it appears in the view area return 0;
...
void Firm::draw_selected() {
  /*
          ...
          {
                  //------- Only draw_selected the portion within the zoom
     window area ----//

                  if( y1 >= ZOOM_Y1 )		// square top
                          vga_back.bar( MAX(x1,ZOOM_X1), y1, MIN(x2,ZOOM_X2),
     y1, frameColor );

                  if( y2 <= ZOOM_Y2 )		// square bottom
                          vga_back.bar( MAX(x1,ZOOM_X1), y2, MIN(x2,ZOOM_X2),
     y2, frameColor );

                  if( x1 >= ZOOM_X1 )		// square left
                          vga_back.bar( x1, MAX(y1,ZOOM_Y1), x1,
     MIN(y2,ZOOM_Y2), frameColor );

                  if( y1 <= ZOOM_X2 )		// square left
                          vga_back.bar( x2, MAX(y1,ZOOM_Y1), x2,
     MIN(y2,ZOOM_Y2), frameColor );
        ...
        //------------ display hit point bar -----------//
                  ...
                  if( firm_res[firm_id]->buildable )
                  {
                          ...
                          if( x1<=ZOOM_X2 && x2>=ZOOM_X1 && y1<=ZOOM_Y2 &&
     y2>=ZOOM_Y1 )
                          {
                                  int barWidth = (x2-x1+1) * hit_points /
     max_hit_points;

                                  if( hit_points > 0 && x1+barWidth-1 >=
     ZOOM_X1)
                                  {
                                          vga_back.bar( MAX(x1,ZOOM_X1), y1  ,
     MIN(x1+barWidth-1,ZOOM_X2), y1, frameColor ); vga_back.bar(
     MAX(x1,ZOOM_X1), y1+1, MIN(x1+barWidth-1,ZOOM_X2), y2, V_GREEN );
                                  }
                          }
                  }
          }
...
int Firm::can_toggle_town_link(int townRecno) {
  ...
  if (cast_to_FirmLair() && town_array[townRecno]->nation_recno == 0 &&
      nation_recno)
    return 0; // exclude slave town

  if (town_array[townRecno]->nation_recno == 0 && nation_recno)
    return town_array[townRecno]->resistance(nation_recno) <=
           INDEPENDENT_LINK_RESISTANCE;
...
int Firm::construction_frame() {
  ...
  FirmBuild *firmBuild = firm_res.get_build(firm_build_id);
  int r = int(hit_points * firmBuild->under_construction_bitmap_count /
              max_hit_points());
  err_when(r < 0);
...
//
void Firm::left_click() { firm_array.selected_recno = firm_recno; }
