...
void GameSet::load_set_header() {
  ...
  setDir.read(DIR_RES "*.SET", 1); // read in the file list of all game sets in
                                   // the directory, 1-Sort file names
  ...
  for (i = 1; i <= setDir.size(); i++) {
    ...
    dataPtr =
        set_res.read(SET_HEADER_DB); // get the pointer to the header database

    err_when(!dataPtr);

    set_db.open_from_buf(dataPtr); // read in the header database

    setRec = (SetRec *)set_db.read(
        1); // the header database only contains one record
...
void GameSet::open_set(int setId) {
  err_if(setId < 0 || setId > set_count) err_here();
