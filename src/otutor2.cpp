...
int Tutor::select_tutor(int actionMode)
{
    ...
    while (1)
    {
        ...
        if (refreshFlag)
        {
            ...
            if (refreshFlag & TUOPTION_PAGE)
            {
                ...
                for (int j = 0; j < MAX_BROWSE_DISP_REC; ++j)
                {
                    ...
                    vga_front.read_bitmapW(
                        menuX1 + BROWSE_X1, menuY1 + BROWSE_Y1 + j * BROWSE_REC_HEIGHT, menuX1 + BROWSE_X2,
                        menuY1 + BROWSE_Y1 + j * BROWSE_REC_HEIGHT + BROWSE_REC_HEIGHT - 1, browseArea[j].ptr);
                }

                scrollArea.resize(0, 0, SCROLL_X2 - SCROLL_X1 + 1, SCROLL_Y2 - SCROLL_Y1 + 1);
                vga_front.read_bitmapW(menuX1 + SCROLL_X1, menuY1 + SCROLL_Y1, menuX1 + SCROLL_X2, menuY1 + SCROLL_Y2,
                                       scrollArea.ptr);

                textArea.resize(0, 0, TEXT_AREA_WIDTH, TEXT_AREA_HEIGHT);
                vga_front.read_bitmapW(menuX1 + TEXT_AREA_X1, menuY1 + TEXT_AREA_Y1, menuX1 + TEXT_AREA_X2,
                                       menuY1 + TEXT_AREA_Y2, textArea.ptr);
#endif
                scrollUp.paint();
                scrollDown.paint();
                startButton.paint();
            ...
            if (refreshFlag & TUOPTION_TEXT_AREA)
            {
                ...
                vga_front.put_bitmapW(menuX1 + TEXT_AREA_X1, menuY1 + TEXT_AREA_Y1, textArea.ptr);
#endif
                if (browseRecno)
                {
                    ...
                    font_std.put_paragraph(menuX1 + TEXT_AREA_X1, menuY1 + TEXT_AREA_Y1, menuX1 + TEXT_AREA_X2,
                                           menuY1 + TEXT_AREA_Y2, get_intro(browseRecno),
...
#ifdef TU_USE_BACKUP_SURFACE
                // copy from back buffer
                vga_front.put_bitmapW(menuX1 + SCROLL_X1, menuY1 + SCROLL_Y1, scrollArea.ptr);
#endif
                // display scroll bar
                scrollBar.paint();
            }
            ...
            if (refreshFlag & TUOPTION_ALL_BROWSE)
            {
                ...
                for (slot = 0; slot < scrollBar.view_size; ++slot)
                {
                    ...
                    {
#ifdef TU_USE_BACKUP_SURFACE
                        vga_front.put_bitmapW(browseSlotX1, browseSlotY1, browseArea[rec % MAX_BROWSE_DISP_REC].ptr);
#endif
                        if (rec >= 1 && rec <= tutor_count)
                        {
                            int textX = font_bible.put(browseSlotX1 + TEXT_OFFSET_X, browseSlotY1 + TEXT_OFFSET_Y,
                                                       misc.format(rec), 0, browseSlotX2);
                            textX = font_bible.put(textX, browseSlotY1 + TEXT_OFFSET_Y, ". ", 0, browseSlotX2);
                            textX = font_bible.put(textX, browseSlotY1 + TEXT_OFFSET_Y, this->operator[](rec)->des, 0,
                                                   browseSlotX2);
        ...
        else if (mouse.double_click(menuX1 + BROWSE_X1, menuY1 + BROWSE_Y1, menuX1 + BROWSE_X1 + BROWSE_REC_WIDTH - 1,
        ...
        else if (mouse.single_click(menuX1 + BROWSE_X1, menuY1 + BROWSE_Y1, menuX1 + BROWSE_X1 + BROWSE_REC_WIDTH - 1,
                                    menuY1 + BROWSE_Y1 + BROWSE_REC_HEIGHT * MAX_BROWSE_DISP_REC - 1))
        {
            // click on game slot
            int oldValue = browseRecno;
            ...
            if (newValue <= tutor_count)
            {
                if (oldValue != newValue)
                {
                    browseRecno = newValue;
                    refreshFlag |=
                        TUOPTION_BROWSE(newValue - scrollBar.view_recno) | TUOPTION_TEXT_AREA | TUOPTION_PIC_AREA;
