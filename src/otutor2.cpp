...
enum { SCROLL_X1 = 757, SCROLL_Y1 = 309, SCROLL_X2 = 770, SCROLL_Y2 = 492 };
...
int Tutor::select_tutor(int actionMode) {
  ...
  if (tutor_count == 0) {
    box.msg(text_tutorial.str_no_tutorial()); // "Tutorial files not found." );
    return 0;
  }
  ...
  scrollUp.create(menuX1 + SCROLL_X1, menuY1 + SCROLL_Y1 - 17, "SV-UP-U",
                  "SV-UP-D", 1, 0);
  scrollDown.create(menuX1 + SCROLL_X1, menuY1 + SCROLL_Y2 + 1, "SV-DW-U",
                    "SV-DW-D", 1, 0);
  startButton.create(menuX1 + 170, menuY1 + 529, "CARA-SEH", "CARA-SED", 1, 0);
  //	startButton.create(menuX1+170, menuY1+529, "START-U", "START-D",1, 0);
  cancelButton.create(menuX1 + 465, menuY1 + 529, "CARA-SEH", "CARA-SED", 1, 0);
  //	cancelButton.create(menuX1+465, menuY1+529, "CANCEL-U", "CANCEL-D", 1,
  //0);

  SlideVBar scrollBar;
  scrollBar.init_scroll(menuX1 + SCROLL_X1, menuY1 + SCROLL_Y1,
                        menuX1 + SCROLL_X2, menuY1 + SCROLL_Y2,
  ...
  Blob2DW browseArea[MAX_BROWSE_DISP_REC];
  Blob2DW scrollArea;
  Blob2DW textArea;
#endif
  ...
  while (1) {
    ...
    if (refreshFlag) {
      ...
      if (refreshFlag & TUOPTION_PAGE) {
        mouse.hide_area(menuX1, menuY1, menuX1 + TUTOR_MENU_WIDTH,
                        menuY1 + TUTOR_MENU_HEIGHT);
        ...
        for (int j = 0; j < MAX_BROWSE_DISP_REC; ++j) {
          browseArea[j].resize(0, 0, BROWSE_REC_WIDTH, BROWSE_REC_HEIGHT);
          vga_front.read_bitmapW(menuX1 + BROWSE_X1,
                                 menuY1 + BROWSE_Y1 + j * BROWSE_REC_HEIGHT,
                                 menuX1 + BROWSE_X2,
                                 menuY1 + BROWSE_Y1 + j * BROWSE_REC_HEIGHT +
                                     BROWSE_REC_HEIGHT - 1,
                                 browseArea[j].ptr);
        ...
        scrollArea.resize(0, 0, SCROLL_X2 - SCROLL_X1 + 1,
                          SCROLL_Y2 - SCROLL_Y1 + 1);
        vga_front.read_bitmapW(menuX1 + SCROLL_X1, menuY1 + SCROLL_Y1,
                               menuX1 + SCROLL_X2, menuY1 + SCROLL_Y2,
                               scrollArea.ptr);

        textArea.resize(0, 0, TEXT_AREA_WIDTH, TEXT_AREA_HEIGHT);
        vga_front.read_bitmapW(menuX1 + TEXT_AREA_X1, menuY1 + TEXT_AREA_Y1,
      ...
      if (refreshFlag & TUOPTION_PIC_AREA) {
        if (browseRecno &&
            image_tutorial.get_index(this->operator[](browseRecno)->code)) {
          image_tutorial.put_large(&vga_front, 21, 19,
                                   this->operator[](browseRecno)->code);
        } else {
          // draw the background
#ifdef TU_USE_BACKUP_SURFACE
          // copy from ?
      ...
      if (refreshFlag & TUOPTION_TEXT_AREA) {
        ...
        vga_front.put_bitmapW(menuX1 + TEXT_AREA_X1, menuY1 + TEXT_AREA_Y1,
                              textArea.ptr);
#endif
        if (browseRecno) {
          ...
          font_std.put_paragraph(menuX1 + TEXT_AREA_X1, menuY1 + TEXT_AREA_Y1,
                                 menuX1 + TEXT_AREA_X2, menuY1 + TEXT_AREA_Y2,
                                 get_intro(browseRecno),
...
#ifdef TU_USE_BACKUP_SURFACE
        // copy from back buffer
        vga_front.put_bitmapW(menuX1 + SCROLL_X1, menuY1 + SCROLL_Y1,
                              scrollArea.ptr);
#endif
        // display scroll bar
        scrollBar.paint();
          ...
          if (refreshFlag & TUOPTION_BROWSE(slot)) {
            ...
            if (rec >= 1 && rec <= tutor_count) {
              ...
              if (rec == browseRecno) {
                ...
                // vga_front.put_bitmap_trans_decompress( menuX1+BROWSE_X1,
                // menuY1+BROWSE_Y1+slot*BROWSE_REC_HEIGHT,
    ...
    if (scrollBar.detect() == 1) {
      ...
      if (oldValue != scrollBar.set_view_recno(oldValue + 1))
        refreshFlag |= TUOPTION_ALL_BROWSE | TUOPTION_SCROLL;
                                  ...
                                  menuY1 + BROWSE_Y1 +
                                      BROWSE_REC_HEIGHT * MAX_BROWSE_DISP_REC -
                                      1)) {
      // double click on game slot
      int oldValue = browseRecno;
      int newValue =
          scrollBar.view_recno +
          (mouse.click_y(0) - BROWSE_Y1 - menuY1) / BROWSE_REC_HEIGHT;
      ...
      if (newValue <= tutor_count) {
        if (oldValue != newValue) {
          ...
          if (oldValue - scrollBar.view_recno >= 0 &&
              oldValue - scrollBar.view_recno < MAX_BROWSE_DISP_REC)
