...
//							0  - no addition or deletion, just
//return 							-1 - del a record from the array
//
// <int> actionRecno : the recno to be deleted, if actionType is -1.
//							   the recno to be added, if
//actionType is 1. 	<int&> arrayCount:	for return the count of the AI info
//array
//
short *Nation::update_ai_firm_array(int firmId, int actionType, int actionRecno,
                                    int &arrayCount) {
  short *rc;
  ...
  case FIRM_RESEARCH:
    rc =
        update_ai_array(ai_research_count, ai_research_size, &ai_research_array,
  ...
  case FIRM_BEE:
  case FIRM_TERMITE:
    rc = update_ai_array(ai_mons_train_count, ai_mons_train_size,
                         &ai_mons_train_array, AI_MONS_TRAIN_INC_SIZE,
                         actionType, actionRecno);
    arrayCount = ai_mons_train_count;
    break;

  case FIRM_ALCHEMY:
    rc = update_ai_array(ai_mons_alchemy_count, ai_mons_alchemy_size,
                         &ai_mons_alchemy_array, AI_MONS_ALCHEMY_INC_SIZE,
                         actionType, actionRecno);
    arrayCount = ai_mons_alchemy_count;
    break;

  case FIRM_FORTRESS:
    rc = update_ai_array(ai_mons_fort_count, ai_mons_fort_size,
                         &ai_mons_fort_array, AI_MONS_FORT_INC_SIZE, actionType,
                         actionRecno);
    arrayCount = ai_mons_fort_count;
    break;

  case FIRM_ANIMAL:
    rc = update_ai_array(ai_mons_animal_count, ai_mons_animal_size,
                         &ai_mons_animal_array, AI_MONS_ANIMAL_INC_SIZE,
                         actionType, actionRecno);
    arrayCount = ai_mons_animal_count;
    break;

  case FIRM_INCUBATOR:
    rc = update_ai_array(ai_incubator_count, ai_incubator_size,
                         &ai_incubator_array, AI_INCUBATOR_INC_SIZE, actionType,
                         actionRecno);
    arrayCount = ai_incubator_count;
    break;

  case FIRM_MAGIC:
    rc = update_ai_array(ai_mons_magic_count, ai_mons_magic_size,
                         &ai_mons_magic_array, AI_MONS_MAGIC_INC_SIZE,
                         actionType, actionRecno);
...
short *Nation::update_ai_array(short &aiInfoCount, short &aiInfoSize,
                               short **aiInfoArrayPtr, int arrayIncSize,
  ...
  if (actionType == -1) {
    ...
    if (aiInfoCount == aiInfoSize) {
      ...
      *aiInfoArrayPtr =
          (short *)mem_resize(aiInfoArray, aiInfoSize * sizeof(short));
...
void Nation::add_town_info(short townRecno) {
  update_ai_array(ai_town_count, ai_town_size, &ai_town_array, AI_TOWN_INC_SIZE,
                  1, townRecno);
...
void Nation::add_general_info(short unitRecno) {
  ...
  update_ai_array(ai_general_count, ai_general_size, &ai_general_array,
                  AI_GENERAL_INC_SIZE, 1, unitRecno);
}
...
//											that have
//been set stop within the given 											days will be counted as existing ones.
//
int Nation::is_caravan_exist(int firstStop, int secondStop,
                             int setStopInterval) {
...
void Nation::add_ship_info(short unitRecno) {
  update_ai_array(ai_ship_count, ai_ship_size, &ai_ship_array, AI_SHIP_INC_SIZE,
                  1, unitRecno);
}
...
//								 otherwise, return all space,
//regardless of 								 whether there are units there.
//
// [int] firmId - the firm id. of the camp. If this is given,
//						then only firm of this type will be
//counted. 						Otherwise, any camps, forts or lairs will be counted.
//
  ...
  for (int i = 0; i < ai_base_count; ++i) {
    ...
    if (!firm_array.is_deleted(firmRecno) &&
      ...
      if (godPtr->nation_recno == nation_recno &&
          misc.points_distance(xLoc, yLoc, godPtr->next_x_loc(),
                               godPtr->next_y_loc()) <= effectiveRange) {
        return 1;
      }
    }
  }
