...
//							0  - no addition or deletion,
// just
// return 							-1 - del a record from the
// array
//
// <int> actionRecno : the recno to be deleted, if actionType is -1.
//							   the recno to be added,
// if
// actionType is 1. 	<int&> arrayCount:	for return the count of the AI
// info array
//
short *Nation::update_ai_firm_array(int firmId, int actionType, int actionRecno, int &arrayCount)
{
    ...
    {
    case FIRM_BASE:
        rc = update_ai_array(ai_base_count, ai_base_size, &ai_base_array, AI_BASE_INC_SIZE, actionType, actionRecno);
    ...
    case FIRM_LAIR:
        ...
        // ####### end Gilbert 4/5 ########//
        rc = update_ai_array(ai_camp_count, ai_camp_size, &ai_camp_array, AI_CAMP_INC_SIZE, actionType, actionRecno);
        arrayCount = ai_camp_count;
        break;

    case FIRM_MINE:
        rc = update_ai_array(ai_mine_count, ai_mine_size, &ai_mine_array, AI_MINE_INC_SIZE, actionType, actionRecno);
        arrayCount = ai_mine_count;
        break;

    case FIRM_FACTORY:
        rc = update_ai_array(ai_factory_count, ai_factory_size, &ai_factory_array, AI_FACTORY_INC_SIZE, actionType,
                             actionRecno);
        arrayCount = ai_factory_count;
        break;

    case FIRM_MARKET:
        rc = update_ai_array(ai_market_count, ai_market_size, &ai_market_array, AI_MARKET_INC_SIZE, actionType,
                             actionRecno);
        arrayCount = ai_market_count;
        break;

    case FIRM_INN:
        rc = update_ai_array(ai_inn_count, ai_inn_size, &ai_inn_array, AI_INN_INC_SIZE, actionType, actionRecno);
        arrayCount = ai_inn_count;
        break;

    case FIRM_RESEARCH:
        rc = update_ai_array(ai_research_count, ai_research_size, &ai_research_array, AI_RESEARCH_INC_SIZE, actionType,
                             actionRecno);
        arrayCount = ai_research_count;
        break;

    case FIRM_WAR_FACTORY:
        rc = update_ai_array(ai_war_count, ai_war_size, &ai_war_array, AI_WAR_INC_SIZE, actionType, actionRecno);
        arrayCount = ai_war_count;
        break;

    case FIRM_SPY:
        rc = update_ai_array(ai_spy_count, ai_spy_size, &ai_spy_array, AI_SPY_INC_SIZE, actionType, actionRecno);
        arrayCount = ai_spy_count;
        break;

    case FIRM_SPECIAL:
        rc = update_ai_array(ai_special_count, ai_special_size, &ai_special_array, AI_SPECIAL_INC_SIZE, actionType,
                             actionRecno);
    ...
    case FIRM_OFFENSIVE_BUILDING_3:
    case FIRM_OFFENSIVE_BUILDING_4:
        rc = update_ai_array(ai_offend_count, ai_offend_size, &ai_offend_array, AI_OFFEND_INC_SIZE, actionType,
                             actionRecno);
    ...
    case FIRM_WILDE_LISHORR:
        rc = update_ai_array(ai_lishorr_count, ai_lishorr_size, &ai_lishorr_array, AI_LISHORR_INC_SIZE, actionType,
                             actionRecno);
    ...
    case FIRM_BEE:
    case FIRM_TERMITE:
        rc = update_ai_array(ai_mons_train_count, ai_mons_train_size, &ai_mons_train_array, AI_MONS_TRAIN_INC_SIZE,
                             actionType, actionRecno);
    ...
    case FIRM_FORTRESS:
        rc = update_ai_array(ai_mons_fort_count, ai_mons_fort_size, &ai_mons_fort_array, AI_MONS_FORT_INC_SIZE,
                             actionType, actionRecno);
        arrayCount = ai_mons_fort_count;
        break;

    case FIRM_ANIMAL:
        rc = update_ai_array(ai_mons_animal_count, ai_mons_animal_size, &ai_mons_animal_array, AI_MONS_ANIMAL_INC_SIZE,
                             actionType, actionRecno);
        arrayCount = ai_mons_animal_count;
        break;

    case FIRM_INCUBATOR:
        rc = update_ai_array(ai_incubator_count, ai_incubator_size, &ai_incubator_array, AI_INCUBATOR_INC_SIZE,
                             actionType, actionRecno);
        arrayCount = ai_incubator_count;
        break;

    case FIRM_MAGIC:
        rc = update_ai_array(ai_mons_magic_count, ai_mons_magic_size, &ai_mons_magic_array, AI_MONS_MAGIC_INC_SIZE,
                             actionType, actionRecno);
...
// actionType is 1.
//
short *Nation::update_ai_array(short &aiInfoCount, short &aiInfoSize, short **aiInfoArrayPtr, int arrayIncSize,
                               int actionType, int actionRecno)
{
    ...
    if (actionType == -1)
    {
        ...
        for (int i = 0; i < aiInfoCount; i++, aiInfoPtr++)
        {
            if (*aiInfoPtr == actionRecno)
            {
                ...
                else // the record to be deleted is not the last record, somewhere in
                     // the array
                {
                    //---- copy the last record to this slot which has been marked for
                    // deletion
    ...
    else if (actionType == 1)
    {
        ...
        {
#ifdef DEBUG
            short saveDate1 = aiInfoArray[0]; // for vertification of resizing that
                                              // old data are kept
            ...
            *aiInfoArrayPtr = (short *)mem_resize(aiInfoArray, aiInfoSize * sizeof(short));
            ...
            err_when(saveDate1 != aiInfoArray[0]); // for vertification of resizing
                                                   // that old data are kept
...
{
    update_ai_array(ai_town_count, ai_town_size, &ai_town_array, AI_TOWN_INC_SIZE, 1, townRecno);
...
void Nation::add_general_info(short unitRecno)
{
    ...
    update_ai_array(ai_general_count, ai_general_size, &ai_general_array, AI_GENERAL_INC_SIZE, 1, unitRecno);
}
...
{
    update_ai_array(ai_caravan_count, ai_caravan_size, &ai_caravan_array, AI_CARAVAN_INC_SIZE, 1, unitRecno);
}
...
//											that
// have
// been set stop within the given
...
int Nation::is_caravan_exist(int firstStop, int secondStop, int setStopInterval)
{
    ...
    for (int i = 0; i < ai_caravan_count; i++)
    {
        ...
        if ((unitCaravan->stop_array[0].firm_recno == firstStop &&
            ...
            (unitCaravan->stop_array[1].firm_recno == firstStop && unitCaravan->stop_array[0].firm_recno == secondStop))
        {
            if (setStopInterval)
            {
...
{
    update_ai_array(ai_ship_count, ai_ship_size, &ai_ship_array, AI_SHIP_INC_SIZE, 1, unitRecno);
}
...
//						then only firm of this type will
// be
// counted. 						Otherwise, any camps, forts or lairs
// will be counted.
//
...
int Nation::revealed_by_phoenix(int xLoc, int yLoc)
{
    ...
    for (int i = 0; i < ai_base_count; ++i)
    {
        ...
        FirmBase *firmBase;
        if (!firm_array.is_deleted(firmRecno) && (firmBase = firm_array[firmRecno]->cast_to_FirmBase()) &&
            firmBase->god_id == GOD_PERSIAN && !unit_array.is_deleted(firmBase->god_unit_recno))
        {
            Unit *godPtr = unit_array[firmBase->god_unit_recno];
            if (godPtr->nation_recno == nation_recno &&
                misc.points_distance(xLoc, yLoc, godPtr->next_x_loc(), godPtr->next_y_loc()) <= effectiveRange)
            {
                return 1;
            }
        }
