...
int Nation::consider_talk_msg(TalkMsg *talkMsg)
{
    ...
    case TALK_PROPOSE_TRADE_TREATY:
        ...
        return consider_trade_treaty(talkMsg->from_nation_recno) >= 0; // the returned value is the curRating -
                                                                       // acceptRating, if >=0, means it accepts
...
int Nation::consider_friendly_treaty(int withNationRecno)
{
    ...
    if (nationRelation->status >= RELATION_FRIENDLY) // already has a friendly relationship
...
int Nation::consider_cease_war(int withNationRecno)
{
    ...
    if (ai_capture_enemy_town_recno && !town_array.is_deleted(ai_capture_enemy_town_recno) &&
    ...
    if (!ai_should_spend_war(withNation->military_rank_rating(),
    ...
    curRating += ai_trade_with_rating(withNationRecno) * (100 + pref_trading_tendency) /
                 300; // when we have excessive supply, we may want to cease-fire
                      // with our enemy

    curRating -= (military_rank_rating() - 50) / 2; // if our military ranking is high, we may like to continue
                                                    // the war, otherwise the nation should try to cease-fire

    curRating -=
        nationRelation->started_war_on_us_count * 10; // the number of times this nation has started a war with us, the
...
int Nation::consider_alliance_rating(int nationRecno)
{
    ...
    int powerRating =
        nationPtr->military_rank_rating() - military_rank_rating(); // if the nation's power is larger than ours, it's
...
int Nation::consider_exchange_tech(TalkMsg *talkMsg)
{
    ...
    int relationChange =
        (talkMsg->talk_para2 - ourTechLevel) * (5 + pref_use_weapon / 10); // increase relationship by 5 to 15 points
...
int Nation::consider_give_aid(TalkMsg *talkMsg)
{
    ...
    if (nationRelation->status >= RELATION_FRIENDLY && ai_should_spend(importanceRating,
                                                                       talkMsg->talk_para1)) // 0-importance is 0
    {
        if (info.game_date >
            nationRelation->last_change_status_date + 720 - pref_allying_tendency) // we have allied with this nation
...
int Nation::consider_declare_war(TalkMsg *talkMsg)
{
    ...
    if (againstRelation->status >= RELATION_FRIENDLY) // no need to handle RELATION_ALLIANCE separately as
...
int Nation::consider_military_aid(TalkMsg *talkMsg)
{
    ...
    if (ai_general_count * 10 + total_human_count + total_monster_count < 100 - pref_military_courage / 2)
    {
        return 0;
    }
    ...
    int targetCombatLevel =
        mobile_defense_combat_level(baseObj->obj_loc_x1(), baseObj->obj_loc_y1(), baseObj->nation_recno, 0, hasWar);

    if (ai_attack_target(baseObj->obj_loc_x1(), baseObj->obj_loc_y1(), targetCombatLevel, 0,
...
int Nation::consider_accept_surrender_request(TalkMsg *talkMsg)
{
    ...
    int offeredAmt = talkMsg->talk_para1 * 10; // *10 to restore its original value which has been
...
int Nation::ai_overall_relation_rating(int withNationRecno)
{
    ...
    int overallRating = nationRelation->ai_relation_level + (int)nationRelation->good_relation_duration_rating +
                        (int)nationPtr->reputation + nationPtr->military_rank_rating() + trade_rating(withNationRecno) +
                        ai_trade_with_rating(withNationRecno) / 2 + nationPtr->total_alliance_military();
