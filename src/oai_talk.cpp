...
int Nation::ai_process_talk_msg(ActionNode *actionNode) {
  if (talk_res.is_talk_msg_deleted(
          actionNode->action_para)) // if the talk message has been deleted
    return -1;
  ...
  if (talkMsg->reply_type == REPLY_WAITING) {
    ...
    if (rc == 1) // if rc is not 1 or 0, than the consider function have
                 // processed itself, no need to call reply_talk_msg() here
      talk_res.reply_talk_msg(actionNode->action_para, REPLY_ACCEPT,
                              COMMAND_AI);

    else if (rc == 0)
      talk_res.reply_talk_msg(actionNode->action_para, REPLY_REJECT,
                              COMMAND_AI);
...
int Nation::consider_talk_msg(TalkMsg *talkMsg) {
  ...
  if (info.game_date <
      nationRelation->never_accept_until_date_array[talkMsg->talk_id - 1])
    return 0;
  ...
  case TALK_PROPOSE_TRADE_TREATY:
    ...
    //--- if the AI will never propose an alliance treaty to this nation, then
    //it will also never accept an alliance treaty from it ---//

    return consider_trade_treaty(talkMsg->from_nation_recno) >=
           0; // the returned value is the curRating - acceptRating, if >=0,
              // means it accepts
  ...
  case TALK_REQUEST_DECLARE_WAR: // a nation demands you to declare war with
                                 // another nation
...
void Nation::notify_talk_msg(TalkMsg *talkMsg) {
  ...
  switch (talkMsg->talk_id) {
  case TALK_END_TRADE_TREATY: // it's a notification message only, no accept or
                              // reject
    relationChange = -5;
    nationRelation->set_never_accept_until_date(TALK_PROPOSE_TRADE_TREATY, 180);
    break;

  case TALK_END_FRIENDLY_TREATY: // it's a notification message only, no accept
                                 // or reject
  case TALK_END_ALLIANCE_TREATY:
    relationChange = -5;
    nationRelation->set_never_accept_until_date(TALK_PROPOSE_FRIENDLY_TREATY,
                                                180);
    nationRelation->set_never_accept_until_date(TALK_PROPOSE_ALLIANCE_TREATY,
                                                180);
    break;

  case TALK_DECLARE_WAR: // it already drops to zero when the status is set to
                         // hostile
    break;
...
int Nation::consider_trade_treaty(int withNationRecno) {
  ...
  //---- don't accept new trade treaty soon when the trade treaty was terminated
  //not too long ago ----//

  if (info.game_date <
      nationRelation->never_accept_until_date_array[TALK_END_TRADE_TREATY - 1])
    return 0;
...
int Nation::consider_friendly_treaty(int withNationRecno) {
  ...
  if (nationRelation->status >=
      RELATION_FRIENDLY) // already has a friendly relationship
  ...
  int acceptRating = 60 - pref_allying_tendency / 8 -
                     pref_peacefulness / 4; // range of acceptRating: 23 to 60
...
int Nation::consider_alliance_treaty(int withNationRecno) {
  ...
  if (nationRelation->status >=
      RELATION_ALLIANCE) // already has a friendly relationship
    return -1;           // -1 means don't reply
  ...
  int acceptRating = 80 - pref_allying_tendency / 4 -
                     pref_peacefulness / 8; // range of acceptRating: 43 to 80
...
int Nation::consider_cease_war(int withNationRecno) {
  ...
  if (ai_capture_enemy_town_recno &&
      ...
      town_array[ai_capture_enemy_town_recno]->nation_recno ==
          withNationRecno) {
    return -1;
  }
  ...
  if (info.game_date <
      nationRelation->last_change_status_date + 60 +
          (100 - pref_peacefulness)) // more peaceful nation may cease fire
                                     // sooner (but the minimum is 60 days).
    return -1;
  ...
  if (!ai_should_spend_war(
          withNation->military_rank_rating(),
          1)) // if we shouldn't spend any more on war, then return 1
    return 1;
  ...
  curRating += ai_trade_with_rating(withNationRecno) *
               (100 + pref_trading_tendency) /
               300; // when we have excessive supply, we may want to cease-fire
                    // with our enemy

  curRating -= (military_rank_rating() - 50) /
               2; // if our military ranking is high, we may like to continue
                  // the war, otherwise the nation should try to cease-fire

  curRating -=
      nationRelation->started_war_on_us_count *
      10; // the number of times this nation has started a war with us, the
          // higher the number, the more unlikely we will accept cease-fire
...
int Nation::consider_sell_food(TalkMsg *talkMsg) {
  ...
  //--- if after selling the food, the remaining is not enough for its own
  ...
  int relationLevel =
      get_relation(talkMsg->from_nation_recno)->ai_relation_level;
...
int Nation::consider_alliance_rating(int nationRecno) {
  ...
  //--- if the nation has a bad record of starting wars with us before, decrease
  //the rating ---//
  ...
  int powerRating =
      ...
      military_rank_rating(); // if the nation's power is larger than ours, it's
                              // good to form treaty with them
...
static int has_sent_same_msg(TalkMsg *talkMsgPtr) {
  ...
  return talk_res.is_talk_msg_exist(&talkMsg,
                                    1); // 1-check talk_para1 & talk_para2
}
...
int Nation::consider_exchange_tech(TalkMsg *talkMsg) {
  ...
  int giveLevel =
      talkMsg->talk_para3 -
      tech_res[talkMsg->talk_para1]->get_nation_tech_level(nation_recno);
  int gainLevel =
      talkMsg->talk_para4 -
      tech_res[talkMsg->talk_para2]->get_nation_tech_level(nation_recno);
  ...
  int relationChange =
      (talkMsg->talk_para2 - ourTechLevel) *
      (5 + pref_use_weapon / 10); // increase relationship by 5 to 15 points

  change_ai_relation_level(talkMsg->from_nation_recno, relationChange);

  return 1;
}
//------ End of function Nation::consider_exchange_tech ------//
...
int Nation::consider_give_aid(TalkMsg *talkMsg) {
  ...
  if (info.game_date <
      nationRelation->never_accept_until_date_array[TALK_GIVE_AID - 1])
    return 0;
  ...
  if (nationRelation->status >= RELATION_FRIENDLY &&
      ai_should_spend(importanceRating,
                      talkMsg->talk_para1)) // 0-importance is 0
  {
    if (info.game_date >
        nationRelation->last_change_status_date + 720 -
            pref_allying_tendency) // we have allied with this nation for quite
                                   // some while
    {
      nationRelation->set_never_accept_until_date(
          TALK_GIVE_AID, 180); // don't give aid again too soon
      return 1;
    }
  }
...
int Nation::consider_give_tribute(TalkMsg *talkMsg) {
  ...
  if (info.game_date <
      nationRelation->never_accept_until_date_array[TALK_GIVE_TRIBUTE - 1])
    return 0;
  ...
  int militaryDiff =
      fromNation->military_rank_rating() - military_rank_rating();
...
int Nation::consider_declare_war(TalkMsg *talkMsg) {
  //--- if it even won't consider trade embargo, there is no reason that it will
  ...
  int fromRelationRating =
      ai_overall_relation_rating(talkMsg->from_nation_recno);
  ...
  //--- if we don't have a good enough relation with the requesting nation, turn
  //down the request ---//

  if (fromRelation->good_relation_duration_rating < 10)
    return 0;

  //--- if we are more friendly with the against nation than the requesting
  //nation, turn down the request ---//
  ...
  int acceptRating =
      100 + againstNation->total_enemy_military() - military_rank_rating();

  //--- it won't declare war with a friendly or allied nation easily ---//

  if (againstRelation->status >=
      RELATION_FRIENDLY) // no need to handle RELATION_ALLIANCE separately as
                         // ai_overall_relation_relation() has already taken it
                         // into account
    acceptRating += 100;
...
int Nation::consider_trade_embargo(TalkMsg *talkMsg) {
  ...
  //--- if we have a large trade with the against nation or have a larger trade
  //with the against nation than the requesting nation ---//
...
int Nation::consider_military_aid(TalkMsg *talkMsg) {
  ...
  if (info.game_date <
      fromRelation->last_military_aid_date + 200 - pref_allying_tendency)
    return 0;
  ...
  int targetCombatLevel =
      mobile_defense_combat_level(baseObj->obj_loc_x1(), baseObj->obj_loc_y1(),
                                  baseObj->nation_recno, 0, hasWar);

  if (ai_attack_target(baseObj->obj_loc_x1(), baseObj->obj_loc_y1(),
                       targetCombatLevel, 0,
                       1)) // 0-not defense mode, 1-just move to flag
  {
...
int Nation::consider_accept_surrender_request(TalkMsg *talkMsg) {
  ...
  int offeredAmt = talkMsg->talk_para1 *
                   10; // *10 to restore its original value which has been
                       // divided by 10 to cope with <short> upper limit

  //--- if the nation does not have any towns, camps or generals, it will have a
  //larger tendency to surrender. ---//

  int weakNation =
      (ai_town_count == 0 && ai_camp_count == 0 && ai_general_count == 0);
  ...
  //---- don't surrender to the player if the player is already the most
  //powerful nation ---//

  if (!weakNation && !nationPtr->is_ai() &&
      config.ai_aggressiveness >= OPTION_HIGH) {
  ...
  if (!(cash < 100 && profit_365days() < 0) && !weakNation) {
    ...
    //-- if there are only two nations left, don't surrender if we still have
    //some power --//
