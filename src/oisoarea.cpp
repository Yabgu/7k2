...
IsolateArea::IsolateArea() { init(); }
//----------- End of function IsolateArea::IsolateArea -----------//

//--------- Begin of function IsolateArea::~IsolateArea ---------//
IsolateArea::~IsolateArea() { deinit(); }
//----------- End of function IsolateArea::~IsolateArea -----------//
...
int IsolateArea::to_regions_line_left(char dir, int withVerticalCheck) {
  ...
  case DIR_N:
    if (withVerticalCheck)
      return dir == DIR_N || dir == DIR_S || dir == DIR_SW || dir == DIR_W ||
             dir == DIR_NW;
    else
  ...
  case DIR_NE:
    if (withVerticalCheck)
      return dir == DIR_NE || dir == DIR_SW || dir == DIR_W || dir == DIR_NW ||
             dir == DIR_N;
    else
  ...
  case DIR_E:
    if (withVerticalCheck)
      return dir == DIR_E || dir == DIR_W || dir == DIR_NW || dir == DIR_N ||
             dir == DIR_NE;
    else
  ...
  case DIR_SE:
    if (withVerticalCheck)
      return dir == DIR_SE || dir == DIR_NW || dir == DIR_N || dir == DIR_NE ||
             dir == DIR_E;
    else
  ...
  case DIR_S:
    if (withVerticalCheck)
      return dir == DIR_S || dir == DIR_N || dir == DIR_NE || dir == DIR_E ||
             dir == DIR_SE;
    else
  ...
  case DIR_SW:
    if (withVerticalCheck)
      return dir == DIR_SW || dir == DIR_NE || dir == DIR_E || dir == DIR_SE ||
             dir == DIR_S;
    else
  ...
  case DIR_W:
    if (withVerticalCheck)
      return dir == DIR_W || dir == DIR_E || dir == DIR_SE || dir == DIR_S ||
             dir == DIR_SW;
    else
  ...
  case DIR_NW:
    if (withVerticalCheck)
      return dir == DIR_NW || dir == DIR_SE || dir == DIR_S || dir == DIR_SW ||
             dir == DIR_W;
    else
...
//
int IsolateArea::detect_isolate_area(short refXLoc, short refYLoc,
                                     short spaceXLoc, short spaceYLoc) {
  ...
  err_when(misc.points_distance(startXLoc1, startYLoc1, startXLoc2,
                                startYLoc2) != 1);
  ...
  regions_line_dir =
      cal_direction(startXLoc1, startYLoc1, startXLoc2, startYLoc2);
  ...
  if (processXLoc != startXLoc2 || processYLoc != startYLoc2) {
    ...
    while (1) {
      ...
      //-----------------------------------------------------------------------------//
      // consider the shap of 8 (turning 90 degree) and the (startXLoc2,
      // startYLoc2) is the touching point of the two circles.
      //-----------------------------------------------------------------------------//
      if (!cycle) {
        if (processXLoc == startXLoc2 && processYLoc == startYLoc2)
          cycle++;
...
//
void IsolateArea::process_move_around_boundary(char &dir, short &xLoc,
                                               short &yLoc) {
//------------------------------------------------------------------------------//
...
//
// Once the first offset is used, the rest offset can be calculated faster by
// using xCorrOffset and yCorrOffset arrays.
//------------------------------------------------------------------------------//
#define DIR_NUM 8
  static int xOffset[DIR_NUM] = {-1, -1, -1, 0, 1,
                                 1,  1,  0}; // correspoinding to DIR_N, i.e. 0
  static int yOffset[DIR_NUM] = {1, 0, -1, -1, -1, 0, 1, 1}; // ditto

  static int xCorrOffset[DIR_NUM] = {
      -1, 0, 0, 1,
      1,  0, 0, -1}; // corresponding offset from the previous offset in xOffset
  ...
  short checkXLoc = xLoc + xOffset[dirCount]; // use xOffset[] for first step,
                                              // and xCorrOffset for the rest
  short checkYLoc =
      yLoc + yOffset[dirCount]; // ditto, (since A+=B is faster than A=B+C)

  for (int i = 0; i < 8; i++, checkXLoc += xCorrOffset[dirCount],
           checkYLoc += yCorrOffset[dirCount]) {
  ...
  if (!canMove) {
    //-------- cannot move ---------//
    err_when(misc.points_distance(xLoc, yLoc, regions_line_x_loc1,
                                  regions_line_y_loc1) > 1);
  ...
  if (!xDiff || !yDiff || xDiff == yDiff) {
    testDir = cal_direction(regions_line_x_loc1, regions_line_y_loc1, checkXLoc,
                            checkYLoc);

    if (testDir == regions_line_dir) {
      //------ on same region -------//
      if (to_regions_line_left(dir)) {
        if (same_region.with_data &&
            (xLoc != regions_line_x_loc2 || yLoc != regions_line_y_loc2))
          reset_area_info(&same_region);
      } else {
        dist = misc.points_distance(checkXLoc, checkYLoc, regions_line_x_loc1,
                                    regions_line_y_loc1);
...
// return 0 otherwise
//
int IsolateArea::is_isolate_area() { return !diff_region.with_data; }
