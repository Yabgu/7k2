...
static short edit_energy_x1, edit_energy_y1, edit_energy_x2, edit_energy_y2,
    edit_energy_enable;
...
void FirmMagic::disp_main_menu(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    {
      // button_magic[i].create( INFO_X1+13 + i * BUTTON_DISTANCE, INFO_Y1+235,
      // 'A', "INVOKE" );
      button_magic[i].create(INFO_X1 + 23, INFO_Y1 + 80 + 20 * i, INFO_X1 + 210,
                             INFO_Y1 + 80 + 20 * i + 17, disp_magic_button,
                             ButtonCustomPara(this, i + 1));
  ...
  if (cast_delay > 0) {
    ...
    if (ratio < 11)
      vga.active_buf->put_bitmap_trans(INFO_X1 + 80 + 20 + offsetX,
                                       INFO_Y1 + 49 + offsetY,
                                       image_spict.read("MTR_10"));
    else if (ratio < 40)
      vga.active_buf->put_bitmap_trans(INFO_X1 + 80 + 20 + offsetX,
                                       INFO_Y1 + 49 + offsetY,
                                       image_spict.read("MTR_39"));
    else
      vga.active_buf->put_bitmap_trans(INFO_X1 + 80 + 20 + offsetX,
                                       INFO_Y1 + 49 + offsetY,
                                       image_spict.read("MTR_40"));

    vga.active_buf->read_bitmapW(INFO_X1 + 80 + 20 + offsetX,
                                 INFO_Y1 + 49 + offsetY,
                                 INFO_X1 + 94 + 20 + size + offsetX,
                                 INFO_Y1 + 80 + offsetY, hitPointBitmap);
    vga.active_buf->put_bitmap_trans(INFO_X1 + 80 + 20 + offsetX,
                                     INFO_Y1 + 49 + offsetY,
                                     image_spict.read("MTR_00"));
    vga.active_buf->put_bitmapW(INFO_X1 + 80 + 20 + offsetX,
                                INFO_Y1 + 49 + offsetY, hitPointBitmap);
  ...
  for (i = magic_id_lower_limit - 1; i < magic_id_upper_limit; ++i) // bug
  {
    button_magic[i].visible_flag =
        (nation_recno &&
         (tech_res[magic_info_array[i].tech_id]->get_nation_tech_level(
              nation_recno) ||
  ...
  for (i = magic_id_lower_limit - 1; i < magic_id_upper_limit; ++i) // bug
  {
    if (button_magic[i].detect()) {
      ...
      case MAGIC_TELEPORT:
        ...
        power.issue_command(COMMAND_MAGIC_TELEPORT_PRE, firm_recno,
                            menu_mode_para);
        break;
    ...
    if (button_magic[i].visible_flag) {
      ...
      //	help.set_custom_help( button_magic[i].x1, button_magic[i].y1,
      //button_magic[i].x2, button_magic[i].y2,
      //		tech_res[magic_info_array[button_magic[i].custom_para.value
      //-1].tech_id]->tech_des(), str );
      char helpName[10] = "MAGIC0";
      helpName[5] = '1' + i;
      help.set_help(button_magic[i].x1, button_magic[i].y1, button_magic[i].x2,
                    button_magic[i].y2, helpName);
    }
  }
}
...
void FirmMagic::disp_magic_menu(int refreshFlag) {
  ...
  case FIRM_MAGIC_MENU_CAST:
    font_snds.center_put_paragraph(
        INFO_X1 + 20, INFO_Y1 + 20, INFO_X2 - 10, INFO_Y1 + 60,
  ...
  //	font_snds.put( INFO_X1+20, INFO_Y1+50,
  //tech_res[magic_info_array[menu_mode_para-1].tech_id]->tech_des() );
  font_snds.center_put_paragraph(
      INFO_X1 + 20, INFO_Y1 + 60, INFO_X2 - 10, INFO_Y1 + 80,
      tech_res[magic_info_array[menu_mode_para - 1].tech_id]->tech_des(), 2, 0,
      0);
  ...
  if (menu_mode == FIRM_MAGIC_MENU_CAST) {
    ...
    if (world.zoom_matrix->get_detect_location(mouse.cur_x, mouse.cur_y,
                                               &pointedXLoc, &pointedYLoc,
                                               &mobileType)) {
      //	int x2 = font_snds.put( INFO_X1+20, INFO_Y1+70, "Life Point Cost
      //: "); 	font_snds.put( x2, INFO_Y1+70, calc_live_point_cost(
      //menu_mode_para, pointedXLoc, pointedYLoc, teleport_unit_recno) );
      ...
      case MAGIC_DAMAGE_AREA:
        font_snds.center_put_paragraph(
            INFO_X1 + 20, INFO_Y1 + 80, INFO_X2 - 10, INFO_Y1 + 120,
            text_firm.str_cast_cost_live_points(calc_live_point_cost(
                menu_mode_para, pointedXLoc, pointedYLoc, 0)),
            2, 0, 0);
        break;
      ...
      case MAGIC_TELEPORT:
        font_snds.center_put_paragraph(
            INFO_X1 + 20, INFO_Y1 + 80, INFO_X2 - 10, INFO_Y1 + 120,
            text_firm.str_teleport_cost_live_points(calc_live_point_cost(
                menu_mode_para, pointedXLoc, pointedYLoc, teleport_unit_recno)),
            2, 0, 0);
        break;
      }
...
int FirmMagic::set_teleport_unit(int unitRecno) {
  ...
  err_when(menu_mode_para != MAGIC_TELEPORT ||
           menu_mode != FIRM_MAGIC_MENU_UNIT);
...
static void disp_magic_button(ButtonCustom *button, int) {
  char *magicName =
      tech_res[FirmMagic::magic_info_array[button->custom_para.value - 1]
                   .tech_id]
          ->tech_des();
...
void FirmMagic::disp_edit_mode(int &refreshFlag, int &y, int dispBG) {
  ...
  x += 20;
  x = edit_energy_x2 = font_blue.put((edit_energy_x1 = x), (edit_energy_y1 = y),
                                     cast_delay, 0, x2);
...
int FirmMagic::detect_edit_mode() {
  ...
  if (edit_energy_enable) {
    if (mouse.any_click(edit_energy_x1, edit_energy_y1, edit_energy_x2,
                        edit_energy_y2, LEFT_BUTTON)) {
      cast_delay += 10;
...
void FirmMagic::right_click(int xLoc, int yLoc) {
  ...
  if (locPtr->is_firm() &&
      transfer_energy(locPtr->firm_recno(), COMMAND_PLAYER))
    return;
