...
void Sys::disp_view_mode(int observeMode)
{
    ...
    static short darkenXM1[MODE_TO_DISPLAY_COUNT] = {819, 931, 819, 931,
                                                     819, 931, 819, 931}; //{ 810, 931, 819, 940, 810, 931, 819, 940 };
    static short darkenYM1[MODE_TO_DISPLAY_COUNT] = {80,  80,  102, 102,
    ...
    //	if (current_display_mode.mode_id == MODE_ID_800x600x16)
    {
        for (int i = 0; i < 8; i++)
        {
            ...
            // darkenX[i] + darkenWidth -1, darkenY[i]
            // + darkenHeight -3, text_reports.str_report_mode(i+1)); //
            // scroll_name[i]); change in GERMAN version
            font_bld.center_put(darkenX[i], darkenY[i], darkenX[i] + darkenWidth - 1, darkenY[i] + darkenHeight - 3,
    ...
    */
    ...
    if (view_mode >= MIN_MODE_TO_DISPLAY && view_mode <= MAX_MODE_TO_DISPLAY)
    {
        //		if (current_display_mode.mode_id == MODE_ID_800x600x16)
        {
            ...
            font_bld.center_put(darkenX[view_mode - 1] + 1, darkenY[view_mode - 1] + 1,
                                darkenX[view_mode - 1] + darkenWidth, darkenY[view_mode - 1] + darkenHeight - 2,
                                text_reports.str_report_mode(view_mode)); // scroll_name[view_mode-1]);
        }
        /*		else if (current_display_mode.mode_id == MODE_ID_1024x768x16)
                        {
    ...
    if (observeMode || !nation_array.player_recno)
    {
        for (int j = 1; j <= 7; ++j)
        {
            vga_back.adjust_brightness(darkenX[j - MIN_MODE_TO_DISPLAY], darkenY[j - MIN_MODE_TO_DISPLAY],
...
void Sys::detect_view()
{
    ...
    if (view_mode == MODE_NORMAL &&
        game.game_mode == GAME_TUTORIAL) // tutorial text is only displayed in non-report mode
    {
        if (tutor.detect())
            return;
    }
...
void Sys::set_view_mode(int viewMode, int viewingNationRecno, int viewingSpyRecno)
{
    ...
    {
        if (viewMode != MODE_NORMAL && viewMode != MODE_RANK) // other reports are not available except the
