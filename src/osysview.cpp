...
//
// <int> observeMode		- force observe mode display (darken view mode 1 -
// 7)
//									needed in
//Game::game_end, nation_array.player_recno has not yet set to 0
void Sys::disp_view_mode(int observeMode) {
  ...
  const int MODE_TO_DISPLAY_COUNT =
      MAX_MODE_TO_DISPLAY - MIN_MODE_TO_DISPLAY + 1;

  //###### end Gilbert 4/1 #####//
  static short darkenXM0[MODE_TO_DISPLAY_COUNT] = {6, 82, 158, 234,
                                                   6, 82, 158, 234};
  static short darkenYM0[MODE_TO_DISPLAY_COUNT] = {7, 7, 7, 7, 29, 29, 29, 29};
  static short darkenXM1[MODE_TO_DISPLAY_COUNT] = {
      819, 931, 819, 931,
      819, 931, 819, 931}; //{ 810, 931, 819, 940, 810, 931, 819, 940 };
  static short darkenYM1[MODE_TO_DISPLAY_COUNT] = {
      80,  80,  102, 102,
      124, 124, 146, 146}; //{  10,  10,  35,  35,  60,  60,  85,  85 };
  ...
  help.short_back_buf.hide_area(SCROLL_MENU_X1, SCROLL_MENU_Y1, SCROLL_MENU_X2,
                                SCROLL_MENU_Y2);
  ...
  //	if (current_display_mode.mode_id == MODE_ID_800x600x16)
  {
    for (int i = 0; i < 8; i++) {
      ...
      //			font_bld.center_put(darkenX[i], darkenY[i]-2,
      //darkenX[i] + darkenWidth -1, 									 darkenY[i] + darkenHeight -3,
      //text_reports.str_report_mode(i+1)); // scroll_name[i]);
      // change in GERMAN version
      font_bld.center_put(
          darkenX[i], darkenY[i], darkenX[i] + darkenWidth - 1,
          darkenY[i] + darkenHeight - 3,
          text_reports.str_report_mode(i + 1)); // scroll_name[i]);
    }
  }
  /*	else if (current_display_mode.mode_id == MODE_ID_1024x768x16)
          {
                  ...
                  image_button.put_back( SCROLL_MENU_X1, SCROLL_MENU_Y1,
     scrollName);
          }
          else
                  err_here();
  */
  ...
  if (view_mode >= MIN_MODE_TO_DISPLAY && view_mode <= MAX_MODE_TO_DISPLAY) {
    //		if (current_display_mode.mode_id == MODE_ID_800x600x16)
    {
      image_button.put_back(darkenX[view_mode - 1], darkenY[view_mode - 1],
                            "SR800-DW");
      font_bld.center_put(darkenX[view_mode - 1] + 1,
                          darkenY[view_mode - 1] + 1,
                          darkenX[view_mode - 1] + darkenWidth,
                          darkenY[view_mode - 1] + darkenHeight - 2,
                          text_reports.str_report_mode(
                              view_mode)); // scroll_name[view_mode-1]);
    }
    /*		else if (current_display_mode.mode_id == MODE_ID_1024x768x16)
                    {
                            // find the size of that scroll
                            scrollName[7] = '0' + view_mode;
                            image_button.put_back( SCROLL_MENU_X1,
       SCROLL_MENU_Y1, scrollName);
                    }
                    else
                            err_here();*/
  }
  ...
  help.short_front_buf.hide_area(SCROLL_MENU_X1, SCROLL_MENU_Y1, SCROLL_MENU_X2,
                                 SCROLL_MENU_Y2);

  vga_util.blt_buf(SCROLL_MENU_X1, SCROLL_MENU_Y1, SCROLL_MENU_X2,
                   SCROLL_MENU_Y2, 0);
  ...
  sys.blt_virtual_buf_area(SCROLL_MENU_X1, SCROLL_MENU_Y1, SCROLL_MENU_X2,
                           SCROLL_MENU_Y2);
#endif
}
  ...
  if (view_mode == MODE_NORMAL) {
    ...
    if (!report_disp_frame_no) {
      ...
      report_disp_frame_no =
          1; // report_disp_frame_no = 1 means report has not been drawn
    } else {
      vga.use_back();
      Vga::opaque_flag = config.opaque_report;
...
void Sys::detect_view() {
  ...
  enableAction =
      ...
      !remote.is_enable(); // AMPLUS allows action when paused in single player
  ...
  if (view_mode == MODE_NORMAL &&
      game.game_mode ==
          GAME_TUTORIAL) // tutorial text is only displayed in non-report mode
  {
    if (tutor.detect())
      return;
  }
  ...
  if (!(mouse.event_skey_state & SHIFT_KEY_MASK)) {
    //		if( mouse.press_area( ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2,
    //RIGHT_BUTTON) )
    if (mouse.any_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, RIGHT_BUTTON)) {
      //			if( power.detect_select(mouse.cur_x,
      //mouse.cur_y, mouse.cur_x, mouse.cur_y, 1, 0) )		// 1 - recall
      //group
      if (power.detect_select(
              mouse.click_x(RIGHT_BUTTON), mouse.click_y(RIGHT_BUTTON),
              mouse.click_x(RIGHT_BUTTON), mouse.click_y(RIGHT_BUTTON), 1,
              0)) // 1 - recall group
        return;
    }
  }
...
// [int] viewingNationRecno - which nation the player is viewing at with the
// reports.
//										(default: the
//player nation)
// [int] viewingSpyRecno 	 - >0 if the spy is viewing secret reports of
// other nations
//
void Sys::set_view_mode(int viewMode, int viewingNationRecno,
                        int viewingSpyRecno) {
  ...
  if (nation_array.is_deleted(info.default_viewing_nation_recno)) {
    if (viewMode != MODE_NORMAL &&
        viewMode != MODE_RANK) // other reports are not available except the
                               // normal and rank report
      return;
  }
  ...
  if (info.viewing_spy_recno) {
    if (!spy_array.is_deleted(info.viewing_spy_recno)) {
      ...
      int needViewSecretSkill =
          spy_array.needed_view_secret_skill(info.viewing_spy_recno);
      int escapeChance = spyPtr->spy_skill - needViewSecretSkill;
      int killFlag = 0;

      if (escapeChance > 0) {
        if (misc2.random(escapeChance / 15) ==
            0) // use misc2 instead of misc to maintain mulitplayer sync
          killFlag = 1;
      }
