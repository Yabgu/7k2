...
template <typename M>
static typename M::key_type max_key(const M *map, const typename M::key_type &dflt = typename M::key_type())
{
...
int OpenALAudio::init_wav()
{
    ...
    alcGetIntegerv(this->al_device, ALC_ALL_ATTRIBUTES, attributes.size(), &attributes[0]);
...
int OpenALAudio::play_resided_wav(char *buf, const DsVolume &vol)
{
    ...
    size = static_cast<uint8_t>(buf[4]) | (static_cast<uint8_t>(buf[5]) << 8) | (static_cast<uint8_t>(buf[6]) << 16) |
...
//         0 - not played
//
int OpenALAudio::play_loop_wav(const char *file_name, int repeat_offset, const DsVolume &vol)
{
    int id;
    StreamContext *sc;
    ...
    MSG("play_loop_wav(\"%s\", %i, (%li, %li)\n", file_name, repeat_offset, vol.ds_vol, vol.ds_pan);
...
bool OpenALAudio::StreamContext::stream_data(int new_buffer_count)
{
    ...
    for (;;)
    {
        ...
        alBufferData(buf, format, data, frames_read * this->stream->frame_size(), this->stream->frame_rate());
        if (!check_al())
            goto err;
