...
static void set_source_panning(ALuint source, int panning) {
  ...
  alSource3f(source, AL_POSITION, PANNING_MAX_X * panning / 10000.f, 0,
             PANNING_Z);
}
...
static void set_source_volume(ALuint source, int volume_millibels) {
  ...
  MSG("set_source_volume(%x, %f)\n", source,
      millibels_to_ratio(volume_millibels));
...
/* returns an unused key that's greater than 0 */
template <typename M> static typename M::key_type unused_key(const M *map) {
...
OpenALAudio::~OpenALAudio() { this->deinit(); }
...
int OpenALAudio::init() {
  ...
  this->init_flag =
      this->wav_init_flag || this->mid_init_flag || this->cd_init_flag;
...
int OpenALAudio::init_wav() {
  ...
  alcGetIntegerv(this->al_device, ALC_ALL_ATTRIBUTES, attributes.size(),
                 &attributes[0]);
...
void OpenALAudio::stop_mid() { WARN_UNIMPLEMENTED("stop_mid"); }

// Play digitized wav from the wav resource file
//
// <char*>        file_name = name of the wav in the resource file
//
// return : <int> non-zero - wav loaded and is playing, return a serial no. to
// be referred in stop_wav and is_wav_playing
//                0 - wav not played
//
...
int OpenALAudio::play_resided_wav(char *buf, const DsVolume &vol) {
  ...
  size = static_cast<uint8_t>(buf[4]) | (static_cast<uint8_t>(buf[5]) << 8) |
...
int OpenALAudio::get_free_wav_ch() {
  ...
  free_count = this->max_sources - static_cast<int>(this->streams.size());
...
//
int OpenALAudio::stop_wav(int id) { return this->stop_long_wav(id); }
...
int OpenALAudio::play_long_wav(const char *file_name, const DsVolume &vol) {
  ...
  MSG("play_long_wav(\"%s\", (%li, %li))\n", file_name, vol.ds_vol, vol.ds_pan);
...
void OpenALAudio::fade_out_loop_wav(int id, int fade_duration_msec) {
  ...
  sc->fade_frames = sc->stream->frame_rate() * fade_duration_msec / 1000;
}
...
void OpenALAudio::stop_cd() { WARN_UNIMPLEMENTED("stop_cd"); }
...
void OpenALAudio::toggle_mid(int midFlag) { WARN_UNIMPLEMENTED("toggle_mid"); }
...
void OpenALAudio::toggle_cd(int cdFlag) { WARN_UNIMPLEMENTED("toggle_cd"); }
...
bool OpenALAudio::StreamContext::stream_data(int new_buffer_count) {
  ...
  for (;;) {
    ...
    alBufferData(buf, format, data, frames_read * this->stream->frame_size(),
