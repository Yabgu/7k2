...
enum { PATROL_ALL = 0, PATROL_NO_GENERAL = 1, PATROL_NO_INJURED_SOILDER = 2 };
...
static char
    disp_combat_or_skill; // 0=normal, 1=combat, 2=skill, 3=spying, 4=loyalty
...
static short edit_spy_x1, edit_spy_y1, edit_spy_x2, edit_spy_y2,
    edit_spy_enable;
static short clear_spy_x1, clear_spy_y1, clear_spy_x2, clear_spy_y2,
    clear_spy_enable;
...
static void disp_soldier_hit_points(int x1, int y1, int x2, int hitPoints,
                                    int maxHitPoints);
static void disp_training_bar(int x1, int y1, int x2, int hitPoints,
                              int maxHitPoints);
...
void FirmCamp::put_info(int refreshFlag) {
  ...
  // ##### end Gilbert 21/9 ######//
    ...
    if (last_menu_mode !=
        firm_menu_mode) // if changing menu mode pass repaint to sub-menu
    {
...
void FirmCamp::disp_camp_info(int dispY1, int refreshFlag) {
  if (is_own()) {
    ...
    if (refreshFlag == INFO_REPAINT) {
      ...
      // button_reward.create( INFO_X1+13+BUTTON_DISTANCE, INFO_Y1+235, 'A',
      // "REWARDCB" ); button_defense.create( INFO_X1+13+2*BUTTON_DISTANCE,
      // INFO_Y1+235, 'A', defense_flag ? "DEFENSE1" : "DEFENSE0" );

      if (!is_monster())
        button_patrol.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 281, 'A',
                             "PATROL");
      else
        button_patrol.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 281, 'A',
                             "F_PATROL");
      ...
      if (!is_monster())
        button_defense.create(
            INFO_X1 + 13 + 2 * BUTTON_DISTANCE, INFO_Y1 + 281, 'A',
            defense_flag ? (char *)"DEFENSE1" : (char *)"DEFENSE0");
      else
        button_defense.create(
            INFO_X1 + 13 + 2 * BUTTON_DISTANCE, INFO_Y1 + 281, 'A',
            defense_flag ? (char *)"F_DEFEN1" : (char *)"F_DEFEN0");

      if (!is_monster())
        button_promote.create(INFO_X1 + 13 + 2 * BUTTON_DISTANCE, INFO_Y1 + 235,
                              'A', "PROMOTE");
      else
        button_promote.create(INFO_X1 + 13 + 2 * BUTTON_DISTANCE, INFO_Y1 + 235,
                              'A', "F_PROMOT");
    ...
    if (overseer_recno) {
      ...
      int i;
      for (i = 0; i < soldier_count && soldier_array[i].is_under_training();
           ++i)
        ;
    ...
    button_patrol.paint();
    //	vga.active_buf->bar_alpha( button_patrol.x1, button_patrol.y1+15,
    //button_patrol.x1+BUTTON_ACTION_WIDTH-1,
    //button_patrol.y1+BUTTON_ACTION_HEIGHT-16, 1, 0 );
    font_whbl.center_put_paragraph(button_patrol.x1, button_patrol.y1,
                                   button_patrol.x1 + BUTTON_ACTION_WIDTH - 1,
                                   button_patrol.y1 + BUTTON_ACTION_HEIGHT - 1,
                                   str);

    // ###### begin Gilbert 15/4 ########//
    if (nation_array[nation_recno]->cash >= REWARD_COST &&
        ((overseer_recno && unit_array[overseer_recno]->rank_id != RANK_KING &&
          unit_res[unit_array[overseer_recno]->unit_id]
              ->class_info.has_loyalty) ||
         (selected_soldier_id && selected_soldier_id <= soldier_count &&
          unit_res[soldier_array[selected_soldier_id - 1].unit_id]
              ->class_info.has_loyalty)))
    ...
    if (!is_monster())
      button_defense.update_bitmap(
          defense_flag ? (char *)"DEFENSE1" : (char *)"DEFENSE0"); // call paint
    else
      button_defense.update_bitmap(defense_flag ? (char *)"F_DEFEN1"
                                                : (char *)"F_DEFEN0");
    ...
    if ((button_promote.visible_flag = !overseer_recno)) {
      ...
      if (selected_soldier_id > 0 && selected_soldier_id <= soldier_count) {
        ...
        if (soldierPtr->race_id != 0 && soldierPtr->rank_id == RANK_SOLDIER &&
            !soldierPtr->is_under_training() && soldierPtr->skill_level() > 0)
        // ##### end Gilbert 24/3 ######//
        {
  ...
  if (is_own()) {
    ...
    if ((rc = button_patrol.detect(0, 0, 1))) {
      if (rc == 2) {
        ...
        short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_CAMP_PATROL,
                                                             2 * sizeof(short));
    ...
    if (button_defense.detect()) {
      se_ctrl.immediate_sound(!defense_flag ? (char *)"TURN_ON"
                                            : (char *)"TURN_OFF");
      ...
      if (!remote.is_enable()) {
        ...
        short *shortPtr = (short *)remote.new_send_queue_msg(
            MSG_F_CAMP_TOGGLE_PATROL, 2 * sizeof(short));
        shortPtr[0] = firm_recno;
        shortPtr[1] = !defense_flag;
      }

      //			button_defense.update_bitmap( defense_flag ?
      //"DEFENSE1" : "DEFENSE0" );
    }
...
void FirmCamp::disp_soldier_list(int dispY1, int refreshFlag, int dispSpyMenu) {
  ...
  for (int inc = -1; inc <= 1; inc += 2) {
    ...
    for (int i = inc >= 0 ? 0 : soldier_count; i >= 0 && i <= soldier_count;
         i += inc) {
      // display soldier i
      ...
      if (i == 0) {
        ...
        // ####### begin Gilbert 24/3 #########//
        if (unit_res[soldierPtr->unit_id]->class_info.has_loyalty &&
            nation_recno)
          // ####### end Gilbert 24/3 #########//
      ...
      if (inc < 0) {
        ...
        // Soldier *soldierPtr = &soldier_array[i-1];
        info.draw_unit_icon(
            x + SOLDIER_X_SPACING / 2, y, unitId, nation_recno, windowX1,
            windowY1, windowX2, windowY2,
            (combatLevel < BASIC_COMBAT_TRAIN)
                ? (((BASIC_COMBAT_TRAIN - combatLevel) << 6) + 33)
                : 1);
      } else {
        // second round is ascending to draw the frame

        if (info.draw_unit_icon(
                x + SOLDIER_X_SPACING / 2, y, unitId, nation_recno, windowX1,
                windowY1, windowX2, windowY2,
                inAreaFlag | (i == selected_soldier_id ? 3 : 0)) &
            4) {
        ...
        if (!dispSpyMenu) {
          if (disp_combat_or_skill) // display skill level
          {
            ...
            if (attribute >= 0) // hide attribute on some cases
              font->center_put(x, yHp, x + SOLDIER_X_SPACING,
                               yHp + font->max_font_height,
                               misc.format(attribute));
          } else if (ownSpy) // display spy icon
          {
            vga.active_buf->put_bitmap_trans(x + SOLDIER_X_SPACING / 2 - 8,
                                             yHp - 5, image_icon.read("U_SPY"));
          } else if (unitInfo->class_info
                         .has_combat_level) // display combat skill
          {
            font->center_put(x, yHp, x + SOLDIER_X_SPACING,
                             yHp + font->max_font_height,
          ...
          // display spy skill
          err_when(!ownSpy);
          font_whbl.center_put(x, yHp, x + SOLDIER_X_SPACING,
                               yHp + font->max_font_height,
        ...
        if (i > 0 && soldier_array[i - 1].is_under_training())
          disp_training_bar(x, yHp + font->max_font_height + 2,
                            x + SOLDIER_X_SPACING * 7 / 8 - 1,
                            soldier_array[i - 1].skill_level(),
                            BASIC_COMBAT_TRAIN);
        else
          disp_soldier_hit_points(x, yHp + font->max_font_height + 2,
                                  x + SOLDIER_X_SPACING * 7 / 8 - 1, hp, maxHp);
      }
    }
  }
  ...
  /*
          ...
          if( overseer_recno )
          {
                  ...
                  {
                          vga_front.rect( x-2, y-2, x+UNIT_LARGE_ICON_WIDTH+1,
  y+UNIT_LARGE_ICON_HEIGHT+1, 2, V_YELLOW );
                  }
                  else
                  {
                          vga_util.blt_buf( x-2, y-2, x+UNIT_LARGE_ICON_WIDTH+1,
  y-1, 0 ); vga_util.blt_buf( x-2, y+UNIT_LARGE_ICON_HEIGHT+1,
  x+UNIT_LARGE_ICON_WIDTH+1, y+UNIT_LARGE_ICON_HEIGHT+2, 0 ); vga_util.blt_buf(
  x-2, y-2, x-1, y+UNIT_LARGE_ICON_HEIGHT+2, 0 ); vga_util.blt_buf(
  x+UNIT_LARGE_ICON_WIDTH, y-2, x+UNIT_LARGE_ICON_WIDTH+1,
  y+UNIT_LARGE_ICON_HEIGHT+2, 0 );
                  }
                  ...
                  if( refreshFlag == INFO_REPAINT )
                  {
                          vga_front.put_bitmap(x, y,
  unit_res[overseerUnit->unit_id]->get_large_icon_ptr(overseerUnit->rank_id) );
                  }

                  //-------- set help parameters --------//

                  if( mouse.in_area(x, y, x+UNIT_LARGE_ICON_WIDTH+3,
  y+UNIT_LARGE_ICON_HEIGHT+3) ) help.set_unit_help( overseerUnit->unit_id,
  overseerUnit->rank_id, x, y, x+UNIT_LARGE_ICON_WIDTH+3,
  y+UNIT_LARGE_ICON_HEIGHT+3);
                  ...
                  if( refreshFlag == INFO_REPAINT )
                          font_san.put( x1, y, overseerUnit->unit_name(0), 0,
  INFO_X2-2 );		// 0-ask unit_name() not to return the title of the unit
                  ...
                  if( overseerUnit->rank_id != RANK_KING )
                  {
                          ...
                          int x2 = info.disp_loyalty( x1, y-1, x1,
  overseerUnit->loyalty, overseerUnit->target_loyalty, nation_recno, refreshFlag
  );
                          ...
                          if( overseerUnit->spy_recno )
                          {
                                  ...
                                  {
                                          vga_front.put_bitmap( x2+5, y+1,
  image_icon.get_ptr("U_SPY") ); x2 += 15;
                                  }
                          }
  ...
  || last_unit_id_array[i] != soldierPtr->unit_id )
                          {
                                  vga_front.put_bitmap(x+2, y+2,
  soldierPtr->small_icon_ptr());
                          }
  ...
  ); else vga_front.rect( x, y, x+27, y+23, 2, vga_front.color_up );

                          //------ display hit points bar --------//

                          disp_soldier_hit_points( x+2, y+24, x+25,
  soldierPtr->shown_hit_points(), soldierPtr->max_hit_points() );
                          ...
                          {
                                  Spy* spyPtr =
  spy_array[soldierPtr->spy_recno];

                                  //------ if this is the player's spy -------//

                                  if( nation_array.player_recno &&
                                           spyPtr->true_nation_recno ==
  nation_array.player_recno )
                                  ...
                                  //--------------------------------------------//
                                  //
                                  // If this is an enemy spy and this firm
  belongs
                                  // to the player and there is a player's
  phoenix
                                  // over this firm and the spying skill of the
  spy
                                  // is low (below 40)
                                  //
                                  //--------------------------------------------//
  ...
  //							nation_array.player_recno
  &&
  //					 (~nation_array)->revealed_by_phoenix(loc_x1,
  loc_y1) )
  //				{
  ...
  //				}
                          ...
                          if( spyIconName )
                          {
                                  vga_front.put_bitmap( x+30, y+6,
  image_icon.get_ptr(spyIconName) ); vga_util.blt_buf( x+40, y+6, x+49, y+15, 0
  ); vga_util.blt_buf( x+30, y+16, x+49, y+26, 0 );
                          }
                          else
                          {
                                  font_san.disp(x+30, y+6,
  soldierPtr->skill.combat_level, 1, x+49);
                          }
                          ...
                          if( mouse.in_area(x, y, x+27, y+23) )
                                  help.set_unit_help( soldierPtr->unit_id, 0, x,
  y, x+27, y+23 );
                  }
                  else
                  {
                          if( last_race_id_array[i] != 0 ||
...
// when selectSpyMenu is 0, return 1 if left click on a unit, return 2 if right
// click on a unit when selectSpyMenu is 1, return spy_recno of the clicked spy,
// 0 if no own spy is clicked
//
int FirmCamp::detect_soldier_list(int selectSpyMenu) {
  ...
  for (int i = 0; i <= soldier_count; ++i) {
    ...
    int rc = info.draw_unit_icon(
        x + SOLDIER_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1,
        windowX2, windowY2, 24); // detect left button (8) and right button(16)
      ...
      if (rc & 8) {
        ...
        if (i == 0) {
          if (remote.is_enable()) {
            ...
            short *shortPtr = (short *)remote.new_send_queue_msg(
                MSG_FIRM_MOBL_OVERSEER, sizeof(short));
            shortPtr[0] = firm_recno;
          } else {
  ...
  if (is_own() && selectSpyMenu == 0 && swap_item_src >= 0 &&
      mouse.release_click()) {
    if (pointed_soldier_id >= 0) {
      swap_item(swap_item_src, pointed_soldier_id, swap_item_id,
                COMMAND_PLAYER);
    } else {
      swap_item_src = -1; // reset from selecting swap target
    }
  ...
  if( is_own() )
  {
          ...
          int rc = mouse.single_click(INFO_X1+6, INFO_Y1+58,
                                  INFO_X1+5+UNIT_LARGE_ICON_WIDTH,
  INFO_Y1+57+UNIT_LARGE_ICON_HEIGHT, 2 );
          ...
          else if( rc==2 && is_own() )
          {
                  ...
                  {
                          // packet structure : <firm recno>
                          short *shortPtr=(short
  *)remote.new_send_queue_msg(MSG_FIRM_MOBL_OVERSEER, sizeof(short));
                          shortPtr[0] = firm_recno;
                  }
                  else
                  {
                          assign_overseer(0);		// the overseer quits
  the camp
                  }
                  return 1;
          }
  ...
  for( i=0 ; i<soldier_count ; i++ )
  {
          ...
          case 2:
                  ...
                  {
                          //--- if the town where the unit lives belongs to the
  nation of this firm ---//
...
#define HIT_BAR_TYPE_COUNT 3
  ...
  enum { HIT_BAR_DARK_BORDER = 3, HIT_BAR_BODY = 1 };

  int barWidth = (x2 - x1 + 1) * hitPoints / MAX(hitPoints, maxHitPoints);

  vga.active_buf->bar(x1, y1, x1 + barWidth - 1, y1 + 1,
                      hitBarColor + HIT_BAR_BODY);
  ...
  vga.active_buf->bar(x1, y1, x1 + barWidth - 1, y1,
                      hitBarColor + HIT_BAR_DARK_BORDER);
...
void FirmCamp::disp_soldier_info(int dispY1, int refreshFlag) {
  ...
  if (selected_soldier_id > 0 && selected_soldier_id <= soldier_count) {
    ...
    if (soldierPtr->race_id) {
      if (soldierPtr->is_human()) {
        if (soldierPtr->hero_id)
          ...
          str = race_res[soldierPtr->race_id]->get_name(
              soldierPtr->name_id); // unit name
        str += " (";
    ...
    if (unitInfo->class_info.has_loyalty && nation_recno) {
      if (soldierPtr->loyalty != soldierPtr->target_loyalty(firm_recno))
        info.disp_loyalty(
            x, y + 12,
            INFO_X2 - 99 -
                font_snds.text_width(misc.format(soldierPtr->loyalty, 4)) -
                font_snds.text_width(
                    misc.format(soldierPtr->target_loyalty(firm_recno), 4)) -
                font_snds.text_width("11"),
            soldierPtr->loyalty, soldierPtr->target_loyalty(firm_recno),
            nation_recno, refreshFlag, disp_combat_or_skill == 4);
      else
        info.disp_loyalty(
            x, y + 12,
            INFO_X2 - 99 -
    ...
    if (unitInfo->class_info.has_combat_level) {
      // x2 = (disp_combat_or_skill==1?font_blu2:font_snds).put( x+110, y+12,
      // "Combat" ) + 10;
      x2 = (disp_combat_or_skill == 1 ? font_blu2 : font_snds)
               .put(x + 110, y + 12, text_unit.str_combat_level()) +
           10;
      font_snds.right_put(INFO_X2 - 10, y + 12,
                          misc.format(soldierPtr->combat_level(), 4));
    }
    ...
    if (unitInfo->class_info.has_skill_level) {
      //			x2 =
      //(disp_combat_or_skill==2?font_blu2:font_snds).put( x+110, y+26,
      //"Leadership" ) + 10;
      x2 = (disp_combat_or_skill == 2 ? font_blu2 : font_snds)
               .put(x + 110, y + 26, text_unit.str_leadership()) +
           10;
      font_snds.right_put(INFO_X2 - 10, y + 26,
                          misc.format(soldierPtr->skill_level(), 4));
    }
...
void FirmCamp::disp_overseer_info(int dispY1, int refreshFlag) {
  ...
  if (unitInfo->class_info.has_loyalty && unitPtr->rank_id != RANK_KING &&
    ...
    if (unitPtr->loyalty != unitPtr->target_loyalty)
      info.disp_loyalty(
          x, y + 12,
          INFO_X2 - 99 -
              font_snds.text_width(misc.format(unitPtr->loyalty, 4)) -
              font_snds.text_width(misc.format(unitPtr->target_loyalty, 4)) -
              font_snds.text_width("11"),
          unitPtr->loyalty, unitPtr->target_loyalty, nation_recno, refreshFlag,
          disp_combat_or_skill == 4);
    else
      info.disp_loyalty(
          x, y + 12,
          INFO_X2 - 99 - font_snds.text_width(misc.format(unitPtr->loyalty, 4)),
          unitPtr->loyalty, unitPtr->target_loyalty, nation_recno, refreshFlag,
          disp_combat_or_skill == 4);
  }
  ...
  if (unitInfo->class_info.has_combat_level) {
    ...
    x2 = (disp_combat_or_skill == 1 ? font_blu2 : font_snds)
         ...
         10;
    font_snds.right_put(INFO_X2 - 10, y + 12,
                        misc.format(unitPtr->combat_level(), 4));
    //		x2 = (disp_combat_or_skill==1?font_blu2:font_snds).put( x+110, y+15,
    //text_unit.str_combat_level() ) + 10; 		font_snds.right_put( INFO_X2-10,
    //y+15, misc.format(unitPtr->combat_level(),4) ); SXM
  }

  // ------- display leadership -------//

  if (unitInfo->class_info.has_skill_level && unitPtr->skill_level() > 0) {
    // x2 = (disp_combat_or_skill==2?font_blu2:font_snds).put( x+110, y+26,
    // "Leadership" ) + 10;
    // SXM
    x2 = (disp_combat_or_skill == 2 ? font_blu2 : font_snds)
             .put(x + 110, y + 26, text_unit.str_leadership()) +
         10;
    font_snds.right_put(INFO_X2 - 10, y + 26,
                        misc.format(unitPtr->skill_level(), 4));
    //		x2 = (disp_combat_or_skill==2?font_blu2:font_snds).put( x+110, y+29,
    //text_unit.str_leadership() ) + 10; 		font_snds.right_put( INFO_X2-10, y+29,
    //misc.format(unitPtr->skill_level(),4) ); SXM
  }
...
//
void FirmCamp::disp_train_menu(int refreshFlag) { err_here(); }
//--------- End of function FirmCamp::disp_train_menu ---------//

//--------- Begin of function FirmCamp::detect_train_menu ---------//
//
void FirmCamp::detect_train_menu() { err_here(); }
...
static void disp_debug_info(FirmCamp *firmPtr, int refreshFlag) {
  ...
  font_san.field(x, y, "patrol unit count", x2, firmPtr->patrol_unit_count, 1,
                 INFO_X2, refreshFlag);
  font_san.field(x, y + 16, "coming unit count", x2, firmPtr->coming_unit_count,
                 1, INFO_X2, refreshFlag);
...
int FirmCamp::explore_for_player() {
  return Firm::explore_for_player() || player_spy_count > 0;
}
...
void FirmCamp::disp_edit_mode(int &refreshFlag, int &y, int dispBG) {
  ...
  if (selected_soldier_id > 0 && selected_soldier_id <= soldier_count) {
    ...
    x = edit_hp_x2 = font_blue.put((edit_hp_x1 = x + 20), (edit_hp_y1 = y),
                                   (int)soldierPtr->hit_points, 0, x2);
    ...
    if (unitInfo->class_info.has_loyalty) // && rank_id != RANK_KING )
    {
      ...
      x = edit_loyalty_x2 =
          font_blue.put((edit_loyalty_x1 = x + 20), (edit_loyalty_y1 = y),
                        soldierPtr->loyalty, 0, x2);
    ...
    if (unitInfo->class_info.has_combat_level) {
      ...
      x = edit_combat_x2 =
          font_blue.put((edit_combat_x1 = x + 20), (edit_combat_y1 = y),
                        soldierPtr->skill.actual_combat_level(NULL), 0, x2);
      edit_combat_y2 = edit_combat_y1 + font_blue.max_font_height;
      edit_combat_enable = 1;
      if (soldierPtr->combat_level() !=
          soldierPtr->skill.actual_combat_level(NULL)) {
        x = font_whbl.put(x, y, "(", 0, x2);
      ...
      if (sys.debug_session) // maximum combat level is a higher secret
      {
        ...
        x = edit_max_combat_x2 =
            font_blue.put((edit_max_combat_x1 = x), (edit_max_combat_y1 = y),
                          soldierPtr->skill.max_combat_level, 0, x2);
    ...
    if (unitInfo->class_info.has_skill_level) {
      x = font_whbl.put(x1, y, text_unit.str_leadership(), 0,
                        x2); // "Leadership"
      x = edit_skill_x2 =
          font_blue.put((edit_skill_x1 = x + 20), (edit_skill_y1 = y),
                        soldierPtr->skill.actual_skill_level(NULL), 0, x2);
      edit_skill_y2 = edit_skill_y1 + font_blue.max_font_height - 1;
      edit_skill_enable = 1;
      if (soldierPtr->skill_level() !=
          soldierPtr->skill.actual_skill_level(NULL)) {
        x = font_whbl.put(x, y, "(", 0, x2);
      ...
      if (sys.debug_session) // maximum skill level is a higher secret
      {
        ...
        x = edit_max_skill_x2 =
            font_blue.put((edit_max_skill_x1 = x), (edit_max_skill_y1 = y),
                          soldierPtr->skill.max_skill_level, 0, x2);
    ...
    if (soldierPtr->spy_recno) {
      ...
      if (!nation_array.is_deleted(trueNationRecno)) {
        ...
        clear_spy_enable = 1;
        x = clear_spy_x2 = font_whbl.put((clear_spy_x1 = x), (clear_spy_y1 = y),
                                         trueNationChar, 0, x2);
        clear_spy_y2 = clear_spy_y1 + NATION_COLOR_BAR_HEIGHT - 1;
        x += 10;
      }
      x = edit_spy_x2 =
          font_blue.put(edit_spy_x1 = x, edit_spy_y1 = y,
                        spy_array[soldierPtr->spy_recno]->spy_skill, 0, x2);
      ...
      for (char n = 1; n <= nation_array.size(); ++n) {
        if (!nation_array.is_deleted(n)) {
          add_spy_str[nationCount] = FIRST_NATION_COLOR_CODE_IN_TEXT +
                                     nation_array[n]->color_scheme_id;
      ...
      x = add_spy_x2 =
          font_whbl.put((add_spy_x1 = x), (add_spy_y1 = y), add_spy_str, 0, x2);
      add_spy_y2 = add_spy_y1 + NATION_COLOR_BAR_HEIGHT - 1;
    }
    y += 15;
  } else if (selected_soldier_id == 0 &&
             !unit_array.is_truly_deleted(overseer_recno)) {
    ...
    x = font_whbl.put(x1, y, text_unit.str_hit_points(), 0, x2); // "Hit Points"
    x = edit_hp_x2 = font_blue.put((edit_hp_x1 = x + 20), (edit_hp_y1 = y),
                                   (int)overseerPtr->hit_points, 0, x2);
    edit_hp_y2 = edit_hp_y1 + font_blue.max_font_height;
    edit_hp_enable = 1;
    x = font_whbl.put(x, y, " / ", 0, x2);
    ...
    if (unitInfo->class_info.has_combat_level) {
      ...
      x = edit_combat_x2 =
          font_blue.put((edit_combat_x1 = x + 20), (edit_combat_y1 = y),
                        overseerPtr->skill.actual_combat_level(NULL), 0, x2);
      edit_combat_y2 = edit_combat_y1 + font_blue.max_font_height;
      edit_combat_enable = 1;
      if (overseerPtr->combat_level() !=
          overseerPtr->skill.actual_combat_level(NULL)) {
        x = font_whbl.put(x, y, "(", 0, x2);
      ...
      if (sys.debug_session) // maximum combat level is a higher secret
      {
        ...
        x = edit_max_combat_x2 =
            font_blue.put((edit_max_combat_x1 = x), (edit_max_combat_y1 = y),
                          overseerPtr->skill.max_combat_level, 0, x2);
        edit_max_combat_y2 = edit_max_combat_y1 + font_blue.max_font_height;
        edit_max_combat_enable = edit_combat_enable;
      }
    }
    ...
    if (unitInfo->class_info.has_skill_level) {
      ...
      x = edit_skill_x2 =
          font_blue.put((edit_skill_x1 = x + 20), (edit_skill_y1 = y),
                        overseerPtr->skill.actual_skill_level(NULL), 0, x2);
      edit_skill_y2 = edit_skill_y1 + font_blue.max_font_height - 1;
      edit_skill_enable = 1;
      if (overseerPtr->skill_level() !=
          overseerPtr->skill.actual_skill_level(NULL)) {
        x = font_whbl.put(x, y, "(", 0, x2);
      ...
      if (sys.debug_session) // maximum skill level is a higher secret
      {
        ...
        x = edit_max_skill_x2 =
            font_blue.put((edit_max_skill_x1 = x), (edit_max_skill_y1 = y),
                          overseerPtr->skill.max_skill_level, 0, x2);
        edit_max_skill_y2 = edit_max_skill_y1 + font_blue.max_font_height;
        edit_max_skill_enable = edit_skill_enable;
      }
    }
    ...
    if (overseerPtr->spy_recno) {
      ...
      // put true nation recno
      int trueNationRecno =
          spy_array[overseerPtr->spy_recno]->true_nation_recno;
      if (!nation_array.is_deleted(trueNationRecno)) {
        char trueNationChar[] = "1";
        trueNationChar[0] = FIRST_NATION_COLOR_CODE_IN_TEXT +
                            nation_array[trueNationRecno]->color_scheme_id;
      ...
      x = edit_spy_x2 =
          font_blue.put(edit_spy_x1 = x, edit_spy_y1 = y,
                        spy_array[overseerPtr->spy_recno]->spy_skill, 0, x2);
      edit_spy_y2 = edit_spy_y1 + font_blue.max_font_height;
      edit_spy_enable = 1;
  ...
  if (selected_soldier_id > 0 && selected_soldier_id <= soldier_count) {
    ...
    if (edit_combat_enable) {
      ...
      if (mouse.any_click(edit_combat_x1, edit_combat_y1, edit_combat_x2,
          ...
          !soldierPtr
               ->is_under_training()) // not allow decrease when basic training
      {
    ...
    if (edit_max_combat_enable) {
      if (mouse.any_click(edit_max_combat_x1, edit_max_combat_y1,
                          edit_max_combat_x2, edit_max_combat_y2,
                          LEFT_BUTTON)) {
    ...
    if (edit_max_skill_enable) {
      if (mouse.any_click(edit_max_skill_x1, edit_max_skill_y1,
                          edit_max_skill_x2, edit_max_skill_y2, LEFT_BUTTON)) {
      ...
      if (mouse.any_click(edit_max_skill_x1, edit_max_skill_y1,
                          edit_max_skill_x2, edit_max_skill_y2, RIGHT_BUTTON)) {
        int newLevel = soldierPtr->skill.max_skill_level;
        newLevel -= 20;
    ...
    if (clear_spy_enable && soldierPtr->spy_recno) {
      if (mouse.single_click(clear_spy_x1, clear_spy_y1, clear_spy_x2,
                             clear_spy_y2, RIGHT_BUTTON)) {
    ...
    if (add_spy_nation[0] &&
        !soldierPtr->spy_recno) // each character represents one nation
    {
      int interval = (add_spy_x2 - add_spy_x1 + 1) / strlen(add_spy_str);
      int x1 = add_spy_x1;
      int x2 = add_spy_x1 + interval - 1;
      for (char n = 0; add_spy_nation[n];
           ++n, (x1 += interval), (x2 += interval)) {
        if (!nation_array.is_deleted(add_spy_nation[n]) &&
          ...
          newSpy->cloaked_nation_recno =
              nation_recno; // current nation of the firm
          newSpy->race_id = soldierPtr->race_id;
          newSpy->name_id = soldierPtr->name_id;

          err_when(newSpy->race_id < 1 || newSpy->race_id > MAX_RACE);

          //-- Spy always registers its name twice as his name will be freed up
          //in deinit(). Keep an additional right because when a spy is assigned
          //to a town, the normal program will free up the name id., so we have
          //to keep an additional copy
          if (!newSpy->name_id) // if this soldier does not have a name, give
                                // him one now as a spy must reserve a name (see
                                // below on use_name_id() for reasons)
    ...
    if (add_spy_nation[0] &&
        !overseerPtr->spy_recno) // each character represents one nation
    {
      int interval = (add_spy_x2 - add_spy_x1 + 1) / strlen(add_spy_str);
      int x1 = add_spy_x1;
      int x2 = add_spy_x1 + interval - 1;
