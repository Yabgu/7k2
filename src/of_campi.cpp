...
void FirmCamp::disp_camp_info(int dispY1, int refreshFlag)
{
    if (is_own())
    {
        ...
        if (refreshFlag == INFO_REPAINT)
        {
            ...
            if (!is_monster())
                button_defense.create(INFO_X1 + 13 + 2 * BUTTON_DISTANCE, INFO_Y1 + 281, 'A',
        ...
        font_whbl.center_put_paragraph(button_patrol.x1, button_patrol.y1, button_patrol.x1 + BUTTON_ACTION_WIDTH - 1,
                                       button_patrol.y1 + BUTTON_ACTION_HEIGHT - 1, str);

        // ###### begin Gilbert 15/4 ########//
        if (nation_array[nation_recno]->cash >= REWARD_COST &&
            ((overseer_recno && unit_array[overseer_recno]->rank_id != RANK_KING &&
              unit_res[unit_array[overseer_recno]->unit_id]->class_info.has_loyalty) ||
             (selected_soldier_id && selected_soldier_id <= soldier_count &&
              unit_res[soldier_array[selected_soldier_id - 1].unit_id]->class_info.has_loyalty)))
    ...
    for (int inc = -1; inc <= 1; inc += 2)
    {
        ...
        for (int i = inc >= 0 ? 0 : soldier_count; i >= 0 && i <= soldier_count; i += inc)
        {
            ...
            if (inc < 0)
            {
                ...
                // Soldier *soldierPtr = &soldier_array[i-1];
                info.draw_unit_icon(
                    x + SOLDIER_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1, windowX2, windowY2,
            ...
            else
            {
                ...
                if (info.draw_unit_icon(x + SOLDIER_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1,
                                        windowX2, windowY2, inAreaFlag | (i == selected_soldier_id ? 3 : 0)) &
                    4)
                {
                ...
                if (!dispSpyMenu)
                {
                    ...
                    {
                        vga.active_buf->put_bitmap_trans(x + SOLDIER_X_SPACING / 2 - 8, yHp - 5,
                                                         image_icon.read("U_SPY"));
                    }
                    ...
                    if (itemId)
                    {
                        ...
                        if (iconPtr)
                            vga.active_buf->put_bitmap_trans(
                                x + SOLDIER_X_SPACING / 2 - ((Bitmap *)iconPtr)->get_width() / 2, yHp + 53, iconPtr);
                    }
                }
                else
                {
                ...
                if (i > 0 && soldier_array[i - 1].is_under_training())
                    ...
                    disp_soldier_hit_points(x, yHp + font->max_font_height + 2, x + SOLDIER_X_SPACING * 7 / 8 - 1, hp,
                                            maxHp);
            }
        }
    }
...
int FirmCamp::detect_soldier_list(int selectSpyMenu)
{
    ...
    for (int i = 0; i <= soldier_count; ++i)
    {
        ...
        int rc = info.draw_unit_icon(x + SOLDIER_X_SPACING / 2, y, unitId, nation_recno, windowX1, windowY1, windowX2,
                                     windowY2,
                                     24); // detect left button (8) and right button(16)
...
void FirmCamp::disp_soldier_info(int dispY1, int refreshFlag)
{
    ...
    if (selected_soldier_id > 0 && selected_soldier_id <= soldier_count)
    {
        ...
        else
        {
            ...
            if (unit_res[soldierPtr->unit_id]->class_info.has_weapon_version && soldierPtr->get_weapon_version() > 1)
            // ###### end Gilbert 24/3 #######//
            {
                str += " ";
        ...
        if (unitInfo->class_info.has_loyalty && nation_recno)
        {
            if (soldierPtr->loyalty != soldierPtr->target_loyalty(firm_recno))
                info.disp_loyalty(x, y + 12,
                                  INFO_X2 - 99 - font_snds.text_width(misc.format(soldierPtr->loyalty, 4)) -
                                      font_snds.text_width(misc.format(soldierPtr->target_loyalty(firm_recno), 4)) -
                                      font_snds.text_width("11"),
                                  soldierPtr->loyalty, soldierPtr->target_loyalty(firm_recno), nation_recno,
                                  refreshFlag, disp_combat_or_skill == 4);
            else
        ...
        if (unitInfo->class_info.has_combat_level)
        {
            ...
            // "Combat" ) + 10;
            x2 =
                (disp_combat_or_skill == 1 ? font_blu2 : font_snds).put(x + 110, y + 12, text_unit.str_combat_level()) +
                10;
        ...
        if (unitInfo->class_info.has_skill_level)
        {
            ...
            //"Leadership" ) + 10;
            x2 = (disp_combat_or_skill == 2 ? font_blu2 : font_snds).put(x + 110, y + 26, text_unit.str_leadership()) +
                 10;
...
void FirmCamp::detect_soldier_info()
{
    ...
    if (selected_soldier_id > 0 && selected_soldier_id <= soldier_count)
    {
        ...
        if (unitInfo->class_info.has_loyalty && nation_recno && mouse.single_click(x, y + 14, x + 79, y + 27))
        {
            // detect loyalty
            disp_combat_or_skill = 4;
        }
        else if (unitInfo->class_info.has_combat_level && mouse.single_click(x + 110, y + 14, x + 189, y + 27))
        {
            // detect combat level
            disp_combat_or_skill = 1;
        }
        else if (unitInfo->class_info.has_skill_level && soldierPtr->skill_level() > 0 &&
...
void FirmCamp::disp_overseer_info(int dispY1, int refreshFlag)
{
        ...
        if (unitPtr->loyalty != unitPtr->target_loyalty)
            info.disp_loyalty(
                ...
                INFO_X2 - 99 - font_snds.text_width(misc.format(unitPtr->loyalty, 4)) -
                    font_snds.text_width(misc.format(unitPtr->target_loyalty, 4)) - font_snds.text_width("11"),
    ...
    if (unitInfo->class_info.has_combat_level)
    {
        ...
        // SXM
        x2 =
            (disp_combat_or_skill == 1 ? font_blu2 : font_snds).put(x + 110, y + 12, text_unit.str_combat_level()) + 10;
        font_snds.right_put(INFO_X2 - 10, y + 12, misc.format(unitPtr->combat_level(), 4));
        //		x2 = (disp_combat_or_skill==1?font_blu2:font_snds).put( x+110,
        // y+15,
        // text_unit.str_combat_level() ) + 10; 		font_snds.right_put(
        // INFO_X2-10, y+15, misc.format(unitPtr->combat_level(),4) ); SXM
    }
    ...
    if (unitInfo->class_info.has_skill_level && unitPtr->skill_level() > 0)
    {
        ...
        //		x2 = (disp_combat_or_skill==2?font_blu2:font_snds).put( x+110,
        // y+29,
        // text_unit.str_leadership() ) + 10; 		font_snds.right_put( INFO_X2-10,
        // y+29, misc.format(unitPtr->skill_level(),4) ); SXM
    }
...
void FirmCamp::detect_overseer_info()
{
    ...
    else if (unitPtr->skill_level() > 0 && mouse.single_click(x + 110, y + 28, x + 189, y + 41))
    {
        // detect skill
        disp_combat_or_skill = 2;
    }
...
void FirmCamp::disp_edit_mode(int &refreshFlag, int &y, int dispBG)
{
    ...
    if (selected_soldier_id > 0 && selected_soldier_id <= soldier_count)
    {
        ...
        if (unitInfo->class_info.has_loyalty) // && rank_id != RANK_KING )
        {
            ...
            x = edit_loyalty_x2 =
                font_blue.put((edit_loyalty_x1 = x + 20), (edit_loyalty_y1 = y), soldierPtr->loyalty, 0, x2);
        ...
        if (unitInfo->class_info.has_combat_level)
        {
            x = font_whbl.put(x1, y, text_unit.str_combat_level(), 0,
                              x2); //	"Combat Level"
            x = edit_combat_x2 = font_blue.put((edit_combat_x1 = x + 20), (edit_combat_y1 = y),
            ...
            {
                x = font_whbl.put(x, y, " / ", 0, x2);
                x = edit_max_combat_x2 = font_blue.put((edit_max_combat_x1 = x), (edit_max_combat_y1 = y),
        ...
        if (unitInfo->class_info.has_skill_level)
        {
            x = font_whbl.put(x1, y, text_unit.str_leadership(), 0,
                              x2); // "Leadership"
            x = edit_skill_x2 = font_blue.put((edit_skill_x1 = x + 20), (edit_skill_y1 = y),
            ...
            {
                x = font_whbl.put(x, y, " / ", 0, x2);
                x = edit_max_skill_x2 = font_blue.put((edit_max_skill_x1 = x), (edit_max_skill_y1 = y),
        ...
        if (soldierPtr->spy_recno)
        {
            ...
            x = edit_spy_x2 =
                font_blue.put(edit_spy_x1 = x, edit_spy_y1 = y, spy_array[soldierPtr->spy_recno]->spy_skill, 0, x2);
    ...
    else if (selected_soldier_id == 0 && !unit_array.is_truly_deleted(overseer_recno))
    {
        ...
        if (unitInfo->class_info.has_loyalty) // && rank_id != RANK_KING )
        {
            x = font_whbl.put(x1, y, text_unit.str_loyalty(), 0, x2); // "Loyalty"
            x = edit_loyalty_x2 =
                font_blue.put((edit_loyalty_x1 = x + 20), (edit_loyalty_y1 = y), overseerPtr->loyalty, 0, x2);
            edit_loyalty_y2 = edit_loyalty_y1 + font_blue.max_font_height;
            edit_loyalty_enable = 1;
            x = font_whbl.put(x, y, " -> ", 0, x2);
        ...
        if (overseerPtr->spy_recno)
        {
            ...
            x = edit_spy_x2 =
                font_blue.put(edit_spy_x1 = x, edit_spy_y1 = y, spy_array[overseerPtr->spy_recno]->spy_skill, 0, x2);
            edit_spy_y2 = edit_spy_y1 + font_blue.max_font_height;
            edit_spy_enable = 1;
        }
...
int FirmCamp::detect_edit_mode()
{
    ...
    if (selected_soldier_id > 0 && selected_soldier_id <= soldier_count)
    {
        ...
        {
            if (mouse.any_click(edit_max_skill_x1, edit_max_skill_y1, edit_max_skill_x2, edit_max_skill_y2,
                                LEFT_BUTTON))
            {
        ...
        if (add_spy_nation[0] && !soldierPtr->spy_recno) // each character represents one nation
        {
            ...
            for (char n = 0; add_spy_nation[n]; ++n, (x1 += interval), (x2 += interval))
            {
                    ...
                    mouse.single_click(x1, add_spy_y1, x2, add_spy_y2, LEFT_BUTTON))
                {
                    ...
                    // in deinit(). Keep an additional right because when a spy is
                    // assigned to a town, the normal program will free up the name id.,
                    // so we have to keep an additional copy
