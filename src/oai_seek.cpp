...
//
int Nation::seek_mine(short &xLoc, short &yLoc, short &refXLoc,
                      short &refYLoc) {
  ...
  for (i = site_array.size(); i > 0; i--) {
    ...
    if (is_action_exist(-1, -1, sitePtr->map_x_loc, sitePtr->map_y_loc,
                        ACTION_AI_BUILD_FIRM, FIRM_MINE))
      continue;
    ...
    for (j = 0; j < ai_town_count; j++) {
      ...
      dist = misc.points_distance(sitePtr->map_x_loc, sitePtr->map_y_loc,
      ...
      for (connected = 0, k = townPtr->linked_firm_count - 1; k >= 0; k--) {
        err_when(!townPtr->linked_firm_array[k] ||
                 firm_array.is_deleted(townPtr->linked_firm_array[k]));
      ...
      if (dist < minDist[siteId] || (connected && dist <= effectiveDis)) {
        ...
        for (int ix = sitePtr->map_x_loc - firmInfo->loc_width + 1;
             ix <= sitePtr->map_x_loc && !canBuild; ix++) {
          if (ix < 0 || ix >= MAX_WORLD_X_LOC)
            continue;

          for (int iy = sitePtr->map_y_loc - firmInfo->loc_height + 1;
               iy <= sitePtr->map_y_loc && !canBuild; iy++) {
  ...
  //			raw material
  //---------------------------------------------------------------------------//
  int weight, pos;   // weight is the such kind of material mined, pos is the
                     // position in the array
  int siteRecno = 0; // siteRecno is the recno of site to build
  int withoutThisRaw =
      0; // withoutThisRaw shows that this raw material is strongly recommended
  ...
  for (pos = -1, weight = 0x7FFFFF, j = 0; j < MAX_RAW; j++) {
    ...
    if (!raw_kind_mined[j]) // no such kind of material and there is a possible
                            // site
    {
      if (withoutThisRaw) {
        if (minDist[j] <
            closestDist) // more than one kind of material we don't have
        {
    ...
    } else if (!withoutThisRaw &&
               weight > raw_kind_mined[j]) // scan for the kind of material with
                                           // least num of this site
    {
  ...
  if (siteRecno) {
    ...
    err_when((xLoc - refXLoc) >= firm_res[FIRM_MINE]->loc_width ||
             (yLoc - refYLoc) >= firm_res[FIRM_MINE]->loc_height);
    //--------------------------------------------------------------//
    // do some adjustment such that the firm will be built far away
    // from other firms by at least one step.
    //--------------------------------------------------------------//
    seek_best_build_mine_location(xLoc, yLoc, sitePtr->map_x_loc,
                                  sitePtr->map_y_loc);
...
//
// <short&> xLoc	 -	a possible x location to build the mine, the final
// location is also returned by this reference
...
//
void Nation::seek_best_build_mine_location(short &xLoc, short &yLoc,
                                           short mapXLoc, short mapYLoc) {
  //--------------- define parameters -----------------//
  ...
  for (ix = xLeftLimit; ix <= mapXLoc; ix++) {
    ...
    for (iy = yLeftLimit; iy <= mapYLoc; iy++) {
      ...
      if (world.can_build_firm(ix, iy, FIRM_MINE)) {
        ...
        //----------------------------------------//
        weight = 0;
        cal_location_score(ix, iy, firmInfo->loc_width, firmInfo->loc_height,
                           weight);
...
//
void Nation::cal_location_score(short x1, short y1, short width, short height,
                                int &score) {
  //-----------------------------------------------------------------//
  ...
  //------- upper right corner ---------//
  if (x1 < MAX_WORLD_X_LOC - 1 && y1 > 0 &&
      world.get_loc(x1 + 1, y1 - 1)->can_build_firm())
    score += 50;

  //----------- lower left corner ----------//
  if (x1 > 0 && y1 < MAX_WORLD_Y_LOC - 1 &&
      world.get_loc(x1 - 1, y1 + 1)->can_build_firm())
    score += 50;

  //------- lower right corner ---------//
  if (x1 < MAX_WORLD_X_LOC - 1 && y1 < MAX_WORLD_Y_LOC - 1 &&
      world.get_loc(x1 + 1, y1 + 1)->can_build_firm())
    score += 50;
}
...
// [short] distance 			 - the distance between the given place
// and
//										new
//place.
//
...
//
int Nation::find_best_place_loc(short buildFirmId, short refXLoc, short refYLoc,
        ...
        if (townPtr->nation_recno != nation_recno) {
          ...
          else // more friendly nations get higher weights
          {
            ...
            if (relationStatus >= RELATION_NEUTRAL)
              weightAdd =
                  weightAdd * (relationStatus - RELATION_NEUTRAL + 1) / 4;
          }
        }
...
//-------- End of function Nation::find_best_place_loc --------//
