...
int Nation::seek_mine(short &xLoc, short &yLoc, short &refXLoc, short &refYLoc)
{
        ...
        for (j = 0; j < ai_town_count; j++)
        {
            ...
            if (dist < minDist[siteId] || (connected && dist <= effectiveDis))
            {
                ...
                for (int ix = sitePtr->map_x_loc - firmInfo->loc_width + 1; ix <= sitePtr->map_x_loc && !canBuild; ix++)
                {
                    ...
                    for (int iy = sitePtr->map_y_loc - firmInfo->loc_height + 1; iy <= sitePtr->map_y_loc && !canBuild;
                         iy++)
                    {
    ...
    for (pos = -1, weight = 0x7FFFFF, j = 0; j < MAX_RAW; j++)
    {
        ...
        else if (!withoutThisRaw && weight > raw_kind_mined[j]) // scan for the kind of material with
...
//
// <short&> xLoc	 -	a possible x location to build the mine, the
// final location is also returned by this reference
...
//
int Nation::find_best_place_loc(short buildFirmId, short refXLoc, short refYLoc, short &resultXLoc, short &resultYLoc,
                                short distance)
{
