...
int PlayerProfile::register_menu()
{
    ...
    SlideBar scrollBar;
    scrollBar.init_scroll(SCROLL_BAR_X1, SCROLL_BAR_Y1, SCROLL_BAR_X2, SCROLL_BAR_Y2, SCROLL_MAX_COLUMN,
    ...
    // 0, 1 ); 	GetA &fileNameField = getGroup[2]; 	fileNameField.init( 200,
    // 460, 400, file_name, SAVE_GAME_DIR_LEN, &font_san, 0, 1 ); 	GetA
    // &dirNameField =
    // getGroup[3]; 	dirNameField.init( 200, 480, 400, save_dir,
    // SAVE_GAME_DIR_LEN,
    //&font_san, 0, 1 );
    ...
    while (1)
    {
        ...
        if (refreshFlag)
        {
            ...
            case PROFILE_MENU_MAIN:
                if (refreshFlag & PPOPTION_PAGE)
                {
                    ...
                    fontThin.center_put(BUTTON7_X1, BUTTON7_Y1, BUTTON7_X2, BUTTON7_Y2,
                                        text_game_menu.str_finish_select_profile()); // continue button
                    fontThin.center_put_paragraph(BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2,
                        ...
                        for (int c = 0; c < columns; ++c, (x1 += columnWidth))
                        {
                            ...
                            for (int i = 0; i < SCROLL_LINE_COUNT; ++i, y += SCROLL_LINE_HEIGHT)
                            {
                                ...
                                if (rec > 0 && rec <= profileCount)
                                {
                                    ...
                                    fontPtr->center_put(x1, y, x1 + columnWidth - 1, y + SCROLL_LINE_HEIGHT - 1,
            ...
            case PROFILE_MENU_DEL:
                if (refreshFlag & PPOPTION_PAGE)
                {
                    ...
                    {
                        fontBlack.center_put_paragraph(136, 287, 672, 387,
        ...
        case PROFILE_MENU_MAIN: {
            ...
            if (mouse.any_click(RIGHT_BUTTON) || mouse.single_click(BUTTON9_X1, BUTTON9_Y1, BUTTON9_X2, BUTTON9_Y2))
            {
                return 0;
            }
        }
        ...
        case PROFILE_MENU_NEW:
            ...
            if (mouse.single_click(BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2, BUTTON4_Y2) || mouse.any_click(RIGHT_BUTTON))
            {
                if (profileCount > 0)
                {
    ...
    for (int i = 0; i < count; ++i)
    {
        if (strcmp(loginName, profileArray[i].login_name) == 0)
        {
            *this = profileArray[i];
            ...
            // save, the file is latest for next time
            // return 2;
        }
    }
