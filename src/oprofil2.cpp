...
int PlayerProfile::register_menu() {
  ...
  if (is_registered()) // try to load it again
  {
    ...
    if (!misc.is_file_exist(full_path) || !f.file_open(full_path) ||
        !reload()) {
      init();
    }
  } else {
  ...
  SlideBar scrollBar;
  scrollBar.init_scroll(SCROLL_BAR_X1, SCROLL_BAR_Y1, SCROLL_BAR_X2,
                        SCROLL_BAR_Y2, SCROLL_MAX_COLUMN, disp_scroll_bar_func);
  ...
  playerNameField.init(200, 300, 600, keyinPlayerName, PLAYER_NAME_LEN,
                       &fontBlack, 1, 1);

  //	GetA &descField = getGroup[1];
  //	descField.init( 200, 440, 790, player_desc, PLAYER_DESC_LEN, &font_san,
  //0, 1 ); 	GetA &fileNameField = getGroup[2]; 	fileNameField.init( 200, 460,
  //400, file_name, SAVE_GAME_DIR_LEN, &font_san, 0, 1 ); 	GetA &dirNameField =
  //getGroup[3]; 	dirNameField.init( 200, 480, 400, save_dir, SAVE_GAME_DIR_LEN,
  //&font_san, 0, 1 );
  ...
  scrollLeft.create_text(BROWSE_X1, SCROLL_BAR_Y1, SCROLL_BAR_X1 - 1,
                         SCROLL_BAR_Y2, " ");
  scrollRight.create_text(SCROLL_BAR_X2 + 1, SCROLL_BAR_Y1, BROWSE_X2,
                          SCROLL_BAR_Y2, " ");
  ...
  while (1) {
    ...
    if (refreshFlag) {
      if (refreshFlag & PPOPTION_ENUM_PROFILE) {
        ...
        profileColumnCount =
            (profileCount + SCROLL_LINE_COUNT - 1) / SCROLL_LINE_COUNT;
        ...
        if (selectedProfile == 0 && profileDir.size() > 0) {
          ...
          for (i = 2; i <= profileDir.size(); ++i) {
            if (profileDir[i]->time.dwHighDateTime >
                    latestTime.dwHighDateTime ||
                profileDir[i]->time.dwHighDateTime ==
                        latestTime.dwHighDateTime &&
                    (profileDir[i]->time.dwLowDateTime >
                     latestTime.dwLowDateTime)) {
      ...
      case PROFILE_MENU_MAIN:
        if (refreshFlag & PPOPTION_PAGE) {
          ...
          fontBlack.center_put(119, 95, 678, 118,
                               text_game_menu.str_profile_s1());
          // fontBlack.center_put( 113, 392, 663, 414, "If your name is not
          // here, click on \"New Player\" below");
          fontBlack.center_put_paragraph(113, 392, 688, 414,
                                         text_game_menu.str_profile_s2());
          fontThin.center_put(
              BUTTON7_X1, BUTTON7_Y1, BUTTON7_X2, BUTTON7_Y2,
              text_game_menu.str_finish_select_profile()); // continue button
          fontThin.center_put_paragraph(
              BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2,
              text_game_menu.str_new_profile()); // new button
          fontThin.center_put(
              BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2, BUTTON4_Y2,
              text_game_menu.str_delete_profile()); // delete button
          fontThin.center_put(BUTTON9_X1, BUTTON9_Y1, BUTTON9_X2, BUTTON9_Y2,
                              text_game_menu.str_cancel()); // cancel button
          vga.use_front();
        ...
        if (refreshFlag & PPOPTION_SCROLL_BAR) {
          ...
          if (columns > 0) {
            int columnWidth =
                (BROWSE_X2 - BROWSE_X1 + 1) / columns; // column width
            int x1 = BROWSE_X1;

            for (int c = 0; c < columns; ++c, (x1 += columnWidth)) {
              int y = BROWSE_Y1;
              for (int i = 0; i < SCROLL_LINE_COUNT;
                   ++i, y += SCROLL_LINE_HEIGHT) {
                int rec =
                    (scrollBar.view_recno + c) * SCROLL_LINE_COUNT + i + 1;

                // copy blank from back buffer
                vga_util.blt_buf(x1, y, x1 + columnWidth - 1,
                                 y + SCROLL_LINE_HEIGHT - 1, 0);
                ...
                if (rec > 0 && rec <= profileCount) {
                  ...
                  fontPtr->center_put(x1, y, x1 + columnWidth - 1,
                                      y + SCROLL_LINE_HEIGHT - 1,
                                      profileArray[rec - 1].player_name);
                }
              }
            }
          }
          ...
          if (profileColumnCount > SCROLL_MAX_COLUMN) {
            ...
            vga_util.blt_buf(BROWSE_X1, SCROLL_BAR_Y1, BROWSE_X2, SCROLL_BAR_Y2,
                             0);
          }
        }
        break;
      ...
      case PROFILE_MENU_NEW: // ask new player
        if (refreshFlag & PPOPTION_PAGE) {
          vga.use_back();
          vga_util.disp_image_file("CHOOSE");
          fontBlack.center_put(136, 187, 672, 217,
                               text_game_menu.str_profile_new());
          fontThin.center_put(BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2,
                              text_game_menu.str_finish_select_profile());
          fontThin.center_put(BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2, BUTTON4_Y2,
      ...
      case PROFILE_MENU_DEL:
        if (refreshFlag & PPOPTION_PAGE) {
          vga.use_back();
          vga_util.disp_image_file("CHOOSE");
          fontBlack.center_put_paragraph(136, 187, 672, 287,
                                         text_game_menu.str_profile_del());
          if (profileArray[selectedProfile - 1].login_name[0]) {
            fontBlack.center_put_paragraph(
                136, 287, 672, 387,
                text_game_menu.str_profile_lost_login_info());
          }
          fontThin.center_put(BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2,
                              text_game_menu.str_profile_del_yes());
          fontThin.center_put(BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2, BUTTON4_Y2,
                              text_game_menu.str_profile_del_no());
          vga.use_front();
          vga_util.blt_buf(0, 0, VGA_WIDTH - 1, VGA_HEIGHT - 1, 0);
        }
        break;
    ...
    case PROFILE_MENU_MAIN: {
      ...
      if (columns > 0) {
        ...
        for (int c = 0; c < columns; ++c, (x1 += columnWidth)) {
          ...
          for (int i = 0; i < SCROLL_LINE_COUNT; ++i, y += SCROLL_LINE_HEIGHT) {
            ...
            int clickCount;
            if (rec > 0 && rec <= profileCount &&
                (clickCount = mouse.any_click(x1, y, x1 + columnWidth,
                                              y + SCROLL_LINE_HEIGHT - 1))) {
              ...
              if (clickCount >= 2) {
                *this = profileArray[selectedProfile - 1];
                if (save() && re_create_directory()) // call save, the file is
                                                     // latest for next time
                  return 2;
              }
            }
          }
      ...
      if (profileColumnCount > SCROLL_MAX_COLUMN) {
        ...
        if (scrollLeft.detect()) {
          ...
          if (scrollBar.set_view_recno(scrollBar.view_recno - 1) !=
              oldViewRecno) {
            refreshFlag |= PPOPTION_SCROLL_BAR;
          }
        }
      ...
      if (selectedProfile > 0 && selectedProfile <= profileCount &&
        ...
        *this = profileArray[selectedProfile - 1];
        if (save() && re_create_directory()) // call save, the file is latest
                                             // for next time
          return 2;
      }
    ...
    case PROFILE_MENU_NEW:
      ...
      // ------- detect geta fields group -------//
      {
        ...
        if (strcmp(oldPlayerName, keyinPlayerName) !=
            0) // if player name changed, update file name field
        {
    ...
    case PROFILE_MENU_DEL:
      if (mouse.single_click(BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2)) {
        ...
        if (stricmp(file_name, profileArray[selectedProfile - 1].file_name) ==
            0) {
...
int PlayerProfile::re_create_directory() {
  ...
  do {
    ...
    if (searchPtr !=
            beginPtr // if path is "\SAVE\GILBERT", searchPtr == beginPtr
...
static void disp_scroll_bar_func(SlideBar *scroll, int) {
  ...
  if (rectLeft > scroll->scrn_x1) // fill region above scroll button
    vga_front.bar(scroll->scrn_x1, scroll->scrn_y1, rectLeft - 1,
                  scroll->scrn_y2, V_BLACK);
  vga_front.bar(rectLeft, scroll->scrn_y1, rectRight, scroll->scrn_y2,
                VGA_YELLOW + 1);
  if (rectRight - rectLeft > 6) {
    vga_front.d3_panel_up(rectLeft, scroll->scrn_y1, rectRight, scroll->scrn_y2,
                          2, 0);
  }
  if (rectRight < scroll->scrn_x2) // fill region below scroll button
    vga_front.bar(rectRight + 1, scroll->scrn_y1, scroll->scrn_x2,
                  scroll->scrn_y2, V_BLACK);
}

// ------- begin of function PlayerProfile::load_count_profiles ----//
//
// <PlayerProfile *> profileArray - array to store player profiles, NULL to
// count <int>             maxLoad      - maximum number of profiles to load
//
int PlayerProfile::load_count_profiles(PlayerProfile *profileArray,
                                       int maxLoad) {
  char full_path[MAX_PATH + 1];
  Directory profileDir;
  if (!misc.path_cat(full_path, sys.dir_config, "*.prf", MAX_PATH)) {
...
int PlayerProfile::load_by_login_name(char *loginName) {
  ...
  for (int i = 0; i < count; ++i) {
    if (strcmp(loginName, profileArray[i].login_name) == 0) {
      *this = profileArray[i];
      return 2;
      //			if( save() && re_create_directory() )	// call
      //save, the file is latest for next time 				return 2;
    }
  }
