...
static Button3D button_spy_mobilize, button_spy_action, button_spy_reward, button_assassinate, button_steal_tech,
    button_view_secret, button_create_incident;
        ...
        if (spyPtr)
        {
            ...
            if ((button_assassinate.visible_flag = !is_own() && firm_res[firm_id]->need_overseer))
            {
                button_assassinate.enable_flag =
                    spyPtr->spy_skill >= MIN_ASSASSIN_SPYING_SKILL && selected_soldier_id > 0 &&
                    selected_soldier_id <= soldier_count && overseer_recno &&
                    !unit_array[overseer_recno]->is_own(); // overseer cannot assassinate
...
//					  >0 - the recno of the player's spy who
// can
// capture this 							 firm.
//
...
void FirmCamp::detect_spy_menu()
{
    ...
    if (firm_menu_mode == FIRM_MENU_SPY)
    {
        ...
        // button_steal_tech.y1, 				button_steal_tech.x2, button_steal_tech.y2,
        // "Steal Technology" );
        //		}
...
void FirmCamp::disp_spy_button(int, int, int refreshFlag)
{
    ...
    // other than the spy button, so always display it left-aligned 		x =
    // INFO_X1;
    ...
    {
        // ##### begin Gilbert 31/12 ####//
        //		button_spy_menu.create( INFO_X1+13+BUTTON_DISTANCE*2,
        // INFO_Y1+281, 'A', "SPYMENU" ); 		button_bribe.create(
        // INFO_X1+13+BUTTON_DISTANCE ,
    ...
    if (player_spy_count > 0)
    {
        ...
        if (nation_recno != nation_array.player_recno) // only display the bribe button for
...
int FirmCamp::can_soldier_capture(int captureNationRecno)
{
    ...
    //		else
    //		{
    //			otherUnitCount++;		// must be an own unit in camps
    // and
