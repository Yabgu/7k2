...
static Button3D button_spy_mobilize, button_spy_action, button_spy_reward,
    button_assassinate, button_steal_tech, button_view_secret,
    button_create_incident;
...
void FirmCamp::disp_spy_menu(int refreshFlag) {
  ...
  Spy *spyPtr = NULL;
  if (selected_soldier_id == 0 && overseer_recno &&
      unit_array[overseer_recno]->is_own_spy()) {
    spyPtr = spy_array[unit_array[overseer_recno]->spy_recno];
  } else if (selected_soldier_id > 0 && selected_soldier_id <= soldier_count &&
             soldier_array[selected_soldier_id - 1].is_own_spy()) {
  ...
  if (firm_menu_mode == FIRM_MENU_SPY) {
    if (refreshFlag == INFO_REPAINT) {
      ...
      button_spy_action.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 235,
                               'A', "CHG-MISS");
      ...
      // #### begin Gilbert 31/12 ######//
      button_create_incident.create(INFO_X1 + 13 + 3 * BUTTON_DISTANCE,
                                    INFO_Y1 + 235, 'A', "INCIDENT");
      button_create_incident.enable_flag = 0;
      button_steal_tech.create(INFO_X1 + 13 + 2 * BUTTON_DISTANCE,
                               INFO_Y1 + 235, 'A', "STEAL_T");
      // #### end Gilbert 31/12 ######//
      button_steal_tech.enable_flag = 0;
      button_assassinate.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 281,
                                'A', "ASSASSIN");
      button_assassinate.enable_flag = 0;
      button_view_secret.create(INFO_X1 + 13 + 2 * BUTTON_DISTANCE,
                                INFO_Y1 + 281, 'A', "VSECRET");
    ...
    if (spyPtr) {
      ...
      button_spy_action.visible_flag =
          !is_own(); // cannot change action in inns
      ...
      if ((button_create_incident.visible_flag = !is_own())) {
        button_create_incident.enable_flag =
            ...
            tech_res[TECH_INCIDENT]->get_nation_tech_level(
      ...
      if ((button_steal_tech.visible_flag = !is_own())) {
        button_steal_tech.enable_flag =
            tech_res[TECH_STEAL_TECH]->get_nation_tech_level(
                nation_array.player_recno) &&
      ...
      if ((button_view_secret.visible_flag = !is_own())) {
        button_view_secret.enable_flag =
            ...
            tech_res[TECH_INFO_THEFT]->get_nation_tech_level(
      ...
      if ((button_assassinate.visible_flag =
               !is_own() && firm_res[firm_id]->need_overseer)) {
        button_assassinate.enable_flag =
            ...
            overseer_recno &&
            !unit_array[overseer_recno]
                 ->is_own(); // overseer cannot assassinate
...
//					  >0 - the recno of the player's spy who can
//capture this 							 firm.
//
int FirmCamp::can_player_spy_capture() {
  if (nation_recno ==
      nation_array.player_recno) // this is the player's own firm.
    return 0;
  ...
  if (firm_res[firm_id]->need_overseer) {
    if (overseer_recno) {
      ...
      if (spyRecno) {
        if (spy_array[spyRecno]->true_nation_recno ==
            nation_array.player_recno) {
          return spyRecno;
        }
      }
  ...
  if (firm_menu_mode == FIRM_MENU_SPY) {
    if (button_spy_mobilize.detect()) {
      if (!remote.is_enable()) {
        ...
        short *shortPtr = (short *)remote.new_send_queue_msg(MSG_SPY_LEAVE_FIRM,
                                                             sizeof(short));
    ...
    //		{
    //			help.set_custom_help(button_create_incident.x1,
    //button_create_incident.y1, 				button_create_incident.x2,
    //button_create_incident.y2, "Create Incident" );
    //		}
    ...
    //		{
    //			help.set_custom_help(button_steal_tech.x1,
    //button_steal_tech.y1, 				button_steal_tech.x2, button_steal_tech.y2, "Steal
    //Technology" );
    //		}

    if (button_assassinate.detect()) {
      int attackRating, defenseRating, otherDefenderCount;
      if (spyPtr->get_assassinate_rating(overseer_recno, attackRating,
                                         defenseRating, otherDefenderCount)) {
        action_spy_recno = spyPtr->spy_recno;
        firm_menu_mode = FIRM_MENU_ASSASSINATE_CONFIRM;
        spy_action_chance = attackRating - defenseRating;
        if (!config.show_debug_info) // to show the actual value, not bounded to
                                     // [0,100]
        {
          // ###### begin Gilbert 27/1 ######//
...
void FirmCamp::disp_spy_button(int, int, int refreshFlag) {
  //	if( !is_own() )		// if not own firm, there is not other button
  //other than the spy button, so always display it left-aligned 		x = INFO_X1;
  //// if own firm, the x passed will be space position on the interface
  //already, taking into account of the other buttons on interface

  //-------------------------------------//

  if (refreshFlag == INFO_REPAINT) {
    // ##### begin Gilbert 31/12 ####//
    //		button_spy_menu.create( INFO_X1+13+BUTTON_DISTANCE*2, INFO_Y1+281,
    //'A', "SPYMENU" ); 		button_bribe.create( INFO_X1+13+BUTTON_DISTANCE ,
    //INFO_Y1+281, 'A', "SELBRIBE" );

    button_spy_menu.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 235, 'A',
                           "SPYMENU");
    button_bribe.create(INFO_X1 + 13 + 2 * BUTTON_DISTANCE, INFO_Y1 + 235, 'A',
                        "SELBRIBE");
  ...
  if (player_spy_count > 0) {
    ...
    if (nation_recno !=
        nation_array.player_recno) // only display the bribe button for
                                   // non-player owned towns
    {
      //-------- display the bribe button -------//

      button_bribe.enable_flag =
          (selected_soldier_id || overseer_recno)
              ? can_spy_bribe(selected_soldier_id, nation_array.player_recno)
              : 0;
      button_bribe.paint();
    ...
    if ((button_capture.visible_flag = !is_own())) {
      button_capture.enable_flag =
          can_soldier_capture(nation_array.player_recno);
    }
...
void FirmCamp::detect_spy_button() {
  if (player_spy_count > 0) {
    ...
    if (nation_recno !=
        nation_array
            .player_recno) // only display the bribe button for non-player towns
    {
      if (button_bribe.detect()) {
        if (player_spy_count > 1) {
          ...
          action_target_recno =
              selected_soldier_id; // recno of bribe/assassinate target
        } else if (player_spy_count == 1) {
          firm_menu_mode = FIRM_MENU_SET_BRIBE_AMOUNT;
          bribe_result = BRIBE_NONE;
          action_spy_recno = get_player_spy_recno(
              firm_recno); // the player has only one spy in this firm
  ...
  if (button_capture.detect() &&
    ...
    if (overseer_recno && unit_array[overseer_recno]->spy_recno &&
        unit_array[overseer_recno]->true_nation_recno() ==
            nation_array.player_recno) {
    ...
    if (captureSpyRecno) {
      if (!remote.is_enable()) {
        ...
        // packet structure <spy recno>
        short *shortPtr = (short *)remote.new_send_queue_msg(
            MSG_SPY_CAPTURE_FIRM, sizeof(short));
        *shortPtr = captureSpyRecno;
      }
    } else {
      if (!remote.is_enable()) {
        capture_firm(
            nation_array.player_recno); // update RemoteMsg::firm_capture
      } else {
        // packet structure <firm recno> <capturer's nation recno>
        short *shortPtr = (short *)remote.new_send_queue_msg(MSG_FIRM_CAPTURE,
                                                             2 * sizeof(short));
        *shortPtr = firm_recno;
...
static int get_player_spy_recno(int firmRecno) {
  ...
  for (int i = 1; i <= spyCount; i++) {
    ...
    if (spyPtr->spy_place == SPY_FIRM && spyPtr->spy_place_para == firmRecno &&
...
int FirmCamp::can_soldier_capture(int captureNationRecno) {
  ...
  //----- if this firm needs an overseer, can only capture it when the overseer
  //is the spy ---//
  ...
  //--- if this firm doesn't need an overseer, can capture it if all the units
  //in the firm are the player's spies ---//
  ...
  //			 spy_array[soldierPtr->spy_recno]->true_nation_recno ==
  //captureNationRecno )
  //		{
  //			captureUnitCount++;
  //			continue;
  ...
  //		else
  //		{
  //			otherUnitCount++;		// must be an own unit in camps and
  //bases if the unit is not a spy
  //		}
  //	}
...
void FirmCamp::disp_spy_info(int dispY1, int refreshFlag) {
  ...
  if (unit_res[unitId]->race_id) {
    ...
    //	info.disp_loyalty( x, y+14, x+60, unitLoyalty, unitLoyalty,
    //nation_recno, refreshFlag );
    info.disp_loyalty(INFO_X1 + 20, y + 12,
                      INFO_X2 - 99 -
                          font_snds.text_width(misc.format(unitLoyalty, 4)),
