...
int SEInfo::match(char subjectType, short subjectId, const char *act, char objectType, short objectId)
{
    // object_type == -1, match all object type
    // object_id == -1, match all object id of that type
    return subject_type == subjectType && (subject_id == -1 || subject_id == subjectId) &&
...
void SERes::build_index()
{
    ...
    SEInfoIndex *seIndex = se_index_array =
        (SEInfoIndex *)mem_resize(se_index_array, sizeof(SEInfoIndex) * se_index_count);
    memset(se_index_array, 0, sizeof(SEInfoIndex) * se_index_count);

    SETypeIndex *typeIndex = type_index_array =
        (SETypeIndex *)mem_resize(type_index_array, sizeof(SETypeIndex) * type_index_count);
...
// <char *> act              name of the action, first four chars are
// significant [char] objectType         type of object 'S'= sprite,
//                           'U'=unit, 'R'=race, 'F'=firm, 'T'=town
//                           default : 0 (none)
...
//
SEInfo *SERes::scan(char subjectType, short subjectId, const char *act, char objectType, short objectId, int findFirst)
{
    err_when(!init_flag);
    ...
    foundFlag = 0;
    for (i = startRec, seIndex = se_index_array + startRec; i <= endRec; ++i, ++seIndex)
    {
    ...
    // ----------- search the se_array ----------//
    for (i = startRec, seInfo = se_array + startRec; i <= endRec; ++i, ++seInfo)
    {
...
short SERes::scan_id(char subjectType, short subjectId, const char *act, char objectType, short objectId, int findFirst)
{
    SEInfo *seInfo;
    if ((seInfo = scan(subjectType, subjectId, act, objectType, objectId, findFirst)) != NULL)
...
void SERes::sound(short xLoc, short yLoc, short frame, char subjectType, short subjectId, const char *action,
                  char objectType, short objectId)
{
    ...
    if ((seInfo = scan(subjectType, subjectId, action, objectType, objectId)) != NULL && frame == seInfo->out_frame)
    {
...
//
void SERes::far_sound(short xLoc, short yLoc, short frame, char subjectType, short subjectId, const char *action,
                      char objectType, short objectId)
{
    //### begin trevor 20/8 ###//
    if (!config.sound_effect_flag)
...
int SERes::mark_select_object_time() // return false if this sound should be
                                     // skipped due to too frequent
{
...
// --------- begin of function SERes::mark_select_object_time ----------//
int SERes::mark_command_time() // return false if this sound should be skipped
                               // due to too frequent
{
    unsigned long t = misc.get_time();
