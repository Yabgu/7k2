...
unsigned long SERes::select_sound_length =
    600; // time between successive select sound
...
void SERes::build_index() {
  ...
  SEInfoIndex *seIndex = se_index_array = (SEInfoIndex *)mem_resize(
      se_index_array, sizeof(SEInfoIndex) * se_index_count);
  memset(se_index_array, 0, sizeof(SEInfoIndex) * se_index_count);

  SETypeIndex *typeIndex = type_index_array = (SETypeIndex *)mem_resize(
      type_index_array, sizeof(SETypeIndex) * type_index_count);
  ...
  for (i = 0, seInfo = se_array; i < se_array_count; ++i, ++seInfo) {
    if (lastType != seInfo->subject_type) {
      ...
      err_when(typeIndex <
               type_index_array); // must not enter here for the first time
...
SEInfo *SERes::scan(char subjectType, short subjectId, const char *act,
  ...
  int foundFlag = 0;
  for (i = 0, typeIndex = type_index_array; i < type_index_count;
       ++i, ++typeIndex) {
  ...
  for (i = startRec, seIndex = se_index_array + startRec; i <= endRec;
       ++i, ++seIndex) {
    if (subjectId == seIndex->subject_id &&
        subjectType == seIndex->subject_type) {
...
short SERes::scan_id(char subjectType, short subjectId, const char *act,
                     char objectType, short objectId, int findFirst) {
  SEInfo *seInfo;
  if ((seInfo = scan(subjectType, subjectId, act, objectType, objectId,
                     findFirst)) != NULL)
...
void SERes::sound(short xLoc, short yLoc, short frame, char subjectType,
                  short subjectId, const char *action, char objectType,
                  short objectId) {
  //### begin trevor 20/8 ###//
  ...
  short relXLoc =
      xLoc - (world.zoom_matrix->top_x_loc + world.zoom_matrix->center_x);
  short relYLoc =
      yLoc - (world.zoom_matrix->top_y_loc + world.zoom_matrix->center_y);
...
//
void SERes::far_sound(short xLoc, short yLoc, short frame, char subjectType,
                      short subjectId, const char *action, char objectType,
                      short objectId) {
  //### begin trevor 20/8 ###//
  if (!config.sound_effect_flag)
