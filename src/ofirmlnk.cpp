...
void Firm::setup_link()
{
    ...
    for (firmRecno = firm_array.size(); firmRecno > 0; firmRecno--)
    {
        ...
        if (firmPtr->nation_recno != nation_recno &&
            firmPtr->firm_id != FIRM_LAIR) // A nation lair can be linked to an independent lair
            continue;
        ...
        if (!firm_res.is_linkable_to_firm(firm_id, race_id, nation_recno, firmPtr->firm_id, firmPtr->race_id,
                                          firmPtr->nation_recno))
            continue;

        //------- determine the default link status ------//

        if (firmPtr->nation_recno == nation_recno) // if the two firms are of the same nation, get the
    ...
    for (townRecno = town_array.size(); townRecno > 0; townRecno--)
    {
        ...
        if (misc.points_distance(townPtr->center_x, townPtr->center_y, center_x, center_y) >
            world.effective_distance(firm_id, 0))
        {
            continue;
        }
        ...
        */
        ...
        if (townPtr->linked_firm_count < MAX_LINKED_FIRM_TOWN)
        {
            ...
            err_when(townPtr->linked_firm_count > 0 && townPtr->linked_firm_array[townPtr->linked_firm_count - 1] ==
    ...
    if (can_set_active_link(0) &&
        (!active_link_town_recno || !can_set_active_link(active_link_town_recno))) // if current active_link_town is
                                                                                   // active, don't change
    {
...
//						 if this is -1, the only one side will
// be
// set even though the nation recno of the firm and town are the same
// (default: 0)
//
...
int Firm::can_train_from_town(int townRecno, int requiredLinkedFirmId, int raceId)
{
    ...
    if (townPtr->nation_recno != nation_recno || (raceId && townPtr->race_id != raceId) ||
        townPtr->loyalty < MIN_RECRUIT_LOYALTY || townPtr->recruitable_pop(1) <= 0)
    {
        return 0;
    }
    ...
    for (int i = townPtr->linked_firm_count - 1; i >= 0; --i)
    {
        ...
        if (firmPtr->firm_id == requiredLinkedFirmId && firmPtr->nation_recno == townPtr->nation_recno &&
            (!firm_res[firmPtr->firm_id]->same_race_worker || firmPtr->race_id == townPtr->race_id) &&
            townPtr->linked_firm_enable_array[i] == LINK_EE && !firmPtr->under_construction)
        {
            return 1;
        }
    }
