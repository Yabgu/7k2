...
// Description : Object Firm - functions for setting links between firms and
// towns
...
void Firm::setup_link() {
  ...
  for (firmRecno = firm_array.size(); firmRecno > 0; firmRecno--) {
    ...
    if (firmPtr->nation_recno != nation_recno &&
        firmPtr->firm_id !=
            FIRM_LAIR) // A nation lair can be linked to an independent lair
      continue;

    //---------- check if the firm is close enough to this firm -------//

    if (misc.points_distance(firmPtr->center_x, firmPtr->center_y, center_x,
                             center_y) >
        world.effective_distance(firmPtr->firm_id, firm_id)) {
      continue;
    }
    ...
    if (!firm_res.is_linkable_to_firm(firm_id, race_id, nation_recno,
                                      firmPtr->firm_id, firmPtr->race_id,
    ...
    if (firmPtr->nation_recno ==
        nation_recno) // if the two firms are of the same nation, get the
                      // default link status which is based on the types of the
                      // firms
      defaultLinkStatus = firmInfo->default_firm_link_status(firmPtr->firm_id);
    else
      defaultLinkStatus = LINK_DD; // if the two firms are of different nations,
                                   // default link status is both side disabled
    ...
    if (linked_firm_count < MAX_LINKED_FIRM_FIRM) {
      ...
      linked_firm_count++;
    } else // we must link it as it is linked both sides, if one side is linked
           // and the other is not, that will cause a bug
    {
      err_here();
    }

    if (firmPtr->linked_firm_count < MAX_LINKED_FIRM_FIRM) {
      if (defaultLinkStatus ==
          LINK_ED) // Reverse the link status for the opposite linker
      ...
      firmPtr->linked_firm_enable_array[firmPtr->linked_firm_count] =
          defaultLinkStatus;
  ...
  for (townRecno = town_array.size(); townRecno > 0; townRecno--) {
    ...
    //----- only linkable if the firm and the town belong to the same nation or
    //the firm can influence a foreign town, i.e. Camp, Fort and Fryhtan Lair,
    ...
    if (misc.points_distance(townPtr->center_x, townPtr->center_y, center_x,
                             center_y) > world.effective_distance(firm_id, 0)) {
      continue;
    }
    ...
    {
      if (nation_recno == 0 ||
          townPtr->resistance(nation_recno) > INDEPENDENT_LINK_RESISTANCE)
        defaultLinkStatus = LINK_ED;
    }
    /*
                    //---------------------------------------------------//
                    ...
                    {
                            if( townPtr->nation_recno==0 ||
       !townPtr->has_linked_own_camp ) defaultLinkStatus = LINK_EE;
                    }
    */
    //-------- add the link now -------//
    ...
    if (townPtr->linked_firm_count < MAX_LINKED_FIRM_TOWN) {
      ...
      err_when(townPtr->linked_firm_count > 0 &&
               townPtr->linked_firm_array[townPtr->linked_firm_count - 1] ==
                   firm_recno); // BUGHERE check double linking error

      townPtr->linked_firm_array[townPtr->linked_firm_count] = firm_recno;
      townPtr->linked_firm_enable_array[townPtr->linked_firm_count] =
          defaultLinkStatus;
  ...
  if (can_set_active_link(0) &&
      (!active_link_town_recno ||
       !can_set_active_link(
           active_link_town_recno))) // if current active_link_town is active,
                                     // don't change
  {
...
void Firm::release_firm_link(int releaseFirmRecno) {
  ...
  for (int i = 0; i < linked_firm_count; i++) {
    if (linked_firm_array[i] == releaseFirmRecno) {
      ...
      misc.del_array_rec(linked_firm_array, linked_firm_count,
                         sizeof(linked_firm_array[0]), i + 1);
      misc.del_array_rec(linked_firm_enable_array, linked_firm_count,
                         sizeof(linked_firm_enable_array[0]), i + 1);
      linked_firm_count--;
      return;
    }
  }
...
void Firm::release_town_link(int releaseTownRecno) {
  ...
  for (int i = 0; i < linked_town_count; i++) {
    if (linked_town_array[i] == releaseTownRecno) {
      ...
      misc.del_array_rec(linked_town_array, linked_town_count,
                         sizeof(linked_town_array[0]), i + 1);
      misc.del_array_rec(linked_town_enable_array, linked_town_count,
                         sizeof(linked_town_enable_array[0]), i + 1);
      linked_town_count--;
      return;
    }
  }
...
// [int] setBoth - if this is 1, it will set the link to either LINK_EE or
// LINK_DD (and no LINK_ED or LINK_DD)
//						 if this is -1, the only one side will be
//set even though the nation recno of the firm and town are the same 					    (default:
//0)
//
void Firm::toggle_firm_link(int linkId, int toggleFlag, char remoteAction,
                            int setBoth) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <firm recno> <link Id> <toggle Flag>
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_FIRM_TOGGLE_LINK_FIRM, 3 * sizeof(short));
  ...
  int linkedNationRecno =
      firm_array[linked_firm_array[linkId - 1]]->nation_recno;

  int sameNation = linkedNationRecno == nation_recno;

  //	int sameNation = linkedNationRecno == nation_recno ||    // if one of
  //the linked end is an indepdendent firm/nation, consider this link as a
  //single nation link 						  linkedNationRecno == 0 ||
  //                  nation_recno == 0;
...
//0)
//
void Firm::toggle_town_link(int linkId, int toggleFlag, char remoteAction,
                            int setBoth) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <firm recno> <link Id> <toggle Flag>
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_FIRM_TOGGLE_LINK_TOWN, 3 * sizeof(short));
    shortPtr[0] = firm_recno;
    shortPtr[1] = linkId;
    shortPtr[2] = toggleFlag;
    return;
  ...
  int linkedNationRecno =
      town_array[linked_town_array[linkId - 1]]->nation_recno;
...
//
// <short *>townRecnoArray   array of townRecnoArray to hold town recnos. The
// array size is MAX_RACE. [int] raceId              race_id required of the
// towns, 0 if default of firm (default:0) [int] requiredLinkedFirmIed  town
// consider if that town has enabled firm linked (default:0) return : number of
// suitable town
//
int Firm::scan_best_recruitable_town(short *townRecnoArray, int raceId,
                                     int requiredLinkedFirmId) {
...
// [int] raceId					- the race id of the unit to be
// trained
//										  (default: 0 -
//any race)
//
...
//
int Firm::can_train_from_town(int townRecno, int requiredLinkedFirmId,
                              int raceId) {
  ...
  if (!requiredLinkedFirmId) // if the training does not required a linked firm,
                             // return true now
    return 1;
  ...
  for (int i = townPtr->linked_firm_count - 1; i >= 0; --i) {
    ...
    if (firmPtr->firm_id == requiredLinkedFirmId &&
        ...
        (!firm_res[firmPtr->firm_id]->same_race_worker ||
         firmPtr->race_id == townPtr->race_id) &&
        townPtr->linked_firm_enable_array[i] == LINK_EE &&
