...
FirmMonsterFortress::FirmMonsterFortress(int supportUnitId) {
  memset(sizeof(Firm) + (char *)this, 0,
         sizeof(FirmMonsterFortress) - sizeof(Firm));
...
void FirmMonsterFortress::init_derived() {
  ...
  if (nation_recno)
    max_hit_point_ratio =
        firm_res[firm_id]->get_nation_tech_level(nation_recno) * 0.25f + 0.75f;
  else
    max_hit_point_ratio = 1.0f;

  if (nation_recno &&
      tech_res[TECH_FORTRESS_CAMOUFLAGE]->get_nation_tech_level(nation_recno)) {
    rock_id = rock_res.search(
        "R", obj_loc_width(), obj_loc_width(), obj_loc_height(),
        obj_loc_height(), 0, 0,
        terrain_res[world.get_loc(center_x, center_y)->terrain_id]
            ->average_type);

    if (!rock_id) {
      rock_id = rock_res.search(
          "PR", obj_loc_width() - 1, obj_loc_width(), obj_loc_height() - 1,
          obj_loc_height(), 0, 0,
          terrain_res[world.get_loc(center_x, center_y)->terrain_id]
              ->average_type);
    }
...
//
void FirmMonsterFortress::deinit() { Firm::deinit(); }
...
int FirmMonsterFortress::builder_stay_after_construction() {
  ...
  err_when(under_construction); // assume finish construction for
                                // assign_extra_builder to work correctly

  // recalcuate hit_points and place_max_hit_points

  for (int i = 0; i < extra_builder_count; ++i) {
    hit_points +=
        unit_array[extra_builder_recno[i]]->hit_points * max_hit_point_ratio;
    place_max_hit_points +=
        unit_array[extra_builder_recno[i]]->max_hit_points() *
        max_hit_point_ratio;
  }
...
void FirmMonsterFortress::assign_extra_builder(int builderRecno) {
  ...
  //--- if the new builder's nation is not the same as the firm's nation, don't
  //assign ---//
...
int FirmMonsterFortress::mobilize_extra_builder(int builderId) {
  ...
  int spaceFound = locate_space(is_being_deleted, xLoc, yLoc, loc_x2, loc_y2,
  ...
  if (spaceFound) {
    ...
    unit_array.del(
        builderRecno); // delete it when there is no space for the unit
  }

  //--------- del builder recno from the builder array -------------//

  misc.del_array_rec(extra_builder_recno, MAX_EXTRA_BUILDER,
                     sizeof(extra_builder_recno[0]), builderId);
  --extra_builder_count;
  ...
  if (!unit_array.is_deleted(builderRecno)) {
    unitPtr->update_loyalty();
    unitPtr->home_camp_firm_recno =
        0; // builder do not set home_camp_firm_recno
  }
...
void FirmMonsterFortress::kill_extra_builder(int builderId) {
  ...
  if (!sys.signal_exit_flag)
    game.update_condition_on_killed(
        unit_array[builderRecno]->unique_id); // check unit killed
  // ####### end Gilbert 24/2 ########//
...
void FirmMonsterFortress::assign_soldier(int unitRecno) {
  ...
  //--- if the new archer's nation is not the same as the firm's nation, don't
  //assign ---//
...
int FirmMonsterFortress::mobilize_soldier(int soldierId, char remoteAction) {
  ...
  for (int r = 0; r <= 6 && !spaceFound; r += unitPtr->loc_width) {
    ...
    // int spaceFound = locate_space(is_being_deleted, xLoc, yLoc, loc_x2+2,
    // loc_y2+2, spriteInfo->loc_width, spriteInfo->loc_height);
    spaceFound = world.locate_space(xLoc, yLoc, loc_x2 + r, loc_y2 + r,
                                    unitPtr->loc_width, unitPtr->loc_height,
                                    unitPtr->mobile_type);
  }
  ...
  misc.del_array_rec(archer_unit_recno, MAX_FORTRESS_SOLDIER,
                     sizeof(archer_unit_recno[0]), soldierId);
  --archer_count;
...
void FirmMonsterFortress::kill_soldier(int soldierId) {
  ...
  if (!sys.signal_exit_flag)
    game.update_condition_on_killed(
        unit_array[unitRecno]->unique_id); // check unit killed
  // ####### end Gilbert 24/2 ########//
...
int FirmMonsterFortress::kill_unit(int unitRecno) {
  ...
  for (i = 0; i < extra_builder_count && unitRecno != extra_builder_recno[i];
       ++i)
    ;
...
//
bool FirmMonsterFortress::can_accept_assign(int unitRecno,
                                            int actionNationRecno) {
  if (under_construction)
    return false;
  ...
  // ######### begin Gilbert 25/3 #######//
  // if( unitInfo->unit_class == UNIT_CLASS_HUMAN )	// it must not be a
  // human
...
void FirmMonsterFortress::change_nation(int newNationRecno) {
  ...
  for (i = 0; i < extra_builder_count; i++) {
    ...
    unitPtr->unit_mode =
        0; // reset it first so unitPtr->change_nation() will not attempt to
           // call Firm::change_nation() in a recusive way
...
void FirmMonsterFortress::tranform_from_units(int unitRecno, char remoteAction)
{
        ...
        if( firmXLoc < 0 && unitPtr->obj_loc_x1() > 0 && unitPtr->obj_loc_y1() >
0 )
        {
                ...
                Location *locPtr = world.get_loc( unitPtr->obj_loc_x1(),
unitPtr->obj_loc_y1()-1 ); topUnit = locPtr->unit_recno( UNIT_LAND );

                locPtr = world.get_loc( unitPtr->obj_loc_x1()-1,
unitPtr->obj_loc_y1() ); leftUnit = locPtr->unit_recno( UNIT_LAND );

                locPtr = world.get_loc( unitPtr->obj_loc_x1()-1,
unitPtr->obj_loc_y1()-1 ); topLeftUnit = locPtr->unit_recno( UNIT_LAND );
                ...
                if( topUnit && leftUnit && topLeftUnit
                        ...
                        // same nation
                        && unit_array[topUnit]->nation_recno ==
unitPtr->nation_recno
                        && unit_array[leftUnit]->nation_recno ==
unitPtr->nation_recno
                        && unit_array[topLeftUnit]->nation_recno ==
unitPtr->nation_recno

                        // no duplicated units
                        && unitRecno != topUnit && unitRecno != leftUnit &&
unitRecno != topLeftUnit
                        && topUnit != leftUnit && topUnit != topLeftUnit &&
leftUnit != topLeftUnit )
                {
        ...
        if( firmXLoc < 0 && unitPtr->obj_loc_x2() < MAX_WORLD_X_LOC-1 &&
unitPtr->obj_loc_y1() > 0 )
        {
                // find unit on top, top left and left
                short& topUnit = builderUnit[1];
                ...
                Location *locPtr = world.get_loc( unitPtr->obj_loc_x2(),
unitPtr->obj_loc_y1()-1 ); topUnit = locPtr->unit_recno( UNIT_LAND );

                locPtr = world.get_loc( unitPtr->obj_loc_x2()+1,
unitPtr->obj_loc_y1() ); rightUnit = locPtr->unit_recno( UNIT_LAND );

                locPtr = world.get_loc( unitPtr->obj_loc_x2()+1,
unitPtr->obj_loc_y1()-1 ); topRightUnit = locPtr->unit_recno( UNIT_LAND );
                ...
                if( topUnit && rightUnit && topRightUnit
                        ...
                        && unit_array[topUnit]->nation_recno ==
unitPtr->nation_recno
                        && unit_array[rightUnit]->nation_recno ==
unitPtr->nation_recno
                        && unit_array[topRightUnit]->nation_recno ==
unitPtr->nation_recno

                        // no duplicated units
                        && unitRecno != topUnit && unitRecno != rightUnit &&
unitRecno != topRightUnit
                        && topUnit != rightUnit && topUnit != topRightUnit &&
rightUnit != topRightUnit )
                {
        ...
        if( firmXLoc < 0 && unitPtr->obj_loc_x1() > 0 && unitPtr->obj_loc_y2() <
MAX_WORLD_Y_LOC-1 )
        {
                ...
                Location *locPtr = world.get_loc( unitPtr->obj_loc_x1(),
unitPtr->obj_loc_y2()+1 ); bottomUnit = locPtr->unit_recno( UNIT_LAND );

                locPtr = world.get_loc( unitPtr->obj_loc_x1()-1,
unitPtr->obj_loc_y2() ); leftUnit = locPtr->unit_recno( UNIT_LAND );

                locPtr = world.get_loc( unitPtr->obj_loc_x1()-1,
unitPtr->obj_loc_y2()+1 ); bottomLeftUnit = locPtr->unit_recno( UNIT_LAND );
                ...
                if( bottomUnit && leftUnit && bottomLeftUnit
                        ...
                        && unit_array[leftUnit]->unit_id == unitPtr->unit_id
                        && unit_array[bottomLeftUnit]->unit_id ==
unitPtr->unit_id

                        // same nation
                        && unit_array[bottomUnit]->nation_recno ==
unitPtr->nation_recno
                        && unit_array[leftUnit]->nation_recno ==
unitPtr->nation_recno
                        && unit_array[bottomLeftUnit]->nation_recno ==
unitPtr->nation_recno

                        // no duplicated units
                        && unitRecno != bottomUnit && unitRecno != leftUnit &&
unitRecno != bottomLeftUnit
                        && bottomUnit != leftUnit && bottomUnit !=
bottomLeftUnit && leftUnit != bottomLeftUnit )
                {
        ...
        if( firmXLoc < 0 && unitPtr->obj_loc_x2() < MAX_WORLD_X_LOC-1 &&
unitPtr->obj_loc_y2() < MAX_WORLD_Y_LOC-1 )
        {
...
unitPtr->obj_loc_y2()+1 ); bottomUnit = locPtr->unit_recno( UNIT_LAND );
                ...
                locPtr = world.get_loc( unitPtr->obj_loc_x2()+1,
unitPtr->obj_loc_y2() ); rightUnit = locPtr->unit_recno( UNIT_LAND );

                locPtr = world.get_loc( unitPtr->obj_loc_x2()+1,
unitPtr->obj_loc_y2()+1 ); bottomRightUnit = locPtr->unit_recno( UNIT_LAND );
                ...
                if( bottomUnit && rightUnit && bottomRightUnit
                        ...
                        && unit_array[bottomUnit]->unit_id == unitPtr->unit_id
                        && unit_array[rightUnit]->unit_id == unitPtr->unit_id
                        && unit_array[bottomRightUnit]->unit_id ==
unitPtr->unit_id
...
unitPtr->nation_recno
                        && unit_array[rightUnit]->nation_recno ==
unitPtr->nation_recno
                        && unit_array[bottomRightUnit]->nation_recno ==
unitPtr->nation_recno

                        // no duplicated units
                        && unitRecno != bottomUnit && unitRecno != rightUnit &&
unitRecno != bottomRightUnit
                        && bottomUnit != rightUnit && bottomUnit !=
bottomRightUnit && rightUnit != bottomRightUnit )
                {
        ...
        if( firmXLoc >= 0 )
        {
                ...
                {
                        if( unit_array.selected_recno == builderUnit[i] )
// check selected_recno instead of selected_flag selectedFlag = 1;
// because if the display interface is any of the units, display the firm
interface unit_array[builderUnit[i]]->deinit_sprite(0);	 	// 0-if the unit
is currently selected, deactivate it.
                }
                ...
                if( unitPtr->race_id )
                {
                        if( unitPtr->is_monster() )
                                buildCode =
monster_res[unitPtr->monster_id()]->firm_build_code; else buildCode =
race_res[unitPtr->race_id]->code;
                }
                else
                        err_here();

                int firmRecno = firm_array.build_firm(firmXLoc, firmYLoc,
unitPtr->nation_recno, FIRM_FORTRESS, buildCode, unitRecno );
...
//			err_here();
                        ...
                        for( i = 0; i < MAX_EXTRA_BUILDER; ++i )
                        {
                                unit_array[builderUnit[i]]->init_sprite(
                                        unit_array[builderUnit[i]]->next_x_loc(),
unit_array[builderUnit[i]]->next_y_loc() );
                        }
                }
...
//-------- Begin function
//FirmMonsterFortress::check_transform_from_units------//
bool FirmMonsterFortress::check_transform_from_units(int unitRecno,
                                                     int &firmXLoc,
                                                     int &firmYLoc,
                                                     short *builderUnit) {
  if (unit_array.is_deleted(unitRecno))
  ...
  if (!check && unitPtr->obj_loc_x1() > 0 && unitPtr->obj_loc_y1() > 0) {
    ...
    Location *locPtr =
        world.get_loc(unitPtr->obj_loc_x1(), unitPtr->obj_loc_y1() - 1);
    ...
    locPtr =
        world.get_loc(unitPtr->obj_loc_x1() - 1, unitPtr->obj_loc_y1() - 1);
    ...
    if (topUnit && leftUnit &&
        ...
        // no duplicated units
        && unitRecno != topUnit && unitRecno != leftUnit &&
        unitRecno != topLeftUnit && topUnit != leftUnit &&
        topUnit != topLeftUnit && leftUnit != topLeftUnit) {
      firmXLoc = unit_array[topLeftUnit]->obj_loc_x1();
      firmYLoc = unit_array[topLeftUnit]->obj_loc_y1();
      check = true;
      return true;
  ...
  if (!check && unitPtr->obj_loc_x2() < MAX_WORLD_X_LOC - 1 &&
      unitPtr->obj_loc_y1() > 0) {
    // find unit on top, top left and left
    short &topUnit = builderUnit[1];
    short &rightUnit = builderUnit[2];
    ...
    Location *locPtr =
        world.get_loc(unitPtr->obj_loc_x2(), unitPtr->obj_loc_y1() - 1);
    topUnit = locPtr->unit_recno(UNIT_LAND);
    ...
    locPtr =
        world.get_loc(unitPtr->obj_loc_x2() + 1, unitPtr->obj_loc_y1() - 1);
    topRightUnit = locPtr->unit_recno(UNIT_LAND);

    if (topUnit && rightUnit &&
        topRightUnit
        ...
        // no duplicated units
        && unitRecno != topUnit && unitRecno != rightUnit &&
        unitRecno != topRightUnit && topUnit != rightUnit &&
        topUnit != topRightUnit && rightUnit != topRightUnit) {
      firmXLoc = unit_array[topUnit]->obj_loc_x1();
      firmYLoc = unit_array[topUnit]->obj_loc_y1();
      check = true;
      return true;
  ...
  if (!check && unitPtr->obj_loc_x1() > 0 &&
    ...
    Location *locPtr =
        world.get_loc(unitPtr->obj_loc_x1(), unitPtr->obj_loc_y2() + 1);
    ...
    locPtr =
        world.get_loc(unitPtr->obj_loc_x1() - 1, unitPtr->obj_loc_y2() + 1);
    bottomLeftUnit = locPtr->unit_recno(UNIT_LAND);

    if (bottomUnit && leftUnit &&
        bottomLeftUnit
        ...
        // no duplicated units
        && unitRecno != bottomUnit && unitRecno != leftUnit &&
        unitRecno != bottomLeftUnit && bottomUnit != leftUnit &&
        bottomUnit != bottomLeftUnit && leftUnit != bottomLeftUnit) {
      firmXLoc = unit_array[leftUnit]->obj_loc_x1();
      firmYLoc = unit_array[leftUnit]->obj_loc_y1();
      check = true;
      return true;
  ...
  if (!check && unitPtr->obj_loc_x2() < MAX_WORLD_X_LOC - 1 &&
    ...
    Location *locPtr =
        world.get_loc(unitPtr->obj_loc_x2(), unitPtr->obj_loc_y2() + 1);
    bottomUnit = locPtr->unit_recno(UNIT_LAND);
    ...
    locPtr =
        world.get_loc(unitPtr->obj_loc_x2() + 1, unitPtr->obj_loc_y2() + 1);
    bottomRightUnit = locPtr->unit_recno(UNIT_LAND);

    if (bottomUnit && rightUnit &&
        bottomRightUnit
        ...
        // no duplicated units
        && unitRecno != bottomUnit && unitRecno != rightUnit &&
        unitRecno != bottomRightUnit && bottomUnit != rightUnit &&
        bottomUnit != bottomRightUnit && rightUnit != bottomRightUnit) {
      firmXLoc = unit_array[unitRecno]->obj_loc_x1();
      firmYLoc = unit_array[unitRecno]->obj_loc_y1();
      check = true;
      return true;
...
//-------- Begin function
//FirmMonsterFortress::transform_from_units------------//
void FirmMonsterFortress::transform_from_units(int unitRecno, int firmXLoc,
                                               int firmYLoc, short *builderUnit,
                                               char remoteAction) {
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <unit recno> <firmXLoc> <firmYLoc> < 4 x unit recnos>
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_UNITS_TRANSFORM_MFORT, (3 + MAX_EXTRA_BUILDER) * sizeof(short));
  ...
  if (nation_array[unitPtr->nation_recno]->cash <
          firm_res[FIRM_FORTRESS]->setup_cost ||
      nation_array[unitPtr->nation_recno]->live_points <
  ...
  for (i = 0; i < MAX_EXTRA_BUILDER; ++i) {
    if (unit_array.selected_recno ==
        builderUnit[i]) // check selected_recno instead of selected_flag
      selectedFlag = 1; // because if the display interface is any of the units,
                        // display the firm interface
    unit_array[builderUnit[i]]->deinit_sprite(
        0); // 0-if the unit is currently selected, deactivate it.
  }
  ...
  int firmRecno =
      firm_array.build_firm(firmXLoc, firmYLoc, unitPtr->nation_recno,
...
// ----------//
//
// if builderArray is NULL, used by local player only (see selected_flag of
// unit)
//
bool FirmMonsterFortress::can_build_fortress(int unitRecno, int xLoc1,
                                             int yLoc1, short *builderArray) {
  err_when(!builderArray &&
           !unit_array[unitRecno]
                ->is_own()); // if builderArray is NULL, only for local player,
                             // because this func is going to read selected_flag

  FirmInfo *firmInfo = firm_res[FIRM_FORTRESS];

  // if any same nation unit inside build area, remove the unit_recno in
  // location

  if (xLoc1 < 0 || yLoc1 < 0 || xLoc1 > MAX_WORLD_X_LOC ||
      yLoc1 > MAX_WORLD_Y_LOC)
    return 0;
  ...
  case 3: // land or sea firm
    ...
    for (yLoc = yLoc1; yLoc <= yLoc2; yLoc++) {
      ...
      for (; xLoc <= xLoc2; xLoc++, locPtr++) {
        ...
        // if( locPtr->has_site() && !((firmId == FIRM_MINE || firmId ==
        // FIRM_ALCHEMY) && site_array[locPtr->site_recno()]->site_type ==
        // SITE_RAW) )
        //	return 0;
      }
    }
...
//
int FirmMonsterFortress::swap_item(int fromSoldierId, int toSoldierId,
                                   int verifyItemId, char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    // packet structure : <firm recno>, <from soldier Id>, <to Soldier Id>,
    // <verify item id>
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_CAMP_SWAP_ITEM,
                                                         4 * sizeof(short));
  ...
  if (srcItem.id &&
      !item_res.can_pick(srcItem.id, archer_unit_recno[toSoldierId - 1]))
    return 0;
  if (destItem.id &&
      !item_res.can_pick(destItem.id, archer_unit_recno[fromSoldierId - 1]))
    return 0;
