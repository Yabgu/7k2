...
void FirmMonsterFortress::init_derived()
{
    ...
    if (nation_recno && tech_res[TECH_FORTRESS_CAMOUFLAGE]->get_nation_tech_level(nation_recno))
    {
        rock_id = rock_res.search("R", obj_loc_width(), obj_loc_width(), obj_loc_height(), obj_loc_height(), 0, 0,
                                  terrain_res[world.get_loc(center_x, center_y)->terrain_id]->average_type);

        if (!rock_id)
        {
            rock_id =
                rock_res.search("PR", obj_loc_width() - 1, obj_loc_width(), obj_loc_height() - 1, obj_loc_height(), 0,
                                0, terrain_res[world.get_loc(center_x, center_y)->terrain_id]->average_type);
        }
    }
}
...
int FirmMonsterFortress::mobilize_extra_builder(int builderId)
{
    ...
    int spaceFound =
        locate_space(is_being_deleted, xLoc, yLoc, loc_x2, loc_y2, spriteInfo->loc_width, spriteInfo->loc_height);
...
int FirmMonsterFortress::mobilize_soldier(int soldierId, char remoteAction)
{
    ...
    for (int r = 0; r <= 6 && !spaceFound; r += unitPtr->loc_width)
    {
        ...
        spaceFound = world.locate_space(xLoc, yLoc, loc_x2 + r, loc_y2 + r, unitPtr->loc_width, unitPtr->loc_height,
                                        unitPtr->mobile_type);
    }
...
void FirmMonsterFortress::patrol()
{
    ...
    if (rc && nation_recno == nation_array.player_recno && firm_array.selected_recno == firm_recno)
    {
...
void FirmMonsterFortress::change_nation(int newNationRecno)
{
    ...
    for (i = 0; i < extra_builder_count; i++)
    {
        ...
        unitPtr->unit_mode = 0; // reset it first so unitPtr->change_nation() will not attempt to
...
bool FirmMonsterFortress::check_transform_from_units(int unitRecno, int &firmXLoc, int &firmYLoc, short *builderUnit)
{
    ...
    if (!check && unitPtr->obj_loc_x1() > 0 && unitPtr->obj_loc_y1() > 0)
    {
        ...
        if (topUnit && leftUnit &&
            ...
            // same unit id
            && unit_array[topUnit]->unit_id == unitPtr->unit_id && unit_array[leftUnit]->unit_id == unitPtr->unit_id &&
            unit_array[topLeftUnit]->unit_id == unitPtr->unit_id
            ...
            // no duplicated units
            && unitRecno != topUnit && unitRecno != leftUnit && unitRecno != topLeftUnit && topUnit != leftUnit &&
    ...
    if (!check && unitPtr->obj_loc_x2() < MAX_WORLD_X_LOC - 1 && unitPtr->obj_loc_y1() > 0)
    {
        ...
        if (topUnit && rightUnit &&
            ...
            // same unit id
            && unit_array[topUnit]->unit_id == unitPtr->unit_id && unit_array[rightUnit]->unit_id == unitPtr->unit_id &&
            unit_array[topRightUnit]->unit_id == unitPtr->unit_id
            ...
            // no duplicated units
            && unitRecno != topUnit && unitRecno != rightUnit && unitRecno != topRightUnit && topUnit != rightUnit &&
...
void FirmMonsterFortress::transform_from_units(int unitRecno, int firmXLoc, int firmYLoc, short *builderUnit,
                                               char remoteAction)
{
    if (!remoteAction && remote.is_enable())
    {
        ...
        short *shortPtr =
            (short *)remote.new_send_queue_msg(MSG_UNITS_TRANSFORM_MFORT, (3 + MAX_EXTRA_BUILDER) * sizeof(short));
    ...
    int firmRecno =
        firm_array.build_firm(firmXLoc, firmYLoc, unitPtr->nation_recno, FIRM_FORTRESS, buildCode, unitRecno);
    ...
    else
    {
        ...
        for (i = 0; i < MAX_EXTRA_BUILDER; ++i)
        {
            unit_array[builderUnit[i]]->init_sprite(unit_array[builderUnit[i]]->next_x_loc(),
                                                    unit_array[builderUnit[i]]->next_y_loc());
        }
    }
...
{
    err_when(!builderArray && !unit_array[unitRecno]->is_own()); // if builderArray is NULL, only for local player,
    ...
    if (unitRecno > 0)
    {
        ...
        if (unitPtr->unit_id != UNIT_ROCK || unitPtr->is_visible() && unitPtr->region_id() != destRegionId)
        {
            return 0;
        }
    }
        ...
        for (yLoc = yLoc1; yLoc <= yLoc2; yLoc++)
        {
            ...
            for (; xLoc <= xLoc2; xLoc++, locPtr++)
            {
                ...
                if (!locPtr->can_build_firm(teraMask))
                {
                    ...
                    {
                        isBuilderFlag =
                            unit_array[locUnitRecno]->selected_flag && unit_array[locUnitRecno]->unit_id == UNIT_ROCK;
                    }
