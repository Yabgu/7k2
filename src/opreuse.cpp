...
void PathReuse::find_paths()
{
    ...
    for (i = 1; i < n_units; i++)
    {
        ...
        if (cur_paths[i] && (abs(cur_paths[i]->x - dx) <= size_x[i] && abs(cur_paths[i]->y - dy) <= size_y[i]))
...
bool PathReuse::find_leader_path(int sx, int sy, int dx, int dy)
{
    ...
    if (node && node->parent)
    {
        ...
        err_when(!currentNode);          // as the source node should always be in the path,
                                         // there should always
                                         // be a valid current node present, if it is not, it
                                         // is an error.
    }
    else
    {
...
PathReuse::ReuseNode *PathReuse::search_next_unblocked_from_last_blocked(ReuseNode *blockednode,
                                                                         ReuseNode *lastUnblocked, int i)
{
...
bool PathReuse::check_occupancy(ReuseNode *node, int i)
{
    ...
    while (!occ && is < size_x[i])
    {
        ...
        {
            if (tmpLocX < 0 || tmpLocX >= MAX_WORLD_X_LOC || tmpLocY < 0 || tmpLocY >= MAX_WORLD_Y_LOC ||
                !is_location_walkable(tmpLocX, tmpLocY))
                occ = true;
            js++;
            tmpLocY++;
        }
...
PathReuse::ReuseNode *PathReuse::search_last_unblocked_node(ReuseNode *blockedNode, int i)
{
    ...
    {
        searchNode = searchNode->parent;
        err_when(searchNode == blockedNode); // that mean the node next to blocked node is not
...
PathReuse::ReuseNode *PathReuse::find_path_to_unblocked_node(ReuseNode *blockedNode, ReuseNode *firstUnblocked, int i)
{
    ...
    found =
        path_finder.find_path(blockedNode->x, blockedNode->y, destX, destY, RangeX1, RangeY1, RangeX2, RangeY2, false);
...
void PathReuse::de_occupy_destination(int sourceX, int sourceY, int destRangeX1, int destRangeY1, int destRangeX2,
...
void PathReuse::re_occupy_destination(int sourceX, int sourceY, int destRangeX1, int destRangeY1, int destRangeX2,
                                      int destRangeY2, int i)
{
    int xLoc, yLoc;
    Location *locPtr;
    ...
    for (yLoc = destRangeY1; yLoc <= destRangeY2; yLoc++)
    {
        ...
        for (xLoc = destRangeX1; xLoc <= destRangeX2; xLoc++, locPtr++, index++)
        {
            if (xLoc >= sourceX && xLoc < sourceX + size_x[i] && yLoc >= sourceY && yLoc < sourceY + size_y[i])
            {
                continue;
            }
