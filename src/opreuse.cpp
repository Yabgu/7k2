...
PathReuse::PathReuse() {
  ...
  save_loc_buf =
      (SaveLocation *)mem_add(sizeof(SaveLocation) * PATH_FIND_LOC_BUFFER_SIZE);
...
//-----------Begin of function PathReuse::~PathReuse------------//
PathReuse::~PathReuse() {
  // check whether the basic variable are valid and the corresponding flag are
  // turn on, if yes deleled memory of the basic variable
...
void PathReuse::set_handle_power_nation(int handlePowerFlag,
                                        char *nationPassable, int nationRecno) {
...
void PathReuse::find_paths() {
  ...
  for (i = 0; i < n_units; i++) {
    ...
    for (is = 0, temperoryLocationX = locX; is < size_x[i];
         is++, temperoryLocationX++) {
      for (js = 0, temperoryLocationY = locY; js < size_y[i];
           js++, temperoryLocationY++)
        world.get_loc(temperoryLocationX, temperoryLocationY)
            ->remove_unit(mobile_type); // set occupied state to false
    }
  }
  ...
  find_leader_path(leader_start_x, leader_start_y, leader_dest_x,
                   leader_dest_y);

  // note that the leader path, cur_paths[0] and hence leader_path, has already
  // been "compressed" because of this, you might not be able to check the
  // occupancy of offset of nodes "removed" from the leader path

  int leaderDifferenceX = leader_dest_x - leader_start_x;
  int leaderDifferenceY = leader_dest_y - leader_start_y;
  int maxRightOutOfRange = 0, maxBottomOutOfRange = 0, maxLeftOutOfRange = 0,
      maxTopOutOfRange = 0;
  int diff;
  ...
  for (i = 1; i < n_units; i++) {
    ...
    // setting the destination of the unit to be of the same offset as the
    // source location this can make the troop try their best to maintain their
    // troop shape
    dx = leaderDifferenceX + sx;
    dy = leaderDifferenceY + sy;
    ...
    // call a second time since there may be duplicate nodes after smoothing the
    // path.
    remove_path_to_duplicate_nodes(i);
  ...
  for (i = 0; i < n_units; i++) {
    for (is = 0, temperoryLocationX = oldLocX[i]; is < size_x[i];
         is++, temperoryLocationX++) {
      for (js = 0, temperoryLocationY = oldLocY[i]; js < size_y[i];
           js++, temperoryLocationY++)
        world.get_loc(temperoryLocationX, temperoryLocationY)
            ->set_unit(the_units[i]->sprite_recno,
                       mobile_type); // set occupied state to false
    }
  }
...
//
// This Function calculate the offset, that is, the relative position from the
// leader of all the units, so that their offset path can be found accordingly
//
...
// the detail of the pathfinding algorithm can be seen in the file pathfind.cpp
// and pathfind.h
//
// <int> sx - the x-coordinate of the source location
// <int> sy - the y-coordinate of the source location
...
// <int> dy - the y-coordinate of the destination
//
// return: <bool> - true if the leader path is found, false if the leader path
// is not found
bool PathReuse::find_leader_path(int sx, int sy, int dx, int dy) {
  ...
  path_finder.set_attribute(size_x[0], size_y[0], mobile_type,
                            PFIND_IGNORE_UNIT);
  if (handle_power_nation_flag) {
    err_when(!nation_recno);
    Nation *nationPtr = nation_array[nation_recno];
    path_finder.set_handle_power_nation(handle_power_nation_flag,
                                        nationPtr->relation_passable_array);
  } else
    path_finder.set_handle_power_nation(0);

  if (found = path_finder.find_path(sx, sy, dx, dy, RangeX1, RangeY1, RangeX2,
                                    RangeY2, false)) {
    node = path_finder.return_path_node();
  } else {
    // if the leader path cannot be found, the closest node to the destination
    // should be used.
  ...
  // this copies the path from the data structure of PathFinder to that of
  // PathReuse
  if (node && node->parent) {
    ...
    cur_paths[0] =
        currentNode; // cur_paths points to the destination of the path
    ...
    cur_paths_head[0] =
        leader_path; // cur_paths_head also points to the souurce of the path
    err_when(!currentNode); // as the source node should always be in the path,
                            // there should always
    // be a valid current node present, if it is not, it is an error.
  } else {
    // if even the closest cannot be found, we have to set all these pointer to
    // NULL
    leader_path = NULL;
    cur_paths[0] = NULL;
...
// The offset path method is used if the source offset is exactly the same as
// the destination offset, this is always true for troop, since I have set this.
// but more generally this may not be true. Another method is the join offset
// path method. The method is used whenever the source offset is not the same as
// the destination offset, and this actually seldom happen. Another method is
// the normal path finding, this should be done whenever the leader cannot be
// found, since the offset path relative to the leader path cannot be calculated
// by using the leader path, and each unit will then call pathfinding
// individually
//
// <int> sx - the x-coordinate of the source location
// <int> sy - the y-coordinate of the source location
...
// This function finds the paths of the units other than the leader by the use
// of the offset path method. This method is used whenever the destination
// offset of the unit from the leader is the same as the source offset of the
// unit from the leader. This is almost always the case, since these offset are
// set to be the same due to the troop should try to maintain its shape as close
// as possible.
//
// <int> sx - the x-coordinate of the source location
// <int> sy - the y-coordinate of the source location
...
void PathReuse::offset_path(int sx, int sy, int dx, int dy, int i) {
  ...
  while (node->next) {
    ...
    // finding the last unblocked node before the last blocking location and
    // then find a path to it if the path to the unblocked node is not valid,
    // find the second last unblocked node and so on until a path is found
    lastUnblocked = search_last_unblocked_node(node, i);
    while (lastUnblocked &&
           !(next = find_path_to_unblocked_node(currentNode, lastUnblocked, i)))
      lastUnblocked =
          search_next_unblocked_from_last_blocked(node, lastUnblocked, i);
...
//-----------------Begin of function
//PathReuse::search_next_unblocked_from_last_blocked------//
// This function searches the next unblocked node from the last blocked node,
// this is done whenever the path to the last unblocked node cannot be found,
// then the next unblocked counted from the last unblocked node should then be
// used
//
// <ReuseNode*> blockednode - this is the first blocked node in the leader path
// <ReuseNode*> lastunblocked - this is the last unblocked node found by
// search_last_unblocked_node <int> i - this is the identity of the unit in the
// unit aarray
//
// return: <ReuseNode*> - the next unblocked node count from the last blocked
// node is returned

PathReuse::ReuseNode *PathReuse::search_next_unblocked_from_last_blocked(
    ReuseNode *blockednode, ReuseNode *lastUnblocked, int i) {
...
//						- return false if the node is not
//occupied
bool PathReuse::check_occupancy(ReuseNode *node, int i) {
  ...
  is = 0;
  tmpLocX = placeX;
  // as long as one of the node in the vicinity of the size of the unit is
  // occupied the place cannot be walked by the unit
  while (!occ && is < size_x[i]) {
    js = 0;
    tmpLocY = placeY;
    while (!occ && js < size_y[i]) {
      if (tmpLocX < 0 || tmpLocX >= MAX_WORLD_X_LOC || tmpLocY < 0 ||
          tmpLocY >= MAX_WORLD_Y_LOC || !is_location_walkable(tmpLocX, tmpLocY))
        occ = true;
      js++;
      tmpLocY++;
    }
...
bool PathReuse::check_this_location_occupied(int placeX, int placeY, int i) {
  ...
  is = 0;
  tmpLocX = placeX;
  // the location cannot be walked if one of the node in the vicinity of the
  // unit size is blocked
  while (!occ && is < size_x[i]) {
    js = 0;
    tmpLocY = placeY;
    while (!occ && js < size_y[i]) {
...
//-------------------Begin of function
//PathReuse::search_last_unblocked_node-------//
// This function searches the last unblocked node of the offset path from the
// leader path this is done whenever the offset path is blocked, skip all the
// unblocked node, and find the last one. This helps to smooth the path
// <ReuseNode*> - this is the first blocked node of the offset path in the
// leader path <int> i - this is the identity of the unit in the unit array
// return: <ReuseNode*> - this return the last unblocked node
PathReuse::ReuseNode *
PathReuse::search_last_unblocked_node(ReuseNode *blockedNode, int i) {
  //------Define local variables------//
  ReuseNode *searchNode;
  searchNode = cur_paths[0]; // points to the destination

  // seeking for the first unblocked node from the tail, this can be the
  // destination
  while (check_occupancy(searchNode, i) && searchNode->parent) {
    searchNode = searchNode->parent;
  ...
  while (!check_occupancy(searchNode, i) && searchNode->parent) {
    searchNode = searchNode->parent;
    err_when(searchNode ==
             blockedNode); // that mean the node next to blocked node is not
                           // blocked. should not be here
  }
...
//-------------------Begin of function
//PathReuse::search_first_unblocked_node------//
// This function searches the first unblocked node in the offset path if the
// offset path is blocked somewhere <ReuseNode* &> - reference of the blocked
// node is input return <ReuseNode*> - the first unblocked node in the offset
// path is returned
PathReuse::ReuseNode *
PathReuse::search_first_unblocked_node(ReuseNode *&blockedNode, int i) {
  ...
  if (check_occupancy(searchNode, i) &&
      !searchNode->next) // case of destination blocked
    return NULL;
...
//-------------------Begin of function
//PathReuse::find_path_to_unblocked_node--------//
// This function finds the path from the blocked node in the offset path to the
// unblocked node of the offset path one it is found The path is found by using
// the path finding algorithm <ReuseNode*> blocked_node - this is the blocked
// node of the offset path in the leader path <ReuseNode*> firstUnblocked - this
// is the first unblocked node of the offset path in the leader path <int> i -
// this is the identity of the unit in the unit array return: <ReuseNode*>- the
// first node of the found path is returned
PathReuse::ReuseNode *
PathReuse::find_path_to_unblocked_node(ReuseNode *blockedNode,
                                       ReuseNode *firstUnblocked, int i) {
  //-------Define local variables -------//
  ...
  // do the path finding
  bool found;
  the_units[i]->get_object_range(destX, destY, RangeX1, RangeY1, RangeX2,
                                 RangeY2);
  if (handle_power_nation_flag) {
    err_when(!nation_recno);
    Nation *nationPtr = nation_array[nation_recno];
  ...
  path_finder.set_attribute(size_x[i], size_y[i], mobile_type,
                            PFIND_IGNORE_UNIT);
  found = path_finder.find_path(blockedNode->x, blockedNode->y, destX, destY,
                                RangeX1, RangeY1, RangeX2, RangeY2, false);
  ...
  err_when(unblockedPathNode->x != blockedNode->x ||
           unblockedPathNode->y != blockedNode->y);
...
//							  steps needed for walking through
//the whole path.
//
// <int> i - this is the identity of the unit in the unit array
//
...
PathResult *PathReuse::retrieve(int &pathCount, int &totalSteps, int i) {
  ...
  pathCount--;  // the last node is the source location which we don't need in
...
//
// This function allows the unit other than the leader unit to do the normal
// path searching after the path is found, it is then copy to our data structure
// for retrieve this is done whenever the leader path cannot be found <int> sx -
// the x-coordinate of the source location <int> sy - the y-coordinate of the
// source location <int> dx - the x-coordinate of the destination <int> dy - the
// y-coordinate of the destination <int> i - the identity of the unit in the
// array return: <bool> - true if the path is found, false if the path is not
// found
bool PathReuse::do_normal_path_find(int sx, int sy, int dx, int dy, int i) {
  ...
  path_finder.set_attribute(size_x[i], size_y[i], mobile_type,
                            PFIND_IGNORE_UNIT);
  found = path_finder.find_path(sx, sy, dx, dy, RangeX1, RangeY1, RangeX2,
                                RangeY2, false);
...
//------------------End of function
//PathReuse::do_normal_pathfind---------------//

// The routines below are for the purpose of smoothing the paths
//------------------Begin of function
//PathReuse::remove_path_to_duplicate_nodes-------// if a path lead to a
// repeated position, that path should be deleted
// <int> i - this is the identity of the unit in the unit array
// return <void> - there is no return variable for void function
void PathReuse::remove_path_to_duplicate_nodes(int i) {
  ReuseNode *theNode = cur_paths_head[i];
  ReuseNode *searchNode;
  // for each node from the head, if there is a corresponding node found from
  // finding the tail
  ...
  while (theNode && theNode->next) {
    ...
    while (searchNode) {
      // same node !!!
      if (searchNode->next && searchNode->x == theNode->x &&
          searchNode->y == theNode->y) {
        theNode->next = searchNode->next;
        searchNode->next->parent = theNode;
        break;
      } else if (searchNode->x == theNode->x &&
                 searchNode->y == theNode->y) // destination reached
      {
...
//-----------------End of function
//PathReuse::remove_path_to_duplicate_nodes--------------//

//-----------------Begin of function
//PathReuse::smoothing_path-----------------------//
...
void PathReuse::smoothing_path(int i) {
  ...
  while (parent = node->parent) {
    ...
    else if ((abs(vx2 - vx) == 0 && abs(vy2 - vy) == 2) ||
             (abs(vx2 - vx) == 2 && abs(vy2 - vy) == 0)) // right angle
    {
      if (!check_this_location_occupied(avgx = (parent->x + node->next->x) / 2,
                                        avgy = (parent->y + node->next->y) / 2,
                                        i)) {
...
void PathReuse::de_occupy_destination(int sourceX, int sourceY, int destRangeX1,
                                      int destRangeY1, int destRangeX2,
                                      int destRangeY2, int i) {
  int xLoc, yLoc;
  Location *locPtr;
  ...
  for (yLoc = destRangeY1; yLoc <= destRangeY2; yLoc++) {
    ...
    for (xLoc = destRangeX1; xLoc <= destRangeX2; xLoc++, locPtr++, index++) {
      if (xLoc >= sourceX &&
          xLoc < sourceX + size_x[i] && // overlap with the source location
...
void PathReuse::re_occupy_destination(int sourceX, int sourceY, int destRangeX1,
                                      int destRangeY1, int destRangeX2,
                                      int destRangeY2, int i) {
  int xLoc, yLoc;
  Location *locPtr;
  ...
  for (yLoc = destRangeY1; yLoc <= destRangeY2; yLoc++) {
    ...
    for (xLoc = destRangeX1; xLoc <= destRangeX2; xLoc++, locPtr++, index++) {
      if (xLoc >= sourceX && xLoc < sourceX + size_x[i] && yLoc >= sourceY &&
          yLoc < sourceY + size_y[i]) {
        continue;
      }
