...
void Campaign::init_nation_resource(int nationRecno, int resourceLevel)
{
    ...
    if (nationRecno == PLAYER_NATION_RECNO || (~nation_array)->get_relation_status(nationRecno) == RELATION_ALLIANCE)
    {
...
int Campaign::king_oversee_town_recno(int nationRecno)
{
    ...
    if (!kingRecno) // add a unit as the king
    {
        ...
        {
            if (!world.locate_space_random(xLoc, yLoc, MAX_WORLD_X_LOC - 1, MAX_WORLD_Y_LOC - 1, 1, 1,
...
// the
// nation. 								  (default:
// 1) [int] soldierAddCount - number of soldiers to be added to the fort. [int]
// combatLevel     - the average combat level of the soldiers in the fort.
//
int Campaign::create_nation_town(int nationRecno, int xLoc, int yLoc, int buildFort, int sameRaceTown,
                                 int soldierAddCount, int combatLevel)
{
    ...
    townPtr->init_pop(initPop, initLoyalty, 0,
    ...
    int unitRecno =
        create_unit(nationPtr->nation_recno, unitId, nationPtr->king_unit_recno == 0 ? RANK_KING : RANK_GENERAL,
...
int Campaign::create_independent_town(int xLoc, int yLoc, int raceId, int initPop, int initResistance)
{
    ...
    townPtr->init_pop(initPop, initResistance, 0,
                      1); // last 1-first initialization at the beginning of the game
...
//
int Campaign::create_monster_lair(int xLoc, int yLoc, int nationRecno, int soldierCount, int avgCombatLevel,
                                  int monsterId)
{
    ...
    int unitRecno = create_unit(nationRecno, unitId, nationPtr->king_unit_recno == 0 ? RANK_KING : RANK_GENERAL,
...
int Campaign::create_economic_firm(int townRecno)
{
    ...
    int firmRecno =
        create_firm(sitePtr->map_x_loc, sitePtr->map_y_loc, nationPtr->nation_recno, FIRM_MINE, nationPtr->race_id);
...
int Campaign::create_military_firm(int townRecno, int powerRating)
{
    ...
    if (shouldBuildArray[2] && tech_res.tech_class(TECH_CLASS_WEAPON)->total_tech_count(PLAYER_NATION_RECNO) > 0)
    {
...
// [int] unitId			- if this is given, the troop will encompass
// this unit types only. [char] useLastLocation - if this is 1, then new units
// will be created next to units created last time. Otherwise, a random location
// will be selected.
//									 (default:0)
//
...
void Campaign::create_troop2(int nationRecno, int totalUnit, int avgCombatLevel, int unitId, char useLastLocation)
{
    ...
    {
        create_unit_group(nationRecno, teamSize + misc.random(3) - 1, unitId, 1, 0, xLoc,
                          yLoc); // create two groups of monsters
...
//							  on the given edge
// only. 							  1-Top, 2-Bottom, 3-Left,
// 4-Right [int] xLoc, yLoc  - if this is given, the troop will be created
...
//			to unit_group.
//
int Campaign::create_unit_group(int nationRecno, int addCount, int unitId, int adjustCount, int whichEdge, int xLoc,
                                int yLoc)
{
    ...
    for (int i = 0; i < addCount; i++)
    {
        ...
        if ((i == 0 || nationPtr->king_unit_recno == 0) &&
            unit_res[unitId]->race_id) // don't set it to General for weapons
                                       // and other non-human units.
        {
...
void Campaign::set_nation_allied(int nationRecno1, int nationRecno2)
{
    ...
    static short neverConsiderArray[] = {TALK_END_ALLIANCE_TREATY, TALK_DECLARE_WAR, TALK_REQUEST_SURRENDER,
                                         TALK_SURRENDER, 0};
...
int Campaign::general_defect_form_new_nation(int nationRecno)
{
    ...
    for (i = 0; i < nationPtr->ai_general_count; i++)
    {
        ...
        if (unitPtr->unit_mode == UNIT_MODE_OVERSEE && unitPtr->is_human() && unitPtr->rank_id != RANK_KING)
        {
            break;
        }
    }
    ...
    int newNationRecno =
        nation_array.new_nation(NATION_HUMAN, NATION_AI, unitPtr->race_id, nation_array.random_unused_color());
...
{
    if (monster_reinforcement_count == 0 || info.game_date < next_monster_reinforcement_date)
    {
        return;
    }
    ...
    next_monster_reinforcement_date = info.game_date + (MAX_CAMPAIGN_DIFFICULTY - campaign_difficulty + 1) * 60;
}
...
void Campaign::monster_reinforcement_to_lair(int whichEdge)
{
    ...
    for (i = firm_array.size(); i > 0; i--)
    {
        ...
        if (firmLair->setup_date + (firmLair->firm_recno % 30) == info.game_date && !firmLair->under_construction &&
    ...
    int createdCount = create_unit_group(firmLair->nation_recno, createCount, 0, 0, whichEdge);
...
void Campaign::think_enemy_attack(int attackNationRecno, int targetNationRecno, int attackFrequency)
{
    ...
    for (i = unit_array.size(); i > 0; i--)
    {
        ...
        if (unitPtr->nation_recno == attackNationRecno && unitPtr->rank_id >= RANK_GENERAL && unitPtr->is_all_stop())
        {
...
int Campaign::create_monster_special_firm(int firmRecno)
{
    ...
    for (int firmId = FIRST_MONSTER_SPECIAL_FIRM; firmId <= LAST_MONSTER_SPECIAL_FIRM; firmId++)
    {
        ...
        if (firmInfo->get_build_id(firmPtr->race_id) && firmInfo->nation_can_build(nationRecno, firmPtr->race_id, 0))
        {
...
// <int> specialFirmCount			- no. of special Fryhtan structures
// to be added [int] soldierCombatLevel	   - if given, the combat level of
// soldiers in the lair will be around this number
//
int Campaign::create_lair(int campaignNationRecno, int nationRecno, int lairAddCount, int independentTownAddCount,
                          int specialFirmCount, int soldierCombatLevel)
{
    ...
    for (int i = townStateArray.size(); i > 0; i--)
    {
        ...
        if (lairAddCount > 0)
        {
            ...
            int firmRecno = create_monster_lair(xLoc, yLoc, nationRecno, soldierCount, thisCombatLevel, monsterId);
...
void Campaign::unveil_new_tech()
{
    ...
    for (i = 0; i < 10; i++) // maximum 10 tries
    {
        ...
        {
            short techClassIdArray[] = {TECH_CLASS_PRODUCTION, TECH_CLASS_INFANTRY, TECH_CLASS_WEAPON,
                                        TECH_CLASS_SPYING};
    ...
    for (i = 0; i < 10; i++) // maximum 10 tries
    {
        short techClassIdArray[] = {TECH_CLASS_PLANT,    TECH_CLASS_INSECT,     TECH_CLASS_ANIMAL, TECH_CLASS_ALCHEMY,
                                    TECH_CLASS_FORTRESS, TECH_CLASS_INCUBATION, TECH_CLASS_MAGIC};
...
int Campaign::unveil_one_tech(short *techClassIdArray, int techClassCount)
{
    ...
    for (int i = 0; i < techClassCount; i++)
    {
        ...
        {
            if (target_state_recno && get_nation(state_array[target_state_recno]->campaign_nation_recno)->is_monster())
            {
                continue;
            }
        }
...
int Campaign::add_weapon_to_camp(int campFirmRecno)
{
    ...
    TechClass *techClass = tech_res.tech_class(TECH_CLASS_WEAPON); // war_tech_class is set the which
    ...
    if (!world.locate_space(xLoc, yLoc, firmCamp->loc_x2, firmCamp->loc_y2, spriteInfo->loc_width,
...
void Campaign::create_royal_units(int campaignNationRecno)
{
    ...
    if (!is_nation_deleted(campaignNationRecno) && (cNation = get_nation(campaignNationRecno)) &&
        ...
        (kingPtr = unit_array[nationPtr->king_unit_recno])->get_cur_loc(kingXLoc, kingYLoc))
    {
        for (int i = 0; i < cNation->noble_count; ++i)
        {
            ...
            for (int r = 2; r <= 20; r += 2)
            {
                ...
                if (world.locate_space(xLoc, yLoc, MIN(kingXLoc + r, world.max_x_loc - 1),
...
{
    int nationRecno = nation_array.new_nation(NATION_HUMAN, NATION_AI, nation_array.random_unused_race(),
