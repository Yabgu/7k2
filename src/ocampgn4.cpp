...
void Campaign::init_player_resource(int resourceLevel) {
  ...
  town_array.set_all_loyalty_to_target(); // set the loyalty of all towns to
                                          // their targets.
}
...
void Campaign::init_nation_resource(int nationRecno, int resourceLevel) {
  ...
  //--- if this nation is the player nation or a nation in alliance with the
  //player, then the higher the difficulty level, the less resource the nation
  //will get ---//
  ...
  } else //--- if this nation is an enemy, the higher the difficulty level, the
         //more resource it will get ---//
  {
...
void Campaign::random_init_human_tech(int nationRecno)
{
        ...
        if( has_situation )
        {
                make_should_build_array( shouldBuildArray, MAX_INIT_TECH,
cur_situation.tech_level );
        }
        else
        {
        ...
        if( shouldBuildArray[0] )
                tech_res.tech_class(TECH_CLASS_PRODUCTION)->random_init(nationRecno,
0, 2);

        if( shouldBuildArray[1] )
                tech_res.tech_class(TECH_CLASS_SPYING)->random_init(nationRecno,
0, 2);

        if( shouldBuildArray[2] )
                tech_res.tech_class(TECH_CLASS_INFANTRY)->random_init(nationRecno,
0, 2);

        if( shouldBuildArray[3] )
        {
                tech_res.tech_class(TECH_CLASS_SPU_STRUCTURE)->random_init(nationRecno,
1, 1); tech_res.tech_class(TECH_CLASS_SPU_BERSERKER)->random_init(nationRecno,
0, 1);
        }

        if( shouldBuildArray[4] )
                tech_res.tech_class(TECH_CLASS_WEAPON)->random_init(nationRecno,
1, 3);
}
...
int Campaign::king_oversee_town_recno(int nationRecno) {
  ...
  //-- if the king is not added (This can be due to insufficient space for
  //building forts on the map --//
  ...
  if (!kingRecno) // add a unit as the king
  {
    ...
    if (i > 0) {
      ...
      if (!world.locate_space_random(xLoc, yLoc, MAX_WORLD_X_LOC - 1,
                                     MAX_WORLD_Y_LOC - 1, 1, 1,
                                     MAX_WORLD_X_LOC * MAX_WORLD_Y_LOC)) {
        return 0;
      }
    }
    ...
    kingRecno =
        create_unit(nationRecno, unitId, RANK_KING, xLoc, yLoc, xLoc, yLoc);

    if (!kingRecno)
      return 0;

    //------ since the king is not in a fort, return a town belonging to the
    //kingdom randomly ----//
...
int Campaign::create_work_firm(Place *placePtr, int firmId) {
  ...
  int firmRecno =
      create_firm_next_to_place(placePtr, firmId, nationPtr->race_id);
...
int Campaign::create_nation_town(int nationRecno, int xLoc, int yLoc,
                                 int buildFort, int sameRaceTown,
                                 int soldierAddCount, int combatLevel) {
  ...
  int townRecno =
      town_array.generate_town(nationPtr->nation_recno, raceId, xLoc, yLoc);
  ...
  if (has_situation) {
    ...
    initPop = MAX_TOWN_POPULATION * cur_situation.population_level / 100 +
              misc.random(20) - 10;
  ...
  townPtr->init_pop(
      initPop, initLoyalty, 0,
      1); // last 1-first initialization at the beginning of the game
  ...
  int firmRecno =
      create_firm_next_to_place(townPtr, FIRM_FORT, nationPtr->race_id);
  ...
  int unitRecno = create_unit(
      nationPtr->nation_recno, unitId,
      nationPtr->king_unit_recno == 0 ? RANK_KING : RANK_GENERAL,
  ...
  if (!nationPtr->king_unit_recno)
    set_king(nationRecno, unitRecno,
             1); // 1-this is the first king of the nation

  firm_array[firmRecno]->cast_to_FirmCamp()->assign_overseer(
      unitRecno); // assign the king as the overseer of the command base
  ...
  for (i = 0; i < soldierAddCount; i++)
    firmFort->recruit_soldier(townRecno, false, COMMAND_AUTO,
                              1); // 1-create at no cost
...
int Campaign::create_independent_town(int xLoc, int yLoc, int raceId,
                                      int initPop, int initResistance) {
  if (!world.can_build_town(xLoc, yLoc))
    return 0;
...
//								  (if not given, the race of
//the nation is used.)
//
// return: <int> >0  - recno of the lair created.
//					  ==0 - no lair is created.
//
int Campaign::create_monster_lair(int xLoc, int yLoc, int nationRecno,
                                  int soldierCount, int avgCombatLevel,
                                  int monsterId) {
...
int Campaign::create_economic_firm(int townRecno) {
  ...
  int firmRecno =
      create_firm(sitePtr->map_x_loc, sitePtr->map_y_loc,
                  nationPtr->nation_recno, FIRM_MINE, nationPtr->race_id);
  ...
  if (has_situation)
    firmMine->reserve_qty =
        firmMine->reserve_qty * cur_situation.mine_raw_level / 100;
  else
...
//
void Campaign::make_should_build_array(char *arrayPtr, int arraySize,
                                       int buildLevel) {
...
int Campaign::create_military_firm(int townRecno, int powerRating) {
  ...
  make_should_build_array(shouldBuildArray, MAX_BUILD_MILITARY_FIRM,
                          powerRating);

  //------- create special training structures -------//

  if (shouldBuildArray[0] &&
      firm_res[FIRM_SPECIAL]->nation_can_build(
          nationPtr->nation_recno, nationPtr->race_id, 0)) // 0-don't check cash
  {
    firmRecno =
        create_firm_next_to_place(townPtr, FIRM_SPECIAL, townPtr->race_id);
    ...
    if (firmRecno) {
      ...
      for (i = unitCount; i > 0; i--) {
        firmSpecial->recruit_trainee(townRecno, TRAIN_UNIT_SPECIAL,
                                     COMMAND_AUTO);
      }
    }
  }
  ...
  if (shouldBuildArray[2] &&
      tech_res.tech_class(TECH_CLASS_WEAPON)
              ->total_tech_count(PLAYER_NATION_RECNO) > 0) {
  ...
  if (shouldBuildArray[3] && powerRating >= 70) {
    if (firm_res[FIRM_BASE]->nation_can_build(nationPtr->nation_recno,
                                              nationPtr->race_id)) {
...
//							  on the given edge
//only. 							  1-Top, 2-Bottom, 3-Left, 4-Right
//
...
//										  be
//geographically next to nextToNation.
//
...
int Campaign::most_state_nation(int raceType, int nextToCampaignNation) {
  ...
  for (int i = nation_count(); i > 0; i--) {
    ...
    if (nextToCampaignNation &&
        !state_array.are_nation_adjacent(i, nextToCampaignNation))
      continue;
...
// [int] unitId			- if this is given, the troop will encompass this
// unit types only. [char] useLastLocation - if this is 1, then new units will
// be created next to units created last time. Otherwise, a random location will
// be selected.
//									 (default:0)
//
void Campaign::create_troop(int nationRecno, int totalUnit, int avgCombatLevel,
                            int unitId, char useLastLocation) {
  if (nationRecno == PLAYER_NATION_RECNO && this_battle_use_fryhtan) {
    //---- create a human troop of 50% of the given size ----//

    create_troop2(nationRecno, totalUnit / 2, avgCombatLevel, unitId,
                  useLastLocation); // 1/2 of the total size
    ...
    create_troop2(nationRecno, totalUnit, avgCombatLevel, unitId,
                  useLastLocation); // 1/2 of the total size
  }
  ...
  if (nationRecno == PLAYER_NATION_RECNO ||
    ...
    create_wagon(nationRecno, 60 - campaign_difficulty * 10 + misc.random(10),
                 1); // 1-useLastLocation
  }
}
...
//									 (default:0)
//
void Campaign::create_troop2(int nationRecno, int totalUnit, int avgCombatLevel,
                             int unitId, char useLastLocation) {
  int xLoc, yLoc;
  ...
  for (int i = 0; i < teamCount; i++) {
    create_unit_group(nationRecno, teamSize + misc.random(3) - 1, unitId, 1, 0,
                      xLoc, yLoc); // create two groups of monsters
...
// [char] useLastLocation - if this is 1, then new units will be created next to
// units created last time. Otherwise, a random location will be selected.
//									 (default:0)
//
void Campaign::create_wagon(int nationRecno, int totalPop,
                            char useLastLocation) {
  int xLoc, yLoc;
  ...
  for (int i = 0; i < wagonCount; i++) {
    int unitRecno = create_unit(nationRecno, UNIT_WAGON, RANK_SOLDIER, xLoc,
                                yLoc, xLoc, yLoc);
...
// <int> addCount    - number of units to be created. (the maximum is
// MAX_TEAM_MEMBER) [int] unitId 	   - id. of the unit to be created
//							  (if this is not given, the main
//unit id. of the nation will be used.)
// [int] adjustCount - if this is 1, then the addCount will be adjusted based on
// the strength
//							  of the unit. (default:
//0)
// [int] whichEdge   - if this is given, the location will be
...
int Campaign::create_unit_group(int nationRecno, int addCount, int unitId,
                                int adjustCount, int whichEdge, int xLoc,
                                int yLoc) {
  ...
  //---- if the unit is a monster, adjust addCount based on the strength of the
  //unit, more powerful units will have fewer units ----//
  ...
  for (int i = 0; i < addCount; i++) {
    ...
    if ((i == 0 || nationPtr->king_unit_recno == 0) &&
        unit_res[unitId]->race_id) // don't set it to General for weapons and
                                   // other non-human units.
    {
    ...
    int unitRecno =
        create_unit(nationRecno, unitId, rankId, xLoc, yLoc, xLoc, yLoc);
  ...
  if (unit_res[unitId]->race_id) // only form a team when there is a human
                                 // leader in the team
    unit_group.form_team(); // the unit group forms a team, with the highest
                            // ranked unit being the leader
...
void Campaign::capture_linked_place(int mainFirmRecno) {
  ...
  for (int i = 0; i < mainFirm->linked_town_count; i++) {
    ...
    townPtr->bonus_loyalty = 30; // set a bonus loyalty towards the new nation
                                 // so the town won't rebel too soon.
...
void Campaign::set_nation_hostile(int nationRecno1, int nationRecno2) {
  ...
  if (nationPtr1->nation_type == NATION_AI)
    nationPtr1->get_relation(nationRecno2)
        ->ai_never_consider[TALK_REQUEST_CEASE_WAR - 1] = 1;

  if (nationPtr2->nation_type == NATION_AI)
    nationPtr2->get_relation(nationRecno1)
        ->ai_never_consider[TALK_REQUEST_CEASE_WAR - 1] = 1;
}
...
void Campaign::set_nation_allied(int nationRecno1, int nationRecno2) {
  ...
  static short neverConsiderArray[] = {TALK_END_ALLIANCE_TREATY,
                                       TALK_DECLARE_WAR, TALK_REQUEST_SURRENDER,
                                       TALK_SURRENDER, 0};

  if (nationPtr1->nation_type == NATION_AI)
    nationPtr1->get_relation(nationRecno2)
        ->set_ai_never_consider(neverConsiderArray);
...
int Campaign::should_general_defect(int intervalDays) {
  ...
  next_general_defect_date =
      info.game_date + intervalDays + misc.random(intervalDays * 2);
  next_rebel_date =
      info.game_date + intervalDays + misc.random(intervalDays * 2);
...
int Campaign::general_defect_form_new_nation(int nationRecno) {
  ...
  for (i = 0; i < nationPtr->ai_general_count; i++) {
    ...
    if (unitPtr->unit_mode == UNIT_MODE_OVERSEE && unitPtr->is_human() &&
  ...
  int newNationRecno =
      nation_array.new_nation(NATION_HUMAN, NATION_AI, unitPtr->race_id,
                              nation_array.random_unused_color());
  ...
  //------ set vars so that the new nation will not start sending diplomatic to
  //the player nation (the nation it defected from) too soon ---//

  NationRelation *nationRelation = newNation->get_relation(nationRecno);

  for (i = 0; i < MAX_TALK_TYPE; i++) {
    nationRelation->never_accept_until_date_array[i] =
        info.game_date + misc.random(90) + 90;
  }
...
//
int Campaign::general_defect_to_other_nation(int fromNationRecno,
                                             int toNationRecno) {
  //---- determine which general is going to defect ----//
...
int Campaign::town_defect(int nationRecno, int defectToNationRecno) {
  ...
  for (int loopCount = 1; loopCount <= 2 && !bestTown; loopCount++) {
    for (int i = 0; i < nationPtr->ai_town_count; i++) {
      ...
      if (!townPtr->has_linked_own_camp ||
          loopCount == 2) // in the 2nd loop, ignore checking linked camps
      {
        bestTown = townPtr;
        break;
      }
  ...
  if (defectToNationRecno == 0 && misc.random(2) == 0) {
    ...
    for (int i = nation_array.size(); i > 0; i--) {
      ...
      if (!nation_array.is_deleted(nationRecno) && nationRecno != nationRecno &&
...
void Campaign::monster_reinforcement(int nationRecno, int whichEdge) {
  ...
  int createdCount =
      create_unit_group(nationRecno, createCount, 0, 0, whichEdge);
  ...
  next_monster_reinforcement_date =
      info.game_date + (MAX_CAMPAIGN_DIFFICULTY - campaign_difficulty + 1) * 60;
}
...
void Campaign::monster_reinforcement_to_lair(int whichEdge) {
  ...
  int createdCount =
      create_unit_group(firmLair->nation_recno, createCount, 0, 0, whichEdge);
  ...
  if (unit_group.size() > 0)
    unit_group.assign(firmLair->loc_x1, firmLair->loc_y1, UNIT_LAND,
                      COMMAND_AUTO);
...
void Campaign::create_raw_site() {
  ...
  site_array.generate_raw_site(
      siteCount); // 3 to 5 raw material deposits at the startup
}
...
//50)
//
void Campaign::think_enemy_attack(int attackNationRecno, int targetNationRecno,
                                  int attackFrequency) {
  static short thinkIntervalArray[] = {150, 80, 30, 10, 5};

  int thinkInterval = thinkIntervalArray[campaign_difficulty - 1] *
                      (110 - attackFrequency) / 50;

  if ((info.game_date + 1 - info.game_start_date) % thinkInterval >
      0) // think about it once every 5 days. So there are possibilities for the
         // AI to do something else in the interval.
    return;
  ...
  for (i = unit_array.size(); i > 0; i--) {
    ...
    if (unitPtr->nation_recno == attackNationRecno &&
        unitPtr->rank_id >= RANK_GENERAL && unitPtr->is_all_stop()) {
...
void Campaign::think_enemy_attack2(int targetNationRecno, Unit *unitPtr) {
  ...
  if (unitPtr->unit_mode == UNIT_MODE_OVERSEE) {
    FirmCamp *firmCamp =
        firm_array[unitPtr->unit_mode_para]->cast_to_FirmCamp();
...
int Campaign::create_monster_special_firm(int firmRecno) {
  ...
  for (int firmId = FIRST_MONSTER_SPECIAL_FIRM;
       firmId <= LAST_MONSTER_SPECIAL_FIRM; firmId++) {
    ...
    if (firmInfo->get_build_id(firmPtr->race_id) &&
      ...
      if (firmId ==
          FIRM_ANIMAL) // only build a maximum of 2 animal hiring structures.
      {
      ...
      return create_firm_next_to_place(firmPtr, firmId, firmPtr->race_id,
                                       nationRecno);
    }
  }
...
//							    will be made
available to the monster nation in
//								 the beginning.
//
...
beginning.
//
void Campaign::random_init_monster_tech(int monsterNationRecno, int
ownTechChance, int otherTechChance)
{
        ...
        for( int i=1 ; i<=MAX_TECH ; i++ )
        {
                ...
                {
                        int maxTechLevel =
techInfo->max_tech_level(monsterNationRecno);

                        techInfo->set_nation_tech_level( monsterNationRecno,
misc.random(maxTechLevel)+1 );

                        //---- if this tech has parent tech, also enable the
child tech ----//
...
// <int> specialFirmCount			- no. of special Fryhtan structures to
// be added [int] soldierCombatLevel	   - if given, the combat level of
// soldiers in the lair will be around this number
//
int Campaign::create_lair(int campaignNationRecno, int nationRecno,
                          int lairAddCount, int independentTownAddCount,
                          int specialFirmCount, int soldierCombatLevel) {
  //------- set the range of placements of objects -------//

  enum { TOWN_STATE_MAP_WIDTH = 100, TOWN_STATE_MAP_HEIGHT = 100 };
  ...
  for (int i = townStateArray.size(); i > 0; i--) {
    ...
    int xLoc =
        (int)stateInfo->center_x * MAX_WORLD_X_LOC / TOWN_STATE_MAP_WIDTH;
    int yLoc =
        (int)stateInfo->center_y * MAX_WORLD_Y_LOC / TOWN_STATE_MAP_HEIGHT;
    ...
    if (lairAddCount > 0) {
      ...
      //--- adjust the soldier combat level to balance power differences in
      //different Fryhtan species ---//
      ...
      thisCombatLevel =
          thisCombatLevel * stdMonsterHitPoint / unitInfo->hit_points;
      thisCombatLevel = MIN(100, thisCombatLevel);
      thisCombatLevel = MAX(20, thisCombatLevel);

      //---------------------------------------------//

      int firmRecno = create_monster_lair(xLoc, yLoc, nationRecno, soldierCount,
                                          thisCombatLevel, monsterId);
    ...
    //----- add independent towns -------//
      ...
      create_independent_town(xLoc, yLoc, raceId, initPopulation,
                              initResistance);
...
void Campaign::unveil_new_tech() {
  ...
  {
    if (misc.random(6) ==
        0) // 1/6 chance to unveil special units and special structures
    {
      if (unveil_spu_tech())
        break;
    } else {
      short techClassIdArray[] = {TECH_CLASS_PRODUCTION, TECH_CLASS_INFANTRY,
                                  TECH_CLASS_WEAPON, TECH_CLASS_SPYING};
  ...
  for (i = 0; i < 10; i++) // maximum 10 tries
  {
    short techClassIdArray[] = {TECH_CLASS_PLANT,    TECH_CLASS_INSECT,
                                TECH_CLASS_ANIMAL,   TECH_CLASS_ALCHEMY,
                                TECH_CLASS_FORTRESS, TECH_CLASS_INCUBATION,
                                TECH_CLASS_MAGIC};
  ...
  //--- unveil a specific tech (this is set in the last game that the player
  //own) ---//
...
int Campaign::unveil_one_tech(short *techClassIdArray, int techClassCount) {
  ...
  for (int i = 0; i < techClassCount; i++) {
    ...
    //------- only give new Spy tech to research if the game is against a human
    //kingdom ---//

    if (techClassId == TECH_CLASS_SPYING) {
      if (target_state_recno &&
          get_nation(state_array[target_state_recno]->campaign_nation_recno)
              ->is_monster()) {
        continue;
      }
    }
...
int Campaign::create_town(int nationRecno, int townAddCount, int hasFortPercent,
                          int combatLevel) {
  enum { TOWN_STATE_MAP_WIDTH = 100, TOWN_STATE_MAP_HEIGHT = 100 };
  ...
  for (int i = townStateArray.size(); i > 0 && townAddCount > 0; i--) {
    ...
    if (create_nation_town(nationRecno, xLoc, yLoc, buildFort, 1, 100,
                           combatLevel))
      townAddCount--;
  }
...
int Campaign::add_weapon_to_camp(int campFirmRecno) {
  ...
  TechClass *techClass =
      tech_res.tech_class(TECH_CLASS_WEAPON); // war_tech_class is set the which
                                              // tech class of weapon to build

  //---- if the player has none of the weapon technology yet, add the basic one
  //----//

  if (tech_res[techClass->first_tech_id]->get_nation_tech_level(
          firmCamp->nation_recno) == 0) {
    tech_res[techClass->first_tech_id]->set_nation_tech_level(
        firmCamp->nation_recno, 1);
  }
  ...
  for (int techId = techClass->first_tech_id;
       techId < techClass->first_tech_id + techClass->tech_count; ++techId) {
  ...
  if (!world.locate_space(xLoc, yLoc, firmCamp->loc_x2, firmCamp->loc_y2,
                          ...
                          unitInfo->mobile_type)) {
    return 0;
  }

  int unitRecno = unit_array.add_unit(weaponUnitId, firmCamp->nation_recno, 0,
                                      0, xLoc, yLoc);
...
void Campaign::rebel_form_nation() {
  ...
  nationPtr->cash =
      (float)3000 + campaign_difficulty * 3000 + misc.random(3000);
  nationPtr->food =
      (float)1500 + campaign_difficulty * 1500 + misc.random(1500);

  //------ set vars so that the new nation will not start sending diplomatic to
  //the player nation (the nation it defected from) too soon ---//

  NationRelation *nationRelation =
      nationPtr->get_relation(nation_array.player_recno);
...
void Campaign::create_royal_units(int campaignNationRecno) {
  ...
  if (!is_nation_deleted(campaignNationRecno) &&
      ...
      (kingPtr = unit_array[nationPtr->king_unit_recno])
          ->get_cur_loc(kingXLoc, kingYLoc)) {
    for (int i = 0; i < cNation->noble_count; ++i) {
      ...
      for (int r = 2; r <= 20; r += 2) {
        ...
        if (world.locate_space(
                xLoc, yLoc, MIN(kingXLoc + r, world.max_x_loc - 1),
                MIN(kingYLoc + r, world.max_y_loc - 1), spriteInfo->loc_width,
                spriteInfo->loc_height, unitInfo->mobile_type)) {
          spaceFlag = 1;
          break;
        }
      }
      ...
      if (spaceFlag) {
        if (unit->create_unit(nationPtr->nation_recno, xLoc, yLoc) > 0) {
          ...
          if (kingPtr->unit_mode == UNIT_MODE_OVERSEE &&
              firm_array[kingPtr->unit_mode_para]->can_accept_assign(
                  royalUnit->sprite_recno)) {
            firm_array[kingPtr->unit_mode_para]->assign_unit(
                royalUnit->sprite_recno);
          }
        }
      } else {
...
void Campaign::create_offensive_structure(int nationRecno, int buildCount) {
  ...
  win_to_unveil_tech_id = techId; // if the player wins the campaign game, this
                                  // tech id. will be unveiled in the next game.
  ...
  for (int i = 100; i > 0; i--) // maximum 100 tries
  {
    ...
    if (townRecno) {
      create_firm_next_to_place(town_array[townRecno], firmId,
                                nationPtr->race_id);
    ...
    while (1) {
      townRecno = random_pick_town_with_camp(
          nationRecno, 10); // only pick towns with <= 10 links
...
//
int Campaign::create_rebel_nation(int townCount, int hasFortPercent,
                                  int militaryStrength) {
  int nationRecno = nation_array.new_nation(
      NATION_HUMAN, NATION_AI, nation_array.random_unused_race(),
      nation_array.random_unused_color()); // 2nd parameter = the race id., 3rd
                                           // parameters = color scheme id.
  ...
  int townRecno2 = random_pick_town_with_camp(
      nationRecno, 2); // only pick towns with <= 2 links
