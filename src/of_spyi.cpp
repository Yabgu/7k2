...
//
void FirmSpy::detect_info() { FirmTrain::detect_info(); }
...
void FirmSpy::disp_firm_info(int dispY1, int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    button_train_inf.create(INFO_X1 + 13 + BUTTON_DISTANCE, INFO_Y1 + 281, 'A',
                            "TRAIN-I");
    button_train_inf.enable_flag = 0;
    button_train_spu.create(INFO_X1 + 13 + 2 * BUTTON_DISTANCE, INFO_Y1 + 281,
                            'A', "TRAIN-S");
  ...
  if (is_own()) {
    button_train_civ.enable_flag =
        can_recruit(active_link_town_recno, TRAIN_UNIT_CIVILIAN);
    button_train_inf.enable_flag =
        can_recruit(active_link_town_recno, TRAIN_UNIT_INFANTRY);
    button_train_spu.enable_flag =
        can_recruit(active_link_town_recno, TRAIN_UNIT_SPECIAL);
...
void FirmSpy::disp_train_menu(int refreshFlag) {
  ...
  int requiredLinkedFirmId = 0;
  int raceCount = scan_best_recruitable_town( townRecnoArray, 0,
  required_linked_firm[cur_train_type] );
  ...
  yLoyalty+font_snds.max_font_height, misc.format((int) townPtr->loyalty) );
  }
  */
}
...
void FirmSpy::detect_train_menu() {
  ...
  for( int i = 0; i < raceCount; ++i )
  {
          ...
          {
                  recruit_trainee( townRecnoArray[i], cur_train_type,
  COMMAND_PLAYER ); break;
          }
  }
  */
}
...
//
void FirmSpy::disp_spy_menu(int refreshFlag) { err_here(); }
// ------- end of function FirmSpy::disp_spy_menu ------//

// ------- begin of function FirmSpy::detect_spy_menu ------//
//
void FirmSpy::detect_spy_menu() { err_here(); }
