...
void Unit::execute_move()
{
    ...
    if (team_info && team_info->formation_id && team_info->member_count > 0)
    {
        ...
        {
            int unitRecno = team_info->member_unit_array[i]; // the leader is a member in
...
int Unit::build_firm_now()
{
    ...
    int check =
        firm_array.build_firm(cur_order.loc_x, cur_order.loc_y, nation_recno, cur_order.para, buildCode, sprite_recno);
    unit_check_build_firm_profile_time += misc.get_time() - startTime;
    return check;
#else
    return firm_array.build_firm(cur_order.loc_x, cur_order.loc_y, nation_recno, cur_order.para, buildCode,
                                 sprite_recno);
#endif
}
...
void Unit::execute_settle_town()
{
    ...
    if (locPtr->is_town())
    {
        ...
        {
            int aiActionId = cur_order.ai_action_id; // move the ai_action_id to the new action.
...
void Unit::execute_assign()
{
    ...
    if (!is_next_to_order_place())
    {
        // stop_order();		// cancel the action, as the unit cannot
...
int Unit::validate_assign()
{
    ...
    if (baseObj->obj_loc_x1() != cur_order.loc_x || baseObj->obj_loc_y1() != cur_order.loc_y)
    {
        return 0;
    }
...
void Unit::execute_go_cast_power()
{
    ...
    if (cur_action == SPRITE_IDLE)
    {
        ...
        if (misc.points_distance(next_x_loc(), next_y_loc(), cur_order.loc_x, cur_order.loc_y) <= DO_CAST_POWER_RANGE)
        {
    ...
    else if (cur_action == SPRITE_MOVE) // check idle first before move
    {
        ...
        if (cur_order.para)
        {
            if (base_obj_array.is_deleted(cur_order.para) || !base_obj_array[cur_order.para]->is_visible())
            {
                stop_order();
                return;
            }
            else
            {
                ...
                int diff = misc.points_distance(cur_order.loc_x, cur_order.loc_y, targetObj->obj_loc_x1(),
                                                targetObj->obj_loc_y1());

                if (diff > 0)
                {
                    if (misc.points_distance(move_to_loc_x, move_to_loc_y, targetObj->obj_loc_x1(),
    ...
    else if (cur_action == SPRITE_ATTACK)
    {
        ...
        if (cur_frame == cur_attack_info()->bullet_out_frame)
        {
            ...
            {
                BaseObj *targetObj;
                if (!base_obj_array.is_deleted(cur_order.para) && (targetObj = base_obj_array[cur_order.para]) &&
                    targetObj->is_visible() && area_distance(targetObj) <= DO_CAST_POWER_RANGE)
                {
                    cur_order.loc_x = targetObj->obj_loc_x1();
                    cur_order.loc_y = targetObj->obj_loc_y1();
                }
...
void Unit::execute_transform_fortress()
{
    ...
    if (FirmMonsterFortress::check_transform_from_units(sprite_recno, fortressXLoc, fortressYLoc, builderUnit))
    {
        ...
        for (i = 0; i < MAX_EXTRA_BUILDER; ++i)
        {
            ...
            if (coBuilder->cur_order.mode != cur_order.mode || coBuilder->cur_action != SPRITE_IDLE)
                break;
        }

        if (i >= MAX_EXTRA_BUILDER) // all four builder is ok
        {
            FirmMonsterFortress::transform_from_units(sprite_recno, fortressXLoc, fortressYLoc, builderUnit,
                                                      COMMAND_AUTO);
