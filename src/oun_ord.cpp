...
void Unit::execute_move() {
  ...
  if (team_info && team_info->formation_id && team_info->member_count > 0) {
    ...
    for (i = 0; i < team_info->member_count; i++) {
      int unitRecno =
          team_info->member_unit_array[i]; // the leader is a member in
    ...
    if (i == team_info->member_count &&
        team_info->formation_id == FORMATION_PRE_SQUARE) {
    ...
    if (i == team_info->member_count) {
      ...
      stop_order(); // stop order, if the members are still moving, retain the
                    // order and await all of them to stop.
    }
  } else {
    // ##### begin Gilbert 12/5 #########//
    // avoid move again when too close the the destination
    // because a unit may ask other units to move away
    if (misc.points_distance(next_x_loc(), next_y_loc(), move_to_loc_x,
                             move_to_loc_y) > STD_REACH_DEST_RANGE) {
...
//
void Unit::build_firm(int buildXLoc, int buildYLoc, int firmId,
                      char remoteAction) {
  // NEWCODEHERE
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(MSG_UNIT_BUILD_FIRM,
                                                         4 * sizeof(short));
...
void Unit::execute_build_firm() {
  ...
  //---- if the unit cannot build on the specific location or the nation cannot
  //build the firm because of lack of cash or scroll, return now ---//

  if (!world.can_build_firm(cur_order.loc_x, cur_order.loc_y, cur_order.para,
                            sprite_recno) ||
  ...
  if (!is_next_to_order_place()) {
    stop_order(); // cancel the action, as the unit cannot successfully reach
                  // the destination
    return;
  }
...
int Unit::build_firm_now() {
  ...
  int check =
      firm_array.build_firm(cur_order.loc_x, cur_order.loc_y, nation_recno,
...
void Unit::settle_town(int townXLoc, int townYLoc, char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_UNITS_SETTLE, 4 * sizeof(short));
    shortPtr[0] = townXLoc;
    shortPtr[1] = townYLoc;
...
void Unit::execute_settle_town() {
  ...
  if (locPtr->is_town()) {
    ...
    if (townPtr->nation_recno == nation_recno) {
      int aiActionId =
          cur_order.ai_action_id; // move the ai_action_id to the new action.
                                  // Reset it first so that action_failure()
                                  // won't be called
...
//
// Settle to a town. It has no remoteAction parameter as UnitGroup::assign()
// already has. So that UnitGroup::assign() should be called instead of this for
// assign action.
//
// If there is no town on the location, build one.
// If there is already one on the location, move into the town.
...
//									   (default:
//nation_recno of the unit)
//
void Unit::assign(int destXLoc, int destYLoc, int actionNationRecno,
                  bool forceMoveFlag) {
#ifdef DEBUG
  unsigned long startTime = misc.get_time();
#endif
...
// <int> destXLoc, destYLoc - the location to build
//
int Unit::set_assign(int destXLoc, int destYLoc, int actionNationRecno,
                     bool forceMoveFlag) {
  //---- no action if the unit is dead or dying ------//
...
void Unit::execute_assign() {
  ...
  if (!is_next_to_order_place()) {
    // stop_order();		// cancel the action, as the unit cannot successfully
  ...
  if (!placePtr->under_construction) {
    completed_order(); // the unit will be deleted after calling assign_unit(),
                       // so complete_order() must be called first
...
int Unit::validate_assign() {
  ...
  return baseObj->can_accept_assign(sprite_recno,
                                    cur_order.action_nation_recno);
}
...
//
void Unit::go_cast_power(int castXLoc, int castYLoc, char remoteAction,
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr =
        (short *)remote.new_send_queue_msg(MSG_U_GOD_CAST, 5 * sizeof(short));
    shortPtr[0] = sprite_recno;
    shortPtr[1] = castXLoc;
    shortPtr[2] = castYLoc;
    // shortPtr[3] = magicId;
...
//
int Unit::set_go_cast_power(int castXLoc, int castYLoc,
                            int targetBaseObjRecno) {
  //---- no action if the unit is dead or dying ------//
...
void Unit::execute_go_cast_power() {
  ...
  if (cur_action == SPRITE_IDLE) {
    ...
    if (misc.points_distance(next_x_loc(), next_y_loc(), cur_order.loc_x,
                             cur_order.loc_y) <= DO_CAST_POWER_RANGE) {
      //--- set the current attack mode to the cast spell attack mode (whose
      //attack range must be 1) ---//
      ...
      set_attack_dir(next_x_loc(), next_y_loc(), cur_order.loc_x,
                     cur_order.loc_y);
    ...
    if (cur_order.para) {
      if (base_obj_array.is_deleted(cur_order.para) ||
        ...
        int diff = misc.points_distance(cur_order.loc_x, cur_order.loc_y,
                                        targetObj->obj_loc_x1(),
                                        targetObj->obj_loc_y1());

        if (diff > 0) {
          if (misc.points_distance(
                  move_to_loc_x, move_to_loc_y, targetObj->obj_loc_x1(),
                  targetObj->obj_loc_y1()) > DO_CAST_POWER_RANGE ||
...
void Unit::execute_transform_fortress() {
  ...
  if (FirmMonsterFortress::check_transform_from_units(
          sprite_recno, fortressXLoc, fortressYLoc, builderUnit)) {
    ...
    if (i >= MAX_EXTRA_BUILDER) // all four builder is ok
    {
      FirmMonsterFortress::transform_from_units(
          sprite_recno, fortressXLoc, fortressYLoc, builderUnit, COMMAND_AUTO);
