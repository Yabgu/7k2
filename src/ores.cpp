...
// time [int]   useCommonBuf = whether use the common buffer to store the data
// or not
//                     (default:0)
//
...
char *Resource::read(int recNo) {
  ...
  if (recNo < 1 ||
      recNo > rec_count) // when no in debug mode, err_when() will be removed
    return NULL;
...
//          NULL    if the record has not index to data/all data read into
//          memory
//
File *Resource::get_file(int recNo, int &dataSize) {
  err_when(!init_flag || read_all); // if read_all, all data in file has been
                                    // read in and the file has been closed
