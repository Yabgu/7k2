...
void Game::main_loop_all(int isLoadedGame)
{
    ...
    while (1)
    {
        ...
        if (sys.signal_exit_flag == 4) // 4-this is set after loading a game for exiting the current
...
int Game::select_campaign_menu()
{
    ...
    for (i = 0; i < campaignCount; ++i)
    {
        ...
        campaignGroup[i].create(cx - w / 2 - 10, cy - h / 2 - 5, cx + w / 2 + 10, cy + h / 2 + 5, i_disp_text_button,
    ...
    for (i = 0; i < 5; ++i)
    {
        ...
        h = font_thin_black.text_height();
        campDiffGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
            ...
            if (refreshFlag)
            {
                ...
                if (optionMode == OPTION_BASIC)
                {
                    if (refreshFlag & SGOPTION_PAGE)
                    {
                        ...
                        font_bold_black.right_put(playerNameField.x, playerNameField.y, text_game_menu.str_king_name());
