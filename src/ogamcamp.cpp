...
enum { OPTION_CAMPAIGN, OPTION_BASIC, OPTION_ADVANCED };
...
void Game::run_campaign() {
  ...
  Config saveConfig =
      config; // save the original config as the campaign will overwrite it.

  int rc = select_campaign_menu();

  //---- while we save the original config, we need to overwrite it with new
  //settings the player has just chosen ---//
  ...
  while (rc > 0) {
    ...
    if (game.debug_log) {
      ...
      for (i = 0; i < cur_campaign->max_stage; i++) {
        game.debug_log->printf(
            "Stage %d: %d\n\r", i + 1,
            cur_campaign->stage_run_count_array[i]); // stage run counters
        total_stage_run_count_array[i] +=
            cur_campaign->stage_run_count_array[i];
      }
      ...
      for (i = 0; i < cur_campaign->max_event; i++) {
        game.debug_log->printf(
            "Event %d: %d\n\r", i + 1,
            cur_campaign->event_run_count_array[i]); // event run counters
        total_event_run_count_array[i] +=
            cur_campaign->event_run_count_array[i];
      }
  ...
  if (game.debug_log) {
    ...
    for (i = 0; i < maxStage; i++)
      game.debug_log->printf(
          "Stage %d: %d\n\r", i + 1,
          total_stage_run_count_array[i]); // stage run counters
    ...
    for (i = 0; i < maxEvent; i++)
      game.debug_log->printf(
          "Event %d: %d\n\r", i + 1,
          total_event_run_count_array[i]); // event run counters
...
void Game::main_loop_all(int isLoadedGame) {
  ...
  while (1) {
    ...
    if (sys.signal_exit_flag ==
        4) // 4-this is set after loading a game for exiting the current
           // main_loop() so that it can enter the correct main_loop() (campaign
           // & game) later.
    {
      isLoadedGame = 1;
      sys.signal_exit_flag = 0;
...
int Game::select_campaign_menu() {
  // static char *campaignList[] = { "Occidental Heroes" };	//, "Human
  // Learning Campaign", "Fryhtan Campaign", "Fryhtan Learning Campaign", };
  ...
  for (i = 0; i < campaignCount; ++i) {
    ...
    // cy = 136;
    cx = 390;
    cy = (136 * ((campaignCount - i) * 2 - 1) + 280 * (i * 2 + 1)) /
         (campaignCount * 2);

    w = font_thin_black.text_width(campaignList[i]);
    h = font_thin_black.text_height();
    campaignGroup[i].create(cx - w / 2 - 10, cy - h / 2 - 5, cx + w / 2 + 10,
                            cy + h / 2 + 5, i_disp_text_button,
                            ButtonCustomPara(campaignList[i], 0), 0);
  }
  ...
  for (i = 0; i < 5; ++i) {
    ...
    campDiffGroup[i].create(cx, cy, cx + w + 10, cy + h + 10,
                            i_disp_text_button,
                            ButtonCustomPara(campDiffButtonStr[i], i + 1), 0);
    cx += w + 20;
  }
  ...
  /*
          ...
          for( i = 0; i < 2; ++i )
          {
                  ...
                  spyMethodGroup[i].create( cx, cy, cx+w+10, cy+h+10,
                          i_disp_text_button, ButtonCustomPara( spyMethodStr[i],
     i), 0 ); cx += w+20;
          }
  */
  ...
  for (i = 0; i < 4; ++i) {
    ...
    randomEventGroup[i].create(cx, cy, cx + w + 10, cy + h + 6,
                               i_disp_text_button,
                               ButtonCustomPara(randomEventStr[i], i), 0);
    if ((i + 1) % 2 == 0) {
      // line feed
      cx = option3X;
      cy += 26;
    ...
    while (1) {
      ...
      if (refreshFlag) {
        ...
        if (optionMode == OPTION_BASIC) {
          if (refreshFlag & SGOPTION_PAGE) {
            ...
            font_bold_black.center_put(116, 126, 303, 146,
                                       text_game_menu.str_nationality());
            font_bold_black.center_put(382, 129, 600, 149,
                                       text_game_menu.str_color());
            font_bold_black.center_put(341, 305, 654, 324,
                                       text_game_menu.str_difficulty_level());
        ...
        if (optionMode == OPTION_ADVANCED) {
          if (refreshFlag & SGOPTION_PAGE) {
            ...
            //						font_bold_black.put_paragraph( 126, 339, option3X-10,
            //389-1, 							text_game_menu.str_spy_methodology() );
      ...
      if (optionMode == OPTION_CAMPAIGN) {
        if (campaignGroup.detect() >= 0 ||
            campaignGroup[campaignGroup()].detect(
        ...
        if (raceGroup.detect() >= 0) {
          ...
          tempConfig.campaign_difficulty =
              campDiffGroup[campDiffGroup()].custom_para.value;
