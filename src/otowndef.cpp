...
int Town::mobilize_defender(int attackerNationRecno) {
  if (population <=
      1) // do not call out defenders any more if there is only one unit left in
         // the town, otherwise the town will be gone.
    return 0;
  ...
  if (nation_recno) {
    ...
    if (!attackerNationRecno) // if independent units do not attack independent
                              // towns
      return 0;
  ...
  if (rebel_recno) // if this town is controlled by rebels
  {
    if (curLoyalty <
        town_defender_count *
            2) // rebel towns have more rebel units coming out to defend
      return 0;
  } else {
    if (curLoyalty <
        town_defender_count *
            5) // if no more defenders are allowed for the current loyalty
      return 0;
  }
  ...
  float loyaltyDec =
      2 * curLoyalty / population; // decrease in loyalty or resistance
  ...
  int unitRecno =
      mobilize_town_people(1, 0); // 1-dec pop, 0-don't mobilize spy town people
  ...
  unitPtr->skill.set_skill_level(
      (int)loyaltyDec); // if the unit is a town defender, this var is temporary
                        // used for storing the loyalty that will be added back
                        // to the town if the defender returns to the town

  int combatLevel =
      town_combat_level + misc.random(20) - 10; // -10 to +10 random difference
  ...
  if (rebel_recno)
    rebel_array[rebel_recno]
        ->mobile_rebel_count++; // increase the no. of mobile rebel units
...
void Town::reduce_defender_count() {
  ...
  if (rebel_recno)
    rebel_array[rebel_recno]
        ->mobile_rebel_count--; // decrease the no. of mobile rebel units
}
