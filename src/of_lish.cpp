...
// Filename   : OF_LISH.CPP
// Description: Class of FirmLishorr
...
enum {
  ...
  ATTACK_X_RANGE = 8,
  ATTACK_Y_RANGE = 8,
  ATTACK_ANIMATION_DELAY = 14,
  ATTACK_DELAY = 10
};

//-------- Begin of function FirmLishorr::FirmMonsterTrain ------//
//
FirmLishorr::FirmLishorr() {
  memset(sizeof(Firm) + (char *)this, 0, sizeof(FirmLishorr) - sizeof(Firm));
  animation_offset = 0;
  camouflage_flag = 0;
...
//-------- End of function FirmLishorr::FirmMonsterTrain ------//

//-------- Begin of function FirmLishorr::init ------//
void FirmLishorr::init(int xLoc, int yLoc, int nationRecno, int firmId,
                       char *buildCode, short builderRecno) {
  ...
  if (!nation_recno ||
      (nation_recno && tech_res[TECH_CAMOUFLAGE_ABILITY]->get_nation_tech_level(
                           nation_recno))) {
...
//-------- End of function FirmLishorr::init ------//

//--------- Begin of function Firm::deinit --------//
//
void FirmLishorr::deinit() {
  ...
  if (firm_build_id != base_firm_build_id) {
    ...
    firmBuild
        ->load_bitmap_res(); // load one again for deinit to free_bitmap_res
  }
  // ##### patch end Gilbert 1/9 ######//

  Firm::deinit();
}
//------ begin of function FirmLishorr::process_monster_firm -------//
//
void FirmLishorr::process_monster_firm() {
  BaseObj *targetObj;

  //	if (firm_attack_delay > 0)
  //		firm_attack_delay --;

  if (firm_id == FIRM_WILDE_LISHORR && nation_recno != 0)
    //	nation_recno = 0;
    change_nation(0);

  if (firm_attack_animation_delay == ATTACK_ANIMATION_DELAY - 4 &&
      !base_obj_array.is_deleted(target_obj_recno)) {
    ...
    else if (base_obj->cast_to_Firm())
      bullet_array.add_bullet(this, base_obj->cast_to_Firm());
    else if (base_obj->cast_to_Town())
      ...
      for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++) {
        ...
        if (locPtr->unit_recno(UNIT_LAND)) {
          ...
          if (firm_id != FIRM_WILDE_LISHORR) {
            ...
            if (unitPtr
                    ->spy_recno) // if the target unit is our spy, don't attack
            {
            ...
            if (nation_recno && unitPtr->nation_recno) {
              NationRelation *nationRelation =
                  nation_array[nation_recno]->get_relation(
                      unitPtr->nation_recno);

              if (nationRelation->status != RELATION_HOSTILE ||
                  !nationRelation->should_attack)
                continue;
            }
          }
          ...
          if ((((unitPtr->nation_recno != nation_recno) &&
                (firm_id != FIRM_WILDE_LISHORR)) ||
               ((unitPtr->nation_recno != nation_recno) &&
                (unitPtr->monster_id() != MONSTER_KHARSHUF) &&
                (firm_id == FIRM_WILDE_LISHORR))) &&
        ...
        //--- if there is an enemy firm here ---//
          ...
          if ((((firmPtr->nation_recno != nation_recno) &&
               ...
               ((firmPtr->nation_recno != nation_recno) &&
                (firmPtr->monster_id() != MONSTER_KHARSHUF) &&
                (firm_id == FIRM_WILDE_LISHORR))) &&
              (firmPtr->firm_id != FIRM_WILDE_LISHORR) &&
  ...
  if (propagate_count > 0 && firm_id == FIRM_WILDE_LISHORR) {
    short t = weather.temp_c();
    //	short tempEffect = 5 - abs( 32 - t ); //reproduce with 5 degree
    ...
    for (int firmRecno = firm_array.size(); firmRecno > 0; --firmRecno) {
      ...
      // #### patch begin Gilbert 17/12 ######//
      //			if( firmPtr->nation_recno !=
      //(~nation_array)->nation_recno ) 				continue;
      // #### patch end Gilbert 17/12 ######//
      if (firmPtr->firm_id == FIRM_WILDE_LISHORR)
        chance += 10;
    }

    //### begin trevor 24/3 ####//
    if (misc.random(chance) < t &&
        (info.game_month == 2 || info.game_month == 7)) {
      Firm *firmPtr = firm_array[firm_recno];
      int firmRecno = create_firm_next_to_place(firmPtr, FIRM_WILDE_LISHORR,
                                                -MONSTER_KHARSHUF, 0, 6);
      if (firmRecno)
        propagate_count--;
    }
    //### end trevor 24/3 ####//
...
//------ End of function FirmLishorr::process_monster_firm -------//

//------ Begin of function FirmLishorr::attack_target -------//
int FirmLishorr::attack_target(BaseObj *targetObj) {
  ...
  if (targetObj->cast_to_Unit()) {
    ...
    animation_offset =
        1 + get_dir(center_x, center_y, ((Unit *)targetObj)->next_x_loc(),
                    ((Unit *)targetObj)->next_y_loc());
    ...
    animation_offset =
        1 + get_dir(center_x, center_y, ((Firm *)targetObj)->center_x,
                    ((Firm *)targetObj)->center_y);
    ...
    animation_offset =
        1 + get_dir(center_x, center_y, ((Town *)targetObj)->center_x,
                    ((Town *)targetObj)->center_y);
  } else
    err_here();
...
//------ End of function FirmLishorr::attack_target -------//

//------ begin of function FirmLishorr::return_fire -------//
//
int FirmLishorr::return_fire(BaseObj *targetObj) {
...
//------ end of function FirmLishorr::return_fire -------//

//------ begin of function FirmLishorr::bullet_damage -------//

float FirmLishorr::bullet_damage() {
  int weatherFactor = 1;
  ...
  if (GAME_VERSION <= 101) {
    if (nation_recno)
      return ((LISHORR_BULLET_DAMAGE *
               (6 + (tech_res[TECH_LISHORR]->get_nation_tech_level(nation_recno)
                     << weatherFactor))) >>
              3);
    else
...
//------ begin of function FirmLishorr::bullet_radius -------//

short FirmLishorr::bullet_radius() { return LOCATE_WIDTH - 2; }
//------ end of function FirmLishorr::bullet_radius -------//

//------ begin of function FirmLishorr::bullet_fire -------//

char FirmLishorr::bullet_fire() { return 0; }
//------ end of function FirmLishorr::bullet_fire -------//

//------ begin of function FirmLishorr::bullet_id -------//

short FirmLishorr::bullet_id() {
  FirmBuild *firmBuild = firm_res.get_build(base_firm_build_id);

  return firmBuild->bullet_sprite_id;
}
//------ end of function FirmLishorr::bullet_id -------//

//------ begin of function FirmLishorr::bullet_speed -------//

short FirmLishorr::bullet_speed() { return LISHORR_BULLET_SPEED; }
//------ end of function FirmLishorr::bullet_speed -------//

//------ begin of function FirmLishorr::bullet_init_z -------//

short FirmLishorr::bullet_init_z() { return altitude; }
//------ end of function FirmLishorr::bullet_init_z -------//

//------ Begin of function FirmLishorr::get_dir -------//
//
// Compare the current and destination locations and
// return which direction the sprite should move towards or attack at.
//
int FirmLishorr::get_dir(int curX, int curY, int destX, int destY) {
  ...
  if (destX == curX) {
    ...
    // west side
    if (destY > curY) { // south west quadrant
      if (2 * xDiff <= squSize)
        return 7; // DIR_S;
  ...
  {
    // east side
    if (destY > curY) { // south east quadrant
      if (2 * xDiff <= squSize)
        return 7; // DIR_S;
      else if (2 * yDiff <= squSize)
        return 1; // DIR_E;
...
//------- End of function FirmLishorr::get_dir --------//

// -------- begin of function FirmLishorr::draw ---------//
void FirmLishorr::draw(int displayLayer) {
  if (!plant_id || camouflage_flag) {
    ...
    world.zoom_matrix->put_bitmap_offset(
        loc_x2 * LOCATE_WIDTH, loc_y2 * LOCATE_HEIGHT,
        world.interpolate_z(loc_x2 * LOCATE_WIDTH, loc_y2 * LOCATE_HEIGHT),
        plantBitmap->bitmap_ptr, plantBitmap->offset_x, plantBitmap->offset_y,
        NULL, 0, 1);
    //	plantBitmap->draw(center_x, center_y);
    //	rock_res.draw(rock_id, loc_x1, loc_y1, altitude, 1 );
  }
}
// -------- end of function FirmLishorr::draw ---------//

//----------- Begin of function FirmLishorr::find_best_place_loc ----------//
//
...
int FirmLishorr::find_best_place_loc(short buildFirmId, short refXLoc,
                                     short refYLoc, short &resultXLoc,
                                     short &resultYLoc, short distance) {
#define IDEAL_INTER_PLACE_SPACE 3
  ...
  refX1 -= placeLocWidth /
           2; // since we use loc_x1 as the building reference, we need to shift
              // it so it will match the use of center_x in effective distance
  ...
  int xLoc, yLoc; // inner locations in the matrix receives more weights than
                  // outer locations do
  int t1, t2;
  ...
  for (yLoc = refY1; yLoc <= refY2; yLoc++) {
    ...
    for (xLoc = refX1; xLoc <= refX2; xLoc++, refMatrixPtr++, locPtr++) {
      ...
      if (locPtr->region_id != buildRegionId ||
          locPtr->is_plateau() != buildIsPlateau || locPtr->is_power_off()) {
        *refMatrixPtr = -1000;
      } else {
        *refMatrixPtr = 10 - MAX(t1, t2); // it's negative value, and the value
                                          // is lower for the outer ones
      }
    }
  }
  ...
  for (yLoc = refY1; yLoc <= refY2; yLoc++) {
    ...
    for (xLoc = refX1; xLoc <= refX2; xLoc++, locPtr++) {
      ...
      if (locPtr->is_firm()) {
        ...
        if (buildFirmId == FIRM_MARKET ||
            buildFirmId == FIRM_FACTORY) // only factories & market places need
                                         // building close to other firms
        {
          int rc = 1;
          ...
          if (buildFirmId ==
              FIRM_MARKET) // build a market place close to mines and factories
          {
            if (firmPtr->firm_id != FIRM_MINE &&
                firmPtr->firm_id !=
                    FIRM_FACTORY) // market places should be built close to
                                  // factories and mines and they are the only
                                  // two firms that influence the location of
                                  // the market place
              rc = 0;
          } else if (buildFirmId == FIRM_FACTORY) // build a factory close to
                                                  // mines and market places
          {
            if (firmPtr->firm_id != FIRM_MINE &&
                firmPtr->firm_id !=
                    FIRM_MARKET) // market places should be built close to
                                 // factories and mines and they are the only
                                 // two firms that influence the location of the
                                 // market place
              rc = 0;
          }
          ...
          if (rc) {
            effectiveDis =
                world.effective_distance(firmPtr->firm_id, buildFirmId);
        ...
        refCX1 = firmPtr->loc_x1 -
                 distance; // add negative weights on space around this firm
        refCY1 = firmPtr->loc_y1 - distance; // so to prevent firms from
                                             // building right next to the firm
        refCX2 = firmPtr->loc_x2 +
                 distance; // and leave some space for walking path.
      ...
      //------- if there is a town on the location ------//
        ...
        if (townPtr->nation_recno != nation_recno) {
          if (townPtr->nation_recno == 0) // it's an independent town
            weightAdd = weightAdd *
                        (100 - (int)townPtr->resistance(nation_recno)) / 100;
          else // more friendly nations get higher weights
          {
            ...
            //		weightAdd = weightAdd * (relationStatus-RELATION_NEUTRAL+1) /
            //4;
          }
        }

        refCX1 = townPtr->loc_x1 -
                 distance; // add negative weights on space around this firm
        refCY1 = townPtr->loc_y1 - distance; // so to prevent firms from
                                             // building right next to the firm
        refCX2 = townPtr->loc_x2 +
                 distance; // and leave some space for walking path.
      ...
      if (weightAdd) {
        for (yLocB = MAX(refY1, refBY1); yLocB <= MIN(refY2, refBY2); yLocB++) {
          ...
          refMatrixPtr =
              refMatrix + (yLocB - refY1) * refWidth + (xLocB - refX1);
...
//-------- End of function FirmLishorr::find_best_place_loc --------//

//-------- Begin of function FirmLishorr::create_firm_next_to_place -------//
int FirmLishorr::create_firm_next_to_place(Place *placePtr, int firmId,
                                           int firmRaceId, int nationRecno,
                                           short distance) {
  short buildXLoc, buildYLoc;

  if (!find_best_place_loc(firmId, placePtr->loc_x1, placePtr->loc_y1,
                           buildXLoc, buildYLoc, distance))
    return 0;
  ...
  int firmRecno = firm_array.build_firm(
      buildXLoc, buildYLoc, nationRecno, firmId, buildCode, 0, false,
      true); // true-no cost for building the firm
...
//-------- End of function FirmLishorr::create_firm_next_to_place -------//

//-------- Begin of function FirmLishorr::right_click -------//
//
// This function is called when the current firm is selected
// and the mouse right-click on a locatoin.
//
void FirmLishorr::right_click(int xLoc, int yLoc) {
  if (firm_id == FIRM_WILDE_LISHORR)
    return;
  ...
  if (base_obj->cast_to_Unit() || base_obj->cast_to_Firm() ||
      base_obj->cast_to_Town()) {
    if (!remote.is_enable())
      return_target_obj_recno = baseObjRecno;
    else {
      // packet structure : <firm recno> <base obj recno>
      short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_LISH_ATTACK,
                                                           2 * sizeof(short));
...
//-------- End of function FirmLishorr::right_click -------//

//-------- Begin of function FirmLishorr::is_visible -------//
int FirmLishorr::is_visible(int nationRecno) {
  if ((firm_id == FIRM_LISHORR && nation_recno != nationRecno && plant_id) ||
      (firm_id == FIRM_WILDE_LISHORR && plant_id))
    return 0;

  return 1;
}
//-------- End of function FirmLishorr::is_visible -------//
