...
void FirmLishorr::init(int xLoc, int yLoc, int nationRecno, int firmId, char *buildCode, short builderRecno)
{
...
void FirmLishorr::process_monster_firm()
{
    ...
    if (firm_attack_animation_delay == ATTACK_ANIMATION_DELAY - 4 && !base_obj_array.is_deleted(target_obj_recno))
    {
            ...
            for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++)
            {
                ...
                if (locPtr->unit_recno(UNIT_LAND))
                {
                    ...
                    if (firm_id != FIRM_WILDE_LISHORR)
                    {
                        ...
                        if (unitPtr->spy_recno) // if the target unit is our spy, don't attack
                        {
                            ...
                            if (spyPtr->true_nation_recno == nation_recno && spyPtr->notify_cloaked_nation_flag == 0)
                                continue;
                        }
                        ...
                        if (nation_recno && unitPtr->nation_recno)
                        {
                            NationRelation *nationRelation =
                                nation_array[nation_recno]->get_relation(unitPtr->nation_recno);
                    ...
                    if ((((unitPtr->nation_recno != nation_recno) && (firm_id != FIRM_WILDE_LISHORR)) ||
                         ((unitPtr->nation_recno != nation_recno) && (unitPtr->monster_id() != MONSTER_KHARSHUF) &&
                ...
                else if (locPtr->is_firm())
                {
                    ...
                    if (firm_id != FIRM_WILDE_LISHORR)
                    {
                        ...
                        {
                            NationRelation *nationRelation =
                                nation_array[nation_recno]->get_relation(firmPtr->nation_recno);
                    ...
                    if ((((firmPtr->nation_recno != nation_recno) && (firm_id != FIRM_WILDE_LISHORR)) ||
                         ((firmPtr->nation_recno != nation_recno) && (firmPtr->monster_id() != MONSTER_KHARSHUF) &&
                          (firm_id == FIRM_WILDE_LISHORR))) &&
                        (firmPtr->firm_id != FIRM_WILDE_LISHORR) && (firmPtr->firm_id != FIRM_LISHORR))
...
int FirmLishorr::attack_target(BaseObj *targetObj)
{
    ...
    {
        if (((Unit *)targetObj)->next_x_loc() < xLoc1 || ((Unit *)targetObj)->next_x_loc() > xLoc2 ||
            ((Unit *)targetObj)->next_y_loc() < yLoc1 || ((Unit *)targetObj)->next_y_loc() > yLoc2 ||
            ((Unit *)targetObj)->unit_id == UNIT_CARAVAN || ((Unit *)targetObj)->nation_recno == nation_recno)
            return 0;

        animation_offset =
            1 + get_dir(center_x, center_y, ((Unit *)targetObj)->next_x_loc(), ((Unit *)targetObj)->next_y_loc());
    }
    else if (targetObj->cast_to_Firm())
    {
        if (((Firm *)targetObj)->center_x < xLoc1 || ((Firm *)targetObj)->center_x > xLoc2 ||
            ((Firm *)targetObj)->center_y < yLoc1 || ((Firm *)targetObj)->center_y > yLoc2 ||
            ((Firm *)targetObj)->nation_recno == nation_recno)
            return 0;

        animation_offset =
            1 + get_dir(center_x, center_y, ((Firm *)targetObj)->center_x, ((Firm *)targetObj)->center_y);
    }
    else if (targetObj->cast_to_Town())
    {
        if (((Town *)targetObj)->center_x < xLoc1 || ((Town *)targetObj)->center_x > xLoc2 ||
            ((Town *)targetObj)->center_y < yLoc1 || ((Town *)targetObj)->center_y > yLoc2 ||
            (nation_recno && ((Town *)targetObj)->resistance(nation_recno) <= 0))
            return 0;

        animation_offset =
            1 + get_dir(center_x, center_y, ((Town *)targetObj)->center_x, ((Town *)targetObj)->center_y);
    }
    else
        err_here();
...
short FirmLishorr::bullet_radius()
{
...
short FirmLishorr::bullet_speed()
{
    return LISHORR_BULLET_SPEED;
}
//------ end of function FirmLishorr::bullet_speed -------//

//------ begin of function FirmLishorr::bullet_init_z -------//

short FirmLishorr::bullet_init_z()
{
    return altitude;
}
...
int FirmLishorr::find_best_place_loc(short buildFirmId, short refXLoc, short refYLoc, short &resultXLoc,
...
#define IDEAL_INTER_PLACE_SPACE 3
    ...
    refX1 -= placeLocWidth / 2; // since we use loc_x1 as the building reference, we need to shift
    ...
    for (yLoc = refY1; yLoc <= refY2; yLoc++)
    {
        ...
        for (xLoc = refX1; xLoc <= refX2; xLoc++, refMatrixPtr++, locPtr++)
        {
            ...
            if (locPtr->region_id != buildRegionId || locPtr->is_plateau() != buildIsPlateau || locPtr->is_power_off())
            {
                *refMatrixPtr = -1000;
            }
            else
    ...
    for (yLoc = refY1; yLoc <= refY2; yLoc++)
    {
        ...
        for (xLoc = refX1; xLoc <= refX2; xLoc++, locPtr++)
        {
            ...
            if (locPtr->is_firm())
            {
                ...
                if (buildFirmId == FIRM_MARKET || buildFirmId == FIRM_FACTORY) // only factories & market places need
                                                                               // building close to other firms
                {
                    ...
                    if (buildFirmId == FIRM_MARKET) // build a market place close to mines and factories
                    {
                        if (firmPtr->firm_id != FIRM_MINE &&
                            firmPtr->firm_id != FIRM_FACTORY) // market places should be built close to
                    ...
                    {
                        if (firmPtr->firm_id != FIRM_MINE &&
                            firmPtr->firm_id != FIRM_MARKET) // market places should be built close
                                                             // to factories and mines and they are
                                                             // the only two firms that influence
...
int FirmLishorr::create_firm_next_to_place(Place *placePtr, int firmId, int firmRaceId, int nationRecno, short distance)
{
    ...
    int firmRecno = firm_array.build_firm(buildXLoc, buildYLoc, nationRecno, firmId, buildCode, 0, false,
...
void FirmLishorr::right_click(int xLoc, int yLoc)
{
    ...
    if (base_obj->cast_to_Unit() || base_obj->cast_to_Firm() || base_obj->cast_to_Town())
    {
        ...
        else
        {
            ...
            short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_LISH_ATTACK, 2 * sizeof(short));
