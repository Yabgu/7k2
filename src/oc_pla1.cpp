...
void CampaignEastWest::plot_a1_next_day()
{
    ...
    if (playerNation->kill_monster_score >= 100 &&
        nationRelation->ai_never_consider[TALK_PROPOSE_ALLIANCE_TREATY - 1]) // check ai_never_cosinder[] so that
                                                                             ...
                                                                             // alliance treaty is only done once
    {
        ...
        static short neverConsiderArray[] = {TALK_END_TRADE_TREATY, TALK_END_FRIENDLY_TREATY, TALK_END_ALLIANCE_TREATY,
                                             0};

        nationRelation->set_ai_never_consider(neverConsiderArray);

        talk_res.ai_send_talk_msg(playerNation->nation_recno, plot_nation_recno1, TALK_PROPOSE_ALLIANCE_TREATY);
    }

    //-- if the player's Fryhtan battle score > 300 and is allied with the rebel,
    // the rebel will surrender to the player --//

    if (playerNation->kill_monster_score >= 300 && nationRelation->status == RELATION_ALLIANCE)
    {
        nationRelation->ai_never_consider[TALK_SURRENDER - 1] = 0;

        talk_res.ai_send_talk_msg(playerNation->nation_recno, plot_nation_recno1, TALK_SURRENDER);
