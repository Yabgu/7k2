...
void CampaignEastWest::plot_a1_next_day() {
  ...
  //--- the rebel will propose an alliance treaty to the player if the player's
  //Fryhtan battle score > 100 ---//

  if (playerNation->kill_monster_score >= 100 &&
      nationRelation->ai_never_consider[TALK_PROPOSE_ALLIANCE_TREATY -
                                        1]) // check ai_never_cosinder[] so that
                                            // the action of proposing an
                                            // alliance treaty is only done once
  {
    nationRelation->ai_never_consider[TALK_PROPOSE_ALLIANCE_TREATY - 1] = 0;

    static short neverConsiderArray[] = {TALK_END_TRADE_TREATY,
                                         TALK_END_FRIENDLY_TREATY,
                                         TALK_END_ALLIANCE_TREATY, 0};
  ...
  //-- if the player's Fryhtan battle score > 300 and is allied with the rebel,
  //the rebel will surrender to the player --//
