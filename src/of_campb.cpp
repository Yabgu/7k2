...
int FirmCamp::can_spy_bribe(int bribeSoldierId, int briberNationRecno) {
  ...
  if (spyRecno) {
    canBribe = spy_array[spyRecno]->true_nation_recno !=
               briberNationRecno; // only when the unit is not yet a spy of the
                                  // player. Still display the bribe button when
                                  // it's a spy of another nation
  } else {
    if (bribeSoldierId) {
      canBribe = soldier_array[bribeSoldierId - 1]
                     .is_human(); // cannot bribe if it's a weapon
    } else {
      canBribe = unit_array[overseer_recno]->is_human() &&
                 unit_array[overseer_recno]->rank_id !=
                     RANK_KING; // cannot bribe a king
    }
  }
...
//soldier.
//
// return: <int> >0 - bribing succeeded, return the spy recno of the bribed unit
// (as it has been turned into a spy)
//					  0 - bribing failed
//
int FirmCamp::spy_bribe(int bribeAmount, short briberSpyRecno,
                        short soldierId) {
  if (!can_spy_bribe(
          soldierId,
          spy_array[briberSpyRecno]
              ->true_nation_recno)) // this can happen in multiplayer as there
                                    // is a one frame delay when the message is
                                    // sent and when it is processed
    return 0;

  //---------------------------------------//

  int succeedChance =
      spy_bribe_succeed_chance(bribeAmount, briberSpyRecno, soldierId);

  Spy *spyPtr = spy_array[briberSpyRecno];

  nation_array[spyPtr->true_nation_recno]->add_expense(EXPENSE_BRIBE,
                                                       (float)bribeAmount, 0);
  ...
  if (succeedChance > 0 && misc.random(100) < succeedChance) {
    ...
    newSpy->spy_loyalty =
        MIN(100, MAX(30, succeedChance)); // within the 30-100 range
    ...
    if (soldierId) {
      ...
      if (!newSpy->name_id) // if this soldier does not have a name, give him
                            // one now as a spy must reserve a name (see below
                            // on use_name_id() for reasons)
    ...
    //-- Spy always registers its name twice as his name will be freed up in
    //deinit(). Keep an additional right because when a spy is assigned to a
    //town, the normal program will free up the name id., so we have to keep an
    //additional copy
  ...
  {
    int spyTrueNationRecno =
        spyPtr->true_nation_recno; // keep before spy killed

    spyPtr->get_killed(
        0); // the spy gets killed when the action failed.
            // 0 - don't display new message for the spy being killed, so we
            // already display the msg on the interface

    if (firm_recno == firm_array.selected_recno &&
        spyTrueNationRecno == nation_array.player_recno) {
      // ----- for showing bribe result on the briber machine ------//
...
//					  0 - bribing failed
//
int FirmCamp::spy_bribe_succeed_chance(int bribeAmount, short briberSpyRecno,
                                       short soldierId) {
  Spy *spyPtr = spy_array[briberSpyRecno];
  ...
  {
    err_when(spy_array[targetSpyRecno]->true_nation_recno ==
             spyPtr->true_nation_recno); // the player shouldn't be able to
                                         // bribe units of his own

    succeedChance = 0;
  } else {
    int techLevel = 0;
    if (spyPtr->true_nation_recno)
      techLevel = tech_res[TECH_BRIBE]->get_nation_tech_level(
          spyPtr->true_nation_recno);
    // ##### patch begin Gilbert 16/2 #####//
    if (god_res[GOD_PERSIAN]->nation_prayer_count(spyPtr->true_nation_recno) >
        0)
      techLevel++;
    ...
    //-- the chance is higher if the spy or the spy's king is racially
    //homongenous to the bribe target,
    ...
    //		if( unitLoyalty > 60 )			// harder for bribe units with over 60
    //loyalty 			succeedChance -= (unitLoyalty-60); 		if( unitLoyalty > 70 )
    //// harder for bribe units with over 70 loyalty 			succeedChance -=
    //(unitLoyalty-70); 		if( unitLoyalty > 80 )			// harder for
    //bribe units with over 80 loyalty 			succeedChance -= (unitLoyalty-80); 		if(
    //unitLoyalty > 90 )			// harder for bribe units with over 90
    //loyalty 			succeedChance -= (unitLoyalty-90);
    if (unitLoyalty > 60)
  ...
  if (bribe_result == BRIBE_NONE) {
    ...
    for (int i = 0; i < BRIBE_AMOUNT_COUNT; i++) {
      if (refreshFlag == INFO_REPAINT)
        button_bribe_amount[i].create(
            INFO_X1 + 18, y, INFO_X2 - 14, y + BRIBE_OPTION_HEIGHT - 1,
            i_disp_bribe_button, ButtonCustomPara(NULL, bribe_amount_array[i]),
            0);

      button_bribe_amount[i].enable_flag =
          (~nation_array)->cash >= bribe_amount_array[i];
    ...
    if (refreshFlag == INFO_REPAINT) {
      ...
      spy_action_chance =
          spy_bribe_succeed_chance(bribe_amount_array[button_bribe_amount()],
    ...
    if (config.show_debug_info) {
      ...
      font_snds.put(INFO_X1 + 18, y + 5,
                    text_firm.str_bribe_chance(spy_action_chance));
    }

    if (refreshFlag == INFO_REPAINT) {
      button_go.create(INFO_X1 + 13, INFO_Y1 + 281, 'A', "BRIBE");
    }
    button_go.enable_flag =
        (~nation_array)->cash >= bribe_amount_array[button_bribe_amount()];
    button_go.paint();
  } else {
    err_here();
  }
...
void FirmCamp::detect_bribe_menu() {
  ...
  if (bribe_result == BRIBE_NONE) {
    if (button_go.detect()) {
      ...
      if (!remote.is_enable()) {
        spy_bribe(bribe_amount_array[button_bribe_amount()], action_spy_recno,
                  action_target_recno);
        action_spy_recno = 0;
      } else {
        // packet structure <firm recno> <spy recno> <bribe target : worker
        // (0=overseer)> <amount>
        short *shortPtr = (short *)remote.new_send_queue_msg(MSG_FIRM_BRIBE,
                                                             4 * sizeof(short));
        *shortPtr = firm_recno;
    ...
    if ((rc = button_bribe_amount.detect()) >= 0) {
      // update chance
      spy_action_chance = spy_bribe_succeed_chance(
          bribe_amount_array[rc], action_spy_recno, action_target_recno);
      if (!config.show_debug_info) {
        spy_action_chance = MIN(spy_action_chance, 0);
        spy_action_chance = MAX(spy_action_chance, 100);
      }
...
int FirmCamp::validate_cur_bribe() {
  if (spy_array.is_deleted(action_spy_recno) ||
      spy_array[action_spy_recno]->true_nation_recno !=
          nation_array.player_recno) {
    return 0;
  }

  //	return can_spy_bribe( selected_soldier_id,
  //spy_array[action_spy_recno]->true_nation_recno );
  return can_spy_bribe(action_target_recno,
                       spy_array[action_spy_recno]->true_nation_recno);
}
...
static void i_disp_bribe_button(ButtonCustom *button, int) {
  ...
  // ######### begin Gilbert 19/2 ##########//
  if (!button->pushed_flag)
    font_bld.center_put(button->x1, button->y1, button->x2, button->y2 - 2,
                        str);
  else
  ...
  if (!button->enable_flag) {
    vga.active_buf->bar_alpha(button->x1, button->y1, button->x2, button->y2, 2,
                              V_BLACK);
  }
}
...
void FirmCamp::disp_bribe_unit(int dispY1) {
  ...
  if (action_target_recno) {
    ...
    unitName =
        unit_res[unitId]->name; // don't display soldier name, can spot spy
...
void FirmCamp::disp_bribe_result(int refreshFlag) {
  ...
  case BRIBE_SUCCEED:
    ...
    font_whbl.put(x, y,
                  text_firm.str_bribe_success()); // "Bribing Succeeded." );
    break;
  ...
  case BRIBE_FAIL:
    ...
    // font_whbl.put( x, y, text_firm.str_bribe_execute() ); //"Your Spy Was
    // Caught And Executed." );
    font_whbl.put_paragraph(x, y, INFO_X2 - 14, INFO_Y2,
                            text_firm.str_bribe_execute());
    break;
...
void FirmCamp::disp_assassinate_confirm(int refreshFlag) {
  if (refreshFlag == INFO_REPAINT) {
    button_yes.create_text(INFO_X1 + 50, INFO_Y1 + 100,
                           text_firm.str_yes()); // "Yes" );
    button_no.create_text(INFO_X1 + 155, INFO_Y1 + 100,
                          text_firm.str_no()); // "No" );
  }
  ...
  if (overseer_recno && !unit_array.is_deleted(overseer_recno)) {
    ...
    str = text_firm.str_ask_assassin(
        text_unit.str_title(overseerPtr->race_id, overseerPtr->rank_id));
    font_whbl.center_put_paragraph(INFO_X1 + 40, INFO_Y1, INFO_X2 - 30,
                                   INFO_Y1 + 100, str, 2, 0, 0);
  }
  ...
  font_whbl.center_put_paragraph(INFO_X1 + 20, INFO_Y1 + 80, INFO_X2 - 10,
                                 INFO_Y1 + 100, str, 2, 0, 0);
...
void FirmCamp::disp_assassinate_result(int refreshFlag) {
  ...
  if (assassinate_result == ASSASSINATE_SUCCEED_AT_LARGE) {
    vga.active_buf->put_bitmap(INFO_X1, INFO_Y1, image_gameif.read("MISSBASE"));
    // font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1, INFO_X2 -30,
    // INFO_Y1 +100, "Assassination Succeeded.", 2, 0, 0);
    // font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1 +100, INFO_X2 -30,
    // INFO_Y1 +200, "Your Spy Escaped.", 2, 0, 0);
    // font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1, INFO_X2 -30,
    // INFO_Y1 +100, text_firm.str_assassin_success(), 2, 0, 0);
    // font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1 +100, INFO_X2 -30,
    // INFO_Y1 +200, text_firm.str_assassin_escape(), 2, 0, 0);
    font_whbl.put(
        INFO_X1 + 20, INFO_Y1 + 16,
        text_firm.str_assassin_success()); // "Assassination Succeeded." );
    font_whbl.put_paragraph(
        INFO_X1 + 20, INFO_Y1 + 36, INFO_X2 - 14, INFO_Y1 + 76,
    ...
    // INFO_Y1 +100, "Assassination Succeeded.", 2, 0, 0);
    // font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1 +100, INFO_X2 -30,
    // INFO_Y1 +200, "Your Spy Was Caught And Executed.", 2, 0, 0);
    // font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1, INFO_X2 -30,
    // INFO_Y1 +100, text_firm.str_assassin_success(), 2, 0, 0);
    // font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1 +100, INFO_X2 -30,
    // INFO_Y1 +200, text_firm.str_assassin_executed(), 2, 0, 0);
    font_whbl.put(
        INFO_X1 + 20, INFO_Y1 + 16,
        text_firm.str_assassin_success()); // "Assassination Succeeded." );
    font_whbl.put_paragraph(
        INFO_X1 + 20, INFO_Y1 + 36, INFO_X2 - 14, INFO_Y1 + 76,
        text_firm
            .str_assassin_executed()); // "Your Spy Was Caught And Executed." );
  }
  // ###### begin Gilbert 17/6 ########//
  ...
  // ###### begin Gilbert 17/6 ########//
  {
    vga.active_buf->put_bitmap(INFO_X1, INFO_Y1, image_gameif.read("EXECUTE"));
    // font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1, INFO_X2 -30,
    // INFO_Y1 +100, "Assassination Failed.", 2, 0, 0);
    // font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1 +100, INFO_X2 -30,
    // INFO_Y1 +200, "Your Spy Was Caught And Executed.", 2, 0, 0);
    // font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1, INFO_X2 -30,
    // INFO_Y1 +100, text_firm.str_assassin_fail(), 2, 0, 0);
    // font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1 +100, INFO_X2 -30,
    // INFO_Y1 +200, text_firm.str_assassin_executed(), 2, 0, 0);
...
void FirmCamp::detect_steal_tech_confirm() {
  ...
  //	help.set_custom_help(button_go.x1, button_go.y1, button_go.x2,
  //button_go.y2, "Steal Technology"); 	if( rc )
  if (button_yes.detect()) {
...
void FirmCamp::disp_steal_tech_result(int refreshFlag) {
  ...
  } else if (steal_tech_result > 0) // success
  {
    ...
    //		int techLevel =
    //tech_res[steal_tech_result]->get_nation_tech_level(nation_array.player_recno);
    //		if( techLevel > 1 )
    //		{
    //			str += " ";
    ...
    int techLevel = tech_res[steal_tech_result]->get_nation_tech_level(
        nation_array.player_recno);
    str = text_firm.str_steal_tech_success(
        tech_res[steal_tech_result]->tech_des(),
        (techLevel > 1 ? techLevel : 0)); // suppress version if version 1

    font_whbl.put_paragraph(INFO_X1 + 20, INFO_Y1 + 16, INFO_X2 - 14,
                            INFO_Y1 + 56, str);
  } else if (steal_tech_result == -1) // fail
  {
    ...
    font_whbl.put_paragraph(
        INFO_X1 + 20, INFO_Y1 + 36, INFO_X2 - 14, INFO_Y1 + 76,
        text_firm.str_steal_tech_executed()); // "Your Spy Was Caught And
                                              // Executed." );
  } else if (steal_tech_result == -2)         // success but no tech got
  {
    ...
    font_whbl.put_paragraph(
        INFO_X1 + 20, INFO_Y1 + 36, INFO_X2 - 14, INFO_Y1 + 76,
        text_firm.str_steal_tech_no_yield()); // "Cannot find any useful
                                              // technology." );
  } else {
    err_here();
  }
...
void FirmCamp::disp_incident_nation(int refreshFlag) {
  ...
  // US : font_whbl.center_put_paragraph( INFO_X1 +40 , INFO_Y1, INFO_X2 -30,
  // INFO_Y1 +60, str, 2, 0, 0);
  font_whbl.center_put_paragraph(INFO_X1 + 16, INFO_Y1, INFO_X2 - 6,
                                 INFO_Y1 + 60, str, 2, 0, 0);
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    for (nationRecno = 1; nationRecno <= nation_array.size(); ++nationRecno) {
      ...
      if (nationRecno ==
              nation_array.player_recno // exclude target nation and own nation
          || nationRecno == nation_recno)
        continue;

      //			if(
      //nation_array[nation_recno]->get_relation_status(nationRecno) <=
      //RELATION_HOSTILE ) 				continue;		// exclude nation which are
      //already hostile

      //			button_nation_group[button_nation_count].create(
      //INFO_X1+18 + 40*button_nation_count, INFO_Y1+75, 				INFO_X1+18 +
      //40*(button_nation_count+1) - 1, INFO_Y1+96, 				disp_nation_button,
      //ButtonCustomPara(NULL, nationRecno), 0 );
      button_nation_group[button_nation_count].create(
          INFO_X1 + 18, INFO_Y1 + 55 + button_nation_count * 24, INFO_X2 - 10,
          INFO_Y1 + 55 + (button_nation_count + 1) * 24 - 1, disp_nation_button,
          ButtonCustomPara(NULL, nationRecno), 0);
    ...
    action_target_recno =
        button_nation_group[button_nation_group()].custom_para.value;
    ...
    button_go.create(INFO_X1 + 13, INFO_Y1 + 281, 'A',
                     "INCIDENT"); // BUGHERE : create incident button
  }
...
//button_go.y2, "Create Incident");
  ...
  if (rc) {
    ...
    //		// switch menu mode before call action, it change
    //create_incident_result 		spyPtr->create_incident( nation_recno,
    //action_target_recno, COMMAND_PLAYER );

    firm_menu_mode = FIRM_MENU_INCIDENT_CONFIRM;
    spy_action_chance =
        spyPtr->get_incident_rating(nation_recno, action_target_recno);
    se_ctrl.immediate_sound("TURN_ON");
    return;
  }
}
...
void FirmCamp::disp_incident_confirm(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    button_go.create(INFO_X1 + 13, INFO_Y1 + 281, 'A',
                     "INCIDENT"); // BUGHERE : steal tech icon
    button_cancel.create(INFO_X1 + 13 + 3 * BUTTON_DISTANCE, INFO_Y1 + 281, 'A',
                         "PREVMENU");
  }
  font_whbl.put_paragraph(
      INFO_X1 + 18, INFO_Y1 + 12, INFO_X2 - 14, INFO_Y1 + 72,
      text_firm.str_ask_incident()); // "Do you really want to create an
                                     // incident?" );
  ...
  font_whbl.put(INFO_X1 + 18, INFO_Y1 + 73,
                text_firm.str_incident_chance(spy_action_chance));
...
void FirmCamp::disp_incident_result(int refreshFlag) {
  ...
  } else if (create_incident_result > 0) // success
  {
    vga.active_buf->put_bitmap(INFO_X1, INFO_Y1, image_gameif.read("MISSBASE"));
    font_whbl.put_paragraph(
        INFO_X1 + 20, INFO_Y1 + 16, INFO_X2 - 14, INFO_Y1 + 56,
  ...
  } else                                   // fail
  {
    ...
    font_whbl.put_paragraph(
        INFO_X1 + 20, INFO_Y1 + 36, INFO_X2 - 14, INFO_Y1 + 76,
        text_firm
            .str_incident_execute()); // "Your Spy Was Caught And Executed." );
  }
...
static void disp_nation_button(ButtonCustom *button, int repaintBody) {
  ...
  if (!nation_array.is_deleted(nationRecno)) {
    nation_array.disp_nation_color(x1 + 6, y1 + 8,
                                   nation_array[nationRecno]->nation_color);
    // vga.active_buf->bar( x1+6, y1+6, x1+16, y1+16,
    // nation_array[nationRecno]->nation_color );
    font_bld.put(x1 + 22, y1 + 4, nation_array[nationRecno]->nation_name(), 0,
                 x2 - 4);
  }
}
// ---------- begin of static function disp_nation_button --------//
