...
int FirmCamp::can_spy_bribe(int bribeSoldierId, int briberNationRecno)
{
    ...
    {
        canBribe =
            spy_array[spyRecno]->true_nation_recno != briberNationRecno; // only when the unit is not yet a spy of
                                                                         // the player. Still display the bribe
                                                                         // button when it's a spy of another nation
    }
    else
    {
        if (bribeSoldierId)
...
{
    if (!can_spy_bribe(soldierId,
                       spy_array[briberSpyRecno]->true_nation_recno)) // this can happen in multiplayer as there
...
int FirmCamp::spy_bribe_succeed_chance(int bribeAmount, short briberSpyRecno, short soldierId)
{
    ...
    if (targetSpyRecno) // if the bribe target is also a spy
    {
        err_when(spy_array[targetSpyRecno]->true_nation_recno ==
                 spyPtr->true_nation_recno); // the player shouldn't be able
                                             // to bribe units of his own
    ...
    else
    {
        ...
        //		if( unitLoyalty > 60 )			// harder for bribe units with over
        // 60
        // loyalty 			succeedChance -= (unitLoyalty-60); 		if( unitLoyalty > 70
        // )
        //// harder for bribe units with over 70 loyalty 			succeedChance
        ///-=
        //(unitLoyalty-70); 		if( unitLoyalty > 80 )			// harder
        // for
        // bribe units with over 80 loyalty 			succeedChance -= (unitLoyalty-80);
        // if(
        // unitLoyalty > 90 )			// harder for bribe units with over
        // 90 loyalty 			succeedChance -= (unitLoyalty-90);
        if (unitLoyalty > 60)
...
void FirmCamp::disp_bribe_menu(int refreshFlag)
{
    ...
    if (bribe_result == BRIBE_NONE)
    {
        ...
        if (refreshFlag == INFO_REPAINT)
        {
            ...
            spy_action_chance = spy_bribe_succeed_chance(bribe_amount_array[button_bribe_amount()], action_spy_recno,
                                                         action_target_recno);
...
void FirmCamp::detect_bribe_menu()
{
    ...
    if (bribe_result == BRIBE_NONE)
    {
        ...
        {
            // update chance
            spy_action_chance = spy_bribe_succeed_chance(bribe_amount_array[rc], action_spy_recno, action_target_recno);
            if (!config.show_debug_info)
            {
                spy_action_chance = MIN(spy_action_chance, 0);
                spy_action_chance = MAX(spy_action_chance, 100);
...
void FirmCamp::disp_bribe_unit(int dispY1)
{
    ...
    //	y += 15;
    font_bld.put_paragraph(x, y, INFO_X2 - 16, y + 36, text_firm.str_ask_bribe_unit(unitName));
    y += 36;
...
void FirmCamp::detect_assassinate_confirm()
{
    ...
    if (spy_array.is_deleted(action_spy_recno) || !(spyPtr = spy_array[action_spy_recno]) ||
        spyPtr->spy_place != SPY_FIRM || spyPtr->spy_place_para != firm_recno || !overseer_recno ||
        unit_array[overseer_recno]->is_own())
    {
...
void FirmCamp::disp_incident_nation(int refreshFlag)
{
    ...
    if (refreshFlag == INFO_REPAINT)
    {
        ...
        for (nationRecno = 1; nationRecno <= nation_array.size(); ++nationRecno)
        {
            ...
            // RELATION_HOSTILE ) 				continue;
            // // exclude nation which are already hostile

            //			button_nation_group[button_nation_count].create(
            // INFO_X1+18 + 40*button_nation_count, INFO_Y1+75,
            ...
            button_nation_group[button_nation_count].create(INFO_X1 + 18, INFO_Y1 + 55 + button_nation_count * 24,
                                                            INFO_X2 - 10,
                                                            INFO_Y1 + 55 + (button_nation_count + 1) * 24 - 1,
                                                            disp_nation_button, ButtonCustomPara(NULL, nationRecno), 0);
    ...
    if (button_nation_count > 0)
    {
        ...
        {
            button_nation_group[b].visible_flag = !nation_array.is_deleted(button_nation_group[b].custom_para.value);
        }
