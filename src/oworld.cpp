...
int World::detect_scroll()
{
    ...
    case 0: {
        ...
        if (next_scroll_time && misc.get_time() < next_scroll_time) // just scrolled not too long ago, wait for a
    ...
    case 1: // method 1, depend on time of scrolling
    {
        ...
        if (last_scroll_flag == currentScrollFlag && currentScrollFlag)
        {
            ...
            if (scrollX != 0 || scrollY != 0)
            {
                ...
                if (zoom_matrix->top_x_loc != last_scroll_top_x_loc || zoom_matrix->top_y_loc != last_scroll_top_y_loc)
                {
        ...
        for (; xLoc <= xLoc2; (xLoc += 2), (locPtr += 2), (writePtr += writePtrInc))
        {
            ...
            case MAP_MODE_TERRAIN:
                ...
                if (world.map_matrix->filter_object_flag ||
                    world.map_matrix->filter_nation_flag) // do not anything except filtered objects
                                                          // when filters are on
                ...
                else if (locPtr->has_dirt())
                {
                    ...
                    *writePtr = vga_back.translate_color(
                        terrain_res.get_map_tile(locPtr->road_terrain_id)[tileYOffset + (xLoc & TERRAIN_TILE_X_MASK)]);
                else
                {
                    if (xLoc < 2 || terrain_res[locPtr->terrain_id]->average_type >=
                                        terrain_res[(locPtr - 2)->terrain_id]->average_type)
                    {
                        // get terrain pixel from terrain_res
                        *writePtr = vga_back.translate_color(
                            terrain_res.get_map_tile(locPtr->terrain_id)[tileYOffset + (xLoc & TERRAIN_TILE_X_MASK)]);
                    }
                    else
                    {
                        ...
                        if (!relation->has_contact)
                        {
                            ...
                            else
                            {
                                if (!relation->contact_msg_flag)
                                {
                                    ...
                                    short *shortPtr =
                                        (short *)remote.new_send_queue_msg(MSG_NATION_CONTACT, 2 * sizeof(short));
...
int World::is_explored_quick(int xLoc1, int yLoc1, int xLoc2, int yLoc2)
{
    ...
    if (xLoc1 == xLoc2 && yLoc1 == yLoc2)
        ...
        return get_loc(xLoc1, yLoc1)->explored() || get_loc(xLoc2, yLoc1)->explored() ||
               get_loc(xLoc1, yLoc2)->explored() || get_loc(xLoc2, yLoc2)->explored();
}
...
void World::load_map(char *fileName)
{
    ...
    int terrainId =
        terrain_res.scan(baseType, MIDDLE_MASK, baseType, MIDDLE_MASK, baseType, MIDDLE_MASK, baseType, MIDDLE_MASK,
    ...
    for (y = yLoc1; y <= yLoc2; y++)
    {
        ...
        for (x = xLoc1; x <= xLoc2; x++, locPtr++)
        {
            if (!locPtr->can_move(mobileType) ||
                (buildFlag && (locPtr->is_power_off() || locPtr->has_site()))) // if build a firm/town, there must
                                                                               // not be any sites in the area
            {
                return 0;
            }
        }
       ...
       misc.points_distance(townPtr->center_x, townPtr->center_y, xLoc1, yLoc2 );
                    ...
                    if( bestTown )
                    {
                            path_finder.find_path( xLoc1, yLoc1,
                                                                                      bestTown->center_x,
       bestTown->center_y, bestTown->loc_x1, bestTown->loc_y1, bestTown->loc_x2,
       bestTown->loc_y2 );
...
//
int World::locate_space(int &xLoc1, int &yLoc1, int xLoc2, int yLoc2, int spaceLocWidth, int spaceLocHeight,
                        int mobileType, int regionId, int buildFlag)
{
    if (!regionId)
    ...
    while (1)
    {
        ...
        if (x >= 0 && y >= 0 && x + spaceLocWidth - 1 < MAX_WORLD_X_LOC && y + spaceLocHeight - 1 < MAX_WORLD_Y_LOC)
        {
            if (mobileType == UNIT_LAND || (x % 2 == 0 && y % 2 == 0))
            {
                ...
                if (locPtr->region_id == regionId && locPtr->is_plateau() == isPlateau &&
            ...
            if (j % 2)
            {
                    ...
                    y + spaceLocHeight - 1 < MAX_WORLD_Y_LOC)
                {
                    if (mobileType == UNIT_LAND || (x % 2 == 0 && y % 2 == 0))
                    {
                        ...
                        if (locPtr->region_id == regionId && locPtr->is_plateau() == isPlateau &&
                            check_unit_space(x, y, x + spaceLocWidth - 1, y + spaceLocHeight - 1, mobileType,
                                             buildFlag))
                        {
                            xLoc1 = x;
                            yLoc1 = y;
...
//                0 - free space found
//
int World::locate_space_random(int &xLoc1, int &yLoc1, int xLoc2, int yLoc2, int spaceLocWidth, int spaceLocHeight,
                               int maxTries, int regionId, int buildSite, char teraMask)
{
...
//								  if the builder unit
// stands
// on the building area, still consider the area as buildable
...
int World::can_build_firm(int xLoc1, int yLoc1, int firmId, short unitRecno)
{
    ...
    if (unitRecno > 0)
    {
        ...
        if (unitPtr->is_visible() && unitPtr->region_id() != get_loc(xLoc1, yLoc1)->region_id)
        {
            return 0;
        }
    }
    ...
    case 3: // land or sea firm
        ...
        for (yLoc = yLoc1; yLoc <= yLoc2; yLoc++)
        {
            ...
            for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++)
            {
                ...
                if (locPtr->has_site() && !((firmId == FIRM_MINE || firmId == FIRM_ALCHEMY) &&
    ...
    case 4: // special firm, such as harbor
        ...
        err_when(pierFlag != 0 && pierFlag != 1 && pierFlag != 2 && pierFlag != 4 && pierFlag != 8);
...
int World::can_build_site(int xLoc1, int yLoc1, int siteWidth, int siteHeight, int outerSpace)
{
    ...
    if (outerSpace > 0)
    {
        ...
        for (yLoc = yLoc3; yLoc <= yLoc4; ++yLoc)
        {
            ...
            for (xLoc = xLoc3; xLoc <= xLoc4; ++xLoc, ++locPtr)
            {
                if (!locPtr->walkable(LOCATE_WALK_LAND) && !locPtr->can_clear_obstacle())
                    return 0;
...
// 0)
//
void World::draw_link_line(int srcFirmId, int srcTownRecno, int srcXLoc1, int srcYLoc1, int srcXLoc2, int srcYLoc2,
                           int givenEffectiveDis)
{
    ...
    int srcX = ZOOM_X1 + world.zoom_matrix->calc_zoom_x((srcXLoc1 + srcXLoc2 + 1) * LOCATE_WIDTH / 2,
                                                        (srcYLoc1 + srcYLoc2 + 1) * LOCATE_HEIGHT / 2, altitude);
    int srcY = ZOOM_Y1 + world.zoom_matrix->calc_zoom_y((srcXLoc1 + srcXLoc2 + 1) * LOCATE_WIDTH / 2,
                                                        (srcYLoc1 + srcYLoc2 + 1) * LOCATE_HEIGHT / 2, altitude);
    ...
    if (!srcFirmId || firm_res[srcFirmId]->is_linkable_to_town) // don't draw link line to town if it's an inn
    {
        for (i = town_array.size(); i > 0; i--)
        {
            ...
            if (misc.points_distance(townPtr->center_x, townPtr->center_y, srcXLoc, srcYLoc) > effectiveDis)
            {
                continue;
            }
            ...
            townX = ZOOM_X1 + world.zoom_matrix->calc_zoom_x(
                                  ...
                                  (townPtr->loc_y1 + townPtr->loc_y2 + 1) * LOCATE_HEIGHT / 2, townPtr->altitude);
    ...
    for (i = firm_array.size(); i > 0; i--)
    {
        ...
        firmX = ZOOM_X1 + world.zoom_matrix->calc_zoom_x((firmPtr->loc_x1 + firmPtr->loc_x2 + 1) * LOCATE_WIDTH / 2,
                                                         (firmPtr->loc_y1 + firmPtr->loc_y2 + 1) * LOCATE_HEIGHT / 2,
                                                         firmPtr->altitude);
        firmY = ZOOM_Y1 + world.zoom_matrix->calc_zoom_y((firmPtr->loc_x1 + firmPtr->loc_x2 + 1) * LOCATE_WIDTH / 2,
                                                         (firmPtr->loc_y1 + firmPtr->loc_y2 + 1) * LOCATE_HEIGHT / 2,
                                                         firmPtr->altitude);
...
int World::detect_firm_town()
{
    ...
    if (firm_array.selected_recno)
    {
        ...
        if (firmPtr->should_show_info() &&     // only if should_show_info() is 1, we can
    ...
    if (town_array.selected_recno)
    {
        ...
        if (townPtr->nation_recno == nation_array.player_recno && townPtr->draw_detect_link_line(1)) // 1-detect action
        {
            return 1;
        }
    }
...
void World::earth_quake()
{
    ...
    for (i = firm_array.size(); i > 0; i--)
    {
        ...
        if (firmPtr->hit_points <= 0)
        {
            ...
            if (firmPtr->is_own())
                firmDie++;
            se_res.sound(firmPtr->center_x, firmPtr->center_y, 1, 'F', firmPtr->firm_id, "DIE");
    ...
    for (i = unit_array.size(); i > 0; i--)
    {
        ...
        if (!unitPtr->is_visible() || unitPtr->mobile_type == UNIT_AIR || unitPtr->mobile_type == UNIT_SEA)
            continue;
        // ###### end Gilbert 30/8 ######//
        ...
        float damage =
            (float)weather.quake_rate(unitPtr->cur_x_loc(), unitPtr->cur_y_loc()) * unitPtr->max_hit_points() / 400;
        // ####### patch end Gilbert 18/1 #######//
...
void World::lightning_strike(short cx, short cy, short radius)
{
    ...
    for (i = unit_array.size(); i > 0; i--)
    {
            ...
            unitPtr->cur_y_loc() + unitPtr->sprite_info->loc_height > cy - radius)
        {
            ...
            // ---- add news -------//
            if (unitPtr->is_own())
                news_array.lightning_damage(unitPtr->cur_x_loc(), unitPtr->cur_y_loc(), NEWS_LOC_UNIT, i,
                                            unitPtr->hit_points <= (float)0);
    ...
    for (i = firm_array.size(); i > 0; i--)
    {
        ...
        Firm *firmPtr = firm_array[i];
        if (firmPtr->loc_x1 <= cx + radius && firmPtr->loc_x2 >= cx - radius && firmPtr->loc_y1 <= cy + radius &&
            firmPtr->loc_y2 >= cy - radius)
        {
            ...
            // ---- add news -------//
            if (firmPtr->is_own())
                news_array.lightning_damage(firmPtr->center_x, firmPtr->center_y, NEWS_LOC_FIRM, i,
                                            firmPtr->hit_points <= (float)0);
    ...
    for (i = town_array.size(); i > 0; i--)
    {
        ...
        if (townPtr->loc_x1 <= cx + radius && townPtr->loc_x2 >= cx - radius && townPtr->loc_y1 <= cy + radius &&
            townPtr->loc_y2 >= cy - radius)
        {
            // ---- add news -------//
            if (townPtr->nation_recno == nation_array.player_recno)
                news_array.lightning_damage(townPtr->center_x, townPtr->center_y, NEWS_LOC_TOWN, i, 0);
...
{
    err_when(STD_TOWN_LOC_WIDTH != 6); // reduce town_town, firm_town, camp_town if town size is smaller
...
#define EFFECTIVE_FIRM_TOWN_DISTANCE                                                                                   \
    dist_firm_town[config.building_size - 1] // maximum distance between the town and the base until it's \
                    // no longer considered as command base of the town
#define EFFECTIVE_FIRM_FIRM_DISTANCE                                                                                   \
    dist_firm_firm[config.building_size - 1] // maximum distance between the town and the base until it's \
                    // no longer considered as command base of the town
...
#define EFFECTIVE_LAIR_LAIR_DISTANCE dist_lair_lair[config.building_size - 1]
    ...
    if (placeId1 > 0 && placeId2 == 0)
    {
        if (placeId1 == FIRM_CAMP || placeId1 == FIRM_FORT || placeId1 == FIRM_LAIR)
        {
...
// <int> returnIfWar	 		  	  - whether return -1 if there
// is war
//											 around
// the
// given area.
// <int&> hasWar					  - a var for returning whether
// there is war
//											 around
// the
// given area. 											 1 - if
// there is war 2 - if the attacker nation is involved in the war <int>
// scanRange 				  - the scanning range of the area
//
...
//              going on.
//
int World::net_attacker_power_in_area(int targetXLoc, int targetYLoc, int attackerNationRecno, int defenderNationRecno,
                                      int returnIfWar, int &hasWar, int scanRange)
{
    ...
    for (yLoc = yLoc1; yLoc <= yLoc2; yLoc++)
    {
        ...
        for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++)
        {
            ...
            if (locPtr->unit_recno(UNIT_LAND))
            {
                ...
                if (unitPtr->cur_order.mode != UNIT_MOVE && unitPtr->cur_order.mode != UNIT_ATTACK)
                {
                    continue;
                }
