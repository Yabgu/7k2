...
World::World() { memset(this, 0, sizeof(World)); }
...
//
void World::disp_zoom() { zoom_matrix->disp(); }
...
void World::process() {
  ...
  if (lightning_signal == 106 && config.weather_effect) {
    lightning_strike(misc.random(MAX_MAP_WIDTH), misc.random(MAX_MAP_HEIGHT),
                     1);
  }
...
void World::next_day() {
  ...
  // ######## begin Gilbert 16/12 #######//
  if (weather.is_quake() &&
      (config.random_event_frequency || magic_weather.is_quake))
  // ######## end Gilbert 16/12 #######//
  {
    earth_quake();
  }
  ...
  for (int y = 0; y < MAX_WORLD_Y_LOC; y++) {
    for (int x = 0; x < MAX_WORLD_X_LOC; x++) {
      ...
      if ((unitRecno = locPtr->unit_recno(UNIT_LAND))) {
        ...
        err_when(unitPtr->next_x_loc() > x ||
                 unitPtr->next_x_loc() + unitPtr->loc_width <= x);
        err_when(unitPtr->next_y_loc() > y ||
                 unitPtr->next_y_loc() + unitPtr->loc_height <= y);
      }
      // ####### end Gilbert 26/2 ######//
...
int World::detect_scroll() {
  ...
  int inChatMode = sys.view_mode == MODE_NATION &&
  ...
  case 0: {
    ...
    if (next_scroll_time &&
        misc.get_time() <
            next_scroll_time) // just scrolled not too long ago, wait for a
                              // little while before next scroll.
      return 0;
    ...
    // ##### begin Gilbert 30/3 #######//
    if (mouse.cur_x == MOUSE_X1 ||
        !inChatMode && mouse.arrow_key_state & ARROW_LEFT_KEY_MASK)
    // ##### end Gilbert 30/3 #######//
    {
    ...
    // ##### begin Gilbert 30/3 #######//
    if (mouse.cur_x == MOUSE_X2 ||
        !inChatMode && mouse.arrow_key_state & ARROW_RIGHT_KEY_MASK)
    // ##### end Gilbert 30/3 #######//
    {
      zoom_matrix->scroll(1, -1);
      rc = 1;
    ...
    // ##### begin Gilbert 30/3 #######//
    if (mouse.cur_y == MOUSE_Y1 ||
        !inChatMode && mouse.arrow_key_state & ARROW_UP_KEY_MASK)
    // ##### end Gilbert 30/3 #######//
    {
      zoom_matrix->scroll(-1, -1);
      rc = 1;
    ...
    // ##### begin Gilbert 30/3 #######//
    if (mouse.cur_y == MOUSE_Y2 ||
        !inChatMode && mouse.arrow_key_state & ARROW_DOWN_KEY_MASK)
    // ##### end Gilbert 30/3 #######//
    {
      zoom_matrix->scroll(1, 1);
      rc = 1;
  ...
  case 1: // method 1, depend on time of scrolling
  {
    ...
    if (last_scroll_flag == currentScrollFlag && currentScrollFlag) {
      ...
      // record our own last_scroll_top_x/y_loc instead of using
      // zoom_matrix->top_x/y_loc because if in smooth scroll,
      // zoom_matrix->top_x/y_loc doesn't follow closely

      int scrollX =
          vectorX * (int)step + begin_scroll_top_x_loc - last_scroll_top_x_loc;
      int scrollY =
          vectorY * (int)step + begin_scroll_top_y_loc - last_scroll_top_y_loc;
...
#error
  ...
  case GO_LOC_SELECT_LAND: // sea also
    if (locPtr->unit_recno(UNIT_LAND)) {
      ...
      unit_array[locPtr->unit_recno(UNIT_LAND)]->select(
          true); // true - set this to unit_array.selected_recno to this unit
      info.disp();
    } else if (locPtr->unit_recno(UNIT_SEA)) {
      power.reset_selection();
      unit_array[locPtr->unit_recno(UNIT_SEA)]->select(
          true); // true - set this to unit_array.selected_recno to this unit
      info.disp();
  ...
  case GO_LOC_SELECT_AIR:
    if (locPtr->unit_recno(UNIT_AIR)) {
      power.reset_selection();
      unit_array[locPtr->unit_recno(UNIT_AIR)]->select(
          true); // true - set this to unit_array.selected_recno to this unit
      info.disp();
    }
    break;
...
void World::explore(int xLoc1, int yLoc1, int xLoc2, int yLoc2) {
  ...
  for (int n = 0; n < MAX_NATION + 2; ++n)
    nationColorArray[n] =
        vga_back.translate_color(nation_array.nation_power_color_array[n]);
  ...
  for (yLoc = yLoc1; yLoc <= yLoc2; (yLoc & 1 ? ++pixelY : --pixelX), ++yLoc) {
    ...
    for (; xLoc <= xLoc2;
         (xLoc += 2), (locPtr += 2), (writePtr += writePtrInc)) {
      ...
      case MAP_MODE_TERRAIN:
        // ###### begin Gilbert 10/2 ######//
        if (world.map_matrix->filter_object_flag ||
            world.map_matrix
                ->filter_nation_flag) // do not anything except filtered objects
                                      // when filters are on
          ...
          *writePtr = fireColor;
                 ...
                 xLoc + 1 < max_x_loc && locPtr[1].is_plant())
          *writePtr = plantColor;
          ...
          *writePtr = vga_back.translate_color(terrain_res.get_map_tile(
              locPtr->road_terrain_id)[tileYOffset +
                                       (xLoc & TERRAIN_TILE_X_MASK)]);
        else {
          if (xLoc < 2 ||
            ...
            *writePtr = vga_back.translate_color(terrain_res.get_map_tile(
                locPtr
                    ->terrain_id)[tileYOffset + (xLoc & TERRAIN_TILE_X_MASK)]);
          } else {
      ...
      case MAP_MODE_SPOT:
        /*				if( locPtr->sailable() )
                                        ...
                                        {
                                                Rock *dirtPtr =
           dirt_array[locPtr->dirt_recno()]; RockInfo *dirtInfo =
           rock_res.get_rock_info(dirtPtr->rock_recno); if (dirtInfo->rock_type
           == 'P') *writePtr = seaColor; else *writePtr = rockColor;
                                        }

                                        else if( locPtr->is_plant() ||
           xLoc+1<max_x_loc && locPtr[1].is_plant() ) *writePtr = plantColor;
                                        else*/
        *writePtr = mapModeColor;
        break;
      ...
      if (!locPtr->explored()) {
        ...
        if (locPtr->is_firm()) {
          ...
          if (firmPtr->nation_recno > 0 && nation_array.player_recno) {
            NationRelation *relation =
                (~nation_array)->get_relation(firmPtr->nation_recno);
              ...
              if (!remote.is_enable()) {
                ...
                if (!relation->contact_msg_flag) {
                  ...
                  short *shortPtr = (short *)remote.new_send_queue_msg(
                      MSG_NATION_CONTACT, 2 * sizeof(short));
        ...
        if (locPtr->is_town()) {
          ...
          if (townPtr->nation_recno > 0 && nation_array.player_recno) {
            NationRelation *relation =
                (~nation_array)->get_relation(townPtr->nation_recno);
...
void World::load_map(char *fileName) {
  ...
  loc_matrix = (Location *)mem_resize(loc_matrix,
                                      max_x_loc * max_y_loc * sizeof(Location));
  ...
  int terrainId = terrain_res.scan(baseType, MIDDLE_MASK, baseType, MIDDLE_MASK,
                                   baseType, MIDDLE_MASK, baseType, MIDDLE_MASK,
                                   1); // 1-get the first instance
  ...
  mapFile.file_read(loc_matrix,
                    sizeof(Location) * MAX_WORLD_X_LOC * MAX_WORLD_Y_LOC);
...
// [int]  buildFlag 		 = whether the located area is for building a
// firm/town
//									if so, the location must
//no have any raw site. 									(default: 0)
//
// return 1 for true. 0 for false
//
int World::check_unit_space(int xLoc1, int yLoc1, int xLoc2, int yLoc2,
                            int mobileType, int buildFlag) {
  ...
  //----- also make sure that the 2 surrounding circles do not have any
  ...
  for (y = yLoc1; y <= yLoc2; y++) {
    ...
    for (x = xLoc1; x <= xLoc2; x++, locPtr++) {
      if (!locPtr->can_move(mobileType) ||
          (buildFlag &&
           (locPtr->is_power_off() ||
            locPtr->has_site()))) // if build a firm/town, there must not be any
                                  // sites in the area
      {
        return 0;
      }
  ...
  /*
          //--- try to do a path search to the nearest town, if the search
     fails, we assume that location is isolated from outside ---//
          ...
          if( check_unit_space_avoid_mountain )
          {
                  ...
                  for( int i=town_array.size() ; i>0 ; i-- )
                  {
                          ...
                          int curDistance =
     misc.points_distance(townPtr->center_x, townPtr->center_y, xLoc1, yLoc2 );
                  ...
                  if( bestTown )
                  {
                          ...
                          {
                                  if( ++check_unit_space_avoid_mountain > 100 )
     // if too many path searching failures, give up avoiding mountains
...
// the unit
//
// <int&> xLoc1          = the upper left x location of the building, also for
// returning the result location <int&> yLoc1          = the upper left y
// location of the building <int>  xLoc2          = the lower right x location
// of the building <int>  yLoc2          = the lower right y location of the
// building <int>  spaceLocWidth  = the location width of the required space
// <int>  spaceLocHeight = the location height of the required space
// [int]  mobileType		 = mobile type (default: UNIT_LAND)
// [int]  regionId		 = specify the region no. of the location to
// locate
//									(default: region no. of
//xLoc1, yLoc1)
// [int]  buildFlag 		 = whether the located area is for building a
// firm/town
//									if so, the location must
...
int World::locate_space(int &xLoc1, int &yLoc1, int xLoc2, int yLoc2,
                        int spaceLocWidth, int spaceLocHeight, int mobileType,
                        int regionId, int buildFlag) {
  if (!regionId)
  ...
  while (1) {
    ...
    if (x >= 0 && y >= 0 && x + spaceLocWidth - 1 < MAX_WORLD_X_LOC &&
        y + spaceLocHeight - 1 < MAX_WORLD_Y_LOC) {
      if (mobileType == UNIT_LAND || (x % 2 == 0 && y % 2 == 0)) {
        locPtr = get_loc(x, y);

        if (locPtr->region_id == regionId &&
            locPtr->is_plateau() == isPlateau &&
            check_unit_space(x, y, x + spaceLocWidth - 1,
                             y + spaceLocHeight - 1, mobileType, buildFlag)) {
          xLoc1 = x;
          yLoc1 = y;
          return 1;
        }
    ...
    //-----------------------------------------------------------//
    if (xLoc <= 0 && yLoc <= 0 && width >= MAX_WORLD_X_LOC &&
        height >= MAX_WORLD_Y_LOC)
    ...
    // if(width==xLoc2-xLoc1+spaceLocWidth &&
    // height==yLoc2-yLoc1+spaceLocHeight) // terminate the checking
    // return 0;
  }
...
//
// <int&> xLoc1          = the scaning range, also for returning the result
// location <int&> yLoc1          = the scaning range <int>  xLoc2          =
// the scaning range <int>  yLoc2          = the scaning range <int>
// spaceLocWidth  = the location width of the required space <int>
// spaceLocHeight = the location height of the required space <int>  maxTries =
// maximum no. of tries [int]  regionId		 = if this is specified, the
// result location will
//									be in this
//region.
...
int World::locate_space_random(int &xLoc1, int &yLoc1, int xLoc2, int yLoc2,
                               int spaceLocWidth, int spaceLocHeight,
                               int maxTries, int regionId, int buildSite,
                               char teraMask) {
  int i, x, y, xTemp, xLoc, yLoc, canBuildFlag;
  int scanWidth =
      xLoc2 - xLoc1 - spaceLocWidth + 2; // xLoc2-xLoc1+1-spaceLocWidth+1;
  int scanHeight =
      yLoc2 - yLoc1 - spaceLocHeight + 2; // yLoc2-yLoc1+1-spaceLocHeight+1;
  Location *locPtr;
  ...
  for (i = 0; i < maxTries; i++) {
    ...
    for (y = yLoc + spaceLocHeight - 1; y >= yLoc; y--) {
      ...
      for (x = xTemp; x >= xLoc; x--, locPtr--) {
        if ((buildSite ? !locPtr->can_build_site(teraMask)
                       : !locPtr->can_build_firm(teraMask)) ||
            locPtr->is_power_off()) {
          canBuildFlag = 0;
          break;
        }
    ...
    err_when(buildSite && !locPtr->can_build_site(
                              teraMask)); //-*** hard codes for mine size 3x3
    return 1;
  }
...
//								  if the builder unit stands
//on the building area, still consider the area as buildable 								  (default: -1, do
//not take the builder into account)
//
int World::can_build_firm(int xLoc1, int yLoc1, int firmId, short unitRecno) {
  ...
  //--- if the unit's region is different from the destination's region, do
  //nothing ---//
  ...
  case 3: // land or sea firm
    ...
    for (yLoc = yLoc1; yLoc <= yLoc2; yLoc++) {
      ...
      for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++) {
        ...
        if (locPtr->has_site() &&
            !((firmId == FIRM_MINE || firmId == FIRM_ALCHEMY) &&
  ...
  case 4: // special firm, such as harbor
    ...
    err_when(pierFlag != 0 && pierFlag != 1 && pierFlag != 2 && pierFlag != 4 &&
             pierFlag != 8);
...
//
int World::can_build_site(int xLoc1, int yLoc1, int siteWidth, int siteHeight,
                          int outerSpace) {
  Location *locPtr;
...
//       srcXLoc2, srcYLoc2
//
// [int] giveEffectiveDis   - use this value as the effective distance if this
// is given
//										(default:
//0)
//
void World::draw_link_line(int srcFirmId, int srcTownRecno, int srcXLoc1,
                           int srcYLoc1, int srcXLoc2, int srcYLoc2,
                           int givenEffectiveDis) {
  if (srcFirmId == FIRM_INN) // FirmInn's link is only for scan for neighbor
                             // inns quickly, the link line is not displayed
    return;
  ...
  int srcX =
      ZOOM_X1 + world.zoom_matrix->calc_zoom_x(
                    (srcXLoc1 + srcXLoc2 + 1) * LOCATE_WIDTH / 2,
                    (srcYLoc1 + srcYLoc2 + 1) * LOCATE_HEIGHT / 2, altitude);
  int srcY =
      ZOOM_Y1 + world.zoom_matrix->calc_zoom_y(
                    (srcXLoc1 + srcXLoc2 + 1) * LOCATE_WIDTH / 2,
                    (srcYLoc1 + srcYLoc2 + 1) * LOCATE_HEIGHT / 2, altitude);
  ...
  if (!srcFirmId ||
      firm_res[srcFirmId]
          ->is_linkable_to_town) // don't draw link line to town if it's an inn
  {
    for (i = town_array.size(); i > 0; i--) {
      if (town_array.is_deleted(i) ||
          i == srcTownRecno) // don't draw the same town recno
        continue;
      ...
      if (misc.points_distance(townPtr->center_x, townPtr->center_y, srcXLoc,
                               srcYLoc) > effectiveDis) {
        continue;
      }
      ...
      // townPtr->center_y)->is_plateau()==1)
      //	 != (world.get_loc(srcXLoc, srcYLoc)->is_plateau()==1) )
      // use loc_x1/y1 instead of center_x/y to comply with world.locate_space
      if ((world.get_loc(townPtr->loc_x1, townPtr->loc_y1)->is_plateau() ==
           1) != (world.get_loc(srcXLoc1, srcYLoc1)->is_plateau() == 1)) {
        continue;
      }
  ...
  for (i = firm_array.size(); i > 0; i--) {
    ...
    if (misc.points_distance(firmPtr->center_x, firmPtr->center_y, srcXLoc,
                             srcYLoc) > effectiveDis)
      continue;
    ...
    // if( (world.get_loc(firmPtr->center_x,
    // firmPtr->center_y)->is_plateau()==1)
    //	 != (world.get_loc(srcXLoc, srcYLoc)->is_plateau()==1) )
...
void World::set_all_power() {
  ...
  for (i = town_array.size(); i > 0; i--) {
    ...
    set_power(townPtr->loc_x1, townPtr->loc_y1, townPtr->loc_x2,
              townPtr->loc_y2, (char)townPtr->nation_recno);
  }
  ...
  for (i = firm_array.size(); i > 0; i--) {
    ...
    set_power(firmPtr->loc_x1, firmPtr->loc_y1, firmPtr->loc_x2,
              firmPtr->loc_y2, (char)firmPtr->nation_recno);
  }
}
...
//
void World::set_power(int xLoc1, int yLoc1, int xLoc2, int yLoc2,
                      int nationRecno) {
  //------- reset power_nation_recno first ------//

  // use xLoc1, yLoc1 instead of center location to comply with
  // world.locate_space int	plateauResult = (get_loc((xLoc1+xLoc2)/2,
  // (yLoc1+yLoc2)/2)->is_plateau()==1);
...
//
void World::restore_power(int xLoc1, int yLoc1, int xLoc2, int yLoc2,
                          int townRecno, int firmRecno) {
  int nationRecno;
  ...
  //--- if some power areas are freed up, see if neighbor towns/firms should
  //take up these power areas ----//

  if (sys.map_need_redraw) // when calls set_all_power(), the nation_recno of
                           // the calling firm must be reset
    set_all_power();
...
int World::detect_firm_town() {
  ...
  if (firm_array.selected_recno) {
    ...
    if (firmPtr
            ->should_show_info() && // only if should_show_info() is 1, we can
                                    // detect links from this firm (it is not
                                    // limited to player firms, as firms with
                                    // player's workers should be allowed for
                                    // resigning the player worker from the firm
  ...
  /*
          ...
          if( rc==1 )			// left click
          {
                  ...
                  for( i=firm_array.size() ; i>0 ; i-- )
                  {
                          ...
                          if( misc.is_touch( mouseAbsX, mouseAbsY, mouseAbsX,
     mouseAbsY, firmPtr->abs_x1, firmPtr->abs_y1, firmPtr->abs_x2,
     firmPtr->abs_y2 ) )
                          {
                                  power.reset_selection();
                                  ...
                                  if( firmPtr->nation_recno ==
     nation_array.player_recno && se_res.mark_select_object_time() )
                                  {
                                          se_res.sound(firmPtr->center_x,
     firmPtr->center_y, 1, 'F', firmPtr->firm_id, firmPtr->under_construction ?
     "SELU" : "SEL" );
                                  }
                                  return 1;
                          }
                  ...
                  for( i=town_array.size() ; i>0 ; i-- )
                  {
                          ...
                          if( misc.is_touch( mouseAbsX, mouseAbsY, mouseAbsX,
     mouseAbsY, townPtr->abs_x1, townPtr->abs_y1, townPtr->abs_x2,
     townPtr->abs_y2 ) )
                          {
                                  power.reset_selection();
     ...
     nation_array.player_recno
                                  ...
                                  {
                                          se_res.sound(townPtr->center_x,
     townPtr->center_y, 1, 'T', 0, "SEL" );
                                  }
                                  return 1;
                          }
                  }
...
void World::earth_quake() {
  ...
  for (i = firm_array.size(); i > 0; i--) {
    if (firm_array.is_deleted(i) ||
        !firm_res[firm_array[i]->firm_id]->buildable)
      continue;
    ...
    if (firmPtr->hit_points <= 0) {
      ...
      if (firmPtr->is_own())
        firmDie++;
      se_res.sound(firmPtr->center_x, firmPtr->center_y, 1, 'F',
                   firmPtr->firm_id, "DIE");
  ...
  for (i = unit_array.size(); i > 0; i--) {
    ...
    // ####### patch begin Gilbert 18/1 #######//
    //		float damage = (float)
    //weather.quake_rate(unitPtr->cur_x_loc(),unitPtr->cur_y_loc() ) *
    //			unitPtr->max_hit_points() / 200;
    float damage =
        (float)weather.quake_rate(unitPtr->cur_x_loc(), unitPtr->cur_y_loc()) *
  ...
  news_array.earthquake_damage(unitDamage - unitDie, unitDie, townDamage,
                               firmDamage - firmDie, firmDie);
}
...
void World::lightning_strike(short cx, short cy, short radius) {
  ...
  for (i = unit_array.size(); i > 0; i--) {
    ...
    if (unitPtr->cur_x_loc() <= cx + radius &&
      ...
      unitPtr->hit_points -=
          (float)unitPtr->sprite_info->lightning_damage / ATTACK_SLOW_DOWN;
  ...
  for (i = firm_array.size(); i > 0; i--) {
    ...
    Firm *firmPtr = firm_array[i];
    if (firmPtr->loc_x1 <= cx + radius && firmPtr->loc_x2 >= cx - radius &&
        firmPtr->loc_y1 <= cy + radius && firmPtr->loc_y2 >= cy - radius) {
  ...
  for (i = town_array.size(); i > 0; i--) {
    ...
    if (townPtr->loc_x1 <= cx + radius && townPtr->loc_x2 >= cx - radius &&
        townPtr->loc_y1 <= cy + radius && townPtr->loc_y2 >= cy - radius) {
      // ---- add news -------//
      if (townPtr->nation_recno == nation_array.player_recno)
...
void World::visit(int xLoc1, int yLoc1, int xLoc2, int yLoc2, int range,
                  int extend) {
...
void World::visit_shell(int xLoc1, int yLoc1, int xLoc2, int yLoc2,
                        int visitLevel) {
...
void World::process_visibility() {
  if (sys.frame_count & 1) {
    ...
    if (config.fog_of_war && !(nation_array.player_recno &&
                               god_res[GOD_JAPANESE]->nation_prayer_count(
                                   nation_array.player_recno) > 0))
    // ##### patch end Gilbert 18/1 ######//
    {
#ifdef DEBUG
...
process_visit_level2:
    ...
    if (!config.explore_whole_map || config.fog_of_war) {
      sys.zoom_need_redraw =
          1; // request world.zoom_matrix->disp() to call draw()
    }
  }
  // #### end Gilbert 8/2 #####//
}
...
void World::disp_next(int seekDir, int sameNation) {
  ...
  if (unit_array.selected_recno) {
    ...
    while (1) {
      ...
      // ###### begin Gilbert 31/5 ########//
      // if( !world.get_loc(unitPtr->next_x_loc(),
      // unitPtr->next_y_loc())->explored() )
      ...
      if (unit_res[unitPtr->unit_id]->unit_class == unitClass) {
        ...
        world.go_loc(unitPtr->cur_x_loc(), unitPtr->cur_y_loc(),
                     unitPtr->mobile_type == UNIT_AIR ? GO_LOC_SELECT_AIR
                                                      : GO_LOC_SELECT_LAND);
        // ####### end Gilbert 31/5 #########//
        return;
      }
  ...
  if (town_array.selected_recno) {
    ...
    Town *townPtr;
    int isSlaveTown = town_array[townRecno]->is_pay_tribute_to_monster(
        nation_array.player_recno);
    ...
    while (1) {
      ...
      //---- if the originally selected town is a slave town, the next town must
      //be a slave town also ----//

      //			if( isSlaveTown &&
      //!townPtr->is_pay_tribute_to_monster(nation_array.player_recno) )
      //				continue;
  ...
  if (site_array.selected_recno) {
    ...
    while (1) {
      ...
      // )
      if (!world.is_explored(sitePtr->map_x_loc, sitePtr->map_y_loc,
                             sitePtr->map_x2, sitePtr->map_y2))
        // ####### end Gilbert 31/5 #########//
        continue;
...
void World::disp_next_with_filter(int seekDir) {
  ...
  for (int i = objCount; i > 0; i--) {
    ...
    if (!world.get_loc(baseObj->obj_loc_x1(), baseObj->obj_loc_y1())
             ->explored())
      continue;
    ...
    if (map_matrix->filter_object_flag) {
      ...
      case OBJ_TOWN:
        //					objPara =
        //baseObj->cast_to_Town()->is_pay_tribute_to_monster(nation_array.player_recno);
        break;
      }
...
LocationCorners *World::get_loc_corner(int xLoc, int yLoc,
                                       LocationCorners *lc) {
...
int World::interpolate_z(int curX, int curY) {
  LocationCorners locCorner;
  return get_loc_corner(curX / LOCATE_WIDTH, curY / LOCATE_HEIGHT, &locCorner)
      ->interpolate_z(curX, curY);
}
...
int World::effective_distance(int placeId1, int placeId2) {
  err_when(STD_TOWN_LOC_WIDTH !=
           6); // reduce town_town, firm_town, camp_town if town size is smaller
               // in small building set
  ...
  err_when(config.building_size <= 0 ||
           config.building_size > 2); // can handle building_size 1 and 2

#define EFFECTIVE_TOWN_TOWN_DISTANCE                                           \
  dist_town_town[config.building_size -                                        \
                 1] // The minimum distance between 2 towns
#define EFFECTIVE_FIRM_TOWN_DISTANCE                                           \
  dist_firm_town[config.building_size -                                        \
                 1] // maximum distance between the town and the base until it's
                    // no longer considered as command base of the town
#define EFFECTIVE_FIRM_FIRM_DISTANCE                                           \
  dist_firm_firm[config.building_size -                                        \
                 1] // maximum distance between the town and the base until it's
                    // no longer considered as command base of the town
...
#define EFFECTIVE_LAIR_LAIR_DISTANCE dist_lair_lair[config.building_size - 1]
  ...
  //---- swap placeId1 and placeId2 so that placeId1 is always firm and placeId2
  //is also town
...
// <int> returnIfWar	 		  	  - whether return -1 if there is
// war
//											 around the
//given area.
// <int&> hasWar					  - a var for returning whether there
// is war
//											 around the
//given area. 											 1 - if there is war 											 2 - if the attacker nation is involved in the
//war
// <int> scanRange 				  - the scanning range of the
// area
//
...
//              going on.
//
int World::net_attacker_power_in_area(int targetXLoc, int targetYLoc,
                                      int attackerNationRecno,
                                      int defenderNationRecno, int returnIfWar,
                                      int &hasWar, int scanRange) {
  ...
  float netCombatLevel =
      0; // the higher the rating, the easier we can attack the target town.
  int xLoc, yLoc;
  Unit *unitPtr;
  Location *locPtr;
  ...
  for (yLoc = yLoc1; yLoc <= yLoc2; yLoc++) {
    ...
    for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++) {
      ...
      if (locPtr->unit_recno(UNIT_LAND)) {
        ...
        //-- only count when the unit's order is moving or attacking. Assigning
        //and other actions are not counted ---//
        ...
        //--- if the baseObj is a firm, only count obj_power() once, when the
        //location is the center of the firm ---//
