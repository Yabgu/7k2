...
int TalkRes::add_trade_embargo_choices()
{
    ...
    for (int i = 1; i <= nation_array.size(); i++)
    {
        ...
        if (i == cur_talk_msg.from_nation_recno || i == cur_talk_msg.to_nation_recno)
        {
            continue;
        }

        if (fromNation->get_relation(i)->trade_treaty == 0 && toNation->get_relation(i)->trade_treaty == 1)
        {
            //------ add color bar -------//
...
int TalkRes::add_declare_war_choices()
{
    ...
    choice_question = text_talk.str_ask_declare_war(); //"Declare war on which kingdom?";
...
#define MIN_FOOD_PURCHASE_PRICE 5
    ...
    else if (!cur_talk_msg.talk_para2)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (i == 0 || (~nation_array)->cash >=
                              cur_talk_msg.talk_para1 * priceArray[i] / 10) // i==0 to at least add one option
...
int TalkRes::add_tribute_choices()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        if (cur_talk_msg.talk_id == TALK_DEMAND_TRIBUTE || // when demand tribute, the amount can be
                                                           // sent to any
            cur_talk_msg.talk_id == TALK_DEMAND_AID || (~nation_array)->cash >= tributeAmtArray[i])
        {
...
int TalkRes::add_exchange_tech_choices()
{
    ...
    for (i = 1; i <= tech_res.tech_count && talk_choice_count < MAX_TALK_CHOICE - 1;
         i++) // -1 reserve one free space for "Cancel"
    {
        ...
        // ###### begin Gilbert 6/5 ######//
        if (techInfo->class_id == TECH_CLASS_MEGA_WEAPON || techInfo->class_id == TECH_CLASS_OFFENSIVE_STRUCTURE)
        {
            continue;
        }
        // ###### end Gilbert 6/5 ######//
...
int TalkRes::add_request_surrender_choices()
{
    ...
    static const char *strArray[] = {"$5000.",  "$10000.", "$20000.",  "$35000.",
...
int TalkRes::add_surrender_choices()
{
    ...
    str = text_talk.str_ask_confirm_surrender(nation_array[cur_talk_msg.to_nation_recno]->nation_name());
    choice_question = str;
...
int TalkRes::can_send_msg(int toNationRecno, int fromNationRecno, int talkId)
{
    ...
    case TALK_PROPOSE_TRADE_TREATY:
        return relationStatus != RELATION_ALLIANCE && // allied nations are oblied
               ...
               relationStatus != RELATION_HOSTILE && !toNation->get_relation(fromNationRecno)->trade_treaty;

    case TALK_PROPOSE_FRIENDLY_TREATY:
        return relationStatus == RELATION_TENSE || relationStatus == RELATION_NEUTRAL;

    case TALK_PROPOSE_ALLIANCE_TREATY:
        return relationStatus == RELATION_FRIENDLY || relationStatus == RELATION_NEUTRAL;
    ...
    case TALK_REQUEST_MILITARY_AID:
        return fromNation->is_at_war() && (relationStatus == RELATION_FRIENDLY || relationStatus == RELATION_ALLIANCE);

    case TALK_REQUEST_TRADE_EMBARGO:
        return relationStatus == RELATION_FRIENDLY || relationStatus == RELATION_ALLIANCE;
    ...
    case TALK_DECLARE_WAR:
        return relationStatus != RELATION_ALLIANCE && relationStatus != RELATION_FRIENDLY &&
    ...
    case TALK_EXCHANGE_TECH:
        return relationStatus >= RELATION_TENSE && fromNation->total_tech_level() > 0 &&
...
//
int TalkRes::ai_send_talk_msg(int toNationRecno, int fromNationRecno, int talkId, int talkPara1, int talkPara2,
                              int forceSend, int talkPara3, int talkPara4)
{
    ...
    if (oppTalkId)
    {
        fromNation->get_relation(toNationRecno)->set_never_accept_until_date(oppTalkId, 120);
    }
...
void TalkRes::send_talk_msg(TalkMsg *talkMsgPtr, char remoteAction, int forceSend)
{
    ...
    {
        if (talkMsgPtr->reply_type == REPLY_WAITING) // for messages that do not need a reply, duplication in
...
void TalkRes::disp_talk()
{
    ...
    if (!info.player_reply_mode && !can_send_any_msg(cur_talk_msg.to_nation_recno, nation_array.player_recno))
    {
        if (!(choice_question &&
              strcmp(choice_question, MESSAGE_SENT_STR) == 0)) // if it's currently displaying the has
                                                               // sent notification, display that message
        {
            //			char* msgStr = "You've sent too many messages to
            // this kingdom. "
            // "You cannot send any new messages until the existing ones are
            // processed.";
    ...
    if (!reply_talk_msg_recno && !cur_talk_msg.talk_id) // not replying and haven't selected a message type yet.
    {
        for (int i = 0; i < MAX_TALK_TYPE; i++)
        {
            if (available_talk_id_array[i] !=
                can_send_msg(cur_talk_msg.to_nation_recno, cur_talk_msg.from_nation_recno, i + 1))
            {
                set_talk_choices();
                break;
            }
...
int TalkRes::detect_talk()
{
    ...
    if (reply_talk_msg_recno && is_talk_msg_deleted(reply_talk_msg_recno)) // the message may become invalid during the
                                                                           // replying period
    {
    ...
    {
        if (!is_talk_msg_deleted(reply_talk_msg_recno)) // the message may become invalid during the
                                                        // replying period
        {
            if (choicePara == 1)
...
void TalkRes::process_talk_msg()
{
    ...
    for (i = talk_msg_count(); i > 0; i--)
    {
        ...
        if (nation_array[talkMsg->from_nation_recno]->nation_type == NATION_AI &&
            talkMsg->reply_type == REPLY_WAITING && info.game_date > talkMsg->date + DISP_NEWS_DAYS)
        {
...
void TalkRes::del_talk_msg(int talkMsgRecno)
{
    ...
    if (nationPtr->nation_type == NATION_AI &&
        (talkMsg->reply_type == REPLY_NOT_NEEDED || // even if a reply is not needed, the message
         ...
         talkMsg->reply_type == REPLY_WAITING))
    {
        ...
        for (int i = nationPtr->action_count(); i > 0; i--)
        {
            ...
            if (actionNode->action_mode == ACTION_AI_PROCESS_TALK_MSG && actionNode->action_para == talkMsgRecno)
            {
...
int TalkRes::wait_msg_count(int toNationRecno, int fromNationRecno)
{
    ...
    for (i = talk_msg_count(); i > 0; i--)
    {
        ...
        if (talkMsg->reply_type == REPLY_WAITING && talkMsg->to_nation_recno == toNationRecno &&
            talkMsg->from_nation_recno == fromNationRecno &&
...
void TalkRes::del_all_nation_msg(int nationRecno)
{
    ...
    for (i = talk_msg_count(); i > 0; i--)
    {
        ...
        if (talkMsg->from_nation_recno == nationRecno || talkMsg->to_nation_recno == nationRecno ||
