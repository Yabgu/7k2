...
#define MESSAGE_SENT_STR (text_talk.str_msg_sent())
...
int TalkRes::set_talk_choices() {
  ...
  case TALK_DEMAND_TRIBUTE:
    ...
    // add the choice question here because we use the same function for both
    // tribute and aid
    if (rc)
      choice_question = text_talk.str_ask_tribute(); // "How much tribute?";
    break;
  ...
  case TALK_DEMAND_AID:
    rc = add_tribute_choices();
    if (rc)
      choice_question = text_talk.str_ask_aid(); // "How much aid?";
    break;

  case TALK_EXCHANGE_TECH:
    rc = add_exchange_tech_choices();
    break;
  ...
  if (rc)
    ...
    add_talk_choice(text_talk.str_cancel_(), 0);
...
void TalkRes::add_main_choices() {
  ...
  int rc;
  int relationStatus =
      (~nation_array)->get_relation_status(cur_talk_msg.to_nation_recno);

  for (int i = 1; i <= MAX_TALK_TYPE; i++) {
    if (!can_send_msg(cur_talk_msg.to_nation_recno, nation_array.player_recno,
                      i))
      continue;
...
int TalkRes::add_trade_embargo_choices() {
  ...
  choice_question = text_talk.str_ask_embargo(); // "Request an embargo on trade
                                                 // with which kingdom?";
  ...
  for (int i = 1; i <= nation_array.size(); i++) {
    ...
    if (fromNation->get_relation(i)->trade_treaty == 0 &&
      ...
      colorCodeStr[0] =
          FIRST_NATION_COLOR_CODE_IN_TEXT + nation_array[i]->color_scheme_id;
...
int TalkRes::add_declare_war_choices() {
  ...
  choice_question =
      text_talk.str_ask_declare_war(); //"Declare war on which kingdom?";
  ...
  for (int i = 1; i <= nation_array.size(); i++) {
    ...
    //--- can only ask another nation to declare war with a nation that is
    //currently at war with our nation ---//

    if (fromNation->get_relation_status(i) == RELATION_HOSTILE &&
        toNation->get_relation_status(i) != RELATION_HOSTILE) {
      //------ add color bar -------//
...
#define MIN_FOOD_PURCHASE_PRICE 5
  ...
  if (!cur_talk_msg.talk_para1) {
    choice_question =
        text_talk
            .str_ask_buy_food(); //"How much food do you want to purchase?";
    ...
    choice_question = text_talk.str_ask_food_price(); // "How much do you offer
                                                      // for 10 units of food?";
    ...
    for (int i = 0; i < 4; i++) {
      if (i == 0 ||
          (~nation_array)->cash >= cur_talk_msg.talk_para1 * priceArray[i] /
                                       10) // i==0 to at least add one option
...
int TalkRes::add_tribute_choices() {
  ...
  static const char *tributeStrArray[] = {"$500.", "$1000.", "$2000.", "$3000.",
                                          "$4000."};
  static short tributeAmtArray[] = {500, 1000, 2000, 3000, 4000};

  for (int i = 0; i < 5; i++) {
    if (cur_talk_msg.talk_id ==
            TALK_DEMAND_TRIBUTE || // when demand tribute, the amount can be
                                   // sent to any
...
int TalkRes::add_exchange_tech_choices() {
  ...
  if (!cur_talk_msg.talk_para1)
    choice_question =
        text_talk.str_ask_offer_tech(); // "Technology you offer?";
  else
    choice_question =
        text_talk.str_ask_request_tech(); //"Technology you request?";

  for (i = 1;
       i <= tech_res.tech_count && talk_choice_count < MAX_TALK_CHOICE - 1;
       i++) // -1 reserve one free space for "Cancel"
  {
    techInfo = tech_res[i];
    ...
    // ###### begin Gilbert 24/12 #######//
    int maxTechLevel = MAX(techInfo->max_tech_level(fromNationRecno),
                           techInfo->max_tech_level(toNationRecno));
...
//----- End of function TalkRes::add_exchange_tech_choices -------//
...
int TalkRes::add_request_surrender_choices() {
  ...
  choice_question =
      text_talk.str_ask_surrender_price(); // "How much do you offer?";

  static const char *strArray[] = {"$5000.",   "$10000.", "$20000.",
                                   "$35000.",  "$50000.", "$75000.",
                                   "$100000.", "$150000."};

  static int amtArray[] = {5000,  10000, 20000,  35000,
  ...
  for (int i = 0; i < 8; i++) {
    if ((~nation_array)->cash >= amtArray[i]) {
      add_talk_choice(
          strArray[i],
          amtArray[i] / 10); // divided by 10 to cope with the limit of <short>
    }
  }
...
int TalkRes::add_surrender_choices() {
  ...
  // must static str to store string because choice_question is a char *
  str = text_talk.str_ask_confirm_surrender(
      nation_array[cur_talk_msg.to_nation_recno]->nation_name());
  choice_question = str;

  // add_talk_choice( "Confirm.", 1 );
  add_talk_choice(text_talk.str_confirm_(), 1);
...
int TalkRes::can_send_any_msg(int toNationRecno, int fromNationRecno) {
  return wait_msg_count(toNationRecno, fromNationRecno) <
         MAX_WAIT_MSG_PER_NATION;
}
...
int TalkRes::can_send_msg(int toNationRecno, int fromNationRecno, int talkId) {
  ...
  switch (talkId) {
  case TALK_PROPOSE_TRADE_TREATY:
    return relationStatus != RELATION_ALLIANCE && // allied nations are oblied
                                                  // to trade with each other
           relationStatus != RELATION_HOSTILE &&
           !toNation->get_relation(fromNationRecno)->trade_treaty;

  case TALK_PROPOSE_FRIENDLY_TREATY:
    return relationStatus == RELATION_TENSE ||
           relationStatus == RELATION_NEUTRAL;

  case TALK_PROPOSE_ALLIANCE_TREATY:
    return relationStatus == RELATION_FRIENDLY ||
           relationStatus == RELATION_NEUTRAL;
  ...
  case TALK_END_ALLIANCE_TREATY:
    return relationStatus == RELATION_ALLIANCE;

  case TALK_REQUEST_MILITARY_AID:
    return fromNation->is_at_war() && (relationStatus == RELATION_FRIENDLY ||
                                       relationStatus == RELATION_ALLIANCE);
  ...
  case TALK_REQUEST_CEASE_WAR:
    return relationStatus == RELATION_HOSTILE;

  case TALK_REQUEST_DECLARE_WAR: // can only request an allied nation to declare
                                 // war with another nation
  {
    if (relationStatus != RELATION_ALLIANCE)
      return 0;
    ...
    for (int i = nation_array.size(); i > 0; i--) {
      ...
      if (fromNation->get_relation(i)->status == RELATION_HOSTILE)
        return 1;
    }
  ...
  case TALK_GIVE_TRIBUTE:
  case TALK_DEMAND_TRIBUTE:
    return relationStatus <= RELATION_NEUTRAL;
  ...
  case TALK_EXCHANGE_TECH:
    return relationStatus >= RELATION_TENSE &&
           fromNation->total_tech_level() > 0 &&
           toNation->total_tech_level() > 0;

  case TALK_REQUEST_SURRENDER:
    // ######## begin Gilbert 7/5 #######//
    // return 1;
    return fromNation->is_human() == toNation->is_human();
    // ######## end Gilbert 7/5 #######//
...
void TalkRes::add_talk_choice(const char *talkStr, int talkPara,
  ...
  strncpy(talk_choice_array[talk_choice_count - 1].str, talkStr,
          TalkChoice::CHOICE_TEXT_LEN);

  talk_choice_array[talk_choice_count - 1].str[TalkChoice::CHOICE_TEXT_LEN] =
      '\0';
...
//									be checked. (default:
//0)
// [int] talkPara3		 - talk para 3
// [int] talkPara4		 - talk para 4
//
int TalkRes::ai_send_talk_msg(int toNationRecno, int fromNationRecno,
                              int talkId, int talkPara1, int talkPara2,
                              int forceSend, int talkPara3, int talkPara4) {
  Nation *fromNation = nation_array[fromNationRecno];

  if (!fromNation->is_ai())
    return 0;

  //--- if the AI has been hard-wired to never consider this diplomatic option
  //with the toNationRecno, then do nothing ---//

  if (fromNation->get_relation(toNationRecno)->ai_never_consider[talkId - 1])
    return 0;
  ...
  if (oppTalkId) {
    fromNation->get_relation(toNationRecno)
        ->set_never_accept_until_date(oppTalkId, 120);
  }
  ...
  talkMsg.talk_para3 = talkPara3;
  talkMsg.talk_para4 = talkPara4;
...
// [int]  	  forceSend	   - if this is 1, the message will be sent
// without
//									  checking can_send_msg.
//(default: 0)
//
void TalkRes::send_talk_msg(TalkMsg *talkMsgPtr, char remoteAction,
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    char *dataPtr =
        remote.new_send_queue_msg(MSG_SEND_TALK_MSG, sizeof(TalkMsg));
  ...
  if (nation_array[talkMsgPtr->from_nation_recno]->nation_type == NATION_AI) {
    if (talkMsgPtr->reply_type ==
        REPLY_WAITING) // for messages that do not need a reply, duplication in
                       // the message log is allowed.
    {
      if (is_talk_msg_exist(talkMsgPtr,
                            0)) // 0-don't check talk_para1 & talk_para2
        return;
    }
  }

  //--- in a multiplayer game, when the msg comes back from the network,
  //can_send_msg might be different, so we have to check it again ---//

  if (!forceSend &&
      !can_send_msg(talkMsgPtr->to_nation_recno, talkMsgPtr->from_nation_recno,
                    talkMsgPtr->talk_id))
    return;
...
void TalkRes::send_talk_msg_now(TalkMsg *talkMsgPtr) {
  ...
  talkMsgPtr->relation_status =
      toNation->get_relation_status(talkMsgPtr->from_nation_recno);
  ...
  case NATION_OWN: // can be from both AI or a remote player
    if (news_array.diplomacy(talk_msg_array.recno()))
      se_ctrl.immediate_sound(talkMsgPtr->talk_id == TALK_DECLARE_WAR
                                  ? (char *)"DECL_WAR"
                                  : (char *)"GONG");
    break;
  ...
  case NATION_AI:
    if (talkMsgPtr->reply_type == REPLY_WAITING) {
      ...
      toNation->add_action(0, 0, 0, 0, ACTION_AI_PROCESS_TALK_MSG,
                           talk_msg_array.recno());
  ...
  case NATION_REMOTE: // do nothing here as NATION_OWN handle both msg from AI
                      // and a remote player
    break;
  }
}
...
//
void TalkRes::reply_talk_msg(int talkMsgRecno, char replyType,
                             char remoteAction) {
  //-------- send multiplayer -----------//

  if (!remoteAction && remote.is_enable()) {
    // packet structure : <talkRecno:int> <reply type:char> <padding:char>
    char *charPtr = remote.new_send_queue_msg(MSG_REPLY_TALK_MSG,
  ...
  case NATION_AI:
    fromNation->ai_notify_reply(
        talkMsgRecno); // notify the AI nation about this reply.
    break;
  ...
  //---- set the replying nation's never accept date so that the replying nation
  //will not make an offer soon after it rejects the offer

  nation_array[talkMsgPtr->to_nation_recno]
      ->get_relation(talkMsgPtr->from_nation_recno)
...
void TalkRes::disp_talk() {
  ...
  if (!info.player_reply_mode && !can_send_any_msg(cur_talk_msg.to_nation_recno,
                                                   nation_array.player_recno)) {
    if (!(choice_question && strcmp(choice_question, MESSAGE_SENT_STR) ==
                                 0)) // if it's currently displaying the has
                                     // sent notification, display that message
    {
      //			char* msgStr = "You've sent too many messages to
      //this kingdom. " 								"You cannot send any new messages until the existing
      //ones are processed.";
      font_bld.put_paragraph(TALK_X1, TALK_Y1, TALK_X2, TALK_Y2,
                             text_talk.str_too_many_msg(), 4);
      return;
    }
  }
  ...
  if (!reply_talk_msg_recno &&
      !cur_talk_msg
           .talk_id) // not replying and haven't selected a message type yet.
  {
    for (int i = 0; i < MAX_TALK_TYPE; i++) {
      if (available_talk_id_array[i] !=
          can_send_msg(cur_talk_msg.to_nation_recno,
                       cur_talk_msg.from_nation_recno, i + 1)) {
        set_talk_choices();
        break;
      }
    }
  ...
  for (int i = 0; i < talk_choice_count; i++, y += TALK_LINE_HEIGHT) {
    if (i + 1 ==
        cur_choice_id) // this is the one the mouse cursor is currently on
      // vga_back.adjust_brightness( TALK_X1, y, TALK_X2, y+TALK_LINE_HEIGHT-1,
      // -3 );
      vga_back.bar_alpha(TALK_X1, y, TALK_X2, y + TALK_LINE_HEIGHT - 1, 1,
                         VGA_GRAY + 8);
    ...
    // ###### begin Gilbert 4/1 ########//
    //		font_san.put_paragraph( TALK_X1+3, y+2, TALK_X2-3,
    //y+font_san.height()+1, str );
    font_bld.put_paragraph(TALK_X1 + 3, y + 2, TALK_X2 - 3,
                           y + font_bld.height() + 1, str);
    // ###### end Gilbert 4/1 ########//
  }
}
...
int TalkRes::detect_talk_choices() {
  ...
  if (newChoiceId &&
      mouse.single_click(TALK_X1, y, TALK_X2, y + TALK_LINE_HEIGHT - 1))
    return newChoiceId;
...
int TalkRes::detect_talk() {
  //----- if the message which player is reply is deleted (this can only happen
  //when the player stays on the reply message screen for almost 1 year without
  //doing anything and then the message is automatically deleted after having
  //being kept for 1 year ---//

  if (reply_talk_msg_recno &&
      is_talk_msg_deleted(
          reply_talk_msg_recno)) // the message may become invalid during the
                                 // replying period
  {
  ...
  if (strcmp(talk_choice_array[choiceId - 1].str, text_talk.str_cancel_()) ==
          0 ||
  ...
  if (cur_talk_msg.talk_id == 0) {
    ...
    cur_talk_msg.talk_para3 =
        choicePara2; // if the talk is Exchange Tech, choicePara2 is the version
...
void TalkRes::del_talk_msg(int talkMsgRecno) {
  ...
  if (nationPtr->nation_type == NATION_AI &&
      (talkMsg->reply_type ==
           REPLY_NOT_NEEDED || // even if a reply is not needed, the message
                               // will still be sent to the AI for notification.
       talkMsg->reply_type == REPLY_WAITING)) {
...
void TalkRes::player_reply(int talkMsgRecno) {
  ...
  msgStr =
      talkMsg->msg_str(nation_array.player_recno); // make a static copy of it.
  choice_question = msgStr;

  //---- see if this message has a second line -----//

  msgStr2 = talkMsg->msg_str(nation_array.player_recno, 0,
                             1); // 1-display the second line of the question
  ...
  if (talkMsg->can_accept()) // whether the replier can accept the request or
                             // demand of the message
...
void TalkRes::del_all_nation_msg(int nationRecno) {
  ...
  for (i = talk_msg_count(); i > 0; i--) {
    ...
    if (talkMsg->from_nation_recno == nationRecno ||
        ...
        !talkMsg->is_valid_to_disp()) // if the nation is the talk_para1 (e.g.
                                      // TRADE_EMBARGO), is_valid_to_disp() will
                                      // return 0 and the talk message should
                                      // than be deleted
    {
      del_talk_msg(i);
    }
...
void TalkRes::create_incident(int nation1, int nation2) {
  ...
  //-- if we are currently allied or friendly with the nation, we need to
  //terminate the friendly/alliance treaty first --//
  ...
  if (toNation->get_relation_status(nation2) == RELATION_ALLIANCE) {
    ...
    talkMsg.talk_id = TALK_END_ALLIANCE_TREATY;
    send_talk_msg(&talkMsg, COMMAND_AUTO,
                  1); // force sending the message and bypass can_send_msg()
  }
