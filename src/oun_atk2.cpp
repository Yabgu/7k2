...
void Unit::choose_best_attack_mode(int attackDistance, char targetMobileType) {
  ...
  err_when(attackModeBeingUsed < 0 ||
           attackModeBeingUsed > MAX_UNIT_ATTACK_TYPE);
  ...
  if (attack_count > 1) {
    ...
    for (UCHAR i = 0; i < attack_count; i++) {
      ...
      if (can_attack_with(attackInfoChecking) &&
          attackInfoChecking->attack_range >= attackDistance) {
        //-------------------- able to attack ----------------------//
        canAttack = 1;
        ...
        if (attackDistance == 1 &&
            (targetMobileType != UNIT_AIR && mobile_type != UNIT_AIR)) {
          ...
          if (attackInfoSelected->attack_range == attackDistance) {
            if (attackInfoChecking->attack_range == attackDistance &&
              ...
              attackModeBeingUsed =
                  UCHAR(i); // choose the one with strongest damage
              attackInfoSelected = attackInfoChecking;
            }
            continue;
        ...
        if (checkingDamageWeight == selectedDamageWeight) {
          if (attackInfoChecking->attack_range <
              attackInfoSelected->attack_range) {
          ...
          //--------------------------------------------------------------------------//
          // select one that can do the attacking immediately with the strongest
          // damage point
          //--------------------------------------------------------------------------//
          attackModeBeingUsed = UCHAR(i);
          attackInfoSelected = attackInfoChecking;
      ...
      if (!canAttack) {
        //------------------------------------------------------------------------------//
        // if unable to attack the target, choose the mode with longer
        // attack_range and heavier damage
        //------------------------------------------------------------------------------//
        if (can_attack_with(attackInfoChecking) &&
            (attackInfoChecking->attack_range >
                 attackInfoMaxRange->attack_range ||
             (attackInfoChecking->attack_range ==
                  attackInfoMaxRange->attack_range &&
              attackInfoChecking->attack_damage >
                  attackInfoMaxRange->attack_damage))) {
    ...
    if (canAttack)
      cur_attack = attackModeBeingUsed; // choose the strongest damage mode if
                                        // able to attack
    else
      cur_attack = maxAttackRangeMode; //	choose the longest attack range
                                       //if unable to attack
  } else {
...
//
int Unit::cal_distance(int targetXLoc, int targetYLoc, int targetWidth,
                       int targetHeight) {
...
int Unit::can_attack_with(int attackId) {
  ...
  if (attackInfo->consume_power >= 20 && is_human() &&
    ...
    if (!nation_recno ||
        !tech_res[TECH_SPU_BERSERK(race_id)]->get_nation_tech_level(
            nation_recno)) {
      return 0;
    }
  }
  ...
  int skillBonus;
  //	if( race_id == RACE_VIKING && nation_recno && attackInfo->consume_power
  //> 20
  ...
  // ##### patch begin Gilbert 29/3 ######//
    ...
    skillBonus =
        25; // grokken in monster fortress can fire at most combat level
...
float Unit::actual_damage() {
  ...
  if (100 == MAX_COMBAT_TRAIN) {
    deciDamage += attackInfo->pierce_damage *
                  misc.random(1 + MIN(combatLevel, MAX_COMBAT_TRAIN));
  } else {
    deciDamage += attackInfo->pierce_damage *
                  misc.random(1 + MIN(combatLevel, MAX_COMBAT_TRAIN)) * 100 /
                  MAX_COMBAT_TRAIN;
  }
  ...
  int itemDamage =
      item.ability(attackInfo->attack_range > 1 ? ITEM_ABILITY_ARROW_DAMAGE
  ...
  if (combatLevel > MAX_COMBAT_TRAIN) {
    deciDamage += combatLevel *
                  (100 / 50); // 1 more basic damage for every 50 combat skill
  }
  ...
  if (nation_recno && is_human()) {
    if (unit_id == race_res[race_id]->infantry_unit_id ||
      ...
      if (attack_range() <= 1)
        attackDamage +=
            tech_res[TECH_INFANTRY_CLOSE_COMBAT]->get_nation_tech_level(
                nation_recno) *
            2;
      else
        attackDamage +=
            tech_res[TECH_INFANTRY_RANGE_COMBAT]->get_nation_tech_level(
                nation_recno) *
            2;
    }
  }
  ...
  //--- if this unit is led by a general, its attacking ability is influenced by
  //the general ---//
  //
  ...
  if (leader_unit_recno) {
    if (unit_array.is_deleted(leader_unit_recno)) {
      ...
      if (leaderXLoc >= 0 &&
        ...
        deciDamage +=
            MIN(leaderBonus,
                1000); // 10 damage, avoid cannon or catapult too strong
        // ##### end Gilbert 17/6 #####//
        // ##### begin Gilbert 29/3 ######//
  ...
  // return (float) attackDamage / ATTACK_SLOW_DOWN;		// lessen all
  // attacking damages, thus slowing down all battles.
  // ###### begin Gilbert 17/6 #######//
  return (float)deciDamage * 0.01f +
         (float)attackDamage; // divide ATTACK_SLOW_DOWN in calculating damage
                              // ###### end Gilbert 17/6 #######//
}
...
float Unit::obj_armor() {
  ...
  // ##### end Gilbert 22/10 ########//
  {
    if (unit_id == race_res[race_id]->infantry_unit_id ||
        unit_id == race_res[race_id]->special_unit_id) {
      armor +=
          tech_res[TECH_INFANTRY_DEFENSE]->get_nation_tech_level(nation_recno) *
          2;
    }
  }
...
void Unit::add_attack_effect() {
  ...
  if (attackInfo->effect2_id != 0) {
    ...
    if (!base_obj_array.is_deleted(cur_order.para) &&
        ...
        unit_res[targetUnit->unit_id]->class_info.life >
            5) // exclude insect and wagon
    {
  ...
  if (effectId) {
    ...
    Effect::create(effectId, x, y, SPRITE_IDLE, cur_dir,
                   mobile_type == UNIT_AIR ? 8 : 2, 0);
  }
}
...
void Unit::add_die_effect() {
  ...
  if (effectId) {
    Effect::create(effectId, cur_x, cur_y, SPRITE_IDLE, 1,
                   mobile_type == UNIT_AIR ? 8 : 2, 0);
  }
}
...
void Unit::get_hit_x_y(short *xPtr, short *yPtr, char distance) {
  ...
  case 0: // north
    ...
    *yPtr = cur_y + LOCATE_HEIGHT * loc_height / 2 - LOCATE_HEIGHT * 1 / 2 -
            LOCATE_HEIGHT * distance;
    break;
  case 1: // north east
    // later replace 1 with effect's loc_width
    *xPtr = cur_x + LOCATE_WIDTH * loc_width / 2 - LOCATE_WIDTH * 1 / 2 +
            LOCATE_WIDTH * distance;
    *yPtr = cur_y + LOCATE_HEIGHT * loc_height / 2 - LOCATE_HEIGHT * 1 / 2 -
            LOCATE_HEIGHT * distance;
    break;
  case 2: // east
