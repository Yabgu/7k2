...
int Unit::can_attack_with(int attackId)
{
    ...
    //
    if (attackInfo->consume_power >= 20 && is_human() && unit_id == race_res[race_id]->special_unit_id)
    {
        err_when(race_id > MAX_RACE);

        if (!nation_recno || !tech_res[TECH_SPU_BERSERK(race_id)]->get_nation_tech_level(nation_recno))
        {
            return 0;
        }
    }
    ...
    if (nation_recno && attackInfo->consume_power > 20 && god_res[GOD_VIKING]->nation_prayer_count(nation_recno) > 0)
        skillBonus = 25; // viking soldier can use power attack earlier
    // ##### patch end Gilbert 26/2 ######//
    // ##### patch begin Gilbert 29/3 ######//
    else if (unit_id == UNIT_ROCK && attackInfo->consume_power <= 0 && unit_mode == UNIT_MODE_OVERSEE &&
             !firm_array.is_deleted(unit_mode_para) && firm_array[unit_mode_para]->firm_id == FIRM_FORTRESS)
        skillBonus = 25; // grokken in monster fortress can fire at most combat level
    // ##### patch end Gilbert 29/3 ######//
    else
        skillBonus = 0;

    return ((combat_level() + skillBonus >= attackInfo->combat_level && cur_power >= attackInfo->min_power) ||
            (sys.testing_session));
}
...
float Unit::actual_damage()
{
    ...
    {
        deciDamage +=
            attackInfo->pierce_damage * misc.random(1 + MIN(combatLevel, MAX_COMBAT_TRAIN)) * 100 / MAX_COMBAT_TRAIN;
    }
    ...
    {
        if (unit_id == race_res[race_id]->infantry_unit_id || unit_id == race_res[race_id]->special_unit_id)
        {
            if (attack_range() <= 1)
