...
static long gold_coin_amount_array[GOLD_COIN_AMOUNT_COUNT] = {
    500, 600, 700, 800, 900, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000, 7000, 8000, 9000, 10000,
};
...
void ScenarioEditor::disp_misc_main(int refreshFlag)
{
    ...
    if (rock_or_site == 0) // terrain objects mode
    {
        ...
        if (refreshFlag == INFO_REPAINT)
        {
            ...
            for (int i = 0; i < ROCK_TYPES; ++i)
            {
                ...
                {
                    button_rock_type[i].create_text(INFO_X1 + 5, INFO_Y1 + 30 + j * 21, INFO_X2 - 5,
                ...
                else
                {
                    ...
                    button_rock_type[i].create_text(INFO_X1 + 5, INFO_Y1 + 30 + 0 * 21, INFO_X2 - 5,
        ...
        if (refreshFlag == INFO_REPAINT)
        {
            browse_rock.init(INFO_X1 + 5, INFO_Y1 + 140, INFO_X2 - 5, INFO_Y2 - 30, -1, 25,
                             rock_browse_count[rock_type], disp_rock_rec);
    ...
    else if (rock_or_site == 1) // site mode
    {
        ...
        case SITE_RAW:
            if (refreshFlag == INFO_REPAINT)
            {
                browse_raw.init(INFO_X1 + 5, INFO_Y1 + 140, INFO_X2 - 5, INFO_Y2 - 50, -1, 36, MAX_RAW, disp_raw_rec);
        ...
        case SITE_SCROLL:
            if (refreshFlag == INFO_REPAINT)
            {
                browse_scroll.init(INFO_X1 + 5, INFO_Y1 + 140, INFO_X2 - 5, INFO_Y2 - 30, -1, 45, MAX_RACE,
                                   disp_scroll_rec);
        ...
        case SITE_GOLD_COIN:
            if (refreshFlag == INFO_REPAINT)
            {
                browse_gold.init(INFO_X1 + 5, INFO_Y1 + 140, INFO_X2 - 5, INFO_Y2 - 30, -1, 25, GOLD_COIN_AMOUNT_COUNT,
                                 disp_gold_rec);
        ...
        case SITE_ITEM:
            if (refreshFlag == INFO_REPAINT)
            {
                browse_item.init(INFO_X1 + 5, INFO_Y1 + 140, INFO_X2 - 5, INFO_Y2 - 30, -1, 25, ITEM_TYPE_COUNT,
                                 disp_item_rec);
        ...
        case SITE_WEAPON_BLUEPRINT:
            if (refreshFlag == INFO_REPAINT)
            {
                browse_tech.init(INFO_X1 + 5, INFO_Y1 + 140, INFO_X2 - 5, INFO_Y2 - 50, -1, 35,
                                 tech_res.tech_class(TECH_CLASS_MEGA_WEAPON)->tech_count, disp_tech_rec);
    ...
    else if (rock_or_site == 2) // plant mode
    {
        ...
        if (refreshFlag == INFO_REPAINT)
        {
            browse_plant.init(INFO_X1 + 5, INFO_Y1 + 140, INFO_X2 - 5, INFO_Y2 - 30, -1, 25, plant_res.plant_count,
                              disp_plant_rec);
        ...
        for (int i = 0; i < PLANT_SIZE_MAX; ++i)
        {
            ...
            button_plant_size[i].enable_flag =
                !browse_plant.none_record && plant_res[plant_browse_recno]->bitmap_count > i;
        }
    ...
    else if (rock_or_site == 1)
    {
                    ...
                    world.zoom_matrix->get_detect_location(mouse.click_x(0), mouse.click_y(0), &xLoc, &yLoc, NULL))
                {
                    ...
                    if (world.can_build_site(xLoc, yLoc, siteWidth, siteHeight, INTER_PLACE_SPACE))
                        site_array.add_site(xLoc, yLoc, SITE_RAW, raw_browse_recno,
                                            MAX_RAW_RESERVE_QTY * (50 + misc.random(50)) / 100);
                }
        ...
        case SITE_ITEM:
            if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, 0))
            {
                ...
                {
                    if (world.get_loc(xLoc, yLoc)->can_build_site())
                        site_array.add_site(xLoc, yLoc, SITE_ITEM, item_browse_recno,
        ...
        case SITE_WEAPON_BLUEPRINT:
            if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, 0))
            {
                ...
                {
                    if (world.get_loc(xLoc, yLoc)->can_build_site())
                        site_array.add_site(xLoc, yLoc, SITE_WEAPON_BLUEPRINT,
                                            tech_browse_recno +
                                                tech_res.tech_class(TECH_CLASS_MEGA_WEAPON)->first_tech_id - 1);
                }
    ...
    else if (rock_or_site == 2) // plant
    {
                               ...
                               0)) // left button, add rock
        {
                ...
                world.zoom_matrix->get_detect_location(mouse.click_x(0), mouse.click_y(0), &xLoc, &yLoc, NULL))
            {
                ...
                {
                    locPtr->set_plant(plant_res[plant_browse_recno]->first_bitmap + plant_size, LOCATE_WIDTH / 2,
                                      LOCATE_HEIGHT / 2);
...
{
    RockInfo *rockInfo =
        rock_res.get_rock_info(scenario_editor.rock_browse_array[scenario_editor.rock_type][recNo - 1]);
