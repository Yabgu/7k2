...
// static char *rock_type_str[ROCK_TYPES] = { "Rock", "Dirt", "Flat Rock",
// "Lake" }; static char *site_type_str[MAX_SITE_TYPE] = { "Raw", "Power",
// "Coins", "Item", "Weapon" };
...
static ButtonGroup
    button_rock_type(ROCK_TYPES); // rock, hill, dirt, blocking dirt
static ButtonGroup button_site_type(
    MAX_SITE_TYPE); // SITE_RAW, SITE_SCROLL, SITE_GOLD_COIN, SITE_ITEM,
...
void ScenarioEditor::init_misc_mode() {
  ...
  err_when(sizeof(rock_browse_array) / sizeof(rock_browse_array[0]) <
           ROCK_TYPES); // check enough storage

  // [0] is the whole array
  // [1], [2], [3] points to the inside of [0]
  rock_browse_array[0] =
      (short *)mem_add(rock_res.rock_info_count * sizeof(short));
  int i, j;
  int k = 0;
...
void ScenarioEditor::disp_misc_main(int refreshFlag) {
  ...
  if (refreshFlag == INFO_REPAINT) {
    button_rock_or_site[0].create_text(
        INFO_X1 + 5, INFO_Y1 + 5, INFO_X1 + 74, INFO_Y1 + 25,
        text_editor.str_misc_mode(0), 0); // "Terrain", 0 );
    button_rock_or_site[1].create_text(
        INFO_X1 + 76, INFO_Y1 + 5, INFO_X1 + 145, INFO_Y1 + 25,
        text_editor.str_misc_mode(1), 0); // "Objects", 0 );
    button_rock_or_site[2].create_text(
        INFO_X1 + 147, INFO_Y1 + 5, INFO_X1 + 216, INFO_Y1 + 25,
  ...
  if (rock_or_site == 0) // terrain objects mode
  {
    ...
    if (refreshFlag == INFO_REPAINT) {
      ...
      for (int i = 0; i < ROCK_TYPES; ++i) {
        //				button_rock_type[i].create_text(
        //INFO_X1+5+i*50, INFO_Y1+30, INFO_X1+52+i*50, INFO_Y1+50,
        //					text_editor.str_misc_mode(rock_or_site,
        //i), 0 ); // rock_type_str[i], 0 );
        if (rock_browse_count[i]) {
          button_rock_type[i].create_text(
              INFO_X1 + 5, INFO_Y1 + 30 + j * 21, INFO_X2 - 5,
              INFO_Y1 + 49 + j * 21, text_editor.str_misc_mode(rock_or_site, i),
              0); // rock_type_str[i], 0 );
          j++;
        } else {
          ...
          button_rock_type[i].create_text(
              INFO_X1 + 5, INFO_Y1 + 30 + 0 * 21, INFO_X2 - 5,
              INFO_Y1 + 49 + 0 * 21, text_editor.str_misc_mode(rock_or_site, i),
              0); // rock_type_str[i], 0 );
  ...
  } else if (rock_or_site == 1) // site mode
  {
    ...
    if (refreshFlag == INFO_REPAINT) {
      for (int i = 0; i < MAX_SITE_TYPE; ++i) {
        //				button_site_type[i].create_text(
        //INFO_X1+5+i*42, INFO_Y1+30, INFO_X1+45+i*42, INFO_Y1+50,
        //					text_editor.str_misc_mode(rock_or_site,
        //i), 0 );
        button_site_type[i].create_text(
            INFO_X1 + 5, INFO_Y1 + 30 + i * 21, INFO_X2 - 5,
            INFO_Y1 + 49 + i * 21, text_editor.str_misc_mode(rock_or_site, i),
            0);
      }
    }
    ...
    case SITE_SCROLL:
      if (refreshFlag == INFO_REPAINT) {
        browse_scroll.init(INFO_X1 + 5, INFO_Y1 + 140, INFO_X2 - 5,
                           INFO_Y2 - 30, -1, 45, MAX_RACE, disp_scroll_rec);
    ...
    case SITE_WEAPON_BLUEPRINT:
      if (refreshFlag == INFO_REPAINT) {
        browse_tech.init(
            INFO_X1 + 5, INFO_Y1 + 140, INFO_X2 - 5, INFO_Y2 - 50, -1, 35,
            tech_res.tech_class(TECH_CLASS_MEGA_WEAPON)->tech_count,
            disp_tech_rec);
  ...
  } else if (rock_or_site == 2) // plant mode
  {
    ...
    for (int i = 0; i < PLANT_SIZE_MAX; ++i) {
      if (refreshFlag == INFO_REPAINT) {
        button_plant_size[i].create_text(
            INFO_X1 + 5, INFO_Y1 + 30 + i * 21, INFO_X2 - 5,
            INFO_Y1 + 49 + i * 21, text_editor.str_misc_mode(rock_or_site, i),
            0);
      }

      button_plant_size[i].enable_flag =
          !browse_plant.none_record &&
...
void ScenarioEditor::disp_misc_view() {
  ...
  if (rock_or_site == 0) // terrain objects mode
  {
    if (mouse.in_area(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2) &&
        world.zoom_matrix->get_detect_location(mouse.cur_x, mouse.cur_y, &xLoc,
                                               &yLoc, NULL)) {
      ...
      // construction marker
        ...
        int rockId =
            rock_browse_array[rock_type][rock_browse_recno[rock_type] - 1];
        ...
        if (rockInfo->rock_type == ROCK_BLOCKING_TYPE) {
          if (world.can_add_rock(xLoc, yLoc, xLoc + rockInfo->loc_width - 1,
                                 yLoc + rockInfo->loc_height - 1))
            validAction = 1;
        } else {
          if (world.can_add_dirt(xLoc, yLoc, xLoc + rockInfo->loc_width - 1,
                                 yLoc + rockInfo->loc_height - 1))
            validAction = 1;
        }
      }
    ...
    if (mouse.in_area(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2) &&
      ...
      // construction marker
        ...
        if ((drawMarker =
          ...
          validAction = world.can_build_site(xLoc, yLoc, siteWidth, siteHeight,
                                             INTER_PLACE_SPACE)
                            ? 1
                            : 0;
        else
          validAction = 0;
                 ...
                 site_type == SITE_WEAPON_BLUEPRINT &&
                     !browse_tech.none_record) {
        drawMarker = 1;
...
int ScenarioEditor::detect_misc_view() {
  ...
  if (rock_or_site == 0) // terrain objects mode
  {
    ...
    if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2,
                           0)) // left button, add rock
    {
      int xLoc, yLoc;
      if (!browse_rock.none_record &&
          world.zoom_matrix->get_detect_location(
    ...
    if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2,
                           1)) // right button, remove rock
    {
      int xLoc, yLoc;
      if (world.zoom_matrix->get_detect_location(
              mouse.click_x(1), mouse.click_y(1), &xLoc, &yLoc, NULL)) {
        Location *locPtr = world.get_loc(xLoc, yLoc);
        if (locPtr->is_rock()) {
    ...
    case SITE_RAW:
      if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, 0)) {
        ...
        if (!browse_raw.none_record &&
          ...
          if (world.can_build_site(xLoc, yLoc, siteWidth, siteHeight,
                                   INTER_PLACE_SPACE))
            site_array.add_site(xLoc, yLoc, SITE_RAW, raw_browse_recno,
                                MAX_RAW_RESERVE_QTY * (50 + misc.random(50)) /
                                    100);
        }
      ...
      if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, 0)) {
        ...
        if (!browse_gold.none_record &&
            world.zoom_matrix->get_detect_location(
                mouse.click_x(0), mouse.click_y(0), &xLoc, &yLoc, NULL)) {
          if (world.get_loc(xLoc, yLoc)->can_build_site())
            site_array.add_site(xLoc, yLoc, SITE_GOLD_COIN,
                                gold_coin_amount_array[gold_browse_recno - 1]);
        }
    ...
    case SITE_ITEM:
      if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, 0)) {
        ...
        if (!browse_item.none_record &&
          ...
          if (world.get_loc(xLoc, yLoc)->can_build_site())
            site_array.add_site(
                xLoc, yLoc, SITE_ITEM, item_browse_recno,
                item_res.random_para(item_browse_recno, misc.rand()));
        }
    ...
    case SITE_WEAPON_BLUEPRINT:
      if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2, 0)) {
        ...
        if (!browse_tech.none_record &&
          ...
          if (world.get_loc(xLoc, yLoc)->can_build_site())
            site_array.add_site(
                xLoc, yLoc, SITE_WEAPON_BLUEPRINT,
                tech_browse_recno +
                    tech_res.tech_class(TECH_CLASS_MEGA_WEAPON)->first_tech_id -
                    1);
        }
    ...
    if (mouse.double_click(ZOOM_X1, ZOOM_Y1, ZOOM_X2, ZOOM_Y2,
                           1)) // double right click to remove site
    {
      int xLoc, yLoc;
      if (world.zoom_matrix->get_detect_location(
              mouse.click_x(1), mouse.click_y(1), &xLoc, &yLoc, NULL)) {
                           ...
                           0)) // left button, add rock
    {
      ...
      if (!browse_plant.none_record &&
        ...
        if (locPtr->can_add_plant()) {
          locPtr->set_plant(plant_res[plant_browse_recno]->first_bitmap +
                                plant_size,
...
static void disp_tech_rec(int recNo, int x, int y, int refreshFlag) {
  int techId =
      tech_res.tech_class(TECH_CLASS_MEGA_WEAPON)->first_tech_id + recNo - 1;
