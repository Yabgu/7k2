...
static char report_mode_disp_order[MAX_NATION_REPORT_MODE] = {
    NATION_REPORT_TALK, NATION_REPORT_INFO, NATION_REPORT_CHAT, NATION_REPORT_TALK_LOG, NATION_REPORT_DEBUG};
...
//												  to
// this report. 							  INFO_UPDATE  - the report
// is already in this mode, just keep displaying and updating.
//
void Info::disp_nation(int refreshFlag)
{
    int hideNationBrowse = nation_report_mode == NATION_REPORT_CHAT && hide_nation_browse;
...
#undef X_SHIFT
    ...
    {
        browse_nation.init(REPORT_BROWSE_X1, REPORT_BROWSE_Y1 + 35, REPORT_BROWSE_X2, REPORT_BROWSE_Y2, 0, 20,
...
int Info::nation_filter(int recNo)
{
    ...
    for (i = 1; i <= nation_array.size(); i++)
    {
        ...
        if (i == info.viewing_nation_recno || !viewingNation || viewingNation->get_relation(i)->has_contact)
        {
            nationCount++;
        }
...
static void put_nation_rec(int recNo, int x, int y, int refreshFlag)
{
    ...
    if (nationRecno != info.viewing_nation_recno)
    {
        ...
        {
            font_bld.put(x + 510, y,
                         misc.format(nationPtr->get_relation(info.viewing_nation_recno)->ai_relation_level));
            font_bld.put(
                x + 180, y,
                misc.format((int)nationPtr->get_relation(info.viewing_nation_recno)->good_relation_duration_rating));
        }
    }
}
...
static void disp_detail(int refreshFlag)
{
    ...
    // ##### patch begin Gilbert 26/8 ########//
    if (refreshFlag == INFO_REPAINT || info.nation_report_mode != last_nation_report_mode)
    {
...
static void disp_nation_info()
{
    ...
    if (nationRecno == info.viewing_nation_recno)
    {
        ...
        if (nationPtr->is_monster())
            font_bld.put_field(x1, y += lineSpacing, text_reports.str_your_live_points(), x2,
                               (int)nationPtr->live_points);

        font_bld.field(x1, y += lineSpacing, text_reports.str_your_peace_time(), x2, nationPtr->peace_duration_str(),
                       x3, INFO_REPAINT, "PEACE");
    }
    else
    {
        ...
        font_bld.field(x1, y, text_reports.str_enemy_import_from(nationPtr->nation_name()), x2,
                       (int)viewingNation->get_relation(nationRecno)->import_365days(IMPORT_TOTAL), 2, x3, INFO_REPAINT,
                       "IMPORT");
        ...
        font_bld.field(x1, y += lineSpacing, text_reports.str_enemy_export_to(nationPtr->nation_name()), x2,
                       (int)nationPtr->get_relation(info.viewing_nation_recno)->import_365days(IMPORT_TOTAL), 2, x3,
                       INFO_REPAINT, "EXPORT");
        ...
        font_bld.field(x1, y += lineSpacing, text_reports.str_enemy_peace_time(nationPtr->nation_name()), x2,
                       nationPtr->peace_duration_str(), x3, INFO_REPAINT, "PEACE");
        ...
        font_bld.field(x1, y += lineSpacing, text_reports.str_enemy_status_duration(nationRelation->status_str()), x2,
                       nationRelation->status_duration_str(), x3, INFO_REPAINT, "STATTIME");
        ...
        // "ALLOWATK" );
        font_bld.field(x1, y += lineSpacing, text_reports.str_enemy_allow_attack(nationPtr->nation_name()), x2, "", x3,
                       INFO_REPAINT, "ALLOWATK");
        ...
        button_allow_attack[0].create_text(x2 + 54, y, x2 + 100, y + lineSpacing - 1,
        ...
        if (viewingNation->get_relation(nationRecno)->status == RELATION_ALLIANCE)
        {
            ...
            // INFO_REPAINT );
            font_bld.field(x1, y += lineSpacing, text_reports.str_ally_cash(nationPtr->nation_name()), x2,
            ...
            // INFO_REPAINT );
            font_bld.field(x1, y += lineSpacing, text_reports.str_ally_food(nationPtr->nation_name()), x2,
    ...
    if (info.viewing_nation_recno != nationRecno)
    {
        ...
        {
            if (nation_array.is_deleted(i) || i == nationRecno || i == info.viewing_nation_recno)
            {
                continue;
            }
...
static void disp_nation_talk()
{
    ...
    if (nationRecno != info.viewing_nation_recno && info.viewing_nation_recno == nation_array.player_recno)
    {
...
static void disp_talk_msg_sent(int refreshFlag)
{
    ...
    {
        browse_talk_msg.init(REPORT_DET_X1, REPORT_DET_Y1, REPORT_DET_X2, REPORT_DET_Y2, 0, 30,
                             info.talk_msg_disp_array.size(), put_talk_msg_rec, 1);
...
static void put_talk_msg_rec(int recNo, int x1, int y, int refreshFlag)
{
    ...
    font_bld.put(x1, y + 13, talkMsg->msg_str(info.viewing_nation_recno, talkMsgDisp->is_reply), 0,
                 browse_talk_msg.ix2);
}
...
static void disp_nation_chat(int refreshFlag)
{
    ...
    */
    ...
    {
        get_chat.init(REPORT_DET_X1 + 10, REPORT_TALK_Y2 - 26, REPORT_DET_X2 - 10, info.player_chat_str, CHAT_STR_LEN,
                      &font_san, 0, 0);
        get_chat.clear();
    ...
    int nationRecno;
    if (info.chat_receiver_type == CHAT_RECEIVER_CURRENT && info.browse_nation_recno >= 1 &&
        info.browse_nation_recno <= nc)
