...
static short human_civilian_build_firm_button[] = {
    FIRM_CAMP,     FIRM_FORT,        FIRM_MINE, FIRM_FACTORY, FIRM_MARKET, FIRM_INN,
    FIRM_RESEARCH, FIRM_WAR_FACTORY, FIRM_SPY,  FIRM_SPECIAL, FIRM_BASE,   0,
};
...
void Unit::disp_info(int refreshFlag)
{
    ...
    vga.active_buf->put_bitmap_trans_remap_decompress(INFO_X1 + 16, INFO_Y1 - 28, nationPict,
...
void Unit::disp_main_menu(int refreshFlag)
{
    ...
    if (is_own_spy()) // spy menu
    {
        ...
        vga.active_buf->put_bitmap_trans_remap_decompress(INFO_X1 + 113, INFO_Y1 + 89, spyBox,
                                                          game.get_color_remap_table(nation_recno, 0));
        if (spy_array[spy_recno]->camouflage_count > 0)
        {
            spyBox = image_icon.get_ptr("SPYCLOAK");
            vga.active_buf->put_bitmap_trans_remap_decompress(INFO_X1 + 113 + 13, INFO_Y1 + 89 + 21, spyBox,
                                                              game.get_color_remap_table(nation_recno, 0));
        }
    ...
    if (refreshFlag == INFO_REPAINT || last_item_menu_offset != item_menu_offset)
    {
        ...
        button_use_item.create_text(INFO_X1 + 127 - item_menu_offset, INFO_Y1 + 120, INFO_X1 + 164 - item_menu_offset,
                                    INFO_Y1 + 140,
...
int Unit::should_show_info()
{
    ...
    if (nation_array.player_recno && (~nation_array)->revealed_by_phoenix(next_x_loc(), next_y_loc()))
    {
        return 1;
    }
...
void Unit::disp_basic_info(int dispY1, int refreshFlag)
{
    ...
    int dispName = unit_id == UNIT_CARAVAN ||
                   unit_res[unit_id]->unit_class == UNIT_CLASS_SHIP; // only display names for caravans and
                                                                     // ships as their names are not displayed
                                                                     // in the other part of the interface
    if (nation_recno)
    {
        int dispTitle = is_own() || nation_recno == 0; // only display title for own units or
...
void Unit::disp_unit_profile(int dispY1, int refreshFlag)
{
    ...
    if (unit_res[unit_id]->unit_class != UNIT_CLASS_GOD)
    {
        ...
        if (nation_recno && nation_array.player_recno && nation_recno != nation_array.player_recno)
        {
...
#if (defined(CHINESE))
            // SXM:Risk
            font_whbl.center_put(INFO_X1 + 12, INFO_Y1 + 3, INFO_X2, INFO_Y1 + 21, unitNameStr, 0, 1);
            font_zoom.center_put(INFO_X1 + 12, INFO_Y1 + 18, INFO_X2, INFO_Y1 + 33,
                                 text_unit.str_unit_desc(relationStr, str), 0, 0);
#else
            font_whbl.center_put(INFO_X1 + 12, INFO_Y1 + 9, INFO_X2, INFO_Y1 + 21, unitNameStr, 0, 1);
        ...
        case UNIT_CLASS_WAGON:
            font_whbl.center_put(INFO_X1 + 12, INFO_Y1 + 12, INFO_X2, INFO_Y1 + 30,
                                 text_unit.str_race_wagon(race_res[race_id]->adjective, is_own() ? name_id : 0), 0, 1);
            break;
        ...
        case UNIT_CLASS_ANIMAL:
            if (nation_recno)
            {
                font_whbl.center_put(INFO_X1 + 12, INFO_Y1 + 9, INFO_X2, INFO_Y1 + 21,
                                     ...
                                     0, 1);
                font_whbl.center_put(INFO_X1 + 12, INFO_Y1 + 21, INFO_X2, INFO_Y1 + 33, relationStr, 0, 1);
            }
            else
            {
                font_whbl.center_put(INFO_X1 + 12, INFO_Y1 + 9, INFO_X2, INFO_Y1 + 21, unit_name(0), 0, 1);
            }
            break;

        case UNIT_CLASS_MONSTER:
            font_whbl.center_put(INFO_X1 + 12, INFO_Y1 + 9, INFO_X2, INFO_Y1 + 21, unit_name(rank_id != RANK_SOLDIER),
                                 0, 1);
        ...
        unit_res[unit_id]->unit_class != UNIT_CLASS_GOD)
    {
        ...
        int y1 =
            INFO_Y1 + 140 + ZOOM_LOC_X_HEIGHT + spriteFrame->offset_y; // - (sprite_info->loc_width * ZOOM_LOC_X_HEIGHT+
                                                                       // sprite_info->loc_height * ZOOM_LOC_Y_HEIGHT);
        y1 = y1 - ((sprite_info->max_bitmap_height + sprite_info->max_height) >> 2) -
        ...
        vga.active_buf->put_bitmap_area_trans_remap_decompress(x1, y1, (char *)bitmapPtr, srcX1, srcY1, srcX2, srcY2,
                                                               colorRemapTable);
    ...
    case UNIT_CLASS_MONS_WEAPON:
        ...
        if (is_visible() && mouse.single_click(INFO_X1 + 12, INFO_Y1 + 6, INFO_X1 + 215, INFO_Y1 + 172))
        {
            if ((item.id || is_own_spy()) &&
                mouse.single_click(INFO_X1 + 119, INFO_Y1 + 87, INFO_X1 + 215, INFO_Y1 + 172))
                return 0;
            else if ((item.id && is_own_spy()) &&
                     mouse.single_click(INFO_X1 + 12, INFO_Y1 + 87, INFO_X1 + 215, INFO_Y1 + 172))
                return 0;
            else if (unit_res[unit_id]->unit_class == UNIT_CLASS_HUMAN && is_royal &&
                     mouse.single_click(INFO_X2 - 25, INFO_Y1 + 5, INFO_X2 - 5, INFO_Y1 + 25) &&
                     game.is_campaign_mode())
                return 0;
            else if (unit_res[unit_id]->unit_class == UNIT_CLASS_HUMAN && !is_royal &&
                     mouse.single_click(INFO_X2 - 25, INFO_Y1 + 64, INFO_X2 - 5, INFO_Y1 + 84) &&
                     game.is_campaign_mode())
                return 0;
            else
            {
...
void Unit::disp_hit_point()
{
    ...
    if (max_hit_points())
    {
        ...
        {
            bitmapLeft =
                image_spict.read(ratio < 11 ? (char *)"MTR_B2" : ratio < 40 ? (char *)"MTR_B3" : (char *)"MTR_B4");
            bitmapRight = image_spict.read("MTR_B1");
        }
        else
        {
            bitmapLeft =
                image_spict.read(ratio < 11 ? (char *)"MTR_10" : ratio < 40 ? (char *)"MTR_39" : (char *)"MTR_40");
        ...
        {
            //	str = (int)hitPoints;
            font_whbl.center_put(INFO_X1 + 33, INFO_Y1 + 45, INFO_X1 + 65, INFO_Y1 + 57,
                                 misc.format((int)hitPoints, 1));
            //	str = (int)max_hit_points();
            font_whbl.center_put(INFO_X1 + 166, INFO_Y1 + 45, INFO_X1 + 200, INFO_Y1 + 57,
                                 misc.format((int)max_hit_points(), 1));
        }
        else
        {
            str = (int)hitPoints;
...
#define BUTTON_Y(j) (INFO_Y1 + 235 + (281 - 235) * j)
    ...
    if (nation_recno == nation_array.player_recno && nation_array.player_recno)
    {
        ...
        if (team_info && team_info->formation_id)
        {
            ...
            if (team_info->formation_id < 7)
            {
                ...
                vga.active_buf->put_bitmap_trans_decompress(INFO_X1 + (team_info->formation_id - 1) * 28 + 47,
                                                            INFO_Y1 - 29, image_button.read(bdw));
            }
            else
            {
                bdw[1] = '6';
                vga.active_buf->put_bitmap_trans_decompress(INFO_X1 + 5 * 28 + 47, INFO_Y1 - 29,
        ...
        button_succeed_king.visible_flag =
            nation_array.player_recno && nation_recno == nation_array.player_recno && can_succeed_king();
        button_succeed_king.paint();

        //------- display aggressive mode button ------//
        // Weapon also need this button

        if ((button_behavior_mode.visible_flag = unit_res[unit_id]->unit_class != UNIT_CLASS_WAGON))
        {
        ...
        if ((button_build.visible_flag = unit_res[unit_id]->unit_class == UNIT_CLASS_HUMAN ||
                                         unit_res[unit_id]->unit_class == UNIT_CLASS_MONSTER))
        {
            ...
            if (firmId <= MAX_FIRM_TYPE && nation_recno == nation_array.player_recno) // a spy cannot build structure
                                                                                      // for another nation
            {
        ...
        button_return_camp.visible_flag = home_camp_firm_recno && !firm_array.is_deleted(home_camp_firm_recno) &&
        ...
        if (auto_retreat_hit_point && home_camp_firm_recno)
        {
            ...
            font_whbl.center_put(
                button_return_camp.x1, button_return_camp.y1, button_return_camp.x1 + BUTTON_ACTION_WIDTH - 1,
                button_return_camp.y1 + BUTTON_ACTION_HEIGHT - 1, misc.format(auto_retreat_hit_point, 3));
        }
        ...
        if (is_own_spy())
        {
            ...
            button_spy_notify.update_bitmap(notifyFlag ? (char *)"SPYNOTI1"
        ...
        if ((button_transform_fort.visible_flag = unit_id == UNIT_ROCK))
        {
            button_transform_fort.enable_flag =
                nation_recno && firm_res[FIRM_FORTRESS]->get_nation_tech_level(nation_recno);
            button_transform_fort.paint();
        }

        //---- display button for changing nation color scheme ----//
        //	if( sys.debug_session )
        //		button_change_color.paint_text( INFO_X1+ 78, INFO_Y1 +60,
        //"Change
        // Nation Color" );
    }
    else if (is_own_spy())
    {
        ...
        button_reward.visible_flag = 0;
        if (unit_res[unit_id]->unit_class == UNIT_CLASS_HUMAN || unit_res[unit_id]->unit_class == UNIT_CLASS_MONSTER)
        {
            if (nation_recno == nation_array.player_recno && // you can't promote your spy in other nation
                rank_id != RANK_KING)
            {
...
void Unit::detect_button()
{
    ...
    if (is_own())
    {
        ...
        if (unit_res[unit_id]->unit_class == UNIT_CLASS_HUMAN && rank_id != RANK_KING && button_set_royal.detect())
        {
            if (is_royal)
                is_royal = 0;
            else
...
#ifdef DEBUG
                font_whbl.center_put(INFO_X2 - 30, INFO_Y1 + 65, INFO_X2 - 10, INFO_Y1 + 85, misc.format(totalRoyal), 0,
                                     1);
#endif
        ...
        if (button_transform_fort.detect())
        {
            ...
            // fortressXLoc, fortressYLoc, builderUnit, COMMAND_PLAYER);
            // else
                ...
                unit_array.selected_count >= 4)
            {
                ...
                {
                    Unit *unitPtr;
                    if (!unit_array.is_deleted(i) && (unitPtr = unit_array[i]) && unitPtr->selected_flag &&
                        unitPtr->nation_recno == nation_recno && unitPtr->unit_id == unit_id)
                    {
                        if (++grokkenCount >= 4)
                            break;
                    }
...
static void group_change_behavior_mode()
{
    ...
    if (remote.is_enable())
    {
        ...
        shortPtr = (short *)remote.new_send_queue_msg(MSG_UNIT_CHANGE_AGGRESSIVE_MODE,
...
static void group_reward()
{
    ...
    for (int i = unit_array.size(); i > 0; i--)
    {
        ...
        if (unitPtr->selected_flag && unitPtr->is_own() && unit_res[unitPtr->unit_id]->class_info.has_loyalty &&
    ...
    for (int i = 0; buildFirmButton[i]; ++i)
    {
        ...
        if (refreshFlag == INFO_REPAINT)
        {
            button_build_array[i].create(
                ...
                ButtonCustomPara(game.get_color_remap_table(nation_recno, 0), ((int)race_id << 16) | firmId));
...
void Unit::disp_build(int refreshFlag)
{
    ...
    font_snds.put_paragraph(
        ...
        text_unit.str_select_build(buildId ? firm_res.get_build(buildId)->name : firm_res[power.command_para]->name));
    ...
    font_snds.put_paragraph(INFO_X1 + 20, INFO_Y1 + 60, INFO_X1 + 216, INFO_Y1 + 80,
                            text_unit.str_build_cost(firm_res[power.command_para]->setup_cost,
...
void Unit::disp_settle(int refreshFlag)
{
    ...
    font_snds.put_paragraph(INFO_X1 + 20, INFO_Y1 + 25, INFO_X1 + 216, INFO_Y1 + 79, text_unit.str_select_settle());
...
void Unit::disp_unit_info()
{
    ...
    if (debug2_enable_flag)
    {
        ...
        if (unit_res[unit_id]->unit_class == UNIT_CLASS_MONSTER)
        {
            ...
            // sprite_recno, 1, INFO_X2-2, INFO_REPAINT);
            // font_san.field( x+20, y, " "
            //, x+22, next_x_loc(), 1, INFO_X2-2, INFO_REPAINT);
            // font_san.field( x+50,
            ...
            // cur_order.mode, 1, INFO_X2-2, INFO_REPAINT);
            // font_san.field( x+120, y, "
            //" , x+122, cur_order.para, 1, INFO_X2-2, INFO_REPAINT);
            // font_san.field(
            ...
            y -= 20;
            //				font_san.field( x+100, y, " " , x+102,
            // pushed_order.mode, 1, INFO_X2-2, INFO_REPAINT);
            // font_san.field( x+120, y, " " , x+122, pushed_order.para, 1, INFO_X2-2,
            // INFO_REPAINT);
            //				font_san.field( x+140, y, " " , x+142,
            // pushed_order.loc_x, 1, INFO_X2-2, INFO_REPAINT);
            // font_san.field( x+160, y, " " , x+162, pushed_order.loc_y, 1,
            // INFO_X2-2, INFO_REPAINT);
            y -= 20;
            //				font_san.field( x+160, y, " " , x+162,
    ...
    if (rank_id != RANK_KING && (nation_recno || spy_recno) && unitInfo->class_info.has_loyalty)
    {
                          ...
                          // this is a spy and this spy is ours
        {
            ...
            info.disp_loyalty(INFO_X1 + 20, INFO_Y1 + 192,
                              INFO_X2 - 99 - font_snds.text_width(misc.format(spy_array[spy_recno]->spy_loyalty, 4)),
                              spy_array[spy_recno]->spy_loyalty, spy_array[spy_recno]->spy_loyalty, true_nation_recno(),
                              INFO_REPAINT);
        }
        else if (nation_recno)
        {
            ...
            if ((int)loyalty != (int)target_loyalty)
                ...
                info.disp_loyalty(INFO_X1 + 20, INFO_Y1 + 192,
                                  INFO_X2 - 99 - font_snds.text_width(misc.format((int)loyalty, 4)), (int)loyalty,
                                  (int)target_loyalty, nation_recno, INFO_REPAINT);
        }
    }
    else
    {
...
void Unit::disp_spy_menu()
{
    ...
    if (canChangeOwnCloak != lastCanChangeCloak || nation_array.nation_count != lastNationCount)
    {
    ...
    font_whbl.center_put(INFO_X1 + 158, INFO_Y1 + 150, INFO_X1 + 217, INFO_Y1 + 162,
    ...
    for (int i = 0; i <= nation_array.size(); i++)
    {
        ...
        {
            if (i != nation_recno && i != true_nation_recno()) // only display the current cloaked nation
                                                               // and its true nation
                continue;
        }
        else
        ...
        if (i == nation_recno)
            vga.active_buf->put_bitmap_trans_remap_decompress(x + (addedCount % 3) * SPY_CLOAK_WIDTH,
...
static void disp_debug_info(Unit *unitPtr, int dispY1, int refreshFlag)
{
    ...
    static const char *action_mode_str_array[] = {
        "Stop", "Move", "Attack", "Build Firm", "Settle Town", "Assign", "Go Cast Power", "Transform Fortress",
    };

    static const char *unit_mode_str_array[] = {
        "",      "Oversee", "Defend town", "Defend camp", "Construct Firm", "Construct Town",
        "Rebel", "Monster", "On Ship",     "In Harbor",
    };

    static const char *cur_action_str_array[] = {
        "Idle", "Ready to move", "Move", "Wait", "Attack", "Turn", "Ship extra move", "Die",
    };
...
void Unit::disp_item_menu()
{
    ...
    font_whbl.put_paragraph(INFO_X1 + 127 + 3 - item_menu_offset, INFO_Y1 + 94, INFO_X1 + 218 - item_menu_offset,
                            INFO_Y1 + 140, item.item_name());

    if (is_own())
        font_whbl.put_paragraph(INFO_X1 + 127 + 2 - item_menu_offset, INFO_Y1 + 140, INFO_X1 + 218 - item_menu_offset,
                                INFO_Y1 + 177,
...
void Unit::detect_item_menu()
{
    ...
    // INFO_Y1+94, INFO_X1+218 - item_menu_offset, INFO_Y1+170,
    // item.item_desc() );
    //	}
...
void Unit::disp_auto_menu(int refreshFlag)
{
    ...
    {
        if (refreshFlag == INFO_REPAINT)
            button_auto_retreat_array[i].create(x + (i / 5) * 94, y + (i % 5) * 23, x + (i / 5) * 94 + 91,
    ...
    if (refreshFlag == INFO_REPAINT)
        button_auto_retreat_disabled.create(x + (i / 5) * 94, y + (i % 5) * 23, x + (i / 5) * 94 + 91,
                                            y + (i % 5) * 23 + 20, disp_auto_menu_button,
                                            ButtonCustomPara(game.get_color_remap_table(nation_recno, 0), i));
...
int Unit::disp_edit_mode(int refreshFlag)
{
    ...
    if (unitInfo->class_info.has_combat_level)
    {
        x = font_whbl.put(x1, y, text_unit.str_combat_level(), 0,
                          x2); //	"Combat Level"
        x = edit_combat_x2 =
            font_blue.put((edit_combat_x1 = x + 20), (edit_combat_y1 = y), skill.actual_combat_level(NULL), 0, x2);
        ...
        if (sys.debug_session) // maximum combat level is a higher secret
        {
            ...
            x = edit_max_combat_x2 =
                font_blue.put((edit_max_combat_x1 = x), (edit_max_combat_y1 = y), skill.max_combat_level, 0, x2);
    ...
    if (unitInfo->class_info.has_skill_level)
    {
        ...
        x = edit_skill_x2 =
            font_blue.put((edit_skill_x1 = x + 20), (edit_skill_y1 = y), skill.actual_skill_level(NULL), 0, x2);
        ...
        if (sys.debug_session) // maximum skill level is a higher secret
        {
            x = font_whbl.put(x, y, " / ", 0, x2);
            x = edit_max_skill_x2 =
                font_blue.put((edit_max_skill_x1 = x), (edit_max_skill_y1 = y), skill.max_skill_level, 0, x2);
...
void Unit::detect_edit_mode()
{
    ...
    {
        if (mouse.any_click(edit_max_combat_x1, edit_max_combat_y1, edit_max_combat_x2, edit_max_combat_y2,
                            LEFT_BUTTON))
        {
...
{
    return is_visible() && UnitB::explore_for_player() && !is_camouflage(nation_array.player_recno);
}
