...
static short edit_loyalty_x1, edit_loyalty_y1, edit_loyalty_x2, edit_loyalty_y2,
    edit_loyalty_enable;
static short edit_combat_x1, edit_combat_y1, edit_combat_x2, edit_combat_y2,
    edit_combat_enable;
static short edit_max_combat_x1, edit_max_combat_y1, edit_max_combat_x2,
    edit_max_combat_y2, edit_max_combat_enable;
static short edit_skill_x1, edit_skill_y1, edit_skill_x2, edit_skill_y2,
    edit_skill_enable;
static short edit_max_skill_x1, edit_max_skill_y1, edit_max_skill_x2,
    edit_max_skill_y2, edit_max_skill_enable;
static short edit_spy_x1, edit_spy_y1, edit_spy_x2, edit_spy_y2,
    edit_spy_enable;
// ##### end Gilbert 5/3 #####//

static short human_civilian_build_firm_button[] = {
    FIRM_CAMP,   FIRM_FORT,    FIRM_MINE,     FIRM_FACTORY,
    FIRM_MARKET, FIRM_INN,     FIRM_RESEARCH, FIRM_WAR_FACTORY,
    FIRM_SPY,    FIRM_SPECIAL, FIRM_BASE,     0,
};
...
void Unit::disp_info(int refreshFlag) {
  if (sprite_recno != last_unit_recno) {
    ...
    if (last_menu_mode !=
        unit_menu_mode) // if changing menu mode pass repaint to sub-menu
    {
  ...
  vga.active_buf->put_bitmap_trans_remap_decompress(
      INFO_X1 + 16, INFO_Y1 - 28, nationPict,
      game.get_color_remap_table(nation_recno, 0));
...
void Unit::disp_main_menu(int refreshFlag) {
  ...
  //		vga.active_buf->put_bitmap( INFO_X1, INFO_Y1,
  //image_gameif.read("WEAPBASE") ); 	else
  if (unit_res[unit_id]->unit_class == UNIT_CLASS_WAGON) {
    ...
    for (int i = 0; i < 10; i++) {
      if (value[i] <= wagonPtr->population)
        info.draw_unit_icon(INFO_X1 + offsetX[i], INFO_Y1 + offsetY[i],
                            race_res[race_id]->civilian_unit_id, nation_recno,
                            INFO_X1, INFO_Y1, INFO_X2, INFO_Y2, 1);
      if (value[i] <= wagonPtr->spy_count) {
        if (wagonPtr->spy_recno[value[i] - 1] &&
            spy_array[wagonPtr->spy_recno[value[i] - 1]]->true_nation_recno ==
                nation_array.player_recno) {
          char *bitmapPtr = image_icon.get_ptr("U_SPY");
          vga.active_buf->put_bitmap_trans(
              INFO_X1 + offsetX[i] - ((Bitmap *)bitmapPtr)->get_width() / 2,
              INFO_Y1 + offsetY[i] - ((Bitmap *)bitmapPtr)->get_height() / 2 -
                  8,
              bitmapPtr);

          //	world.zoom_matrix->put_bitmap_offset( cur_x, cur_y, get_z(),
          //iconPtr, 		-maxHitBarWidth/2 - ((Bitmap *)iconPtr)->get_width(),
          //		spriteInfo->max_height + (spriteInfo->loc_width +
          //spriteInfo->loc_height) * 6 + yOffset, 		NULL, 0, 0);
        }
      }
    }
    vga.active_buf->put_bitmap_trans(INFO_X1 + 37, INFO_Y1 + 148,
                                     image_gameif.read("MASK"));
    //	vga.active_buf->put_bitmap_half_decompress( INFO_X1, INFO_Y1,
    //image_gameif.read("WAGBASE") );
  } else
  ...
  if (is_own_spy()) // spy menu
  {
    ...
    vga.active_buf->put_bitmap_trans_remap_decompress(
        INFO_X1 + 113, INFO_Y1 + 89, spyBox,
        game.get_color_remap_table(nation_recno, 0));
    if (spy_array[spy_recno]->camouflage_count > 0) {
      spyBox = image_icon.get_ptr("SPYCLOAK");
      vga.active_buf->put_bitmap_trans_remap_decompress(
          INFO_X1 + 113 + 13, INFO_Y1 + 89 + 21, spyBox,
          game.get_color_remap_table(nation_recno, 0));
    }

    if (config.show_debug_info)
      font_whbl.center_put(
          INFO_X1 + 122, INFO_Y1 + 90, INFO_X1 + 154, INFO_Y1 + 110,
          misc.format(spy_array[spy_recno]->camouflage_count > 0
                          ? spy_array[spy_recno]->camouflage_count
                          : spy_array[spy_recno]->camouflage_power,
                      4));
    else if (spy_array[spy_recno]->can_camouflage())
      font_whbl.center_put(INFO_X1 + 132, INFO_Y1 + 90, INFO_X1 + 144,
                           INFO_Y1 + 110, "C");
  ...
  if (refreshFlag == INFO_REPAINT ||
    ...
    last_item_menu_offset = item_menu_offset;
    // button_drop_item.create_text(INFO_X1+176 - item_menu_offset, INFO_Y1+144,
    // INFO_X1+213 - item_menu_offset, INFO_Y1+164, "drop" );
    button_use_item.set_font(&font_bld);
    button_use_item.create_text(INFO_X1 + 127 - item_menu_offset, INFO_Y1 + 120,
                                INFO_X1 + 164 - item_menu_offset, INFO_Y1 + 140,
                                text_unit.str_item_on()); //"on" );
  }
...
void Unit::disp_basic_info(int dispY1, int refreshFlag) {
  ...
  int dispName = unit_id == UNIT_CARAVAN ||
                 unit_res[unit_id]->unit_class ==
                     UNIT_CLASS_SHIP; // only display names for caravans and
                                      // ships as their names are not displayed
                                      // in the other part of the interface
  if (nation_recno) {
    int dispTitle =
        is_own() || nation_recno == 0; // only display title for own units or
                                       // rebel leader of independent units
    if (dispName)
      font_snds.center_put(INFO_X1 + 21, INFO_Y1 + 3, INFO_X2 - 2, INFO_Y1 + 30,
                           unit_name(dispTitle));
    //------- display the nation color box -------//
  }
  if (dispName)
    font_snds.center_put(INFO_X1, INFO_Y1, INFO_X1 + 100, INFO_Y1 + 21,
                         unit_name());
}
...
int Unit::detect_basic_info() {
  /*
     ...
     ---//
          ...
          if( is_visible() && mouse.single_click( INFO_X1, INFO_Y1, INFO_X2,
     INFO_Y1+21 ) )
          {
...
void Unit::disp_unit_profile(int dispY1, int refreshFlag) {
  ...
  if (unit_res[unit_id]->unit_class != UNIT_CLASS_GOD) {
    ...
    if (nation_recno && nation_array.player_recno &&
      ...
      NationRelation *nationRelation =
          nation_array[nation_recno]->get_relation(nation_array.player_recno);
    ...
    case UNIT_CLASS_HUMAN:
      if (unit_mode == UNIT_MODE_REBEL) {
        ...
        str = text_unit.str_race_rebel_title(race_res[race_id]->adjective,
                                             rank_id);
      } else {
        unitNameStr = unit_name(1);
        if (unit_mode == UNIT_MODE_TOWN_DEFENDER)
          str = text_unit.str_race_defender(race_id,
                                            race_res[race_id]->adjective);
        else if (hero_id)
          str = text_unit.str_race_hero(race_res[race_id]->adjective);
        else if (is_civilian())
          str = text_unit.str_race_civilian(race_id,
                                            race_res[race_id]->adjective);
        else
          str =
              text_unit.str_race_soldier(race_id, race_res[race_id]->adjective);
      }
    ...
    case UNIT_CLASS_CARAVAN:
      font_snds.center_put(
          INFO_X1 + 82, INFO_Y1 + 15, INFO_X1 + 222, INFO_Y1 + 30,
          text_unit.str_unit_desc(relationStr, unit_name(0)), 0, 1);
      break;

    case UNIT_CLASS_WAGON:
      font_whbl.center_put(
          INFO_X1 + 12, INFO_Y1 + 12, INFO_X2, INFO_Y1 + 30,
          text_unit.str_race_wagon(race_res[race_id]->adjective,
                                   is_own() ? name_id : 0),
          0, 1);
      break;
    ...
    case UNIT_CLASS_MONS_WEAPON: {
      ...
      // str, 0, 1 );
      font_whbl.center_put(
          INFO_X1 + 12, INFO_Y1 + 9, INFO_X2, INFO_Y1 + 21,
          text_unit.str_unit_desc(relationStr, unit_name(dispTitle)), 0, 1);
    } break;
    ...
    case UNIT_CLASS_ANIMAL:
      if (nation_recno) {
        font_whbl.center_put(
            INFO_X1 + 12, INFO_Y1 + 9, INFO_X2, INFO_Y1 + 21,
            text_unit.str_king_s_unit(nation_array[nation_recno]->king_name(1),
                                      unit_name(0)),
            0, 1);
    ...
    case UNIT_CLASS_MONSTER:
      ...
      font_whbl.center_put(
          INFO_X1 + 12, INFO_Y1 + 21, INFO_X2, INFO_Y1 + 33,
  ...
  if (nation_recno == nation_array.player_recno && can_resign()) {
    if (refreshFlag == INFO_REPAINT) {
      ...
      //		button_resign.create( INFO_X1+188, INFO_Y1+50,
      //"WEAPX-UP", "WEAPX-DN" ); 	else 		button_resign.create( INFO_X1+80,
      //INFO_Y1+127, "UNITX-UP", "UNITX-DN" );
  ...
  if ((item_menu_offset == 0 || !item.id) &&
    ...
    cur_dir = temp;
    Bitmap *bitmapPtr = (Bitmap *)sprite_info->res_bitmap.read_imported(
        spriteFrame->bitmap_offset);
    short *colorRemapTable =
        sprite_info->get_color_remap_table(nation_recno, 0);

    int x1 = INFO_X1 + 60 + spriteFrame->offset_x + 7;
    //	int y1 = INFO_Y1 +82 - bitmapPtr->get_height();
    int y1 =
        INFO_Y1 + 140 + ZOOM_LOC_X_HEIGHT +
        spriteFrame->offset_y; // - (sprite_info->loc_width * ZOOM_LOC_X_HEIGHT+
                               // sprite_info->loc_height * ZOOM_LOC_Y_HEIGHT);
    y1 = y1 -
         ((sprite_info->max_bitmap_height + sprite_info->max_height) >> 2) -
         (sprite_info->loc_width * ZOOM_LOC_X_HEIGHT);
    ...
    vga.active_buf->put_bitmap_area_trans_remap_decompress(
        x1, y1, (char *)bitmapPtr, srcX1, srcY1, srcX2, srcY2, colorRemapTable);
...
int Unit::detect_unit_profile() {
  if (is_own_spy() && mouse.single_click(INFO_X1 + 119, INFO_Y1 + 87,
                                         INFO_X1 + 158, INFO_Y1 + 172)) {
    if (spy_array[spy_recno]->camouflage_count > 0)
  ...
  case UNIT_CLASS_GOD:
  case UNIT_CLASS_WAGON:
    if (is_visible() && mouse.single_click(INFO_X1 + 12, INFO_Y1 + 6,
                                           INFO_X1 + 215, INFO_Y1 + 221)) {
      world.go_loc(next_x_loc(), next_y_loc());
      return 1;
    }
    break;
  ...
  case UNIT_CLASS_ANIMAL:
  case UNIT_CLASS_MONS_WEAPON:
    //	if( is_visible() && mouse.single_click( INFO_X1 +12, INFO_Y1 +6, INFO_X1
    //+215, INFO_Y1+221 ))
    if (is_visible() && mouse.single_click(INFO_X1 + 12, INFO_Y1 + 6,
                                           INFO_X1 + 215, INFO_Y1 + 172)) {
      if ((item.id || is_own_spy()) &&
          mouse.single_click(INFO_X1 + 119, INFO_Y1 + 87, INFO_X1 + 215,
                             INFO_Y1 + 172))
        return 0;
      else if ((item.id && is_own_spy()) &&
               mouse.single_click(INFO_X1 + 12, INFO_Y1 + 87, INFO_X1 + 215,
                                  INFO_Y1 + 172))
        return 0;
      else if (unit_res[unit_id]->unit_class == UNIT_CLASS_HUMAN && is_royal &&
               mouse.single_click(INFO_X2 - 25, INFO_Y1 + 5, INFO_X2 - 5,
                                  INFO_Y1 + 25) &&
               game.is_campaign_mode())
        return 0;
      else if (unit_res[unit_id]->unit_class == UNIT_CLASS_HUMAN && !is_royal &&
               mouse.single_click(INFO_X2 - 25, INFO_Y1 + 64, INFO_X2 - 5,
                                  INFO_Y1 + 84) &&
               game.is_campaign_mode())
        return 0;
      else {
...
void Unit::disp_hit_point() {
  ...
  if (max_hit_points()) {
    ...
    if (barType == 0) {
      bitmapLeft = image_spict.read(ratio < 11 ? (char *)"MTR_B2"
                                               : ratio < 40 ? (char *)"MTR_B3"
                                                            : (char *)"MTR_B4");
      bitmapRight = image_spict.read("MTR_B1");
    } else {
      bitmapLeft = image_spict.read(ratio < 11 ? (char *)"MTR_10"
                                               : ratio < 40 ? (char *)"MTR_39"
                                                            : (char *)"MTR_40");
    ...
    vga.active_buf->put_bitmap_area_trans(x, y, bitmapLeft, 0, 0, xCut - 1,
                                          35 - 1);
    vga.active_buf->put_bitmap_area_trans(x, y, bitmapRight, xCut, 0, 106 - 1,
                                          35 - 1);
    mouse.show_area();

    if (barType == 0) {
      //	str = (int)hitPoints;
      font_whbl.center_put(INFO_X1 + 33, INFO_Y1 + 45, INFO_X1 + 65,
                           INFO_Y1 + 57, misc.format((int)hitPoints, 1));
      //	str = (int)max_hit_points();
      font_whbl.center_put(INFO_X1 + 166, INFO_Y1 + 45, INFO_X1 + 200,
                           INFO_Y1 + 57, misc.format((int)max_hit_points(), 1));
    } else {
      str = (int)hitPoints;
      str += " / ";
      str += (int)max_hit_points();
    ...
    /*	else
            {
                    ...
                    vga.active_buf->put_bitmap( INFO_X1 +17, INFO_Y1 +89,
       image_spict.read("BOD-00") ); vga.active_buf->read_bitmapW( INFO_X1 +17,
       INFO_Y1 +89, INFO_X1 +64, INFO_Y1 +97 +size, hitPointBitmap );

                    vga.active_buf->put_bitmap( INFO_X1 +17, INFO_Y1 +89,
       image_spict.read("BOD-67") ); vga.active_buf->put_bitmapW( INFO_X1 +17,
       INFO_Y1 +89, hitPointBitmap );

                    mem_del( hitPointBitmap );
                    str = hitPoints *100 / (int)max_hit_points();
                    str += "%";
                    font_snds.center_put( INFO_X1+76, INFO_Y1+101, INFO_X1+125,
       INFO_Y1+120, str );
            }*/
  }
}
...
#define BUTTON_Y(j) (INFO_Y1 + 235 + (281 - 235) * j)
  ...
  if (refreshFlag == INFO_REPAINT) {
    ...
    //	button_set_royal.create_text(INFO_X2-72, INFO_Y1+65, INFO_X2-50,
    //INFO_Y1+85, "R", 0); 	button_set_royal.enable_flag = 0;
  ...
  if (nation_recno == nation_array.player_recno && nation_array.player_recno) {
    ...
    if (team_info && team_info->formation_id) {
      ...
      if (team_info->formation_id < 7) {
        ...
        bdw[1] = '6';
        vga.active_buf->put_bitmap_trans_decompress(
            INFO_X1 + 5 * 28 + 47, INFO_Y1 - 29, image_button.read(bdw));
      }
    }
    //---- if currently in the mode of selecting a unit to succeed the king
    //----//

    button_succeed_king.visible_flag =
        nation_array.player_recno &&
        nation_recno == nation_array.player_recno && can_succeed_king();
    ...
    if ((button_behavior_mode.visible_flag =
      ...
      button_behavior_mode.update_bitmap(
          iconName); //	update_bitmap calls paint
    }
    ...
    if ((button_build.visible_flag =
      ...
      if (firmId <= MAX_FIRM_TYPE &&
          nation_recno ==
              nation_array.player_recno) // a spy cannot build structure for
                                         // another nation
      {
    ...
    // ###### end Gilbert 24/3 #######//
    {
      if (rank_id == RANK_SOLDIER && skill_level() > 0 && race_id &&
          !is_civilian()) {
    ...
    // ###### end Gilbert 24/3 #########//
    {
      if (nation_recno ==
              nation_array
                  .player_recno && // you can't promote your spy in other nation
    ...
    button_return_camp.visible_flag =
        home_camp_firm_recno && !firm_array.is_deleted(home_camp_firm_recno) &&
    ...
    if (auto_retreat_hit_point && home_camp_firm_recno) {
      vga.active_buf->bar_alpha(
          button_return_camp.x1, button_return_camp.y1 + 15,
          button_return_camp.x1 + BUTTON_ACTION_WIDTH - 1,
          button_return_camp.y1 + BUTTON_ACTION_HEIGHT - 16, 1, 0);
      font_whbl.center_put(button_return_camp.x1, button_return_camp.y1,
                           button_return_camp.x1 + BUTTON_ACTION_WIDTH - 1,
                           button_return_camp.y1 + BUTTON_ACTION_HEIGHT - 1,
    ...
    if (is_own_spy()) {
      ...
      button_spy_notify.update_bitmap(
          notifyFlag ? (char *)"SPYNOTI1"
                     : (char *)"SPYNOTI0"); // update_bitmap calls paint
    ...
    if (unit_res[unit_id]->unit_class == UNIT_CLASS_HUMAN &&
        rank_id != RANK_KING) {
      if (is_royal)
        button_set_royal.create_text(INFO_X2 - 25, INFO_Y1 + 5, INFO_X2 - 5,
                                     INFO_Y1 + 25, "R", 0);
      else
        button_set_royal.create_text(INFO_X2 - 25, INFO_Y1 + 64, INFO_X2 - 5,
                                     INFO_Y1 + 84, "R", 0);
    ...
    if ((button_transform_fort.visible_flag = unit_id == UNIT_ROCK)) {
      button_transform_fort.enable_flag =
          nation_recno &&
          firm_res[FIRM_FORTRESS]->get_nation_tech_level(nation_recno);
      button_transform_fort.paint();
    }

    //---- display button for changing nation color scheme ----//
    //	if( sys.debug_session )
    //		button_change_color.paint_text( INFO_X1+ 78, INFO_Y1 +60, "Change
    //Nation Color" );
  } else if (is_own_spy()) {
    // -------- display settle button --------//
    ...
    if ((button_hire.visible_flag =
             is_hero_for_hire() && nation_array.player_recno)) {
...
void Unit::detect_button() {
  ...
  if (is_own()) {
    ...
    help.set_help(INFO_X1 + 47, INFO_Y1 - 29, INFO_X1 + 243, INFO_Y1,
                  "FORMTION");
    ...
    if (button_succeed_king.detect()) {
      if (!remote.is_enable()) {
        ...
        short *shortPtr = (short *)remote.new_send_queue_msg(
            MSG_UNIT_SUCCEED_KING, 2 * sizeof(short));
    ...
    if (rank_id == RANK_SOLDIER) {
      if (button_promote.detect()) {
        if (!remote.is_enable()) {
          ...
          short *shortPtr = (short *)remote.new_send_queue_msg(
              MSG_UNIT_SET_RANK, 2 * sizeof(short));
          *shortPtr = sprite_recno;
    ...
    if (sys.debug_session && button_change_color.detect()) {
      ...
      nationPtr->nation_color =
          game.color_remap_array[nationPtr->color_scheme_id - 1].main_color;
      se_ctrl.immediate_sound("TURN_ON");
    }
    ...
    if (unit_res[unit_id]->unit_class == UNIT_CLASS_HUMAN &&
        rank_id != RANK_KING && button_set_royal.detect()) {
      if (is_royal)
        is_royal = 0;
      else {
        int totalRoyal = 0;
        ...
        for (i = 1; i <= unit_array.size(); ++i) {
          ...
          if (unitPtr->nation_recno !=
              nation_array.player_recno) // throw away cloaked spy
            continue;
...
#ifdef DEBUG
        font_whbl.center_put(INFO_X2 - 30, INFO_Y1 + 65, INFO_X2 - 10,
                             INFO_Y1 + 85, misc.format(totalRoyal), 0, 1);
#endif
    ...
    if (button_transform_fort.detect()) {
      ...
      //			if(FirmMonsterFortress::check_transform_from_units(sprite_recno,
      //fortressXLoc, fortressYLoc, builderUnit))
      //				FirmMonsterFortress::transform_from_units(sprite_recno,
      //fortressXLoc, fortressYLoc, builderUnit, COMMAND_PLAYER); 			else
      //				power.issue_command(COMMAND_TRANSFORM_FORTRESS,
      //sprite_recno, FIRM_FORTRESS);

      // ###### begin Gilbert 23/3 ######//
      if (nation_array[nation_recno]->cash >=
              firm_res[FIRM_FORTRESS]->setup_cost &&
          nation_array[nation_recno]->live_points >=
              firm_res[FIRM_FORTRESS]->setup_live_points_cost &&
        ...
        for (int i = 1; i <= unit_array.size(); ++i) {
          Unit *unitPtr;
          if (!unit_array.is_deleted(i) && (unitPtr = unit_array[i]) &&
              unitPtr->selected_flag && unitPtr->nation_recno == nation_recno &&
        ...
        if (grokkenCount >= 4) {
          power.issue_command(COMMAND_TRANSFORM_FORTRESS, sprite_recno,
                              FIRM_FORTRESS);
        }
      }
      se_ctrl.immediate_sound("TURN_ON");
    ...
    if (unit_res[unit_id]->unit_class == UNIT_CLASS_HUMAN && is_royal) {
      help.set_help(INFO_X2 - 25, INFO_Y1 + 5, INFO_X2 - 5, INFO_Y1 + 25,
                    "ROYALOFF");
    } else if (unit_res[unit_id]->unit_class == UNIT_CLASS_HUMAN && !is_royal) {
      help.set_help(INFO_X2 - 25, INFO_Y1 + 64, INFO_X2 - 5, INFO_Y1 + 84,
                    "ROYAL-ON");
    }
  } else // not is_own()
  {
...
static void group_change_behavior_mode() {
  ...
  if (remote.is_enable()) {
    ...
    shortPtr = (short *)remote.new_send_queue_msg(
        MSG_UNIT_CHANGE_AGGRESSIVE_MODE,
        (unit_array.selected_count + 4) * sizeof(short));
    // reserve two short more in case selected_recno is slightly smalled then
    // the actual selected
...
static void group_reward() {
  ...
  if (remote.is_enable()) {
    ...
    shortPtr = (short *)remote.new_send_queue_msg(
        MSG_UNIT_REWARD, (unit_array.selected_count + 4) * sizeof(short));
    // reserve two short more in case selected_recno is slightly smalled then
    // the actual selected
...
static void group_change_spy_notify_flag() {
  ...
  for (int i = unit_array.size(); i > 0; i--) {
    ...
    if (unitPtr->selected_flag && unitPtr->is_own_spy()) {
      if (!remote.is_enable()) {
        spy_array[unitPtr->spy_recno]->notify_cloaked_nation_flag =
            newNotifyFlag;
      } else {
        // packet structure : <spy recno> <new notify_cloaked_nation_flag>
        short *shortPtr = (short *)remote.new_send_queue_msg(
            MSG_SPY_CHANGE_NOTIFY_FLAG, sizeof(short) * 2);
  ...
  button_spy_notify.update_bitmap(newNotifyFlag ? (char *)"SPYNOTI1"
                                                : (char *)"SPYNOTI0");
  ...
  for (int i = unit_array.size(); i > 0; i--) {
    ...
    if (unitPtr->selected_flag && unitPtr->is_own_spy()) {
      if (!remote.is_enable()) {
        ...
        short *shortPtr = (short *)remote.new_send_queue_msg(
            MSG_SPY_DROP_IDENTITY, sizeof(short));
...
void Unit::disp_build_menu(int refreshFlag) {
  ...
  for (int i = 0; buildFirmButton[i]; ++i) {
    ...
    //----- see if there is a firm build available for the given race/monster
    //type -----//
    ...
    if (is_monster())
      firmBuildId =
          firmInfo->get_build_id(monster_res[monster_id()]->firm_build_code);
    else
    ...
    if (firmId >= FIRM_OFFENSIVE_BUILDING_1 &&
        firmId <= FIRM_OFFENSIVE_BUILDING_4) {
      int techId =
          firmId - FIRM_OFFENSIVE_BUILDING_1 + TECH_OFFENSIVE_STRUCTURE_1;
      if (!tech_res[techId]->available_level &&
          (!nation_recno ||
           !firm_res[firmId]->get_nation_tech_level(nation_recno)))
        continue;
    }
    ...
    if (refreshFlag == INFO_REPAINT) {
      button_build_array[i].create(
          x, y, x + 187, y + 17, disp_firm_button,
          ButtonCustomPara(game.get_color_remap_table(nation_recno, 0),
  ...
  if (refreshFlag == INFO_REPAINT)
    button_cancel.create(INFO_X1 + 13 + 3 * BUTTON_DISTANCE, INFO_Y1 + 281, 'A',
                         "CANCEL");
...
void Unit::disp_build(int refreshFlag) {
  ...
  //	str += ".";
  //	font_snds.put_paragraph( INFO_X1+20, INFO_Y1+25, INFO_X1 +216, INFO_Y1
  //+79, str );
  ...
  font_snds.put_paragraph(
      INFO_X1 + 20, INFO_Y1 + 25, INFO_X1 + 216, INFO_Y1 + 79,
      text_unit.str_select_build(buildId ? firm_res.get_build(buildId)->name
  ...
  //	str += " + ";
  //	str +=
  //misc.format(firm_res[power.command_para]->setup_live_points_cost,1); 	str +=
  //" life points";
  font_snds.put_paragraph(
      INFO_X1 + 20, INFO_Y1 + 60, INFO_X1 + 216, INFO_Y1 + 80,
  ...
  if (refreshFlag == INFO_REPAINT)
    ...
    //);
    button_cancel2.create(INFO_X1 + 13 + 3 * BUTTON_DISTANCE, INFO_Y1 + 281,
                          'A', "CANCEL");
...
void Unit::disp_settle(int refreshFlag) {
  ...
  font_snds.put_paragraph(INFO_X1 + 20, INFO_Y1 + 25, INFO_X1 + 216,
                          INFO_Y1 + 79, text_unit.str_select_settle());
...
void Unit::disp_unit_info() {
  ...
  if (debug2_enable_flag) {
    ...
    if (unit_res[unit_id]->unit_class == UNIT_CLASS_MONSTER) {
      ...
      //				font_san.field( x, y, " " , x+2,
      //sprite_recno, 1, INFO_X2-2, INFO_REPAINT); 				font_san.field( x+20, y, " "
      //, x+22, next_x_loc(), 1, INFO_X2-2, INFO_REPAINT); 				font_san.field( x+50,
      //y, " " , x+52, next_y_loc(), 1, INFO_X2-2, INFO_REPAINT);
      //				font_san.field( x+70, y, " " , x+72,
      //nation_recno, 1, INFO_X2-2, INFO_REPAINT);

      //				font_san.field( x+100, y, " " , x+102,
      //cur_order.mode, 1, INFO_X2-2, INFO_REPAINT); 				font_san.field( x+120, y, "
      //" , x+122, cur_order.para, 1, INFO_X2-2, INFO_REPAINT); 				font_san.field(
      //x+140, y, " " , x+142, cur_order.loc_x, 1, INFO_X2-2, INFO_REPAINT);
      //				font_san.field( x+160, y, " " , x+162,
      //cur_order.loc_y, 1, INFO_X2-2, INFO_REPAINT);
      y -= 20;
      //				font_san.field( x+100, y, " " , x+102,
      //pushed_order.mode, 1, INFO_X2-2, INFO_REPAINT); 				font_san.field( x+120,
      //y, " " , x+122, pushed_order.para, 1, INFO_X2-2, INFO_REPAINT);
      //				font_san.field( x+140, y, " " , x+142,
      //pushed_order.loc_x, 1, INFO_X2-2, INFO_REPAINT); 				font_san.field( x+160,
      //y, " " , x+162, pushed_order.loc_y, 1, INFO_X2-2, INFO_REPAINT);
      y -= 20;
      //				font_san.field( x+160, y, " " , x+162,
      //cur_action, 1, INFO_X2-2, INFO_REPAINT);
    }
    // ###### end Gilbert 10/12 #######//
  }
#endif
  ...
  if (unitInfo->unit_class == UNIT_CLASS_WAGON) {
    ...
    font_whbl.center_put(INFO_X1 + 12, INFO_Y1 + 60, INFO_X2, INFO_Y1 + 80,
                         str);
    return;
  }
  ...
  if (rank_id != RANK_KING && (nation_recno || spy_recno) &&
    ...
    if (is_own_spy()) // only display spy loyalty instead of unit loyalty if
                      // this is a spy and this spy is ours
    {
      //	info.disp_loyalty( INFO_X1+20, INFO_Y1+208, INFO_X1+80,
      //spy_array[spy_recno]->spy_loyalty, spy_array[spy_recno]->spy_loyalty,
      //true_nation_recno(), INFO_REPAINT );
      info.disp_loyalty(INFO_X1 + 20, INFO_Y1 + 192,
                        INFO_X2 - 99 -
                            font_snds.text_width(misc.format(
                                spy_array[spy_recno]->spy_loyalty, 4)),
                        spy_array[spy_recno]->spy_loyalty,
                        spy_array[spy_recno]->spy_loyalty, true_nation_recno(),
                        INFO_REPAINT);
      ...
      if ((int)loyalty != (int)target_loyalty)
        info.disp_loyalty(
            INFO_X1 + 20, INFO_Y1 + 192,
            INFO_X2 - 99 - font_snds.text_width(misc.format((int)loyalty, 4)) -
  ...
  if (unitInfo->class_info.has_combat_level) {
    x2 = font_snds.put(INFO_X1 + 130, INFO_Y1 + 192,
                       text_unit.str_combat_level()); // "Combat" );
    font_snds.right_put(INFO_X2 - 10, INFO_Y1 + 192,
                        misc.format(combat_level(), 4));
  ...
  if (unitInfo->class_info.has_skill_level && skill_level() > 0) {
    x2 = font_snds.put(INFO_X1 + 130, INFO_Y1 + 206,
                       text_unit.str_leadership()); //"Leadership" );
    font_snds.right_put(INFO_X2 - 10, INFO_Y1 + 206,
                        misc.format(skill_level(), 4));
  ...
  if (max_hit_points()) {
    x2 = font_snds.put(INFO_X1 + 20, INFO_Y1 + 206,
                       text_unit.str_hit_points()); // "Hit Points" );
    int hitPoints;
    if (hit_points > (float)0 && hit_points < (float)1)
      hitPoints = 1; // display 1 for value between 0 and 1
    else
  ...
  if (is_own_spy()) {
    x2 = font_snds.put(INFO_X1 + 130, INFO_Y1 + 178,
                       text_unit.str_spy_skill()); // "Spying" );
    font_snds.right_put(INFO_X2 - 10, INFO_Y1 + 178,
                        misc.format(spy_array[spy_recno]->spy_skill, 4));
...
void Unit::disp_spy_menu() {
  ...
  //-- if the spy can always change back to its original color. So if it is
  //currently cloaked as another nation, it should be able to revert back any
  //time ---//
  ...
  if (!canChangeOwnCloak) {
    font_whbl.put_paragraph(
        INFO_X1 + 128, INFO_Y1 + 104, INFO_X1 + 217, INFO_Y1 + 165,
        text_unit.str_no_change_cloak()); // "Enemies Nearby !! Cannot change
                                          // cloak." );
    return;
  }

  //-------------------Display Cloak for selection--------------------------//
  font_whbl.center_put(INFO_X1 + 158, INFO_Y1 + 150, INFO_X1 + 217,
                       INFO_Y1 + 162,
                       text_unit.str_spy_cloak()); //"Spy Cloak:" );
  ...
  for (int i = 0; i <= nation_array.size(); i++) {
    ...
    if (canChangeAnyCloak) {
      if (i > 0 && i <= nation_array.size()) // exclude independent town
      {
        if (nation_array.is_deleted(i) ||
            !nationPtr->get_relation(i)->has_contact)
          continue;
      }
    } else if (canChangeOwnCloak) {
      if (i != nation_recno &&
          i != true_nation_recno()) // only display the current cloaked nation
                                    // and its true nation
        continue;
    } else
      err_here();
    ...
    if (i == nation_recno)
      vga.active_buf->put_bitmap_trans_remap_decompress(
          x + (addedCount % 3) * SPY_CLOAK_WIDTH,
          y + (addedCount / 3) * SPY_CLOAK_WIDTH, nationPict2,
          game.get_color_remap_table(i, 0));
    else
      vga.active_buf->put_bitmap_trans_remap_decompress(
          x + (addedCount % 3) * SPY_CLOAK_WIDTH,
          y + (addedCount / 3) * SPY_CLOAK_WIDTH, nationPict1,
          game.get_color_remap_table(i, 0));
  }
}
...
void Unit::detect_spy_menu() {
  ...
  for (i = 0; i <= nation_array.size(); i++) {
    ...
    if (mouse.single_click(x + (addedCount % 3) * SPY_CLOAK_WIDTH,
                           y + (addedCount / 3) * SPY_CLOAK_WIDTH,
                           x + (addedCount % 3 + 1) * SPY_CLOAK_WIDTH - 1,
                           y + (addedCount / 3 + 1) * SPY_CLOAK_WIDTH - 1)) {
      changeFlag = 1;
      break;
    }
  }
...
static void disp_firm_button(ButtonCustom *button, int) {
  ...
  char *firmBuildName =
      firmBuild ? firm_res.get_build(firmBuild)->name : firm_res[firmId]->name;

  if (!button->enable_flag) {
    vga.active_buf->put_bitmap(button->x1, button->y1,
                               image_icon.read("BUILDUP"));
    font_bld.center_put(button->x1, button->y1 - 2, button->x2, button->y2 - 2,
                        firmBuildName);
    // vga_back.adjust_brightness( button->x1+1, button->y1+1, button->x2-1,
    // button->y2-1, -5 );
    vga.active_buf->bar_alpha(button->x1 + 1, button->y1 + 1, button->x2 - 1,
                              button->y2 - 1, 1, 0);
  } else if (button->pushed_flag) {
    vga.active_buf->put_bitmap(button->x1, button->y1,
                               image_icon.read("BUILDDWN"));
    //		vga_util.blt_buf(button->x1, button->y1, button->x2, button->y2,
    //0);
    font_bld.center_put(button->x1 + 1, button->y1 - 1, button->x2 + 1,
                        button->y2 - 1, firmBuildName);
  } else if (mouse.in_area(button->x1, button->y1, button->x2, button->y2)) {
    vga.active_buf->put_bitmap(button->x1, button->y1,
                               image_icon.read("BUILDSEL"));
    font_bld.center_put(button->x1, button->y1 - 2, button->x2, button->y2 - 2,
                        firmBuildName);
  } else {
    vga.active_buf->put_bitmap(button->x1, button->y1,
...
static void disp_debug_info(Unit *unitPtr, int dispY1, int refreshFlag) {
  ...
  static const char *action_mode_str_array[] = {
      "Stop",        "Move",   "Attack",        "Build Firm",
      "Settle Town", "Assign", "Go Cast Power", "Transform Fortress",
  };
  ...
  static const char *cur_action_str_array[] = {
      "Idle", "Ready to move",   "Move", "Wait", "Attack",
      "Turn", "Ship extra move", "Die",
  };

  int x = INFO_X1 + 13, y = INFO_Y1 + 235;

  font_san.disp(INFO_X2 - 80, y, unitPtr->sprite_recno, 1, INFO_X2 - 41);
  font_san.disp(INFO_X2 - 80, y + 16, unitPtr->cur_order.ai_action_id, 1,
                INFO_X2 - 41);
  font_san.disp(INFO_X2 - 80, y + 32, unitPtr->home_camp_firm_recno, 1,
                INFO_X2 - 41);

  font_san.disp(INFO_X2 - 40, y, unitPtr->leader_unit_recno, 1, INFO_X2 - 3);

  font_san.disp(x, y, action_mode_str_array[unitPtr->cur_order.mode],
                INFO_X2 - 81);
  if (unitPtr->pushed_order.mode >= 0)
    font_san.disp(x, y += 16, action_mode_str_array[unitPtr->pushed_order.mode],
                  INFO_X2 - 81);
  font_san.disp(x, y += 16, unit_mode_str_array[unitPtr->unit_mode],
                INFO_X2 - 81);
  font_san.disp(x, y += 16, cur_action_str_array[unitPtr->cur_action - 1],
                INFO_X2 - 81);
}
...
void Unit::disp_item_menu() {
  ...
  if (item_menu_offset == 0)
    vga.active_buf->put_bitmap(INFO_X1 + 113, INFO_Y1 + 89,
                               image_icon.read("ITEMBOX"));
  //	vga.active_buf->bar( INFO_X1+127, INFO_Y1+90, INFO_X1+127+92-1,
  //INFO_Y1+90+73-1, V_BLACK);

  char *iconBitmap = item_res.item_icon(item.id);
  if (iconBitmap) {
    vga.active_buf->put_bitmap(INFO_X2 - item_menu_offset - ITEM_ICON_WIDTH -
                                   10,
                               INFO_Y1 + 90 + 16, iconBitmap);
  }

  font_whbl.put_paragraph(INFO_X1 + 127 + 3 - item_menu_offset, INFO_Y1 + 94,
                          INFO_X1 + 218 - item_menu_offset, INFO_Y1 + 140,
                          item.item_name());

  if (is_own())
    font_whbl.put_paragraph(
        INFO_X1 + 127 + 2 - item_menu_offset, INFO_Y1 + 140,
        INFO_X1 + 218 - item_menu_offset, INFO_Y1 + 177,
        text_unit.str_item_drop()); // "Right click to drop" );

  if (config.show_debug_info)
    font_whbl.put(INFO_X1 + 127 + 3 - item_menu_offset, INFO_Y1 + 124,
                  item.para);
...
void Unit::detect_item_menu() {
  ...
  if (mouse.single_click(INFO_X2 - item_menu_offset - ITEM_ICON_WIDTH - 10,
                         INFO_Y1 + 90 + 16, INFO_X2 - item_menu_offset - 10,
                         INFO_Y1 + 90 + 16 + ITEM_ICON_HEIGHT - 1, 1)) {
    drop_item(COMMAND_PLAYER);
  } else {
    help.set_custom_help(INFO_X2 - item_menu_offset - ITEM_ICON_WIDTH - 10,
                         INFO_Y1 + 90 + 16, INFO_X2 - item_menu_offset - 10,
                         INFO_Y1 + 90 + 16 + ITEM_ICON_HEIGHT - 1,
                         item.item_name(), item.item_desc());
  }

  //	if( button_desc_item.detect() )
  //	{
  //		font_whbl.put_paragraph( INFO_X1+127+3 - item_menu_offset,
  //INFO_Y1+94, INFO_X1+218 - item_menu_offset, INFO_Y1+170, 			item.item_desc() );
  //	}

  if ((item.can_turn() || item.can_use_manually()) &&
      button_use_item.detect()) {
    // NEWCODEHERE
    ...
    // ###### end Gilbert 5/3 ####//
      ...
      // ###### begin Gilbert 5/3 ########//
      short *shortPtr = (short *)remote.new_send_queue_msg(MSG_UNIT_USE_ITEM,
                                                           3 * sizeof(short));
...
void Unit::disp_auto_menu(int refreshFlag) {
  ...
  //	headingStr = "Automatically Retreat this (Left-click) or all your
  //Generals (Right-click) to Home Fort when hit point is below :";

  //	str = headingStr;
  font_bld.center_put_paragraph(INFO_X1 + 17, INFO_Y1 + 20, INFO_X2 - 7,
                                INFO_Y1 + 70, text_unit.str_auto_retreat_inst(),
                                0, 0, 0);
  ...
  for (i = 0; i < BUTTON_AUTO_RETREAT_COUNT; i++) {
    if (refreshFlag == INFO_REPAINT)
      button_auto_retreat_array[i].create(
          x + (i / 5) * 94, y + (i % 5) * 23, x + (i / 5) * 94 + 91,
          y + (i % 5) * 23 + 20, disp_auto_menu_button,
          ButtonCustomPara(game.get_color_remap_table(nation_recno, 0), i));
    button_auto_retreat_array[i].paint();
  }

  i = BUTTON_AUTO_RETREAT_COUNT;
  if (refreshFlag == INFO_REPAINT)
    button_auto_retreat_disabled.create(
        x + (i / 5) * 94, y + (i % 5) * 23, x + (i / 5) * 94 + 91,
        y + (i % 5) * 23 + 20, disp_auto_menu_button,
        ButtonCustomPara(game.get_color_remap_table(nation_recno, 0), i));
...
static void disp_auto_menu_button(ButtonCustom *button, int) {
  ...
  if (button->pushed_flag) {
    vga.active_buf->put_bitmap(button->x1, button->y1,
                               image_icon.read("BUILDDW3"));
    //		vga_util.blt_buf(button->x1, button->y1, button->x2, button->y2,
    //0);
    font_bld.center_put(button->x1 + 1, button->y1, button->x2 + 1, button->y2,
                        str);
  } else if (mouse.in_area(button->x1, button->y1, button->x2, button->y2)) {
    vga.active_buf->put_bitmap(button->x1, button->y1,
                               image_icon.read("BUILDSE3"));
    font_bld.center_put(button->x1, button->y1 - 1, button->x2, button->y2 - 1,
                        str);
  } else {
    vga.active_buf->put_bitmap(button->x1, button->y1,
                               image_icon.read("BUILDUP3"));
    font_bld.center_put(button->x1, button->y1 - 1, button->x2, button->y2 - 1,
                        str);
  }
}
...
void Unit::detect_auto_menu() {
  ...
  for (i = 0, hitPointPercentage = 10; i < BUTTON_AUTO_RETREAT_COUNT;
       hitPointPercentage += 10, i++) {
  ...
  if (rc == 1) {
    if (!remote.is_enable()) {
      ...
      short *shortPtr = (short *)remote.new_send_queue_msg(
          MSG_UNIT_AUTO_RETREAT, 2 * sizeof(short));
      *shortPtr = sprite_recno;
...
void Unit::disp_hire_menu(int refreshFlag) {
  ...
  font_snds.put_paragraph(
      INFO_X1 + 22, INFO_Y1 + 18, INFO_X2 - 10, INFO_Y1 + 200,
...
void Unit::detect_hire_menu() {
  ...
  if (button_hire.detect()) {
    ...
    if (!remote.is_enable()) {
      ...
      // ###### begin Gilbert 25/1 ########//
      short *shortPtr = (short *)remote.new_send_queue_msg(MSG_UNIT_HIRE_HERO,
                                                           2 * sizeof(short));
...
void Unit::disp_hire_result(int refreshFlag) {
  ...
  } else // if fail
  {
    if (is_hero_for_hire()) {
      ...
      font_snds.put_paragraph(INFO_X1 + 22, INFO_Y1 + 18, INFO_X2 - 10,
                              INFO_Y1 + 200,
...
int Unit::disp_edit_mode(int refreshFlag) {
  ...
  x = edit_hp_x2 = font_blue.put((edit_hp_x1 = x + 20), (edit_hp_y1 = y),
                                 (int)hit_points, 0, x2);
  ...
  if (unitInfo->class_info.has_loyalty && rank_id != RANK_KING) {
    ...
    x = edit_loyalty_x2 = font_blue.put((edit_loyalty_x1 = x + 20),
                                        (edit_loyalty_y1 = y), loyalty, 0, x2);
  ...
  if (unitInfo->class_info.has_combat_level) {
    x = font_whbl.put(x1, y, text_unit.str_combat_level(), 0,
                      x2); //	"Combat Level"
    x = edit_combat_x2 =
        font_blue.put((edit_combat_x1 = x + 20), (edit_combat_y1 = y),
                      skill.actual_combat_level(NULL), 0, x2);
    ...
    {
      x = font_whbl.put(x, y, " / ", 0, x2);
      x = edit_max_combat_x2 =
          font_blue.put((edit_max_combat_x1 = x), (edit_max_combat_y1 = y),
                        skill.max_combat_level, 0, x2);
  ...
  if (unitInfo->class_info.has_skill_level) {
    ...
    x = edit_skill_x2 =
        font_blue.put((edit_skill_x1 = x + 20), (edit_skill_y1 = y),
                      skill.actual_skill_level(NULL), 0, x2);
    edit_skill_y2 = edit_skill_y1 + font_blue.max_font_height - 1;
    // ###### begin Gilbert 26/5 ##########//
    edit_skill_enable =
        1; // skill.actual_skill_level(NULL) >= CITIZEN_SKILL_LEVEL;
    // ###### end Gilbert 26/5 ##########//
    ...
    if (sys.debug_session) // maximum skill level is a higher secret
    {
      x = font_whbl.put(x, y, " / ", 0, x2);
      x = edit_max_skill_x2 =
          font_blue.put((edit_max_skill_x1 = x), (edit_max_skill_y1 = y),
                        skill.max_skill_level, 0, x2);
  ...
  if (spy_recno) {
    ...
    if (!nation_array.is_deleted(trueNationRecno)) {
      ...
      trueNationChar[0] = FIRST_NATION_COLOR_CODE_IN_TEXT +
                          nation_array[trueNationRecno]->color_scheme_id;
      x = font_whbl.put(x, y, trueNationChar, 0, x2);
      x += 10;
    }
    x = edit_spy_x2 = font_blue.put(edit_spy_x1 = x, edit_spy_y1 = y,
                                    spy_array[spy_recno]->spy_skill, 0, x2);
  ...
  font_blue.put(x1, INFO_Y2 - 28,
                text_unit.str_left_inc()); // "Left click to increase" );
  font_blue.put(x1, INFO_Y2 - 14,
                text_unit.str_right_dec()); // "Right click to decrease" );
...
void Unit::detect_edit_mode() {
  if (edit_hp_enable) {
    if (mouse.any_click(edit_hp_x1, edit_hp_y1, edit_hp_x2, edit_hp_y2,
                        LEFT_BUTTON)) {
      hit_points += 10.0f;
  ...
  if (edit_loyalty_enable) {
    if (mouse.any_click(edit_loyalty_x1, edit_loyalty_y1, edit_loyalty_x2,
                        edit_loyalty_y2, LEFT_BUTTON)) {
      loyalty += 10;
      if (loyalty > 100)
        loyalty = 100;
    }
    if (mouse.any_click(edit_loyalty_x1, edit_loyalty_y1, edit_loyalty_x2,
                        edit_loyalty_y2, RIGHT_BUTTON)) {
      loyalty -= 10;
      if (loyalty < 0)
        loyalty = 0;
    }
  ...
  if (edit_combat_enable) {
    if (mouse.any_click(edit_combat_x1, edit_combat_y1, edit_combat_x2,
                        edit_combat_y2, LEFT_BUTTON)) {
  ...
  if (edit_max_combat_enable) {
    if (mouse.any_click(edit_max_combat_x1, edit_max_combat_y1,
                        edit_max_combat_x2, edit_max_combat_y2, LEFT_BUTTON)) {
    ...
    if (mouse.any_click(edit_max_combat_x1, edit_max_combat_y1,
                        edit_max_combat_x2, edit_max_combat_y2, RIGHT_BUTTON)) {
      int newLevel = skill.max_combat_level;
      newLevel -= 20;
  ...
  if (edit_skill_enable) {
    if (mouse.any_click(edit_skill_x1, edit_skill_y1, edit_skill_x2,
                        edit_skill_y2, LEFT_BUTTON)) {
      skill.inc_skill_level(5.0f);
    }
    if (mouse.any_click(edit_skill_x1, edit_skill_y1, edit_skill_x2,
                        edit_skill_y2, RIGHT_BUTTON)) {
  ...
  if (edit_max_skill_enable) {
    if (mouse.any_click(edit_max_skill_x1, edit_max_skill_y1, edit_max_skill_x2,
                        edit_max_skill_y2, LEFT_BUTTON)) {
    ...
    if (mouse.any_click(edit_max_skill_x1, edit_max_skill_y1, edit_max_skill_x2,
                        edit_max_skill_y2, RIGHT_BUTTON)) {
      int newLevel = skill.max_skill_level;
      newLevel -= 20;
  ...
  if (edit_spy_enable && spy_recno) {
    ...
    if (mouse.any_click(edit_spy_x1, edit_spy_y1, edit_spy_x2, edit_spy_y2,
                        LEFT_BUTTON)) {
...
//  in report mode (especially chat mode), button.detect() should not detect
//  keyboard
//
int Unit::send_hotkey(unsigned scanCode, unsigned skeyState) {
  err_when(sprite_recno !=
           unit_array.selected_recno); // must be the main selected
