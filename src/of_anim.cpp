...
short FirmAnimal::animal_unit_id[ANIMAL_TYPE_COUNT] = {
    UNIT_JAGUAR,
    UNIT_GORILLA,
    UNIT_RHINO,
};
...
int FirmAnimal::can_hire(int recNo) {
  ...
  return unit_res[animalUnit->unit_id]->get_nation_tech_level(nation_recno) >
             0 &&
...
int FirmAnimal::hire(short recNo) {
  ...
  if (nationPtr->cash < animalUnit->hire_cost ||
      nationPtr->live_points < animalUnit->live_point_cost)
    return 0;
  ...
  err_when(animalUnit->combat_level() <= 0 ||
           animalUnit->combat_level() > 1000);
...
void FirmAnimal::add_animal_unit(int unitId, int animalLevel) {
  ...
  case 1: // 20-30
    ...
    break;
  case 2: // 50-100
    animalUnit->skill.init(
        unitId, MAX_COMBAT_TRAIN / 2 + misc.random(MAX_COMBAT_TRAIN / 2), 0);
    break;
  case 3: // 100-200
    animalUnit->skill.init(
        unitId,
        MAX_COMBAT_TRAIN + misc.random(MAX_COMBAT_BATTLE - MAX_COMBAT_TRAIN),
        0);
    break;
  default:
    err_here();
...
void FirmAnimal::del_animal_unit(int recNo) {
  ...
  misc.del_array_rec(animal_unit_array, animal_unit_count, sizeof(AnimalUnit),
                     recNo);
...
void FirmAnimal::update_add_hire_list() {
  ...
  /*
          ...
          for( int i=0 ; i<ANIMAL_TYPE_COUNT ; i++ )
          {
                  if( tech_res[ animalTechIdArray[i]
     ]->get_nation_tech_level(nation_recno) )
                  {
  ...
  */
  ...
  //	{
  //		if( !nation_recno ||
  //unit_res[animal_unit_id[i]]->get_nation_tech_level(nation_recno) )
  //		{
