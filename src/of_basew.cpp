...
WishInfo wish_info_array[MAX_WISH] = {
    // id                    rare, desc
    {WISH_FINISH_TECH, 90,
     "Technology breakthrough - complete the current research instantly"},
    ...
    {WISH_RAW, 60,
     "Creation of a new mineral deposit and instantly building a mine on top "
     "of it."},
    ...
    {WISH_KILL_SPY_BUILDING, 60,
     "Reveal and kill a few enemy spies in a town or structures of yours"},
    {WISH_KILL_SPY_NATION, 40,
     "Reveal and kill a few enemy spies in your kingdom"},
    {WISH_CLEAR_SPY_NATION, 10,
     "Reveal and kill every enemy spy in your kingdom"},
    {WISH_HEAL_BUILDING, 80,
     "Heal all of your wounded soldiers in a structures"},
    {WISH_HEAL_NATION, 10, "Heal all of your wonded soldiers in your kingdom"},
    {WISH_LOYALTY_BUILDING_S, 5,
     "Increase the loyalty of a selected town or the loyalty of all units in a "
     "structure by 20-50"},
    {WISH_LOYALTY_BUILDING_L, 10,
     "Increase the loyalty of a selected town or the loyalty of all units in a "
     "structure to 100"},
    {WISH_LOYALTY_NATION_S, 10,
     "Increase the loyalty of everybody in your kingdom by 20-50"},
    {WISH_LOYALTY_NATION_L, 20,
     "Increase the loyalty of everybody in your kingdom to 100"},
    {WISH_COMBAT_BUILDING, 50,
     "Increase the combat level of all soldiers in a structure by 30-70"},
};

int FirmBase::can_make_wish() { return pray_points >= (float)MAX_PRAY_POINTS; }
...
void FirmBase::invoke_random_effect(int effectId, char remoteAction) {
  ...
  if (!remoteAction && remote.is_enable()) {
    ...
    short *shortPtr = (short *)remote.new_send_queue_msg(
        MSG_F_BASE_INVOKE_EFFECT, 2 * sizeof(short));
  ...
  case WISH_FINISH_TECH:
    // Technology breakthrough! Your present research project has been
    // completed!
    ...
    // Technology breakthrough! You have acquired <technology> technology!
    {
      ...
      for (techId = 1; techId <= MAX_TECH; ++techId) {
        ...
        if (tech_res[techId]->can_research(nation_recno) &&
            nation_ptr()->research_tech_id !=
                techId) // if the nation is currently researching this tech,
                        // don't consider it
        {
      ...
      if (validTechIdCount > 0) {
        ...
        techInfo->set_nation_tech_level(
            nation_recno, techInfo->get_nation_tech_level(nation_recno) + 1);
  ...
  case WISH_NEW_SPY:
    // A veteran <race> spy has entered your service!
    {
      ...
      case 1:
        ...
        skill.init(unitId,
                   MAX_COMBAT_TRAIN +
                       misc.random(MAX_COMBAT_BATTLE - MAX_COMBAT_TRAIN),
                   BASIC_SKILL_TRAIN +
                       misc.random(MAX_SKILL_TRAIN - BASIC_SKILL_TRAIN));
        break;
      ...
      if (unitRecno) {
        ...
        if (effectId == WISH_NEW_HUMAN) {
          ...
          unitPtr->spy_recno =
              spy_array.add_spy(unitPtr->sprite_recno, 100 - misc.random(20));
  ...
  case WISH_NEW_MONSTER:
    // A fryhtan has entered your service!
    {
      ...
      skill.init(unitId,
                 BASIC_COMBAT_TRAIN +
                     misc.random(MAX_COMBAT_TRAIN - BASIC_COMBAT_TRAIN),
                 BASIC_SKILL_TRAIN +
                     misc.random(MAX_SKILL_TRAIN - BASIC_SKILL_TRAIN));
  ...
  case WISH_NEW_HERO:
    // A Hero, <name>, has offered you his service!
    {
      ...
      if (locate_space(is_being_deleted, xLoc, yLoc, loc_x2, loc_y2, 2, 2)) {
        int unitRecno =
            hero_res.create_hero(xLoc, yLoc, nation_recno, race_id, 0, 0, 1);
        if (unitRecno) {
          Unit *unitPtr = unit_array[unitRecno];
  ...
  case WISH_ITEM:
    // An artifact has been uncovered near here! Pick it up before it is
    // discovered by your enemies
    {
      int xLoc = loc_x1 - 2, yLoc = loc_y1 - 2;

      // assume locate width and height are bigger, size 2

      if (locate_space(is_being_deleted, xLoc, yLoc, loc_x2 + 2, loc_y2 + 2, 1,
                       1)) {
        Item randomItem;
        ...
        if (randomItem.id) {
          site_array.add_site(xLoc, yLoc, SITE_ITEM, randomItem.id,
                              randomItem.para);
  ...
  case WISH_CASH:
    // A peasant has discovered a hidden hoard of gold has has gladly turned it
    // all over to you!
    ...
    // A deposit has been found and a new mine built on top of it!
    {
      ...
      if (siteRecno) {
        ...
        int newFirmRecno = firm_array.build_firm(
            ...
            nation_recno, FIRM_MINE,
            race_res[race_id]->code, // nation_ptr->race_id may be negative
  ...
  case WISH_CAPTURE_UNIT:
    // An enemy (soldier or civilian) has defected and joined your Kingdom!
    {
      ...
      for (i = unitArraySize; i > 0; --i) {
        ...
        if (unitPtr->rank_id != RANK_KING && unitPtr->is_visible() &&
            !unitPtr->is_nation(nation_recno) && !unitPtr->spy_recno &&
            unitPtr->race_id && unitPtr->unit_id != UNIT_WAGON &&
    ...
    // An enemy <fort> has recognized your greatness and surrendered to you!
    {
      ...
      for (i = firmArraySize; i > 0; --i) {
        ...
        if (firmPtr->nation_recno != nation_recno &&
            (camp = firmPtr->cast_to_FirmCamp()) &&
            firmPtr->is_human() // don't change monster lair
            && (!camp->overseer_recno ||
                unit_array[camp->overseer_recno]->rank_id !=
                        RANK_KING // don't change camp with a king
          ...
          resultStr = text_firm.process(
              text_firm.get_format_str(24, "BAWC"),
  ...
  case WISH_CAPTURE_TOWN:
    // An enemy town, tired of living under oppressive rule, has declared for
    // you!
    {
      ...
      for (i = townArraySize; i > 0; --i) {
        ...
        if (townPtr->nation_recno != nation_recno &&
            (townPtr->surrender(nation_recno),
  ...
  case WISH_KILL_SPY_NATION:
    // <count> enemy spies in your Kingdom has been exposed and executed!
    {
      ...
      if (killCount > 0) {
        resultStr =
            text_firm.process(text_firm.get_format_str(27, "BAWF"), killCount);
      }
    }
    break;
  ...
  case WISH_HEAL_BUILDING:
    // All of the wounded in a(n) (structure) of yours have been healed of their
    // wounds!
    {
      // select building
      ...
      if (bestFirm && bestWeight > 16) // don't heal if they are healthy
      {
        ...
        resultStr =
            text_firm.process(text_firm.get_format_str(29, "BAWH"),
                              firm_res.get_build(firmPtr->firm_build_id)->name);
  ...
  case WISH_LOYALTY_BUILDING_L:
    ...
    // The loyalty of everyone in a(n) (structure) of yours has increased by
    // (points) points! The loyalty of the people of (town) has increased to its
    // maximum! The loyalty of everyone in a(n) (structure) of yours has
    // increased to its maximum!
    {
      int loyalInc =
          (effectId == WISH_LOYALTY_BUILDING_S ? misc.random(30) + 20 : 100);
      ...
      if (misc.random(2) == 0) {
        ...
        for (int i = firmArraySize; i > 0; --i) {
          ...
          if (firmPtr->cast_to_FirmCamp()) {
            ...
            if (loyalInc < 100) {
              resultStr = text_firm.process(
                  text_firm.get_format_str(33, "BAWL"),
                  firm_res.get_build(firmPtr->firm_build_id)->name, loyalInc);
            } else {
              resultStr = text_firm.process(
                  text_firm.get_format_str(34, "BAWM"),
                  firm_res.get_build(firmPtr->firm_build_id)->name);
            }
  ...
  case WISH_LOYALTY_NATION_L:
    // The loyalty of every subject in your Kingdom has increased by (points)
    // points! The loyalty of every subject in your Kingdom has increased to its
    // maximum!
    {
      int loyalInc =
          (effectId == WISH_LOYALTY_NATION_S ? misc.random(30) + 20 : 100);
      ...
      if (loyalInc < 100) {
        resultStr =
            text_firm.process(text_firm.get_format_str(35, "BAWN"), loyalInc);
      } else {
  ...
  case WISH_COMBAT_BUILDING:
    // The combat level of all soldiers in a(n) (structure) of yours has
    // increased by (points) points!
    {
      ...
      for (int i = firm_array.size(); i > 0; --i) {
        ...
        if (firmPtr->cast_to_FirmCamp()) {
          ...
          resultStr = text_firm.process(
              text_firm.get_format_str(37, "BAWP"),
          ...
          for (int s = 0; s < train->trainee_count; ++s) {
            if (!train->trainee_array[s].is_under_training &&
                train->trainee_array[s].train_combat_potential >
                    0) // avoid increase civilian combat
  ...
  if (firm_recno == firm_array.selected_recno) {
    err_when(sizeof(invoked_effect_text) ==
             sizeof(char *)); // error when invoked_effect_text changed to char
                              // * from char[]
