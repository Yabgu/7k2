...
void FirmBase::invoke_random_effect(int effectId, char remoteAction)
{
        ...
        // Technology breakthrough! You have acquired <technology> technology!
        {
            ...
            for (techId = 1; techId <= MAX_TECH; ++techId)
            {
                ...
                if (tech_res[techId]->can_research(nation_recno) &&
                    nation_ptr()->research_tech_id != techId) // if the nation is currently researching this tech,
                                                              // don't consider it
                {
            ...
            if (validTechIdCount > 0)
            {
                ...
                resultStr = text_firm.process(text_firm.get_format_str(14, "BAW2"), techInfo->tech_des());
            }
        }
        break;
    ...
    case WISH_NEW_SPY:
        // A veteran <race> spy has entered your service!
        {
            ...
            case 1:
                ...
                skill.init(unitId, MAX_COMBAT_TRAIN + misc.random(MAX_COMBAT_BATTLE - MAX_COMBAT_TRAIN),
                           BASIC_SKILL_TRAIN + misc.random(MAX_SKILL_TRAIN - BASIC_SKILL_TRAIN));
                break;
            ...
            if (unitRecno)
            {
                ...
                {
                    resultStr =
                        text_firm.process(text_firm.get_format_str(15, "BAW3"), race_res[unitPtr->race_id]->name);
                }
                else if (effectId == WISH_NEW_SPY)
                {
                    unitPtr->spy_recno = spy_array.add_spy(unitPtr->sprite_recno, 100 - misc.random(20));
                    resultStr =
                        text_firm.process(text_firm.get_format_str(16, "BAW4"), race_res[unitPtr->race_id]->name);
                }
                else
                {
                    err_here();
    ...
    case WISH_NEW_MONSTER:
        // A fryhtan has entered your service!
        {
            ...
            skill.init(unitId, BASIC_COMBAT_TRAIN + misc.random(MAX_COMBAT_TRAIN - BASIC_COMBAT_TRAIN),
                       BASIC_SKILL_TRAIN + misc.random(MAX_SKILL_TRAIN - BASIC_SKILL_TRAIN));
        ...
        // A Hero, <name>, has offered you his service!
        {
            ...
            if (locate_space(is_being_deleted, xLoc, yLoc, loc_x2, loc_y2, 2, 2))
            {
                ...
                if (unitRecno)
                {
                    ...
                    resultStr = text_firm.process(text_firm.get_format_str(18, "BAW6"), unitPtr->unit_name(0));
    ...
    case WISH_RAW:
        // A deposit has been found and a new mine built on top of it!
        {
            ...
            {
                // ###### begin Gilbert 29/6 #######//
                int newFirmRecno = firm_array.build_firm(site_array[siteRecno]->map_x_loc,
                                                         site_array[siteRecno]->map_y_loc, nation_recno, FIRM_MINE,
    ...
    case WISH_CAPTURE_UNIT:
        // An enemy (soldier or civilian) has defected and joined your Kingdom!
        {
            ...
            for (i = unitArraySize; i > 0; --i)
            {
                ...
                if (unitPtr->rank_id != RANK_KING && unitPtr->is_visible() && !unitPtr->is_nation(nation_recno) &&
                    !unitPtr->spy_recno && unitPtr->race_id && unitPtr->unit_id != UNIT_WAGON &&
                    unitPtr->unit_mode != UNIT_MODE_REBEL)
                {
                    unitPtr->betray(nation_recno);

                    resultStr = text_firm.process(text_firm.get_format_str(23, "BAWB"), unitPtr->unit_name());
    ...
    case WISH_CAPTURE_CAMP:
        // An enemy <fort> has recognized your greatness and surrendered to you!
        {
            ...
            for (i = firmArraySize; i > 0; --i)
            {
                ...
                if (firmPtr->nation_recno != nation_recno && (camp = firmPtr->cast_to_FirmCamp()) &&
                    firmPtr->is_human() // don't change monster lair
                    && (!camp->overseer_recno ||
                        unit_array[camp->overseer_recno]->rank_id != RANK_KING // don't change camp with a king
        ...
        // you!
        {
            ...
            for (i = townArraySize; i > 0; --i)
            {
                ...
                if (townPtr->nation_recno != nation_recno &&
                    (townPtr->surrender(nation_recno), townPtr->nation_recno == nation_recno))
                {
    ...
    case WISH_KILL_SPY_BUILDING:
        // <number> enemy spy has/spyies have been exposed and executed!
        {
            ...
            if (spyPlace != SPY_UNDEFINED)
            {
                ...
                if (killCount > 0)
                {
                    resultStr = text_firm.process(text_firm.get_format_str(26, "BAWE"), killCount);
                }
            }
        }
        break;
    ...
    case WISH_KILL_SPY_NATION:
        // <count> enemy spies in your Kingdom has been exposed and executed!
        {
            ...
            if (killCount > 0)
            {
                resultStr = text_firm.process(text_firm.get_format_str(27, "BAWF"), killCount);
            }
        }
        break;
        ...
        // wounds!
        {
            ...
            for (int i = firm_array.size(); i > 0; --i)
            {
                ...
                if (firmPtr->cast_to_FirmCamp())
                {
                    ...
                    {
                        weight += camp->soldier_array[s].max_hit_points() - (int)camp->soldier_array[s].hit_points;
                    }
        ...
        // increased to its maximum!
        {
            ...
            if (misc.random(2) == 0)
            {
                ...
                for (int i = townArraySize; i > 0; --i)
                {
                    ...
                    {
                        resultStr =
                            text_firm.process(text_firm.get_format_str(31, "BAWJ"), townPtr->town_name(), loyalInc);
                    }
                    else
                    {
                        resultStr = text_firm.process(text_firm.get_format_str(32, "BAWK"), townPtr->town_name());
                    }
    ...
    case WISH_LOYALTY_NATION_L:
        ...
        // maximum!
        {
            ...
            if (loyalInc < 100)
            {
                resultStr = text_firm.process(text_firm.get_format_str(35, "BAWN"), loyalInc);
            }
            else
            {
    ...
    {
        err_when(sizeof(invoked_effect_text) == sizeof(char *)); // error when invoked_effect_text changed to char
                                                                 // * from char[]
