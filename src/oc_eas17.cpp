...
enum
{
    STAGE_17_MONSTER_NATION_RECNO = 2
};
...
{
    int lairAddCount = 9 + misc.random(3); // add 6 to 7 Fryhtan Lairs
    ...
    create_lair(cur_monster_nation_recno, STAGE_17_MONSTER_NATION_RECNO, lairAddCount, independentTownAddCount,
...
void CampaignEastWest::stage_17_process_game_result()
{
    ...
    else
    {
        ...
        else
        {
            ...
            info.game_year = game_year; // revert info.game_year back to campaign's game year so
                                        // that the game's time will not advance
