...
void Nation::think_reduce_expense()
{
    ...
    for (int i = firm_array.size(); i > 0; i--)
    {
        ...
        if (firmPtr->firm_id == FIRM_WAR_FACTORY || firmPtr->firm_id == FIRM_RESEARCH)
        {
...
//
int Nation::ai_should_spend(int importanceRating, float spendAmt, float spendLivePoints)
{
    if (cash < spendAmt)
        return 0;
    ...
    if (spendLivePoints > 0)
    {
        ...
        float curLivePointsLevel = (float)100 * (live_points - spendLivePoints) / stdLivePointsLevel;
