...
void Nation::think_reduce_expense() {
  ...
  for (int i = firm_array.size(); i > 0; i--) {
    ...
    if (firmPtr->firm_id == FIRM_WAR_FACTORY ||
      ...
      curRating = 100 - (int)firmPtr->cast_to_FirmWork()->productivity;
    } else
      continue;
...
//										consider generally
//- whether the 										nation should spend money generally.
//
// [float] spendLivePoints	 - the live points to be used.
//
int Nation::ai_should_spend(int importanceRating, float spendAmt,
                            float spendLivePoints) {
  if (cash < spendAmt)
    return 0;

  float fixedExpense = fixed_expense_365days();
  float stdCashLevel =
      MAX(fixedExpense, 2000) * (150 + pref_cash_reserve) / 100;
  ...
  if (spendLivePoints > 0) {
    int stdLivePointsLevel = 2000 * (50 + pref_cash_reserve) / 100;

    float curLivePointsLevel =
        (float)100 * (live_points - spendLivePoints) / stdLivePointsLevel;

    if (importanceRating < (100 - curLivePointsLevel))
      return 0;
  }
...
// [int] considerCeaseFire	  - whether this function is called when
// considering ceasing fire
//										 (default:0)
//
int Nation::ai_should_spend_war(int enemyMilitaryRating,
                                int considerCeaseFire) {
  int importanceRating = 30 + pref_military_development / 5; // 30 to 50

  importanceRating += military_rank_rating() - enemyMilitaryRating * 2;

  if (considerCeaseFire) // only when we are very powerful, we will start a
                         // battle. So won't cease fire too soon after declaring
                         // war
