...
short Plasma::get_pix(short x, short y) { return matrix[y * (max_x + 1) + x]; }
...
void Plasma::generate(int genMethod, int grainFactor, int randomSeed) {
  ...
  for (n = 0; n < 4; n++)
    rnd[n] = 1 + (((misc.rand() / MAX_COLOR) * (MAX_COLOR - 1)) >>
                  (SHIFT_VALUE - 11));
...
U16 Plasma::adjust(int xa, int ya, int x, int y, int xb, int yb) {
  ...
  pseudoRandom =
      (((S32)get_pix(xa, ya) + (S32)get_pix(xb, yb) + 1) >> 1) + pseudoRandom;
...
// <short *> minHeights			(input) lower limit of each group, must be
// strictly increasing
