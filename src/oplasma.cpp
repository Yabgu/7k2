...
enum
{
    SHIFT_VALUE = 18, // shift based on no. of colors
...
// --------- Begin of function Plasma::Plasma -----------//
Plasma::Plasma(Plasma &p)
{
    memcpy(this, &p, sizeof(Plasma));
    matrix = (short *)mem_add((max_x + 1) * (max_y + 1) * sizeof(short));
    memcpy(matrix, p.matrix, (max_x + 1) * (max_y + 1) * sizeof(short));
}
// --------- End of function Plasma::Plasma -----------//

// --------- Begin of function Plasma::operator= -----------//
Plasma &Plasma::operator=(Plasma &p)
{
    deinit();
...
// --------- End of function Plasma::operator= -----------//
...
void Plasma::init(short x, short y, short initValue)
{
    deinit();
    max_x = x;
    max_y = y;
...
// <short *> minHeights			(input) lower limit of each group, must
// be strictly increasing <int *> freq					(output)
// frequency of each group <int> return value			total frequency
//
...
void Plasma::shuffle_level(short minHeight, short maxHeight, short amplitude)
{
    ...
    for (y = 1; y < max_y; ++y)
    {
        ...
        {
            short h;
            if ((h = get_pix(x, y)) >= minHeight && h <= maxHeight && (h = get_pix(x - 1, y - 1)) >= minHeight &&
                h <= maxHeight && (h = get_pix(x, y - 1)) >= minHeight && h <= maxHeight &&
                (h = get_pix(x + 1, y - 1)) >= minHeight && h <= maxHeight && (h = get_pix(x - 1, y)) >= minHeight &&
                h <= maxHeight && (h = get_pix(x + 1, y)) >= minHeight && h <= maxHeight &&
                (h = get_pix(x - 1, y + 1)) >= minHeight && h <= maxHeight && (h = get_pix(x, y + 1)) >= minHeight &&
                h <= maxHeight && (h = get_pix(x + 1, y + 1)) >= minHeight && h <= maxHeight)
            {
                h = get_pix(x, y);
