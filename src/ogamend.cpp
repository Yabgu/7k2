...
//								  0 - if you are just
// defeated
// or surrender 										to another
// kingdom, and no other kingdom has won yet. [int] playerDestroyed - whether
// the player's nation has been destroyed or not.
//									(default:
// 0)
...
void Game::game_end(int winNationRecno, int playerDestroyed, int surrenderToNationRecno, int retireFlag)
{
    ...
    if (remote.is_enable() && remote.is_lobbied() && serial_repository.remove_last(generated_serial))
    {
        ...
        if (max_ranking > 0)
        {
            ...
            // sumLoserRank ); 			int lastY = 0;
            // for( linear.inc(); !linear.is_end(); linear.inc() )
            //			{
            //				if( winnerArray[linear.x-1] ==
            // nation_array.player_recno ) 					rankInc
            // = linear.y - lastY; lastY = linear.y;
            //			}
    ...
    if (!retireFlag && !game.is_campaign_mode() && game.game_mode != GAME_TUTORIAL) // don't display this when retire
    {
        const char *fileName;
...
static void disp_win_str(int winNationRecno)
{
    err_when(game.result_win_condition_id < 0 || game.result_win_condition_id > END_CONDITION_COUNT);
...
static void disp_losing_str(int surrenderToNationRecno)
{
    ...
    if (surrenderToNationRecno) // you surrender to another kingdom
    {
        ...
        str = text_end_condition.str_lose_surrender(nation_array[surrenderToNationRecno]->nation_name(),
...
static void disp_retire_str()
{
    font_scre.center_put(0, 0, VGA_WIDTH - 1, 139, text_end_condition.str_you_retire(date.date_str(info.game_date)));
}
...
static void disp_stat(int y)
{
    ...
    //); 	put_stat( y+=20, "Kingdom's Ships Missing",
    // nationPtr->own_ship_destroyed
    //);
...
static void disp_ranking()
{
    ...
    font_scre.center_put_paragraph(x + 670 - 32 - 7, y + 6, x + 707, y + 28, text_reports.str_fryhtan_battling(), 0, 0,
                                   0);
#else
    font_scre.put(x + 20 + 100, y + 7, text_unit.str_nation(0)); // "Kingdom" );
    ...
    font_scre.center_put_paragraph(x + 670 - 32, y, x + 707, y + 28, text_reports.str_fryhtan_battling(), 0, 0, 0);
#endif
...
static int disp_score(int winFlag)
{
    ...
    if (info.goal_score_bonus && winFlag &&
        !nation_array[viewNationRecno]->cheat_enabled_flag) // if cheated, don't display bonus score, as
