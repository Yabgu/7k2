...
//								  0 - if you are just defeated
//or surrender 										to another kingdom, and no other kingdom 									   has won yet.
// [int] playerDestroyed - whether the player's nation has been destroyed or
// not.
//									(default:
//0)
...
//
void Game::game_end(int winNationRecno, int playerDestroyed,
                    int surrenderToNationRecno, int retireFlag) {
  //--- if the player has already won/lost the game and is just
  //staying/observing the game ---//

  if (game_has_ended &&
      !retireFlag) // don't repeat displaying the winning/losing screen
    return;
  ...
  // ####### begin Gilbert 15/7 #########//
  int realWin =
      nation_array.player_recno && winNationRecno == nation_array.player_recno;
  // ####### end Gilbert 15/7 #########//
  ...
  if (remote.is_enable() && remote.is_lobbied() &&
    ...
    for (i = 1; i <= MAX_NATION; ++i) {
          ...
          nation_array[i]->check_win(1)) // 1-checking only
      {
        ...
        if (nation_array.player_recno == i)
          isPlayerWin = 1;
      } else // otherwise loser, notice player dropped may become ai, new nation
             // formed is also ai
      {
        loserArray[losers] = i;
        losers++;
    ...
    if (max_ranking > 0) {
      ...
      //			LinearCounter linear( 0, 0, winners,
      //sumLoserRank ); 			int lastY = 0; 			for( linear.inc(); !linear.is_end();
      //linear.inc() )
      //			{
      //				if( winnerArray[linear.x-1] ==
      //nation_array.player_recno ) 					rankInc = linear.y - lastY; 				lastY =
      //linear.y;
      //			}

      // player_rank[nation_array.player_recno-1] may not be as same as
      // player_profile.ranking
      ...
      if (rankInc > 0) {
        ...
        if ((long)player_profile.ranking < -rankInc)
          ...
          player_profile.ranking +=
              (WORD)rankInc; // += 'ocz rankInc is negative.
      }
    }
  ...
  if (game.is_campaign_mode())
    sys.signal_exit_flag =
        3; // 3-quit a campaign game only, but still remain in the campaign
  else
  ...
  if (!(game.is_campaign_mode() &&
        game.campaign()
            ->auto_test_flag)) // do not wait if auto_test_flag is on.
    mouse.wait_press(60);      // 60 seconds to time out
  ...
  if (!game.is_campaign_mode()) {
    ...
    info.set_rank_data(0); // count all nations, not only those that have
  ...
  if (!game.is_campaign_mode() && !game_has_ended &&
      game.game_mode != GAME_TUTORIAL) {
    if (!game_file_array.add_hall_of_fame(totalScore, (config.race_id < 0)))
      vga_util.finish_disp_image_file(); // if add_hall_of_fame() has displayed
                                         // the bitmap, it should have called
                                         // vga.finish_disp_image_file() already
  } else {
  ...
  if (!retireFlag && !game.is_campaign_mode() &&
      !remote.is_enable()) // can't stay in the game in a multiplayer game
  {
    vga_front.bar(0, 0, VGA_WIDTH - 1, VGA_HEIGHT - 1,
                  V_BLACK); // clear the screen

    char powerWinFlag = power.win_opened;

    power.win_opened = 1;

    if (box.ask(text_game_menu.str_stay_in_game(), text_game_menu.str_no_yes(1),
                text_game_menu.str_no_yes(0))) // "Yes", "No"
...
static void disp_win_str(int winNationRecno) {
  ...
  //	else
  //		winStr +=
  //game.win_condition[game.result_win_condition_recno].result_str;
  ...
  if (game.win_require_all_condition)
    ...
    winFormatStr =
        game.win_condition[game.result_win_condition_recno].result_str;
  if (winNationRecno == nation_array.player_recno)
    winStr = text_end_condition.str_you_win_str((~nation_array)->race_id,
                                                winFormatStr);
  else
    winStr = text_end_condition.str_other_win_str(
        nation_array[winNationRecno]->nation_name(), winFormatStr);
  // ######## end Gilbert 10/6 ###########//
...
static void disp_losing_str(int surrenderToNationRecno) {
  ...
  // You failed to achieve the goal within the time limit
    ...
    // str = "Your Kingdom has Failed to Achieve its Goal Within the Time
    // Limit.";
    ...
    //		str  = "Your Kingdom";
    //		str +=
    //game.lose_condition[game.result_lose_condition_recno].result_str;
    char *winFormatStr =
        game.lose_condition[game.result_lose_condition_recno].result_str;
    str = text_end_condition.str_you_lose_str((~nation_array)->race_id,
                                              winFormatStr);
...
static void disp_stat(int y) {
  ...
  put_stat(y, text_end_condition.str_duration_of_kingdom(),
           info.game_duration_str());
  put_stat(y += 20, text_end_condition.str_game_time(), info.play_time_str());

  put_stat(y += 30, text_end_condition.str_final_population(),
           nationPtr->all_population());
  put_stat(y += 20, text_end_condition.str_final_treasure(),
           misc.format((int)nationPtr->cash, 2));

  put_stat(y += 30, text_end_condition.str_kill_soldiers(),
           nationPtr->enemy_soldier_killed);
  put_stat(y += 20, text_end_condition.str_lose_soldiers(nationTitle),
           nationPtr->own_soldier_killed);

  put_stat(y += 30, text_end_condition.str_kill_monsters(),
           nationPtr->enemy_frythan_killed);
  put_stat(y += 20, text_end_condition.str_lose_monsters(nationTitle),
           nationPtr->own_frythan_killed);

  put_stat(y += 30, text_end_condition.str_kill_weapons(),
           nationPtr->enemy_weapon_destroyed);
  put_stat(y += 20, text_end_condition.str_lose_weapons(nationTitle),
           nationPtr->own_weapon_destroyed);
  ...
  put_stat(y += 30, text_end_condition.str_destroy_buildings(),
           nationPtr->enemy_firm_destroyed);
  put_stat(y += 20, text_end_condition.str_lose_buildings(nationTitle),
           nationPtr->own_firm_destroyed);

  put_stat(y += 30, text_end_condition.str_kill_civilians(),
           nationPtr->enemy_civilian_killed);
  put_stat(y += 20, text_end_condition.str_lose_civilians(nationTitle),
           nationPtr->own_civilian_killed);
}
...
static void disp_ranking() {
  ...
  font_scre.put(x + 260 + 30 + 24, y + 7,
                text_reports.str_population()); // "Population" );
  font_scre.put(x + 370 + 15 + 16, y + 7,
                text_reports.str_military()); //"Military" );

  font_scre.put(x + 470 - 15 + 18, y + 7,
                text_reports.str_economy()); // "Economy" );
  font_scre.put(x + 562 - 25 + 24, y + 7,
                text_reports.str_reputation()); // "Reputation" );

  font_scre.center_put_paragraph(x + 670 - 32 - 7, y + 6, x + 707, y + 28,
                                 text_reports.str_fryhtan_battling(), 0, 0, 0);
#else
  font_scre.put(x + 20 + 100, y + 7, text_unit.str_nation(0)); // "Kingdom" );
  font_scre.put(x + 260 + 30, y + 7,
                text_reports.str_population()); // "Population" );
  font_scre.put(x + 370 + 15, y + 7,
                text_reports.str_military()); //"Military" );

  font_scre.put(x + 470 - 15, y + 7,
                text_reports.str_economy()); // "Economy" );
  font_scre.put(x + 562 - 25, y + 7,
                text_reports.str_reputation()); // "Reputation" );
...
static void put_ranking(int y, int nationRecno) {
  ...
  font_scre.center_put(x + 260 + 30, y, x + 340 + 30, y2,
                       info.get_rank_pos_str(1, nationRecno));
  font_scre.center_put(x + 370 + 15, y, x + 435 + 15, y2,
                       info.get_rank_pos_str(2, nationRecno));
  font_scre.center_put(x + 470 - 15, y, x + 540 - 15, y2,
                       info.get_rank_pos_str(3, nationRecno));
  font_scre.center_put(x + 562 - 25, y, x + 640 - 25, y2,
                       info.get_rank_pos_str(4, nationRecno));
  font_scre.center_put(x + 670 - 32, y, x + 730 - 32, y2,
                       info.get_rank_pos_str(5, nationRecno));
}
...
static int disp_score(int winFlag) {
  ...
  int difficultyRating = config.race_id >= 0 ? config.human_difficulty_rating
  ...
  str += " (";
  str += text_reports.str_difficulty_rating(); // translate.process(
                                               // "(Difficulty Rating)" );
  str += ")";
  ...
  if (info.goal_score_bonus && winFlag &&
      !nation_array[viewNationRecno]
           ->cheat_enabled_flag) // if cheated, don't display bonus score, as
                                 // there is no space for displaying both
  {
    str += "+  ";
