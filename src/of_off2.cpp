...
void FirmOffensive2::init(int xLoc, int yLoc, int nationRecno, int firmId,
  ...
  /*	if(!nation_recno || ( nation_recno &&
     tech_res[TECH_CAMOUFLAGE_ABILITY]->get_nation_tech_level(nation_recno)) )
          {
                  plant_id = plant_res.scan(0,
     terrain_res[l->terrain_id]->average_type, 0); is_ai = 0;
          }*/
...
void FirmOffensive2::process_monster_firm() {
  ...
  if (firm_attack_animation_delay == ATTACK_ANIMATION_DELAY - 6 &&
      ...
      for (xLoc = xLoc1; xLoc <= xLoc2; xLoc++, locPtr++) {
        ...
        //--- if there is an enemy firm here ---//
          ...
          if (firmPtr->nation_recno != nation_recno &&
              firmPtr->firm_id != FIRM_OFFENSIVE_BUILDING_4) {
...
float FirmOffensive2::bullet_damage() { return OFFENSIVE2_BULLET_DAMAGE; }
...
char FirmOffensive2::bullet_fire() { return 2; }
...
short FirmOffensive2::bullet_speed() { return OFFENSIVE2_BULLET_SPEED; }
//------ end of function FirmOffensive2::bullet_speed -------//

//------ begin of function FirmOffensive2::bullet_init_z -------//

short FirmOffensive2::bullet_init_z() { return altitude; }
...
int FirmOffensive2::find_best_place_loc(short buildFirmId, short refXLoc,
...
int FirmOffensive2::create_firm_next_to_place(Place *placePtr, int firmId,
...
void FirmOffensive2::right_click(int xLoc, int yLoc) {
  ...
  if (base_obj->cast_to_Unit() || base_obj->cast_to_Firm() ||
    ...
    if (!remote.is_enable())
      ...
      short *shortPtr = (short *)remote.new_send_queue_msg(MSG_F_OFF2_ATTACK,
                                                           2 * sizeof(short));
