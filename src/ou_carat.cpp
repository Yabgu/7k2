...
static char
    processed_raw_qty_array[MAX_RAW]; // 1 for not unload but can up load, 2 for
                                      // unload but not up load
...
void UnitCaravan::market_unload_goods() {
  FirmMarket *curMarket =
      (FirmMarket *)firm_array[stop_array[dest_stop_id - 1].firm_recno];
  ...
  for (i = 0; i < MAX_MARKET_GOODS; i++, marketGoods++) {
    if (marketGoods->raw_id) {
      ...
      // marketGoods->stock_qty<curMarket->max_stock_qty) || // no supply and
      // stock isn't full 	 (marketGoods->stock_qty<CARAVAN_UNLOAD_TO_MARKET_QTY &&
      //	 //##### begin trevor 16/7 #######//
      //	  marketGoods->month_demand > marketGoods->supply_30days()) ) //
      //demand > supply
      //	 //##### end trevor 16/7 #######//
      if (marketGoods->stock_qty < curMarket->max_stock_qty) {
        ...
        if (raw_qty_array[goodsId]) // have this goods
        {
          //---------- process unload -------------//
          unloadQty =
              (short)MIN(raw_qty_array[goodsId],
                         curMarket->max_stock_qty - marketGoods->stock_qty);
          ...
          withEmptySlot++;
          // processed_raw_qty_array[goodsId] = 0; // reset to zero for handling
          // empty slot
        }
      ...
      // stock isn't full
      //	 //##### begin trevor 16/7 #######//
      //	 (marketGoods->stock_qty<50 && marketGoods->month_demand >
      //marketGoods->supply_30days()) ) // demand > supply
      //	 //##### end trevor 16/7 #######//
      if (marketGoods->stock_qty < curMarket->max_stock_qty) {
        if (product_raw_qty_array[goodsId]) // have this goods
        {
          unloadQty =
              (short)MIN(product_raw_qty_array[goodsId],
                         curMarket->max_stock_qty - marketGoods->stock_qty);
          product_raw_qty_array[goodsId] -= unloadQty;
          err_when(product_raw_qty_array[goodsId] < 0);
          marketGoods->stock_qty += unloadQty;
          processed_product_raw_qty_array[goodsId] += 2;
        } else if (!marketGoods->stock_qty &&
                   !marketGoods->supply_30days()) // no supply, no stock,
                                                  // without this goods
        {
          withEmptySlot++;
          // processed_product_raw_qty_array[goodsId] = 0; // reset to zero for
          // handling empty slot
        }
...
// different slot
//
int UnitCaravan::market_unload_goods_in_empty_slot(FirmMarket *curMarket,
                                                   int position) {
  ...
  for (processed = 0, j = 0; j < MAX_PRODUCT; j++) {
    ...
    for (int debugCount = 0; debugCount < MAX_MARKET_GOODS;
         ++debugCount, debugGoods++)
    ...
    marketGoods->stock_qty =
        (float)0; // BUGHERE, there is a case that marketGoods->stock_qty > 0
    //-**************************************************-//
    processed_product_raw_qty_array[j] += 2;
    curMarket->set_goods(0, j + 1, position);

    short unloadQty =
        (short)MIN(product_raw_qty_array[j],
                   curMarket->max_stock_qty - marketGoods->stock_qty);
  ...
  if (!processed) {
    for (j = 0; j < MAX_PRODUCT; j++) {
      ...
      short unloadQty = (short)MIN(
          raw_qty_array[j], curMarket->max_stock_qty - marketGoods->stock_qty);
...
void UnitCaravan::market_load_goods() {
  ...
  for (int i = 0; i < MAX_MARKET_GOODS; i++, marketGoods++) {
    if (marketGoods->raw_id) {
      if (stopPtr->pick_up_array[marketGoods->raw_id + PICK_UP_RAW_FIRST - 2])
        market_load_goods_now(marketGoods, marketGoods->stock_qty);
    } else if (marketGoods->product_raw_id) {
      if (stopPtr->pick_up_array[marketGoods->product_raw_id +
                                 PICK_UP_PRODUCT_FIRST - 2])
        market_load_goods_now(marketGoods, marketGoods->stock_qty);
    }
  }
}
...
void UnitCaravan::market_auto_load_goods() {
  ...
  MarketGoods *marketGoods = curMarket->market_goods_array;
  // int	isOurMarket = (curMarket->nation_recno==nation_recno); // is 1 or
  // 0
  int goodsId;
  short loadQty;
  ...
  // default value
  //----------------------------------------------------------------------//
  // short minFirmStockQty = (int)curMarket->max_stock_qty/5; // keep at least
  // 20% capacity in the firm if the market is not for sale
  ...
  for (int i = 0; i < MAX_MARKET_GOODS; i++, marketGoods++) {
    ...
    // else if(marketGoods->product_raw_id && isOurMarket) // only load product
    // in our market
    else if (marketGoods->product_raw_id) {
      err_when(marketGoods->raw_id);
...
//
void UnitCaravan::market_load_goods_now(MarketGoods *marketGoods,
                                        float loadQty) {
  Nation *nationPtr = nation_array[nation_recno];
  int marketNationRecno =
      firm_array[stop_array[dest_stop_id - 1].firm_recno]->nation_recno;
  short qty;
  int goodsId;
  ...
  if (marketGoods->product_raw_id) {
    ...
    qty = MIN(MAX_CARAVAN_CARRY_QTY - product_raw_qty_array[goodsId],
              (int)loadQty);
    if (marketNationRecno !=
        nation_recno) // calculate the qty again if this is not our own market
    {
      qty = (nationPtr->cash > 0)
                ? (short)MIN(nationPtr->cash / PRODUCT_PRICE, qty)
                : 0;

      if (qty)
        nationPtr->import_goods(IMPORT_PRODUCT, marketNationRecno,
                                (float)qty * PRODUCT_PRICE);
    }

    product_raw_qty_array[goodsId] += qty;
    err_when(product_raw_qty_array[goodsId] < 0 ||
             product_raw_qty_array[goodsId] > MAX_CARAVAN_CARRY_QTY);
    ...
    if (marketNationRecno !=
        nation_recno) // calculate the qty again if this is not our own market
    {
      qty = (nationPtr->cash > 0) ? (short)MIN(nationPtr->cash / RAW_PRICE, qty)
                                  : 0;

      if (qty)
        nationPtr->import_goods(IMPORT_RAW, marketNationRecno,
                                (float)qty * RAW_PRICE);
    }

    raw_qty_array[goodsId] += qty;
    err_when(raw_qty_array[goodsId] < 0 ||
             raw_qty_array[goodsId] > MAX_CARAVAN_CARRY_QTY);
    marketGoods->stock_qty -= qty;
  }
...
void UnitCaravan::mine_load_goods(char pickUpType) {
  ...
  err_when(pickUpType >= PICK_UP_PRODUCT_FIRST &&
  ...
  if (pickUpType == AUTO_PICK_UP ||
      curMine->raw_id == searchRawId) // auto_pick_up or is the raw to pick up
  {
    int goodsId = curMine->raw_id - 1;
    short maxLoadQty =
        (pickUpType != AUTO_PICK_UP)
            ? (short)curMine->stock_qty
            : MAX(0, (int)(curMine->stock_qty -
                           MIN_FIRM_STOCK_QTY)); // max Qty mine can supply
    short qty = MIN(MAX_CARAVAN_CARRY_QTY - raw_qty_array[goodsId],
                    maxLoadQty); // max Qty caravan can carry
...
void UnitCaravan::factory_unload_goods() {
  ...
  //--- if the factory does not have any stock and there is no production, set
  //it to type of raw materials the caravan is carring ---//

  if (curFactory->stock_qty == 0 && curFactory->raw_stock_qty == 0 &&
  ...
  {
    short qty =
        MIN(raw_qty_array[goodsId],
            (short)(curFactory->max_raw_stock_qty - curFactory->raw_stock_qty));
    raw_qty_array[goodsId] -= qty;
    err_when(raw_qty_array[goodsId] < 0);
...
void UnitCaravan::factory_load_goods(char pickUpType) {
  ...
  if (pickUpType == AUTO_PICK_UP ||
      curFactory->product_raw_id ==
          searchProductRawId) // auto_pick_up or is the product to pick up
  {
    int goodsId = curFactory->product_raw_id - 1;
    short maxLoadQty =
        (pickUpType != AUTO_PICK_UP)
            ? (short)curFactory->stock_qty
            : MAX(0, (int)(curFactory->stock_qty -
                           MIN_FIRM_STOCK_QTY)); // max Qty factory can supply
    short qty = MIN(MAX_CARAVAN_CARRY_QTY - product_raw_qty_array[goodsId],
                    maxLoadQty); // max Qty caravan can carry
