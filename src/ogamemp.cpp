...
struct MpStructNation : public MpStructBase
{
    ...
    MpStructNation(short n, DWORD playerId, short scheme, short race, char *playerName, short useGemStones, WORD rank)
        : MpStructBase(MPMSG_DECLARE_NATION), nation_recno(n), dp_player_id(playerId), color_scheme(scheme),
          race_id(race), use_gem_stones(useGemStones), ranking(rank)
    {
        strcpy(player_name, playerName);
    }
    void init(short n, DWORD playerId, short scheme, short race, char *playerName, short useGemStones, WORD rank)
    {
...
struct MpStructNewPlayer : public MpStructBase
{
    ...
    MpStructNewPlayer(short bal, char *name, char *pass)
        : MpStructBase(MPMSG_NEW_PLAYER), player_balance(bal), game_version(GAME_VERSION),
          debug_version(debug_version_flag()), ranking(player_profile.ranking)
    {
...
struct MpStructAcceptNewPlayer : public MpStructBase
{
    ...
    ENetAddress address;
    char make_contact;
    MpStructAcceptNewPlayer(PID_TYPE p, char *name, ENetAddress *address, char contact)
        : MpStructBase(MPMSG_ACCEPT_NEW_PLAYER), player_id(p), make_contact(contact)
    {
...
struct MpStructRefuseNewPlayer : public MpStructBase
{
    ...
    MpStructRefuseNewPlayer(PID_TYPE p, int reasonId)
        : MpStructBase(MPMSG_REFUSE_NEW_PLAYER), player_id(p), reason_id(reasonId)
    {
    }
...
struct MpStructAcceptColor : public MpStructBase
{
    ...
    MpStructAcceptColor(PID_TYPE p, short c)
        : MpStructBase(MPMSG_ACCEPT_COLOR), request_player_id(p), color_scheme_id(c)
    {
    }
};
...
struct MpStructRefuseColor : public MpStructBase
{
    ...
    MpStructRefuseColor(PID_TYPE p, short c)
        : MpStructBase(MPMSG_REFUSE_COLOR), request_player_id(p), color_scheme_id(c)
    {
    }
};
...
struct MpStructSetGemStones : public MpStructBase
{
    ...
    MpStructSetGemStones(PID_TYPE p, short useGemStones, long totalGemStones)
        : MpStructBase(MPMSG_SET_GEM_STONE), player_id(p), use_gem_stones(useGemStones),
...
struct MpStructLoadGameNewPlayer : public MpStructBase
{
    ...
    MpStructLoadGameNewPlayer(Nation *n, DWORD frame, long seed, short bal, char *name, char *pass)
        : MpStructBase(MPMSG_LOAD_GAME_NEW_PLAYER), game_version(GAME_VERSION), debug_version(debug_version_flag()),
          nation_recno(n->nation_recno), color_scheme_id(n->color_scheme_id), race_id(n->race_id), frame_count(frame),
          random_seed(seed), player_balance(bal)
    {
        strncpy(this->name, name, MP_FRIENDLY_NAME_LEN);
        strncpy(this->pass, pass, MP_FRIENDLY_NAME_LEN);
    }
...
struct MpStructLatencySend : public MpStructBase
{
    ...
    MpStructLatencySend(int testId, DWORD sendTime)
        : MpStructBase(MPMSG_TEST_LATENCY_SEND), test_id(testId), send_time(sendTime)
    {
    }
};
...
struct MpStructProcessFrameDelay : public MpStructBase
{
    ...
    MpStructProcessFrameDelay(int newFrameDelay)
        : MpStructBase(MPMSG_SET_PROCESS_FRAME_DELAY), common_process_frame_delay(newFrameDelay)
    {
    }
};
...
struct MpStructPlayerDisconnect : public MpStructBase
{
    ...
    MpStructPlayerDisconnect(PID_TYPE lost_player) : MpStructBase(MPMSG_PLAYER_DISCONNECT), lost_player_id(lost_player)
    {
    }
};
...
void Game::load_mp_game(char *fileName, int lobbied, char *game_host)
{
    ...
    remote.init_start_mp();
    remote.init_send_queue(sys.frame_count,
...
#define SVOPTION_ALL 0x7fffffff
    ...
    {
        serviceButtonGroup[b].create(SCROLL_SHEET_X1 + 20, sy1 + b * ySpacing, SCROLL_SHEET_X2 - 20,
                                     sy1 + b * ySpacing + sh - 1, disp_service_button, ButtonCustomPara(NULL, b + 1),
                                     0);
    }
        ...
        while (1)
        {
            ...
            if (refreshFlag)
            {
                if (refreshFlag & SVOPTION_PAGE)
                {
                    ...
                    font_bold_black.center_put(SCROLL_SHEET_X1, SCROLL_SHEET_Y1, SCROLL_SHEET_X2, SCROLL_SHEET_Y1 + 20,
                                               text_game_menu.str_mp_select_service()); //"Select Connection Method") ;
...
#define SMOPTION_ALL 0x7fffffff
    ...
    getName.init(SCROLL_SHEET_X1 + 30, SCROLL_SHEET_Y1 + 120, SCROLL_SHEET_X2 - 30, config.player_name,
                 Config::PLAYER_NAME_LEN, &font_san, 0, 1);
    getSaveFile.init(SCROLL_SHEET_X1 + 30, SCROLL_SHEET_Y1 + 220, SCROLL_SHEET_X2 - 30, saveFileName, 8, &font_san, 0,
                     1);
    ...
    while (1)
    {
        ...
        if (refreshFlag)
        {
            if (refreshFlag & SMOPTION_PAGE)
            {
                ...
                font_bold_black.put(SCROLL_SHEET_X1 + 30, SCROLL_SHEET_Y1 + 100,
                                    text_game_menu.str_mp_your_name()); // "Your Name" );
                font_bold_black.put(SCROLL_SHEET_X1 + 30, SCROLL_SHEET_Y1 + 200,
                ...
                if (mp_obj.is_lobbied() != 1) // 1 = create game
                    font_thin_black.center_put(BUTTON3_X1, BUTTON3_Y1, BUTTON3_X2, BUTTON3_Y2,
        ...
        if (rc)
        {
            ...
            if (misc.str_icmpx(getSaveFile.input_field, "AUTO") || misc.str_icmpx(getSaveFile.input_field, "AUTO2"))
            {
                if (!box.ask(text_game_menu.str_change_auto_save())) // "It is not recommended to use
...
void mp_info_box_show(InfoBox *info, const char *tell_string, const char *button_text)
{
    ...
    info->button.create_text(box.box_x1 + (box.box_x2 - box.box_x1 + 1) / 2 - 10, box.box_y2 - box_button_margin,
                             button_text);
    info->button.paint();
    info->visible = 1;
}
...
int mp_info_box_detect(InfoBox *info)
{
    ...
    if (info->button.detect(info->button.str_buf[0], KEY_RETURN) || info->button.detect(KEY_ESC) || mouse.any_click(1))
    {
        mouse.get_event();
        info->visible = 0;
        box.close();
...
int Game::input_box(const char *tell_string, char *buf, int len)
{
    ...
    font_san.put_paragraph(box_x1 + box_side_margin, box_y1 + box_top_margin, box_x2 - box_side_margin,
                           box_y2 - box_top_margin, tell_string, 2);
    ...
    input_box.init(box_x1 + box_side_margin, box_y1 + box_top_margin + font_san.text_height() + 2,
...
#define SSOPTION_ALL 0x7fffffff
    ...
    while (1)
    {
        ...
        int bitmapHeight = ((Bitmap *)bitmapPtr)->get_height();
        if (page > 0 &&
            mouse.any_click((LSCROLL_X2 + LSCROLL_X1 - bitmapWidth) / 2, (LSCROLL_Y2 + LSCROLL_Y1 - bitmapHeight) / 2,
                            (LSCROLL_X2 + LSCROLL_X1 + bitmapWidth) / 2, (LSCROLL_Y2 + LSCROLL_Y1 + bitmapHeight) / 2))
        {
            --page;
        ...
        if (page < maxPage - 1 &&
            mouse.any_click((RSCROLL_X2 + RSCROLL_X1 - bitmapWidth) / 2, (RSCROLL_Y2 + RSCROLL_Y1 - bitmapHeight) / 2,
                            (RSCROLL_X2 + RSCROLL_X1 + bitmapWidth) / 2, (RSCROLL_Y2 + RSCROLL_Y1 + bitmapHeight) / 2))
        {
            ++page;
            refreshFlag |= SSOPTION_DISP_SESSION | SSOPTION_SCROLL_BAR;
        }
...
int Game::mp_join_session(int session_id, char *player_name)
{
    ...
    password[0] = 0;
    if (session->password[0] && !input_box("Enter the game's password:", password, MP_FRIENDLY_NAME_LEN + 1))
    {
        return 0;
    }
    ...
    buttonCancel.create_text(box.box_x1 + width / 2 + 2, box.box_y2 - box_button_margin, (char *)"Cancel");
...
int Game::mp_select_option(NewNationPara *nationPara, int *mpPlayerCount)
{
    ...
    else
    {
        ...
        MpStructNewPlayer msgNewPlayer(PLAYER_RATIO_CDROM, config.player_name, current_session->password);
    ...
    GetA messageField;
    messageField.init(126 + 8, 528 + 4, 663 - 4, typingMsg.content, MpStructChatMsg::MSG_LENGTH, &font_san, 0, 1);
    ...
    // SXM -30
#if (defined(CHINESE))
    speciesGroup[0].create(cx - w / 2 - 30, cy - h / 2 - 5, cx + w / 2 + 10, cy + h / 2 + 5, i_disp_text_button,
                           ButtonCustomPara(text_game_menu.str_species_humans(), 0), 0);
#else
    speciesGroup[0].create(cx - w / 2 - 10, cy - h / 2 - 5, cx + w / 2 + 10, cy + h / 2 + 5, i_disp_text_button,
                           ButtonCustomPara(text_game_menu.str_species_humans(), 0), 0);
#endif
    // SXM
    ...
    cy = 116;
#if (defined(CHINESE))
    speciesGroup[1].create(cx - w / 2 - 10 - 30, cy - h / 2 - 5, cx + w / 2 + 10, cy + h / 2 + 5, i_disp_text_button,
                           ButtonCustomPara(text_game_menu.str_species_fryhtans(), 1), 0);
#else
    speciesGroup[1].create(cx - w / 2 - 10, cy - h / 2 - 5, cx + w / 2 + 10, cy + h / 2 + 5, i_disp_text_button,
                           ButtonCustomPara(text_game_menu.str_species_fryhtans(), 1), 0);
#endif
    ...
    for (i = 0; i < 3; ++i)
    {
        ...
        h = font_thin_black.text_height();
        frameDelayGroup[i].create(cx, cy, cx + w + 10, cy + 20 - 1, i_disp_text_button,
    ...
    {
        gemStoneGroup[i].create(cx, cy, cx + 22, cy + 20 - 1, i_disp_int_button, ButtonCustomPara(NULL, i), 0);
        if (i > 0 && player_profile.gem_stones < i || i == 0 && player_profile.gem_stones > 0)
    ...
    playerNameField.init(364, 89, 688, tempConfig.player_name, tempConfig.PLAYER_NAME_LEN, &font_bold_black, 0, 1);
    ...
    for (i = 0; i < MAX_RACE; ++i, (cy += raceGroupYSpacing))
    {
        ...
        h = raceGroupYSpacing;
        raceGroup[i].create(cx - w / 2 - 5, cy - h / 2, cx + w / 2 + 5, cy + h / 2, i_disp_race_button,
    ...
    for (i = 0; i < MAX_MONSTER_TYPE; ++i, (cy += monsterGroupYSpacing)) // MAX_MONSTER
    {
        ...
        monsterRaceGroup[i].create(cx - w / 2 - 5, cy - h / 2, cx + w / 2 + 5, cy + h / 2, i_disp_race_button,
                                   ButtonCustomPara(NULL, -(i + 1)), 0);
    }
    ...
    {
        colorGroup[i].create(cx, cy, cx + colorButtonWidth - 1, cy + colorButtonHeight - 1, i_disp_color_button,
                             ButtonCustomPara(NULL, i + 1), 0);
    }
    ...
    {
        aiHumanNationGroup[i].create(cx - 10, cy - 10, cx + 10, cy + 10, i_disp_int_button, ButtonCustomPara(NULL, i),
                                     0);
    }
    ...
    for (i = 0; i < MAX_NATION; ++i, (cx += nationButtonSpacing))
    {
        aiMonsterNationGroup[i].create(cx - 10, cy - 10, cx + 10, cy + 10, i_disp_int_button, ButtonCustomPara(NULL, i),
                                       0);
    }
    ...
    for (i = 0; i < OPTION_HARDEST - OPTION_EASIEST + 1; ++i)
    {
        ...
        diffGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                            ButtonCustomPara(diffButtonStr[i], i + OPTION_EASIEST), 0);
        cx += w + 12;
    }
    ...
    h = font_thin_black.text_height();
    diffGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                        ButtonCustomPara(text_game_menu.str_difficulty_custom(), OPTION_CUSTOM), 0);
    cx += w + 12;
    ...
    for (i = 0; i < terrainGroup.button_num; ++i)
    {
        ...
        h = font_thin_black.text_height();
        terrainGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
    ...
    for (i = 0; i < buildingSizeGroup.button_num; ++i)
    {
        ...
        h = font_thin_black.text_height();
        buildingSizeGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
    ...
    exploreGroup[0].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                           ButtonCustomPara(text_game_menu.str_world_map(0), 0), 0);
    cx += w + 20;
    ...
    exploreGroup[1].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                           ButtonCustomPara(text_game_menu.str_world_map(1), 1), 0);
    ...
    for (i = 0; i < 4; ++i)
    {
        ...
        h = font_thin_black.text_height();
        treasureGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
    ...
    for (i = 0; i < 4; ++i)
    {
        w = font_thin_black.text_width(text_game_menu.str_treasure(i + 1));
        h = font_thin_black.text_height();
        aiTreasureGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                                  ButtonCustomPara(text_game_menu.str_treasure(i + 1), i + 1), 0);
        cx += w + 20;
    }
    ...
    for (i = 0; i < 5; ++i)
    {
        ...
        h = font_thin_black.text_height();
        aiAggressiveGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
    ...
    for (i = 0; i < 2; ++i)
    {
        ...
        h = font_thin_black.text_height();
        spyMethodGroup[i].create(cx, cy, cx + w + 10, cy + h + 5, i_disp_text_button,
    ...
    for (i = 0; i < 2; ++i)
    {
        ...
        h = font_thin_black.text_height();
        randomStartUpGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
    ...
    for (i = 0; i < 7; ++i)
    {
        rawSiteGroup[i].create(cx, cy, cx + 15, cy + font_thin_black.height() + 10, i_disp_int_button,
                               ButtonCustomPara(NULL, i + 1), 0);
        cx += 25;
    }
    ...
    for (i = 0; i < 3; ++i)
    {
        ...
        h = font_thin_black.text_height();
        townStartGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                                 ButtonCustomPara(text_game_menu.str_few_to_many(i + 1), startTownArray[i]), 0);
        cx += w + 20;
    }
    ...
    for (i = 0; i < 3; ++i)
    {
        w = font_thin_black.text_width(text_game_menu.str_low_to_high(i + 1));
        h = font_thin_black.text_height();
        townResistGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
    ...
    for (i = 0; i < 2; ++i)
    {
        w = font_thin_black.text_width(text_game_menu.str_no_yes(i));
        h = font_thin_black.text_height();
        townEmergeGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                                  ButtonCustomPara(text_game_menu.str_no_yes(i), i), 0);
        cx += w + 20;
    }
    ...
    for (i = 0; i < 2; ++i)
    {
        w = font_thin_black.text_width(text_game_menu.str_no_yes(i));
        h = font_thin_black.text_height();
        nationEmergeGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                                    ButtonCustomPara(text_game_menu.str_no_yes(i), i), 0);
        cx += w + 20;
    }
    ...
    for (i = 0; i < 4; ++i)
    {
        ...
        h = font_thin_black.text_height();
        randomEventGroup[i].create(cx, cy, cx + w + 10, cy + h + 6, i_disp_text_button,
    ...
    // ###### begin Gilbert 11/3 #########//
    ButtonCustom clearEnemyButton, clearMonsterButton, allyWinButton, enoughPeopleButton, enoughIncomeButton,
        enoughScoreButton, timeLimitButton;
    ...
    clearEnemyButton.create(option5X, 145, option5X + w - 1, 145 + h - 1, i_disp_tick_button, ButtonCustomPara(NULL, 0),
                            0, 1);
    clearEnemyButton.enable_flag = 0;
    clearMonsterButton.create(option5X, 178, option5X + w - 1, 178 + h - 1, i_disp_tick_button,
                              ButtonCustomPara(NULL, 0), 0, tempConfig.goal_destroy_monster);
    allyWinButton.create(option5X, 211, option5X + w - 1, 211 + h - 1, i_disp_tick_button, ButtonCustomPara(NULL, 0), 0,
                         tempConfig.goal_alliance_win_flag);
    enoughPeopleButton.create(option5X, 244, option5X + w - 1, 244 + h - 1, i_disp_tick_button,
                              ButtonCustomPara(NULL, 0), 0, tempConfig.goal_population_flag);
    enoughIncomeButton.create(option5X, 277, option5X + w - 1, 277 + h - 1, i_disp_tick_button,
                              ButtonCustomPara(NULL, 0), 0, tempConfig.goal_economic_score_flag);
    enoughScoreButton.create(option5X, 310, option5X + w - 1, 310 + h - 1, i_disp_tick_button,
                             ButtonCustomPara(NULL, 0), 0, tempConfig.goal_total_score_flag);
    timeLimitButton.create(option5X, 343, option5X + w - 1, 343 + h - 1, i_disp_tick_button, ButtonCustomPara(NULL, 0),
                           0, tempConfig.goal_year_limit_flag);
        ...
        while (1)
        {
            ...
            // ######### end Gilbert 14/5 ########//
            {
                if (optionMode == OPTION_SPECIES)
                {
                    ...
                    {
                        // "Gem Stones (xxx)" );
                        font_bold_black.put(126, 393, text_game_menu.str_mp_gem_stones(player_profile.gem_stones), 1,
                                            360 - 10);
                        // 15% per each gemstones
                        font_small.put(126, 413, text_game_menu.str_mp_gem_stones_inst(15 * selfUseGemStones), 1,
                                       360 - 10);
                ...
                if (optionMode == OPTION_BASIC)
                {
                    if (refreshFlag & SGOPTION_PAGE)
                    {
                        ...
                        if (speciesGroup() == 0)
                            font_bold_black.center_put(116, 126, 303, 146,
                        ...
                        font_bold_black.center_put(297, 225 - 20, 698, 243 - 20,
                ...
                if (optionMode == OPTION_GOAL)
                {
                    if (refreshFlag & SGOPTION_PAGE)
                    {
                        ...
                        font_bold_black.put(option5X, 112,
                                            text_game_menu.str_goal()); // You will be victorious when you have:
                        ...
                        font_thin_black.put_paragraph(
                            ...
                            text_game_menu.str_defeat_fryhtan_lairs()); // "Destroyed All Independent
                                                                        // Fryhtan Lairs" );
                        font_thin_black.put_paragraph(
                            option5X + tickWidth + 10, 211, option5X3 + 25, 244 - 1,
                            text_game_menu.str_defeat_except_ally()); // "Defeated All except one
                                                                      // Allied Kingdom" );
                        font_thin_black.put_paragraph(
                    ...
                    if (refreshFlag & SGOPTION_ENOUGH_PEOPLE)
                    {
                        ...
                        font_bold_black.put(peopleInc.x1 - 65, peopleInc.y1, misc.format(tempConfig.goal_population), 1,
                                            peopleInc.x1 - 1);
                        peopleInc.paint();
                        peopleDec.paint();
                    }
                    ...
                    if (refreshFlag & SGOPTION_ENOUGH_SCORE)
                    {
                        ...
                        font_bold_black.put(scoreInc.x1 - 65, scoreInc.y1, misc.format(tempConfig.goal_total_score), 1,
                                            scoreInc.x1 - 1);
                        scoreInc.paint();
                        scoreDec.paint();
                    }
                    if (refreshFlag & SGOPTION_TIME_LIMIT)
                    {
                        ...
                        font_thin_black.put(yearInc.x1 - 60, yearInc.y1, text_game_menu.str_goal_time_units(),
                                            1); // "years"
                        yearInc.paint();
                        yearDec.paint();
                    }
                ...
                if (mRefreshFlag & MGOPTION_PLAYERS)
                {
                    ...
                    for (p = 0; p < regPlayerCount; ++p)
                    {
                        ...
                        {
                            vga.active_buf->bar(tickX[p], tickY[p], tickX[p] + nameTickWidth - 1,
                                                tickY[p] + nameTickHeight - 1, V_WHITE);
                            vga.active_buf->d3_panel_up(tickX[p], tickY[p], tickX[p] + nameTickWidth - 1,
                                                        tickY[p] + nameTickHeight - 1, 4, 0);
                        }
                        else
                        {
                            vga.active_buf->d3_panel_down(tickX[p], tickY[p], tickX[p] + nameTickWidth - 1,
                                                          tickY[p] + nameTickHeight - 1, 4, 0);
                        ...
                        // put color of player
                        vga.active_buf->bar(tickX[p] + nameTickWidth, tickY[p], tickX[p] + nameTickWidth * 2 - 1,
                        ...
                        vga.active_buf->d3_panel_up(tickX[p] + nameTickWidth, tickY[p],
                                                    tickX[p] + nameTickWidth * 2 - 1, tickY[p] + nameTickHeight - 1, 4,
                                                    0);
                        PlayerDesc *dispPlayer = mp_obj.search_player(regPlayerId[p]);
                        int x2 = font_bld.put(tickX[p] + nameTickWidth * 2 + 3, tickY[p] + 0,
                                              dispPlayer ? dispPlayer->friendly_name_str() : (char *)"?anonymous?", 0,
                                              tickX[p] + 260);
#ifdef MAX_GEM_STONES
                        if (totalGemStones[p])
                        {
                ...
                {
                    font_bld.center_put(40, 85, 82, 105, text_game_menu.str_level_in_score(),
                                        1); // "Level"
                    font_bold_black.center_put(
                        40, 105, 82, 125, misc.format(tempConfig.multi_player_difficulty(regPlayerCount - 1, 0)), 1);
                }
            ...
            if (recvPtr)
            {
                ...
                else
                {
                    ...
                    case MPMSG_SEND_CONFIG:
                        ...
                        {
                            mp_info_box_show(
                                &info_box, text_game_menu.str_mp_refuse_reason(MpStructRefuseNewPlayer::REFUSE_VERSION),
                                "Ok");
                            break;
                        }
                        ...
                        if (remote.is_host)
                        {
                            ...
                            else if (!newPlayerMsg->is_compatible())
                            {
                                ...
                                mp_obj.send(from, &msgRefuse, sizeof(msgRefuse));
                                     ...
                                     mp_obj.auth_player(from, ((MpStructNewPlayer *)recvPtr)->name,
                                                        ((MpStructNewPlayer *)recvPtr)->pass))
                            {
                                ...
                                MpStructAcceptNewPlayer msgAccept(from, newPlayerMsg->name,
                                ...
                                if (useGemStoneFlag)
                                {
                                    for (c = 0; c < regPlayerCount; ++c)
                                    {
                                        MpStructSetGemStones msgUseGemStones(regPlayerId[c], useGemStones[c],
                                                                             totalGemStones[c]);
                    ...
                    case MPMSG_ACCEPT_NEW_PLAYER:
                        hostPlayerId = from;
                        if (regPlayerCount < MAX_NATION &&
                            ((MpStructAcceptNewPlayer *)recvPtr)->player_id != mp_obj.get_my_player_id())
                        {
                            MpStructAcceptNewPlayer *newb = (MpStructAcceptNewPlayer *)recvPtr;
                            mp_obj.add_player(newb->player_id, newb->player_name, &newb->address, newb->make_contact);
                    ...
                    case MPMSG_ACQUIRE_COLOR:
                        if (remote.is_host)
                        {
                            ...
                            {
                                // reply refuse color
                                MpStructRefuseColor msgRefuseColor(from,
                                                                   ((MpStructAcquireColor *)recvPtr)->color_scheme_id);
                    ...
                    case MPMSG_ACCEPT_COLOR:
                        ...
                        { // other player change color
                            ...
                            for (p = 0; p < regPlayerCount &&
                                        regPlayerId[p] != ((MpStructAcceptColor *)recvPtr)->request_player_id;
                                 ++p)
                                ;
                    ...
                    case MPMSG_SET_GEM_STONE: {
                        ...
                        if (!remote.is_host && !useGemStoneFlag)
                        {
                            ...
                            MpStructSetGemStones msgSetGemStones(mp_obj.get_my_player_id(), selfUseGemStones,
                    ...
                    case MPMSG_SET_PROCESS_FRAME_DELAY: {
                        ...
                        {
                            if (((MpStructProcessFrameDelay *)recvPtr)->common_process_frame_delay >=
                    ...
                    case MPMSG_PLAYER_DISCONNECT:
                        if (!remote.is_host)
                        {
                            ...
                            for (q = 0; q < regPlayerCount &&
                                        ((MpStructPlayerDisconnect *)recvPtr)->lost_player_id != regPlayerId[q];
                                 ++q)
                                ;
                            if (q < regPlayerCount)
                            {
            ...
            if (optionMode == OPTION_SPECIES)
            {
                ...
                else if (mouse.single_click(SCROLL_SHEET_X1, SCROLL_SHEET_Y1, (SCROLL_SHEET_X1 + SCROLL_SHEET_X2) / 2,
                                            368 - 5))
                {
                ...
                else if (mouse.single_click((SCROLL_SHEET_X1 + SCROLL_SHEET_X2) / 2 + 1, SCROLL_SHEET_Y1,
                                            SCROLL_SHEET_X2, 368 - 5))
                {
...
#ifdef MAX_GEM_STONES
                else if (useGemStoneFlag && !selfReadyFlag && gemStoneGroup.detect() >= 0)
                {
            ...
            if (remote.is_host && !selfReadyFlag)
            {
                ...
                else if (optionMode == OPTION_ADVANCED)
                {
                    ...
                    {
                        tempConfig.ai_aggressiveness = aiAggressiveGroup[aiAggressiveGroup()].custom_para.value;
                        tempConfig.difficulty_level = OPTION_CUSTOM;
                        configChange = 1;
            ...
            else if (mouse.single_click(BUTTON9_X1, BUTTON9_Y1, BUTTON9_X2, BUTTON9_Y2))
            {
                if (box.ask(text_game_menu.str_mp_confirm_quit()))
                {
                    ...
                    break; // break while(1)
                     ...
                     (keyCode = messageField.detect()) != 0) // keyCode may be non-zero if after mapIdField.detect()
            {
    ...
    if (retFlag > 0)
    {
        ...
        if (remote.is_host)
        {
            ...
            for (p = 0; p < regPlayerCount; ++p)
            {
                ...
                nationPara[playerCount].init(playerCount + 1, playerId, playerColor[p], playerRace[p],
                ...
                ((MpStructNation *)setupString.reserve(sizeof(MpStructNation)))
                    ->init(playerCount + 1, playerId, playerColor[p], playerRace[p], player->friendly_name_str(),
        ...
        else
        {
            ...
            while (offset < recvLen)
            {
                ...
                case MPMSG_DECLARE_NATION: {
                    ...
                    nationPara[playerCount].init(msgNation->nation_recno, msgNation->dp_player_id,
                                                 msgNation->color_scheme, msgNation->race_id, msgNation->player_name,
                ...
                {
                    err_here();
                    box.msg(text_game_menu.str_mp_connect_str_corrupt()); // "Connection string from
                                                                          // host is corrupted" );
                    return -1;
                }
            } // end while
            ...
            if (!recvConfig)
            {
                box.msg(text_game_menu.str_mp_config_lost()); // "Cannot get game configuration
                                                              // info from the host." );
            ...
            if (playerCount == 0)
            {
                box.msg(text_game_menu.str_mp_nation_info_lost()); //"Cannot get kingdom information
                                                                   // from the host." );
                return -1;
            }
            ...
            if (!ownPlayerFound)
            {
                box.msg(text_game_menu.str_mp_host_not_recognize()); // "The host cannot recognize
                                                                     // your machine." );
                return -1;
            }
            ...
            {
                box.msg(text_game_menu.str_mp_miss_connect_info()); //"Cannot receive important
...

        {
            ...
            if (recvEndSetting < playerCount - 1)
            {
                box.msg(text_game_menu.str_mp_other_player_fail()); // "Some player(s)
                                                                    // encountered errors
...
int Game::mp_select_load_option(char *fileName)
{
    ...
    else
    {
        ...
        MpStructLoadGameNewPlayer msgNewPlayer(~nation_array, sys.frame_count, misc.get_random_seed(),
            ...
            // ######### end Gilbert 14/5 ########//
            {
                if (optionMode == OPTION_SPECIES)
                {
                    if (refreshFlag & SGOPTION_PAGE)
                    {
                        ...
                        {
                            // "Gem Stones (xxx)" );
                            font_bold_black.put(126, 393, text_game_menu.str_mp_gem_stones(player_profile.gem_stones),
                                                0, 360 - 10);
                    ...
                    // use 					if( mRefreshFlag & MGOPTION_GEM_STONES
                    // && useGemStoneFlag
                    // )
                    //					{
                    //						font_bold_black.put_paragraph( 126,
                    // 408,
                    // 360-10, 458-1,
                ...
                if (optionMode != OPTION_SPECIES && refreshFlag & SGOPTION_DIFFICULTY)
                {
                    ...
                    // 1 );
                    if (tempConfig.race_id >= 0)
                        font_bold_black.center_put(40, 105, 82, 125, misc.format(tempConfig.human_difficulty_rating),
                                                   1);
                    else
                        font_bold_black.center_put(40, 105, 82, 125, misc.format(tempConfig.monster_difficulty_rating),
                                                   1);
                }
                ...
                else
                {
                    ...
                    case MPMSG_LOAD_GAME_NEW_PLAYER:
                        if (remote.is_host)
                        {
                            ...
                            else if (!(!nation_array.is_deleted(newPlayerMsg->nation_recno) &&
                                       nation_array[newPlayerMsg->nation_recno]->color_scheme_id ==
                                       ...
                                       newPlayerMsg->random_seed == misc.get_random_seed()))
                            {
                                ...
                                MpStructRefuseNewPlayer msgRefuse(from,
                                                                  MpStructRefuseNewPlayer::REFUSE_LOAD_GAME_UNSYNC);
                                mp_obj.send(from, &msgRefuse, sizeof(msgRefuse));
                            }
    ...
    if (retFlag > 0)
    {
        ...
        if (remote.is_host)
        {
            ...
            for (p = 0; p < regPlayerCount; ++p)
            {
                ...
                for (short nationRecno = 1; nationRecno <= nation_array.size(); ++nationRecno)
                {
                        ...
                        playerColor[p] == nation_array[nationRecno]->color_scheme_id)
                    {
                        ...
                        ((MpStructNation *)setupString.reserve(sizeof(MpStructNation)))
                            ->init(nationRecno, nationPtr->player_id, nationPtr->color_scheme_id, nationPtr->race_id,
            ...
            //				// ensure it is a valid player
            //				PlayerDesc *player =
            // mp_obj.get_player(d); 				PID_TYPE playerId =
            // player ? player->pid() : 0;
            //				if( !playerId || !player->connecting )
            //					continue;
            //				for( p = 0; p < regPlayerCount &&
            // regPlayerId[p] != playerId; ++p); 				if( p >=
            // regPlayerCount )
            //// not found 					continue;
            //
            //				// match nation color
            ...
            // player->friendly_name_str()); playerCount++;
            // break;
            //					}
            //				}
            //			}
        ...
        else
        {
            ...
            while (offset < recvLen)
            {
                ...
                case MPMSG_DECLARE_NATION: {
                    ...
                    {
                        box.msg(text_game_menu.str_mp_nation_info_incorrect()); // "Incorrect kingdom
...
static void disp_service_button(ButtonCustom *button, int)
{
    ...
    (button->pushed_flag ? font_bold_red : font_bold_black)
        .put(button->x1 + 16, button->y1 + 6, service_name_str, 1, button->x2 - 10);

    font_bld.put(button->x1 + 16, button->y1 + 26, service_name_str_long, 1, button->x2 - 10);
...
static void i_disp_color_button(ButtonCustom *button, int repaintBody)
{
    ...
    if (!button->pushed_flag)
    {
        ...
        vga.active_buf->put_bitmap_area_trans_remap_decompress(button->x1 - clipX1, button->y1 - clipY1,
                                                               image_button.read("F-COLOR"), clipX1, clipY1, clipX2,
                                                               clipY2, colorRemapTable);
    }
    else
    {
        vga.active_buf->put_bitmap_trans_remap_decompress(button->x1, button->y1, image_button.read("F-COL-DW"),
                                                          colorRemapTable);
    }
}
    ...
    if (*cmdStr == '#')
    {
        ...
        case 'r': {
            ...
            for (int i = 1; i <= nation_array.size(); ++i)
            {
                ...
                str = text_game_menu.str_mp_ranking(nation_array[i]->nation_name(), nation_array[i]->color_scheme_id,
