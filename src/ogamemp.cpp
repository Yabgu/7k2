...
// Description : Main Game Object - Multiplayer Game (using Imagic multiplayer
// SDK)
...
static char
    sub_game_mode; // 0 = new multiplayer game, 1 = load multiplayer game
...
static int is_compatible_version(int remoteVersion, int debugVersion) {
  ...
  //	for( int i = 0; i <
  //sizeof(compatbile_version)/sizeof(compatbile_version[0]); ++i )
  //	{
...
struct MpStructNewPlayer : public MpStructBase {
  ...
  WORD ranking;
  MpStructNewPlayer(short bal, char *name, char *pass)
      : MpStructBase(MPMSG_NEW_PLAYER), player_balance(bal),
        game_version(GAME_VERSION), debug_version(debug_version_flag()),
  ...
  int is_compatible() {
    return is_compatible_version(game_version, debug_version);
  }
};
...
struct MpStructAcceptNewPlayer : public MpStructBase {
  ...
  ENetAddress address;
  char make_contact;
  MpStructAcceptNewPlayer(PID_TYPE p, char *name, ENetAddress *address,
                          char contact)
      : MpStructBase(MPMSG_ACCEPT_NEW_PLAYER), player_id(p),
...
struct MpStructRefuseNewPlayer : public MpStructBase {
  ...
  PID_TYPE player_id;
  int reason_id;
  MpStructRefuseNewPlayer(PID_TYPE p, int reasonId)
      : MpStructBase(MPMSG_REFUSE_NEW_PLAYER), player_id(p),
        reason_id(reasonId) {}

  char *reason_str() { return text_game_menu.str_mp_refuse_reason(reason_id); }
};

struct MpStructAcquireColor : public MpStructBase {
  short color_scheme_id;
  MpStructAcquireColor(short c)
      : MpStructBase(MPMSG_ACQUIRE_COLOR), color_scheme_id(c) {}
};

struct MpStructAcceptColor : public MpStructBase {
  PID_TYPE request_player_id;
  short color_scheme_id;
  MpStructAcceptColor(PID_TYPE p, short c)
      : MpStructBase(MPMSG_ACCEPT_COLOR), request_player_id(p),
        color_scheme_id(c) {}
};
...
struct MpStructRefuseColor : public MpStructBase {
  PID_TYPE request_player_id;
  short color_scheme_id;
  MpStructRefuseColor(PID_TYPE p, short c)
      : MpStructBase(MPMSG_REFUSE_COLOR), request_player_id(p),
        color_scheme_id(c) {}
};

struct MpStructAcquireRace : public MpStructBase {
  short race_id;
  MpStructAcquireRace(short raceId)
      : MpStructBase(MPMSG_ACQUIRE_RACE), race_id(raceId) {}
};

struct MpStructAcceptRace : public MpStructBase {
  PID_TYPE request_player_id;
  short race_id;
  MpStructAcceptRace(PID_TYPE p, short raceId)
      : MpStructBase(MPMSG_ACCEPT_RACE), request_player_id(p), race_id(raceId) {
  }
};
...
struct MpStructRefuseRace : public MpStructBase {
  PID_TYPE request_player_id;
  short race_id;
  MpStructRefuseRace(PID_TYPE p, short raceId)
      : MpStructBase(MPMSG_REFUSE_RACE), request_player_id(p), race_id(raceId) {
  }
};

struct MpStructPlayerReady : public MpStructBase {
  PID_TYPE player_id;
  MpStructPlayerReady(PID_TYPE p)
      : MpStructBase(MPMSG_PLAYER_READY), player_id(p) {}
};

struct MpStructPlayerUnready : public MpStructBase {
  PID_TYPE player_id;
  MpStructPlayerUnready(PID_TYPE p)
      : MpStructBase(MPMSG_PLAYER_UNREADY), player_id(p) {}
};
...
struct MpStructSetGemStones : public MpStructBase {
  ...
  short use_gem_stones;
  MpStructSetGemStones(PID_TYPE p, short useGemStones, long totalGemStones)
      : MpStructBase(MPMSG_SET_GEM_STONE), player_id(p),
        use_gem_stones(useGemStones), total_gem_stones(totalGemStones) {}
};
...
struct MpStructLoadGameNewPlayer : public MpStructBase {
  ...
  MpStructLoadGameNewPlayer(Nation *n, DWORD frame, long seed, short bal,
                            char *name, char *pass)
      : MpStructBase(MPMSG_LOAD_GAME_NEW_PLAYER), game_version(GAME_VERSION),
        debug_version(debug_version_flag()), nation_recno(n->nation_recno),
        color_scheme_id(n->color_scheme_id), race_id(n->race_id),
        frame_count(frame), random_seed(seed), player_balance(bal) {
    strncpy(this->name, name, MP_FRIENDLY_NAME_LEN);
    strncpy(this->pass, pass, MP_FRIENDLY_NAME_LEN);
  }
...
struct MpStructChatMsg : public MpStructBase {
  ...
  MpStructChatMsg(char *fromName, const char *message)
      : MpStructBase(MPMSG_SEND_CHAT_MSG) {
    if (fromName)
...
struct MpStructSyncLevel : public MpStructBase {
  ...
  MpStructSyncLevel(char syncLevel)
      : MpStructBase(MPMSG_SEND_SYNC_TEST_LEVEL), sync_test_level(syncLevel) {}
};

struct MpStructLatencySend : public MpStructBase {
  int test_id;
  DWORD send_time;
  MpStructLatencySend(int testId, DWORD sendTime)
      : MpStructBase(MPMSG_TEST_LATENCY_SEND), test_id(testId),
        send_time(sendTime) {}
};

struct MpStructLatencyReturn : public MpStructLatencySend {
  MpStructLatencyReturn(const MpStructLatencySend &ls)
      : MpStructLatencySend(ls) {
...
struct MpStructProcessFrameDelay : public MpStructBase {
  ...
  MpStructProcessFrameDelay(int newFrameDelay)
      : MpStructBase(MPMSG_SET_PROCESS_FRAME_DELAY),
        common_process_frame_delay(newFrameDelay) {}
};

struct MpStructPlayerId : public MpStructBase {
  PID_TYPE your_id;
  MpStructPlayerId(PID_TYPE id) : MpStructBase(MPMSG_PLAYER_ID), your_id(id) {}
};

struct MpStructPlayerDisconnect : public MpStructBase {
  PID_TYPE lost_player_id;
  MpStructPlayerDisconnect(PID_TYPE lost_player)
      : MpStructBase(MPMSG_PLAYER_DISCONNECT), lost_player_id(lost_player) {}
};
...
void Game::multi_player_game(int lobbied, char *game_host) {
  ...
  if (!mp_obj.is_initialized()) {
    box.msg(text_game_menu
                .str_mp_error_dplay()); //"Cannot initialize DirectPlay.");
    mp_obj.deinit();
    return;
  }
  ...
  case 1: // create game
  {
    ...
    if (!input_box("Enter the name of the game:", game_name,
                   MP_FRIENDLY_NAME_LEN + 1)) {
      mp_obj.deinit();
      return;
    }
    password[0] = 0;
    if (!input_box("Set the game's password:", password,
                   MP_FRIENDLY_NAME_LEN + 1))
      password[0] = 0;
    if (!mp_obj.create_session(game_name, password, config.player_name,
                               MAX_NATION)) {
  ...
  // config game session ...
  NewNationPara *nationPara =
      (NewNationPara *)mem_add(sizeof(NewNationPara) * MAX_NATION);
  int mpPlayerCount = 0;
  int choice = 0;
  ...
  for (p = 0; p < mpPlayerCount; ++p) {
    if (nationPara[p].dp_player_id == mp_obj.get_my_player_id()) {
      // remote.init_send_queue(1, nation_array.player_recno);	// but
      // nation_array.player_recno is not set
      remote.init_send_queue(
          1, nationPara[p]
                 .nation_recno); // initialize the send queue for later sending
      break;
    }
  }
  ...
  if (mp_obj.is_lobbied()) {
    ...
    // generate a serial number, to guard against bonus of winning again using
    // load game
...
void Game::load_mp_game(char *fileName, int lobbied, char *game_host) {
  ...
  for (nationRecno = 1; nationRecno <= nation_array.size(); ++nationRecno) {
    if (!nation_array.is_deleted(nationRecno) &&
        (nation_array[nationRecno]->is_own() ||
         nation_array[nationRecno]->is_remote())) {
      ++gamePlayerCount;
    }
  }
  ...
  case 2: // join game
  {
    ...
    for (nationRecno = 1; nationRecno <= nation_array.size(); ++nationRecno)
      if (!nation_array.is_deleted(nationRecno) &&
          !nation_array[nationRecno]->is_ai())
        ++gamePlayerCount;
  ...
  remote.init_send_queue(
      sys.frame_count,
      nation_array.player_recno); // initialize the send queue for later sending
...
#define SVOPTION_ALL 0x7fffffff
  ...
  for (b = 0; b < buttonCount; ++b) {
    serviceButtonGroup[b].create(
        SCROLL_SHEET_X1 + 20, sy1 + b * ySpacing, SCROLL_SHEET_X2 - 20,
        sy1 + b * ySpacing + sh - 1, disp_service_button,
        ButtonCustomPara(NULL, b + 1), 0);
  }
    ...
    while (1) {
      ...
      if (refreshFlag) {
        if (refreshFlag & SVOPTION_PAGE) {
          ...
          font_bold_black.center_put(
              SCROLL_SHEET_X1, SCROLL_SHEET_Y1, SCROLL_SHEET_X2,
              SCROLL_SHEET_Y1 + 20,
              text_game_menu
                  .str_mp_select_service()); //"Select Connection Method") ;

          // -------- display service button --------//

          font_thin_black.center_put(
              BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2,
              text_game_menu.str_continue()); //"Continue" );
          font_thin_black.center_put(
      ...
      if (serviceButtonGroup() >= 0) {
        ...
        if (mouse.double_click(button->x1, button->y1, button->x2,
                               button->y2)) {
...
#define SMOPTION_ALL 0x7fffffff
  ...
  if (mp_obj.is_lobbied() && mp_obj.get_lobbied_name()) {
    // use name from lobby
    strncpy(config.player_name, mp_obj.get_lobbied_name(),
            Config::PLAYER_NAME_LEN);
  ...
  if (defSaveFileName) {
    ...
    if (newLen > 8)
      misc.str_cut(saveFileName, extractedFileName, 1,
                   8); // saveFileName has 8 char only plus string terminator
    else if (newLen > 1)
      misc.str_cut(saveFileName, extractedFileName, 1, newLen - 1);
    else
      err_here();
    if (misc.str_icmpx(saveFileName, "AUTO") ||
        misc.str_icmpx(saveFileName, "AUTO2")) {
  ...
  getName.init(SCROLL_SHEET_X1 + 30, SCROLL_SHEET_Y1 + 120,
               SCROLL_SHEET_X2 - 30, config.player_name,
               Config::PLAYER_NAME_LEN, &font_san, 0, 1);
  getSaveFile.init(SCROLL_SHEET_X1 + 30, SCROLL_SHEET_Y1 + 220,
                   SCROLL_SHEET_X2 - 30, saveFileName, 8, &font_san, 0, 1);
  ...
  while (1) {
    ...
    if (refreshFlag) {
      if (refreshFlag & SMOPTION_PAGE) {
        ...
        // font_bold_black.center_put( SCROLL_SHEET_X1, SCROLL_SHEET_Y1,
        // SCROLL_SHEET_X2, SCORLL_SHEET_Y1+20, "Enter your name" ;
        font_bold_black.put(
            SCROLL_SHEET_X1 + 30, SCROLL_SHEET_Y1 + 100,
            text_game_menu.str_mp_your_name()); // "Your Name" );
        font_bold_black.put(
            SCROLL_SHEET_X1 + 30, SCROLL_SHEET_Y1 + 200,
            text_game_menu.str_mp_save_file_name()); // "Save Game File Name" );
        ...
        if (mp_obj.is_lobbied() != 2) // 2 = join game
          font_thin_black.center_put(
              BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2,
              text_game_menu.str_create()); // "Create" );

        if (mp_obj.is_lobbied() != 1) // 1 = create game
          font_thin_black.center_put(BUTTON3_X1, BUTTON3_Y1, BUTTON3_X2,
                                     BUTTON3_Y2,
                                     text_game_menu.str_join()); //"Join" );

        font_thin_black.center_put(BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2,
                                   BUTTON4_Y2,
                                   text_game_menu.str_cancel()); // "Cancel" );
    ...
    else if (mp_obj.is_lobbied() != 2 &&
             mouse.single_click(BUTTON2_X1, BUTTON2_Y1, BUTTON2_X2,
                                BUTTON2_Y2)) {
      rc = 1;
    }
    ...
    else if (mp_obj.is_lobbied() != 1 &&
             mouse.single_click(BUTTON3_X1, BUTTON3_Y1, BUTTON3_X2,
                                BUTTON3_Y2)) {
      rc = 2;
    }
    ...
    else if (mouse.single_click(BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2,
                                BUTTON4_Y2)) {
      rc = 0;
      break;
    }
    ...
    if (rc) {
      ...
      if (misc.str_icmpx(getSaveFile.input_field, "AUTO") ||
        ...
        if (!box.ask(
                text_game_menu
                    .str_change_auto_save())) // "It is not recommended to use
                                              // this save game file name, do
    ...
    if (rc) {
      ...
      if ((newLen = misc.str_str(getSaveFile.input_field, ".")) > 0) {
        misc.str_cut(getSaveFile.input_field, getSaveFile.input_field, 1,
                     newLen - 1);
      }
...
void mp_info_box_show(InfoBox *info, const char *tell_string,
  ...
  info->button.create_text(box.box_x1 + (box.box_x2 - box.box_x1 + 1) / 2 - 10,
                           box.box_y2 - box_button_margin, button_text);
  info->button.paint();
  info->visible = 1;
}
...
int Game::input_box(const char *tell_string, char *buf, int len) {
  ...
  font_san.put_paragraph(box_x1 + box_side_margin, box_y1 + box_top_margin,
                         box_x2 - box_side_margin, box_y2 - box_top_margin,
                         tell_string, 2);
  ...
  input_box.init(box_x1 + box_side_margin,
                 ...
                 box_x2 - box_side_margin, buf, len, &font_san, 0, 0);
  ...
  while (1) {
    ...
    if (buttonCancel.detect(KEY_ESC) || mouse.any_click(1)) {
      mouse.get_event();
      break;
    }
...
#define SSOPTION_ALL 0x7fffffff
  ...
  while (1) {
    ...
    if (refreshFlag) {
      if (refreshFlag & SSOPTION_PAGE) {
        ...
        font_bold_black.center_put(
            SCROLL_SHEET_X1, SCROLL_SHEET_Y1, SCROLL_SHEET_X2,
            SCROLL_SHEET_Y1 + 20,
            text_game_menu.str_mp_select_session()); // "Select A Game" );

        // -------- display button -------//

        //				font_thin_black.center_put( BUTTON2_X1,
        //BUTTON2_Y1, BUTTON2_X2, BUTTON2_Y2, text_game_menu.str_join()); //
        //"Join" );
        font_thin_black.center_put(BUTTON4_X1, BUTTON4_Y1, BUTTON4_X2,
                                   BUTTON4_Y2,
                                   text_game_menu.str_cancel()); // "Cancel" );
      ...
      if (refreshFlag & SSOPTION_DISP_SESSION) {
        for (b = 0, s = page * SESSION_PER_PAGE + 1; b < SESSION_PER_PAGE;
             ++b, ++s) {
          int x1 = sx1;
          int x2 = sx2;
          ...
          if (mp_obj.get_session(s)) {
            ...
            font_bld.center_put(x1, y1, x2, y2,
                                mp_obj.get_session(s)->name_str());
      ...
      // display scroll left/right
      {
        ...
        if (page > 0) {
          ...
          vga.active_buf->put_bitmap_trans_decompress_hmirror(arrowX1, arrowY1,
                                                              bitmapPtr);
          mouse.show_area();
        } else {
        ...
        if (page < maxPage - 1) {
          mouse.hide_area(arrowX1, arrowY1, arrowX2, arrowY2);
          vga.active_buf->put_bitmap_trans_decompress(arrowX1, arrowY1,
                                                      bitmapPtr);
          mouse.show_area();
        } else {
          vga_util.blt_buf(arrowX1, arrowY1, arrowX2, arrowY2, 0);
    ...
    for (b = 0, s = page * SESSION_PER_PAGE + 1;
         b < SESSION_PER_PAGE && mp_obj.get_session(s); ++b, ++s) {
      int x1 = sx1;
      int x2 = sx2;
      int y1 = sy1 + ySpacing * b;
      int y2 = y1 + ySpacing - 4;
      ...
      if (mouse.double_click(x1, y1, x2, y2)) {
        ...
        // suspend the refreshTime, so session list won't update immediate after
        // release dragging
        refreshTime = misc.get_time();
    ...
    int bitmapHeight = ((Bitmap *)bitmapPtr)->get_height();
    if (page > 0 &&
        mouse.any_click((LSCROLL_X2 + LSCROLL_X1 - bitmapWidth) / 2,
    ...
    if (page < maxPage - 1 &&
        mouse.any_click((RSCROLL_X2 + RSCROLL_X1 - bitmapWidth) / 2,
    ...
    if (!(mouse.skey_state & SHIFT_KEY_MASK) &&
        misc.get_time() - refreshTime > pollTime)
...
int Game::mp_join_session(int session_id, char *player_name) {
  ...
  password[0] = 0;
  if (session->password[0] && !input_box("Enter the game's password:", password,
                                         MP_FRIENDLY_NAME_LEN + 1)) {
    return 0;
  }
  ...
  buttonCancel.create_text(box.box_x1 + width / 2 + 2,
                           box.box_y2 - box_button_margin, (char *)"Cancel");
...
int Game::mp_select_option(NewNationPara *nationPara, int *mpPlayerCount) {
  ...
  char backupOptionMode; // when switch to OPTION_CHAT_LOG, backup the current
                         // optionMode
  ...
  char raceAssignedBiased[MAX_MONSTER_TYPE + 1 +
                          MAX_RACE]; // master copy belongs to host's
  const int raceBias =
      MAX_MONSTER_TYPE; // access raceAssignedBiased[raceBias + raceId]
  char *raceAssigned_ = raceAssignedBiased +
                        MAX_MONSTER_TYPE; // change back to raceAssigned later
  ...
  int recvEndSetting =
      0; // number of MPMSG_END_SETTING received, it may come earlier
#ifdef MAX_GEM_STONES
  ...
  if (remote.is_host) {
    ...
    if (useGemStoneFlag) {
      ...
      // update himself
      for (p = 0;
           p < regPlayerCount && regPlayerId[p] != mp_obj.get_my_player_id();
           ++p)
        ;
      if (p < regPlayerCount) {
  ...
  if (remote.is_host) {
    ...
    for (p = 0;
         p < regPlayerCount && regPlayerId[p] != mp_obj.get_my_player_id(); ++p)
      ;
    if (p < regPlayerCount)
  ...
  if (remote.is_host) {
    ...
    MpStructNewPlayer msgNewPlayer(PLAYER_RATIO_CDROM, config.player_name,
                                   current_session->password);
    mp_obj.send(1, &msgNewPlayer, sizeof(msgNewPlayer));

    // ####### begin Gilbert 15/4 ##########//
    // put a message to the window
    MpStructChatMsg waitReplyMessage(
        NULL, text_game_menu
                  .str_mp_wait_host()); // "Waiting response from host..." );
    ...
    if (mp_obj.is_lobbied()) {
      if (player_profile.icon_id + 1 <= MAX_RACE) // monster icon
      {
        ...
        // hostPlayerId is not yet known
        MpStructAcquireRace msgAcquire(
            -(player_profile.icon_id + 1 - MAX_RACE));
        mp_obj.send(1, &msgAcquire, sizeof(msgAcquire));
      }
    }
  }
  ...
  //	GetA mapIdField;
  //	mapIdField.init( MAPID_X1, offsetY+83, 700, msgSeedStr.seed_str,
  //msgSeedStr.RANDOM_SEED_MAX_LEN, &font_san, 0, 1);

  // ------ initialize human / fryhtan button ------//

  ButtonCustomGroup speciesGroup(2);

  w = font_thin_black.text_width(
      text_game_menu.str_species_humans()); //  "Humans"
  h = font_thin_black.text_height();
  cx = 256;
  cy = 116;
  // SXM -30
#if (defined(CHINESE))
  speciesGroup[0].create(
      cx - w / 2 - 30, cy - h / 2 - 5, cx + w / 2 + 10, cy + h / 2 + 5,
      i_disp_text_button,
      ButtonCustomPara(text_game_menu.str_species_humans(), 0), 0);
#else
  speciesGroup[0].create(
      cx - w / 2 - 10, cy - h / 2 - 5, cx + w / 2 + 10, cy + h / 2 + 5,
      i_disp_text_button,
      ButtonCustomPara(text_game_menu.str_species_humans(), 0), 0);
#endif
  // SXM
  ...
  cy = 116;
#if (defined(CHINESE))
  speciesGroup[1].create(
      cx - w / 2 - 10 - 30, cy - h / 2 - 5, cx + w / 2 + 10, cy + h / 2 + 5,
      i_disp_text_button,
      ButtonCustomPara(text_game_menu.str_species_fryhtans(), 1), 0);
#else
  speciesGroup[1].create(
      cx - w / 2 - 10, cy - h / 2 - 5, cx + w / 2 + 10, cy + h / 2 + 5,
      i_disp_text_button,
  ...
  char frameDelaySettingArray[3] = {
      2, 4, 5}; // corresponding process frame delay value
  ...
  for (i = 0; i < 3; ++i) {
    ...
    h = font_thin_black.text_height();
    frameDelayGroup[i].create(
        cx, cy, cx + w + 10, cy + 20 - 1, i_disp_text_button,
        ButtonCustomPara(text_game_menu.str_low_to_high(i + 1), i), 0);
    cx += w + 20;
  }
  // ###### end Gilbert 11/3 #########//
  ...
  for (i = 0; i <= MAX_GEM_STONES; ++i) {
    gemStoneGroup[i].create(cx, cy, cx + 22, cy + 20 - 1, i_disp_int_button,
                            ButtonCustomPara(NULL, i), 0);
  ...
  for (i = 0; i < MAX_MONSTER_TYPE;
       ++i, (cy += monsterGroupYSpacing)) // MAX_MONSTER
  {
    w = font_thin_black.text_width(monster_res[i + 1]->name);
    h = monsterGroupYSpacing;
    monsterRaceGroup[i].create(cx - w / 2 - 5, cy - h / 2, cx + w / 2 + 5,
                               cy + h / 2, i_disp_race_button,
                               ButtonCustomPara(NULL, -(i + 1)), 0);
  }
  ...
  for (i = 0; i < MAX_COLOR_SCHEME; ++i, (cx += colorButtonWidth)) {
    colorGroup[i].create(cx, cy, cx + colorButtonWidth - 1,
                         cy + colorButtonHeight - 1, i_disp_color_button,
                         ButtonCustomPara(NULL, i + 1), 0);
  }
  ...
  for (i = 0; i < MAX_NATION; ++i, (cx += nationButtonSpacing)) {
    aiHumanNationGroup[i].create(cx - 10, cy - 10, cx + 10, cy + 10,
                                 i_disp_int_button, ButtonCustomPara(NULL, i),
                                 0);
  }
  // aiHumanNationGroup[0].visible_flag = 0;		// disable 0 computer
  // player
  ...
  // aiMonsterNationGroup[0].visible_flag = 0;		// disable 0 computer
  // player

  // ---------- initialize difficulty_level button group -------//

  ButtonCustomGroup diffGroup(OPTION_HARDEST - OPTION_EASIEST +
                              2); // 0-7 + OPTION_CUSTOM
  ...
  for (i = 0; i < OPTION_HARDEST - OPTION_EASIEST + 1; ++i) {
    ...
    h = font_thin_black.text_height();
    diffGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                        ButtonCustomPara(diffButtonStr[i], i + OPTION_EASIEST),
                        0);
    cx += w + 12;
  }
  ...
  h = font_thin_black.text_height();
  diffGroup[i].create(
      cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
      ButtonCustomPara(text_game_menu.str_difficulty_custom(), OPTION_CUSTOM),
      0);
  cx += w + 12;
  ...
  for (i = 0; i < terrainGroup.button_num; ++i) {
    ...
    h = font_thin_black.text_height();
    terrainGroup[i].create(
        cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
        ButtonCustomPara(text_game_menu.str_terrain_set(i + 1), i + 1), 0);
    cx += w + 12;
  }
  ...
  for (i = 0; i < buildingSizeGroup.button_num; ++i) {
    ...
    h = font_thin_black.text_height();
    buildingSizeGroup[i].create(
        cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
        ButtonCustomPara(text_game_menu.str_building_set(i + 1), i + 1), 0);
    cx += w + 12;
  }
  ...
  cy = 99;
  w = font_thin_black.text_width(
      text_game_menu.str_world_map(0)); // "Unexplored"
  h = font_thin_black.text_height();
  exploreGroup[0].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                         ButtonCustomPara(text_game_menu.str_world_map(0), 0),
                         0);
  cx += w + 20;
  w = font_thin_black.text_width(text_game_menu.str_world_map(1)); // "Explored"
  exploreGroup[1].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                         ButtonCustomPara(text_game_menu.str_world_map(1), 1),
                         0);
  ...
  h = font_thin_black.text_height();
  fogGroup[0].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                     ButtonCustomPara(text_game_menu.str_off_on(0), 0), 0);
  cx += w + 20;
  w = font_thin_black.text_width(text_game_menu.str_off_on(1));
  fogGroup[1].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                     ButtonCustomPara(text_game_menu.str_off_on(1), 1), 0);
  ...
  for (i = 0; i < 4; ++i) {
    ...
    h = font_thin_black.text_height();
    treasureGroup[i].create(
        cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
        ButtonCustomPara(text_game_menu.str_treasure(i + 1), i + 1), 0);
    cx += w + 20;
  }
  ...
  // #### begin Gilbert 10/2 #########//
  //	char *aggressiveStr[5] = { "Very Low", "Low", "Medium", "Great", "Very
  //great" };
  ...
  for (i = 0; i < 5; ++i) {
    ...
    h = font_thin_black.text_height();
    aiAggressiveGroup[i].create(
        cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
        ButtonCustomPara(text_game_menu.str_aggressiveness(i), i), 0);
    if (aiAggressiveGroup[i].x2 > SCROLL_SHEET_X2) { // line feed
  ...
  for (i = 0; i < 2; ++i) {
    ...
    h = font_thin_black.text_height();
    spyMethodGroup[i].create(
        cx, cy, cx + w + 10, cy + h + 5, i_disp_text_button,
        ButtonCustomPara(text_game_menu.str_need_research(i), i), 0);
    cx += w + 24;
  }
  ...
  for (i = 0; i < 2; ++i) {
    ...
    h = font_thin_black.text_height();
    randomStartUpGroup[i].create(
        cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
        ButtonCustomPara(text_game_menu.str_no_yes(i), i), 0);
    cx += w + 20;
  }
  ...
  for (i = 0; i < 2; ++i) {
    w = font_thin_black.text_width(text_game_menu.str_no_yes(i));
    h = font_thin_black.text_height();
    nearRawGroup[i].create(cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
                           ButtonCustomPara(text_game_menu.str_no_yes(i), i),
                           0);
    cx += w + 20;
  }
  ...
  for (i = 0; i < 3; ++i) {
    ...
    h = font_thin_black.text_height();
    townStartGroup[i].create(
        cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
        ButtonCustomPara(text_game_menu.str_few_to_many(i + 1),
                         startTownArray[i]),
        0);
    cx += w + 20;
  }
  ...
  for (i = 0; i < 3; ++i) {
    w = font_thin_black.text_width(text_game_menu.str_low_to_high(i + 1));
    h = font_thin_black.text_height();
    townResistGroup[i].create(
        cx, cy, cx + w + 10, cy + h + 10, i_disp_text_button,
        ButtonCustomPara(text_game_menu.str_low_to_high(i + 1), i + 1), 0);
    cx += w + 20;
  }
  ...
  for (i = 0; i < 4; ++i) {
    ...
    h = font_thin_black.text_height();
    randomEventGroup[i].create(
        cx, cy, cx + w + 10, cy + h + 6, i_disp_text_button,
        ButtonCustomPara(text_game_menu.str_never_to_frequent(i), i), 0);
    if ((i + 1) % 2 == 0) {
      // line feed
      cx = option4X;
      cy += 26;
  ...
  // ###### begin Gilbert 11/3 #########//
  ButtonCustom clearEnemyButton, clearMonsterButton, allyWinButton,
      enoughPeopleButton, enoughIncomeButton, enoughScoreButton,
      timeLimitButton;
  Button peopleInc, peopleDec, incomeInc, incomeDec, scoreInc, scoreDec,
      yearInc, yearDec;
    ...
    while (1) {
      ...
      // ######### end Gilbert 14/5 ########//
      {
        if (optionMode == OPTION_SPECIES) {
          if (refreshFlag & SGOPTION_PAGE) {
            ...
            for (i = OPTION_SPECIES; i <= OPTION_GOAL; ++i) {
              ...
              Font *fontPtr =
                  (i == optionMode ? &font_bold_red : &font_bold_black);

              fontPtr->center_put(
                  OPTION_SWITCH_X1,
                  OPTION_SWITCH_Y1 + OPTION_SWITCH_Y_SPACING * i,
                  OPTION_SWITCH_X2,
                  OPTION_SWITCH_Y2 + OPTION_SWITCH_Y_SPACING * i,
            ...
            if (!vga.use_back_buf) {
              image_interface.put_front(
                  (SCROLL_SHEET_X1 + SCROLL_SHEET_X2) / 2 - pictureWidth -
                      pictureXoffset,
                  SCROLL_SHEET_Y2 - 300 + pictureYoffset, "HUMANS");
              image_interface.put_front(
                  (SCROLL_SHEET_X1 + SCROLL_SHEET_X2) / 2 + pictureXoffset,
                  SCROLL_SHEET_Y2 - 300 + pictureYoffset, "FRYHTANS");
            } else {
              image_interface.put_back((SCROLL_SHEET_X1 + SCROLL_SHEET_X2) / 2 -
                                           pictureWidth - pictureXoffset,
                                       SCROLL_SHEET_Y2 - 300 + pictureYoffset,
                                       "HUMANS");
            ...
            // "Network Latency"
            font_bold_black.put(126, 368, text_game_menu.str_mp_latency(), 0,
                                360 - 10);
            ...
            if (remote.is_host) {
              font_thin_black.center_put(
                  BUTTON7_X1, BUTTON7_Y1, BUTTON7_X2, BUTTON7_Y2,
            ...
            font_thin_black.center_put(
                BUTTON9_X1, BUTTON9_Y1, BUTTON9_X2, BUTTON9_Y2,
                text_game_menu.str_cancel()); // "Cancel" );
          ...
          if (mRefreshFlag & MGOPTION_GEM_STONES && useGemStoneFlag) {
            ...
            font_bold_black.put(
                126, 393,
                text_game_menu.str_mp_gem_stones(player_profile.gem_stones), 1,
                360 - 10);
            // 15% per each gemstones
            font_small.put(
                126, 413,
                text_game_menu.str_mp_gem_stones_inst(15 * selfUseGemStones), 1,
                360 - 10);
        ...
        if (optionMode == OPTION_BASIC) {
          if (refreshFlag & SGOPTION_PAGE) {
            ...
            font_bold_black.right_put(
                playerNameField.x, playerNameField.y,
                text_game_menu.str_kingdom_of()); // "Kingdom of " );

            if (speciesGroup() == 0)
              font_bold_black.center_put(
                  116, 126, 303, 146,
                  text_game_menu.str_nationality()); // "Nationality");
            else if (speciesGroup() == 1)
              font_bold_black.center_put(
                  116, 126, 303, 146,
                  text_game_menu.str_species()); // "Fryhtan Species");
            font_bold_black.center_put(
                382, 129, 600, 149, text_game_menu.str_color()); // "Color" );
            font_bold_black.center_put(
                297, 225 - 20, 698, 243 - 20,
                text_game_menu.str_ai_nation_count()); // "Computer Controlled
                                                       // Kingdoms" );
            font_bold_black.put(aiHumanNationGroup[MAX_NATION - 1].x2 + 10,
                                aiHumanNationGroup[MAX_NATION - 1].y1,
                                text_game_menu.str_human()); // "Human" );
            font_bold_black.put(aiMonsterNationGroup[MAX_NATION - 1].x2 + 10,
                                aiMonsterNationGroup[MAX_NATION - 1].y1,
                                text_game_menu.str_fryhtan()); //"Fryhtan" );
            font_bold_black.center_put(
                341, 305 - 30, 654, 324 - 30,
                text_game_menu.str_difficulty_level()); // "Difficulty Level" );
#ifndef ONLY_TERRAIN_SET_1
            font_bold_black.center_put(
                370, 380 - 60, 633, 393 - 60,
                text_game_menu.str_terrain_set()); // "Terrain Type" );
#endif
#ifndef ONLY_SMALL_BUILDING
            font_bold_black.center_put(
                341, 365, 660, 384,
                text_game_menu.str_building_set()); // "Building Size" );
#endif
          ...
          if (refreshFlag & SGOPTION_RACE) {
            ...
            } else if (speciesGroup() == 1) // monster page
            {
              if (tempConfig.race_id < 0) {
                ...
                if (!vga.use_back_buf) {
                  mouse.hide_area(picX, picY, picX + picWidth - 1,
                                  picY + picHeight - 1);
          ...
          if (refreshFlag & SGOPTION_COLOR) {
            ...
            vga.active_buf->put_bitmap_trans_remap_decompress(
                ...
                game.color_remap_array[tempConfig.player_nation_color]
                    .color_table);
            ...
            vga_util.blt_buf(
                colorButtonFrameX, colorButtonFrameY,
        ...
        if (optionMode == OPTION_ADVANCED) {
          if (refreshFlag & SGOPTION_PAGE) {
            ...
            font_bold_black.put_paragraph(
                126, 99, option3X - 10, 143 - 1,
                text_game_menu.str_world_map()); // "World Map" );
            font_bold_black.put_paragraph(
                126, 143, option3X - 10, 183 - 1,
                text_game_menu.str_fog_of_war()); // "Fog of War" );
            font_bold_black.put_paragraph(
                126, 183, option3X - 10, 235 - 1,
                //	"Human Player's Treasure" );
                text_game_menu.str_your_treasure()); // "Your Treasure" );
            font_bold_black.put_paragraph(
                126, 235, option3X - 10, 289 - 1,
                text_game_menu.str_ai_treasure()); // "Computer's Treasure" );
            font_bold_black.put_paragraph(
                126, 289, option3X - 10, 339 - 1,
                text_game_menu
                    .str_ai_aggressiveness()); // "Computer's Aggressiveness" );
#if (defined(GERMAN))
            font_bold_black.put_paragraph(
                126, 348, option3X - 10, 389 - 1,
                text_game_menu
                    .str_spy_methodology()); // "Espionage Methodologies" );
#else
            font_bold_black.put_paragraph(
                126, 339, option3X - 10, 389 - 1,
                text_game_menu
                    .str_spy_methodology()); // "Espionage Methodologies" );
#endif
            font_bold_black.put_paragraph(
                126, 389, option3X - 10, 449 - 1,
        ...
        if (optionMode == OPTION_ADVANCE2) {
          if (refreshFlag & SGOPTION_PAGE) {
            ...
            font_bold_black.put_paragraph(
                126, 100, option4X - 10, 152 - 1,
                text_game_menu.str_start_up_raw_site()); // "Natural Resources
                                                         // at Start" );
            font_bold_black.put_paragraph(
                126, 152, option4X - 10, 206 - 1,
                text_game_menu
                    .str_raw_nearby()); // "Natural Resources Nearby" );
            font_bold_black.put_paragraph(
                126, 206, option4X - 10, 246 - 1,
                text_game_menu
                    .str_independent_town()); // "Independent Towns" );
            font_bold_black.put_paragraph(
                126, 246, option4X - 10, 293 - 1,
                text_game_menu
                    .str_town_resistance()); // "Independent Town Resistance" );
            font_bold_black.put_paragraph(
                126, 293, option4X - 10, 340 - 1,
                text_game_menu.str_new_town_emerge()); // "New Towns Emerge" );
            font_bold_black.put_paragraph(
                126, 340, option4X - 10, 384 - 1,
                text_game_menu
                    .str_new_kingdom_emerge()); // "New Kingdoms Emerge" );
            font_bold_black.put_paragraph(
                126, 384, option4X - 10, 434 - 1,
          ...
          if (refreshFlag & SGOPTION_START_TOWN) {
            for (i = 0;
                 tempConfig.start_up_independent_town > startTownArray[i]; ++i)
              ;
        ...
        if (optionMode == OPTION_GOAL) {
          if (refreshFlag & SGOPTION_PAGE) {
            ...
            font_bold_black.put(
                option5X, 112,
                text_game_menu
                    .str_goal()); // You will be victorious when you have:

            font_thin_black.put_paragraph(
                option5X + tickWidth + 10, 145, option5X3 + 25, 178 - 1,
                text_game_menu.str_defeat_others()); // "Defeated All Others" );
            font_thin_black.put_paragraph(
                option5X + tickWidth + 10, 178, option5X3 + 25, 211 - 1,
                text_game_menu
                    .str_defeat_fryhtan_lairs()); // "Destroyed All Independent
                                                  // Fryhtan Lairs" );
            font_thin_black.put_paragraph(
                option5X + tickWidth + 10, 211, option5X3 + 25, 244 - 1,
                text_game_menu
                    .str_defeat_except_ally()); // "Defeated All except one
                                                // Allied Kingdom" );
            font_thin_black.put_paragraph(
                option5X + tickWidth + 10, 244, option5X2 - 65, 277 - 1,
                text_game_menu
                    .str_reach_population()); // "Achieved A Population Of" );
            font_thin_black.put_paragraph(
                option5X + tickWidth + 10, 277, option5X2 - 65, 310 - 1,
                text_game_menu
                    .str_reach_economy()); // "Reached An Economic Score Of");
            font_thin_black.put_paragraph(
                option5X + tickWidth + 10, 310, option5X2 - 65, 343 - 1,
                text_game_menu
                    .str_reach_total_score()); //"Reached A Total Score Of");
            font_thin_black.put_paragraph(
                option5X + tickWidth + 10, 343, option5X2 - 100, 376 - 1,
                text_game_menu.str_goal_time_limit()); // "Achieved The Selected
                                                       // Within" );
          ...
          if (refreshFlag & SGOPTION_TIME_LIMIT) {
            ...
            int x2 =
                font_bold_black.put(yearInc.x1 - 100, yearInc.y1,
                                    misc.format(tempConfig.goal_year_limit), 1,
                                    yearInc.x1 - 60 - 1);
            font_thin_black.put(yearInc.x1 - 60, yearInc.y1,
                                text_game_menu.str_goal_time_units(),
                                1); // "years"
            yearInc.paint();
            yearDec.paint();
          }
        ...
        if (mRefreshFlag & MGOPTION_PLAYERS) {
          ...
          for (p = 0; p < regPlayerCount; ++p) {
            ...
            if (!playerReadyFlag[p]) {
              vga.active_buf->bar(tickX[p], tickY[p],
                                  tickX[p] + nameTickWidth - 1,
                                  tickY[p] + nameTickHeight - 1, V_WHITE);
              vga.active_buf->d3_panel_up(tickX[p], tickY[p],
                                          tickX[p] + nameTickWidth - 1,
                                          tickY[p] + nameTickHeight - 1, 4, 0);
            } else {
              vga.active_buf->d3_panel_down(
                  tickX[p], tickY[p], tickX[p] + nameTickWidth - 1,
                  tickY[p] + nameTickHeight - 1, 4, 0);
              vga.active_buf->put_bitmap_trans_decompress(
                  tickX[p] + 4, tickY[p] + 4, image_button.read("TICK-S"));
            }

            // put color of player
            vga.active_buf->bar(
                tickX[p] + nameTickWidth, tickY[p],
                tickX[p] + nameTickWidth * 2 - 1, tickY[p] + nameTickHeight - 1,
                game.color_remap_array[playerColor[p]].main_color);
            vga.active_buf->d3_panel_up(tickX[p] + nameTickWidth, tickY[p],
                                        tickX[p] + nameTickWidth * 2 - 1,
                                        tickY[p] + nameTickHeight - 1, 4, 0);
            PlayerDesc *dispPlayer = mp_obj.search_player(regPlayerId[p]);
            int x2 =
                font_bld.put(tickX[p] + nameTickWidth * 2 + 3, tickY[p] + 0,
                             dispPlayer ? dispPlayer->friendly_name_str()
                                        : (char *)"?anonymous?",
                             0, tickX[p] + 260);
#ifdef MAX_GEM_STONES
            if (totalGemStones[p]) {
              ...
              if (sys.debug_session) {
                x2 = font_bld.put(x2, tickY[p] + 0, useGemStones[p], 0,
                                  tickX[p] + 260);
                x2 = font_bld.put(x2, tickY[p] + 0, "/", 0, tickX[p] + 260);
              }
              x2 = font_bld.put(x2, tickY[p] + 0, totalGemStones[p], 0,
                                tickX[p] + 260);
              x2 = font_bld.put(x2, tickY[p] + 0, ")", 0, tickX[p] + 260);
            }
#endif
        ...
        if (mRefreshFlag & MGOPTION_IN_MESSAGE) {
          ...
          if (p <= 0)
            p = 1;
          for (; p <= messageList.size();
               ++p, (ny += font_cara.max_font_height)) {
            int nx = 132;
            if (((MpStructChatMsg *)messageList.get(p))->sender[0]) {
              nx = font_cara.put(
                  nx, ny, ((MpStructChatMsg *)messageList.get(p))->sender, 0,
                  661);
              nx = font_cara.put(nx, ny, " : ", 0, 661);
            }
            nx = font_cara.put(nx, ny,
                               ((MpStructChatMsg *)messageList.get(p))->content,
                               0, 661);
          }
          ...
          // display button
          if (optionMode != OPTION_CHAT_LOG)
            vga.active_buf->d3_panel_up(CHAT_LOG_X1, CHAT_LOG_Y1, CHAT_LOG_X2,
                                        CHAT_LOG_Y2);
          else
            vga.active_buf->d3_panel_down(CHAT_LOG_X1, CHAT_LOG_Y1, CHAT_LOG_X2,
                                          CHAT_LOG_Y2);
          // ######### end Gilbert 6/7 ##########//
        }
        ...
        if (optionMode != OPTION_SPECIES && refreshFlag & SGOPTION_DIFFICULTY) {
          font_bld.center_put(40, 85, 82, 105,
                              text_game_menu.str_level_in_score(),
                              1); // "Level"
          font_bold_black.center_put(
              40, 105, 82, 125,
              misc.format(
                  tempConfig.multi_player_difficulty(regPlayerCount - 1, 0)),
              1);
        }
      ...
      if (sysMsgCount < 0 && from) {
        ...
        if (remote.is_host) {
          ...
          if (q < regPlayerCount) {
            ...
            memmove(regPlayerId + q, regPlayerId + q + 1,
                    (MAX_NATION - 1 - q) * sizeof(regPlayerId[0]));
            regPlayerId[MAX_NATION - 1] = 0;
            memmove(playerReadyFlag + q, playerReadyFlag + q + 1,
                    (MAX_NATION - 1 - q) * sizeof(playerReadyFlag[0]));
            playerReadyFlag[MAX_NATION - 1] = 0;
            short freeColor = playerColor[q];
            memmove(playerColor + q, playerColor + q + 1,
                    (MAX_NATION - 1 - q) * sizeof(playerColor[0]));
            ...
            memmove(playerRace + q, playerRace + q + 1,
                    (MAX_NATION - 1 - q) * sizeof(playerRace[0]));
            playerRace[MAX_NATION - 1] = 0;
            if (freeRace != 0)
              raceAssigned_[freeRace]--;
            memmove(playerBalance + q, playerBalance + q + 1,
                    (MAX_NATION - 1 - q) * sizeof(playerBalance[0]));
            playerBalance[MAX_NATION - 1] = 0;
#ifdef MAX_GEM_STONES
            memmove(useGemStones + q, useGemStones + q + 1,
                    (MAX_NATION - 1 - q) * sizeof(useGemStones[0]));
            useGemStones[MAX_NATION - 1] = 0;
            memmove(totalGemStones + q, totalGemStones + q + 1,
                    (MAX_NATION - 1 - q) * sizeof(totalGemStones[0]));
            totalGemStones[MAX_NATION - 1] = 0;
#endif
#ifdef APPLY_RANKING
            memmove(playerRanking + q, playerRanking + q + 1,
                    (MAX_NATION - 1 - q) * sizeof(playerRanking[0]));
      ...
      if (recvPtr) {
        if (((MpStructBase *)recvPtr)->msg_id == MPMSG_START_GAME) {
          ...
          case MPMSG_ABORT_GAME:
            mp_info_box_show(&info_box, text_game_menu.str_mp_host_abort(),
                             "Ok");
            break;
          case MPMSG_SEND_CONFIG:
            ...
            if (((MpStructConfig *)recvPtr)->game_config.validate()) {
              tempConfig.change_game_setting(
                  ((MpStructConfig *)recvPtr)->game_config);
              refreshFlag |= SGOPTION_ALL_OPTIONS;
            } else {
              mp_info_box_show(&info_box,
                               text_game_menu.str_mp_refuse_reason(
                                   MpStructRefuseNewPlayer::REFUSE_VERSION),
                               "Ok");
              break;
            }
          ...
          case MPMSG_NEW_PLAYER:
            if (remote.is_host) {
              ...
              if (regPlayerCount >= MAX_NATION) {
                ...
                MpStructRefuseNewPlayer msgRefuse(
                    from, MpStructRefuseNewPlayer::REFUSE_SEAT_FULL);
                mp_obj.send(from, &msgRefuse, sizeof(msgRefuse));
              } else if (!newPlayerMsg->is_compatible()) {
                // reply refuse new player
                MpStructRefuseNewPlayer msgRefuse(
                    from, MpStructRefuseNewPlayer::REFUSE_VERSION);
                mp_obj.send(from, &msgRefuse, sizeof(msgRefuse));
              } else if (regPlayerCount < MAX_NATION &&
                         mp_obj.auth_player(
                             from, ((MpStructNewPlayer *)recvPtr)->name,
                ...
                MpStructAcceptNewPlayer msgAccept(
                    from, newPlayerMsg->name,
                    mp_obj.search_player(from)->get_address(), 1);
                ...
                for (t = 0; t < MAX_COLOR_SCHEME; ++t, ++c) {
                  ...
                  if (!colorAssigned[c]) {
                    ...
                    mp_obj.send(BROADCAST_PID, &msgAcceptColor,
                                sizeof(msgAcceptColor));
                    break;
                  }
                }
                ...
                for (c = 0; c < regPlayerCount; ++c) {
                  if (playerColor[c]) {
                    MpStructAcceptColor msgAcceptColor(regPlayerId[c],
                                                       playerColor[c]);
                ...
                MpStructProcessFrameDelay msgframeDelay(
                    frameDelaySettingArray[frameDelayGroupSetting]);
                ...
                if (useGemStoneFlag) {
                  for (c = 0; c < regPlayerCount; ++c) {
                    MpStructSetGemStones msgUseGemStones(
                        regPlayerId[c], useGemStones[c], totalGemStones[c]);
                    mp_obj.send(from, &msgUseGemStones,
                                sizeof(msgUseGemStones));
                  }
                }
#endif
#ifdef APPLY_RANKING
                ...
                MpStructRefuseNewPlayer msgRefuse(
                    from, MpStructRefuseNewPlayer::REFUSE_REASON_OTHER);
                mp_obj.send(from, &msgRefuse, sizeof(msgRefuse));
              }
            }
            break;
          case MPMSG_LOAD_GAME_NEW_PLAYER: {
            ...
            MpStructRefuseNewPlayer msgRefuse(
                from, MpStructRefuseNewPlayer::REFUSE_HOST_NEW_GAME);
            mp_obj.send(from, &msgRefuse, sizeof(msgRefuse));
          } break;
          case MPMSG_ACCEPT_NEW_PLAYER:
            hostPlayerId = from;
            if (regPlayerCount < MAX_NATION &&
                ((MpStructAcceptNewPlayer *)recvPtr)->player_id !=
                    mp_obj.get_my_player_id()) {
              MpStructAcceptNewPlayer *newb =
                  (MpStructAcceptNewPlayer *)recvPtr;
              mp_obj.add_player(newb->player_id, newb->player_name,
                                &newb->address, newb->make_contact);

              // search if this player has existed
              for (p = 0;
                   p < regPlayerCount && regPlayerId[p] != newb->player_id; ++p)
                ;
            ...
            // ####### begin Gilbert 15/4 ##########//
            // put a message to the window
            if (((MpStructAcceptNewPlayer *)recvPtr)->player_id ==
                mp_obj.get_my_player_id()) {
              MpStructChatMsg waitReplyMessage(
                  NULL, text_game_menu.str_mp_host_ok()); //"Ok" );
              messageList.linkin(&waitReplyMessage);
          ...
          case MPMSG_ACQUIRE_RACE:
            if (remote.is_host) {
              ...
              if (cl <= MAX_RACE && p < regPlayerCount &&
                  (shareRace ||
                   raceAssigned_[cl] ==
                       0)) // more than one player can use the same race
              {
                ...
                // reply refuse race
                MpStructRefuseRace msgRefuseRace(
                    from, ((MpStructAcquireRace *)recvPtr)->race_id);
                mp_obj.send(from, &msgRefuseRace, sizeof(msgRefuseRace));
              }
            }
            break;
          case MPMSG_ACCEPT_RACE:
            if (((MpStructAcceptRace *)recvPtr)->request_player_id ==
                mp_obj.get_my_player_id()) {
              tempConfig.race_id =
                  char(((MpStructAcceptRace *)recvPtr)->race_id);
              refreshFlag |=
                  SGOPTION_RACE |
                  SGOPTION_DIFFICULTY; // change species also change difficulty
            }
            break;
          case MPMSG_REFUSE_RACE:
            if (((MpStructRefuseRace *)recvPtr)->request_player_id ==
                mp_obj.get_my_player_id()) {
          ...
          case MPMSG_ACQUIRE_COLOR:
            if (remote.is_host) {
              ...
              if (!cl) {
                for (cl = 1; cl <= MAX_COLOR_SCHEME && colorAssigned[cl - 1];
                     ++cl)
                  ;
              }
              int p;
              for (p = 0; p < regPlayerCount && regPlayerId[p] != from; ++p)
                ;
              if (cl <= MAX_COLOR_SCHEME && !colorAssigned[cl - 1] &&
                  p < regPlayerCount) {
                if (playerColor[p] > 0)
                ...
                // reply refuse color
                MpStructRefuseColor msgRefuseColor(
                    from, ((MpStructAcquireColor *)recvPtr)->color_scheme_id);
                mp_obj.send(from, &msgRefuseColor, sizeof(msgRefuseColor));
              }
            }
            break;
          case MPMSG_ACCEPT_COLOR:
            if (((MpStructAcceptColor *)recvPtr)->request_player_id ==
                mp_obj.get_my_player_id()) {
              tempConfig.player_nation_color =
                  char(((MpStructAcceptColor *)recvPtr)->color_scheme_id);
            ...
            { // other player change color
              ...
              for (p = 0;
                   p < regPlayerCount &&
                   regPlayerId[p] !=
                       ((MpStructAcceptColor *)recvPtr)->request_player_id;
                   ++p)
                ;
          ...
          case MPMSG_REFUSE_COLOR:
            if (((MpStructRefuseColor *)recvPtr)->request_player_id ==
                mp_obj.get_my_player_id()) {
              refreshFlag |= SGOPTION_COLOR;
              // sound effect here
            }
          ...
          case MPMSG_PLAYER_READY: {
            for (int p = 0; p < regPlayerCount; ++p) {
              if (regPlayerId[p] ==
                  ((MpStructPlayerReady *)recvPtr)->player_id) {
                playerReadyFlag[p] = 1;
                mRefreshFlag |= MGOPTION_PLAYERS;
              }
            }
          ...
          case MPMSG_PLAYER_UNREADY: {
            for (int p = 0; p < regPlayerCount; ++p) {
              if (regPlayerId[p] ==
                  ((MpStructPlayerUnready *)recvPtr)->player_id) {
                playerReadyFlag[p] = 0;
                mRefreshFlag |= MGOPTION_PLAYERS;
              }
            }
            if (remote.is_host) {
              // forward message to everyone
              mp_obj.send(BROADCAST_PID, recvPtr,
                          sizeof(MpStructPlayerUnready));
            }
          } break;
#ifdef MAX_GEM_STONES
          case MPMSG_SET_GEM_STONE: {
            for (int p = 0; p < regPlayerCount; ++p) {
              if (regPlayerId[p] ==
                  ((MpStructSetGemStones *)recvPtr)->player_id) {
                useGemStones[p] =
                    ((MpStructSetGemStones *)recvPtr)->use_gem_stones;
                totalGemStones[p] =
                    ((MpStructSetGemStones *)recvPtr)->total_gem_stones;
                mRefreshFlag |= MGOPTION_PLAYERS;
              }
            }
            if (!remote.is_host && !useGemStoneFlag) {
              ...
              MpStructSetGemStones msgSetGemStones(mp_obj.get_my_player_id(),
                                                   selfUseGemStones,
                                                   player_profile.gem_stones);
              mp_obj.send(BROADCAST_PID, &msgSetGemStones,
                          sizeof(msgSetGemStones));

              // update himself
              int p;
              for (p = 0; p < regPlayerCount &&
                          regPlayerId[p] != mp_obj.get_my_player_id();
                   ++p)
                ;
              if (p < regPlayerCount) {
                useGemStones[p] = selfUseGemStones;
          ...
          case MPMSG_SET_PROCESS_FRAME_DELAY: {
            for (frameDelayGroupSetting = 2; frameDelayGroupSetting > 0;
                 --frameDelayGroupSetting) {
              if (((MpStructProcessFrameDelay *)recvPtr)
                      ->common_process_frame_delay >=
                  frameDelaySettingArray[frameDelayGroupSetting])
                break;
            }
          ...
          case MPMSG_SEND_SYNC_TEST_LEVEL:
            remote.sync_test_level =
                ((MpStructSyncLevel *)recvPtr)->sync_test_level;
            break;
          ...
          case MPMSG_REFUSE_NEW_PLAYER:
            mp_info_box_show(&info_box,
                             ((MpStructRefuseNewPlayer *)recvPtr)->reason_str(),
                             "Ok");
            break;
          case MPMSG_PLAYER_ID:
            if (mp_obj.set_my_player_id(
                    ((MpStructPlayerId *)recvPtr)->your_id)) {
          ...
          case MPMSG_PLAYER_DISCONNECT:
            if (!remote.is_host) {
              int q;
              mp_obj.delete_player(
                  ((MpStructPlayerDisconnect *)recvPtr)->lost_player_id);
      ...
      if (optionMode == OPTION_SPECIES) {
        if (speciesGroup.detect() >= 0 ||
            speciesGroup[speciesGroup()].detect(
                0, 0, 0, 1)) // detect pushed button, but suspend pop
        {
          optionMode = OPTION_BASIC; // auto change to basic mode
          refreshFlag = SGOPTION_ALL;
          mRefreshFlag = MGOPTION_ALL;
        } else if (mouse.single_click(SCROLL_SHEET_X1, SCROLL_SHEET_Y1,
                                      (SCROLL_SHEET_X1 + SCROLL_SHEET_X2) / 2,
                                      368 - 5)) {
          speciesGroup.push(0, 0);   // switch to human mode
          optionMode = OPTION_BASIC; // auto change to basic mode
          refreshFlag = SGOPTION_ALL;
          mRefreshFlag = MGOPTION_ALL;
        } else if (mouse.single_click(
                       (SCROLL_SHEET_X1 + SCROLL_SHEET_X2) / 2 + 1,
                       SCROLL_SHEET_Y1, SCROLL_SHEET_X2, 368 - 5)) {
          ...
          if (speciesGroup() == 0 && raceGroup.detect() >= 0 ||
            ...
            if (remote.is_host) {
              ...
              for (p = 0; p < regPlayerCount &&
                   ...
                   ++p)
                ;
              if (r <= MAX_RACE && r >= -MAX_MONSTER_TYPE &&
                  p < regPlayerCount &&
                  (shareRace ||
                   raceAssigned_[r] ==
                       0)) // more than one player can use the same race
            ...
            if (remote.is_host) {
              if (!colorAssigned[r - 1]) {
                ...
                for (p = 0; p < regPlayerCount &&
                     ...
                     ++p)
                  ;
                if (r <= MAX_COLOR_SCHEME && !colorAssigned[r - 1] &&
                    p < regPlayerCount) {
                  ...
                  MpStructAcceptColor msgAcceptColor(mp_obj.get_my_player_id(),
                                                     r);
                  mp_obj.send(BROADCAST_PID, &msgAcceptColor,
                              sizeof(msgAcceptColor));
                  mRefreshFlag |= MGOPTION_PLAYERS;
      ...
      if (remote.is_host && !selfReadyFlag) {
        ...
        if (optionMode == OPTION_BASIC) {
          if (aiHumanNationGroup.detect() >= 0) {
            int newHumanNationCount =
                aiHumanNationGroup[aiHumanNationGroup()].custom_para.value;
            tempConfig.ai_human_nation_count = newHumanNationCount;

            if (newHumanNationCount > 0) {
              // 1 player + ai human + ai monster <= MAX_NATION
              if (1 + tempConfig.ai_human_nation_count +
                      tempConfig.ai_monster_nation_count >
                  MAX_NATION)
                tempConfig.ai_monster_nation_count =
                    MAX_NATION - 1 - tempConfig.ai_human_nation_count;
            }
            ...
            int newMonsterNationCount =
                aiMonsterNationGroup[aiMonsterNationGroup()].custom_para.value;
            ...
            if (newMonsterNationCount > 0) {
              ...
              if (1 + tempConfig.ai_human_nation_count +
                      tempConfig.ai_monster_nation_count >
                  MAX_NATION)
                tempConfig.ai_human_nation_count =
                    MAX_NATION - 1 - tempConfig.ai_monster_nation_count;
            }
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
            refreshFlag |= SGOPTION_AI_NATION | SGOPTION_DIFFICULTY;
            ...
            if (diffGroup[diffGroup()].custom_para.value != OPTION_CUSTOM) {
              tempConfig.change_difficulty(
                  diffGroup[diffGroup()].custom_para.value);
              configChange = 1;
            ...
            tempConfig.terrain_set =
                terrainGroup[terrainGroup()].custom_para.value;
            ...
            tempConfig.building_size =
                buildingSizeGroup[buildingSizeGroup()].custom_para.value;
            configChange = 1;
        ...
        // -------- detect advanced option ---------//
          ...
          if (exploreGroup.detect() >= 0) {
            tempConfig.explore_whole_map =
                exploreGroup[exploreGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
            ...
            tempConfig.start_up_cash =
                treasureGroup[treasureGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
            // refreshFlag |= SGOPTION_TREASURE;
            refreshFlag |= SGOPTION_DIFFICULTY;
          } else if (aiTreasureGroup.detect() >= 0) {
            tempConfig.ai_start_up_cash =
                aiTreasureGroup[aiTreasureGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
            ...
            tempConfig.spy_methodology =
                spyMethodGroup[spyMethodGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
            // refreshFlag |= SGOPTION_SPY_METHOD;
            refreshFlag |= SGOPTION_DIFFICULTY;
          } else if (randomStartUpGroup.detect() >= 0) {
            tempConfig.random_start_up =
                randomStartUpGroup[randomStartUpGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
        ...
        // -------- detect advanced option ---------//
          ...
          if (rawSiteGroup.detect() >= 0) {
            tempConfig.start_up_raw_site =
                rawSiteGroup[rawSiteGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
            // refreshFlag |= SGOPTION_RAW;
            refreshFlag |= SGOPTION_DIFFICULTY;
          } else if (nearRawGroup.detect() >= 0) {
            tempConfig.start_up_has_mine_nearby =
                nearRawGroup[nearRawGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
            ...
            tempConfig.start_up_independent_town =
                townStartGroup[townStartGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
            // resfreshFlag |= SGOPTION_START_TOWN;
            refreshFlag |= SGOPTION_DIFFICULTY;
          } else if (townResistGroup.detect() >= 0) {
            tempConfig.independent_town_resistance =
                townResistGroup[townResistGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
            // resfreshFlag |= SGOPTION_TOWN_RESIST;
            refreshFlag |= SGOPTION_DIFFICULTY;
          } else if (townEmergeGroup.detect() >= 0) {
            tempConfig.new_independent_town_emerge =
                townEmergeGroup[townEmergeGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
            // refreshFlag |= SGOPTION_TOWN_EMERGE;
            refreshFlag |= SGOPTION_DIFFICULTY;
          } else if (nationEmergeGroup.detect() >= 0) {
            tempConfig.new_nation_emerge =
                nationEmergeGroup[nationEmergeGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
            // refreshFlag |= SGOPTION_NATION_EMERGE;
            refreshFlag |= SGOPTION_DIFFICULTY;
          } else if (randomEventGroup.detect() >= 0) {
            tempConfig.random_event_frequency =
                randomEventGroup[randomEventGroup()].custom_para.value;
            tempConfig.difficulty_level = OPTION_CUSTOM;
            configChange = 1;
        ...
        // -------- detect goal option ----------//
          ...
          // ######## end Gilbert 11/3 #########//
            ...
            tempConfig.goal_economic_score_flag =
                enoughIncomeButton.pushed_flag;
            configChange = 1;
      ...
      for (i = OPTION_SPECIES; i <= OPTION_GOAL; ++i) {
        if (mouse.single_click(OPTION_SWITCH_X1,
                               OPTION_SWITCH_Y1 + OPTION_SWITCH_Y_SPACING * i,
                               OPTION_SWITCH_X2,
                               OPTION_SWITCH_Y2 +
                                   OPTION_SWITCH_Y_SPACING * i)) {
          optionMode = i;
          refreshFlag = SGOPTION_ALL;
          mRefreshFlag = MGOPTION_ALL;
        }
      ...
      if (mouse.any_click(CHAT_LOG_X1, CHAT_LOG_Y1, CHAT_LOG_X2, CHAT_LOG_Y2)) {
        if (optionMode != OPTION_CHAT_LOG) {
          ...
          err_when(backupOptionMode < OPTION_SPECIES ||
                   backupOptionMode > OPTION_GOAL);
      ...
      if (p < regPlayerCount &&
          (mouse.any_click(BUTTON8_X1, BUTTON8_Y1, BUTTON8_X2, BUTTON8_Y2) ||
           mouse.any_click(tickX[p], tickY[p], tickX[p] + nameTickWidth - 1,
                           tickY[p] + nameTickHeight - 1))) {
        mRefreshFlag |= MGOPTION_PLAYERS;
        if (!selfReadyFlag) {
      ...
      // -------- detect return button ---------//
      ...
      else if (mouse.single_click(BUTTON9_X1, BUTTON9_Y1, BUTTON9_X2,
                                  BUTTON9_Y2)) {
        if (box.ask(text_game_menu.str_mp_confirm_quit())) {
          ...
          break; // break while(1)
                 ...
                 (keyCode = messageField.detect()) !=
                     0) // keyCode may be non-zero if after mapIdField.detect()
      {
  ...
  if (retFlag > 0) {
    ...
    if (remote.is_host) {
...

      {
        ...
        memcpy(setupString.reserve(sizeof(msgStart)), &msgStart,
               sizeof(msgStart));
      }
      ...
      {
        MpStructConfig msgConfig(tempConfig);
        memcpy(setupString.reserve(sizeof(msgConfig)), &msgConfig,
               sizeof(msgConfig));
      }
      ...
      info.init(INFO_DEFAULT_YEAR, INFO_DEFAULT_MONTH,
                INFO_DEFAULT_DAY); // init game_date to make
                                   // nation_array.new_nation successful
      ...
      for (p = 0; p < regPlayerCount; ++p) {
        ...
        nationPara[playerCount].init(playerCount + 1, playerId, playerColor[p],
                                     playerRace[p], player->friendly_name_str()
#ifdef MAX_GEM_STONES
                                                        ,
                                     useGemStones[p]
#else
        ...
        ((MpStructNation *)setupString.reserve(sizeof(MpStructNation)))
            ->init(playerCount + 1, playerId, playerColor[p], playerRace[p],
                   player->friendly_name_str(),
                   nationPara[playerCount].use_gem_stones,
                   nationPara[playerCount].ranking);
        playerCount++;
      }

      *mpPlayerCount = playerCount;
      config.human_difficulty_rating =
          config.multi_player_difficulty(playerCount - 1, 1);
      config.monster_difficulty_rating =
          config.multi_player_difficulty(playerCount - 1, -1);
...
#else
      remote.set_process_frame_delay(
          frameDelaySettingArray[frameDelayGroupSetting]);
#endif
      // ###### end Gilbert 11/3 ########//
      {
        MpStructProcessFrameDelay msgFrameDelay(
            remote.get_process_frame_delay());
        memcpy(setupString.reserve(sizeof(msgFrameDelay)), &msgFrameDelay,
               sizeof(msgFrameDelay));
      }
      ...
      {
        MpStructSyncLevel msgSyncTest(remote.sync_test_level);
        memcpy(setupString.reserve(sizeof(msgSyncTest)), &msgSyncTest,
               sizeof(msgSyncTest));
      }
      ...
      while (offset < recvLen) {
        ...
        case MPMSG_SEND_CONFIG:
          ...
          if (((MpStructConfig *)recvPtr)->game_config.validate()) {
            ...
            ++recvConfig;
            config =
                tempConfig; // nation_array.new_nation reads setting from config
          }
        ...
        case MPMSG_DECLARE_NATION: {
          if (playerCount == 0)
            info.init(INFO_DEFAULT_YEAR, INFO_DEFAULT_MONTH,
                      INFO_DEFAULT_DAY); // info.init for the first time

          MpStructNation *msgNation = (MpStructNation *)recvPtr;
          nationPara[playerCount].init(
              msgNation->nation_recno, msgNation->dp_player_id,
              msgNation->color_scheme, msgNation->race_id,
              msgNation->player_name, msgNation->use_gem_stones,
              msgNation->ranking);
        ...
        case MPMSG_SET_PROCESS_FRAME_DELAY:
          remote.set_process_frame_delay(((MpStructProcessFrameDelay *)recvPtr)
                                             ->common_process_frame_delay);
          ++recvSetFrameDelay;
        ...
        if (!recvStartMsg || offset <= oldOffset) {
          err_here();
          box.msg(text_game_menu
                      .str_mp_connect_str_corrupt()); // "Connection string from
                                                      // host is corrupted" );
          return -1;
        }
      } // end while
      ...
      if (!recvSeed) {
        box.msg(text_game_menu.str_mp_seed_lost()); // "Cannot get random seeds
                                                    // from the host." );
        return -1;
      }
      err_when(recvSeed > 1);
      if (!recvConfig) {
        box.msg(text_game_menu
                    .str_mp_config_lost()); // "Cannot get game configuration
                                            // info from the host." );
      ...
      if (playerCount == 0) {
        box.msg(
            text_game_menu
                .str_mp_nation_info_lost()); //"Cannot get kingdom information
                                             //from the host." );
        return -1;
      }
      ...
      if (!ownPlayerFound) {
        box.msg(text_game_menu
                    .str_mp_host_not_recognize()); // "The host cannot recognize
                                                   // your machine." );
        return -1;
      }
      ...
      if (!recvSetFrameDelay || !recvSyncTestLevel) {
        box.msg(text_game_menu
                    .str_mp_miss_connect_info()); //"Cannot receive important
    ...
    if (remote.sync_test_level == 0) {
      remote.set_alternating_send(
          playerCount > 4 ? 4 : playerCount); // automatic setting
    }

    {
      ...
      if (recvEndSetting < playerCount - 1) {
        box.msg(text_game_menu
                    .str_mp_other_player_fail()); // "Some player(s) encountered
...
int Game::mp_select_load_option(char *fileName) {
  ...
  if (remote.is_host) {
    ...
    memset(colorAssigned, 0,
           sizeof(colorAssigned)); // assume all color are unassigned

    current_session = mp_obj.get_current_session();

    MpStructLoadGameNewPlayer msgNewPlayer(
        ~nation_array, sys.frame_count, misc.get_random_seed(),
        PLAYER_RATIO_CDROM, config.player_name, current_session->password);
    mp_obj.send(1, &msgNewPlayer, sizeof(msgNewPlayer));
  ...
  // #### begin Gilbert 10/2 #########//
  const char *aggressiveStr[5] = {"Very Low", "Low", "Medium", "Great",
                                  "Very great"};
    ...
    while (1) {
      ...
      // ######### end Gilbert 14/5 ########//
      {
        if (optionMode == OPTION_SPECIES) {
          if (refreshFlag & SGOPTION_PAGE) {
            ...
            if (mRefreshFlag & MGOPTION_GEM_STONES && game.total_gem_stones) {
              ...
              font_bold_black.put(
                  126, 393,
                  text_game_menu.str_mp_gem_stones(player_profile.gem_stones),
                  0, 360 - 10);
          ...
          //#ifdef MAX_GEM_STONES
          //					// display number gem stone to
          //use 					if( mRefreshFlag & MGOPTION_GEM_STONES && useGemStoneFlag )
          //					{
          //						font_bold_black.put_paragraph( 126, 408,
          //360-10, 458-1,
          //							text_game_menu.str_mp_gem_stones(player_profile.gem_stones)
          //); // "Gem Stones (xxx)" ); 						gemStoneGroup.paint(selfUseGemStones);
          //					}
          //#endif
          // other ...
        }
        ...
        if (optionMode != OPTION_SPECIES && refreshFlag & SGOPTION_DIFFICULTY) {
          ...
          // font_bold_black.center_put( 40, 105, 82, 125,
          // misc.format(tempConfig.multi_player_difficulty(regPlayerCount-1,0)),
          // 1 );
          if (tempConfig.race_id >= 0)
            font_bold_black.center_put(
                40, 105, 82, 125,
                misc.format(tempConfig.human_difficulty_rating), 1);
          else
            font_bold_black.center_put(
                40, 105, 82, 125,
                misc.format(tempConfig.monster_difficulty_rating), 1);
        }
      ...
      if (recvPtr) {
        if (((MpStructBase *)recvPtr)->msg_id == MPMSG_START_GAME) {
          ...
          case MPMSG_NEW_PLAYER: {
            // incorrect message, reject
            MpStructRefuseNewPlayer msgRefuse(
                from, MpStructRefuseNewPlayer::REFUSE_HOST_LOAD_GAME);
            mp_obj.send(from, &msgRefuse, sizeof(msgRefuse));
          } break;
          case MPMSG_LOAD_GAME_NEW_PLAYER:
            if (remote.is_host) {
              MpStructLoadGameNewPlayer *newPlayerMsg =
                  (MpStructLoadGameNewPlayer *)recvPtr;
              if (regPlayerCount >= MAX_NATION) {
                // reply refuse new player
                MpStructRefuseNewPlayer msgRefuse(
                    from, MpStructRefuseNewPlayer::REFUSE_SEAT_FULL);
                ...
                MpStructRefuseNewPlayer msgRefuse(
                    from, MpStructRefuseNewPlayer::REFUSE_VERSION);
                mp_obj.send(from, &msgRefuse, sizeof(msgRefuse));
              } else if (!(!nation_array.is_deleted(
                               newPlayerMsg->nation_recno) &&
                           nation_array[newPlayerMsg->nation_recno]
                                   ->color_scheme_id ==
                               newPlayerMsg->color_scheme_id &&
                           nation_array[newPlayerMsg->nation_recno]->race_id ==
                               newPlayerMsg->race_id &&
                           !colorAssigned[newPlayerMsg->color_scheme_id - 1] &&
                           newPlayerMsg->frame_count == sys.frame_count &&
                           newPlayerMsg->random_seed ==
                               misc.get_random_seed())) {
                // reply refuse new player
                MpStructRefuseNewPlayer msgRefuse(
                    from, MpStructRefuseNewPlayer::REFUSE_LOAD_GAME_UNSYNC);
                mp_obj.send(from, &msgRefuse, sizeof(msgRefuse));
              } else if (!mp_obj.auth_player(from, newPlayerMsg->name,
                                             newPlayerMsg->pass)) {
                // reply refuse new player
                MpStructRefuseNewPlayer msgRefuse(
                    from, MpStructRefuseNewPlayer::REFUSE_REASON_OTHER);
                ...
                MpStructAcceptColor msgAcceptColor(
                    from, newPlayerMsg->color_scheme_id);
                mp_obj.send(BROADCAST_PID, &msgAcceptColor,
                            sizeof(msgAcceptColor));
          ...
          case MPMSG_ACCEPT_NEW_PLAYER:
            ...
            break;
          case MPMSG_ACCEPT_COLOR: { // other player change color
            err_when(remote.is_host);
            int p;
            for (p = 0; p < regPlayerCount &&
                        regPlayerId[p] !=
      ...
      if (remote.is_host &&
        ...
        if (q >= regPlayerCount) // not all playerReadyFlag[p] = 1;
        {
          if (regPlayerCount != maxPlayer) {
            box.msg(
                text_game_menu.str_mp_lack_players(regPlayerCount, maxPlayer));
          } else {
            //						MpStructBase
            //msgStart(MPMSG_START_GAME); 						mp_obj.send(BROADCAST_PID, &msgStart,
            //sizeof(msgStart));
            retFlag = 1;
            break; // break while(1)
          }
      ...
      } else if ((keyCode = messageField.detect()) !=
                 0) // keyCode may be non-zero if after mapIdField.detect()
      {
        mRefreshFlag |= MGOPTION_OUT_MESSAGE;
        if (keyCode == KEY_RETURN && strlen(typingMsg.content) > 0) {
          if (!process_load_game_chat_command(messageField.input_field,
                                              &messageList)) {
            // add to own list
            messageList.linkin(&typingMsg);
  ...
  if (retFlag > 0) {
    ...
    if (remote.is_host) {
      ...
      for (p = 0; p < regPlayerCount; ++p) {
        ...
        // match nation color
        for (short nationRecno = 1; nationRecno <= nation_array.size();
             ++nationRecno) {
          Nation *nationPtr;
          if (!nation_array.is_deleted(nationRecno) &&
            ...
            ((MpStructNation *)setupString.reserve(sizeof(MpStructNation)))
                ->init(nationRecno, nationPtr->player_id,
                       nationPtr->color_scheme_id, nationPtr->race_id,
                       player->friendly_name_str(), 0,
                       0); // pass useGemStones and ranking as 0, since checking
                           // only
            playerCount++;
            break;
          }
      ...
      //			for( int d = 1; playerCount<MAX_NATION &&
      //mp_obj.get_player(d); ++d)
      //			{
      //				// ensure it is a valid player
      //				PlayerDesc *player =
      //mp_obj.get_player(d); 				PID_TYPE playerId = player ? player->pid() : 0;
      //				if( !playerId || !player->connecting )
      //					continue;
      //				for( p = 0; p < regPlayerCount &&
      //regPlayerId[p] != playerId; ++p); 				if( p >= regPlayerCount )
      //// not found 					continue;
      //
      //				// match nation color
      //				for( short nationRecno = 1; nationRecno
      //<= nation_array.size(); ++nationRecno)
      //				{
      //					Nation *nationPtr;
      //					if(
      //!nation_array.is_deleted(nationRecno) && (nationPtr =
      //nation_array[nationRecno])
      //						&& (nationPtr->is_own()
      //|| nationPtr->is_remote())
      //						&& playerColor[p] ==
      //nation_array[nationRecno]->color_scheme_id )
      //					{
      //						nationPtr->player_id =
      //playerId; 						nationPtr->next_frame_ready = 0;
      //						((MpStructNation
      //*)setupString.reserve(sizeof(MpStructNation)))->init( 							nationRecno,
      //nationPtr->player_id, 							nationPtr->color_scheme_id, nationPtr->race_id,
      //player->friendly_name_str()); 						playerCount++; 						break;
      //					}
      //				}
      //			}
      ...
      while (offset < recvLen) {
        ...
        case MPMSG_DECLARE_NATION: {
          ...
          if (nation_array.is_deleted(nationRecno) ||
            ...
            box.msg(
                text_game_menu
                    .str_mp_nation_info_incorrect()); // "Incorrect kingdom
          ...
          if (nationPtr->is_own() &&
              msgNation->dp_player_id == mp_obj.get_my_player_id()) {
            ownPlayerFound = 1;
          }
          nationPtr->next_frame_ready = 0;
          ...
          // nation_array.set_custom_both_name(nationRecno,
          // msgNation->player_name);
...
static void disp_service_button(ButtonCustom *button, int) {
  ...
  (button->pushed_flag ? font_bold_red : font_bold_black)
      .put(button->x1 + 16, button->y1 + 6, service_name_str, 1,
           button->x2 - 10);

  font_bld.put(button->x1 + 16, button->y1 + 26, service_name_str_long, 1,
               button->x2 - 10);

  if (button->pushed_flag) {
    vga.active_buf->rect(button->x1, button->y1, button->x2, button->y2, 2,
                         V_BLACK);
  }
}
...
static void i_disp_text_button(ButtonCustom *button, int repaintBody) {
  ...
  // top center align
  fontPtr->center_put(button->x1, button->y1, button->x2,
                      button->y1 + fontPtr->font_height - 1,
...
static void i_disp_race_button(ButtonCustom *button, int repaintBody) {
  ...
  // top center align
  fontPtr->center_put(
      button->x1, button->y1, button->x2, button->y1 + fontPtr->font_height - 1,
...
static void i_disp_tick_button(ButtonCustom *button, int repaintBody) {
  if (!button->pushed_flag) {
    ...
    vga.active_buf->put_bitmap_trans_decompress(button->x1 + 4, button->y1 + 4,
                                                image_button.read("TICK"));
    vga.active_buf->d3_panel_down(button->x1, button->y1, button->x2,
                                  button->y2, 4, 0);
  }
}
    ...
    case 'r': {
      ...
      for (int i = 1; i <= nation_array.size(); ++i) {
        ...
        str = text_game_menu.str_mp_ranking(
            nation_array[i]->nation_name(), nation_array[i]->color_scheme_id,
