...
enum {
  ...
  CURSOR_TRIGGER_EXPLODE, // special of CURSOR_UNIT_C, when pointing at an
                          // explosive cart
  CURSOR_CAPTURE_FIRM,
};
...
public:
  ...
  char processing_flag; // whether process() is being run or not, prevent nested
                        // call by interrupt
  ...
  short frame_top_save_scr[MAX_VGA_WIDTH +
                           2]; // for saving the nation selection frame
  short frame_bottom_save_scr[MAX_VGA_WIDTH +
                              2]; // +4 is for the width & height info
