...
#define DISP_NEWS_DAYS                                                         \
  60 // how long a news should stay on the screen before it disappears
#define DISP_NEWS_COUNT                                                        \
  5 // maximum no. of news message displayed on the screen at a time
...
enum {
  ...
  NEWS_FIRM_ATTACKED,  // when your firm is first come under attacked
  NEWS_FIRM_CAPTURED,  // when your firm is taken over by another nation or you
                       // take over the firm of another nation
  NEWS_TOWN_DESTROYED, // when the last unit in the town is killed by an enemy
  NEWS_TOWN_ATTACKED,  // when your town is first come under attacked
  ...
  NEWS_UNIT_BETRAY, // your unit betray or other nation's betray and turn
                    // towards you
  NEWS_UNIT_ASSASSINATED,
  NEWS_ASSASSINATOR_CAUGHT,
  NEWS_GENERAL_DIE,
  NEWS_GENERAL_FORM_NEW_NATION,
  NEWS_RAW_EXHAUST,
  ...
  NEWS_EV_FINISH_TECH,
  NEWS_EV_NEW_TECH,
  NEWS_EV_FOOD_DESTROYED,
  NEWS_EV_UNIT_DIE,
  NEWS_EV_SOLDIER_DIE,
  NEWS_EV_ITEM_FOUND,
  NEWS_EV_TOWN_PEOPLE_KILLED,
  NEWS_EV_FACTORY_DESTROYED,
  NEWS_EV_MINE_DESTROYED,
  NEWS_EV_INN_DESTROYED,
  NEWS_TORNADO_HIT,
  NEWS_BLUEPRINT_ACQUIRED,
  // ##### begin Gilbert 16/4 #######//
  NEWS_MAGIC_TOWER_ATTACK,
  // ##### end Gilbert 16/4 #######//
  NEWS_CHAT_MSG,
  NEWS_MULTI_RETIRE,
...
public:
  ...
  char type; // news type   // type may be > NEWS_TYPE_NUM, for indicating that
             // the news has been displayed in the stock window, do display it
             // on the newspaper again
  char news_type() { return type % NEWS_TYPE_NUM; }

  int32_t news_date; // date of the news

  char nation_color1; // nation color, can't use nation_recno directly, because
                      // it may bankrupt one day
  char nation_color2;
  char nation_race_id1;
  char nation_race_id2;
  int nation_recno1; // nation res. id of the nation that generate the news
  int nation_recno2; // if the news is related to two nations (e.g. one nation
                     // buys the stock of another nation)
  ...
  short loc_type_para;
  WORD
      loc_type_para2; // must use WORD as it will be used to store unit name id.
...
public:
  ...
  void set_loc(int xLoc, int yLoc, int locType, int locTypePara = 0,
               int locTypePara2 = 0);
  int is_loc_valid();
  ...
  void firm_worn_out();
  void event_finish_tech();
  void event_new_tech();
  void event_food_destroyed();
  void event_unit_die();
  void event_soldier_die();
  void event_item_found();
  void event_town_people_killed();
  void event_factory_destroyed();
  void event_mine_destroyed();
  void event_inn_destroyed();
  void tornado_hit();
  void blueprint_acquired();
  // ##### begin Gilbert 16/4 #######//
  void magic_tower_attack();
  // ##### end Gilbert 16/4 #######//
  void chat_msg();
  void multi_retire();
...
public:
  ...
  News *add_news(int newsId, int newsType, int nationRecno = 0,
                 int nationRecno2 = 0, int forceAdd = 0);
  void remove(int newsId, int shortPara1 = -1, int nationRecno1 = -1);

  //------ functions for adding news -------//

  int diplomacy(int talkMsgRecno);
  void town_rebel(int townRecno, int rebelCount);
  void migrate(int srcTownRecno, int desTownRecno, int raceId,
               int migratedCount, int firmRecno = 0);
  ...
  void firm_destroyed(int firmRecno, BaseObj *attackObj);
  void firm_attacked(int firmRecno, BaseObj *attackObj);
  void firm_captured(int firmRecno, int takeoverNationRecno, int spyTakeover);
  void town_destroyed(char *townName, int xLoc, int yLoc, BaseObj *attackObj);
  void town_attacked(char *townName, int xLoc, int yLoc, BaseObj *attackObj);
  void town_abandoned(int townRecno);
  void town_surrendered(int townRecno, int toNationRecno);

  void firm_idle(int firmRecno);
  ...
  void assassinator_caught(int spyRecno, int targetRankId, int targetRaceId);
  void general_die(int unitRecno);
  void general_form_new_nation(int unitRecno);
  void raw_exhaust(int rawId, int xLoc, int yLoc, int firmBuildId);
  void tech_researched(int techId, int techVersion);
  void lightning_damage(int xLoc, int yLoc, int objectId, int recno,
                        int objectDie);
  void earthquake_damage(int unitDamage, int unitDie, int townDamage,
  ...
  void event_finish_tech(int techId, int techVersion);
  void event_new_tech(int techId, int techVersion);
  void event_food_destroyed(int foodLost);
  void event_unit_die(int unitRecno);
  void event_soldier_die(int firmRecno, int peopleKilled);
  void event_item_found(int townRecno);
  void event_town_people_killed(int townRecno, int peopleKilled);
  void event_factory_destroyed(int firmRecno, int peopleKilled);
  void event_mine_destroyed(int firmRecno, int peopleKilled);
  void event_inn_destroyed(int firmRecno);
  void tornado_hit(int tornadoRecno);
  void blueprint_acquired(int acquiredNationRecno, int techId);
  // ##### begin Gilbert 16/4 #######//
  void magic_tower_attack(int attackerBaseObjRecno, int magicId,
