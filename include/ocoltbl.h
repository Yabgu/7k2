...
struct PalDesc {
  ...
  PalDesc(void *palPtr, int pitch, int size, int bitWidth,
          BYTE *rPtr = (unsigned char *)0, int rCount = 0)
      : pal((BYTE *)palPtr), pal_pitch(pitch), pal_size(size),
        bit_width(bitWidth), reserved_color(rPtr), reserved_count(rCount) {}
...
struct PalDescFile : public PalDesc {
  PalDescFile(char *fileName, int headerSize, int pitch, int size, int bitWidth,
              BYTE *rPtr = (unsigned char *)0, int rCount = 0);
  ~PalDescFile();
};
...
public:
  ...
  void generate_table(int absScale, PalDesc &palD,
                      RGBColor (*fp)(RGBColor, int, int));
  void generate_table_fast(int absScale, PalDesc &palD,
                           RGBColor (*fp)(RGBColor, int, int));
