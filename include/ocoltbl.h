...
struct PalDesc
{
    ...
    PalDesc(void *palPtr, int pitch, int size, int bitWidth, BYTE *rPtr = (unsigned char *)0, int rCount = 0)
        : pal((BYTE *)palPtr), pal_pitch(pitch), pal_size(size), bit_width(bitWidth), reserved_color(rPtr),
          reserved_count(rCount)
    {
    }

    BYTE normalize(BYTE c)
    {
        return bit_width == 8 ? c : (c << (8 - bit_width)) + (1 << (7 - bit_width));
    }
    BYTE red(int n)
    {
        return normalize(pal[pal_pitch * n]);
    }
    BYTE green(int n)
    {
        return normalize(pal[pal_pitch * n + 1]);
    }
    BYTE blue(int n)
    {
        return normalize(pal[pal_pitch * n + 2]);
    }
    RGBColor get_rgb(int n)
    {
        return RGBColor(red(n), green(n), blue(n));
    }
    int is_reserved(BYTE c, int &reservedIndex)
    {
        if (reserved_color && reservedIndex < reserved_count && c == reserved_color[reservedIndex])
        {
            reservedIndex++;
            return 1;
        }
...
{
    PalDescFile(char *fileName, int headerSize, int pitch, int size, int bitWidth, BYTE *rPtr = (unsigned char *)0,
                int rCount = 0);
    ~PalDescFile();
};
...
class ColorTable
{
  ...
  public:
    ...
    {
        return remap_table_array + scale + abs_scale;
    }
