...
//------- Define constant -----------//
//
// Note: if there is a leader unit in the group, it is always the first unit in
// the array
//
...
public:
  ...
  void assign(int destXLoc, int destYLoc, int targetMobileType,
              char remoteAction, int actionNationRecno = -1,
              bool forceMoveFlag = false);
  void exe_assign(int destXLoc, int destYLoc, int targetMobileType,
  ...
  void right_click(int xLoc, int yLoc, char remoteAction, bool waypt,
                   bool shiftAtk, bool forceMove);
...
private:
  ...
  //--------- other functions ----------//
  void cluster_units_for_path_reuse(int destX, int destY, int centerX,
                                    int centerY);
  void cluster_units_for_attack(int targetRecno, int targetLocX, int targetLocY,
                                int targetWidth, int targetHeight);

  void group_by_mobile_type();
  void run_action(int destXLoc, int destYLoc, int orderId, int orderPara,
                  int actionNationRecno = -1, bool forceMoveFlag = false);
};

extern UnitGroup unit_group, unit_group_land, unit_group_air,
    unit_group_array[9], unit_group_remote;
