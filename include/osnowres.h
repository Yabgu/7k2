...
struct SnowInfo
{
    ...
    int rand_next(unsigned rand)
    {
        return is_leaf() ? snow_map_id : next_file[rand % next_count]->snow_map_id;
    }
    int rand_prev(unsigned rand)
    {
        return is_root() ? snow_map_id : prev_file[rand % prev_count]->snow_map_id;
    }
    SnowInfo *rand_next_ptr(unsigned rand)
    {
        return is_leaf() ? this : next_file[rand % next_count];
    }
    SnowInfo *rand_prev_ptr(unsigned rand)
    {
        return is_root() ? NULL : prev_file[rand % prev_count];
    }
    short bitmap_width()
    {
        return *(short *)bitmap_ptr;
    }
    short bitmap_height()
    {
        return *(((short *)bitmap_ptr) + 1);
    }
...
#else
    SnowInfo *operator[](int snowMapId)
    {
