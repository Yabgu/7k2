...
struct SpriteRec
{
    ...
    char max_height[SPRITE_MAX_HEIGHT_LEN];
    ...
    char bullet_delay[DAMAGE_LEN];

    char max_bitmap_height[SPRITE_MAX_HEIGHT_LEN];
};
...
struct SpriteAttack
{
    short first_frame_recno; // first frame recno to frame_array.
    char frame_count;        // no. of frames in the movement
    char attack_delay;       // no. of frames should be delayed between attack motions.
                             // (i.e. when one motion is complete, it will delay
                             // <delay_frames> before move on to the next action motion
                             // in the cycle
};
...
struct SpriteInfo
{
  public:
    ...
    char remap_bitmap_flag;
    BYTE *pal_ptr;   // uncoded palette pointer, in the sprite palette resource
    short *palw_ptr; // coded palette pointer, owned by SpriteInfo
