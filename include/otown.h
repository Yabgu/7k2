...
#define MIN_RECRUIT_LOYALTY                                                    \
  30 // only when loyalty > this, the unit can be recruited
#define MIN_NATION_DEFEND_LOYALTY                                              \
  30 // if the loyalty of the town people > this, they will defense against the
     // attack
#define MIN_INDEPENDENT_DEFEND_LOYALTY 30
#define SURRENDER_LOYALTY                                                      \
  30 // when the loyalty of the town is <= this, and being attacked by a nation,
     // the town will surrender
#define SURRENDER_RESISTANCE 30
#define REBEL_LOYALTY 29 // if loyalty <= REBEL_LOYALTY, the unit may rebel

#define INDEPENDENT_LINK_RESISTANCE                                            \
  50 // only if the resistance of an independent town is lower than this, the
     // town will enable its link with a foreign firm

#define TAX_PER_UNIT 3 // amount of tax can be collected from each unit
#define COLLECT_TAX_LOYALTY_DECREASE                                           \
  10 // Reduce 10 loyalty points per tax collection
...
#define RECEIVED_HIT_PER_KILL                                                  \
  200 // no. of received hits will result in one death of town people
...
public:
  ...
  short town_name_id;
  //	char  town_name[TOWN_NAME_LEN+1];		// the user will be able to
  ...
  short slot_object_id_array[MAX_TOWN_LAYOUT_SLOT]; // the race id. of each slot
                                                    // building
  ...
  float
      received_hit_count; // no. of received hit by attackers, when this >
                          // RECEIVED_HIT_PER_KILL, a town people will be killed
  ...
  char independ_town_nation_relation; // each bit n is high representing this
                                      // independent town will attack nation n.
  ...
  char has_linked_own_camp; // whether the town has linked military camps of the
                            // same nation
  char has_linked_enemy_camp; // whether the town has linked military camps of
                              // the same nation

  //----------- AI vars ------------//

  char ai_town;
  char ai_link_checked; // AI check firm and town locatin by links, disable
                        // checking by setting this parameter to 1

  char town_combat_level; // combat level of the people in this town
  char has_product_supply[MAX_PRODUCT]; // whether this town has the supply of
                                        // these products
...
public:
  ...
  void init(int nationRecno, int raceId, int xLoc, int yLoc,
            int builderRecno = 0);
  void deinit();
  ...
  int can_train();
  int can_migrate(int destTownRecno,
                  int migrateNow = 0); // if not called by Town::mirgate, don't
                                       // set migrateNow to TRUE
  ...
  int mobilize_wagon(int decPop, int wagonPopulation, short spyArrayCount,
                     short *spyArray);
  ...
  void toggle_firm_link(int linkId, int toggleFlag, char remoteAction,
                        int setBoth = 0);
  void toggle_town_link(int linkId, int toggleFlag, char remoteAction,
                        int setBoth = 0);
  ...
  int protection_needed(); // an index from 0 to 100 indicating the military
...
public:
  ...
  int race_wander_pop_array[MAX_RACE]; // no. of wandering people of each race.
                                       // They are people for setting up
                                       // independent towns later
...
public:
  ...
  int add_town(int nationRecno, int raceId, int xLoc, int yLoc,
               int builderRecno = 0, bool noCost = false);
  ...
  int think_town_loc(int maxTries, int &retXLoc, int &retYLoc, int rangeX1 = -1,
                     int rangeY1 = -1, int rangeX2 = -1, int rangeY2 = -1);
