...
struct DPPlayer
{
    ...
    char connecting; // initially set to 1,
                     // clear after DPSYS_DESTROYPLAYERORGROUP received from
                     // DirectPlay
...
class MultiPlayerDP
{
  ...
  public:
    MultiPlayerDP()
        : service_providers(sizeof(DPServiceProvider), 10), current_sessions(sizeof(DPSessionDesc), 10),
          player_pool(sizeof(DPPlayer), 8), init_flag(0){};
    ~MultiPlayerDP(){};
    void pre_init(){};
    ...
    DPServiceProvider *get_service_provider(int i)
    {
        return NULL;
    ...
    int create_player(char *friendlyName, char *formalName, LPVOID lpData = NULL, DWORD dataSize = 0, DWORD flags = 0)
    {
        return 0;
    }
    ...
    void before_receive()
    {
    } // dummy function to compatible with IMMPLAY, call before receive
    void after_send()
    {
    } // dummy function to compatible with IMMPLAY, call after send
