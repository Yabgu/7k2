...
protected:
  ...
  short target_monster_nation_recno; // the target Fryhtan nation of the
                                     // Human-Fryhtan joint attack game
  short new_kingdom_nation_recno;

  //---- common campaign vars ----//

  short hero_debut_state_count; // The hero debut event will happen when the
                                // number of player states drops to this number
  char fryhtan_joint_attack_reward;
  char which_edge; // for determining which edge the reinforcement should come
                   // from
  char defeated_eastern_empire_invasion; // this is true if the player has
                                         // defeated an invasion from the
                                         // eastern empire
  short mega_tech_id;
  ...
  char disband_fryhtan_reply; // 1 - agree, -1 - refuse, 2-refused, but the
                              // Fryhtan has revolted and form a new kingdom. So
                              // disband_fryhtan_reply is no longer useful.
  char fryhtan_surrendered_state_recno; // a state that Fryhtans have
                                        // surrendered to you.
...
private:
  ...
  int get_triangle_states1(int &humanStateRecno, int &monsterStateRecno1,
                           int &monsterStateRecno2);
  int get_triangle_states2(int &humanStateRecno, int &eastStateRecno,
