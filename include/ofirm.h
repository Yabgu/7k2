...
{
    ASSASSINATE_FAIL,
    ASSASSINATE_SUCCEED_AT_LARGE, // assassination succeed and the assassinator is
                                  // at large
    ASSASSINATE_SUCCEED_KILLED,   // assassination succeed and the assassinator is
                                  // caught and executed
};
...
class Firm : public Place
{
  public:
    ...
    short upgrading_firm_id; // if this firm is being upgraded, this is the id. of
                             // the upgrade firm.
    short upgrade_completion_percent;

    int can_sell()
    {
        return hit_points >= (int)max_hit_points() * CAN_SELL_HIT_POINTS_PERCENT / 100;
    }

    //-------- firm name vars ---------//

    short closest_town_name_id; // name id. of the town that is closest to this
                                // firm when it is built
    ...
    char ai_processed; // some ai actions are processed once only in the processing
                       // day. To prevent multiple checking in the processing day
    char ai_status;
    char ai_link_checked; // AI checks firms and towns location by links, disable
                          // checking by setting this parameter to 1
    char ai_sell_flag;    // this is true if the AI has queued the command to sell
                          // this firm
  ...
  public:
    ...
    virtual int can_set_active_link(int townRecno)
    {
        return 0;
    }
    virtual void set_active_link(int townRecno,
                                 char remoteAction); // 0 to clear; may not change
    ...
    virtual void free_all_people()
    {
        ;
    } // call when the firm is destroyed
    ...
    virtual int builder_stay_after_construction()
    {
        return 0;
    } // called by process_construction() after the building is completely built.
    // Derived class like FirmCamp will have a function body other than return
    // 0.

    // -------- rally point ----------//

    virtual int can_set_rally_point(int destBaseObjRecno = 0)
    {
        return 0;
    }
    ...
    virtual FirmWork *cast_to_FirmWork()
    {
        return 0;
    }
    ...
    virtual FirmCamp *cast_to_FirmCamp()
    {
        return 0;
    }
    virtual FirmFort *cast_to_FirmFort()
    {
        return 0;
    }
    virtual FirmMine *cast_to_FirmMine()
    {
        return 0;
    }
    virtual FirmFactory *cast_to_FirmFactory()
    {
        return 0;
    }
    virtual FirmMarket *cast_to_FirmMarket()
    {
        return 0;
    }
    virtual FirmInn *cast_to_FirmInn()
    {
        return 0;
    }
    virtual FirmResearch *cast_to_FirmResearch()
    {
        return 0;
    }
    virtual FirmWar *cast_to_FirmWar()
    {
        return 0;
    }
    virtual FirmTrain *cast_to_FirmTrain()
    {
        return 0;
    }
    virtual FirmSpy *cast_to_FirmSpy()
    {
        return 0;
    }
    ...
    virtual FirmLair *cast_to_FirmLair()
    {
        return 0;
    }
    ...
    virtual FirmMonsterAlchemy *cast_to_FirmMonsterAlchemy()
    {
        return 0;
    }
    virtual FirmLishorr *cast_to_FirmLishorr()
    {
        return 0;
    }
    ...
    virtual FirmAnimal *cast_to_FirmAnimal()
    {
        return 0;
    }
    virtual FirmIncubator *cast_to_FirmIncubator()
    {
        return 0;
    }
    virtual FirmMagic *cast_to_FirmMagic()
    {
        return 0;
    }
    virtual FirmOffensive *cast_to_FirmOffensive()
    {
        return 0;
    }
    virtual FirmOffensive2 *cast_to_FirmOffensive2()
    {
        return 0;
    }
