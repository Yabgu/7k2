...
#define CAN_SELL_HIT_POINTS_PERCENT                                            \
  80 // only when the firm's hit points is higher than this can the firm be sold
...
public:
  ...
  int can_sell() {
    return hit_points >=
           (int)max_hit_points() * CAN_SELL_HIT_POINTS_PERCENT / 100;
  }
  ...
  char
      ai_processed; // some ai actions are processed once only in the processing
...
public:
  ...
  virtual void init(int xLoc, int yLoc, int nationRecno, int firmId,
  ...
  int scan_best_recruitable_town(short *townRecnoArray, int raceId = 0,
                                 int requiredLinkedFirmId = 0);
  int can_train_from_town(int townRecno, int requiredLinkedFirmId,
                          int raceId = 0);
  ...
  virtual void think_linked_town_change_nation(int linkedTownRecno,
                                               int oldNationRecno,
                                               int newNationRecno);
