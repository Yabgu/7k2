...
#define MAX_MONSTER_GENERAL_IN_FIRM                                            \
  8                               // maximum no. of monster generals in a firm
#define MAX_SOLDIER_PER_GENERAL 8 // maximum soldier can be led by a general
#define MAX_MONSTER_IN_FIRM                                                    \
  (1 + MAX_MONSTER_GENERAL_IN_FIRM * (1 + MAX_SOLDIER_PER_GENERAL))

#define MONSTER_ATTACK_NEIGHBOR_RANGE                                          \
  6 // it will attack any firms/towns within 3 location away from it
#define MONSTER_EXPAND_RANGE                                                   \
  8 // the new firm built should be within this distance with the current firm
...
public:
  BYTE monster_id;
  BYTE unit_id;
  short mobile_unit_recno; // unit recno of this monster when it is a mobile
                           // unit this is only used as a reference for soldiers
                           // to find their leaders
  short combat_level;
  short hit_points;
  short max_hit_points;

  BYTE soldier_monster_id; // monster id. of the soldiers led by this monster
                           // general
...
public:
  enum {
    MAX_WAITING_SOLDIER = 30
  }; // maximum no. of soldiers queued in the monster firm waiting for their
     // generals
  ...
  int total_defender() {
    return defending_king_count + defending_general_count +
  ...
  short waiting_soldier_array[MAX_WAITING_SOLDIER]; // the unit recno of their
                                                    // generals are kept here

  char monster_nation_relation; // each bit n is high representing this
                                // independent town will attack nation n.
  short defend_target_recno; // used in defend mode, store recno of the latest
                             // unit attacking this firm
...
public:
  ...
  FirmMonster *cast_to_FirmMonster() { return this; }
  ...
  int can_assign_monster(int unitRecno); // call Unit::firm_can_assign instead
  ...
  void being_attacked(int attackerUnitRecno, float damage);
...
private:
  ...
  int mobilize_monster(int monsterId, int rankId, int combatLevel,
                       int hitPoints = 0);
