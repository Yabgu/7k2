...
public:
  ...
  int cur_cargo_width,
      cur_cargo_height; // cur_x2_loc = cur_x_loc + cur_cargo_width
  ...
  int determinant_shift;  // log2(loc_x_width * loc_y_height - loc_x_height *
                          // loc_y_width);
  int center_x, center_y; // location different vector from center of window to
                          // top left of window
  ...
  char own_matrix;
  char cur_cargo_rect_type; // cur_cargo_width/height in 0 = rectangle of x/y
                            // loc or 1 = rectangle of column/row
  char disp_rect_type;      // disp_x/y_loc in 0 = rectangle of x/y loc or 1 =
                            // rectangle of column/row
...
public:
  ...
  void init(int, int, int, int, int, int, float, float, float, float, char,
            int);
  ...
  virtual int get_detect_location(int scrnX, int scrnY, int *locX, int *locY,
                                  char *mobileType);
  // if outside the screen, return 0

  virtual int get_close_location(int scrnX, int scrnY, int *locX, int *locY,
                                 char *mobileType);
  ...
  int calc_loc_pos_x(
      int xLoc,
      int yLoc); // loc x/y to window position (relative to image_x1, image_y1)
