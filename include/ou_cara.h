...
#define MAX_CARAVAN_WAIT_TERM                                                  \
  8 // no. of term the caravan is in the market to upload/download cargo
#define POPULATION_PER_CARAVAN                                                 \
  10 // the population size for supporting one caravan
...
enum {
  NO_STOP_DEFINED = 0, // used for journey_status
  ON_WAY_TO_FIRM = 1,
  SURROUND_FIRM,
  INSIDE_FIRM,
};
...
public:
  ...
  char journey_status; // 1 for not unload but can up load, 2 for unload but not
                       // up load
  char dest_stop_id; // destination stop id. the stop which the train currently
                     // is moving towards
  ...
  CaravanStop stop_array[MAX_STOP_FOR_CARAVAN]; // an array of firm_recno
                                                // telling train stop stations

  int last_set_stop_date;   // the date when stops were last set.
  int last_load_goods_date; // the last date when the caravan load goods from a
                            // firm
...
private:
  ...
  int appear_in_firm_surround(
      int &xLoc, int &yLoc,
      Firm *firmPtr); // select a suitable location to leave the stop
};
#pragma pack()
