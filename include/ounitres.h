...
struct UnitRec {
  enum {
    ...
    BUILD_DAYS_LEN = 3,
    COST_LEN = 3,
    CARRY_CAPACITY_LEN = 3,
    FREE_WEAPON_COUNT_LEN = 1,
    FILE_NAME_LEN = 8,
    BITMAP_PTR_LEN = 4,
    SPRITE_ID_LEN = 3,
    RACE_ID_LEN = 3,
  ...
  char weapon_power; // an index from 1 to 9 indicating the powerfulness of the
                     // weapon
...
struct UnitInfo {
  ...
  char free_weapon_count; // only for ships. It's the no. of free weapons can be
  ...
  char nation_tech_level_array[MAX_NATION]; // each nation's tech level on this
                                            // unit
  ...
  short nation_unit_count_array[MAX_NATION]; // mobile units + soldiers in
                                             // camps, not including workers and
                                             // prayers in bases
...
public:
  int is_loaded(); // whether the sprite data of this unit is in the memory or
                   // not
...
public:
  ...
  const char *unit_name(int flags, int unitId, int nameId, int heroId,
...
private:
  ...
  UnitClassInfo *get_class_info(
      int unitClassId); // use UnitInfo::class_info instead of get_class_info
};
