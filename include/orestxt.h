...
private:
  enum {
    DEF_TXT_BUF_SIZE = 5120
  }; // default buffer size : 5K
     // this size should be large enough for average text unit, also we don't
     // need realloc very frequently
  char init_flag;
  char res_file_name[MAX_PATH + 1]; // e.g. text file name, e.g. BOOK.RTX
                                    // we need to keep the file name, because
                                    // the file is opened only when reading the
                                    // content, and close when not necessary
