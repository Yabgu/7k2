...
class PlayerProfile
{
  public:
    ...
    {
        PLAYER_NAME_LEN = 20
    }; // should be as same as NationBase::KING_NAME_LEN, and
    ...
    WORD gem_stones;
    BYTE next_login_pass_id; // a random number to check against server db same
                             // field, guard against loading old profile
    BYTE demo_try_count;     // increase in demo version, number of new/load lobbied
                             // mp game started
  ...
  public:
    ...
    static int load_count_profiles(PlayerProfile *profileArray,
                                   int maxLoad); // null to count
