...
class Battle {
private:
  short placement_range_x1, placement_range_y1, placement_range_x2,
      placement_range_y2; // the range where new objects should be placed
...
public:
  ...
  void create_road();
  int create_unit(int nationRecno, int unitId, int rankId, int nextXLoc1 = -1,
                  int nextYLoc1 = -1, int nextXLoc2 = -1, int nextYLoc2 = -1);
  int create_town(int nationRecno, int raceId, int xLoc, int yLoc,
                  int initPop = 0);
  int create_town_next_to_place(Place *placePtr, int nationRecno, int raceId,
                                int initPop = 0);
  int create_town_random_loc(int nationRecno, int raceId, int &xLoc, int &yLoc,
                             int initPop = 0);
  int create_firm(int xLoc, int yLoc, int nationRecno, int firmId,
                  int firmRaceId);
  int create_firm_next_to_place(Place *placePtr, int firmId, int firmRaceId,
                                int nationRecno = -1, short distance = 0);
