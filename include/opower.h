...
enum ScreenObjectType {
  ...
  SCREEN_OBJECT_SPY_UNIT, // our spy, stealthed to other nation, selected object
                          // only
  SCREEN_OBJECT_SPY_GROUP, // our spy, stealthed to other nation, selected
                           // object only
...
public:
  ...
  int choose_cursor(int scrnX, int scrnY, ScreenObjectType selectedObjectType,
...
public:
  ...
  Location *test_detect(int curX, int curY, char *mobileType = NULL,
                        int *retXLoc = NULL, int *retYLoc = NULL);
  int detect_select(int selX1, int selY1, int selX2, int selY2, int recallGroup,
                    int shiftSelect);
