...
struct MagicInfo {
  ...
  int bullet_speed;
  int cast_radius;
  int live_point_extra_range; // if cast outside cast_range, cost on
                              // min_live_point per distance
  int cost_extra_range; // if cast outside cast_range, cost on min_live_point
                        // per distance
};
#pragma pack()
...
public:
  ...
  void init(int xLoc, int yLoc, int nationRecno, int firmId, char *buildCode,
            short builderRecno);
  void deinit();
  ...
  int cast_magic(int magicId, int targetXLoc, int targetYLoc,
                 int targetUnitRecno, char remoteAction);
  int process_delayed_magic(int magicId, int targetXLoc, int targetYLoc,
                            int targetUnitRecno);
  int calc_live_point_cost(int magicId, int targetXLoc, int targetYLoc,
                           int targetUnitRecno);
  int calc_cash_cost(int magicId, int targetXLoc, int targetYLoc,
                     int targetUnitRecno);
