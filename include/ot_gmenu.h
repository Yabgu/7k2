...
public:
  ...
  char *
  str_profile_s2(); // if your name is not here, click on 'new player' below
  char *str_profile_new();     // Please enter your name
  char *str_profile_del();     // Are you certain that you wish to delete this
                               // persno from the list?
  ...
  const char *str_mp_service_provider_name(
      int); // 1=IPX, 2=TCP/IP, 3=MODEM, 4=SERIAL, 5=IM online
  ...
  char *str_mp_ranking(char *nationName, char colorScheme, int, int, int, int,
                       int, int);
