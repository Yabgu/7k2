...
enum {
  COMMON_DATA_BUF_SIZE = 64 * 1024
}; // keep a 64K common buffer for all temporary memory storage like reading
   // files temporarily into memory
...
enum { VERSION_FULL, VERSION_DEMO, VERSION_MULTIPLAYER_ONLY };
...
public:
  ...
  char signal_exit_flag; //	1-quit to windows
                         // 2-quit to main menu
                         // 3-quit a campaign game only, but still remain in the
                         // campaign 4-this is set after loading a game for
                         // exiting the current main_loop() so that it can enter
                         // the correct main_loop() (campaign & game) later.

  char quick_exit_flag() { return signal_exit_flag; }

  char need_redraw_flag; // set to 1 if task switched back. After redraw, clear
                         // it
  ...
  int day_frame_count;
  DWORD next_frame_time; // next frame's time for maintaining a specific game
                         // speed

  //----- multiplayer vars ----//

  DWORD frame_count;   // frame count, for is_sync_frame only
  char is_sync_frame;  // whether sync should take place at the current frame
                       // (for handling one sync per n frames)
...
public:
  ...
  void set_view_mode(int viewMode, int viewingNationRecno = 0,
                     int viewingSpyRecno = 0);
