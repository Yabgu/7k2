...
class FileReaderVisitor
{
  ...
  public:
    ...
    template <typename FileT, typename MemT> bool visit(MemT *v)
    {
        return this->reader->read<FileT, MemT>(v);
    }

    template <typename T> bool visit(T **v)
    {
        return this->reader->read<T>(v);
    }

    template <typename FileT, typename MemT> bool visit_array(MemT *array, size_t len)
    {
...
{
template <typename FileT, typename MemT, typename Visitor> bool visit(Visitor *vis, MemT *val)
{
    return vis->template visit<FileT, MemT>(val);
}

template <typename FileT, typename MemT, typename Visitor> bool visit_array(Visitor *vis, MemT *array, size_t len)
{
    return vis->template visit_array<FileT, MemT>(array, len);
}

template <typename T, typename Visitor> bool visit_pointer(Visitor *vis, T **ptr)
{
    return vis->visit(ptr);
}

template <typename T>
bool write_with_record_size(File *file, T *obj, void (*visit_obj)(FileWriterVisitor *v, T *obj), uint16_t rec_size)
{
    FileWriter w;
    FileWriterVisitor v;
...
template <typename T>
bool read_with_record_size(File *file, T *obj, void (*visit_obj)(FileReaderVisitor *v, T *obj),
