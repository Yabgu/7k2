...
struct RockInfo
{
    ...
    //	short block_offset[MAX_ROCK_HEIGHT][MAX_ROCK_WIDTH];	// make
    // RockRes::locate_block faster
    short *palw_ptr; // pointing to part of the res_pal
    int valid_terrain(char terrainType)
    {
...
struct RockBitmapInfo
{
    ...
    short width()
    {
        return ((Bitmap *)bitmap_ptr)->get_width();
    }
    short height()
    {
        return ((Bitmap *)bitmap_ptr)->get_height();
    }
    void draw(RockInfo *rockInfo, short xLoc, short yLoc, short curZ);
    char choose_next(long path)
    {
        return path ? next_frame : alt_next;
    }
};
...
class RockRes
{
  ...
  public:
    ...
    // return rockId
    short search(const char *rockTypes, short minWidth, short maxWidth, short minHeight, short maxHeight,
                 int animatedFlag = -1, int findFirst = 0, char terrainType = 0);
