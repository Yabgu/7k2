...
struct GodRec {
  enum {
    ...
    UNIT_CODE_LEN = 8,
    PRAY_POINTS_LEN = 3,
    CAST_POWER_RANGE_LEN = 3,
    RACE_ID_LEN = 3,
    UNIT_ID_LEN = 3,
    OUT_EFFECT_CODE_LEN = 8,
    OUT_EFFECT_ID_LEN = 3
  };
...
public:
  ...
  char race_id;
  char unit_id;
  short exist_pray_points; // pray points consumption for the god to exist for
                           // 100 frames
  short power_pray_points; // pray points consumption for each casting of its
                           // power
  char can_cast_power;     // whether this god creature can cast power or not
  ...
  int is_nation_know(int nationRecno) {
    return nation_know_array[nationRecno - 1];
  }

  short prayer_count[MAX_NATION];
  int nation_prayer_count(int nationRecno) {
    return prayer_count[nationRecno - 1];
  }
