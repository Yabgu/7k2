...
public:
  ...
  int get_close_location(int scrnX, int scrnY, int *locX, int *locY,
                         char *mobileType);
...
public:
  DynArray
      land_disp_sort_array; // an array for displaying objects in a sorted order
...
public:
  ...
  void put_bitmap_remap_clip(int x, int y, char *bitmapPtr,
  ...
  void put_bitmap_offset(int curX, int curY, int curZ, char *bitmapPtr,
                         ...
                         short *colorRemapTable = NULL, int hMirror = 0,
                         int compressedFlag = 1);

  void put_bitmap(int curX, int curY, int curZ, char *bitmapPtr,
                  short *colorRemapTable = NULL, int hMirror = 0,
                  int compressedFlag = 1) {
    put_bitmap_offset(curX, curY, curZ, bitmapPtr, 0, 0, colorRemapTable,
                      hMirror, compressedFlag);
  }

  void put_bitmapW_offset(int curX, int curY, int curZ, short *bitmapPtr,
                          int offsetX, int offsetY, int hMirror = 0,
                          int compressedFlag = 0);
  ...
  void put_center_text(int curX, int curY, int curZ, const char *str,
                       char black = 1, Font *font_ptr = &font_zoom);
