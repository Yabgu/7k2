...
class MapMatrix : public Matrix
{
  public:
    char last_map_mode;
    char map_mode;
    char power_mode; // 1-also display power regions on the zoom map, 0-only
                     // display power regions on the mini map
...
class ZoomMatrix : public Matrix
{
  public:
    ...
    short lightning_x1, lightning_y1, lightning_x2,
        lightning_y2; // save on save game

    BYTE *last_draw_state; // one byte per location, 0=blacken, 1=fog, 2=fully
                           // visible
  ...
  public:
    ...
    int get_detect_location(int scrnX, int scrnY, int *locX, int *locY,
                            char *mobileType); // if outside the screen, return 0
    int get_close_location(int scrnX, int scrnY, int *locX, int *locY, char *mobileType);

    void get_zoom_pos(int locX, int locY, int &xPoint, int &yPoint);

    void put_bitmap_offset(int curX, int curY, int curZ, char *bitmapPtr, int offsetX, int offsetY,
                           short *colorRemapTable = NULL, int hMirror = 0, int compressedFlag = 1);

    void put_bitmap(int curX, int curY, int curZ, char *bitmapPtr, short *colorRemapTable = NULL, int hMirror = 0,
                    int compressedFlag = 1)
    {
        put_bitmap_offset(curX, curY, curZ, bitmapPtr, 0, 0, colorRemapTable, hMirror, compressedFlag);
    }

    void put_bitmapW_offset(int curX, int curY, int curZ, short *bitmapPtr, int offsetX, int offsetY, int hMirror = 0,
                            int compressedFlag = 0);

    void put_bitmapW(int curX, int curY, int curZ, short *bitmapPtr, int hMirror = 0, int compressed = 0)
    {
    ...
    void put_center_text(int curX, int curY, int curZ, const char *str, char black = 1, Font *font_ptr = &font_zoom);
