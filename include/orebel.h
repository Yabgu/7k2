...
public:
  ...
  short town_recno; // the town controlled by the rebel, one rebel can only
                    // control one town
...
public:
  ...
  void set_action(int actionMode, int actionPara = 0, int actionPara2 = 0) {
    action_mode = actionMode, action_para = actionPara;
    action_para2 = actionPara2;
  }

  void town_being_attacked(BaseObj *attackerObj);
...
public:
  ...
  void all_rebel_attack(int targetObjRecno);
...
public:
  ...
  int create_rebel(int unitRecno, int hostileNationRecno,
                   int actionMode = REBEL_IDLE, int actionPara = 0);
