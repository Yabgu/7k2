...
//---- at least wait for 1 year after a nation is deleted before setting up a
//new nation. ---//
...
public:
  ...
  short nation_count; // no. of nations, it's different from nation_array.size()
                      // which is a DynArrayB
  ...
  short independent_town_count_race_array[MAX_RACE]; // the no. of independent
                                                     // towns each race has
...
public:
  ...
  int new_nation(int nationClassId, int nationType, int raceId,
                 int colorSchemeId, unsigned long dpPlayerId = 0);
  ...
  Nation *operator~() {
    return (Nation *)get_ptr(player_recno);
  } // { return player_ptr; }
#endif

  int is_deleted(int recNo) { return get_ptr(recNo) == NULL; }

  Nation *get_unpacked_nation(
      int recNo); // given a packed recno and return the unpacked nation ptr
