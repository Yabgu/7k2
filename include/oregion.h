...
public:
  ...
  BYTE region_sorted_array[MAX_REGION]; // an array of region id. sorted by the
                                        // region size
...
public:
  ...
  RegionInfo *operator[](int region) { return region_info_array + region - 1; }

  RegionStat *get_region_stat(int regionId) {
    return region_stat_array + region_info_array[regionId - 1].region_stat_id -
           1;
  }
