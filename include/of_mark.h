...
#define PEASANT_GOODS_MONTH_DEMAND                                             \
  0.5 // No. of unit of goods a peasant buys in a month
#define WORKER_GOODS_MONTH_DEMAND                                              \
  1 // No. of unit of goods a worker buys in a month

#define MAX_MARKET_GOODS                                                       \
  6 // Maximum no. of types of goods the market trades for
...
struct MarketGoods {
  ...
  float cur_month_supply; // supply from direct linked firms only. One of its
                          // uses is determining whether we have enough supply
                          // to export
...
public:
  ...
  MarketGoods *
  market_product_array(int productId); // pointers to market_goods_array

  int free_slot_count();
  int stock_value_index(); // for AI, a 0-100 index number telling the total
                           // value of the market's stock
...
private:
  ...
  int ai_create_new_trade(Firm *firmPtr, int stop1PickUpType,
                          int stop2PickUpType);
};
#pragma pack()
