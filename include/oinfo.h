...
class Info {
public:
  int game_start_date; // set to Game::game_start_year in Info::init(), the
                       // actual date the game begins, not the scenario begins
  int game_date;
  int game_day;
  int game_month;
  int game_year;
  ...
  uint32_t
      total_play_time; // total time the player has played in all saved games
  ...
  short viewing_nation_recno; // which nation the player is viewing at with the
                              // reports.
  short viewing_spy_recno; // the recno of the spy viewing the secret report of
                           // other nations
  ...
  char last_write_offset; // it is used for an offset locator for excluding
                          // static member vars during reading and writing files
...
public:
  ...
  // ##### begin Gilbert 2/9 #######//
  int draw_unit_icon(int x, int y, int unitId, int nationRecno, int windowX1,
                     int windowY1, int windowX2, int windowY2, int para);
  // ##### end Gilbert 2/9 #######//
  ...
  int disp_loyalty(int x, int y, int x2, int curLoyalty, int targetLoyalty,
                   int nationRecno, int refreshFlag, int useBlueFont = 0);
