...
#define MAX_NATION_CONTRIBUTION                                                \
  10000 // there is an upper limit nation_contribution as it is a <short>
...
enum {
  UNIT_MODE_OVERSEE =
      1, // unit_mode_para is the recno of the firm the unit is overseeing
...
enum { UNIT_AGGRESSIVE = 1, UNIT_DEFENSIVE, UNIT_STAND_GROUND };
...
public:
  ...
  virtual void
  stop_move(); // it should not be virtual as if it is called at the UnitB
...
private:
  void get_object_range(int curXLoc, int curYLoc, int &rangeX1, int &rangeY1,
                        int &rangeX2, int &rangeY2);
...
public:
  ...
  // int			last_ask_attack_date;		 // date which ask team member or
  // neighbour to attack
...
public:
  ...
  void init(int unitId, int nationRecno, int rankId, int unitLoyalty,
            int startXLoc, int startYLoc);
  void deinit();
  ...
  void assign(int destXLoc, int destYLoc, int actionNationRecno = -1,
              bool forceMoveFlag = false);
  void go_cast_power(int castXLoc, int castYLoc, char remoteAction,
                     int targetBaseObjRecno = 0);
  int return_camp();
  ...
  int set_assign(int destXLoc, int destYLoc, int actionNationRecno,
                 bool forceMoveFlag);
...
protected:
  ...
  int cal_distance(int targetXLoc, int targetYLoc, int targetWidth,
                   int targetHeight);
...
public:
  ...
  int add_unit(int unitId, int nationRecno, int rankId = 0, int unitLoyalty = 0,
