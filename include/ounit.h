...
enum
{
    UNIT_STOP,
    UNIT_MOVE,
    UNIT_ATTACK,      // para=BaseObj recno of the target, loc_x & loc_y=the current
    ...
    UNIT_ASSIGN,      // para=BaseObj recno of the place to be assigned to, loc_x &
                      // loc_y=the assign destination
    UNIT_GO_CAST_POWER,
...
enum
{
    UNIT_MODE_OVERSEE = 1,    // unit_mode_para is the recno of the firm the unit is overseeing
    UNIT_MODE_TOWN_DEFENDER,  // unit_mode_para is the recno of the town the unit
                              // is defending
    UNIT_MODE_CAMP_DEFENDER,  // unit_mode_para is the recno of the camp where the
                              // unit is from
    UNIT_MODE_CONSTRUCT_FIRM, // unit_mode_para is the recno of the firm the unit
                              // is constructing
    UNIT_MODE_CONSTRUCT_TOWN, // unit_mode_para is the recno of the town the unit
                              // is constructing
    UNIT_MODE_REBEL,          // unit_mode_para is the recno of the rebel group the unit
                              // belongs to
    UNIT_MODE_MONSTER,        // unit_mode_para is the recno of the firm recno of the
                              // monster firm it belongs to
    UNIT_MODE_ON_SHIP,        // unit_mode_para is the recno of the ship unit this unit
                              // is on
    UNIT_MODE_IN_HARBOR,      // for ships only, unit_mode_para is the recno of the
                              // harbor this marine unit is in
};
...
struct UnitOrder
{
  public:
    ...
    WORD ai_action_id;
    WORD name_id; // for storing name_id of the target unit if the order is
                  // attacking a unit
...
class UnitB : public Sprite
{
  ...
  public:
    ...
    int retry_state;
    UnitOrder cur_order;
    virtual int is_visible()
    {
        return cur_x >= 0;
    } // whether the unit is visible on the map, it is not invisable if cur_x ==
    // -1
    ...
    int obj_loc_x2()
    {
        return next_x_loc() + loc_width - 1;
    }
    int obj_loc_y2()
    {
        return next_y_loc() + loc_height - 1;
    }
    ...
    short move_to_loc_x; // the destination location of the current move_to()
                         // call, for re-searching the path when it is blocked.
    short move_to_loc_y;
    short wait_count;
  ...
  public:
    ...
    virtual void stop_move(); // it should not be virtual as if it is called at the UnitB
                              // level, it will just reset movement. If it is called at the
                              // Unit level, it will also reset action_mode
    ...
    //--- check whether the unit is in formation, if yes, should retry until it
    // reach destination ---//
  ...
  private:
    ...
    void set_no_longer_blocked()
    {
        number_of_times_being_blocked = 0;
    }
    void set_not_seriously_blocked()
    {
        number_of_times_being_blocked = MAX_UNIT_BLOCKED_TIME - 3;
    }
    void set_first_time_blocked()
    {
        number_of_times_being_blocked = 1;
    }
    bool is_blocked()
    {
        return (number_of_times_being_blocked != 0);
    }
    ...
    void del_old_path();
    void set_path(PathResult *aPath)
    {
        cur_path = aPath;
    }
    void set_cur_path_result_id(short count)
    {
        cur_path_result_id = count;
    }
    void set_steps_remaining(int remainSteps)
    {
        steps_remaining = remainSteps;
    }
};
#pragma pack()
...
class Unit : public UnitB
{
  ...
  public:
    ...
    char unit_mode;
    short unit_mode_para; // if unit_mode==UNIT_MODE_REBEL, unit_mode_para is
                          // rebel_recno this unit belongs to
    short rebel_recno()
    {
        return unit_mode == UNIT_MODE_REBEL ? unit_mode_para : 0;
    }
    ...
    int in_team()
    {
        return leader_unit_recno || team_info;
    }
    ...
    void set_ai_action_id(int aiActionId)
    {
    ...
    AttackInfo *cur_attack_info()
    {
        return attack_info_array + cur_attack;
    }
    char attack_count;
    char attack_range()
    {
        return attack_info_array[cur_attack].attack_range;
    }

    short range_attack_x_loc; // -1 for unable to do range_attack, use to store
                              // previous range attack location
    short range_attack_y_loc; // -1 for unable to do range_attack, use to store
                              // previous range attack location
    ...
    char can_guard_flag; // bit0= standing guard, bit1=moving guard
                         // copy from sprite_info->can_guard_flag when
                         // skill.combat level is high enough
    int can_stand_guard()
    {
    ...
    char behavior_mode;
    int get_behavior_mode()
    {
        return is_in_formation() ? UNIT_STAND_GROUND : behavior_mode;
    } // if formation is on, it's always in stand ground mode
    ...
    // int			last_ask_attack_date;		 // date which ask team member
    // or neighbour to attack
  ...
  public:
    ...
    void explore_on_move(int curNextXLoc, int curNextYLoc, int newNextXLoc,
                         int newNextYLoc); // explore outside only
    void visit_area();
    ...
    virtual int send_hotkey(unsigned scanCode,
                            unsigned skeyState); // called by sys::process_key and functions it calls
    ...
    {
        return this;
    } // derived from BaseObj
    virtual UnitGod *cast_to_UnitGod()
    {
        return 0;
    }
    virtual UnitMonster *cast_to_UnitMonster()
    {
        return 0;
    }
    virtual UnitCaravan *cast_to_UnitCaravan()
    {
        return 0;
    }
    virtual UnitWagon *cast_to_UnitWagon()
    {
        return 0;
    }
    virtual UnitExpCart *cast_to_UnitExpCart()
    {
        return 0;
    }
  ...
  protected:
    ...
    //	int				can_attack()		{ return attack_count >
    // 0; } 	virtual int 	can_attack_target(int baseObjRecno);
...
{
  public:
    short selected_recno; // if more than one unit is selected, it is always set
                          // to the unit with the highest rank.
    int selected_count;
  ...
  public:
    ...
    int add_unit(int unitId, int nationRecno, int rankId = 0, int unitLoyalty = 0, int startXLoc = -1,
                 int startYLoc = -1);
