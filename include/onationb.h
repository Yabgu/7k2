...
enum {
  RELATION_LEVEL_PER_STATUS = 20
}; // relation_level = status * NATION_LEVEL_PER_STATUS
...
enum { IMPORT_RAW, IMPORT_PRODUCT, IMPORT_TOTAL };
...
struct NationRelation // many-to-many relationships between nations
{
  ...
  int is_friendly() {
    return status >= RELATION_FRIENDLY && status <= RELATION_ALLIANCE;
  }
  ...
  char ai_relation_level; // AI's subjectively relation levels towards the
                          // others, the opposite nation's relation level is not
                          // the same as this
  char ai_secret_attack;
  char ai_demand_trade_treaty;

  float good_relation_duration_rating; // a rating indicate how long does a good
                                       // relation (friendly/alliance) lasts
  short started_war_on_us_count; // how many times this nation has started a war
                                 // with us, the more the times the worse this
                                 // nation is.
  ...
  float import_365days(int importType) {
    return last_year_import[importType] * (365 - info.year_day) / 365 +
           cur_year_import[importType];
  }

  int last_talk_reject_date_array[MAX_TALK_TYPE]; // the date which the last
                                                  // diplomatic request was
                                                  // rejected.
  int never_accept_until_date_array[MAX_TALK_TYPE];
  int last_military_aid_date;
  char ai_never_consider[MAX_TALK_TYPE]; // if this is 1, the AI will never
  ...
  int last_give_gift_date;       // the last date which the current nation give
                                 // tribute, aid or technology to this nation
  short total_given_gift_amount; // the total amount of gift the current nation
...
public:
  ...
  short nation_recno;
  char nation_type;
  short campaign_nation_recno; // if >0, it points to the CampaignNation in a
                               // campaign
  ...
  char
      next_frame_ready; // for indicating whether the next frame is ready or not
  ...
  void reset_all_finance(); // for scenario creation and editing, reset all
                            // current year finances

  int yearly_food_consumption() {
    return UNIT_FOOD_YEAR_CONSUMPTION * all_population();
  }
  int yearly_food_production() {
    return PEASANT_FOOD_YEAR_PRODUCTION * total_jobless_population;
  }
  ...
  short auto_retreat_hit_point; // auto retreat if hit point reaches this
                                // presentage
  void set_auto_retreat(int hitPointPercentage);

  //------- town auto policy -------------//

  short auto_collect_tax_loyalty; // auto collect tax if the loyalty reaches
                                  // this level
  short auto_grant_loyalty; // auto grant if the loyalty drop below this level

  //------- research vars --------//

  short research_tech_id; // the id. of the tech this firm is currently
                          // researching
  // ###### begin Gilbert 14/3 #######//
  //	float 	research_progress;		// percent completed on researching
  //the current technology
  float fraction_research_progress; // fractional part of research progress
  float research_progress(); // percent completed on researching the current
                             // technology
  ...
  float income_365days(int incomeType) {
    return last_year_income_array[incomeType] * (365 - info.year_day) / 365 +
  ...
  float expense_365days(int expenseType) {
    return last_year_expense_array[expenseType] * (365 - info.year_day) / 365 +
  ...
  float food_change_365days() {
    return last_year_food_change * (365 - info.year_day) / 365 +
  ...
  float reputation_change_365days() {
    return last_year_reputation_change * (365 - info.year_day) / 365 +
  ...
  float live_points_change_365days() {
    return last_year_live_points_change * (365 - info.year_day) / 365 +
  ...
  char relation_status_array[MAX_NATION];   // replace status in struct
                                            // NationRelation
  char relation_passable_array[MAX_NATION]; // for seeking to indicate whether
                                            // passing other nation region
  char relation_should_attack_array[MAX_NATION];
  char is_allied_with_player; // for fast access in visiting world functions

  void set_relation_status(short nationRecno, char newStatus,
                           char recursiveCall = 0);
  ...
  void set_relation_should_attack(short nationRecno, char newValue,
                                  char remoteAction);
  ...
  int all_population() {
    return total_population + total_human_count + total_monster_count;
  }
  ...
  short raw_count_array[MAX_RAW]; // no. of natural resources site this nation
                                  // possesses
...
public:
  ...
  virtual void init(int nationType, int raceId, int colorSchemeId,
                    DWORD playerId = 0);
  virtual void deinit();
  ...
  int has_king_successor_candidate(); // whether the nation has any people (but
                                      // not counting the king). If no, then the
                                      // nation is going to end.
  ...
  int revealed_by_phoenix(
      int xLoc, int yLoc); // will be overloaded by Nation::revealed_by_phoenix
                           // for faster access
...
extern short nation_hand_over_flag; // 0 for no nation hand over, 1-MAX_NATION
                                    // for nation hand over
