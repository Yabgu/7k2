...
class NationBase
{
  public:
    ...
    int peaceful_days()
    {
        return info.game_date - last_war_date;
    }
    char *peace_duration_str();

    void set_at_war_today()
    {
        is_at_war_today = 1;
    }
    int is_at_war()
    ...
    short research_tech_id; // the id. of the tech this firm is currently
                            // researching
    // ###### begin Gilbert 14/3 #######//
    //	float 	research_progress;		// percent completed on
    // researching
    ...
    {
        return last_year_live_points_change * (365 - info.year_day) / 365 + cur_year_live_points_change;
    }
    ...
    int is_own()
    {
        return nation_type == NATION_OWN;
    }
    int is_ai()
    {
        return nation_type == NATION_AI;
    }
    int is_remote()
    {
        return nation_type == NATION_REMOTE;
    }
  ...
  public:
    ...
    int revealed_by_phoenix(int xLoc, int yLoc); // will be overloaded by Nation::revealed_by_phoenix
                                                 // for faster access
