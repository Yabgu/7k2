...
#define SPY_ENEMY_RANGE                                                        \
  5 // only allow changing spy cloak if there are no enemy units within 5
    // locations from the unit

#define MIN_VIEW_SECRET_SPYING_SKILL                                           \
  20 // the minimum spying skill required for viewing secret reports
#define MIN_BRIBE_SPYING_SKILL 30
#define MIN_ASSASSIN_SPYING_SKILL 50
#define MIN_TECH_THEFT_SPYING_SKILL 70
#define MIN_CREATE_INCIDENT_SPYING_SKILL 90
...
public:
  ...
  short spy_place_para; // it can be town_recno, firm_recno or unit_recno
                        // depending on what spy_place is
  ...
  char notify_cloaked_nation_flag; // whether the spy will send a surrendering
                                   // message to the cloaked nation when it
                                   // changes its cloak to the nation
  char exposed_flag; // this is set to 1 when the spy finished stealing the
                     // secret of a nation.
  ...
  char cloaked_extra_combat_level; // extra combat level added to a spy
  char cloaked_extra_skill_level;  // extra combat level

  int cloaked_rank_id();
  int cloaked_combat_level();
  int cloaked_skill_level();
...
public:
  ...
  int get_loc(int &x1, int &y1, int &x2, int &y2);
  int get_host_base_obj_recno();

  int assassinate(int targetUnitRecno, int remoteAction);
  int get_assassinate_rating(int targetUnitRecno, int &attackRating,
                             int &defenseRating, int &defenderCount);

  int steal_tech(int targetNationRecno, char remoteAction);
  int get_steal_rating(int targetNationRecno);

  int create_incident(int targetNationRecno, int involvedNationRecno,
                      char remoteAction);
  int get_incident_rating(int targetNationRecno, int involvedNationRecno);
  ...
  int can_camouflage();
  void start_camouflage(char remoteAction);
  void stop_camouflage(char remoteAction);
...
public:
  ...
  int add_spy(int unitRecno, int spySkill, int trueNation = -1);
  int add_spy();
  void del_spy(int spyRecno);

  void next_day();

  int find_town_spy(int townRecno, int spySeq);
  ...
  void change_cloaked_nation(int spyPlace, int spyPlacePara,
  ...
  int total_spy_skill_level(int spyPlace, int spyPlacePara, int spyNationRecno,
                            int &spyCount);
