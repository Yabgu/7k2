...
enum
{
    CAMPAIGN_PLAYER_NATION_RECNO = 1
};

enum
{
    MIN_CAMPAIGN_DIFFICULTY = 1,
...
class CampaignNation
{
  public:
    ...
    char custom_name[CUSTOM_NAME_LEN + 1]; // if custom_name[0] is not NULL, use custom name instead
                                           // of the normal name
...
class Campaign : public Battle
{
  ...
  protected:
    ...
    // run
    int stage_run_count(int stageId)
    {
        return has_stage_run(stageId);
    }
    ...
    void attack_state(int attackerStateRecno, int targetStateRecno, int attackResult, int firstStep = 0,
                      int lastStep = 0, int attacker2StateRecno = 0);
    ...
    void attack_animation(int attackerUnitId, int targetUnitId, int attackerColorSchemeId, int targetColorSchemeId,
                          int srcScrnX, int srcScrnY, int destScrnX, int destScrnY, int attackResult, int firstStep,
                          int lastStep);

    void attack_animation(CampaignAnimationUnit *attackerArray, int attackerCount, CampaignAnimationUnit *defenderUnit,
    ...
    int random_pick_monster_campaign_nation(int excludedStateRecno = 0,
    ...
    int create_nation_town(int nationRecno, int xLoc, int yLoc, int buildFort, int sameRaceTown = 1,
                           int soldierAddCount = 100, int combatLevel = 0);
    int create_independent_town(int xLoc, int yLoc, int raceId, int initPop, int initResistance);
    int create_monster_lair(int xLoc, int yLoc, int nationRecno, int soldierCount, int avgCombatLevel,
                            int monsterId = 0);
    ...
    int create_unit_group(int nationRecno, int addCount, int unitId = 0, int adjustCount = 0, int whichEdge = 0,
    ...
    void create_troop2(int nationRecno, int totalUnit, int avgCombatLevel = 0, int unitId = 0,
                       char useLastLocation = 0);
    void create_wagon(int nationRecno, int totalPop, char useLastLocation);
    int create_lair(int campaignNationRecno, int nationRecno, int lairAddCount, int independentTownAddCount,
                    int specialFirmCount, int soldierCombatLevel = 0);
