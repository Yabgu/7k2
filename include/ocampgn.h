...
enum { MIN_CAMPAIGN_DIFFICULTY = 1, MAX_CAMPAIGN_DIFFICULTY = 5 };
...
enum {
  MINISTER_OF_WAR = 1, // id. for human ministers
...
enum {
  MINISTER_ORDO = 1, // id. for Fryhtan ministers - they share the same array
};

//------- define mountain layout constants -------//

enum { MOUNTAIN_LAYOUT_X_COUNT = 20, MOUNTAIN_LAYOUT_Y_COUNT = 20 };

//--------- Define campaign id. -------//

enum {
  CAMPAIGN_EAST_WEST = 1,
...
struct CampaignSituation {
  ...
  char
      unit_combat_level; // combat level of existing units and soldiers in forts
};
...
public:
  char should_attack; // whether units should automatically attack units/firms
                      // of this nation when the relationship is hostile
...
public:
  ...
  char custom_name[CUSTOM_NAME_LEN +
                   1]; // if custom_name[0] is not NULL, use custom name instead
                       // of the normal name
  ...
  CampaignMember king;
  CampaignMember noble[MAX_NOBLE + MAX_ROYAL_UNIT]; // reserve space for royal
                                                    // added in battle
  int noble_count;

  char know_base_array[MAX_RACE]; // whether the unit knows how to constructure
                                  // seat of power or not

  char tech_level_array[MAX_TECH]; // each nation's the technology level of this
                                   // unit,
  char research_progress_array[MAX_TECH];

  CampaignNationRelation
      relation_array[MAX_NATION]; // inter-relationship with other nations

  int get_relation_status(int nationRecno) {
    return relation_array[nationRecno - 1].status;
  }
...
public:
  ...
  char auto_test_flag;
  char replay_stage_flag; // when this is set to 1, the stage game will be
                          // restarted.
  ...
  char campaign_difficulty; // the true campaign difficulty that scenarios are
                            // generated based on.

  //------- campaign game info -------//

  short attacker_state_recno;
  short attacker2_state_recno; // this is used when there are more than one
                               // attacking nation involved in a battle
  ...
  char this_battle_use_fryhtan; // whether the player wants to use Fryhtans in
                                // this battle
...
public:
  ...
  const char *king_name() {
    return get_nation(CAMPAIGN_PLAYER_NATION_RECNO)->king_name();
  }
  ...
  void render_attackable_terrain(int par, char *selectableStateArray,
  ...
  static void put_center_text(int x, int y, const char *str, char black,
                              Font *fontPtr, int darkness = 1);
...
protected:
  ...
  int event_run_count(int eventId) {
    return has_event_run(eventId);
  } // has_event_run() actually return the number of times the event has been
    // run
  ...
  int get_relation_status(int nationRecno) {
    return get_nation(CAMPAIGN_PLAYER_NATION_RECNO)
        ->get_relation_status(nationRecno);
  }
  ...
  void attack_state(int attackerStateRecno, int targetStateRecno,
                    int attackResult, int firstStep = 0, int lastStep = 0,
                    int attacker2StateRecno = 0);
  void flash_state(int stateRecno) { ; }
  void rebel_attack_state(int stateRecno, int attackResult, int firstStep,
                          int lastStep);

  void attack_animation(int attackerUnitId, int targetUnitId,
                        int attackerColorSchemeId, int targetColorSchemeId,
                        int srcScrnX, int srcScrnY, int destScrnX,
                        int destScrnY, int attackResult, int firstStep,
                        int lastStep);

  void attack_animation(CampaignAnimationUnit *attackerArray, int attackerCount,
                        CampaignAnimationUnit *defenderUnit, int firstStep,
                        int lastStep);

  void state_change_nation(int stateRecno, int newNationRecno);
  int temp_state_change_nation(int stateRecno, int newNationRecno);
  int random_pick_attack_state(int attackNationRecno, int targetNationRecno,
                               int excludedStateRecno = 0);
  int random_pick_state(int nationRecno, int neighborStateRecno = 0);
  int random_pick_monster_campaign_nation(
      int excludedStateRecno = 0,
      int nextToNationRecno = CAMPAIGN_PLAYER_NATION_RECNO, int mustNextTo = 0);

  int select_attackable_state(char *selectableStateArray,
                              int selectableStateCount);
  ...
  void disp_state_map(int dispX1, int dispY1, int dispWidth, int dispHeight,
                      int par);
  void disp_intro();
  ...
  void disp_text(int boxX1, int boxY1, int boxX2, int boxY2, char *dialogText,
                 int animationRaceId = 0);
  int detect_reply();
  ...
  //	void		random_init_monster_tech(int monsterNationRecno, int
  ...
  int create_nation_town(int nationRecno, int xLoc, int yLoc, int buildFort,
                         int sameRaceTown = 1, int soldierAddCount = 100,
                         int combatLevel = 0);
  int create_independent_town(int xLoc, int yLoc, int raceId, int initPop,
                              int initResistance);
  int create_monster_lair(int xLoc, int yLoc, int nationRecno, int soldierCount,
                          int avgCombatLevel, int monsterId = 0);
  ...
  int create_unit_group(int nationRecno, int addCount, int unitId = 0,
                        int adjustCount = 0, int whichEdge = 0, int xLoc = -1,
                        int yLoc = -1);
  ...
  void create_troop(int nationRecno, int totalUnit, int avgCombatLevel = 0,
                    int unitId = 0, char useLastLocation = 0);
  void create_troop2(int nationRecno, int totalUnit, int avgCombatLevel = 0,
                     int unitId = 0, char useLastLocation = 0);
  void create_wagon(int nationRecno, int totalPop, char useLastLocation);
  int create_lair(int campaignNationRecno, int nationRecno, int lairAddCount,
                  int independentTownAddCount, int specialFirmCount,
                  int soldierCombatLevel = 0);
  int create_town(int nationRecno, int townAddCount, int hasFortPercent,
                  int combatLevel = 0);
  void create_offensive_structure(int nationRecno, int buildCount);
  void create_offensive_structure(int townRecno);
  int create_rebel_nation(int townCount, int hasFortPercent,
  ...
  void think_enemy_attack(int monsterNationRecno, int targetNationRecno,
