...
#define PATH_FIND_LOC_BUFFER_SIZE                                              \
  (30 * 30) // a buffer for backing up location data before overwritting them
            // for searching
...
struct SaveLocation // for saving Location before they are modified for
                    // researching
{
...
public:
  ...
  void set_handle_power_nation(int handlePowerFlag,
                               char *nationPassable = NULL);
  ...
  bool find_path(int sourceX, int sourceY, int destX, int destY,
                 ...
                 int destRangeY2, bool raiseIt = 1, bool raiseDest = 1,
                 int maxTries = 0, int stopAtRange = 0);
...
private:
  bool find_path2(int sourceX, int sourceY, int destinationX, int destinationY,
  ...
  PathNode *get_from_list(int x, int y,
                          PathNode *); // gets node from open/closed list
