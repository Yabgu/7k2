...
public:
  ...
  unsigned char get_eqv_pattern_id(unsigned char patternId,
                                   unsigned char seqno); // seqno = 0 to 3
  ...
  HillSetInfo *get_first_hill_set(short hSetId) {
    return hset_info_array[hSetId - 1].first_hill_set;
  }
  HillSetInfo *get_first_hill_set(HSetInfo *hSetInfo) {
    return hSetInfo->first_hill_set;
  }
  HillSetInfo *get_next_hill_set(HillSetInfo *hillSetInfo) {
    return hillSetInfo->next_hill_set;
  }
  ...
  HSetInfo *random_hset(char isSide, char maxLength, char maxWidth,
