...
enum {
  STD_ACTION_RETRY_COUNT = 4, // retry this number of times before giving up
  MAX_SCORE = 600,

  MIGRATE_DEMAND_SUPPLY_DIFF =
      25, // suppy > demand + MIGRATE_DEMAND_SUPPLY_DIFF
...
enum {
  ACTION_DYNAMIC, // for ActionNode::action_type
  ACTION_FIXED
};

enum {
  ACTION_AI_BUILD_FIRM = 1, // define ActionNode action_type
...
struct ActionNode {
  ...
  short action_para2; // parameter for the action. e.g. firm race id. for
                      // building FirmBase
  ...
  char retry_count; // number of term to wait before this action is removed from
                    // the array if it cannot be processed
  char instance_count; // no. of times this action needs to be carried out

  short group_unit_array[MAX_ACTION_GROUP_UNIT]; // for group unit actions, the
                                                 // no. of units in the array is
                                                 // stored in instance_count

  char processing_instance_count;
  char processed_instance_count;

  int32_t next_retry_date; // continue processing this action after this date,
                           // this is used when training a unit for construction
};
#pragma pack()
...
#define MAX_AI_ATTACK_TARGET                                                   \
  10 // attacking more than one target in an attack mission
...
public:
  ...
  char pref_military_development; // pref_military_development +
                                  // pref_economic_development = 100
  char pref_economic_development;
  char pref_inc_pop_by_capture; // pref_inc_pop_by_capture +
                                // pref_inc_pop_by_growth = 100
  ...
  char pref_keep_general; // whether to keep currently non-useful the general,
                          // or demote them.
  char pref_keep_skilled_unit; // whether to keep currently non-useful skilled
                               // units, or assign them to towns.
  char pref_diplomacy_retry;   // tedency to retry diplomatic actions after
                               // previous ones have been rejected.
...
public:
  ...
  //--------------------------------------------------------------//
  void init(int nationType, int raceId, int colorSchemeId,
            DWORD playerId); // init local parameters
  void deinit();

  void init_all_ai_info();
  void init_ai_info(short **aiInfoArray, short &aiInfoCount, short &aiInfoSize,
                    int arrayInitSize);

  void deinit_all_ai_info();

  void init_personalty();
  int revealed_by_phoenix(
      int xLoc,
      int yLoc); // override NationBase::revealed_by_phoenix for faster access
  ...
  //-----------------------------------------------------------//
  short *update_ai_firm_array(int firmId, int actionType, int actionRecno,
                              int &arrayCount);

  short *update_ai_array(short &aiInfoCount, short &aiInfoSize,
                         short **aiInfoArray, int arrayIncSize, int actionType,
                         int actionRecno);
  ...
  void del_firm_info(
      char firmId,
      short firmRecno); // remove useless information as firm is removed
  ...
  void
  assign_firm_overseer(char firm_id, short firm_recno,
                       short overseerUnitRecno); // add overseer information
  void remove_firm_overseer(char firm_id,
                            short firm_recno); // remove overseer information
  int is_caravan_exist(int firstMarket, int secondMarket,
                       int setStopInternal = 0);
  ...
  void seek_best_build_mine_location(short &xLoc, short &yLoc, short mapXLoc,
                                     short mapYLoc);
  void cal_location_score(short x1, short y1, short width, short height,
                          int &weight);
  int find_best_place_loc(short firmId, short refXLoc, short refYLoc,
  ...
  //------------------------------------------------------------//
  int add_action(short xLoc, short yLoc, short refXLoc, short refYLoc,
                 int actionType, int actionPara, int instanceCount = 1,
                 int unitRecno = 0, int actionPara2 = 0,
                 short *groupUnitArray = NULL);
  int add_action(ActionNode *actionNode, int immediateProcess = 0);
  void del_action(int actionRecno);
  int is_action_exist(short actionXLoc, short actionYLoc, short refXLoc,
                      short refYLoc, int actionType, int actionPara,
                      int unitRecno = 0);
  int is_action_exist(int actionType, int actionPara, int regionId = 0);
  int is_build_action_exist(int firmId, int xLoc, int yLoc);

  int process_action(int priorityActionRecno = 0,
                     int processActionMode = 0); // waiting --> processing
  int process_action_id(int actionId);

  void action_finished(WORD aiActionId, short unitRecno = 0,
                       int actionFailure = 0);
  ...
  int check_firm_ready(
      short xLoc, short yLoc,
      int firmId = 0); // check whether firm exists and belongs to our nation
  int check_town_ready(
      short xLoc,
      short yLoc); // check whether town exists and belongs to our nation

  //------------------------------------------------------------//
  // functions used to find skilled units
  //------------------------------------------------------------//
  Unit *ai_get_unit(int isCivilian, int raceId, ActionNode *actionNode);
  Unit *ai_find_unit(int isCivilian, int raceId, short destX, short destY,
                     char &resultFlag, int actionId = 0);
  Unit *ai_find_unit_from_camp(int raceId, short destX, short destY);
  int hire_unit(int raceId, int isCivilian, int hireSpy, short destX,
                short destY);
  int recruit_peasant(int raceId, short destX, short destY,
  ...
  int ai_attack_target(int targetXLoc, int targetYLoc, int targetCombatLevel,
                       int defenseMode = 0, int justMoveToFlag = 0,
                       int attackerMinCombatLevel = 0,
                       int attackerCampRecno = 0, int useAllCamp = 0,
                       int retainCombatLevel = 0);
  void ai_attack_target_sync();
  void ai_attack_target_execute(int directAttack);
  int ai_attack_order_nearby_mobile(int targetXLoc, int targetYLoc,
  ...
  int mobile_defense_combat_level(int targetXLoc, int targetYLoc,
  ...
  int ai_assign_spy(int targetXLoc, int targetYLoc, int spyRaceId,
                    int mobileOnly);
  int ai_assign_spy_to_town(int townRecno, int raceId = 0);
  int ai_assign_spy_to_firm(int firmRecno);

  Spy *ai_find_spy(int targetXLoc, int targetYLoc, int targetRaceId,
                   int isCivilian, int mobileOnly);
