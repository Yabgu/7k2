...
#define MIN_LAND_COST                                                          \
  500000 // Minimum land cost even there is no population at all
...
public:
  ...
  char last_scroll_flag; // bit 0/1/2/3 represend scroll rightleft/down/up
                         // respectively
  ...
  static int max_x_loc,
      max_y_loc; // these must be static vars as MAX_WORLD_X_LOC &
                 // MAX_WORLD_Y_LOC are defined to use them.
...
public:
  ...
  int is_fully_explored(int xLoc1, int yLoc1, int xLoc2,
                        int yLoc2); // fully explored
  int is_explored(int xLoc1, int yLoc1, int xLoc2,
                  int yLoc2); // any location explored
  int is_explored_quick(int xLoc1, int yLoc1, int xLoc2,
                        int yLoc2); // four corner only
  // always call unveil before visit //
  void visit(int xLoc1, int yLoc1, int xLoc2, int yLoc2, int range,
             int extend = 0);
  ...
  int can_build_site(int xLoc1, int yLoc1, int siteWidth, int siteHeight,
                     int outerSpace);
  int can_destruct_wall(int xLoc1, int yLoc1, short nationRecno);
  void build_wall_tile(int xLoc1, int yLoc1, short nationRecno,
                       char remoteAction);
  void destruct_wall_tile(int xLoc1, int yLoc1, short nationRecno,
                          char remoteAction);

  int locate_space(int &xLoc1, int &yLoc1, int xLoc2, int yLoc2,
                   int spaceLocWidth, int spaceLocHeight,
                   int mobileType = UNIT_LAND, int regionId = 0,
                   int buildFlag = 0);
  int check_unit_space(int xLoc1, int yLoc1, int xLoc2, int yLoc2,
                       int mobileType = UNIT_LAND, int buildFlag = 0);

  int locate_space_random(int &xLoc1, int &yLoc1, int xLoc2, int yLoc2,
                          int spaceLocWidth, int spaceLocHeight, int maxTries,
                          int regionId = 0, int buildSite = 0,
                          char teraMask = 1);

  int net_attacker_power_in_area(int targetXLoc, int targetYLoc,
                                 int attackerWorldRecno, int defenderWorldRecno,
                                 int returnIfWar, int &hasWar, int scanRange);

  int is_adjacent_region(int x, int y, int regionId);

  void draw_link_line(int srcFirmId, int srcTownRecno, int srcXLoc1,
  ...
  void restore_power(int xLoc1, int yLoc1, int xLoc2, int yLoc2, int townRecno,
                     int firmRecno);
  ...
  void plant_spray(short *plantIdArray, char area, char distance, short x,
                   short y);
...
private:
  ...
  void build_west_wall(short x1, short y1, short y2, short townRec,
                       short initHp);
  void build_east_wall(short x1, short y1, short y2, short townRec,
                       short initHp);
  void build_north_wall(short x1, short x2, short y1, short townRec,
                        short initHp);
  void build_south_wall(short x1, short x2, short y1, short townRec,
                        short initHp);
