...
#define DEF_DYNARRAY_BLOCK_SIZE                                                \
  30 // default allocation block size (no. of unities each block has)

//---------- Define sort types --------//

enum { SORT_INT = 1, SORT_SHORT, SORT_CHAR, SORT_CHAR_PTR, SORT_CHAR_STR };
...
inline void DynArray::push(void *ent) { linkin(ent); }
...
//
inline void DynArray::start() { cur_pos = MIN(1, last_ele); }

inline void DynArray::end() { cur_pos = last_ele; }
...
inline int DynArray::recno() { return cur_pos; }

inline int DynArray::size() const { return last_ele; }

//----------- END OF FUNCTION DynArray::jump,go,pos,size ---------//

//-------- BEGIN OF FUNCTION DynArray::isstart,isend ------//

inline int DynArray::is_start() { return (cur_pos <= 1); }

inline int DynArray::is_end() { return (cur_pos >= last_ele); }
