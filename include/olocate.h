...
// loc_x_width, loc_x_height, loc_y_width, loc_y_height are multiplied by 32
// times
...
// if a location has been explored, visibility is bewtween
// EXPLORED_VISIBILITY-FULL_VISIBILITY don't set EXPLORED_VISIBILITY to multiple
// of 8
...
struct Location
{
  public:
    ...
    long c00, c10, c01, c20, c11,
        c02; // scaled up by C_MULTIPLIER or shift left by C_MULTIPLIER_SHIFT
  ...
  public:
    ...
    void walkable_on(int teraMask)
    {
        loc_flag |= teraMask;
    }
    void walkable_off(int teraMask)
    {
        loc_flag &= ~teraMask;
    }
    ...
    void explored_on()
    {
        if (visit_level < EXPLORED_VISIBILITY)
            visit_level = EXPLORED_VISIBILITY;
    }
    void explored_off()
    {
        visit_level = 0;
    ...
    void set_visited()
    {
        visit_level = MAX_VISIT_LEVEL;
    }
    void set_visited(unsigned char v)
    {
        if (visit_level < v)
            visit_level = v;
    }
    ...
    int can_build_town()
    {
        return !cargo_recno && (loc_flag & LOCATE_WALK_LAND) && !(loc_flag & LOCATE_BLOCK_MASK) && !has_site() &&
               !is_power_off();
    }
    int town_recno()
    {
    ...
    int can_build_wall()
    {
        return !cargo_recno && (loc_flag & LOCATE_WALK_LAND) && !(loc_flag & (LOCATE_BLOCK_MASK | LOCATE_SITE_MASK)) &&
               !has_site();
    }
    void set_wall(int wallId, int townRecno, int hitPoints);
    void set_wall_creating();
    void set_wall_destructing();
    void chg_wall_id(int wallId)
    {
        extra_para = wallId;
    }
    int wall_id()
    ...
    int is_wall_creating()
    {
        return wall_hit_point() > 0;
    }
    int is_wall_destructing()
    {
        return wall_hit_point() < 0;
    }
    ...
    {
        return !cargo_recno && (loc_flag & teraMask) && !(loc_flag & (LOCATE_BLOCK_MASK | LOCATE_SITE_MASK)) &&
               !has_site();
    }
    ...
    RegionType region_type()
    {
        ...
        if (walkable())
        {
            regionType = REGION_LAND;
        }
        else if (sailable())
        {
            regionType = REGION_SEA;
        }
        else
        {
            regionType = REGION_INPASSABLE;
        }
        return regionType;
    }
    ...
    void set_fire_str(char str)
    {
        fire_level = str;
    }
    void set_fire_src(char src)
    {
        flammability = src;
    }
    void add_fire_str(char str)
    {
        fire_level += str;
    }
    void add_fire_src(char src)
    {
        flammability += src;
    }
    int can_set_fire()
    ...
    int is_accessible(int mobileType); // whether the location is accessible to
                                       // the unit of the specific mobile type
    ...
    {
        return is_accessible(mobileType) && (mobileType == UNIT_AIR ? !air_cargo_recno : !cargo_recno);
    }
    ...
    void clear_harbor_bit()
    {
        loc_flag &= ~LOCATE_HARBOR_BIT;
    }
    int can_build_whole_harbor()
    {
        return loc_flag & LOCATE_HARBOR_BIT;
    }
...
struct Corner
{
  ...
  public:
    void set_altitude(short h)
    {
        altitude = h;
    }
    short get_altitude()
    {
        return sea_altitude ? sea_altitude : altitude;
    }
    void set_sea_altitude(short s)
    {
        sea_altitude = (s >= altitude ? s : 0);
    }
    short get_sea_altitude()
    {
        return sea_altitude;
    }
    int is_sea()
    ...
    void set_gradient_x(float dzdx)
    {
        gradient_x = short(dzdx * LOCATE_WIDTH);
    }
    float get_gradient_x()
    {
        return (float)gradient_x / LOCATE_WIDTH;
    }
    void set_gradient_y(float dzdy)
    {
        gradient_y = short(dzdy * LOCATE_HEIGHT);
    }
    float get_gradient_y()
    {
        return (float)gradient_y / LOCATE_HEIGHT;
    }
};
