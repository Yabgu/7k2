...
#define LOCATE_POWER_OFF                                                       \
  0x1000 // true if no power_nation_recno can be set in this location
#define LOCATE_HARBOR_BIT                                                      \
  0x2000 // true if the terrain is suitable to build harbor (x,y) to (x+2,y+2)
...
public:
  ...
  int walkable(int teraMask) { return loc_flag & teraMask; }
  void walkable_reset();
  ...
  int can_build_site(int teraMask = LOCATE_WALK_LAND) {
    return (loc_flag & teraMask) && !(loc_flag & LOCATE_SITE_MASK) &&
           !has_site();
  }
  ...
  // ----------- dirt -------------//
  int can_add_dirt() { return !(loc_flag & LOCATE_SITE_MASK); }
  ...
  int can_build_firm(int teraMask = LOCATE_WALK_LAND) {
    return !cargo_recno && (loc_flag & teraMask) &&
           !(loc_flag & LOCATE_BLOCK_MASK) && !is_power_off();
  }
  ...
  int can_build_town() {
    return !cargo_recno && (loc_flag & LOCATE_WALK_LAND) &&
           !(loc_flag & LOCATE_BLOCK_MASK) && !has_site() && !is_power_off();
  }
  int town_recno() {
    if (is_town())
      return cargo_recno;
  ...
  int wall_grade() {
    return wall_hit_point() >= 0 ? (wall_hit_point() + 24) / 25
                                 : (wall_hit_point() - 24) / 25;
  }
  ...
  RegionType region_type() {
    return walkable() ? REGION_LAND
                      : (sailable() ? REGION_SEA : REGION_INPASSABLE);
  }
...
struct LocationCorners {
  ...
  void render_special(Blob2DW *, int effectId, char *texturePtr, int color1,
                      int color2);
  void render_special(BitmapW *, int effectId, char *texturePtr, int color1,
                      int color2, int x, int y);
