...
class GameFile
{
  public:
    ...
    int campaign_id;          // duplicate from game.cur_campaign->campaign_id or null if
                              // not campaign
...
{
  public:
    char demo_format; // whether write the game in shareware format or not (only
    ...
    char same_version;            // true if major version of the load game is same as that
                                  // of the program
#endif
    //#### end alex 5/3 ####//
  ...
  public:
    ...
    //   int save_new_game(const char* =NULL); // save a new game immediately
    //   without prompting menu
    int save_new_game(const char *fileName); // save a new game immediately
                                             // without prompting menu

    int read_hall_of_fame();
    int write_hall_of_fame(); // it may be called by group_res.gen_group() in
                              // writting default name
