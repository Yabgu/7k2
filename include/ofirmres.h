...
struct FirmBuild {
  ...
  char firm_id;
  //	char  build_code[BUILD_CODE_LEN+1];		// building code, either a
  //race code or a custom code for each firm's own use, it is actually read from
  //FirmBuildRec::race_code[] 	char  race_id;
...
struct FirmBitmap {
  ...
  void draw_at(int curX, int curY, int curZ, short *colorTable,
...
public:
  ...
  int is_linkable_to_firm(int firmId1, int firmRaceId1, int firmNationRecno1,
                          int firmId2, int firmRaceId2, int firmNationRecno2);
