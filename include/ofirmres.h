...
enum
{
    FIRM_UNDER_CONSTRUCTION = 'U', // definitions of FirmBuild::mode
    FIRM_IDLE = 'I',
    FIRM_ACTIVE = 'A',
};
...
struct FirmInfo
{
    ...
    char only_build_race_id; // if this > 0, then only unit of this race can build
                             // the firm

    char tera_type;
    char buildable;    // whether this building can be built by the player or it
                       // exists in the game since the beginning of the game. If
                       // setup_cost==0, this firm is not buildable
    ...
    int get_nation_firm_count(int nationRecno)
    {
        return nation_firm_count_array[nationRecno - 1];
    }

    char nation_tech_level_array[MAX_NATION];
    int get_nation_tech_level(int nationRecno)
    {
        return nation_tech_level_array[nationRecno - 1];
    }
    void set_nation_tech_level(int nationRecno, char techLevel)
    {
...
struct FirmBuild
{
    ...
    char firm_id;
    //	char  build_code[BUILD_CODE_LEN+1];		// building code, either
    // a
    // race code or a custom code for each firm's own use, it is actually read
    // from FirmBuildRec::race_code[] 	char  race_id;
    ...
    short under_construction_bitmap_recno; // bitmap recno of the firm that is
                                           // under construction
    ...
    short first_bitmap(int frameId)
    {
        return first_bitmap_array[frameId - 1];
    }
    short bitmap_count(int frameId)
    {
        return bitmap_count_array[frameId - 1];
    }
    short frame_delay(int frameId)
    {
...
class FirmRes
{
  ...
  public:
    ...
    FirmBitmap *get_bitmap(int bitmapId)
    {
    ...
    FirmGroup *get_group(int buildId)
    {
        return firm_group_array + buildId - 1;
    }
    ...
    int is_linkable_to_firm(int firmId1, int firmRaceId1, int firmNationRecno1, int firmId2, int firmRaceId2,
