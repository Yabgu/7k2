...
public:
  ...
  BitmapW *back_ground_bitmap;

  char mouse_drag_flag;

  unsigned last_key_code;
  int last_auto_repeat_count;

public:
  GetA();
  void init(int x1, int y1, int x2, char *field, unsigned length, Font *,
            char align, char detectEsc);
  void set_background(BitmapW *);
  int height();
  void clear();
  unsigned detect_key();
  int detect_click();
  ...
  void select_whole();
  // ##### begin Gilbert 12/10 ######//
  void text_changed(char *newStr = 0);
  // ##### end Gilbert 12/10 ######//
};
