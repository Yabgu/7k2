...
enum
{
    TOTAL_TERRAIN_TYPE = 4
};
enum
{
    MAX_GEN_TERRAIN_TYPE = 3
}; // map generator will not generate light dirt or above
...
struct TerrainInfo
{
    ...
    char alternative_count_with_extra;    // no. of alternative bitmaps of this
                                          // terrain
    char alternative_count_without_extra; // no. of alternative bitmaps of this
                                          // terrain
    unsigned char flags;
    ...
    int can_snow()
    {
        return flags & TERRAIN_FLAG_SNOW;
    }
};
...
class TerrainRes
{
  ...
  public:
    ...
    int scan(int nwType, int nwSubType, int neType, int neSubType, int swType, int swSubType, int seType, int seSubType,
             int firstInstance = 0, int includeExtra = 0, int special = 0);

    int scan(int primaryType, int secondaryType, int patternId, int firstInstance = 0, int includeExtra = 0,
             int special = 0);

    //---- function related to terrain pattern substitution ----//

    int search_pattern(int nwPatternId, TerrainSubInfo **resultArray, int maxResult);
