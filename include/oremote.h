...
enum {
  MSG_QUEUE_HEADER = FIRST_REMOTE_MSG_ID,
  MSG_QUEUE_TRAILER,
  MSG_NEW_NATION,
...
class Remote {
  enum {
    COMMON_MSG_BUF_SIZE = 1024, // Remote allocates a common RemoteMsg object
                                // with a data buffer of this size.
    SEND_QUEUE_BUF_SIZE = 1024, // The default queue buffer size
    SEND_QUEUE_BUF_INC_SIZE =
        1024, // If the queue is full, expand with this size
    RECEIVE_QUEUE_BUF_SIZE = 8192, // The default queue buffer size
    RECEIVE_QUEUE_BUF_INC_SIZE =
        2048,                    // If the queue is full, expand with this size
...
public:
  ...
  RemoteQueue
      send_queue[SEND_QUEUE_BACKUP]; // 0 for the latest, other for backup
  ...
  int alternating_send_rate; // 1=every frame, 2=send one frame per two
                             // frames...
