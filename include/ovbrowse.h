...
typedef void (*BrowDispFP)(
    int recNo, int x, int y,
...
public:
  ...
  static char
      press_record; // for detecting pulling records upwards and downloads
  static VBrowse *press_vbrowse_ptr;

public:
  VBrowse() { init_flag = 0; }

  virtual void init(int, int, int, int, int, int, int, int, BrowDispFP, int = 1,
                    int = MIN_INTER_SPACE, int = 1);
