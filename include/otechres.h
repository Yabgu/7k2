...
public:
  ...
  int total_tech_count(int nationRecno); // the number of tech researched by
                                         // each nation in this tech class
...
public:
  ...
  short available_level; // this tech is only available for research if
                         // available_level >= 1. This is for campaign only.
  char nation_tech_level_array[MAX_NATION]; // each nation's the technology
                                            // level of this unit,
...
public:
  ...
  short tech_count;
  short
      total_tech_level_array[1 + MAX_RACE]; // the sum of research levels of all
                                            // technology, for each nation race
...
public:
  ...
  int researchable_tech_count(int nationRecno, int raceId,
                              int checkLivePoint = 1);
  short total_tech_level(int nationRecno); // the sum of research levels of all
                                           // technology, for each nation race
  int suggest_next_tech(int nationRecno, int finishedTechId);
  int find_researchable_requirement(int nationRecno, int techId,
                                    int recurCount = 0);
