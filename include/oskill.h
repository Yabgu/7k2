...
enum {
  CITIZEN_COMBAT_LEVEL = 10,
  CITIZEN_SKILL_LEVEL = 10,
};

enum {
  BASIC_COMBAT_TRAIN =
      20, // combat level of a unit that has completed basic training
  MAX_COMBAT_TRAIN = 100,  // max combat level through training
  MAX_COMBAT_BATTLE = 200, //	max combat level through battling
                           // the absolute maximum combat level varies from unit
                           // to unit and is stored in Skill::max_combat_level

  BASIC_SKILL_TRAIN =
      20, // skill level of a unit that has completed basic training
  MAX_SKILL_TRAIN =
      100, // max skill level that can be achieved through training
};
